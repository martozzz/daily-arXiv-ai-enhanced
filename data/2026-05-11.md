...
TL;DR: GraphDC是一个基于分治法的多智能体框架，通过将图分解为子图进行局部推理然后整合，显著提升LLM在大规模图算法任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)在许多数学问题上表现出强大潜力，但在图算法任务上表现不佳，因为图在拓扑上天然更复杂，通常需要系统性的多步骤推理，特别是在大规模图上。

Method: GraphDC是一个分治多智能体框架，具体包括三个步骤：1）将输入图分解为更小的子图；2）为每个子图分配专门的智能体进行局部推理；3）使用主智能体整合局部输出并考虑子图间信息以生成最终解决方案。

Result: 广泛的实验表明，GraphDC在不同任务和规模上一致优于现有方法，特别是在大型图实例上，其中直接端到端推理的可靠性较低。

Conclusion: 论文的主要结论是GraphDC框架通过分治法有效解决了LLM在图算法任务中的局限性，特别是在大规模图实例上展现了更好的可扩展性、推理效率和鲁棒性。
...
TL;DR: 研究发现，思维链推理模型中的位置偏见随推理轨迹长度增加而加剧，即使在大模型中亦然。推理并未消除偏见，而是将其转化为长度累积的偏见。


<details>
  <summary>Details</summary>
Motivation: 研究旨在检验“思维链推理和经过推理调优的模型（如DeepSeek-R1）能通过仔细思考减少表面启发式偏见”这一常见假设。具体关注多项选择问答中的位置偏见问题，探究推理轨迹长度如何影响位置偏见，并验证推理是否真的能消除这种偏见。

Method: 本文采用实验方法，通过测量多个数据集（MMLU、ARC-Challenge、GPQA）上13种推理模式配置的位置偏见。研究使用位置偏见评分（PBS）量化偏见，并分析其与推理轨迹长度的相关性。通过控制准确率，使用偏相关性分析。此外，采用截断干预实验，从轨迹中不同点恢复生成，观察位置偏好的变化。还比较了直接回答模式与思维链推理模式的位置偏见差异。

Result: 研究发现，在13种推理模式配置中，有12种显示位置偏见评分（PBS）与推理轨迹长度呈正偏相关（相关系数0.11-0.41，p<0.05）。所有12种开放权重推理配置在长度四分位数上显示单调增加的PBS。截断干预实验表明，从轨迹较晚点恢复的生成更倾向于转向位置偏好的选项（R1-Qwen-7B在绝对位置桶中达16%-32%）。在671B参数模型中，总体PBS降至0.019，但长度效应在最长四分位数中仍显著（PBS=0.071）。直接回答位置偏见与思维链推理偏见表现出不同模式，且与轨迹长度无关。

Conclusion: 本文的结论是，具备推理能力的模型在多项选择问答中容易产生位置偏见的加剧，这种偏见与推理轨迹长度正相关。研究发现，即使在大规模模型（如671B参数）中，位置偏见的整体规模减小，但长度效应在长轨迹中依然存在。文章强调，推理模型不应默认视为位置鲁棒的，并提供了诊断工具来审计推理模型中的位置偏见，建议在评估流程中考虑这一点。
...
TL;DR: 提出了一种新的复合移动Tabu搜索方法解决空间重划问题中的连通性约束挑战，通过在线性时间内生成保持连通性的复合移动，显著提升了搜索效率和解决方案质量。


<details>
  <summary>Details</summary>
Motivation: 空间重新分区是一个实际的组合优化问题，需要高质量解、快速响应和灵活性以适应多准则目标和交互式优化。主要挑战是连通性约束：在整数规划或启发式搜索中强制连通性会严重缩小可行邻域，削弱探索能力，并使搜索陷入局部最优。

Method: 提出复合移动Tabu搜索（CM-Tabu），通过分析每个区域的连通图，利用关节点和双连通分量在线性时间内生成候选的单单元移动和复合移动。当边界单元无法单独重新分配而不断开其区域时，方法会识别能够一起移动的最小单元集合，或能够交换的单元对（或单元集合）作为保持连通性的复合移动。

Result: 大量实验表明，相对于传统的Tabu搜索和其他基线方法，所提出的方法显著提高了解决方案质量、运行间鲁棒性和计算效率。例如，在费城案例中，该方法能够始终达到人口平等的理论全局最优，并支持多准则权衡。CM-Tabu提供了适合实际实践的优化性能。

Conclusion: 论文提出的CM-Tabu方法通过引入复合移动机制，有效克服了传统Tabu搜索在空间重划问题中由于连通性约束导致的搜索受限问题。该方法能够在保持连通性的同时显著扩展可行邻域空间，大幅提升解的质量、计算效率和鲁棒性。实验证明该方法在真实案例中能够持续达到理论全局最优，且支持多准则权衡，适用于实际决策支持工作流程。
...
TL;DR: 提出评估递归推理收敛性的理论框架，用顺序间隙分析迭代效果，并给出局部收敛条件。


<details>
  <summary>Details</summary>
Motivation: 传统递归推理系统中，推理状态表示和迭代停止条件通常隐含且缺乏理论基础，需要系统化的理论框架来评估收敛质量。

Method: 使用认知状态图表示推理状态，定义顺序间隙作为评估收敛性的度量，通过理论分析给出局部收敛的必要条件和充分条件。

Result: 提出了顺序间隙的概念和评估框架，证明了线性化顺序间隙在固定点附近非退化的局部条件，为递归推理系统的收敛性提供了理论基础。

Conclusion: 该研究为递归推理系统提供了理论分析框架，通过状态图表示和顺序间隙分析来评估系统收敛性，为多种智能系统设计提供了理论指导。
...
TL;DR: 提出一种基于隐藏状态互信息与谱划分的实用方法，可有效检测多智能体系统中的联盟结构，在强化学习和语言模型场景中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过观察智能体行为往往难以区分真正的信息耦合与虚假的相似性，因为重要的联盟可能在内部表征层面形成，而在外部行为变化显现之前就存在。需要一种能够从智能体内部神经表征检测联盟结构的实用方法。

Method: 构建基于智能体隐藏状态的成对互信息图，应用谱划分识别最显著的联盟边界。

Result: 在多智能体强化学习环境中成功恢复了编程的层次化和动态联盟结构，并正确拒绝了仅行为协调而无信息耦合的误报；在大语言模型场景中识别了描述性提示所隐含的联盟结构，跟踪了动态团队重组，并揭示了显式标签主导冲突交互模式的表征层次；恢复的分区揭示了标量跨智能体互信息无法区分的子群组织。

Conclusion: 基于隐藏状态互信息与谱划分的方法能够有效识别多智能体系统中的联盟结构，为监控分布式AI系统中的涌现组织提供了有价值的诊断工具。
...
TL;DR: CASCADE框架使大语言模型智能体在部署期间通过经验学习不断改进，将经验重用形式化为上下文赌博机问题，在多样化任务上显著提升了成功率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的传统生命周期存在训练与部署分离的局限性，部署后学习基本停止，这与能够通过环境交互持续适应的自然智能形成鲜明对比。

Method: 提出CASCADE框架，将经验重用形式化为上下文赌博机问题，实现探索与利用的权衡平衡，并为智能体提供显式演化的情节记忆系统，通过积累、选择和精炼任务相关案例将过去经验转化为可操作知识。

Result: 在16个多样化任务（医疗诊断、法律分析、代码生成、网页搜索、工具使用和具身交互）上，CASCADE将宏观平均成功率提高了20.9%，优于零样本提示，并始终超越基于梯度和基于记忆的基准方法。

Conclusion: 通过形式化部署时学习作为LLM生命周期的第三阶段，CASCADE为LLM智能体提供了显式、演化的情节记忆机制，使其能够在部署期间持续从经验中学习，无需修改模型参数，从而建立了持续改进AI系统的基础。
...
TL;DR: 本文提出了LLM智能体记忆机制的进化框架，将其发展划分为存储、反思、经验三个阶段，分析了核心驱动力，为下一代智能体设计提供了统一的理论视角。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体记忆机制研究呈现碎片化状态，在操作系统工程和认知科学之间摇摆，缺乏统一的技术综合视角和连贯的演化观点，需要弥合这一理论鸿沟。

Method: 提出一个进化的三阶段框架（存储、反思、经验），形式化定义每个阶段，分析核心驱动因素（长范围一致性需求、动态环境挑战、持续学习目标），并特别探索前沿经验阶段的两个变革性机制。

Result: 建立了一个将LLM智能体记忆机制发展划分为存储、反思、经验三个演进阶段的理论框架，识别了三个核心驱动因素，并深入探讨了经验阶段的主动探索和跨轨迹抽象机制。

Conclusion: 该工作提出了一个统一的理论框架来理解LLM智能体记忆机制的演进，为下一代智能体系统提供了稳健的设计原则和清晰的发展路线图，弥合了当前研究中的理论与实践鸿沟。
...
TL;DR: 提出有限答案偏好稳定化框架来研究语言模型答案偏好何时稳定，通过δ函数测量发现模型内部偏好早在答案可解析前就稳定了，信号追踪模型最终输出且可线性恢复，但难以实现可靠的生成控制。


<details>
  <summary>Details</summary>
Motivation: 语言模型通常在给出最终答案前会生成推理过程，但可见的答案并不揭示模型答案偏好何时稳定。为了更好地理解语言模型决策过程的时间动态特性，需要研究模型内部答案偏好稳定化的时机。

Method: 提出有限答案偏好稳定化框架，定义精确对数赔率编码δ(ξ)=S_θ(yes∣ξ)-S_θ(no∣ξ)。通过投影模型自身在有限答案集上的继续概率，定义基于解析器的答案起始点、回顾性稳定时间和领先时间。在Qwen3-4B-Instruct上进行受控延迟裁决任务实验，并使用解析器清洁复现进行验证。

Result: 在Qwen3-4B-Instruct的受控延迟裁决任务中，上下文有限答案投影在答案可解析之前就已经稳定，在主模板中平均领先17-31个token，在解析器清洁复现中领先时间更短但仍为正值。δ信号追踪模型的最终输出而非真实答案，可以从紧凑隐藏摘要中线性恢复，部分与光标进程分离，并作为共享信息在不同组件间传递。

Conclusion: 研究通过有限答案偏好稳定化这一可计算对象，揭示了语言模型在生成回答前内部答案偏好稳定的时间特性。δ(ξ)函数提供了一种无需贪婪展开或学习探针的精确测量方法，能够捕捉模型最终输出的稳定信号，并且该信号可以从紧凑的隐藏摘要中线性恢复，部分与光标进程分离，可以在不同组件间作为共享信息传递。虽然δ对局部变化敏感，但不足以实现可靠的生成控制。
...
TL;DR: Weblica框架通过HTTP缓存和LLM环境合成，构建可扩展的网页环境，使强化学习训练能覆盖数千任务，模型在导航基准上超越同类开源模型。


<details>
  <summary>Details</summary>
Motivation: 网页环境具有复杂性、开放性和持续变化的特点，这使得为视觉网页agent扩展训练数据具有挑战性。现有的数据收集方法仅限于离线轨迹监督微调或少数模拟环境的强化学习训练，无法捕捉网页环境的多样性。

Method: 1) 使用HTTP级缓存技术捕获并回放稳定的视觉状态，同时保留交互行为；2) 基于真实网站和核心网页导航技能，利用大语言模型进行环境合成。通过这些技术构建了可复现、可扩展的网页环境框架Weblica。

Result: 使用该框架将强化学习训练扩展到数千个多样化环境和任务中。Weblica-8B模型在多个网页导航基准测试中超越了类似规模的开源基线模型，同时使用更少的推理步骤，能够有效利用额外的测试时计算资源，并且与API模型相比具有竞争力。

Conclusion: Weblica框架通过HTTP级缓存和基于大语言模型的环境合成，能够构建可复现、可扩展的网页环境，显著提升了视觉网页agent的训练规模和数据多样性。最佳模型Weblica-8B在多个网页导航基准测试中超越了同类规模的开源模型，表现出强大的泛化能力。
...
TL;DR: 研究利用SCALAR框架探索AI智能体与研究人员互动对科学结果的影响，发现多轮对话始终优于单次尝试，但其效果高度依赖于Actor-Critic配对，增大模型规模无法解决最难问题，反馈策略在非对称配对中尤为重要。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在科研级物理推理任务上展现出日益增长的能力，以及AI智能体系统的普及，一个实践问题变得重要：研究人员与AI智能体之间的互动如何影响科学研究结果？探索这种交互机制对于理解如何最优地利用AI辅助科学发现至关重要。

Method: 采用SCALAR（结构化批评者-执行者循环AI推理）框架的Actor-Critic-Judge管道，应用于量子场论和弦理论问题。具体包括：（1）改变Actor角色设定（2）调整Critic反馈策略（3）实验不同模型家族和规模参数变化。通过多轮迭代过程：Actor提出解决方案，Critic提供迭代反馈，独立Judge根据参考解决方案评估对话记录。

Result: 研究发现：（1）整体上多轮对话相比单次尝试持续提升性能（2）改进机制和不同提示策略的价值强烈依赖于具体的Actor-Critic配对（3）在单一模型家族内增加模型规模（如从DeepSeek-R1 8B到70B）能改善较简单问题的表现，但无法移除最困难的瓶颈（4）Critic反馈策略在非对称Actor-Critic设置（如轻量Haiku Actor配合更强大的Sonnet Critic）中效果最明显，建设性反馈能提高平均得分（5）在同族Actor-Critic设置中，策略效应较弱：宽松反馈有时更有利，而严格和对抗性反馈无益。

Conclusion: SCALAR框架为评估AI驱动科学发现中的交互结构提供了可控测试平台，研究揭示了AI智能体与研究人员之间互动机制的重要性。多轮对话相比单次尝试始终能提升性能，但不同提示策略的效果高度依赖于具体的Actor-Critic配对。增大模型规模并不能完全解决最困难问题，而反馈策略在非对称Actor-Critic设置中尤为重要。这些发现强调了在AI辅助科学研究中精心设计人机交互机制的必要性。
...
TL;DR: 本文提出Agent-BOM，一种用于智能体安全审计的统一结构表征，通过分层属性有向图模型连接静态能力基础和动态运行时状态，解决了现有碎片化审计方法无法捕捉智能体系统语义执行轨迹的问题。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的智能体系统通过动态工具调用、状态记忆管理和多智能体协作来执行复杂自主任务。但这种语义驱动的执行范式在低级物理事件和高级执行意图之间产生了严重的语义鸿沟，使得事后安全审计极为困难。现有表征机制（如静态SBOM和运行时日志）仅提供碎片化的证据，无法捕捉认知状态演化、能力绑定、持久性记忆污染以及跨交互智能体的级联风险传播。

Method: 1) 提出Agent-BOM作为智能体安全审计的统一结构表征，将智能体系统建模为分层属性有向图，分离静态能力基础（如模型、工具、长期记忆）和动态运行时语义状态（如目标、推理轨迹、行动）。2) 基于Agent-BOM开发基于图查询的路径级风险评估范式，并以OWASP Agentic Top 10为例实例化。3) 在OpenClaw环境中实现审计插件，从实时执行中构建Agent-BOM。

Result: 在代表性真实世界智能体攻击场景上的评估表明，Agent-BOM能够重建隐秘的攻击链，包括跨会话记忆中毒和工具误用、能力供应链劫持和意外代码执行、多智能体生态系统劫持以及权限和信任滥用。

Conclusion: Agent-BOM为复杂智能体生态系统的安全审计提供了一个统一且可审查的基础，能够有效支持根本原因分析和安全裁决。
...
TL;DR: 研究发现当前生成式AI在语言任务上表现卓越（第98百分位数以上），但在视觉感知推理任务上表现很差（第1百分位数以下）。跨代发展不平衡，且存在严重模态偏差——模型更擅长语言符号操作而非视觉处理。


<details>
  <summary>Details</summary>
Motivation: 追求人工通用智能需要超越狭隘任务性能的认知能力评估方法，传统任务表现评估不足以揭示模型的真实认知架构。需系统性评估模型认知能力分布、追踪跨代演化轨迹、识别架构偏见和发展局限性。

Method: 引入心理测量框架评估生成式AI的认知剖面，使用韦克斯勒成人智力量表改编任务进行初始评估。开发人工智能商数基准测试，应用于六个世代和两个模型家族。采用跨模态比较方法，对比语言与视觉形式下的抽象定量推理表现。

Result: 领先多模态模型在认知能力上呈现极端不均衡分布：语言理解和工作记忆接近天花板水平（>98百分位数），而知觉推理接近地板水平（<1百分位数）。AIQ基准测试显示跨代存在显著但不平衡的性能增益。发现尖锐的模态分离现象：语言呈现的抽象定量推理远快于视觉类比形式发展。抽象视觉推理有所改善，但视觉知觉组织基本停滞。

Conclusion: 生成式AI模型的认知能力发展不平衡，单纯依赖规模扩展和优化的AGI发展路径可能不足以克服基本架构限制，无法实现平衡的、类人的通用智能。认知剖面存在显著的非对称增益和模态间分离（语言与视觉处理差异显著）。
...
TL;DR: 研究发现LLM在战略规划（四子棋游戏）中的决策实际上由浅层推理驱动，而非深层前瞻，这与人类规划模式形成鲜明对比。


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型（特别是推理模型）能够生成包含对未来结果显式深思的扩展思维链推理，但这种深思是否构成真正的规划、其结构如何以及哪些方面驱动性能仍然不清楚。该研究旨在深入理解LLM的规划机制。

Method: 研究人员开发了一种新方法来表征LLM规划，通过在四子棋游戏中提取和量化推理轨迹中的搜索树。通过将计算模型拟合到提取的搜索树上，分析了计划的结构及其如何影响移动决策。

Result: 研究发现，与人类相比，LLM的搜索更浅，且性能由搜索广度而非深度预测。最惊人的发现是，尽管LLM在推理轨迹中扩展了深层节点，但它们的移动选择最好用完全忽略这些节点的短视模型来解释。因果干预研究进一步证实，移动选择主要由浅层而非深层节点驱动。

Conclusion: 论文发现LLM规划与人类规划的关键差异在于，人类规划主要由深度搜索驱动，而LLM虽然生成了深层的推理轨迹，但其决策实际上由浅层节点驱动，并未充分利用深层前瞻信息。这一发现为在战略领域对齐LLM与人类规划提供了针对性指导。
...
TL;DR: 提出AGWM世界模型，通过DAG显式建模动作的先决条件依赖，解决传统模型在多步预测中无法准确跟踪动作动态可执行性的问题，在仿真环境中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统世界模型学习静态的状态转移函数，将动作与结果的频繁共现内化为通用因果规则，但忽略了动作生效所需的前提条件。在交互环境中，动作会改变未来的可供性空间，存在结构变化事件（SC事件）使得动作在某些状态下变得可执行或不可执行。传统模型在多步预测中难以准确判断动作可执行性，导致预测误差随预测步长累积。

Method: 提出AGWM（Affordance-Grounded World Model），学习一个抽象的可供性结构，用有向无环图（DAG）表示动作间的先决条件依赖关系，显式跟踪动作的动态可执行性。

Result: 在基于游戏的仿真环境中进行的实验表明，AGWM方法实现了更低的多步预测误差、对新配置更好的泛化能力以及更强的可解释性。

Conclusion: AGWM方法通过显式建模动作间的先决条件依赖关系，有效解决了传统世界模型在面对结构化变化事件时的多步预测缺陷，在仿真环境中验证了其在预测精度、泛化能力和可解释性方面的优势。
...
### [15] [How Well Do LLMs Perform on the Simplest Long-Chain Reasoning Tasks: An Empirical Study on the Equivalence Class Problem](https://arxiv.org/abs/2605.06882)
*Chun Zheng,Lianlong Wu,Bingqian Li,Lvting Liu,Yi Zhou*

Main category: cs.AI

TL;DR: 论文通过等价类问题评估LLMs的长链推理能力，发现非推理模型完全失败，推理模型虽表现更好但仍有限，且两类模型在不同难度特征下表现最差。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）近年取得了显著进步，但其在推理任务（特别是长链推理）上的真实能力仍不明确。本研究旨在通过一个简单但需要长链推理的任务——等价类问题（ECP），来系统评估LLMs的推理能力，并探索影响其性能的关键因素。

Method: 研究采用等价类问题（ECP）作为最简单的长链推理任务，评估了推理和非推理两类代表性LLMs在不同问题实例上的表现，测试变量包括变量数量、连接概率、提示格式等多种因素。通过系统的实验设计，分析了不同参数设置下模型性能的变化规律。

Result: 实验结果显示：1）非推理LLMs完全无法解决ECP任务；2）推理模型表现显著优于非推理模型，但仍难以完全解决该问题；3）对于非推理模型，最难的问题实例与相变点ln n/(n-1)一致，反映了问题的混沌性；4）对于推理模型，最难的问题实例与最大图直径一致，反映了问题的推理难度。

Conclusion: 本研究通过ECP任务系统评估了LLMs的长链推理能力，揭示了不同类型模型在处理该任务时的不同失败模式。研究结果表明当前LLMs的长链推理能力仍有限，特别是推理模型也难以完全解决ECP问题。此外，研究发现非推理模型在相变点处表现最差，而推理模型则在图直径最大时面临最大困难，这为理解LLMs的推理机制提供了重要见解。
...
### [16] [Beyond the Black Box: Interpretability of Agentic AI Tool Use](https://arxiv.org/abs/2605.06890)
...
### [17] [Mitigating Cognitive Bias in RLHF by Altering Rationality](https://arxiv.org/abs/2605.06895)
...
### [18] [Self-Programmed Execution for Language-Model Agents](https://arxiv.org/abs/2605.06898)
*Luke J. O'Connor*
...
### [19] [Multi-Objective Constraint Inference using Inverse reinforcement learning](https://arxiv.org/abs/2605.06951)
...
### [20] [Learning and Reusing Policy Decompositions for Hierarchical Generalized Planning with LLM Agents](https://arxiv.org/abs/2605.06957)
...
### [21] [Optimal Experiments for Partial Causal Effect Identification](https://arxiv.org/abs/2605.06993)
...
### [22] [Adaptive auditing of AI systems with anytime-valid guarantees](https://arxiv.org/abs/2605.07002)
...
### [23] [Behavior Cue Reasoning: Monitorable Reasoning Improves Efficiency and Safety through Oversight](https://arxiv.org/abs/2605.07021)
...
### [24] [The Context Gathering Decision Process: A POMDP Framework for Agentic Search](https://arxiv.org/abs/2605.07042)
...
### [25] [2.5-D Decomposition for LLM-Based Spatial Construction](https://arxiv.org/abs/2605.07066)
...
### [26] [TeamBench: Evaluating Agent Coordination under Enforced Role Separation](https://arxiv.org/abs/2605.07073)
...
### [27] [Online Allocation with Unknown Shared Supply](https://arxiv.org/abs/2605.07080)
...
### [28] [ARMOR: An Agentic Framework for Reaction Feasibility Prediction via Adaptive Utility-aware Multi-tool Reasoning](https://arxiv.org/abs/2605.07103)
...
### [29] [Switchcraft: AI Model Router for Agentic Tool Calling](https://arxiv.org/abs/2605.07112)
...
### [30] [AdaTKG: Adaptive Memory for Temporal Knowledge Graph Reasoning](https://arxiv.org/abs/2605.07121)
...
### [31] [Can You Break RLVER? Probing Adversarial Robustness of RL-Trained Empathetic Agents](https://arxiv.org/abs/2605.07138)
...
### [32] [SREGym: A Live Benchmark for AI SRE Agents with High-Fidelity Failure Scenarios](https://arxiv.org/abs/2605.07161)
...
### [33] [Repeated Deceptive Path Planning against Learnable Observer](https://arxiv.org/abs/2605.07174)
...
### [34] [Three-in-One World Model: Energy-Based Consistency, Prediction, and Counterfactual Inference for Marketing Intervention](https://arxiv.org/abs/2605.07199)
*Junichiro Niimi*
...
### [35] [Towards Autonomous Business Intelligence via Data-to-Insight Discovery Agent](https://arxiv.org/abs/2605.07202)
...
### [36] [HMACE: Heterogeneous Multi-Agent Collaborative Evolution for Combinatorial Optimization](https://arxiv.org/abs/2605.07214)
...
### [37] [MEMOREPAIR: Barrier-First Cascade Repair in Agentic Memory](https://arxiv.org/abs/2605.07242)
...
### [38] [EnvSimBench: A Benchmark for Evaluating and Improving LLM-Based Environment Simulation](https://arxiv.org/abs/2605.07247)
...
### [39] [Can Agents Price a Reaction? Evaluating LLMs on Chemical Cost Reasoning](https://arxiv.org/abs/2605.07251)
...
### [40] [Structured Role-Aware Policy Optimization for Multimodal Reasoning](https://arxiv.org/abs/2605.07274)
...
### [41] [Signal Reshaping for GRPO in Weak-Feedback Agentic Code Repair](https://arxiv.org/abs/2605.07276)
...
### [42] [SOM: Structured Opponent Modeling for LLM-based Agents via Structural Causal Model](https://arxiv.org/abs/2605.07301)
...
### [43] [When Stored Evidence Stops Being Usable: Scale-Conditioned Evaluation of Agent Memory](https://arxiv.org/abs/2605.07313)
...
### [44] [Implicit Compression Regularization: Concise Reasoning via Internal Shorter Distributions in RL Post-Training](https://arxiv.org/abs/2605.07316)
...
### [45] [Discovering Ordinary Differential Equations with LLM-Based Qualitative and Quantitative Evaluation](https://arxiv.org/abs/2605.07323)
...
### [46] [Tools as Continuous Flow for Evolving Agentic Reasoning](https://arxiv.org/abs/2605.07339)
...
### [47] [Confidence-Aware Alignment Makes Reasoning LLMs More Reliable](https://arxiv.org/abs/2605.07353)
...
### [48] [GraphReAct: Reasoning and Acting for Multi-step Graph Inference](https://arxiv.org/abs/2605.07357)
...
### [49] [Offline Policy Optimization with Posterior Sampling](https://arxiv.org/abs/2605.07393)
...
### [50] [Bounded Fitting for Expressive Description Logics](https://arxiv.org/abs/2605.07452)
...
### [53] [Model-Driven Policy Optimization in Differentiable Simulators via Stochastic Exploration](https://arxiv.org/abs/2605.07520)
...
### [54] [From Feasible to Practical: Pareto-Optimal Synthesis Planning](https://arxiv.org/abs/2605.07521)
...
### [55] [Multi-Environment POMDPs with Finite-Horizon Objectives](https://arxiv.org/abs/2605.07537)
...
### [56] [From Pixels to Prompts: Vision-Language Models](https://arxiv.org/abs/2605.07544)
*Khang Hoang Nhat Vo*
...
### [57] [Open-Ended Task Discovery via Bayesian Optimization](https://arxiv.org/abs/2605.07572)
...
### [58] [Parallel Lifted Planning via Semi-Naive Datalog Evaluation](https://arxiv.org/abs/2605.07584)
...
### [59] [Inference Time Causal Probing in LLMs](https://arxiv.org/abs/2605.07631)
...
### [60] [Learning to Communicate Locally for Large-Scale Multi-Agent Pathfinding](https://arxiv.org/abs/2605.07637)
...
### [61] [Tacit Knowledge Extraction via Logic Augmented Generation and Active Inference](https://arxiv.org/abs/2605.07639)
...
### [62] [FactoryBench: Evaluating Industrial Machine Understanding](https://arxiv.org/abs/2605.07675)
...
### [63] [GASim: A Graph-Accelerated Hybrid Framework for Social Simulation](https://arxiv.org/abs/2605.07692)
...
### [64] [Finite-Time Analysis of MCTS in Continuous POMDP Planning](https://arxiv.org/abs/2605.07703)
...
### [65] [Hierarchical Task Network Planning with LLM-Generated Heuristics](https://arxiv.org/abs/2605.07707)
...
### [66] [Online Goal Recognition using Path Signature and Dynamic Time Warping](https://arxiv.org/abs/2605.07736)
...
### [67] [Alternating Target-Path Planning for Scalable Multi-Agent Coordination](https://arxiv.org/abs/2605.07744)
...
### [68] [RuleSafe-VL: Evaluating Rule-Conditioned Decision Reasoning in Vision-Language Content Moderation](https://arxiv.org/abs/2605.07760)
...
### [69] [Exact Regular-Constrained Variable-Order Markov Generation via Sparse Context-State Belief Propagation](https://arxiv.org/abs/2605.07839)
*François Pachet*
...
### [70] [AgentEscapeBench: Evaluating Out-of-Domain Tool-Grounded Reasoning in LLM Agents](https://arxiv.org/abs/2605.07926)
...
### [71] [TraceFix: Repairing Agent Coordination Protocols with TLA+ Counterexamples](https://arxiv.org/abs/2605.07935)
...
### [72] [The Limits of AI-Driven Allocation: Optimal Screening under Aleatoric Uncertainty](https://arxiv.org/abs/2605.07979)
...
### [73] [Abductive Reasoning with Probabilistic Commonsense](https://arxiv.org/abs/2605.08011)
...
### [74] [Learning CLI Agents with Structured Action Credit under Selective Observation](https://arxiv.org/abs/2605.08013)
*Haoyang Su,Ying Wen*
...
### [75] [Reason to Play: Behavioral and Brain Alignment Between Frontier LRMs and Human Game Learners](https://arxiv.org/abs/2605.08019)
...
### [76] [VecCISC: Improving Confidence-Informed Self-Consistency with Reasoning Trace Clustering and Candidate Answer Selection](https://arxiv.org/abs/2605.08070)
...
### [77] [RateQuant: Optimal Mixed-Precision KV Cache Quantization via Rate-Distortion Theory](https://arxiv.org/abs/2605.06675)
*Fei Zuo,Zikang Zhou,Hao Cong,Xiaoyan Xi,Ho Fai Leung*

Main category: cs.LG

TL;DR: RateQuant：基于率失真理论的KV缓存混合精度量化方法，通过校准失真模型解决不同量化器间的失配问题，显著提升量化效果。


<details>
  <summary>Details</summary>
Motivation: KV缓存在LLM推理中线性增长，成为主要内存瓶颈。现有混合精度量化方法假设所有量化器具有相同的失真曲线，但实际上不同量化器的失真衰减率差异很大，导致比特分配错误。

Method: RateQuant首先通过小型校准集拟合每个量化器的失真模型，然后使用率失真理论中的反向注水算法（reverse waterfilling）以闭式解的形式解决比特分配问题。

Result: 在Qwen3-8B模型上，以2.5平均比特进行量化时，校准后的RateQuant将KIVI的困惑度从49.3降至14.9（降低70%），并将QuaRot的困惑度提高了6.6 PPL。整个校准过程在单个GPU上仅需1.6秒，且推理时无额外开销。

Conclusion: 论文提出RateQuant方法成功解决了KV缓存混合精度量化的失真模型失配问题，为LLM推理内存优化提供了一种高效、准确的解决方案。
...
### [78] [LKV: End-to-End Learning of Head-wise Budgets and Token Selection for LLM KV Cache Eviction](https://arxiv.org/abs/2605.06676)
*Enshuai Zhou,Yifan Hao,Chao Wang,Rui Zhang,Di Huang,Jiaming Guo,Xing Hu,Zidong Du,Qi Guo,Yunji Chen*

Main category: cs.LG

TL;DR: 提出LKV框架，通过端到端优化学习KV缓存压缩的策略，在高压缩率下实现接近无损的长上下文推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩方法依赖启发式预算分配和选择机制，导致资源分配不当和性能受限，需要任务目标驱动的优化方法。

Method: 引入LKV框架，包含LKV-H学习任务优化的全局预算，LKV-T推导KV重要性而不需物化注意力矩阵，将KV压缩建模为端到端可微分优化问题。

Result: 在LongBench和RULER基准测试中，LKV在高压缩率下达到最先进性能，在LongBench上仅保留15%KV缓存就实现接近无损的性能，学习化预算是保真度的主要驱动力。

Conclusion: LKV通过端到端优化学习任务优化的KV缓存预算和内在重要性，显著超越了基于启发式的压缩方法，在高压缩率下实现了接近无损的性能，其中学习化预算分配是关键驱动力。
...
### [79] [Breaking the Illusion: When Positive Meets Negative in Multimodal Decoding](https://arxiv.org/abs/2605.06679)
*Yubo Jiang,Yitong An,Xin Yang,Abudukelimu Wuerkaixi,Xuxin Cheng,Fengying Xie,Zhiguo Jiang,Cao Liu,Ke Zeng,Haopeng Zhang*

Main category: cs.LG

TL;DR: 研究提出了一种训练无关的PND解码框架，通过正负双路径对比抑制物体幻觉，提升视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）常常因过度依赖语言先验而产生物体幻觉（生成与视觉现实相矛盾的内容），这会削弱模型的可信度。研究发现VLMs存在注意力不平衡问题，即视觉特征的权重不足。

Method: 提出了一种称为Positive-and-Negative Decoding（PND）的训练无关推理框架。该方法建立双路径对比机制：正路径放大视觉证据，负路径构建反事实样本来惩罚先验主导的生成。通过对这两个路径的输出进行对比，在解码过程中引导生成更视觉接地的结果。

Result: 在POPE、MME和CHAIR三个基准数据集上的实验表明，PND方法在不进行再训练的情况下实现了最先进的性能表现。

Conclusion: PND提供了一种无需再训练的方法，通过在解码过程中直接干预，有效解决了VLMs中的物体幻觉问题，实现了视觉保真度的提升，在多个基准测试中达到了最先进的性能。
...
### [80] [On the Role of Strain and Vorticity in Numerical Integration Error for Flow Matching](https://arxiv.org/abs/2605.06680)
*Chenxi Tao,Seung-Kyum Choi*

Main category: cs.LG

TL;DR: 通过分析速度场雅可比分解（应变率vs涡度），揭示应变控制误差放大、涡度仅线性贡献。提出加权雅可比正则化，实验证明可降低积分误差和提升低NFE下的生成质量。


<details>
  <summary>Details</summary>
Motivation: 流匹配通过积分学习的速度场生成数据，积分步数（NFE）直接影响推理成本。研究哪些速度场特性控制积分误差，以指导设计更高效的生成模型，降低推理计算成本。

Method: 1. 通过将速度场的雅可比分解为对称部分（应变率S）和反对称部分（涡度Ω），理论分析应变和涡度对积分误差的不同作用；2. 提出加权雅可比正则化方法，包含应变权重α和涡度权重β；3. 在2D合成数据和CIFAR-10上进行实验验证。

Result: 1. 理论证明应变控制指数误差放大（通过对数范数），涡度仅贡献线性局部截断误差；2. 最优传输速度场是无旋的且具有零物质导数，具有二阶欧拉精度；精确位移插值对应的拉格朗日质点动力学可被欧拉法精确积分；3. 2D合成实验验证理论预测，在NFE=5时积分误差降低2.7倍；CIFAR-10初步实验显示一致趋势，轻量微调在NFE=10时将FID提升14%，同时保持高NFE质量。

Conclusion: 流匹配中的积分误差主要由应变率控制，而涡度仅贡献线性局部截断误差。通过加权雅可比正则化（应变权重α，涡度权重β）进行微调，可以有效降低推理时的积分误差，在低NFE下获得显著性能提升，同时保持高NFE质量。
...
### [81] [A Hierarchical Ensemble Pipeline for Anomaly Detection in ESA Satellite Telemetry](https://arxiv.org/abs/2605.06681)
*Lorenzo Riccardo Allegrini,Geremia Pompei*

Main category: cs.LG

TL;DR: 本文提出分层集成管道用于卫星多元遥测数据异常检测，结合形状和统计特征提取，通过分层建模在ESA-ADB基准上展现出强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决欧洲航天局（ESA）提供的多元遥测数据中异常检测的挑战，特别是在现实卫星运行场景中检测微弱和不明显异常的需求。

Method: 该方法集成了基于形状和统计特征提取的技术，采用逐通道建模、通道内堆叠和跨通道聚合的分层架构。使用时间序列交叉验证和两级掩码策略进行训练和验证，防止信息泄露。

Result: 在欧洲航天局异常检测基准（ESA-ADB）挑战中获得优异表现，显示出强大的泛化能力，成功检测出卫星遥测数据中的复杂异常模式。

Conclusion: 分层集成管道通过结合形状和统计特征提取、通道内建模和跨通道聚合，在卫星遥测数据异常检测中展现出强大的泛化能力，验证了分层建模在处理现实卫星数据中微弱异常的有效性。
...
### [82] [Toeplitz MLP Mixers are Low Complexity, Information-Rich Sequence Models](https://arxiv.org/abs/2605.06683)
*Benjamin L. Badger,Ethan Roland*

Main category: cs.LG

TL;DR: 提出TMM架构，用Toeplitz矩阵乘法替代注意力，降低计算复杂度，同时保留更多输入信息，在多个任务中表现更优。


<details>
  <summary>Details</summary>
Motivation: 基于Transformer的大语言模型受限于注意力机制的二次时空计算复杂度。为此，需要设计一种更高效的子二次复杂度架构，以提升训练和推理效率。

Method: 采用三角掩码 Toeplitz 矩阵乘法替代注意力机制，形成 Toeplitz MLP Mixer (TMM) 架构。该方法在训练时具有 O(dn log n) 时间复杂度和 O(dn) 空间复杂度，推理时则为 O(dn) 时间空间复杂度。

Result: TMM实现了更高的训练效率（每单位计算和显存获得更低的损失），并展现出更强的输入信息保留能力，从而在复制任务、信息检索和上下文学习基准测试中优于可比架构。

Conclusion: Toeplitz MLP Mixer (TMM) 在减少计算复杂度的同时，通过缺乏架构偏置保留了更多输入信息，从而在多项任务中表现出色。从算子索引理论的角度分析，训练后的非可逆模型的Toeplitz层反而更可能接近可逆。
...
### [83] [From Canopy to Collision: A Hybrid Predictive Framework for Identifying Risk Factors in Tree-Involved Traffic Crashes](https://arxiv.org/abs/2605.06684)
*Abdul Azim,Ahmed Hossain,Soumyadip Maitra,Panick Kalambay*

Main category: cs.LG

TL;DR: 本研究开发了一个分析框架来识别影响树木碰撞严重程度的风险因素。研究发现未使用安全带是最强预测因子，同时揭示了车辆年龄、超速、驾驶员受损以及多个因素交互作用对碰撞后果的重要影响。


<details>
  <summary>Details</summary>
Motivation: 涉及树木的碰撞事故代表着路肩外碰撞的关键子集，通常由于高能量冲击导致致命或重伤。研究旨在开发和实施一个全面的分析框架，识别和量化影响树木碰撞严重程度的风险因素。

Method: 研究采用多步骤建模框架：1) 使用CatBoost机器学习分类模型识别与严重程度相关的关键因素；2) 应用SHAP工具量化和可视化影响因素；3) 使用二元逻辑回归模型估计因子效应并验证SHAP结果；4) 通过SHAP交互图分析关键因素的联合效应。

Result: 结果显示，未使用约束装置是最具影响力的预测因子，未系安全带的乘员经历严重后果的可能性提高近三倍。车辆年龄、超速违规和驾驶员受损也显示出显著影响。研究还发现了多个关键交互作用，包括光照条件与车辆年龄、超速与光照条件、约束使用与车辆年龄以及路面状况与超速等。

Conclusion: 基于研究结果，作者建议针对性的安全系统干预措施，包括加强安全带执法、在低能见度条件下的速度管理以及车辆队伍现代化，以减轻涉及树木碰撞的严重程度风险。
...
### [84] [Robustness of Refugee-Matching Gains to Off-Policy Evaluation Choices](https://arxiv.org/abs/2605.06686)
*Kirk Bansak,Elisabeth Paulson,Dominik Rothenhäusler,Jeremy Ferwerda,Jens Hainmueller,Michael Hotard*

Main category: cs.LG

TL;DR: 通过多种评估方法验证难民匹配政策影响估计的稳健性，结果显示估计值在不同方法下保持稳定，支持先前研究结论。


<details>
  <summary>Details</summary>
Motivation: 基于Bansak等人（2018）关于难民匹配能改善难民安置结果的研究，本文旨在验证这些反事实影响评估结果的稳定性和稳健性。

Method: 采用多种反事实评估方法，包括逆概率加权（IPW）和多种增强逆概率加权（AIPW）变体，同时考虑了不同的建模架构和分配程序变体。

Result: 所有情境下影响估计值在幅度上保持一致，大多数情况下统计显著，且与Bansak等人（2018）的原始结果一致。

Conclusion: 本文研究表明难民匹配政策的影响评估结果在不同方法和设置下具有稳定性，验证了Bansak等人（2018）研究的可靠性，为政策制定者提供了可信的实证依据。
...
### [85] [Conditional generation of antibody sequences with classifier-guided germline-absorbing discrete diffusion](https://arxiv.org/abs/2605.06720)
*Justin Sanders,Luca Giancardo,Lan Guo,Yue Zhao,Kemal Sonmez,Nina Cheng,Melih Yilmaz*

Main category: cs.LG

TL;DR: 提出基于胚系吸收扩散的抗体设计新方法，有效减少胚系偏置并实现高质量条件生成，在疏水性和结合亲和力改善任务中优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 现有基于蛋白质语言模型的抗体设计方法存在两个关键限制：一是主要记忆胚系序列而不是模拟有生物学意义的体细胞变异；二是缺乏灵活的分类器引导条件生成支持。

Method: 提出了两种主要方法：1）使用离散扩散微调实现抗体序列建模和分类器条件生成；2）引入胚系吸收扩散，将胚系序列作为吸收状态而非掩码序列，避免学习V(D)J重组统计信息。

Result: 胚系扩散将非胚系残基预测准确率从26%提升到46%，接近真实生物变异的理论上限。在改进疏水性和预测结合亲和力的条件生成任务中，该模型在类别依从性和样本质量之间实现了更好的权衡，显著优于EvoProtGrad方法。

Conclusion: 本研究提出的胚系吸收扩散模型通过生物启发的吸收状态设计，有效分离了胚系序列偏置，实现了抗体序列的高质量条件生成，为抗体药物设计提供了新的计算框架。
...
### [86] [Enabling Unsupervised Training of Deep EEG Denoisers With Intelligent Partitioning](https://arxiv.org/abs/2605.06724)
*Qiyu Rao,Haozhe Tian,Homayoun Hamedmoghadam,Danilo Mandic*

Main category: cs.LG

TL;DR: iPSD通过智能分区实现自监督EEG去噪，无需干净参考信号，在极低信噪比下表现优异。


<details>
  <summary>Details</summary>
Motivation: 可穿戴脑电图（EEG）去噪面临神经信号微弱、与噪声频谱重叠以及传统方法无法处理时变噪声的挑战，深度学习需要无法获得的干净参考信号。

Method: 提出智能分区自监督去噪（iPSD）方法，通过将一个EEG信号段划分为具有相同底层信号但独立噪声实现的多个子信号，实现自监督训练。

Result: iPSD在极低信噪比（低至-10 dB）和挑战性噪声（如EMG）下达到最优性能，频谱保真度比基线方法高出数个数量级。

Conclusion: iPSD方法消除了对干净参考信号的需求，通过智能分区实现了自监督去噪，为可穿戴EEG的去噪问题提供了一种新颖有效的解决方案。
...
### [87] [Transformer-Based Wildlife Species Classification from Daily Movement Trajectories](https://arxiv.org/abs/2605.06726)
*Obed Irakoze,Prasenjit Mitra*

Main category: cs.LG

TL;DR: Transformer模型在野生动物物种识别任务中优于传统序列模型，特征增强帮助弱势物种识别，1小时分辨率效果最佳。


<details>
  <summary>Details</summary>
Motivation: 仅从日常运动数据推断野生动物物种身份是一个具有挑战性的任务，需要开发更有效的方法来从大规模GPS轨迹数据中识别物种，特别是针对数据有限的物种。

Method: 使用来自Movebank平台的7种野生动物GPS轨迹数据，通过训练序列模型进行物种识别。采用留出整个追踪研究或区域进行测试的评估协议，比较了Transformer、LSTM、CNN和Temporal CNN等模型。分析了位移编码与包含速度、方向、转向行为等运动描述符的特征表示，并对比了1小时和30分钟时间分辨率的效果。

Result: Transformer模型在所有基线模型中表现最佳，平衡准确率提高了约8-22个百分点。在大象二元分类任务中，Transformer的平衡准确率达到0.83，AUC为0.92。特征增强显著提升了弱势物种（如大型食肉动物、狮子和斑马）的识别性能。1小时时间分辨率相比30分钟分辨率能够在减少数据缺失和确保时间覆盖一致性的同时获得更好的整体性能。

Conclusion: 研究发现Transformer模型在动物轨迹识别任务上显著优于传统序列模型，特征增强对弱势物种识别有显著帮助，而1小时的时间分辨率能够在数据完整性和模式捕捉之间提供更好的平衡。
...
### [88] [Medical Imaging Classification with Cold-Atom Reservoir Computing using Auto-Encoders and Surrogate-Driven Training](https://arxiv.org/abs/2605.06727)
*Nuno Batista,Ana Morgado,Oscar Ferraz,Sagar Silva Pratapsi,Jorge Lobo,Gabriel Falcao*

Main category: cs.LG

TL;DR: 提出了一个基于中性原子储层计算的混合量子-经典流程，结合引导自编码器处理高维医学图像分类，通过可微替代模型克服量子梯度障碍，在息肉检测任务中表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 解决医疗影像分类（特别是息肉检测）中高维数据处理的挑战，同时克服量子测量不可微分性导致的'梯度障碍'，为量子机器学习在医疗领域的实际应用提供可行方案。

Method: 1. 使用引导自编码器学习紧凑且具有判别性的图像表示
2. 构建可微分的量子层替代模型以实现端到端反向传播
3. 将学习到的潜在表示编码为里德堡哈密顿量的脉冲失谐参数
4. 通过期望值获得量子嵌入并传递给线性分类器
5. 联合优化分类准确率和自编码器图像重建质量

Result: 1. 相比使用PCA或非引导自编码器的传统方法，该方法在性能上表现更优
2. 消融研究证明了该方法对各种量子参数和训练参数的鲁棒性
3. 在NISQ时代背景下展示了该方法在实际医疗影像应用中的灵活性

Conclusion: 该研究提出的混合量子-经典流程结合了引导自编码器和量子储层计算，有效解决了高维医学图像分类难题，克服了量子测量不可微分带来的训练障碍，为当前NISQ时代的医疗影像应用提供了一种稳健且灵活的解决方案。
...
### [89] [The E$Δ$-MHC-Geo Transformer: Adaptive Geodesic Operations with Guaranteed Orthogonality](https://arxiv.org/abs/2605.06729)
*Arash Shahmansoori*

Main category: cs.LG

TL;DR: EΔ-MHC-Geo Transformer：一种混合正交架构，结合Cayley旋转和Householder反射，通过自适应选择门在所有β值保持正交性，解决反射问题并实现优越的性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法的正交性限制：DDL的Householder算子只在β∈{0,2}时保持正交，有限的Cayley残差混合器无法处理特征值-1的情况（即反射操作），这限制了模型访问O(n)的两个连通分量。

Method: 1. 提出EΔ-MHC-Geo Transformer架构，整合流形约束超连接(mHC)、深度Delta学习(DDL)和Cayley变换；2. 使用数据依赖的Cayley旋转算子Q(x)=(I+(β/2)A(x))^{-1}(I-(β/2)A(x))，在所有β值和输入下保持正交性；3. 引入EΔ-MHC-Geo混合模型，通过学习的选择门γ(X)组合Cayley旋转和Householder反射；4. 使用中点崩溃正则化器4γ(1-γ)鼓励边界门决策。

Result: 1. 在长时稳定性方面，比JPmHC提高1.9倍，比GPT提高3.8倍；2. 在接近π的旋转变换上，比JPmHC提高4.5倍；3. 强范数保持性，平均偏差仅为0.001；4. 在诊断反射任务中，否定余弦对齐度达0.96；5. 所有结果均使用比基线少33%的层数实现（每个模型约1.79M参数）。

Conclusion: EΔ-MHC-Geo Transformer通过创新的混合架构，在保持正交性的同时解决了输入自适应正交连接的限制。该方法通过Cayley旋转和Householder反射的结合，实现了对所有β值的无条件正交性，并且在诊断反射任务中表现出色。
...
### [90] [Semantic State Abstraction Interfaces for LLM-Augmented Portfolio Decisions: Multi-Axis News Decomposition and RL Diagnostics](https://arxiv.org/abs/2605.06730)
...
### [91] [On Training in Imagination](https://arxiv.org/abs/2605.06732)
...
### [92] [Beyond Factor Aggregation: Gauge-Aware Low-Rank Server Representations for Federated LoRA](https://arxiv.org/abs/2605.06733)
...
### [93] [Gated QKAN-FWP: Scalable Quantum-inspired Sequence Learning](https://arxiv.org/abs/2605.06734)
...
### [95] [Geometric Kolmogorov--Arnold Network (GeoKAN)](https://arxiv.org/abs/2605.06740)
...
### [96] [A Closed-Form Upper Bound for Admissible Learning-Rate Steps in Belief-Space Dynamics](https://arxiv.org/abs/2605.06741)
*Zixi Li,Youzhen Li*
...
### [97] [Gradient Extrapolation-Based Policy Optimization](https://arxiv.org/abs/2605.06755)
...
### [98] [Physics-based Digital Twins for Integrated Thermal Energy Systems Using Active Learning](https://arxiv.org/abs/2605.06756)
...
### [99] [Sparse Attention as a Range Searching Problem: Towards an Inference-Efficient Index for KV Cache](https://arxiv.org/abs/2605.06763)
...
### [100] [Revisiting Adam for Streaming Reinforcement Learning](https://arxiv.org/abs/2605.06764)
...
### [101] [Distributional Process Reward Models: Calibrated Prediction of Future Rewards via Conditional Optimal Transport](https://arxiv.org/abs/2605.06785)
...
### [102] [Conformal Agent Error Attribution](https://arxiv.org/abs/2605.06788)
...
### [103] [MIND: Monge Inception Distance for Generative Models Evaluation](https://arxiv.org/abs/2605.06797)
...
### [104] [From Model to Data (M2D): Shifting Complexity from GNNs to Graphs for Transparent Graph Learning](https://arxiv.org/abs/2605.06814)
...
### [105] [A Theory of Online Learning with Autoregressive Chain-of-Thought Reasoning](https://arxiv.org/abs/2605.06819)
...
### [106] [A Rod Flow Model for Adam at the Edge of Stability](https://arxiv.org/abs/2605.06821)
...
### [107] [SHARP: A Self-Evolving Human-Auditable Rubric Policy for Financial Trading Agents](https://arxiv.org/abs/2605.06822)
...
### [108] [A Unified Measure-Theoretic View of Diffusion, Score-Based, and Flow Matching Generative Models](https://arxiv.org/abs/2605.06829)
...
### [109] [Why DDIM Hallucinates More than DDPM: A Theoretical Analysis of Reverse Dynamics](https://arxiv.org/abs/2605.06831)
...
### [110] [Attribution-Based Neuron Utility for Plasticity Restoration in Deep Networks](https://arxiv.org/abs/2605.06834)
...
### [111] [On Privacy Leakage in Tabular Diffusion Models: Influential Factors, Attacker Knowledge, and Metrics](https://arxiv.org/abs/2605.06835)
...
### [112] [How to Compress KV Cache in RL Post-Training? Shadow Mask Distillation for Memory-Efficient Alignment](https://arxiv.org/abs/2605.06850)
...
### [113] [Benchmarked Yet Not Measured -- Generative AI Should be Evaluated Against Real-World Utility](https://arxiv.org/abs/2605.06856)
...
### [114] [Christoffel-DPS: Optimal sensor placement in diffusion posterior sampling for arbitrary distributions](https://arxiv.org/abs/2605.06861)
...
### [115] [Multi-Objective Multi-Agent Bandits: From Learning Efficiency to Fairness Optimization](https://arxiv.org/abs/2605.06864)
...
### [116] [Dataset Watermarking for Closed LLMs with Provable Detection](https://arxiv.org/abs/2605.06865)
...
### [117] [A Finite-Iteration Theory for Asynchronous Categorical Distributional Temporal-Difference Learning](https://arxiv.org/abs/2605.06866)
...
### [118] [When Descent Is Too Stable: Event-Triggered Hamiltonian Learning to Optimize](https://arxiv.org/abs/2605.06868)
...
### [119] [Continuous First, Discrete Later: VQ-VAEs Without Dimensional Collapse](https://arxiv.org/abs/2605.06870)
...
### [120] [On the Divergence of Differential Temporal Difference Learning without Local Clocks](https://arxiv.org/abs/2605.06874)
...
### [121] [Temporal Attention for Adaptive Control of Euler-Lagrange Systems with Unobservable Memory](https://arxiv.org/abs/2605.06877)
...
### [122] [Better Protein Function Prediction by Modeling Survivorship Bias](https://arxiv.org/abs/2605.06879)
...
### [123] [Don't Retrain, Align: Adapting Autoregressive LMs to Diffusion LMs via Representation Alignment](https://arxiv.org/abs/2605.06885)
...
### [124] [Streaming Adversarial Robustness in Fuzzy ARTMAP: Mechanism-Aligned Evaluation, Progressive Training, and Interpretable Diagnostics](https://arxiv.org/abs/2605.06902)
...
### [125] [Conservative Flows: A New Paradigm of Generative Models](https://arxiv.org/abs/2605.06905)
...
### [126] [TraXion: Rethinking Pre-training Frameworks for Mobility and Beyond](https://arxiv.org/abs/2605.06906)
...
### [127] [Same Signal, Opposite Meaning: Direction-Informed Adaptive Learning for LLM Agents](https://arxiv.org/abs/2605.06908)
...
### [128] [Dual-Scale Temporal Fusion Reveals Structured Predictability in Subseasonal-to-Seasonal Temperature Prediction](https://arxiv.org/abs/2605.06911)
...
### [129] [LLMs are not (consistently) Bayesian: Quantifying internal (in)consistencies of LLMs' probabilistic beliefs](https://arxiv.org/abs/2605.06915)
...
### [130] [Tyche: One Step Flow for Efficient Probabilistic Weather Forecasting](https://arxiv.org/abs/2605.06916)
...
### [131] [Target-Aware Data Augmentation for SAT Prediction](https://arxiv.org/abs/2605.06931)
...
### [132] [MAGIQ: A Post-Quantum Multi-Agentic AI Governance System with Provable Security](https://arxiv.org/abs/2605.06933)
...
### [133] [Learned Lyapunov Shielding for Adaptive Control](https://arxiv.org/abs/2605.06934)
*Giansalvo Cirrincione,Adriano Fagiolini*
...
### [134] [A Reproducible Optimisation Protocol for Calibrating Prompt-Based Large Language Model Workflows in Evidence Synthesis](https://arxiv.org/abs/2605.06937)
*Teo Susnjak*
...
### [135] [A Generalized Singular Value Theory for Neural Networks](https://arxiv.org/abs/2605.06938)
...
### [136] [Bias and Uncertainty in LLM-as-a-Judge Estimation](https://arxiv.org/abs/2605.06939)
*James Fiedler*
...
### [137] [ProtoSSL: Interpretable Prototype Learning from Unlabeled Time-Series Data](https://arxiv.org/abs/2605.06943)
...
### [138] [Adaptive Memory Decay for Log-Linear Attention](https://arxiv.org/abs/2605.06946)
...
### [139] [Rollback-Free Stable Brick Structures Generation](https://arxiv.org/abs/2605.06947)
...
### [140] [$f$-Divergence Regularized RLHF: Two Tales of Sampling and Unified Analyses](https://arxiv.org/abs/2605.06977)
...
### [141] [PLOT: Progressive Localization via Optimal Transport in Neural Causal Abstraction](https://arxiv.org/abs/2605.06979)
...
### [142] [FastOmniTMAE: Parallel Clause Learning for Scalable and Hardware-Efficient Tsetlin Embeddings](https://arxiv.org/abs/2605.06982)
...
### [143] [Response Time Enhances Alignment with Heterogeneous Preferences](https://arxiv.org/abs/2605.06987)
...
### [144] [Why Does Agentic Safety Fail to Generalize Across Tasks?](https://arxiv.org/abs/2605.06992)
...
### [145] [Inductive Power Grid Cascading Failure Analysis with GRU-Gated Graph Attention](https://arxiv.org/abs/2605.07010)
...
### [146] [Dual-Agent Co-Training for Health Coaching via Implicit Adversarial Preference Optimization](https://arxiv.org/abs/2605.07011)
...
### [147] [FlashMol: High-Quality Molecule Generation in as Few as Four Steps](https://arxiv.org/abs/2605.07020)
...
### [148] [Self Driving Datasets: From 20 Million Papers to Nuanced Biomedical Knowledge at Scale](https://arxiv.org/abs/2605.07022)
...
### [149] [CommFuse: Hiding Tail Latency via Communication Decomposition and Fusion for Distributed LLM Training](https://arxiv.org/abs/2604.24013)
...
### [150] [Delulu: A Verified Multi-Lingual Benchmark for Code Hallucination Detection in Fill-in-the-Middle Tasks](https://arxiv.org/abs/2605.07024)
...
### [151] [A Systematic Investigation of The RL-Jailbreaker in LLMs](https://arxiv.org/abs/2605.07032)
...
### [152] [Learning Material-Aware Hamiltonian Risk Fields for Safe Navigation](https://arxiv.org/abs/2605.07038)
...
### [153] [PACEvolve++: Improving Test-time Learning for Evolutionary Search Agents](https://arxiv.org/abs/2605.07039)
...
### [154] [Unlocking High-Fidelity Molecular Generation from Mass Spectra via Dual-Stream Line Graph Diffusion](https://arxiv.org/abs/2605.07048)
...
### [155] [Towards Differentially Private Reinforcement Learning with General Function Approximation](https://arxiv.org/abs/2605.07049)
*Yi He,Xingyu Zhou*
...
### [156] [Integrating Causal DAGs in Deep RL: Activating Minimal Markovian States with Multi-Order Exposure](https://arxiv.org/abs/2605.07057)
...
### [157] [Dr. Post-Training: A Data Regularization Perspective on LLM Post-Training](https://arxiv.org/abs/2605.07063)
...
### [158] [PolarAdamW: Disentangling Spectral Control and Schur Gauge-Equivariance in Matrix Optimisation](https://arxiv.org/abs/2605.07067)
*Haozhou Zhang*
...
### [159] [Less Random, More Private: What is the Optimal Subsampling Scheme for DP-SGD?](https://arxiv.org/abs/2605.07072)
...
### [160] [ModelLens: Finding the Best for Your Task from Myriads of Models](https://arxiv.org/abs/2605.07075)
...
### [161] [Test-Time Compositional Generalization in Diffusion Models via Concept Discovery](https://arxiv.org/abs/2605.07078)
...
### [162] [Actor-Critic with Active Importance Sampling](https://arxiv.org/abs/2605.07094)
...
### [163] [Query-efficient model evaluation using cached responses](https://arxiv.org/abs/2605.07096)
...
### [164] [CarCrashNet: A Large-Scale Dataset and Hierarchical Neural Solver for Data-Driven Structural Crash Simulation](https://arxiv.org/abs/2605.07098)
...
### [165] [Almost Sure Convergence Rates of Stochastic Approximation and Reinforcement Learning via a Poisson-Moreau Drift](https://arxiv.org/abs/2605.07104)
...
### [166] [Theoretical Limits of Language Model Alignment](https://arxiv.org/abs/2605.07105)
...
### [167] [Solving Max-Cut to Global Optimality via Feasibility-Preserving Graph Neural Networks](https://arxiv.org/abs/2605.07113)
...
### [168] [Where to Spend Rollouts: Hit-Utility Optimal Rollout Allocation for Group-Based RLVR](https://arxiv.org/abs/2605.07114)
...
### [169] [Conformal-Style Quantile Analyses for Stochastic Bandits](https://arxiv.org/abs/2605.07115)
...
### [170] [Stabilized neural Hamilton--Jacobi--Bellman solvers: Error analysis and applications in model-based reinforcement learning](https://arxiv.org/abs/2605.07116)
...
### [171] [When Symbol Names Should Not Matter: A Logistic Theory of Fresh-Symbol Classification](https://arxiv.org/abs/2605.07120)
...
### [172] [Convergence and Emergence of In-Context Reinforcement Learning with Chain of Thought](https://arxiv.org/abs/2605.07123)
...
### [173] [The Position Curse: LLMs Struggle to Locate the Last Few Items in a List](https://arxiv.org/abs/2605.07127)
...
### [174] [Simple KNN-Based Outlier Detection Achieves Robust Clustering](https://arxiv.org/abs/2605.07130)
...
### [175] [GAD in the Wild: Benchmarking Graph Anomaly Detection under Realistic Deployment Challenges](https://arxiv.org/abs/2605.07133)
...
### [176] [Adaptive Negative Reinforcement for LLM Reasoning:Dynamically Balancing Correction and Diversity in RLVR](https://arxiv.org/abs/2605.07137)
...
### [177] [Regret-Oracle Complexity Tradeoffs in Agnostic Online Learning](https://arxiv.org/abs/2605.07155)
...
### [178] [Learned Lagrangian Models of PDEs via Euler-Lagrange Residual Minimization](https://arxiv.org/abs/2605.07157)
...
### [179] [Neurosymbolic Imitation Learning with Human Guidance: A Privileged Information Approach](https://arxiv.org/abs/2605.07166)
...
### [180] [Cost-Ordered Feasibility for Multi-Armed Bandits with Cost Subsidy](https://arxiv.org/abs/2605.07171)
...
### [181] [Learning Multi-Relational Graph Representations for DNA Methylation-Based Biological Age Estimation](https://arxiv.org/abs/2605.07175)
...
### [184] [Coupling Models for One-Step Discrete Generation](https://arxiv.org/abs/2605.07193)
...
### [186] [Arrow: A Foundation Model for Causal Discovery](https://arxiv.org/abs/2605.07204)
...
### [187] [Experience Sharing in Mutual Reinforcement Learning for Heterogeneous Language Models](https://arxiv.org/abs/2605.07244)
...
### [188] [FAME: Forecasting Academic Impact via Continuous-Time Manifold Evolution](https://arxiv.org/abs/2605.07208)
...
### [189] [Bifurcation Models: Learning Set-Valued Solution Maps with Weight-Tied Dynamics](https://arxiv.org/abs/2605.07277)
...
### [190] [Predictive but Not Plannable: RC-aux for Latent World Models](https://arxiv.org/abs/2605.07278)
...
### [191] [Mask2Cause: Causal Discovery via Adjacency Constrained Causal Attention](https://arxiv.org/abs/2605.07280)
...
### [192] [Improved Model-based Reinforcement Learning with Smooth Kernels](https://arxiv.org/abs/2605.07218)
...
### [193] [Generative Modeling with Flux Matching](https://arxiv.org/abs/2605.07319)
...
### [194] [On the Robustness of Distribution Support under Diffusion Guidance](https://arxiv.org/abs/2605.07220)
...
### [195] [SparseRL-Sync: Lossless Weight Synchronization with ~100x Less Communication](https://arxiv.org/abs/2605.07330)
...
### [196] [Don't Learn the Shape: Forecasting Periodic Time Series by Rank-1 Decomposition](https://arxiv.org/abs/2605.07222)
*Takato Honda*
...
### [197] [Rethinking Importance Sampling in LLM Policy Optimization: A Cumulative Token Perspective](https://arxiv.org/abs/2605.07331)
...
### [198] [Modulated learning for private and distributed regression with just a single sample per client device](https://arxiv.org/abs/2605.07233)
...
### [199] [Mage: Multi-Axis Evaluation of LLM-Generated Executable Game Scenes Beyond Compile-Pass Rate](https://arxiv.org/abs/2605.07342)
...
### [200] [Sample Complexity of Stochastic Optimization with Integer Variables](https://arxiv.org/abs/2605.07239)
...
### [201] [MISA: Mixture of Indexer Sparse Attention for Long-Context LLM Inference](https://arxiv.org/abs/2605.07363)
...
### [202] [Unsolvability Ceiling in Multi-LLM Routing: An Empirical Study of Evaluation Artifacts](https://arxiv.org/abs/2605.07395)
...
### [203] [When Are Experts Misrouted? Counterfactual Routing Analysis in Mixture-of-Experts Language Models](https://arxiv.org/abs/2605.07260)
...
### [204] [Rubric-based On-policy Distillation](https://arxiv.org/abs/2605.07396)
...
### [205] [PerCaM-Health: Personalized Dynamic Causal Graphs for Healthcare Reasoning](https://arxiv.org/abs/2605.07267)
...
### [206] [Tracking Large-scale Shared Bikes with Inertial Motion Learning in GNSS Blocked Environments](https://arxiv.org/abs/2605.07412)
...
### [207] [bispectrum: Selective $G$-Bispectra Made Practical](https://arxiv.org/abs/2605.07270)
...
### [208] [Physical Simulators as Do-Operators: Causal Discovery under Latent Confounders for AI-for-Science](https://arxiv.org/abs/2605.07467)
*Tsuyoshi Okita*
...
### [209] [SHRED: Retain-Set-Free Unlearning via Self-Distillation with Logit Demotion](https://arxiv.org/abs/2605.07482)
...
### [210] [Does Your Neural Network Extrapolate? Feature Engineering as Identifiability Bias for OOD Generalization](https://arxiv.org/abs/2605.07483)
...
### [211] [Excluding the Target Domain Improves Extrapolation: Deconfounded Hierarchical Physics Constraints](https://arxiv.org/abs/2605.07485)
*Tsuyoshi Okita*
...
### [212] [The Convergence Gap: Instruction-Tuned Language Models Stabilize Later in the Forward Pass](https://arxiv.org/abs/2605.07282)
*Yifan Zhou*
...
### [213] [Why Self-Inconsistency Arises in GNN Explanations and How to Exploit It](https://arxiv.org/abs/2605.07527)
...
### [214] [Instruction Tuning Changes How Upstream State Conditions Late Readout: A Cross-Patching Diagnostic](https://arxiv.org/abs/2605.07284)
*Yifan Zhou*
...
### [215] [Pretraining Induces a Reusable Spectral Basis for Downstream Task Adaptation](https://arxiv.org/abs/2605.07302)
...
### [216] [Ensemble Distributionally Robust Bayesian Optimisation](https://arxiv.org/abs/2605.07565)
...
### [217] [Latent Order Bandits](https://arxiv.org/abs/2605.07304)
...
### [218] [Your Language Model is Its Own Critic: Reinforcement Learning with Value Estimation from Actor's Internal States](https://arxiv.org/abs/2605.07579)
...
### [219] [Revisiting Transformer Layer Parameterization Through Causal Energy Minimization](https://arxiv.org/abs/2605.07588)
...
### [220] [An Efficient Hybrid Sparse Attention with CPU-GPU Parallelism for Long-Context Inference](https://arxiv.org/abs/2605.07719)
...
### [221] [Beyond Linear Attention: Softmax Transformers Implement In-Context Reinforcement Learning](https://arxiv.org/abs/2605.07333)
...
### [222] [Curated Synthetic Data Doesn't Have to Collapse: A Theoretical Study of Generative Retraining with Pluralistic Preferences](https://arxiv.org/abs/2605.07724)
...
### [223] [CellScientist: Dual-Space Hierarchical Orchestration for Closed-Loop Refinement of Virtual Cell Models](https://arxiv.org/abs/2605.07335)
...
### [224] [Drifting Field Policy: A One-Step Generative Policy via Wasserstein Gradient Flow](https://arxiv.org/abs/2605.07727)
...
### [225] [Intelligent Truck Matching in Full Truckload Shipments using Ping2Hex approach](https://arxiv.org/abs/2605.07733)
...
### [226] [When Losses Align: Gradient-Based Composite Loss Weighting for Efficient Pretraining](https://arxiv.org/abs/2605.07756)
...
### [227] [POETS: Uncertainty-Aware LLM Optimization via Compute-Efficient Policy Ensembles](https://arxiv.org/abs/2605.07775)
...
### [228] [QuadNorm: Resolution-Robust Normalization for Neural Operators](https://arxiv.org/abs/2605.07375)
...
### [229] [Zero-Shot Neural Network Evaluation with Sample-Wise Activation Patterns](https://arxiv.org/abs/2605.07378)
...
### [230] [Neural Operators as Efficient Function Interpolators](https://arxiv.org/abs/2605.07792)
...
### [231] [StreamPhy: Streaming Inference of High-Dimensional Physical Dynamics via State Space Models](https://arxiv.org/abs/2605.07384)
...
### [232] [Toward Privileged Foundation Models:LUPI for Accelerated and Improved Learning](https://arxiv.org/abs/2605.07799)
...
### [233] [Convex Optimization with Nested Evolving Feasible Sets](https://arxiv.org/abs/2605.07386)
...
### [234] [Prune-OPD: Efficient and Reliable On-Policy Distillation for Long-Horizon Reasoning](https://arxiv.org/abs/2605.07804)
...
### [235] [Approximation-Free Differentiable Oblique Decision Trees](https://arxiv.org/abs/2605.07837)
...
### [236] [\mathsf{VISTA}: Decentralized Machine Learning in Adversary Dominated Environments](https://arxiv.org/abs/2605.07841)
...
### [237] [Have Graph -- Will Lift? The Case for Higher-Order Benchmarks](https://arxiv.org/abs/2605.07397)
*Bastian Rieck*
...
### [238] [On the Tradeoffs of On-Device Generative Models in Federated Predictive Maintenance Systems](https://arxiv.org/abs/2605.07860)
...
### [240] [KL for a KL: On-Policy Distillation with Control Variate Baseline](https://arxiv.org/abs/2605.07865)
...
### [241] [Trajectory as the Teacher: Few-Step Discrete Flow Matching via Energy-Navigated Distillation](https://arxiv.org/abs/2605.07924)
...
### [242] [Risk-Consistent Multiclass Learning from Random Label-Subset Membership Queries](https://arxiv.org/abs/2605.07413)
...
### [243] [INO-SGD: Addressing Utility Imbalance under Individualized Differential Privacy](https://arxiv.org/abs/2605.07930)
...
### [244] [SR$^2$-LoRA: Self-Rectifying Inter-layer Relations in Low-Rank Adaptation for Class-Incremental Learning](https://arxiv.org/abs/2605.07420)
...
### [245] [It Just Takes Two: Scaling Amortized Inference to Large Sets](https://arxiv.org/abs/2605.07972)
...
### [246] [A Flexible Adaptive Stable Clustering Algorithm for Archive-Scale Online Mass Spectrometry](https://arxiv.org/abs/2605.07424)
...
### [247] [Where's the Plan? Locating Latent Planning in Language Models with Lightweight Mechanistic Interventions](https://arxiv.org/abs/2605.07984)
*Nicole Ma,Nick Rui*
...
### [248] [GameGen-Verifier: Parallel Keypoint-Based Verification for LLM-Generated Games via Runtime State Injection](https://arxiv.org/abs/2605.07442)
...
### [249] [Graph-Structured Hyperdimensional Computing for Data-Efficient and Explainable Process-Structure-Property Prediction](https://arxiv.org/abs/2605.07999)
...
### [250] [Position: Mechanistic Interpretability Must Disclose Identification Assumptions for Causal Claims](https://arxiv.org/abs/2605.08012)
...
### [251] [Inference-Time Attribute Distribution Alignment for Unconditional Diffusion](https://arxiv.org/abs/2605.07456)
...
### [252] [Beyond Pairs: Your Language Model is Secretly Optimizing a Preference Graph](https://arxiv.org/abs/2605.08037)
...
### [253] [Estimation of Motor Unit Parameters from Surface Electromyograms using an Informed Autoencoder](https://arxiv.org/abs/2605.07458)
...
### [255] [Approximation Error Upper and Lower Bounds for Hölder Class with Transformers](https://arxiv.org/abs/2605.07463)
...
### [256] [Uncovering Hidden Systematics in Neural Network Models for High Energy Physics](https://arxiv.org/abs/2605.07470)
...
### [257] [Transfer Learning Across Fast- and Full-Simulation Domains in High-Energy Physics](https://arxiv.org/abs/2605.07471)
*Matthias Schott,Lucie Flek*
...
### [258] [NPMixer: Hierarchical Neighboring Patch Mixing for Time Series Forecasting](https://arxiv.org/abs/2605.07476)
...
### [259] [ExpThink: Experience-Guided Reinforcement Learning for Adaptive Chain-of-Thought Compression](https://arxiv.org/abs/2605.07501)
...
### [260] [Tessellations of Semi-Discrete Flow Matching](https://arxiv.org/abs/2605.07513)
...
### [261] [SGD for Variational Inference: Tackling Unbounded Variance via Preconditioning and Dynamic Batching](https://arxiv.org/abs/2605.07531)
...
### [262] [On the Invariance and Generality of Neural Scaling Laws](https://arxiv.org/abs/2605.07546)
...
### [263] [Disagreement-Regularized Importance Sampling for Adversarial Label Corruption](https://arxiv.org/abs/2605.07551)
...
### [265] [Bilevel Graph Structure Learning, Revisited: Inner-Channel Origins of the Reported Gain](https://arxiv.org/abs/2605.07577)
...
### [266] [Optimal Recourse Summaries via Bi-Objective Decision Tree Learning](https://arxiv.org/abs/2605.07598)
...
### [267] [Learning Large-Scale Modular Addition with an Auxiliary Modulus](https://arxiv.org/abs/2605.07648)
...
### [268] [Stochastic Transition-Map Distillation for Fast Probabilistic Inference](https://arxiv.org/abs/2605.07661)
...
### [269] [Direction-Preserving Number Representations](https://arxiv.org/abs/2605.07662)
...
### [270] [Structured Coupling for Flow Matching](https://arxiv.org/abs/2605.07676)
...
### [271] [Fortifying Time Series: DTW-Certified Robust Anomaly Detection](https://arxiv.org/abs/2605.07690)
...
### [272] [Toward Better Geometric Representations for Molecule Generative Models](https://arxiv.org/abs/2605.07693)
...
### [273] [Bayesian Fine-tuning in Projected Subspaces](https://arxiv.org/abs/2605.07706)
...
### [274] [Robust and Reliable AI for Predictive Quality in Semiconductor Materials Manufacturing with MLOps and Uncertainty Quantification](https://arxiv.org/abs/2605.07752)
...
### [276] [Efficient Verification of Neural Control Barrier Functions with Smooth Nonlinear Activations](https://arxiv.org/abs/2605.07757)
...
### [277] [Pre-trained Tabular Foundation Models as Versatile Summary Networks for Neural Posterior Estimation](https://arxiv.org/abs/2605.07765)
...
### [278] [Training-Induced Escape from Token Clustering in a Mean-Field Formulation of Transformers](https://arxiv.org/abs/2605.07772)
...
### [279] [Spectral Surgery: Class-Targeted Post-Hoc Rebalancing via Hessian Spike Perturbation](https://arxiv.org/abs/2605.07790)
...
### [280] [Flexible Routing via Uncertainty Decomposition](https://arxiv.org/abs/2605.07805)
...
### [281] [The Minimax Rate of Second-Order Calibration](https://arxiv.org/abs/2605.07808)
...
### [282] [OrScale: Orthogonalised Optimization with Layer-Wise Trust-Ratio Scaling](https://arxiv.org/abs/2605.07815)
*Yuxuan Lou,Yang You*
...
### [283] [Scaling Categorical Flow Maps](https://arxiv.org/abs/2605.07820)
...
### [284] [RelAgent: LLM Agents as Data Scientists for Relational Learning](https://arxiv.org/abs/2605.07840)
...
### [285] [Distributional simplicity bias and effective convexity in Energy Based Models](https://arxiv.org/abs/2605.07844)
...
### [286] [Actor-Critic Algorithm for Dynamic Expectile and CVaR](https://arxiv.org/abs/2605.07857)
...
### [287] [ADKO: Agentic Decentralized Knowledge Optimization](https://arxiv.org/abs/2605.07863)
...
### [288] [Black-box model classification under the discriminative factorization](https://arxiv.org/abs/2605.07878)
...
### [289] [Enhancing Federated Quadruplet Learning: Stochastic Client Selection and Embedding Stability Analysis](https://arxiv.org/abs/2605.07888)
...
### [290] [Adaptive Regularization for Sparsity Control in Bregman-Based Optimizers](https://arxiv.org/abs/2605.07892)
...
### [291] [Curvature Beyond Positivity: Greedy Guarantees for Arbitrary Submodular Functions](https://arxiv.org/abs/2605.07902)
...
### [292] [Flatness and Gradient Alignment Are Both Necessary: Spectral-Aware Gradient-Aligned Exploration for Multi-Distribution Learning](https://arxiv.org/abs/2605.07914)
...
### [293] [Tree SAE: Learning Hierarchical Feature Structures in Sparse Autoencoders](https://arxiv.org/abs/2605.07922)
...
### [294] [Prototype Guided Post-pretraining for Single-Cell Representation Learning](https://arxiv.org/abs/2605.07938)
...
### [295] [Slowly Annealed Langevin Dynamics: Theory and Applications to Training-Free Guided Generation](https://arxiv.org/abs/2605.07950)
...
### [296] [Convergent Stochastic Training of Attention and Understanding LoRA](https://arxiv.org/abs/2605.07959)
...
### [297] [Graph Representation Learning Augmented Model Manipulation on Federated Fine-Tuning of LLMs](https://arxiv.org/abs/2605.07961)
...
### [298] [FLAM: Evaluating Model Performance with Aggregatable Measures in Federated Learning](https://arxiv.org/abs/2605.07962)
...
### [299] [Aggregation in conformal e-classification](https://arxiv.org/abs/2605.07963)
*Vladimir Vovk*
...
### [300] [When Diffusion Model Can Ignore Dimension: An Entropy-Based Theory](https://arxiv.org/abs/2605.07969)
...
### [301] [Self-Play Enhancement via Advantage-Weighted Refinement in Online Federated LLM Fine-Tuning with Real-Time Feedback](https://arxiv.org/abs/2605.07977)
...
### [302] [Susceptibilities and Patterning: A Primer on Linear Response in Bayesian Learning](https://arxiv.org/abs/2605.07980)
...
### [303] [Bayesian Sensitivity of Causal Inference Estimators under Evidence-Based Priors](https://arxiv.org/abs/2605.07993)
...
### [304] [Interpreting Reinforcement Learning Agents with Susceptibilities](https://arxiv.org/abs/2605.08007)
...
### [305] [Adaptive Domain Decomposition Physics-Informed Neural Networks for Traffic State Estimation with Sparse Sensor Data](https://arxiv.org/abs/2605.08028)
...
### [306] [Don't Get Your Kroneckers in a Twist: Gaussian Processes on High-Dimensional Incomplete Grids](https://arxiv.org/abs/2605.08036)
...
### [307] [Reinforcement Learning for Exponential Utility: Algorithms and Convergence in Discounted MDPs](https://arxiv.org/abs/2605.08053)
...
### [308] [GRAPHLCP: Structure-Aware Localized Conformal Prediction on Graphs](https://arxiv.org/abs/2605.08074)
...
### [309] [Zero-Shot Imagined Speech Decoding via Imagined-to-Listened MEG Mapping](https://arxiv.org/abs/2605.08075)
