...
TL;DR: EarthMind-SR是一种大气超分辨率奠基模型，能够将全球AI天气预报从0.25度(~28公里)降尺度到1公里分辨率，同时生成67小时预报，零样本可迁移到不同地理区域。


<details>
  <summary>Details</summary>
Motivation: 解决传统数值天气预报模型在公里尺度上计算成本过高的问题，为能源、农业和灾害管理等需要精细时空细节的应用提供可访问的预报。

Method: 模型采用基于潜在一致性模型(LCM)扩散框架的三维U-Net结构，以GraphCast预报为输入，以NOAA的AORC数据为目标，在美国本土(CONUS)上进行补丁式样本训练。

Result: 模型在八个耦合表面变量上实现了近乎零的偏差；径向功率谱密度分析显示模型在10公里到100公里波长范围内保持了精细大气结构；在CONUS的三个案例研究中验证了性能，并在印度和德国展示了零样本全球可迁移性。

Conclusion: EarthMind-SR作为开源的奠基模型，为公里尺度AI天气预报建立了新范式，为区域微调、蒸馏以及气候服务和灾害预测等下游应用提供了平台。
...
TL;DR: 本文提出了SilIF方法，通过为隔离森林增加基于轮廓的得分层来提高无监督异常检测性能。该方法在欺诈检测基准数据集上显著提升了AUC-PR，但在某些数据集上无改进，并分析了差异原因。


<details>
  <summary>Details</summary>
Motivation: 在交易欺诈检测等标注稀缺的场景中，无监督异常检测至关重要。虽然隔离森林因其可扩展性和易部署性成为最受欢迎的经典方法之一，但其性能仍有提升空间。本文旨在增强隔离森林的检测能力，通过在树结构诱导的表示空间中引入轮廓得分来改进传统方法。

Method: SilIF方法通过在隔离森林的树结构诱导的表示空间中增加轮廓得分层来增强传统隔离森林。具体步骤包括：1)为每个数据点提取每棵树的路径长度向量（即"指纹"）；2)将这些指纹聚类成结构组；3)计算每个点的轮廓得分，衡量其与所属组和最近替代组的匹配程度；4)通过超参数α将轮廓信号与基础IF得分结合。

Result: 在IEEE-CIS欺诈检测基准数据集（约59万笔交易，3.5%欺诈率）上，SilIF（α=1.0）相比普通隔离森林在平均AUC-PR上提升了0.0080，且在全部五个随机种子实验中均表现更优（配对t检验p=0.046）。在另一个合成信用卡数据集（Sparkov）上，轮廓增强并没有带来改进。研究还分析了导致这两种不同结果的条件特征。

Conclusion: SilIF是一个对隔离森林的可调节增强方法，通过在诱导表示空间中计算轮廓得分，可以有效提升异常检测性能，特别是在某些数据集上表现显著提升，但也诚实地指出了该方法并非在所有场景下都有帮助。
...
TL;DR: PushCen-ADFL是一个通信高效的异步去中心化联邦学习框架，通过质心空间中的耦合设计解决了通信开销、聚合偏差和模型漂移问题，显著提升性能并降低通信成本。


<details>
  <summary>Details</summary>
Motivation: 异步去中心化联邦学习消除了中心协调和全局同步，适用于大规模异构系统。但频繁的点对点通信、有向拓扑上的异步更新和非IID数据导致通信开销过大、聚合偏差和严重的模型漂移问题。

Method: 提出PushCen-ADFL框架，在共享质心表示空间中采用：1）质心形式的消息交换；2）平均保持的推和混合技术校正聚合偏差；3）轻量级质心正则化减轻异质性和延迟导致的模型漂移；4）有界发送者去重缓冲区处理不规则异步到达。

Result: 在视觉数据集上的实验表明，PushCen-ADFL在数据异构性下将准确率提高了高达6%，同时将每次推送的通信成本降低了80%以上，实现了良好的准确率-通信权衡。

Conclusion: PushCen-ADFL是一个高效的异步去中心化联邦学习框架，通过在共享质心表示空间中耦合通信、聚合和局部稳定化，有效解决了异步去中心化联邦学习中的通信开销、聚合偏差和模型漂移问题。
...
TL;DR: 提出针对语义约束违规的异常检测方法，通过神经规则评估器编译逻辑规则并学习子树MLP门，结合奇美拉训练生成反事实示例，提高对组合性和关系性约束的异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 许多实际异常不仅仅是稀有输入，而是违反语义约束的行为。研究在这种场景下的异常检测问题：约束以学习到的视觉概念上的逻辑规则给出，但真正的规则违规在训练过程中是稀有或完全不存在的。

Method: 将每个逻辑约束编译为有向无环图，为内部逻辑运算符学习特征感知的子树MLP门，每个门将子节点特征和边级否定映射为父节点表示和规则满足概率。引入奇美拉训练策略：在特征层面进行操作数级别的反事实构建，通过连接不同样本的子树特征生成合成训练数据。

Result: 在CLEVRER、OpenImages和VidOR数据集上，该方法在规则级异常AUROC方面优于独立事件和同图像语义训练基线，特别是在组合性和关系性规则方面表现更好。方法同时产生标量异常分数和规则级归因。

Conclusion: 提出了一种神经规则评估器方法来处理语义约束异常检测问题，通过编译逻辑约束为有向无环图并学习基于特征的子树MLP门，结合奇美拉训练策略，提高了在稀有语义约束违规情况下的检测性能。
...
TL;DR: InfoQuant是一种无需训练的激活量化方法，通过峰值抑制正交变换和自适应异常token选择，使激活分布更易于量化，在大语言模型低比特量化中显著提升精度。


<details>
  <summary>Details</summary>
Motivation: 当前的低比特激活量化面临两个主要问题：激活值包含异常值，且其分布与低比特均匀量化器不匹配。现有的后训练量化方法虽然尝试解决这些问题，但很少明确指定哪些激活分布真正容易离散化。

Method: 该方法采用峰值抑制正交变换（PSOT）来重塑激活分布，使其更加量化友好，并结合自适应异常token选择来提高PSOT在优化过程中的鲁棒性。

Result: InfoQuant在多个大语言模型系列上均优于现有的后训练量化和端到端训练基线方法。在W4A4KV4配置下，保持97%的浮点精度，并将LLaMA-2 13B的性能差距比现有最佳方法减少了42%。

Conclusion: 这篇论文提出了一种新的激活量化方法InfoQuant，能够显著提升大语言模型低比特量化的精度，特别是在W4A4KV4配置下保持了97%的浮点精度，比现有方法有42%的性能提升。
...
### [6] [GAC: Noise-Aware Adaptive Mixing for Hybrid SFT-RL Post-Training](https://arxiv.org/abs/2605.26184)
*Yuelin Hu,Zhenbo Yu,Zhengxue Cheng,Wei Liu,Li Song*

Main category: cs.LG

TL;DR: 提出自适应混合权重控制器GAC，利用梯度方差和信号不一致性在线估计，在混合后训练中动态调整监督微调与强化学习的权重，显著提升性能且开销小。


<details>
  <summary>Details</summary>
Motivation: 混合后训练通常结合监督微调和强化学习，但固定的混合调度无法适应两个训练信号相对噪声随时间变化的情况。需要一种自适应方法来动态调整混合权重。

Method: 提出GAC（Gradient Agreement Controller）方法，利用在线估计的梯度方差和两个训练信号之间的不一致性来推导自适应混合权重。方法包含平滑处理、先验引导和有界更新，同时重用现有训练张量。

Result: 在数学、代码、科学和逻辑基准测试中，GAC方法一致地改进了混合后训练性能，优于强大的固定和基于规则的基线方法。模型规模越大，改进效果越明显，且训练开销小于1%。

Conclusion: 本研究证明了通过自适应混合权重控制相比固定或基于规则的基线方法能显著改进混合后训练性能，特别是在大规模模型上效果更明显。该方法具有高效性，训练开销小于1%，为自适应混合训练提供了一种实用且有效的解决方案。
...
### [7] [Max-Window Scale Estimation for Near-Lossless HiF8 W8A8 Quantization-Aware Training](https://arxiv.org/abs/2605.26189)
*Yingying Cheng,Jinquan Shi,Li Zhou,Zhiyang He,Zhaoyi Sun,Fan Zhang,Jie Sun*

Main category: cs.LG

TL;DR: 通过DTS框架系统研究HiF8 W8A8量化训练的故障模式，发现amax饱和和灾难性遗忘两个正交问题。采用保守DTS策略和BF16预热结合低学习率QAT的方法，在保持极低性能损失的同时解决了这两个问题。


<details>
  <summary>Details</summary>
Motivation: 低位浮点格式的量化感知训练虽然能实现高效的大语言模型部署，但会引入微妙且难以通过标准训练指标检测的故障模式。这些故障在训练损失上不可见，但对模型性能有严重影响，需要系统性方法进行识别和解决。

Method: 通过八个对照实验，采用延迟张量缩放（DTS）框架进行系统性分析。针对发现的两个正交故障模式，分别设计解决方案：1）对amax饱和问题采用基于64步历史窗口的保守最大算法DTS策略；2）对灾难性遗忘问题采用500步BF16预热后以低学习率（lr=10^{-5}）进行QAT的训练方法。

Result: 最终配置在OpenPangu-Embedded-1B上取得了显著的性能保持：相对于匹配的BF16基线，MMLU仅下降0.43%，HellaSwag下降0.58%，ARC-Challenge下降0.22%。训练损失在10,000步中的平均精度误差仅为0.11%，同时有效消除了amax饱和和灾难性遗忘两个故障模式。

Conclusion: 本文通过延迟张量缩放（DTS）视角系统地研究了OpenPangu-Embedded-1B的低位浮点量化感知训练（QAT），揭示了两个正交且难以发现的故障模式。研究发现组合使用保守的DTS策略和BF16预热加低学习率优化的方法能有效同时解决量化饱和和灾难性遗忘问题，在保持高性能的同时实现高效LLM部署。
...
### [8] [Modeling Dynamic Mixtures of Time-Delay Systems from Streaming Time Series](https://arxiv.org/abs/2605.26191)
*Ren Fujiwara,Yasuko Matsubara,Yasushi Sakurai*

Main category: cs.LG

TL;DR: 本文提出DelayMix框架，通过将流式时间序列视为时滞系统动态混合，利用固定长度的系统张量表示捕获历史和延迟特征，实现快速适应非平稳数据流的高效建模方法。


<details>
  <summary>Details</summary>
Motivation: 时序数据流自适应建模面临的主要挑战包括：1）由环境因素或输入延迟变化引起的快速系统变化（状态回归）会降低模型性能；2）为每个时间序列模式使用多个小型模型时，需要在准确性、鲁棒性和内存使用之间进行权衡。

Method: 本文提出了一种在线框架，将流式时间序列视为动态混合的时滞系统。该方法通过构建使用系统马尔可夫参数序列的总结系统张量，捕捉系统的动态行为和延迟特性，并采用张量分解算法从张量中提取相关历史模型，选择最适合当前状态回归的系统。整个框架保持固定的表示长度，实现快速适应环境变化且计算高效。

Result: 在实际数据集上的测试表明，DelayMix相比其他方法具有一致的优越性能，特别是在高度非平稳数据上，实现了更优的预测精度和更快的延迟适应能力。

Conclusion: 本文提出的DelayMix方法通过动态混合时滞系统的方式有效解决了时序数据流中的自适应建模问题，特别是在处理具有非平稳特性数据时表现出优越的性能，为复杂环境下的时序预测提供了高效可靠的解决方案。
...
### [9] [Co-folding model guided by structural proteomics](https://arxiv.org/abs/2605.26192)
*Alon Shtrikman,Nitzan Simchi,Michal Ran Shchory,Sagie Brodsky,Eran Seger,Kirill Pevzner*

Main category: cs.LG

TL;DR: AIMS-Fold是一个整合结构蛋白质组学数据与扩散模型的引导式框架，通过XL-MS和HDX-MS数据提升蛋白质复合物构象预测准确性，在诱导接近药物设计方面优于纯计算方法。


<details>
  <summary>Details</summary>
Motivation: 现有蛋白质结构生成模型能够准确预测单个蛋白质的静态结构，但在预测蛋白质复合物的构象状态时通常失败，这对于蛋白质设计和诱导接近模式（如抗体和PROTACs）至关重要。结构蛋白质组学技术（如XL-MS和HDX-MS）提供了有价值的空间和动态信息，但将这些稀疏、异质性的数据整合到现有模型中仍是一个挑战。

Method: AIMS-Fold框架采用推理时引导扩散方法，在生成采样过程中，通过可微分的物理势能函数，将来自XL-MS的空间约束和HDX-MS的溶剂可及性剖面作为引导信号，主动引导扩散模型的采样轨迹，从而优化蛋白质结构预测。

Result: 研究发现，这些结构蛋白质组学方法单独使用时都能提高预测准确性，而将它们整合使用时能产生协同改进。特别重要的是，通过利用这些实验限制，AIMS-Fold在具有挑战性的诱导接近目标上比纯计算方法（如Boltz-2）达到了更高的准确性。

Conclusion: 该研究提出了一个结合结构蛋白质组学数据与预训练扩散模型的AIMS-Fold框架，通过引导扩散采样过程，显著提高了蛋白质复合物构象状态预测的准确性，为诱导接近药物的结构基础药物设计提供了一个强大的计算工具。
...
### [10] [Bridging Classification and Reconstruction: Cooperative Time Series Anomaly Detection](https://arxiv.org/abs/2605.26193)
*Qideng Tang,Dai Chaofan,Wubin Ma,Yahui Wu,Haohao Zhou,Tao Zhang,Huan Li,Dalin Zhang*

Main category: cs.LG

TL;DR: 本文提出了CoAD框架，它通过协作设计将分类和重建两种时间序列异常检测范式结合起来。分类模块为重建模块提供概率信息的软掩码，重建模块帮助分类模块改善泛化能力。这种设计能有效检测被现有方法忽略的细微复杂异常，并在实验中展现出了卓越的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 时间序列异常检测是数据挖掘的重要研究主题，但现有深度学习方法在检测细微和持续异常方面存在不足。Outlier Exposure（OE）和Masked Autoencoder（MAE）是两种有前景的范式，但各有限制：OE方法泛化能力差，MAE方法存在掩码错位问题。需要一种能够结合两者优势、弥补各自缺陷的方法。

Method: CoAD框架将分类范式（基于Outlier Exposure）和重建范式（基于Masked Autoencoder）统一起来。分类模块生成概率信息的软掩码供重建模块使用，而重建模块反过来缓解分类模块的泛化问题。此外，分类模块经过精心设计，解决了分类粒度不当和忽略频率信息的问题。

Result: 在高质量基准数据集上的大量实验表明，CoAD在严格的评估协议下，在检测性能上显著超越了现有最先进的深度学习和传统数据挖掘方法。同时，CoAD是一个轻量级框架，比现有SOTA方法快得多，适合大规模实时应用。

Conclusion: 本文提出了CoAD这一新颖的时序异常检测框架，通过分类与重建模块的协作设计，有效解决了现有方法的局限性。实验证明CoAD在性能上显著优于现有方法，并且具有轻量高效的特点，展现了深度学习在时间序列异常检测领域的潜力。
...
### [11] [On the Role of Inductive Bias in Time-Series Pretraining: A Case Study in Learning Generalizable Representations for Clinical Time Series](https://arxiv.org/abs/2605.26194)
*Sharmita Dey,Diego Paez-Granados*

Main category: cs.LG

TL;DR: 研究针对临床时间序列学习中的小样本和异质性问题，提出PathoFM基础模型预训练框架，通过局部重建、时间连续性和无监督上下文动力学三种目标组合，生成能够泛化到不同患者和任务类型（分类与回归）的稳健表征。


<details>
  <summary>Details</summary>
Motivation: 临床时间序列学习通常受到小样本、异质队列和协议漂移的限制，而下游应用既包括分类任务（如病理诊断）也包括回归任务（如时间预测）。这些限制使得基础模型预训练具有吸引力，但需要解决的重要问题是预训练目标应该施加哪些归纳偏置，以使表征能够在不同任务类型和受试者之间有效迁移。

Method: 提出了PathoFM方法，这是一种基于编码器的Transformer模型，使用三种互补目标进行预训练：局部完成（重建连续掩码段以增强局部结构）、时间连续性（通过观察前缀预测掩码的中期延续以增强平滑性和因果一致性）、无监督上下文内动力学（通过注意力机制在受试者示例窗口条件下进行支持-查询重建）。

Result: 针对脊髓损伤病理步态分析的研究，通过PathoFM方法验证了动力学中心混合目标的优势，能够产生最平衡的迁移效果，在保持波形结构的同时兼顾判别能力。

Conclusion: 通过实证比较不同的目标函数家族，研究发现以动力学为中心的混合目标能够产生最均衡的迁移效果：分组目标有利于判别边距但会损害连续目标所需的幅度保真度，而仅重建目标能保持波形结构但在分类任务上可能表现不佳。结合局部重建与时间连续性，并在可获取样本条件下添加上下文条件化，可以获得具有强泛化能力的患者通用表征。
...
### [12] [From Privacy to Generalization: Linear Max-Information Bounds for DP-SGD](https://arxiv.org/abs/2605.26222)
...
### [15] [Unified Neural Scaling Laws](https://arxiv.org/abs/2605.26248)
...
### [16] [Quantized Keys Steal Attention: Bias Correction for KV-Cache Compression in Video Diffusion](https://arxiv.org/abs/2605.26266)
...
### [17] [Scaling World-Model Reinforcement Learning Through Diffusion Policy Optimization](https://arxiv.org/abs/2605.26282)
...
### [18] [Two-Parameter Flows for Learning Population Dynamics of Physical Systems](https://arxiv.org/abs/2605.26285)
...
### [19] [Stateful Inference for Low-Latency Multi-Agent Tool Calling](https://arxiv.org/abs/2605.26289)
*Victor Norgren*
...
### [20] [Dynamic Link Prediction with Temporally Enhanced Signed Graph Neural Networks](https://arxiv.org/abs/2605.26290)
...
### [21] [Classification and detection of multiple UAVs using rational Gaussian wavelet neural networks](https://arxiv.org/abs/2605.26310)
...
### [22] [Curriculum Learning for Safety Alignment](https://arxiv.org/abs/2605.26315)
...
### [23] [Semigroup Consistency as a Diagnostic for Learned Physics Simulators](https://arxiv.org/abs/2605.26324)
*Lennon J. Shikhman*
...
### [24] [Reparametrizing Shampoo and SOAP for Subspace Basis Updates and BFloat16 Storage](https://arxiv.org/abs/2605.26327)
...
### [25] [QAM-W: Joint 2D Codebook Quantization for LLM Weights via Hadamard Rotation and Activation-Aware Scaling](https://arxiv.org/abs/2605.26339)
...
### [26] [A PAC-Bayesian View of Generalisation for Physics-Informed Machine Learning](https://arxiv.org/abs/2605.26341)
...
### [27] [MechRL: Reinforcement Learning Agents Perform Circuit Discovery for Mechanistic Interpretability](https://arxiv.org/abs/2605.26343)
*Barsat Khadka*
...
### [28] [When Correct Demonstrations Hurt: Rethinking the Role of Exemplars in In-Context Learning](https://arxiv.org/abs/2605.26350)
...
### [29] [Energy-Gated Attention and Wavelet Positional Encoding: Complementary Inductive Biases for Transformer Attention](https://arxiv.org/abs/2605.26355)
*Athanasios Zeris*
...
### [30] [Balancing Plasticity and Stability with Fast and Slow Successor Features](https://arxiv.org/abs/2605.26357)
...
### [31] [Online Learning on Hidden-Convex Losses via Algorithmic Equivalence: Optimal Regret, Geometric Barrier, and Bandit Feedback](https://arxiv.org/abs/2605.26373)
...
### [32] [Function-Valued Causal Influence in Nonlinear Time Series](https://arxiv.org/abs/2605.26408)
...
### [33] [When Does Deep RL Beat Calibrated Baselines? A Benchmark Study on Adaptive Resource Control](https://arxiv.org/abs/2605.26418)
...
### [34] [Amortized Factor Inference Networks for Posterior Inference](https://arxiv.org/abs/2605.26419)
...
### [35] [FM-fMRI: Event Conditioned Flow Matching for Rest-to-Task fMRI Time-Series Synthesis](https://arxiv.org/abs/2605.26423)
...
### [36] [Aperiodic and Low-Frequency Spectral Bias in Reconstruction based EEG Foundation Models](https://arxiv.org/abs/2605.26434)
...
### [37] [DDGAD: Trajectory Dynamics for Diffusion-Based Graph Anomaly Detection](https://arxiv.org/abs/2605.26446)
...
### [38] [MuCon: Clipped Muon Updates for LLM Training](https://arxiv.org/abs/2605.26459)
*Albert Yi*
...
### [39] [Diffuse to Detect: Generative Diffusion Models for Unsupervised IC Anomaly Detection](https://arxiv.org/abs/2605.26468)
...
### [40] [Variational Inference for Evidential Deep Learning](https://arxiv.org/abs/2605.26477)
...
### [41] [Extra-Merge: Tracing the Rank-1 Subspace of Model Merging in Language Model Pre-Training](https://arxiv.org/abs/2605.26484)
...
### [42] [The Stability of Singular Distribution: A Spectral Perspective on the Two-Phase Dynamics of Language Model Pre-training](https://arxiv.org/abs/2605.26489)
...
### [43] [Beyond Pairwise Preferences: Listwise Reward-Aware Alignment for Diffusion Models](https://arxiv.org/abs/2605.26491)
...
### [44] [Dense2MoE: Pushing the Pareto Frontier of On-Device LLMs via Unified Pruning and Upcycling](https://arxiv.org/abs/2605.26496)
...
### [45] [SIKA-GP: Accelerating Gaussian Process Inference with Sparse Inducing Kernel Approximations for Bayesian Deep Learning](https://arxiv.org/abs/2605.26509)
...
### [46] [Open-Weight LLM Fine-Tuning Defenses are Susceptible to Simple Attacks](https://arxiv.org/abs/2605.26526)
...
### [47] [Recursive Flow Matching](https://arxiv.org/abs/2605.26535)
...
### [48] [Aligning Few-Step Generative Models by Amortizing Sample-based Variational Inference](https://arxiv.org/abs/2605.26552)
...
### [49] [Linear and Neural Dueling Bandits with Delayed Feedback](https://arxiv.org/abs/2605.26554)
...
### [50] [Auditing and Fixing Economic Validity in Tabular Foundation Models for Discrete Choice](https://arxiv.org/abs/2605.26559)
...
### [51] [Beyond Holistic Models: Systematic Component-level Benchmarking of Deep Multivariate Time-Series Forecasting](https://arxiv.org/abs/2605.26562)
...
### [52] [Distribution-Aware Conformal Prediction: A Framework for generating efficient prediction intervals for time series](https://arxiv.org/abs/2605.26569)
...
### [53] [Separate Aggregation of Split Network for Personalized Federated Learning](https://arxiv.org/abs/2605.26571)
...
### [54] [Focal Reward: Balanced Reinforcement Learning under Rubric-Based Rewards](https://arxiv.org/abs/2605.26579)
...
### [55] [On the Error-Correcting Effects of Stochasticity in Discrete Diffusion](https://arxiv.org/abs/2605.26582)
...
### [56] [Near-Optimal Regret in Adversarial Kernel Bandits](https://arxiv.org/abs/2605.26585)
...
### [57] [Few-shot Cross-country Generalization of Tabular Machine Learning and Foundation Models for Childhood Anemia Prediction under Distribution Shift](https://arxiv.org/abs/2605.26589)
...
### [58] [Geometry-Aware Contrastive Learning for Few-Shot Automatic Modulation Recognition](https://arxiv.org/abs/2605.26600)
...
### [59] [Spend Your Rollouts Where It Counts: Rollout Allocation for Group-Based RL Post-Training](https://arxiv.org/abs/2605.26606)
...
### [60] [PIDM-DP: Physics-Informed Diffusion with Dormand-Prince Integration for Chaotic System Identification and State Reconstruction across Multiple Dynamical Regimes](https://arxiv.org/abs/2605.26619)
*Shailendra Dabral*
...
### [61] [RT-Lynx: Putting the GEMM Sparsity In a Right Way for Diffusion Models](https://arxiv.org/abs/2605.26632)
...
### [62] [More Expressive Feedforward Layers: Part I. Token-Adaptive Mixing of Activations](https://arxiv.org/abs/2605.26647)
...
### [67] [Model Merging on Loss Landscape: A Geometry Perspective](https://arxiv.org/abs/2605.26693)
...
### [68] [SL-BiLEM: Structured Learnable Behavior-in-the-Loop Epidemic Modeling for Forecasting and Policy Evaluation](https://arxiv.org/abs/2605.26704)
...
### [69] [Image Feature Fusion-based Federated Client Unlearning (FCU)](https://arxiv.org/abs/2605.26715)
...
### [70] [MTL-FNO: A Lightweight Multi-Task Fourier Neural Operator for Sparse Field Reconstruction](https://arxiv.org/abs/2605.26718)
...
### [71] [APEX: Amplitude Anchors and Phase Priors for Target-Scarce Higher-Frequency Wave Prediction](https://arxiv.org/abs/2605.26732)
...
### [72] [Stabilizing Recurrent Dynamics for Test-Time Scalable Latent Reasoning in Looped Language Models](https://arxiv.org/abs/2605.26733)
...
### [73] [Localizing Memorized Regions in Diffusion Models via Coordinate-Wise Curvature Differences](https://arxiv.org/abs/2605.26756)
...
### [75] [Adversarial Training for Robust Coverage Network under Worst-case Facility Losses](https://arxiv.org/abs/2605.26763)
...
### [76] [Towards Generalization-Oriented Models for Vehicle Routing Problems with Mixture-of-Experts](https://arxiv.org/abs/2605.26776)
*Changhao Miao,Yuntian Zhang,Tongyu Wu,Fang Deng,Chen Chen*
...
### [77] [Ratio-Variance Regularized Policy Optimization](https://arxiv.org/abs/2605.26784)
...
### [78] [Pretrained Approximators for Low-Thrust Trajectory Cost and Reachability](https://arxiv.org/abs/2605.26790)
...
### [79] [Latent Recurrent Transformer: Architecture Exploration, Training Strategies, and Scaling Behavior](https://arxiv.org/abs/2605.26797)
...
### [80] [PATE-TabTransGAN: Differentially Private Synthetic Tabular Data Generation via Transformer-Based Student Discrimination](https://arxiv.org/abs/2605.26802)
*M. Youssef,M. Woźniak*
...
### [81] [Innovation: An Almost Characterization of Hallucination](https://arxiv.org/abs/2605.26808)
...
### [82] [The Kalman Evolve: Closing the Gap in Kalman Filtering via Interpretable Algorithm Discovery](https://arxiv.org/abs/2605.26830)
...
### [83] [MONA: Muon Optimizer with Nesterov Acceleration for Scalable Language Model Training](https://arxiv.org/abs/2605.26842)
...
### [84] [Not All Disagreement Is Learnable: Token Teachability in On-Policy Distillation](https://arxiv.org/abs/2605.26844)
...
### [85] [Learning Energy-Based Models from Stochastic Interpolants using Spatiotemporal Differences](https://arxiv.org/abs/2605.26850)
...
### [86] [RAPNet: Accelerating Algebraic Multigrid with Learned Sparse Corrections](https://arxiv.org/abs/2605.26854)
...
### [87] [Generalist Graph Anomaly Detection via Prototype-Based Distillation](https://arxiv.org/abs/2605.26857)
...
### [88] [The Strongest Teacher Is Not Always the Best Teacher: Student-Centric Answer Selection](https://arxiv.org/abs/2605.26872)
...
### [89] [Negligible in Size, Significant in Effect: On Scale Vectors in Large Language Models](https://arxiv.org/abs/2605.26895)
...
### [90] [SPHERE-JEPA: Spherical Prediction with Homogeneous Embeddings](https://arxiv.org/abs/2605.26900)
...
### [91] [EEG-FM-Audit: A Systematic Evaluation and Analysis Pipeline for EEG Foundation Models](https://arxiv.org/abs/2605.26910)
...
### [92] [Agile Online Model Selection: Resolving Adaptation Lag via Safeguarded Large Learning Rates](https://arxiv.org/abs/2605.26919)
...
### [93] [When Muon Optimizer Meets Adversarial Training: A Theoretical and Empirical Study](https://arxiv.org/abs/2605.26929)
...
### [94] [RLVR Datasets and Where to Find Them: Tracing Data Lineage for Better Training Data](https://arxiv.org/abs/2605.26971)
...
### [95] [Convergence of Spectral Descent for Non-smooth Optimization](https://arxiv.org/abs/2605.26977)
...
### [96] [TED: Related Party Transaction guided Tax Evasion Detection on Heterogeneous Graph](https://arxiv.org/abs/2605.26984)
...
### [97] [Sampling Data with Chains of Forward-Backward Diffusion Steps](https://arxiv.org/abs/2605.27006)
...
### [98] [SCENT: Aligning Mass Spectra with Molecular Structure for Olfactory Perception](https://arxiv.org/abs/2605.27009)
...
### [99] [SQARL: A Size-Agnostic Reinforcement Learning approach for Circuit Allocation in Distributed Quantum Architectures](https://arxiv.org/abs/2605.27027)
...
### [100] [Less is More: Early Stopping Rollout for On-Policy Distillation](https://arxiv.org/abs/2605.27028)
...
### [101] [Learning Dynamic Graph Representations through Timespan View Contrasts](https://arxiv.org/abs/2605.27063)
...
### [102] [Learning to Orchestrate Agents under Uncertainty](https://arxiv.org/abs/2605.27073)
...
### [103] [Two Speeds of Learning: A Representation-Readout Decomposition of Grokking and Double Descent](https://arxiv.org/abs/2605.27078)
...
### [104] [Trust Region Q Adjoint Matching](https://arxiv.org/abs/2605.27079)
...
### [105] [ReMoE: Boosting Expert Reuse through Router Fine-Tuning in Memory-Constrained MoE LLM Inference](https://arxiv.org/abs/2605.27081)
...
### [106] [Adversarial Dual On-Policy Distillation from Expressive Flow-based Teacher](https://arxiv.org/abs/2605.27095)
...
### [107] [Mildly Overparameterized ReLU Networks on Orthogonal Data: Incremental Learning and Implicit Bias](https://arxiv.org/abs/2605.27097)
...
### [108] [High-Quality Synthetic Financial Time-Series using a GAN-Diffusion Framework](https://arxiv.org/abs/2605.27113)
...
### [109] [DEI: Diversity in Evolutionary Inference for Quality-Diversity Search](https://arxiv.org/abs/2605.27130)
...
### [110] [Deep-layer limit and stability analysis of the basic forward-backward-splitting induced network (II): learning problems](https://arxiv.org/abs/2605.27133)
*Xuan Lin,Chunlin Wu*
...
### [111] [The Role of Causal Features in Strategic Classification for Robustness and Alignment](https://arxiv.org/abs/2605.27163)
...
### [112] [Nonlinear Data Integration via Kernel Methods for Data Collaboration Analysis](https://arxiv.org/abs/2605.27219)
...
### [113] [Explainable Comparison of Feature-Based and Deep Learning Models for TROPOMI Methane Plume Screening](https://arxiv.org/abs/2605.27236)
...
### [114] [Symbolic Regression via Latent Iterative Refinement](https://arxiv.org/abs/2605.27245)
...
### [115] [LUCoS: Latent Unsupervised Context Selection for Tabular Foundation Models](https://arxiv.org/abs/2605.27254)
...
### [116] [Falcon-X: A Time Series Foundation Model for Heterogeneous Multivariate Modeling](https://arxiv.org/abs/2605.27286)
...
### [117] [Kan Extension Transformers: A Categorical Unification of Attention, Diffusion, and Predict-Detach Self-Conditioning](https://arxiv.org/abs/2605.27259)
*Sridhar Mahadevan*
...
### [118] [Guiding LLM Post-training Data Engineering with Model Internals from Sparse Autoencoders](https://arxiv.org/abs/2605.27354)
...
### [119] [Transfer Learning using 66 Diseases for Disease Forecasting Applications](https://arxiv.org/abs/2605.27269)
...
### [120] [MobileMoE: Scaling On-Device Mixture of Experts](https://arxiv.org/abs/2605.27358)
...
### [121] [Causal Risk Minimization for High-Dimensional Treatments](https://arxiv.org/abs/2605.27281)
...
### [122] [Detectability in Diversity: Improved Canary Crafting for Privacy Auditing in One Run](https://arxiv.org/abs/2605.27292)
...
### [123] [BASIS: Batchwise Advantage Estimation from Single-Rollout Information Sharing for LLM Reasoning](https://arxiv.org/abs/2605.27293)
...
### [124] [Normal Guidance is what Attention Needs](https://arxiv.org/abs/2605.27306)
...
### [125] [Greening AI Inference with Accuracy and Latency-aware User Incentives](https://arxiv.org/abs/2605.27309)
...
### [126] [Probabilistic Smoothing with Ratio-Monotone Transforms for Global Optimization](https://arxiv.org/abs/2605.27316)
...
### [127] [From Scores to Gibbs Correctors: Accelerating Uniform-Rate Discrete Diffusion Models](https://arxiv.org/abs/2605.27352)
...
### [128] [BrickAnything: Geometry-Conditioned Buildable Brick Generation with Structure-Aware Tokenization](https://arxiv.org/abs/2605.26182)
*Zhengyang Ni,Feng Yan,Yu Guo,Fei Wang*

Main category: cs.AI

TL;DR: BrickAnything是一个几何条件自回归框架，用于从3D形状生成可建造的砖结构，通过结构感知树标记化和稳定性优化技术提高生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有砖结构生成方法要么依赖启发式优化（当目标形状在预定义约束下不可行时会失效），要么生成砖序列但不显式建模底层3D几何和装配关系。需要一种能够从多样3D表示生成可建造砖结构的方法。

Method: 1. 使用点云作为统一的几何接口；2. 提出结构感知树标记化方法，通过局部附着关系表示砖结构；3. 采用基于偏好的对齐后训练；4. 引入有效性约束解码和自适应回滚机制。

Result: BrickAnything能够生成几何忠实且物理可实现的砖结构，提出的标记化方法相比传统排序策略有效减少了回滚和重新生成。

Conclusion: BrickAnything通过在几何条件自回归框架中引入结构感知树形标记化，有效提升了从3D形状生成可建造砖块结构的能力，为构建几何忠实且物理可实现的砖结构提供了新方法。
...
### [129] [Can LLMs Introspect? A Reality Check](https://arxiv.org/abs/2605.26242)
*Shashwat Singh,Tal Linzen,Shauli Ravfogel*

Main category: cs.AI

TL;DR: 本文挑战了大型语言模型具有元认知监控能力的观点，认为现有证据不足，模型表现可能只是基于表面模式匹配而非真正的内省。


<details>
  <summary>Details</summary>
Motivation: 质疑现有研究关于大型语言模型能够检测和报告自身内部状态的结论，认为这种结论可能为时过早，需要区分真正内省与基于表面线索的模式匹配。

Method: 重新审视两种评估范式：一是检验模型能否可靠区分对其内部状态的干预和输入操纵；二是分析模型预测自身隐藏状态衍生标签的任务。引入重新标记的控制设置，迫使模型依赖内部表示而非任务语义。

Result: 在第一个范式中，模型无法可靠区分对其内部状态的干预和输入操纵，表明其成功检测到的是广义异常而非特定内部状态干预。在第二个范式中，仅访问输入的分类器与模型自身通过上下文预测的性能相当，且重新标记的控制设置下模型表现接近随机水平。

Conclusion: 当前研究结果表明，现有证据不足以确定大型语言模型具有元认知监控能力。真正的内省与基于表面线索的模式匹配难以区分，且仅凭行为证据无法确立强烈的内省主张。
...
### [130] [Is Agent Memory a Database? Rethinking Data Foundations for Long-Term AI Agent Memory](https://arxiv.org/abs/2605.26252)
*Abdelghny Orogat,Essam Mansour*

Main category: cs.AI

TL;DR: 本文提出了一个新的记忆管理框架GEM，用于解决长期AI智能体记忆系统的常见问题。通过将记忆视为状态轨迹而非孤立记录，引入状态级操作和正确性条件，挑战了传统记录级数据库范式的局限性。


<details>
  <summary>Details</summary>
Motivation: 长期运行的AI智能体需要持久化记忆来支持跨会话学习、减少重复上下文注入，并能审计过去决策。当前的内存系统和数据库范式将记忆视为存储，仅提供记录、嵌入或边的局部正确性，无法满足长期记忆需求，导致四种常见失败模式：不受控制的增长、语义修订缺失、容量驱动的遗忘和只读检索。

Method: 提出了Governed Evolving Memory（GEM）框架，取代记录级数据库操作，引入四种状态级操作符：注入、修订、遗忘和检索。提出了六个正确性条件来规范状态演变。通过三个结构观察证明，任何记录级系统都无法满足这些条件。构建了MemState原型系统，在属性图后端实现GEM抽象，验证其可行性。

Result: 提出的GEM框架将长期智能体记忆视为新的数据管理工作负载，其正确性是状态轨迹的属性而非单个记录的属性。MemState原型证明了GEM的可行性，同时暴露了与原生引擎的差距。研究为记忆中心的数据管理确定了三个主要研究方向。

Conclusion: 长期AI智能体需要新的存储范式，GEM（Governed Evolving Memory）作为状态级操作的管理框架提供了一个可行的方向。这项研究为记忆中心的数据管理开辟了新的研究方向，挑战了以记录为中心的现有数据库范式。
...
### [131] [Your Agents Are Aging Too: Agent Lifespan Engineering for Deployed Systems](https://arxiv.org/abs/2605.26302)
*Jianing Zhu,Yeonju Ro,John Robertson,Kevin Wang,Junbo Li,Haris Vikalo,Aditya Akella,Zhangyang Wang*

Main category: cs.AI

TL;DR: 该论文指出长期AI智能体存在可靠性老化问题，提出了名为AgingBench的纵向评估框架来分析压缩、干扰、修订和维护四种老化机制，发现智能体老化是多维度的，需要全生命周期的诊断和修复策略。


<details>
  <summary>Details</summary>
Motivation: 当前长期运行的AI智能体在部署后作为持续性操作系统运行，但仍然像新初始化模型一样被评估。初始基准测试错过了基本的系统问题：智能体部署后保持可靠的时间有多长？智能体的可靠性应是全生命周期属性，而不仅仅是基础模型的快照属性。

Method: 提出AgingBench纵向可靠性基准测试，将智能体老化分解为四个机制：压缩老化、干扰老化、修订老化和维护老化。使用时间依赖图和配对反事实探针来诊断存储管道的写入、检索和利用阶段。

Result: 在7个场景、14个模型、多种存储策略、手动控制和自主智能体的400多次运行中显示：智能体老化不是一维的；行为测试可能保持正常而事实精度衰减；派生状态跟踪可能在单个模型内急剧崩溃；相同的错误答案需要不同的修复方法，取决于诊断图谱的指示。

Conclusion: 可靠的智能体部署需要全生命周期评估、机制级诊断和针对性的阶段修复，而不仅仅是更强的初始模型。AgingBench为实现这一目标提供了系统化的评估框架。
...
### [132] [OmniToM: Benchmarking Theory of Mind in LLMs via Explicit Belief Modeling](https://arxiv.org/abs/2605.26322)
*Adam Bawatneh,Sagar Sapkota,Amrit Singh Bedi,Santu Karmaker,Mubarak Shah*

Main category: cs.AI

TL;DR: 研究开发了OmniToM基准测试来直接评估LLMs的心智状态表征能力，发现模型在理解角色信念和将其从事实转化为心理状态方面存在显著瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型的心智理论评估通常使用端点问答范式，仅通过社交推理查询的最终答案判断性能。这种方法掩盖了模型是否真正构建了底层的心智状态表示，特别是在涉及分歧、演化或错误信念的场景中。为了填补这一研究空白，需要直接评估这些心理表征。

Method: 设计了OmniToM基准测试，该测试要求模型对叙事中所有相关角色的信念结构进行显式建模。这些结构由信念命题组成：关于角色认为世界中或另一角色心智状态为真的最小陈述。测试分为两个阶段：信念提取（从故事中提取与社会动态相关的信念）和信念标注（使用七维模式为每个信念分配标签）。该方法基于895个来自ToMBench故事语料库的故事，并增加了22,343个标注的信念命题，采用人类校准的LLM辅助标注管道。

Result: 在零样本评估中，OmniToM揭示了一个角色特定的信念追踪瓶颈：当前大型语言模型在将叙述事实转化为角色的信念和共享心智状态时，在知识获取和表征决策方面存在困难。模型难以处理将叙事事实转化为演员信念和共享心智状态所需的知识访问和表征决策。

Conclusion: OmniToM证实现有大型语言模型在将叙述事实转化为角色信念和共享心智状态方面存在能力限制，揭示了模型在信念追踪和理解他人心理状态方面的瓶颈。
...
### [133] [Managing Uncertainty in LLM-Generated Procedural Knowledge for Virtual Laboratory Planning](https://arxiv.org/abs/2605.26333)
*Polychronis Karpodinis,Dimitris Kalles*

Main category: cs.AI

TL;DR: 开发了一个框架来管理LLM为虚拟实验室生成的程序知识中的不确定性，通过结构化表示和约束转换来修复不完整的实验步骤。


<details>
  <summary>Details</summary>
Motivation: 教育虚拟实验室虽能扩大实验培训规模和可访问性，但创建新的模拟实验程序成本高昂。大型语言模型虽能协助生成详细实验步骤，但其输出存在多种问题：可能遗漏必要动作、步骤顺序错误、产生逻辑错误或与设备不兼容的指令，不能直接作为可执行计划。

Method: 作者提出了一个原型框架：利用结构化领域表示和不确定的LLM生成状态转换样本来提取候选程序规则，然后将这些规则转化为明确且可检查的约束条件，并用这些约束来修复不确定的程序步骤。

Result: 提出了一个管理LLM生成程序知识不确定性的框架，可用于虚拟实验室领域（涉及仪器、容器、工具和材料传输操作），将不确定的程序知识转化为可执行的规划约束。

Conclusion: 该框架为虚拟实验室规划中的LLM生成程序知识不确定性管理提供了初步方案，能够将不确定的程序规则转化为可检查的约束条件来修复不完整的实验步骤，为结构化交互环境中的动作规划问题提供了新的解决思路。
...
TL;DR: 针对大语言模型幻觉检测中自动选择高性能层的问题，论文提出了一种新的训练自由标准FEPoID，能一致性地识别最优层，并通过截断策略显著提高检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究显示幻觉相关信号在大语言模型（LLMs）的中间层比最终层编码更强烈。虽然已有工作试图利用这一特性进行幻觉检测，但如何自动选择高性能层的方法仍然不足，缺乏原则性的方法来解决这一问题。

Method: 论文首先提出了几种关于为何幻觉信号在中间层更强的假设，并评估了对应的自动层选择标准。当发现这些标准效果不佳时，作者提出了新的选择标准FEPoID（内在维度的第一个有效峰值），并研究了LLMs的生成行为，引入了简单但有效的截断策略。

Result: 研究发现之前提出的假设标准在多样化的LLM架构、规模和任务中都不能一致性地提供令人满意的性能。相比之下，新提出的FEPoID标准能一致性地识别最优或接近最优的层，表现出更好的检测性能，并且训练免费、计算开销可忽略。

Conclusion: 论文提出了一种新的训练自由层选择标准FEPoID，它能一致性地识别最优或接近最优的层，并通过简单的截断策略进一步放大了幻觉相关信号，显著提高了幻觉检测的整体性能。
...
TL;DR: CARL通过对比动作表示学习可复用技能，解决了HRL中技能复用性不足的问题。该方法利用局部动态信息，在不同全局上下文中识别相似动态对应的动作序列，从而提升高级策略的推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统分层强化学习虽然承诺通过发现和复用时域扩展技能来解决长时域任务，但实际获得的技能往往难以真正在不同情境中复用。本文旨在解决技能复用性不足这一核心挑战，通过利用局部动态的直觉：在不同全局上下文中，相似的局部转变需要相似类型的动作序列。

Method: CARL基于对比动作表示，通过局部动态建模学习可复用技能。核心思想是识别在不同全局上下文中具有相似局部动态的情况，并对齐其所需的动作序列表示。通过学习这些表示，算法能够确定哪些技能可以在哪些场景中复用，从而提升HRL中高级策略的规划效率。

Result: CARL算法在复杂人形机器人环境中展现出有意义技能聚类的定性结果，同时通过与HIQL算法整合，在OGBench基准测试上实现了更好的下游任务性能。

Conclusion: CARL通过引入对比动作表示来捕获局部动态信息，从而解决技能复用问题。该方法在复杂环境中展现了有意义技能聚类的潜力，并通过与HIQL结合在OGBench基准测试中提升了性能。结果表明，基于局部动态的技能表示能够有效提升分层强化学习在长时域任务中的效率和泛化能力。
...
TL;DR: 该论文针对大型多模态模型在开放式环境中的创造性工具使用能力不足问题，提出了MM-CreativityBench评估基准和affordance-grounded alignment对齐方法，通过偏好学习和知识库监督提升模型的实体识别和视觉落地能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型（LMMs）虽然在感知和推理方面进步迅速，但在开放式环境中的创造性问题解决能力尚未得到充分验证。传统测试多关注模式识别而非真正理解如何在物理约束下重新利用场景元素。需要评估模型在视觉丰富、物理约束环境中识别可用性并组合视觉和物理落地解决方案的能力。

Method: 1. 引入MM-CreativityBench基准测试框架，包含带有结构化视图的场景图像；2. 提出affordance-grounded alignment方法，将创造性工具使用转化为偏好学习问题；3. 使用直接偏好优化（DPO）技术，鼓励模型偏好基于视觉证据的属性-可供性推理；4. 结合来自可供性知识库的监督，指导更广泛的实体探索和多轮规划。

Result: 实验表明当前LMMs在创造性工具使用任务中表现不佳，经常忽视相关实体、对关键部件检查不足或产生未基于图像的幻觉属性。提出的affordance-grounded alignment方法在实体和部件选择方面带来一致改进，同时显著减少幻觉和落地相关错误。

Conclusion: 论文通过引入MM-CreativityBench基准测试和提出affordance-grounded alignment方法，显著提升了大型多模态模型在创造性工具使用场景中的实体选择、部件识别能力，并大幅减少了幻觉和落地相关的错误，为多模态模型在开放环境中的创造性问题解决能力评估提供了有效框架。
