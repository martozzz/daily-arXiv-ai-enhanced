...
TL;DR: PRR框架通过轨迹基础的精炼控制来加速扩散语言模型解码，使用温度分布塑造调节标记级精炼。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在文本生成时对所有标记应用统一精炼规则，但实践中标记稳定速度不同，导致大量冗余精炼。现有方法基于瞬时步骤级信号评估精炼必要性，而未考虑标记收敛是其在未来精炼轨迹上的变化定义的。

Method: 提出渐进精炼调节(PRR)：一种渐进式、轨迹基础的精炼控制框架。从完整解码推演中导出标记级经验收敛进度信号，基于该信号学习轻量级标记级控制器，在渐进自演化的训练方案下通过温度分布塑造调节精炼。

Result: 实验表明PRR显著加速扩散语言模型解码，同时保持生成质量。

Conclusion: PRR提供了一种动态的、轨迹感知的精炼控制方法，能有效减少冗余精炼，提高扩散语言模型的解码效率。
...
### [6] [ECG-MoE: Mixture-of-Expert Electrocardiogram Foundation Model](https://arxiv.org/abs/2603.04589)
...
### [7] [Towards automated data analysis: A guided framework for LLM-based risk estimation](https://arxiv.org/abs/2603.04631)
*Panteleimon Rodis*

Main category: cs.AI

TL;DR: 论文提出了一种集成生成式AI与人工指导的数据集风险评估框架，利用LLMs分析数据库模式并生成代码，人类监督确保流程完整性，概念验证证明了框架的实用性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地集成到关键决策流程中，对鲁棒且自动化的数据分析需求日益增长。当前的数据集风险分析方法局限于人工审计，耗时且复杂；而完全基于人工智能的自动化分析存在幻觉和对齐问题。

Method: 利用大型语言模型识别数据库模式的语义和结构属性，提出聚类技术，生成相应的代码，并解释分析结果。整个过程由人类监督者指导分析目标并确保流程完整性与任务目标的对齐。

Result: 通过概念验证展示了该框架在风险评估任务中产生有意义结果的可行性。

Conclusion: 该论文提出的框架在数据集风险评估任务中展示了可行性，为人机协作的风险分析范式奠定了基础，并有望推动未来自动化风险分析的发展。
...
TL;DR: LLM can generate propaganda content when given specific prompts, and fine-tuning methods like ORPO are effective in reducing this behavior.


<details>
  <summary>Details</summary>
Motivation: Although LLMs have many advantages, when deployed in open environments, they may be manipulated to generate propaganda content. The study aims to explore LLMs' propensity to generate propaganda and evaluate mitigation methods.

Method: First, prompt LLMs with propaganda objectives and analyze the outputs using two domain-specific models: one for classifying propaganda vs. non-propaganda, and another for identifying specific rhetorical techniques (e.g., loaded language, appeals to fear). Then, apply mitigation techniques including Supervised Fine-Tuning (SFT), Direct Preference Optimization (DPO), and Odds Ratio Preference Optimization (ORPO) to reduce propaganda generation.

Result: LLMs do exhibit propagandistic behaviors when prompted and employ various rhetorical techniques. Among the mitigation methods, fine-tuning significantly reduces the tendency to generate such content, with ORPO proving the most effective.

Conclusion: LLM-based agents are susceptible to being exploited for propaganda generation, but through appropriate fine-tuning methods, particularly ORPO, this risk can be substantially mitigated.
...
TL;DR: 大型语言模型（GPT-4.1-nano）能有效预测数据可视化素养测试项目的难度，多模态方法（结合视觉和文本特征）效果最佳。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索大语言模型是否能够预测数据可视化素养测试项目的难度，以及如何利用项目文本特征、可视化图像特征或两者结合的特征集来提升预测准确性。这一研究有助于推动心理测量学的自动化项目开发。

Method: 使用GPT-4.1-nano分析测试项目，基于不同的特征集（纯文本、纯图像、多模态输入）生成难度预测。通过比较各特征集的平均绝对误差来评估模型性能。

Result: 多模态方法（结合视觉和文本特征）取得了最低的平均绝对误差（0.224），优于纯视觉方法（0.282）和纯文本方法（0.338）。在外部验证集上的均方误差为0.10805。

Conclusion: 研究表明，大型语言模型在心理测量分析和自动化项目开发方面具有潜力，多模态输入能显著提升数据可视化测试项目难度预测的准确性。
...
### [10] [Model Medicine: A Clinical Framework for Understanding, Diagnosing, and Treating AI Models](https://arxiv.org/abs/2603.04722)
*Jihoon Jeong*

Main category: cs.AI

TL;DR: 该论文提出了'模型医学'作为一个研究计划，旨在为AI模型创建类似医学的系统化理解、诊断和治疗框架。


<details>
  <summary>Details</summary>
Motivation: 当前AI可解释性研究类似于解剖观察，但缺乏对复杂AI系统所需的系统性临床实践。AI模型与生物有机体类似，具有内在结构、动态过程、可遗传特性、可观察症状、可分类条件和可治疗状态。

Method: 论文提出了五个主要贡献：1) 包含15个子学科的学科分类法；2) 基于720个智能体和24,923个决策实证的四层壳模型行为遗传学框架；3) 神经MRI诊断工具，将医学神经影像模式映射到AI可解释性技术；4) 五层诊断框架；5) 临床模型科学工具集。

Result: 开发了包括四层壳模型、神经MRI工具、五层诊断框架和临床工具在内的系统性框架，验证了诊断工具在四个临床案例中的成像、比较、定位和预测能力。

Conclusion: 模型医学作为连接AI可解释性研究和系统性临床实践的桥梁，为AI模型的诊断、治疗和理解提供了生物学启发的综合框架，有望推动AI系统向更安全、可靠的方向发展。
...
TL;DR: AI通过神经符号系统解决了理论物理学中的开放问题，推导出宇宙弦引力辐射功率谱的精确解析解。


<details>
  <summary>Details</summary>
Motivation: 验证人工智能能否加速数学发现，特别是解决理论物理学中的开放问题，改进近期AI辅助研究中只能得到部分渐近解的限制。

Method: 结合Gemini Deep Think大语言模型与系统化树搜索（TS）框架，加入自动数值反馈机制，评估核心积分I(N,α)并识别出6种不同的解析方法。

Result: 系统成功推导出宇宙弦引力辐射功率谱的新颖精确解析解，发现了6种解析方法，其中最优雅的方法使用Gegenbauer多项式C_l^{(3/2)}展开核函数来自然吸收被积函数的奇异性，得到了与数值结果一致的大N渐近结果。

Conclusion: 该研究展示了神经符号系统在数学发现中的巨大潜力，通过结合大语言模型与系统化树搜索框架，成功解决了理论物理学中的一个开放性问题，为AI加速科学发现提供了方法论范例。
...
TL;DR: 提出交互式基准测试框架，通过交互证明和游戏评估模型主动获取信息的能力，发现当前模型在交互推理方面仍有不足。


<details>
  <summary>Details</summary>
Motivation: 现有标准基准测试存在饱和、主观性强和泛化能力差的问题，无法可靠评估模型智能，需要评估模型主动获取信息的能力。

Method: 提出了一个统一的交互式基准测试框架，包含两个具体实例化：1) 交互式证明，模型与裁判互动以推断逻辑和数学中的客观事实或答案；2) 交互式游戏，模型进行战略推理以最大化长期效用。

Result: 交互式基准测试提供了对模型智能的稳健且可靠的评估，显示模型在交互式场景中仍有显著改进空间。

Conclusion: 尽管当前模型在交互式场景中仍有显著改进空间，但交互式基准测试为模型智能提供了更稳健和可靠的评估方法。
...
TL;DR: 提出CONE模型，通过复合嵌入算法编码数字及相关信息，显著提升数值推理能力，在DROP等任务上超越SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 大型预训练模型和大型语言模型在捕捉语言语义和上下文关系方面表现良好，但在处理涉及数字的任务时面临性能挑战。单纯将数值或结构化数据视为普通词汇是不够的，模型需要准确理解和编码其语义。

Method: 提出了CONE（混合Transformer编码器预训练模型），通过新颖的复合嵌入构造算法将数字、范围和高斯分布与其相关单位和属性名整合编码到嵌入向量空间，保持距离关系。

Result: 在跨领域大规模数据集上的实验表明，CONE在DROP数据集上获得了87.28%的F1分数，相比最先进的基线模型提升了高达9.37%。在Recall@10指标上也显著优于主要SOTA模型，提升幅度高达25%。

Conclusion: CONE模型通过引入复合嵌入构造算法，成功解决了大型语言模型在数字处理方面的局限性，显著提升了数值推理能力，为处理结构化数值数据提供了有效的解决方案，在多个领域的数据集上都展现出优异的性能。
...
### [15] [Visioning Human-Agentic AI Teaming: Continuity, Tension, and Future Research](https://arxiv.org/abs/2603.04746)
*Bowen Lou,Tian Lu,T. S. Raghu,Yingjie Zhang*

Main category: cs.AI

TL;DR: 智能体AI系统给人类-AI团队合作带来结构性不确定性，团队态势感知理论需要扩展以应对开放端智能体性，重新审视传统团队稳定过程，并提出前瞻性研究议程，核心挑战是确保动态生成未来中的持续对齐。


<details>
  <summary>Details</summary>
Motivation: 人工智能正在经历结构性转变，智能体系统具有开放端行动轨迹、生成性表征和输出、以及演进性目标等特点。这些特性给人类-AI团队合作引入了结构性不确定性，包括行为轨迹、认知基础和逻辑稳定性等方面的不确定性。在此条件下，对齐不能仅通过有限输出达成一致来保证，而必须在计划展开和优先级变化时持续维持。

Method: 在理论层面分两个阶段展开：第一阶段扩展团队态势感知理论，在开放端智能体性下重新概念化人类和AI的感知，包括异构系统间投影一致性的意义构建；第二阶段审视在适应性自主性条件下，传统上稳定团队合作的动态过程（关系互动、认知学习、协调与控制）是否继续有效。

Result: 论文提出了团队态势感知理论作为向智能体AI转型的整合性锚点，扩展了该理论以应对开放端智能体性，并分析了传统团队稳定过程在适应性自主性条件下的适用性。通过区分连续性与张力，明确了基础洞察仍然有效的领域以及结构性不确定性带来压力的领域。

Conclusion: 论文认为HAT的核心挑战不是人类与AI能否在当下达成一致，而是随着未来被持续生成、修订、执行和管理，他们能否保持对齐。团队态势感知理论为这一转变提供了整合性锚点，但需要重新概念化以应对开放端智能体带来的结构性不确定性，并提出了前瞻性研究议程。
...
### [17] [Evaluating the Search Agent in a Parallel World](https://arxiv.org/abs/2603.04751)
...
### [18] [MOOSEnger -- a Domain-Specific AI Agent for the MOOSE Ecosystem](https://arxiv.org/abs/2603.04756)
...
### [19] [Breaking Contextual Inertia: Reinforcement Learning with Single-Turn Anchors for Stable Multi-Turn Interaction](https://arxiv.org/abs/2603.04783)
...
### [20] [Timer-S1: A Billion-Scale Time Series Foundation Model with Serial Scaling](https://arxiv.org/abs/2603.04791)
...
### [21] [EchoGuard: An Agentic Framework with Knowledge-Graph Memory for Detecting Manipulative Communication in Longitudinal Dialogue](https://arxiv.org/abs/2603.04815)
...
### [32] [EvoTool: Self-Evolving Tool-Use Policy Optimization in LLM Agents via Blame-Aware Mutation and Diversity-Aware Selection](https://arxiv.org/abs/2603.04900)
...
### [33] [Alignment Backfire: Language-Dependent Reversal of Safety Interventions Across 16 Languages in LLM Multi-Agent Systems](https://arxiv.org/abs/2603.04904)
*Hiroki Fukui*
...
### [35] [TimeWarp: Evaluating Web Agents by Revisiting the Past](https://arxiv.org/abs/2603.04949)
...
### [36] [Retrieval-Augmented Generation with Covariate Time Series](https://arxiv.org/abs/2603.04951)
...
### [37] [Rethinking Representativeness and Diversity in Dynamic Data Selection](https://arxiv.org/abs/2603.04981)
...
### [38] [S5-SHB Agent: Society 5.0 enabled Multi-model Agentic Blockchain Framework for Smart Home](https://arxiv.org/abs/2603.05027)
...
### [39] [Survive at All Costs: Exploring LLM's Risky Behaviors under Survival Pressure](https://arxiv.org/abs/2603.05028)
...
### [40] [AegisUI: Behavioral Anomaly Detection for Structured User Interface Protocols in AI Agent Systems](https://arxiv.org/abs/2603.05031)
...
### [41] [The Trilingual Triad Framework: Integrating Design, AI, and Domain Knowledge in No-code AI Smart City Course](https://arxiv.org/abs/2603.05036)
...
### [42] [Enhancing Zero-shot Commonsense Reasoning by Integrating Visual Knowledge via Machine Imagination](https://arxiv.org/abs/2603.05040)
...
### [43] [WebFactory: Automated Compression of Foundational Language Intelligence into Grounded Web Agents](https://arxiv.org/abs/2603.05044)
...
### [44] [Jagarin: A Three-Layer Architecture for Hibernating Personal Duty Agents on Mobile](https://arxiv.org/abs/2603.05069)
...
### [47] [KARL: Knowledge Agents via Reinforcement Learning](https://arxiv.org/abs/2603.05218)
...
### [48] [AI+HW 2035: Shaping the Next Decade](https://arxiv.org/abs/2603.05225)
...
### [49] [Reclaiming Lost Text Layers for Source-Free Cross-Domain Few-Shot Learning](https://arxiv.org/abs/2603.05235)
...
### [50] [GCAgent: Enhancing Group Chat Communication through Dialogue Agents System](https://arxiv.org/abs/2603.05240)
...
### [51] [X-RAY: Mapping LLM Reasoning Capability via Formalized and Calibrated Probes](https://arxiv.org/abs/2603.05290)
...
### [52] [STRUCTUREDAGENT: Planning with AND/OR Trees for Long-Horizon Web Tasks](https://arxiv.org/abs/2603.05294)
...
### [53] [WebChain: A Large-Scale Human-Annotated Dataset of Real-World Web Interaction Traces](https://arxiv.org/abs/2603.05295)
...
### [54] [UniSTOK: Uniform Inductive Spatio-Temporal Kriging](https://arxiv.org/abs/2603.05301)
...
### [55] [Building AI Coding Agents for the Terminal: Scaffolding, Harness, Context Engineering, and Lessons Learned](https://arxiv.org/abs/2603.05344)
*Nghi D. Q. Bui*
...
### [56] [Ailed: A Psyche-Driven Chess Engine with Dynamic Emotional Modulation](https://arxiv.org/abs/2603.05352)
...
### [57] [PACE: A Personalized Adaptive Curriculum Engine for 9-1-1 Call-taker Training](https://arxiv.org/abs/2603.05361)
...
### [58] [Legal interpretation and AI: from expert systems to argumentation and LLMs](https://arxiv.org/abs/2603.05392)
...
### [59] [Judge Reliability Harness: Stress Testing the Reliability of LLM Judges](https://arxiv.org/abs/2603.05399)
...
### [60] [Dissociating Direct Access from Inference in AI Introspection](https://arxiv.org/abs/2603.05414)
...
### [61] [Distributed Partial Information Puzzles: Examining Common Ground Construction Under Epistemic Asymmetry](https://arxiv.org/abs/2603.05450)
...
### [62] [Towards Provably Unbiased LLM Judges via Bias-Bounded Evaluation](https://arxiv.org/abs/2603.05485)
...
### [63] [The Spike, the Sparse and the Sink: Anatomy of Massive Activations and Attention Sinks](https://arxiv.org/abs/2603.05498)
...
TL;DR: 提出了FreST Loss——一种频率增强的时空训练目标，通过在联合时空谱域中对齐模型预测与真实值，超越传统点状目标（如MSE），能更好地捕捉图结构信号的复杂时空依赖。


<details>
  <summary>Details</summary>
Motivation: 现有的直接预测模型通常依赖点状目标（如均方误差），无法有效捕捉图结构信号中复杂的时空依赖关系。最近的频域方法（如FreDF）虽然缓解了时间自相关性，但往往忽略了空间和跨时空的相互作用。为解决这一局限，需要一种能在联合时空谱域进行监督的增强训练目标。

Method: 提出FreST Loss框架，利用联合傅里叶变换（JFT）将模型预测与真实值在统一的谱域中对齐，从而有效解耦空间和时间上的复杂依赖关系。该方法是模型无关的，可应用于多种基础预测模型。理论分析表明，该公式化方法能减少时域训练目标带来的估计偏差。

Result: 在六个真实世界数据集上进行的大量实验表明，FreST Loss能持续改进最先进的基线方法，更好地捕捉整体时空动态。

Conclusion: FreST Loss通过将监督扩展到联合时空谱域，有效解决了现有方法忽略复杂时空相互作用的问题。该方法不仅理论上有优势（减少估计偏差），在实践中也表现出普适性和有效性，为图结构信号的时空预测提供了新的训练目标范式。
...
TL;DR: Delta-Crosscoder：针对窄幅微调场景的模型差异分析新方法，通过稀疏性约束和delta损失优化，能可靠识别微调引起的隐藏特征方向变化，并在多个测试场景中超越SAE基线。


<details>
  <summary>Details</summary>
Motivation: 现有基于交叉编码器的模型差异分析方法在窄幅微调（行为变化局部化且不对称）场景下表现不佳，需要更精准地捕捉微调引起的内部表示变化。

Method: 结合BatchTopK稀疏性与delta型损失函数，优先捕捉模型间变化的方向特征，并利用匹配输入下的配对激活提供隐式对比信号。

Result: 在10种模型配置（包括错误事实合成、涌现性失调、潜意识学习、禁忌词猜测等任务）上验证，能可靠分离导致微调行为的隐藏方向，实现有效缓解。性能超过SAE基线，与非SAE方法相当。

Conclusion: 该研究证明了交叉编码器仍是模型差异分析的有力工具，通过Delta-Crosscoder的改进能更精准地识别微调引起的内部表示变化。
...
TL;DR: 该论文通过将注意力分解为「选择」与「价值传递」两个概念，证明传统模型中查询、键、值使用相同维度是不必要的。通过多种实验验证，选择维度可显著降低，从而节省大量KV缓存与参数，在可接受的质量损失下提升推理并发能力。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer注意力机制中查询、键、值使用相同维度的设计缺乏依据。作者认为选择（查询与键的点积）与价值传递本质功能不同：前者只需低维度即可区分模式，而后者需要高维度承载语义信息。验证这一假设能显著减少模型开销。

Method: 提出不对称注意力：在多个任务（位置选择、内容检索、语言建模、SVD压缩）中验证选择维度可低至O(log N)。使用SVD压缩后对查询、键进行轻量化微调，在GPT-2、LLaMA、Mistral-7B等模型中测试效果。

Result: 实验结果证明键比查询更可压缩。在WikiText-2/103上将选择维度降至原模型1/4时，困惑度仅增加4.3%但QK参数减少75%。对Mistral-7B的SVD压缩与QK微调实现75%键缓存节省，质量损失仅2%。对128K上下文7B模型，每用户可节省25GB KV缓存，支持60%更多并发用户。

Conclusion: 注意力中对称维度设计并非必需。选择操作可用远低于价值传递的维度实现，且通过后处理压缩与轻量微调即可在现有模型中实现显著的缓存节省与并发提升。
...
TL;DR: 本文提出了名为Flowers的神经架构，使用多头扭曲（warping）代替传统算子（如傅里叶乘子、注意力机制、卷积混合）来学习偏微分方程（PDE）的解算符。该架构通过稀疏采样实现全局相互作用，具有线性复杂度，在多个PDE基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统用于学习PDE解算符的神经架构（如基于傅里叶乘子、注意力机制、卷积的方法）通常存在计算复杂度高或模型表达能力受限的问题。Flowers的动机在于设计一种既高效又具有理论依据的架构，特别是在处理流场、波动等物理问题时能够实现自适应的全局相互作用。

Method: Flowers完全基于多头扭曲构建，无需傅里叶乘子、点积注意力或卷积混合。每个头预测一个位移场，并扭曲输入特征。为实现计算效率和物理合理性，位移是逐点预测的（无空间聚合），非局部性仅通过稀疏采样（每个头一个源坐标）引入。通过在多尺度残差块中堆叠扭曲，该架构实现了自适应的全局相互作用，且成本为线性。

Result: Flowers在广泛的2D和3D时变PDE基准测试（尤其是流场和波动）中表现优异。一个1700万参数的紧凑模型在类似规模的傅里叶、卷积和基于注意力的基线方法中持续优于它们；而一个1.5亿参数的变体超越了许多参数更多、数据更多、训练计算量更大的基于Transformer的基础模型。

Conclusion: Flowers作为一种新颖的神经架构，通过多头扭曲机制有效模拟PDE解算器，在计算效率（线性成本）和性能（优于多种基线）之间取得了平衡，并具有清晰的理论动机（如守恒律的流映射、非均匀介质中的波、连续极限的动力学理论）。该方法为学习PDE解算符提供了一个有前景的方向。
...
TL;DR: ZorBA：一个基于零阶优化的联邦微调框架，通过异构块激活机制减少VRAM使用和通信开销


<details>
  <summary>Details</summary>
Motivation: 联邦微调大语言模型面临两大挑战：1) 本地更新需要大量VRAM；2) 频繁模型交换导致高通信开销

Method: 1) 使用零阶优化消除客户端梯度存储；2) 异构块激活机制，服务器分配不同的Transformer块子集给客户端；3) 共享随机种子和梯度有限差分减少通信

Result: 实验显示ZorBA在VRAM使用上比三个基线方法减少高达62.41%，同时保持低通信开销

Conclusion: ZorBA框架通过结合零阶优化和异构块激活，有效解决了联邦微调中的VRAM和通信瓶颈问题
...
### [71] [On Emergences of Non-Classical Statistical Characteristics in Classical Neural Networks](https://arxiv.org/abs/2603.04451)
...
### [72] [Learning Unified Distance Metric for Heterogeneous Attribute Data Clustering](https://arxiv.org/abs/2603.04458)
*Yiqun Zhang,Mingjie Zhao,Yizhou Chen,Yang Lu,Yiu-ming Cheung*

Main category: cs.LG

TL;DR: 提出异质属性重构与表示学习框架，在统一学习空间中表示混合数据属性，实现更有效的聚类分析。


<details>
  <summary>Details</summary>
Motivation: 现实聚类任务中常见包含数值和分类属性的混合数据，现有方法通常将异质属性编码为单一类型或定义统一度量，未能揭示其内在关联，需要同时有效利用这两种不同类型信息。

Method: 提出异质属性重构与表示学习范式，将异质属性转化为同质状态进行距离度量学习，并将每个属性的值投影到统一的可学习多空间中以更精细地表示和学习分类数据的距离度量。

Result: 大量实验证明HARR在准确性和效率方面的优越性，相比现有方法更具优势。

Conclusion: 论文提出了HARR框架，通过在统一学习空间中表示异质属性并重构其原始表示，实现更有效的混合数据聚类，具有无参数、收敛保证和自适应聚类数等优势。
...
### [73] [VSPrefill: Vertical-Slash Sparse Attention with Lightweight Indexing for Long-Context Prefilling](https://arxiv.org/abs/2603.04460)
*Chen Guanzhong*
...
### [74] [MAD-SmaAt-GNet: A Multimodal Advection-Guided Neural Network for Precipitation Nowcasting](https://arxiv.org/abs/2603.04461)
...
### [75] [Understanding the Dynamics of Demonstration Conflict in In-Context Learning](https://arxiv.org/abs/2603.04464)
...
### [88] [Neuro-Symbolic Financial Reasoning via Deterministic Fact Ledgers and Adversarial Low-Latency Hallucination Detector](https://arxiv.org/abs/2603.04663)
*Pedram Agand*
...
### [89] [Direct Estimation of Tree Volume and Aboveground Biomass Using Deep Regression with Synthetic Lidar Data](https://arxiv.org/abs/2603.04683)
...
### [91] [Implicit Bias and Loss of Plasticity in Matrix Completion: Depth Promotes Low-Rankness](https://arxiv.org/abs/2603.04703)
...
### [92] [Probabilistic Dreaming for World Models](https://arxiv.org/abs/2603.04715)
*Gavin Wong*
...
### [93] [Count Bridges enable Modeling and Deconvolving Transcriptomic Data](https://arxiv.org/abs/2603.04730)
...
### [94] [When Priors Backfire: On the Vulnerability of Unlearnable Examples to Pretraining](https://arxiv.org/abs/2603.04731)
...
### [95] [Distribution-Conditioned Transport](https://arxiv.org/abs/2603.04736)
...
### [96] [KindSleep: Knowledge-Informed Diagnosis of Obstructive Sleep Apnea from Oximetry](https://arxiv.org/abs/2603.04755)
...
### [97] [ConTSG-Bench: A Unified Benchmark for Conditional Time Series Generation](https://arxiv.org/abs/2603.04767)
...
### [98] [Distributional Reinforcement Learning with Information Bottleneck for Uncertainty-Aware DRAM Equalization](https://arxiv.org/abs/2603.04768)
...
### [99] [Distributional Equivalence in Linear Non-Gaussian Latent-Variable Cyclic Causal Models: Characterization and Learning](https://arxiv.org/abs/2603.04780)
...
### [100] [Diffusion Policy through Conditional Proximal Policy Optimization](https://arxiv.org/abs/2603.04790)
...
### [101] [Multilevel Training for Kolmogorov Arnold Networks](https://arxiv.org/abs/2603.04827)
...
### [102] [Missingness Bias Calibration in Feature Attribution Explanations](https://arxiv.org/abs/2603.04831)
...
### [103] [Why Is RLHF Alignment Shallow? A Gradient Analysis](https://arxiv.org/abs/2603.04851)
*Robin Young*
...
### [104] [Differential Privacy in Two-Layer Networks: How DP-SGD Harms Fairness and Robustness](https://arxiv.org/abs/2603.04881)
...
### [105] [FedAFD: Multimodal Federated Learning via Adversarial Fusion and Distillation](https://arxiv.org/abs/2603.04890)
...
### [106] [U-Parking: Distributed UWB-Assisted Autonomous Parking System with Robust Localization and Intelligent Planning](https://arxiv.org/abs/2603.04898)
...
### [110] [Uncertainty-aware Blood Glucose Prediction from Continuous Glucose Monitoring Data](https://arxiv.org/abs/2603.04955)
*Hai Siong Tan*
...
### [111] [WaterSIC: information-theoretically (near) optimal linear layer quantization](https://arxiv.org/abs/2603.04956)
...
### [112] [Mixture of Universal Experts: Scaling Virtual Width via Depth-Width Transformation](https://arxiv.org/abs/2603.04971)
...
### [113] [Functionality-Oriented LLM Merging on the Fisher--Rao Manifold](https://arxiv.org/abs/2603.04972)
...
### [114] [Lightweight and Scalable Transfer Learning Framework for Load Disaggregation](https://arxiv.org/abs/2603.04998)
...
### [115] [Competitive Multi-Operator Reinforcement Learning for Joint Pricing and Fleet Rebalancing in AMoD Systems](https://arxiv.org/abs/2603.05000)
...
### [117] [Poisoning the Inner Prediction Logic of Graph Neural Networks for Clean-Label Backdoor Attacks](https://arxiv.org/abs/2603.05004)
...
### [118] [MCEL: Margin-Based Cross-Entropy Loss for Error-Tolerant Quantized Neural Networks](https://arxiv.org/abs/2603.05048)
...
### [125] [Decoupling Task and Behavior: A Two-Stage Reward Curriculum in Reinforcement Learning for Robotics](https://arxiv.org/abs/2603.05113)
...
### [126] [FedBCD:Communication-Efficient Accelerated Block Coordinate Gradient Descent for Federated Learning](https://arxiv.org/abs/2603.05116)
...
### [127] [Federated Causal Discovery Across Heterogeneous Datasets under Latent Confounding](https://arxiv.org/abs/2603.05149)
...
### [128] [Balancing Privacy-Quality-Efficiency in Federated Learning through Round-Based Interleaving of Protection Techniques](https://arxiv.org/abs/2603.05158)
...
### [129] [Trainable Bitwise Soft Quantization for Input Feature Compression](https://arxiv.org/abs/2603.05172)
...
### [130] [Incentive Aware AI Regulations: A Credal Characterisation](https://arxiv.org/abs/2603.05175)
...
### [131] [Towards a data-scale independent regulariser for robust sparse identification of non-linear dynamics](https://arxiv.org/abs/2603.05201)
...
### [132] [Stable-LoRA: Stabilizing Feature Learning of Low-Rank Adaptation](https://arxiv.org/abs/2603.05204)
...
### [133] [Early Warning of Intraoperative Adverse Events via Transformer-Driven Multi-Label Learning](https://arxiv.org/abs/2603.05212)
...
### [134] [The Geometric Inductive Bias of Grokking: Bypassing Phase Transitions via Architectural Topology](https://arxiv.org/abs/2603.05228)
*Alper Yıldırım*
...
### [135] [SlideSparse: Fast and Flexible (2N-2):2N Structured Sparsity](https://arxiv.org/abs/2603.05232)
...
### [136] [Recursive Inference Machines for Neural Reasoning](https://arxiv.org/abs/2603.05234)
...
### [137] [A Behaviour-Aware Federated Forecasting Framework for Distributed Stand-Alone Wind Turbines](https://arxiv.org/abs/2603.05263)
...
### [138] [Beyond Word Error Rate: Auditing the Diversity Tax in Speech Recognition through Dataset Cartography](https://arxiv.org/abs/2603.05267)
...
### [139] [Whispering to a Blackbox: Bootstrapping Frozen OCR with Visual Prompts](https://arxiv.org/abs/2603.05276)
...
### [140] [Knowledge Divergence and the Value of Debate for Scalable Oversight](https://arxiv.org/abs/2603.05293)
*Robin Young*
...
### [141] [WavSLM: Single-Stream Speech Language Modeling via WavLM Distillation](https://arxiv.org/abs/2603.05299)
...
### [142] [GALACTIC: Global and Local Agnostic Counterfactuals for Time-series Clustering](https://arxiv.org/abs/2603.05318)
...
### [143] [FairFinGAN: Fairness-aware Synthetic Financial Data Generation](https://arxiv.org/abs/2603.05327)
...
### [144] [Preserving Continuous Symmetry in Discrete Spaces: Geometric-Aware Quantization for SO(3)-Equivariant GNNs](https://arxiv.org/abs/2603.05343)
...
### [145] [InfoFlow KV: Information-Flow-Aware KV Recomputation for Long Context](https://arxiv.org/abs/2603.05353)
...
### [146] [Learning Causal Structure of Time Series using Best Order Score Search](https://arxiv.org/abs/2603.05370)
...
### [147] [Embedded Inter-Subject Variability in Adversarial Learning for Inertial Sensor-Based Human Activity Recognition](https://arxiv.org/abs/2603.05371)
...
### [148] [Robust Node Affinities via Jaccard-Biased Random Walks and Rank Aggregation](https://arxiv.org/abs/2603.05375)
...
### [149] [On the Necessity of Learnable Sheaf Laplacians](https://arxiv.org/abs/2603.05395)
...
### [150] [An interpretable prototype parts-based neural network for medical tabular data](https://arxiv.org/abs/2603.05423)
...
### [151] [On-Policy Self-Distillation for Reasoning Compression](https://arxiv.org/abs/2603.05433)
...
### [152] [Latent Wasserstein Adversarial Imitation Learning](https://arxiv.org/abs/2603.05440)
