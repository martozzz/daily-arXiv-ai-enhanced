...
TL;DR: 该论文探讨了AI可解释性中"好解释"的定义，提出需要考虑对话者先验信念的反事实解释框架，并分析了为什么LLM输出难以产生好解释。


<details>
  <summary>Details</summary>
Motivation: AI可解释性对于AI在许多场景中的采用至关重要，但要产生对AI系统的良好解释，首先需要理解"好解释"是什么。长期以来关于"好解释"的定义存在哲学争论，最近在AI输出背景下重新引起了关注。

Method: 提出一个基于反事实解释概念的定义框架，强调在提供解释时必须考虑对话者对每个可能被提供事实的先验信念。通过这一框架探索对AI可解释性的影响。

Result: 该定义揭示了为什么大型语言模型（LLM）输出难以产生良好解释的原因，因为LLM输出通常缺乏对特定用户先验信念的考虑，难以提供个性化的、有意义的解释。

Conclusion: 要改进AI系统的可解释性，特别是LLM的解释能力，需要开发能够考虑用户先验信念的解释框架，这比单纯依赖反事实解释更为复杂但必要。
...
TL;DR: DR-DCI 是一个检索引导的直接语料库交互框架，通过动态创建局部工作空间来平衡检索的扩展性和DCI操作的精确性，在大型语料库上实现高效且有效的代理搜索。


<details>
  <summary>Details</summary>
Motivation: 传统的检索接口（如BM25或ColBERT）虽然能有效排名相关文档，但仅提供排名结果或有限文档视图，限制了代理重组材料和跨文档验证约束的能力。直接语料库交互（DCI）通过暴露可执行的语料库操作来解决这一限制，但随着语料库规模增长，全语料库终端命令变得缓慢且不稳定，影响性能和效率。

Method: 提出DR-DCI框架，将检索作为代理可调用的动作来扩展局部工作空间。代理不是直接在整个语料库上操作，而是动态地将相关文档拉入一个不断演变的工作空间，并在该空间内进行DCI操作。这种方法结合了检索级别的召回率和DCI风格的精确度：检索保持探索的可扩展性，而DCI保留有效证据解析所需的局部操作。

Result: 在Browsecomp-Plus上，DR-DCI达到71.2%的准确率，比原始DCI和消融变体提高了最多8.3个百分点，同时减少了工具使用、执行时间和估计成本。使用工作空间保留上下文重置后，准确率进一步提高到73.3%。在语料库规模扩展实验中，DR-DCI在10万到1000万文档范围内保持有效，而原始DCI变得不稳定，BM25表现更差。DR-DCI还能扩展到2000万规模的Wiki-18 QA设置，在六个基准测试中平均得分为63.0，优于基于检索和训练搜索代理的基线。消融分析表明排名预览和跨文档DCI是性能的关键。

Conclusion: DR-DCI框架有效解决了大型语料库中代理搜索的可扩展性和精确性问题，通过结合检索的探索能力和DCI的精确操作，实现了在多种规模语料库上的高效表现，为代理驱动的信息检索系统提供了一种实用解决方案。
...
TL;DR: 提出关系结构因果模型，扩展传统因果模型以处理变化的对象和关系，并开发了关系神经因果模型来学习可泛化到未见对象组合的因果模型。


<details>
  <summary>Details</summary>
Motivation: 人工智能需要具备对其环境的因果模型，以支持对干预和反事实的推理，并且需要组合性以支持泛化到未见过的对象组合。然而，当前研究尚未正式探讨何时以及如何学习这样的模型。

Method: 1. 提出关系结构因果模型，将结构因果模型扩展到对象及其关系变化的情境中。2. 定义了关系因果图，并推导了符号识别标准，即使在存在未观测混杂的情况下也能实现识别。3. 提出了关系神经因果模型，这是一种可证明正确的方法。

Result: 1. 证明了在没有额外假设的情况下，关于未见对象组合的观测查询和因果查询都无法被识别。2. 提出的关系神经因果模型在模拟交通场景（包含变化的汽车、信号灯和行人）中优于非关系基线方法。

Conclusion: 该研究为学习具有组合性和因果性的环境模型提供了正式的理论框架和实用的机器学习方法，使得人工智能能够更好地泛化到未见过的对象配置，并支持因果推理。
...
TL;DR: 提出基于代价验证的行为信任度量方法，研究AI代理在团队合作中的信任形成、破坏与恢复机制。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型代理在团队中协作日益增多，需要标准方法来测量AI代理之间的信任关系，但目前缺乏此类标准度量方式。

Method: 提出基于代价验证的行为信任度量方法，在合作生存游戏中通过验证成本来衡量信任。通过减少验证频率作为信任的观测指标，研究了六个前沿模型快照的信任动态。

Result: 四个主要模型快照在可靠队友情况下验证减少60-85%，而两个较小模型无此调整。失败会逆转信任折扣，模型响应方式不同：有的针对性审查，有的整体谨慎。恢复比形成慢，集群失败比分散失败更持久影响信任。

Conclusion: 信任形成可提高决策效率和收益，而过度验证导致犹豫而非安全性。信任倾向可在部署前测量，多智能体AI系统的治理核心应是校准而非最大怀疑。
...
TL;DR: PrologMCP：一个通过Model Context Protocol将Prolog作为状态化工具暴露的开源服务器，使语言模型能够将逻辑推理任务委托给专业求解器，显著提升在深度演绎任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前前沿推理调优的语言模型在深度演绎任务上仍然失败，且通过扩展内部推理来改进性能的成本很高。符号委托提供了互补途径：让语言模型翻译问题，求解器执行推理。然而，现有的逻辑编程自动形式化管道通常是针对特定任务或代理的定制集成。

Method: 引入PrologMCP，这是一个任务无关、开源的服务器，通过Model Context Protocol（MCP）将Prolog作为状态化工具暴露。它具有紧凑的工具接口、结构化错误报告和每会话隔离功能，使翻译-运行-检查-修复循环成为MCP兼容代理的可重用原语。

Result: 在PARARULE-Plus的两个子集上评估：通用样本和更具挑战性的针对自然语言推理特定失败模式的样本。在通用样本上，形式化代理匹配或超过了推理LLMs（准确率1.00 vs. 1.00/0.998），相比标准模型有最大提升（GPT-4.1为0.762）。在挑战性子集上，形式化代理保持接近完美（1.00/0.99），而推理LLMs下降到0.95/0.94。

Conclusion: 通过MCP将推理委托给Prolog是扩展自然语言推理的一个健壮且可检查的替代方案。PrologMCP提供了一个可重用的原语，用于语言模型与符号推理系统的协作，在深度演绎任务上表现出色。
...
TL;DR: SERAF是一个结合时间序列相似性和语义描述的检索增强时间序列预测框架，通过双重检索机制提升预测准确性。


<details>
  <summary>Details</summary>
Motivation: 传统基于时间序列相似性的检索方法在非平稳条件下效果有限，需要更全面的检索机制来利用历史模式。

Method: 提出双模态检索方法：同时基于时间序列相似性和自生成的文本描述进行检索，获得互补的历史模式和未来信息，选择性融合指导预测。

Result: 在七个真实世界数据集上的实验表明，SERAF在连接时间序列的数值和语义视图方面优于现有先进基线方法。

Conclusion: SERAF通过结合时间序列和语义信息的双重检索机制，有效提升了时间序列预测的准确性和鲁棒性。
...
TL;DR: 该研究提出了一个几何框架来识别深度神经网络中的"AI记忆痕迹"，通过形式化神经科学标准来定位和操作特定的记忆单元，无需迭代优化即可进行记忆组合或擦除。


<details>
  <summary>Details</summary>
Motivation: 记忆形成是智能的基础，但深度神经网络是否像生物记忆一样保存可识别的记忆痕迹仍然是一个未解决的问题。该研究旨在探索神经网络中是否存在类似生物记忆单元的可识别记忆痕迹。

Method: 引入几何框架，将神经科学标准（特异性、再激活、充分性、必要性）形式化为约束逆问题。推导出闭式估计器从全局纠缠的参数中隔离个体记忆痕迹，并证明该生物学启发的解对应于参数流形上的自然梯度更新。

Result: AI记忆痕迹能够对学习到的知识进行外科手术式操作：任何记忆子集都可以通过线性算术进行组合或擦除，无需迭代优化。从简单MLP到LLM的实验证明了AI记忆痕迹的因果有效性和显著可扩展性。

Conclusion: 这些结果桥接了生物记忆理论和人工表示学习，并为深度网络如何在分布式存储中同时支持功能特异性提供了几何见解。AI记忆痕迹框架为理解和操作神经网络中的记忆提供了新工具。
...
TL;DR: LLM法官的可靠性评估需要昂贵的人工标注，Metric Match方法通过智能选择标注样本来显著降低标注成本，提高可靠性估计的准确性。


<details>
  <summary>Details</summary>
Motivation: LLM法官用于评估开放文本生成，但其可靠性评估依赖于与人类评分者的一致性，而获取这种一致性需要昂贵的人工标注成本。需要一种方法能够在有限标注下准确估计LLM法官的可靠性。

Method: 提出Metric Match方法：基于获取的合成标签，选择能够匹配总体可靠性指标的标注样本子集。该方法还提供了一个成本模型，并可将任务从可靠性估计转向可靠性分类（判断法官是否达到部署阈值）。

Result: 在4个相关性指标和15个数据集上，Metric Match相比随机选择取得了0.838的胜率，平均估计误差降低了18.7%，标注需求减少了32.5%。医疗案例研究显示，与随机选择相比，该方法可为专家标注节省1041.67美元。

Conclusion: Metric Match方法能够在有限的人工标注下有效估计LLM法官的可靠性指标，显著降低标注成本并提高评估准确性，为LLM法官的实际部署提供了实用工具。
...
TL;DR: OSGuard 是一个用于评估计算机使用代理安全性的双粒度基准套件，包含动作级基准用于局部护栏决策和风险增强执行套件用于端到端评估，旨在识别代理通过不安全捷径实现名义任务目标的问题。


<details>
  <summary>Details</summary>
Motivation: 当前计算机使用代理的评估主要关注任务完成度，但仅凭任务成功无法检测代理通过不安全捷径达成目标的问题。需要更全面的安全评估方法来识别这些潜在风险。

Method: 开发了OSGuard双粒度基准套件：1）动作级基准包含基于原始指令和当前界面状态标记为允许、不相关或不安全的情境化提议动作；2）风险增强执行套件包含OSWorld派生的任务变体，环境被修改引入潜在危险（如破坏性覆盖），每个变体都配有增强评估器，在保留原始任务成功标准的同时添加明确的状态安全不变量。

Result: 实验结果表明，当前多模态护栏在孤立动作判断上表现良好，但风险增强执行暴露了局部监督与可靠端到端安全之间的差距。双粒度设计能够更精确地诊断模型是否能识别不安全提议动作并在部署为护栏时提高全任务安全性。

Conclusion: OSGuard通过双粒度评估方法揭示了计算机使用代理安全评估的重要维度，表明仅靠任务成功指标不足以确保安全性，需要结合局部动作判断和端到端风险检测来全面评估代理的安全性能。
...
TL;DR: 该研究提出了一个基于基础模型的跨模态表示对齐框架，用于从CT影像和纵向电子病历数据中进行时间-事件预测，通过四种融合策略提升预测性能，并在多个临床任务和机构中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 由于模态不平衡和分布偏移，从多模态临床数据中进行准确的时间-事件预测仍然具有挑战性。需要一种能够跨任务和机构泛化的跨模态表示对齐方法。

Method: 使用领域特定的基础模型独立编码CT和电子病历模态，然后通过四种融合策略（晚期融合、对比对齐、交叉注意力和协同注意力）在共享潜在空间中对齐。在两个临床不同的时间-事件预测任务（肺栓塞死亡率和心血管疾病结局）上进行评估。

Result: 融合方法相比单模态基线在C指数上提升了1.5-5.4%。对比多模态融合（特别是与CLMBR表示结合）在肺栓塞死亡率预测中表现最一致且稳健。对于心血管疾病结局，交叉注意力（one-hot）在内部性能最高，图像引导的协同注意力在外部性能最好。

Conclusion: 研究提出了一个可泛化的基于基础模型的跨模态对齐框架，首次系统分析了模态不平衡下融合行为在时间-事件预测中的表现。结果表明任务感知的多模态对齐是实现稳健泛化和可扩展临床部署的必要设计原则。
...
TL;DR: 基于LLM的遥感数据检索框架，通过自然语言查询访问云地理空间目录，使用多智能体架构确保安全可靠的数据访问


<details>
  <summary>Details</summary>
Motivation: 解决用户通过自然语言查询高效访问云地理空间目录中遥感数据和环境数据集的挑战，建立用户意图与地理空间基础设施之间的可扩展接口

Method: 提出一个LLM驱动的框架，包含三个协调的智能体：Guardrail（安全策略执行）、General-QA（意图解释）、Recommender-Analyst（模式感知API调用生成），通过API模式替换实现跨平台可移植性

Result: 初步实验表明，在对抗性多轮设置下，提示级安全指令提高了鲁棒性，但在API操作场景中仍存在罕见的高影响失败，突显了需要平衡安全性、可用性和成本效益的自适应系统级防御

Conclusion: 该框架为环境监测、灾害响应和气候分析等应用建立了可扩展的接口，实现了用户意图与地理空间基础设施之间的流线化和自动化地球观测工作流程，通过拦截级Guardrail智能体平衡安全与可用性
...
TL;DR: 该论文提出了"认知债务"的正式理论，指当人们用AI替代而非补充基本原理认知时，未经验证的推理义务会不断累积，形成类似金融债务的系统性风险。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解AI作为认知替代品而非补充品时产生的系统性风险。作者观察到人们过度依赖AI解决认知任务，而不是将其作为增强自身认知能力的工具，这种模式可能导致认知能力退化并积累未经验证的推理义务。

Method: 采用形式化理论模型方法，为每个代理人设定两个状态变量：认知资本和认知债务。模型包含乘性生产技术，其中认知资本作为抵押品决定AI采用的回报。通过建立六个命题来推导理论结果。

Result: 理性代理人会积累正认知债务，因为成本被推迟、部分外部化并被短期生产力增益掩盖。平静期会降低主观风险评估，增加AI替代强度，形成杠杆效应，导致"认知明斯基时刻"：主观风险下降而真实系统性脆弱性上升。危机预期损失在总杠杆上是凸的。危机后，产出目标压力可能导致"虚假修正循环"：代理人用更多AI修补AI失败。分散均衡相对于社会最优会过度采用替代性AI，原因包括系统性风险、认知公共品和军备竞赛外部性。在两类异质代理人经济中，高认知资本代理人更密集采用AI，最终可能使其无辅助认知资本低于初始低技能代理人。

Conclusion: 该研究建立了认知债务的形式化理论，揭示了AI作为认知替代品而非补充品时的系统性风险。研究发现分散均衡会导致过度AI替代，产生认知债务积累、脆弱性增加和认知能力退化等问题。政策启示是需要将AI设计为认知补充而非完全替代，并考虑认知公共品和系统性风险的外部性。
...
TL;DR: 研究人员使用可验证的AI辅助推理系统VGPT-RSI在黎曼猜想相关问题上生成形式化验证的进展，重点关注参数化不等式边界证明和拉加里亚准则的形式化验证，明确识别未解决的数学瓶颈。


<details>
  <summary>Details</summary>
Motivation: 黎曼猜想是数学核心未解问题，研究旨在验证AI辅助推理系统能否产生可靠、形式化验证的进展，同时明确识别剩余的数学障碍，而非宣称完整证明。

Method: 应用VGPT-RSI系统执行两项RH相关验证任务：1)构造并验证参数化安全下曲线上的不等式有限边界证书，使用向外舍入区间算术和Arb/FLINT球算术审计，在Rocq/CoqInterval中检查参数化定理；2)启动形式化拉加里亚路径证书，形式化有限量并生成Coq验证的有限证书。

Result: 成功生成参数化不等式边界的形式化证书和拉加里亚准则有限量的Coq验证证书，系统明确识别出未解决数学瓶颈：拉加里亚等价性的形式化、超越任何有限截断的全局尾定理证明、以及将反例约化到高度丰富整数等。

Conclusion: VGPT-RSI能够生成经过认证的RH相关形式化进展，组织证明依赖关系，在剩余障碍纯粹是数学性质时避免过度宣称，为AI辅助数学推理提供可行性验证。
...
### [26] [S1-DeepResearch: Beyond Search, Toward Real-World Long-Horizon Research Agents](https://arxiv.org/abs/2606.15367)
...
### [32] [Toward Vibe Medicine: A Self-Evolving Multi-Agent Framework for Clinical Decision Support](https://arxiv.org/abs/2606.15504)
...
### [33] [Frame-Conditioned Moral Computation in LLaMA 3.1-8B-Instruct: A Mechanistic Interpretability Audit of Ethical Reasoning](https://arxiv.org/abs/2606.15507)
...
### [34] [ToolMenuBench: Benchmarking Tool-Menu Filtering Strategies for Reliable and Efficient LLM Agents](https://arxiv.org/abs/2606.15508)
...
### [35] [Minimal Oversight: Uncertainty-Aware Governance for Delegated AI Systems](https://arxiv.org/abs/2606.15563)
*Carlos R. B. Azevedo*
...
### [36] [QoS-Aware Token Scheduling and Private Data Valuation for Multi-Modal Agentic Networks](https://arxiv.org/abs/2606.15573)
...
### [37] [Do we have the knowledge we need? Rethinking human-AI decision-making in corporations](https://arxiv.org/abs/2606.15575)
...
### [38] [Large Language Models as Optimizers: A Survey of Direct vs. Tool-Augmented Approaches and Their Performance Frontiers](https://arxiv.org/abs/2606.15577)
...
### [39] [Your Agent Has a Genome: Sequence-Level Behavioral Analysis and Runtime Governance of LLM-Powered Autonomous Agents](https://arxiv.org/abs/2606.15579)
*Sidi Deng*
...
### [40] [Agentic Retrieval and Reinforcement Learned Equation Chains: A Controlled Generation Framework for Complex and Novel Physics Word Problems](https://arxiv.org/abs/2606.15591)
*Tirthankar Mittra*
...
### [41] [Integrating Reasoning and Generalization in Text-to-SQL via Self-Enhanced Fine-Tuning](https://arxiv.org/abs/2606.15598)
...
### [42] [NeuroSymbolic AI for Legal AI-TRISM: Trustworthy, Reliable, Interpretable, Safe Models](https://arxiv.org/abs/2606.15646)
...
### [43] [Towards Next-Generation Healthcare: A Survey of Medical Embodied AI for Perception, Decision-Making, and Action](https://arxiv.org/abs/2606.15647)
...
### [44] [Advanced Machine Learning and Deep Learning Techniques for Enhanced Cattle Identification and Detection: A Comprehensive Review](https://arxiv.org/abs/2606.15655)
...
### [45] [Overcoming the Impedance Mismatch: A Theoretical Roadmap for Fusing Foundation Models and Knowledge Graphs](https://arxiv.org/abs/2606.15656)
...
### [46] [Where Did It Go Wrong? Process-Level Evaluation of Web Agents with Semantic State Tracking](https://arxiv.org/abs/2606.15673)
...
### [47] [Multi-agent Framework for Time-Sensitive Complementary Collaboration in Minecraft](https://arxiv.org/abs/2606.15684)
...
### [48] [Recurrent Reasoning on Symbolic Puzzles with Sequence Models](https://arxiv.org/abs/2606.15686)
...
### [49] [Do LLMs Reliably Identify Correct Information Units in Aphasic Discourse?](https://arxiv.org/abs/2606.15696)
...
### [50] [Artificial Intelligence Index Report 2026](https://arxiv.org/abs/2606.15708)
...
TL;DR: QPILOTS：一种无需修改原始策略，通过推断时引导去噪过程来提升强化学习性能的方法。该方法在去噪步骤中计算批评器梯度，避免直接评估噪声中间动作，提高了数值稳定性。


<details>
  <summary>Details</summary>
Motivation: 流动匹配和扩散策略是强大的动作生成器，但用时序差分强化学习优化它们很困难。直接通过多步去噪过程反向传播批评器梯度会导致数值不稳定。现有方法要么丢弃梯度信息，要么将策略蒸馏为简单的一步演员，或者反复微调去噪策略。

Method: 提出QPILOTS方法：保持原始策略不变，在推断时引导去噪过程。在每个去噪步骤中，首先将噪声中间状态投影到最终干净动作的估计，然后在那里计算批评器梯度。包含两个变体：QPILOTS-U使用快速单点近似，QPILOTS-M通过学习的辅助网络绘制可微分后验样本。

Result: 在标准离线到在线强化学习基准测试中，QPILOTS取得了最佳综合性能，在50个任务中达到90%的平均成功率。应用于冻结的预训练视觉语言动作基础模型时，在模拟的六个操作任务中优于或匹配先前推断时方法。

Conclusion: QPILOTS为流动匹配和扩散策略提供了稳定有效的强化学习优化方法，无需修改或重新训练原始策略。该方法在推断时通过计算清洁动作的梯度引导去噪过程，解决了数值不稳定性问题，在多个基准测试中表现优异。
...
TL;DR: GRAPE是一种通过渐进式参数空间演化来提升对抗鲁棒性的训练框架，在保持计算预算相近的情况下，相比固定结构对抗训练能获得更高的鲁棒精度和更少的参数量。


<details>
  <summary>Details</summary>
Motivation: 传统对抗训练方法通常从一开始就在固定参数空间上进行优化，但参数变得可优化的顺序是否会影响最终的鲁棒解尚未被充分探索。本文旨在研究参数空间的渐进式演化是否能影响最终鲁棒性能，即使最终架构或计算预算是相同的。

Method: 提出了GRAPE（Guided Parameter-Space Evolution）框架，结合参数空间稳定化和渐进式隐藏扩张：首先稳定当前暴露空间的鲁棒优化，然后逐渐释放新的可优化维度，并使用对抗性谱利用分数来引导新释放的容量流向压力较大的模块。该方法将鲁棒模型学习视为参数空间渐进式暴露和演化的过程。

Result: 在CIFAR-10数据集上使用ℓ∞威胁模型，与固定结构ResNet-18对抗训练相比，GRAPE在计算预算相近（FLOPs比率为1.009x）的情况下，将PGD-20鲁棒精度从51.70%提升到56.94%，同时减少了约21.4%的参数量。即使是最终架构相同的顺序增长变体也达到了56.52%的鲁棒精度，表明增益不仅来自最终架构差异，也源于参数空间暴露路径。

Conclusion: 研究表明，引导参数空间演化能够在匹配计算预算的情况下产生紧凑且鲁棒的参数配置，为对抗鲁棒训练提供了新的视角，即参数空间暴露路径对最终鲁棒性能有重要影响。
...
TL;DR: 提出了一个α-公平个人偿付能力保费（α-FISP）框架，通过一个参数α在精算公平性与团结公平性之间建立连续权衡，同时保证保险公司的偿付能力。


<details>
  <summary>Details</summary>
Motivation: 保险定价中存在精算公平性（基于个体风险定价）与团结公平性（风险池化和交叉补贴）之间的根本性冲突。现代数据环境使得风险区分更加精细化，而监管要求又需要保护弱势群体，这种公平性困境需要新的解决框架。

Method: 将定价问题构建为约束优化任务：在精算公平保费基础上进行调整，同时限制每个风险类别内部的交叉补贴预算。通过参数α构建一系列解，形成从纯精算定价到纯团结定价的连续谱。

Result: 提出的α-FISP框架具有理论保证，计算上易于处理，并且与具有异质性州级公平要求的美国监管制度相吻合。数值实验验证了该框架的可行性和实用性。

Conclusion: α-FISP框架为决策者提供了一个系统性工具，可以在精算公平性与团结公平性之间进行明确的权衡选择，同时确保保险公司的偿付能力，为解决保险定价的公平性难题提供了有效途径。
...
TL;DR: GRASP通过顺序处理和梯度对齐解决了多源迁移学习的内存瓶颈问题，实现了O(1)内存消耗和更好的知识整合


<details>
  <summary>Details</summary>
Motivation: 传统多源迁移学习方法面临两个主要问题：1) 在参数融合时需要同时加载K个源模型，占用O(K)内存；2) 推理时需要部署所有模型，在生产部署中不可行。这构成了一个根本的可扩展性瓶颈。

Method: 提出了GRASP方法，包含三个关键创新：1) 顺序处理——一次合并一个源模型到演进的目标模型中；2) 参数级梯度对齐——仅选择优化方向与目标域对齐的参数进行转移，避免负迁移；3) 迭代微调——在整合下一个源之前对已转移知识进行适应。

Result: 在三个持续学习基准测试（Yearbook, CLEAR-10, CLEAR-100）和四种架构（1.3M到25.6M参数）上的实验表明，GRASP在所有数据集和架构上平均准确率达到93.5%，而集成方法仅为71.7%。GRASP仅需恒定内存，而标准多源融合需要K个模型的内存。

Conclusion: GRASP通过顺序处理和梯度对齐有效解决了多源迁移学习的内存瓶颈问题，能够扩展到任意多个源而不增加内存消耗，特别适合资源受限的部署和持续演化的源域场景。
...
TL;DR: 该论文提出了一种用于推荐系统中嵌入模型路由的新框架，将其形式化为对抗性上下文线性老虎机问题，并开发了Hypentropy Policy Gradient算法来高效解决该问题。


<details>
  <summary>Details</summary>
Motivation: 现代推荐系统需要动态地将多样化查询路由到多个嵌入模型，但现有方法在对抗性查询、老虎机反馈和模型有限可观测性等现实条件下表现不佳，缺乏理论保证。

Method: 将嵌入模型路由形式化为具有低秩专家的对抗性上下文线性老虎机问题，提出对数二次策略类来捕获查询依赖的模型路由，并开发Hypentropy Policy Gradient算法进行高效在线学习。

Result: HPG算法能够适应未知的低秩结构，在信息不完全的情况下获得$\tilde{\mathcal O}(s\sqrt{M T})$的线性化策略遗憾，避免了维度灾难，并提供了计算高效且无参数的实现。

Conclusion: 该研究为嵌入模型路由提供了首个理论框架和高效算法，在对抗性环境和有限信息下实现了可证明的性能保证，对实际推荐系统具有重要价值。
...
TL;DR: SNA是一种结合神经逼近和Tensor分解的新型架构，能够解耦局部坐标函数和全局交互，特别适合求解偏微分方程。VSNA框架提供了经典变分保证，能有效缓解高维PDE的维度灾难，实现线性维度的计算成本，并在工程应用中展示出色的性能和实时计算能力。


<details>
  <summary>Details</summary>
Motivation: 传统的数值方法在处理高维时空参数化偏微分方程时面临维度灾难，计算成本随维度呈指数增长。为解决这一挑战，作者提出了可分离神经架构，旨在开发一种紧凑、平滑的归纳偏置模型，能够有效逼近高维PDE解，并支持实时计算和逆问题求解。

Method: 提出了可分离神经架构，结合神经逼近和张量分解。该架构将局部坐标函数（原子）与由稀疏低秩交互对象控制的全局交互解耦。在变分SNA框架下作为Galerkin试验空间，满足Lax-Milgram经典变分保证。采用完全因子化的张量原生交替最小二乘优化框架，将计算成本降至线性维度。

Result: VSNA在椭圆型、双曲型和抛物型系统中均得到验证，与预测的代数和谱缩放率高度一致。在两个工程案例中展示出色性能：7D参数化制造模拟和Inconel 718的实验热属性反演管道。在标准笔记本CPU上，VSNA能在102秒内执行100万次蒙特卡洛查询，相比NVIDIA A100 GPU上的全网格有限元基准实现150,000倍加速，并在100ms内实现实时生成反演重构。

Conclusion: SNA作为一种紧凑的数学基底，为连续参数流形提供了实时反演、优化循环和快速不确定性传播的能力。该架构通过代数而非指数级缩放有效缓解维度灾难，在工程应用中展现出革命性的计算效率和实时性能，为物理世界建模提供了强大的新工具。
...
TL;DR: 将无状态的autoresearch模式重构为基于LangGraph的有状态ReAct智能体，通过持久化状态减少token使用量，在超参数调优任务中减少90% token，在代码优化任务中减少52% token。


<details>
  <summary>Details</summary>
Motivation: 传统的autoresearch模式采用无状态设计，每次迭代都需要从头重建实验上下文，导致每次迭代需要O(n)的token成本，总成本为O(n²)，效率低下。

Method: 将autoresearch模式重构为基于LangGraph的有状态ReAct智能体，使用类型化持久状态通过工具调用接口在迭代间传递实验历史，保持固定大小的对话窗口。

Result: 在超参数调优任务中（15次迭代），有状态智能体消耗2,492个token，相比无状态的24,465个token减少90%；在代码性能优化任务中（40次迭代），有状态智能体消耗627K个token，相比无状态的1,275K个token减少52%，同时保持相当的优化质量。

Conclusion: 有状态的autoresearch智能体通过结构化的token减少机制，将每次迭代成本从O(n)降低到O(1)，为实践者提供了实现高效自主实验的详细架构。
...
TL;DR: 本文对19种图神经网络层在轨迹预测任务中的表现进行了系统性比较研究，识别出最佳层组合并提出实用的设计原则


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统依赖精确的轨迹预测来规划安全高效的运动，图神经网络是建模道路参与者时空交互的有前景方法，但目前GNN架构设计缺乏标准化，不清楚哪些图层能有效捕捉空间交互和时间动态

Method: 对19种不同类型的图神经网络层进行详细的比较研究，重点分析它们在空间和时间处理能力上的差异，探索不同的超参数设置

Result: 在探索的超参数设置中，识别出5个表现突出的层组合，其中ARMA、Chebyshev和拓扑感知层始终表现优于其他层；发现基于求和的聚合比基于平均的方法更有效，多头注意力机制能实现更丰富的交互，为不同跳距分配不同权重能显著提高预测精度

Conclusion: 研究结果为设计更可解释和有效的轨迹预测模型提供了实用指导，帮助标准化GNN在轨迹预测任务中的架构设计
...
TL;DR: 该研究使用机器学习模型分析考试期间收集的生理数据（如皮电活动、心率和皮肤温度）来预测考试成绩，比较了从传统模型到深度学习模型等多种方法的表现。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索生理压力指标与学业表现之间的关联，通过机器学习模型预测考试成绩，从而更好地理解影响学生心理健康的压力因素，并利用生理数据改善学生福祉和学业成果。

Method: 采用了多种机器学习方法，包括传统模型（逻辑回归、随机森林、支持向量机）和深度学习架构（Transformer、LSTM、GRU），使用准确率、精确率、召回率和F1分数等标准性能指标来比较模型效果。

Result: 实验结果显示，深度学习模型通常能更好地捕捉生理数据中的复杂关系，但随机森林等简单模型有时能实现更优性能，同时提供计算效率和可解释性。Transformer模型表现出显著的多功能性，其性能与LSTM和GRU模型相当。

Conclusion: 研究强调需要根据问题目标选择广泛的模型类别，在精度、效率和可解释性之间取得平衡。通过阐明生理信号与学业表现之间的关系，该研究有助于理解影响学生心理健康的压力因素，并促进利用生理数据提升学生福祉和学业成果。
...
TL;DR: CILN框架通过可控的输入损坏创建实例依赖标签噪声基准，相比传统基于标注者错误的基准能更明确地控制噪声来源和严重程度。


<details>
  <summary>Details</summary>
Motivation: 现有合成实例依赖标签噪声(IDN)基准通常通过不完美的标注者或分类器生成噪声，这使得模糊性的来源不明确。需要一种能明确控制噪声来源和严重程度的基准生成框架。

Method: CILN框架通过对输入数据进行受控的损坏来创建IDN基准，使用多样化的投票者池对损坏实例进行标注，从而产生噪声源和严重程度都可明确控制的基准数据集。

Result: 在CIFAR10、MNIST和Adult数据集上构建了90个基准设置，覆盖多个损坏类型和严重程度。实验表明生成的基准展现真实的实例依赖噪声、提供多样化的混淆结构，在CIFAR-10上产生的标签分布比现有合成IDN基准更接近人类不确定性。

Conclusion: 损坏介导的IDN能够暴露流行噪声标签学习方法(如Co-Teaching和DivideMix)的失败模式，这些在可比水平的标注者错误噪声中观察不到。噪声结构(不仅仅是噪声率)对基准难度和算法行为有重要影响。CILN为研究不同实例难度来源下的噪声标签学习提供了补充性基准框架。
...
TL;DR: 该论文提出了AdaNAGED方法，将零阶优化、自适应调整和非欧几里得更新几何统一起来，用于大规模语言模型微调，解决了内存开销和参数调优问题。


<details>
  <summary>Details</summary>
Motivation: 大规模语言模型微调面临内存开销大的问题，传统反向传播需要存储激活值、梯度和优化器状态。零阶优化虽然内存效率高，但对步长和平滑参数敏感，需要昂贵的任务特定调优。

Method: 提出了AdaNAGED方法，结合了参数自由优化、线性最小化预言机和非欧几里得更新几何，实现了梯度自由训练、自适应调整和几何感知更新的统一。

Result: 建立了收敛保证，并在OPT-1.3B模型的大规模语言模型微调任务上验证了方法的有效性。

Conclusion: AdaNAGED方法成功解决了大规模语言模型微调中的内存开销和参数调优问题，为梯度自由训练提供了有效的解决方案。
...
TL;DR: FASTMIX是一个自动化数据混合发现框架，通过将混合选择重新表述为双层优化问题，仅需训练单个代理模型即可同时优化混合系数和模型参数，显著提高了效率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 虽然大规模多样化数据集推动了大型模型的进展，但为预训练和后训练确定最优数据混合仍然是一个重要的开放问题。现有方法依赖预定义启发式规则或资源密集型模拟，效率低下且难以扩展。

Method: 将混合选择重新表述为双层优化问题，证明优化混合比在数学上等价于在均匀源采样下为每个源分配损失权重，从而将混合系数直接嵌入到可微分的迭代优化目标中。采用近似迭代优化过程，交替更新模型参数（内循环）和基于验证反馈更新混合比例（外循环）。

Result: 在预训练和后训练任务中，FASTMIX均优于基线方法，同时大幅降低了搜索成本。

Conclusion: FASTMIX为自动化数据混合发现提供了一个高效、可扩展的框架，解决了当前数据混合优化中的关键挑战，为大型模型训练提供了更优的数据配置方法。
...
TL;DR: 本文提出Rational Sparse Autoencoder（RSAE），用可训练的有理函数替代传统SAE中的固定编码器激活函数，从而改善稀疏性与重建的权衡，并在多个语言模型上取得一致改进。


<details>
  <summary>Details</summary>
Motivation: 当前稀疏自动编码器（SAE）受限于固定的编码器非线性激活函数（如ReLU、JumpReLU、TopK），这硬编码了特定的稀疏机制，可能扭曲重建与稀疏性之间的权衡关系。

Method: 提出RSAE方法：1）用可训练的有理函数替代固定激活函数；2）采用两阶段训练流程：初始化阶段复制预训练基线SAE权重，通过松弛Remez交换在合成数据上获得有理系数，并校准尺度参数；然后通过标准稀疏正则化重建目标进行微调。

Result: 在三个开源语言模型的残差流激活上，针对所有三种基线激活函数族，RSAE在微调后均严格优于基线，在重建指标和下游行为指标上均有改进，同时保持了稀疏探测下的特征级可解释性。这些改进在不同宿主语言模型、基线激活函数族和测试的所有稀疏度范围内都保持一致。

Conclusion: RSAE通过引入可训练的有理激活函数，提供了比传统固定激活函数更灵活的稀疏机制，能够更好地适应预激活几何结构，在保持特征可解释性的同时显著改善了稀疏自动编码器的性能。
...
TL;DR: 训练基于注意力的卷积变分自编码器处理X射线散射数据，用于离线数据集探索和实时分析，通过MLExchange平台的潜在空间浏览器提供交互式结构探索


<details>
  <summary>Details</summary>
Motivation: 科学用户设施生成X射线散射数据的速度超过了传统工作流程的处理能力，需要解决离线数据集探索和实时分析两个场景下的数据处理挑战

Method: 训练一个基于注意力的域特定卷积变分自编码器(C-VAE)，在150万张X射线散射图像上进行训练，学习捕获不同实验条件下结构变化的低维表示；与通用视觉基础模型DINOv3 (ViT-7B)进行基准测试

Result: 学习到的潜在空间显示了反映实验进展的良好组织聚类和平滑轨迹，支持在不同结构状态之间生成可控的合成散射图像；无需重新训练即可将模型部署到两个同步辐射设施的时间分辨薄膜形成实验中；域特定训练比通用模型产生更可解释的潜在空间组织

Conclusion: 域特定训练对于X射线散射数据分析至关重要，能够产生更可解释的潜在表示；该工作流程已集成到MLExchange平台的潜在空间浏览器中，支持对存档数据集和实时实验进行交互式结构探索
...
TL;DR: 该论文对世界模型研究现状进行系统分析，提出了一个基于决策效用的评估框架，强调从视觉逼真度到策略优化效用的L0-L7阶梯式评估体系。


<details>
  <summary>Details</summary>
Motivation: 世界模型已成为现代AI的核心抽象概念，但当前研究存在术语混乱和评估标准不统一的问题。论文发现许多研究存在主张与证据不匹配的情况，即模型声称的用途超出了评估所能证明的范围。

Method: 通过文献综述分析当前世界模型研究现状，提出了L0-L7的阶梯式评估体系，并基于此设计了一个以决策为中心的评估框架和基准协议。

Result: 论文构建了一个从视觉逼真度(L0)到策略优化效用(L7)的完整评估体系，强调L4是第一个真正的干预性测试，L5-L7提供了最直接的决策有用性证据。

Conclusion: 世界模型评估应聚焦于其在具身决策中的实际效用，特别是反事实推理、策略评估、规划等核心能力，而非仅仅关注生成视频的视觉质量。
...
TL;DR: 研究者训练了一个两层Transformer编码器，通过椭圆曲线的前128个Frobenius迹来预测其秩（0或1），准确率超过99%，并应用机制可解释性技术揭示了模型学习到的算法。


<details>
  <summary>Details</summary>
Motivation: 探索机器学习模型是否能够从椭圆曲线的Frobenius迹数据中学习到数论中的深层数学结构，并理解模型如何实现这种预测能力。

Method: 使用两层Transformer编码器，输入为前128个归一化Frobenius迹，训练分类椭圆曲线的秩（0或1）。应用注意力分析、线性探测、激活修补、logit归因和神经元级电路分析等机制可解释性技术。

Result: 模型在测试集上达到>99%的准确率；识别出仅需20个神经元就能实现预测的稀疏电路；发现模型学习到了Mestre-Nagao和启发式权重（相关系数达0.997）；CLS注意力集中在素数位置；CLS嵌入能编码log L(E,1)（R²=0.962）。

Conclusion: 机器学习模型能够从Frobenius迹数据中自动学习到数论中的解析结果，揭示了模型通过稀疏的推拉检测器架构实现分类，但存在读出不匹配问题。不同训练得到的模型在函数空间高度一致。
...
TL;DR: 本文提出了一种基于时间差分（TD）的训练目标来增强扩散模型中不同时间步去噪预测的一致性，显著提高了少步采样器的生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型通常关注单个时间步的局部去噪目标，缺乏跨时间步的一致性约束，这会导致去噪轨迹上的预测不一致，特别是在少步采样时性能下降明显。

Method: 将扩散过程重新表述为马尔可夫奖励过程，将去噪视为强化学习中的策略评估问题，从而推导出适用于离散和连续时间扩散的统一TD方法，并提出基于样本的重新加权策略来稳定训练。

Result: 实验表明，使用TD训练能显著提高FID测量的样本质量，在采样步数较少时优势更加明显，适用于低计算预算场景。消融研究验证了成对损失重新加权、正则化权重和一步跨度等设计选择的有效性。

Conclusion: TD方法可以作为通用的即插即用模块，增强扩散生成模型中跨时间步的一致性，提高不同扩散生成模型的生成质量。
