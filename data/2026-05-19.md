...
### [1] [ANNEAL: Adapting LLM Agents via Governed Symbolic Patch Learning](https://arxiv.org/abs/2605.16309)
*Safayat Bin Hakim,Keyan Guo,Wenkai Tan,Alvaro Velasquez,Shouhuai Xu,Houbing Herbert Song*

Main category: cs.AI

TL;DR: ANNEAL是一个神经-符号智能体系统，通过失败驱动知识获取（FDKA）机制，将重复故障转化为过程知识图的受控符号编辑，实现持久的结构修复和治理保障。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体能够从单个执行错误中恢复，但在重复遇到相同故障时会失败，因为基础的流程知识（操作符模式、前置条件和约束）没有得到修复。现有自演化方法通过更新提示、内存或模型权重来解决问题，但没有直接修复编码任务执行方式的符号结构，也缺乏部署所需的安全治理保障。

Method: ANNEAL采用神经-符号框架，核心机制是失败驱动知识获取（FDKA）。当出现重复故障时，FDKA会定位责任操作符，通过受限的LLM生成类型化补丁，然后通过多维评分、符号防护和金丝雀测试验证补丁，最后提交到过程知识图中。每个被接受的编辑都具备完整的溯源和确定性回滚能力。

Result: 在四个领域的27个多次种子运行中，ANNEAL是唯一能够提交持久结构修复的系统。强基线系统如ReAct和Reflexion虽然在单次恢复中表现出色，但对重复故障的保留失败率仍为72-100%，而ANNEAL在测试的重复故障场景中将失败率降至0%。移除FDKA机制会消除所有结构修复，并将成功率降低高达26.7个百分点。

Conclusion: ANNEAL系统证明了神经-符号方法在持久故障消除方面的有效性。与基于权重的适应性和基于提示的适应性相比，受控的符号修复为智能体的自演化提供了互补的范式，能够确保持久的结构修复并提供治理保障。
...
### [2] [From Prompts to Protocols: An AI Agent for Laboratory Automation](https://arxiv.org/abs/2605.16552)
*Angelos Angelopoulos,James F. Cahoon,Ron Alterovitz*

Main category: cs.AI

TL;DR: 使用大语言模型与实验室编排系统集成，实现科学家通过自然语言交互创建和监控自动化实验室协议，显著提高协议生成成功率和减少操作复杂度。


<details>
  <summary>Details</summary>
Motivation: 自动化科学实验室能加速新材料的发现和测试，但需要科学家编写代码、管理配置文件并处理复杂软件基础设施，导致流程繁琐且耗时。

Method: 提出AI智能体架构，集成大语言模型与实验编排系统(EOS)，采用智能体循环机制，包含自动验证和错误纠正功能，并通过可视化图编辑器实现协议的可视化编辑和同步。

Result: 在化学、生物学和材料科学三个模拟自动化实验室中评估，AI智能体首次尝试协议生成成功率达到97%，并将所需接口操作减少了一个数量级。

Conclusion: 该研究开发了一个将大语言模型与实验室编排系统集成的AI智能体架构，通过自然语言交互实现了实验室协议的自动生成和执行，显著提高了自动化实验室的工作效率和易用性。
...
TL;DR: Skim是一个基于推测执行的Web Agent框架，通过利用网站的稳定结构模式，为大多数查询提供轻量级快速路径，显著降低成本和时间开销。


<details>
  <summary>Details</summary>
Motivation: 当前Web Agent的高成本并非内在任务属性，而是现有框架组成方式的问题：前沿模型推理、浏览器渲染和ReAct式规划被不加区分地应用于所有任务步骤，而许多网站的查询具有可预测结构。

Method: Skim采用离线分析器捕获网站的稳定URL模式、答案格式和任务轨迹映射，运行时将查询匹配到模板，使用小模型合成目标URL并提取答案，使用轻量级验证器把关快速路径输出，罕见错误时回退到完整Agent。

Result: 在三个主要Agent（WebVoyager、AgentOccam、BrowserUse）的标准评估中，Skim将每任务的中位数成本降低了1.9倍，延迟减少了33.4%，同时没有准确度损失。

Conclusion: Skim是一个高效的推测执行框架，通过利用专用网站的可预测结构，显著降低了Web Agent的成本和延迟，同时保持准确性。该方法证明了大多数网页查询可以通过轻量级组件绕过重型处理。
...
### [4] [Scalable Uncertainty Reasoning in Knowledge Graphs](https://arxiv.org/abs/2605.16568)
*Jingcheng Wu*

Main category: cs.AI

TL;DR: 提出针对知识图谱三种不确定性层次（不精确属性值、概率三元组存在性、不完整模式知识）的模块化推理框架，旨在平衡语义精确性与计算效率。


<details>
  <summary>Details</summary>
Motivation: 知识图谱对语义数据集成至关重要但其建模的真实世界数据常具有不确定性，当前语义Web标准缺乏对这类不确定性推理的原生支持，而朴素扩展往往导致计算不可处理。

Method: 采用三层次模块化方法：1) 为连续属性定义概率字面值和相应的查询代数；2) 基于编译的框架将SPARQL来源转换为可处理的概率电路以处理不确定三元组；3) 面向统计模式推理的拓扑感知几何嵌入。

Result: 提出一个专门处理知识图谱三个不确定性层次的模块化框架，每个层次采用定制化技术，核心假设是专门的推理机制能够协调语义精确性和计算可处理性。

Conclusion: 该论文提出通过专门的代数、逻辑和几何推理机制，为知识图谱中的不确定性提供模块化框架，旨在实现语义精确性与计算可处理性的统一。
...
### [5] [TTE-Flash: Accelerating Reasoning-based Multimodal Representations via Think-Then-Embed Tokens](https://arxiv.org/abs/2605.16638)
...
TL;DR: 本研究提出使用潜在思维标记替代显式思维链来降低多模态表示的计算开销，开发了TTE-Flash-2B模型，在保持恒定推理成本的同时实现了更优性能，潜在思维标记具有良好的可解释性，且在不同任务中展示了可扩展性。


<details>
  <summary>Details</summary>
Motivation: Universal Multimodal Embedding (UME) 在结合Chain-of-Thought (CoT)推理方面展现出显著优势，但生成显式CoT推理轨迹会带来巨大的计算开销。尽管显式CoT方法有效，但其过高的计算成本在实际应用中是难以承受的。因此需要寻找既能保持CoT推理优势，又能显著降低计算成本的替代方案。

Method: 本研究提出使用潜在思维标记（latent think tokens）替代显式思维链推理。这些思维标记被解释为潜在的变量，可以产生显式思维链追踪作为观测变量。方法优化包括：1）思维标记通过思维链生成损失函数进行优化，2）后续嵌入标记通过对比损失函数进行优化。研究探讨了两个关键架构设计：1）如何从同一个LLM骨干中提取思维标记和嵌入标记，2）如何将这两个相关任务作为依赖任务进行训练。

Result: 提出的TTE-Flash-2B模型在MMEB-v2基准上超越了显式CoT对应模型。潜在思维标记在文本和视觉上都具有良好的可解释性。在15个视频数据集上的零样本评估显示，随着思维标记数量的增加，模型性能呈缩放行为增长。这为基于任务需求的自适应思维预算分配提供了实验基础。

Conclusion: 本研究通过使用潜在思维标记替代显式思维链，开发了TTE-Flash-2B多模态表示模型。该模型在保持恒定推理成本的同时，实现了高性能、推理感知的表示，同时在MMEB-v2基准上超越了显式思维链对应模型。潜在思维标记具有良好的可解释性，且展示了随着思维标记数量增加的缩放行为，为基于任务需求的动态思维预算分配奠定了基础。
...
### [6] [LinAlg-Bench: A Forensic Benchmark Revealing Structural Failure Modes in LLM Mathematical Reasoning](https://arxiv.org/abs/2605.16675)
*Shradha Agarwal,Deepak Rajbhar,Tariq J*

Main category: cs.AI

TL;DR: LinAlg-Bench基准评估显示LLMs在线性代数计算中存在4x4尺度行为阈值：低于该尺度模型因执行错误失败，高于该尺度则转向计算放弃和结构化幻觉，表明工作记忆限制是主要原因。


<details>
  <summary>Details</summary>
Motivation: 需要系统评估大型语言模型在结构化线性代数计算中的能力，探究其数学失败的深层原因和模式，超越简单的二元准确性评估，了解失败是否具有结构性规律。

Method: 研究团队开发了LinAlg-Bench诊断基准，包含10个前沿大型语言模型在3x3、4x4和5x5矩阵严格维度梯度上的结构化线性代数计算评估。基准涵盖9种任务类型和660个SymPy认证的问题，共评估6600个模型输出。引入三阶段自动化法医学管道，将1156个失败案例分类为十个主要错误标签及细粒度子类型。

Result: 发现一个尖锐的4x4尺度行为阈值：低于该尺度，模型通过执行错误失败（如符号跟踪失败、算术漂移和奇偶错误）；高于该尺度，失败转变为计算放弃，模型通过工具角色扮演、约束一致的虚构和结构化幻觉来虚构响应而非尝试计算。这种"虚构到放弃"的转变在所有模型层级和架构中近乎普遍。还识别出三种规模涌现的错误类型，并显示解决方案策略刚性几乎是5x5行列式准确性的完美预测指标。

Conclusion: 该研究揭示了LLMs在线性代数计算中存在一个关键的4x4尺度行为阈值：在该规模以下模型主要通过执行错误失败；在该规模以上则转向计算放弃和结构化幻觉。这表明失败更多源于工作记忆限制而非知识缺失，并为改进LLMs的符号推理能力提供了诊断依据。
...
### [7] [Recall Isn't Enough: Bounding Commitments in Personalized Language Systems](https://arxiv.org/abs/2605.16712)
*Rui Tang,Yichi Zhang,Xi Chen,Chen Dong,Youwei Yang,Yumeng Shen*

Main category: cs.AI

TL;DR: CBEA+LCV通过结构化承诺验证和有界证据激活，解决了长上下文系统中承诺失败的问题，实现了零故障率和显著负载减少，提供了可靠的契约边界操作方案。


<details>
  <summary>Details</summary>
Motivation: 当前长上下文和记忆系统将个性化视为召回问题，但实践中许多失败发生在系统承诺阶段：将噪声线索转化为硬约束、丢弃罕见证据、忘记后续义务或在不可行情况下仍然回答。需要系统性的承诺验证和证据激活机制来解决这些问题。

Method: CBEA采用契约边界证据激活方法，通过类型覆盖、尾部见证和后果债务来激活有限证据集；LCV(词典序承诺验证)在生成散文前验证结构化承诺，并将不可行状态路由到修复、弃权或重新契约等处理方式。系统通过显式承诺控制和输入压缩减少负载。

Result: CBEA+LCV在360个测试案例和三种生成后端上，在验证器范围内达到了零失败率，可用性为0.49-0.60。使用相同LCV门控的原始和长上下文基线只能在0.003-0.092的可用性下达到零失败率。CBEA+LCV减少了74-75%的中位输入负载，召回比例较低(0.012)，但提供了可靠的承诺控制。

Conclusion: CBEA与LCV的结合提供了一个显式承诺控制的操作点，能够显著减少输入负载并实现零故障性能，但这并非意味着通用记忆优势。该系统通过结构化承诺验证和证据激活，实现了一种有界但可靠的个性化处理方案。
...
### [8] [Baba in Wonderland: Online Self-Supervised Dynamics Discovery for Executable World Models](https://arxiv.org/abs/2605.16725)
*SeungWon Seo,DongHeun Han,SeongRae Noh,HyeongYeop Kang*

Main category: cs.AI

TL;DR: Alice是一个学习可执行世界模型的系统，通过在理念失衡的环境中精炼失败更新的类别冲突来改进模型，实验证明该方法显著提升了学习效果。


<details>
  <summary>Details</summary>
Motivation: 可执行世界模型只有在程序捕捉环境转换规律而非其表面词汇中的语义捷径时，才能被阅读、编辑、执行并用于规划。本研究关注在线可执行世界模型学习中的先验不匹配问题，在这种情况下，智能体必须仅从交互证据中归纳状态相关的动态，而无需规则描述、奖励信号或可信的词汇先验。

Method: 研究者提出了Alice系统，这是一个处理失败候选更新的闭环系统。当某个候选解释解释了新转换但丢失了先前已解释的转换时，这种保留冲突揭示了现有程序错误地混合的动态关系。Alice将这些冲突精炼为假设类别，这些类别既为更新提供了紧凑的、类别分层的保留反例，又引导前沿探索朝向对当前程序来说是新颖且代表性不足的转换。

Result: 实验在Baba in Wonderland上进行，这是Baba Is You的一个先验不匹配变体，保留了模拟器动态但将具有语义意义的规则-属性标签替换为无关词汇。实验结果表明，Alice在先验不匹配条件下显著提升了可执行世界模型学习。

Conclusion: 论文表明Alice系统在理念失衡的环境中显著提高了可执行世界模型学习的能力，并且其核心组件（类别精炼和类别感知探索）都对这一进步做出了重要贡献。
...
### [9] [A Global-Local Graph Attention Network for Traffic Forecasting](https://arxiv.org/abs/2605.16726)
*Tianchi Zhang*
...
### [10] [Body-Grounded Perspective Formation and Conative Attunement in Artificial Agents](https://arxiv.org/abs/2605.16728)
*Hongju Pae*
...
### [11] [State Contamination in Memory-Augmented LLM Agents](https://arxiv.org/abs/2605.16746)
...
### [12] [NeuroMAS: Multi-Agent Systems as Neural Networks with Joint Reinforcement Learning](https://arxiv.org/abs/2605.16757)
...
### [13] [Multi-Paradigm Agent Interaction in Practice:A Systematic Analysis of Generator-Evaluator, ReAct Loop,and Adversarial Evaluation in the buddyMe Framework](https://arxiv.org/abs/2605.16821)
...
### [14] [Voices in the Loop: Mapping Participatory AI](https://arxiv.org/abs/2605.16827)
*Rashid Mushkani*
...
### [15] [Sketch Then Paint: Hierarchical Reinforcement Learning for Diffusion Multi-Modal Large Language Models](https://arxiv.org/abs/2605.16842)
...
### [16] [Artificial Adaptive Intelligence: The Missing Stage Between Narrow and General Intelligence](https://arxiv.org/abs/2605.16844)
*Boris Kriuk*
...
### [17] [Learning to Learn from Multimodal Experience](https://arxiv.org/abs/2605.16857)
...
### [18] [Reasoning Can Be Restored by Correcting a Few Decision Tokens](https://arxiv.org/abs/2605.16874)
...
### [19] [Virtual Nodes Guided Dynamic Graph Neural Network for Brain Tumor Segmentation with Missing Modalities](https://arxiv.org/abs/2605.16880)
...
### [20] [NGM: A Plug-and-Play Training-Free Memory Module for LLMs](https://arxiv.org/abs/2605.16893)
...
### [21] [TOBench: A Task-Oriented Omni-Modal Benchmark for Real-World Tool-Using Agents](https://arxiv.org/abs/2605.16909)
...
### [22] [From Static Risk to Dynamic Trajectories: Toward World-Model-Inspired Clinical Prediction](https://arxiv.org/abs/2605.16927)
...
### [23] [How do Humans Process AI-generated Hallucination Contents: a Neuroimaging Study](https://arxiv.org/abs/2605.16953)
...
### [24] [Harnessing AI for Inverse Partial Differential Equation Problems: Past, Present, and Prospects](https://arxiv.org/abs/2605.16966)
...
### [25] [Brain Vascular Age Prediction Using Cerebral Blood Flow Velocity and Machine Learning Algorithms](https://arxiv.org/abs/2605.16969)
...
### [26] [A Conflict-aware Evidential Framework for Reliable Sleep Stage Classification](https://arxiv.org/abs/2605.17021)
...
### [27] [Reliability and Effectiveness of Autonomous AI Agents in Supply Chain Management](https://arxiv.org/abs/2605.17036)
...
### [28] [Evidential Information Fusion on Possibilistic Structure](https://arxiv.org/abs/2605.17038)
...
### [29] [PersonaArena: Dynamic Simulation for Evaluating and Enhancing Persona-Level Role-Playing in Large Language Models](https://arxiv.org/abs/2605.17044)
...
### [30] [Towards Human-Level Book-Writing Capability](https://arxiv.org/abs/2605.17064)
...
### [31] [AnchorDiff: Topology-Aware Masked Diffusion with Confidence-based Rewriting for Radiology Report Generation](https://arxiv.org/abs/2605.17071)
...
### [32] [RAGA: Reading-And-Graph-building-Agent for Autonomous Knowledge Graph Construction and Retrieval-Augmented Generation](https://arxiv.org/abs/2605.17072)
...
### [33] [Scientific Logicality Enriched Methodology for LLM Reasoning: A Practice in Physics](https://arxiv.org/abs/2605.17104)
...
### [34] [Capturing LLM Capabilities via Evidence-Calibrated Query Clustering](https://arxiv.org/abs/2605.17110)
...
### [35] [F2IND-IT! -- Multimodal Fuzzy Fake Indian News Detection using Images and Text](https://arxiv.org/abs/2605.17115)
...
### [36] [Latent Heuristic Search: Continuous Optimization for Automated Algorithm Design](https://arxiv.org/abs/2605.17137)
...
### [37] [Dynamics of collective creativity in AI art competitions](https://arxiv.org/abs/2605.17141)
...
### [38] [MADP: A Multi-Agent Pipeline for Sustainable Document Processing with Human-in-the-Loop](https://arxiv.org/abs/2605.17159)
...
### [39] [From Imitation to Interaction: Mastering Game of Schnapsen with Shallow Reinforcement Learning](https://arxiv.org/abs/2605.17162)
...
### [40] [Responsible Agentic AI Requires Explicit Provenance](https://arxiv.org/abs/2605.17169)
...
### [41] [CAREBench: Evaluating LLMs' Emotion Understanding by Assessing Cognitive Appraisal Reasoning](https://arxiv.org/abs/2605.17176)
...
### [42] [Towards Robust Argumentative Essay Understanding via TIDE: An Interactive Framework with Trial and Debate](https://arxiv.org/abs/2605.17247)
...
### [43] [CatalyticMLLM: A Graph-Text Multimodal Large Language Model for Catalytic Materials](https://arxiv.org/abs/2605.17254)
*Yanjie Li*
...
### [44] [CAM-Bench: A Benchmark for Computational and Applied Mathematics in Lean](https://arxiv.org/abs/2605.17255)
...
### [45] [Is VLA Reasoning Faithful? Probing Safety of Chain-of-Causation](https://arxiv.org/abs/2605.17268)
...
### [46] [A2RBench: An Automatic Paradigm for Formally Verifiable Abstract Reasoning Benchmark Generation](https://arxiv.org/abs/2605.17278)
...
### [47] [MetaCogAgent: A Metacognitive Multi-Agent LLM Framework with Self-Aware Task Delegation](https://arxiv.org/abs/2605.17292)
...
### [48] [CyberCorrect: A Cybernetic Framework for Closed-Loop Self-Correction in Large Language Models](https://arxiv.org/abs/2605.17305)
...
### [49] [Reasoning Before Diagnosis: Physician-Inspired Structured Thinking for ECG Classification](https://arxiv.org/abs/2605.17308)
...
### [50] [HyperPersona: A Multi-Level Hypergraph Framework for Text-Based Automatic Personality Prediction](https://arxiv.org/abs/2605.17355)
...
### [51] [ADR: An Agentic Detection System for Enterprise Agentic AI Security](https://arxiv.org/abs/2605.17380)
...
### [52] [Heterogeneous Information-Bottleneck Coordination Graphs for Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.17393)
...
### [53] [Computational Challenges in Token Economics: Bridging Economic Theory and AI System Design](https://arxiv.org/abs/2605.17410)
*Ou Wu,Yingjun Deng*
...
### [54] [Multi-Party Multi-Objective Optimization as Consensus Search: Runtime Analysis of Cross-Party Recombination](https://arxiv.org/abs/2605.17454)
...
### [55] [The Capability Paradox: How Smarter Auditors Make Multi-Agent Systems Less Secure](https://arxiv.org/abs/2605.17480)
...
### [56] [RAG-based EEG-to-Text Translation Using Deep Learning and LLMs](https://arxiv.org/abs/2605.17503)
...
### [57] [Self-supervised Hierarchical Visual Reasoning with World Model](https://arxiv.org/abs/2605.17537)
...
### [58] [Memory-Guided Tree Search with Cross-Branch Knowledge Transfer for LLM Solver Synthesis](https://arxiv.org/abs/2605.17539)
...
### [59] [Generalization or Memorization? Brittleness Testing for Chess-Trained Language Models](https://arxiv.org/abs/2605.17565)
*Ethan Tang*
...
### [60] [ECG-WM: A Physiology-Informed ECG World Model for Clinical Intervention Simulation](https://arxiv.org/abs/2605.17580)
...
### [61] [NeuSymMS: A Hybrid Neuro-Symbolic Memory System for Persistent, Self-Curating LLM Agents](https://arxiv.org/abs/2605.17596)
...
### [62] [GraphMind: From Operational Traces to Self-Evolving Workflow Automation](https://arxiv.org/abs/2605.17617)
...
### [63] [Prediction of Challenging Behaviors Associated with Profound Autism in a Classroom Setting Using Wearable Sensors](https://arxiv.org/abs/2605.17618)
...
### [64] [Episodic-Semantic Memory Architecture for Long-Horizon Scientific Agents](https://arxiv.org/abs/2605.17625)
*Nikola Milosevic*
...
### [65] [WebGameBench: Requirement-to-Application Evaluation for Coding Agents via Browser-Native Games](https://arxiv.org/abs/2605.17637)
...
### [66] [Causal Intervention-Based Memory Selection for Long-Horizon LLM Agents](https://arxiv.org/abs/2605.17641)
...
### [67] [SAPO: Step-Aligned Policy Optimization for Reasoning-Based Generative Recommendation](https://arxiv.org/abs/2605.17648)
...
### [68] [Multimodal Cultural Heritage Knowledge Graph Extension with Language and Vision Models](https://arxiv.org/abs/2605.17669)
...
### [69] [EGI: A Multimodal Emotional AI Framework for Enhancing Scrum Master Real-time Self-Awareness](https://arxiv.org/abs/2605.17684)
...
### [70] [EXG: Self-Evolving Agents with Experience Graphs](https://arxiv.org/abs/2605.17721)
...
### [71] [Divergence-Suppressing Couplings for Rectified Flow](https://arxiv.org/abs/2605.17733)
...
### [72] [Harnessing LLM Agents with Skill Programs](https://arxiv.org/abs/2605.17734)
...
### [73] [Agents for Experiments, Experiments for Agents: A Design Grammar for AI-Enabled Experimental Science](https://arxiv.org/abs/2605.17746)
...
### [74] [Surface-Form Neural Sparse Retrieval: Robust Fuzzy Matching for Industrial Music Search](https://arxiv.org/abs/2605.17762)
...
### [75] [Entropy-Gradient Inversion: Moving Toward Internal Mechanism of Large Reasoning Models](https://arxiv.org/abs/2605.17770)
...
### [76] [STRIDE: A Self-Reflective Agent Framework for Reliable Automatic Equation Discovery](https://arxiv.org/abs/2605.17790)
...
### [77] [Accelerating AI-Powered Research: The PuppyChatter Framework for Usable and Flexible Tooling](https://arxiv.org/abs/2605.17809)
...
### [78] [Going Headless? On the Boundaries of Vertical AI Firms](https://arxiv.org/abs/2605.17812)
...
### [79] [Interactive Evaluation Requires a Design Science](https://arxiv.org/abs/2605.17829)
...
### [80] [Remembering More, Risking More: Longitudinal Safety Risks in Memory-Equipped LLM Agents](https://arxiv.org/abs/2605.17830)
...
### [81] [KISS - Knowledge Infrastructure for Scientific Simulation: A Scaffolding for Agentic Earth Science](https://arxiv.org/abs/2605.17856)
...
### [82] [PAIR: Prefix-Aware Internal Reward Model for Multi-Turn Agent Optimization](https://arxiv.org/abs/2605.17877)
...
### [83] [Evaluating Cognitive Age Alignment in Interactive AI Agents](https://arxiv.org/abs/2605.17894)
...
### [84] [LAST-RAG: Literature-Anchored Stochastic Trajectory Retrieval-Augmented Generation for Knowledge-Conditioned Degradation Model Selection](https://arxiv.org/abs/2605.17902)
...
### [85] [Agentic Chunking and Bayesian De-chunking of AI Generated Fuzzy Cognitive Maps: A Model of the Thucydides Trap](https://arxiv.org/abs/2605.17903)
...
### [86] [SVFSearch: A Multimodal Knowledge-Intensive Benchmark for Short-Video Frame Search in the Gaming Vertical Domain](https://arxiv.org/abs/2605.17946)
...
### [87] [Reconciling Contradictory Views on the Effectiveness of SFT in LLMs: An Interaction Perspective](https://arxiv.org/abs/2605.17967)
...
### [88] [Unleashing LLMs in Bayesian Optimization: Preference-Guided Framework for Scientific Discovery](https://arxiv.org/abs/2605.17976)
...
### [89] [Shared Backbone PPO for Multi-UAV Communication Coverage with Connection Preservation](https://arxiv.org/abs/2605.17999)
*Z. Jiang*
...
### [90] [TeleCom-Bench: How Far Are Large Language Models from Industrial Telecommunication Applications?](https://arxiv.org/abs/2605.18025)
...
### [91] [New Insight of Variance reduce in Zero-Order Hard-Thresholding: Mitigating Gradient Error and Expansivity Contradictions](https://arxiv.org/abs/2605.18035)
...
### [92] [DocOS: Towards Proactive Document-Guided Actions in GUI Agents](https://arxiv.org/abs/2605.18048)
...
### [93] [LLM-Guided Communication for Cooperative Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.18077)
...
### [94] [Learning to Solve Compositional Geometry Routing Problems](https://arxiv.org/abs/2605.18094)
...
### [95] [Safety Geometry Collapse in Multimodal LLMs and Adaptive Drift Correction](https://arxiv.org/abs/2605.18104)
...
### [96] [TaskGround: Structured Executable Task Inference for Full-Scene Household Reasoning](https://arxiv.org/abs/2605.18109)
...
### [97] [POST: Prior-Observation Adversarial Learning of Spatio-Temporal Associations for Multivariate Time Series Anomaly Detection](https://arxiv.org/abs/2605.18128)
...
### [98] [Generative AI and the Productivity Divide: Human-AI Complementarities in Education](https://arxiv.org/abs/2605.18143)
...
### [99] [Evidence-Grounded Frontier Mapping and Agentic Hypothesis Generation in Nanomedicine](https://arxiv.org/abs/2605.18144)
...
### [100] [Whispers in the Noise: Surrogate-Guided Concept Awakening via a Multi-Agent Framework](https://arxiv.org/abs/2605.18150)
...
### [101] [TRACE: Trajectory Correction from Cross-layer Evidence for Hallucination Reduction](https://arxiv.org/abs/2605.18163)
*Tej Sanibh Ranade*
...
### [102] [Visualizing the Invisible: Generative Visual Grounding Empowers Universal EEG Understanding in MLLMs](https://arxiv.org/abs/2605.18172)
...
### [103] [Scalable Environments Drive Generalizable Agents](https://arxiv.org/abs/2605.18181)
...
### [104] [Pairwise Preference Reward and Group-Based Diversity Enhancement for Superior Open-Ended Generation](https://arxiv.org/abs/2605.18191)
...
### [105] [Beyond the Cartesian Illusion: Testing Two-Stage Multi-Modal Theory of Mind under Perceptual Bottlenecks](https://arxiv.org/abs/2605.18194)
...
### [106] [DARE-EEG: A Foundation Model for Mining Dual-Aligned Representation of EEG](https://arxiv.org/abs/2605.18298)
...
### [107] [SD-Search: On-Policy Hindsight Self-Distillation for Search-Augmented Reasoning](https://arxiv.org/abs/2605.18299)
...
### [108] [Causely: A Causal Intelligence Layer for Enterprise AI A Benchmark Study on SRE and Reliability Workflows](https://arxiv.org/abs/2605.18327)
...
### [109] [QSTRBench: a New Benchmark to Evaluate the Ability of Language Models to Reason with Qualitative Spatial and Temporal Calculi](https://arxiv.org/abs/2605.18380)
...
### [110] [When Fireflies Cluster; Enhancing Automatic Clustering via Centroid-Guided Firefly Optimization](https://arxiv.org/abs/2605.18460)
...
### [111] [OCCAM: Open-set Causal Concept explAnation and Ontology induction for black-box vision Models](https://arxiv.org/abs/2605.18481)
...
### [112] [A Practical Noise2Noise Denoising Pipeline for High-Throughput Raman Spectroscopy](https://arxiv.org/abs/2605.18511)
...
### [113] [AMR-SD: Asymmetric Meta-Reflective Self-Distillation for Token-Level Credit Assignment](https://arxiv.org/abs/2605.18529)
...
### [114] [VISAFF: Speaker-Centered Visual Affective Feature Learning for Emotion Recognition in Conversation](https://arxiv.org/abs/2605.18547)
...
### [115] [Query-Conditioned Knowledge Alignment for Reliable Cross-System Medical Reasoning](https://arxiv.org/abs/2605.18570)
...
### [116] [When Outcome Looks Right But Discipline Fails: Trace-Based Evaluation Under Hidden Competitor State](https://arxiv.org/abs/2605.18580)
...
### [117] [Latent Action Reparameterization for Efficient Agent Inference](https://arxiv.org/abs/2605.18597)
...
### [118] [Learning Lifted Action Models from Traces with Minimal Information About Actions and States](https://arxiv.org/abs/2605.18627)
...
### [119] [SCICONVBENCH: Benchmarking LLMs on Multi-Turn Clarification for Task Formulation in Computational Science](https://arxiv.org/abs/2605.18630)
...
### [120] [AI for Auto-Research: Roadmap & User Guide](https://arxiv.org/abs/2605.18661)
...
### [121] [GIM: Evaluating models via tasks that integrate multiple cognitive domains](https://arxiv.org/abs/2605.18663)
...
### [122] [Position: A Three-Layer Probabilistic Assume-Guarantee Architecture Is Structurally Required for Safe LLM Agent Deployment](https://arxiv.org/abs/2605.18672)
...
### [123] [Efficient Lookahead Encoding and Abstracted Width for Learning General Policies in Classical Planning](https://arxiv.org/abs/2605.18674)
...
### [124] [Learning Quantifiable Visual Explanations Without Ground-Truth](https://arxiv.org/abs/2605.18681)
...
### [125] [Democratizing Large-Scale Re-Optimization with LLM-Guided Model Patches](https://arxiv.org/abs/2605.18692)
...
### [126] [SkillGenBench: Benchmarking Skill Generation Pipelines for LLM Agents](https://arxiv.org/abs/2605.18693)
...
### [127] [What Does the AI Doctor Value? Auditing Pluralism in the Clinical Ethics of Language Models](https://arxiv.org/abs/2605.18738)
...
### [128] [Actionable World Representation](https://arxiv.org/abs/2605.18743)
...
### [129] [Systematic Optimization of Real-Time Diffusion Model Inference on Apple M3 Ultra](https://arxiv.org/abs/2605.16259)
*Yoichi Ochiai*

Main category: cs.LG

TL;DR: 该研究在苹果M3 Ultra芯片上系统评估了扩散模型优化的多种技术，发现苹果芯片的统一内存架构形成了与NVIDIA GPU完全不同的优化规律。通过CoreML转换蒸馏专用模型，最终实现了22.7 FPS的实时相机img2img变换。


<details>
  <summary>Details</summary>
Motivation: 尽管基于NVIDIA GPU的实时图像生成扩散模型发展迅速，但在苹果芯片等非CUDA平台上的系统化优化研究仍然非常有限，这限制了相关技术在苹果生态系统中的应用。

Method: 在Apple M3 Ultra（60核GPU，512GB统一内存）上进行了10个阶段的全面优化实验，包括CoreML转换、量化、Token Merging、神经引擎利用、紧凑模型探索、帧插值、kNN搜索合成、pix2pix-turbo、光流失帧技术和知识蒸馏，并通过关键性能指标量化评估各种方法的有效性。

Result: 通过将专门蒸馏的SDXS-512模型转换为CoreML格式并结合3线程相机流水线，实现了512x512分辨率下22.7 FPS的实时相机img2img变换。研究揭示了多项与NVIDIA GPU优化经验相悖的关键发现。

Conclusion: 本研究系统地展示了苹果芯片统一内存架构下扩散模型优化的特殊挑战，揭示了与NVIDIA GPU截然不同的优化规律，为苹果芯片上的扩散模型推理提供了实用指导。
...
### [130] [Reducing Credit Assignment Variance via Counterfactual Reasoning Paths](https://arxiv.org/abs/2605.16302)
*Fei Ding,Yongkang Zhang,Yeling Peng,Youwei Wang,Guoxiong Zhou,Zijian Zeng*

Main category: cs.LG

TL;DR: 论文提出IBPO框架解决LLM多步推理强化学习中稀疏奖励导致的信用分配问题，通过反事实比较构建隐式优势估计器，将终端奖励转化为步级学习信号，在数学和代码推理任务中显著提升训练稳定性和性能。


<details>
  <summary>Details</summary>
Motivation: 基于大规模语言模型的强化学习在多步推理任务中依赖稀疏的终端奖励，导致信用分配条件差：最终反馈会均匀传播到所有中间决策上，造成梯度方差高、训练不稳定和大量无效更新，导致模型失败并无法持续改进。

Method: 提出反事实比较信用分配框架，通过在同一输入下采样多种推理轨迹，将其差异视为对替代决策的隐式近似，构建隐式过程级优势估计器，将稀疏的终端奖励转化为步级敏感学习信号。在此基础上提出了隐式行为策略优化（IBPO）算法。

Result: IBPO在数学和代码推理基准测试中显著提高了训练稳定性和性能上限。

Conclusion: 本文提出的IBPO框架通过反事实比较为基础的信用分配机制，显著提升了LLM多步推理训练中的稳定性和性能上限，为充分释放LLM的推理潜力提供了有前景的研究方向。
...
### [131] [SignMuon: Communication-Efficient Distributed Muon Optimization](https://arxiv.org/abs/2605.16311)
*Neel Mishra,Kushagara Trivedi,Pawan Kumar*

Main category: cs.LG

TL;DR: Sign-Muon是一种1比特矩阵感知优化器，结合符号聚合和极坐标方法，大幅减少分布式训练通信开销，同时保持高质量训练效果，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 分布式大模型训练面临两方面的瓶颈：高质量梯度通信的开销大，以及传统坐标式优化器忽略了权重张量的矩阵结构。需要开发既能大幅减少通信开销，又能保持矩阵结构感知的高效优化算法。

Method: 提出1比特矩阵感知优化器Sign-Muon，结合signSGD的多数投票符号聚合与Muon的极坐标步框架。每个工作节点通过Newton-Schulz迭代计算动量极坐标因子形成方向，仅传输逐元素的符号，通过多数投票聚合；本地极坐标步在无额外通信成本下强制正交性。

Result: 1. 理论分析：在谱范数光滑性和有界方差随机梯度假设下，Sign-Muon达到O(1/√T)的非凸收敛速率；在单峰对称噪声下，M个工作节点的多数投票将随机项减少1/√M
2. α-β模型分析：仅需每次迭代一次整数sum-allreduce，正交化完全本地化，相比float32减少32倍带宽，相比int8减少4倍
3. 实验验证：在330个CIFAR-10/ResNet-50配置中获最佳验证精度92.15%；4-GPU变体在匹配有效批次下达到92.02%，训练时间减少37%；在nanoGPT上比其他基于符号的基线获得更低困惑度和更优性能，在16个GPU上呈现良好的弱扩展性

Conclusion: Sign-Muon通过1比特矩阵感知优化器，有效解决了分布式训练中的通信瓶颈和传统坐标式优化器的局限性，在带宽节省、训练效率和模型精度方面表现出色。
...
### [132] [A Structural Threshold in Decision Capacity Governs Collapse in Self-Play Reinforcement Learning](https://arxiv.org/abs/2605.16315)
*Arahan Kujur*

Main category: cs.LG

TL;DR: 自学习智能体在决策容量上的锐利阈值决定其对规则扰动的鲁棒性：消除所有正向可达或有决策导致坍塌，保留一个即可避免。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解在非对称规则扰动下，自学习强化学习智能体为何会突然坍塌，识别影响这种坍塌的关键阈值因素，为构建稳定且具有鲁棒性的多智能体学习系统提供理论指导。

Method: 通过多种实验设置验证理论：1）在不同域（扑克变种、矩阵博弈、骰子游戏）测试；2）使用多个学习算法；3）设计两种对照实验（冻结基线和固定对手）；4）分析不同条件下的行为模式，包括函数逼近、时不变性和可逆性；5）计算正向可达或有动作容量。

Result: 研究发现了一个锐利阈值：当正向可达或有动作容量降为零时，智能体坍塌到近最大损失的确定性剥削吸引子；保留至少一个这样的决策点即可防止坍塌。坍塌的严重程度通过正向可达或有动作容量连续缩放。该现象具有时不变性，动作恢复时可完全逆转，且函数逼近会加剧崩塌效应。

Conclusion: 论文揭示了决策能力阈值的关键作用，证明当自学习智能体失去所有正向可达的或有决策能力时，会迅速坍塌到确定性剥削吸引子这一固定点；而保留至少一个正向可达或有决策点即可避免坍塌。这为零和博弈中基于自学习的算法设计提供了重要理论依据。
...
### [133] [Forecasting Medium-Horizon Alzheimer's Disease Progression: Residual Gap-Aware Transformers for 24-Month CDR-SB Change from ADNI Clinical and Biomarker Histories](https://arxiv.org/abs/2605.16319)
*Ran Tong,Tong Wang,Lanruo Wang,Yin Ni*

Main category: cs.LG

TL;DR: 本研究开发了一个基于统计锚定和间隔感知残差学习的transformer模型，用于预测阿尔茨海默病中期病程(24个月CDR-SB变化)。该方法结合混合效应统计基准和transformer残差学习，在ADNI数据上表现出比现有方法更好的预测性能。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病中期病程预测面临挑战：未来临床评分可能仍与基线严重程度相关，而生物标志物历史数据不规则且不完全观察。

Method: 提出了结合混合效应统计模型和基于transformer的残差学习方法，包括：使用参与者级随机截距的混合效应参考模型、观察级三元组标记化处理不规则历史数据、学习式非负时间间隔惩罚的自注意力机制。

Result: 通过在五次参与者级随机种子重复测试中，提出的模型在所有报告指标上均达到最佳平均测试性能：相对于混合效应基线，MSE降低了13.1%，预测-观察相关性提高了26.4%，在平均误差和相关性方面优于GRU-D和STraTS。

Conclusion: 该研究表明，基于统计锚定和间隔感知残差学习的方法为阿尔茨海默病中期病程预测提供了一个有效的框架，能够显著提高预测准确性。
...
### [134] [AdaGraph: A Graph-Native Clustering Algorithm That Overcomes the Curse of Dimensionality and Enables Scientific Discovery](https://arxiv.org/abs/2605.16320)
*Ahmed Elmahdi*

Main category: cs.LG

TL;DR: AdaGraph是一种图原生聚类算法，基于结构中心机器学习范式，解决了高维数据维度灾难问题，在合成和真实数据集上优于传统聚类方法。


<details>
  <summary>Details</summary>
Motivation: 解决高维数据中传统基于距离的几何中心聚类方法面临维度灾难的问题，提出结构中心计算方法以保持有意义的关系结构，适用于任意高维数据。

Method: AdaGraph采用结构中心机器学习范式，完全在kNN图拓扑中操作，利用SLCD原型部署框架进行扩展。该方法不需要预先指定聚类数量k，能原生处理噪声，并使用Graph-SCOPE作为无监督调优指标。

Result: 在10个合成基准测试中，Graph-SCOPE实现平均ARI=0.900，在9/10数据集上正确选择k，性能优于Silhouette、Davies-Bouldin和Calinski-Harabasz指标。在肝癌基因表达分析中发现了其他方法无法解析的基因模块；在自然语言文本聚类中相对HDBSCAN提升62%；在材料科学聚类中在所有三个数据集上获得最高Graph-SCOPE得分。

Conclusion: AdaGraph作为一种基于图结构的聚类算法，通过结构中心机器学习范式有效解决了高维数据中的维度灾难问题，在多个科学领域展现出优于传统几何中心方法的性能，证明了结构中心计算在处理高维复杂数据时的优势。
...
### [135] [Orth-Dion: Eliminating Geometric Mismatch in Distributed Low-Rank Spectral Optimization](https://arxiv.org/abs/2605.16341)
...
TL;DR: 本文分析了Dion低秩梯度压缩方法的收敛问题，发现列归一化导致几何不匹配和额外的√r收敛因子。提出了Orth-Dion方法，用QR正交化替换列归一化，在相同通信成本下达到与精确谱方法相当的收敛性能。


<details>
  <summary>Details</summary>
Motivation: 现有的低秩梯度压缩方法Dion虽然与全分片数据并行训练兼容，但相比全秩谱方法收敛速度较慢。研究者希望理解这种收敛差距的根本原因，并提出改进方法。

Method: 研究对Dion方法进行了深入分析，发现列归一化会导致与Muon优化器隐式目标之间的几何不匹配。理论分析表明这种不匹配会导致收敛速度额外的√r因子。为解决这一问题，提出了Orth-Dion方法，用QR正交化替换列归一化。

Result: 理论分析显示Dion的收敛差距是几何性的，源于列归一化无法产生目标秩-r极性因子。Orth-Dion在非欧几里得光滑性条件下达到O(√(L_r/T))的收敛速率，与精确谱方法匹配同时保持与Dion相同的每步通信成本。大规模语言模型预训练实验验证了预测的√r缩放，并表明Orth-Dion以Dion的通信成本弥合了与Muon的收敛差距。

Conclusion: 本研究提出Orth-Dion方法，通过将Dion方法中的列归一化替换为QR正交化，在保持相同通信成本的同时，实现了与精确谱方法相当的收敛速度。该方法消除了列归一化引入的几何不匹配问题，并通过理论分析和实验验证了其优越性能。
...
### [136] [LoopQ: Quantization for Recursive Transformers](https://arxiv.org/abs/2605.16343)
*Rui Fang,Hsi-Wen Chen,Ming-Syan Chen*

Main category: cs.LG

TL;DR: LoopQ是一个专门为循环语言模型设计的后训练量化框架，通过解决分布偏移、状态重用和递归错误积累这三个核心挑战，显著提升了量化模型的性能，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 循环语言模型通过递归重用Transformer块来提高参数效率，允许在固定模型大小下进行更深入的计算。然而，这种重用使循环语言模型在后训练量化下更加脆弱，存在三个主要挑战：角色间分布偏移、跨循环的状态重用、以及递归错误积累。

Method: 提出了LoopQ，一个循环感知的后训练量化框架，通过以下四个关键技术：1) 激活缩放平衡循环角色间的分布偏移；2) 选择性变换在不同循环阶段应用不同的量化参数；3) 跨循环状态对齐保持循环状态的一致性；4) 轨迹感知优化针对错误积累进行优化。

Result: 在七个基准测试中，与最强的静态后训练量化基线相比，LoopQ在W4A4量化下将平均下游准确率提高了68.8%，并将平均困惑度降低了87.7%。这证明了LoopQ在保持模型深度计算优势的同时，显著提高量化模型质量的有效性。

Conclusion: 循环语言模型的量化面临独特挑战，需要专门的"循环感知"量化方法。LoopQ通过激活缩放、选择性变换、跨循环状态对齐和轨迹感知优化等技术，有效解决了循环语言模型的量化难题，显著提升了量化模型的性能。
...
### [137] [Goal-Conditioned Supervised Learning for LLM Fine-Tuning](https://arxiv.org/abs/2605.16345)
*Shijun Li,Kaiwen Dong,Xiang Gao,Joydeep Ghosh*

Main category: cs.LG

TL;DR: 提出目标条件有监督学习（GCSL）框架，将反馈信号作为显式目标，通过有监督学习让模型生成符合目标的响应。该方法更好地利用分级反馈，避免SFT和经典GCSL的有限学习效应，在多任务上超越现有离线微调基线。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对齐方法存在不足：在线方法成本高且部署困难；离线方法中，SFT将分级反馈简化为二元监督，DPO需要配对偏好数据但此类数据通常难以获取或构建成本高。需要一种更有效利用分级反馈且保留有监督学习效率的离线微调方法。

Method: 提出了目标条件有监督学习（GCSL）框架：1）将反馈信号直接视为显式目标；2）引入创新目标表达方式，将学习定义为持续追求高于目标质量阈值的结果，而非模仿高质量子集样本；3）采用自然语言目标表示以更好地利用LLM的语义理解和推理能力。

Result: 在三个任务上评估：无毒生成、代码生成和LLM推荐。结果表明该方法在所有任务上均持续超越标准的离线微调基线方法，同时保留有监督学习的效率、可扩展性和简单数据要求。

Conclusion: GCSL作为一种离线微调框架，通过将反馈信号直接作为显式目标，并以有监督学习的方式训练模型达成该目标，在保留有监督学习效率的同时，能够更好地利用分级反馈，在多任务上超越了现有离线微调基线方法。
...
### [138] [PropGuard: Safeguarding LLM-MAS via Propagation-Aware Exploration and Remediation](https://arxiv.org/abs/2605.16346)
*Bingyu Yan,Xiaoming Zhang,Jinyu Hou,Chaozhuo Li,Ziyi Zhou,Xiaozhe Zhang,Litian Zhang*

Main category: cs.LG

TL;DR: PropGuard 是一个用于保护 LLM 多智能体系统的传播感知框架，通过双视图时空图分析和源头引导修复机制，有效地检测和修复恶意指令传播，保持系统安全的同时维护协作效率。


<details>
  <summary>Details</summary>
Motivation: LLM 多智能体系统（LLM-MAS）在解决复杂任务方面展现出潜力，但它们之间的交互带来了新的安全风险：恶意指令可能通过消息、工具或记忆在智能体之间传播，导致系统级安全威胁。现有的防御方法主要依赖本地过滤或基于图的异常检测，但这些方法往往无法追踪细粒度的传播路径，或在修复污染状态时不破坏良性协作。

Method: PropGuard 建立了一个双视图时空图，结合了响应中心的风险估计和全状态证据保留。通过风险先验指导，一个经过 GE-GRPO 训练的检测器在全状态图上顺序探索，以恢复紧凑的可疑传播子图。然后通过子图感知的诊断来验证有害传播，并应用源头引导的修复来纠正上游污染并重放受影响的下游交互。

Result: 在四种通信架构和五种攻击场景下的实验表明，PropGuard 能够持续降低攻击成功率，同时保持较高的任务级别防御成功率，实现了效果与效率的良好平衡。

Conclusion: PropGuard 作为一种传播感知的防御框架，通过在组合式诊断和源头引导的修复机制之间取得平衡，有效提升了 LLM-MAS 的安全防护能力，未来可扩展应用于更复杂的开放世界多智能体场景。
...
### [139] [HPC-LLM: Practical Domain Adaptation and Retrieval-Augmented Generation for HPC Support](https://arxiv.org/abs/2605.16347)
...
### [140] [Flow-Direct: Feedback-Efficient and Reusable Guidance for Flow Models via Non-Parametric Guidance Field](https://arxiv.org/abs/2605.16348)
...
### [141] [Geometric Asymmetry in MoE Specialization: Functional Decorrelation and Representational Overlap](https://arxiv.org/abs/2605.16349)
*Feilong Liu*
...
### [142] [Federated Nested Learning: Collaborative Training of Self-Referential Memories for Test-Time Adaptation](https://arxiv.org/abs/2605.16350)
...
### [143] [Augmenting Human Evaluation with LLM Judges: How Many Human Reviews Do You Need?](https://arxiv.org/abs/2605.16354)
*Jane Paik Kim*
...
### [144] [LEAF: A Living Benchmark for Event-Augmented Forecasting](https://arxiv.org/abs/2605.16358)
...
### [145] [ProxyKV: Cross-Model Proxy Pruning for Efficient Long-Context LLM Inference](https://arxiv.org/abs/2605.16360)
...
### [146] [TailedTS: Benchmark Dataset for Heavy-Tailed Time Series Prediction and Periodicity Quantification](https://arxiv.org/abs/2605.16361)
...
### [147] [When Is Rank-1 Steering Cheap? Geometry, Granularity, and Budgeted Search](https://arxiv.org/abs/2605.16362)
...
### [148] [ORACLE: Anticipating Scams from Partial Trajectories in Streaming App Usage](https://arxiv.org/abs/2605.16363)
...
### [149] [Machine Learning-Based Pre-Test Risk Stratification for PCR-Confirmed Chlamydia Using Patient-Reported Data and Urine Biomarkers](https://arxiv.org/abs/2605.16365)
...
### [150] [Lost or Hidden? A Concept-Level Forgetting in Supervised Continual Learning](https://arxiv.org/abs/2605.16374)
...
### [151] [M$^2$FedAQI: Multimodal Federated Learning for Air Quality Prediction on Heterogeneous Edge Devices](https://arxiv.org/abs/2605.16375)
...
### [152] [Mixing Times of Glauber Dynamics on Masked Language Models](https://arxiv.org/abs/2605.16378)
...
### [153] [An Information-Theoretic Criterion for Efficient Data Synthesis](https://arxiv.org/abs/2605.16379)
...
### [154] [ReTAMamba: Reliability-Aware Temporal Aggregation with Mamba for Irregular Clinical Time Series Prediction](https://arxiv.org/abs/2605.16380)
...
### [155] [QuantFPFlow: Quantum Amplitude Estimation for Fokker--Planck Policy Optimisation in Continuous Reinforcement Learning](https://arxiv.org/abs/2605.16429)
...
### [156] [A Theory of Training Profit-Optimal LLMs](https://arxiv.org/abs/2605.16430)
...
### [157] [Edge-AI-Driven Learning-to-Rank for Decentralized Task Allocation in Circular Smart Manufacturing](https://arxiv.org/abs/2605.16433)
...
### [158] [GPU-Accelerated Deep Learning for Heatwave Prediction and Urban Heat Risk Assessment](https://arxiv.org/abs/2605.16435)
*Adis Alihodžić*
...
### [159] [Byzantine-Resilient Federated Learning via QUBO-Based Client Selection on Quantum Annealers](https://arxiv.org/abs/2605.16438)
...
### [160] [DeepArrhythmia: Segment-Contextualized ECG Arrhythmia Classification via Selective Evidence Acquisition](https://arxiv.org/abs/2605.16441)
...
### [161] [Two-Valued Symmetric Circulant Matrices: Applications in Deep Learning](https://arxiv.org/abs/2605.16443)
...
### [162] [Membership Inference Attacks on Discrete Diffusion Language Models](https://arxiv.org/abs/2605.16445)
...
### [163] [Avoiding Structural Failure Modes in Tabular Fair SSL: Online Primal-Dual Allocation under Confidence Gating](https://arxiv.org/abs/2605.16446)
...
### [164] [Nested Spatio-Temporal Time Series Forecasting](https://arxiv.org/abs/2605.16447)
...
### [165] [PESD-TSF: A Period-Aware and Explicit Structured Decomposition Framework for Long-Term Time Series Forecasting](https://arxiv.org/abs/2605.16449)
...
### [166] [Physics-Guided Geometric Diffusion for Macro Placement Generation](https://arxiv.org/abs/2605.16451)
...
### [167] [Peak-Detector: Explainable Peak Detection via Instruction-Tuned Large Language Models in Physiological Sign](https://arxiv.org/abs/2605.16452)
...
### [168] [QuChaTeR: A Hybrid Quantum-Chaotic Temporal Framework for Earthquake Prediction](https://arxiv.org/abs/2605.16454)
*Emir Kaan Özdemir*
...
### [169] [Identifiable Token Correspondence for World Models](https://arxiv.org/abs/2605.16457)
...
### [170] [Seeking the Unfamiliar but Memorable: Conceptual Creativity as Meta-Learning](https://arxiv.org/abs/2605.16477)
*Mengye Ren*
...
### [171] [Global Convergence of Sampling-Based Nonconvex Optimization through Diffusion-Style Smoothing](https://arxiv.org/abs/2605.16520)
...
### [172] [Hypergraph Pattern Machine: Compositional Tokenization for Higher-Order Interactions](https://arxiv.org/abs/2605.16527)
...
### [173] [Multiscale Supervised Unbalanced Optimal Transport Flow Matching](https://arxiv.org/abs/2605.16529)
...
### [174] [Boundedly Rational Meta-Learning in Sequential Consumer Choice](https://arxiv.org/abs/2605.16532)
...
### [175] [Symphony for Speech-to-Text: Supporting Real-Time Medical Voice Interfaces](https://arxiv.org/abs/2605.16545)
...
### [176] [World Model-Enabled Causal Digital Twins for Semantic Communications in Physical AI Systems](https://arxiv.org/abs/2605.16547)
...
### [177] [Automatic Unsupervised Ensemble Outlier Model Selection--Extended Version](https://arxiv.org/abs/2605.16567)
...
### [178] [Wavelet Flow Matching for Multi-Scale Physics Emulation](https://arxiv.org/abs/2605.16573)
...
### [179] [Structure-Aware Masking for Protein Representation Learning](https://arxiv.org/abs/2605.16581)
...
### [180] [How Few-Shot Examples Add Up: A Causal Decomposition of Function Vectors in In-Context Learning](https://arxiv.org/abs/2605.16591)
...
### [181] [Where Pretraining writes and Alignment reads: the asymmetry of Transformer weight space](https://arxiv.org/abs/2605.16600)
...
### [182] [R2V Agent: Teaching SLMs When to Ask for Help](https://arxiv.org/abs/2605.16604)
...
### [183] [To MRL or not to MRL: Text Embeddings are Robust to Truncation Without Matryoshka Embeddings, Except In Heavy Truncation Scenarios](https://arxiv.org/abs/2605.16608)
...
### [184] [Tensor Cookbook: Mastering Tensors through Diagrams](https://arxiv.org/abs/2605.16610)
...
### [185] [Learning What Evaluators Value: A Reliable Approach to Modeling Evaluator Preferences](https://arxiv.org/abs/2605.16615)
...
### [186] [MLReplicate: Benchmarking Autonomous Research Systems for Machine Learning Reproducibility](https://arxiv.org/abs/2605.16616)
...
### [187] [SCOUT: Cyclic Causal Discovery Under Soft Interventions with Unknown Targets](https://arxiv.org/abs/2605.16620)
...
### [188] [Does Weight Decay Enhance Training Stability?](https://arxiv.org/abs/2605.16622)
...
### [189] [Learning How to Cube](https://arxiv.org/abs/2605.16632)
...
### [190] [MedMIX: Modality-Internal Expert Fusion for Multimodal Medical Diagnosis](https://arxiv.org/abs/2605.16639)
...
### [191] [Provably Shorter Scratchpads in Hybrid DeltaNet-Attention Decoders](https://arxiv.org/abs/2605.16640)
*Tomasz Steifer*
...
### [192] [In-context learning enables continental-scale subsurface temperature prediction from sparse local observations](https://arxiv.org/abs/2605.16665)
...
### [193] [GraViti: Graph-Level Variational Autoencoders with Relaxed Permutation Invariance](https://arxiv.org/abs/2605.16668)
...
### [194] [Identify Then Project: Contrastive Learning of Latent Dynamics from Partial Observations with Port-Hamiltonian Structure](https://arxiv.org/abs/2605.16682)
...
### [195] [Scalable Knowledge Editing for Mixture-of-Experts LLMs via Tensor-Structured Updates](https://arxiv.org/abs/2605.16686)
...
### [196] [EfficientTDMPC: Improved MPC Objectives for Sample-Efficient Continuous Control](https://arxiv.org/abs/2605.16692)
...
### [197] [Your SaaS Is an Insurance Product: A Modeling Framework](https://arxiv.org/abs/2605.16699)
*Caio Gomes*
...
### [198] [Convex Dataset Valuation for Post-Training](https://arxiv.org/abs/2605.16704)
...
### [199] [Isolating Nonlinear Independent Sources in fMRI with $β$-TCVAE Models](https://arxiv.org/abs/2605.16708)
...
### [200] [EmoMind: Decoding Affective Captions from Human Brain fMRI](https://arxiv.org/abs/2605.16739)
...
### [201] [Propagation of Chaos in Contextual Flow Maps](https://arxiv.org/abs/2605.16747)
...
### [202] [Learning Unbiased Permutations via Flow Matching](https://arxiv.org/abs/2605.16755)
*Yimeng Min,Carla P. Gomes*
...
### [203] [Distinguishable Deletion: Unifying Knowledge Erasure and Refusal for Large Language Model Unlearning](https://arxiv.org/abs/2605.16776)
...
### [204] [Lever: Speculative LLM Inference on Smartphones](https://arxiv.org/abs/2605.16786)
...
### [205] [The Unlearnability Phenomenon in RLVR for Language Models](https://arxiv.org/abs/2605.16787)
...
### [206] [TIER: Trajectory-Invariant Execution Rewards for Multi-Step Tool Composition](https://arxiv.org/abs/2605.16790)
...
### [207] [PULSE: Generative Phase Evolution for Non-Stationary Time Series Forecasting](https://arxiv.org/abs/2605.16793)
...
### [208] [Cross-Domain Molecular Relational Learning: Leveraging Chemical Structure-Activity Analysis](https://arxiv.org/abs/2605.16799)
...
### [209] [FIM-LoRA: Task-Informative Rank Allocation for LoRA via Calibration-Time Gradient-Variance Estimation](https://arxiv.org/abs/2605.16800)
...
### [210] [Cross-modal Affinity-aligned Multimodal Learning Analytics for Predicting Student Collaboration Satisfaction in Game-Based Learning](https://arxiv.org/abs/2605.16806)
...
### [211] [Informative Graph Structure Learning](https://arxiv.org/abs/2605.16809)
...
### [212] [Jacobian-Guided Anisotropic Noise Reshaping for Enhancing Representation Utility under Local Differential Privacy](https://arxiv.org/abs/2605.16812)
...
### [213] [Decoupling KL and Trajectories: A Unified Perspective for SFT, DAgger, Offline RL, and OPD in LLM Distillation](https://arxiv.org/abs/2605.16826)
...
### [214] [PhysioSeq2Seq: A Hybrid Physiological Digital Twin and Sequence-to-Sequence LSTM for Long-Horizon Glucose Forecasting in Type 1 Diabetes](https://arxiv.org/abs/2605.16860)
...
### [215] [SE-GA: Memory-Augmented Self-Evolution for GUI Agents](https://arxiv.org/abs/2605.16883)
...
### [216] [Tensor Channel Equivariant Graph Neural Networks for Molecular Polarizability Prediction](https://arxiv.org/abs/2605.16891)
...
### [217] [ArtifactLinker: Linking Scientific Artifacts for Automatic State-of-the-Art Discovery](https://arxiv.org/abs/2605.16902)
...
### [218] [AIM: Adversarial Information Masking for Faithfulness Evaluation of Saliency Maps](https://arxiv.org/abs/2605.16905)
...
### [219] [Extending Pretrained 10-Second ECG Foundation Models to Longer Horizons](https://arxiv.org/abs/2605.16975)
...
### [220] [Decision-Aware Proximal Bridge Learning for Optimal Treatment Selection](https://arxiv.org/abs/2605.16989)
...
### [221] [Ranking-Aware Calibration for Reliable Multimodal Reinforcement Learning](https://arxiv.org/abs/2605.16999)
...
### [222] [BoLT: A Benchmark to Democratize Black-box Optimization Research for Expensive LLM Tasks](https://arxiv.org/abs/2605.17000)
...
### [223] [When Dynamics Shift, Robust Task Inference Wins: Offline Imitation Learning with Behavior Foundation Models Revisited](https://arxiv.org/abs/2605.17017)
...
### [224] [Why Do Reasoning Models Lose Coverage? The Role of Data and Forks in the Road](https://arxiv.org/abs/2605.17026)
...
### [225] [Privacy Policy Enforcement Guardrails for Data-Sensitive Retrieval-Augmented Generation](https://arxiv.org/abs/2605.17034)
...
### [226] [D$^2$Evo: Dual Difficulty-Aware Self-Evolution for Data-Efficient Reinforcement Learning](https://arxiv.org/abs/2605.17037)
...
### [227] [Privacy-Preserving Generation Fraud Detection for Distributed Photovoltaic Systems: A Solar Irradiance-Fused Federated Learning Framework](https://arxiv.org/abs/2605.17039)
...
### [228] [1GC-7RC: One Graphic Card -- Seven Research Challenges! How Good Are AI Agents at Doing Your Job?](https://arxiv.org/abs/2605.17046)
...
### [229] [S-Bus: Automatic Read-Set Reconstruction for Multi-Agent LLM State Coordination](https://arxiv.org/abs/2605.17076)
*Sajjad Khan*
...
### [230] [Scale Determines Whether Language Models Organize Representation Geometry for Prediction](https://arxiv.org/abs/2605.17084)
*Weilun Xu*
...
### [231] [Mechanism Learning: Prototype-Anchored Mechanism Inference for Scientific Forecasting](https://arxiv.org/abs/2605.17091)
...
### [232] [Parallel Recursive LSTM](https://arxiv.org/abs/2605.17108)
...
### [233] [DynMuon: A Dynamic Spectral Shaping View of Muon](https://arxiv.org/abs/2605.17109)
...
### [234] [Differentiable Optimization Layers for Guaranteed Fairness in Deep Learning](https://arxiv.org/abs/2605.17118)
...
### [235] [An Analytical Multiple Criteria Framework for Temporal and Dynamic Business-to-Business Customer Segmentation in Manufacturing](https://arxiv.org/abs/2605.17151)
...
### [236] [Stress-Testing Neural Network Verifiers with Provably Robust Instances](https://arxiv.org/abs/2605.17153)
...
### [237] [When Bits Break Recourse: Counterfactual-Faithful Quantization](https://arxiv.org/abs/2605.17160)
...
### [238] [TriAxialKV: Toward Extreme Low-Precision KV-Cache Quantization for Agentic Inference Tasks](https://arxiv.org/abs/2605.17170)
...
### [239] [OpenJarvis: Personal AI, On Personal Devices](https://arxiv.org/abs/2605.17172)
...
### [240] [The Geometry of Projection Heads: Conditioning, Invariance, and Collapse](https://arxiv.org/abs/2605.17180)
*Faris Chaudhry*
...
### [241] [OPTNet: Ordering Point Transformer Network for Post-disaster 3D Semantic Segmentation](https://arxiv.org/abs/2605.17197)
...
### [242] [Anytime and Difficulty-Adaptive PAC-Bayes for Constrained Density-Ratio Network with Continual Learning Guarantees](https://arxiv.org/abs/2605.17212)
*Paulo Akira F. Enabe*
...
### [243] [FishBack: Pullback Fisher Geometry for Optimal Activation Steering in Transformers](https://arxiv.org/abs/2605.17231)
...
### [244] [Dimension-Free Convergence of Discrete Diffusion Models: Adjoint Equations Induce the Right Space](https://arxiv.org/abs/2605.17232)
...
### [245] [Active Budget Allocation for Efficient Scaling Law Estimation via Surrogate-Guided Pruning](https://arxiv.org/abs/2605.17234)
...
### [246] [Learning in Position-Aware Multinomial Logit Bandits: From Multiplicative to General Position Effects](https://arxiv.org/abs/2605.17238)
...
### [247] [Drift Flow Matching](https://arxiv.org/abs/2605.17244)
...
### [248] [Fidelity Probes for Specification--Code Alignment](https://arxiv.org/abs/2605.17246)
...
### [249] [Towards Principled Test-Time Adaptation for Time Series Forecasting](https://arxiv.org/abs/2605.17250)
...
### [250] [When Molecular Similarity Works: Property Cliffs Reveal Hidden Errors](https://arxiv.org/abs/2605.17265)
...
### [251] [Calibeating for general proper losses: A Bregman divergence approach](https://arxiv.org/abs/2605.17269)
...
### [252] [State-of-the-Art Claims Require State-of-the-Art Evidence](https://arxiv.org/abs/2605.17273)
*YongKyung Oh*
...
### [253] [How Do Electrocardiogram Models Scale?](https://arxiv.org/abs/2605.17276)
...
### [254] [UNR-Explainer: Counterfactual Explanations for Unsupervised Node Representation Learning Models](https://arxiv.org/abs/2605.17285)
...
### [255] [LEAP: Learnable End-to-End Adaptive Pruning of Large Language Models](https://arxiv.org/abs/2605.17289)
...
### [256] [Step-wise Rubric Rewards for LLM Reasoning](https://arxiv.org/abs/2605.17291)
...
### [257] [DISA: Offline Importance Sampling for Distribution-Matching LLM-RL](https://arxiv.org/abs/2605.17295)
...
### [258] [Learning Higher-Order Structure from Incomplete Spatiotemporal Data: Multi-Scale Hypergraph Laplacians with Neural Refinement](https://arxiv.org/abs/2605.17316)
...
### [259] [Leveraging Error Diversity in Group Rollouts for Reinforcement Learning](https://arxiv.org/abs/2605.17333)
...
### [260] [Bridging the Gap between Sparse Matrix Reordering and Factorization: A Deep Learning Framework for Fill-in Reduction](https://arxiv.org/abs/2605.17339)
...
### [261] [Olivia: Harmonizing Time Series Foundation Models with Power Spectral Density](https://arxiv.org/abs/2605.17340)
...
### [262] [\textsc{MasFACT}: Continual Multi-Agent Topology Learning via Geometry-Aware Posterior Transfer](https://arxiv.org/abs/2605.17361)
...
### [263] [Learning Fill-in Reduction Ordering via Graph Policy Optimization for Sparse Matrices](https://arxiv.org/abs/2605.17362)
...
### [264] [FML-bench: A Controlled Study of AI Research Agent Strategies from the Perspective of Search Dynamics](https://arxiv.org/abs/2605.17373)
...
### [265] [Self-Supervised Learning for Sparse Matrix Reordering](https://arxiv.org/abs/2605.17403)
...
### [266] [IVF-TQ: Streaming-Robust Approximate Nearest Neighbor Search via a Codebook-Free Residual Layer](https://arxiv.org/abs/2605.17415)
*Tarun Sharma*
...
### [267] [Learning Displacement-Robust Representations for Landslide Early Warning under Rainfall Forecast Uncertainty](https://arxiv.org/abs/2605.17419)
...
### [268] [Progressive Generalization Augmentation with Deeply Coupled RND-PPO and Domain-Prioritized Noise Injection for Robust Crop Management Reinforcement Learning](https://arxiv.org/abs/2605.17428)
*Wu Yang*
...
### [269] [Radial-Angular Geometry for Reliable Update Diagnosis in Noisy-Label Learning](https://arxiv.org/abs/2605.17429)
...
### [270] [MATE: Solving Contextual Markov Decision Processes with Memory of Accumulated Transition Embeddings](https://arxiv.org/abs/2605.17431)
...
### [271] [DP-SelFT: Differentially Private Selective Fine-Tuning for Large Language Models](https://arxiv.org/abs/2605.17432)
...
### [272] [ClaHF: A Human Feedback-inspired Reinforcement Learning Framework for Improving Classification Tasks](https://arxiv.org/abs/2605.17458)
...
### [273] [TriOpt: A Scalable Algorithm for Linear Causal Discovery](https://arxiv.org/abs/2605.17465)
...
### [274] [WinQ: Accelerating Quantization-Aware Training of Language Models Around Saddle Points](https://arxiv.org/abs/2605.17471)
...
### [275] [Beyond Linear Superposition: Discovering Climate Features in AI Weather Models with KAN-SAE](https://arxiv.org/abs/2605.17493)
*Minjong Cheon*
...
### [276] [Self-Supervised On-Policy Distillation for Reasoning Language Models](https://arxiv.org/abs/2605.17497)
...
### [277] [t-gems: text-guided exit modules for decreasing clip image encoder](https://arxiv.org/abs/2605.17499)
...
### [278] [The Silent Brush: Evaluating Artistic Style Leakage in AI Art Generation](https://arxiv.org/abs/2605.17500)
...
### [279] [BESplit: Bias-Compensated Split Federated Learning with Evidential Aggregation](https://arxiv.org/abs/2605.17508)
...
### [280] [Coordinate Heterogeneity Governs Binary Quantization: From InfoNCE to Recall](https://arxiv.org/abs/2605.17524)
*Wenxuan Xiao*
...
### [281] [CasualSynth: Generating Structurally Sound Synthetic Data](https://arxiv.org/abs/2605.17528)
...
### [282] [Q-LocalAdam: Memory-Efficient Client-Side Adaptive Optimization for Edge Federated Learning](https://arxiv.org/abs/2605.17552)
...
### [283] [PFlow-T: A Persistence-Driven Forward Process for Topology-Controlled Generation](https://arxiv.org/abs/2605.17555)
...
### [284] [Beyond Accuracy: Robustness, Interpretability and Expressiveness of EEG Foundation Models](https://arxiv.org/abs/2605.17562)
...
### [285] [Structured Neural Marked Point Processes for Interpretable Event Interaction Modeling](https://arxiv.org/abs/2605.17568)
...
### [286] [How Off-Policy Can GRPO Be? Mu-GRPO for Efficient LLM Reinforcement Learning](https://arxiv.org/abs/2605.17570)
...
### [287] [UniAlign: A Model-Agnostic Framework for Robust Network Traffic Classification under Distribution Shifts](https://arxiv.org/abs/2605.17575)
...
### [288] [Scale-Equivariant Generative Forecasting: Weight-Tied Dilated Convolutions, Wavelet Scattering Inputs, and Spectral-Consistency Training for Self-Similar Time Series](https://arxiv.org/abs/2605.17582)
*Andrea Morandi*
...
### [289] [Form and Function: Machine Unlearning as a Problem of Misaligned States](https://arxiv.org/abs/2605.17590)
*Kennon Stewart*
...
### [290] [Venom: A PyTorch Generative Modeling Toolkit](https://arxiv.org/abs/2605.17605)
*Liang Yan*
...
### [291] [The Neural Tangent Kernel for Classification](https://arxiv.org/abs/2605.17606)
...
### [292] [Adaptive Generate-Rank-Verify: Inference-Time Search with Costly Verification](https://arxiv.org/abs/2605.17609)
...
### [293] [Verifier-Guided Code Translation via Meta-Step Decoding](https://arxiv.org/abs/2605.17626)
...
### [294] [TabKDE: Simple and Scalable Tabular Data Generation with Kernel Density Estimates](https://arxiv.org/abs/2605.17642)
...
### [295] [Counterfactual Explanations Under Concept Drift](https://arxiv.org/abs/2605.17651)
...
### [296] [When a Zero-Shooter Cheats: Improving Age Estimation via Activation Steering](https://arxiv.org/abs/2605.17658)
...
### [297] [Bug or Feature$^2$: Weight Drift, Activation Sparsity, and Spikes](https://arxiv.org/abs/2605.17659)
...
### [298] [PEIRA: Learning Predictive Encoders through Inter-View Regressor Alignment](https://arxiv.org/abs/2605.17671)
...
### [299] [Exact Convex Reformulations of Linear Neural Networks via Completely Positive Lifting](https://arxiv.org/abs/2605.17692)
...
### [300] [Fine-tuning Pocket-Aware Diffusion Models via Denoising Policy Optimization](https://arxiv.org/abs/2605.17693)
...
### [301] [Agent Bazaar: Enabling Economic Alignment in Multi-Agent Marketplaces](https://arxiv.org/abs/2605.17698)
...
### [302] [Toy Combinatorial Interpretability Models Reveal Lottery Tickets in Early Feature Space](https://arxiv.org/abs/2605.17704)
...
### [303] [L-Drive: Beyond a Single Mapping-Latent Context Drives Time Series Forecasting](https://arxiv.org/abs/2605.17730)
...
### [304] [Testable and Actionable Calibration for Full Swap Regret](https://arxiv.org/abs/2605.17749)
...
### [305] [OSCAR: Offline Spectral Covariance-Aware Rotation for 2-bit KV Cache Quantization](https://arxiv.org/abs/2605.17757)
...
### [306] [Memisis: Orchestrating and Evaluating Synthetic Data for Tabular Health Datasets](https://arxiv.org/abs/2605.17758)
...
### [307] [AURORA: Contextual Orthogonalization for Geometric Representation Learning in Healthcare Foundation Models](https://arxiv.org/abs/2605.17765)
...
### [308] [Learning Variable-Length Tokenization for Generative Recommendation](https://arxiv.org/abs/2605.17779)
...
### [309] [Revisiting the Adam-SGD Gap in LLM Pre-Training: The Role of Large Effective Learning Rates](https://arxiv.org/abs/2605.17787)
...
### [310] [HydroAgent: Closing the Gap Between Frontier LLMs and Human Experts in Hydrologic Model Calibration via Simulator-Grounded RL](https://arxiv.org/abs/2605.17792)
...
### [311] [When Accuracy Is Not Enough: Uncertainty Collapse between Noisy Label Learning and Out-of-Distribution Detection](https://arxiv.org/abs/2605.17795)
...
### [312] [GenTS: A Comprehensive Benchmark Library for Generative Time Series Models](https://arxiv.org/abs/2605.17804)
...
### [313] [AMO: Adaptive Muon Orthogonalization](https://arxiv.org/abs/2605.17806)
...
### [314] [A Unified Framework for Data-Free One-Step Sampling via Wasserstein Gradient Flows](https://arxiv.org/abs/2605.17808)
...
### [315] [One Model, Two Roles: Emergent Specialization in a Shared Recurrent Transformer](https://arxiv.org/abs/2605.17811)
...
### [316] [Content-Style Identification via Differential Independence](https://arxiv.org/abs/2605.17827)
...
### [317] [Agentic Cost-Aware Query Planning with Knowledge Distillation for Big Data Analytics](https://arxiv.org/abs/2605.17831)
...
### [318] [Efficient Bilevel Optimization for Meta Label Correction in Noisy Label Learning](https://arxiv.org/abs/2605.17833)
...
### [319] [Balancing Knowledge Distillation for Imbalance Learning with Bilevel Optimization](https://arxiv.org/abs/2605.17839)
...
### [320] [SNLP: Layer-Parallel Inference via Structured Newton Corrections](https://arxiv.org/abs/2605.17842)
...
### [321] [Learning over Positive and Negative Edges with Contrastive Message Passing](https://arxiv.org/abs/2605.17854)
...
### [322] [$\boldsymbol{f}$-OPD: Stabilizing Long-Horizon On-Policy Distillation with Freshness-Aware Control](https://arxiv.org/abs/2605.17862)
...
### [323] [DAD4TS: Data-Augmentation-Oriented Diffusion Model for Time-Series Forecasting with Small-Scale Data](https://arxiv.org/abs/2605.17866)
...
### [324] [HINT-SD: Targeted Hindsight Self-Distillation for Long-Horizon Agents](https://arxiv.org/abs/2605.17873)
...
### [325] [CoX-MoE: Coalesced Expert Execution for High-Throughput MoE Inference with AMX-Enabled CPU-GPU Co-Execution](https://arxiv.org/abs/2605.17889)
...
### [326] [Lightweight Gaussian Process Inference in C++ on Metal and CUDA](https://arxiv.org/abs/2605.17898)
*Yu-Hsueh Fang*
...
### [327] [DCFold: Efficient Protein Structure Generation with Single Forward Pass](https://arxiv.org/abs/2605.17899)
...
### [328] [Domain Transfer Becomes Identifiable via a Single Alignment](https://arxiv.org/abs/2605.17918)
...
### [329] [InfoFlow: A Framework for Multi-Layer Transformer Analysis](https://arxiv.org/abs/2605.17930)
...
### [330] [Training data attribution in diffusion models via mirrored unlearning and noise-consistent skew](https://arxiv.org/abs/2605.17938)
...
### [331] [Enhancing the Code Reasoning Capabilities of LLMs via Consistency-based Reinforcement Learning](https://arxiv.org/abs/2605.17958)
...
### [332] [Function graph transformers universally approximate operators between function spaces](https://arxiv.org/abs/2605.17968)
...
### [333] [SAFE-SVD: Sensitivity-Aware Fidelity-Enforcing SVD for Physics Foundation Models](https://arxiv.org/abs/2605.17985)
...
### [334] [MARR: Module-Adaptive Residual Reconstruction for Low-Bit Post-Training Quantization](https://arxiv.org/abs/2605.17997)
...
### [335] [RL4RLA: Teaching ML to Discover Randomized Linear Algebra Algorithms Through Curriculum Design and Graph-Based Search](https://arxiv.org/abs/2605.18004)
...
### [336] [Scalable Decision-Focused Learning through Cost-Sensitive Regression](https://arxiv.org/abs/2605.18005)
...
### [337] [Uncertainty Reliability Under Domain Shift: An Investigation for Data-Driven Blood Pressure Estimation in Photoplethysmography](https://arxiv.org/abs/2605.18008)
...
### [338] [LogRouter: Adaptive Two-Level LLM Routing for Log Question Answering in Big Data Systems](https://arxiv.org/abs/2605.18015)
...
### [339] [Federated Learning by Utility-Constrained Stochastic Aggregation for Improving Rational Participation](https://arxiv.org/abs/2605.18020)
...
### [340] [Unveiling Memorization-Generalization Coexistence: A Case Study on Arithmetic Tasks with Label Noise](https://arxiv.org/abs/2605.18022)
*Linyu Liu,Pinyan Lu*
...
### [341] [Interaction-Breaking Adversarial Learning Framework for Robust Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.18024)
...
### [342] [FedSDR: Federated Self-Distillation with Rectification](https://arxiv.org/abs/2605.18028)
...
### [343] [Protection Is (Nearly) All You Need: Structural Protection Dominates Scoring in Globally Capped KV Eviction](https://arxiv.org/abs/2605.18053)
*Gabriel Garcia*
...
### [344] [FLAG: Foundation model representation with Latent diffusion Alignment via Graph for spatial gene expression prediction](https://arxiv.org/abs/2605.18055)
...
### [345] [Improving Spatio-Temporal Residual Error Propagation by Mitigating Over-Squashing](https://arxiv.org/abs/2605.18068)
...
### [346] [Equilibrium Selection in Multi-Agent Policy Gradients via Opponent-Aware Basin Entry](https://arxiv.org/abs/2605.18078)
...
### [347] [The Expressive Power of Low Precision Softmax Transformers with (Summarized) Chain-of-Thought](https://arxiv.org/abs/2605.18079)
...
### [348] [pyforce-1.0.0: Python Framework for data-driven model Order Reduction of multi-physiCs problEms](https://arxiv.org/abs/2605.18082)
...
### [349] [Elastic-dLLM: Position Preserving Context Compression and Augmentation of Diffusion LLMs](https://arxiv.org/abs/2605.18165)
...
### [350] [Ringmaster LMO: Asynchronous Linear Minimization Oracle Momentum Method](https://arxiv.org/abs/2605.18174)
...
### [351] [UTOPYA: A Multimodal Deep Learning Framework for Physics-Informed Anomaly Detection and Time-Series Prediction](https://arxiv.org/abs/2605.18188)
...
### [352] [Dual-Rate Diffusion: Accelerating diffusion models with an interleaved heavy-light network](https://arxiv.org/abs/2605.18190)
...
### [353] [Concise and Logically Consistent Conformal Sets for Neuro-Symbolic Concept-Based Models](https://arxiv.org/abs/2605.18202)
...
### [354] [A Simplex Witness Certificate for Constant Collapse in Variational Autoencoders](https://arxiv.org/abs/2605.18224)
...
### [355] [Are Sparse Autoencoder Benchmarks Reliable?](https://arxiv.org/abs/2605.18229)
*David Chanin*
...
### [356] [Attacking the First-Principle: A Black-Box, Query-Free Targeted Mimicry Attack on Binary Function Classifiers](https://arxiv.org/abs/2605.18231)
...
### [357] [Privacy Preserving Reinforcement Learning with One-Sided Feedback](https://arxiv.org/abs/2605.18246)
...
### [358] [Temporal Task Diversity: Inductive Biases Under Non-Stationarity in Synthetic Sequence Modelling](https://arxiv.org/abs/2605.18281)
...
### [359] [PH-Dreamer: A Physics-Driven World Model via Port-Hamiltonian Generative Dynamics](https://arxiv.org/abs/2605.18303)
...
### [360] [Alignment Dynamics in LLM Fine-Tuning](https://arxiv.org/abs/2605.18309)
...
### [361] [Dynamic Elliptical Graph Factor Models via Riemannian Optimization with Geodesic Temporal Regularization](https://arxiv.org/abs/2605.18316)
...
### [362] [The Symmetries of Three-Layer ReLU Networks](https://arxiv.org/abs/2605.18319)
...
### [363] [ISEP: Implicit Support Expansion for Offline Reinforcement Learning via Stochastic Policy Optimization](https://arxiv.org/abs/2605.18320)
...
### [364] [Prune, Update and Trim: Robust Structured Pruning for Large Language Models](https://arxiv.org/abs/2605.18331)
...
### [365] [Decoupled Conformal Optimisation: Efficient Prediction Sets via Independent Tuning and Calibration](https://arxiv.org/abs/2605.18354)
...
### [366] [Proximal basin hopping: global optimization with guarantees](https://arxiv.org/abs/2605.18364)
...
### [367] [Beyond Inference-Time Search: Reinforcement Learning Synthesizes Reusable Solvers](https://arxiv.org/abs/2605.18374)
...
### [368] [Graph Hierarchical Recurrence for Long-Range Generalization](https://arxiv.org/abs/2605.18387)
...
### [369] [Beyond Square Roots: Explicit Memory-Efficient Factorization for Multi-Epoch Private Learning](https://arxiv.org/abs/2605.18379)
...
### [370] [Modelling Customer Trajectories with Reinforcement Learning for Practical Retail Insights](https://arxiv.org/abs/2605.18449)
...
### [371] [Generating Physically Consistent Molecules with Energy-Based Models](https://arxiv.org/abs/2605.18381)
...
### [372] [Scheduling That Speaks: An Interpretable Programmatic Reinforcement Learning Framework](https://arxiv.org/abs/2605.18454)
...
### [373] [TabH2O: A Unified Foundation Model for Tabular Prediction](https://arxiv.org/abs/2605.18383)
...
### [374] [GAMMA: Global Bit Allocation for Mixed-Precision Models under Arbitrary Budgets](https://arxiv.org/abs/2605.18475)
...
### [375] [Modality vs. Morphology: A Framework for Time Series Classification for Biological Signals](https://arxiv.org/abs/2605.18483)
...
### [376] [Spherical Harmonic Optimal Transport: Application to Climate Models Comparisons](https://arxiv.org/abs/2605.18389)
...
### [377] [DBES: A Systematic Benchmark and Metric Suite for Evaluating Expert Specialization in Large-Scale MoEs](https://arxiv.org/abs/2605.18498)
...
### [378] [Generative Adversarial Learning from Deterministic Processes](https://arxiv.org/abs/2605.18425)
...
### [379] [DiPRL: Learning Discrete Programmatic Policies via Architecture Entropy Regularization](https://arxiv.org/abs/2605.18508)
*Chengpeng Hu,Yingqian Zhang,Hendrik Baier*
...
### [380] [Text2CAD-Bench: A Benchmark for LLM-based Text-to-Parametric CAD Generation](https://arxiv.org/abs/2605.18430)
...
### [381] [Probing for Representation Manifolds in Superposition](https://arxiv.org/abs/2605.18537)
*Alexander Modell*
...
### [382] [Heterogeneous Tasks Offloading in Vehicular Edge Computing: A Federated Meta Deep Reinforcement Learning Approach](https://arxiv.org/abs/2605.18437)
...
### [383] [Randomized Advantage Transformation (RAT): Computing Natural Policy Gradients via Direct Backpropagation](https://arxiv.org/abs/2605.18591)
*Mingfei Sun*
...
### [384] [AMARIS: A Memory-Augmented Rubric Improvement System for Rubric-Based Reinforcement Learning](https://arxiv.org/abs/2605.18592)
...
### [385] [Stochastic Penalty-Barrier Methods for Constrained Machine Learning](https://arxiv.org/abs/2605.18618)
...
### [386] [Adaptive Experimentation for Censored Survival Outcomes](https://arxiv.org/abs/2605.18459)
...
### [387] [Position: Weight Space Should Be a First-Class Generative AI Modality](https://arxiv.org/abs/2605.18632)
...
### [388] [Data Presentation Over Architecture: Resampling Strategies for Credit Risk Prediction with Tabular Foundation Models](https://arxiv.org/abs/2605.18635)
...
### [389] [Post-Trained MoE Can Skip Half Experts via Self-Distillation](https://arxiv.org/abs/2605.18643)
...
### [390] [An Assessment of Human vs. Model Uncertainty in Soft-Label Learning and Calibration](https://arxiv.org/abs/2605.18648)
...
### [391] [Pocket Foundation Models: Distilling TFMs into CPU-Ready Gradient-Boosted Trees](https://arxiv.org/abs/2605.18654)
...
### [392] [Offline Contextual Bandits in the Presence of New Actions](https://arxiv.org/abs/2605.18509)
...
### [393] [XCTFormer: Leveraging Cross-Channel and Cross-Time Dependencies for Enhanced Time-Series Analysis](https://arxiv.org/abs/2605.18534)
...
### [394] [COOPO: Cyclic Offline-Online Policy Optimization Algorithm](https://arxiv.org/abs/2605.18675)
...
### [395] [Beyond Scaling: Agents Are Heading to the Edge](https://arxiv.org/abs/2605.18535)
...
### [396] [Ensembling Tabular Foundation Models - A Diversity Ceiling And A Calibration Trap](https://arxiv.org/abs/2605.18696)
...
### [397] [Distilling Tabular Foundation Models for Structured Health Data](https://arxiv.org/abs/2605.18702)
*Aditya Tanna,Nassim Bouarour,Mohamed Bouadi,Vinay Kumar Sankarapu,Pratinav Seth*
...
### [409] [General Preference Reinforcement Learning](https://arxiv.org/abs/2605.18721)
