...
TL;DR: 论文指出基于扰动的结构有效性审计本身是脆弱的，其结论可能因不可见的实施细节而被静默制造，作者通过自我审计展示了五类管道故障，并提出六点尽职调查门控作为补充性证据协议


<details>
  <summary>Details</summary>
Motivation: 治理框架要求AI提供商和审计师提供记录在案的评估证据，基于扰动的结构有效性审计是这种证据的常见形式。然而，作者认为这些审计本身是脆弱的：它们的结论可能因读者在报告数字中看不到的实施细节而被静默制造

Method: 作者定义了五类管道故障（F1-F5），并在安全基准测试和开源权重指令调优模型上进行了自我审计。使用统一的六点尽职调查门控评估，每个单元格都落在非确认性桶中，没有单元格达到确认性状态

Result: 在单一的双模型、五基准案例研究中，所有评估单元格都被归类到非确认性类别，没有一个达到确认性标准。这验证了审计过程本身的脆弱性

Conclusion: 作者将尽职调查门控定位为保障级证据的保留和披露协议，是补充（而非替代）经典结构有效性证据的机制，而不是达成基准有效性结论的途径。该分类是说明性的非穷尽起点，而非审计失败的全面划分
...
TL;DR: 该研究探讨时间序列基础模型在电力价格预测中的表现，提出双数据集基准框架，发现TSFMs具有竞争力但与领域特定方法结合效果更佳。


<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型在零样本预测中表现良好，但其在协变量驱动、非平稳环境中的泛化能力尚未充分探索。电力价格预测由于复杂时序依赖、分布偏移和对结构信息的强依赖而成为具有挑战性的测试平台。

Method: 提出双数据集基准框架以减轻污染风险并实现公平评估。考察电力价格预测的关键方面，包括点预测和概率预测性能、尾部行为、价格尖峰，并与领域特定方法进行系统比较。

Result: 时间序列基础模型非常有竞争力，通常优于通用基准方法，但性能关键依赖协变量支持，并不总是超越专门为电力价格预测定制的领域特定方法。有趣的是，TSFMs与领域特定方法的简单集成显示显著潜力，表明这两种方法捕捉了互补的预测信息。

Conclusion: 时间序列基础模型在电力价格预测中具有竞争力，但与领域特定方法结合可以进一步提升预测性能，两种方法具有互补性，集成方法显示出重要潜力。
...
TL;DR: QuantFlow是一个概率预测框架，结合倒置序列嵌入、双向Mamba状态空间解码器、分位数回归和联邦学习，用于时间序列预测，支持隐私保护和不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 当前的时间序列预测基础模型多依赖于集中化数据和Transformer注意力机制，这限制了其在处理长序列、高维度数据和隐私敏感信号时的应用。

Method: QuantFlow采用倒置序列嵌入将每个变量在整个观测窗口内嵌入，使用双向Mamba状态空间解码器进行前向和反向处理，并通过分位数回归输出五个条件分位数。TSMixup方法通过Dirichlet加权的插值扩展时间多样性，同时保留序列结构。框架还结合联邦学习，无需集中原始数据。

Result: 在加密货币、交通、电力、变压器温度、流感和天气等多个数据集上的实验显示，QuantFlow在ETTm1数据集上获得0.2834的均方误差，在Weather数据集上获得0.2218的均方误差。在20个客户端的非独立同分布部署中，经过三个通信轮次后仍保持有用准确率。

Conclusion: 选择性状态空间建模为扩展性、不确定性感知和隐私保护的时间序列预测提供了有前景的基础。同时研究也揭示了该方法在处理非规则流行病学信号和长时域泛化方面的局限性。
...
TL;DR: GRAFT是一种基于神经编解码语言模型的逐词发音调节机制，通过短语音样本改善复杂词汇的发音准确性，在盲听实验和客观测试中表现优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 现有文本到语音系统虽然能达到较高的可理解性和自然度，但存在文本歧义问题，特别是对罕见专有名词、外来词和技术术语的发音不准确。即使是基于音素的模型也缺乏对逐词发音的直接声学控制机制。

Method: GRAFT采用逐词发音条件调节机制，通过短语音样本编码的语音标记绑定到文本提示中的词汇位置。训练过程中使用语音转换技术分离提示说话人和目标说话人的声音特征。

Result: 在盲听实验中，人类评测者明显优先选择GRAFT生成的语音。在五种语言的客观基准测试中，GRAFT将目标词汇的音素错误率降低了22-39%，优于其他开源零样本系统，同时在说话人相似性和自然度方面表现良好。

Conclusion: GRAFT是一种有效的逐词发音调节机制，能够在文本到语音系统中显著改善专有名词、外来词和技术术语等复杂词汇的发音准确性，同时保持语音的自然度和说话人相似性。
...
TL;DR: 应用联邦学习实现隐私保护的分布式无人机目标检测，在不集中数据的情况下获得接近集中训练的效果，显著优于单无人机训练


<details>
  <summary>Details</summary>
Motivation: 传统目标检测模型训练需要集中化处理无人机采集的航拍图像，这会引发隐私、监管、存储和带宽等问题，尤其在分布式无人机部署场景中，将视觉数据传输到集中式基础设施通常不切实际或不理想

Method: 应用联邦学习于目标检测任务，使用Sherpa.ai FL平台在KIIT-MiTA数据集上实现联邦目标检测流程，并与单无人机训练和集中式训练基线进行比较，采用mAP@0.50和mAP@0.50:0.95作为评估指标

Result: 提出的联邦学习方法在保持接近集中式训练性能的同时，显著超越单无人机训练。性能最佳的轻量级模型（YOLO26 nano）在mAP@0.50和mAP@0.50:0.95上分别获得了52.89%和67.80%的相对提升

Conclusion: 联邦学习在保持训练效果接近集中式训练的同时，显著优于单无人机训练，能够在不需要数据集中化的前提下，实现可扩展、高性能且保护隐私的分布式无人机群目标检测
...
TL;DR: 针对现代生成器的结构偏差（过度产生规范模式而缺少类内变化），提出一种保真度-多样性选择方法，通过选择信息子集显著提高合成数据效用


<details>
  <summary>Details</summary>
Motivation: 现有方法需要训练/微调生成器或使用轻量级后处理适配，这些方法既依赖于特定生成器又需要专业知识。本文研究在固定生成图像池中，仅通过选择信息子集是否能提高下游效用

Method: 将每个真实类别分为规范同质(HO)子集和非冗余异质(HE)子集，通过保真度-多样性准则对合成图像评分，奖励语义对齐同时惩罚规范冗余

Result: 该方法在多个基准测试中始终优于最先进的数据选择基线方法，最多可减少40%合成样本量实现与真实数据相当的性能。该准则在更强任务调优生成器上仍然有效，在分类和分割任务上都有提升

Conclusion: 后生成选择不是更好生成器的替代品，而是提高合成数据效用的补充机制
...
TL;DR: 本研究开发了一个粒度感知的EEG特征流水线，用于预测儿科精神病理学的四个维度（p因子、内化、外化和注意力问题）。通过多尺度特征组织和平衡选择策略，在儿童脑网络中验证了方法的可行性，结果显示信号较弱但可检测，粒度感知选择可作为有效的特征降维策略。


<details>
  <summary>Details</summary>
Motivation: 脑电图（EEG）为研究维度性精神病理学的神经生理相关性提供了非侵入性方法，但现有研究在EEG范式和特征粒度方面的系统证据仍然有限。需要开发系统性的方法来评估不同粒度EEG特征在预测精神病理学维度方面的效果。

Method: 1. 开发了粒度感知的EEG特征流水线，将多尺度描述符组织为全局、区域和通道三个层次。2. 使用健康脑网络（HBN）队列数据，评估四个EEG范式（范式未具体说明）对四个精神病理学维度的预测能力。3. 采用基于树的模型和粒度平衡特征选择方法。4. 在独立的PEARL队列上进行探索性跨数据集验证。

Result: 1. 与常规方法相比，树基模型和粒度平衡特征选择在特定条件下显示出有前景的改进，但效应量仍较为有限。2. 所选标记物的可视化显示出与维度相关的特定空间和频谱模式，这些模式与现有神经生理学知识基本一致。3. 在独立PEARL队列上的探索性验证表明，所提出的选择原则在协议变化下技术上仍然可行（但不声称跨数据集泛化能力）。

Conclusion: 多尺度EEG特征包含与维度性精神病理学相关的微弱但可检测的信号。粒度感知选择可以作为未来基于EEG的表型研究中有用的特征降维策略。该研究设置代表了一个具有挑战性的可行性测试，而非临床筛查场景。
...
TL;DR: Lifting Neural Operator (LiNO)是一种基于第二代小波提升方案的多分辨率神经算子，能够自适应地从数据中学习多尺度分解，在多个偏微分方程基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的神经算子在同时捕捉全局动力学和细尺度结构方面存在困难，需要一种能够有效表示多尺度特征的分层多尺度分解框架。

Method: 基于第二代小波提升方案构建多分辨率算子(LiNO)，通过参数化提升变换直接从数据中学习多分辨率分解。这种提升变换对底层解函数具有自适应性，且构造上完全可逆，实现了信息保持的多尺度算子学习。

Result: 在Darcy流、Poisson方程、Allen-Cahn方程、可压缩Navier-Stokes方程和Gray-Scott反应-扩散系统等多个基准测试中，LiNO相比现有最先进的神经算子表现出更强的性能。

Conclusion: 自适应多分辨率算子为科学机器学习提供了一个有前景的方向，能够有效处理多尺度现象、输运主导动力学和混沌系统等广泛的物理行为。
...
TL;DR: 本文研究了电动汽车动力系统热波动的实验室到现场转换问题，采用集成批量预测区间(EnbPI)方法处理自相关时间序列的不确定性边界，并通过密度比加权改进在真实协变量偏移下的覆盖性能，最终将校准模型应用于真实F1遥测数据作为无监督分布外诊断。


<details>
  <summary>Details</summary>
Motivation: 高性能电动汽车动力系统的热波动预测困难，因为内部温度在实验室外很少可观测，且基于实验室驱动周期校准的模型在真实负载下会失效。这构成了实验室到现场的转换问题，需要在不完全可观测条件下提供可靠的不确定性估计。

Method: 采用集成批量预测区间(EnbPI)方法，这是一种留一法引导集成的一致性预测方法，专门处理自相关时间序列。使用真实CALCE锂离子循环数据(A123 SP20电池，FUDS配置文件)进行校准。为应对协变量偏移，引入加权EnbPI程序，结合EnbPI的集成残差和密度比加权(Tibshirani等人，2019)，通过概率域分类器估计密度比。

Result: 在真实CALCE测试条件(US06高速公路驾驶计划，45°C)下的评估显示，未加权的EnbPI边界从分布内的95.00%名义覆盖率下降到70.13%。加权EnbPI将覆盖率恢复到72.42%，虽然有改进但不够理想。将校准模型应用于2023年F1遥测数据(蒙扎和银石赛道，车手VER)，由于没有内部热通道，仅报告无监督标记率(蒙扎65.6%，银石58.0%，远高于5%的分布内基准率)，并发现标记与制动/DRS区域之间关联不一致。

Conclusion: 一致性域适应是解决该问题的有前景但仅部分有效的工具。研究表明，在真实协变量偏移下，未经调整的一致性预测覆盖性能显著下降，而加权方法提供了有限改进。应用真实F1遥测数据作为无监督诊断显示高标记率，但缺乏标签限制了明确结论。该方法在某些情况下有效，但仍有局限性，需要进一步研究。
...
TL;DR: 研究人工智能在资源问题上的风险厌恶训练能否作为安全防护机制，发现低风险训练下的风险厌恶能够部分泛化到极高风险场景，但尚未达到可靠的安全防护水平


<details>
  <summary>Details</summary>
Motivation: 研究者希望通过训练AI在资源问题上具备风险厌恶特性，为AI可能出现的错位问题提供安全防护机制。即使AI错位，如果它们风险厌恶，会更倾向于选择低风险、低回报的策略（如合作），而不是高风险、高回报的策略（如反抗），从而限制错位带来的负面影响

Method: 研究者创建了RiskAverseOOD基准来测量风险厌恶在分布外环境下的泛化能力。他们使用多种方法（SFT、tie训练、DPO、激活导向等）训练Qwen3-8B模型在低风险情况下做出风险厌恶选择，然后测试这些模型在极高风险情况下的表现

Result: 研究发现低风险训练获得的风险厌恶能够在98个数量级的范围内部分泛化到极高风险场景。从基线2%的安全选择率提升到约70%（SFT和tie训练）、52%（DPO）和39%（激活导向）。微调的奖励模型能够可靠地给风险厌恶推理打出高于风险中性或过度风险厌恶替代方案的分数（99.6%配对准确率）。这些效应在不同规模模型（Qwen3-1.7B和Qwen3-14B）和不同模型家族（Gemma-3-12B-IT和Llama-3.1-8B-Instruct）中得到了复现

Conclusion: 在低风险环境下学习的风险厌恶能够泛化到分布外的极高风险场景，但尚不足以作为可靠的故障安全机制。达到那种水平的可靠性是一个待解决的开放性问题
...
TL;DR: LARA（拉格朗日奖励增强）是一个在安全约束下进行推理时间对齐的通用框架，通过将约束问题转化为一维凸优化，使用校准的对偶变量增强奖励信号，改善了有用性与无害性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有推理时间对齐方法通常只优化单一标量分数，要么忽略显式安全约束，要么通过手动调整的惩罚项来编码约束，缺乏系统的方法来处理安全约束。

Method: LARA从基于KL正则化的约束目标出发，包含奖励模型和成本模型。它将约束对偶化，将优化问题简化为一个一维凸问题。在小校准集上估计对偶变量，该变量定义了可增强奖励，可作为现有推理时间对齐方法的即插即用评分信号。

Result: LARA在序列级和令牌级推理时间对齐方法上都进行了评估，发现它能改善有用性与无害性之间的权衡。其中Best-of-N重排序在推理时间方法中表现最佳，接近基于微调的直接对齐基线。

Conclusion: LARA提供了一个通用的推理时间对齐框架，能够在安全约束下有效运行，无需重复的权重更新，通过系统化的对偶变量校准方法处理约束，优于现有的手动调整惩罚方法。
...
TL;DR: 本研究分析了transformer中的induction heads机制，发现在k阶马尔可夫链上训练的transformer通过两种平滑机制实现上下文学习：软上下文匹配估计器和BOS令牌诱导的加性伪计数，揭示了transformer学会正则化上下文估计而非简单计数。


<details>
  <summary>Details</summary>
Motivation: Induction heads被认为是transformer中支持上下文学习的核心注意力机制，但其实现的估计器缺乏精确的数学描述，需要对其工作机制进行深入分析。

Method: 研究在k阶马尔可夫链上训练的transformer，识别两种互补的平滑机制，并构建解耦的transformer来验证这些机制。

Result: 发现了两种平滑机制：1)有限注意力权重下的软上下文匹配估计器，实现类Jelinek-Mercer平滑；2)BOS令牌诱导加性伪计数，实现Dirichlet风格平滑。训练后的transformer恢复了预测的注意力模式，并在多种设置中优于经典计数基线。

Conclusion: 研究将induction heads的机制解释性与经典统计平滑理论联系起来，揭示了transformer学习正则化上下文估计的能力，而不仅仅是进行简单的计数操作，为理解transformer的上下文学习机制提供了新的理论框架。
...
TL;DR: 本文提出BDLM Mamba-注意力混合架构，解决了现有混合扩散模型中反向Mamba扫描无法跨块精确缓存的问题，实现了长上下文生成的高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的高吞吐量长上下文生成是一个核心挑战。当前方法存在技术限制：减少内存访问的高效注意力变体与线性时间混频器如Mamba，以及通过一次生成多个令牌来增加并行计算的方法，在结合时面临技术挑战。先前混合扩散模型如DiffuMamba中的双向Mamba混频包含相对于因果生成的反向扫描，这需要扫描整个序列，其状态不是前缀特有的，无法在块扩散时精确缓存。

Method: 提出BDLM Mamba-注意力混合架构，通过将反向Mamba扫描限制在去噪块内，实现了跨块的精确缓存。该架构在长上下文生成中结合了Mamba的高效性和注意力的能力。

Result: 在87M参数的DCLM实验中，BDLM Mamba-H取得了最佳的C4-en验证困惑度，优于BDLM注意力和全序列基线。在350M参数下仍与BDLM注意力保持竞争力。对于长上下文推理，BDLM Mamba-H在65K令牌下达到全序列DiffuMamba-H吞吐量的19.7倍，在262K令牌下达到BDLM注意力吞吐量的3.7倍。

Conclusion: 通过将反向Mamba扫描限制在去噪块内，BDLM Mamba-H混合架构实现了跨块的精确缓存，解决了混合扩散模型中的关键技术挑战，证明了Mamba混合架构是潜在的长上下文扩散架构，能够显著提高长上下文生成的吞吐量。
...
TL;DR: Sparse-Reslim：一种参数免费的路由模块，通过稀疏令牌处理（仅25%空间令牌通过昂贵的中层Transformer块）减少天气预测模型的冗余计算，同时保持全网格表示，显著提升速度（2.5-3.18倍）和内存效率（2.2倍），同时提高预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于ViT的天气预测模型在所有空间令牌上应用统一计算，但邻近的大气网格点通常包含相似值，大区域随时间平滑演变，导致大量中间计算冗余。标准令牌效率方法（如剪枝或合并）虽能降低成本，但天气预测是时空密集预测问题，每个网格单元必须保留物理上有意义的表示，特别是在自回归展开中。

Method: 引入Sparse-Reslim参数免费路由模块：1）仅路由25%的空间令牌通过昂贵的中层Transformer块；2）将这些块视为残差更新：计算路由令牌的变化，并将此增量散布回完整序列；3）未选中的令牌保持其预路由表示不变，确保没有网格单元被丢弃或替换为掩码令牌，无需融合层或额外参数。

Result: 在ERA5分辨率高达0.25°操作标准下，对两种模型系列（确定性Transformer和扩散模型）进行评估：1）在每个评估变量上提高预测精度；2）训练速度在主设置中约快2.5倍，0.25°分辨率下达到3.18倍加速；3）峰值内存降低超过2.2倍。控制分解表明精度提升主要来自稀疏路由本身，随机令牌选择提供额外正则化效益而无选择器开销。

Conclusion: Sparse-Reslim通过稀疏令牌处理使天气预测模型兼容固定网格要求，在保持所有网格单元物理有意义表示的同时，显著减少冗余计算，实现了成本降低和预测精度提升的双重优势，为天气预测模型提供了有效的计算效率解决方案。
...
TL;DR: 本文研究了离散扩散模型在分子优化中的在线适应问题，提出了包含采集、奖励塑造、去偏、回放和有效性控制的实用优化方案，在有限评估预算下显著提升了分子优化性能。


<details>
  <summary>Details</summary>
Motivation: 分子优化通常从预训练的生成模型开始，该模型捕获了有效的分子结构先验。但在测试时，目标不是从该先验中采样，而是使用有限的评估预算将生成转向特定任务的高奖励分子。本文研究了离散扩散模型的这种适应问题。

Method: 1. 研究了在线适应循环中的关键选择：候选评估选择、奖励模型更新、反馈重用、距离预训练先验的偏移程度
2. 在六个小分子结合亲和力任务和三个蛋白质适应性任务上进行控制研究
3. 提出了包含采集函数、奖励塑造、模型去偏、回放机制和有效性控制的完整在线适应方案

Result: 1. 采集、奖励塑造和模型去偏为获得更高奖励提供了互补途径，尤其对小分子
2. 回放进一步稳定了学习过程
3. 有效性惩罚确保了小分子探索保持在有效的分子流形上
4. 提出的优化方案在相同的评估预算和GPU时间下，优于离线微调和推理时间搜索基线
5. 当高奖励候选需要更大的预训练先验偏移时，收益最大

Conclusion: 本文提出了一个实用的反馈高效分子优化方案：包含采集、奖励塑造、去偏、回放和有效性控制的在线微调。该方案在有限评估预算下显著提升了性能，特别适用于需要较大偏离预训练先验的高奖励分子生成。
...
TL;DR: 提出一种无需模型反转的数据无标签元学习方法，通过预训练模型为无标签数据分配软标签构建元训练任务，并通过任务加权机制提升性能，相比现有方法获得显著速度和精度提升。


<details>
  <summary>Details</summary>
Motivation: 现实应用中获取有标签数据成本高或存在隐私限制，数据无标签元学习（DFML）可以解决这个问题，但现有DFML方法依赖模型反转生成训练数据，这一过程通常困难且计算昂贵。

Method: 提出新颖的元学习设置，避免模型反转，联合利用预训练模型和无标签数据：1）通过预训练模型为无标签数据分配软标签来生成元训练任务；2）引入基于任务置信度和类别分布平衡的任务加权机制，确保有效的元学习。

Result: 大量实验表明，该方法显著降低计算成本并提高泛化能力：相比最先进的DFML方法，实现最高104倍的加速，并在少样本分类精度上获得8.4%到36.4%的改进。

Conclusion: 该方法有效解决了传统DFML方法中模型反转的效率和难度问题，通过预训练模型和无标签数据的联合利用，提供了一种高效、实用的数据无标签元学习解决方案。
...
TL;DR: 本研究提出了一种不确定性感知强化学习框架，通过在金融交易中整合多种不确定性估计方法，显著提升RL模型在动态市场环境中的适应性和性能。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习模型在金融交易中难以适应高度不确定的市场环境，面对随机波动、模型局限性和市场状态突变等挑战时，常导致次优交易决策。金融市场的高度不确定性需要更鲁棒的RL方法来应对动态变化。

Method: 提出一个整合分布不确定性、认知不确定性和随机不确定性估计的RL框架，采用SHAP加权重构不确定性、MC Dropout和基于LSTM的技术指标共识机制来增强不确定性估计能力。

Result: 在五个主要美国股指上的实验结果表明，配备不确定性估计的RL代理在收益和风险管理方面显著优于传统模型，验证了不确定性感知框架的有效性。

Conclusion: 该研究推动了强化学习在金融交易中的不确定性估计方法，为动态市场环境提供了更鲁棒的解决方案，未来可扩展到其他资产类别和替代RL架构以获得更大适应性。
...
### [38] [Spectral Rewiring for Exploration, Purification, and Model Merging](https://arxiv.org/abs/2607.03065)
...
### [39] [Robustness Meets Uncertainty: Evidential Adversarial Training for Robust Selective Classification](https://arxiv.org/abs/2607.03075)
...
### [40] [STELLA: Efficient Sensor-to-LLM Translation for On-Device Human Activity Recognition](https://arxiv.org/abs/2607.03089)
...
### [41] [Stacked LoRA for Subject-Adaptive EEG Foundation Models in Motor Imagery Decoding](https://arxiv.org/abs/2607.03094)
...
### [42] [Heterogeneous Graph Condensation via Role-Aware Clustering](https://arxiv.org/abs/2607.03097)
...
### [43] [Observable- and Positional-Encoding-Dependent Symmetry Readout from Neural Network Weights](https://arxiv.org/abs/2607.03108)
...
### [44] [Integrating Physics-Informed Neural Networks for Safe Reinforcement Learning in a 1-DoF Helicopter System](https://arxiv.org/abs/2607.03125)
...
### [45] [ACPO: Adaptive Credit Policy Optimization via Fine-Grained Surrogate Entropy](https://arxiv.org/abs/2607.03126)
...
### [46] [Anticipatory Reinforcement Learning for Trajectory Tracking](https://arxiv.org/abs/2607.03132)
...
### [47] [Sample-Efficient Pareto Front Modeling for Energy-Aware Reinforcement Learning Using Bayesian Optimization](https://arxiv.org/abs/2607.03140)
*Georg Schäfer,Jakob Rehrl,Stefan Huber,Simon Hirlaender*
...
### [48] [CuBAS: Information Geometric Curvature-Based Adaptive Sampling for Supervised Classification](https://arxiv.org/abs/2607.03145)
...
### [49] [Rethinking Neural Nonlinearity as Gating](https://arxiv.org/abs/2607.03148)
...
### [50] [Decentralised Federated Learning over Temporal Networks: The Role of Heterogeneities](https://arxiv.org/abs/2607.03171)
...
### [51] [Understanding electricity consumption behaviour through Inverse Reinforcement Learning](https://arxiv.org/abs/2607.03176)
...
### [52] [A Bayesian Framework for Evaluating Scenario Compatibility in Generative Population Synthesis](https://arxiv.org/abs/2607.03190)
...
### [53] [Reduced-Order Models: The Mother of World Models](https://arxiv.org/abs/2607.03198)
*Rajat Ghosh*
...
### [54] [OpFlow: Learning Opportunity-Conditioned Choice Potentials for Robust OD Flow Prediction](https://arxiv.org/abs/2607.03200)
...
### [55] [Transition Information Density: Morphological Trajectories, Synesthetic Perception, and Structured Interpolation in Neural Training (or: The Synesthetic AI)](https://arxiv.org/abs/2607.03210)
*Sam Mao*
...
### [56] [Joint distribution of upstream runoff governs downstream river-discharge prediction uncertainty in distributed ML models](https://arxiv.org/abs/2607.03217)
...
### [57] [PhenoNEST: A Neuro-Symbolic Framework for Ontology-Aware Multimodal Plant Phenotyping and Trait Discovery](https://arxiv.org/abs/2607.03245)
...
### [58] [Unbiased Alignment for Large Language Models with Noisy Preferences](https://arxiv.org/abs/2607.03248)
...
### [59] [From Global to Local: Efficient Regional Weather Downscaling with Global Weather Foundation Model](https://arxiv.org/abs/2607.03279)
...
### [60] [A harmonised dataset for Earth system foundation models](https://arxiv.org/abs/2607.03298)
...
### [61] [Statistically Meaningful Geometry (SMG) Beyond the Euclidean Paradigm, with Application to Generative AI](https://arxiv.org/abs/2607.03329)
...
### [62] [FedAvg for HAR: Exploring the Tradeoff Between Personalized and Generalization Accuracy](https://arxiv.org/abs/2607.03334)
...
### [63] [CSympNet-ID: conformal-symplectic map learning for linearly damped Hamiltonian systems](https://arxiv.org/abs/2607.03339)
...
### [64] [The Multiscale Single-Index Model: A Stylized Model for Hierarchical Feature Learning](https://arxiv.org/abs/2607.03347)
*Joan Bruna*
...
### [65] [PedestrianDiffusion: Multimodal Generative Denoising and Dense State Estimation for Inertial Navigation](https://arxiv.org/abs/2607.03349)
...
### [66] [Amortising Bayesian Experimental Design for Sequential Information Gathering in LLMs](https://arxiv.org/abs/2607.03426)
...
### [67] [How Much of the Routing Gap Is Real? Decomposing the Router-to-Oracle Gap into Reproducible Specialist Advantage and Single-Draw Label Noise](https://arxiv.org/abs/2607.03436)
*Teng-Ruei Chen*
...
### [68] [No Time Like the Present: Agentic Test-Time Training for LLM Agents](https://arxiv.org/abs/2607.03441)
...
### [69] [Best-of-Better-$N$: Generating Pre-Aligned Responses with In-Context Learning](https://arxiv.org/abs/2607.03453)
...
### [70] [Towards Diverse and Comprehensive Benchmarks for Mutual Information Estimation](https://arxiv.org/abs/2607.03487)
...
### [71] [A Near-Linear-Time Solver for Graph $p$-Laplacian Semi-Supervised Learning via Continuation in $p$](https://arxiv.org/abs/2607.03503)
*Oren E. Livne*
...
### [72] [Mixture-of-Gaussians-Guided Schedule Design for Brownian Bridge Diffusion Models](https://arxiv.org/abs/2607.03517)
...
### [73] [On the Convergence of Adam, Revisited](https://arxiv.org/abs/2607.03519)
...
### [74] [Co-Adaptive Multi-Task LoRA: Transfer-Aware, Label-Free Control of Domain Participation](https://arxiv.org/abs/2607.03522)
...
### [75] [Aligning Language Models with Selective Prediction](https://arxiv.org/abs/2607.03528)
...
### [76] [WeightCLIP: Aligning Datasets and Models for Weight Space Learning](https://arxiv.org/abs/2607.03551)
...
### [77] [Teacher Supervision over Representation Equivalence Classes](https://arxiv.org/abs/2607.03572)
*Sang Il Han*
...
### [79] [When Geometry Aligns: Dihedral Hidden-State Transformations in UNet, ViT, and DiT Architectures](https://arxiv.org/abs/2607.03580)
...
### [80] [Modular Foundation Models for Time-Series Perception in Digital Twins](https://arxiv.org/abs/2607.03585)
...
### [81] [Graph Classification via Network Usable Information: From Representation Evaluation to Structure Selection](https://arxiv.org/abs/2607.03587)
...
### [82] [Implicit Bias of SGD in Multivariate ReLU Networks: Effective Width Collapse](https://arxiv.org/abs/2607.03613)
...
### [83] [Reflected Schrödinger Bridge Matching](https://arxiv.org/abs/2607.03626)
...
### [84] [LLM-Guided Transportation Hub Capacity Planning with Textual Business Inputs](https://arxiv.org/abs/2607.03651)
...
### [85] [Phase-Preserving Trimodal Transformer for Tropical Forest Biomass Estimation Using Optical and PolInSAR Data](https://arxiv.org/abs/2607.03663)
...
### [86] [A Structural Interpretation of GELU and Threshold-Transmission Activations via the First-Order Loss Function](https://arxiv.org/abs/2607.03664)
*Roberto Rossi*
...
### [87] [Validation-Induced Shapley Shifts: How Validation Structure Distorts Data Valuation](https://arxiv.org/abs/2607.03675)
...
### [88] [Rethinking AI-Generated Text Detection: A Strong Baseline and the Distribution-Shift Problem That Remains](https://arxiv.org/abs/2607.03680)
...
### [89] [PIEFS: Physics-Informed Eigenfunction Features with Learnable Scaling](https://arxiv.org/abs/2607.03692)
...
### [90] [Social Networks of LLM Agents](https://arxiv.org/abs/2607.03695)
...
### [91] [SAVER: Stochastic Adaptive Variance-Driven Exploration and Reconstruction for Low-Dose Computed Tomography](https://arxiv.org/abs/2607.03761)
...
### [92] [FedACT: Federated Adaptive Coordinate Trust Modulation for Robust Transformer Training under Data Heterogeneity](https://arxiv.org/abs/2607.03763)
...
### [93] [Conservative Subject Invariant EMG-based Gesture Recognition](https://arxiv.org/abs/2607.03783)
...
### [94] [Tensor-Train Joint Modeling for Few-Step Discrete Diffusion](https://arxiv.org/abs/2607.03788)
...
### [95] [Foundations of Equivariant Deep Learning: Unifying Graph and Sheaf Neural Networks](https://arxiv.org/abs/2607.03798)
*Yoshihiro Maruyama*
...
### [96] [Punching Above Their Weight: Classification-Head Fine-Tuning of Tiny Language Models (TLMs) for Verifiable Multiple-Choice Tasks](https://arxiv.org/abs/2607.03801)
...
### [97] [Stable Global Weighting of Flow Mixtures using Simplex Exponential Moving Average](https://arxiv.org/abs/2607.03809)
...
### [98] [Adversarial LassoNet: Robust Feature Selection via Stability-Driven Sparse Learning](https://arxiv.org/abs/2607.03839)
...
### [99] [NeSy-CSA: A Neuro-Symbolic Framework for Open-Ended Critical Scenario Attribution](https://arxiv.org/abs/2607.03847)
...
### [100] [A Unified Framework for Quantized and Continuous Strong Lottery Tickets](https://arxiv.org/abs/2607.03860)
...
### [101] [A Gradient Flow Perspective on Minimum MMD Estimation](https://arxiv.org/abs/2607.03871)
...
### [102] [AdaptiveSD A Stability-Aware, Runtime-Adaptive Speculative Decoding Framework with Multi-Policy Orchestration for CPU-Constrained LLM Inference](https://arxiv.org/abs/2607.03876)
*Sadra Saremi*
...
### [103] [CDCP: Conditional Diffusion Model with Contextual Prompts for Multi-task Offline Safe Reinforcement Learning](https://arxiv.org/abs/2607.03903)
...
### [104] [Transformers with Physics-Informed Encodings and Simulation-Based Inference for Robust Detection of Eccentric Binary Black Holes in Pulsar Timing Array Data](https://arxiv.org/abs/2607.03904)
...
### [105] [NeuroOnline: Bridging Pretraining and Online Adaptation for EEG Foundation Models](https://arxiv.org/abs/2607.03925)
...
### [106] [MPSelectTune: Prompt-type Selection for Fine-tuning improves Concept Unlearning in LLMs](https://arxiv.org/abs/2607.03932)
...
### [107] [Order-based Causal Discovery for Multistage Processes](https://arxiv.org/abs/2607.03971)
...
### [108] [MANCE: Manifold Aware Concept Erasure](https://arxiv.org/abs/2607.03973)
...
### [109] [Knowing When to Stop: Predicting Execution-Consistency Convergence in Text-to-SQL](https://arxiv.org/abs/2607.03991)
...
### [110] [Directional Curvature from Armijo Backtracking: A Low-Cost Sharpness Probe and a Calibration-Free Learning-Rate Safeguard for Adam](https://arxiv.org/abs/2607.03998)
...
### [111] [When Does Small Data Work? Accuracy and Efficiency Trade-offs Between Tabular Foundation Models and Conventional Methods for Crowd-State Classification at Hajj and Umrah](https://arxiv.org/abs/2607.04013)
...
### [112] [A Unified Algebraic Framework for Classification Performance Evaluation](https://arxiv.org/abs/2607.04028)
*Ronaldo C. Prati*
...
### [113] [OmniOpt: Taxonomy, Geometry, and Benchmarking of Modern Optimizers](https://arxiv.org/abs/2607.04033)
...
### [114] [Reward-Gated On-Policy Distillation](https://arxiv.org/abs/2607.04037)
...
### [115] [A Unified Framework for In-Context Learning with Causal and Masked Language Models](https://arxiv.org/abs/2607.04081)
...
### [116] [FedSPM: Routing-Enabled Federated Learning under Dual Heterogeneity via Semiparametric Mixture](https://arxiv.org/abs/2607.04085)
...
### [117] [Target-Aware Interaction-Guided Reinforcement Learning for Black-Box Node Injection Attacks on Graph Neural Networks](https://arxiv.org/abs/2607.04091)
*Yi Lan,Ye Yuan*
...
### [118] [Dictionaries, Not Darwin: Set-Level Selection Beats LLM Evolution in Scientific Equation Discovery](https://arxiv.org/abs/2607.04108)
*Pan Li*
...
### [119] [DynaVieW: Schema-Guided World Modeling for Understanding Hierarchical Visual Dynamics](https://arxiv.org/abs/2607.04112)
...
### [120] [Asymptotic-Preserving A Posteriori Analysis of Diffusion and Flow-Matching Samplers](https://arxiv.org/abs/2607.04113)
*Shiheng Zhang*
...
### [121] [GlacierCastAI: Predicting Glacier Retreat from Multi-Modal Satellite Imagery and Climate Signals](https://arxiv.org/abs/2607.04117)
...
### [122] [Parametric Memory Decoding for Zero-Shot Routing in LoRA-Based External Parametric Memory](https://arxiv.org/abs/2607.04118)
...
### [123] [CertMix: Certified, Data-Efficient Metamaterial Design by Affine Mixing of Aligned Neural-Implicit Weight Spaces](https://arxiv.org/abs/2607.04123)
*Yifan Wang*
...
### [124] [MDL Meets Latent Confounders: LNML-based Causal Discovery](https://arxiv.org/abs/2607.04133)
...
### [125] [Masked Generative-Contrastive Representation Learning for Cross-Dataset EEG-Based Emotion Recognition](https://arxiv.org/abs/2607.04139)
...
### [126] [Binary Iterative Method for Non-targeted Adversarial Attack](https://arxiv.org/abs/2607.04145)
...
### [127] [Mask-based Predictive Representations for Reinforcement Learning](https://arxiv.org/abs/2607.04153)
*Kai Zhao*
...
### [128] [Geometry of Ordinal Representations in Language Models](https://arxiv.org/abs/2607.04167)
...
### [129] [FedFFT: Taming Client Drift in Federated SAM via Spectral Perturbation Filtering](https://arxiv.org/abs/2607.04170)
...
### [130] [Physics-Informed Graph Learning with Uncertainty Awareness for Open-Set Domain Generalization in Fault Diagnosis](https://arxiv.org/abs/2607.04188)
...
### [131] [SpecGradFilter: A Spectral Gradient Filtering Framework for Taming Federated Heterogeneity](https://arxiv.org/abs/2607.04189)
...
### [132] [Exploring Convolutional Neural Processes for Weather Downscaling](https://arxiv.org/abs/2607.04190)
*Francisco Passos*
...
### [133] [Channel-Adaptive Robust Aggregation for Over-the-Air Federated Learning in Heterogeneous Networks](https://arxiv.org/abs/2607.04218)
...
### [134] [Quantize the Target, Quantize the Drafter: Efficient Inference with Qwen3.5-4B](https://arxiv.org/abs/2607.04244)
...
### [135] [Signal or Noise? Understanding Generative Models for Real-World Sensor Time Series](https://arxiv.org/abs/2607.04245)
...
### [136] [On Preserving Geometrical Invariance for Superpixel Image Classification using Graph Transformer](https://arxiv.org/abs/2607.04262)
...
### [137] [HiFA4: Training-Free 4-bit FlashAttention on Ascend HIF4 NPUs for LLM Inference](https://arxiv.org/abs/2607.04302)
...
### [138] [SAD-LoRA: Spectral Alignment for Low-Rank Knowledge Distillation](https://arxiv.org/abs/2607.04306)
...
### [139] [On the effectiveness of reward functions in reinforcement learning for confidence calibration of large language models](https://arxiv.org/abs/2607.04332)
...
### [140] [Structure-Specific Representational Priors Causally Control the Grokking Delay](https://arxiv.org/abs/2607.04333)
*Gunner Levi Howe*
...
### [141] [One Framework for All: Cross-Modal Membership Inference for Generative Models](https://arxiv.org/abs/2607.04339)
...
### [142] [How Many Initial Points Does Bayesian Optimization Need?](https://arxiv.org/abs/2607.04356)
...
### [143] [RL Forgets! Towards Continual Policy Optimization](https://arxiv.org/abs/2607.04364)
...
### [144] [NKI-Agent: Domain-Specific Fine-Tuning and Agentic Tool Use for Neuron Kernel Generation](https://arxiv.org/abs/2607.04395)
...
### [145] [Learning Task-Sufficient World Models by Synergizing Agentic Exploration and Structured Modeling](https://arxiv.org/abs/2607.04409)
...
### [146] [Environmental Drivers of Respiratory Disease: A District Level Analysis](https://arxiv.org/abs/2607.04416)
...
### [147] [Full-Stack FP4: Stable LLM Pretraining with Quantized Projections, Optimizers, and Attention](https://arxiv.org/abs/2607.04422)
...
### [148] [Covert Trait Propagation Is Representation Alignment: Mechanistic Evidence from Hidden-Channel Distillation](https://arxiv.org/abs/2607.04432)
...
### [149] [Knowledge-Informed Local Causal Discovery of Optimal Adjustment Sets](https://arxiv.org/abs/2607.04447)
...
### [150] [A Deep Learning-based surrogate model for Severe Accidents in nuclear reactors using ASTEC](https://arxiv.org/abs/2607.04450)
...
### [151] [Operator-on-F complements value-equivalence: a planning-time diagnostic for latent world models](https://arxiv.org/abs/2607.04464)
*Donna Vakalis*
...
### [152] [Regime-Conditional Stabilisation of LLM-Augmented Cooperative Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2607.04470)
...
### [153] [LeukocyteCount: Automatic Identification and Counting for leukocytes using Deep Learning](https://arxiv.org/abs/2607.04486)
...
### [154] [Two Black Boxes, One Solver: Encoder Probing and Decoder Attribution for Neural Multi-Attribute VRP under Hard-Mask and Recourse Decoders](https://arxiv.org/abs/2607.04487)
*Sohaib Afifi*
...
### [155] [Lyapunov-Guided Training for Hardware-Safe Neural Networks Under Fixed-Point Arithmetic](https://arxiv.org/abs/2607.04531)
...
### [157] [Auto: The AGI Compiler](https://arxiv.org/abs/2607.04542)
...
### [158] [Predicting Therapeutic Outcome via Aligning Patient-Specific Knowledge Graph and Gene-Level Perturbation Representations](https://arxiv.org/abs/2607.04557)
...
### [159] [ManifoldFlow: SPD-Relaxed Stiefel Layers with Learnable Singular Spectrum](https://arxiv.org/abs/2607.04535)
...
### [160] [A Few Teacher Steps Go a Long Way: Cost-Efficient On-Policy Data Augmentation for Agent Post-Training](https://arxiv.org/abs/2607.04574)
...
### [161] [Machine Learning for Depression Screening and Intervention: an Original Circadian Rhythm Score-based Methodology](https://arxiv.org/abs/2607.04648)
...
### [162] [URSA: Chemistry-Aware Benchmark for Utilitarian Retrosynthesis Assessment](https://arxiv.org/abs/2607.04688)
...
### [163] [RSPO: Reward-Swap Policy Optimization for Multi-Turn LLM Agents](https://arxiv.org/abs/2607.04713)
...
### [164] [Beyond the Need for Speed: Energy-Aware Code Generation via Simulation-Guided Reinforcement Learning](https://arxiv.org/abs/2607.04577)
...
### [165] [Trust Region Policy Distillation](https://arxiv.org/abs/2607.04751)
...
### [166] [Score Distributions, Not Cells: Evaluating Single-Cell Perturbations Under Class Overlap](https://arxiv.org/abs/2607.04595)
...
### [167] [Multi-Turn On-Policy Distillation with Prefix Replay](https://arxiv.org/abs/2607.04763)
...
### [168] [Minimum Block Width for Universal Approximation by Residual Neural Networks with Inner Width One](https://arxiv.org/abs/2607.04597)
...
### [169] [Predicting Drafted Deck Strength for "Magic: the Gathering"](https://arxiv.org/abs/2607.04782)
...
### [170] [Measuring What Matters: A Unified Evaluation Framework for GNN Explainability](https://arxiv.org/abs/2607.04600)
...
### [171] [Pretraining Curricula Enable Selective Fine-tuning](https://arxiv.org/abs/2607.04846)
...
### [172] [Reliability and Identifiability in Persona-Trained Monte Carlo: Variance Decomposition, Stability Bounds, and the Identifiability of Heterogeneous News Reaction](https://arxiv.org/abs/2607.04627)
*Salavat Ishbulatov*
...
### [173] [Input Pathways Shape Few-Shot, Not Zero-Shot, Binding in Tiny Transformers: A Fully-Enumerable Study](https://arxiv.org/abs/2607.04926)
*Yoshiyuki Ootani*
...
### [174] [The Map Behind the Flow: Finite-Step Gradient Descent as a Dynamical System](https://arxiv.org/abs/2607.04993)
*Thomas Hofmann*
...
### [175] [A Physics-Regulated Neural Framework for Learning 3D Grain Growth Dynamics](https://arxiv.org/abs/2607.04680)
...
### [176] [ImputeECG: Deep Learning Reconstruction of Complete 12-Lead Electrocardiograms from Incomplete Recordings for Cardiac Assessment](https://arxiv.org/abs/2607.05009)
...
### [177] [Hyperparameter Transfer in Graph Neural Networks](https://arxiv.org/abs/2607.05017)
...
### [178] [F-ACVAE: A Federated Adaptive Conditional Variational Auto-Encoder for Privacy-Preserving Intrusion Detection in IoT Networks](https://arxiv.org/abs/2607.04698)
...
### [179] [Grokking Is Conditional and Fragile: A Fully-Tractable, Multi-Seed Study at 12K Parameters](https://arxiv.org/abs/2607.05104)
*Yoshiyuki Ootani*
...
### [180] [Hierarchical Scaffolding Enables Human-Like Cognitive Selectivity under Data Scarcity](https://arxiv.org/abs/2607.04709)
...
### [181] [Localized LoRA-MoE: Block-wise Low-Rank Experts With Adaptive Routing](https://arxiv.org/abs/2607.05114)
...
### [182] [PDEFlow: Autonomous Agentic PDE Pipelines for Neural Operator Learning and Solver-Free Inference](https://arxiv.org/abs/2607.05134)
...
### [183] [Relational Multi-Agent Reinforcement Learning for Dynamic Pricing in High-Speed Railway Markets](https://arxiv.org/abs/2607.05179)
...
### [184] [Noisy-Channel Minimum Bayes Risk Decoding](https://arxiv.org/abs/2607.05198)
...
### [185] [MARLIN: De Novo Molecular Structure Elucidation from Tandem Mass Spectra without a Ground-Truth Formula](https://arxiv.org/abs/2607.04774)
...
### [186] [CanniUplift: A Holistic Framework for Mitigating Seller and Incentive Cannibalization in E-commerce Uplift Modeling](https://arxiv.org/abs/2607.05242)
...
### [187] [Towards Personalized Differentially Private Learning for Decentralized Local Graphs](https://arxiv.org/abs/2607.04777)
...
### [188] [Adaptive Inference Batching using Policy Gradients](https://arxiv.org/abs/2607.05272)
*Ruslan Sharifullin*
...
### [189] [Air Quality Downscaling with Station-Guided Pseudo-Supervision](https://arxiv.org/abs/2607.05292)
...
### [190] [Compressed Computation under $L^4$ Loss is likely Computation in Superposition](https://arxiv.org/abs/2607.04800)
...
### [191] [TREK: Distill to Explore, Reinforce to Refine](https://arxiv.org/abs/2607.05339)
...
### [192] [Layer-Parallel Inference Reduces Encrypted Nonlinear Depth in Transformers](https://arxiv.org/abs/2607.04819)
...
### [193] [What Does a Discrete Diffusion Model Learn?](https://arxiv.org/abs/2607.05381)
...
### [194] [KinEMbed: Decoding Kinematics from Electromyography via Cross-Modal Contrastive Learning](https://arxiv.org/abs/2607.04820)
...
### [195] [Weak-to-Strong Generalization via Direct On-Policy Distillation](https://arxiv.org/abs/2607.05394)
...
### [196] [Probably Correct Optimal Stable Matching under Two-Sided Uncertainty](https://arxiv.org/abs/2607.04824)
...
### [197] [Representing and Detecting Label Ambiguity in IMU-Based Exercise Evaluation](https://arxiv.org/abs/2607.04842)
...
### [235] [iFLYTEK-Embodied-Omni Technical Report](https://arxiv.org/abs/2607.02542)
...
TL;DR: iFLYTEK-Embodied-Omni是一个统一的多模态基础模型，通过共享的自注意力机制联合建模视觉（视频/图像）、语言和动作，采用脑-小脑协作设计实现从指令理解到动作执行的端到端处理。


<details>
  <summary>Details</summary>
Motivation: 现有通用具身智能体方法通常专精于视觉语言推理、基于视频的世界建模或动作生成等单一能力，而级联流水线（先合成未来观测再推断动作）会引入接口瓶颈和累积预测误差，需要统一的多模态模型来解决这些问题。

Method: 提出iFLYTEK-Embodied-Omni框架，包含特定模态的视觉语言模型、视频生成模型和动作生成模型，通过共享的多模态自注意力机制进行通信。采用脑-小脑协作设计：视觉语言模型和视频生成模型形成高级大脑负责指令理解、任务规划、进度跟踪和未来视觉状态预测；动作生成模型作为低级小脑将规划的子目标和共享的多模态上下文转换为可执行的动作块。

Result: 构建了包含人类演示和机器人交互的动作标注和无动作具身视频数据，结合具身推理、具身感知和通用图像文本数据的综合数据集。采用四阶段训练策略：首先分别训练VLM、VGM和AGM，然后联合微调完整模型。

Conclusion: iFLYTEK-Embodied-Omni通过统一的Omni框架解决了多模态具身智能中的碎片化问题，避免了级联方法的局限性，为实现更连贯、高效的具身智能系统提供了新的架构方向。
...
### [236] [Internal Pluralism and the Limits of Pairwise Comparisons](https://arxiv.org/abs/2607.02672)
*Bailey Flanigan,Michelle Si*

Main category: cs.AI

TL;DR: 本文研究了在内部多元主义情境下，本地成对比较方法在获取决策规则偏好时的局限性，并提出了一种允许报告不确定性的替代方法。


<details>
  <summary>Details</summary>
Motivation: 当前的偏好学习方法基于本地成对比较，但该方法隐含了两个强假设：1）本地比较足以推断个人对自动化决策规则的偏好；2）人们总能明确回答这些比较。研究者担心这些假设在内部多元主义（个体根据多种权威优先级评估决策规则）情境下会失效。

Method: 提出了一个形式化模型来描述对决策规则的多元主义偏好，分析了强制本地成对比较数据的两种失效情况：1）全局优先级（如比例性、平等主义、平等待遇）的本地不充分性；2）优先级冲突导致的内部冲突和行为扭曲。然后研究了允许报告不确定性的替代方案。

Result: 研究发现允许报告不确定性可以显著减少准确学习偏好所需的查询数量。模型还指向更优的偏好学习方法：直接获取这些优先级，从而产生更忠实和可解释的个人价值描述。

Conclusion: 在内部多元主义情境下，强制本地成对比较方法存在根本局限性。允许报告不确定性可以提高学习效率，而直接获取优先级的方法能更好地捕捉个人的多元价值系统。
...
### [237] [ASK in the Dark: Uncertainty-Gated LLM Assistance under Partial Observability](https://arxiv.org/abs/2607.02686)
*Juarez Monteiro,Nathan Gavenski,Guilherme Lima,Francisco Galuppo,Odinaldo Rodrigues,Adriano Veloso*

Main category: cs.AI

TL;DR: ASK+方法通过向小型语言模型提供轨迹感知上下文和结构化思维链，使其在部分可观察强化学习环境中从被动的冗余检查转变为信息更丰富的咨询者，显著提升了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 在部分可观察环境下运行的强化学习智能体面临信息不完整的问题，理论上可以从小型语言模型的广泛推理先验中获益。然而现有的不确定性门控方法在几乎所有测试环境中都显示零覆盖率，表明SLM几乎无法独立贡献动作。作者发现失败原因是裸的自我中心提示提供的信息不足以进行真正的推理，这主要是上下文问题而非能力问题。

Method: 提出了ASK+方法，该方法向SLM提供轨迹感知上下文（部分揭示的地图、访问过的位置和动作历史）以及结构化思维链推理。这种方法将SLM从被动的冗余检查转换为信息更丰富的咨询者，能够偶尔纠正策略。同时验证了预测熵信号测量的是动作不确定性而非状态不确定性，在POMDP中仍然有效。

Result: ASK+方法在各个环境中都取得了显著提升：在DoorKey任务中，成功率从89%提升到93%；在FourRooms中，成功率从53%提升到70%；在HigherLower中，准确率达到73.7%，匹配了SLM-only的上限。更重要的是，2B参数的Qwen3.5模型匹配甚至超越了4B参数的版本，表明提示设计和选择性门控主导了模型规模的影响。

Conclusion: 通过改进的上下文提示和思维链推理，SLM可以在部分可观察强化学习环境中发挥有效指导作用，且提示设计和选择性门控比模型规模更重要，实现了无需大型模型的指导方法。
...
### [238] [Automated Data Readiness for Scientific AI](https://arxiv.org/abs/2607.02771)
...
TL;DR: REDI是一个开源框架，通过统一的五阶段流水线（摄取、预处理、转换、结构化、输出）自动化科学数据集转换为AI就绪格式，提供可重现性和代理可调用性，解决了现有框架在自动化转换、就绪性评估、溯源跟踪和代理原生部署方面的不足。


<details>
  <summary>Details</summary>
Motivation: 领导计算设施管理着大规模科学数据集，这些数据集在用作AI训练数据前通常需要大量转换。然而，现有的框架无法完全统一自动化转换、就绪性评估、溯源跟踪和代理原生部署，存在明显的技术缺口。

Method: REDI框架采用统一的五阶段流水线：1）摄取，2）预处理，3）转换，4）结构化，5）输出。每个阶段都配备检测工具以实现可重现性，并可部署为代理可调用技能。配套工具SetGo自动化FAIR合规性和目录发布。在气候、蛋白质组学、材料科学和核聚变等多个领域进行评估，使用来源检测的性能分析来识别关键瓶颈。

Result: REDI成功将所有评估数据集从原始格式转换为AI就绪格式，输出结果与领域专家参考数据验证一致。初步结果显示在Frontier超级计算机上气候案例实现了接近理想的并行扩展，达到100个节点。来源检测的性能分析显示文件I/O是流水线的主要成本，格式选择是首要优化手段。

Conclusion: REDI作为一个跨领域平台，为科学AI提供了自动化数据就绪性解决方案，将数据准备瓶颈转化为可重现、可重用的社区资产，通过统一框架解决了科学数据集转换为AI训练数据的核心挑战。
...
### [239] [SwarmResearch: Orchestrating Coding Agents for Open-Ended Discovery](https://arxiv.org/abs/2607.02807)
*Yuvraj Virk,Zack Edds,Chunqiu Steven Xia,Lingming Zhang*

Main category: cs.AI

TL;DR: SwarmResearch是一个新颖的多代理系统，通过协调器-子代理架构解决传统长期运行编码代理在开放式优化问题中收敛于单一方法的问题。该系统使用Shepherd Agent指导多个Search Agent，实现了更高层次的探索和更好的优化效果。


<details>
  <summary>Details</summary>
Motivation: 传统的长期运行编码代理（如autoresearch）在解决开放式优化问题时存在明显局限：它们倾向于收敛到单一高层方法，然后进行低层次编辑，而忽略了可能更优越的其他方法。作者认为这种行为的根源在于两个系统设计选择：1）在单个长期运行代理中累积上下文；2）只暴露单个程序状态供编辑。

Method: 提出SwarmResearch架构，包含Shepherd Agent（协调器）和Search Agent（搜索代理）两个层级。Shepherd Agent使用全局上下文引导一个Search Agent种群，每个Search Agent在各自的git分支中操作，拥有本地上下文。该系统能够根据搜索深度动态调整并行度，而不是采用固定的串行或并行代理扩展。

Result: 在开放式优化任务上，SwarmResearch在13/15的任务中发现了比当前最先进的LLM引导进化和多代理技术更好或相当的解。通过协调器引导的扩展，SwarmResearch能够根据不同的搜索深度自适应调整并行度，从而发现性能更好的解决方案。

Conclusion: SwarmResearch通过其新颖的协调器-子代理架构解决了传统编码代理在开放式优化中的探索不足问题。通过分离全局和本地上下文，允许多个搜索代理并行探索不同方法，该系统实现了更高层次的探索多样性，从而在大多数任务上取得了更好的优化结果。
...
### [240] [Object-Centric Environment Modeling for Agentic Tasks](https://arxiv.org/abs/2607.02846)
*Yiyang Li,Tianyi Ma,Zehong Wang,Yijun Ma,Yanfang Ye*

Main category: cs.AI

TL;DR: OCM方法将智能体经验组织成可执行的对象中心化环境模型，包含对象知识和过程知识两个相互验证的代码库，通过在线更新和渐进式知识披露机制提高智能体性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型智能体可以通过积累经验来改进，但自由形式的文本记忆会随着交互增加而变得难以维护、验证和复用。现有的符号方法学习可执行技能或程序化世界模型，但通常存储局部过程或假设简化的动态。

Method: 提出对象中心化环境建模（OCM）方法，维护两个相互连接的代码库：对象知识（定义环境实体和机制的Python类）和过程知识（记录可复用交互模式）。采用在线工作方式：每次交互后反思轨迹、更新知识库并验证过程与对象模型的兼容性。未来交互时使用渐进式知识披露机制，先检查代码签名，必要时再阅读源代码。

Result: 实验结果表明，OCM在基准测试中取得了最佳平均排名，并显著减少了无效行动，证明智能体可以从构建对象中心化环境模型中获益。

Conclusion: LLM 智能体可以通过构建以对象为中心的环境模型来显著提升性能，OCM方法通过将经验组织成可执行的对象知识库和过程知识库，实现了有效的知识积累和复用，为智能体在复杂环境中的长期学习和适应性提供了新思路。
...
### [241] [MedCalc-Pro: Solving Complex Medical Calculations with LLM Agents](https://arxiv.org/abs/2607.02879)
*Siran Zhao,Ruihui Hou,Ziyue Huai,Chennuo Zhang,Tong Ruan*

Main category: cs.AI

TL;DR: 提出了MedCalc-Pro医疗计算基准测试，包含三个渐进式挑战性任务设置，并提出支持多工具选择和嵌套工具调用的通用代理框架


<details>
  <summary>Details</summary>
Motivation: 现有LLM医疗计算基准大多基于简化设置，而真实临床场景需要多个计算器联合评估、嵌套尺度计算和模糊查询，当前评估方法无法充分反映实际需求

Method: 1) 创建MedCalc-Pro基准，包含2,268个真实临床案例，涵盖14个科室的77个医疗计算器，设置单计算器、多计算器和嵌套计算器三种任务；2) 提出通用代理框架，支持多工具选择和嵌套工具调用，通过结构化验证和证据审查抑制参数错误传播

Result: 对开源、闭源和医疗专用LLM进行系统比较，结果显示提出的框架在所有三种任务设置中均取得最佳性能

Conclusion: 这项工作为评估和应用LLM在具有挑战性的医疗计算场景中提供了新的基准和方法，填补了真实临床需求与现有评估方法之间的差距
...
### [242] [Oyster-II: Reinforcement Learning for Constructive Safety Alignment in Large Language Models](https://arxiv.org/abs/2607.02914)
*Jiyang Guan,Yong Xie,Jun Chen,Jiexi Liu,Zipeng Ye,Defeng Li,Jiayu Shen,Jialing Tao,Hui Xue*

Main category: cs.AI

TL;DR: 提出Oyster-II，一个基于强化学习的建设性安全对齐框架，有效解决了先前方法的局限性，在安全性和帮助性方面都取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 传统拒绝导向的安全对齐策略虽能缓解有害内容生成，但常常无法满足用户的合法需求，过度拒绝可能安全且具有建设性的敏感查询。Oyster-I的建设性安全范式存在两个关键局限性：对分布外场景的安全性泛化不足，以及安全思维链过度泛化现象，导致对良性查询的过度安全推理，降低帮助性和用户体验。

Method: 采用基于强化学习的建设性安全对齐框架，使用Zero-RL范式结合多阶段强化学习策略，超越了基于监督微调的传统方法。

Result: Oyster-II在广泛的基准测试中全面超越Qwen3-14B及其前身Oyster-I，在安全维度上达到与Qwen3-Max和Qwen3.5-397B相当的跨尺度性能。

Conclusion: Oyster-II represents一种重要的进步，通过强化学习框架有效解决了Oyster-I的局限性，在保持安全性的同时显著提升了帮助性和用户体验，为大语言模型的安全对齐提供了新的方向。
...
### [243] [VERITAS: Towards a General-Purpose Replication Tool for Scientific Research](https://arxiv.org/abs/2607.02931)
*Haokun Liu,Filbert Aurelian Tjiaranata,Chenhao Tan*

Main category: cs.AI

TL;DR: VERITAS是一个领域无关的复制验证框架，使用CLI编码代理自动验证科学研究论文的可复现性，生成重要性加权的复制分数、修复日志和修补后的代码库。


<details>
  <summary>Details</summary>
Motivation: AI工具加速了科学发表，但评审系统难以跟上，且独立验证研究结果变得更加困难但也更重要。现有方法主要作为基准测试存在，缺乏通用的复制工具。

Method: 基于CLI编码代理构建的领域无关复制框架。给定论文和/或代码库后，提取论文主张，运行方法学并解决出现的问题，根据实验运行证据判断每个主张。

Result: 在涵盖计算机科学、社会科学、医学和天体物理学的65篇论文基准测试(CORE-Bench和ReplicationBench)中，VERITAS在相同模型和宿主环境下优于两个强基准，在所有指标上均取得领先，达到最先进性能。

Conclusion: VERITAS提供了一个有效的通用科研复现验证工具，能够在多学科领域自动化验证研究可复现性，解决了现有方法局限于特定基准测试的问题。
...
### [244] [A Sliding-Window-Based Reinforcement Learning for Dynamic Assembly Flow Shop Scheduling with Multi-Product Delivery](https://arxiv.org/abs/2607.02941)
*Junhao Qiu,Jianjun Liu,Ting Liu,Rongjie Liao,Zhantao Li,Qingfu Zhang*

Main category: cs.AI

TL;DR: 提出基于滑动窗口的强化学习框架来解决柔性装配流水车间中带复杂配料约束的在线调度问题，通过异构图MDP建模、滑动窗口过滤、时空图编码和动态动作映射，显著减少订单延迟并展现鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 多产品配料配送在混合制造系统中对实时调度提出重大挑战，动态订单到达同时改变供应依赖性和可行的任务-机器分配集合，需要有效解决柔性装配流水车间调度问题中的复杂配料约束。

Method: 提出了滑动窗口强化学习框架，包括：1）滑动窗口过滤机制过滤非活动节点并优先处理配料关键操作；2）时空图编码网络跟踪连续决策状态中的瓶颈转移；3）动态动作映射模块和带约束等待策略适应变化拓扑下的动作空间。

Result: 在来自家电制造商的真实实例上的实验表明，SWRL相比经典调度规则和现有深度强化学习方法实现了持续的延迟减少，并且在变化的资源配置、订单负载和到达集中度下都表现出鲁棒性能。

Conclusion: SWRL framework consistently outperforms经典调度规则和现有深度强化学习方法，在减少订单延迟方面表现显著，并且在不同资源配置、订单负载和到达集中度下都表现出鲁棒性能，为混合制造系统中的实时多产品配料配送调度提供了有效解决方案。
...
### [245] [Evaluating Generative Agents with Actions Grounded in Socially Distributed Task Environments using Incognita](https://arxiv.org/abs/2607.02975)
*Dan C. Hsu,Luke Lu*

Main category: cs.AI

TL;DR: 这篇论文提出了Incognita框架，用于评估社交分布式任务环境中智能体的表现，该框架结合了社交互动和实体化执行，并在零售环境中测试了三种生成模型，展示了从0%到17.2%的成功率提升。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试要么提供可执行动作、持久状态和可验证结果，要么提供语言智能体间的丰富社交互动，但缺乏结合两者的评估环境。需要在社交分布式任务环境中研究智能体的决策制定，其中任务相关知识分布在角色隔离的参与者中，关键动作只能通过他们访问。

Method: 提出了Incognita框架，基于Concordia平台，分离社交互动与实体化执行。被评估的智能体将消息路由给用户或专业实体；专业人员调解可接受的操作；确定性子环境在规范状态上执行接受的操作；离线评估器使用继承奖励评分结果。将tau-bench零售基准转换为多实体环境，同时保持最终状态奖励语义。

Result: 在18个按社交广度分层的任务上评估了三种生成智能体模型，共540次试验。成功率从0%提升到8.9%和17.2%；过早完成任务的比例从100%下降到87%和58%。更强的模型引出更多隐藏知识、接触更多实体、尝试更多实体化写入，但可靠性仍然较低。

Conclusion: 社交分布式任务环境能够在可靠性成功之前暴露智能体行为，包括知识引出、来源选择、实体化动作尝试和过早完成任务信念。这表明了在复杂社交环境中评估智能体能力的重要性，即使总体成功率较低。
...
### [246] [Reinforcement Learning for Evidence-Seeking Diagnostic Reasoning with Large Language Models](https://arxiv.org/abs/2607.02983)
*Shengyi Hua,Kangzhe Hu,Conghui He,Xiaofan Zhang,Shaoting Zhang*

Main category: cs.AI

TL;DR: 论文提出了一种创新的医学诊断方法，将诊断过程形式化为迭代证据寻求任务，通过强化学习与可验证奖励机制引导大语言模型从被动推理转向主动临床智能助手。


<details>
  <summary>Details</summary>
Motivation: 当前以推理为中心的大语言模型主要在被动推理模式下运行，假设信息完整，但现实世界的临床智能本质上是需要战略证据获取的迭代调查过程。这种差距需要在临床环境中得到弥合。

Method: 论文将医学诊断形式化为迭代证据寻求任务，利用可验证奖励的强化学习在闭环环境中激发内在推理，并设计了强制执行诊断精度和检查一致性的奖励函数。同时引入了基于检索增强生成的检查模拟器作为高保真临床预言机。

Result: 实验结果显示，该框架使大语言模型能够从被动响应者转变为自主助手。模型表现与更大规模、推理增强的基准模型相当，而检查模拟器在生成生物学合理的临床反馈方面优于原始大语言模型。

Conclusion: 论文提出的方法成功地将大语言模型从被动推理转向主动证据寻求模式，创建了一种接近真实临床医生推理过程的自主诊断系统，为医学人工智能的发展提供了新方向。
...
### [247] [Beyond Forecasting: The Belief-to-Trade Layer in Prediction-Market Agents](https://arxiv.org/abs/2607.03015)
*Yishu Wang,Yuxuan Wang,Jiaqi Deng,Hanyang Tang*

Main category: cs.AI

TL;DR: 提出了Raven-Agent，这是首个预测市场的自主交易代理，在受控回放测试中实现了正收益和正风险调整收益


<details>
  <summary>Details</summary>
Motivation: 预测未来事件作为通用AI测试基准受到关注，但现有基准显示校准概率评分与交易结果之间存在显著差距。交易不仅需要预测能力，还需要其他技能

Method: 开发了第一个预测市场的自主交易代理架构Raven-Agent，通过在存档决策集上进行受控回放测试

Result: 在受控回放测试中，Raven-Agent是所有测试策略中唯一实现正收益和唯一实现正风险调整收益的架构

Conclusion: Raven-Agent展示了作为预测市场自主交易代理的有效性，为解决预测与交易表现之间的差距提供了解决方案，并已开源代码
...
### [248] [Human-Centric Reflective Architecture for Human-AI Collaborative Decision-Making](https://arxiv.org/abs/2607.03025)
...
### [249] [Silicon Sampling via Cross-Survey Transfer](https://arxiv.org/abs/2607.03091)
...
### [250] [APeB: Benchmarking Personalization Ability of Large Language Model Agents](https://arxiv.org/abs/2607.03162)
...
### [251] [Organizational Memory for Agentic Business Process Execution](https://arxiv.org/abs/2607.03228)
...
### [252] [Embodied Operators and Benchmarking: Toward Reusable and Deployable Embodied Intelligence Systems](https://arxiv.org/abs/2607.03283)
...
### [253] [Reflective Dialogue or Prompt Refinement? Effects of Tutor Scaffolding on Students' Independent LLM Use for Programming](https://arxiv.org/abs/2607.03303)
...
### [254] [When Aggregate Alignment Misleads: Auditing Policy Repair Without Per-State Expert Actions](https://arxiv.org/abs/2607.03386)
...
### [255] [From Mobile Data to Business Insights: An End-to-End Analytics Framework for Large-Scale Urban Mobility Analysis and Decision Support](https://arxiv.org/abs/2607.03394)
...
### [256] [Efficient bias mitigation in T2I diffusion models using Concept Graphs](https://arxiv.org/abs/2607.03397)
...
### [257] [Personalized Causal Recourse: A Human-In-The-Loop Approach](https://arxiv.org/abs/2607.03425)
...
### [258] [Demonstrating Generalization Failures via Mixtures of Conditional Policies](https://arxiv.org/abs/2607.03478)
...
### [259] [MentalThink: Shaping Thoughts in Mental SVG World](https://arxiv.org/abs/2607.03530)
...
### [260] [Applying Answer Set Programming with Fuzzy Membership Functions: a Case Study](https://arxiv.org/abs/2607.03550)
...
### [261] [How to Avoid Debate: Scalable AI Safety via Doubly-Efficient Interactive Proofs](https://arxiv.org/abs/2607.03561)
...
### [262] [The Role of Rigor in Artificial Intelligence](https://arxiv.org/abs/2607.03634)
*Timothy Nguyen*
...
### [263] [Robust Feasible Route Construction through Collaborative Partition Optimization](https://arxiv.org/abs/2607.03694)
...
### [264] [Agent Reinforcement Learning via Pivotal-Aware Self-Feedback Retry](https://arxiv.org/abs/2607.03702)
...
### [265] [Explainable Reinforcement Learning for Adaptive Traffic Signal Control](https://arxiv.org/abs/2607.03703)
...
### [266] [Can Conversational Temporal Dynamics Improve Depression Detection in Dyads? A Preliminary Investigation in Multi-Modality Perspectives](https://arxiv.org/abs/2607.03744)
...
### [267] [Bridging Interleaved Multi-Modal Reasoning as a Unified Decision Process](https://arxiv.org/abs/2607.03748)
...
### [268] [Folding, Reasoning, and Scaling with Open-source Drug Discovery Engine](https://arxiv.org/abs/2607.03787)
...
### [269] [Evaluating LLM Uncertainty in Long-Form Generation Using Deterministic Ground Truth](https://arxiv.org/abs/2607.03870)
...
### [270] [Harness-Aware Self-Evolving: Co-Evolving Model Weights, Harness, and Task Solutions](https://arxiv.org/abs/2607.03935)
...
### [271] [Online Linear Programming for Multi-Objective Routing in LLM Serving](https://arxiv.org/abs/2607.03948)
...
### [272] [Explainable AI for Screening Abuse-Related Trauma in Bangladeshi Children: A Training-Free Multimodal Framework Evaluated on Noise-Aware Synthetic Data](https://arxiv.org/abs/2607.04010)
...
### [273] [What is Left for Us? Second Scholarship Against the Degradation of Research by AI](https://arxiv.org/abs/2607.04049)
...
### [274] [PLACEMEM: Toward a Compute-Aware Memory Plane for Lifelong Agents](https://arxiv.org/abs/2607.04089)
*Sukanta Ganguly*
...
### [275] [Forethought: Verifiable Reasoning from Neurosymbolic Primitive Programming](https://arxiv.org/abs/2607.04096)
...
### [276] [Language models guide symbolic equation discovery by controlling search](https://arxiv.org/abs/2607.04156)
...
### [277] [A Clustering-Based Framework for Identifying Suspicious Trading Patterns in Capital Market](https://arxiv.org/abs/2607.04184)
...
### [278] [Agentic IoT: Architectures, Applications, and Challenges Toward the Internet of Agents](https://arxiv.org/abs/2607.04219)
...
### [279] [Unsupervised Features Mining via Activation Geometry](https://arxiv.org/abs/2607.04222)
...
### [280] [Biological Motifs for Agentic Control](https://arxiv.org/abs/2607.04240)
*Bogdan Banu*
...
### [281] [Progress- and Reliability-Oriented Group Policy Optimization for Agentic Reinforcement Learning](https://arxiv.org/abs/2607.04242)
...
### [282] [Shortcut Learning in Legal Judgment Prediction: Empirical Evidence from the UK Employment Tribunal](https://arxiv.org/abs/2607.04261)
...
### [283] [Agentic SABRE: An Uncertainty-Aware Neuro-Symbolic Multi-Agent Framework for Adaptive Ransomware Detection](https://arxiv.org/abs/2607.04292)
...
### [284] [HAS-Bench: Evaluating LLM-Based Human-Agent Systems under Configurable Human Participation](https://arxiv.org/abs/2607.04329)
...
### [285] [Do GUI Agents Believe Their Eyes? Diagnosing State-Belief Reliance on Pixels versus Structure](https://arxiv.org/abs/2607.04334)
...
### [286] [Server-side Anti-cheat in FPS games for Aimbot detection using Deep learning and Machine learning](https://arxiv.org/abs/2607.04336)
...
### [287] [Nemotron-Labs-3-Puzzle-75B-A9B: Compressing Hybrid MoE LLMs](https://arxiv.org/abs/2607.04371)
...
### [288] [Decentralized Aggregation of LLM Predictions via Wagering Mechanisms](https://arxiv.org/abs/2607.04389)
...
### [289] [MechMath Agent Team: LLM Driven Agents for Mathematical Research](https://arxiv.org/abs/2607.04394)
...
### [290] [LLM-as-a-Tutor: Policy-Aware Prompt Adaptation for Non-Verifiable RL](https://arxiv.org/abs/2607.04412)
...
### [291] [Agent Step Value: State-Transition Measurement with State-Grounded LLM Evaluators](https://arxiv.org/abs/2607.04419)
...
### [292] [ResearchStudio-Idea: An Evidence-Grounded Research-Ideation Skill Suite from ML Conference Outcomes](https://arxiv.org/abs/2607.04439)
...
### [293] [Why Pure Reasoning is Not Enough: Nature as the Source of Mathematical Innovation](https://arxiv.org/abs/2607.04505)
...
### [294] [Compressing the Validation Bottleneck: An Agentic Self-Driving Lab for Scientific Discovery](https://arxiv.org/abs/2607.04508)
...
### [295] [VLA Grounder: Language-Conditioning Space Optimization for Black-Box VLA Models](https://arxiv.org/abs/2607.04517)
...
### [296] [Measuring Harness-Induced Belief Divergence in Multi-Step LLM Agents](https://arxiv.org/abs/2607.04528)
*Haiwen Yi,Xinyuan Song*
...
### [297] [Heaviside Continuity of Rolling Coefficients for Eliminating Epistemic Entropy in Large Language Models](https://arxiv.org/abs/2607.04562)
...
### [298] [Detecting Answer-Driven Reasoning in LLM-Based Educational Tutors via Truncated Chain-of-Thought Auditing](https://arxiv.org/abs/2607.04572)
...
### [299] [Attention Limited Reward Learning](https://arxiv.org/abs/2607.04590)
*Wenqian Xing*
...
### [300] [Governed Individuation: Cryptographically Decoupling an Agent's Learning from Its Authority](https://arxiv.org/abs/2607.04613)
...
### [301] [MRMS: A Multi-Resolution Memory Substrate for Long-Lived AI Agents](https://arxiv.org/abs/2607.04617)
...
### [302] [Formal Disco: Scalable Open-Ended Generation of Formally Verified Programs](https://arxiv.org/abs/2607.04631)
...
### [303] [Integrated Altruistic and Fairness Preference Induces Advanced Mutual Cooperation in Sequential Social Dilemmas](https://arxiv.org/abs/2607.04710)
...
### [304] [FORGE: Research-Trajectory Hijacking Attacks on Deep Research Agents](https://arxiv.org/abs/2607.04718)
...
### [305] [FM-ChangeNet: Learning Change through Pathwise Feature Transport](https://arxiv.org/abs/2607.04750)
...
### [306] [AgenticPD: A Stage-Aware Agentic Framework for Physical Design QoR Optimization](https://arxiv.org/abs/2607.04758)
...
### [307] [CARL: Constraint-Aware Reinforcement Learning for Planning with LLMs](https://arxiv.org/abs/2607.04854)
...
### [308] [Medi-Gemma: A Hybrid Clinical Decision Support System Integrating Deterministic EMR Analytics and Retrieval-Augmented Generation](https://arxiv.org/abs/2607.04907)
...
### [309] [STAPO: Selective Trajectory-Aware Policy Optimization for LLM Agent Training](https://arxiv.org/abs/2607.04963)
...
### [310] [Quantum-Inspired Harmonic Decision Models: A Computational Framework for Music Generation](https://arxiv.org/abs/2607.05007)
...
### [311] [Toward Trustworthy Large Language Model Agents in Healthcare](https://arxiv.org/abs/2607.05055)
...
### [312] [Diffusion-Guided Uncertainty-Aware Delayed Policy Optimization](https://arxiv.org/abs/2607.05064)
...
### [313] [ASSEMCAD: Production-Ready CAD Assembly Generation from Natural Language](https://arxiv.org/abs/2607.05123)
...
### [314] [TacReasoner: A Dynamic Tactile-Language Framework for Interactive Reasoning in Real-World Scenarios](https://arxiv.org/abs/2607.05131)
...
### [315] [DSpark: Confidence-Scheduled Speculative Decoding with Semi-Autoregressive Generation](https://arxiv.org/abs/2607.05147)
...
### [316] [The Changing Role of Symbolic Methods in Artificial Intelligence](https://arxiv.org/abs/2607.05168)
*Jun Sun*
...
### [317] [AgentGym2: Benchmarking Large Language Model Agents in De-Idealized Real-World Environments](https://arxiv.org/abs/2607.05174)
...
### [318] [CP-WSP: A Declarative CP-SAT Framework for Configurable Multi-Constraint Workforce Scheduling](https://arxiv.org/abs/2607.05177)
...
### [319] [Rethinking On-Policy Self-Distillation for Thinking Models](https://arxiv.org/abs/2607.05184)
...
### [320] [ClassicLogic: A Knowledge-Driven Benchmark of Classic Puzzle Games for Evaluating Compositional Generalization](https://arxiv.org/abs/2607.05185)
...
### [321] [Reason, Reward, Refine: Step-Level Errors Corrections with Structured Feedback for Physics Reasoning in Small Language Models](https://arxiv.org/abs/2607.05199)
...
### [322] [EvoAgentBench: Benchmarking Agent Self-Evolution via Ability Transfer](https://arxiv.org/abs/2607.05202)
...
### [323] [MoP-JEPA: Hard-Assigned Predictor Mixtures for Stochastic JEPA World Models](https://arxiv.org/abs/2607.05238)
...
### [324] [MetaSkill-Evolve: Recursive Self-Improvement of LLM Agents via Two-Timescale Meta-Skill Evolution](https://arxiv.org/abs/2607.05297)
...
### [325] [Evaluating and Understanding Model Editing for Medical Vision Language Models](https://arxiv.org/abs/2607.05310)
...
### [326] [OptiAgent: End-to-End Optimization Modeling via Multi-Agent Iterative Refinement](https://arxiv.org/abs/2607.05346)
...
### [327] [Graph Sparse Sampling: Breaking the Curse of the Horizon in Continuous MDP Planning](https://arxiv.org/abs/2607.05359)
...
### [328] [SovereignPA-Bench: Evaluating User-Owned Personal Agents under Evolving Intent, Platform Mediation, and Consent Constraints](https://arxiv.org/abs/2607.05363)
*Dylan Zongmin Liu*
...
### [329] [LLM-as-a-Verifier: A General-Purpose Verification Framework](https://arxiv.org/abs/2607.05391)
