...
TL;DR: LABBench2是LAB-Bench基准测试的进化版本，包含近1900个任务，旨在更真实地衡量AI系统在科学任务中的实际能力，结果显示当前前沿模型在更现实情境下性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 随着AI在科学发现中应用的增加，需要测量AI系统在科学领域的实际能力，而不仅仅是知识记忆和推理能力。LAB-Bench作为初步尝试，需要进化到更真实地评估AI执行有意义科学工作的能力。

Method: 开发LABBench2基准测试，包含近1900个任务，作为LAB-Bench的延续但提供更现实的上下文。评估当前前沿模型在这些任务上的表现，并与LAB-Bench结果进行比较。

Result: 虽然LAB-Bench和LABBench2测量的能力都有显著提升，但LABBench2在难度上提供了有意义的跳跃（模型特定准确率在不同子任务中下降26%到46%），表明在性能改进方面仍有持续空间。

Conclusion: LABBench2延续了LAB-Bench作为AI科学研究能力事实基准测试的传统，希望它能继续帮助推进AI工具在这些核心研究功能方面的发展。提供了数据集和评估工具供社区使用。
...
TL;DR: 提出一种基于线性规划虚拟间隙分析(VGA)的两步多准则分析方法，从悲观视角评估备选方案，解决主观偏见和数据多样性问题，提高决策可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统的多准则分析方法(MCDM)在估计准则参数时容易受到主观评价和偏见的影响，同时数据的多样性会降低参数的精确度，从而影响结果的可靠性。

Method: 采用两步法整合两个新型虚拟间隙分析(VGA)模型：第一步从悲观视角评估每个备选方案，同时使用定量和定性准则，以及基数和序数数据；第二步对备选方案进行优先级排序以消除最不利选项。

Result: 提出了一种可靠且可扩展的线性规划方法，能够高效有效地在决策支持系统中进行全面评估。

Conclusion: 基于线性规划的新型VGA模型能够有效解决多准则分析中的主观性和数据多样性问题。通过悲观视角的评估和优先级排序方法，该方法能够在决策支持系统中实现高效、可靠的备选方案评估和筛选。
...
TL;DR: 提出了一个基于当前最佳实践的标准化AI日志分析流程框架，并通过Inspect Scout库提供具体代码示例和详细指导
...
Method: 基于当前最佳实践提出一个标准化分析流程，使用Inspect Scout库提供具体代码示例，详细指导每个步骤并突出常见陷阱

Result: 开发了一个完整的日志分析框架，为研究人员提供了严谨且可复现的日志分析基础

Conclusion: 该框架填补了AI日志分析领域标准化方法的空白，通过提供具体工具和指导，使研究人员能够进行更系统、可靠的日志分析
...
TL;DR: 提出了一种用于微控制器上持续目标检测的自适应分层压缩框架，通过元学习实现高效特征压缩，在100KB内存限制下有效缓解灾难性遗忘
...
Method: 提出自适应分层压缩框架，包含三个关键创新：1）基于MAML的真实压缩，通过梯度下降在5步内适应新任务；2）分层多尺度压缩，根据FPN冗余模式设置不同压缩比；3）双内存架构结合短期和长期存储库，在100KB预算下进行重要性整合

Result: 在CORe50、TiROD和PASCAL VOC基准测试中，与三种标准基线方法相比，AHC能够在100KB重放预算内实现实用的持续检测，通过均值池化压缩特征重放结合EWC正则化和特征蒸馏获得有竞争力的准确率
...
TL;DR: 本论文研究了面向混合系统的可解释AI规划，解决在安全关键领域应用AI规划时缺乏决策解释的问题。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能技术在复杂安全关键领域（如智能电网、自动驾驶、医疗健康等）的广泛应用，人们对AI系统的可解释性需求日益增长。现有的自动规划系统虽然功能强大，但缺乏向用户提供决策解释的能力，这成为规划社区面临的主要挑战。

Method: 本论文采用了一种基于混合系统的可解释AI规划(XAIP)方法，专注于设计能够生成解释的规划系统，重点研究如何将AI规划技术与解释生成机制相结合，以构建可解释的决策系统。

Result: 论文实现了针对混合系统的可解释AI规划框架，能够为各种复杂的实际应用场景生成决策解释，提高了AI系统的透明度和可信任度。

Conclusion: 本论文提出并验证了一种适用于混合系统的可解释AI规划框架，该框架能够生成对决策过程和结果的全面解释，有助于提升用户对AI系统的信任度，特别是对于安全关键领域的应用具有重要意义。
...
TL;DR: 提出Vigil主动式智能体系统，用于整个on-call生命周期，在人工支持介入后仍能持续协助，并具备自我改进机制，已在字节跳动云平台成功部署10个月。


<details>
  <summary>Details</summary>
Motivation: 当前云服务平台中大量on-call对话给人工支持分析师带来沉重工作负担。现有基于大语言模型的反应式智能体仅在初期作为一线支持，当问题升级到人工支持时通常会被断开连接，导致无法协助后续查询、跟踪解决进度或从失败案例中学习。

Method: 提出Vigil主动式智能体系统，与现有反应式智能体的主要区别在于：1）在整个on-call生命周期内工作，特别是在人工支持介入后的阶段；2）无需用户显式调用，主动整合到客户与分析师对话中提供协助；3）采用持续自我改进机制，从人工解决案例中提取知识并自主更新能力。

Result: Vigil已在字节跳动的云平台Volcano Engine上部署超过十个月，基于该部署的全面评估证明了其有效性和实用性。评估结果表明Vigil能够显著降低分析师工作负载并提高问题解决效率。

Conclusion: Vigil系统作为一个在整个on-call生命周期内工作的主动式智能体，通过整合到客户与分析师之间的对话中，在人工支持介入后仍能持续提供协助，并具备持续自我改进机制，显著降低了分析师工作负载并提高了问题解决效率，证明了其在真实云服务平台中的有效性和实用性。
...
TL;DR: 提出面向对象世界建模（OOWM）框架，将具身推理结构化，使用UML类图和活动图显式表示状态空间和转换逻辑，结合SFT和GRPO训练，在MRoom-30k基准上显著优于非结构化文本基线。


<details>
  <summary>Details</summary>
Motivation: 标准思维链（CoT）提示虽然赋予大语言模型推理能力，但其依赖线性自然语言，不足以有效建模具身任务中的世界。文本虽灵活，但无法显式表示状态空间、对象层次结构和因果依赖关系，限制了机器人规划的鲁棒性。

Method: 提出OOWM框架，将世界模型定义为显式符号元组W=⟨S,T⟩，包含状态抽象G_state（实例化环境状态S）和控制策略G_control（表示转换逻辑T）。使用UML类图将视觉感知映射为对象层次结构，使用活动图将规划操作化为可执行控制流。采用三阶段训练管道，结合监督微调（SFT）和组相对策略优化（GRPO），利用最终计划的结果奖励隐式优化面向对象推理结构。

Result: 在MRoom-30k基准上的广泛评估表明，OOWM在规划连贯性、执行成功率和结构保真度方面显著优于非结构化文本基线，为结构化具身推理建立了新范式。

Conclusion: OOWM通过软件工程形式主义的结构化方法，有效解决了标准CoT在具身任务中的局限性，为构建更鲁棒、可解释的机器人世界模型提供了新途径。
...
TL;DR: 分析基于可能性理论的形式背景因子化方法，研究相关集合对性质，并将其扩展到模糊框架以计算模糊背景的独立子背景


<details>
  <summary>Details</summary>
Motivation: 数据集因子化在许多方法中是一个有趣的过程，但很多时候计算数据集的因子化不可能或效率低下。需要为模糊背景开发计算独立子背景的机制

Method: 分析基于可能性理论算子的形式背景因子化方法，研究产生因子化的集合对性质，并将经典框架的性质扩展到模糊框架

Result: 分析了从形式背景获取独立子背景的方法，研究了产生因子化的集合对的性质，并将经典案例中的性质扩展到模糊框架中

Conclusion: 该方法为从模糊形式背景中计算独立子背景提供了理论基础和计算机制，扩展了形式概念分析的应用范围
...
TL;DR: 通过耦合多智能体LLM与潜在基础模型，建立自动化探索PDE系统的新范式，实现低成本参数空间搜索，发现复杂流场中的新标度规律。


<details>
  <summary>Details</summary>
Motivation: 传统研究使用实验室实验或计算昂贵的数值模拟探索偏微分方程的解空间，这限制了自动化和大规模探索。与药物发现或材料科学等可以自然利用大语言模型的领域不同，连续、高维且混沌的物理现象缺乏离散化、可标记的表示形式。

Method: 方法包括两个核心组件：1) 潜在基础模型（LFM），这是一种生成模型，学习参数化模拟的明确、紧凑和解耦的潜在表示；2) 层次化的多智能体架构，通过假设、实验、分析和验证的闭环实现自主探索。LFM作为按需替代模拟器，允许智能体以可忽略成本查询任意参数配置。

Result: 应用于Re = 500下的串联圆柱绕流时，该框架自主评估了超过1,600个参数-位置组合，并发现了不同的标度规律：最小位移厚度的机制依赖的双模态结构，以及最大动量厚度的稳健线性标度。两种情况都表现出在近尾流到共涡脱落机制转变时出现的双极值结构。

Conclusion: 该研究提出了一种将多智能体大语言模型与潜在基础模型结合的新范式，通过将学习到的物理表示与智能推理相结合，为偏微分方程系统下的自动化科学发现建立了通用框架。该框架能够实现低成本、自动化的参数空间探索，发现复杂物理现象中的新规律。
...
TL;DR: 针对移动Agent评估基准测试与真实第三方应用场景不匹配的问题，提出了MobiFlow框架，通过多轨迹融合图构建算法支持动态交互，在20个应用的240个真实任务上实现更准确的性能评估。


<details>
  <summary>Details</summary>
Motivation: 现有的主流评估基准测试（如AndroidWorld）通过连接系统级Android模拟器来评估移动Agent，但在现实场景中，许多第三方应用不暴露系统级API来确定任务是否成功，导致基准测试与真实使用场景不匹配，难以准确评估模型性能。

Method: 提出了MobiFlow评估框架，包括：1) 基于多轨迹融合的高效图构建算法来压缩状态空间；2) 支持动态交互；3) 包含20个广泛使用的第三方应用和240个多样化真实任务；4) 丰富的评估指标。

Result: 与AndroidWorld相比，MobiFlow的评估结果显示出与人类评估更高的对齐度，并且能够在真实工作负载下指导未来基于GUI模型的训练。

Conclusion: MobiFlow是一个基于真实第三方应用场景的移动Agent评估框架，相比基于系统级Android模拟器的传统基准测试方法，它能更好地评估模型性能并指导未来GUI模型的训练。
...
### [11] [Spatial Competence Benchmark](https://arxiv.org/abs/2604.09594)
*Jash Vira,Ashley Harris*

Main category: cs.AI

TL;DR: 本文提出SCBench空间能力基准，包含三层能力评估框架，实验显示前沿模型在空间能力上存在单调递减的准确率模式，全局约束违反是主要失败原因。


<details>
  <summary>Details</summary>
Motivation: 现有的空间评估方法局限于通过3D变换或视觉问答探测孤立的空间原语，缺乏对空间能力的系统评估框架，无法全面衡量模型维护环境内部表示并用于推理和规划的能力。

Method: 研究者设计了空间能力基准SCBench，采用三个层次化的能力分类，包含需要执行性输出的任务，通过确定性检查器或基于模拟器的评估器进行验证。通过实验探究了输出令牌限制对模型准确率的影响。

Result: 实验结果显示，三个前沿模型在SCBench上表现出沿能力阶梯单调递减的准确率模式；模型准确率的提升主要集中在低计算预算下并快速饱和；主要失败原因是局部合理的几何结构违反了全局约束。

Conclusion: 本文提出了一种新颖的空间能力基准测试SCBench，揭示了当前前沿大模型在分层空间能力上存在单调递减的准确率模式，且模型的改进主要体现在低计算预算下的精度提升，而全局约束违反是主要失败原因。该基准为系统评估空间推理能力提供了新的工具和方法。
...
TL;DR: DERM-3R：一种资源高效的多模态智能体框架，用于在有限数据和计算下建模中医皮肤科诊断与治疗，通过三个协作智能体模拟中医工作流程，在103例银屑病数据上微调后表现优于大型模型。


<details>
  <summary>Details</summary>
Motivation: 皮肤疾病给全球带来巨大且日益增长的负担，现代疗法虽能快速控制急性症状，但长期效果常受单靶点范式、复发过程和系统性并发症关注不足的限制。中医提供了互补的整体疗法，但实践受限于知识非标准化、多模态记录不完整和专家推理的可扩展性差。

Method: 基于真实世界工作流程，将决策制定重构为三个核心问题，设计三个协作智能体（DERM-Rec, DERM-Rep, DERM-Reason）分别处理细粒度病变识别、多视角病变表示与专家级发病机制建模、整体推理进行证型鉴别和治疗规划，使用轻量级多模态LLM并在103个真实世界中医银屑病案例上进行部分微调。

Result: 尽管使用最小数据和参数更新，DERM-3R在皮肤科推理任务中表现优异，通过自动指标、LLM-as-a-judge评估和医师评估，显示其匹配甚至超越大型通用多模态模型。

Conclusion: DERM-3R框架通过结构化、领域感知的多智能体建模，为皮肤科和整合医学中复杂临床任务提供了实践可行的替代方案，证明了即使在有限数据和计算资源下，也能实现媲美甚至超越大型通用多模态模型的性能。
...
TL;DR: CID-TKG是一个用于时序知识图谱推理的协作学习框架，通过整合进化动力学和历史不变性语义作为有效的归纳偏置，在推理任务中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有时序知识图谱推理方法主要依赖时间不变或弱时间依赖的结构，忽视了进化动力学，导致固有的局限性。为了克服这一限制，需要开发能够同时捕捉长期结构规律和短期时间转换的框架。

Method: 提出CID-TKG框架，构建历史不变性图捕捉长期结构规律，构建进化动力学图建模短期时间转换。使用专用编码器学习每个结构的表示，通过将关系分解为视图特定表示，并使用对比目标对齐视图特定查询表示，以缓解两个结构间的语义差异。

Result: 广泛的实验验证表明，CID-TKG在外推设置下实现了最先进的性能。

Conclusion: CID-TKG通过整合进化动力学和历史不变性语义作为有效的归纳偏置，成功克服了现有方法的局限性，为时序知识图谱推理提供了新的协作学习框架。
...
### [19] [General-purpose LLMs as Models of Human Driver Behavior: The Case of Simplified Merging](https://arxiv.org/abs/2604.09609)
...
### [20] [Beyond Theory of Mind in Robotics](https://arxiv.org/abs/2604.09612)
*Malte F. Jung*
...
### [21] [The Geometry of Knowing: From Possibilistic Ignorance to Probabilistic Certainty -- A Measure-Theoretic Framework for Epistemic Convergence](https://arxiv.org/abs/2604.09614)
*Moriba Kemessia Jah*
...
### [22] [AdaQE-CG: Adaptive Query Expansion for Web-Scale Generative AI Model and Data Card Generation](https://arxiv.org/abs/2604.09617)
...
### [23] [Competing with AI Scientists: Agent-Driven Approach to Astrophysics Research](https://arxiv.org/abs/2604.09621)
...
### [24] [How LLMs Might Think](https://arxiv.org/abs/2604.09674)
...
### [25] [Belief-Aware VLM Model for Human-like Reasoning](https://arxiv.org/abs/2604.09686)
...
### [26] [Tipiano: Cascaded Piano Hand Motion Synthesis via Fingertip Priors](https://arxiv.org/abs/2604.09692)
...
### [27] [The Myth of Expert Specialization in MoEs: Why Routing Reflects Geometry, Not Necessarily Domain Expertise](https://arxiv.org/abs/2604.09780)
...
### [28] [Pioneer Agent: Continual Improvement of Small Language Models in Production](https://arxiv.org/abs/2604.09791)
...
### [29] [Controllable and Verifiable Tool-Use Data Synthesis for Agentic Reinforcement Learning](https://arxiv.org/abs/2604.09813)
...
### [30] [EE-MCP: Self-Evolving MCP-GUI Agents via Automated Environment Generation and Experience Learning](https://arxiv.org/abs/2604.09815)
...
### [31] [COMPOSITE-Stem](https://arxiv.org/abs/2604.09836)
...
### [32] [Steered LLM Activations are Non-Surjective](https://arxiv.org/abs/2604.09839)
...
### [33] [MEMENTO: Teaching LLMs to Manage Their Own Context](https://arxiv.org/abs/2604.09852)
...
### [34] [Instructing LLMs to Negotiate using Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2604.09855)
...
### [35] [Evolutionary Token-Level Prompt Optimization for Diffusion Models](https://arxiv.org/abs/2604.09861)
...
### [36] [What do your logits know? (The answer may surprise you!)](https://arxiv.org/abs/2604.09885)
...
### [37] [In-situ process monitoring for defect detection in wire-arc additive manufacturing: an agentic AI approach](https://arxiv.org/abs/2604.09889)
...
### [38] [GLEaN: A Text-to-image Bias Detection Approach for Public Comprehension](https://arxiv.org/abs/2604.09923)
...
### [39] [HealthAdminBench: Evaluating Computer-Use Agents on Healthcare Administration Tasks](https://arxiv.org/abs/2604.09937)
...
### [40] [New Hybrid Fine-Tuning Paradigm for LLMs: Algorithm Design and Convergence Analysis Framework](https://arxiv.org/abs/2604.09940)
...
### [41] [FinTrace: Holistic Trajectory-Level Evaluation of LLM Tool Calling for Long-Horizon Financial Tasks](https://arxiv.org/abs/2604.10015)
...
### [42] [AI Achieves a Perfect LSAT Score](https://arxiv.org/abs/2604.10034)
*Bonmu Ku*
...
### [43] [LoopGuard: Breaking Self-Reinforcing Attention Loops via Dynamic KV Cache Intervention](https://arxiv.org/abs/2604.10044)
...
### [44] [Learning Hierarchical and Geometry-Aware Graph Representations for Text-to-CAD](https://arxiv.org/abs/2604.10075)
...
### [45] [Ontological Trajectory Forecasting via Finite Semigroup Iteration and Lie Algebra Approximation in Geopolitical Knowledge Graphs](https://arxiv.org/abs/2604.10087)
*Qihang Wu*
...
### [46] [Trust Your Memory: Verifiable Control of Smart Homes through Reinforcement Learning with Multi-dimensional Rewards](https://arxiv.org/abs/2604.10110)
...
### [47] [Learning from Emptiness: De-biasing Listwise Rerankers with Content-Agnostic Probability Calibration](https://arxiv.org/abs/2604.10150)
...
### [48] [SpecMoE: A Fast and Efficient Mixture-of-Experts Inference via Self-Assisted Speculative Decoding](https://arxiv.org/abs/2604.10152)
...
### [49] [Inductive Reasoning for Temporal Knowledge Graphs with Emerging Entities](https://arxiv.org/abs/2604.10164)
...
### [50] [MAVEN-T: Multi-Agent enVironment-aware Enhanced Neural Trajectory predictor with Reinforcement Learning](https://arxiv.org/abs/2604.10169)
*Wenchang Duan*
...
### [51] [PoreDiT: A Scalable Generative Model for Large-Scale Digital Rock Reconstruction](https://arxiv.org/abs/2604.10171)
...
### [52] [Credit-Budgeted ICPC-Style Coding: When Agents Must Pay for Every Decision](https://arxiv.org/abs/2604.10182)
...
### [58] [STARS: Skill-Triggered Audit for Request-Conditioned Invocation Safety in Agent Systems](https://arxiv.org/abs/2604.10286)
...
### [59] [Dead Cognitions: A Census of Misattributed Insights](https://arxiv.org/abs/2604.10288)
...
### [60] [AI Organizations are More Effective but Less Aligned than Individual Agents](https://arxiv.org/abs/2604.10290)
...
### [61] [TimeSeriesExamAgent: Creating Time Series Reasoning Benchmarks at Scale](https://arxiv.org/abs/2604.10291)
...
### [63] [From GPT-3 to GPT-5: Mapping their capabilities, scope, limitations, and consequences](https://arxiv.org/abs/2604.10332)
...
### [64] [Zero-shot World Models Are Developmentally Efficient Learners](https://arxiv.org/abs/2604.10333)
...
### [65] [VeriTrans: Fine-Tuned LLM-Assisted NL-to-PL Translation via a Deterministic Neuro-Symbolic Pipeline](https://arxiv.org/abs/2604.10341)
...
### [66] [ClawVM: Harness-Managed Virtual Memory for Stateful Tool-Using LLM Agents](https://arxiv.org/abs/2604.10352)
...
### [67] [Beyond Monologue: Interactive Talking-Listening Avatar Generation with Conversational Audio Context-Aware Kernels](https://arxiv.org/abs/2604.10367)
...
### [68] [TrajOnco: a multi-agent framework for temporal reasoning over longitudinal EHR for multi-cancer early detection](https://arxiv.org/abs/2604.10386)
...
### [69] [CWCD: Category-Wise Contrastive Decoding for Structured Medical Report Generation](https://arxiv.org/abs/2604.10410)
...
### [70] [Safety Guarantees in Zero-Shot Reinforcement Learning for Cascade Dynamical Systems](https://arxiv.org/abs/2604.10429)
...
### [71] [VeriSim: A Configurable Framework for Evaluating Medical AI Under Realistic Patient Noise](https://arxiv.org/abs/2604.10441)
...
### [72] [PEMANT: Persona-Enriched Multi-Agent Negotiation for Travel](https://arxiv.org/abs/2604.10475)
...
### [73] [Tracing the Roots: A Multi-Agent Framework for Uncovering Data Lineage in Post-Training LLMs](https://arxiv.org/abs/2604.10480)
...
### [74] [CHAIRO: Contextual Hierarchical Analogical Induction and Reasoning Optimization for LLMs](https://arxiv.org/abs/2604.10502)
...
### [75] [CARO: Chain-of-Analogy Reasoning Optimization for Robust Content Moderation](https://arxiv.org/abs/2604.10504)
...
### [76] [Cooperation in Human and Machine Agents: Promise Theory Considerations](https://arxiv.org/abs/2604.10505)
*M. Burgess*
...
### [77] [A Progressive Training Strategy for Vision-Language Models to Counteract Spatio-Temporal Hallucinations in Embodied Reasoning](https://arxiv.org/abs/2604.10506)
...
### [78] [Beyond Compliance: A Resistance-Informed Motivation Reasoning Framework for Challenging Psychological Client Simulation](https://arxiv.org/abs/2604.10507)
...
### [79] [Thinking Fast, Thinking Wrong: Intuitiveness Modulates LLM Counterfactual Reasoning in Policy Evaluation](https://arxiv.org/abs/2604.10511)
*Yanjie He*
...
### [80] [Agent Mentor: Framing Agent Knowledge through Semantic Trajectory Analysis](https://arxiv.org/abs/2604.10513)
...
### [81] [From Perception to Planning: Evolving Ego-Centric Task-Oriented Spatiotemporal Reasoning via Curriculum Learning](https://arxiv.org/abs/2604.10517)
...
### [82] [Agent^2 RL-Bench: Can LLM Agents Engineer Agentic RL Post-Training?](https://arxiv.org/abs/2604.10547)
...
### [83] [Failure Ontology: A Lifelong Learning Framework for Blind Spot Detection and Resilience Design](https://arxiv.org/abs/2604.10549)
...
### [85] [Enhancing Cross-Problem Vehicle Routing via Federated Learning](https://arxiv.org/abs/2604.10652)
...
### [86] [Governed Reasoning for Institutional AI](https://arxiv.org/abs/2604.10658)
*Mamadou Seck*
...
### [87] [Preference-Agile Multi-Objective Optimization for Real-time Vehicle Dispatching](https://arxiv.org/abs/2604.10664)
...
### [88] [Principles Do Not Apply Themselves: A Hermeneutic Perspective on AI Alignment](https://arxiv.org/abs/2604.10673)
*Behrooz Razeghi*
...
### [89] [FedRio: Personalized Federated Social Bot Detection via Cooperative Reinforced Contrastive Adversarial Distillation](https://arxiv.org/abs/2604.10678)
...
### [90] [Do LLMs Build Spatial World Models? Evidence from Grid-World Maze Tasks](https://arxiv.org/abs/2604.10690)
...
### [91] [Camyla: Scaling Autonomous Research in Medical Image Segmentation](https://arxiv.org/abs/2604.10696)
...
### [92] [SciPredict: Can LLMs Predict the Outcomes of Scientific Experiments in Natural Sciences?](https://arxiv.org/abs/2604.10718)
...
### [93] [Teaching Language Models How to Code Like Learners: Conversational Serialization for Student Simulation](https://arxiv.org/abs/2604.10720)
...
### [94] [When More Thinking Hurts: Overthinking in LLM Test-Time Compute Scaling](https://arxiv.org/abs/2604.10739)
...
### [95] [Learning Preference-Based Objectives from Clinical Narratives for Sequential Treatment Decision-Making](https://arxiv.org/abs/2604.10783)
...
### [96] [TorchUMM: A Unified Multimodal Model Codebase for Evaluation, Analysis, and Post-training](https://arxiv.org/abs/2604.10784)
...
### [97] [CheeseBench: Evaluating Large Language Models on Rodent Behavioral Neuroscience Paradigms](https://arxiv.org/abs/2604.10825)
*Zacharie Bugaud*
...
### [98] [Your Model Diversity, Not Method, Determines Reasoning Strategy](https://arxiv.org/abs/2604.10827)
...
### [99] [A Benchmark for Gap and Overlap Analysis as a Test of KG Task Readiness](https://arxiv.org/abs/2604.10853)
...
### [100] [Beyond Statistical Co-occurrence: Unlocking Intrinsic Semantics for Tabular Data Clustering](https://arxiv.org/abs/2604.10865)
...
### [101] [A Quantitative Definition of Intelligence](https://arxiv.org/abs/2604.10873)
*Kang-Sin Choi*
...
### [102] [ZoomR: Memory Efficient Reasoning through Multi-Granularity Key Value Retrieval](https://arxiv.org/abs/2604.10898)
...
### [103] [CASK: Core-Aware Selective KV Compression for Reasoning Traces](https://arxiv.org/abs/2604.10900)
...
### [104] [EvoNash-MARL: A Closed-Loop Multi-Agent Reinforcement Learning Framework for Medium-Horizon Equity Allocation](https://arxiv.org/abs/2604.10911)
...
### [105] [CSPO: Alleviating Reward Ambiguity for Structured Table-to-LaTeX Generation](https://arxiv.org/abs/2604.10918)
...
### [106] [RAG-KT: Cross-platform Explainable Knowledge Tracing with Multi-view Fusion Retrieval Generation](https://arxiv.org/abs/2604.10960)
...
### [107] [Delving Aleatoric Uncertainty in Medical Image Segmentation via Vision Foundation Models](https://arxiv.org/abs/2604.10963)
...
### [108] [CFMS: A Coarse-to-Fine Multimodal Synthesis Framework for Enhanced Tabular Reasoning](https://arxiv.org/abs/2604.10973)
...
### [109] [ATANT v1.1: Positioning Continuity Evaluation Against Memory, Long-Context, and Agentic-Memory Benchmarks](https://arxiv.org/abs/2604.10981)
...
### [110] [Back to the Barn with LLAMAs: Evolving Pretrained LLM Backbones in Finetuning Vision Language Models](https://arxiv.org/abs/2604.10985)
...
### [111] [WebForge: Breaking the Realism-Reproducibility-Scalability Trilemma in Browser Agent Benchmark](https://arxiv.org/abs/2604.10988)
...
### [112] [MAFIG: Multi-agent Driven Formal Instruction Generation Framework](https://arxiv.org/abs/2604.10989)
...
### [113] [Sanity Checks for Agentic Data Science](https://arxiv.org/abs/2604.11003)
...
### [114] [Diffusion-CAM: Faithful Visual Explanations for dMLLMs](https://arxiv.org/abs/2604.11005)
...
### [115] [Min-$k$ Sampling: Decoupling Truncation from Temperature Scaling via Relative Logit Dynamics](https://arxiv.org/abs/2604.11012)
...
### [116] [Introspective Diffusion Language Models](https://arxiv.org/abs/2604.11035)
...
### [117] [Intelligent Approval of Access Control Flow in Office Automation Systems via Relational Modeling](https://arxiv.org/abs/2604.11040)
...
### [118] [From Topology to Trajectory: LLM-Driven World Models For Supply Chain Resilience](https://arxiv.org/abs/2604.11041)
*Jia Luo*
...
### [119] [EmergentBridge: Improving Zero-Shot Cross-Modal Transfer in Unified Multimodal Embedding Models](https://arxiv.org/abs/2604.11043)
...
### [120] [Hodoscope: Unsupervised Monitoring for AI Misbehaviors](https://arxiv.org/abs/2604.11072)
...
### [121] [Towards Proactive Information Probing: Customer Service Chatbots Harvesting Value from Conversation](https://arxiv.org/abs/2604.11077)
...
### [122] [Do Agent Rules Shape or Distort? Guardrails Beat Guidance in Coding Agents](https://arxiv.org/abs/2604.11088)
...
### [123] [Frugal Knowledge Graph Construction with Local LLMs: A Zero-Shot Pipeline, Self-Consistency and Wisdom of Artificial Crowds](https://arxiv.org/abs/2604.11104)
*Pierre Jourlin*
...
### [124] [Persona Non Grata: Single-Method Safety Evaluation Is Incomplete for Persona-Imbued LLMs](https://arxiv.org/abs/2604.11120)
...
### [125] [A Proposed Biomedical Data Policy Framework to Reduce Fragmentation, Improve Quality, and Incentivize Sharing in Indian Healthcare in the era of Artificial Intelligence and Digital Health](https://arxiv.org/abs/2604.11125)
...
### [126] [MADQRL: Distributed Quantum Reinforcement Learning Framework for Multi-Agent Environments](https://arxiv.org/abs/2604.11131)
...
### [127] [From Answers to Arguments: Toward Trustworthy Clinical Diagnostic Reasoning with Toulmin-Guided Curriculum Goal-Conditioned Learning](https://arxiv.org/abs/2604.11137)
...
### [128] [Environmental Footprint of GenAI Research: Insights from the Moshi Foundation Model](https://arxiv.org/abs/2604.11154)
...
### [129] [Mobile GUI Agent Privacy Personalization with Trajectory Induced Preference Optimization](https://arxiv.org/abs/2604.11259)
...
### [130] [Inspectable AI for Science: A Research Object Approach to Generative AI Governance](https://arxiv.org/abs/2604.11261)
...
### [131] [Consistency of AI-Generated Exercise Prescriptions: A Repeated Generation Study Using a Large Language Model](https://arxiv.org/abs/2604.11287)
*Kihyuk Lee*
...
### [132] [PaperScope: A Multi-Modal Multi-Document Benchmark for Agentic Deep Research Across Massive Scientific Papers](https://arxiv.org/abs/2604.11307)
...
### [133] [Select Smarter, Not More: Prompt-Aware Evaluation Scheduling with Submodular Guarantees](https://arxiv.org/abs/2604.11328)
...
### [134] [Dynamic Summary Generation for Interpretable Multimodal Depression Detection](https://arxiv.org/abs/2604.11334)
...
### [135] [CoRe-ECG: Advancing Self-Supervised Representation Learning for 12-Lead ECG via Contrastive and Reconstructive Synergy](https://arxiv.org/abs/2604.11359)
...
### [136] [The Missing Knowledge Layer in Cognitive Architectures for AI Agents](https://arxiv.org/abs/2604.11364)
*Michaël Roynard*
...
### [137] [Learning from Contrasts: Synthesizing Reasoning Paths from Diverse Search Trajectories](https://arxiv.org/abs/2604.11365)
...
### [138] [From Agent Loops to Structured Graphs:A Scheduler-Theoretic Framework for LLM Agent Execution](https://arxiv.org/abs/2604.11378)
*Hu Wei*
...
### [139] [Beyond RAG for Cyber Threat Intelligence: A Systematic Evaluation of Graph-Based and Agentic Retrieval](https://arxiv.org/abs/2604.11419)
...
### [140] [Escaping the Context Bottleneck: Active Context Curation for LLM Agents via Reinforcement Learning](https://arxiv.org/abs/2604.11462)
...
### [141] [Three Roles, One Model: Role Orchestration at Inference Time to Close the Performance Gap Between Small and Large Agents](https://arxiv.org/abs/2604.11465)
...
### [142] [From Attribution to Action: A Human-Centered Application of Activation Steering](https://arxiv.org/abs/2604.11467)
...
### [143] [OOM-RL: Out-of-Money Reinforcement Learning Market-Driven Alignment for LLM-Based Multi-Agent Systems](https://arxiv.org/abs/2604.11477)
*Kun Liu,Liqun Chen*
...
### [144] [On the Complexity of the Discussion-based Semantics in Abstraction Argumentation](https://arxiv.org/abs/2604.11480)
...
### [145] [Anthropogenic Regional Adaptation in Multimodal Vision-Language Model](https://arxiv.org/abs/2604.11490)
...
### [146] [Lectures on AI for Mathematics](https://arxiv.org/abs/2604.11504)
...
### [147] [PAC-BENCH: Evaluating Multi-Agent Collaboration under Privacy Constraints](https://arxiv.org/abs/2604.11523)
...
### [148] [Limited Perfect Monotonical Surrogates constructed using low-cost recursive linkage discovery with guaranteed output](https://arxiv.org/abs/2604.11524)
...
### [149] [Problem Reductions at Scale: Agentic Integration of Computationally Hard Problems](https://arxiv.org/abs/2604.11535)
...
### [150] [A collaborative agent with two lightweight synergistic models for autonomous crystal materials research](https://arxiv.org/abs/2604.11540)
...
### [151] [SemaClaw: A Step Towards General-Purpose Personal AI Agents through Harness Engineering](https://arxiv.org/abs/2604.11548)
...
### [152] [UniToolCall: Unifying Tool-Use Representation, Data, and Evaluation for LLM Agents](https://arxiv.org/abs/2604.11557)
...
### [153] [Intersectional Sycophancy: How Perceived User Demographics Shape False Validation in Large Language Models](https://arxiv.org/abs/2604.11609)
...
### [154] [Context Kubernetes: Declarative Orchestration of Enterprise Knowledge for Agentic AI Systems](https://arxiv.org/abs/2604.11623)
...
### [155] [RationalRewards: Reasoning Rewards Scale Visual Generation Both Training and Test Time](https://arxiv.org/abs/2604.11626)
...
### [156] [Why Do Large Language Models Generate Harmful Content?](https://arxiv.org/abs/2604.11663)
...
### [157] [DreamKG: A KG-Augmented Conversational System for People Experiencing Homelessness](https://arxiv.org/abs/2604.11703)
...
### [158] [Agentic Driving Coach: Robustness and Determinism of Agentic AI-Powered Human-in-the-Loop Cyber-Physical Systems](https://arxiv.org/abs/2604.11705)
...
### [159] [A Mamba-Based Multimodal Network for Multiscale Blast-Induced Rapid Structural Damage Assessment](https://arxiv.org/abs/2604.11709)
...
### [160] [SWE-AGILE: A Software Agent Framework for Efficiently Managing Dynamic Reasoning Context](https://arxiv.org/abs/2604.11716)
...
### [161] [Collaborative Multi-Agent Scripts Generation for Enhancing Imperfect-Information Reasoning in Murder Mystery Games](https://arxiv.org/abs/2604.11741)
...
### [162] [Retrieval Is Not Enough: Why Organizational AI Needs Epistemic Infrastructure](https://arxiv.org/abs/2604.11759)
...
### [163] [GenTac: Generative Modeling and Forecasting of Soccer Tactics](https://arxiv.org/abs/2604.11786)
...
### [164] [Detecting Safety Violations Across Many Agent Traces](https://arxiv.org/abs/2604.11806)
...
### [165] [The Diffusion-Attention Connection](https://arxiv.org/abs/2604.09560)
*Julio Candanedo*

Main category: cs.LG

TL;DR: 本文统一了Transformer、扩散映射和磁拉普拉斯算子，证明它们是同一马尔可夫几何在不同区域的表现，通过QK双向散度、专家乘积和薛定谔桥建立了系统连接框架。


<details>
  <summary>Details</summary>
Motivation: 目前Transformer、扩散映射和磁拉普拉斯算子通常被当作独立的工具使用，作者认为它们本质上是统一的数学框架在不同参数区域的表现形式。这种分离的认识限制了对其内在联系的理解，因此需要建立一个统一的理论框架来揭示它们之间的本质联系。

Method: 1. 从预softmax查询得分构建马尔可夫几何
2. 定义QK"双向散度"函数
3. 通过指数化和归一化处理得到注意力机制、扩散映射和磁扩散
4. 使用专家乘积方法
5. 运用薛定谔桥理论连接不同机制

Result: 成功证明了Transformer注意力机制、扩散映射和磁扩散都是同一马尔可夫几何体系的不同表现形式，并通过QK双向散度的指数归一化形式统一表示。建立了平衡态、非平衡稳态和驱动动力学之间的系统连接框架。

Conclusion: 本文提出了一种统一的数学框架，将Transformer、扩散映射和磁拉普拉斯算子视为同一马尔可夫几何的不同表现形式，通过QK双向散度、专家乘积和薛定谔桥等方法将它们系统性地联系起来，为理解这些看似分离的工具提供了统一的理论基础。
...
### [166] [Fairboard: a quantitative framework for equity assessment of healthcare models](https://arxiv.org/abs/2604.09656)
*James K. Ruffle,Samia Mohinta,Chris Foulon,Mohamad Zeina,Zicheng Wang,Sebastian Brandner,Harpreet Hyare,Parashkev Nachev*

Main category: cs.LG

TL;DR: 评估18个开源脑肿瘤分割模型的公平性，发现患者身份比模型选择更能影响性能，临床因素比架构更重要，模型存在脑区特异性偏差，发布了Fairboard公平性监控仪表板。


<details>
  <summary>Details</summary>
Motivation: 虽然已有超过1,000个FDA授权的AI医疗设备，但对模型在不同患者亚组中性能一致性的正式公平性评估却很罕见。本研究旨在评估脑肿瘤分割模型的公平性。

Method: 评估了18个开源脑肿瘤分割模型在648个胶质瘤患者上的表现，使用两个独立数据集（共11,664次模型推断）。评估维度包括：单变量分析、贝叶斯多变量分析、空间分析和表征分析。

Result: 1. 患者身份比模型选择更能解释性能方差；2. 临床因素（分子诊断、肿瘤等级、切除程度）比模型架构更能预测分割准确性；3. 体素级空间元分析发现特定脑区存在局部偏差，这些偏差具有区间特异性且在不同模型中表现一致；4. 在高维特征空间中，模型性能显著聚类，表明患者特征空间存在算法脆弱性轴；5. 新模型趋向更高公平性但无正式公平性保证。

Conclusion: 尽管新模型有向更高公平性发展的趋势，但没有模型提供正式的公平性保证。Fairboard开源仪表板的发布有望降低医疗影像中公平性监控的门槛。
...
### [167] [Deliberative Alignment is Deep, but Uncertainty Remains: Inference time safety improvement in reasoning via attribution of unsafe behavior to base model](https://arxiv.org/abs/2604.09665)
*Pankayaraj Pathmanathan,Furong Huang*

Main category: cs.LG

TL;DR: 本文研究了审议对齐在语言模型中的影响，发现教师模型与学生模型之间存在对齐差距，并提出了一种BoN采样方法，通过将不安全行为归因于基础模型来提升模型安全性，同时保持实用性。


<details>
  <summary>Details</summary>
Motivation: 尽管拒绝训练在大型语言模型中的广泛应用提升了模型安全性，但现有对齐方法存在浅层性问题。审议对齐通过从更强的推理模型蒸馏推理能力来建立更深层次的安全性，但其实际效果和影响需要深入研究。

Method: 首先分析教师模型与学生模型之间的对齐差距，发现审议对齐模型可能保留基础模型的不安全行为。基于此观察，提出BoN采样方法，在潜在空间中将不安全行为归因于基础LLM，从而降低不安全响应的排名。

Result: 在7个教师模型和6个不同类别和规模的学生模型中，BoN采样方法在多个安全基准测试中显著降低了攻击成功率：DAN降低28.2%，WildJailbreak降低31.3%，StrongREJECT降低35.4%。这些安全增益在RL训练后仍然保持。

Conclusion: 审议对齐存在教师-学生模型对齐差距，BoN采样方法能有效提升模型安全性并最小化实用性损失。研究还揭示了安全推理的不确定性，并强调将不安全行为明确归因于基础模型的重要性。
...
### [168] [Human-like Working Memory Interference in Large Language Models](https://arxiv.org/abs/2604.09670)
*Hua-Dong Xiong,Li Ji-An,Jiaqi Huang,Robert C. Wilson,Kwonjoon Lee,Xue-Xin Wei*

Main category: cs.LG

TL;DR: 研究发现预训练大语言模型存在表征干扰导致的工作记忆限制，这与人类记忆局限性相似，源于模型将多个记忆项编码为纠缠表征而非直接复制，需要通过干扰控制来隔离目标信息


<details>
  <summary>Details</summary>
Motivation: 探究为什么拥有1000亿参数并具有完全上下文访问能力的transformer架构大语言模型，会表现出与人类相似的工作记忆限制，理解这种限制的深层机制和计算原理

Method: 通过分析双层transformer在解决工作记忆任务的能力，比较多种预训练LLMs的表现，研究干扰特征（记忆负荷增加、新近性偏差、刺激统计偏差），并实施有针对性的干预措施来抑制刺激内容信息

Result: LLMs表现出与人类相似的工作记忆干扰特征；模型工作记忆能力与标准基准测试表现正相关；LLMs普遍采用多个记忆项纠缠表征的共同计算机制；成功回忆依赖于干扰控制；针对性干预提升表现提供因果关系证据

Conclusion: 研究发现了预训练大语言模型中表征干扰是工作记忆能力的核心约束因素，揭示了生物和人工智能系统在工作记忆限制上可能面临的共同计算挑战：在干扰下选择任务相关信息
...
### [169] [Belief-State RWKV for Reinforcement Learning under Partial Observability](https://arxiv.org/abs/2604.09671)
*Liu Xiao*

Main category: cs.LG

TL;DR: 本文提出了一种基于RWKV循环序列模型的强化学习新框架，将固定大小的循环状态显式解释为信念状态而非不透明的隐藏向量，通过维护紧凑的不确定性感知状态来改进部分可观测环境下的策略性能。


<details>
  <summary>Details</summary>
Motivation: 针对传统固定状态策略在部分可观测环境中的关键弱点：它们可以存储证据但无法存储置信度。现有方法使用单一汇总隐藏向量h_t，缺乏对不确定性的显式建模，限制了在噪声环境下的鲁棒性。

Method: 提出信念状态强化学习框架，将循环状态重新解释为信念状态b_t = (μ_t, Σ_t)，其中μ_t表示均值，Σ_t表示协方差，从RWKV风格的循环统计中推导得出。策略和价值函数同时依赖于记忆和不确定性，而非单一汇总向量。

Result: 在包含隐藏情节级观测噪声的试点强化学习实验中，信念状态策略几乎匹配最佳循环基线的整体性能，同时在最难分布内机制和保持噪声偏移下略微提高了回报。消融研究表明，这种简单的信念读取方法比两种更结构化的扩展（门控记忆控制和特权信念目标）表现更好。

Conclusion: 将固定大小循环状态显式解释为信念状态而非不透明隐藏向量，能够有效改进部分可观测环境下的强化学习性能。简单的信念读取方法已经展现出优势，但需要更丰富的基准测试来进一步验证和完善该方法。
...
### [170] [Active Inference with a Self-Prior in the Mirror-Mark Task](https://arxiv.org/abs/2604.09673)
*Dongmin Kim,Hoshinori Kanazawa,Yasuo Kuniyoshi*

Main category: cs.LG

TL;DR: 通过基于Transformer的自我先验和主动推理计算模型，模拟婴儿能够自发产生镜像自我识别测试中的行为（约70%成功率），为自我意识的发育提供了统一的自由能原理计算框架。


<details>
  <summary>Details</summary>
Motivation: 镜像自我识别测试被广泛用作自我意识的指标，但其背后的计算机制尚不清楚。研究旨在探索是否可以通过单一机制（自我先验）和主动推理框架，无需外部奖励就能自发产生镜像测试中的行为，从而为自我意识的发育起源提供计算解释。

Method: 使用基于Transformer实现的自我先验模型学习熟悉的多感官体验密度，通过主动推理驱动行为。模拟婴儿仅依靠视觉和本体感觉（无触觉输入），在面对镜子中的标记时，自我先验检测到与学习分布的差异，从而自发地导向标记指向行为。

Result: 模拟婴儿在约70%的情况下自发发现了镜子中自己脸上的贴纸并将其移除。预期自由能在贴纸移除后显著降低，证实自我先验作为区分自我与非自我的内部标准。跨模态采样进一步证明自我先验能够捕获视觉-本体感觉关联，作为概率性的身体模式。

Conclusion: 该研究提出了一个基于自我先验（self-prior）和主动推理的计算模型，能够自发产生镜像自我识别测试中的关键行为。自我先验作为区分自我与非自我的内部标准，不仅解释了镜像测试的行为表现，也为自我意识发展的研究提供了统一的计算框架。
...
### [171] [A Comparative Theoretical Analysis of Entropy Control Methods in Reinforcement Learning](https://arxiv.org/abs/2604.09676)
*Ming Lei,Christophe Baehr*

Main category: cs.LG

TL;DR: 本文对强化学习中两种熵控制策略进行了比较理论分析：传统熵正则化和基于协方差的机制，揭示了后者在保持渐近无偏性方面的优势


<details>
  <summary>Details</summary>
Motivation: 强化学习已成为增强大语言模型推理能力的关键方法，但可扩展训练常因策略熵快速崩溃而受阻，导致过早收敛和性能饱和。需要深入理解不同熵控制策略的理论特性

Method: 建立了softmax参数化下熵动力学的统一框架，证明熵变化由对数概率与logit更新的协方差控制。比较分析了传统熵正则化和基于协方差机制的理论特性

Result: 传统熵正则化引入密集、持续的偏差，修改了平稳条件，导致次优策略；而基于协方差的方法选择性地正则化高协方差token的子集，在正则化系数退火时实现渐近无偏性
...
### [172] [STaR-DRO: Stateful Tsallis Reweighting for Group-Robust Structured Prediction](https://arxiv.org/abs/2604.09737)
...
TL;DR: 本文提出了一个两部分的框架，用于可控推理和鲁棒微调，以解决结构化预测中的格式漂移、标签歧义、证据幻觉和元数据条件混淆等问题，并在临床通信挖掘任务上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 结构化预测面临本体约束标签生成、证据基础和有效结构构建的挑战，特别是在存在歧义、标签偏斜和异构群体难度的情况下。需要解决格式漂移、标签歧义、证据幻觉和元数据条件混淆等问题，以提高结构化生成的可靠性和准确性。

Method: 1. 任务无关的提示策略：结合XML指令结构、消歧规则、验证式推理、模式约束和自验证，解决上下文结构化生成中的各种问题。
2. STaR-DRO鲁棒优化方法：结合Tsallis镜像下降与动量平滑、中心化的群体损失信号和有界超额乘子，仅对持续困难的群体进行加权，避免波动性大的指数梯度重加权和不必要地降低简单群体的权重。

Result: 在EPPC Miner基准测试中，提示工程将四个Llama模型的零样本性能平均提高了+15.44 F1（跨代码、子代码和跨度）。在监督微调基础上，STaR-DRO进一步提升了最困难的语义决策：在Llama-3.3-70B-Instruct上，代码F1从79.24提高到81.47，子代码F1从67.78提高到69.30，同时保持跨度性能，并在最困难的临床类别上将群体验证交叉熵降低了29.6%。

Conclusion: 该框架通过可控推理和鲁棒微调的组合方法，有效解决了结构化预测中的关键挑战，特别是在临床通信挖掘等实际应用中，对罕见和困难群体的性能提升直接增强了患者中心护理分析的通信挖掘可靠性。
...
### [173] [Efficient Matrix Implementation for Rotary Position Embedding](https://arxiv.org/abs/2604.09742)
*Chen Minqi,Zhongqi Yue,Shihao Zhang,Yun Xu,Peng Wu,kaixiang Xu,Zeyi Huang,Hanwang Zhang*

Main category: cs.LG

TL;DR: 提出RoME（Rotary Matrix position Embedding）作为RoPE的高效矩阵实现，通过统一矩阵变换替代向量操作，消除维度特定操作，实现在NPU上的并行执行加速。


<details>
  <summary>Details</summary>
Motivation: RoPE在现代Transformer架构中广泛应用，但现有实现依赖向量级的分割与合并操作，带来不可忽视的计算开销。在多维（2D和3D）场景下，额外向量操作和不均衡特征划分进一步降低硬件利用率。

Method: 提出RoME方法，将RoPE从向量操作重新表述为统一的矩阵变换，消除维度特定操作，简化实现，并允许在现代NPU的Cube和Vector单元上融合并行执行。

Result: 实验表明RoME在算子级别和完整模型级别均能带来显著加速效果，实现了更高效的计算性能。

Conclusion: RoME通过矩阵化重构提供了与RoPE数学等价但计算更高效的实现方案，解决了传统RoPE实现中的计算效率问题，适合现代硬件架构。
...
### [174] [Explainable Human Activity Recognition: A Unified Review of Concepts and Mechanisms](https://arxiv.org/abs/2604.09799)
*Mainak Kundu,Catherine Chen,Rifatul Islam,Ismail Uysal,Ria Kanjilal*

Main category: cs.LG

TL;DR: 本文对可解释人工智能在人类活动识别领域的应用进行了全面综述，提出了一个统一的解释性框架，并建立了基于解释机制的分类体系，旨在推动构建更透明、可信赖的活动识别系统。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习显著提升了人类活动识别的性能，但由此产生的模型往往不透明，限制了信任度、可靠性和实际部署。可解释人工智能因此成为使HAR系统更加透明和以人为本的关键方向。

Method: 本文提出了一个统一的视角，将解释性的概念维度与算法解释机制分开，减少了先前综述中的模糊性。基于这一区分，建立了一个机制中心的XAI-HAR方法分类体系，涵盖了主要的解释范式。综述还考察了这些方法如何处理HAR的时间、多模态和语义复杂性。

Result: 本文系统总结了可解释HAR方法的可解释性目标、解释目标和局限性，分析了当前的评估实践，并突出了实现可靠和可部署XAI-HAR的关键挑战。

Conclusion: 本文为构建更透明、可信赖的人类活动识别系统提供了系统性的综述和框架，指出了未来研究方向，以更好地支持人类理解和决策制定。
...
### [175] [Below-ground Fungal Biodiversity Can be Monitored Using Self-Supervised Learning Satellite Features](https://arxiv.org/abs/2604.09818)
*Robin Young,Michael E. Van Nuland,E. Toby Kiers,Tomáš Větrovský,Petr Kohout,Petr Baldrian,Srinivasan Keshav*

Main category: cs.LG

TL;DR: 利用自监督学习处理卫星图像，可高分辨率预测地下外生菌根真菌多样性，实现从公里级到米级的监测突破


<details>
  <summary>Details</summary>
Motivation: 目前90%的菌根真菌多样性热点未受保护，传统监测方法受限于时间和成本，难以在景观尺度上有效监测地下真菌群落

Method: 采用自监督学习（SSL）处理卫星图像，结合欧洲和亚洲约12,000个野外样本数据，预测地下外生菌根真菌丰富度

Result: 模型解释了超过一半的物种丰富度方差，SSL特征成为最具信息量的预测因子，空间分辨率提高10,000倍（从1km到10m），首次实现景观尺度下地下生物多样性的时间动态监测
...
### [176] [Relational Preference Encoding in Looped Transformer Internal States](https://arxiv.org/abs/2604.09870)
*Jan Kirin*
...
### [177] [Efficient Personalization of Generative User Interfaces](https://arxiv.org/abs/2604.09876)
...
### [178] [SemEnrich: Self-Supervised Semantic Enrichment of Radiology Reports for Vision-Language Learning](https://arxiv.org/abs/2604.09887)
...
### [179] [Improving Pediatric Emergency Department Triage with Modality Dropout in Late Fusion Multimodal EHR Models](https://arxiv.org/abs/2604.09905)
...
### [180] [Last-Iterate Convergence of Randomized Kaczmarz and SGD with Greedy Step Size](https://arxiv.org/abs/2604.09909)
...
### [181] [Regularized Entropy Information Adaptation with Temporal-Awareness Networks for Simultaneous Speech Translation](https://arxiv.org/abs/2604.09916)
...
### [182] [A Tale of Two Temperatures: Simple, Efficient, and Diverse Sampling from Diffusion Language Models](https://arxiv.org/abs/2604.09921)
...
### [183] [K-STEMIT: Knowledge-Informed Spatio-Temporal Efficient Multi-Branch Graph Neural Network for Subsurface Stratigraphy Thickness Estimation from Radar Data](https://arxiv.org/abs/2604.09922)
...
### [184] [A Hybrid Intelligent Framework for Uncertainty-Aware Condition Monitoring of Industrial Systems](https://arxiv.org/abs/2604.09932)
...
### [185] [Vestibular reservoir computing](https://arxiv.org/abs/2604.09943)
...
### [186] [SLM Finetuning for Natural Language to Domain Specific Code Generation in Production](https://arxiv.org/abs/2604.09952)
...
### [187] [From Recency Bias to Stable Convergence Block Kaczmarz Methods for Online Preference Learning in Matchmaking Applications](https://arxiv.org/abs/2604.09964)
*James Nguyen*
...
### [188] [Muon$^2$: Boosting Muon via Adaptive Second-Moment Preconditioning](https://arxiv.org/abs/2604.09967)
...
### [189] [LoDAdaC: a unified local training-based decentralized framework with adaptive gradients and compressed communication](https://arxiv.org/abs/2604.09970)
...
### [190] [Towards Multi-Source Domain Generalization for Sleep Staging with Noisy Labels](https://arxiv.org/abs/2604.10009)
...
### [191] [Closed-Form Concept Erasure via Double Projections](https://arxiv.org/abs/2604.10032)
...
### [192] [Cross-Validated Cross-Channel Self-Attention and Denoising for Automatic Modulation Classification](https://arxiv.org/abs/2604.10054)
...
### [193] [When Can You Poison Rewards? A Tight Characterization of Reward Poisoning in Linear MDPs](https://arxiv.org/abs/2604.10062)
...
### [194] [Transformers Learn the Optimal DDPM Denoiser for Multi-Token GMMs](https://arxiv.org/abs/2604.10074)
...
### [195] [Attention Sink in Transformers: A Survey on Utilization, Interpretation, and Mitigation](https://arxiv.org/abs/2604.10098)
...
### [196] [End-to-end Automated Deep Neural Network Optimization for PPG-based Blood Pressure Estimation on Wearables](https://arxiv.org/abs/2604.10117)
...
### [197] [Consensus-based Recursive Multi-Output Gaussian Process](https://arxiv.org/abs/2604.10146)
...
### [198] [A Temporally Augmented Graph Attention Network for Affordance Classification](https://arxiv.org/abs/2604.10149)
...
### [199] [Tracing the Thought of a Grandmaster-level Chess-Playing Transformer](https://arxiv.org/abs/2604.10158)
...
### [200] [Virtual Smart Metering in District Heating Networks via Heterogeneous Spatial-Temporal Graph Neural Networks](https://arxiv.org/abs/2604.10166)
...
### [201] [Wolkowicz-Styan Upper Bound on the Hessian Eigenspectrum for Cross-Entropy Loss in Nonlinear Smooth Neural Networks](https://arxiv.org/abs/2604.10202)
...
### [202] [Mild Over-Parameterization Benefits Asymmetric Tensor PCA](https://arxiv.org/abs/2604.10208)
...
### [203] [Exploring the impact of fairness-aware criteria in AutoML](https://arxiv.org/abs/2604.10224)
...
### [204] [A Multi-head Attention Fusion Network for Industrial Prognostics under Discrete Operational Conditions](https://arxiv.org/abs/2604.10248)
...
### [205] [The Phase Is the Gradient: Equilibrium Propagation for Frequency Learning in Kuramoto Networks](https://arxiv.org/abs/2604.10272)
*Mani Rash Ahmadi*
...
### [206] [A Diffusion-Contrastive Graph Neural Network with Virtual Nodes for Wind Nowcasting in Unobserved Regions](https://arxiv.org/abs/2604.10328)
...
### [207] [Integrating SAINT with Tree-Based Models: A Case Study in Employee Attrition Prediction](https://arxiv.org/abs/2604.10337)
...
### [208] [WaterAdmin: Orchestrating Community Water Distribution Optimization via AI Agents](https://arxiv.org/abs/2604.10343)
...
### [209] [Battery health prognosis using Physics-informed neural network with Quantum Feature mapping](https://arxiv.org/abs/2604.10362)
...
### [210] [Structural Gating and Effect-aligned Lag-resolved Temporal Causal Discovery Framework with Application to Heat-Pollution Extremes](https://arxiv.org/abs/2604.10371)
*Rui Chen,Jinsong Wu*
...
### [211] [Intent-aligned Formal Specification Synthesis via Traceable Refinement](https://arxiv.org/abs/2604.10392)
...
### [212] [Latent Instruction Representation Alignment: defending against jailbreaks, backdoors and undesired knowledge in LLMs](https://arxiv.org/abs/2604.10403)
...
### [213] [CARE-ECG: Causal Agent-based Reasoning for Explainable and Counterfactual ECG Interpretation](https://arxiv.org/abs/2604.10420)
...
### [214] [Replicable Composition](https://arxiv.org/abs/2604.10423)
...
### [215] [Membership Inference Attacks Expose Participation Privacy in ECG Foundation Encoders](https://arxiv.org/abs/2604.10424)
...
### [216] [Towards Green Wearable Computing: A Physics-Aware Spiking Neural Network for Energy-Efficient IMU-based Human Activity Recognition](https://arxiv.org/abs/2604.10458)
...
### [217] [Rethinking the Diffusion Model from a Langevin Perspective](https://arxiv.org/abs/2604.10465)
...
### [218] [Exact Finite-Sample Variance Decomposition of Subagging: A Spectral Filtering Perspective](https://arxiv.org/abs/2604.10469)
...
### [219] [CodeQuant: Unified Clustering and Quantization for Enhanced Outlier Smoothing in Low-Precision Mixture-of-Experts](https://arxiv.org/abs/2604.10496)
...
### [220] [IceCache: Memory-efficient KV-cache Management for Long-Sequence LLMs](https://arxiv.org/abs/2604.10539)
...
### [221] [WaveMoE: A Wavelet-Enhanced Mixture-of-Experts Foundation Model for Time Series Forecasting](https://arxiv.org/abs/2604.10544)
...
### [222] [Topology-Aware PAC-Bayesian Generalization Analysis for Graph Neural Networks](https://arxiv.org/abs/2604.10553)
*Xinping Yi*
...
### [223] [Heterogeneous Connectivity in Sparse Networks: Fan-in Profiles, Gradient Hierarchy, and Topological Equilibria](https://arxiv.org/abs/2604.10560)
*Nikodem Tomczak*
...
### [224] [ReadMOF: Structure-Free Semantic Embeddings from Systematic MOF Nomenclature for Machine Learning](https://arxiv.org/abs/2604.10568)
...
### [225] [WOODELF-HD: Efficient Background SHAP for High-Depth Decision Trees](https://arxiv.org/abs/2604.10569)
...
### [226] [Calibration Collapse Under Sycophancy Fine-Tuning: How Reward Hacking Breaks Uncertainty Quantification in LLMs](https://arxiv.org/abs/2604.10585)
*Subramanyam Sahoo*
...
### [227] [Preventing Latent Rehearsal Decay in Online Continual SSL with SOLAR](https://arxiv.org/abs/2604.10586)
...
### [228] [Distributionally Robust PAC-Bayesian Control](https://arxiv.org/abs/2604.10588)
...
### [229] [MoEITS: A Green AI approach for simplifying MoE-LLMs](https://arxiv.org/abs/2604.10603)
...
### [230] [Mitigating Privacy Risk via Forget Set-Free Unlearning](https://arxiv.org/abs/2604.10636)
...
### [231] [Energy-Efficient Federated Edge Learning For Small-Scale Datasets in Large IoT Networks](https://arxiv.org/abs/2604.10662)
...
### [232] [Skill-SD: Skill-Conditioned Self-Distillation for Multi-turn LLM Agents](https://arxiv.org/abs/2604.10674)
...
### [233] [SCOPE: Signal-Calibrated On-Policy Distillation Enhancement with Dual-Path Adaptive Weighting](https://arxiv.org/abs/2604.10688)
...
### [234] [Communication-Efficient Gluon in Federated Learning](https://arxiv.org/abs/2604.10689)
...
### [235] [Bringing Value Models Back: Generative Critics for Value Modeling in LLM Reinforcement Learning](https://arxiv.org/abs/2604.10701)
...
### [236] [INCRT: An Incremental Transformer That Determines Its Own Architecture](https://arxiv.org/abs/2604.10703)
...
### [237] [PokeRL: Reinforcement Learning for Pokemon Red](https://arxiv.org/abs/2604.10812)
...
### [238] [Online Covariance Estimation in Averaged SGD: Improved Batch-Mean Rates and Minimax Optimality via Trajectory Regression](https://arxiv.org/abs/2604.10814)
...
### [239] [Slithering Through Gaps: Capturing Discrete Isolated Modes via Logistic Bridging](https://arxiv.org/abs/2604.10821)
...
### [240] [Transformers Learn Latent Mixture Models In-Context via Mirror Descent](https://arxiv.org/abs/2604.10848)
...
### [241] [Task2vec Readiness: Diagnostics for Federated Learning from Pre-Training Embeddings](https://arxiv.org/abs/2604.10849)
...
### [242] [Query Lower Bounds for Diffusion Sampling](https://arxiv.org/abs/2604.10857)
...
### [243] [DIB-OD: Preserving the Invariant Core for Robust Heterogeneous Graph Adaptation via Decoupled Information Bottleneck and Online Distillation](https://arxiv.org/abs/2604.10882)
...
### [244] [Learning to Adapt: In-Context Learning Beyond Stationarity](https://arxiv.org/abs/2604.10946)
...
### [245] [UniPROT: Uniform Prototype Selection via Partial Optimal Transport with Submodular Guarantees](https://arxiv.org/abs/2604.10952)
...
### [246] [Hypergraph Neural Diffusion: A PDE-Inspired Framework for Hypergraph Message Passing](https://arxiv.org/abs/2604.10955)
...
### [247] [Continuous-time Online Learning via Mean-Field Neural Networks: Regret Analysis in Diffusion Environments](https://arxiv.org/abs/2604.10958)
...
### [248] [Learning to Test: Physics-Informed Representation for Dynamical Instability Detection](https://arxiv.org/abs/2604.10967)
...
### [249] [Robust Adversarial Policy Optimization Under Dynamics Uncertainty](https://arxiv.org/abs/2604.10974)
...
### [250] [Tracking High-order Evolutions via Cascading Low-rank Fitting](https://arxiv.org/abs/2604.10980)
*Zhao Song*
...
### [251] [Flow-Controlled Scheduling for LLM Inference with Provable Stability Guarantees](https://arxiv.org/abs/2604.11001)
...
### [252] [K-Way Energy Probes for Metacognition Reduce to Softmax in Discriminative Predictive Coding Networks](https://arxiv.org/abs/2604.11011)
*Jon-Paul Cacioli*
...
### [253] [Optimal Stability of KL Divergence under Gaussian Perturbations](https://arxiv.org/abs/2604.11026)
...
### [254] [RTMC: Step-Level Credit Assignment via Rollout Trees](https://arxiv.org/abs/2604.11037)
...
### [255] [Rethinking Token-Level Credit Assignment in RLVR: A Polarity-Entropy Analysis](https://arxiv.org/abs/2604.11056)
...
### [256] [Pando: Do Interpretability Methods Work When Models Won't Explain Themselves?](https://arxiv.org/abs/2604.11061)
...
### [257] [A Faster Path to Continual Learning](https://arxiv.org/abs/2604.11064)
...
### [258] [CausalGaze: Unveiling Hallucinations via Counterfactual Graph Intervention in Large Language Models](https://arxiv.org/abs/2604.11087)
...
### [259] [Quantum-Gated Task-interaction Knowledge Distillation for Pre-trained Model-based Class-Incremental Learning](https://arxiv.org/abs/2604.11112)
...
### [260] [Distributionally Robust K-Means Clustering](https://arxiv.org/abs/2604.11118)
...
### [261] [Reducing Hallucination in Enterprise AI Workflows via Hybrid Utility Minimum Bayes Risk (HUMBR)](https://arxiv.org/abs/2604.11141)
...
### [262] [A Full Compression Pipeline for Green Federated Learning in Communication-Constrained Environments](https://arxiv.org/abs/2604.11146)
...
### [263] [Gradient-Variation Regret Bounds for Unconstrained Online Learning](https://arxiv.org/abs/2604.11151)
...
### [264] [Towards Situation-aware State Modeling for Air Traffic Flow Prediction](https://arxiv.org/abs/2604.11198)
...
### [265] [ShapShift: Explaining Model Prediction Shifts with Subgroup Conditional Shapley Values](https://arxiv.org/abs/2604.11200)
...
### [266] [Unified Graph Prompt Learning via Low-Rank Graph Message Prompting](https://arxiv.org/abs/2604.11257)
...
### [267] [AbLWR:A Context-Aware Listwise Ranking Framework for Antibody-Antigen Binding Affinity Prediction via Positive-Unlabeled Learning](https://arxiv.org/abs/2604.11272)
...
### [268] [Mycelium-Index: A Streaming Approximate Nearest Neighbor Index with Myelial Edge Decay, Traffic-Driven Reinforcement, and Adaptive Living Hierarchy](https://arxiv.org/abs/2604.11274)
*Anton Pakhunov*
...
### [269] [Sheaf Diffusion with Adaptive Local Structure for Spatio-Temporal Forecasting](https://arxiv.org/abs/2604.11275)
...
### [270] [Representation-Aligned Multi-Scale Personalization for Federated Learning](https://arxiv.org/abs/2604.11278)
...
### [271] [THEIA: Learning Complete Kleene Three-Valued Logic in a Pure-Neural Modular Architecture](https://arxiv.org/abs/2604.11284)
*Augustus Haoyang Li*
...
### [272] [The Past Is Not Past: Memory-Enhanced Dynamic Reward Shaping](https://arxiv.org/abs/2604.11297)
...
### [273] [Beyond Fixed False Discovery Rates: Post-Hoc Conformal Selection with E-Variables](https://arxiv.org/abs/2604.11305)
...
### [274] [Learning Discrete Diffusion of Graphs via Free-Energy Gradient Flows](https://arxiv.org/abs/2604.11311)
...
### [275] [S$^3$: Structured Sparsity Specification](https://arxiv.org/abs/2604.11315)
*Ayoub Ghriss*
...
### [276] [Active Bayesian Inference for Robust Control under Sensor False Data Injection Attacks](https://arxiv.org/abs/2604.11410)
...
### [277] [Exact Certification of Neural Networks and Partition Aggregation Ensembles against Label Poisoning](https://arxiv.org/abs/2604.11416)
...
### [278] [Emulating Non-Differentiable Metrics via Knowledge-Guided Learning: Introducing the Minkowski Image Loss](https://arxiv.org/abs/2604.11422)
...
### [279] [Low-rank Optimization Trajectories Modeling for LLM RLVR Acceleration](https://arxiv.org/abs/2604.11446)
...
### [280] [Learning How Much to Think: Difficulty-Aware Dynamic MoEs for Graph Node Classification](https://arxiv.org/abs/2604.11473)
...
### [281] [Structural Consequences of Policy-Based Interventions on the Global Supply Chain Network](https://arxiv.org/abs/2604.11479)
...
### [282] [CAGenMol: Condition-Aware Diffusion Language Model for Goal-Directed Molecular Generation](https://arxiv.org/abs/2604.11483)
...
### [283] [Quantization Dominates Rank Reduction for KV-Cache Compression](https://arxiv.org/abs/2604.11501)
*Samuel Salfati*
...
### [284] [Not All Forgetting Is Equal: Architecture-Dependent Retention Dynamics in Fine-Tuned Image Classifiers](https://arxiv.org/abs/2604.11508)
...
### [285] [Generative Path-Finding Method for Wasserstein Gradient Flow](https://arxiv.org/abs/2604.11519)
...
### [286] [Continuous Adversarial Flow Models](https://arxiv.org/abs/2604.11521)
...
### [287] [TempusBench: An Evaluation Framework for Time-Series Forecasting](https://arxiv.org/abs/2604.11529)
...
### [288] [Eliciting Medical Reasoning with Knowledge-enhanced Data Synthesis: A Semi-Supervised Reinforcement Learning Approach](https://arxiv.org/abs/2604.11547)
...
### [289] [bacpipe: a Python package to make bioacoustic deep learning models accessible](https://arxiv.org/abs/2604.11560)
...
### [290] [Layerwise Dynamics for In-Context Classification in Transformers](https://arxiv.org/abs/2604.11613)
...
### [291] [SCNO: Spiking Compositional Neural Operator -- Towards a Neuromorphic Foundation Model for Nuclear PDE Solving](https://arxiv.org/abs/2604.11625)
...
### [292] [Inter-Layer Hessian Analysis of Neural Networks with DAG Architectures](https://arxiv.org/abs/2604.11639)
...
### [293] [Towards Autonomous Mechanistic Reasoning in Virtual Cells](https://arxiv.org/abs/2604.11661)
...
### [294] [Fairness is Not Flat: Geometric Phase Transitions Against Shortcut Learning](https://arxiv.org/abs/2604.11704)
...
### [295] [KL Divergence Between Gaussians: A Step-by-Step Derivation for the Variational Autoencoder Objective](https://arxiv.org/abs/2604.11744)
...
### [296] [Autonomous Diffractometry Enabled by Visual Reinforcement Learning](https://arxiv.org/abs/2604.11773)
...
### [297] [ClawGUI: A Unified Framework for Training, Evaluating, and Deploying GUI Agents](https://arxiv.org/abs/2604.11784)
...
### [298] [A Mechanistic Analysis of Looped Reasoning Language Models](https://arxiv.org/abs/2604.11791)
...
### [299] [Solving Physics Olympiad via Reinforcement Learning on Physics Simulators](https://arxiv.org/abs/2604.11805)
...
### [300] [Physics-Informed State Space Models for Reliable Solar Irradiance Forecasting in Off-Grid Systems](https://arxiv.org/abs/2604.11807)
