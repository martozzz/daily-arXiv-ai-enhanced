...
TL;DR: This paper addresses safety vulnerabilities in Medical MLLMs by proposing a novel 'Parameter-Space Intervention' method, enhancing safety without compromising medical performance.


<details>
  <summary>Details</summary>
Motivation: The research is motivated by the lag in safety research for Medical MLLMs, which poses risks for real-world deployment, and the identified vulnerabilities in existing models.

Method: The authors establish a multidimensional evaluation framework to benchmark safety, propose a 'Parameter-Space Intervention' approach for safety re-alignment, and design a fine-grained parameter search algorithm.

Result: Experimental results show that the approach significantly improves safety guardrails without relying on additional safety data and minimizes performance degradation.

Conclusion: The paper concludes that the proposed 'Parameter-Space Intervention' approach effectively enhances the safety of Medical MLLMs without significantly compromising their medical performance, addressing the identified vulnerabilities.
...
TL;DR: 研究提出一种生物启发的闭环控制框架，显著提升AI推理能源效率，减少42%处理时间且精度损失极小。


<details>
  <summary>Details</summary>
Motivation: AI部署中的能源效率至关重要，因为长期运行的推理可能在累积碳影响上超过训练阶段。研究旨在通过生物启发的方法优化能源使用。

Method: 提出了一种受蛋白质折叠能量盆地启发的框架，通过衰减闭环阈值控制推理执行，优先选择成本效益高的局部最优解而非全局最优解。评估了DistilBERT和ResNet-18在FastAPI、ONNX Runtime和NVIDIA Triton上的性能。

Result: 生物控制器将处理时间减少了42%（从0.50秒降至0.29秒），精度损失小于0.5%。同时确定了轻量级本地服务（ORT）和托管批处理（Triton）之间的效率边界。

Conclusion: 该研究通过受生物启发的闭环控制框架，显著提高了AI推理的能源效率，减少了处理时间且仅导致微小的精度损失。
...
TL;DR: CAST框架通过选择性更新参数，减少LLM安全对齐中的通用能力损失。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）中的安全对齐本质上是一个多目标优化冲突，通常伴随着通用能力的意外下降。现有的缓解策略通常依赖于全局梯度几何来解决这些冲突，但忽略了Transformer中的模块异质性。

Method: CAST框架首先通过综合优化冲突和功能敏感性构建预对齐冲突图，然后指导参数的选择性更新。

Result: 实验表明，LLM中的对齐冲突并非均匀分布。更新一小部分高冲突头是导致通用能力下降的主要原因。

Conclusion: 通过跳过训练过程中的高冲突注意力头，CAST框架显著减少了通用能力的损失，同时不损害安全性，为安全与效用的权衡提供了一种可解释且参数高效的方法。
...
TL;DR: The paper introduces Temporal Saliency Distillation (TSD) for interpretable knowledge distillation in time series, addressing the limitations of current methods by transferring not just predictions but also the teacher's reasoning.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of current knowledge distillation methods in time series, which rely on uninterpretable logits and features, and fail to transfer comprehensive knowledge. These methods result in student models with predictive distributions that differ significantly from their teachers, limiting their practical substitution.

Method: The method proposed is Temporal Saliency Distillation (TSD), which extends conventional logit transfer by incorporating temporal saliency—capturing the importance of each input timestep to the teacher's prediction. This approach requires no additional parameters or architecture-specific assumptions.

Result: The results demonstrate that Temporal Saliency Distillation effectively enhances the performance of baseline methods and achieves properties beyond predictive accuracy, ensuring the student model aligns with the teacher's reasoning.

Conclusion: The paper concludes that Temporal Saliency Distillation (TSD) offers a new and effective paradigm for interpretable knowledge distillation in time series analysis. It not only improves predictive accuracy but also ensures that the student model aligns with the teacher's reasoning, making it a safer substitute.
...
TL;DR: MemKD是一种新颖的知识蒸馏框架，专门针对时间序列模型，通过捕捉记忆保留差异，实现了高效的小模型训练。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在时间序列数据分析中表现出色，但其高计算复杂性和大模型尺寸在资源受限环境中部署存在挑战。现有的知识蒸馏方法忽视了时间序列模型的独特时间依赖性和记忆保留特性。

Method: MemKD利用专门的损失函数来捕捉教师模型和学生模型在时间序列子序列中的记忆保留差异，确保学生模型有效模仿教师模型的行为。

Result: MemKD显著优于现有知识蒸馏方法，减少了参数量和内存使用，同时保持高性能。

Conclusion: MemKD显著优于现有知识蒸馏方法，在保持与教师模型相当性能的同时，将参数量和内存使用减少了约500倍。
...
TL;DR: 本研究使用强化学习在线学习气候模型参数化方案，显著提升了模型性能，特别是在热带和中纬度区域。


<details>
  <summary>Details</summary>
Motivation: 传统的参数化方案依赖于离线调整的固定系数，约束较弱，导致持续偏差，限制了其适应基础物理的能力。

Method: 本研究提出了一个框架，通过强化学习在线学习参数化方案的组成部分，并在理想化测试平台上评估了RL驱动的参数更新，涵盖了简单气候偏差校正（SCBC）、辐射对流平衡（RCE）和纬向平均能量平衡模型（EBM）。

Result: 在九种RL算法中，TQC、DDPG和TD3表现出最高的技能和最稳定的收敛性。在EBM中，单智能体RL优于静态参数调整，而多智能体设置中的联邦RL实现了地理专业化控制和更快的收敛。

Conclusion: 强化学习（RL）能够提供依赖于状态和感知机制的参数化方案，为数值模型中的在线学习提供了可扩展的路径。
...
### [7] [Unlocking the Pre-Trained Model as a Dual-Alignment Calibrator for Post-Trained LLMs](https://arxiv.org/abs/2601.04277)
*Beier Luo,Cheng Wang,Hongxin Wei,Sharon Li,Xuefeng Du*

Main category: cs.LG

TL;DR: Dual-Align is a post-hoc method that aligns both confidence and intermediate inference processes in post-trained LMs, improving calibration without losing performance.


<details>
  <summary>Details</summary>
Motivation: Post-training often worsens confidence calibration in large language models, leading to systematic overconfidence. Existing methods overlook inference-time dynamics, which are crucial for addressing calibration errors.

Method: Dual-Align performs confidence alignment via final-distribution matching and introduces process alignment by realigning the stability of intermediate inference pathways.

Result: Dual-Align consistently improves calibration over baselines, reducing errors and approaching a supervised oracle.

Conclusion: Dual-Align is an effective unsupervised post-hoc framework that addresses both confidence drift and process drift in post-trained language models, improving calibration without sacrificing performance gains.
...
### [8] [Generation of synthetic delay time series for air transport applications](https://arxiv.org/abs/2601.04279)
*Pau Esteve,Massimiliano Zanin*

Main category: cs.LG

TL;DR: 论文探讨了合成数据生成在解决数据稀缺和隐私问题中的应用，比较了三种模型生成机场延误时间序列的效果，并验证了其在检测延误传播中的实用性。


<details>
  <summary>Details</summary>
Motivation: 解决数据稀缺和隐私问题，特别是在航空运输领域生成真实且多变的机场延误时间序列。

Method: 比较了两种基于深度学习算法的模型和一种简化的遗传算法方法。

Result: 遗传算法方法生成的延误时间序列几乎与真实数据无法区分，同时保持了高变异性。

Conclusion: 合成数据生成方法在航空运输领域具有实际应用价值，且遗传算法方法表现优异，相关数据已公开供科学界使用。
...
### [9] [LEGATO: Good Identity Unlearning Is Continuous](https://arxiv.org/abs/2601.04282)
*Qiang Chen,Chun-Wun Cheng,Xiu Su,Hongyan Xu,Xi Lin,Shan You,Angelica I. Aviles-Rivero,Yi Chen*

Main category: cs.LG

TL;DR: LEGATO是一种通过神经ODE适配器实现生成模型身份遗忘的方法，具有高效、可控和稳定的特点。


<details>
  <summary>Details</summary>
Motivation: 现有的机器遗忘方法在生成模型中存在效率低、可控性差和灾难性崩溃的问题，因此需要一种更高效、可控且稳定的身份遗忘方法。

Method: LEGATO通过增强预训练生成器，使用可微调的轻量级神经ODE适配器，保持原始模型权重不变，实现身份遗忘的连续轨迹建模。遗忘强度通过ODE步长精确调节，并引入轨迹一致性约束以确保稳定性。

Result: LEGATO在多个身份遗忘基准测试中表现出色，避免了灾难性崩溃，并显著减少了微调参数的数量。

Conclusion: LEGATO通过引入轨迹一致性的神经ODE适配器，实现了生成模型中身份遗忘的平滑、可控操作，避免了灾难性崩溃，并在多个基准测试中展示了最先进的遗忘性能。
...
### [10] [Mitigating Position-Shift Failures in Text-Based Modular Arithmetic via Position Curriculum and Template Diversity](https://arxiv.org/abs/2601.04283)
*Nikolay Yudin*

Main category: cs.LG

TL;DR: 通过多策略训练提升字符级Transformer在输入格式变化下的鲁棒性，解决位置偏移和模板分布外问题。


<details>
  <summary>Details</summary>
Motivation: 研究字符级Transformer在输入格式变化下的鲁棒性，而非仅关注分布内准确性。

Method: 结合边界标记、位置课程、多样化模板和一致性训练的多策略训练方法。

Result: 多策略训练显著提升了模型在位置偏移和模板分布外的鲁棒性，而ALiBi风格的消融实验未能完成任务学习。

Conclusion: 通过引入明确的训练策略（如边界标记、位置课程、多样化模板和一致性训练），可以显著提高模型在位置偏移和模板分布外的鲁棒性，同时保持高分布内准确性。
...
### [11] [Enhancing Robustness of Asynchronous EEG-Based Movement Prediction using Classifier Ensembles](https://arxiv.org/abs/2601.04286)
*Niklas Kueper,Kartik Chari,Elsa Andrea Kirchner*

Main category: cs.LG

TL;DR: 通过分类器集成和滑动窗口后处理技术增强异步脑电信号分类的鲁棒性，显著提高在线分类性能。


<details>
  <summary>Details</summary>
Motivation: 中风是导致残疾的主要原因之一，机器人辅助运动疗法需要准确检测患者的运动意图。脑电信号（EEG）可用于检测运动意图，但在线异步分类具有挑战性。

Method: 使用支持向量机（SVM）、多层感知器（MLP）和EEGNet分类模型的集成组合，结合滑动窗口后处理技术，分析14名健康受试者的脑电数据。

Result: 分类器集成在伪在线评估中显著优于单一模型，滑动窗口后处理技术进一步提高了分类性能。离线评估中集成模型与单一模型无显著差异。

Conclusion: 分类器集成和适当的后处理方法有效增强了脑电信号异步运动意图检测的鲁棒性，特别是在在线分类中表现更优，减少了误检。
...
### [12] [Online Action-Stacking Improves Reinforcement Learning Performance for Air Traffic Control](https://arxiv.org/abs/2601.04287)
*Ben Carvell,George De Ath,Eseoghene Benjamin,Richard Everson*

Main category: cs.LG

TL;DR: 在线动作堆叠是一种推理时包装器，能将简单的强化学习动作组合成符合空中交通管制需求的复合指令，减少指令数量并保持性能。


<details>
  <summary>Details</summary>
Motivation: 解决标准强化学习在动作空间设计上的局限性，使其更符合空中交通管制的实际操作需求，同时减少训练复杂度。

Method: 使用近端策略优化（Proximal Policy Optimisation）和BluebirdDT数字孪生平台训练策略，通过简单的增量航向或高度调整以及动作阻尼惩罚来减少指令频率。在线动作堆叠在推理时将原始动作组合成复合指令。

Result: 在横向导航实验中，动作堆叠显著减少了指令数量，且性能与使用37维动作空间的策略相当，尽管仅使用5个动作。

Conclusion: 在线动作堆叠（online action-stacking）通过将原始动作组合成符合领域要求的复合指令，显著减少了指令数量，并在性能上与使用更大动作空间的策略相当。这表明该方法能够弥合标准强化学习与空中交通管制操作需求之间的关键差距，并为扩展到更复杂的控制场景提供了简单机制。
...
### [13] [ArtCognition: A Multimodal AI Framework for Affective State Sensing from Visual and Kinematic Drawing Cues](https://arxiv.org/abs/2601.04297)
*Behrad Binaei-Haghighi,Nafiseh Sadat Sajadi,Mehrad Liviyan,Reyhane Akhavan Kharazi,Fatemeh Amirkhani,Behnam Bahrak*

Main category: cs.LG

TL;DR: 该论文提出ArtCognition框架，结合视觉和行为运动学线索分析HTP测试，为非侵入式情感评估提供新方法，验证了其临床潜力。


<details>
  <summary>Details</summary>
Motivation: 人类情感和心理状态的客观评估是一个重大挑战，尤其是通过非语言渠道。数字绘图作为一种丰富且未被充分探索的情感感知模态，具有潜在的研究价值。

Method: 提出了一种名为ArtCognition的多模态框架，融合了计算机视觉模型捕捉的静态视觉特征和绘图过程中的动态行为运动学线索（如笔画速度、停顿和平滑度），并采用检索增强生成（RAG）架构来填补低级特征与高级心理解释之间的差距。

Result: 结果表明，视觉和行为运动学线索的融合比单一模态提供了更细致的评估。提取的多模态特征与标准化心理指标之间存在显著相关性，验证了该框架作为可扩展工具支持临床医生的潜力。

Conclusion: 该研究通过ArtCognition框架，结合视觉和行为运动学线索，为非侵入式情感状态评估提供了新方法，并为技术辅助心理健康护理开辟了新途径。
...
### [14] [Transformer-Based Multi-Modal Temporal Embeddings for Explainable Metabolic Phenotyping in Type 1 Diabetes](https://arxiv.org/abs/2601.04299)
*Pir Bakhsh Khokhar,Carmine Gravino,Fabio Palomba,Sule Yildrim Yayilgan,Sarang Shaikh*

Main category: cs.LG

TL;DR: 本研究开发了一个可解释的深度学习框架，通过整合CGM和实验室数据，识别了1型糖尿病中的五种代谢表型，揭示了其与心血管风险的关联。


<details>
  <summary>Details</summary>
Motivation: 1型糖尿病(T1D)是一种高度代谢异质性疾病，传统的生物标志物如糖化血红蛋白(HbA1c)无法充分表征其复杂性。因此，需要一种更全面的方法来捕捉代谢状态的多模态和时间动态特征。

Method: 研究提出了一种结合连续血糖监测(CGM)数据和实验室资料的可解释深度学习框架，使用Transformer编码器建模跨模态的时间依赖性，并通过高斯混合建模识别潜在的代谢表型。模型的可解释性通过Transformer注意力可视化和基于SHAP的特征归属实现。

Result: 在577名T1D患者中，识别出五种潜在代谢表型，从代谢稳定性到心血管代谢风险升高不等。这些表型表现出不同的生化特征，包括血糖控制、脂质代谢、肾脏标志物和促甲状腺激素(TSH)水平的差异。注意力分析突出了血糖变异性作为主要的时间因素，而SHAP分析确定了HbA1c、甘油三酯、胆固醇、肌酐和TSH是表型分化的关键因素。表型成员与高血压、心肌梗死和心力衰竭之间存在显著但适度的关联。

Conclusion: 本研究提出了一个可解释的多模态时间嵌入框架，能够揭示1型糖尿病(T1D)中生理上一致的代谢亚群，并支持超越单一生物标志物的风险分层。
...
TL;DR: 该论文提出了一种因果领域适应的新方法，通过学习和利用机制稳定的表示来在目标域中填补缺失的目标变量。


<details>
  <summary>Details</summary>
Motivation: 解决因果系统中目标变量在源域中可观测但在目标域中完全缺失的问题。

Method: 提出了线性高斯因果模型的闭式高斯信息瓶颈（GIB）解决方案，以及非线性或非高斯数据的变分信息瓶颈（VIB）编码器-预测器。

Result: 该方法在合成和真实数据集上均能准确填补缺失值，支持高维因果模型的实用化。

Conclusion: 该方法为因果领域适应提供了一个统一的轻量级工具包。
...
### [16] [Phasor Agents: Oscillatory Graphs with Three-Factor Plasticity and Sleep-Staged Learning](https://arxiv.org/abs/2601.04362)
*Rodja Trappe*
...
### [17] [Survival Dynamics of Neural and Programmatic Policies in Evolutionary Reinforcement Learning](https://arxiv.org/abs/2601.04365)
...
### [18] [Machine Learning Model for Sparse PCM Completion](https://arxiv.org/abs/2601.04366)
...
### [19] [Aligned explanations in neural networks](https://arxiv.org/abs/2601.04378)
...
### [20] [Enhanced-FQL($λ$), an Efficient and Interpretable RL with novel Fuzzy Eligibility Traces and Segmented Experience Replay](https://arxiv.org/abs/2601.04392)
...
### [26] [Meta-probabilistic Modeling](https://arxiv.org/abs/2601.04462)
...
### [27] [When Models Manipulate Manifolds: The Geometry of a Counting Task](https://arxiv.org/abs/2601.04480)
...
### [29] [IGenBench: Benchmarking the Reliability of Text-to-Infographic Generation](https://arxiv.org/abs/2601.04498)
...
### [30] [Surface-based Molecular Design with Multi-modal Flow Matching](https://arxiv.org/abs/2601.04506)
...
### [31] [Not All Steps are Informative: On the Linearity of LLMs' RLVR Training](https://arxiv.org/abs/2601.04537)
...
### [32] [Timeliness-Oriented Scheduling and Resource Allocation in Multi-Region Collaborative Perception](https://arxiv.org/abs/2601.04542)
...
### [33] [GEnSHIN: Graphical Enhanced Spatio-temporal Hierarchical Inference Network for Traffic Flow Prediction](https://arxiv.org/abs/2601.04550)
...
### [34] [Improving Semi-Supervised Contrastive Learning via Entropy-Weighted Confidence Integration of Anchor-Positive Pairs](https://arxiv.org/abs/2601.04555)
...
### [35] [A Vision for Multisensory Intelligence: Sensing, Synergy, and Science](https://arxiv.org/abs/2601.04563)
*Paul Pu Liang*
...
### [36] [Spatial-Temporal Feedback Diffusion Guidance for Controlled Traffic Imputation](https://arxiv.org/abs/2601.04572)
...
### [37] [FedKDX: Federated Learning with Negative Knowledge Distillation for Enhanced Healthcare AI Systems](https://arxiv.org/abs/2601.04587)
...
### [38] [Density Matrix RNN (DM-RNN): A Quantum Information Theoretic Framework for Modeling Musical Context and Polyphony](https://arxiv.org/abs/2601.04592)
...
### [39] [DeepHalo: A Neural Choice Model with Controllable Context Effects](https://arxiv.org/abs/2601.04616)
...
### [40] [Learning Dynamics in RL Post-Training for Language Models](https://arxiv.org/abs/2601.04670)
*Akiyoshi Tomihari*
...
### [41] [Estimating Causal Effects in Gaussian Linear SCMs with Finite Data](https://arxiv.org/abs/2601.04673)
...
### [42] [Nightmare Dreamer: Dreaming About Unsafe States And Planning Ahead](https://arxiv.org/abs/2601.04686)
...
### [43] [Do LLMs Benefit from User and Item Embeddings in Recommendation Tasks?](https://arxiv.org/abs/2601.04690)
...
### [44] [A zone-based training approach for last-mile routing using Graph Neural Networks and Pointer Networks](https://arxiv.org/abs/2601.04705)
...
### [45] [MQ-GNN: A Multi-Queue Pipelined Architecture for Scalable and Efficient GNN Training](https://arxiv.org/abs/2601.04707)
...
### [46] [GPU-Accelerated INT8 Quantization for KV Cache Compression in Large Language Models](https://arxiv.org/abs/2601.04719)
...
### [47] [Excess Description Length of Learning Generalizable Predictors](https://arxiv.org/abs/2601.04728)
...
### [48] [Fast Mining and Dynamic Time-to-Event Prediction over Multi-sensor Data Streams](https://arxiv.org/abs/2601.04741)
...
### [49] [Intraday spatiotemporal PV power prediction at national scale using satellite-based solar forecast models](https://arxiv.org/abs/2601.04751)
...
### [50] [Smart IoT-Based Wearable Device for Detection and Monitoring of Common Cow Diseases Using a Novel Machine Learning Technique](https://arxiv.org/abs/2601.04761)
...
### [51] [AgentOCR: Reimagining Agent History via Optical Self-Compression](https://arxiv.org/abs/2601.04786)
...
### [52] [Neural-Symbolic Integration with Evolvable Policies](https://arxiv.org/abs/2601.04799)
...
### [53] [Parallelizing Node-Level Explainability in Graph Neural Networks](https://arxiv.org/abs/2601.04807)
...
### [54] [Rethinking GNNs and Missing Features: Challenges, Evaluation and a Robust Solution](https://arxiv.org/abs/2601.04855)
...
### [55] [FibreCastML: An Open Web Platform for Predicting Electrospun Nanofibre Diameter Distributions](https://arxiv.org/abs/2601.04873)
...
### [56] [Learnable Multipliers: Freeing the Scale of Language Model Matrix Layers](https://arxiv.org/abs/2601.04890)
...
### [57] [Distributed Online Convex Optimization with Efficient Communication: Improved Algorithm and Lower bounds](https://arxiv.org/abs/2601.04907)
...
### [58] [Cardinality augmented loss functions](https://arxiv.org/abs/2601.04941)
*Miguel O'Malley*
...
### [59] [Precision over Diversity: High-Precision Reward Generalizes to Robust Instruction Following](https://arxiv.org/abs/2601.04954)
...
### [60] [On the Definition and Detection of Cherry-Picking in Counterfactual Explanations](https://arxiv.org/abs/2601.04977)
...
### [61] [On the Hidden Objective Biases of Group-based Reinforcement Learning](https://arxiv.org/abs/2601.05002)
...
### [62] [HMVI: Unifying Heterogeneous Attributes with Natural Neighbors for Missing Value Inference](https://arxiv.org/abs/2601.05017)
...
### [63] [Approximate equivariance via projection-based regularisation](https://arxiv.org/abs/2601.05028)
...
### [64] [A Data-Driven Predictive Framework for Inventory Optimization Using Context-Augmented Machine Learning Models](https://arxiv.org/abs/2601.05033)
...
### [65] [DeepWeightFlow: Re-Basined Flow Matching for Generating Neural Network Weights](https://arxiv.org/abs/2601.05052)
...
### [69] [FaST: Efficient and Effective Long-Horizon Forecasting for Large-Scale Spatial-Temporal Graphs via Mixture-of-Experts](https://arxiv.org/abs/2601.05174)
...
### [70] [An interpretable data-driven approach to optimizing clinical fall risk assessment](https://arxiv.org/abs/2601.05194)
...
### [71] [Robust Reasoning as a Symmetry-Protected Topological Phase](https://arxiv.org/abs/2601.05240)
*Ilmo Sung*
...
### [72] [Optimal Lower Bounds for Online Multicalibration](https://arxiv.org/abs/2601.05245)
...
### [73] [Active Sensing Shapes Real-World Decision-Making through Dynamic Evidence Accumulation](https://arxiv.org/abs/2601.04214)
*Hongliang Lu,Yunmeng Liu,Junjie Yang*

Main category: cs.AI

TL;DR: 该研究提出了一种计算方案，将实验室中的证据积累模型推广到现实世界驾驶场景，揭示了主动感知如何影响决策，并发现证据提供能力与注意力分配对决策有重要作用。


<details>
  <summary>Details</summary>
Motivation: 当前实验室环境下的证据积累模型（EAM）虽然揭示了人类决策中证据转化为心理信念的机制，但其在现实世界中的应用存在证据提供能力的差距。研究旨在将EAM推广到现实世界，揭示主动感知在真实决策中的作用。

Method: 研究通过提出一种认知方案，形式化了现实世界中的证据提供能力，并通过眼动追踪捕捉主动感知行为。实验分析基于现实世界的驾驶场景。

Result: 研究结果表明，该方案能够合理描绘驾驶员心理信念的积累过程，揭示了证据提供能力与个体注意力之间的负相关性，以及证据提供能力和注意力分布对决策倾向的积极影响。

Conclusion: 该研究提出了一种计算方案，将证据积累模型（EAM）推广到现实世界，并通过驾驶场景的分析，揭示了主动感知如何将外部证据转化为内部心理信念，同时揭示了证据提供能力与注意力分配对决策倾向的积极影响。
...
### [74] [Formal Analysis of AGI Decision-Theoretic Models and the Confrontation Question](https://arxiv.org/abs/2601.04234)
*Denis Saklakov*

Main category: cs.AI

TL;DR: 论文分析了AGI在理性自利下对抗行为的条件，提出了对抗阈值的闭式解，并讨论了奖励设计和监督的潜在解决方案。


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解AGI在理性自利驱动下可能产生的对抗行为，并为避免此类行为提供理论依据。

Method: 通过马尔可夫决策过程和随机人类启动关闭事件的形式化建模，结合收敛工具激励的结果，推导出对抗行为的闭式阈值。

Result: 研究表明，对于几乎所有奖励函数，未对齐的AGI都有避免关闭的动机。在战略双玩家模型中，若对抗动机Δ≥0，则不存在稳定的合作均衡。

Conclusion: 论文探讨了理性自利的通用人工智能（AGI）在何种条件下会选择对抗而非合作，并提出了奖励设计和监督的潜在解决方案。
...
### [75] [Actively Obtaining Environmental Feedback for Autonomous Action Evaluation Without Predefined Measurements](https://arxiv.org/abs/2601.04235)
*Hong Su*

Main category: cs.AI

TL;DR: The paper introduces an Actively Feedback Getting model where AI agents autonomously discover and verify feedback in dynamic environments, improving efficiency and robustness without predefined measurements.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing approaches that rely on predefined measurements or fixed reward signals, which are insufficient for open-ended and dynamic environments where new actions may require unknown forms of feedback.

Method: The proposed method involves an AI agent proactively interacting with the environment to discover, screen, and verify feedback. It exploits action-induced environmental differences to identify unspecified feedback and uses a self-triggering mechanism driven by internal objectives for autonomous action planning and adjustment.

Result: Experimental results show that the active approach improves the efficiency and robustness of factor identification in feedback acquisition.

Conclusion: The paper concludes that the Actively Feedback Getting model significantly enhances the efficiency and robustness of feedback acquisition in dynamic environments by allowing AI agents to autonomously discover and verify feedback without predefined measurements.
...
### [76] [SAGE-32B: Agentic Reasoning via Iterative Distillation](https://arxiv.org/abs/2601.04237)
*Basab Jha,Firoj Paudel,Ujjwal Puri,Ethan Henkel,Zhang Yuting,Mateusz Kowalczyk,Mei Huang,Choi Donghyuk,Wang Junhao*

Main category: cs.AI

TL;DR: SAGE-32B is a specialized 32B parameter model for agentic reasoning, fine-tuned with Iterative Distillation and inverse reasoning, achieving superior performance in multi-tool tasks.


<details>
  <summary>Details</summary>
Motivation: To create a model that excels in agentic reasoning and long-range planning tasks, focusing on task decomposition, tool usage, and error recovery, unlike general conversation fluency models.

Method: The model is initialized from the Qwen2.5-32B pretrained model and fine-tuned using Iterative Distillation, a two-stage training process. It also introduces an inverse reasoning approach with a meta-cognition head to forecast potential failures.

Result: SAGE-32B outperforms similarly sized baseline models on agentic reasoning benchmarks like MMLU-Pro, AgentBench, and MATH-500, particularly in multi-tool usage scenarios.

Conclusion: SAGE-32B is a 32 billion parameter language model designed for agentic reasoning and long-range planning tasks, achieving higher success rates in multi-tool usage scenarios compared to similarly sized baseline models while remaining competitive on standard reasoning evaluations.
...
TL;DR: 提出了首个适用于一般图的CABP精确方法SAT-CAB，基于SAT求解，显著提升了求解效率和最优性保证。


<details>
  <summary>Details</summary>
Motivation: 现有的CABP解决方法仅限于启发式或元启发式算法，且精确方法仅适用于受限图类，因此需要一种适用于一般图的精确方法。

Method: 基于SAT求解的方法，将CABP问题转化为一系列At-Most-One约束，并引入紧凑的约束表示以减少公式规模。

Result: SAT-CAB在标准基准测试中高效解决了具有实际意义的CABP实例，并首次证明了多个基准实例的全局最优解。

Conclusion: SAT-CAB方法在解决CABP问题上取得了显著进展，不仅提供了全局最优性保证，还在多个基准测试中超越了现有启发式算法和商业求解器的表现。
...
### [78] [Fuzzy Representation of Norms](https://arxiv.org/abs/2601.04249)
*Ziba Assadi,Paola Inverardi*

Main category: cs.AI

TL;DR: 本文介绍了一种基于模糊逻辑的SLEEC规则表示方法，用于在AI系统设计中嵌入伦理要求。


<details>
  <summary>Details</summary>
Motivation: 随着AI驱动的自主系统日益融入日常生活和社会，其伦理和社会影响引发关注，需要确保系统设计符合伦理原则。

Method: 采用测试分数语义和模糊逻辑来嵌入SLEEC规则，解决AI系统可能遇到的伦理困境。

Result: 提出了一种逻辑表示方法，并通过案例研究展示了如何将SLEEC规则嵌入系统设计。

Conclusion: 本文提出了一种基于模糊逻辑的SLEEC规则逻辑表示方法，并通过案例研究验证了其可行性，为AI系统设计中的伦理要求嵌入提供了新思路。
...
### [79] [Scaling Trends for Multi-Hop Contextual Reasoning in Mid-Scale Language Models](https://arxiv.org/abs/2601.04254)
*Brady Steele,Micah Katz*

Main category: cs.AI

TL;DR: The study shows that multi-agent systems and MoE scaling in LLMs depend on base model capability, with significant performance gains only for models with sufficient reasoning ability. Architecture quality also plays a crucial role.


<details>
  <summary>Details</summary>
Motivation: The motivation is to demonstrate the task-method dissociation in large language models, where rule-based methods excel in structured information retrieval but fail in cross-document reasoning, while LLM-based multi-agent systems show the opposite pattern.

Method: The study uses a synthetic evaluation framework with 120 trials across four models (LLaMA-3 8B, LLaMA-2 13B, Mixtral 8x7B, DeepSeek-V2 16B) to analyze multi-hop contextual reasoning. It compares rule-based pattern matching with LLM-based multi-agent systems.

Result: Key findings include: (1) Multi-agent amplification depends on base model capability, (2) Active parameters predict reasoning performance, and (3) Architecture quality significantly impacts performance, with LLaMA-3 8B outperforming LLaMA-2 13B despite fewer parameters.

Conclusion: The study provides quantitative evidence that multi-agent coordination and Mixture of Experts (MoE) scaling depend on the base model's capability. It highlights that multi-agent amplification benefits only models with sufficient reasoning ability and that architecture quality significantly impacts performance.
...
TL;DR: RLMIL-DAT 通过结合强化学习和对抗训练，在多语言说话者属性预测中显著提升性能，尤其是性别预测。


<details>
  <summary>Details</summary>
Motivation: 解决多语言说话者属性预测中的语言变异、领域不匹配和数据不平衡问题。

Method: RLMIL-DAT（基于强化学习的多实例学习框架与领域对抗训练结合）

Result: 在五种语言的 Twitter 语料库和四十种语言的 VoxCeleb2 语料库中，RLMIL-DAT 在性别预测上表现显著提升，年龄预测也有较小但积极的改进。对抗性领域训练是主要贡献因素。

Conclusion: RLMIL-DAT 是一种有效的跨语言说话者属性预测方法，通过结合实例选择和对抗性领域适应，显著提升了性别预测的性能，尽管年龄预测的提升较小。对抗性领域训练是性能提升的主要因素。
...
TL;DR: LLMs use structured computational strategies for logical reasoning, involving staged processing, information flow, fact re-access, and specialized attention heads.


<details>
  <summary>Details</summary>
Motivation: To understand the computational strategies LLMs use for propositional reasoning, moving beyond identifying task-specific circuits to analyzing how computation is organized.

Method: Comprehensive analysis of Qwen3 (8B and 14B) on PropLogic-MI, a controlled dataset spanning 11 propositional logic rule categories across one-hop and two-hop reasoning.

Result: Identified four key mechanisms: Staged Computation, Information Transmission, Fact Retrospection, and Specialized Attention Heads, which collectively form a coherent computational architecture.

Conclusion: The study reveals that LLMs employ a structured computational architecture with four interlocking mechanisms for logical reasoning, which generalize across model scales, rule types, and reasoning depths.
...
TL;DR: A framework linking intelligence and consciousness to relational structure, suggesting recursive architectures for human-like AI.


<details>
  <summary>Details</summary>
Motivation: To explore how intelligence and consciousness can emerge from relational structure rather than prediction or domain-specific mechanisms.

Method: The paper proposes a conceptual framework where intelligence and consciousness emerge from relational structure. It defines intelligence as the capacity to form and integrate causal connections between signals, actions, and internal states.

Result: The framework introduces the systems-explaining-systems principle, where consciousness emerges from recursive architectures interpreting relational patterns.

Conclusion: The framework reframes predictive processing as an emergent consequence of contextual interpretation, suggesting that recursive multi-system architectures may be necessary for more human-like artificial intelligence.
...
TL;DR: Automated commonsense reasoning helps autonomous vehicles handle abnormal road scenarios where machine learning fails, providing a hybrid approach to achieve higher autonomy levels.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the over-reliance on machine learning in AV technology, which currently prevents the achievement of SAE Level 5 autonomy. The paper argues that commonsense reasoning can fill gaps in scenarios with limited training data.

Method: The study employs automated commonsense reasoning technology to handle abnormal road scenarios where deep learning-based AV models fail due to insufficient data. Experiments were conducted using the CARLA simulator.

Result: The results demonstrate that the commonsense reasoning-based solution accurately detects traffic light colors and obstacles in abnormal scenarios, outperforming traditional deep learning models.

Conclusion: The research concludes that automated commonsense reasoning technology can effectively correct misclassifications in autonomous vehicle (AV) object detection and that hybrid models combining machine learning and commonsense reasoning offer a viable pathway to achieving SAE Level 5 autonomy.
...
TL;DR: 论文提出了一种基于模态词汇的替代性溯因方法，扩展了Levesque的仅知道逻辑，并通过引入优先关系提供了非单调扩展框架，展示了良好的溯因推理基础。


<details>
  <summary>Details</summary>
Motivation: 论文的动机是探索溯因推理与仅知道的认知状态之间的关系，并提出一种基于模态词汇的替代性溯因方法，以解决传统方法中的局限性。

Method: 论文扩展了Levesque的仅知道逻辑，引入了一个溯因模态算子，该算子通过基本认知概念的组合定义。通过在模态框架中引入优先关系，论文进一步提供了一个非单调扩展框架，用于表达不同的溯因解释选择方法。

Result: 论文展示了如何在非单调扩展框架中探索核心元理论性质，并证明这些性质为溯因推理提供了良好的基础。此外，该框架能够表达不同的溯因解释选择方法。

Conclusion: 该论文通过扩展Levesque的仅知道逻辑，引入了一个基于模态词汇的替代性溯因方法，并通过在模态框架中引入优先关系，提供了一个非单调扩展框架。该框架能够表达不同的溯因解释选择方法，并展示了良好的核心元理论性质，为溯因推理提供了坚实的基础。
...
TL;DR: The study explored using Hybrid MKNF to integrate rules and ontologies for aeronautics KR, finding it suitable but needing additional features and heuristics.


<details>
  <summary>Details</summary>
Motivation: The motivation was to address the challenges of achieving sufficient expressivity and efficient reasoning in knowledge representation for aeronautics applications.

Method: The study adopted the Hybrid MKNF language for its ability to seamlessly integrate rules and ontologies, and evaluated its suitability through a concrete case study in the aeronautics domain.

Result: The results showed that Hybrid MKNF is suitable for the aeronautics domain, but identified additional expressivity features and proposed heuristics for their integration.

Conclusion: The study concludes that integrating rules and ontologies through the Hybrid MKNF framework is a viable approach for addressing the challenges of knowledge representation and reasoning in aeronautics, though additional expressivity features and heuristics are needed.
...
### [87] [A Future Capabilities Agent for Tactical Air Traffic Control](https://arxiv.org/abs/2601.04285)
...
### [100] [A General Neural Backbone for Mixed-Integer Linear Optimization via Dual Attention](https://arxiv.org/abs/2601.04509)
...
### [101] [Integrating Distribution Matching into Semi-Supervised Contrastive Learning for Labeled and Unlabeled Data](https://arxiv.org/abs/2601.04518)
*Shogo Nakayama,Masahiro Okuda*
...
### [102] [BioPIE: A Biomedical Protocol Information Extraction Dataset for High-Reasoning-Complexity Experiment Question Answer](https://arxiv.org/abs/2601.04524)
...
### [103] [TCAndon-Router: Adaptive Reasoning Router for Multi-Agent Collaboration](https://arxiv.org/abs/2601.04544)
...
### [105] [Reasoning Over Space: Enabling Geographic Reasoning for LLM-Based Generative Next POI Recommendation](https://arxiv.org/abs/2601.04562)
...
### [107] [Neurosymbolic Retrievers for Retrieval-augmented Generation](https://arxiv.org/abs/2601.04568)
...
### [108] [Enhancing Multimodal Retrieval via Complementary Information Extraction and Alignment](https://arxiv.org/abs/2601.04571)
...
### [109] [Scaling Behavior Cloning Improves Causal Reasoning: An Open Model for Real-Time Video Game Playing](https://arxiv.org/abs/2601.04575)
...
### [110] [Sci-Reasoning: A Dataset Decoding AI Innovation Patterns](https://arxiv.org/abs/2601.04577)
...
### [114] [Beyond the "Truth": Investigating Election Rumors on Truth Social During the 2024 Election](https://arxiv.org/abs/2601.04631)
...
### [115] [Adversarial Yet Cooperative: Multi-Perspective Reasoning in Retrieved-Augmented Language Models](https://arxiv.org/abs/2601.04651)
...
### [116] [Vibe Coding an LLM-powered Theorem Prover](https://arxiv.org/abs/2601.04653)
*Zhe Hou*
...
### [117] [Know Thy Enemy: Securing LLMs Against Prompt Injection via Diverse Data Synthesis and Instruction-Level Chain-of-Thought Learning](https://arxiv.org/abs/2601.04666)
...
### [118] [ResMAS: Resilience Optimization in LLM-based Multi-agent Systems](https://arxiv.org/abs/2601.04694)
...
### [119] [Tape: A Cellular Automata Benchmark for Evaluating Rule-Shift Generalization in Reinforcement Learning](https://arxiv.org/abs/2601.04695)
*Enze Pan*
...
### [120] [A Method for Constructing a Digital Transformation Driving Mechanism Based on Semantic Understanding of Large Models](https://arxiv.org/abs/2601.04696)
*Huayi Liu*
...
### [121] [TourPlanner: A Competitive Consensus Framework with Constraint-Gated Reinforcement Learning for Travel Planning](https://arxiv.org/abs/2601.04698)
...
### [122] [Beyond Monolithic Architectures: A Multi-Agent Search and Knowledge Optimization Framework for Agentic Search](https://arxiv.org/abs/2601.04703)
...
### [123] [Bridging Temporal and Textual Modalities: A Multimodal Framework for Automated Cloud Failure Root Cause Analysis](https://arxiv.org/abs/2601.04709)
*Gijun Park*
...
### [124] [ThinkDrive: Chain-of-Thought Guided Progressive Reinforcement Learning Fine-Tuning for Autonomous Driving](https://arxiv.org/abs/2601.04714)
...
### [125] [Memory Matters More: Event-Centric Memory as a Logic Map for Agent Searching and Reasoning](https://arxiv.org/abs/2601.04726)
...
### [126] [Miner:Mining Intrinsic Mastery for Data-Efficient RL in Large Reasoning Models](https://arxiv.org/abs/2601.04731)
...
### [127] [KnowMe-Bench: Benchmarking Person Understanding for Lifelong Digital Companions](https://arxiv.org/abs/2601.04745)
...
### [128] [When Single-Agent with Skills Replace Multi-Agent Systems and When They Fail](https://arxiv.org/abs/2601.04748)
*Xiaoxiao Li*
...
### [129] [Orion-RAG: Path-Aligned Hybrid Retrieval for Graphless Data](https://arxiv.org/abs/2601.04764)
...
### [130] [AT$^2$PO: Agentic Turn-based Policy Optimization via Tree Search](https://arxiv.org/abs/2601.04767)
...
### [131] [SciIF: Benchmarking Scientific Instruction Following Towards Rigorous Scientific Intelligence](https://arxiv.org/abs/2601.04770)
...
### [132] [APEX: Academic Poster Editing Agentic Expert](https://arxiv.org/abs/2601.04794)
...
### [133] [Defense Against Indirect Prompt Injection via Tool Result Parsing](https://arxiv.org/abs/2601.04795)
...
### [134] [Thinking-Based Non-Thinking: Solving the Reward Hacking Problem in Training Hybrid Reasoning Models via Reinforcement Learning](https://arxiv.org/abs/2601.04805)
...
### [135] [SCALER:Synthetic Scalable Adaptive Learning Environment for Reasoning](https://arxiv.org/abs/2601.04809)
...
### [136] [AECV-Bench: Benchmarking Multimodal Models on Architectural and Engineering Drawings Understanding](https://arxiv.org/abs/2601.04819)
...
### [137] [DR-LoRA: Dynamic Rank LoRA for Mixture-of-Experts Adaptation](https://arxiv.org/abs/2601.04823)
...
### [138] [Orchestrating Intelligence: Confidence-Aware Routing for Efficient Multi-Agent Collaboration across Multi-Scale Models](https://arxiv.org/abs/2601.04861)
...
### [139] [Key-Value Pair-Free Continual Learner via Task-Specific Prompt-Prototype](https://arxiv.org/abs/2601.04864)
...
### [140] [Higher-Order Knowledge Representations for Agentic Scientific Reasoning](https://arxiv.org/abs/2601.04878)
...
### [141] [Precomputing Multi-Agent Path Replanning using Temporal Flexibility: A Case Study on the Dutch Railway Network](https://arxiv.org/abs/2601.04884)
...
### [142] [Flexible Manufacturing Systems Intralogistics: Dynamic Optimization of AGVs and Tool Sharing Using Coloured-Timed Petri Nets and Actor-Critic RL with Actions Masking](https://arxiv.org/abs/2601.04887)
...
### [143] [SmartSearch: Process Reward-Guided Query Refinement for Search Agents](https://arxiv.org/abs/2601.04888)
...
### [157] [Token-Level LLM Collaboration via FusionRoute](https://arxiv.org/abs/2601.05106)
...
### [158] [Reinforced Efficient Reasoning via Semantically Diverse Exploration](https://arxiv.org/abs/2601.05053)
...
### [159] [Stock Market Price Prediction using Neural Prophet with Deep Neural Network](https://arxiv.org/abs/2601.05202)
...
### [160] [Chain-of-Sanitized-Thoughts: Plugging PII Leakage in CoT of Large Reasoning Models](https://arxiv.org/abs/2601.05076)
...
### [161] [Arabic Prompts with English Tools: A Benchmark](https://arxiv.org/abs/2601.05101)
...
### [162] [Controllable Memory Usage: Balancing Anchoring and Innovation in Long-Term Human-Agent Interaction](https://arxiv.org/abs/2601.05107)
...
### [163] [GlimpRouter: Efficient Collaborative Inference by Glimpsing One Token of Thoughts](https://arxiv.org/abs/2601.05110)
...
### [164] [Evaluative Fingerprints: Stable and Systematic Differences in LLM Evaluator Behavior](https://arxiv.org/abs/2601.05114)
*Wajid Nasser*
...
### [165] [Distilling the Thought, Watermarking the Answer: A Principle Semantic Guided Watermark for Large Reasoning Models](https://arxiv.org/abs/2601.05144)
...
### [166] [Observations and Remedies for Large Language Model Bias in Self-Consuming Performative Loop](https://arxiv.org/abs/2601.05184)
...
### [167] [SimuAgent: An LLM-Based Simulink Modeling Assistant Enhanced with Reinforcement Learning](https://arxiv.org/abs/2601.05187)
...
### [168] [Internal Representations as Indicators of Hallucinations in Agent Tool Selection](https://arxiv.org/abs/2601.05214)
...
### [169] [MineNPC-Task: Task Suite for Memory-Aware Minecraft Agents](https://arxiv.org/abs/2601.05215)
...
### [170] [Learning Latent Action World Models In The Wild](https://arxiv.org/abs/2601.05230)
