...
TL;DR: NAVI-Orbital 是首个在轨部署的视觉-语言模型系统，于2026年4月16日成功在低地球轨道卫星上实现自主多模态推理，通过语义压缩将地球观测数据转化为文本描述，颠覆了传统"先采集再下传"的带宽模式。


<details>
  <summary>Details</summary>
Motivation: 地球观测数据生成速度远超下行带宽和人工处理能力，导致星上采集与地面情报之间出现日益扩大的差距，亟需在轨智能处理方案。

Method: 在LEO卫星上部署NAVI-Orbital软件系统，使用Gemma 3视觉-语言模型进行本地多模态推理，采用LangGraph图状态机协调检测和对话专用智能体，通过自然语言指令代替传统命令序列进行任务重定向。

Result: 地面基准测试在7,960张AID数据集上达到88.16%准确率；经过Flatsat验证和真实在轨捕获（包括未校正的YAM-9图像，无需微调，使用硬件加速GPU推理）证明其可行性。

Conclusion: 在卫星级边缘计算机上运行基础模型是可行的，通过轨道上的语义压缩能够逆转传统"先采集再下传所有数据"的带宽瓶颈。
...
TL;DR: 提出了CaVe-VLM-CoT，一种基于反射的代理RAG框架，通过五阶段闭环流程（提取器、检索器、求解器、引用注入器、验证器）强制证据基础推理，并引入CaVeScore综合指标评估检索质量、步骤级引用忠实性和跨模态基础，在ScienceQA和MMMU上取得改进。


<details>
  <summary>Details</summary>
Motivation: 现有链式思维和检索增强方法无法强制执行步骤级引用基础，也无法将验证失败回传至检索进行修正，导致VLM仍会产生流畅但视觉上不忠实的幻觉输出。

Method: 提出CaVe-VLM-CoT，一个五阶段闭环管道：Extractor提取查询，Retriever检索证据，Solver推理求解，Citation Injector注入引用，Verifier验证并触发未基础声明的结构化反馈至Extractor进行针对性重检索。同时提出包含23个组件级指标和CaVeScore综合指标的评估套件。

Result: 在ScienceQA上达到87.1%准确率和56.6% CaVeScore，在MMMU（30个科目）上达到55.2%准确率和35.7% CaVeScore，无需任何架构或提示修改。

Conclusion: 通过闭环反馈机制实现证据基础推理，有效减少VLM幻觉，并建立了全面的多维度评估体系。
...
TL;DR: 本文研究了共享工作空间中人类-AI团队协作的协调问题，发现缺乏结构协调时增加协作者会降低性能，而结合共享群组记忆和模拟人在回路门控的支架结构能显著提升团队表现。


<details>
  <summary>Details</summary>
Motivation: 尽管AI智能体能力日益增强，但许多科学和专业任务仍需要人类的判断和上下文专业知识。如何有效协调人类与AI智能体之间的责任分工是一个关键问题。

Method: 利用Collaborative Gym环境和DiscoveryBench任务，通过1,482个会话实验，对比了有无结构协调的团队表现，并评估了结合共享群组记忆与模拟人在回路（HITL）门控的支架结构。

Result: 增加相关协作者在缺乏协调结构时会降低性能；所提出的支架结构在三人团队中表现最佳，提供了更清晰的责任信号和更强的专业知识路由。

Conclusion: 人类-AI团队如何协调和整合专业知识与其所拥有的能力同样重要。
...
TL;DR: DeFAb基准测试：将数十年的公开知识库转化为可废止溯因推理的正式实例，用于评估模型的逻辑严谨性和理论推理能力，结果显示前沿模型在这类任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型在逻辑推理和理论构建方面存在不足，尤其是在处理可废止推理（defeasible reasoning）时，模型往往生成流利但理论破坏性的文本，缺乏逻辑严谨性。需要创建一个能够准确测量模型在理论修订和创造性推理方面能力的基准。

Method: 开发了DeFAb数据集和生成流水线，将OpenCyc、YAGO、Wikidata等分类层次与ConceptNet、UMLS等行为属性图相结合，生成了372,648+个实例，涵盖33.75M条具体化规则。每个假设必须通过多项式时间验证：有效推导、保守性和最小性。还创建了DeFAb-Hard和CONJURE变体用于更难的评估。

Result: 规则型逻辑求解器在基准测试中以100%准确率在50微秒内解决所有实例；而最佳前沿语言模型仅达到65%准确率，在渲染鲁棒性评估下（四种表面渲染的最坏情况）下降至23.5%。四个前沿模型在可废止推理方面表现不可靠：Level 2的渲染鲁棒准确率为7.8-23.5%；思维链变化（约36个百分点）超过任何模型间差距；匹配污染控制揭示了+19.4个百分点的Level 3差距。

Conclusion: DeFAb基准通过将逻辑严谨性作为衡量创造力和理论推理的工具，能够有效评估模型在理论修订方面的能力。当前前沿语言模型尚未可靠内化可废止推理能力，而基于规则的符号方法表现优异。该基准为偏好优化提供了精确的奖励信号，并有望推动语言模型在逻辑推理方面的发展。
...
TL;DR: 将锂生产决策问题建模为部分可观察马尔可夫决策过程，使用信念状态规划方法优化矿山开发时机、位置和生产技术选择，在不同价格模型下优于启发式策略


<details>
  <summary>Details</summary>
Motivation: 锂生产决策面临地质、价格不确定性以及提取技术选择的复杂性，现有模型未充分考虑价格波动、需求不确定性和不同提取技术，需要更稳健的决策框架

Method: 将问题建模为部分可观察马尔可夫决策过程(POMDP)，采用信念状态规划方法求解，结合不同价格模型（静态、线性、指数和随机）和提取技术不确定性

Result: POMDP求解器在不同锂价格制度下优于人类启发式方法，通过优化勘探、生产和技术选择的序列，实现更高需求满足度和更平衡的经济环境效益

Conclusion: 通过整合不同价格模型和提取技术，POMDP框架为锂矿山开发决策提供了更稳健的策略，能够动态适应价格变化并优化全生命周期效益
...
TL;DR: 本文提出ForecastBench-Sim，一个基于Freeciv策略游戏构建的模拟世界预测基准，用于评估AI系统的概率推理能力，通过游戏回滚生成可即时解析的连续、二值、条件及因果预测问题，弥补现实世界预测中结果滞后、尾事件稀疏和反事实难评分等缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有通用AI预测基准受限于现实世界约束：结果解析缓慢、尾事件罕见、反事实问题难以评分，需要一种可控、可即时解析的预测任务来研究动态世界状态下的概率推理。

Method: 基于Freeciv回合制策略游戏构建模拟世界，预测器接收固定世界报告（结构化游戏状态快照）并回答关于隐藏未来状态的问题，随后继续模拟并评分预测；利用模拟特性生成连续/二值预测问题、配对干预世界用于条件/因果问题，以及罕见或破坏性事件的已解析示例。

Result: 描述了基准流水线、问题家族、评分协议和发布产出，并报告了模型评估和匿名人类试点的验证切片结果。

Conclusion: ForecastBench-Sim旨在通过提供可控、即时解析的任务来补充现实世界预测基准，用于研究动态世界状态下的概率推理。
...
TL;DR: 本文形式化证明了通用型智能体要想在多个环境和目标中接近最优地行动，必须存储记忆信息。当两个领域存在观测瓶颈但需要不兼容的最优动作时，任何接近最优的策略都必须在瓶颈处产生不同的记忆分布，从而得出分离定理：足够成功的智能体不能仅依赖当前观测，还必须保留领域相关信息。此外，若记忆包含足够信息来估计相关目标的价值，则可近似重建智能体的局部转移动力学。这些结果将记忆刻画为支撑领域区分、转移模型重建和规划的基板。


<details>
  <summary>Details</summary>
Motivation: 形式化理解通用型智能体为何需要记忆，以及在何种条件下记忆是必要的。探索记忆在多个环境和目标下对近最优决策的支撑作用。

Method: 通过建立观测瓶颈与不兼容最优动作的理论框架，推导分离定理；进一步分析记忆信息与价值估计、转移动力学重建之间的关系。

Result: 得出分离定理：成功智能体必须保留领域相关信息；记忆可近似重建局部转移动力学。

Conclusion: 记忆是支撑通用型智能体进行领域区分、转移模型重建和规划的关键基板。
...
TL;DR: R2D-RL是一个将RoboCup 2D足球仿真平台与Python多智能体强化学习框架连接的中间件环境，支持全场和场景训练、混合动作空间、EPV奖励塑造和并行执行。


<details>
  <summary>Details</summary>
Motivation: RoboCup 2D足球仿真平台成熟的服务器-客户端架构难以直接与现代基于Python的多智能体强化学习工作流兼容，需要构建一个中间层来桥接两者。

Method: 通过共享内存通信和周期级同步，将RCSS2D平台和基于HELIOS的玩家客户端连接到Python MARL接口，支持离散和混合参数化动作空间、动作掩码、基于期望控球值(EPV)的奖励塑造以及并行执行。

Result: 提供了前场进球场景和11对11全场基准测试，并给出了基线结果。

Conclusion: R2D-RL有效解决了RCSS2D与Python MARL框架之间的兼容性问题，为多智能体强化学习研究提供了一个实用的机器人足球环境。
...
TL;DR: ProfiLLM是一个用于网约车调度的LLM数据流水线，通过工具增强的全局知识挖掘和效用对齐的画像探索，提升生产匹配系统的性能


<details>
  <summary>Details</summary>
Motivation: 工业网约车调度系统中使用结构化数值特征为主，但重要的行为信号（如司机区域偏好）是语境化的，自然适合用LLM生成的用户画像表达。然而将这种画像扩展到实时调度系统面临三大挑战：平台日志超出LLM上下文窗口、长尾用户缺乏足够交互、生成的画像未必能提升预测效用。

Method: ProfiLLM包含两个核心模块：1）工具增强的全局知识挖掘：配备27个分析工具的LLM代理挖掘平台级数据，生成可重用的全局知识、自适应用户聚类规则和区域级供需先验；2）效用对齐的画像探索：为每个聚类生成多个候选画像，通过轻量级下游效用代理评估，迭代优化最佳候选，构建偏好对用于DPO微调。

Result: 在滴滴生产调度器部署中，ProfiLLM在结果预测中实现相对AUC提升高达+6.14%，调度模拟中GMV增益高达+4.35%，在线A/B测试（14天）中GMV提升+0.47%、完成率提升+0.33%、接单前取消率降低-0.82%。

Conclusion: ProfiLLM成功将LLM作为语义特征提取器集成到工业网约车调度系统中，通过解决平台规模数据挖掘、长尾用户画像和效用对齐等挑战，显著提升了调度系统的预测准确性和商业指标。
...
TL;DR: 这篇论文提出了WorldLines基准测试和ObsMem记忆框架，用于评估和改善具身智能体在家庭环境中的长期记忆和任务规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有的长期记忆基准主要评估语言检索和问答，而具身基准则侧重于短期任务执行，缺乏在动态环境中测试长期记忆使用的能力。需要一个能够评估具身智能体在长期家庭协助中记忆和规划能力的基准。

Method: 提出了WorldLines基准测试，构建包含对话、动作、执行反馈、对象和设备状态变化的扩展时间家庭轨迹，并将其转换为用于记忆QA和具身任务规划的证据链接样本。同时提出了ObsMem框架，维护可见性感知记忆和动作原生的状态轨迹。

Result: 实验揭示了在部分可观察性、覆盖的世界状态以及将长期记忆转化为具身计划方面的持续挑战，而ObsMem为这种设置提供了更强的参考架构。

Conclusion: WorldLines基准测试和ObsMem框架为解决具身智能体在长期家庭协助中的记忆和规划问题提供了重要工具，揭示了当前挑战并为未来发展指明了方向。
...
TL;DR: Xcientist是一个将科学研究过程外部化为可审计、可追溯构件的研究框架，通过契约化流程防止"主张漂移"，确保AI科学家的合成与验证过程可归因、可检查、可问责。


<details>
  <summary>Details</summary>
Motivation: AI系统在自动化科学工作流时，连接先验证据、生成想法、实验和最终主张的推理过程通常是隐式的（隐含在模型推理中），导致科学过程的不可见和不可审计。

Method: 提出Xcientist研究框架，将文献证据、想法状态、实现计划、消融记录和修复轨迹外部化为持久化的研究构件，通过合约化流程组织整个研究过程，实现可检查的自动化科学研究。

Result: 在无训练记忆系统、图结构交通预测和多尺度物理信息神经网络三个领域的实验中，Xcientist能够保持从问题构建到机制设计、验证和有界修订的可追踪轨迹，有效防止了"主张漂移"这一自动化研究的失败模式。

Conclusion: AI科学家的评估不应仅基于最终产物，还应考虑其合成与验证过程是否保持可归因性、可检查性和科学可问责性。
...
TL;DR: BeliefDiffusion结合扩散模型和模型预测控制，通过显式建模多模态信念分布并在聚合配置上进行规划，有效提升了部分可观测环境下的导航性能。


<details>
  <summary>Details</summary>
Motivation: 部分可观测环境中的导航存在感知混淆和高维信念空间多模态难以捕捉的问题，现有基于信念的方法和生成模型在长期规划方面存在不足。

Method: 提出BeliefDiffusion框架，包含两个步骤：(1) 基于观测历史利用扩散模型想象可能的环境配置；(2) 在聚合配置上利用模型预测控制(MPC)进行高效导航策略规划。

Result: 在合成地图环境实验中，BeliefDiffusion在导航成功率和路径效率上显著优于无模型强化学习基线和其他生成方法。

Conclusion: 显式地将多模态信念表征纳入规划过程，能够在部分可观测环境下实现更鲁棒的导航。
...
TL;DR: PROPEL是一种解决任务生成瓶颈的框架，通过轻量级激活探针预测求解器通过率，避免昂贵的求解器内循环训练，使生成的任务更适合当前模型的学习能力边界。


<details>
  <summary>Details</summary>
Motivation: 强化学习训练智能体的主要限制是前沿任务供应不足：固定任务分布会饱和，而简单的合成生成方法产生的任务要么太简单、要么太困难、要么有问题。特别是在软件工程任务中，单个求解器运行可能需要几十分钟，使得求解器内循环的生成器训练不可行。

Method: PROPEL框架训练一个轻量级激活探针，利用一次性标注的生成任务和求解器结果数据集。探针从冻结的生成器参考模型中预测目标求解器的通过率，在生成器优化期间作为求解率的代理，将生成器评估减少到单次前向传播。

Result: 在数学、代码和软件工程任务的多个模型规模上，PROPEL成功将生成任务推向目标求解率：对于编码任务，生成的学习边界任务从10.1%增加到20.0%（Qwen2.5-3B-Instruct求解器），从5.3%增加到12.6%（Qwen2.5-7B-Instruct求解器）。对于软件工程任务，从未见过的仓库中，生成任务的目标求解率占比从9.8%增加到19.6%（Qwen3.5-27B求解器）。

Conclusion: PROPEL通过求解器摊销的方法有效解决了任务生成中的前沿任务供应瓶颈问题，显著提高了生成任务在当前模型学习能力边界上的比例，避免了昂贵的求解器内循环评估成本。
...
TL;DR: 提出CodeBlock框架，通过选择结构完整的代码单元而非孤立token，在仅监督1.9%的响应token下实现优于全token监督的性能


<details>
  <summary>Details</summary>
Motivation: 现有token级选择方法在自然语言SFT中只监督高价值token，但直接应用于代码时会破坏程序结构和语义完整性，因为代码依赖结构完整性和定义-使用关系

Method: CodeBlock框架：1)选择高质量指令-响应对；2)将代码响应划分为语法一致的代码项；3)通过聚合核心逻辑token的广义交叉熵估计其效用；4)使用数据流可达性和桥接信号重新排序；训练时仅对选中的代码项和信息丰富的自然语言token计算损失

Result: 在6个代码生成基准测试中，CodeBlock在仅使用1.9%的监督响应token的情况下，平均pass@1优于全token SFT和竞争性选择基线方法

Conclusion: 基于结构完整代码证据的选择比均匀token级监督更有效，CodeBlock通过结构感知的稀疏监督在保持上下文完整性的同时显著减少监督token数量，提高了代码LLM的微调效率
...
TL;DR: Artemis：一种面向脑区级别的因果框架，通过独立学习每个脑区的特定混杂因子表示来解决神经影像数据中人口统计学因素（年龄、性别）对脑连接性与临床结果关系的混淆问题


<details>
  <summary>Details</summary>
Motivation: 多模态神经影像（fMRI功能连接和DTI结构连接）结合图神经网络能够非侵入式分析脑网络，但年龄、性别等人口统计学因素会系统性地混淆脑连接与临床结果之间的关系，导致GNN学习虚假相关性而非因果不变表示。现有因果GNN方法虽在图建模层面引入因果性，但未考虑临床神经影像数据中真实存在的混杂因子，且脑网络基于图谱分割构建，各脑区对人口统计学因素敏感性不同，需要区域感知的调整

Method: 提出Artemis区域级因果框架：通过轻量级参数为每个脑区独立学习特定混杂因子表示，进行因果干预。该调整充分利用多模态功能和结构特征进行图推理，作为兼容任意GNN骨干网络的插件模块

Result: 在三个基准数据集上的实验：ADNI用于疾病诊断、OASIS用于痴呆分期、HCP用于性别分类，均显示相对于代表性GNN基线方法的持续改进。多个支持性实验进一步证明了统计显著性和神经科学可解释性

Conclusion: Artemis通过区域级因果干预有效解决了神经影像数据中人口统计学混杂因子问题，提升了GNN模型在脑网络分析中的性能和可解释性，为临床神经影像分析提供了可靠的因果表示学习方法
...
### [34] [A Link between Shock-wave Theory and Symmetry-reduced Stochastic Gradient Descent for Artificial Neural Networks](https://arxiv.org/abs/2606.18303)
*Taiki Miyagawa*

Main category: cs.LG

TL;DR: 本文建立了冲击波理论与随机梯度下降对称商化学习动力学之间的数学联系，通过微分几何、李群理论和流体力学工具，推导出商流形上的粘性Hamilton-Jacobi方程和Burgers型方程，并严格证明了冲击波的形成。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习理论难以解释训练过程中的相变现象，参数空间中的对称性冗余导致原始参数范数等观测指标具有误导性，缺乏一个统一的数学框架来理解和预测训练动力学中的突变行为。

Method: 利用微分几何和李群理论对参数对称性进行商化处理，结合局部熵粗粒化方法，在商流形上推导出粘性Hamilton-Jacobi方程；进一步假设参数动力学可由商空间上的梯度场概括，推导出粗粒化损失函数的梯度满足Burgers型方程，并严格证明冲击波的形成。

Result: 证明多层感知机、卷积神经网络、Transformer和平均场网络均服从上述Hamilton-Jacobi或Burgers型方程。对称校正的商观测指标相比原始参数范数能更可靠地监测、预测和控制训练阶段转变。

Conclusion: 该框架为深度学习训练动力学提供了严格的数学基础，对称商化观测指标可作为实用的诊断工具，用于监控和预测深度神经网络训练中的相变。
...
### [35] [Attribution-Guided and Coverage-Maximized Pruning for Structural MoE Compression](https://arxiv.org/abs/2606.18304)
*Yifu Ding,Jiacheng Wang,Ge Yang,Yongcheng Jing,Jinyang Guo,Xianglong Liu,Dacheng Tao*

Main category: cs.LG

TL;DR: 本文提出一种针对MoE模型的结构化剪枝框架，通过将剪枝比例分配建模为通道分数覆盖最大化问题并采用归因近似求解，在DeepSeek和Qwen模型上实现50%剪枝率下保持精度，Qwen3-30B-A3B内存减少5.27倍。


<details>
  <summary>Details</summary>
Motivation: 现有MoE压缩方法主要在专家层面进行粗粒度剪枝，无法捕捉细粒度冗余，导致剪枝预算分配不当且压缩效果有限。作者观察到MoE专家内部信息高度集中于少量通道，即便是重要专家也存在大量冗余。

Method: 提出了一种针对MoE模型的结构化剪枝框架，将剪枝比例分配形式化为通道分数覆盖最大化问题，并采用基于归因的近似方法高效求解。

Result: 在DeepSeek和Qwen MoE模型上，50%或25%结构化剪枝结合4-bit量化仍保持精度；在Qwen3-30B-A3B上内存减少5.27倍，全面优于现有基线。

Conclusion: 本文提出的结构化剪枝框架在DeepSeek和Qwen MoE模型上，结合4-bit量化后，在50%或25%的结构化剪枝率下仍能保持模型精度，并在Qwen3-30B-A3B上实现5.27倍的内存缩减，全面超越现有基线方法。
...
### [36] [Fisher Width: A Geometric Measure of Complexity on Statistical Manifolds](https://arxiv.org/abs/2606.18306)
*Vu Khac Ky*

Main category: cs.LG

TL;DR: 本文提出Fisher宽度（Fisher width），将高维概率与压缩感知中经典的Gaussian宽度概念推广到统计流形上，用Fisher信息度量替代欧氏度量，使得该复杂度度量对局部统计曲率敏感且具有重参数化不变性。论文建立了Fisher宽度的基本理论（包括浓度不等式、度量扰动稳定性和谱比较界），并证明了Fisher-Lipschitz假设类的泛化界，在MNIST上进行了实证评估。


<details>
  <summary>Details</summary>
Motivation: Gaussian宽度是欧氏空间中的重要几何复杂度度量，但统计模型具有由Fisher信息度量诱导的自然黎曼几何，其中方向根据统计可区分性而非欧氏长度进行缩放。现有Gaussian宽度本质上是欧氏的，无法捕捉统计流形的曲率信息，因此需要一种适配统计几何的宽度概念。

Method: 在参数点θ处，用局部度量张量G(θ)^{1/2}替代欧氏单位矩阵，测量Fisher重缩放后的集合的Gaussian宽度，使结果对局部统计曲率敏感且具有重参数化不变性。在此基础上建立理论性质，并推导Fisher-Lipschitz假设类的泛化界，提出可计算估计量。

Result: Fisher宽度保留了Gaussian宽度的关键结构特征（浓度、度量扰动稳定性、与欧氏基线的谱比较界），同时能够捕捉欧氏度量无法体现的各向异性几何效应。在MNIST上的实证评估验证了其有效性。

Conclusion: Fisher宽度之于统计流形，如同Gaussian宽度之于欧氏凸体。该工作为研究弯曲统计流形上的复杂性与学习问题奠定了基础。
...
### [37] [DRIFT: Refining Instruction Data via On-Policy Data Attribution](https://arxiv.org/abs/2606.18307)
*Zefan Wang,Lincheng Li,Tianyu Yu,Yuan Yao*

Main category: cs.LG

TL;DR: 提出DRIFT方法，利用同策略影响函数优化SFT训练数据分布，通过解决标准影响函数的近端差距和梯度范数偏差问题，提升LLM性能上限。


<details>
  <summary>Details</summary>
Motivation: 现有数据筛选方法在有限预算下能加速训练，但不擅长提升模型能力上限。优化SFT数据分布的关键不再是保留性能的小子集，而是精炼数据分布以包含最有助于提升最终模型的样本。

Method: 提出DRIFT方法：使用模型同策略rollout作为验证目标来解决近端差距问题，基于轨迹正确性进行符号加权，并针对梯度黑客问题对影响分数去偏，使少量验证查询可作为可靠锚点来归因整个数据集。

Result: 在7B参数指令和推理模型上，DRIFT consistently提升了性能上限，优于现有数据筛选基线。

Conclusion: DRIFT通过解决标准影响函数的两个结构性问题（近端差距和梯度范数偏差），有效提升了SFT数据分布优化效果，从而提升LLM能力上限。
...
### [38] [TRIDENT: Breaking the Hybrid-Safety-Physics Coupling for Provably Safe Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2606.18308)
*Zijie Meng,Ziwei Li,Yufei Liu,Zhiyu Li,Jiyuan Liu,Wenhua Nie,Bingcai Wei,Miao Zhang*

Main category: cs.LG

TL;DR: TRIDENT提出首个多智能体强化学习框架，通过Richardson-Romberg梯度校正、Lyapunov约束信任域更新和物理信息残差批评家三个组件协同设计，解决了混合离散-连续行动、训练时安全约束和物理驱动动力学之间的三元耦合偏差问题，在多个场景下将训练违规减少95.5%以上。


<details>
  <summary>Details</summary>
Motivation: 网络化信息物理系统中的安全协调要求学习算法同时处理混合离散-连续动作、严格的训练时安全约束和物理驱动的动力学，但这三个特征形成了有向偏差循环，任何现成模块的简单组合都无法有效解决。

Method: 提出TRIDENT框架，包含三个协同设计的组件：(1) Richardson-Romberg梯度校正，将Gumbel-Softmax偏差从O(τ)降低到O(τ²)；(2) Lyapunov约束的顺序信任域更新，保证每次迭代的可行性；(3) 物理信息残差批评家，分解值函数而非奖励。

Result: 在多个场景（多无人机移动边缘计算、自主交叉口管理、混合SMAC变体）中，TRIDENT比MADDPG减少95.5%的训练违规，比MACPO减少76.3%，同时比最强无约束基线提高13.5%的奖励。证明了Õ(1/√K)的收敛速率到约束纳什均衡和O(√K)的累积违规界。

Conclusion: TRIDENT通过三重组件协同设计有效解决了三元耦合偏差问题，实现了安全高效的网络化信息物理系统多智能体协调。
...
### [39] [SAGE: Retain-Aware Post-Hoc Sanitization of Final Unlearning Vector](https://arxiv.org/abs/2606.18309)
*Jingyuan Zhang,Yucheng Bai,Peixi Wen,Zhehao Huang,Zhengbao He,Hanling Tian,Xinwen Cheng,Haiyin Ran,Xiaolin Huang*

Main category: cs.LG

TL;DR: 提出一种后处理框架SAGE，通过修正最终更新向量来缓解大模型遗忘任务中保留能力与遗忘效果之间的权衡，无需重新运行原始遗忘流程。


<details>
  <summary>Details</summary>
Motivation: 现有的大模型遗忘方法都存在保留与遗忘之间的权衡，且无法在不重新运行遗忘流程的情况下修复对保留能力的损害。作者发现保留激活偏差可以量化遗忘方法对保留能力的损伤程度，这启发了设计一种后处理方案。

Method: 提出SAGE（Spectral Activation-GEometry Sanitization）方法，从小型保留代理数据中收集真实模块输入，提取其主导激活几何结构，通过闭式解优化目标抑制与高能保留方向对齐的更新分量，同时保留原始方法的遗忘载体。

Result: 在多种遗忘方法、模型规模和基准测试上，SAGE一致地缓解了保留-遗忘权衡，验证了最终向量后处理作为机器遗忘中一个实用且未被充分探索的维度。

Conclusion: 后处理修正最终更新向量是一种有效且通用的策略，可以显著提升遗忘方法的保留能力而不损害其遗忘效果，为机器遗忘提供了新的实用方向。
...
### [40] [Ghost Attractor Networks: Basin-Structured Dynamical Decoders for Closed-Loop Sequential Generation](https://arxiv.org/abs/2606.18315)
*Tianyu Wang,Ying Wang,Zhihao Liu,Xi Vincent Wang,Lihui Wang*

Main category: cs.LG

TL;DR: 本文提出Ghost Attractor Networks，一种理论上推导的动态解码器，通过构造具有盆地吸引子结构的潜在空间，解决了大规模Transformer和扩散解码器的内存效率问题，在机器人动作生成中实现高效控制。


<details>
  <summary>Details</summary>
Motivation: 传统的大规模Transformer和扩散解码器存在序列长度相关的内存成本和迭代计算问题。小规模前馈解码器虽然高效，但产生的非结构化潜在表示限制了闭环控制能力，特别是相位条件动作生成和跨步潜在传递需要具有稳定盆地的潜在几何结构。

Method: 提出Ghost Attractor Networks，这是一种理论推导的动态解码器，其潜在状态在学习的势能函数中演化并具有漂移项。该方法通过构造产生盆地-吸引子结构，采用势能-漂移形式满足多模态性、解码器级单次切换和恒定内存三个需求，模式转换通过鞍结分岔和幽灵吸引子逃逸实现。分层相空间分解将一阶盆地收敛与二阶本体感觉细化分离。

Result: 实验表明，Ghost在潜在空间中表现出预测的梯度流收缩，梯度范数在1430个保留样本上经过5个积分步骤衰减67%。在机器人动作解码任务中，230万参数的Ghost匹配了10.7亿参数Diffusion Transformer的离线精度，参数减少462倍，延迟降低32倍，并在离线MSE上超过其他5种200万参数解码器5.9%-29%。在LIBERO-10闭环基准测试中，基于Ghost盆地结构潜在空间的相位调节比前馈MLP基线提升13.5%成功率，持久潜在集成达到95.7%最终成功率。

Conclusion: Ghost Attractor Networks通过构造具有盆地吸引子结构的动态解码器，实现了高效且可控的动作生成，显著优于现有方法，为机器人控制中的序列生成问题提供了新颖有效的解决方案。
...
### [41] [A Survey on Data-Driven Models for Soil Moisture Regression and Classification](https://arxiv.org/abs/2606.18316)
*Ilektra Tsimpidi,George Georgoulas,Vidya Sumathy,George Nikolakopoulos*

Main category: cs.LG

TL;DR: 本文系统综述了基于人工智能的土壤湿度建模方法，将现有方法分为五类：统计时间序列模型、地统计方法、经典机器学习、深度学习和概率/贝叶斯方法，并分析了各类方法在土壤湿度估计与分类中的应用。


<details>
  <summary>Details</summary>
Motivation: 土壤湿度建模是一个复杂的时空学习问题，物理模型计算成本高且可扩展性受限，而数据驱动的AI方法能以更少的建模假设提取经验关系，因此有必要对现有AI方法进行系统梳理和分类。

Method: 采用结构化综述方法，将现有AI土壤湿度模型分为五类：(a) 统计时间序列模型、(b) 地统计方法、(c) 经典机器学习模型、(d) 深度学习模型、(e) 概率/贝叶斯方法，这些模型利用历史土壤湿度记录、气象变量、植被指数、地形、土壤特征和地理位置数据进行回归或分类任务。

Result: 系统梳理了五大类AI方法在土壤湿度估计与分类中的应用，总结了各类方法所利用的数据来源（历史土壤湿度、气象变量、植被指数、地形、土壤特征、地理位置数据）和任务类型（回归/分类）。

Conclusion: AI方法为土壤湿度建模提供了灵活有效的替代方案，覆盖面从传统统计方法到现代深度学习和贝叶斯方法，能够处理复杂的非线性环境交互和异构数据源，弥补物理模型在大规模部署上的不足。
...
### [42] [Enhanced Graph Neural Networks using K-Hop Gaussian Diffusion](https://arxiv.org/abs/2606.18317)
*Xuling Zhang,Peng Wang,Daiyan Li,Aoran Huang,Zeiwei Chen,Yongkui Yang*

Main category: cs.LG

TL;DR: 提出了一种K-Hop高斯扩散核（KHG）作为图神经网络的预处理模块，通过多跳高斯加权扩散平衡局部与全局信息传播，在噪声或结构复杂的图上优于传统消息传递GNN及现有扩散方法。


<details>
  <summary>Details</summary>
Motivation: 现有GNN依赖单跳邻居消息传递，在边噪声大或结构复杂的真实图中信息传播受限；虽然PPR和Heat Kernel等扩散核实现了全局传播，但仍难以处理复杂局部结构和远距离节点噪声。

Method: 提出K-Hop高斯（KHG）扩散核作为图数据的预处理模块，在应用标准GNN之前，通过多跳扩散结合高斯权重对远端节点进行加权，平衡局部和全局信息传播。

Result: 在多个基准数据集上，KHG显著优于传统消息传递GNN、PPR和Heat Kernel扩散方法，尤其在噪声或结构复杂的图中表现突出。

Conclusion: KHG扩散核作为一种有效的图预处理手段，能够更好地处理复杂图结构中的噪声和远距离依赖问题，为GNN性能提升提供了新思路。
...
### [43] [ASTRA: A Scalable Next-Generation ATCO Training Simulator with Autonomous Simpilots](https://arxiv.org/abs/2606.18319)
...
TL;DR: ASTRA是一个端到端训练模拟器，通过微调ASR将新加坡口音航空语音的词错误率从107.80%降至23.45%，并实现AI辅助的ATCO表现评估（准确性91.7%、简洁性88.2%、完整性86.9%），从而自动化模拟飞行员角色，减少对专业人类培训师的依赖。


<details>
  <summary>Details</summary>
Motivation: 空中交通管制操作员（ATCO）培训依赖专业人类培训师（simpilots），培训容量受限；现有自动化方案使用西方语音模型，在新加坡运营环境下表现极差（词错误率高达107.80%）。

Method: 构建ASTRA端到端训练模拟器，包含ASR转录、指令解析、语音响应生成管线，使用本地适配语音模型对新加坡口音航空语音进行微调；集成基于DSPy和Unsloth开源框架的AI辅助表现评估框架。

Result: 微调ASR管线将词错误率降至23.45%；AI评估框架在准确性、简洁性和完整性上分别达到91.7%、88.2%和86.9%。

Conclusion: ASTRA实现了可扩展、标准化的ATCO评估，同时减少教官工作负担，显著优于现有方案。
...
### [44] [SAE Interventions are Unreliable: Post-Intervention Recovery of Suppressed Behavior](https://arxiv.org/abs/2606.18322)
*Mingyue Cui,Linghui Shen,Xingyi Yang*

Main category: cs.LG

TL;DR: 稀疏自编码器（SAEs）在残差激活流中识别"不安全"特征用于模型干预，但研究发现这种特征层面的干预存在可恢复的失败模式：虽然阻止了某个具体路径，但模型行为本身并未被消除。


<details>
  <summary>Details</summary>
Motivation: 当前基于稀疏自编码器特征识别的防御方法假设识别出的"不安全"特征可以作为监控和干预的有效把手，通过钳制特定有害特征就能可靠防止模型不当行为。然而这种假设可能过于乐观，存在隐藏的脆弱性。

Method: 将这种脆弱性形式化为干预后恢复问题，作为约束残差空间优化问题。从干预后的残差状态出发，优化残差扰动以恢复干预前行为，同时保持目标SAE特征的干预后数值不变。使用编码器正交更新进行单层干预，在跨层设置中使用对应的特征映射雅可比矩阵。

Result: 即使在干预持续生效的强威胁模型下，恢复仍然可能。在TPP、unlearning、IOI和拒绝转向实验中，压力测试显示尽管特征层面干预成功，但行为仍可恢复。尤其在安全关键的拒绝转向场景中，实现了95.8%的恢复率，同时防御特征的相对漂移仅为0.131，远低于基于后缀的基线方法。

Conclusion: SAE特征可以支持因果干预，但控制这些特征并不能保证控制底层行为。研究结果揭示了特征层面控制与行为完整性之间的差距：特征级干预虽能阻断特定路径，但模型仍可通过其他路径实现相同行为。
...
### [45] [Why SWAVE May Not Be All You Need:A Concept-Evolution Retrospective on Complex-Valued Recurrent Language Models](https://arxiv.org/abs/2606.18324)
*Ramprasath Ganesaraja,Swathika N,Sahil Dilip Panse*

Main category: cs.LG

TL;DR: SWave是一种复数值循环语言模型，采用Cayley参数化酉变换和旋转隐藏状态来保持长程信号完整性。通过三个阶段演进，解决了虚部通道坍塌问题（cos-domination collapse），最终使用独立实虚部嵌入表的非绑定头结构，实现了稳定训练（最佳PPL 22.0），并总结了六条可迁移的复数值循环训练工程原则。


<details>
  <summary>Details</summary>
Motivation: 传统实数值语言模型在信息编码上存在局限性，复数值表示能提供更丰富的信息编码方式；标准RNN面临状态衰减或爆炸问题，而酉变换的数学保证可以解决长程依赖中的信号退化。

Method: 使用Cayley参数化的酉变换作为循环核心，确保隐藏状态旋转而非衰减；采用ComplexNorm和Wave Propagation Scan作为核心组件；通过非绑定头（独立实虚部嵌入表）替代共振头，解决cos-domination collapse问题；使用实数值squared-ReLU通道混合器替代ComplexGatedUnit；采用log-space反向传播的并行扫描以提升数值稳定性。

Result: 最佳步长PPL达到22.0（第89,861步）；实现了200,000步的稳定训练；形式化描述了cos-domination collapse失败模式；总结出六条可迁移工程原则；构建了从规划到代码的可追溯性方法。

Conclusion: SWave验证了复数值酉循环语言模型的可行性，解决了关键的结构性坍塌问题，并提炼出一套可迁移至其他复数值循环模型训练的工程原则和调试方法论。
...
### [46] [Neural Network Implementation of the Renormalization Group for Fault Diagnosis with Class Imbalance](https://arxiv.org/abs/2606.18326)
*Evgeny Nikulchev,Dmitry Ilin*

Main category: cs.LG

TL;DR: RGNet是一种基于重整化群概念的神经网络架构，通过对特征空间进行层次化粗粒化处理，生成可解释的低维表示（RG-flow），有效解决类别不平衡和多维噪声问题，在AI4I故障预测数据集上取得了竞争性结果。


<details>
  <summary>Details</summary>
Motivation: 实际任务中机器学习模型面临类别不平衡和多维噪声的挑战，需要一种能够同时捕捉局部细节和全局模式、且具有可解释性的通用解决方案。

Method: 提出RGNet架构，基于重整化群概念对特征空间进行层次化粗粒化，顺序压缩输入维度并在分类前拼接所有尺度；引入RG-flow概念作为可解释的低维表示。

Result: 在不平衡的AI4I数据集上的实验表明，RGNet是一种通用、可解释且有竞争力的解决方案；通过t-SNE可视化RG-flow发现离散曲线结构，证实了粗粒化的有效性。

Conclusion: RGNet为不平衡类别的故障预测应用提供了通用、可解释且有竞争力的解决方案。
...
### [47] [Self-CTRL: Self-Consistency Training with Reinforcement Learning](https://arxiv.org/abs/2606.18327)
...
### [48] [ThousandWorlds: A benchmark for climate emulation of potentially habitable exoplanets](https://arxiv.org/abs/2606.18338)
...
### [49] [Do Time Series Foundation Model Benchmarks Hide Regime-Dependent Failures? Evidence from Traffic Speed Forecasting](https://arxiv.org/abs/2606.18367)
...
### [50] [From Sparse Features to Trustworthy Proxies: Certifying SAE-Based Interpretability](https://arxiv.org/abs/2606.18383)
...
### [51] [SCOPE-FL: A Strategy-proof Chain-based Optimal pareto efficient Federated Learning System](https://arxiv.org/abs/2606.18384)
...
### [52] [LLMZero: Discovering Adaptive Training Strategies for RL Post-Training via LLM Agents](https://arxiv.org/abs/2606.18388)
...
### [79] [PACT: Preserving Anchored Cores in Task-vectors for Model Merging](https://arxiv.org/abs/2606.18627)
...
### [80] [MetaboNet-Bench: A Multi-modal Benchmark for Glucose Forecasting in Type 1 Diabetes](https://arxiv.org/abs/2606.18640)
...
### [81] [BLADE: Scalable Bi-level Adaptive Data Selection for LLM Training](https://arxiv.org/abs/2606.18650)
...
### [82] [scGTN: Deep Siamese Graph Transformer Network for Single-cell RNA Sequencing Clustering](https://arxiv.org/abs/2606.18672)
...
### [83] [InTrain: Intrinsic Trainability for Zero-Cost Neural Architecture Search](https://arxiv.org/abs/2606.18676)
...
### [84] [Bounded Context Management for Tabular Foundation Models on Stream Learning](https://arxiv.org/abs/2606.18677)
...
### [85] [Dual-Channel Grounded World Modeling (DCGWM): Structural Prevention of Objective Interference Collapse via Heterogeneous External Grounding with Inward-Only Gradient Flow](https://arxiv.org/abs/2606.18688)
*Akshay Hazare*
...
### [86] [Graph Grounded Cross Attention Transformer Neural Network for Structurally Constrained Full Event Sequence Generation in Predictive Process Monitoring](https://arxiv.org/abs/2606.18726)
...
### [87] [Robust and Interpretable Adaptation of Equivariant Materials Foundation Models via Sparsity-promoting Fine-tuning](https://arxiv.org/abs/2606.18691)
...
### [88] [Private Learning with Public Feature Conditioning](https://arxiv.org/abs/2606.18773)
...
### [90] [Bayesian Anytime Pareto Set Identification for Multi-Objective Multi-Armed Bandits](https://arxiv.org/abs/2606.18785)
...
### [91] [Stealthy World Model Manipulation via Data Poisoning](https://arxiv.org/abs/2606.18697)
...
### [92] [Learning from Own Solutions: Self-Conditioned Credit Assignment for Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2606.18810)
...
### [93] [Contextualizing Biological Language Models across Modalities via Logit-Space Contrastive Alignment](https://arxiv.org/abs/2606.18703)
...
### [94] [Reinforcement Learning Foundation Models Should Already Be A Thing](https://arxiv.org/abs/2606.18812)
...
### [95] [Trainable Photonic Measurement for Physics-Informed PDE Learning](https://arxiv.org/abs/2606.18713)
...
### [96] [Maturing Markov Decision Processes: Decision Making under Increasing Information and Shrinking Action Sets](https://arxiv.org/abs/2606.18820)
...
### [97] [Target-confidence Recourse Using tSeTlin machines: TRUST](https://arxiv.org/abs/2606.18832)
...
### [98] [Low-Cost Neuromorphic Fall Detection Using Synthetic Event Data and Hybrid SNNs](https://arxiv.org/abs/2606.18732)
...
### [99] [Scaling Learning-based AEB with Massive Unlabeled Data](https://arxiv.org/abs/2606.18864)
...
### [100] [Domain-Shift Aware Neural Networks for Unbalance Characterization in Rotating Systems](https://arxiv.org/abs/2606.18882)
...
### [101] [RouteJudge: An Open Platform for Reproducible and Preference-Aware LLM Routing](https://arxiv.org/abs/2606.18774)
...
### [102] [A Controlled Benchmark of Quantum-Latent GAN Augmentation for Brain MRI](https://arxiv.org/abs/2606.18970)
...
### [103] [Online Distributional Prediction via Latent Cluster Geometry Under Drift and Corruption](https://arxiv.org/abs/2606.18778)
...
### [104] [FoMoE: Breaking the Full-Replica Barrier with a Federation of MoEs](https://arxiv.org/abs/2606.19025)
...
### [105] [A Hybrid LSTM--Vision Transformer Architecture for Predicting HRRR Forecast Errors](https://arxiv.org/abs/2606.19026)
...
### [106] [Pareto Q-Learning with Reward Machines](https://arxiv.org/abs/2606.19134)
...
### [109] [GateMem: Benchmarking Memory Governance in Multi-Principal Shared-Memory Agents](https://arxiv.org/abs/2606.18829)
...
### [110] [Compute Efficiency and Serial Runtime Tradeoffs for Stochastic Momentum Methods](https://arxiv.org/abs/2606.19179)
...
### [111] [Forecasting what Matters: Decision-Focused RL for Controlled EV Charging with Unknown Departure Times](https://arxiv.org/abs/2606.19199)
...
### [112] [Seed-Guided Semi-Supervised Clustering by A-Contrario Anomaly Detection](https://arxiv.org/abs/2606.18833)
*Nassir Mohammad*
...
### [113] [Machine Unlearning for the XGBoost Model with Network Intrusion Datasets](https://arxiv.org/abs/2606.19220)
...
### [114] [Identifying Structural Biases from Causal Mechanism Shifts](https://arxiv.org/abs/2606.18834)
...
### [115] [Mechanism-Guided Selective Unlearning for RLVR-Induced Reasoning](https://arxiv.org/abs/2606.19222)
...
### [116] [Semantic Robustness Certification for Vision-Language Models](https://arxiv.org/abs/2606.18839)
...
### [117] [STARE: Surprisal-Guided Token-Level Advantage Reweighting for Policy Entropy Stability](https://arxiv.org/abs/2606.19236)
...
### [118] [Learning from Your Own Mistakes: Constructing Learnable Micro-Reflective Trajectories for Self-Distillation](https://arxiv.org/abs/2606.18844)
...
### [120] [Investigating Inductive Biases for Machine Learning Emulation of Sudden Stratospheric Warmings in Idealised Isca Simulations](https://arxiv.org/abs/2606.18857)
...
### [121] [UBP2: Uncertainty-Balanced Preference Planning for Efficient Preference-based Reinforcement Learning](https://arxiv.org/abs/2606.19328)
