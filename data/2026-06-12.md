...
TL;DR: 论文将幻觉检测建模为最快变化检测问题，发现传统分类评估方法掩盖了检测延迟结构，序列分析方法能更好衡量检测性能


<details>
  <summary>Details</summary>
Motivation: 现有的token级别幻觉检测器使用AUC作为分类器进行评估，而流式监控需要反应时间评估（幻觉开始到警报之间的token数），需要更合适的评估框架

Method: 将幻觉起始检测建模为最快变化检测问题，使用一阶马尔可夫模型描述潜在忠实/幻觉状态，应用经典变化点理论和Lord下界，使用因果循环标注器作为CUSUM算法

Result: 学习到的CUSUM算法在误报率0.01时11-13个token内检测到幻觉，优于线性基准的31个token，但仍有数量级差距；学习到的评分只实现了特征携带散度的1/4.5

Conclusion: 分类指标掩盖了检测延迟结构，序列分析方法能更好地衡量幻觉检测性能，当前方法仍有优化空间，特别是特征利用效率需要提升
...
TL;DR: 论文提出Boltzmann注意力机制，这是一种基于能量的注意力泛化形式，通过可学习的成对耦合参数来显式建模注意力决策之间的交互关系，在长序列任务中表现优于标准softmax注意力。


<details>
  <summary>Details</summary>
Motivation: 标准注意力机制主要通过单个查询-键相似度计算相关性，虽然softmax归一化引入了位置间的竞争，但标准注意力层没有显式参数化注意力决策之间的可学习交互。这限制了注意力机制直接建模协同或拮抗共注意力结构的能力。

Method: 提出Boltzmann注意力，这是一种基于能量的泛化方法，其中注意力模式由交互的Ising模型控制。该方法在通常的数据依赖局部场基础上增加了可学习的成对耦合参数，允许模型表示超出softmax或sigmoid注意力捕获的位置间相关性。

Result: 在字符级语言建模和合成括号匹配任务上的实验表明，Boltzmann注意力在标准Transformer架构中始终优于标准softmax注意力，随着序列长度增加优势更加明显。四组消融实验确认改进来自于可学习的成对耦合。

Conclusion: 显式的位置间交互为基于注意力的序列建模提供了原则性增强。Ising公式还为量子计算采样策略开辟了自然路径：研究表明绝热量子退火提供了实用的训练方法，同时保持与精确Boltzmann计算相当的性能。
...
TL;DR: ReCal框架通过奖励校准机制改进基于强化学习的LLM路由方法，解决异构任务中学习信号不明确和优化偏差问题


<details>
  <summary>Details</summary>
Motivation: 现有的基于强化学习的LLM路由方法在处理异构任务时面临挑战：多目标聚合为单一标量奖励导致信用分配不明确和优化信号冲突，同时奖励信号在不同实例间差异过大，导致优化偏向简单样本而非信息丰富的样本

Method: 提出ReCal奖励校准框架，包含分层奖励分解机制（基于组件优势估计）和分布感知优化策略（通过方差感知重加权和每个数据集的归一化来校准优化变异性）

Result: 在7个数据集上的实验表明，ReCal在路由性能和训练稳定性方面均持续优于基线方法

Conclusion: ReCal框架有效解决了RL-based LLM路由中的奖励信号问题，通过奖励校准提高了路由系统的整体性能
...
TL;DR: T2SP是一种无训练方法，将时间序列转换为结构化符号程序表示，使LLM能更好地理解时间序列数据


<details>
  <summary>Details</summary>
Motivation: LLM在时间序列分析中存在模态不匹配问题，现有的序列化或微调方法将时间结构提取负担放在LLM上，导致长序列性能下降和计算开销大

Method: 提出了T2SP方法，将时间序列分解为趋势、周期和显著事件，并以程序友好的结构化格式表示，与LLM训练的语言和代码模态对齐

Result: 在编辑、描述和问答三个推理任务中，T2SP相比原始字符串表示显著提升性能，减少推理时间，降低失败率

Conclusion: T2SP为时间序列和LLM之间提供了有效的接口，使LLM能够利用其现有推理能力进行时间序列理解
...
TL;DR: 本文提出了UCF（单变量Christoffel函数），解决了基于Christoffel函数的异常检测方法中的维度扩展性问题，在保持理论特性的同时显著提升了计算效率。


<details>
  <summary>Details</summary>
Motivation: 基于Christoffel函数的异常检测方法虽然具有坚实的数学基础和计算效率，但在实际应用中面临严重挑战：需要求逆一个随数据维度指数增长的矩阵，导致即使在中等维度的数据集上也变得不可行。

Method: 引入了UCF（单变量Christoffel函数），该方法基于查询点与支持点之间的平方距离构建，有效避免了原始方法中矩阵尺寸随维度指数增长的问题。

Result: 在ADBench基准测试中，UCF在平均精度指标上一致优于14种最先进的基线方法。解决了Christoffel函数的可扩展性瓶颈，同时保持了关键的on-off支持二分行为和准确的支持形状捕获能力。

Conclusion: 通过解决Christoffel函数的可扩展性瓶颈，本研究为异常检测方法工具箱增加了一个稳健、理论基础坚实且具有普适性的方法，扩展了基于多项式优化的异常检测方法的实际应用范围。
...
TL;DR: 提出Speculative Rollback Correction (SRC)框架，通过固定时间跨度的分支审查机制改进交互式网页代理的模仿学习训练，平衡专家干预时机，提高代理的鲁棒性和多样性。


<details>
  <summary>Details</summary>
Motivation: 在通过专家轨迹进行模仿学习训练交互式网页代理时，专家干预时机是一个关键挑战。延迟干预会导致早期错误积累，使页面状态进入不可恢复状态；而过早或过度干预则会使代理过度依赖专家策略，陷入局部最优和单一僵化轨迹。

Method: 提出SRC框架：学生代理执行短期的推测性片段后，教师进行分支审查；仅当局部进展中断时，教师定位第一个有害偏差并进行回滚；通过硬验证器筛选成功轨迹，保留到轻量级的质量-多样性存档中；使用局部化校正和验证通过的轨迹进行下一动作监督微调。

Result: 在WebArena-Infinity上，SRC收集了977条验证通过的轨迹和9,183个下一动作示例；固定时间跨度审查在恢复与查询权衡方面优于逐步骤审查，同时保留了验证通过的解决方案变体。

Conclusion: SRC框架通过分支级别的模仿学习方法有效解决了专家干预时机问题，在保持解决方案多样性的同时提高了代理的鲁棒性和训练效率，为交互式网页代理的训练提供了有效的新方法。
...
TL;DR: 本文提出了一种基于车辆磨损状态单调非递减假设的状态预测性维护方法，通过选择最新观测数据并转换为表格格式，使用AutoML简化建模过程，在Scania Component X数据集上相比现有方法降低了成本。


<details>
  <summary>Details</summary>
Motivation: 卡车车队的状态预测性维护系统面临挑战：数据量大、传感器故障检测复杂、成本效益平衡困难。为了解决这些问题，需要一种简化且有效的方法。

Method: 基于假设：被监测部件的磨损状态可以表示为单调非递减时间序列。方法包括：选择时间序列中最新的观测数据，将其转换为表格格式，然后使用专为表格数据设计的机器学习模型进行分类，并利用AutoML简化建模过程。

Result: 在Scania Component X数据集上，所提出的方法相比当前最先进方法降低了成本，同时通过AutoML简化了建模过程。

Conclusion: 该方法为卡车车队的条件预测性维护提供了一种有效且成本效益高的解决方案，通过简化数据预处理和建模过程，改善了现有方法的复杂性挑战。
...
TL;DR: 提出DynamicPTQ方法，通过识别对量化敏感的层并进行混合精度激活量化，解决了4位量化中由跨层残差流动态变化引起的量化不稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 在将权重、激活和KV缓存全部量化为4位精度时，激活值的极端值主导激活范围并放大量化误差。现有方法主要通过基于变换的平滑技术来缓解大激活值问题，但忽略了残差流的跨层动态变化。

Method: 引入跳跃比和历史特征信噪比来表征量化行为；提出DynamicPTQ，从残差流动态中识别量化敏感层，仅对这些层分配8位激活精度，而权重、KV缓存和其他激活保持4位精度。

Result: 在LLaMA-2和LLaMA-3上的实验显示，DynamicPTQ在W4A4KV4量化下一致改善了困惑度和零样本QA性能，同时实现了1.05到1.07倍的吞吐量提升，内存开销适度。

Conclusion: DynamicPTQ为稳健的低位大语言模型推理提供了一条实用路径，通过动态混合精度量化策略有效解决了跨层残差变化引起的量化不稳定问题。
...
TL;DR: 本文提出使用d-Simplex固定分类器学习兼容表征的方法，通过结合交叉熵损失和对比损失来捕获高阶依赖关系，实现模型更新期间的无缝检索服务


<details>
  <summary>Details</summary>
Motivation: 学习兼容表征的目标是学习可以在模型更新过程中互换使用的特征表示。现有方法在学习过程中可能无法完全捕获模型更新间表征的高阶依赖关系

Method: 使用d-Simplex固定分类器，通过交叉熵损失和对比损失的凸组合进行训练。该方法不仅捕获高阶依赖关系，而且在兼容性约束下等价于使用交叉熵损失学习

Result: d-Simplex固定分类器学习的稳定表示暗示了兼容性。该方法在模型顺序微调和偶尔替换改进模型的场景中，实现了无需重新处理图库图像的无间断检索服务，同时提升了性能，达到state-of-the-art水平

Conclusion: d-Simplex固定分类器为学习兼容表征提供了理论基础，结合对比损失的训练方法能够有效捕获高阶依赖关系，为未来工作奠定了基础并在实际学习场景中可直接应用
...
TL;DR: 该论文提出了一种基于抽象-精化框架的RNN认证鲁棒性验证方法，通过分区处理跨越零点的激活区间来减少松弛误差，并使用SHAP指导的时间步选择策略控制计算成本


<details>
  <summary>Details</summary>
Motivation: 现有RNN认证局部鲁棒性验证方法面临挑战：非线性松弛引入的近似误差通过循环连接传播并随时间累积，导致线性边界传播方法变得过于保守，无法证明实际鲁棒的输入，特别是当许多预激活区间跨越零点时

Method: 1. 提出抽象-精化框架，通过分区处理跨越零点的激活区间来消除主要松弛误差；2. 引入SHAP指导的时间步选择策略，根据隐藏状态对验证目标的贡献度排序，仅按时间顺序精化最关键的时间步；3. 在精化分支中，ReLU激活变得精确，tanh和sigmoid等平滑激活可以使用更紧的线性包络

Result: 在CIFAR10和MNIST笔画基准测试中，相比仅使用抽象的基线方法，验证成功率和鲁棒性边界紧密度一致提升，同时揭示了ReLU和tanh模型在运行时间上的明显权衡

Conclusion: 提出的抽象-精化框架有效提升了RNN认证鲁棒性验证的性能，通过针对性精化关键时间步实现了验证准确性提升与计算成本控制的平衡，为RNN验证提供了更可靠的解决方案
...
TL;DR: Net-Ev²是一个用于模拟网络中扰动事件传播的新型生成模拟器，通过结合事件线索并保持网络拓扑结构来进行仿真，在四个大规模道路网络上展示了先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在模拟网络事件演化时存在两个主要不足：1) 未能同时建模事件的结构化属性和非结构化语义；2) 未能有效捕捉拓扑结构来模拟网络事件演化。因此需要开发一个能够联合利用事件线索并保持网络拓扑结构的生成模拟器。

Method: Net-Ev²采用两阶段框架：1) 结构引导的掩码预训练；2) 拓扑感知扩散过程，通过U-Net风格的图下采样和上采样在去噪过程中实现。在推理时仅使用自然语言事件输入即可生成模拟。

Result: 研究构建了Net-Ev²-6.5M多模态基准数据集，包含四个大规模道路网络的配准事件和交通数据，并提出了新的拓扑感知度量JL-MMD来评估生成网络动态的拓扑保真度。实验证明Net-Ev²具有先进的性能和强大的泛化能力。

Conclusion: Net-Ev²是一个创新的网络事件演化模拟器，能够有效结合事件语义和网络拓扑结构，为模拟现实世界中扰动事件的传播提供了灵活且有效的解决方案。
...
TL;DR: 提出了μVLA模型，通过在预训练VLA骨干中隔离研究循环机制，使用可学习记忆令牌实现最小化循环结构，评估其在部分可观测环境中的效果


<details>
  <summary>Details</summary>
Motivation: 现有记忆增强的视觉-语言-动作模型同时引入了多种复杂组件（循环、检索、压缩模块等），使得循环机制本身的贡献与其他组件纠缠不清。需要隔离研究循环在VLA模型中的纯效应

Method: 在预训练VLA骨干中，通过添加少量可学习记忆令牌跨时间步传递，使用自注意力更新记忆，采用截断时间反向传播端到端训练，无辅助损失或架构改变。参数化μVLA家族，仅变化记忆宽度、TBPTT长度和记忆更新规则

Result: 在MIKASA-Robo上，μVLA将5个训练任务的平均成功率从0.42提升到0.84（最强设置），在相同记忆结构的保留任务上达到0.23（基线为0.07）。在需要不同记忆结构的任务上表现接近基线。在LIBERO上，最强循环变体达到96.2%平均成功率，表明在全可观测环境下无性能退化

Conclusion: 通过最小化循环结构研究，校准了记忆机制的能力边界：确定了哪些情况下仅循环机制足够，哪些情况需要额外的记忆结构。这种受控隔离研究为理解VLA模型中的记忆机制提供了清晰基准
...
TL;DR: 研究表明，基于机器学习的行为模型在通过模拟冲突预测交通事故频率方面比基于规则的传统模型更有前景，无需位置特定的校准，但当前ML模型尚无法生成现实的碰撞事件。


<details>
  <summary>Details</summary>
Motivation: 现有的微观交通仿真研究采用简化的基于规则的行为模型，能够较好地再现交通流，但往往无法生成真实的冲突动态，限制了碰撞预测的准确性。机器学习行为模型的发展为提高仿真真实性和碰撞频率预测提供了新机会。

Method: 在英国利兹的五个真实信号交叉口进行交通微观仿真，同时使用标准的基于规则模型和最先进的ML模型。通过二维碰撞时间(TTC)指标分析仿真车辆轨迹识别冲突，然后使用极值理论建模预测碰撞频率。

Result: ML模型产生的冲突能够产生与现实世界碰撞数据一致的碰撞预测，而基于规则的模型由于缺乏对特定仿真交叉口的校准，无法进行有意义的预测。直接使用ML生成的仿真碰撞来预测现实世界碰撞频率效果不佳。

Conclusion: 基于ML的行为模型在通过仿真冲突改进碰撞预测方面具有潜力，无需位置特定的模型校准。虽然当前ML模型能够真实地再现冲突，但尚不能生成现实的碰撞，为基于ML的交通微观仿真指明了明确的发展方向。
...
TL;DR: Conformal QoT：一个结合统计保证的QoT估计和操作决策策略的策略驱动框架，能在域偏移下实现可靠的光通路可行性预测，并在开放数据集上将准确率从92%提升到99.6%


<details>
  <summary>Details</summary>
Motivation: 现有的QoT（服务质量）估计方法在实际网络部署中面临域偏移问题，即训练数据和测试数据分布不一致，导致预测准确性下降。需要开发能够在域偏移条件下仍能保持高可靠性的光通路可行性预测方法。

Method: 提出Conformal QoT框架，该方法将统计保证的QoT估计与操作决策策略相结合。框架采用conformal prediction技术提供统计保证的预测区间，并结合策略驱动的决策机制来处理域偏移问题，确保在未知网络条件下的预测可靠性。

Result: 在开放数据集上的实验表明，Conformal QoT能够显著提升光通路可行性预测的准确性，从基准方法的92%提高到99.6%，同时提供统计保证的可靠性，在域偏移条件下保持稳定性能。

Conclusion: Conformal QoT框架成功解决了QoT估计中的域偏移问题，通过结合统计保证的预测和策略驱动决策，实现了高可靠性且准确的光通路可行性预测，为实际网络部署提供了有效的解决方案。
...
TL;DR: Dolph2Vec是一种针对海豚通讯的大规模自监督学习模型，在签名哨声分类和哨声检测任务上显著优于通用基线模型，并能捕获可解释的声学单元结构


<details>
  <summary>Details</summary>
Motivation: 现有的自监督学习模型在生物声学领域过于关注跨物种的泛化能力，而未能针对单个物种的精细通讯系统结构进行优化。研究希望开发专门针对海豚通讯的系统性分析方法

Method: 收集并发布了一个包含5年纵向记录的数据集，来自5只已知的海豚在半自然海洋环境中的录音。采用Wav2Vec2.0架构进行领域适配，创建了Dolph2Vec模型，这是首个大规模、物种特定的自监督学习模型

Result: Dolph2Vec在签名哨声分类和哨声检测两个任务上显著优于通用基线模型。学习到的嵌入和码本结构捕获了可解释的声学单元，这些单元与海豚哨声类别对齐，可能还包含子哨声结构

Conclusion: 自监督学习不仅可以作为模型，还可以作为科学工具来探索动物通讯研究中的假设，能够支持对通讯模式的细粒度分析
...
TL;DR: DPOP是对DPO的改进，通过在偏好损失基础上增加对参考模型生成的响应惩罚，当策略仍对偏好响应评分低于拒绝响应时激活惩罚，显著提升了性能


<details>
  <summary>Details</summary>
Motivation: 现有离线偏好优化方法（如DPO及其变种）仅使用静态数据集中偏好和非偏好响应对，忽略了参考模型对同一提示生成响应的有用信号。DPOP旨在利用这一未使用信息来提升性能

Method: 提出Direct Preference Optimization with Penalization (DPOP)，在基础偏好损失上增加基于门控的惩罚项，针对参考模型贪婪生成响应进行惩罚。惩罚仅在当前策略对偏好响应似然度仍低于非偏好响应时激活

Result: 在AlpacaEval 2.0基准上，DPOP相比DPO、SimPO和AlphaDPO在Llama-3-8b-it和Gemma-2-9b-it模型上分别提升了5.3%和4.4%的长度控制胜率。消融实验显示，类似SimNPO的长度归一化惩罚比NPO和令牌级unlikelihood更有效

Conclusion: DPOP通过有效利用参考模型生成响应信号，显著改进了现有离线偏好优化方法，为强化学习从人类反馈的实用替代方案提供了更强大的工具
...
TL;DR: RGSD是一种无验证器的训练方法，通过让基于评分的教师策略指导无条件学生策略，实现密集的逐令牌学习信号，替代了稀疏的轨迹级奖励


<details>
  <summary>Details</summary>
Motivation: 在开放式领域中使用评分标准作为替代RLVR的方法，但现有基于评分标准的训练方法依赖LLM验证器，这带来了训练时间开销大、易受验证器特定偏见影响、反馈信号稀疏等问题

Method: 提出Rubric-Guided Self-Distillation (RGSD)方法，让基于评分标准的教师策略指导无条件学生策略，通过token-by-token蒸馏将评分条件化教师分布转移到学生模型中，消除了训练过程中对LLM验证器的依赖

Result: 在Qwen-2.5和Qwen3-Thinking模型的医学和科学领域实验中，RGSD仅使用每个提示一次策略采样且无需训练时验证器调用，就能达到与基于验证器的GRPO相当的评分满意度。消融研究表明原始评分比自生成参考响应提供更强的教师丰富信号

Conclusion: RGSD是一种互补的无验证器替代方案，特别在验证器成本或可靠性成为瓶颈时表现出优势，尽管在某些设置中更强的GRPO验证器可能优于RGSD
...
TL;DR: ToolSense是一个开源LLM驱动的诊断框架，用于评估工具检索模型的真实理解能力，揭示当前基准测试的局限性以及模型存在的知识-检索分离问题。


<details>
  <summary>Details</summary>
Motivation: 当前工具检索基准测试使用冗长、完整指定的查询，并且评估时应用受限解码，无法揭示模型是否真正理解其工具。现有的嵌入检索方法可能无法充分捕获专门工具语义，而参数化工具检索虽然表现良好，但缺乏对其实际理解能力的验证。

Method: 提出了ToolSense框架，该框架自动生成三个诊断基准：包含三个模糊级别的现实检索基准（RRB）、多项选择题探测基准和问答探测基准。将该框架应用于ToolBench（约47k个工具），评估了五种参数模型训练配置。

Result: 发现知识-检索分离现象：在RRB查询中，几种配置相比完全指定的ToolBench基准下降了约50-64个百分点，甚至低于嵌入模型基线。尽管某些模型检索性能良好，但在事实探测中得分接近随机，表明检索能力强不一定代表对工具有真正的理解。

Conclusion: 当前的工具检索模型存在知识-检索分离问题，ToolSense提供了一个有效的诊断框架来评估模型对工具的真实理解能力，而不是仅仅衡量检索性能。这有助于揭示现有基准的局限性并推动更真实的工具检索模型开发。
...
TL;DR: Arbor是一个多代理框架，引入结构化树搜索作为在大状态行动空间中自主代理的认知层，通过协调多个代理进行LLM推理优化，实现显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有自主优化系统主要针对孤立目标进行无状态评估，无法有效处理大规模状态空间中的协同优化问题。特别是在LLM推理优化领域，传统方法需要跨多个技术栈的工程团队协作，难以实现全自主的优化过程。

Method: Arbor采用结构化树搜索作为认知层，建立包含评分假设的显式搜索树作为共享工作内存。系统包含协调者代理（驱动优化并委派给领域专家）和批评者代理（通过根本原因分析、自省和测量验证确保稳定性），形成制衡架构。代理能力分解为硬技能（领域专业知识）和软技能（协调协议），支持完全自主的多日优化活动。

Result: Arbor在LLM推理优化中实现了高达193%的推理吞吐量-延迟帕累托改进，相比厂商优化基线显著提升。单一代理没有该架构时仅达到33%吞吐量改进并在数小时内崩溃。系统可泛化到多代硬件平台，运行间方差在2个百分点内，证明方法是硬件无关且可复现的。

Conclusion: Arbor通过结构化树搜索和多代理制衡架构，为大规模状态空间中的自主优化提供了有效的解决方案，在LLM推理优化领域实现了传统方法无法达到的性能提升和稳定性，展示了自主多代理系统的实际应用价值。
...
TL;DR: 提出了一个AI代理决策支持框架，通过优化问题最小化支持使用同时控制"错过支持错误"，采用自适应阈值策略和在线校准方法来减少不必要的支持调用。


<details>
  <summary>Details</summary>
Motivation: 传统决策支持研究人类如何使用机器学习模型做出更好决策，而在现代智能代理系统中，角色正在发生反转：AI代理代表用户行动，而人类和工具成为围绕它们的支持机制。这种角色反转带来了可靠性问题，因为代理错误可能产生严重后果，代理行为必须与人类目标和约束保持一致。

Method: 提出了一个战略决策支持框架，通过优化问题最小化支持使用，同时控制反事实错过支持错误概率。在总体层面，最优策略是基于支持价值的阈值规则。基于此结构，开发了在线算法，自适应地对支持价值评分进行阈值设置，并使用随机探索来控制错过支持错误，无需分布假设。进一步引入了在线即时校准方法，减少不必要的支持调用。

Result: 将该框架应用于多种场景，包括信息收集、人机协作和工具使用，展示了每种场景如何通过相同的战略决策支持视角建模。实验结果表明，该方法在实践中可靠地控制了目标错误，同时显著减少了支持使用。

Conclusion: 论文重新审视了决策支持的两个基本原则——寻求支持的成本-价值权衡和不确定性量化，在AI代理作为中心行动者的情境下，提出了一个有效的战略决策支持框架，能够在控制错误的同时优化支持使用效率。
...
TL;DR: 提出Pythagoras-Prover系列，一个计算高效的Lean定理证明器家族，包括自回归模型和基于扩散的证明器，通过课程学习、动态过滤和增强形式化技术，在MiniF2F基准上以更小规模超越现有大型模型。


<details>
  <summary>Details</summary>
Motivation: 现有的Lean定理证明器需要大量训练和推理计算资源，主要问题包括：已验证的证明数据稀缺、形式化证明搜索中的长推理轨迹，使得监督微调和采样都很昂贵。

Method: 1) 构建按难度分层的Lean已验证语料库用于课程监督微调；2) 采用动态证明推理过滤方案，在8k标记上下文预算内保留信息量大的证明轨迹；3) 提出增强Lean形式化(ALF)，通过自蒸馏扩展稀缺已验证语料，创建形式语句的变体；4) 开发两种生成范式：4B和32B参数的自回归模型，以及首次概念验证的基于扩散的证明器(4B)。

Result: Pythagoras-Prover-4B在MiniF2F-Test上以pass@32达到86.1%，超越DeepSeek-Prover-V2-671B(82.4%)，参数量减少约167倍；Pythagoras-Prover-32B达到开源最佳水平93.0%，解决了672个PutnamBench问题中的93个。在污染敏感的MiniF2F-ALF基准上，32B模型最强，4B模型匹配先前最佳模型Goedel-Prover-V2-32B。

Conclusion: Pythagoras-Prover展示了通过计算高效的方法可以显著改进Lean定理证明，通过课程学习、动态过滤和ALF增强技术，较小的模型能够在形式化定理证明任务上超越大型模型，为实际计算预算下的实用定理证明器提供了新途径。
...
TL;DR: PersonaDrive是一个基于检索增强的驾驶代理系统，通过从人类风格指令驾驶数据集中检索演示，为封闭环驾驶模拟器提供多样化的非自我交通代理行为。


<details>
  <summary>Details</summary>
Motivation: 当前驾驶模拟器中的非自我交通代理行为单一，现有方法通过后验标签或LLM推理奖励权重来引入风格变化，但这些只是风格的代理信号而非人类明确风格指令下的实际驾驶演示。

Method: 提出三步流程：1) 使用图像-文本相似度分数对每个风格的人类驾驶数据进行离线三元组挖掘；2) 训练轻量级检索头，将冻结的视觉特征与小控制编码器在每个风格数据库上融合；3) 微调单一VLA骨干网络，将检索到的上下文点视为路径点预测中的上下文行为演示。

Result: 在Bench2Drive上，PersonaDrive（无风格）驾驶得分比SimLingo提高4.6%，比HiP-AD提高2.5%；在风格条件下，每个风格都获得最高驾驶得分（最弱风格比最强基线DMW高5.4%）；从保守到激进指令，平均速度和加速度分别提升18%和25%。

Conclusion: PersonaDrive通过检索风格指令的人类驾驶演示，实现了无需每个风格重新训练的单骨干网络，为封闭环模拟提供人类风格、风格多样的非自我代理，显著提升了驾驶性能和风格表达能力。
...
TL;DR: 论文提出了Varied Deception测试平台和训练模型生物来评估语言模型谎言检测器的有效性，发现现有检测器在训练模型上表现大幅下降，只有基于思维链的检测器保持较好性能


<details>
  <summary>Details</summary>
Motivation: 评估语言模型谎言检测器需要能够验证模型真实信念与表述相反的测试环境，现有方法无法满足这一要求，使得检测结果难以解释

Method: 创建了13个经过训练的推理模型生物，其隐藏信念在思维链中得到验证并可推广到未见任务；开发了Varied Deception测试平台，涵盖多种诱使撒谎的动机；评估了四种检测器：思维链判断器、概率分类器和两种激活探针

Result: 在诱导撒谎测试中，31个开放权重模型（20亿到1万亿参数）的所有检测器都随模型能力提升而改善；但在训练模型上，基于激活和概率的检测器表现大幅下降，仅思维链判断器保持较强性能（0.82平衡准确率）

Conclusion: 当前谎言检测器无法支持对模型信念的高置信度主张，需要进一步研究以解决现有局限，论文提供了公开的数据集、模型生物和训练好的检测器
...
TL;DR: TrajGenAgent是一个无需微调的语义感知分层LLM-智能体框架，用于生成现实的人类移动轨迹，通过两阶段设计提高时空保真度和语义连贯性。


<details>
  <summary>Details</summary>
Motivation: 人类移动数据对交通、城市规划、疫情控制很重要，但大规模轨迹收集成本高且受隐私限制。现有基于LLM的生成器要么依赖提示工程（缺乏细粒度时空基础），要么依赖轨迹级微调（计算成本高且可能削弱通用推理能力）。

Method: 提出TrajGenAgent框架，采用两层协调器-工作者设计：1）LLM通过上下文学习合成基于个人和工作日的活动链；2）确定性工作流通过个性化POI检索、距离感知位置选择、运动学感知旅行时间传播和LLM时长估计，将每个活动基础化为完整访问。

Result: 在基准和大规模模拟数据集上的实验表明，TrajGenAgent在时空保真度、语义连贯性和个体特异性行为真实性方面优于代表性神经和基于LLM的基线方法，同时避免参数更新。

Conclusion: TrajGenAgent通过语义感知分层智能体框架，在不进行模型微调的情况下实现了高质量的人类移动轨迹生成，平衡了统计精度和语义合理性，并提出了基于异常检测的新评估框架。
...
TL;DR: Evoflux：一种基于进化搜索的推理方法，通过可执行工具工作流的修复来提高紧凑语言模型工具使用的执行可行性


<details>
  <summary>Details</summary>
Motivation: 紧凑语言模型虽然在成本、延迟和部署风险方面具有优势，但在MCP风格的工具使用中存在执行问题。小型规划器生成的看似合理的工作流图经常在工具解析、参数验证、依赖跟踪或执行阶段失败，而小规模数据蒸馏难以覆盖恢复行为以修复失败的计划。

Method: Evoflux是一种推理时进化搜索方法，将紧凑工具使用视为可执行工具工作流的修复。它通过结构化编辑、执行反馈、自适应强度、元指导重新设计和多样性剪枝来进化类型化工作流图。

Result: 在包含实时MCP服务器和250个工具的MCP-Bench任务测试中，Evoflux将执行可行性从大约3%提高到17-24%。相比之下，SFT、SFT+DPO等方法表现不佳甚至低于零-shot性能，ReAct虽然能达到更高峰值但方差和token成本更高。

Conclusion: 执行基础搜索在稀缺教师轨迹预算下更加可靠，进化方法能够有效提高紧凑语言模型工具使用的执行可行性，特别适合处理动态变化的工具目录和复杂的工具依赖关系。
...
TL;DR: 报告分析了从人类水平AGI到人工通用超级智能（ASI）的过渡路径，探讨了可能的瓶颈和挑战，并提出了应对AI加速发展所需的多学科全球性准备。


<details>
  <summary>Details</summary>
Motivation: AGI的发展已从遥不可及的猜测转变为大型AI组织的具体十年目标，这对人类社会将产生深远影响。报告旨在探究AI在AGI后的世界如何继续沿着机器智能连续体发展，特别是从人类水平AGI到超越人类组织的超级智能的过渡。

Method: 报告基于理论上充分理解的机器智能连续体（终点是通用AI），聚焦从AGI到ASI的过渡。首先描述了ASI的特征，然后探讨了四条可能的路径：AGI扩展、AI范式转变、递归改进以及大规模多智能体集体中涌现ASI，最后分析了这些路径上的摩擦和瓶颈。

Result: 报告识别了四条从AGI到ASI的潜在路径，并讨论了每条路径可能遇到的摩擦和瓶颈。由于预测ASI进展存在巨大不确定性，不能排除AI进展在未来几年继续加速的可能性。这意味着单一由人类水平AGI引发的变革性跃迁可能是不准确的，更可能是AI在科学技术各领域引发一系列变革性社会变化。

Conclusion: 应对AI加速发展及其带来的深远社会变革需要全球范围内的多学科协作努力。这不仅仅是技术问题，而是涉及科学、技术、社会各个层面的复杂挑战。
...
TL;DR: 本研究通过在学术医疗中心的电子病历系统中部署LLM系统，开发了一个预响应分类器来预测用户是否会拒绝LLM生成的回答，利用部署特定的上下文信息而非仅基于查询内容，取得了0.719的AUROC性能


<details>
  <summary>Details</summary>
Motivation: 现有静态基准测试主要衡量正确性而非用户接受度，且需要在密集标注的数据集上进行评估，这对于评估临床系统存在重大盲点。需要在实际部署条件下评估LLM系统在临床环境中的真实效用

Method: 在学术医疗中心的电子病历系统中嵌入LLM系统，开发预响应分类器，基于查询内容和部署特定上下文（如提供者类型、部门名称、使用的语言模型）来预测未来交互中用户拒绝LLM响应的风险

Result: 预测模型在4.5个月的用户反馈数据上取得了0.719的AUROC性能。进一步评估了预测在两种下游用例（护栏触发和弃权）中的效益。关键发现是利用部署特定上下文相比仅使用查询内容能更好地预测用户拒绝

Conclusion: 本研究证明了利用部署特定上下文预测用户拒绝的可行性，为针对性护栏机制打开了大门，强调了在实际部署条件下评估临床LLM系统的重要性
...
TL;DR: 本文提出DAF-AGI框架，用于评估AGI定义的可裁判性，并应用于分析当前生成系统是否构成AGI的主张。研究发现仅性能基准定义支持"AGI已到来"的论断。


<details>
  <summary>Details</summary>
Motivation: 关于人工智能通用智能是否已到来的争议缺乏统一标准，不同定义会导致对同一系统的不同判断。这种定义不一致性被视为设计和治理问题，需要系统化的评估框架。

Method: 采用设计科学研究方法论，开发DAF-AGI概念性框架，包含五个评估定义可裁判性的序数标准以及结构化治理审计（作者身份、利益、认证、外部验证和修订权威）。将该框架应用于五个主要测量家族和一个紧缩边界立场。

Result: 基于2024-2025年的证据，关于当前生成系统因在许多认知任务上表现优于受过良好教育的成年人而构成AGI的论断，仅在使用性能基准操作化定义时可获得认证；能力本体论、心理测量和技能获取方法不支持该论断，经济家族仍不确定，紧缩立场拒绝二元裁判。

Conclusion: DAF-AGI提供了一个集成和操作化的新框架，但需要独立应用和验证。论文进一步提出定义主权作为算法主权的重要组成部分，即机构在公共问责下对进口技术类别进行质疑、认证和修订的能力。
...
TL;DR: ToM-U是一个心智理论的形式化计算模型，通过构建局部认知世界模型来推理他人的信念状态，与现有的贝叶斯心智理论和模拟理论等形成对比。


<details>
  <summary>Details</summary>
Motivation: 现有心智理论方法存在问题：贝叶斯方法预设而非推导信念状态，模拟理论和理论理论缺乏形式化的认知状态推理工具。需要一种能形式化处理"谁在何时以何种可信度告诉了谁什么"这类复杂认知状态推理问题的计算框架。

Method: ToM-U通过构建局部认知世界模型(LEWMs)来实现认知状态推理。LEWMs是有向类型图，表示主体、状态节点和认知关系。方法包括：1) LEWM结构形式定义；2) 包含有序信息访问历史的主体节点属性；3) 有界增殖机制处理递归心智化；4) 三种推理程序；5) 捕获失败心智化尝试的结构痕迹的残差函数。

Result: ToM-U提供了一个形式化框架，能够推理他人的认知状态，不预设信念状态而是通过评估候选LEWMs来推导。该方法生成可证伪的预测，解释了心智化失败的模式，这些预测源于模型的结构特性而非辅助假设。

Conclusion: ToM-U是一个领域无关的心智化机制，位于目标推理和其他下游社会认知过程的上游。它填补了现有心智理论方法的空白，提供了一个形式化、可计算、可测试的认知状态推理框架。
...
TL;DR: 该论文探讨了LLM的自我报告与行为一致性，发现理论计划行为比大五人格能更准确预测LLM行为，但一致性受会话上下文和身份诱导影响。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于：1）了解LLM自我报告是否能可靠预测其行为对安全部署至关重要；2）现有研究基于大五人格发现LLM存在显著的自我报告-行为分离，但这可能是因为大五人格预测能力本身就较弱；3）之前的研究方法存在局限性，如会话隔离和弱上下文匹配，无法确定LLM是否真的缺乏内在一致性。

Method: 研究方法包括：1）对比大五人格和理论计划行为两种测量方法；2）在四个行为任务和11个前沿LLM上进行实验；3）同时变化会话上下文和身份诱导；4）考察不同条件下LLM自我报告与行为的一致性。

Result: 研究发现：1）在共享对话中，理论计划行为达到人类水平的一致性，而大五人格无法做到；2）跨独立对话时，一致性仅存在于那些由训练形成的隐含偏见等行为上，当行为受上下文强烈影响时（如奉承行为）一致性会崩溃；3）角色提示使自我报告在不同对话间更一致，但无法使行为与其保持一致。

Conclusion: 论文结论指出：1）粗粒度的人格框架如大五人格可能不是测试部署行为的最佳工具；2）需要更多任务和行为特定的测量工具；3）即使这些特定工具也需要在不同任务和上下文中进行评估。
