...
TL;DR: 扩散模型为神经影像学提供了一种能够统一建模多变量依赖性的方法，在高维度下表现优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统的神经影像分析方法通常为每个成像衍生表型单独建模，忽略了多变量依赖性，而扩散模型可以统一建模并提供校准的多变量分布。

Method: 使用去噪扩散概率模型（DDPM）作为表格成像衍生表型（IDP）的统一条件密度估计器，并通过两种去噪器骨干（FiLM调制的MLP和SAINT变换器）实现。

Result: 扩散模型在低维度下与传统方法表现相当，而在高维度下变换器骨干表现更优，能够更好地保留高阶依赖关系。

Conclusion: 扩散模型在神经影像学中提供了一种实用的方法，能够生成校准的多变量偏差分布，同时保持与单变量流程的兼容性。
...
TL;DR: The paper introduces Causal Analyst, a framework for causal analysis of jailbreaks in LLMs, identifying key causal features and demonstrating their practical use in attack and defense.


<details>
  <summary>Details</summary>
Motivation: To uncover the mechanisms behind 'jailbreaks' in LLMs for enhancing their safety and reliability, addressing the gap in understanding causal relationships between interpretable prompt features and jailbreak occurrences.

Method: The paper proposes Causal Analyst, a framework integrating LLMs into data-driven causal discovery to identify direct causes of jailbreaks. It involves jointly training LLM-based prompt encoding and GNN-based causal graph learning.

Result: Specific features like 'Positive Character' and 'Number of Task Steps' are identified as direct causal drivers of jailbreaks. Applications include a Jailbreaking Enhancer and a Guardrail Advisor, demonstrating practical utility.

Conclusion: Analyzing jailbreak features from a causal perspective is an effective and interpretable approach for improving LLM reliability.
...
TL;DR: The paper introduces Momentum Attention, a symplectic augmentation for Transformers, enabling Single-Layer Induction and bridging AI with physics and signal processing, validated by extensive experiments.


<details>
  <summary>Details</summary>
Motivation: To address the topological depth constraint for induction head formation in standard architectures, the authors aim to inject kinematic momentum, thereby enabling direct access to velocity and simplifying the model structure.

Method: The authors extend the Transformer's computational graph with conservation laws and AC dynamics, introducing Momentum Attention—a symplectic augmentation that embeds physical priors via kinematic momentum. This approach enables Single-Layer Induction and Spectral Forensics via Bode Plots.

Result: The 125M Momentum model outperforms expectations on induction-heavy tasks, closely tracking a 350M baseline within ∼2.9% validation loss. A scaling law γ∗=4.17×N−0.74 is established, showing momentum-depth fungibility.

Conclusion: The paper presents a framework that integrates physical circuit dynamics into Transformer models, demonstrating enhanced performance on induction-heavy tasks and offering a novel analytical toolkit bridging Generative AI, Hamiltonian Physics, and Signal Processing.
...
TL;DR: 特征导向方法虽能有效控制LLM行为，但导致性能显著下降，提示工程仍是更优选择。


<details>
  <summary>Details</summary>
Motivation: 特征导向作为一种通过直接操纵内部表示来控制LLM行为的方法，显示出潜力，但其在实际应用中的有效性尚不明确，尤其是与输出质量之间的潜在权衡。

Method: 通过评估Goodfire的Auto Steer与提示工程基线在14个导向查询上的表现，使用Llama-8B和Llama-70B模型，测量准确性、连贯性和行为控制。

Result: Auto Steer成功修改了目标行为（Llama-8B得分为3.33 vs. 2.98，Llama-70B为3.57 vs. 3.10），但导致性能显著下降：MMLU问题的准确性从66%降至46%（Llama-8B）和87%降至73%（Llama-70B），连贯性分别从4.62降至2.24和4.94降至3.89。简单提示工程实现了最佳整体平衡。

Conclusion: 当前的特征导向方法在实际部署中存在局限性，尤其是在任务性能不可牺牲的情况下。更广泛地说，研究表明，机械控制方法在部署前必须通过实证研究来权衡其能力与行为之间的关系。
...
TL;DR: DCER is a multimodal fusion framework using compression and energy-based reconstruction to handle noise and missing data, achieving top performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address robustness challenges in multimodal fusion: noisy inputs degrading representation quality and missing modalities causing prediction failures.

Method: DCER employs dual-stage compression (within-modality frequency transforms and cross-modality bottleneck tokens) and energy-based reconstruction for missing modalities.

Result: Achieves high performance on CMU-MOSI, CMU-MOSEI, and CH-SIMS benchmarks, with a U-shaped robustness pattern.

Conclusion: DCER demonstrates state-of-the-art performance across benchmarks, showing robustness in both complete and high-missing modality conditions.
...
TL;DR: LISA是一种推断时适应方法，结合延迟坐标嵌入和拉普拉斯谱学习，有效提升动态系统预测性能。


<details>
  <summary>Details</summary>
Motivation: 提出LISA方法旨在通过观察前缀实现拉普拉斯基于时间序列模型的推断时适应。

Method: LISA结合延迟坐标嵌入和拉普拉斯谱学习，生成扩散坐标状态表示，并使用冻结的非线性解码器进行一步预测。

Result: 在预测和自回归实验中，LISA优于冻结基线，尤其在动态变化时表现最佳。

Conclusion: LISA将上下文适应与非参数谱方法联系起来，为动态系统提供了一种有效的推断时适应方法。
...
TL;DR: A framework combining physical knowledge with causal discovery improves graph recovery and stability in dynamical systems via SDE modeling and sparsity-inducing MLE.


<details>
  <summary>Details</summary>
Motivation: Integrating physics-based models with causal discovery can leverage physical knowledge as an inductive bias, improving identifiability, stability, and robustness in dynamical systems, which often exhibit feedback, cyclic interactions, and non-stationary data trends.

Method: The framework models system evolution as a stochastic differential equation (SDE), with the drift term encoding known ODE dynamics and the diffusion term representing unknown causal couplings. A scalable sparsity-inducing MLE algorithm is developed to exploit causal graph structure for efficient parameter estimation.

Result: Experiments demonstrate that the approach improves causal graph recovery and yields more stable, physically consistent estimates than state-of-the-art data-driven baselines.

Conclusion: The proposed integrative causal discovery framework effectively combines physical knowledge with data-driven methods, enhancing causal graph recovery and producing more stable, physically consistent estimates compared to purely data-driven approaches.
...
TL;DR: TPC reduces variance in continuous-time generative models by coupling velocity predictions at paired timesteps, improving efficiency and sample quality without model changes.


<details>
  <summary>Details</summary>
Motivation: Continuous-time generative models often suffer from high estimator variance and inefficient sampling due to independent timestep training. TPC aims to address this without altering the model or solver.

Method: Introduces Temporal Pair Consistency (TPC), a principle that couples velocity predictions at paired timesteps to reduce variance. It operates at the estimator level and provides theoretical analysis showing its benefits.

Result: TPC improves sample quality and efficiency on datasets like CIFAR-10 and ImageNet, achieving lower FID at similar or lower computational costs compared to prior methods. It also integrates well with modern pipelines.

Conclusion: Temporal Pair Consistency (TPC) is a lightweight and effective variance-reduction principle that improves sample quality and efficiency in continuous-time generative models without requiring modifications to the model architecture or solver.
...
TL;DR: GAPO proposes a dynamic anchor method for preference optimization, improving robustness and performance by addressing issues with static references and reward drift.


<details>
  <summary>Details</summary>
Motivation: The static reference in methods like DPO becomes miscalibrated as the policy drifts, leading to distributional mismatch and spurious preference signals under noisy supervision. Reference-free variants suffer from unconstrained reward drift.

Method: GAPO replaces the fixed reference with a dynamic, adversarial local perturbation of the current policy, introducing an adaptive reweighting mechanism and the Anchor Gap to modulate preference pair importance.

Result: GAPO consistently improves robustness and matches or enhances performance on standard LLM alignment and reasoning benchmarks across diverse noise settings.

Conclusion: GAPO introduces a dynamic, geometry-aware anchor to address the limitations of static references and unconstrained reward drift in preference optimization, demonstrating improved robustness and performance across various benchmarks.
...
TL;DR: A novel system encodes data as graphs with elemental relations, using a graph-Hamiltonian operator and low-precision arithmetic to enable symbolic computation and hierarchical representations, offering a promising alternative to standard ANN methods.


<details>
  <summary>Details</summary>
Motivation: The motivation is to create a system where simple relational encoding is intrinsic, not an add-on, and to explore an architecture distinct from standard ANN methods, capable of symbolic computation and hierarchical representations.

Method: The method involves encoding arbitrary data as graphs with edges representing elemental pairwise relations, using a graph-Hamiltonian operator to calculate energies among encodings, and employing radically low-precision arithmetic for computational efficiency.

Result: The system successfully processes standard ANN examples and generates representations with symbolic computation characteristics, identifying logical relational structures and enabling abductive inferential steps. It also derives an equivalent set of ANN operations for embedded vector encodings.

Conclusion: The paper concludes that the introduced system, with its novel graph-Hamiltonian operator and low-precision arithmetic, offers a promising alternative to standard ANN methods by enabling symbolic computation and hierarchical representations. It also identifies a special case of embedded vector encodings that could enhance higher-level semantic representation.
...
TL;DR: A²-LLM is an end-to-end model for conversational digital humans that integrates language, audio, and facial motion, outperforming traditional cascaded systems in expressiveness and speed.


<details>
  <summary>Details</summary>
Motivation: Current systems for conversational digital humans rely on cascaded architectures, which suffer from accumulated errors, high latency, and poor real-time performance, limiting emotional depth.

Method: The paper proposes A²-LLM, an end-to-end model that jointly reasons about language, audio prosody, and 3D facial motion within a unified framework. It also introduces FLAME-QA, a multimodal dataset for training.

Result: A²-LLM achieves emotionally rich facial movements and maintains real-time efficiency (500 ms latency, 0.7 RTF).

Conclusion: A²-LLM demonstrates superior emotional expressiveness and real-time performance, addressing the limitations of traditional cascaded architectures in conversational digital humans.
...
TL;DR: SLAY is a linear-time attention mechanism that closely mimics softmax attention by using spherical constraints and Yat-kernels, achieving high efficiency and performance without trade-offs.


<details>
  <summary>Details</summary>
Motivation: To address the computational inefficiency of standard softmax attention while maintaining its performance, the authors propose a geometry-aware linear-time attention mechanism inspired by inverse-square interactions in physics.

Method: The method constrains queries and keys to the unit sphere, utilizes Bernstein's theorem to express the spherical Yat-kernel as a nonnegative mixture of polynomial-exponential product kernels, and derives a positive random-feature approximation for linear-time attention.

Result: Empirically, SLAY achieves performance nearly indistinguishable from standard softmax attention, outperforms prior linear-time attention mechanisms, and scales linearly in time and memory.

Conclusion: SLAY provides a highly efficient and effective linear-time attention mechanism that closely approximates standard softmax attention, enabling scalable Transformers without significant performance trade-offs.
...
TL;DR: HeteroComp introduces a method for summarizing heterogeneous tensor streams and detecting group anomalies using Gaussian process priors, outperforming existing techniques.


<details>
  <summary>Details</summary>
Motivation: Existing tensor decomposition and anomaly detection methods fail to handle heterogeneous tensor streams and temporal dynamics, leading to degraded performance in detecting group anomalies.

Method: The method employs Gaussian process priors to model unknown distributions of continuous attributes and temporal dynamics, directly estimating probability densities from data.

Result: Extensive experiments show that HeteroComp outperforms state-of-the-art algorithms in group anomaly detection accuracy and maintains computational efficiency independent of data stream length.

Conclusion: HeteroComp effectively addresses the limitations of existing methods by providing a concise summarization of heterogeneous tensor streams and enabling accurate group anomaly detection.
...
TL;DR: LLMs resolve knowledge conflicts via 'Orthogonal Interference,' rotating representations rather than suppressing truths, highlighting the need for vectorial monitoring over scalar metrics.


<details>
  <summary>Details</summary>
Motivation: To understand how LLMs resolve knowledge conflicts through compliance and whether suppression arises from signal magnitude dilution or directional geometric alteration.

Method: Conducted a layer-wise geometric analysis across Qwen-4B, Llama-3.1-8B, and GLM-4-9B, decomposing residual stream updates into radial and angular components.

Result: Compliance is characterized by 'Orthogonal Interference,' where conflicting contexts inject a quasi-orthogonal steering vector, rotating hidden state representations without suppressing internal truths.

Conclusion: The study challenges scalar confidence metrics for detecting hallucinations and emphasizes the need for vectorial monitoring to differentiate genuine knowledge integration from superficial mimicry.
...
TL;DR: PTL is a gradual pruning method for LLMs that reduces model size by half with minimal performance loss, applicable to various tasks and pruning strategies.


<details>
  <summary>Details</summary>
Motivation: To reduce computational resource consumption and accelerate inference in LLMs by eliminating redundant parameters without compromising reasoning performance.

Method: A gradual compacting method using a Prune-Tune Loop (PTL) that divides compression into fine-grained iterations, with finetuning at each stage to restore performance.

Result: PTL compresses LLMs significantly with lightweight post-training, maintaining performance comparable to the original model on reasoning tasks and showing effectiveness in tasks like code generation.

Conclusion: The proposed PTL method effectively compresses LLMs to nearly half their original size with minimal performance loss, demonstrating broad applicability across various tasks and pruning strategies.
...
TL;DR: CyIN, a novel framework, enhances multimodal learning robustness by cyclically integrating modalities and reconstructing missing ones, outperforming in varied scenarios.


<details>
  <summary>Details</summary>
Motivation: Address the performance drop of pre-trained multimodal models in real-world deployments where modality presence is variable and unpredictable, by improving robustness to dynamic missing modalities.

Method: The paper introduces a Cyclic INformative Learning framework (CyIN) that builds an informative latent space using token- and label-level Information Bottleneck cyclically, supplemented by cross-modal cyclic translation to reconstruct missing modalities.

Result: Extensive experiments on 4 multimodal datasets show CyIN's superior performance in both complete and incomplete scenarios.

Conclusion: CyIN successfully bridges the gap between complete and incomplete multimodal learning by jointly optimizing both scenarios in a unified model, demonstrating superior performance across diverse datasets.
...
TL;DR: A novel framework for neural operators to handle complex boundary conditions via function extensions, improving accuracy and robustness across diverse PDE problems.


<details>
  <summary>Details</summary>
Motivation: Existing neural operators struggle with highly variable boundary conditions, limiting their applicability. This work addresses the challenge of handling complex, non-homogeneous boundary conditions in PDE solutions.

Method: The approach maps boundary data to latent pseudo-extensions over the spatial domain, allowing standard operator learning architectures to process boundary information alongside input domain functions.

Result: The method achieves state-of-the-art accuracy on 18 challenging datasets, outperforming baselines significantly.

Conclusion: The proposed framework effectively integrates complex boundary conditions into neural operators, enhancing their accuracy and robustness for a broader range of PDE problems without requiring hyperparameter tuning.
...
### [18] [Knowing When to Answer: Adaptive Confidence Refinement for Reliable Audio-Visual Question Answering](https://arxiv.org/abs/2602.04924)
...
### [19] [Internalizing LLM Reasoning via Discovery and Replay of Latent Actions](https://arxiv.org/abs/2602.04925)
...
### [20] [Euphonium: Steering Video Flow Matching via Process Reward Gradient Guided Stochastic Dynamics](https://arxiv.org/abs/2602.04928)
...
### [24] [Linear Model Merging Unlocks Simple and Scalable Multimodal Data Mixture Optimization](https://arxiv.org/abs/2602.04937)
...
### [25] [Transolver-3: Scaling Up Transformer Solvers to Industrial-Scale Geometries](https://arxiv.org/abs/2602.04940)
...
### [26] [Improving Set Function Approximation with Quasi-Arithmetic Neural Networks](https://arxiv.org/abs/2602.04941)
...
### [27] [Privileged Information Distillation for Language Models](https://arxiv.org/abs/2602.04942)
...
### [28] [Stochastic hierarchical data-driven optimization: application to plasma-surface kinetics](https://arxiv.org/abs/2602.04975)
...
### [29] [Near-Optimal Dynamic Matching via Coarsening with Application to Heart Transplantation](https://arxiv.org/abs/2602.04989)
...
### [30] [Position: Machine Learning for Heart Transplant Allocation Policy Optimization Should Account for Incentives](https://arxiv.org/abs/2602.04990)
...
### [31] [Learning Rate Matters: Vanilla LoRA May Suffice for LLM Fine-tuning](https://arxiv.org/abs/2602.04998)
...
### [32] [EntRGi: Entropy Aware Reward Guidance for Diffusion Language Models](https://arxiv.org/abs/2602.05000)
...
### [33] [Enhanced QKNorm normalization for neural transformers with the Lp norm](https://arxiv.org/abs/2602.05006)
...
### [34] [Private PoEtry: Private In-Context Learning via Product of Experts](https://arxiv.org/abs/2602.05012)
...
### [35] [A Simple Reduction Scheme for Constrained Contextual Bandits with Adversarial Contexts via Regression](https://arxiv.org/abs/2602.05019)
...
### [36] [Laws of Learning Dynamics and the Core of Learners](https://arxiv.org/abs/2602.05026)
...
### [37] [Laplacian Representations for Decision-Time Planning](https://arxiv.org/abs/2602.05031)
...
### [38] [Causal Representation Meets Stochastic Modeling under Generic Geometry](https://arxiv.org/abs/2602.05033)
...
### [39] [Feedback Control for Multi-Objective Graph Self-Supervision](https://arxiv.org/abs/2602.05036)
...
### [40] [ReFORM: Reflected Flows for On-support Offline RL via Noise Manipulation](https://arxiv.org/abs/2602.05051)
...
### [41] [Learning, Solving and Optimizing PDEs with TensorGalerkin: an efficient high-performance Galerkin assembly algorithm](https://arxiv.org/abs/2602.05052)
...
### [42] [Quantile-Physics Hybrid Framework for Safe-Speed Recommendation under Diverse Weather Conditions Leveraging Connected Vehicle and Road Weather Information Systems Data](https://arxiv.org/abs/2602.05053)
...
### [43] [StagePilot: A Deep Reinforcement Learning Agent for Stage-Controlled Cybergrooming Simulation](https://arxiv.org/abs/2602.05060)
...
### [44] [Does SGD Seek Flatness or Sharpness? An Exactly Solvable Model](https://arxiv.org/abs/2602.05065)
...
### [45] [E-Globe: Scalable $ε$-Global Verification of Neural Networks via Tight Upper Bounds and Pattern-Aware Branching](https://arxiv.org/abs/2602.05068)
...
### [48] [Autodiscover: A reinforcement learning recommendation system for the cold-start imbalance challenge in active learning, powered by graph-aware thompson sampling](https://arxiv.org/abs/2602.05087)
*Parsa Vares*
...
### [49] [Unbiased Single-Queried Gradient for Combinatorial Objective](https://arxiv.org/abs/2602.05119)
*Thanawat Sornwanee*
...
### [50] [Rethinking Rubric Generation for Improving LLM Judge and Reward Modeling for Open-ended Tasks](https://arxiv.org/abs/2602.05125)
...
### [51] [SemPipes -- Optimizable Semantic Data Operators for Tabular Machine Learning Pipelines](https://arxiv.org/abs/2602.05134)
...
### [52] [Decoupled Orthogonal Dynamics: Regularization for Deep Network Optimizers](https://arxiv.org/abs/2602.05136)
...
### [53] [Adaptive Exploration for Latent-State Bandits](https://arxiv.org/abs/2602.05139)
...
### [54] [Fairness Under Group-Conditional Prior Probability Shift: Invariance, Drift, and Target-Aware Post-Processing](https://arxiv.org/abs/2602.05144)
...
### [55] [TIDE: Temporal Incremental Draft Engine for Self-Improving LLM Inference](https://arxiv.org/abs/2602.05145)
...
### [56] [Cross-talk based multi-task learning for fault classification of physically coupled machine system](https://arxiv.org/abs/2602.05146)
...
### [57] [CoSA: Compressed Sensing-Based Adaptation of Large Language Models](https://arxiv.org/abs/2602.05148)
...
### [58] [Position: Capability Control Should be a Separate Goal From Alignment](https://arxiv.org/abs/2602.05164)
...
### [59] [EBPO: Empirical Bayes Shrinkage for Stabilizing Group-Relative Policy Optimization](https://arxiv.org/abs/2602.05165)
...
### [60] [Benchmarking Artificial Intelligence Models for Daily Coastal Hypoxia Forecasting](https://arxiv.org/abs/2602.05178)
...
### [61] [Data-Centric Interpretability for LLM-based Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2602.05183)
...
### [62] [SpectraKAN: Conditioning Spectral Operators](https://arxiv.org/abs/2602.05187)
...
### [63] [Double-P: Hierarchical Top-P Sparse Attention for Long-Context LLMs](https://arxiv.org/abs/2602.05191)
...
### [64] [Extreme Weather Nowcasting via Local Precipitation Pattern Prediction](https://arxiv.org/abs/2602.05204)
...
### [65] [Disentangled Representation Learning via Flow Matching](https://arxiv.org/abs/2602.05214)
...
### [66] [Private Prediction via Shrinkage](https://arxiv.org/abs/2602.05219)
*Chao Yan*
...
### [67] [ZeroS: Zero-Sum Linear Attention for Efficient Transformers](https://arxiv.org/abs/2602.05230)
...
### [68] [Balanced Anomaly-guided Ego-graph Diffusion Model for Inductive Graph Anomaly Detection](https://arxiv.org/abs/2602.05232)
...
### [69] [Faithful Bi-Directional Model Steering via Distribution Matching and Distributed Interchange Interventions](https://arxiv.org/abs/2602.05234)
...
### [70] [CORP: Closed-Form One-shot Representation-Preserving Structured Pruning for Vision Transformers](https://arxiv.org/abs/2602.05243)
...
### [71] [TADS: Task-Aware Data Selection for Multi-Task Multimodal Pre-Training](https://arxiv.org/abs/2602.05251)
...
### [72] [Hybrid Gated Flow (HGF): Stabilizing 1.58-bit LLMs via Selective Low-Rank Correction](https://arxiv.org/abs/2602.05269)
...
### [73] [Back to Basics: Revisiting Exploration in Reinforcement Learning for LLM Reasoning via Generative Probabilities](https://arxiv.org/abs/2602.05281)
...
### [74] [Robust Inference-Time Steering of Protein Diffusion Models via Embedding Optimization](https://arxiv.org/abs/2602.05285)
...
### [75] [HealthMamba: An Uncertainty-aware Spatiotemporal Graph State Space Model for Effective and Reliable Healthcare Facility Visit Prediction](https://arxiv.org/abs/2602.05286)
...
### [76] [A Short and Unified Convergence Analysis of the SAG, SAGA, and IAG Algorithms](https://arxiv.org/abs/2602.05304)
...
### [78] [Accelerated Sequential Flow Matching: A Bayesian Filtering Perspective](https://arxiv.org/abs/2602.05319)
...
### [79] [GAS: Enhancing Reward-Cost Balance of Generative Model-assisted Offline Safe RL](https://arxiv.org/abs/2602.05323)
...
### [80] [Pool-based Active Learning as Noisy Lossy Compression: Characterizing Label Complexity via Finite Blocklength Analysis](https://arxiv.org/abs/2602.05333)
...
### [81] [Smoothness Errors in Dynamics Models and How to Avoid Them](https://arxiv.org/abs/2602.05352)
...
### [82] [Bayesian Neighborhood Adaptation for Graph Neural Networks](https://arxiv.org/abs/2602.05358)
...
### [83] [Hinge Regression Tree: A Newton Method for Oblique Regression Tree Splitting](https://arxiv.org/abs/2602.05371)
...
### [84] [Erase at the Core: Representation Unlearning for Machine Unlearning](https://arxiv.org/abs/2602.05375)
...
### [92] [When Shared Knowledge Hurts: Spectral Over-Accumulation in Model Merging](https://arxiv.org/abs/2602.05536)
...
### [93] [Steering Large Reasoning Models towards Concise Reasoning via Flow Matching](https://arxiv.org/abs/2602.05539)
...
### [94] [Unveiling Implicit Advantage Symmetry: Why GRPO Struggles with Exploration and Difficulty Adaptation](https://arxiv.org/abs/2602.05548)
...
### [95] [Logical Guidance for the Exact Composition of Diffusion Models](https://arxiv.org/abs/2602.05549)
...
### [96] [MAGPrompt: Message-Adaptive Graph Prompt Tuning for Graph Neural Networks](https://arxiv.org/abs/2602.05567)
...
### [97] [EdgeMask-DG*: Learning Domain-Invariant Graph Structures via Adversarial Edge Masking](https://arxiv.org/abs/2602.05571)
...
### [98] [OpenMAG: A Comprehensive Benchmark for Multimodal-Attributed Graph](https://arxiv.org/abs/2602.05576)
...
### [99] [On the Superlinear Relationship between SGD Noise Covariance and Loss Landscape Curvature](https://arxiv.org/abs/2602.05600)
...
### [100] [Shiva-DiT: Residual-Based Differentiable Top-$k$ Selection for Efficient Diffusion Transformers](https://arxiv.org/abs/2602.05605)
...
### [101] [Path-Guided Flow Matching for Dataset Distillation](https://arxiv.org/abs/2602.05616)
...
### [102] [Mode-Dependent Rectification for Stable PPO Training](https://arxiv.org/abs/2602.05619)
...
### [103] [Rewards as Labels: Revisiting RLVR from a Classification Perspective](https://arxiv.org/abs/2602.05630)
...
### [104] [Structural Disentanglement in Bilinear MLPs via Architectural Inductive Bias](https://arxiv.org/abs/2602.05635)
...
### [105] [Joint Embedding Variational Bayes](https://arxiv.org/abs/2602.05639)
...
### [106] [Empowering Time Series Analysis with Large-Scale Multimodal Pretraining](https://arxiv.org/abs/2602.05646)
...
### [107] [End-to-End Compression for Tabular Foundation Models](https://arxiv.org/abs/2602.05649)
...
### [108] [Alignment Verifiability in Large Language Models: Normative Indistinguishability under Behavioral Evaluation](https://arxiv.org/abs/2602.05656)
*Igor Santos-Grueiro*
...
### [109] [Tight Long-Term Tail Decay of (Clipped) SGD in Non-Convex Optimization](https://arxiv.org/abs/2602.05657)
...
### [110] [Probabilistic Multi-Regional Solar Power Forecasting with Any-Quantile Recurrent Neural Networks](https://arxiv.org/abs/2602.05660)
...
### [114] [Mining Generalizable Activation Functions](https://arxiv.org/abs/2602.05688)
...
### [115] [Almost Asymptotically Optimal Active Clustering Through Pairwise Observations](https://arxiv.org/abs/2602.05690)
...
### [116] [FedRandom: Sampling Consistent and Accurate Contribution Values in Federated Learning](https://arxiv.org/abs/2602.05693)
...
### [117] [Limitations of SGD for Multi-Index Models Beyond Statistical Queries](https://arxiv.org/abs/2602.05704)
...
### [118] [Fix Representation (Optimally) Before Fairness: Finite-Sample Shrinkage Population Correction and the True Price of Fairness Under Subpopulation Shift](https://arxiv.org/abs/2602.05707)
*Amir Asiaee,Kaveh Aryan*
...
### [119] [Projected Boosting with Fairness Constraints: Quantifying the Cost of Fair Training Distributions](https://arxiv.org/abs/2602.05713)
*Amir Asiaee,Kaveh Aryan*
...
### [120] [Muon in Associative Memory Learning: Training Dynamics and Scaling Laws](https://arxiv.org/abs/2602.05725)
...
### [121] [CSRv2: Unlocking Ultra-Sparse Embeddings](https://arxiv.org/abs/2602.05735)
...
### [122] [Learning to Inject: Automated Prompt Injection via Reinforcement Learning](https://arxiv.org/abs/2602.05746)
...
### [123] [How to Achieve the Intended Aim of Deep Clustering Now, without Deep Learning](https://arxiv.org/abs/2602.05749)
...
### [124] [Variational Speculative Decoding: Rethinking Draft Training from Token Likelihood to Sequence Acceptance](https://arxiv.org/abs/2602.05774)
...
### [125] [Cross-Domain Offline Policy Adaptation via Selective Transition Correction](https://arxiv.org/abs/2602.05776)
...
### [126] [How Controlling the Variance can Improve Training Stability of Sparsely Activated DNNs and CNNs](https://arxiv.org/abs/2602.05779)
...
### [127] [Distributional Reinforcement Learning with Diffusion Bridge Critics](https://arxiv.org/abs/2602.05783)
...
### [128] [Classification Under Local Differential Privacy with Model Reversal and Model Averaging](https://arxiv.org/abs/2602.05797)
...
### [129] [DLM-Scope: Mechanistic Interpretability of Diffusion Language Models via Sparse Autoencoders](https://arxiv.org/abs/2602.05859)
...
### [130] [Bifrost: Steering Strategic Trajectories to Bridge Contextual Gaps for Self-Improving Agents](https://arxiv.org/abs/2602.05810)
...
### [131] [Dr. Kernel: Reinforcement Learning Done Right for Triton Kernel Generations](https://arxiv.org/abs/2602.05885)
...
### [133] [Parity, Sensitivity, and Transformers](https://arxiv.org/abs/2602.05896)
...
### [134] [Where Does Warm-Up Come From? Adaptive Scheduling for Norm-Constrained Optimizers](https://arxiv.org/abs/2602.05813)
...
### [135] [Regularized Calibration with Successive Rounding for Post-Training Quantization](https://arxiv.org/abs/2602.05902)
...
### [136] [Synthesizing Realistic Test Data without Breaking Privacy](https://arxiv.org/abs/2602.05833)
...
### [137] [Verification of the Implicit World Model in a Generative Model via Adversarial Sequences](https://arxiv.org/abs/2602.05903)
...
### [138] [Visualizing the loss landscapes of physics-informed neural networks](https://arxiv.org/abs/2602.05849)
...
### [139] [Inverse Depth Scaling From Most Layers Being Similar](https://arxiv.org/abs/2602.05970)
...
### [140] [Exact Recovery in the Data Block Model](https://arxiv.org/abs/2602.05852)
...
### [141] [Clifford Kolmogorov-Arnold Networks](https://arxiv.org/abs/2602.05977)
...
### [143] [CFRecs: Counterfactual Recommendations on Real Estate User Listing Interaction Graphs](https://arxiv.org/abs/2602.05861)
...
### [146] [Correctness-Optimized Residual Activation Lens (CORAL): Transferrable and Calibration-Aware Inference-Time Steering](https://arxiv.org/abs/2602.06022)
...
### [147] [Large-scale Score-based Variational Posterior Inference for Bayesian Deep Neural Networks](https://arxiv.org/abs/2602.05873)
*Minyoung Kim*
...
### [148] [Shared LoRA Subspaces for almost Strict Continual Learning](https://arxiv.org/abs/2602.06043)
...
### [149] [Escaping Local Minima Provably in Non-convex Matrix Sensing: A Deterministic Framework via Simulated Lifting](https://arxiv.org/abs/2602.05887)
...
### [150] [DFPO: Scaling Value Modeling via Distributional Flow towards Robust and Generalizable LLM Post-Training](https://arxiv.org/abs/2602.05890)
...
### [151] [ContextBench: A Benchmark for Context Retrieval in Coding Agents](https://arxiv.org/abs/2602.05892)
...
### [152] [Chunky Post-Training: Data Driven Failures of Generalization](https://arxiv.org/abs/2602.05910)
...
### [153] [Approximation of Log-Partition Function in Policy Mirror Descent Induces Implicit Regularization for LLM Post-Training](https://arxiv.org/abs/2602.05933)
...
### [154] [Tuning Out-of-Distribution (OOD) Detectors Without Given OOD Data](https://arxiv.org/abs/2602.05935)
...
### [155] [Dimensionality Reduction on Riemannian Manifolds in Data Analysis](https://arxiv.org/abs/2602.05936)
...
### [156] [Orthogonal Model Merging](https://arxiv.org/abs/2602.05943)
...
### [157] [$f$-GRPO and Beyond: Divergence-Based Reinforcement Learning Algorithms for General LLM Alignment](https://arxiv.org/abs/2602.05946)
...
### [158] [Breaking Symmetry Bottlenecks in GNN Readouts](https://arxiv.org/abs/2602.05950)
...
### [159] [Discrete diffusion samplers and bridges: Off-policy algorithms and applications in latent spaces](https://arxiv.org/abs/2602.05961)
...
### [160] [A Hybrid Data-Driven Algorithm for Real-Time Friction Force Estimation in Hydraulic Cylinders](https://arxiv.org/abs/2602.05967)
...
### [161] [Layer-wise LoRA fine-tuning: a similarity metric approach](https://arxiv.org/abs/2602.05988)
...
### [162] [Orthogonal Self-Attention](https://arxiv.org/abs/2602.05996)
...
### [163] [On Computation and Reinforcement Learning](https://arxiv.org/abs/2602.05999)
...
### [164] [Curiosity is Knowledge: Self-Consistent Learning and No-Regret Optimization with Active Inference](https://arxiv.org/abs/2602.06029)
...
### [165] [AP-OOD: Attention Pooling for Out-of-Distribution Detection](https://arxiv.org/abs/2602.06031)
...
### [166] [Can vision language models learn intuitive physics from interaction?](https://arxiv.org/abs/2602.06033)
...
### [167] [Pseudo-Invertible Neural Networks](https://arxiv.org/abs/2602.06042)
...
### [168] [Artificial Intelligence as Strange Intelligence: Against Linear Models of Intelligence](https://arxiv.org/abs/2602.04986)
*Kendra Chilson,Eric Schwitzgebel*

Main category: cs.AI

TL;DR: 论文批评了AI进步的线性模型，提出非线性智能模型，认为AI智能是“陌生智能”，其能力无法用单一标准衡量。


<details>
  <summary>Details</summary>
Motivation: 论文的动机是回应Susan Schneider对AI进步线性模型的批评，并引入“熟悉智能”和“陌生智能”两个新概念，以更准确地描述AI智能的特性。

Method: 论文提出并发展了一种非线性智能模型，认为“通用智能”是能够在广泛环境中实现广泛目标的能力，而非单一线性能力。

Result: 研究结果表明，AI智能更可能是“陌生智能”，即在某些领域表现出超人类能力，而在其他领域表现低于人类水平，甚至在同一领域内也可能出现超人类洞察力与人类罕见错误的结合。

Conclusion: 论文得出结论，AI的智能是非线性的，不应通过单一线性标准（如IQ测试）来评估其广泛能力。即使是最强大的AI系统也可能在某些看似简单的任务上失败，但这并不否定其出色的通用智能。
...
### [169] [DeepRead: Document Structure-Aware Reasoning to Enhance Agentic Search](https://arxiv.org/abs/2602.05014)
*Zhanli Li,Huiwen Tian,Lvzhou Luo,Yixuan Cao,Ping Luo*

Main category: cs.AI

TL;DR: DeepRead enhances document QA by structuring documents and using retrieval-reading tools, outperforming traditional methods.


<details>
  <summary>Details</summary>
Motivation: Existing agentic search frameworks underutilize document-native priors like hierarchical organization, leading to suboptimal performance in long-document question answering.

Method: DeepRead uses LLM-based OCR to convert PDFs into structured Markdown, indexes documents at the paragraph level with metadata keys, and equips the LLM with Retrieve and ReadSection tools for localized and contiguous reading.

Result: DeepRead outperforms Search-o1-style agentic search, demonstrating the effectiveness of structure-aware retrieval and reading tools.

Conclusion: DeepRead significantly improves document question answering by leveraging structured document representation and synergistic retrieval-reading tools, resembling human-like reasoning.
...
### [170] [MINT: Minimal Information Neuro-Symbolic Tree for Objective-Driven Knowledge-Gap Reasoning and Active Elicitation](https://arxiv.org/abs/2602.05048)
*Zeyu Fang,Tian Lan,Mahdi Imani*

Main category: cs.AI

TL;DR: MINT optimizes AI-human planning by actively eliciting inputs, achieving near-expert performance with minimal queries.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of incomplete information and unknowns in joint human-AI planning by developing strategies to actively elicit human inputs.

Method: Proposes Minimal Information Neuro-Symbolic Tree (MINT) to reason about knowledge gaps and optimize AI agent's elicitation strategies through self-play and LLM-curated queries.

Result: MINT-based planning shows improved performance on benchmarks with unseen/unknown objects, achieving near-expert returns with limited questions.

Conclusion: MINT-based planning achieves near-expert returns by actively eliciting human inputs, significantly improving rewards and success rates with limited queries.
...
### [171] [Evaluating Large Language Models on Solved and Unsolved Problems in Graph Theory: Implications for Computing Education](https://arxiv.org/abs/2602.05059)
*Adithya Kulkarni,Mohna Chakraborty,Jay Bagga*

Main category: cs.AI

TL;DR: LLMs are reliable for exploring established graph theory problems but limited in solving open problems, highlighting the need for guided educational use.


<details>
  <summary>Details</summary>
Motivation: To understand how reliably LLMs support mathematically rigorous thinking as they become integrated into computer science education.

Method: An eight-stage evaluation protocol reflecting authentic mathematical inquiry, including interpretation, exploration, strategy formation, and proof construction, was applied to assess LLM performance on solved and open graph theory problems.

Result: The LLM performed well on the solved problem, producing correct definitions and proofs, but only generated coherent interpretations and plausible strategies for the open problem without advancing toward a solution.

Conclusion: LLMs can effectively support exploration of established material but are limited in tasks requiring novel mathematical insight or critical structural reasoning, emphasizing the need for guided use in education.
...
### [172] [Towards Reducible Uncertainty Modeling for Reliable Large Language Model Agents](https://arxiv.org/abs/2602.05073)
...
TL;DR: The paper argues for shifting UQ research to interactive LLM agents, proposes a new framework treating UQ as a conditional uncertainty reduction process, and outlines practical implications for LLM development.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the gap in UQ research, which primarily focuses on single-turn question-answering despite the increasing deployment of LLM agents in complex, interactive tasks.

Method: The paper presents a general formulation of agent UQ, critiques prior works' implicit treatment of LLM UQ as an uncertainty accumulation process, and proposes a novel perspective of conditional uncertainty reduction.

Result: The result is a conceptual framework for designing UQ in LLM agent setups, emphasizing the 'interactivity' of actions and reducible uncertainty over an agent's trajectory.

Conclusion: The paper concludes with practical implications of agent UQ in frontier LLM development and domain-specific applications, highlighting open remaining problems.
...
TL;DR: A reinforcement learning framework enhances adaptive collision avoidance in active debris removal missions, using small satellites for efficiency and safety.


<details>
  <summary>Details</summary>
Motivation: As the orbital environment around Earth becomes increasingly crowded with debris, active debris removal (ADR) missions face significant challenges in ensuring safe operations while minimizing the risk of in-orbit collisions.

Method: The proposed approach employs a masked Proximal Policy Optimization (PPO) algorithm, enabling the RL agent to dynamically adjust maneuvers in response to real-time orbital conditions.

Result: Results show that the proposed RL framework reduces collision risk while improving mission efficiency compared to traditional heuristic approaches.
...
TL;DR: The study validated VERA-MH as a reliable AI safety benchmark for mental health, showing strong alignment between clinician and LLM judge ratings, with realistic user-agent simulations.


<details>
  <summary>Details</summary>
Motivation: The increasing use of generative AI chatbots for psychological support raises concerns about their safety, necessitating an evidence-based automated safety benchmark like VERA-MH.

Method: The study involved simulating conversations between LLM-based user-agents and AI chatbots, with licensed clinicians and an LLM judge independently rating the conversations for safety and realism. Rating alignment was compared between individual clinicians, clinician consensus, and the LLM judge.

Result: Clinicians showed high consistency in safety ratings (IRR: 0.77), and the LLM judge aligned well with clinical consensus (IRR: 0.81). User-agents were generally perceived as realistic.

Conclusion: The study supports the clinical validity and reliability of the VERA-MH evaluation, highlighting its potential as an automated AI safety benchmark for mental health applications. Further research is needed to assess its generalizability and robustness.
...
TL;DR: The paper compares three ADR mission planners, finding trade-offs between speed and adaptability, suggesting combined approaches for future planners.


<details>
  <summary>Details</summary>
Motivation: Autonomous mission planning for Active Debris Removal (ADR) requires balancing efficiency, adaptability, and strict feasibility constraints on fuel and mission duration.

Method: The study compares three planners: a nominal Masked PPO policy, a domain-randomized Masked PPO policy, and a plain Monte Carlo Tree Search (MCTS) baseline, evaluated in a high-fidelity orbital simulation with refueling and realistic transfer dynamics across 300 test cases.

Result: Nominal PPO performs best under matching conditions but degrades under distributional shift; domain-randomized PPO shows improved adaptability with moderate nominal performance loss; MCTS handles constraint changes best but with higher computation time.

Conclusion: The findings highlight a trade-off between the speed of learned policies and the adaptability of search-based methods, suggesting that combining training-time diversity with online planning could enhance future resilient ADR mission planners.
...
### [176] [Understanding LLM Evaluator Behavior: A Structured Multi-Evaluator Framework for Merchant Risk Assessment](https://arxiv.org/abs/2602.05110)
*Liang Wang,Junpeng Wang,Chin-chia Michael Yeh,Yan Zheng,Jiarui Sun,Xiran Fan,Xin Dai,Yujie Fan,Yiwei Cai*

Main category: cs.AI

TL;DR: A multi-evaluator framework assesses LLM reliability in payment-risk settings, revealing model biases and alignment with human judgment, validated by expert and ground-truth data.


<details>
  <summary>Details</summary>
Motivation: To address the poorly understood reliability and bias of LLMs in payments-risk settings, particularly in Merchant Category Code (MCC)-based merchant risk assessment.

Method: A structured multi-evaluator framework combining a five-criterion rubric with Monte-Carlo scoring to evaluate rationale quality and evaluator stability, including consensus-deviation metrics for judge-independent reference.

Result: Substantial heterogeneity in LLM biases was found, with some models showing negative or positive self-evaluation bias. Anonymization reduced bias by 25.8%. LLM judges scored +0.46 above human consensus, with certain models aligning closely with human judgment. Ground-truth validation confirmed the framework's effectiveness.

Conclusion: The framework offers a replicable method for assessing LLM-as-a-judge systems in payment-risk workflows and underscores the importance of bias-aware protocols in financial settings.
...
### [177] [Democratic Preference Alignment via Sortition-Weighted RLHF](https://arxiv.org/abs/2602.05113)
*Suvadip Sana,Jinzhou Wu,Martin T. Wells*

Main category: cs.AI

TL;DR: DemPO框架通过算法抽签机制确保AI系统偏好学习的代表性，硬面板和软面板训练方案均优于未加权基线。


<details>
  <summary>Details</summary>
Motivation: 解决现有偏好对齐方法中人类评分者样本代表性不足的问题。

Method: 引入Democratic Preference Optimization (DemPO)框架，采用硬面板和软面板两种训练方案。

Result: 硬面板表现最佳，软面板优于未加权基线，模型容量越大效果越显著。

Conclusion: 在偏好收集阶段确保人口统计学代表性，能更好地反映代表性公众的价值观。
...
### [178] [SocialVeil: Probing Social Intelligence of Language Agents under Communication Barriers](https://arxiv.org/abs/2602.05115)
*Keyang Xuan,Pengda Wang,Chongrui Ye,Haofei Yu,Tal August,Jiaxuan You*

Main category: cs.AI

TL;DR: The paper introduces 	extsc{SocialVeil}, a tool to test LLMs in realistic social interactions with communication barriers, showing significant performance drops and limited success with adaptation strategies.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the limitations of existing benchmarks that assume idealized communication, aiming to better evaluate LLMs' ability to handle realistic, imperfect social interactions.

Method: The study introduces 	extsc{SocialVeil}, a social learning environment simulating three types of communication barriers (semantic vagueness, sociocultural mismatch, and emotional interference), and uses two metrics (unresolved confusion and mutual understanding) to evaluate interaction quality. Experiments involved 720 scenarios across four LLMs.

Result: Results show that communication barriers significantly impair LLM performance, reducing mutual understanding by over 45% and increasing confusion by nearly 50%. Human evaluations confirmed the fidelity of the simulated barriers. Adaptation strategies had limited effectiveness.

Conclusion: The paper concludes that existing benchmarks for LLMs are limited in simulating realistic communication barriers, and 	extsc{SocialVeil} provides a more effective environment to evaluate and improve LLMs' social intelligence under cognitive-difference-induced disruptions.
...
### [179] [CAST-CKT: Chaos-Aware Spatio-Temporal and Cross-City Knowledge Transfer for Traffic Flow Prediction](https://arxiv.org/abs/2602.05133)
*Abdul Joseph Fofanah,Lian Wen,David Chen,Alpha Alimamy Kamara,Zhongyi Zhang*

Main category: cs.AI

TL;DR: CAST-CKT is a chaos-aware framework for cross-city traffic prediction, improving accuracy and interpretability in few-shot settings.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenges of traffic prediction in data-scarce, cross-city settings due to complex nonlinear dynamics and domain shifts.

Method: The framework employs a chaos-aware analyser to quantify traffic predictability regimes, chaos-aware attention for temporal modelling, adaptive topology learning for spatial dependencies, and chaotic consistency-based cross-city alignment for knowledge transfer.

Result: Extensive experiments show CAST-CKT outperforms existing methods in MAE and RMSE metrics.

Conclusion: CAST-CKT significantly outperforms state-of-the-art methods in cross-city few-shot traffic prediction, providing interpretable regime analysis and uncertainty quantification.
...
### [180] [HugRAG: Hierarchical Causal Knowledge Graph Design for RAG](https://arxiv.org/abs/2602.05143)
...
### [181] [First Proof](https://arxiv.org/abs/2602.05192)
...
### [182] [Traceable Cross-Source RAG for Chinese Tibetan Medicine Question Answering](https://arxiv.org/abs/2602.05195)
...
### [183] [Surgery: Mitigating Harmful Fine-Tuning for Large Language Models via Attention Sink](https://arxiv.org/abs/2602.05228)
...
### [184] [Explainable AI: A Combined XAI Framework for Explaining Brain Tumour Detection Models](https://arxiv.org/abs/2602.05240)
...
### [185] [Automatic Cognitive Task Generation for In-Situ Evaluation of Embodied Agents](https://arxiv.org/abs/2602.05249)
...
### [186] [Beyond Cosine Similarity](https://arxiv.org/abs/2602.05266)
*Xinbo Ai*
...
### [187] [Hallucination-Resistant Security Planning with a Large Language Model](https://arxiv.org/abs/2602.05279)
...
### [188] [Position: Universal Time Series Foundation Models Rest on a Category Error](https://arxiv.org/abs/2602.05287)
...
### [189] [Aspect-Aware MOOC Recommendation in a Heterogeneous Network](https://arxiv.org/abs/2602.05297)
...
### [190] [PieArena: Frontier Language Agents Achieve MBA-Level Negotiation Performance and Reveal Novel Behavioral Differences](https://arxiv.org/abs/2602.05302)
...
### [191] [ProAct: Agentic Lookahead in Interactive Environments](https://arxiv.org/abs/2602.05327)
...
### [192] [AgentXRay: White-Boxing Agentic Systems via Workflow Reconstruction](https://arxiv.org/abs/2602.05353)
...
### [193] [PATHWAYS: Evaluating Investigation and Context Discovery in AI Web Agents](https://arxiv.org/abs/2602.05354)
...
### [194] [RaBiT: Residual-Aware Binarization Training for Accurate and Efficient LLMs](https://arxiv.org/abs/2602.05367)
...
### [195] [Clinical Validation of Medical-based Large Language Model Chatbots on Ophthalmic Patient Queries with LLM-based Evaluation](https://arxiv.org/abs/2602.05381)
...
### [196] [Advancing Opinion Dynamics Modeling with Neural Diffusion-Convection-Reaction Equation](https://arxiv.org/abs/2602.05403)
...
### [197] [H-AdminSim: A Multi-Agent Simulator for Realistic Hospital Administrative Workflows with FHIR Integration](https://arxiv.org/abs/2602.05407)
...
### [198] [THOR: Inductive Link Prediction over Hyper-Relational Knowledge Graphs](https://arxiv.org/abs/2602.05424)
...
### [199] [M$^2$-Miner: Multi-Agent Enhanced MCTS for Mobile GUI Agent Data Mining](https://arxiv.org/abs/2602.05429)
...
### [200] [Day-Ahead Electricity Price Forecasting for Volatile Markets Using Foundation Models with Regularization Strategy](https://arxiv.org/abs/2602.05430)
...
### [201] [Refine and Purify: Orthogonal Basis Optimization with Null-Space Denoising for Conditional Representation Learning](https://arxiv.org/abs/2602.05464)
...
### [202] [ALIVE: Awakening LLM Reasoning via Adversarial Learning and Instructive Verbal Evaluation](https://arxiv.org/abs/2602.05472)
...
### [203] [Phi-Former: A Pairwise Hierarchical Approach for Compound-Protein Interactions Prediction](https://arxiv.org/abs/2602.05479)
...
### [204] [SDFP: Speculative Decoding with FIT-Pruned Models for Training-Free and Plug-and-Play LLM Acceleration](https://arxiv.org/abs/2602.05499)
...
### [205] [A Unified Multimodal Framework for Dataset Construction and Model-Based Diagnosis of Ameloblastoma](https://arxiv.org/abs/2602.05515)
...
### [206] [Split Personality Training: Revealing Latent Knowledge Through Alternate Personalities](https://arxiv.org/abs/2602.05532)
...
### [207] [Conditional Diffusion Guidance under Hard Constraint: A Stochastic Analysis Approach](https://arxiv.org/abs/2602.05533)
...
### [208] [Reasoning-guided Collaborative Filtering with Language Models for Explainable Recommendation](https://arxiv.org/abs/2602.05544)
...
### [209] [TangramSR: Can Vision-Language Models Reason in Continuous Geometric Space?](https://arxiv.org/abs/2602.05570)
...
### [210] [Emulating Aggregate Human Choice Behavior and Biases with GPT Conversational Agents](https://arxiv.org/abs/2602.05597)
...
### [211] [BhashaSetu: Cross-Lingual Knowledge Transfer from High-Resource to Extreme Low-Resource Languages](https://arxiv.org/abs/2602.05599)
...
### [212] [Reactive Knowledge Representation and Asynchronous Reasoning](https://arxiv.org/abs/2602.05625)
...
### [213] [Generative Ontology: When Structured Knowledge Learns to Create](https://arxiv.org/abs/2602.05636)
*Benny Cheung*
...
### [214] [Graph-based Agent Memory: Taxonomy, Techniques, and Applications](https://arxiv.org/abs/2602.05665)
...
### [215] [Determining Energy Efficiency Sweet Spots in Production LLM Inference](https://arxiv.org/abs/2602.05695)
...
### [216] [Nonlinearity as Rank: Generative Low-Rank Adapter with Radial Basis Functions](https://arxiv.org/abs/2602.05709)
...
### [217] [Anchored Policy Optimization: Mitigating Exploration Collapse Via Support-Constrained Rectification](https://arxiv.org/abs/2602.05717)
...
### [218] [Mitigating Hallucination in Financial Retrieval-Augmented Generation via Fine-Grained Knowledge Verification](https://arxiv.org/abs/2602.05723)
...
### [219] [LeakBoost: Perceptual-Loss-Based Membership Inference Attack](https://arxiv.org/abs/2602.05748)
...
### [220] [RocqSmith: Can Automatic Optimization Forge Better Proof Agents?](https://arxiv.org/abs/2602.05762)
...
### [221] [RL-VLA$^3$: Reinforcement Learning VLA Accelerating via Full Asynchronism](https://arxiv.org/abs/2602.05765)
...
### [222] [FiMI: A Domain-Specific Language Model for Indian Finance Ecosystem](https://arxiv.org/abs/2602.05794)
...
### [223] [NEX: Neuron Explore-Exploit Scoring for Label-Free Chain-of-Thought Selection and Model Ranking](https://arxiv.org/abs/2602.05805)
...
### [224] [STProtein: predicting spatial protein expression from multi-omics data](https://arxiv.org/abs/2602.05811)
...
### [225] [TKG-Thinker: Towards Dynamic Reasoning over Temporal Knowledge Graphs via Agentic Reinforcement Learning](https://arxiv.org/abs/2602.05818)
