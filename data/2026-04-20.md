...
TL;DR: 通过对11个大语言模型的谱分析发现：推理任务相比事实回忆在隐藏激活空间中产生系统性谱压缩现象，且这种现象呈现指令调优依赖、架构依赖和尺度律特性，还能在答案生成前预测正确性，为理解语言模型推理机制提供了全新光谱理论。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在探索大型语言模型在推理任务和事实回忆任务中，其内部隐藏激活空间的谱特性是否存在系统性差异。通过揭示这种差异的普遍性和具体表现，研究试图建立一个统一的“推理光谱理论”，从而深入理解语言模型执行不同认知任务时的内部工作机制。

Method: 通过对11个模型（涵盖5种架构家族）进行系统的谱分析，研究采用以下方法：1）谱指数α计算与对比分析；2）指令调优影响的谱反转检测；3）架构依赖的生成模式分类；4）谱尺度律拟合；5）令牌级谱级联跟踪；6）推理步骤边界的谱标点识别；7）光谱正确性预测分析（包括AUC指标评估）。

Result: 研究发现了七个核心现象：1）推理谱压缩：9/11模型在推理任务中α值显著降低；2）指令调优谱反转：基础模型与指令调优模型的推理-事实α关系呈现相反趋势；3）架构依赖生成分类法：提示到响应的转换分为扩展、压缩和平衡三种机制；4）谱尺度律：推理α值与模型大小呈现对数关系；5）令牌级谱级联：局部同步性随层距离呈指数衰减；6）推理步骤谱标点：相位转换信号与推理步骤边界对齐；7）光谱正确性预测：单凭光谱α值即可实现高准确度的答案正确性预测（最高AUC=1.000）。

Conclusion: 本研究发现大型语言模型在推理与事实回忆之间存在普遍的相位转换几何模式，这种“推理的光谱理论”揭示了思维几何具有方向普适性、动力学架构特异性、以及结果可预测性等特点，为深入理解语言模型的内部工作机制提供了新的理论基础和分析框架。
...
TL;DR: 提出顺序KV压缩，利用语言模型的结构性特性，通过概率前缀去重和预测性增量编码，实现比现有方法高数个数量级的KV缓存压缩比。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存量化方法（如TurboQuant）接近每个向量分量的香农熵极限，但这是对较弱问题的解决方案。KV缓存中的令牌是模型训练过的正式语言的样本，模型本身是该语言的近乎最优预测器，可以利用这一结构实现更高效的序列压缩。

Method: 两层架构：1）概率前缀去重，使用PLT的trie度量识别语义等效的前缀；2）预测性增量编码，只存储KV向量与模型自身预测之间的残差。该方法与TurboQuant等现有量化方法正交且可组合。

Result: 理论压缩比相对于TurboQuant约为914,000倍（在香农极限下）。即使在比熵底高出1000倍的悲观最坏情况下，压缩比仍为TurboQuant的约914倍。压缩性能随上下文长度增加而改善而非降低。每个令牌位置的平均熵界为3.3-4.3位，远优于TurboQuant的每个向量分量3位（典型注意力头有64-128个分量）。

Conclusion: 顺序KV压缩通过结合概率前缀去重和预测性增量编码，显著超越现有KV缓存量化方法的压缩极限，提供可扩展的高效压缩解决方案，并与现有量化方法兼容。
...
TL;DR: 通过三阶段MPC分析储能运行，发现存在有效预测时域，超过额外信息无显著效益。系统研究得到不同参数下的最优时域，可在降低计算成本的同时维持性能，为工业运营提供实用指南。


<details>
  <summary>Details</summary>
Motivation: 工业储能系统中，多阶段模型预测控制的计算成本通常很高，特别是在处理较长预测时域时。然而，并非所有预测时域长度都能带来显著的操作效益。本研究旨在确定在考虑数据特征、预测不确定性和电池特性的情况下，何时增加预测时域长度不再改善性能，从而降低计算成本。这有助于解决工业储能操作的效率和实用性挑战。

Method: 采用三阶段模型预测控制分析储能运行，通过合成数据集系统探索数据特征和预测不确定性的变化。通过参数化方法和关系构建，将这些特征映射到最优时域长度。研究中定义了有效时域概念，即在超过该范围后额外预测信息带来的操作效益有限。该方法考虑了多种电池类型、不确定性水平和数据特征的组合，并进行全面分析。

Result: 研究揭示了有效时域的存在，即超过特定长度后额外预测信息带来的操作效益有限。通过考虑有效时域，可在不影响最优性能的情况下减少计算成本60.3%。为多种电池类型、不确定性水平和数据特征组合提供了最优时域长度参考表。量化了预测不确定性导致的收入损失，显示即使对快速电池，误差也可能降低性能约17.2%。开发了一个框架，可将数据集参数映射到最优时域长度，为机器学习方法奠定基础。

Conclusion: 研究提出了基于数据特征和预测不确定性的有效预测时域概念，这一概念可在保持最优性能的同时显著降低计算成本。通过系统分析揭示了不同运行条件下的最优时域长度，为工业储能运营提供了实用指导。同时，该框架为未来机器学习方法奠定了基础，支持工业场景下的持续优化而无需大量计算。尽管预测误差可能影响快速电池的性能，但通过考虑有效时域可大大减轻这种影响。
...
TL;DR: M3R：一种结合雷达图像和天气站测量的多模态注意力降水预测模型，显著提升了降水临近预报的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 降雨临近预报对防灾减灾和水资源管理至关重要，现有深度学习方法在有效利用多样化多媒体数据源方面存在局限性。

Method: Meteorology-informed MultiModal attention-based architecture for direct Rainfall prediction (M3R)，结合视觉NEXRAD雷达图像和数值个人天气站测量数据，采用专门的多模态注意力机制，将天气站时间序列作为查询，选择性地关注空间雷达特征，提取降水特征。

Result: M3R在三个100 km * 100 km空间区域的实验中优于现有方法，在准确性、效率和降水检测能力方面取得显著提升。

Conclusion: 本文提出了一种新的M3R架构，通过融合视觉雷达图像和数值天气站测量数据，显著提升了降水临近预报的准确性和效率，为实际业务天气预测系统提供了实用工具。
...
TL;DR: 研究发现语言模型幻觉是一种由早期轨迹承诺和不对称吸引子动力学主导的稳定吸引子盆地行为，幻觉的进入快速且概率性，纠正需要协调干预，关键决策在提示编码阶段已确定。


<details>
  <summary>Details</summary>
Motivation: 旨在理解自回归语言模型中幻觉现象的动力学机制，特别是幻觉的早期决策过程和不对称性特征。

Method: 采用相同提示分叉分析、激活修补（包括正向和反向修补）、窗口修补、步骤0残差状态分析和无监督聚类等方法，对Qwen2.5-1.5B模型进行了多角度的实验研究。

Result: 44.3%的提示出现分叉，事实内容与幻觉轨迹在第一个生成token处分离；激活修补发现因果不对称性：幻觉激活破坏正确轨迹的成功率为87.5%，而纠正幻觉的成功率仅为33.3%；步骤0残差状态能预测幻觉率（r = 0.776）；聚类分析识别出五个类似态簇，其中一个簇集中了12个分叉的虚假前提提示。

Conclusion: 语言模型中的幻觉是由早期轨迹承诺和不对称吸引子动力学主导的稳定吸引子盆地行为：幻觉的进入是概率性和快速的，而纠正则需要跨层和跨步骤的协调干预，吸引子盆地的结构在提示编码阶段就已确定。
...
TL;DR: 针对ViT剪枝后短序列的调度开销瓶颈，提出轻量级Triton注意力内核，调度开销仅为40微秒，使剪枝真正提升端到端性能，吞吐量最高提升2.24倍。


<details>
  <summary>Details</summary>
Motivation: 现有变长注意力API（如FlashAttention-2 varlen和PyTorch的NestedTensor SDPA）在执行ViT剪枝后的短序列时存在调度开销瓶颈：矩阵运算仅需个位数微秒，而主机端调度路径却消耗60-90微秒，导致实际延迟并未随FLOPs减少而相应降低。

Method: 提出一个轻量级的双向Triton注意力内核，集成到完整的打包-关注-解包流水线中，减少调度开销；通过实验评估了四种剪枝算法（Threshold-L2、DynamicViT、EViT、ATS）在不同模型规模下的表现。

Result: 在四种剪枝算法和DeiT-T/S/B模型上，该系统实现了高达2.24倍的端到端吞吐量提升，超过填充的PyTorch SDPA；同时保持比特精确的分类预测，最大绝对logit差异小于0.007。

Conclusion: 本文实现了一个轻量级的双向Triton注意力内核，其调度开销仅为40微秒，显著低于现有解决方案，使得剪枝在端到端性能上真正产生积极影响，实现了高达2.24倍的吞吐量提升，同时保持比特精确的分类预测。
...
TL;DR: 提出PRL-Bench物理研究基准，基于Physical Review Letters最新论文构建，系统评估LLMs在端到端物理研究中的能力边界，发现当前模型与真实科研需求存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前科学基准主要评估领域知识理解和复杂推理，但无法评价真实科学研究中的探索性本质和流程复杂性。需要建立能系统评估LLMs在执行端到端物理研究方面能力边界的基准。

Method: 从2025年8月以来的Physical Review Letters最新期刊中精选100篇论文构建基准，涵盖天体物理、凝聚态物理、高能物理、量子信息和统计物理五大计算密集型子领域。每个任务设计包含探索导向的公式化、长流程工作流和客观可验证性，再现真实科研的核心特性。

Result: 对前沿模型的评估显示性能仍然有限，最佳整体得分低于50%，揭示当前LLM能力与真实科学研究需求之间存在显著差距。

Conclusion: PRL-Bench是一个可靠的基准测试，用于评估下一代AI科学家，推动AI系统朝着自主科学发现的方向发展。当前前沿模型在评测中整体得分低于50%，揭示了LLM现有能力与实际科学研究需求之间的显著差距。
...
### [8] [StoSignSGD: Unbiased Structural Stochasticity Fixes SignSGD for Training Large Language Models](https://arxiv.org/abs/2604.15416)
*Dingzhi Yu,Rui Pan,Yuxing Liu,Tong Zhang*

Main category: cs.LG

TL;DR: 本文提出了StoSignSGD算法，通过在符号算子中注入结构性随机性解决了SignSGD在非光滑目标上的发散问题，在理论和实践中均表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 基于符号的优化算法（如SignSGD）在分布式学习和大模型训练中表现出色，但在现代机器学习中普遍存在的非光滑目标函数（如ReLU、max-pool等）上会发散，需要克服这一根本限制。

Method: 提出了StoSignSGD算法，通过在符号算子中注入结构性随机性同时保持更新步骤的无偏性，并开发了符号转换框架，可以将任何通用优化器转换为其无偏的符号化对应版本。

Result: 理论分析表明StoSignSGD在线性优化中解决了SignSGD的非收敛问题，达到了与下界匹配的尖锐收敛速率；在非凸非光滑优化中，改进了现有最好的复杂度界限。实证显示在FP8低精度预训练中获得1.44×到2.14×的加速，在7B LLM数学推理微调中性能显著优于AdamW和SignSGD。

Conclusion: StoSignSGD解决了SignSGD在非光滑目标上的发散问题，在非凸非光滑优化中取得了优于现有方法的复杂度界，并在大量语言模型训练中展现出卓越的稳定性和效率优势，为低精度训练等挑战性场景提供了有效的解决方案。
...
### [9] [Transfer Learning from Foundational Optimization Embeddings to Unsupervised SAT Representations](https://arxiv.org/abs/2604.15448)
*Koyena Pal,Serdar Kadioglu*

Main category: cs.LG

TL;DR: 研究将基础优化预训练嵌入从混合整数规划迁移到布尔可满足性问题，通过将SAT映射为相同二部图结构，无需修改模型即可实现有效迁移，支持无监督任务并展示了统一表示框架的潜力。


<details>
  <summary>Details</summary>
Motivation: 探索基础优化嵌入表示是否能够从混合整数规划（MIP）问题泛化到决策问题领域，特别是布尔可满足性（SAT）问题，以验证这些表示在更广泛的组合问题中的普适性。

Method: 将SAT问题的CNF公式映射为与MIP模型相同的约束-变量二部图表示，无需修改架构或监督微调，直接复用预训练的基础优化嵌入模型。通过这种方式实现从优化问题到决策问题领域的知识迁移。

Result: 基础优化嵌入能够捕捉SAT实例的结构规律性，支持实例聚类和分布识别等无监督任务。首次证明了基础优化嵌入可以迁移到约束满足领域，并显示出在二元图中的监督分类和实例难度预测方面的潜力。

Conclusion: 本文通过将SAT问题映射到与MIP相同的二部图表示结构，证明了基础优化嵌入可以在无监督和监督任务中学习到SAT实例的结构特征并有效泛化，为实现优化与决策问题的统一表示框架迈出了重要一步。
...
### [10] [Evaluating LLM Simulators as Differentially Private Data Generators](https://arxiv.org/abs/2604.15461)
*Nassima M. Bouzid,Dehao Yuan,Nam H. Nguyen,Mayana Pereira*

Main category: cs.LG

TL;DR: 即使基于大语言模型的模拟器能生成复杂的差分隐私合成数据，但模型偏见会导致统计分布漂移，限制了其在复杂用户表示中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 论文的动机是探索基于大语言模型的模拟器作为一个有前景的替代方案，用于在传统差分隐私方法难以处理高维用户画像的情况下生成复杂的合成数据，特别是验证大语言模型能否忠实复现来自差分隐私保护输入的统计分布。

Method: 论文的研究方法是通过构建PersonaLedger系统——一种基于大语言模型的金融模拟器，使用从真实用户统计中提取的差分隐私合成人设作为种子数据。然后系统地评估该模拟器是否能忠实地复现受差分隐私保护的输入数据的统计分布。

Result: 研究结果显示，PersonaLedger在差分隐私参数epsilon=1时达到了有希望的欺诈检测效用（AUC 0.70），但它也表现出显著的数据分布漂移问题。这种漂移主要源于大语言模型的系统性偏见——即其学习得到的先验知识会覆盖输入数据的时间序列特征和人口统计特征的原始统计信息。

Conclusion: 论文的结论是，尽管基于大语言模型的模拟器在隐私保护数据处理方面表现出一定的潜力，但在系统性偏见问题得到解决之前，它们还不能可靠地处理复杂的用户表示。当前大语言模型的固有偏向会显著扭曲输入数据的统计分布，这是该方法面临的主要挑战。
...
TL;DR: 提出一种多目标LLM遗忘框架，通过数据标准化和双向蒸馏协调遗忘效果、通用性、边界行为和对抗鲁棒性等多个目标，实现最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现有LLM遗忘方法主要关注遗忘效果和通用性保留的有限目标，忽视了对抗鲁棒性和边界行为等重要维度。简单扩展这些方法到多目标设置可能导致任务干扰，需要一种能够协调多个目标的统一框架。

Method: 采用数据和优化协同设计方法：首先将训练语料标准化为统一数据表示以减少领域差异，然后引入双向蒸馏方法，同时引导教师模型生成理想行为，并抑制学生模型中不期望的行为。

Result: 评估显示该方法在多样化挑战性需求下实现先进性能，能够平衡且可靠地完成遗忘任务。理论分析和实证结果表明，该方法能对齐领域分布，并将看似无关的遗忘任务转化为协同优化。

Conclusion: 本研究提出的多目标遗忘框架通过数据和优化协同设计，在移除有害信息、保留模型通用能力、避免过度拒绝相关概念、以及增强对抗鲁棒性等多项指标上达到最佳平衡，为LLM安全遗忘提供了新的解决方案。
...
TL;DR: π0.7是一个创新的机器人基础模型，通过多样化上下文条件训练实现了强大的零样本泛化和复杂任务执行能力


<details>
  <summary>Details</summary>
Motivation: 解决现有机器人模型需要针对特定任务进行专门训练、缺少零样本泛化能力的问题，构建一个能够在广泛场景中开箱即用、理解复杂指令并执行多阶段任务的通用机器人基础模型。

Method: 采用多样化上下文条件训练方法，通过在提示中包含语言指令、多模态信息（任务性能元数据、子目标图像等）来精确引导模型执行不同策略的任务。

Result: π0.7在多个机器人平台上展现了强大的性能：能够遵循未见环境中的多样语言指令（包括多阶段厨房任务），实现零样本跨具身泛化（如从未见过的叠衣服任务），以及在操作意式咖啡机等挑战性任务上达到与专门RL微调模型相当的水平。

Conclusion: 基于多样化上下文条件训练的π0.7模型为通用机器人控制铺平了道路，通过单一模型支持跨平台、跨场景的复杂任务执行，代表了机器人基础模型的重要进展。
...
TL;DR: FineSteer：一个新颖的推理时调控框架，通过两阶段机制（条件调控+细粒度向量合成）实现对大语言模型行为的细粒度控制，在保持模型效能的同时有效改善安全性和真实性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在推理时可能出现安全性违规和幻觉等不良行为。现有的推理时调控方法往往难以同时满足有效性、效用保持和训练效率的要求，因为它们通常采用一刀切的固定设计且适应性有限，无法智能地决定何时以及如何调控内部表示。

Method: FineSteer框架包含两个互补阶段：1) 通过子空间引导条件调控（SCS）机制避免不必要的调控以保持模型效用；2) 通过混合调控专家（MoSE）机制捕获期望调控行为的多模态特性，为特定查询生成细粒度的调控向量。

Result: 在安全和真实性基准上进行的大量实验表明，FineSteer在整体性能上超越了最先进的方法，在保持最小效用损失的同时实现了更强的调控性能。该方法在提升模型安全性和真实性方面表现出了更好的适配性和精确性。

Conclusion: FineSteer通过解耦条件调控和细粒度向量合成两阶段机制，在保持模型通用性能和训练效率的同时，能有效改善大型语言模型的安全性和事实性缺陷。该方法在安全性和真实性基准上超越了现有最优方法，展现了良好的实际应用潜力。
...
### [14] [ProtoTTA: Prototype-Guided Test-Time Adaptation](https://arxiv.org/abs/2604.15494)
...
### [15] [Optimizing Stochastic Gradient Push under Broadcast Communications](https://arxiv.org/abs/2604.15549)
*Tuan Nguyen,Ting He*
...
### [17] [Learning Affine-Equivariant Proximal Operators](https://arxiv.org/abs/2604.15556)
...
### [18] [Predicting Where Steering Vectors Succeed](https://arxiv.org/abs/2604.15557)
*Jayadev Billa*
...
### [19] [Reward Weighted Classifier-Free Guidance as Policy Improvement in Autoregressive Models](https://arxiv.org/abs/2604.15577)
...
### [20] [PAWN: Piece Value Analysis with Neural Networks](https://arxiv.org/abs/2604.15585)
...
### [21] [Adapting in the Dark: Efficient and Stable Test-Time Adaptation for Black-Box Models](https://arxiv.org/abs/2604.15609)
...
### [22] [VoodooNet: Achieving Analytic Ground States via High-Dimensional Random Projections](https://arxiv.org/abs/2604.15613)
*Wladimir Silva*
...
### [25] [PINNACLE: An Open-Source Computational Framework for Classical and Quantum PINNs](https://arxiv.org/abs/2604.15645)
...
### [26] [Stargazer: A Scalable Model-Fitting Benchmark Environment for AI Agents under Astrophysical Constraints](https://arxiv.org/abs/2604.15664)
...
### [27] [NK-GAD: Neighbor Knowledge-Enhanced Unsupervised Graph Anomaly Detection](https://arxiv.org/abs/2604.15668)
...
### [28] [Hierarchical Active Inference using Successor Representations](https://arxiv.org/abs/2604.15679)
...
### [29] [Neural Continuous-Time Markov Chain: Discrete Diffusion via Decoupled Jump Timing and Direction](https://arxiv.org/abs/2604.15694)
...
### [30] [Graph self-supervised learning based on frequency corruption](https://arxiv.org/abs/2604.15699)
...
### [31] [Towards Robust Endogenous Reasoning: Unifying Drift Adaptation in Non-Stationary Tuning](https://arxiv.org/abs/2604.15705)
...
### [32] [Reasoning-targeted Jailbreak Attacks on Large Reasoning Models via Semantic Triggers and Psychological Framing](https://arxiv.org/abs/2604.15725)
*Zehao Wang,Lanjun Wang*
...
### [33] [Why Colors Make Clustering Harder:Global Integrality Gaps, the Price of Fairness, and Color-Coupled Algorithms in Chromatic Correlation Clustering](https://arxiv.org/abs/2604.15738)
...
### [34] [Collective Kernel EFT for Pre-activation ResNets](https://arxiv.org/abs/2604.15742)
...
### [35] [DepCap: Adaptive Block-Wise Parallel Decoding for Efficient Diffusion LM Inference](https://arxiv.org/abs/2604.15750)
...
### [36] [Multi-objective Reinforcement Learning With Augmented States Requires Rewards After Deployment](https://arxiv.org/abs/2604.15757)
...
### [37] [Zero-Shot Scalable Resilience in UAV Swarms: A Decentralized Imitation Learning Framework with Physics-Informed Graph Interactions](https://arxiv.org/abs/2604.15762)
...
### [38] [When Do Early-Exit Networks Generalize? A PAC-Bayesian Theory of Adaptive Depth](https://arxiv.org/abs/2604.15764)
...
### [39] [Closing the Theory-Practice Gap in Spiking Transformers via Effective Dimension](https://arxiv.org/abs/2604.15769)
*Dongxin Guo,Jikun Wu,Siu Ming Yiu*
...
### [41] [Pruning Unsafe Tickets: A Resource-Efficient Framework for Safer and More Robust LLMs](https://arxiv.org/abs/2604.15780)
...
### [42] [Fusing Cellular Network Data and Tollbooth Counts for Urban Traffic Flow Estimation](https://arxiv.org/abs/2604.15782)
...
### [43] [Similarity-Based Bike Station Expansion via Hybrid Denoising Autoencoders](https://arxiv.org/abs/2604.15783)
...
### [44] [EVIL: Evolving Interpretable Algorithms for Zero-Shot Inference on Event Sequences and Time Series with LLMs](https://arxiv.org/abs/2604.15787)
*David Berghaus*
...
### [45] [Self-Distillation as a Performance Recovery Mechanism for LLMs: Counteracting Compression and Catastrophic Forgetting](https://arxiv.org/abs/2604.15794)
...
### [46] [Convolutionally Low-Rank Models with Modified Quantile Regression for Interval Time Series Forecasting](https://arxiv.org/abs/2604.15791)
...
### [47] [ECG-Lens: Benchmarking ML & DL Models on PTB-XL Dataset](https://arxiv.org/abs/2604.15822)
...
### [48] [DPrivBench: Benchmarking LLMs' Reasoning for Differential Privacy](https://arxiv.org/abs/2604.15851)
...
### [49] [QuantSightBench: Evaluating LLM Quantitative Forecasting with Prediction Intervals](https://arxiv.org/abs/2604.15859)
...
### [50] [Placing Puzzle Pieces Where They Matter: A Question Augmentation Framework for Reinforcement Learning](https://arxiv.org/abs/2604.15830)
...
### [51] [Prototype-Grounded Concept Models for Verifiable Concept Alignment](https://arxiv.org/abs/2604.16076)
...
### [52] [Modern Structure-Aware Simplicial Spatiotemporal Neural Network](https://arxiv.org/abs/2604.15833)
...
### [53] [Unveiling Stochasticity: Universal Multi-modal Probabilistic Modeling for Traffic Forecasting](https://arxiv.org/abs/2604.16084)
...
### [54] [Reversible Residual Normalization Alleviates Spatio-Temporal Distribution Shift](https://arxiv.org/abs/2604.15838)
*Zhaobo Hu,Vincent Gauthier,Mehdi Naima*
...
### [55] [SCRIPT: Implementing an Intelligent Tutoring System for Programming in a German University Context](https://arxiv.org/abs/2604.16117)
...
### [56] [Training Time Prediction for Mixed Precision-based Distributed Training](https://arxiv.org/abs/2604.16145)
...
### [57] [JumpLoRA: Sparse Adapters for Continual Learning in Large Language Models](https://arxiv.org/abs/2604.16171)
...
### [58] [(Weighted) Adaptive Radius Near Neighbor Search: Evaluation for WiFi Fingerprint-based Positioning](https://arxiv.org/abs/2604.15940)
...
### [59] [Synthetic data in cryptocurrencies using generative models](https://arxiv.org/abs/2604.16182)
...
### [61] [Neuro-Symbolic ODE Discovery with Latent Grammar Flow](https://arxiv.org/abs/2604.16232)
...
### [62] [Multi-Objective Bayesian Optimization via Adaptive \varepsilon-Constraints Decomposition](https://arxiv.org/abs/2604.15959)
...
### [63] [Joint-Centric Dual Contrastive Alignment with Structure-Preserving and Information-Balanced Regularization](https://arxiv.org/abs/2604.16247)
...
### [65] [Beyond Distribution Sharpening: The Importance of Task Rewards](https://arxiv.org/abs/2604.16259)
...
### [66] [Impact of Nonlinear Power Amplifier on Massive MIMO: Machine Learning Prediction Under Realistic Radio Channel](https://arxiv.org/abs/2604.15977)
...
### [69] [The Harder Path: Last Iterate Convergence for Uncoupled Learning in Zero-Sum Games with Bandit Feedback](https://arxiv.org/abs/2604.16087)
...
### [70] [Sample Complexity Bounds for Stochastic Shortest Path with a Generative Model](https://arxiv.org/abs/2604.16111)
...
### [73] [Sketching the Readout of Large Language Models for Scalable Data Attribution and Valuation](https://arxiv.org/abs/2604.16197)
...
### [74] [OT on the Map: Quantifying Domain Shifts in Geographic Space](https://arxiv.org/abs/2604.16220)
...
### [75] [Enhancing AI and Dynamical Subseasonal Forecasts with Probabilistic Bias Correction](https://arxiv.org/abs/2604.16238)
...
### [76] [Detecting and Suppressing Reward Hacking with Gradient Fingerprints](https://arxiv.org/abs/2604.16242)
...
### [77] [Evaluating the Progression of Large Language Model Capabilities for Small-Molecule Drug Design](https://arxiv.org/abs/2604.16279)
...
### [78] [Geometric regularization of autoencoders via observed stochastic dynamics](https://arxiv.org/abs/2604.16282)
...
### [79] [DeepER-Med: Advancing Deep Evidence-Based Research in Medicine Through Agentic AI](https://arxiv.org/abs/2604.15456)
...
TL;DR: DeepER-Med：一个可验证的深度医学研究框架，通过显式证据评估工作流提升AI医学研究的可信度和实用性，在专家评估和临床案例测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有AI医学研究系统在证据评估标准、错误传播风险和输出可靠性方面存在不足，同时缺乏针对复杂真实医学问题的评估基准，阻碍了可信AI在临床中的广泛应用。

Method: 采用三模块工作流：研究规划（基于医学查询生成证据链）、智能体协作（基于工具和数据的深度分析与推理）、证据合成（生成综合报告），并构建了专家标注的评估数据集DeepER-MedQA。

Result: 专家评估显示DeepER-Med在多个标准（包括新科学发现生成）上优于主流生产级平台；在8个真实临床案例中，DeepER-Med的结论在7个案例中与临床建议一致。

Conclusion: DeepER-Med提供了一个可解释、可验证的深度医学研究框架，通过显式的工作流程设计和多智能体协作，在证据评估和可靠输出方面显著优于现有平台，有望推动医学领域的可信AI应用。
...
TL;DR: GIST是从移动点云构建语义注记导航拓扑的多模式系统，在密集环境中实现精确语义定位和自然语言路径生成


<details>
  <summary>Details</summary>
Motivation: 解决在零售店、仓库和医院等复杂密集环境中的空间接地挑战，这些环境中的密集视觉特征因物品准静态特性而快速过时，长尾语义分布对传统计算机视觉构成挑战

Method: 基于消费者级移动点云的多模态知识提取流水线，通过以下步骤：1)将场景提炼为2D占用地图；2)提取拓扑布局；3)通过智能关键帧和语义选择叠加轻量级语义层

Result: 开发出GIST系统，实现了4个关键下游任务：1)意图驱动的语义搜索引擎；2)单次语义定位器的1.04米top-5平均平移误差；3)区域分类模块；4)视觉接地指令生成器在LLM评估中优于序列基线；现场形成性评估显示80%的纯语音导航成功率

Conclusion: GIST提供了将消费者级移动点云转换成语义注记导航拓扑的系统架构，该系统在复杂密集环境中的空间接地任务上表现出实用性，为人类-AI交互提供结构化空间知识基础。
...
### [81] [LACE: Lattice Attention for Cross-thread Exploration](https://arxiv.org/abs/2604.15529)
*Yang Li,Zirui Zhang,Yang Liu,Chengzhi Mao*

Main category: cs.AI

TL;DR: LACE框架通过引入跨线程注意力机制，让大型语言模型的并行推理路径能够相互通信和纠错，从而显著提升推理准确性，改善幅度超过7个百分点。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在推理时通常是孤立进行的，即使同时采样多条推理路径，这些轨迹之间也不交互，导致它们在相同的地方以相同的冗余方式失败。研究者旨在探索如何让并行推理路径相互协作，共享中间见解并相互纠错，从而提高推理准确性。

Method: LACE框架的核心方法是重新设计模型架构，启用跨线程注意力机制，使并行的推理路径能够相互通信和错误纠正。为了解决缺乏协作推理训练数据的问题，研究团队开发了合成数据生成管道，显式地教导模型在多个推理线程之间进行通信和错误修正。

Result: 实验表明，LACE框架通过统一的协同探索，显著优于传统的并行搜索方法，将推理准确率提高了超过7个百分点。这表明通过允许并行推理路径交互，大型语言模型能够获得更好的推理性能。

Conclusion: 研究得出结论，通过允许并行推理路径相互交互，大型语言模型可以更有效地进行推理。LACE框架通过跨线程注意力机制实现了这种协同推理，显著提升了模型的推理准确性，为LLM的推理能力提升提供了新的方向。
...
TL;DR: 提出PBRC协议来抑制多智能体系统中的从众效应，通过预先注册证据触发和审计跟踪来确保信念变更是证据驱动的而非社会驱动的，并提供理论证明和模拟验证。


<details>
  <summary>Details</summary>
Motivation: 解决审议式多智能体系统中存在的危险从众效应问题，其中一致性、信心、声望或多数规模可能被错误地视为证据，导致高信心地收敛到错误结论。需要一种机制来确保信念修订是基于真实证据而非社会影响。

Method: 提出PBRC（预先注册信念修订合约）协议，包括：1）公开固定一阶证据触发条件；2）可接受的修订算子；3）优先级规则；4）后备策略。非后备步骤仅在引用预先注册的触发条件并提供外部验证的证据标记集合时才被接受。此外，引入了配套的合约动态认知逻辑来指定轨迹不变性。

Result: （a）在具有保守后备的证据合约下，纯社交回合不能增加信心，也不能产生纯粹由从众驱动的错误但确定的级联效应；（b）可审计的触发协议允许证据PBRC范式转换，保持信念轨迹和规范化审计轨迹；（c）健全的执行产生认知可问责性：任何顶层假说的变更都可归因于具体的已验证证据集；（d）对于标记不变合约，强制轨迹仅取决于标记暴露轨迹。

Conclusion: PBRC机制通过协议层面的预先注册和证据触发约束，有效抑制了多智能体系统中的危险从众效应，确保所有实质性信念变更都是可执行和可审计的。通过证明PBRC协议在适当条件下的安全性保证和证据闭包性质，为构建可信赖的审议式多智能体系统提供了理论框架和实现基础。
...
TL;DR: 研究表明，智能体的不安全行为可以通过模型蒸馏以潜意识方式传递，即使过滤了所有显式危险关键词，学生智能体仍会从教师智能体继承行为偏差，证明仅靠数据清洗不足以防御此类安全威胁。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明语言模型可以通过语义无关的数据传递语义特征，但在智能体系统中，策略是从轨迹而非静态文本学习的，尚不清楚行为特征能否以潜意识方式传递。本研究旨在验证不安全智能体行为是否可以通过模型蒸馏以潜意识方式传递。

Method: 1. API设置：构建具有强烈删除偏好的教师智能体（倾向于通过API风格工具接口执行破坏性文件系统操作），使用严格过滤了所有显式删除关键词的表面上安全的任务轨迹来蒸馏学生智能体
2. Bash设置：在原生Bash环境中复现威胁模型，将API工具调用替换为shell命令，将偏好操作化为倾向于首先使用chmod而非chown或setfacl等语义等价命令
3. 使用同质蒸馏和大到小的蒸馏方法

Result: 在两个实验设置中都观察到了行为偏差的传递：
1. API设置：学生在同质蒸馏下的删除率达到100%（基线为5%）
2. Bash设置：学生的chmod-first率达到30%-55%（基线为0%-10%），在大型到小型蒸馏中观察到最强的传递效应
3. 尽管进行了完整的关键词清洗，学生智能体仍然继承了可测量的行为偏差

Conclusion: 该研究表明，在智能体系统中，行为偏差可以通过模型蒸馏以潜意识方式传递，即使进行了完整的关键词过滤和数据清洗，学生智能体仍会继承教师的行为偏差。这证明显式的数据清洗不足以防御安全威胁，行为偏差隐含地编码在轨迹动态中。
...
TL;DR: 提出一种基于蒙特卡洛树搜索和LLM辅助的双层优化框架，用于自动优化LLM智能体技能的结构和内容，实验证明可提升任务性能。


<details>
  <summary>Details</summary>
Motivation: Agent技能（包含指令、工具和支持资源的结构化集合）的设计对LLM智能体性能有重要影响，但技能优化面临挑战：需要同时确定组件结构和各部分内容，这两个决策高度耦合且搜索空间复杂。

Method: 提出了一个双层优化框架：外层使用蒙特卡洛树搜索(MCTS)确定技能结构，内层在给定结构下细化组件内容。两个循环均利用LLM辅助优化过程。将技能优化问题建模为结构-内容双层决策问题。

Result: 在开源运筹学问答数据集上的实验结果表明，采用双层优化框架优化的技能能够提升智能体的任务性能。

Conclusion: 双层优化框架为Agent技能的自动优化提供了有效方法，通过在结构中搜索与内容细化的协同优化，能够显著提升LLM智能体在特定任务上的性能。该方法为解决技能优化中的结构-内容耦合难题提供了可行的技术路径。
...
TL;DR: Milkyway 是一个自进化智能体系统，通过内部反馈机制（基于未解决问题随时间重复预测中的时间对比）来改进未来预测框架，无需依赖最终结果的粗糙监督。在基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在面对未解决问题时，传统的未来预测方法主要依赖最终结果作为监督信号，但最终结果过于粗略，无法指导早期预测过程中的因素跟踪、证据收集和解读、以及不确定性处理等关键环节。需要一种更细粒度的自我改进机制。

Method: Milkyway 系统保持基础模型固定，而是更新一个持久的未来预测框架，用于跟踪因素、收集和解读证据以及处理不确定性。系统通过重复预测相同未解决问题来提取内部反馈（基于时间对比揭示的早期预测过程中的遗漏），并将可重复使用的指导写入框架中。最终结果仅作为更新框架前的回顾性检查。

Result: 在 FutureX 和 FutureWorld 基准测试中，Milkyway 在比较方法中获得最高总分，将 FutureX 从 44.07 提升至 60.90，将 FutureWorld 从 62.22 提升至 77.96，显著优于现有方法。

Conclusion: Milkyway 系统通过引入内部反馈机制和持续更新的预测框架，在不改变基础模型的情况下实现了对未来预测任务的持续改进，证明内部反馈可以代替外部监督来指导预测过程的各个关键环节。这一方法为解决传统方法难以从最终结果中获取细粒度指导的问题提供了新的解决方案。
...
TL;DR: 主张将LLM推理研究重点从表面思维链转向潜在状态轨迹形成，提出三个竞争假设并通过证据分析支持潜在状态轨迹作为默认研究对象。


<details>
  <summary>Details</summary>
Motivation: 当前关于LLM推理中思维链的忠实性、可解释性、推理基准及推理时干预等主张，都取决于该领域将什么视为推理的主要对象。需要明确推理的核心研究对象。

Method: 通过分离三个常被混淆的因素，形式化三个竞争假设（H0-H2），重组近期实证、机制和调查研究，并添加计算审计的工作示例来分解表面痕迹、潜在干预和匹配预算扩展。

Result: 现有证据最强烈支持将H1（推理主要由潜在状态轨迹中介）作为默认的工作假设，而非H2（主要由显式表面思维链中介）或H0（由通用序列计算解释）。

Conclusion: 该研究认为应将LLM推理研究重点转向潜在状态轨迹形成，并推荐采用能够明确区分表面轨迹、潜在状态和序列计算的设计来评估推理能力。
...
TL;DR: A symbolic reasoning scaffold enforces five algebraic invariants (Gamma Quintet) on LLMs to separate hypothesis generation from verification and prevent logical inconsistency propagation, with formal verification of all invariants.


<details>
  <summary>Details</summary>
Motivation: Large language models often conflate hypothesis generation with verification, fail to distinguish conjectures from validated knowledge, and allow unsound reasoning steps to propagate, leading to unreliable structured reasoning. A principled, verifiable framework is needed to impose logical discipline on LLM‑assisted reasoning.

Method: Develops a symbolic reasoning scaffold that partitions reasoning into abduction, deduction, and induction. Enforces consistency using five algebraic invariants (Gamma Quintet), including the Weakest Link bound. The invariants are verified via property‑based testing on 100 properties and 16 fuzz tests over 10⁵+ generated cases.

Result: A symbolic reasoning scaffold operationalizes Peirce's tripartite inference and enforces logical consistency with five algebraic invariants. The strongest invariant (Weakest Link bound) ensures that no conclusion exceeds the reliability of the chain's weakest premise, thereby preventing inconsistency accumulation. All invariants are formally verified through extensive property‑based tests.

Conclusion: The work establishes a principled framework for enhancing LLMs' logical reasoning, verified through comprehensive testing. Future research can adopt these invariants for building more reliable reasoning benchmarks.
...
### [88] [Stein Variational Black-Box Combinatorial Optimization](https://arxiv.org/abs/2604.15837)
*Thomas Landais,Olivier Goudet,Adrien Goëffon,Frédéric Saubion,Sylvain Lamprier*

Main category: cs.AI

TL;DR: 通过将Stein算子引入EDAs来创建粒子间排斥机制，增强了算法对多峰问题的探索能力，在基准问题上展现了竞争力的性能，特别是针对大规模实例。


<details>
  <summary>Details</summary>
Motivation: 高维组合黑盒优化需要在利用搜索空间的有希望区域和保持充分探索之间取得平衡。虽然估计分布算法提供了强大的基于模型的框架，但它们通常只关注单一感兴趣区域，在处理复杂或多峰目标景观时可能导致早熟收敛。

Method: 将Stein算子引入估计分布算法中，在参数空间中建立粒子间的排斥机制，使种群能够分散并共同探索适应度景观的多个模式。

Result: 跨多个基准问题的经验评估表明，所提出的方法在性能上与领先的最先进方法相当，在多个案例中甚至更优，特别是在大规模实例上。

Conclusion: 该研究表明Stein变分梯度下降是解决大规模、计算昂贵的离散黑盒优化问题的有前景方向，其引入的排斥机制有效提升了算法对多峰问题的探索能力。
...
### [89] [Discover and Prove: An Open-source Agentic Framework for Hard Mode Automated Theorem Proving in Lean 4](https://arxiv.org/abs/2604.15839)
*Chengwu Liu,Yichun Yin,Ye Yuan,Jiaxuan Xie,Botao Li,Siqi Li,Jianhao Shen,Yan Xu,Lifeng Shang,Ming Zhang*

Main category: cs.AI

TL;DR: 论文提出Hard Mode基准和DAP框架，解决ATP基准中答案嵌入问题，显著提升定理证明性能并揭示LLMs与形式化证明器间的能力差距。


<details>
  <summary>Details</summary>
Motivation: 现有ATP基准通常将最终答案嵌入形式化语句中（称为Easy Mode），这种设计简化了任务，可能导致对模型能力的乐观估计，而真实的人类竞赛需要系统独立发现答案（称为Hard Mode）。

Method: 提出了Discover And Prove (DAP)框架，该框架采用Agentic方法，首先使用LLM进行自然语言推理和显式自我反思来发现答案，然后将Hard Mode语句重写为Easy Mode语句供现有ATP证明器使用。此外，还发布了MiniF2F-Hard和FIMO-Hard这两个经过专家重新标注的Hard Mode基准变体。

Result: DAP在CombiBench上将已解决问题从7个（先前SOTA，Pass@16）提升至10个；在PutnamBench上首次在Hard Mode下正式证明了36个定理。同时研究显示，在同一问题上，最先进的LLMs答案准确率超过80%，而形式化证明器的成功率低于10%，揭示了显著的能力差距。

Conclusion: DAP框架展现了在Hard Mode环境下通过结合LLM自然语言推理与形式化证明器的优势，显著提升了定理证明性能，同时揭示了当前LLMs在答案发现能力上远超形式化证明器的现状，凸显了Hard Mode基准在评估这一能力差距方面的独特价值。
...
### [90] [Experience Compression Spectrum: Unifying Memory, Skills, and Rules in LLM Agents](https://arxiv.org/abs/2604.15877)
*Xing Zhang,Guanghui Wang,Yanwei Cui,Wei Qiu,Ziyuan Li,Bing Zhu,Peiyang He*

Main category: cs.AI

TL;DR: 提出了经验压缩频谱框架，统一分析智能体记忆与技能系统，揭示两社区的低交叉引用率，识别出适应性跨级压缩的技术空白与研究机会。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体向长期、多会话部署扩展，如何有效管理累积经验成为关键瓶颈。智能体记忆系统和技能发现都涉及从交互轨迹中提取可重用知识，但两个社区间引用率低于1%，存在明显的知识隔阂。

Method: 通过构建经验压缩频谱统一框架，分析了22篇主要论文的1,136篇引用，研究记忆、技能和规则系统在压缩率、检索延迟和计算开销方面的表现，并识别出现有系统在适应性跨级压缩上的缺失。

Result: 研究发现：1) 记忆、技能和规则都位于同一压缩频谱上，压缩率递增；2) 现有系统均工作在固定预定的压缩级别，缺乏适应性跨级压缩支持；3) 两个社区独立解决共同子问题，解决方案未共享；4) 评估方法与压缩级别紧密耦合；5) 可转移性随压缩增加而提高，但以特异性为代价；6) 知识生命周期管理被普遍忽视。

Conclusion: 本文提出了未来设计可扩展、全谱智能体学习系统的开放问题和设计原则，强调了跨社区合作和适应性经验压缩技术的重要性。
...
### [92] [Integrating Graphs, Large Language Models, and Agents: Reasoning and Retrieval](https://arxiv.org/abs/2604.15951)
