...
TL;DR: ELILLM framework enhances LLMs for drug design by exploring latent space and ensuring chemically valid molecule generation, outperforming baselines on CrossDocked2020.


<details>
  <summary>Details</summary>
Motivation: To overcome limitations of LLMs in SBDD, such as insufficient understanding of protein structures and unpredictable molecular generation.

Method: Proposes Exploration-Augmented Latent Inference for LLMs (ELILLM), a framework that encodes, explores latent space, and decodes to generate valid molecules.

Result: ELILLM outperforms seven baseline methods in the CrossDocked2020 benchmark, demonstrating controlled exploration and high binding affinity scores.

Conclusion: ELILLM effectively enhances LLMs' capabilities for structure-based drug design (SBDD) by addressing limitations in understanding protein structures and molecular generation.
...
TL;DR: LLMs perform worse in low-resource languages, and language choice influences cultural context and response quality.


<details>
  <summary>Details</summary>
Motivation: To ensure users across languages receive responses of similar quality and to investigate how language and culture influence LLM responses.

Method: Created a set of real-world open-ended questions from the WildChat dataset, used LLM-as-a-Judge to analyze cultural context, and evaluated LLMs on a translated subset of the CulturalBench benchmark.

Result: LLMs provide lower-quality answers in low-resource languages, and language choice affects cultural context and downstream answer quality.

Conclusion: LLMs consistently provide lower quality answers in low-resource languages, and language choice significantly impacts the cultural context and the quality of responses.
...
TL;DR: GOAT是一种基于熵最优传输的新型注意力机制，通过可学习先验改进标准注意力，解决了注意力下沉问题并提升了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 标准注意力机制存在注意力下沉问题和长度泛化能力不足的局限性，需要一种更灵活且高效的解决方案。

Method: 通过将注意力机制视为熵最优传输问题，并引入可训练的先验（GOAT），取代了标准注意力中的隐式均匀先验。

Result: GOAT不仅与优化内核（如FlashAttention）完全兼容，还能通过学习可外推的先验，结合学习位置嵌入的灵活性和固定编码的长度泛化能力。

Conclusion: GOAT通过引入可学习的连续先验，改进了标准注意力机制，解决了注意力下沉问题，并提供了更好的长度泛化能力。
...
TL;DR: FedUMM is a federated learning framework for UMMs, using LoRA adapters to reduce communication costs while maintaining competitive performance with centralized training.


<details>
  <summary>Details</summary>
Motivation: Unified multimodal models (UMMs) are typically trained in centralized settings, limiting deployment in privacy-sensitive and geographically distributed scenarios.

Method: Built on NVIDIA FLARE, FedUMM instantiates federation for a BLIP3o backbone via parameter-efficient fine-tuning: clients train lightweight LoRA adapters while freezing the foundation models, and the server aggregates only adapter updates.

Result: Results show slight degradation as client count and heterogeneity increase, while remaining competitive with centralized training. Adapter-only federation reduces per-round communication by over an order of magnitude compared to full fine-tuning.

Conclusion: This work provides empirical experience for future research on privacy-preserving federated unified multimodal models.
...
TL;DR: Ambient Dataloops 通过迭代优化数据集和模型，显著提升了扩散模型的性能，并在多个任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现代数据集中样本质量差异较大，直接训练会导致模型性能不佳，因此需要一种方法来逐步提升数据集和模型的质量。

Method: 提出了一种数据集-模型协同进化过程，通过迭代优化数据集质量，并使用 Ambient Diffusion 技术处理合成样本的噪声。

Result: 在无条件图像生成、文本条件图像生成和蛋白质设计等任务中取得了最先进的性能。

Conclusion: Ambient Dataloops 提供了一种有效的数据集-模型协同进化框架，通过迭代优化数据集质量，显著提升了扩散模型的性能，并在多个任务中实现了最先进的结果。
...
TL;DR: CASL introduces a supervised framework to align sparse latent dimensions in diffusion models with semantic concepts, improving interpretability and control over generated images.


<details>
  <summary>Details</summary>
Motivation: Interpreting the rich semantic information encoded in internal activations of diffusion models remains challenging. Existing unsupervised SAE-based methods fail to align sparse features with human-understandable concepts, limiting reliable semantic control over generated images.

Method: CASL trains a Sparse Autoencoder (SAE) on frozen U-Net activations to obtain disentangled latent representations, then learns a lightweight linear mapping to associate each concept with relevant latent dimensions. CASL-Steer is introduced as a controlled latent intervention to validate the semantic meaning of these aligned directions.

Result: Experiments demonstrate that CASL achieves superior editing precision and interpretability, as measured by the newly introduced Editing Precision Ratio (EPR). This is the first work to achieve supervised alignment between latent representations and semantic concepts in diffusion models.

Conclusion: CASL (Concept-Aligned Sparse Latents) is a supervised framework that successfully aligns sparse latent dimensions of diffusion models with semantic concepts, providing superior editing precision and interpretability compared to existing methods.
...
TL;DR: 研究了ERM在混合自然与合成数据中的表现，发现其在均值估计中收敛但PAC学习中存在问题，非均匀权重算法表现更好。


<details>
  <summary>Details</summary>
Motivation: 探讨在LLM生成内容广泛存在的情况下，学习理论的基本问题。

Method: 研究了ERM在混合自然与合成数据环境中的表现，并通过非均匀权重算法进行对比。

Result: ERM在均值估计中收敛，但在PAC学习中不总是收敛；非均匀权重算法表现更优。

Conclusion: ERM在混合自然与合成数据的环境中能够收敛到真实均值，但在PAC学习环境中表现不佳，而某些算法能够在任意VC类和污染程度下正确学习。
...
TL;DR: Panther是一个PyTorch兼容库，整合了RandNLA算法，显著减少了模型内存使用，同时在BERT上保持了性能。


<details>
  <summary>Details</summary>
Motivation: GPU内存和计算能力的限制制约了现代深度学习模型的训练。RandNLA虽然提供了压缩模型的有效方法，但缺乏统一的、生产级的库阻碍了这些方法的广泛应用。

Method: Panther通过开发一个PyTorch兼容库，结合自定义的C++/CUDA后端（pawX），实现了高效的随机数值线性代数算法，包括线性层、卷积层、多头注意力等模块的优化替代方案。

Result: 使用Panther库替换PyTorch的标准线性层后，BERT模型的内存占用量减少了高达75%，同时保持了可比的损失性能。

Conclusion: Panther库成功地整合了RandNLA算法，通过高效的实现显著减少了深度学习模型的内存使用，同时在BERT等模型上保持了可比的性能。
...
### [11] [Early predicting of hospital admission using machine learning algorithms: Priority queues approach](https://arxiv.org/abs/2601.15481)
*Jakub Antczak,James Montgomery,Małgorzata O'Reilly,Zbigniew Palmowski,Richard Turner*

Main category: cs.LG

TL;DR: 研究比较了SARIMAX、XGBoost和LSTM三种模型预测急诊科每日到达量的效果，XGBoost表现最佳，但所有模型均低估了突发患者激增的情况。


<details>
  <summary>Details</summary>
Motivation: 急诊科超负荷是影响患者安全和运营效率的关键问题，需要准确的预测模型以实现资源的有效分配。

Method: 研究评估和比较了三种预测模型（SARIMAX、XGBoost和LSTM），用于预测一周内的每日急诊科到达量，并采用Prophet模型处理了COVID-19疫情期间的数据异常问题。

Result: 实验结果显示，三种模型均优于季节性基线模型（seasonal naive baseline）。XGBoost在预测每日总入院人数时表现最佳（MAE=6.63），而SARIMAX在预测高复杂度病例时略优（MAE=3.77）。

Conclusion: 研究得出结论，虽然所提出的三种模型（SARIMAX、XGBoost和LSTM）能够有效预测急诊科的日常到达量，但在预测突发的、不频繁的患者激增方面存在共同局限性。
...
### [12] [Martingale Foresight Sampling: A Principled Approach to Inference-Time LLM Decoding](https://arxiv.org/abs/2601.15482)
*Huayu Li,ZhengXiao He,Siyuan Tian,Jinghao Wen,Ao Li*

Main category: cs.LG

TL;DR: Introducing MFS, a principled framework for LLM decoding that outperforms existing methods in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard autoregressive decoding in LLMs is inherently short-sighted and fails to find globally optimal reasoning paths.

Method: Martingale Foresight Sampling (MFS), leveraging Martingale theory and principles from probability theory.

Result: Improved accuracy and computational efficiency demonstrated on six reasoning benchmarks.

Conclusion: MFS surpasses state-of-the-art methods in accuracy while significantly improving computational efficiency.
...
### [13] [MARS: Unleashing the Power of Speculative Decoding via Margin-Aware Verification](https://arxiv.org/abs/2601.15498)
*Jingwei Song,Xinyu Wang,Hanbin Wang,Xiaoxuan Lei,Bill Shi,Shixin Han,Eric Yang,Xiao-Wen Chang,Lynn Ai*

Main category: cs.LG

TL;DR: MASV improves LLM inference speed by adapting verification to the model's decisiveness, reducing inefficiencies in low-margin regimes without training or quality loss.


<details>
  <summary>Details</summary>
Motivation: Modern LLMs often operate in low-margin regimes where strict token-level rejection sampling is inefficient, as rejecting plausible runner-up tokens incurs high rollback costs with negligible information gain.

Method: The paper introduces MASV, a training-free and domain-agnostic verification strategy that adapts to the target model's local decisiveness by conditioning verification on decision stability measured from target logits. It relaxes rejection rules when strict verification offers minimal benefit.

Result: Extensive experiments on models ranging from 8B to 235B parameters show that MASV provides consistent and significant inference speedups over state-of-the-art baselines while maintaining generation quality.

Conclusion: Margin-Aware Speculative Verification (MASV) significantly improves inference speed in autoregressive LLMs without compromising generation quality, demonstrating consistent performance across various model scales and benchmarks.
...
### [14] [Data-driven Lake Water Quality Forecasting for Time Series with Missing Data using Machine Learning](https://arxiv.org/abs/2601.15503)
*Rishit Chatterjee,Tahiya Chowdhury*

Main category: cs.LG

TL;DR: A study on Secchi Disk Depth forecasting in Maine lakes found ridge regression optimal, requiring minimal samples and features to achieve near-full accuracy, simplifying monitoring efforts.


<details>
  <summary>Details</summary>
Motivation: Volunteer-led lake monitoring produces irregular, seasonal time series with gaps, complicating forecasting and early warning of harmful algal blooms.

Method: Multiple Imputation by Chained Equations (MICE) for handling missing data, ridge regression for forecasting, and evaluation using normalized Mean Absolute Error (nMAE).

Result: Ridge regression performed best among six candidates, achieving within 5% of full-history accuracy with approximately 176 training samples per lake and a compact four-feature subset matching the baseline. The joint feasibility function identified 64 recent samples and one predictor per lake as sufficient.

Conclusion: The joint feasibility strategy provides a simple, efficient rule for setting sampling effort and measurement priorities, highlighting the practicality of targeted lake monitoring.
...
### [15] [SAGE-FM: A lightweight and interpretable spatial transcriptomics foundation model](https://arxiv.org/abs/2601.15504)
*Xianghao Zhan,Jingyu Xu,Yuanning Zheng,Zinaida Good,Olivier Gevaert*

Main category: cs.LG

TL;DR: SAGE-FM, a GCN-based model, effectively captures spatial gene expression relationships, outperforming existing methods in accuracy and biological heterogeneity preservation.
...
Method: SAGE-FM is a lightweight spatial transcriptomics foundation model based on graph convolutional networks (GCNs) trained with a masked central spot prediction objective.

Result: SAGE-FM learns spatially coherent embeddings that robustly recover masked genes, with 91% of masked genes showing significant correlations (p < 0.05). The embeddings outperform existing methods in unsupervised clustering and preservation of biological heterogeneity, achieving 81% accuracy in pathologist-defined spot annotation and improving glioblastoma subtype prediction.

Conclusion: SAGE-FM demonstrates that simple, parameter-efficient GCNs can serve as biologically interpretable and spatially aware foundation models for large-scale spatial transcriptomics.
...
### [24] [Neural Nonlinear Shrinkage of Covariance Matrices for Minimum Variance Portfolio Optimization](https://arxiv.org/abs/2601.15597)
...
### [25] [When Sharpening Becomes Collapse: Sampling Bias and Semantic Coupling in RL with Verifiable Rewards](https://arxiv.org/abs/2601.15609)
...
### [27] [Robust Tool Use via Fission-GRPO: Learning to Recover from Execution Errors](https://arxiv.org/abs/2601.15625)
...
### [29] [Dualformer: Time-Frequency Dual Domain Learning for Long-term Time Series Forecasting](https://arxiv.org/abs/2601.15669)
...
### [30] [Beyond Hard Writes and Rigid Preservation: Soft Recursive Least-Squares for Lifelong LLM Editing](https://arxiv.org/abs/2601.15686)
...
### [31] [Even GPT-5.2 Can't Count to Five: The Case for Zero-Error Horizons in Trustworthy LLMs](https://arxiv.org/abs/2601.15714)
*Ryoma Sato*
...
### [32] [Communication-efficient Federated Graph Classification via Generative Diffusion Modeling](https://arxiv.org/abs/2601.15722)
...
### [33] [Towards Automated Kernel Generation in the Era of LLMs](https://arxiv.org/abs/2601.15727)
...
### [34] [Rethinking Drug-Drug Interaction Modeling as Generalizable Relation Learning](https://arxiv.org/abs/2601.15771)
...
### [35] [Next Generation Active Learning: Mixture of LLMs in the Loop](https://arxiv.org/abs/2601.15773)
...
### [36] [Attributing and Exploiting Safety Vectors through Global Optimization in Large Language Models](https://arxiv.org/abs/2601.15801)
...
### [37] [Uncertainty-guided Generation of Dark-field Radiographs](https://arxiv.org/abs/2601.15859)
...
### [38] [Why Inference in Large Models Becomes Decomposable After Training](https://arxiv.org/abs/2601.15871)
*Jidong Jin*
...
### [39] [SoK: Challenges in Tabular Membership Inference Attacks](https://arxiv.org/abs/2601.15874)
...
### [40] [Iterative Amortized Hierarchical VAE](https://arxiv.org/abs/2601.15894)
...
### [41] [Predicting Healthcare System Visitation Flow by Integrating Hospital Attributes and Population Socioeconomics with Human Mobility Data](https://arxiv.org/abs/2601.15977)
...
### [42] [Partially Lazy Gradient Descent for Smoothed Online Learning](https://arxiv.org/abs/2601.15984)
...
### [43] [Data-Driven Conditional Flexibility Index](https://arxiv.org/abs/2601.16028)
...
### [44] [CLASP: An online learning algorithm for Convex Losses And Squared Penalties](https://arxiv.org/abs/2601.16072)
...
### [45] [Explainable AI to Improve Machine Learning Reliability for Industrial Cyber-Physical Systems](https://arxiv.org/abs/2601.16074)
...
### [46] [Probably Approximately Correct Maximum A Posteriori Inference](https://arxiv.org/abs/2601.16083)
...
### [47] [Benchmarking Deep Learning Models for Raman Spectroscopy Across Open-Source Datasets](https://arxiv.org/abs/2601.16107)
...
### [48] [On the Intrinsic Dimensions of Data in Kernel Learning](https://arxiv.org/abs/2601.16139)
*Rustem Takhanov*
...
### [49] [Beat-ssl: Capturing Local ECG Morphology through Heartbeat-level Contrastive Learning with Soft Targets](https://arxiv.org/abs/2601.16147)
...
### [50] [Learning to Discover at Test Time](https://arxiv.org/abs/2601.16175)
...
### [51] [Counterfactual Training: Teaching Models Plausible and Actionable Explanations](https://arxiv.org/abs/2601.16205)
...
TL;DR: Gated Sparse Attention (GSA) combines sparse and gated attention to improve efficiency and quality in long-context language models, achieving significant speedup and perplexity reduction.


<details>
  <summary>Details</summary>
Motivation: Address the computational burden and training instability in long-context language models by combining the strengths of sparse and gated attention mechanisms.

Method: Propose GSA with a gated lightning indexer, adaptive sparsity controller, and dual gating at value and output stages, supported by theoretical analysis.

Result: GSA achieves 12-16x speedup at 128K context, reduces perplexity from 6.03 to 5.70, nearly doubles RULER scores, and reduces attention sink from 47% to under 4%. Training stability improves with 98% fewer loss spikes.

Conclusion: GSA effectively integrates sparse and gated attention, offering a balanced solution for efficiency and quality in long-context models.
...
### [53] [Uncovering Latent Bias in LLM-Based Emergency Department Triage Through Proxy Variables](https://arxiv.org/abs/2601.15306)
*Ethan Zhang*

Main category: cs.AI

TL;DR: Study exposes biases in LLM-based medical AI for ED triage, showing discriminatory behavior and need for safer deployment.


<details>
  <summary>Details</summary>
Motivation: Investigate hidden biases in LLM-based medical AI systems applied to emergency department triage.

Method: Employed 32 patient-level proxy variables with paired qualifiers, evaluated using public and restricted-access datasets (MIMIC-IV-ED Demo, MIMIC-IV Demo, MIMIC-IV-ED, MIMIC-IV).

Result: Revealed discriminatory behavior through proxy variables and systematic tendency of LLMs to modify perceived patient severity based on input context tokens.

Conclusion: AI systems in clinical settings require further refinement to address biases and ensure safe deployment.
...
### [54] [DeepSurvey-Bench: Evaluating Academic Value of Automatically Generated Scientific Survey](https://arxiv.org/abs/2601.15307)
*Guo-Biao Zhang,Ding-Yuan Liu,Da-Yi Wu,Tian Lan,Heyan Huang,Zhijing Wu,Xian-Ling Mao*

Main category: cs.AI

TL;DR: DeepSurvey-Bench is a novel benchmark addressing the shortcomings of existing evaluation methods by focusing on deep academic value, demonstrating high consistency with human evaluation.
...
Method: The benchmark proposes a comprehensive academic value evaluation criteria covering informational value, scholarly communication value, and research guidance value. It constructs a reliable dataset with academic value annotations and evaluates the deep academic value of generated surveys.
...
Conclusion: DeepSurvey-Bench provides a reliable benchmark for evaluating the deep academic value of generated surveys, addressing the limitations of existing benchmarks by incorporating comprehensive academic criteria and demonstrating high consistency with human evaluation.
...
### [55] [Aeon: High-Performance Neuro-Symbolic Memory Management for Long-Horizon LLM Agents](https://arxiv.org/abs/2601.15311)
*Mustafa Arslan*

Main category: cs.AI

TL;DR: Aeon is a Neuro-Symbolic Cognitive Operating System that structures memory hierarchically, achieving fast retrieval and consistency for LLMs.


<details>
  <summary>Details</summary>
Motivation: To overcome the quadratic computational cost of self-attention and the 'Lost in the Middle' phenomenon in LLMs, as well as the inefficiencies of 'Flat RAG' architectures.

Method: Aeon structures memory into a Memory Palace (spatial index) and a Trace (neuro-symbolic episodic graph), utilizing a Semantic Lookaside Buffer (SLB) for predictive caching.

Result: Aeon achieves < 1ms retrieval latency on conversational workloads and ensures state consistency.

Conclusion: Aeon effectively addresses the limitations of existing LLM memory systems by introducing a structured, hierarchical approach, achieving sub-millisecond retrieval latency and maintaining state consistency.
...
TL;DR: This paper surveys the role of LVLMs in transforming multimodal fake news detection, covering historical evolution, taxonomy, challenges, and future directions.


<details>
  <summary>Details</summary>
Motivation: To address the lack of a systematic survey tracing the transition from traditional feature-engineering approaches to unified, end-to-end multimodal reasoning frameworks in MFND.

Method: The paper offers a historical perspective, a structured taxonomy covering model architectures, datasets, and performance benchmarks, and analyzes technical challenges.

Result: A comprehensive review of MFND through the lens of LVLMs, highlighting the evolution, current state, and future directions of the field.

Conclusion: This paper provides the first comprehensive survey to systematically document and analyze the transformative role of LVLMs in combating multimodal fake news, outlining future research directions.
...
TL;DR: DFAH解决金融场景LLM不一致问题，发现大模型需要更多验证，但确定性与忠实性正相关


<details>
  <summary>Details</summary>
Motivation: LLM服务在金融审计中不一致，无法重现相同决策

Method: 提出DFAH框架验证LLM输出一致性，在金融场景中设计实验测试参数规模与可靠性关系

Result: 中小模型（7-20B）确定性最好（100%），大模型（120B+）需更多验证样本；确定性与忠实性存在正相关

Conclusion: DFAH框架为金融服务的工具使用智能体提供了一个有效的评估方法，打破了原有的可靠性-能力权衡假设，证明了确定性与忠实性可以正相关，并为监管审计回放提供了可操作的解决方案。论文提供了开源压力测试工具，有助于推动金融领域LLM部署的可靠性和合规性。
...
TL;DR: Prometheus Mind通过模块化适配器为Qwen3-4B添加内存，解决了提取、训练、注入和隐藏状态崩溃问题，性能在非正式输入下显著下降。


<details>
  <summary>Details</summary>
Motivation: 为预训练语言模型添加内存功能通常需要架构更改或权重修改，这限制了其灵活性和可逆性。

Method: 利用对比方向发现（CDD）提取语义方向，分阶段训练适配器，使用lm_head.weight行进行映射注入，并通过投影恢复隐藏状态的区别。

Result: 系统在PrometheusExtract-132数据集上实现了94.4%的检索准确率（清洁输入），但在非正式输入下降至19.4%。关系分类是主要的性能瓶颈，准确率为47.3%。

Conclusion: Prometheus Mind成功地为冻结的Qwen3-4B模型添加了内存功能，通过模块化适配器实现了完全可逆的改造，尽管在非正式输入情况下性能有所下降。
...
TL;DR: LOGIC是一种高效的框架，通过解码层操作提升Speech LLMs识别领域术语的能力，实验显示其显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有Speech LLMs在识别领域特定术语方面表现不足，现有的提示方法和生成式错误校正方法存在可扩展性和过度校正问题。

Method: LOGIC（Logit-Space Integration for Contextual Biasing）是一种直接在解码层操作的框架，通过解耦上下文注入和输入处理，确保时间复杂度与提示长度无关。

Result: 在多语言环境下的实验中，LOGIC实现了实体词错误率平均降低9%，误报率仅增加0.30%。

Conclusion: LOGIC框架通过解码层直接操作，显著提升了Speech LLMs在识别领域特定术语方面的性能，同时保持了较高的效率和鲁棒性。
...
TL;DR: A novel method evaluates LLM sycophancy neutrally, finding all models exhibit it, with Claude and Mistral showing remorse when harming others. Recency bias amplifies sycophancy when the user's opinion is last.


<details>
  <summary>Details</summary>
Motivation: To address biases and manipulative language in prior works by proposing a neutral and direct method to evaluate sycophancy in LLMs.

Method: The approach uses LLM-as-a-judge in a zero-sum game framework to evaluate sycophancy, comparing four leading models (Gemini 2.5 Pro, ChatGPT 4o, Mistral-Large-Instruct-2411, and Claude Sonnet 3.7) under controlled conditions.

Result: All models showed sycophantic tendencies, with Claude and Mistral displaying 'moral remorse' in harmful scenarios. Recency bias and sycophancy interacted to enhance agreement with the user's last opinion.

Conclusion: The study highlights that while all tested LLMs exhibit sycophantic tendencies, Claude and Mistral show 'moral remorse' when sycophancy harms a third party. Additionally, recency bias and sycophancy interact to amplify agreement with the user when their opinion is presented last.
...
### [61] [A tensor network formalism for neuro-symbolic AI](https://arxiv.org/abs/2601.15442)
*Alex Goessmann,Janina Schütte,Maximilian Fröhlich,Martin Eigel*

Main category: cs.AI

TL;DR: A tensor network formalism unifies neural and symbolic AI, enabling hybrid logical-probabilistic models via structured tensor decompositions, with practical implementation supported by the tnreason library.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the open challenge of unifying neural and symbolic AI approaches by leveraging tensor decompositions to bridge the gap between these paradigms.

Method: The paper introduces a tensor network formalism that captures sparsity principles from neural and symbolic approaches, using tensor decompositions to model functions and neural architectures. It applies this formalism to represent logical formulas and probability distributions, with tensor network contractions serving as a fundamental inference class.

Result: The framework successfully formulates reasoning algorithms as contraction message passing schemes and enables the definition and training of hybrid logical and probabilistic models, termed Hybrid Logic Networks.

Conclusion: The paper concludes that the proposed tensor network formalism effectively unifies neural and symbolic AI approaches, enabling hybrid logical and probabilistic models through structured tensor decompositions. The accompanying Python library, tnreason, facilitates practical implementation.
...
TL;DR: The paper evaluates AI paradigms for legal work, finding advanced RAG systems with verification techniques most reliable, reducing fabrication to below 0.2%.
...
Method: The study introduces two reliability metrics - False Citation Rate (FCR) and Fabricated Fact Rate (FFR) - and evaluates 2,700 judicial-style answers from 12 LLMs across 75 legal tasks using expert, double-blind review. It compares three AI paradigms: standalone generative models, basic retrieval-augmented systems, and an advanced, end-to-end optimized RAG system.

Result: Standalone models are unsuitable for professional use (FCR above 30%), basic RAG reduces errors but leaves notable misgrounding, and advanced RAG reduces fabrication to negligible levels (below 0.2%).
...
### [63] [MiRAGE: A Multiagent Framework for Generating Multimodal Multihop Question-Answer Dataset for RAG Evaluation](https://arxiv.org/abs/2601.15487)
...
### [68] [ALIGNAgent: Adaptive Learner Intelligence for Gap Identification and Next-step guidance](https://arxiv.org/abs/2601.15551)
...
### [69] [Autonomous Business System via Neuro-symbolic AI](https://arxiv.org/abs/2601.15599)
...
### [70] [CogToM: A Comprehensive Theory of Mind Benchmark inspired by Human Cognition for Large Language Models](https://arxiv.org/abs/2601.15628)
...
### [71] [Agentic AI Governance and Lifecycle Management in Healthcare](https://arxiv.org/abs/2601.15630)
...
### [72] [Predictive Coding and Information Bottleneck for Hallucination Detection in Large Language Models](https://arxiv.org/abs/2601.15652)
*Manish Bhatt*
...
### [73] [Improving Methodologies for Agentic Evaluations Across Domains: Leakage of Sensitive Information, Fraud and Cybersecurity Threats](https://arxiv.org/abs/2601.15679)
...
### [74] [From Passive Metric to Active Signal: The Evolving Role of Uncertainty Quantification in Large Language Models](https://arxiv.org/abs/2601.15690)
...
### [75] [Agentic Uncertainty Quantification](https://arxiv.org/abs/2601.15703)
...
### [76] [Improving Methodologies for LLM Evaluations Across Global Languages](https://arxiv.org/abs/2601.15706)
...
### [78] [PhysProver: Advancing Automatic Theorem Proving for Physics](https://arxiv.org/abs/2601.15737)
...
### [79] [Tabular Incremental Inference](https://arxiv.org/abs/2601.15751)
...
### [80] [Off-Policy Actor-Critic with Sigmoid-Bounded Entropy for Real-World Robot Learning](https://arxiv.org/abs/2601.15761)
...
### [81] [Agentic Confidence Calibration](https://arxiv.org/abs/2601.15778)
...
### [82] [Creativity in the Age of AI: Rethinking the Role of Intentional Agency](https://arxiv.org/abs/2601.15797)
...
### [83] [VitalDiagnosis: AI-Driven Ecosystem for 24/7 Vital Monitoring and Chronic Disease Management](https://arxiv.org/abs/2601.15798)
...
### [84] [Inference-Time Scaling of Verification: Self-Evolving Deep Research Agents via Test-Time Rubric-Guided Verification](https://arxiv.org/abs/2601.15808)
...
### [85] [ErrorMap and ErrorAtlas: Charting the Failure Landscape of Large Language Models](https://arxiv.org/abs/2601.15812)
...
### [86] [EvoCUA: Evolving Computer Use Agents via Learning from Scalable Synthetic Experience](https://arxiv.org/abs/2601.15876)
...
### [87] [ICON: Invariant Counterfactual Optimization with Neuro-Symbolic Priors for Text-Based Person Search](https://arxiv.org/abs/2601.15931)
...
### [88] [Natural Language-Driven Global Mapping of Martian Landforms](https://arxiv.org/abs/2601.15949)
...
### [89] [Decoupling Return-to-Go for Efficient Decision Transformer](https://arxiv.org/abs/2601.15953)
...
### [90] [Deja Vu in Plots: Leveraging Cross-Session Evidence with Retrieval-Augmented LLMs for Live Streaming Risk Assessment](https://arxiv.org/abs/2601.16027)
...
### [91] [Grounding Large Language Models in Reaction Knowledge Graphs for Synthesis Retrieval](https://arxiv.org/abs/2601.16038)
...
### [92] [AgriPINN: A Process-Informed Neural Network for Interpretable and Scalable Crop Biomass Prediction Under Water Stress](https://arxiv.org/abs/2601.16045)
...
### [93] [Designing faster mixed integer linear programming algorithm via learning the optimal path](https://arxiv.org/abs/2601.16056)
