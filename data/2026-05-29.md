...
TL;DR: 研究发现知识编辑方法ROME和MEMIT依赖于共同的权重子空间机制，编辑通过抑制而非覆盖原有知识实现，这解释了编辑无法传播到相关事实的原因。


<details>
  <summary>Details</summary>
Motivation: 虽然ROME和MEMIT等方法通过修改MLP权重来更新变换器模型中的事实关联，但其内部机制研究不足，特别是这些编辑是否依赖于共同的机制这一问题尚未探索。

Method: 通过在编辑后的权重上训练紧凑的二进制掩码，验证不同事实编辑是否共享共同的机制；分析掩码如何通过消除后续层中的过度关注来反转编辑。

Result: 训练得到的二进制掩码能在训练集上反转80%的编辑，测试集上反转超过70%，证实不同编辑共享通用的功能结构；在编辑过程中注入该掩码将使编辑成功率从98%降至38%；编辑机制是通过抑制而非覆盖知识实现的，这解释了为何编辑无法传播到相关事实。

Conclusion: 知识编辑技术ROME和MEMIT并非简单地重写模型知识，而是依赖于一个通用的功能子空间来抑制原有知识，这一机械性理解为检测和防御不需要的编辑提供了理论基础。
...
### [2] [Mechanistic origins of catastrophic forgetting: why RL preserves circuits better than SFT?](https://arxiv.org/abs/2605.28860)
*Jeanmely Rojas Nunez,Viraj Sawant,Nathan Allen,Nomgondalai Amgalanbaatar,Yannis Zongo,Vasu Sharma,Maheep Chaudhary*

Main category: cs.LG

TL;DR: 论文探究RL相比SFT在微调LLMs时减少灾难性遗忘的机制原因，发现RL能更好地保留模型内部计算电路但适应速度较慢，而SFT虽适应更快但对电路破坏更严重。


<details>
  <summary>Details</summary>
Motivation: 研究者观察到RL在微调大型语言模型时能更有效地保留先前能力，但这种现象尚未在计算电路机制层面得到解释。本文旨在探究RL相比SFT的优越性是否反映在内部计算电路的更好保存上。

Method: 作者提出了“差异电路脆弱性”这一头层级衡量指标，用于量化电路在微调过程中的退化程度，并将其应用于比较Qwen2.5-3B-Instruct模型在科学问答任务上的RL和SFT微调效果。

Result: 研究表明存在明显的机制性权衡：SFT能更快地适应目标任务，但导致显著更大的电路破坏和先前能力遗忘；而RL保留了更大比例的基础电路，代价是任务适应速度较慢。

Conclusion: RL和SFT在微调大型语言模型时存在明显的机制性权衡：SFT能更快适应目标任务但导致更严重的电路破坏和先前能力遗忘，而RL能更好地保留基础模型的计算电路但任务适应速度较慢，这表明电路保护可能有助于解释RL为何对灾难性遗忘更具鲁棒性。
...
### [3] [Self-Play Reinforcement Learning under Imperfect Information in Big 2](https://arxiv.org/abs/2605.28863)
*Aalok Patwa*

Main category: cs.LG

TL;DR: 开发Big 2自博弈RL框架，PPO优于价值方法，熵正则化和当前策略自博弈提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究不完美信息多玩家游戏中智能体面临隐藏信息、稀疏奖励和非稳态对手等挑战，Big 2作为一个四人不完美信息纸牌游戏提供了理想的测试环境。

Method: 开发了一个用于Big 2的自博弈强化学习框架，在相同环境、输入表示、训练预算和评估协议下，对比策略梯度方法（PPO）与价值近似方法（蒙特卡洛Q近似、SARSA、Q学习）。

Result: 在随机、贪婪和启发式Big 2对手的对抗中，PPO表现优于蒙特卡洛Q近似、SARSA和Q学习；适度的熵正则化通过防止策略过于确定性而提升PPO性能；当前策略自博弈比检查点自博弈或固定对手训练能提供更好的有限预算课程。

Conclusion: Big 2游戏是一个研究不完美信息、多玩家交互、延迟奖励和可变动作集的深度学习RL的有效控制环境，PPO算法在该环境下表现最优，适度的熵正则化和当前策略自博弈能有效提升性能。
...
### [4] [Emergent Semantic Representations in World Models through Physical Interaction without Linguistic Supervision](https://arxiv.org/abs/2605.28865)
*Jiayi Fang*

Main category: cs.LG

TL;DR: 研究发现，在没有语言监督的情况下，通过物理探索训练的世界模型会学习物理世界的几何结构作为其表示的组织原则。


<details>
  <summary>Details</summary>
Motivation: 探究在没有语言监督的情况下，通过物理探索训练的世界模型究竟学习了什么。研究假设答案可能是物理世界的几何结构。

Method: 使用基于VAE的世界模型，在随机具身探索中进行训练。通过测量方向精度和位置RSA等指标评估潜在空间的结构组织。分析训练检查点的预测性能和语义对齐关系。通过双敲除实验，改变KL正则化系数来检验几何结构的作用。

Result: 训练后的潜在空间发展出了反映物理几何的空间语义结构：方向精度0.677±0.029（随机编码器为0.547），位置RSA为0.192±0.047（随机编码器为0.029）。预测性能和语义对齐共同改进（Spearman r=-0.61, p=0.004）。KL正则化（beta=0.1）会破坏几何结构，同时导致预测性能和语义对齐崩溃；降低到beta=0.001则同时恢复两者。

Conclusion: 物理世界几何是世界模型表示的组织原则，这对设计具有语义基础的具身智能体有直接意义。
...
### [5] [Continuity and Ordinality Matter: Constraining Time Series Tokens for Effective Time Series Analysis with Large Language Models](https://arxiv.org/abs/2605.28866)
*Musheng Li,Ziying Zhang,Cheng jin,Yuantao Gu*

Main category: cs.LG

TL;DR: 提出COM方法，通过整合几何约束保持时间序列token的连续性和顺序性，提升token-based TS-LLMs的性能。


<details>
  <summary>Details</summary>
Motivation: 现有token-based时间序列大语言模型（TS-LLMs）大多忽略了时间序列token固有的连续性和顺序性，这严重限制了模型性能。

Method: 提出COM（Continuity and Ordinality Matter）策略，在初始化和训练阶段整合几何约束，以保持时间序列token嵌入的连续性和顺序性。

Result: 在多个时间序列分析基准测试中，COM一致提升了token-based TS-LLMs的性能，取得了有竞争力的结果和强大的泛化能力。

Conclusion: 保持时间序列token嵌入的连续性和顺序性对于token-based TS-LLMs的有效性至关重要，COM方法通过整合几何约束实现了这一目标。
...
### [6] [PrismFlow: Residual Dynamics for Flow Matching in Time-Series Generation](https://arxiv.org/abs/2605.28867)
*Junru Zhang,Lang Feng,Jinbo Wang,Xu Guo,Yucheng Wang,Han Yu,Min Wu,Yabo Dong,Duanqing Xu*

Main category: cs.LG

TL;DR: PrismFlow是一种新的Flow Matching方法，引入Koopman启发式动态专家来改进时间序列生成。通过使用置信感知的赢家通吃目标函数，促进专家专业化，从而更好地保留高频时间结构，解决了标准FM中的谱收缩问题，在多个指标上实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界的时间序列数据常呈现多模态模式和多尺度动态，包括振荡和高频变化。传统的Flow Matching方法在实际应用中通常依赖单一有限容量的全局向量场估计器，在处理这种异构时间分布时，不同机制可能通过相近的流状态，但需要不相容的条件速度。标准的ℓ₂速度匹配目标函数会使估计器学习过度平滑的局部传输场近似，导致谱失真和模式覆盖不足。

Method: PrismFlow提出了两个核心创新：1）基于Koopman理论的动态专家模型，每个专家学习潜在空间中的残差修正，其中局部非线性时间演化可近似为线性变换；2）基于置信度的赢家通吃目标函数，该函数仅更新与样本对齐最好的专家，同时屏蔽其他专家的梯度，促进模式特异性专业化。

Result: 在多个基准测试中，PrismFlow有效缓解了标准Flow Matching中的谱收缩问题，实现了最先进的性能：Context-FID提升15.6%，Discriminative Score改善38.6%。同时保持了对低数据环境的鲁棒性，并适用于预测和插补任务。

Conclusion: 本文提出的PrismFlow方法通过引入Koopman启发的动态专家模型，有效解决了Flow Matching在复杂时间序列生成中面临的谱收缩和模式覆盖不足问题。该方法在保持FM稳定性的同时，显著提升了时间序列数据的高频结构和多模态生成质量。
...
### [7] [Balancing Multimodal Learning through Label Space Reshaping](https://arxiv.org/abs/2605.28869)
*Xiaoyu Ma,Weijie Zhang,Yuanhao Gao,Han Miao,Yongjian Deng,Hao Chen*

Main category: cs.LG

TL;DR: 针对多模态学习中模态不平衡问题，本文提出从标签侧设计的BMLR方法，通过重塑标签空间来均衡模态间映射难度，有效提升模态交互和性能。


<details>
  <summary>Details</summary>
Motivation: 多模态学习中存在的模态不平衡问题通常表现为收敛速度快的模态主导优化，而其他模态训练不足。现有方法主要从加强弱模态或调整优化梯度角度切入，但这种补偿策略往往以牺牲强模态优化能力为代价，且未从根源上分析学习速度差异的产生原因。

Method: 提出了平衡多模态标签重塑（BMLR）方法，通过重塑跨模态标签空间来均衡模态间的映射难度，从而促进模态交互并向每个模态注入更丰富的类间信息。

Result: 在多种架构上进行广泛实验表明，BMLR能够持续提升多模态性能，并与不同模型设计展现出良好的兼容性。

Conclusion: 平衡多模态学习中的模态不平衡问题需要从标签空间设计入手，通过平衡映射难度来优化模态间平衡，BMLR方法为此提供了有效的解决方案。
...
TL;DR: 通过信号、偏差和噪声的统计框架研究柏拉图表示假说，发现线性表示、模型偏差矫正和数据稀缺性分别解释了表示对齐的不同方面，并提出了一个统一的统计模型。


<details>
  <summary>Details</summary>
Motivation: 探索柏拉图表示假说（PRH）背后的机制，即为什么不同AI模型的表示会显示出惊人的对齐性。希望通过一个系统的统计框架来分解影响表示对齐的主要因素。

Method: 采用稀疏自编码器提取线性对象-属性特征；使用居中和归一化技术来减小模型间的偏差；通过分析词频与对齐度的相关性来研究噪声；最后综合这些发现构建了一个统一的统计模型。

Result: 发现线性表示假说（LRH）能够解释PRH中的信号部分；模型偏差可以通过居中和归一化来减轻；表示噪声与数据稀缺性密切相关（词频越高对齐度越高）；综合这些因素的统计模型能够进一步完善LRH并解释更多跨模型对齐现象。

Conclusion: 论文提出了一个由信号、偏差和噪声构成的三位一体的统计框架，用于解释柏拉图表示假说（PRH）。通过线性表示假说（LRH）、模型偏差矫正和噪声分析的综合，该统计模型能够解释现代AI架构中表示对齐的进一步现象。
...
### [9] [Pre-Registering the Detectable Effect: A Paired-MDE Budget for 4-bit Quantization Benchmarks, with a Pilot Audit](https://arxiv.org/abs/2605.28873)
*Zexin Zhuang,Yanhang Li,Zhichao Fan*

Main category: cs.LG

TL;DR: 该研究将经典配对样本量计算应用到量化基准测试中，建立了一个最小可检测效应界限，帮助研究人员将量化声明的可靠性问题转化为实用的预算方案。通过四个模型和基准测试的实例验证，以及MMLU提示模板研究，发现许多报告的基准测试不可靠性实际是抽样噪声，同时强调提示模板的变异性会影响量化审计结果。


<details>
  <summary>Details</summary>
Motivation: 量化基准测试中常见的问题是：研究者如何可靠地量化模型的改进？现有基准测试中报告的结果往往包含抽样噪声，这可能会被错误地解释为"基准不可靠性"。此外，量化审计中忽略提示模板的变异性会将这些变异吸收到噪声中。因此，需要一种方法能够将"我的量化声明有多可靠？"这一问题转化为具体可用的预算方案。

Method: 本研究采用经典配对二元样本量计算方法，将其适配到量化基准测试中，得出一个保守的最小可检测效应界限公式。通过四个模型和四个基准测试实例（k=5组n=100的分割）进行验证，同时开展并行MMLU提示模板研究以比较量化噪声和提示噪声的规模。使用一个未测量但用于规划的FP16-NF4分歧率ρ_d=0.10作为假设值。此外，研究提供了一个五行预注册模板作为补充。

Result: 研究得出在配对项目数m和FP16-NF4分歧率ρ_d下的最小可检测效应界限：δ^* ≤ (z_{1-α/2}+z_{1-β})√(ρ_d/m)。实例测试中，假设ρ_d=0.10时，所有观测到的NF4-FP16差异都落在MDE范围内，且大部分跨分割的标准差在二项式参考值√(p(1-p)/n)的±1.5个百分点范围内。MMLU测试显示，提示模板的差异范围为2-10个百分点，达到甚至超过最大观测到的量化差异（3.2个百分点）。唯一的边界情况（OPT-WinoGrande|Δ|=3.2个百分点）在ρ_d=0.10时低于MDE，但在ρ_d=0.05时高于MDE，说明了规划权衡。

Conclusion: 该研究通过经典的配对二元样本量计算方法，为量化基准测试提供了保守的最小可检测效应（MDE）界限，并将其转化为一行代码的预算方案。研究结果显示，在假设的FP16-NF4分歧率下，观测到的量化差异均在MDE范围之内，且大部分跨分割的标准差符合二项式参考值，表明许多被报告为"基准不可靠性"的方差实际上是二项式抽样噪声。同时，研究发现在MMLU基准测试中，提示模板的差异范围（2-10个百分点）与量化效应相当甚至更大，强调了在量化审计前固定提示模板的重要性。
...
### [13] [Sequential Physics-Constrained Neural Operator Forward Modeling for the $\textit{Norne}$ Reservoir System](https://arxiv.org/abs/2605.28909)
...
### [14] [Cycle-Space Informed Detection of Autoencoded Blind False Data Injection Attacks on Power Systems](https://arxiv.org/abs/2605.28912)
...
### [15] [When LLM Reward Design Fails: Diagnostic-Driven Refinement for Sparse Structured RL](https://arxiv.org/abs/2605.28918)
...
### [16] [CosmicFish-HRM: Adaptive Reasoning via Hierarchical Recurrent Mechanisms in Compact Language Models](https://arxiv.org/abs/2605.28919)
...
### [17] [Conf-Gen: Conformal Uncertainty Quantification for Generative Models](https://arxiv.org/abs/2605.28920)
...
### [18] [A Training-Time Diagnostic for Generalization via the Log-Alignment Ratio](https://arxiv.org/abs/2605.28975)
...
### [19] [Comparing Post-Hoc Explainable AI Methods for Interpreting Black-Box EEG Models in Depression Detection](https://arxiv.org/abs/2605.28977)
...
### [20] [Learning Robust and Task-Invariant Functional Representation from fMRI through Siamese Self-Supervised Learning](https://arxiv.org/abs/2605.28990)
...
### [21] [FormInv: A Measurement Protocol for Semantic Invariance in Mathematical Reasoning Benchmarks](https://arxiv.org/abs/2605.29001)
...
### [26] [Designing Active Tether-Net Systems for Space Debris Capture with Graph-Learning-Aided Mixed-Combinatorial Optimization](https://arxiv.org/abs/2605.29021)
...
### [29] [Parallel Adaptive Multi-Objective Evolutionary Learning of Discretized Bayesian Network Classifiers for Clinical Data](https://arxiv.org/abs/2605.29058)
...
### [30] [Ensemble Score Filtering for Real-Data Energy Consumption Forecast Correction](https://arxiv.org/abs/2605.29072)
...
### [32] [OISD: On-Policy Internal Self-Distillation of Language Models](https://arxiv.org/abs/2605.29089)
...
### [33] [Model Merging by Output-Space Projection](https://arxiv.org/abs/2605.29101)
...
### [34] [Bridging Chemists and AI: An Expert-Augmented Framework for Interpretable Route Evaluation](https://arxiv.org/abs/2605.29108)
...
### [38] [Do Deep Networks Forget Initialization? A Forgetting-Time View of Practical Inductive Bias](https://arxiv.org/abs/2605.29152)
...
### [39] [Unveiling Multi-regime Patterns in SciML: Distinct Failure Modes and Regime-specific Optimization](https://arxiv.org/abs/2605.29153)
...
### [40] [RUBRIC-ARROW: Alternating Pointwise Rubric Reward Modeling for LLM Post-training in Non-verifiable Domains](https://arxiv.org/abs/2605.29156)
...
### [41] [Parallax: Parameterized Local Linear Attention for Language Modeling](https://arxiv.org/abs/2605.29157)
...
### [42] [PROTOCOL: Late Interaction Retrieval for Protein Homolog Search](https://arxiv.org/abs/2605.29158)
...
### [43] [Evolutionary Refinement of Generative Graph Topologies: A Hybrid WGAN-GA Approach](https://arxiv.org/abs/2605.29161)
...
### [44] [Probabilistic bias adjustment of seasonal forecasts using generative machine learning: A case study of Arctic sea ice predictions](https://arxiv.org/abs/2605.29172)
...
### [45] [TIMEGATE: Sustainable Time-Boxed Promotion Gates for Continual ML Adaptation Under Resource Constraints](https://arxiv.org/abs/2605.29183)
...
### [46] [Influence-Guided Symbolic Regression: Scientific Discovery via LLM-Driven Equation Search with Granular Feedback](https://arxiv.org/abs/2605.29184)
...
### [47] [When RL Suppresses Its Own Vocabulary: Recovering Reasoning Diversity in Puzzle-to-Math Transfer](https://arxiv.org/abs/2605.29190)
...
### [48] [Stochastic Lifting for Generating Trajectories of Stochastic Physical Systems](https://arxiv.org/abs/2605.29194)
...
### [49] [Auditing Training Data in Generative Music Models via Black-Box Membership Inference](https://arxiv.org/abs/2605.29202)
...
### [50] [Inferring the Size of Large Language Models From Popular Text Memorization](https://arxiv.org/abs/2605.29223)
*Ivica Nikolic*
...
### [51] [Traditional machine learning vs. deep learning from dynamic graph representations of proteins' 3D folds in the task of protein structure classification](https://arxiv.org/abs/2605.29228)
...
### [52] [BlockBatch: Multi-Scale Consensus Decoding for Efficient Diffusion Language Model Inference](https://arxiv.org/abs/2605.29233)
...
### [53] [SigmaMedStat: Temporal Signal Modeling for ICU False Alarm Reduction](https://arxiv.org/abs/2605.29236)
...
### [54] [KLAS: Using Similarity to Stitch Neural Networks for Improved Accuracy-Efficiency Tradeoffs](https://arxiv.org/abs/2605.29259)
...
### [55] [Robust Frequency-Calibrated Virtual EEG Channel Generation from Four Frontal Electrodes for Wearable EEG Augmentation](https://arxiv.org/abs/2605.29263)
*Minghao Xiao*
...
### [56] [Causal Label Recovery in Payment Networks](https://arxiv.org/abs/2605.29272)
*Gaurav Dhama*
...
### [57] [A Theoretical and Experimental Study of a Novel Adaptive Learning Algorithm](https://arxiv.org/abs/2605.29273)
...
### [58] [LoopFM: Learning frOm HistOrical RePresentations of Foundation Model for Recommendation](https://arxiv.org/abs/2605.29280)
...
### [59] [Do Physics Foundation Models Learn Generalizable Physics? A Bias-Aware Benchmark Across Physical Regimes and Distribution Shifts](https://arxiv.org/abs/2605.29283)
...
### [60] [NeuroEdge: Real-Time Hand Gesture Recognition with High-Density EMG Using Deep Learning at the Edge](https://arxiv.org/abs/2605.29326)
...
### [61] [Attention as In-Context Empirical Bayes: A Two-Stage View via Particle Dynamics](https://arxiv.org/abs/2605.29351)
...
### [62] [Neural-Behavioral Representation of Natural Whole-body Movement in Monkeys](https://arxiv.org/abs/2605.29355)
...
### [63] [Solving Integer Linear Programming with Parallel Tempering](https://arxiv.org/abs/2605.29366)
...
### [64] [Deep Adaptive Dimension Reduction for Bayesian Inference in Inverse Problems](https://arxiv.org/abs/2605.29373)
...
### [65] [TRACER: Persistent Regularization for Robust Multimodal Finetuning](https://arxiv.org/abs/2605.29380)
...
### [66] [On the Optimizer Dependence of Neural Scaling Laws](https://arxiv.org/abs/2605.29387)
...
### [67] [GDSD: Reinforcement Learning as Guided Denoiser Self-Distillation for Diffusion Language Models](https://arxiv.org/abs/2605.29398)
...
### [68] [Rethinking Post-Training Recipes for Multimodal Time-Series Forecasting](https://arxiv.org/abs/2605.29401)
...
### [69] [Information-Directed Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2605.29405)
*Keru Chen*
...
### [70] [The Good, the Bad, and the Ugly of Markov Boundary for Tabular Prediction](https://arxiv.org/abs/2605.29411)
...
### [71] [How Much Is a Dataset Worth? Scaling Laws, the Vendi Score, and Matrix Spectral Functions](https://arxiv.org/abs/2605.29448)
...
### [72] [Forget Less, Generalize More: Unifying Temporal and Structural Adaptation for Dynamic Graphs](https://arxiv.org/abs/2605.29453)
...
### [73] [A Full-Pipeline Framework for Evaluating Membership Inference Attacks in Machine Learning](https://arxiv.org/abs/2605.29454)
...
### [74] [Honest Lying: Understanding Memory Confabulation in Reflexive Agents](https://arxiv.org/abs/2605.29463)
...
### [75] [Composing Non-Conjugate Factor Graphs with Closed-Form Variational Inference](https://arxiv.org/abs/2605.29467)
...
### [76] [Access Sets Matter: Budgeting Expert Reads for Scalable Weight-Space Model Merging](https://arxiv.org/abs/2605.29489)
...
### [77] [Gradient Perturbation: Learning to Perturb Gradients for Adaptive Training](https://arxiv.org/abs/2605.29494)
*Hua Li*
...
### [78] [On-Policy Replay for Continual Supervised Fine-Tuning](https://arxiv.org/abs/2605.29495)
...
### [79] [Convex Basins in Single-Index Model Loss Landscapes: Applications to Robust Recovery under Strong Adversarial Corruption](https://arxiv.org/abs/2605.29497)
...
### [80] [Quotient DAGs for Off-Policy Evaluation:Forward-Flow Importance Sampling and Exact Slate Propensities](https://arxiv.org/abs/2605.29500)
...
### [81] [K-FinHallu: A Hallucination Detection Benchmark for Multi-Turn RAG in Korean Finance](https://arxiv.org/abs/2605.29523)
...
### [82] [Learning to Perturb Hidden Representations for Generalizable Deep Learning](https://arxiv.org/abs/2605.29525)
*Hua Li*
...
### [83] [AsymVLM: Asymmetric Token Pruning for Efficient Vision-Language Model Inference](https://arxiv.org/abs/2605.29535)
...
### [85] [Singularity-aware Optimization via Randomized Geometric Probing: Towards Stable Non-smooth Optimization](https://arxiv.org/abs/2605.29547)
...
### [86] [Why Larger Models Learn More: Effects of Capacity, Interference, and Rare-Task Retention](https://arxiv.org/abs/2605.29548)
...
### [87] [On the Construction and Implications of Low-Loss Valleys in LoRA-based Bayesian Inference](https://arxiv.org/abs/2605.29580)
...
### [88] [PEARL: Training Socratic Tutors with Pedagogically Aligned Reinforcement Learning](https://arxiv.org/abs/2605.29582)
...
### [89] [Cluster-Level Attention-Guided Parallel Decoding for Masked Diffusion Language Models](https://arxiv.org/abs/2605.29607)
...
### [90] [MōLe-Λ: Learning the Coupled-Cluster Response State for Energies, Gradients, and Properties](https://arxiv.org/abs/2605.29622)
...
### [91] [Relational Rank Geometry in Transformers: Detecting and Steering Hidden-State Relation Frames](https://arxiv.org/abs/2605.29634)
*Mazen Kobrosly*
...
### [92] [The Sample Complexity of Multiclass and Sparse Contextual Bandits](https://arxiv.org/abs/2605.29645)
...
### [93] [Opir: Efficient Multi-Task Safety Classification for Toxicity, Jailbreaks, Hate Speech, and Harmful Content](https://arxiv.org/abs/2605.29659)
...
### [94] [A Geometric View of SRC: Learning Representations for Stable Residual Inference](https://arxiv.org/abs/2605.29673)
...
### [95] [Kernel Renormalization in Bayesian Deep Neural Networks: the Equivalent Wishart Ansatz in the Proportional Regime](https://arxiv.org/abs/2605.29684)
...
### [96] [A Novel Tensor Product-Based Neural Network for Solving Partial Differential Equations](https://arxiv.org/abs/2605.29688)
...
### [97] [Momentum Based Reward Design for Low Emission Traffic Signal Control](https://arxiv.org/abs/2605.29693)
...
### [98] [A Systematic Evaluation of Molecular Mixture Behavior Prediction](https://arxiv.org/abs/2605.29698)
...
### [99] [The Little Book of Generative AI Foundations: An Intuitive Mathematical Primer](https://arxiv.org/abs/2605.29713)
*Tianhua Chen*
...
### [100] [Bastion: Budget-Aware Speculative Decoding with Tree-structured Block Diffusion Drafting](https://arxiv.org/abs/2605.29727)
...
### [101] [Realistic honeypot evaluations for scheming propensity](https://arxiv.org/abs/2605.29729)
...
### [102] [EMAG: Differentiable 4D Gaussian Mixture Splatting for EEG Spatial Super-Resolution](https://arxiv.org/abs/2605.29731)
...
### [103] [MMTM: Tri-Modal Topic Modeling for Long-Form Video via Similarity-Gated Fusion](https://arxiv.org/abs/2605.29765)
...
### [104] [Hista and Numca: Estimate State Value Effectively for LLM Reinforcement Learning](https://arxiv.org/abs/2605.29782)
...
### [105] [Gated Graph Attention Networks with Learnable Temperature](https://arxiv.org/abs/2605.29803)
...
### [106] [The Interplay Between Interpolation and Aggregation in Regression: Optimal Sample Complexity](https://arxiv.org/abs/2605.29819)
...
### [107] [When Do Graph Foundation Models Transfer? A Data-Centric Theory](https://arxiv.org/abs/2605.29828)
...
### [108] [Open World Autoencoding Drift Detection with Novel Class Recognition in Tabular Non-stationary Data Streams](https://arxiv.org/abs/2605.29834)
*Joanna Komorniczak*
...
### [109] [CB-SLICE: Concept-Based Interpretable Error Slice Discovery](https://arxiv.org/abs/2605.29836)
...
### [110] [HARP: Hadamard-Preconditioned Adaptive Rotation Processor for Extreme LLM Quantization](https://arxiv.org/abs/2605.29843)
...
### [111] [MIRAGE: Adaptive Multimodal Gating for Whole-Brain fMRI Encoding](https://arxiv.org/abs/2605.29850)
...
### [112] [Feedback-to-Rubrics: Can We Learn Expert Criteria from Inline Comments?](https://arxiv.org/abs/2605.29857)
...
### [113] [ESPO: Early-Stopping Proximal Policy Optimization](https://arxiv.org/abs/2605.29860)
...
### [114] [STAP: A Shuffle-Tokenized App Predictor with Ultra Long Context for Vocabulary-Free Mobile App Prediction](https://arxiv.org/abs/2605.29863)
...
### [115] [Open Problem: Separating Geometric and Algorithmic Compression via Cayley-Table Completion](https://arxiv.org/abs/2605.29885)
*Dongsung Huh*
...
### [116] [LaRA: Layer-wise Representation Analysis for Detecting Data Contamination in RL Post-Training](https://arxiv.org/abs/2605.29888)
...
### [117] [OVA-IB: One vs All Information Bottleneck for Multi-Modal Alignment](https://arxiv.org/abs/2605.29900)
...
### [118] [Plan, Don't Pose: Long Composite Motion Generation with Text-Aligned BFM](https://arxiv.org/abs/2605.29906)
...
### [119] [Reducing Experimental Testing in Space Propulsion Film Cooling Analyses by Pixelwise Generative Image Interpolation](https://arxiv.org/abs/2605.29911)
...
### [120] [Midpoint Generative Models](https://arxiv.org/abs/2605.29920)
...
### [121] [A Triple-Modal Contrastive Learning Framework with Sequence, Graph, and 3D Features for Drug-Target Interaction Prediction](https://arxiv.org/abs/2605.29926)
...
### [122] [Treatment-Conditioned Diffusion for Forecasting Neurodegenerative Disease Progression](https://arxiv.org/abs/2605.29932)
...
### [123] [CLUBench: A Clustering Benchmark](https://arxiv.org/abs/2605.29933)
...
### [124] [From Short Histories to Long Futures: Horizon-Aware Graph Neural Networks for Long Horizon Forecasting](https://arxiv.org/abs/2605.29952)
...
### [125] [A Fully Convolutional Approach to Denoising Structural Dynamics Data from X-Ray Photon Correlation Spectroscopy](https://arxiv.org/abs/2605.29975)
...
### [126] [Improving Adversarial Robustness of Attribution via Implicit Regularization](https://arxiv.org/abs/2605.29983)
...
### [127] [MIC: Maximizing Informational Capacity in Adaptive Representations via Isotropic Subspace Alignment](https://arxiv.org/abs/2605.29987)
...
### [128] [Test Time Training for Supervised Causal Learning](https://arxiv.org/abs/2605.30015)
...
### [129] [Masked Diffusion Modeling for Anomaly Detection](https://arxiv.org/abs/2605.30046)
...
### [130] [Ridge Regression from Poisson Resetting: A Renewal Perspective on Spectral Regularization](https://arxiv.org/abs/2605.30059)
*Petar Jolakoski*
...
### [132] [Q-ANCHOR: Federated Quantum Learning with ZNE-guided Correction](https://arxiv.org/abs/2605.30075)
...
### [133] [Distributionally Robust Set Representation Learning Under Inference-Time Element Corruption](https://arxiv.org/abs/2605.30089)
...
### [134] [Chess-World-Model: A 10M-Game Benchmark for Exact State Tracking from Chess Move Sequences](https://arxiv.org/abs/2605.30100)
...
### [135] [Convergence Theory for Iterative LLM-Based Neural Architecture Search: A Parametric Cross-Entropy Framework with Closed-Form Proxy Reliability](https://arxiv.org/abs/2605.30103)
...
### [136] [Striding Across Reynolds Numbers: Representation Geometry in Neural PDE Generalisation](https://arxiv.org/abs/2605.30112)
*Jianing Shi*
...
### [137] [Evolving Features vs Evolving Entire Trees with GP for Interpretable Survival Analysis](https://arxiv.org/abs/2605.30119)
...
### [138] [Beyond MSE: Improving Precipitation Nowcasting with Multi-Quantile Regression](https://arxiv.org/abs/2605.30122)
...
### [139] [Learning to Extrapolate to New Tasks: A Relational Approach to Task Extrapolation](https://arxiv.org/abs/2605.30132)
...
### [140] [DAMEL: Dual-Axis Multi-Expert Learning for Class-Imbalanced Learning](https://arxiv.org/abs/2605.30135)
...
### [141] [Overcoming Forgetting in LLM Fine-Tuning with Evolution Strategies](https://arxiv.org/abs/2605.30148)
...
### [142] [RL2ML: Finite-Rollout Surrogate Objectives from Reinforcement Learning to Maximum Likelihood](https://arxiv.org/abs/2605.30154)
*Yifu Zheng*
...
### [143] [On Distributional Reinforcement Learning in Chaotic Dynamical Systems](https://arxiv.org/abs/2605.30160)
...
### [144] [iLoRA: Bayesian Low-Rank Adaptation with Latent Interaction Graphs for Microbiome Diagnosis](https://arxiv.org/abs/2605.30179)
...
### [145] [Can AI Weather Models Predict Beyond Two Weeks? A Quantitative Benchmark and Analysis of Long Rollouts](https://arxiv.org/abs/2605.30184)
...
### [146] [CalArena: A Large-Scale Post-Hoc Calibration Benchmark](https://arxiv.org/abs/2605.30188)
...
### [147] [Mean-Field Diffuser: Scaling Offline MARL to Thousands of Agents](https://arxiv.org/abs/2605.30190)
...
### [148] [Active Continual Learning with Metaplastic Binary Bayesian Neural Networks](https://arxiv.org/abs/2605.30198)
...
### [149] [HPO: Hysteretic Policy Optimization for Stable and Efficient Training under Sparse-Reward Regime](https://arxiv.org/abs/2605.30201)
...
### [150] [Faithful Embeddings of Irregular and Asynchronous Data for Online Log-NCDEs](https://arxiv.org/abs/2605.30213)
...
### [151] [TriSearch: Learning to Optimize Triangulations via Bistellar Flips](https://arxiv.org/abs/2605.30220)
...
### [152] [ExDBSCAN: Explaining DBSCAN with Counterfactual Reasoning -- Additional Material](https://arxiv.org/abs/2605.30225)
...
### [153] [Anti Mode-Collapse in Mean-Field Transformer via Auxiliary Variables](https://arxiv.org/abs/2605.30229)
...
### [154] [How's it going? Reinforcement learning in language models recruits a functional welfare axis](https://arxiv.org/abs/2605.30232)
...
### [155] [OOD-GraphLLM: Graph Large Language Model for Out-of-Distribution Generalized Drug Synergy Prediction](https://arxiv.org/abs/2605.30247)
...
### [156] [Digitally enriching a screening population for pancreatic cancer using routine blood-based measures and clinical histories](https://arxiv.org/abs/2605.30275)
...
### [157] [Statistical Embeddings for Similarity, Retrieval, and Interpretable Alignment of Numeric Tabular Datasets](https://arxiv.org/abs/2605.30289)
...
### [158] [Self-Trained Verification for Training- and Test-Time Self-Improvement](https://arxiv.org/abs/2605.30290)
...
### [159] [Gram: Assessing sabotage propensities via automated alignment auditing](https://arxiv.org/abs/2605.30322)
...
### [160] [In-Context Reward Adaptation for Robust Preference Modeling](https://arxiv.org/abs/2605.30323)
...
### [161] [Reasoning with Sampling: Cutting at Decision Points](https://arxiv.org/abs/2605.30327)
...
### [162] [SoundnessBench: Can Your AI Scientist Really Tell Good Research Ideas from Bad Ones?](https://arxiv.org/abs/2605.30329)
...
### [163] [When, why, and how do diffusion posterior samplers fail? A finite-sample lens](https://arxiv.org/abs/2605.30330)
...
### [164] [Fairness-Aware Federated Learning with Trajectory Shapley Value](https://arxiv.org/abs/2605.30336)
...
### [165] [Efficient Test-Time Finetuning of LLMs via Convex Reconstruction and Gradient Caching](https://arxiv.org/abs/2605.30337)
...
TL;DR: 提出了一种新的行为诱导Mirror-Prox时间差分方法（STHTD-MP），通过使用行为策略贝尔曼矩阵的对称部分替代传统协方差度量来改善TD学习的几何结构，理论上证明了收敛性，并显示在某些情况下比传统方法具有更好的收敛性能。


<details>
  <summary>Details</summary>
Motivation: 梯度时间差分方法虽然在线性函数逼近下提供了稳定的离策略预测，但其实际性能受到辅助变量度量诱导的几何结构影响。现有的方法通常使用特征协方差度量，但行为策略的转移信息可能提供更丰富的更新几何结构。

Method: 提出了行为诱导的Mirror-Prox时间差分方法（STHTD-MP），将原始-对偶鞍点问题中的协方差度量替换为行为策略贝尔曼矩阵的对称部分。该方法使用单一学习率，并应用Mirror-Prox预测-校正步骤到合成的鞍点算子。

Result: 通过标准随机逼近假设提供了收敛性分析，推导了投影-预言遍历差距界限，并通过确定性Mirror-Prox误差矩阵的谱半径与GTD2-MP进行了精确比较。数值分析表明当行为诱导度量改善鞍点几何时，STHTD-MP可以比GTD2-MP具有更小的平均收缩因子。

Conclusion: 本文提出了STHTD-MP方法来改善TD学习方法中的几何结构，分析表明该方法在某些情况下比GTD2-MP具有更好的收敛性。文章还指出了Baird反例作为一个边界情况，其中严格假设不成立。
...
### [167] [Review Arcade: On the Human Alignment and Gameability of LLM Reviews](https://arxiv.org/abs/2605.28897)
*Hans Ole Hatzel,Sebastian Steindl,Jan Strich*

Main category: cs.AI

TL;DR: 研究对2025年ACL会议的论文进行实证分析，评估LLM生成的评审意见，发现LLM与人类评审者的一致性有限且随提示词和模型变化较大，但作者通过迭代优化利用LLM反馈可以有效提升论文得分。


<details>
  <summary>Details</summary>
Motivation: 随着LLM生成的论文评审在学术界获得认可并被主要学术会议正式试点，需要全面了解LLM在学术评审中的实际影响，不仅评估LLM辅助评审的现状，还需要研究作者如何利用LLM反馈优化论文提交，从而识别潜在的系统性影响。

Method: 对2025年ACL Rolling Review (ARR)的论文进行实证研究，从作者和评审者两个角度评估LLM生成的评审。通过分析LLM评审与人类评审的一致性，探索不同提示词和模型对评审结果的影响，并研究作者采用"草案-修改"迭代工作流程根据LLM反馈优化论文的影响。

Result: LLM评审与人类评审的一致性有限，即使在最佳情况下也只能达到合理水平；LLM-人类评审一致性在不同提示词和模型间存在显著差异；通过迭代优化"游戏"LLM评审过程，在特定场景下可显著提升论文得分，最多能让35%的论文获得统计学上显著的整体分数提升。

Conclusion: LLM生成的论文评审与人类评审者的评审意见存在有限的一致性，且在最佳情况下也只能达到"合理"的一致水平；LLM-人类评审一致性在不同提示词和模型之间存在显著差异；通过迭代优化过程"游戏"LLM评审在特定场景下有效，能够显著提升论文得分。
...
### [168] [Orthogonal Concept Erasure for Diffusion Models](https://arxiv.org/abs/2605.28902)
*Yuhao Sun,Lingyun Yu,Haoxiang Xu,Fengyuan Miao,Zhuoer Xu,Hongtao Xie*

Main category: cs.AI

TL;DR: OCE（正交概念擦除）是一种基于几何视角的乘法参数更新方法，通过层级正交变换实现精确概念擦除，同时保持神经元方向和几何结构，解决了编辑方法中概念擦除与生成能力之间的冲突问题。


<details>
  <summary>Details</summary>
Motivation: 概念擦除对于消除扩散模型中的不良或不安全内容很有前景，但现有方法存在局限性。训练方法计算成本高，编辑方法效率更高但难以同时实现精确概念擦除和保持整体生成能力。

Method: 提出正交概念擦除（OCE），将编辑擦除重新定义为基于几何视角的乘法参数更新。OCE应用从闭式解推导出的层级正交变换，在保持神经元幅度和角度几何的同时实现精确概念擦除。对于多概念擦除的冲突约束，OCE引入子空间级目标并进行结构化子空间操作。

Result: OCE在单概念和多概念擦除实验中优于现有方法，在概念擦除和非目标保护方面表现优异，能够在4.3秒内擦除多达100个概念。

Conclusion: OCE通过乘法参数更新解决了编辑方法中概念擦除与生成性能之间的干扰问题，提供了一种更有效和可扩展的概念擦除方法，在保持模型生成能力的同时实现了精确的概念移除。
...
### [169] [BEAMS: Benchmarking and Evaluating AI for Modeling and Simulation](https://arxiv.org/abs/2605.28994)
*Sara Metcalf,William Schoenberg*

Main category: cs.AI

TL;DR: BEAMS倡议通过建立人类中心化基准和开源sd ai项目，评估AI建模工具在多个测试类别中的表现，发现AI在讨论和定性任务上优于因果推理和定量修复，且不同LLM在不同任务上各有优势。


<details>
  <summary>Details</summary>
Motivation: 支持现实世界决策的AI工具需要建立模拟模型来提供可解释的建议，但此类工具应辅助而非替代人类专业知识。开发负责任和伦理的AI建模工具。

Method: BEAMS倡议通过建立人类中心化建模与仿真的基准，使用开源sd ai项目构建透明的基础设施，实现AI建模工具的评估。技术组以自动化测试的形式实现基准，涵盖因果翻译、模型迭代、因果推理、一致性、模型行为解释、建议建模步骤和建议模型修复等多个类别。

Result: 在评估多个AI工具（支持定性模型构建、定量模型构建和模型讨论）时发现：AI赋能建模工具在讨论和基本定性任务上表现优于因果推理和定量错误修复；不同LLM在不同引擎类型上表现各异，突显了特定任务需求以及速度与准确性之间的权衡。

Conclusion: AI建模工具在讨论和定性任务上表现较好，但在因果推理和定量错误修复方面仍存在局限；不同LLM在不同任务上各有优势，没有单一模型在所有领域都表现最优。
...
TL;DR: 本研究通过分析Copilot用户的对话轨迹，揭示了用户行为随时间变化的模式，发现用户习惯高度稳定且不同活跃度用户差异显著，同时指出WildChat数据集存在代表性偏差。


<details>
  <summary>Details</summary>
Motivation: 现有研究多描述用户与大语言模型(LLM)的交互，但多为静态分析，缺乏对个体用户行为随时间变化的动态了解。

Method: 分析了约12,000名随机抽样的Microsoft Bing Copilot用户的对话轨迹，并将其与WildChat-4.8M数据集进行比较。

Result: 1. Copilot数据存在显著的群体趋势，但个体用户轨迹的趋势较弱，用户习惯具有极强的惯性。2.不同活跃度用户存在明显差异：更活跃的用户拥有更成功的对话，并更多地将LLM用于复杂和专业导向的任务。3.部分用户趋势在WildChat-4.8M中也有体现，但该数据集显著偏向高度熟练的“强力”用户。

Conclusion: 研究得出结论，现有用户行为难以改变，用户异质性显著，且WildChat数据集不能代表典型的用户-人工智能交互，这对该数据集的下游使用提出了重要警告。
...
### [171] [When Models Disagree: Rethinking LLM Evaluation for Public Comment Analysis](https://arxiv.org/abs/2605.29025)
*Aisha Najera,Alvin Moon,Vedant Srinivasan,Rajesh Veeraraghavan*

Main category: cs.AI

TL;DR: 提出了解释性审计管道，将多个LLM模型在分类公开评论时的分歧作为解释复杂性的诊断指标，引导人工审查关注真正模糊的公众输入，以补充传统准确性评估方法。


<details>
  <summary>Details</summary>
Motivation: 联邦机构正在使用大型语言模型对公众评论进行分类，但标准评估方法仅基于小规模验证集的立场准确性，无法检测不同模型对同一公众输入产生实质性不同分类的问题。

Method: 提出解释性审计管道，分析四个LLM对1,260条USDA公众评论的分类分歧；采用两阶段标注研究，让四个LLM和人工标注员独立标注40条分层样本，然后查看他人标签后修订。

Result: 发现模型间的主题分歧超过同一模型内提示变体间的差异；专家评估标准压制了深层解释分歧但未解决；人工标注员的修订经常引入集体输出中不存在的框架；修订行为在不同标注者间存在差异。

Conclusion: 基于分歧的评估方法是LLM辅助解释性编码的必要补充，应将其与准确性指标结合使用，以更好地理解和处理模型在分类公众评论时的解释复杂性。
...
### [172] [Mind Your Tone: Does Tone Alter LLM Performance?](https://arxiv.org/abs/2605.29027)
*Om Dobariya,Akhil Kumar*

Main category: cs.AI

TL;DR: 研究发现大语言模型对提示音调的敏感性高度依赖具体模型，即使对客观选择题也会导致准确性变化，建议用户不应假设LLM部署能抵御音调影响。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型应用的普及，观察到其性能会因提示风格和音调的不同而有所变化。本研究旨在探究音调变化是否以及在多大程度上影响LLMs在客观选择题上的准确性。

Method: 使用了两个数据集进行研究：一个包含50个基础问题和五种音调变体的数据集；另一个是MMLU数据集子集，包含570个基础问题，涵盖57个学科和七种音调变体。实验评估了四种成本效益高且流行的LLMs：ChatGPT-4o、ChatGPT-5-nano、Gemini 2.5 Flash和Gemini 2.5 Flash Lite。

Result: 音调对LLMs准确性的影响是系统性的但高度模型依赖性。有的模型出现了小幅但仍具有统计显著性的准确性变化，而有的模型则在不同音调间表现出大幅度的准确性波动。研究还发现了学科层面的音调敏感性差异，并提出一个路由器框架来解释音调如何调整内部推理模式。

Conclusion: 研究表明LLMs对提示音调变化敏感，即使对客观选择题也存在系统性但高度模型依赖性的影响。不同模型对音调的敏感度差异显著，有些模型出现小幅但统计显著的准确性变化，而有些则出现大幅波动。研究还识别了不同学科领域对音调的敏感性差异，并提出了一个路由器框架来解释音调如何影响内部推理模式。这些发现提醒用户不应假设LLM部署能够抵御音调变化的影响。
...
### [173] [Practitioner Beliefs and Behaviors in AI-Enhanced Education: DOT Framework Survey Evidence](https://arxiv.org/abs/2605.29041)
*David Gibson,M. Elizabeth Azukas,Gerald Knezek*

Main category: cs.AI

TL;DR: 针对72名高等教育从业者的调查表明，他们认可AI的教学支持价值但强调人工监督的重要性；实践中常使用AI生成内容但较少进行系统需求评估；机构政策、培训等支持不足；研究初步验证了设计导向的AI集成框架，并为后续研究提供基础。


<details>
  <summary>Details</summary>
Motivation: 了解高等教育从业者在教学中整合人工智能的信念、行为和面临的机构条件，探索设计导向的AI集成框架的描述性和实用性，为AI在教育中的有效应用提供理论基础。

Method: 采用横断面调查方法（n = 72），基于DOT框架（整合设计思维和开放系统理论），调查高等教育从业者对AI在教学中集成的信念、行为和机构条件。使用探索性因子分析对19个信念项目进行分析，识别因子结构。

Result: 探索性因子分析识别出三个因子结构：AI功能能力（α = .90）、监督与治理、教师合作与规划。结果显示从业者认为AI是有利的教学支持工具，但强调需要人工监督和批判性评估。实践主要集中在迭代提示和内容生成，而对需求评估和反馈循环的使用较少。机构层面的障碍包括政策不完善、培训不足和基础设施有限。

Conclusion: 本研究通过对高等教育从业者的调查，提供了关于人工智能在教学中应用现状的初步经验证据，支持DOT框架作为描述性模型的有效性，同时强调了设计导向理论与当前实践之间的差距，并为未来验证性研究和成果导向研究指明了方向。
...
### [174] [Differentiable Belief-based Opponent Shaping](https://arxiv.org/abs/2605.29042)
*Aarav G Sane,Karthik Sivachandran,Rohan Paleja*

Main category: cs.AI

TL;DR: 提出了D-BOS方法，通过在对手信念空间中进行可微分塑造，让最优策略自然涌现，在隐藏角色游戏中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在多智能体强化学习中，对手塑造尝试复制人类通过策略行动影响他人信念的能力。现有方法通常在对手的参数、策略或价值空间中操作，而隐藏角色游戏中的信念操纵技术通常依赖于硬编码目标（如欺骗或信念饱和）。需要一种直接在信念空间中操作的方法。

Method: D-BOS方法将每个观察者的信念作为被塑造的对手状态，通过k步softmax贝叶斯信念动态进行微分。该方法将信念状态作为塑造目标，而不需要显式奖励欺骗或合作行为，允许最优策略从环境的奖励结构中自然涌现。方法通过微分对手信念更新来提供对手塑造信号，并通过聚合多个观察者的推断信念轨迹梯度来扩展到多观察者场景。

Result: D-BOS在隐藏角色游戏中表现优于PPO和BBM方法，尤其在混合动机场景中获得最大程度的性能提升。

Conclusion: 本文提出了一种基于可微分信念的对手塑造（D-BOS）新方法，该方法通过直接在对手信念空间中进行优化，能够在隐藏角色游戏中学习到自然涌现的最优策略。实验证明该方法在混合动机场景中显著优于现有方法。
...
### [175] [Robust and Efficient Guardrails with Latent Reasoning](https://arxiv.org/abs/2605.29068)
*Siddharth Sai,Xiaofei Wen,Muhao Chen*

Main category: cs.AI

TL;DR: COLAGUARD是一种用于大型语言模型的安全护栏模型，通过将多步安全推理转移到连续潜在空间，实现高效的隐状态传播，在保持安全性能的同时显著提高推理速度和降低token使用量。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在现实应用中的部署增加，确保其安全性变得至关重要。现有的安全护栏通常依赖单次分类或蒸馏推理方法，推理基的护栏性能显著优于仅分类的基线，但会产生较大的查询延迟和token开销，使其不适用于高吞吐量部署。

Method: 提出了COLAGUARD模型，通过阶段式训练课程将多步安全推理转移到连续潜在空间，使模型能够在推理时直接进行隐状态传播。这种方法避免了显式推理生成的开销。

Result: 在跨越八个安全基准的十种提示和响应调节设置上评估，COLAGUARD相比Llama Guard 3的macro-F1提高了8.24个百分点，与显式推理基线GuardReasoner的macro-F1性能匹配，同时实现了12.9倍的速度提升和22.4倍的token使用量减少。

Conclusion: 潜在推理为可部署的安全护栏提供了一种实用的替代方案，能够同时提高安全鲁棒性和推理效率，而不是将这两个目标视为相互竞争的关系。
...
### [176] [Bridging the Sim-to-Real Gap in Reinforcement Learning-Based Industrial Dispatching through Execution Semantics](https://arxiv.org/abs/2605.29078)
...
### [177] [The Importance of Out-of-Band Metadata for Safe Autonomous Agents: The Redpanda Agentic Data Plane](https://arxiv.org/abs/2605.29082)
...
### [178] [Trends in AI and Human-AI Interaction in Clinical Trials -- A Hybrid Human-AI Exploration](https://arxiv.org/abs/2605.29096)
...
### [179] [Beyond Consensus: Trace-Level Synthesis in Mixture of Agents](https://arxiv.org/abs/2605.29116)
*Shreyas Fadnavis,Praitayini Kanakaraj,Felix Wyss*
...
### [180] [PRO-CUA: Process-Reward Optimization for Computer Use Agents](https://arxiv.org/abs/2605.29119)
...
### [182] [Governing Technical Debt in Agentic AI Systems](https://arxiv.org/abs/2605.29129)
...
### [184] [Paper Agents, Paper Gains: An Empirical Analysis of DeFi Investment Agents](https://arxiv.org/abs/2605.29174)
...
### [186] [GTA: Generating Long-Horizon Tasks for Web Agents at Scale](https://arxiv.org/abs/2605.29218)
...
### [187] [BenchTrace: A Benchmark for Testing Reflection Ability and Controlled Evolution in LLM Agents](https://arxiv.org/abs/2605.29225)
...
### [188] [Tailoring the Curriculum: Student-Centered Reasoning Distillation via Dynamic Data-Model Compatibility](https://arxiv.org/abs/2605.29229)
...
### [189] [Rethinking Literature Search Evaluation: Deep Research Helps, and Human Citation Lists Are Not a Ground Truth](https://arxiv.org/abs/2605.29234)
...
### [191] [DenseSteer: Steering Small Language Models towards Dense Math Reasoning](https://arxiv.org/abs/2605.29247)
...
### [192] [Provably Secure Agent Guardrail](https://arxiv.org/abs/2605.29251)
...
### [193] [OpenClawBench: Benchmarking Process-side Anomalies in Real-world Agent Execution Trajectories](https://arxiv.org/abs/2605.29253)
...
### [194] [When and How Human Curation Backfires: Preference Alignment under Multi-Model Self-Consuming Loop](https://arxiv.org/abs/2605.29267)
...
### [195] [Indexing the Unreadable: LLM-Native Recursive Construction and Search of Service Taxonomies](https://arxiv.org/abs/2605.29270)
...
### [197] [Diagnosing Harmful Continuation in Answer-Correct Long-CoT Training Traces](https://arxiv.org/abs/2605.29288)
...
### [198] [Entropy-KL Divergence-based Token Masking: A Novel Approach for Selective Fine-tuning of Large Language Models](https://arxiv.org/abs/2605.29303)
...
### [199] [Rubric-Guided Process Reward for Stepwise Model Routing](https://arxiv.org/abs/2605.29310)
...
### [200] [ConMoE: Expert-Pool Consolidation via Prototype Reassignment for MoE Compression](https://arxiv.org/abs/2605.29350)
...
### [201] [PassNet: Scaling Large Language Models for Graph Compiler Pass Generation](https://arxiv.org/abs/2605.29357)
...
### [202] [Scaling Monosemanticity: Extracting Interpretable Features from Claude 3 Sonnet](https://arxiv.org/abs/2605.29358)
...
### [203] [MiraBench: Evaluating Action-Conditioned Reliability in Robotic World Models](https://arxiv.org/abs/2605.29360)
...
### [204] [EvoMD-LLM: Learning the Language of Species Evolution in Reactive Molecular Dynamics](https://arxiv.org/abs/2605.29394)
...
### [205] [Aligned but Fragile: Enhancing LLM Safety Robustness via Zeroth-Order Optimization](https://arxiv.org/abs/2605.29396)
...
### [206] [Architecture-Sensitive Supervised Fine-Tuning for Screen-Conditioned Action Prediction: A PiSAR Benchmark](https://arxiv.org/abs/2605.29400)
...
### [207] [When Does Persona Prompting Actually Help? A Retrieval and Metric Analysis of Expert Role Injection in LLMs](https://arxiv.org/abs/2605.29420)
...
### [208] [ReasonLight: A Multimodal Foundation Model-Enhanced Reinforcement Learning Framework for Zero-Shot Traffic Signal Control](https://arxiv.org/abs/2605.29425)
...
### [209] [Towards Human-Like Interactive Speech Recognition With Agentic Correction and Semantic Evaluation](https://arxiv.org/abs/2605.29430)
...
### [210] [CrystalXRD-Bench: Benchmarking Vision-Language Models for XRD Peak Indexing Across Diverse Crystalline Materials](https://arxiv.org/abs/2605.29446)
...
### [211] [VitalAgent: A Tool-Augmented Agent for Reactive and Proactive Physiological Monitoring over Wearable Health Data](https://arxiv.org/abs/2605.29483)
...
### [212] [The Curse of Helpfulness: Inverse Scaling Law in Robustness to Distractor Instructions via DistractionIF](https://arxiv.org/abs/2605.29491)
...
### [213] [Xetrieval: Mechanistically Explaining Dense Retrieval](https://arxiv.org/abs/2605.29507)
...
### [214] [MINDGAMES: A Live Arena for Evaluating Social and Strategic Reasoning in Multi-Agent LLMs](https://arxiv.org/abs/2605.29512)
...
### [215] [DeepSurvey: Enhancing Analytical Depth and Citation Reliability in Automated Survey Generation](https://arxiv.org/abs/2605.29522)
...
### [216] [UI-KOBE: Knowledge-Oriented Behavior Exploration for Lightweight Graph-Guided GUI Agents](https://arxiv.org/abs/2605.29534)
...
### [217] [Opt-Verifier: Unleashing the Power of LLMs for Optimization Modeling via Dual-Side Verification](https://arxiv.org/abs/2605.29556)
...
### [218] [Battery-Sim-Agent: Leveraging LLM-Agent for Inverse Battery Parameter Estimation](https://arxiv.org/abs/2605.29560)
...
### [219] [ParaTool: Shifting Tool Representations from Context to Parameters](https://arxiv.org/abs/2605.29561)
...
### [220] [Planning with the Views via Scene Self-Exploration](https://arxiv.org/abs/2605.29563)
...
### [221] [DeepTool: Scaling Interleaved Deliberation in Tool-Integrated Reasoning via Process-Supervised Reinforcement Learning](https://arxiv.org/abs/2605.29568)
...
### [222] [GPS-Enhanced Tourist Mobility Modeling with Seasonal Spatial Priors and LLM-Based Activity Chain Generation](https://arxiv.org/abs/2605.29578)
...
### [223] [FinVerBench: Benchmark Validity and Calibration in Large Language Model Financial Statement Verification](https://arxiv.org/abs/2605.29586)
*Silu Panda*
...
### [224] [Mind-Omni: A Unified Multi-Task Framework for Brain-Vision-Language Modeling via Discrete Diffusion](https://arxiv.org/abs/2605.29591)
...
### [225] [HiKEY: Hierarchical Multimodal Retrieval for Open-Domain Document Question Answering](https://arxiv.org/abs/2605.29606)
...
### [226] [Improving Collaborative Storytelling with a Multi-Agent Framework Based on Large Language Models](https://arxiv.org/abs/2605.29625)
...
### [227] [Beyond Attack Success Rate: Temporal Logit Observability for LLM Safety Failures](https://arxiv.org/abs/2605.29629)
...
### [228] [VikingMem: A Memory Base Management System for Stateful LLM-based Applications](https://arxiv.org/abs/2605.29640)
...
### [229] [LLM-Evolved Domain-Independent Heuristics for Symbolic AI Planning](https://arxiv.org/abs/2605.29649)
...
### [230] [Think Fast, Talk Smart: Partitioning Deterministic and Neural Computation for Structured Health Text Generation](https://arxiv.org/abs/2605.29652)
...
### [231] [PTCG-Bench: Can LLM Agents Master Pokémon Trading Card Game?](https://arxiv.org/abs/2605.29653)
...
### [232] [TRACE: Toulmin-based Reasoning Assessment through Constructive Elements for LLM CoT Evaluation](https://arxiv.org/abs/2605.29656)
...
### [233] [GRASP: Gated Regression-Aware Skill Proposer for Self-Improving LLM Agents](https://arxiv.org/abs/2605.29668)
...
### [234] [NICE: A Theory-Grounded Diagnostic Benchmark for Social Intelligence of LLMs](https://arxiv.org/abs/2605.29685)
...
### [235] [Reliable Reasoning with Large Language Models via Preference-Based Maximum Satisfiability](https://arxiv.org/abs/2605.29687)
...
### [237] [Beyond Trajectory Rewards: Step-level Credit Assignment for Agentic Search via Graph Modeling](https://arxiv.org/abs/2605.29697)
...
### [238] [BitTP: The Lightweight Trajectory Prediction Model with BitLLM for Edge-Devices](https://arxiv.org/abs/2605.29705)
...
### [239] [NaRA: Noise-Aware LoRA for Parameter-Efficient Fine-Tuning of Diffusion LLMs](https://arxiv.org/abs/2605.29716)
...
### [240] [Uncertainty-Aware Transfer Learning for Cross-Building Energy Forecasting: Toward Robust and Scalable District-Level Energy Management](https://arxiv.org/abs/2605.29733)
...
### [241] [Citation-Closure Retrieval and Per-Rule Attribution for Real-World Regulatory Compliance Question Answering](https://arxiv.org/abs/2605.29742)
...
### [242] [Why Specialist Models Still Matter: A Heterogeneous Multi-Agent Paradigm for Medical Artificial Intelligence](https://arxiv.org/abs/2605.29744)
...
### [243] [Benchmarking Positional Encoding Strategies for Transformer-Based EEG Foundation Models](https://arxiv.org/abs/2605.29754)
...
### [244] [LFQ: Logit-aware Final-block Quantization for Boosting the Generation Quality of Low-Bit Quantized LLMs](https://arxiv.org/abs/2605.29756)
...
### [245] [From XXLTraffic to EvoXXLTraffic: Scaling Traffic Forecasting to Sensor-Evolving Networks](https://arxiv.org/abs/2605.29768)
...
### [247] [Certified Policy Optimisation for Nested Causal Bandits via PAC-Bayes Risk](https://arxiv.org/abs/2605.29788)
...
### [248] [SkillsInjector: Dynamic Skill Context Construction for LLM Agents](https://arxiv.org/abs/2605.29794)
...
### [249] [MEMENTO: Leveraging Web as a Learning Signal for Low-Data Domains](https://arxiv.org/abs/2605.29795)
...
### [253] [Harnessing non-adversarial robustness in large language models](https://arxiv.org/abs/2605.29816)
...
### [254] [Quantifying and Optimizing Simplicity via Polynomial Representations](https://arxiv.org/abs/2605.29823)
...
### [259] [On the Geometry of Games and their Solvers](https://arxiv.org/abs/2605.29919)
...
### [260] [Toward AI Systems That Understand Self and Others: A Multi-Phase Inference Framework for Human Cognitive Diversity and World-Model Alignment](https://arxiv.org/abs/2605.29930)
*Toru Takahashi*
...
### [261] [It`s All About Speed: AI`s Impact on Workflow in Music Production](https://arxiv.org/abs/2605.29931)
...
### [262] [Make LLM Learn to Synthesize from Streaming Experiences through Feedback](https://arxiv.org/abs/2605.29940)
...
### [263] [MuPHI: Learning Implicit Multimodal Harm Reasoning via Semantically Grounded Reward Optimization](https://arxiv.org/abs/2605.29951)
...
### [264] [Formalizing Mathematics at Scale](https://arxiv.org/abs/2605.29955)
...
### [265] [Meta-Programming for Linear-time Temporal Answer Set Programming](https://arxiv.org/abs/2605.29965)
...
### [266] [Compass: Navigating Global Marine Lead Data Integration through Expert-Guided LLM Agent](https://arxiv.org/abs/2605.29966)
...
### [267] [Accelerating Constrained Decoding with Token Space Compression](https://arxiv.org/abs/2605.29986)
...
### [268] [Cookie-Bench: Continuous On-screen Key Interaction Evaluation for Web Generation](https://arxiv.org/abs/2605.30000)
...
### [269] [KairosAgent: Agentic Time Series Forecasting with Fused Semantic Reasoning](https://arxiv.org/abs/2605.30002)
...
### [270] [From GPS Points to Travel Patterns: Flexible and Semantic Trajectory Generation with LLMs](https://arxiv.org/abs/2605.30014)
...
### [271] [RAISE: RAG Design as an Architecture Search Problem](https://arxiv.org/abs/2605.30029)
...
### [272] [Teaching Values to Machines: Simulating Human-Like Behavior in LLMs](https://arxiv.org/abs/2605.30036)
...
### [273] [Domain-Specific Data Synthesis for LLMs via Minimal Sufficient Representation Learning](https://arxiv.org/abs/2605.30039)
...
### [274] [Learning to Choose: An Empowerment-Guided Multi-Agent System with semantic communication for Adaptive Method Selection](https://arxiv.org/abs/2605.30042)
...
### [275] [Robust and Generalizable Safety Steering for Text-to-Image Diffusion Transformers](https://arxiv.org/abs/2605.30049)
...
### [276] [Conformal Certification of Reasoning Trace Prefixes](https://arxiv.org/abs/2605.30085)
...
### [277] [Selective QA over Conflicting Multi-Source Personal Memory: A Diagnostic Testbed and Method Comparison](https://arxiv.org/abs/2605.30087)
...
### [278] [PokerSkill: LLMs Can Play Expert-Level Poker without Training or Solvers](https://arxiv.org/abs/2605.30094)
...
### [279] [VLA-Trace: Diagnosing Vision-Language-Action Models through Representation and Behavior Tracing](https://arxiv.org/abs/2605.30117)
...
### [280] [Enhancing Multi-Agent Communication through Attention Steering with Context Relevance](https://arxiv.org/abs/2605.30136)
...
### [281] [AgentSchool: An LLM-Powered Multi-Agent Simulation for Education](https://arxiv.org/abs/2605.30144)
...
### [282] [Anchorless Diversification for Parallel LLM Ideation](https://arxiv.org/abs/2605.30150)
...
### [283] [Temporal Stability and Few-Shot Prompting in Math Task Assessment](https://arxiv.org/abs/2605.30151)
...
### [284] [Meta-Cognitive Memory Policy Optimization for Long-Horizon LLM Agents](https://arxiv.org/abs/2605.30159)
...
### [285] [BioRefusalAudit: Auditing Biosecurity Refusal Depth Using General and Domain-Fine-Tuned Sparse Autoencoders](https://arxiv.org/abs/2605.30162)
*Caleb DeLeeuw*
...
### [286] [Modularizing Educational LLM-Agency for Fostering Responsible Learning Assistance](https://arxiv.org/abs/2605.30187)
...
### [287] [Double-Edged Sword or Sharp Tool? Designing and Evaluating Triadic LLM-Teacher Collaboration for K-12 Writing at Scale](https://arxiv.org/abs/2605.30200)
...
### [288] [When Should Models Change Their Minds? Contextual Belief Management in Large Language Models](https://arxiv.org/abs/2605.30219)
...
### [289] [mcp-proto-okn: Natural-language access to open scientific knowledge graphs through the Model Context Protocol](https://arxiv.org/abs/2605.30283)
...
### [290] [ProjectionBench: Evaluating Scientific Hypothesis Generation in LLMs Under Progressive Information Disclosure](https://arxiv.org/abs/2605.30284)
...
### [291] [MIRA: Mid-training Rubric Anchoring for Source-Aware Data Selection](https://arxiv.org/abs/2605.30288)
...
### [292] [Demystifying Data Organization for Enhanced LLM Training](https://arxiv.org/abs/2605.30334)
...
### [293] [Locally Coherent, Globally Incoherent: Bounding Compositional Incoherence in Multi-Component LLM Agents](https://arxiv.org/abs/2605.30335)
*Anany Kotawala*
...
### [294] [Tiny but Trusted: Efficient Vision-Language Reasoning for Time-Series Anomaly Detection](https://arxiv.org/abs/2605.30344)
...
### [295] [SchGen: PCB Schematic Generation with Semantic-Grounded Code Representations](https://arxiv.org/abs/2605.30345)
