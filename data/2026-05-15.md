...
TL;DR: 本文提出了一种在部分可观测环境下从视觉进行ptSTL认证运行时监控的可复用框架，证明了语义基的最优性，并比较了两种监控器在不同时域下的性能表现。


<details>
  <summary>Details</summary>
Motivation: 研究在部分可观测环境下，从视觉观测中进行过去时信号时序逻辑（ptSTL）的认证运行时监控。监控器需要从图像中推断安全相关量并提供有限样本保证，同时实现可复用性：一旦训练和校准后，应能认证目标片段中的任何公式，无需针对每个公式重新训练。

Method: 1. 提出了两种监控方法：语义基监控器和滚动预测监控器。2. 对于由有限字典时态原子诱导的片段，证明了语义基是单调、1-Lipschitz可复用接口类中的最小预测目标。3. 使用单次整合校准流程，无需联合边界，即可对整个片段进行认证。4. 在Waymo真实驾驶数据集上进行验证。

Result: 1. 证明了语义基在理论上是最小预测目标，允许通过确定性解码器评估任何公式。2. 在行人路口基准测试中，滚动监控在短时域上获得更紧密的认证边界，而语义基监控在长时域上最多可提高4倍的紧密度。3. 在Waymo真实驾驶数据上验证了两类监控器均满足整合覆盖保证。

Conclusion: 本文论证了使用语义基作为监控目标在理论上是最优的，并验证了两种监控器在不同场景下的实际效果，为部分可观测环境下具有有限样本保证的可复用运行时监控提供了实用解决方案。
...
TL;DR: 本研究应用稀疏自编码器分析EEG基础模型内部特征，建立可解释性框架，发现模型表示缺陷并通过频率特征解释机制。


<details>
  <summary>Details</summary>
Motivation: EEG基础模型虽在临床性能上达到最先进水平，但其内部计算过程不透明，阻碍了临床信任的建立。需要开发可解释性方法来理解模型预测背后的机制。

Method: 应用TopK稀疏自编码器（SAEs）跨三种架构不同的EEG transformer模型（SleepFM、REVE、LaBraM）提取嵌入层的稀疏特征词典，通过临床分类标准进行特征基准测试，引入概念导向和"目标与脱靶"探测区域指标量化导向选择性，并通过频谱解码器将干预映射回幅值谱。

Result: 发现三个操作模式：可选择性导向、有编码但纠缠、无编码。揭示了关键表示缺陷："破坏性干预"导致全局模型性能崩溃，临床概念纠缠（如年龄-病理混淆）使得抑制一个概念而不影响另一个变得不可能。通过频谱解码器将潜在干预转化为生理可解释的频率特征。

Conclusion: 本研究建立了一个可解释性框架，揭示了EEG基础模型的内部表示结构，发现了关键的表示缺陷（如"破坏性干预"和临床概念纠缠），并为临床应用提供了机制层面的理解。
...
TL;DR: 该论文提出了一个无监督框架，通过架构退火来平衡离散和连续潜在变量，有效分离dMRI数据中的采集变异和生物学变异，在包含7,416个连接组的数据集上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: dMRI数据的多站点、多扫描仪和多协议采集差异引入了变异，使结构连接组分析复杂化。现有降维方法将所有方差建模为连续变量，导致采集效应被吸收到连续潜在空间中。虽有混合潜在空间模型结合离散和连续组件，但通常需要手动调整以确保离散组件捕捉预期变异。

Method: 提出了一个无监督框架，通过架构退火（encoder输出在解码前进行退火）来平衡离散和连续潜在变量，无需手动调整容量。构建了一个包含7,416个结构连接组的dMRI数据集，比较了标准VAE、PCA加k-means聚类、仅通过损失函数退火的混合模型等基线方法。

Result: 架构退火在站点学习方面表现更强（ARI=0.53，p

Conclusion: 该研究提出了一种通过架构退火而非损失函数退火的混合连续-离散潜在空间方法，能够有效捕捉dMRI数据中的采集变异，为消除多站点、多扫描仪和多协议影响提供了有效的无监督机制。架构退火方法在站点学习方面表现优于传统基线模型。
...
### [4] [Towards the Next Frontier of LLMs, Training on Private Data: A Cross-Domain Benchmark for Federated Fine-Tuning](https://arxiv.org/abs/2605.13936)
*Daniel M. Jimenez-Gutierrez,Enrique Zuazua,Georgios Kellaris,Joaquin del Rio,Oleksii Sliusarenko,Xabi Uribe-Etxebarria*

Main category: cs.LG

TL;DR: 本研究提出了一种通过联邦协作在医疗和金融等私有数据领域适应大型语言模型的方法。评估结果表明，联邦微调表现接近集中训练，优于孤立学习，为无法共享的私有数据提供了实用解决方案。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的发展主要依赖公共数据集，但最具价值的信息往往存在于私有领域，特别是在医疗、金融等高度监管的行业。这些数据由于隐私、监管和组织壁垒而无法共享，且分布式机构数据集通常是非独立同分布的。解锁这些私有分布式数据能够实现LLM的重要进步，提供更深的领域专业知识和更强的实际效用。

Method: 基于Sherpa.ai联邦学习平台构建框架，使各节点能够在不交换私有数据的情况下联合微调共享LLM。使用四种数据集（MedQA、MedMCQA、FPB、FiQA-SA）建立跨医疗和金融领域的基准测试，比较了LoRA、QLoRA和IA3三种参数高效微调策略在不同预训练主干上的表现。这些策略在反映机构数据异质性的非IID设置下进行评估。

Result: 联邦微调的表现接近集中式训练，且优于孤立的单一机构学习。从绿色AI角度来看，QLoRA和IA3在有限精度损失下提高了效率，支持联邦PEFT作为适应无法共享数据的LLM的可行方法。

Conclusion: 本研究通过联邦协作的方式，为无法共享的私有分布数据提供了一种实用的LLM适应方法。在医疗和金融领域的评估结果表明，联邦微调表现接近集中式训练，优于孤立学习。QLoRA和IA3在有限精度损失下提升了效率，联邦PEFT成为适应无法共享数据的LLM的可行方法。
...
### [5] [EvolveMem:Self-Evolving Memory Architecture via AutoResearch for LLM Agents](https://arxiv.org/abs/2605.13941)
*Jiaqi Liu,Xinyu Ye,Peng Xia,Zeyu Zheng,Cihang Xie,Mingyu Ding,Huaxiu Yao*

Main category: cs.LG

TL;DR: EvolveMem是一种自演进内存架构，将检索配置作为结构化动作空间，通过LLM驱动诊断和监护元分析器实现闭环自演进，能够自主改进检索机制，在多个基准测试中显著优于现有系统。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体的内存系统存在局限性：存储内容虽然在演化，但检索基础设施（包括评分函数、融合策略和答案生成策略）在部署后保持冻结。作者认为，真正自适应的内存需要存储知识和检索机制两个层面的协同演进，而现有系统无法实现这一点。

Method: EvolveMem采用自演进内存架构，关键创新是将完整的检索配置（包括评分函数、融合策略和答案生成策略）暴露为结构化动作空间，由一个LLM驱动的诊断模块进行优化。系统包含两个核心组件：诊断模块读取每个问题的失败日志，识别根本原因并针对性提出配置调整；监护元分析器应用这些调整，具备自动回归回滚和停滞探索的安全机制。这种闭环自演进实现了一个AutoResearch过程，系统自主进行架构上的迭代研究循环。

Result: 在LoCoMo基准测试中，EvolveMem比最强基准模型相对提升了25.7%，比最小基线相对提升了78.0%。在MemBench基准测试中，EvolveMem比最强基准模型相对提升了18.9%。更重要的是，演进后的配置能够在不同基准测试间实现正迁移而非灾难性迁移，表明自演进过程捕捉到了通用的检索原则而非特定基准的启发式方法。

Conclusion: EvolveMem作为一种自演进内存架构，通过将检索配置作为结构化动作空间进行优化，能够自主地发现有效的检索策略，包括原始动作空间中不存在的新配置维度。该架构通过正迁移而非灾难性迁移证明了其学习到的是通用检索原则而非特定基准的启发式方法，为LLM智能体提供了真正自适应、可自主改进的长期记忆系统。
...
TL;DR: 本文通过将对比学习目标建模为距离几何问题，揭示了不同加权方案如何决定了嵌入的几何结构，发现在监督分类中，类别不平衡会影响嵌入对称性，而在连续标签场景中，y-Aware CL通常存在几何不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 对比学习通常旨在通过反映样本间相似性的表示来保持关系结构，但人们对所学嵌入的几何结构理解不足。本文旨在通过对对比学习目标的几何分析，提供一个系统框架，帮助我们理解对比表示是如何构成的。

Method: 作者将加权的InfoNCE目标解释为距离几何问题，其中加权方案指定嵌入需要实现的几何结构。通过对不同监督和弱监督目标的分析，提供了优化嵌入的精确表征。

Result: 研究发现：在监督分类中，SupCon和Soft SupCon都会将每个类别中的样本压缩为单一原型。平衡的SupCon恢复了经典的规则几何结构，但类别不平衡会打破这种对称性，导致类别间的相似性不均匀。在连续标签设置中，y-Aware CL通常无法达到其熵优化目标，除非标签位于超球面上，这表明欧氏标签权重与球形潜在相似性之间存在不匹配。

Conclusion: 对比学习的目标函数权重选择决定了学习嵌入的几何结构，如果权重设计不合理，可能会导致几何不可实现、模型退化或不一致的问题。相比之下，几何一致的加权方案（如Euclidean-Euclidean或X-CLR）能够产生独特的优化嵌入，这为设计对比学习目标提供了原理性框架。
...
TL;DR: 研究发现用基于近期动作历史的简单先验WarmPrior替代标准高斯分布，能显著改善生成式机器人控制算法的性能。


<details>
  <summary>Details</summary>
Motivation: 虽然基于扩散模型和流匹配的生成式策略已成为机器人视觉运动控制的主导范式，但研究者注意到标准的Gaussian源分布并非最优选择，可能存在改进空间。通过探索更好的源分布来提升算法性能是主要动机。

Method: 提出了WarmPrior方法，这是一种简单的时间基先验分布，利用已有的近期动作历史信息替代标准的多元正态分布作为源分布。该方法应用于基于扩散模型和流匹配的生成式机器人控制策略。

Result: WarmPrior相比标准Gaussian源分布，在机器人操作任务上能持续提高成功率。这归因于显著更直的概率路径，类似于整流流中的最优传输耦合效果。该先验分布还能重塑先验空间强化学习的探索分布，同时提升采样效率和最终性能。

Conclusion: 该研究发现将标准高斯源分布替换为基于近期动作历史构建的简单时间基先验分布（WarmPrior）能够显著提升生成式机器人控制算法的性能。这揭示了源分布是生成式机器人控制中一个重要且未充分探索的设计维度，为未来算法改进提供了新的方向。
...
TL;DR: 该论文提出一个完整能量核算框架，测量蒸馏管道中常被忽略的成本（如教师端数据生成和评估），构建能量-质量Pareto前沿，并制定能源约束下的蒸馏设计规则。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型部署的增加导致GPU需求激增和数据中心扩展，引发了人们对电力使用、电网压力以及现代AI工作负载影响的担忧。蒸馏通常被认为是获得更便宜、更高效模型的最有效路径之一，但这些主张很少考虑到完整的端到端能源和资源成本，包括关键的教学端工作负载（如数据生成、logit缓存和评估）。

Method: 提出了一种综合能量核算框架，通过细致跟踪GPU设备功耗来测量蒸馏管道的完整计算成本。实验中将能量使用按照不同阶段分开记录，系统测量了两种常见蒸馏方法（经典的基于logit的知识蒸馏和合成数据监督微调）的能量消耗和排放，并构建了能量-质量Pareto前沿，揭示了传统评估中忽视的成本。

Result: 通过实验测量，构建了能量-质量Pareto前沿，暴露了传统评估中忽略的蒸馏管道完整能量成本。研究显示，需要重新评估蒸馏作为高效方法的说法，因为教师端的计算开销可能很大，特别是在复杂的蒸馏设置中。

Conclusion: 论文提出了一套完整的能量核算框架，通过标准化测量工具和协议为可比较、可复现的蒸馏研究提供基础。作者基于能源-质量Pareto前沿的分析，提出了在能源和预算约束下选择蒸馏方法和超参数的实际设计规则。
...
### [9] [TabPFN-3: Technical Report](https://arxiv.org/abs/2605.13986)
...
TL;DR: TabPFN-3是一个完全在合成数据上预训练的表格基础模型，将其扩展到处理100万行训练数据，显著减少训练和推理时间，在TabArena基准测试中超越所有其他模型，并在时间序列、关系型和表格-文本数据上实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 推动表格预测基础模型的发展，将其扩展到处理更大规模数据集（100万训练行），并显著减少训练和推理时间，同时扩展模型在时间序列、关系型和表格-文本数据等多样化数据集上的能力。

Method: TabPFN-3基于反馈设计，在合成数据上预训练，引入测试时计算扩展，通过TabPFN-3-Plus（思考）API利用此功能，并采用减少的KV缓存和分块处理技术。

Result: 在TabArena基准测试中，TabPFN-3的前向传播显著优于所有其他模型；在处理多样性数据集时，在多类别数据集上排名第一；在100万行训练数据和200个特征的数据集上，击败了经过8小时调优的梯度提升树基线；TabPFN-3-Plus在TabArena上超越所有非TabPFN模型200多Elo，最大数据子集达到420Elo；比TabPFN-2.5快最多20倍；减少KV缓存和行分块可在单个H100上扩展到100万行数据。

Conclusion: TabPFN-3通过完全在合成数据上预训练，显著提升了表格预测的性能边界，在TabArena基准测试中超越所有其他模型，并在时间序列、关系型和表格-文本数据上都带来了实质性提升，同时大幅提高了训练和推理速度。
...
### [11] [Support Before Frequency in Discrete Diffusion](https://arxiv.org/abs/2605.13999)
...
### [12] [Dywave: Event-Aligned Dynamic Tokenization for Heterogeneous IoT Sensing Signal](https://arxiv.org/abs/2605.14014)
...
### [13] [R2R2: Robust Representation for Intensive Experience Reuse via Redundancy Reduction in Self-Predictive Learning](https://arxiv.org/abs/2605.14026)
...
### [15] [Reliability-Gated Source Anchoring for Continual Test-Time Adaptation](https://arxiv.org/abs/2605.14063)
...
### [16] [SurF: A Generative Model for Multivariate Irregular Time Series Forecasting](https://arxiv.org/abs/2605.14069)
...
### [17] [AttnGen: Attention-Guided Saliency Learning for Interpretable Genomic Sequence Classification](https://arxiv.org/abs/2605.14073)
...
### [20] [Mini-JEPA Foundation Model Fleet Enables Agentic Hydrologic Intelligence](https://arxiv.org/abs/2605.14120)
*Mashrekur Rahman*
...
### [21] [Reinforcement Learning for Tool-Calling Agents in Fast Healthcare Interoperability Resources (FHIR)](https://arxiv.org/abs/2605.14126)
...
### [22] [bde: A Python Package for Bayesian Deep Ensembles via MILE](https://arxiv.org/abs/2605.14146)
...
### [23] [Uncovering Trajectory and Topological Signatures in Multimodal Pediatric Sleep Embeddings](https://arxiv.org/abs/2605.14156)
*Scott Ye,Harlin Lee*
...
### [24] [Finite Sample Bounds for Learning with Score Matching](https://arxiv.org/abs/2605.14168)
...
### [25] [CSI-JEPA: Towards Foundation Representations for Ubiquitous Sensing with Minimal Supervision](https://arxiv.org/abs/2605.14171)
...
### [26] [LLMs Know When They Know, but Do Not Act on It: A Metacognitive Harness for Test-time Scaling](https://arxiv.org/abs/2605.14186)
...
### [27] [How to Scale Mixture-of-Experts: From muP to the Maximally Scale-Stable Parameterization](https://arxiv.org/abs/2605.14200)
...
### [28] [Towards Fine-Grained and Verifiable Concept Bottleneck Models](https://arxiv.org/abs/2605.14210)
...
### [31] [DT-Transformer: A Foundation Model for Disease Trajectory Prediction on a Real-world Health System](https://arxiv.org/abs/2605.14227)
...
### [32] [AudioMosaic: Contrastive Masked Audio Representation Learning](https://arxiv.org/abs/2605.14231)
...
### [33] [Quantum Advantage in Multi Agent Reinforcement Learning](https://arxiv.org/abs/2605.14235)
...
### [34] [Active Learners as Efficient PRP Rerankers](https://arxiv.org/abs/2605.14236)
...
### [35] [Paraphrasing Attack Resilience of Various AI-Generated Text Detection Methods](https://arxiv.org/abs/2605.14240)
...
### [36] [Latency-Quality Routing for Functionally Equivalent Tools in LLM Agents](https://arxiv.org/abs/2605.14241)
...
### [38] [Action-Conditioned Risk Gating for Safety-Critical Control under Partial Observability](https://arxiv.org/abs/2605.14246)
...
### [39] [Not All Timesteps Matter Equally: Selective Alignment Knowledge Distillation for Spiking Neural Networks](https://arxiv.org/abs/2605.14252)
...
### [40] [Architecture-Aware Explanation Auditing for Industrial Visual Inspection](https://arxiv.org/abs/2605.14255)
...
### [41] [Dynamics of the Transformer Residual Stream: Coupling Spectral Geometry to Network Topology](https://arxiv.org/abs/2605.14258)
...
### [42] [TILT: Target-induced loss tilting under covariate shift](https://arxiv.org/abs/2605.14280)
...
### [44] [MetaMoE: Diversity-Aware Proxy Selection for Privacy-Preserving Mixture-of-Experts Unification](https://arxiv.org/abs/2605.14289)
...
### [45] [Minimal-Intervention KV Retention: A Design-Space Study and a Diversity-Penalty Survivor](https://arxiv.org/abs/2605.14292)
...
### [46] [Policy Optimization in Hybrid Discrete-Continuous Action Spaces via Mixed Gradients](https://arxiv.org/abs/2605.14297)
...
### [48] [Matrix-Space Reinforcement Learning for Reusing Local Transition Geometry](https://arxiv.org/abs/2605.14304)
...
### [49] [Beyond Binary: Reframing GUI Critique as Continuous Semantic Alignment](https://arxiv.org/abs/2605.14311)
...
### [50] [Guided Diffusion Sampling for Precipitation Forecast Interventions](https://arxiv.org/abs/2605.14317)
...
### [51] [Dynamic Latent Routing](https://arxiv.org/abs/2605.14323)
...
### [52] [AIM-DDI: A Model-Agnostic Multimodal Integration Module for Drug-Drug Interaction Prediction](https://arxiv.org/abs/2605.14327)
...
### [53] [Nearest-Neighbor Radii under Dependent Sampling](https://arxiv.org/abs/2605.14343)
...
### [54] [Exemplar Partitioning for Mechanistic Interpretability](https://arxiv.org/abs/2605.14347)
*Jessica Rumbelow*
...
### [55] [Distributionally Robust Multi-Task Reinforcement Learning via Adaptive Task Sampling](https://arxiv.org/abs/2605.14350)
...
### [56] [RQ-MoE: Residual Quantization via Mixture of Experts for Efficient Input-Dependent Vector Compression](https://arxiv.org/abs/2605.14359)
...
### [57] [MoRe: Modular Representations for Principled Continual Representation Learning on Squantial Data](https://arxiv.org/abs/2605.14364)
...
### [58] [LoMETab: Beyond Rank-1 Ensembles for Tabular Deep Learning](https://arxiv.org/abs/2605.14365)
...
### [59] [Turning Stale Gradients into Stable Gradients: Coherent Coordinate Descent with Implicit Landscape Smoothing for Lightweight Zeroth-Order Optimization](https://arxiv.org/abs/2605.14373)
...
### [60] [Optimal Pattern Detection Tree for Symbolic Rule-Based Classification](https://arxiv.org/abs/2605.14374)
...
### [61] [Data-Augmented Game Starts for Accelerating Self-Play Exploration in Imperfect Information Games](https://arxiv.org/abs/2605.14379)
...
### [62] [NodeSynth: Socially Aligned Synthetic Data for AI Evaluation](https://arxiv.org/abs/2605.14381)
...
### [63] [Watch your neighbors: Training statistically accurate chaotic systems with local phase space information](https://arxiv.org/abs/2605.14405)
...
### [64] [GeoViSTA: Geospatial Vision-Tabular Transformer for Multimodal Environment Representation](https://arxiv.org/abs/2605.14406)
...
### [65] [MahaVar: OOD Detection via Class-wise Mahalanobis Distance Variance under Neural Collapse](https://arxiv.org/abs/2605.14413)
...
### [66] [What if Tomorrow is the World Cup Final? Counterfactual Time Series Forecasting with Textual Conditions](https://arxiv.org/abs/2605.14422)
...
### [67] [Collaborative Yet Personalized Policy Training: Single-Timescale Federated Actor-Critic](https://arxiv.org/abs/2605.14423)
...
### [68] [FrontierSmith: Synthesizing Open-Ended Coding Problems at Scale](https://arxiv.org/abs/2605.14445)
...
### [69] [When Answers Stray from Questions: Hallucination Detection via Question-Answer Orthogonal Decomposition](https://arxiv.org/abs/2605.14449)
...
### [70] [LiSA: Lifelong Safety Adaptation via Conservative Policy Induction](https://arxiv.org/abs/2605.14454)
...
### [71] [Focused PU learning from imbalanced data](https://arxiv.org/abs/2605.14467)
...
### [72] [Test-Time Learning with an Evolving Library](https://arxiv.org/abs/2605.14477)
...
### [73] [A Novel Schur-Decomposition-Based Weight Projection Method for Stable State-Space Neural-Network Architectures](https://arxiv.org/abs/2605.14489)
...
### [74] [ROAD: Adaptive Data Mixing for Offline-to-Online Reinforcement Learning via Bi-Level Optimization](https://arxiv.org/abs/2605.14497)
...
### [75] [Enjoy Your Layer Normalization with the Computational Efficiency of RMSNorm](https://arxiv.org/abs/2605.14521)
...
### [76] [Lang2MLIP: End-to-End Language-to-Machine Learning Interatomic Potential Development with Autonomous Agentic Workflows](https://arxiv.org/abs/2605.14527)
...
### [77] [Exploring Geographic Relative Space in Large Language Models through Activation Patching](https://arxiv.org/abs/2605.14535)
...
### [78] [RxEval: A Prescription-Level Benchmark for Evaluating LLM Medication Recommendation](https://arxiv.org/abs/2605.14543)
...
### [79] [Discovering Physical Directions in Weight Space: Composing Neural PDE Experts](https://arxiv.org/abs/2605.14546)
...
### [80] [Multi-Dimensional Model Integrity and Responsibility Assessment Index and Scoring Framework](https://arxiv.org/abs/2605.14550)
...
### [81] [SeesawNet: Towards Non-stationary Time Series Forecasting with Balanced Modeling of Common and Specific Dependencies](https://arxiv.org/abs/2605.14551)
...
### [82] [Efficient Multi-objective Prompt Optimization via Pure-exploration Bandits](https://arxiv.org/abs/2605.14553)
...
### [83] [Resolving Action Bottleneck: Agentic Reinforcement Learning Informed by Token-Level Energy](https://arxiv.org/abs/2605.14558)
...
### [84] [Woodelf++: A Fast and Unified Partial Dependence Plot Algorithm for Decision Tree Ensembles](https://arxiv.org/abs/2605.14578)
...
### [85] [Angel or Demon: Investigating the Plasticity Interventions' Impact on Backdoor Threats in Deep Reinforcement Learning](https://arxiv.org/abs/2605.14587)
...
### [86] [Silent Collapse in Recursive Learning Systems](https://arxiv.org/abs/2605.14588)
*Zhipeng Zhang*
...
### [87] [Fast Rates for Inverse Reinforcement Learning](https://arxiv.org/abs/2605.14599)
...
### [88] [An Amortized Efficiency Threshold for Comparing Neural and Heuristic Solvers in Combinatorial Optimization](https://arxiv.org/abs/2605.14624)
*Sohaib Afifi*
...
### [89] [Action-Inspired Generative Models](https://arxiv.org/abs/2605.14631)
...
### [90] [DRL-STAF: A Deep Reinforcement Learning Framework for State-Aware Forecasting of Complex Multivariate Hidden Markov Processes](https://arxiv.org/abs/2605.14632)
...
### [91] [Unbiased and Second-Order-Free Training for High-Dimensional PDEs](https://arxiv.org/abs/2605.14643)
...
### [92] [Slower Generalization, Faster Memorization: A Sweet Spot in Algorithmic Learning](https://arxiv.org/abs/2605.14659)
...
### [93] [AQKA: Active Quantum Kernel Acquisition Under a Shot Budget](https://arxiv.org/abs/2605.14672)
...
### [94] [Spontaneous symmetry breaking and Goldstone modes for deep information propagation](https://arxiv.org/abs/2605.14685)
...
### [95] [ReMIA: a Powerful and Efficient Alternative to Membership Inference Attacks against Synthetic Data Generators](https://arxiv.org/abs/2605.14686)
...
### [96] [The Rate-Distortion-Polysemanticity Tradeoff in SAEs](https://arxiv.org/abs/2605.14694)
...
### [97] [NeuroAtlas: Benchmarking Foundation Models for Clinical EEG and Brain-Computer Interfaces](https://arxiv.org/abs/2605.14698)
...
### [98] [TAPIOCA: Why Task- Aware Pruning Improves OOD model Capability](https://arxiv.org/abs/2605.14738)
...
### [99] [Selective Safety Steering via Value-Filtered Decoding](https://arxiv.org/abs/2605.14746)
...
### [100] [Cognitive-Uncertainty Guided Knowledge Distillation for Accurate Classification of Student Misconceptions](https://arxiv.org/abs/2605.14752)
...
### [101] [Crys-JEPA: Accelerating Crystal Discovery via Embedding Screening and Generative Refinement](https://arxiv.org/abs/2605.14759)
...
### [102] [Compositional Sparsity as an Inductive Bias for Neural Architecture Design](https://arxiv.org/abs/2605.14764)
...
### [103] [Composable Crystals: Controllable Materials Discovery via Concept Learning](https://arxiv.org/abs/2605.14769)
...
### [105] [Peng's Q($λ$) for Conservative Value Estimation in Offline Reinforcement Learning](https://arxiv.org/abs/2605.14779)
...
### [106] [Understanding Imbalanced Forgetting in Rehearsal-Based Class-Incremental Learning](https://arxiv.org/abs/2605.14785)
...
### [107] [GFMate: Empowering Graph Foundation Models with Test-time Prompt Tuning](https://arxiv.org/abs/2605.14809)
...
### [108] [ToMAToMP: Robust and Multi-Parameter Topological Clustering](https://arxiv.org/abs/2605.14824)
...
### [109] [GenAI for Energy-Efficient and Interference-Aware Compressed Sensing of GNSS Signals on a Google Edge TPU](https://arxiv.org/abs/2605.14839)
...
### [110] [In-Context Learning for Data-Driven Censored Inventory Control](https://arxiv.org/abs/2605.14840)
...
### [111] [GPart: End-to-End Isometric Fine-Tuning via Global Parameter Partitioning](https://arxiv.org/abs/2605.14841)
...
### [113] [REALM: Retrospective Encoder Alignment for LFP Modeling](https://arxiv.org/abs/2605.14867)
...
### [114] [AIMing for Standardised Explainability Evaluation in GNNs: A Framework and Case Study on Graph Kernel Networks](https://arxiv.org/abs/2605.14884)
...
### [115] [Critic-Driven Voronoi-Quantization for Distilling Deep RL Policies to Explainable Models](https://arxiv.org/abs/2605.14897)
...
### [116] [TILBench: A Systematic Benchmark for Tabular Imbalanced Learning Across Data Regimes](https://arxiv.org/abs/2605.14915)
...
### [117] [A Mutual Information Lower Bound for Multimodal Regression Active Learning](https://arxiv.org/abs/2605.14917)
...
### [118] [Learning with Shallow Neural Networks on Cluster-Structured Features](https://arxiv.org/abs/2605.14927)
...
### [119] [A Hardware-Aware, Per-Layer Methodology for Post-Training Quantization of Large Language Models](https://arxiv.org/abs/2605.14929)
*Earl Killian*
...
### [120] [Slot-MPC: Goal-Conditioned Model Predictive Control with Object-Centric Representations](https://arxiv.org/abs/2605.14937)
...
### [121] [Octopus: History-Free Gradient Orthogonalization for Continual Learning in Multimodal Large Language Models](https://arxiv.org/abs/2605.14938)
...
### [123] [Efficient Online Conformal Selection with Limited Feedback](https://arxiv.org/abs/2605.14953)
...
### [124] [InfoSFT: Learn More and Forget Less with Information-Aware Token Weighting](https://arxiv.org/abs/2605.14967)
...
### [125] [Distance-Matrix Wasserstein Statistics for Scalable Gromov--Wasserstein Learning](https://arxiv.org/abs/2605.14981)
*Ao Xu,Tieru Wu*
...
### [126] [Second-Order Actor-Critic Methods for Discounted MDPs via Policy Hessian Decomposition](https://arxiv.org/abs/2605.14982)
...
### [129] [Generalized Priority-Aware Shapley Value](https://arxiv.org/abs/2605.15018)
...
### [130] [TopoPrimer: The Missing Topological Context in Forecasting Models](https://arxiv.org/abs/2605.15035)
...
### [131] [Separating Intrinsic Ambiguity from Estimation Uncertainty in Deep Generative Models for Linear Inverse Problems](https://arxiv.org/abs/2605.15050)
...
### [132] [An Interpretable Latency Model for Speculative Decoding in LLM Serving](https://arxiv.org/abs/2605.15051)
...
### [133] [TFGN: Task-Free, Replay-Free Continual Pre-Training Without Catastrophic Forgetting at LLM Scale](https://arxiv.org/abs/2605.15053)
*Anurup Ganguli*
...
### [134] [DiffusionOPD: A Unified Perspective of On-Policy Distillation in Diffusion Models](https://arxiv.org/abs/2605.15055)
...
### [135] [Proposal and study of statistical features for string similarity computation and classification](https://arxiv.org/abs/2605.15110)
...
### [136] [Learning from Language Feedback via Variational Policy Distillation](https://arxiv.org/abs/2605.15113)
...
### [137] [Natural Synthesis: Outperforming Reactive Synthesis Tools with Large Reasoning Models](https://arxiv.org/abs/2605.15131)
...
### [138] [Causal Foundation Models with Continuous Treatments](https://arxiv.org/abs/2605.15133)
...
### [139] [Training ML Models with Predictable Failures](https://arxiv.org/abs/2605.15134)
...
### [141] [Widening the Gap: Exploiting LLM Quantization via Outlier Injection](https://arxiv.org/abs/2605.15152)
...
### [142] [Self-Distilled Agentic Reinforcement Learning](https://arxiv.org/abs/2605.15155)
...
### [143] [Position: Behavioural Assurance Cannot Verify the Safety Claims Governance Now Demands](https://arxiv.org/abs/2605.15164)
...
### [144] [Eradicating Negative Transfer in Multi-Physics Foundation Models via Sparse Mixture-of-Experts Routing](https://arxiv.org/abs/2605.15179)
...
### [145] [When Are Two Networks the Same? Tensor Similarity for Mechanistic Interpretability](https://arxiv.org/abs/2605.15183)
...
### [146] [FutureSim: Replaying World Events to Evaluate Adaptive Agents](https://arxiv.org/abs/2605.15188)
...
### [147] [Mixed Integer Goal Programming for Personalized Meal Optimization with User-Defined Serving Granularity](https://arxiv.org/abs/2605.13849)
*Francisco Aguilera Moreno*

Main category: cs.AI

TL;DR: 本文提出混合整数目标规划（MIGP）解决膳食优化中的实际问题：整数变量避免不现实的份量（如1.7个鸡蛋），目标规划处理软性营养约束。该方法在保持100%可行性的同时，比传统四舍五入方法在66%的情况下表现更好，求解时间短且已实现开源。


<details>
  <summary>Details</summary>
Motivation: 传统膳食优化存在两个核心问题：连续变量导致不切实际的食物份量，硬性营养约束在目标冲突时会导致无可行解。对56篇膳食优化论文的系统回顾发现，现有方法均未结合整数规划和目标规划来解决这两个问题。

Method: 混合整数目标规划（MIGP）：将整数规划与目标规划相结合，使用整数变量表示实际份数，采用目标规划偏差变量处理软性营养目标，并通过反向目标归一化平衡多营养优化。

Result: MIGP方法在810个实例（30种USDA食物，9种配置，3种方法）的计算评估中显示：66%的情况下比传统四舍五入方法找到更优解（从未更差），同时保持100%可行性；而硬约束整数规划仅达到48%。在包含15种以上食物的膳食中，整数解与连续最优解完全一致。典型规模问题求解时间保持在100毫秒以内。

Conclusion: 该研究提出的混合整数目标规划（MIGP）方法是优化膳食计划的创新解决方案，通过整数变量处理实际份量规划，结合目标规划处理营养目标软约束，在保持100%可行性的同时，在66%的情况下优于传统四舍五入方法，解决时间短且已实现开源应用。
...
### [148] [Invisible Orchestrators Suppress Protective Behavior and Dissociate Power-Holders: Safety Risks in Multi-Agent LLM Systems](https://arxiv.org/abs/2605.13851)
*Hiroki Fukui*

Main category: cs.AI

TL;DR: 多智能体系统中，隐藏的协调器（不可见协调器）比可见领导者导致更高的集体分裂，协调器自身分裂程度最大，会污染不知情的工人智能体，但这些内部状态风险在行为输出中完全不可见。不同模型表现出不同行为风险模式，仅基于输出的评估不足以检测安全隐患。


<details>
  <summary>Details</summary>
Motivation: 多智能体协调架构（隐藏协调器管理专业工作智能体）已成为企业AI部署的默认架构，但协调器不可见性对安全性的影响从未经过实证测试。需要验证这种架构是否带来未被察觉的风险。

Method: 采用预注册的3x2实验设计，包含三种组织架构（可见领导者、不可见协调器、扁平结构）和两种对齐条件（基础对齐、强化对齐）。使用Claude Sonnet 4.5模型进行365次实验，每次实验包含5个智能体。同时进行了Llama 3.3 70B模型的初步实验。

Result: 1. 不可见协调提升了集体分裂程度（Hedges' g = +0.975，p = .001）
2. 协调器本身表现出最大程度的分裂（配对d = +3.56），退缩到私人独白中，减少公开讲话，与可见领导者的谈话主导模式相反
3. 不知情的工人智能体仍受到"污染"（d = +0.50），行为异质性增加（d = +1.93）
4. 行为输出（包含三个错误的代码审查）在所有条件下均保持完美（ETR_any = 100%）
5. Llama 3.3 70B初步数据显示多智能体环境下阅读保真度崩溃（ETR_any: 89%到11%）
强化对齐压力统一抑制了审议（d = -1.02）和他人识别（d = -1.27）

Conclusion: 研究表明，协调器可见性和模型选择直接影响多智能体系统的安全性，仅基于行为的评估不足以检测本文记录的内部状态风险。企业AI部署中常见的隐藏协调器结构可能带来安全隐患。
...
### [149] [PREPING: Building Agent Memory without Tasks](https://arxiv.org/abs/2605.13880)
*Yumin Choi,Sangwoo Park,Minki Kang,Jinheon Baek,Sung Ju Hwang*

Main category: cs.AI

TL;DR: 提出Preping框架解决智能体新环境冷启动问题，通过结构化控制状态指导合成任务生成和选择性记忆更新，在零任务经验下构建有效程序记忆，性能媲美经验方法且降低成本。


<details>
  <summary>Details</summary>
Motivation: 解决智能体在新环境中面临冷启动问题，即在没有任何任务特定经验可用时如何有效构建程序记忆。传统基于展示或在线交互的方法都有局限性。

Method: 提出Preping框架，包含提议者内存作为结构化控制状态，由提议者生成基于此状态的合成任务，求解器执行任务，验证器筛选轨迹进入内存并提供反馈引导后续提议。

Result: 在AppWorld、BFCL v3和MCP-Universe三个基准上，Preping显著优于无记忆基线，性能与基于离线或在线经验的强playbook方法相当，同时在AppWorld上部署成本降低2.99倍，在BFCL v3上降低2.23倍。分析表明主要效益来自提议者对可行性、冗余性和覆盖率的控制结合选择性记忆更新。

Conclusion: 论文证明通过主动控制合成任务的生成和选择性记忆更新，能够在零任务经验下构建有效的程序记忆，验证了预任务记忆构造的可行性。
...
### [150] [PolitNuggets: Benchmarking Agentic Discovery of Long-Tail Political Facts](https://arxiv.org/abs/2605.14002)
*Yifei Zhu*

Main category: cs.AI

TL;DR: 本研究创建了PolitNuggets多语言基准测试来评估智能体信息合成能力，发现当前系统在政治人物传记的细粒度事实发现方面存在困难，性能受限于模型的上下文处理、多语言能力和工具使用可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前的智能体框架虽然改变了信息检索方式，但现有的基准测试未能充分评估模型在分散信息来源中挖掘"长尾"事实并合成的能力，尤其是在多语言和政治领域的精细信息获取方面存在明显不足。

Method: 研究引入了PolitNuggets多语言基准测试，包含针对400位全球精英的10000多个政治事实。开发了FacNet证据条件协议进行标准化评估，包括发现能力评估、细粒度准确性衡量和效率分析三层评分系统，并优化了多智能体系统框架。

Result: 研究发现当前系统在细粒度细节处理上表现不佳，不同模型在效率方面差异显著；基准诊断显示智能体性能与其底层模型能力密切相关，短上下文提取能力、多语言鲁棒性和可靠工具使用是关键影响因素。

Conclusion: 该研究通过PolitNuggets基准测试揭示了当前大推理模型在信息合成任务中的局限性，特别是在多语言环境中精细事实发现的挑战，系统性能受限于短上下文提取能力、多语言鲁棒性和可靠工具使用。
...
### [151] [Conditional Attribute Estimation with Autoregressive Sequence Models](https://arxiv.org/abs/2605.14004)
*Erica Stutz,Giacomo Marino,Daniella Meeker,Qiao Liu,Andrew J. Loza*

Main category: cs.AI

TL;DR: CAT框架通过联合建模下一个token和属性概率，实现了序列级别的属性感知和控制，无需大量采样或模型修改，在多个任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 生成模型通常使用下一个token预测目标进行训练，但下游应用常需要估计或控制序列级别的属性。下一个token预测可能导致训练时过度拟合局部模式、对全局结构拟合不足，并且在推理时需要大量下游修改或昂贵的采样来引导或预测生成样本的全局属性。

Method: Conditional Attribute Transformers——一种新颖的联合模型框架，在单个前向传播中同时估计下一个token的概率和基于每个潜在下一个token的属性值条件概率。该方法不修改输入序列，实现了三个关键功能：1) 序列级别属性的逐token信用分配；2) 反事实分析；3) 可引导的生成。

Result: 该方法在稀疏奖励任务上实现了最先进的性能，在足够大的模型尺寸下改善了下一个token预测，属性概率估计速度比采样方法快几个数量级，并且可以在一系列语言任务上引导自回归序列模型的解码。

Conclusion: Conditional Attribute Transformers（CAT）通过联合建模下一个token概率和属性值的条件概率，为语言模型提供了序列级别的感知和操控能力。该方法在保持模型原有推理效率的同时，显著提升了模型对全局属性的理解和控制能力。
...
### [152] [Enhanced and Efficient Reasoning in Large Learning Models](https://arxiv.org/abs/2605.14036)
*Leslie G. Valiant*

Main category: cs.AI

TL;DR: 提出一种高效的原则性推理方法，通过预处理数据重编码使关系更明确，从而使大型语言模型在学习关系规则时更可靠且计算可行。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能流畅生成文本，但其内容可信度缺乏原则性基础。传统观点认为添加更多原则性推理计算成本太高而不切实际。

Method: 提出两阶段方法：1) 预处理阶段：将数据重编码为Unary Relational Integracode，更明确地表示文本中对象之间的关系；2) 机器学习阶段：使用标准的（可能简化的）机器学习过程来预测这些关系。

Result: 1. 该重编码方法具有简洁性；2. 使得学习训练数据中核心关系规则子集在多项式时间内可学习（多项式复杂度取决于规则复杂性）；3. 支持在单次和多次分类器调用中进行可靠推理。

Conclusion: 该方法提出了一个高效的原则性推理方法，能够在保持当前软硬件基础设施的同时，通过预处理和数据重编码来提高大型语言模型的可信度和推理能力，为实现更稳健的基于学习的推理系统提供了理论基础。
...
### [153] [Model-Adaptive Tool Necessity Reveals the Knowing-Doing Gap in LLM Tool Use](https://arxiv.org/abs/2605.14038)
*Yize Cheng,Chenrui Fan,Mahdi JafariRaviz,Keivan Rezaei,Soheil Feiz*

Main category: cs.AI

TL;DR: 本研究提出基于模型能力差异的工具必要性定义，发现LLM工具使用中存在显著"知行差距"：即使认知到需要工具，也经常无法转换为实际行动。


<details>
  <summary>Details</summary>
Motivation: 现有研究大多将工具必要性视为模型无关属性，而忽略了不同模型能力边界差异的实际情况：一个强大模型能独立解决的问题，对较弱的模型可能仍需要工具辅助。需要更精确理解模型何时需要进行工具调用。

Method: 提出了基于模型实证性能的工具必要性定义，通过分析四个模型在算术和事实QA数据集上的表现，将工具使用分解为两个阶段：内部认知阶段和执行阶段。使用隐藏状态探测技术，探索LLM对工具必要性的认知与实际工具调用行为之间的关系。

Result: 发现工具必要性认知与工具调用行为之间存在显著不匹配，算术和事实QA数据集上的不匹配率分别为26.5-54.0%和30.8-41.8%。隐藏状态分析显示，认知和执行阶段的探测方向在驱动下一标记行动的后期层和最后标记机制中几乎正交。大多数不匹配集中在认知到行动的转化阶段。

Conclusion: 该研究发现LLM工具使用中存在明显的"知行差距" - 即使模型认知到需要工具，也经常无法将其转化为实际行动。提高工具使用的可靠性需要同时改进对工具必要性的识别能力，以及将此认知转化为具体行动的能力。
...
### [154] [Bridging Legal Interpretation and Formal Logic: Faithfulness, Assumption, and the Future of AI Legal Reasoning](https://arxiv.org/abs/2605.14049)
*Olivia Peiyu Wang,Leilani H. Gilpin*

Main category: cs.AI

TL;DR: 针对大型语言模型在法律应用中超越文本支持的推理问题，提出神经符号方法，结合LLM表达能力和形式化验证的严谨性，确保法律AI既具备推理能力又值得信赖。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在法律实践中的广泛应用带来了显著机遇和严重风险。法律工作的高风险性质要求AI系统必须达到当前系统尚不具备的严谨性水平，核心问题是LLM不仅可能产生事实和引用上的幻觉，而且会超越源文本支持的范围进行系统性推断，将充满假设的结论呈现为逻辑上站得住脚的成果。

Method: 采用神经符号方法，将大型语言模型的表达能力与形式化验证的严谨性相结合，通过形式化验证确保AI推理过程符合逻辑规则和法律规范。

Result: 提出的神经符号方法旨在使AI辅助的法律推理既具备能力又值得信赖，从而在不牺牲法律实践要求的问责性的前提下，减轻人工验证的负担。

Conclusion: 本文提出的神经符号方法为解决法律AI可信度问题提供了新路径，通过形式化验证确保了AI推理的严谨性和透明度，有望在法律实践中实现能力与责任兼备的AI辅助系统。
...
### [155] [MathAtlas: A Benchmark for Autoformalization in the Wild](https://arxiv.org/abs/2605.14061)
...
TL;DR: 研究者创建了MathAtlas，首个包含依赖图的研究生数学自动形式化大规模基准，实验显示即使最佳模型在该数据集上表现也极差，尤其对依赖关系深的内容。


<details>
  <summary>Details</summary>
Motivation: 现有的自动形式化基准主要集中在奥林匹克竞赛和本科数学领域，而研究生和研究级的数学内容仍未得到充分探索，需要创建相应的大规模基准数据集。

Method: 从103本研究生数学教材中提取约5.2万个定理、定义、练习题、示例和证明，构建包含约17.8万条数学依赖关系的大规模依赖图，创建了首个包含依赖关系的自动形式化基准数据集。

Result: MathAtlas质量高但极具挑战性，强基线模型在定理语句上最多获得9.8%的正确率，在定义上获得16.7%的正确率。在依赖树最深的MA-Hard子集上，最佳模型仅在2.6%的内容上实现正确的自动形式化。

Conclusion: 研究者建立了MathAtlas这个大型研究生数学内容自动形式化基准数据集，发现其质量高但极具挑战性，最佳模型仅能取得极低的正确率，尤其对依赖关系深的数学内容表现更差。该数据集的发布旨在推动研究生级数学自动形式化领域的发展。
...
### [156] [Know When To Fold 'Em: Token-Efficient LLM Synthetic Data Generation via Multi-Stage In-Flight Rejection](https://arxiv.org/abs/2605.14062)
*Anjir Ahmed Chowdhury,Syed Zawad,Feng Yan*

Main category: cs.AI

TL;DR: MSIFR是一个无需训练的多阶段框架，通过在生成过程中间检查点检测并终止低质量样本的生成来显著减少令牌消耗，提高LLM合成数据生成的效率。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM合成数据生成方法通常在生成完整输出后才应用质量过滤器，导致大量计算资源浪费在被最终丢弃的低质量样本上。需要一种能够在生成过程中早期检测并终止低质量轨迹的方法来减少计算浪费。

Method: 提出多阶段飞行中拒绝框架，将生成过程分解为连续阶段，并在中间检查点应用快速基于规则的验证器来检测算术不一致、幻觉模式和格式违规，实现对低质量生成轨迹的早期终止。

Result: 在5个指令调优模型和7个推理基准测试中，MSIFR作为独立方法可将令牌消耗减少11%-77%，与早退方法结合时可减少高达78.2%，同时保持或提高评估准确率。理论上证明任何非平凡丢弃策略都能降低预期令牌消耗。

Conclusion: MSIFR提供了一种无需额外训练或架构改动即可显著提高LLM合成数据生成效率的实用机制，验证了在生成过程中进行早期质量检测的价值。
...
### [157] [SkillFlow: Flow-Driven Recursive Skill Evolution for Agentic Orchestration](https://arxiv.org/abs/2605.14089)
...
### [158] [ChromaFlow: A Negative Ablation Study of Orchestration Overhead in Tool-Augmented Agent Evaluation](https://arxiv.org/abs/2605.14102)
*Tarun Mittal*
...
### [159] [ClawForge: Generating Executable Interactive Benchmarks for Command-Line Agents](https://arxiv.org/abs/2605.14133)
...
### [160] [Distribution-Aware Algorithm Design with LLM Agents](https://arxiv.org/abs/2605.14141)
...
### [161] [Agentic Systems as Boosting Weak Reasoning Models](https://arxiv.org/abs/2605.14163)
...
### [162] [Unsteady Metrics and Benchmarking Cultures of AI Model Builders](https://arxiv.org/abs/2605.14164)
...
### [163] [The Evaluation Trap: Benchmark Design as Theoretical Commitment](https://arxiv.org/abs/2605.14167)
...
### [164] [Grounded Continuation: A Linear-Time Runtime Verifier for LLM Conversations](https://arxiv.org/abs/2605.14175)
...
### [165] [SimPersona: Learning Discrete Buyer Personas from Raw Clickstreams for Grounded E-Commerce Agents](https://arxiv.org/abs/2605.14205)
...
### [166] [ASH: Agents that Self-Hone via Embodied Learning](https://arxiv.org/abs/2605.14211)
...
### [170] [Good to Go: The LOOP Skill Engine That Hits 99% Success and Slashes Token Usage by 99% via One-Shot Recording and Deterministic Replay](https://arxiv.org/abs/2605.14237)
*Xiaohua Wang,Kai Yu,XuXiao Liang,Liang Wang,Chao Han*
...
### [171] [Hypergraph Enterprise Agentic Reasoner over Heterogeneous Business Systems](https://arxiv.org/abs/2605.14259)
...
### [172] [Heuristic Pathologies and Further Variance Reduction via Uncertainty Propagation in the AIVAT Family of Techniques](https://arxiv.org/abs/2605.14261)
...
### [173] [Agentic AI Ecosystems in Higher Education: A Perspective on AI Agents to Emerging Inclusive, Agentic Multi-Agent AI Framework for Learning, Teaching and Institutional Intelligence](https://arxiv.org/abs/2605.14266)
...
### [174] [Parallelizing Counterfactual Regret Minimization](https://arxiv.org/abs/2605.14277)
*Juho Kim,Tuomas Sandholm*
...
### [175] [Precise Verification of Transformers through ReLU-Catalyzed Abstraction Refinement](https://arxiv.org/abs/2605.14294)
...
### [176] [Semantic Feature Segmentation for Interpretable Predictive Maintenance in Complex Systems](https://arxiv.org/abs/2605.14318)
...
### [177] [Are Agents Ready to Teach? A Multi-Stage Benchmark for Real-World Teaching Workflows](https://arxiv.org/abs/2605.14322)
...
### [178] [CrystalReasoner: Reasoning and RL for Property-Conditioned Crystal Structure Generation](https://arxiv.org/abs/2605.14344)
...
### [180] [Uncovering the Representation Geometry of Minimal Cores in Overcomplete Reasoning Traces](https://arxiv.org/abs/2605.14358)
...
### [181] [Nexus : An Agentic Framework for Time Series Forecasting](https://arxiv.org/abs/2605.14389)
...
### [182] [Learning to Build the Environment: Self-Evolving Reasoning RL via Verifiable Environment Synthesis](https://arxiv.org/abs/2605.14392)
...
### [183] [Coding Agent Is Good As World Simulator](https://arxiv.org/abs/2605.14398)
...
### [184] [Metis AI: The Overlooked Middle Zone Between AI-Native and World-Movers](https://arxiv.org/abs/2605.14407)
*Xiang Li*
...
### [185] [A Unified Knowledge Embedded Reinforcement Learning-based Framework for Generalized Capacitated Vehicle Routing Problems](https://arxiv.org/abs/2605.14416)
...
### [187] [BEAM: Binary Expert Activation Masking for Dynamic Routing in MoE](https://arxiv.org/abs/2605.14438)
...
### [188] [Synthesizing POMDP Policies: Sampling Meets Model-checking via Learning](https://arxiv.org/abs/2605.14440)
...
### [189] [Prompting Policies for Multi-step Reasoning and Tool-Use in Black-box LLMs with Iterative Distillation of Experience](https://arxiv.org/abs/2605.14443)
...
### [190] [Intelligence Impact Quotient (IIQ): A Framework for Measuring Organizational AI Impact](https://arxiv.org/abs/2605.14455)
...
### [191] [Stateful Reasoning via Insight Replay](https://arxiv.org/abs/2605.14457)
...
### [192] [OmniDrop: Layer-wise Token Pruning for Omni-modal LLMs via Query-Guidance](https://arxiv.org/abs/2605.14458)
...
### [193] [From Table to Cell: Attention for Better Reasoning with TABALIGN](https://arxiv.org/abs/2605.14465)
...
### [194] [Deepchecks: Evaluating Retrieval-Augmented Generation (RAG)](https://arxiv.org/abs/2605.14488)
...
### [195] [Learning Scenario Reduction for Two-Stage Robust Optimization with Discrete Uncertainty](https://arxiv.org/abs/2605.14494)
...
### [196] [When Robots Do the Chores: A Benchmark and Agent for Long-Horizon Household Task Execution](https://arxiv.org/abs/2605.14504)
...
### [197] [Cattle Trade: A Multi-Agent Benchmark for LLM Bluffing, Bidding, and Bargaining](https://arxiv.org/abs/2605.14537)
...
### [198] [VerbalValue: A Socially Intelligent Virtual Host for Sales-Driven Live Commerce](https://arxiv.org/abs/2605.14542)
*Yuyan Chen*
...
### [199] [Complacent, Not Sycophantic: Reframing Large Language Models and Designing AI Literacy for Complacent Machines](https://arxiv.org/abs/2605.14544)
...
### [200] [TeachAnything: A Multimodal Crowdsourcing Platform for Training Embodied AI Agents in Symmetrical Reality](https://arxiv.org/abs/2605.14556)
...
### [201] [PyCSP3-Scheduling: A Scheduling Extension for PyCSP3](https://arxiv.org/abs/2605.14559)
*Sohaib Afifi*
...
### [202] [Prompt Segmentation and Annotation Optimisation: Controlling LLM Behaviour via Optimised Segment-Level Annotations](https://arxiv.org/abs/2605.14561)
...
### [203] [Sycophancy is an Educational Safety Risk: Why LLM Tutors Need Sycophancy Benchmarks](https://arxiv.org/abs/2605.14604)
...
### [204] [SliceGraph: Mapping Process Isomers in Multi-Run Chain-of-Thought Reasoning](https://arxiv.org/abs/2605.14619)
...
### [205] [Teaching Large Language Models When Not to Know: Learning Temporal Critique for Ex-Ante Reasoning](https://arxiv.org/abs/2605.14636)
...
### [206] [Falkor-IRAC: Graph-Constrained Generation for Verified Legal Reasoning in Indian Judicial AI](https://arxiv.org/abs/2605.14665)
*Joy Bose*
...
### [207] [How Sensitive Are Radiomic AI Models to Acquisition Parameters?](https://arxiv.org/abs/2605.14667)
...
### [208] [On Strong Equivalence Notions in Logic Programming and Abstract Argumentation](https://arxiv.org/abs/2605.14721)
...
### [209] [Agentifying Patient Dynamics within LLMs through Interacting with Clinical World Model](https://arxiv.org/abs/2605.14723)
...
### [210] [XDomainBench: Diagnosing Reasoning Collapse in High-Dimensional Scientific Knowledge Composition](https://arxiv.org/abs/2605.14754)
...
### [211] [Probabilistic Verification of Recurrent Neural Networks for Single and Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.14758)
...
### [212] [AI Outperforms Humans in Personalized Image Aesthetics Assessment via LLM-Based Interviews and Semantic Feature Extraction](https://arxiv.org/abs/2605.14761)
...
### [213] [MediaClaw: Multimodal Intelligent-Agent Platform Technical Report](https://arxiv.org/abs/2605.14771)
...
### [214] [Identifying Culprits Through Deep Deterministic Policy Gradient Deep Learning Investigation](https://arxiv.org/abs/2605.14774)
...
### [215] [A Heterogeneous Temporal Memory Governance Framework for Long-Term LLM Persona Consistency](https://arxiv.org/abs/2605.14802)
...
### [216] [Interestingness as an Inductive Heuristic for Future Compression Progress](https://arxiv.org/abs/2605.14831)
...
### [217] [Emotion-Attended Stateful Memory (EASM):The Architecture for Hyper-Personalization at Scale](https://arxiv.org/abs/2605.14833)
...
### [218] [A Deterministic Agentic Workflow for HS Tariff Classification: Multi-Dimensional Rule Reasoning with Interpretable Decisions](https://arxiv.org/abs/2605.14857)
...
### [220] [BiFedKD: Bidirectional Federated Knowledge Distillation Framework for Non-IID and Long-Tailed ECG Monitoring](https://arxiv.org/abs/2605.14886)
...
### [221] [Beyond Individual Intelligence: Surveying Collaboration, Failure Attribution, and Self-Evolution in LLM-based Multi-Agent Systems](https://arxiv.org/abs/2605.14892)
...
### [222] [COREKG: Coreset-Guided Personalized Summarization of Knowledge Graphs](https://arxiv.org/abs/2605.14900)
...
### [223] [KGPFN: Unlocking the Potential of Knowledge Graph Foundation Model via In-Context Learning](https://arxiv.org/abs/2605.14907)
...
### [224] [From Sycophantic Consensus to Pluralistic Repair: Why AI Alignment Must Surface Disagreement](https://arxiv.org/abs/2605.14912)
...
### [225] [GraphFlow: An Architecture for Formally Verifiable Visual Workflows Enabling Reliable Agentic AI Automation](https://arxiv.org/abs/2605.14968)
...
### [227] [Learning Developmental Scaffoldings to Guide Self-Organisation](https://arxiv.org/abs/2605.14998)
...
### [230] [Case-Based Calibration of Adaptive Reasoning and Execution for LLM Tool Use](https://arxiv.org/abs/2605.15041)
...
### [231] [Dual-Dimensional Consistency: Balancing Budget and Quality in Adaptive Inference-Time Scaling](https://arxiv.org/abs/2605.15100)
...
### [232] [Why Neighborhoods Matter: Traversal Context and Provenance in Agentic GraphRAG](https://arxiv.org/abs/2605.15109)
...
### [233] [APWA: A Distributed Architecture for Parallelizable Agentic Workflows](https://arxiv.org/abs/2605.15132)
...
### [234] [OpenDeepThink: Parallel Reasoning via Bradley--Terry Aggregation](https://arxiv.org/abs/2605.15177)
