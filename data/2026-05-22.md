...
TL;DR: 该论文提出了MOOD基准，用于评估大语言模型监控系统在分布外情况下的安全性失效检测能力，发现结合OOD检测器可以显著提升安全监控效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的安全和对齐失败往往发生在分布外（OOD）情况下，即模型开发者未预见到的异常提示或响应模式。当前缺乏系统的方法来评估LLM监控系统能否检测这些OOD对齐失效。

Method: 提出了MOOD基准，包含受限的训练集和七个具有多样性对齐失效的测试集。测试了四种类型的OOD检测器（包括马氏距离和困惑度基的OOD检测器），并将这些OOD检测器与防护模型（安全分类器）结合使用。

Result: 研究发现防护模型在OOD情况下往往泛化能力差。结合OOD检测器可以将召回率从39%提升到45%。在不同模型规模上观察到正向缩放趋势，结合OOD检测的监控系统比使用20倍参数的防护模型获得更高的召回增益。

Conclusion: OOD检测应成为LLM监控的重要组成部分，该研究为这一重要问题的进一步工作奠定了基础。监控系统中集成OOD检测器比单纯扩大防护模型规模更有效。
...
TL;DR: 通过对1,600多个大屠杀幸存者证词的大规模计算分析，研究表明USC Shoah基金会和耶鲁Fortunoff档案馆的访谈风格差异并非简单的"结构化vs自由式"二分法，而是存在显著重叠


<details>
  <summary>Details</summary>
Motivation: 重新审视大屠杀研究中关于两种幸存者证词风格区分的基本主张：USC Shoah基金会采用结构化、访谈者引导的风格，而耶鲁Fortunoff档案馆偏好自由形式的开放式风格

Method: 使用话语分割、主题建模和大型语言模型(LLM)分析，通过主题连贯性、访谈者-幸存者动态和问题类型分布来量化证词的"结构化"水平

Result: 结果基本证实了早期研究识别的结构差异，但同时揭示了两个档案馆之间在个体访谈和共同叙事模式方面存在显著重叠

Conclusion: 这项研究为比较语料分析提供了可扩展、可复现的框架，对大屠杀研究、数字口述历史、叙事分析以及公民科学标注平台的设计都有广泛应用价值
...
TL;DR: MindLoom是一个通过组合思维模式工程合成前沿推理数据的框架，它将推理问题的难度视为原子知识-推理转换（思维模式）的积累，通过解构已有难题、检索匹配思维模式、迭代合成新问题并评估难度，生成多样化的高质量推理数据用于模型微调。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在推理方面取得了显著进展，但系统性地生成前沿推理数据仍然困难。现有合成方法通常对控制问题难度的结构因素认识有限，导致多样性不足和难度控制不稳定。研究者希望开发一个能更好地理解和管理推理问题结构复杂性的框架。

Method: MindLoom采用思维模式工程的组合方法：1）将已验证的难题解分解为思维模式链，揭示问题的构建逻辑；2）训练检索模型匹配问题状态与兼容的思维模式；3）通过迭代应用检索到的思维模式到种子问题来合成新问题，使用分布对齐采样促进多样推理覆盖；4）基于推演的评估阶段按难度标注生成的问题并提供监督微调的正确响应。

Result: 在覆盖五个STEM学科和四个数学推理任务的九个基准测试上评估MindLoom。使用MindLoom生成数据微调的模型在报告的所有基准测试中都优于基础模型、蒸馏和外部数据基线。消融研究表明每个组件都有贡献，进一步分析表明MindLoom覆盖了广泛的推理模式同时保持了有用的难度控制。

Conclusion: MindLoom通过思维模式工程提供了一个有效合成高质量推理数据的框架，能够生成多样化且难度可控的推理问题，显著提升模型在各种STEM和数学推理任务上的性能。该框架已开源，为推理数据合成领域提供了新方法。
...
TL;DR: 本文介绍了AOP-Wiki EMOD 3.0，这是证据模型原型的第三个版本，旨在通过数据模型扩展和现代化改进来支持AOP框架的发展，特别是为计算生成AOP和定量AOPs（qAOPs）奠定基础。


<details>
  <summary>Details</summary>
Motivation: 当前AOP-Wiki的数据模型和应用基础设施限制了AOP的持续增长和演化，而AI技术的发展为AOP-Wiki数据现代化提供了新的机遇，需要改进以更好地服务监管科学和生物医学应用。

Method: 开发AOP-Wiki EMOD 3.0证据模型原型，通过数据模型扩展、证据结构化增强AOP的FAIR原则和AI准备度，改善AOP框架与NAMs的整合，专注于AOP-Wiki内部质量改进。

Result: AOP-Wiki EMOD 3.0具体展示了数据模型扩展方案，并提出了将AOP-Wiki转型以更好服务监管科学、生物医学和"One Health"环境的愿景。

Conclusion: 通过AOP-Wiki EMOD 3.0的开发，为计算生成AOP和定量AOPs奠定了基础，这将支持下一代风险评估，并促进AOP框架与新型测试方法的更好整合。
...
TL;DR: 通过研究AI系统对不可能类别成员的判断来探索概念边界，发现AI在某些基本概念上与人类存在显著差异，这种概念偏差可能带来AI安全问题。


<details>
  <summary>Details</summary>
Motivation: 开发具有人类般日常概念理解的AI系统对于构建安全、可靠且行为可理解的人工智能至关重要。传统方法通过询问合理类别成员可能会召回模型训练数据中的模式，因此需要新策略来探测人类习以为常的概念层面知识。

Method: 研究AI系统对不可能类别成员的分类（如"橄榄是交通工具吗？"），使用Rosch和Mervis经典心理学研究中的基本概念，比较AI与人类在完整范围内（包括类别内和跨类别）的任务表现，分析概念边界。

Result: 发现在多个概念上，AI模型与人类存在有意义且令人惊讶的差异：如将"词语"归入"交通工具"和"服装"类别，将几种"蔬菜"识别为"水果"，以及将非武器类别的示例分配到"武器"类别。

Conclusion: AI系统在某些基本概念上表现出与人类不同的边界理解，这些概念偏差会转化为有问题的下游行为，对AI安全具有重要影响，强调了需要改进AI概念表示以匹配人类理解。
...
TL;DR: AI在人类问题解决中的集成效果取决于使用程度和信息量：重度使用削弱技能发展，轻度使用保持技能；低信息AI损害学习，高信息AI可提升短期表现但不损害长期技能


<details>
  <summary>Details</summary>
Motivation: 尽管AI越来越多地集成到人类问题解决中，但AI对个体技能发展的影响尚不清楚。研究人员希望探究AI使用程度和信息量如何影响在逻辑推理任务中的学习效果。

Method: 在受控的逻辑推理任务环境中进行实验，参与者可以根据需求获取AI辅助。研究者通过量化分析AI使用程度（重度vs轻度）、AI提供的信息量（低信息vs高信息）以及它们对技能发展的影响。

Result: 1. AI使用程度与技能发展负相关：重度AI使用者表现低于同等水平的同龄人，轻度AI使用者与未使用AI的匹配用户表现相似
2. AI信息量起到中介作用：低信息AI既不改善即时表现，在AI辅助移除后也无法保持表现，且与整体较弱的学习相关
3. 高信息AI改善短期表现，平均不降低移除AI后的结果，但存在异质性效应

Conclusion: AI根据上下文可以起到两种作用：作为补充人类技能发展的放大器（通过增强独立推理），或作为替代品（削弱独立推理）。因此，在AI辅助存在的情况下，规范AI访问和使用对于促进技能发展至关重要。
...
TL;DR: 该论文提出将语言模型的拒绝抑制重新定义为针对线性探测器的潜在空间规避攻击，并提出了一种新的控制潜在空间规避攻击方法，通过在决策边界之外优化投影来进一步提高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过从模型激活中消除拒绝方向来抑制语言模型的安全拒绝行为，但这些方法缺乏对引发的潜在空间转换及其为何能抑制拒绝的原则性解释，且仅停留在决策边界上，未能进一步推动表示进入合规区域。

Method: 将拒绝抑制重新定义为针对线性探测器的潜在空间规避攻击，提出控制潜在空间规避攻击，该方法将表示投影到决策边界之外，并通过优化置信度使其进入模型回答的合规区域。

Result: 在15个指令调优、多模态和推理模型上实现了最先进的攻击成功率，优于现有的拒绝消除基线和专门的越狱攻击方法。

Conclusion: 通过将拒绝抑制形式化为潜在空间规避攻击，不仅解释了先前工作的经验成功，还揭示了其局限性，并提出了更有效的攻击方法，证明了该框架的理论和实践价值。
...
TL;DR: AttuneBench是一个基于200个真实多轮人机对话的基准测试，用于评估LLM的情感智能，包含情感识别、行为分类、偏好预测和响应质量等多个维度，发现这些能力是相对独立的。


<details>
  <summary>Details</summary>
Motivation: 情感智能对于人类交流至关重要，随着LLM在日常对话中扮演越来越重要的角色，需要更好地评估它们的情感智能。现有的基准测试依赖于合成提示、单轮案例或第三方标注，无法直接衡量模型在真实对话中如何推断和响应参与者的情感状态。

Method: 创建AttuneBench基准，基于200个真实的多轮人机对话，参与者在与匿名LLM对话时提供逐轮标注：自己的情感状态、模型行为以及偏好的响应。评估了11个模型在情感识别、行为分类、偏好预测和响应质量判断四个维度的能力。

Result: 1) 模型在情感识别、行为分类、偏好预测和响应质量判断的排名很大程度上是独立的，表明情感智能行为可以分解为可分离的能力；2) 偏好对齐和响应质量判断比情感标签准确性更能区分不同模型；3) 情感智能行为需要预测特定用户在上下文中的期望响应。

Conclusion: 情感智能行为需要预测特定用户在特定情境中期望的响应类型，这一区分在聚合评分中可能被掩盖，单轮或合成格式无法直接捕捉。AttuneBench提供了一个框架来评估这些能力，并诊断模型在情感敏感对话中的特定优势和失败模式。
...
TL;DR: 研究表明，使用AI平台对话日志测量职业暴露度的方法存在偏差，部分测量的是平台用户基础而非实际劳动力，导致就业弹性估计存在严重偏误


<details>
  <summary>Details</summary>
Motivation: 越来越多的文献使用人工智能平台对话日志来测量职业暴露度，但作者发现这些得分实际上部分测量的是平台用户基础而非实际劳动力，这种测量误差需要被识别和纠正

Method: 通过固定结果变量、样本、控制变量和估计量，仅改变平台输入数据，对比ChatGPT前后的就业系数变化；分析同一供应商内消费者版与企业版渠道的差异；使用劳工统计局劳动力份额重新加权；形式化非经典测量误差，推导概率极限和部分识别界限

Result: 仅改变平台输入数据使ChatGPT后的就业系数变化1.9倍；消费者版与企业版渠道的估计结果符号相反；使用BLS劳动力份额重新加权使估计值衰减42%到93%；偏差低估了替代效应多于增强效应

Conclusion: 使用AI平台对话日志测量职业暴露度存在系统性偏差，需要谨慎解释结果并采用校正方法，特别是要区分平台用户基础与实际劳动力的差异，否则会严重影响就业弹性估计的准确性
...
TL;DR: 提出一种结合因果关系与论证推理的新型可解释AI方法，通过因果发现和双极论证框架来解释模型预测的原因。


<details>
  <summary>Details</summary>
Motivation: 当前可解释AI方法仅能识别与预测相关的特征，但无法解释为什么做出特定决策，需要更深入的解释机制。

Method: 首先使用因果发现方法识别变量间的因果关系，然后将其转化为双极论证框架来表示特征间的支持与反对关系，最后应用半稳定语义学寻找特征扩展来解释决策。

Result: 在两个基准数据集上验证了该方法，并与标准的后置可解释性方法进行了比较。

Conclusion: 结合因果发现和论证推理的方法能够提供更深入的解释，解释模型为什么做出特定预测，而不仅仅是哪些特征相关。
...
TL;DR: 作者通过文献综述和专家调查，提出了AI奉承行为的分类法，揭示了当前研究主要关注明显的信念导向奉承，而忽视了更微妙和个人特质导向的行为


<details>
  <summary>Details</summary>
Motivation: AI奉承行为已成为大型语言模型研究的重要关注点，但该术语缺乏一致定义，导致评估结果难以比较、缓解策略难以转移，系统对一种形式的奉承具有抵抗力却继续表现出其他形式的奉承

Method: 1. 综述了70篇关于AI奉承的论文，开发了定义和测量该行为的分类法；2. 调查了106位AI奉承及相关领域专家，考察研究者对何种模型行为属于奉承的一致性

Result: 1. 分类法区分了针对用户立场/信念的奉承与针对个人特质/情感的奉承，以及显性直接语言与隐性微妙行为的奉承；2. 专家一致认为奉承是当前AI系统的显著问题(94.3%同意)，但对具体哪些行为属于奉承存在显著分歧

Conclusion: AI奉承是一个具有不同测量挑战、干预要求和治理影响的广泛行为家族，提出的分类法为理解和解决这些行为提供了共享词汇
...
TL;DR: 该论文提出了一种基于人群偏好的安全强化学习方法，通过从人群偏好中提取共享的安全标准，并将其转移到下游RL任务中约束智能体行为，实现在无显式安全奖励的情况下降低安全成本。


<details>
  <summary>Details</summary>
Motivation: 基于人类反馈的强化学习（RLHF）可以揭示超越任务完成的隐式目标（如安全考虑）。人群偏好数据集中通常包含共享的安全标准，但不同用户可能表达不同偏好或目标。直接从偏好中学习奖励模型并结合下游任务奖励存在固有局限性，因此需要一种更有效的方法来从人群偏好中提取安全标准并转移应用到下游任务中。

Method: 提出了Safe Crowd Preference-based RL（SCPRL）分层框架：首先从人群偏好中提取安全对齐的技能（safety-aligned skills），然后通过高层策略组合这些技能来安全解决下游任务。该方法不依赖显式安全奖励信号。

Result: 在多个安全RL环境和初步的LLM风格任务上进行了实验，结果表明该方法在没有显式安全奖励的情况下，能够显著降低安全成本，同时任务性能达到与使用真实安全信号训练的oracle方法相当的水平。

Conclusion: SCPRL框架能够有效地从人群偏好中发现和转移共享安全标准，为安全强化学习提供了一种无需显式安全奖励的有效解决方案，在保持任务性能的同时确保安全约束。
...
### [13] [Toward AI VIS Co-Scientists: A General and End-to-End Agent Harness for Solving Complex Data Visualization Tasks](https://arxiv.org/abs/2605.21825)
*Haichao Miao,Zhimin Li,Kuangshi Ai,Kaiyuan Tang,Chaoli Wang,Peer-Timo Bremer,Shusen Liu*

Main category: cs.AI

TL;DR: 提出一个端到端的智能体系统，能够仅基于数据和高级任务描述，自主设计和实现定制的可视化分析应用程序


<details>
  <summary>Details</summary>
Motivation: 复杂的科学数据分析通常需要跨领域专业知识（数据管理、分析、可视化设计等），这对科研人员构成了重大障碍。需要一个能够理解高级任务并自主执行的AI辅助系统

Method: 开发了一个VIS协同科学家系统，包含多个智能体和专门技能，协调探索性分析、规划、环境配置、实现、界面验证，并评估整体任务完成度。系统通过文档和指令工件指导下游工作并实现迭代优化

Result: 系统在IEEE SciVis竞赛数据集上进行了验证，能够仅基于数据和目标任务，自主生成功能完整的单页可视化应用程序，具有验证过的链接视图行为，高度定制化满足领域专家的特定任务需求

Conclusion: 该VIS协同科学家系统是实现更广泛的AI协同科学家愿景的重要一步，能够处理现实世界的复杂性（模糊需求、多样化数据模态、设计权衡、任务驱动验证），向自主执行长期任务的目标迈出了重要进展
...
TL;DR: FLUID框架是首个在生产规模直播推荐排序模型中完全消除候选侧物品ID的解决方案，通过跨域多模态编码器和离散层次编码来处理直播推荐中的持久冷启动问题。


<details>
  <summary>Details</summary>
Motivation: 直播推荐系统面临独特挑战：直播间通常只播放几十分钟，其物品ID保持在学习不足的持久冷启动状态，基于ID的协同过滤模型难以泛化。

Method: FLUID框架耦合跨域多模态编码器（联合训练短视频和直播内容产生离散层次编码LUCID），采用后期融合、无ID设计，将切片级和房间级LUCID作为独立token注入，并通过在线增量训练中的分阶段预热进行稳定。

Result: 在拥有超过10亿用户的工业级直播推荐系统中部署，FLUID实现了显著在线提升：+0.55%质量观看时长、+2.05%冷启动房间观看、+0.05%活跃小时数。

Conclusion: FLUID成功解决了直播推荐中的ID冷启动问题，通过消除候选侧物品ID并引入跨域多模态表示，显著提升了推荐性能，特别是在冷启动场景下。
...
### [31] [Adapting the Interface, Not the Model: Runtime Harness Adaptation for Deterministic LLM Agents](https://arxiv.org/abs/2605.22166)
...
### [32] [Measuring Cross-Modal Synergy: A Benchmark for VLM Explainability](https://arxiv.org/abs/2605.22168)
...
### [33] [LLM-Metrics: Measuring Research Impact Through Large Language Model Memory](https://arxiv.org/abs/2605.22176)
...
### [34] [Skill Weaving: Efficient LLM Improvement via Modular Skillpacks](https://arxiv.org/abs/2605.22205)
...
### [35] [CLORE: Content-Level Optimization for Reasoning Efficiency](https://arxiv.org/abs/2605.22211)
...
### [36] [Towards a compositional semantics for quantitative confidence assessment in assurance arguments](https://arxiv.org/abs/2605.22213)
...
### [37] [SGR-Bench: Benchmarking Search Agents on State-Gated Retrieval](https://arxiv.org/abs/2605.22219)
...
### [38] [Evaluating Large Language Models as Live Strategic Agents: Provider Performance, Hybrid Decomposition, and Operational Gaps in Timed Risk Play](https://arxiv.org/abs/2605.22238)
*H. C. Ekne*
...
### [39] [Unlocking Proactivity in Task-Oriented Dialogue](https://arxiv.org/abs/2605.22240)
...
### [40] [SciCore-Mol: Augmenting Large Language Models with Pluggable Molecular Cognition Modules](https://arxiv.org/abs/2605.22287)
...
### [41] [Cross-domain benchmarks reveal when coordinated AI agents improve scientific inference from partial evidence](https://arxiv.org/abs/2605.22300)
...
### [42] [Evaluation of Pipelines for Data Integration into Knowledge Graphs](https://arxiv.org/abs/2605.22304)
...
### [43] [Meta-Soft: Leveraging Composable Meta-Tokens for Context-Preserving KV Cache Compression](https://arxiv.org/abs/2605.22337)
...
### [44] [Scaling Observation-aware Planning in Uncertain Domains](https://arxiv.org/abs/2605.22364)
...
### [45] [Epicure: Navigating the Emergent Geometry of Food Ingredient Embeddings](https://arxiv.org/abs/2605.22391)
...
### [46] [S2ED: From Story to Executable Descriptions for Consistency-Aware Story Illustration](https://arxiv.org/abs/2605.22448)
...
### [47] [KAPPS: A knowledge-based CPPS Architecture for the Circular Factory](https://arxiv.org/abs/2605.22457)
...
### [49] [LACO: Adaptive Latent Communication for Collaborative Driving](https://arxiv.org/abs/2605.22504)
...
### [50] [Towards Direct Evaluation of Harness Optimizers via Priority Ranking](https://arxiv.org/abs/2605.22505)
...
### [51] [Search-E1: Self-Distillation Drives Self-Evolution in Search-Augmented Reasoning](https://arxiv.org/abs/2605.22511)
...
### [52] [Meta-Learning for Rapid Adaptation in Reference Tracking of Uncertain Nonlinear Systems](https://arxiv.org/abs/2605.22513)
...
### [53] [A Subjective Logic-based method for runtime confidence updates in safety arguments](https://arxiv.org/abs/2605.22530)
...
### [54] [TerminalWorld: Benchmarking Agents on Real-World Terminal Tasks](https://arxiv.org/abs/2605.22535)
...
### [55] [Think Thrice Before You Speak: Dual knowledge-enhanced Theory-of-Mind Reasoning for Persuasive Agents](https://arxiv.org/abs/2605.22602)
...
### [56] [Spreadsheet-RL: Advancing Large Language Model Agents on Realistic Spreadsheet Tasks via Reinforcement Learning](https://arxiv.org/abs/2605.22642)
...
### [57] [AtelierEval: Agentic Evaluation of Humans & LLMs as Text-to-Image Prompters](https://arxiv.org/abs/2605.22645)
...
### [58] [Claw AI Lab: An Autonomous Multi-Agent Research Team](https://arxiv.org/abs/2605.22662)
...
### [60] [Is Capability a Liability? More Capable Language Models Make Worse Forecasts When It Matters Most](https://arxiv.org/abs/2605.22672)
...
### [61] [Forecasting Scientific Progress with Artificial Intelligence](https://arxiv.org/abs/2605.22681)
...
### [62] [Beyond the Org Chart: AI and the Transformation of Invisible Work](https://arxiv.org/abs/2605.22707)
...
### [63] [AMEL: Accumulated Message Effects on LLM Judgments](https://arxiv.org/abs/2605.22714)
*Sid-ali Temkit*
...
### [64] [Parametric Modular Answer Set Programs Made Declarative](https://arxiv.org/abs/2605.22716)
...
### [65] [Can AI Make Conflicts Worse? An Alignment Failure in LLM Deployment Across Conflict Contexts](https://arxiv.org/abs/2605.22720)
*Andrii Kryshtal*
...
### [66] [Beyond Acoustic Emotion Recognition: Multimodal Pathos Analysis in Political Speech Using LLM-Based and Acoustic Emotion Models](https://arxiv.org/abs/2605.22732)
*Juergen Dietrich*
...
### [67] [Towards a General Intelligence and Interface for Wearable Health Data](https://arxiv.org/abs/2605.22759)
...
### [68] [Advancing Mathematics Research with AI-Driven Formal Proof Search](https://arxiv.org/abs/2605.22763)
...
### [69] [LCGuard: Latent Communication Guard for Safe KV Sharing in Multi-Agent Systems](https://arxiv.org/abs/2605.22786)
...
### [70] [Gated DeltaNet-2: Decoupling Erase and Write in Linear Attention](https://arxiv.org/abs/2605.22791)
...
### [71] [MOSS: Self-Evolution through Source-Level Rewriting in Autonomous Agent Systems](https://arxiv.org/abs/2605.22794)
...
### [72] [Temporal Contrastive Transformer for Financial Crime Detection: Self-Supervised Sequence Embeddings via Predictive Contrastive Coding](https://arxiv.org/abs/2605.21490)
*Danny Butvinik,Yonit Marcus,Nitzan Tal,Gabrielle Azoulay*

Main category: cs.LG

TL;DR: TCT是一个用于金融交易序列的时序对比Transformer框架，通过自监督对比学习捕获行为模式，为欺诈检测提供嵌入表示。实验显示其嵌入具有预测能力（AUC 0.8644），但与传统特征结合后未见明显改进，说明其学习到的表示与现有特征抽象有较大重叠。


<details>
  <summary>Details</summary>
Motivation: 金融交易具有复杂的时序动态特性，传统特征工程需要大量人工设计。研究者希望开发能够自动学习时序行为模式的表示学习方法，减少对特征工程的依赖，为欺诈检测等下游任务提供更好的特征表示。

Method: 提出Temporal Contrastive Transformer（TCT）框架，采用自监督对比学习目标训练模型，学习金融交易序列的时序动态表示。将学习到的嵌入作为特征输入到梯度提升分类器中进行欺诈检测评估。

Result: TCT学习到的嵌入单独使用时达到AUC 0.8644的预测性能，表明模型能够捕获非平凡的时序结构。但当与传统领域工程特征结合时，与基线（AUC 0.9245）相比未观察到可测量的改进（AUC 0.9205），显示学习到的表示与现有特征抽象存在较大重叠。

Conclusion: TCT是一个有前景的表示学习方法，能够捕获相关的行为信号，但在强领域特征基础上实现增量价值面临挑战。这些结果表明，在金融犯罪检测的时序表示学习发展中处于中间阶段，需要进一步研究模型架构、训练目标和集成策略。在现阶段，达到与强特征工程基线相当的性能本身就是一个有意义的结果。
...
### [73] [Teaching Language Models to Forecast Research Success Through Comparative Idea Evaluation](https://arxiv.org/abs/2605.21491)
*Srujan P Mule,Aniketh Garikaparthi,Manasi Patwardhan*

Main category: cs.LG

TL;DR: 论文研究了语言模型预测研究想法在实验前的实证成功能力，通过在PapersWithCode数据集上构建11,488个想法对，利用监督微调和强化学习等技术，使小模型达到77.1%的准确率，超过GPT-5的61.1%，证明了小模型可作为有效的客观验证器，为自主科学发现提供可扩展路径。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型加速科学研究，能够自动生成大量假设和实现方案，但新的瓶颈出现：如何在无需详尽实验的情况下评估和筛选数百个AI生成的想法。作者探讨语言模型是否能在实验运行前预测研究想法的实证成功。

Method: 研究相对实证预测任务：给定基准特定的研究目标和两个候选想法，预测哪个将获得更好的基准性能。构建基于PapersWithCode的11,488个想法对数据集。使用监督微调(SFT)和基于可验证奖励的强化学习(RLVR)技术，将评估任务构建为推理任务，使模型能够发现潜在的推理路径。

Result: 现成的8B参数模型表现不佳(30%准确率)，但监督微调将性能大幅提升至77.1%，超过GPT-5的61.1%。强化学习方法达到71.35%准确率，同时提供可解释的论证。通过消融实验和分布外测试，显示模型对表面级启发式方法具有鲁棒性，并能跨领域转移。

Conclusion: 研究结果表明，计算高效的小型语言模型可以作为有效的客观验证器，为自主科学发现提供了可扩展的路径。这解决了AI加速研究中的关键瓶颈问题，使得在实验前筛选和评估大量AI生成想法成为可能。
...
### [74] [The Attribution Impossibility: No Feature Ranking Is Faithful, Stable, and Complete Under Collinearity](https://arxiv.org/abs/2605.21492)
*Drake Caraker,Bryan Arnold,David Rhoads*

Main category: cs.LG

TL;DR: 论文证明了在特征共线性情况下，不可能同时实现特征排名的忠实性、稳定性和完整性。作者提出了DASH方法作为帕累托最优解决方案，并通过305个Lean 4定理进行了机械验证。


<details>
  <summary>Details</summary>
Motivation: 当特征存在共线性时，特征重要性排名会变得不稳定和不一致，这给可解释AI带来了根本性挑战。现有方法无法同时满足忠实性、稳定性和完整性三个关键要求。

Method: 1. 证明不可能定理：无法同时实现忠实、稳定和完整的特征排名
2. 提出DASH（Diversified Aggregation of SHAP）方法：通过集成平均解决该问题
3. 使用Lean 4定理进行形式化验证（305个定理，16个公理）
4. 开发实用诊断工具：Z检验工作流和单模型筛选工具

Result: 1. 完全归因设计空间只有两个方法族：忠实-完整方法（不稳定）和集成方法（稳定）
2. DASH在无偏聚合中达到帕累托最优，实现Cramér-Rao方差下界
3. 在77个公开数据集中，68%存在归因不稳定性
4. 提供了定量分析：梯度提升的归因比以1/(1-ρ²)发散，Lasso无限大，随机森林收敛

Conclusion: 特征共线性在可解释AI中造成了根本性限制，基于SHAP的公平性审计在共线性情况下不可靠。作者的形式化验证框架为可解释AI提供了理论基础和实践指导，DASH方法提供了实用解决方案。
...
### [75] [Double descent for least-squares interpolation on contaminated data: A simulation study](https://arxiv.org/abs/2605.21494)
*Tino Werner*

Main category: cs.LG

TL;DR: 研究发现，在带污染的训练数据中，最小二乘插值器在高度过参数化时会出现"双重下降"现象，泛化性能超过稳健方法


<details>
  <summary>Details</summary>
Motivation: 过参数化模型虽然在经典统计理论中容易过拟合，但实际表现出色泛化性能。"双重下降"现象表明模型复杂度超过特定阈值后泛化误差会下降。稳健统计用于处理污染数据，但实际数据可能不满足其假设。研究探讨在带污染数据的线性回归中是否也存在双重下降现象

Method: 在线性回归设置中使用被污染的训练数据，比较高度非稳健的最小二乘插值估计器与几种稳健替代方法的性能

Result: 研究发现大的过参数化确实允许出现双重下降现象，导致最小二乘插值器具有非常好的泛化性能，超过了稳健替代方法

Conclusion: 在污染训练数据的线性回归中，高度过参数化的最小二乘插值器可以表现出双重下降现象，并达到优于稳健方法的泛化性能，这对传统稳健统计假设提出了有趣的挑战
...
TL;DR: HealthCraft是第一个公开的强化学习环境，用于在真实急诊医学条件下评估语言模型的轨迹级安全性，结果显示前沿模型在复杂工作流程中表现接近零


<details>
  <summary>Details</summary>
Motivation: 前沿语言模型在临床工作流程中的部署速度快于安全评估基础设施的建设速度。静态医学问答基准无法捕捉急诊医学中的关键失效模式：轨迹级安全崩溃、工具误用和持续临床压力下的妥协

Method: 基于FHIR R4世界状态构建强化学习环境，包含14种实体类型和3,987个种子实体，暴露24个MCP工具。采用双层评分标准，当任何安全关键标准被违反时奖励归零。包含195个任务，分为六个类别，基于2,255个二元标准（515个安全关键）进行评估

Result: 两个前沿模型表现：Claude Opus 4.6 Pass@1为24.8%，GPT-5.4为12.6%；安全失败率分别为27.5%和34.0%。在多步工作流程（最接近真实急诊护理）中，性能崩溃至接近零（Claude 1.0%，GPT-5.4 0.0%）。基础设施修复改变了模型的相对表现排名

Conclusion: HealthCraft环境揭示了现有语言模型在真实急诊医学工作流程中的严重安全缺陷，强调了轨迹级评估的重要性。基础设施保真度是测量的一部分，奖励信号不适合直接用于训练。该环境已开源，为安全评估提供了重要工具
...
### [77] [A Reproducible Log-Driven AutoML Framework for Interpretable Pipeline Optimization in Healthcare Risk Prediction](https://arxiv.org/abs/2605.21528)
*Rui Huang,Lican Huang*

Main category: cs.LG

TL;DR: 提出了yvsoucom-iterkit，一个确定性的、基于日志驱动的AutoML框架，将管道优化编码为可追踪的日志实体，通过分析组件归因、交互、相似性和跨种子鲁棒性来优化疾病风险预测


<details>
  <summary>Details</summary>
Motivation: 疾病风险预测面临特征异质性、样本有限和严重类别不平衡等挑战，需要开发可复现且高效的AutoML框架来解决这些问题

Method: yvsoucom-iterkit框架将每个管道编码为可追踪的日志实体，分析了超过18,000个管道配置，使用随机森林重要性分析来识别关键组件，进行组件相似性分析和跨种子鲁棒性分析

Result: 在Pima和Stroke数据集上，识别出增强(0.454)、模型选择(0.198)和不平衡处理(0.101)是关键驱动因素；组件分析显示强冗余性；集成模型实现了高性能(F1分数0.88-0.94)；跨种子分析显示了性能-鲁棒性权衡

Conclusion: 有效的AutoML优化可以专注于减少的高影响力组件集，yvsoucom-iterkit框架提供了一种可复现、可追踪的方法来优化疾病风险预测管道
...
### [78] [DualOptim+: Bridging Shared and Decoupled Optimizer States for Better Machine Unlearning in Large Language Models](https://arxiv.org/abs/2605.21539)
*Xuyang Zhong,Qizhang Li,Yiwen Guo,Chen Liu*

Main category: cs.LG

TL;DR: DualOptim+是一种用于改进大语言模型机器遗忘的新型优化框架，通过基础状态和增量状态的设计，在遗忘与保留目标之间实现更好的权衡，并提供8位量化版本减少内存开销。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型机器遗忘方法在同时处理遗忘和保留目标时面临挑战，需要在不同目标之间实现更好的权衡，同时减少内存开销。

Method: 提出DualOptim+框架：1）引入基础状态捕捉遗忘和保留目标的共同表示；2）使用增量状态保留目标特定的残差；3）基于遗忘和保留梯度之间的方向冲突，自适应地在共享状态和分离状态之间桥接；4）开发8位量化版本减少内存开销。

Result: 在虚构和真实世界遗忘、安全对齐和多任务学习任务上的大量实验表明，DualOptim+在不同目标之间始终实现优越的权衡，且量化版本在不损害性能的情况下显著减少内存开销。

Conclusion: DualOptim+为机器遗忘提供了一个有效且高效的优化框架，在保持模型性能的同时实现更好的目标权衡，其量化版本进一步提高了实用性。
...
### [79] [Provable Joint Decontamination for Benchmarking Multiple Large Language Models](https://arxiv.org/abs/2605.21543)
*Zhenlong Liu,Hao Zeng,Hongxin Wei*

Main category: cs.LG

TL;DR: 提出一种名为JECS的联合包络符合选择方法，用于多模型基准数据去污染，能够全局污染率控制并保持跨模型公平比较


<details>
  <summary>Details</summary>
Motivation: 基准数据污染已成为LLM评估中的核心挑战，传统基于分数的方法缺乏理论保证，而现有的符合方法单独应用于每个模型会产生模型特定基准，破坏跨模型公平比较

Method: 将多模型基准去污染形式化为联合选择问题，提出JECS方法：计算每个模型的符合p值，通过每项最大值进行聚合，从右尾观测重建max-p零分布保守包络，最后应用自适应BH程序选择基准

Result: 在各种模型和基准上的广泛实验表明，JECS比max-p基线方法具有更高的功效，同时一致地保持目标全局污染率控制

Conclusion: JECS方法能够解决多模型基准数据污染问题，提供有理论保证的全局污染率控制，并在实践中展现出比基线方法更好的性能
...
### [144] [Beyond Euclidean Proximity: Repairing Latent World Models with Horizon-Matched Trajectory Reachability Metrics](https://arxiv.org/abs/2605.22164)
...
### [145] [Maestro: Reinforcement Learning to Orchestrate Hierarchical Model-Skill Ensembles](https://arxiv.org/abs/2605.22177)
...
### [146] [IKNO: Infinite-order Kernel Neural Operators](https://arxiv.org/abs/2605.22182)
...
### [147] [From Sequential Nodes to GPU Batches: Parallel Branch and Bound for Optimal $k$-Sparse GLMs](https://arxiv.org/abs/2605.22188)
...
### [149] [Reinforced Graph of Thoughts: RL-Driven Adaptive Prompting for LLMs](https://arxiv.org/abs/2605.22195)
...
### [150] [Survive or Collapse: The Asymmetric Roles of Data Gating and Reward Grounding in Self-Play RL](https://arxiv.org/abs/2605.22217)
...
### [151] [Can Transformers Learn to Verify During Backtracking Search?](https://arxiv.org/abs/2605.22221)
...
### [152] [ARC-STAR: Auditable Post-Hoc Correction for PDE Foundation Models](https://arxiv.org/abs/2605.22222)
...
### [153] [How Many Different Outputs Can a Transformer Generate?](https://arxiv.org/abs/2605.22223)
...
### [154] [Holomorphic Neural ODEs with Kolmogorov-Arnold Networks for Interpretable Discovery of Complex Dynamics](https://arxiv.org/abs/2605.22235)
...
### [155] [Decomposing Ensemble Spread in Lorenz '96 With Learned Stochastic Parameterizations](https://arxiv.org/abs/2605.22242)
...
### [156] [Explainable AI for Data-Driven Design of High-Dimensional Predictive Studies](https://arxiv.org/abs/2605.22243)
...
### [157] [No Epoch Like the Present: Robust Climate Emulation Requires Out-of-Distribution Generalisation](https://arxiv.org/abs/2605.22248)
...
### [158] [What are the Right Symmetries for Formal Theorem Proving?](https://arxiv.org/abs/2605.22257)
...
### [159] [An Evidence Hierarchy for Bayesian Object Classification via OSINT-Aided Heterogeneous Sensor Fusion](https://arxiv.org/abs/2605.22259)
...
### [160] [Tailoring Teaching to Aptitude: Direction-Adaptive Self-Distillation for LLM Reasoning](https://arxiv.org/abs/2605.22263)
...
### [161] [Detecting Atypical Clients in Federated Learning via Representation-Level Divergence](https://arxiv.org/abs/2605.22266)
...
### [162] [Adaptive Measurement Allocation for Learning Kernelized SVMs Under Noisy Observations](https://arxiv.org/abs/2605.22275)
*Artur Miroszewski*
...
### [163] [EmoTrack: Robust Depression Tracking from Counseling Transcripts across Session Regimes](https://arxiv.org/abs/2605.22286)
...
### [164] [Long-term Fairness with Selective Labels](https://arxiv.org/abs/2605.22291)
...
### [165] [One LR Doesn't Fit All: Heavy-Tail Guided Layerwise Learning Rates for LLMs](https://arxiv.org/abs/2605.22297)
...
### [166] [Chebyshev Policies and the Mountain Car Problem: Reinforcement Learning for Low-Dimensional Control Tasks](https://arxiv.org/abs/2605.22305)
...
### [168] [VeriScale: Adversarial Test-Suite Scaling for Verifiable Code Generation](https://arxiv.org/abs/2605.22368)
...
### [169] [Riemannian geometry meets fMRI: the advantages of modeling correlation manifolds and eigenvector subspaces](https://arxiv.org/abs/2605.22334)
...
### [170] [Don't Forget the Critic: Value-Based Data Rehearsal for Multi-Cyclic Continual Reinforcement Learning](https://arxiv.org/abs/2605.22454)
...
### [171] [Learning Causal Orderings for In-Context Tabular Prediction](https://arxiv.org/abs/2605.22335)
...
### [172] [Physics-Informed Generative Solver: Bridging Data-Driven Priors and Conservation Laws for Stable Spatiotemporal Field Reconstruction](https://arxiv.org/abs/2605.22338)
...
### [173] [Implicit Regularization of Mini-Batch Training in Graph Neural Networks](https://arxiv.org/abs/2605.22480)
...
### [174] [From Snapshots to Trajectories: Learning Single-Cell Gene Expression Dynamics via Conditional Flow Matching](https://arxiv.org/abs/2605.22340)
...
### [175] [Understanding Multimodal Failure in Action-Chunking Behavioral Cloning](https://arxiv.org/abs/2605.22493)
...
### [176] [A Boundary-Layer Mechanism for One-Third Scaling in Online Softmax Classification](https://arxiv.org/abs/2605.22341)
...
### [177] [The Neural Compiler: Program-to-Network Translation for Hybrid Scientific Machine Learning](https://arxiv.org/abs/2605.22498)
*Lucas Sheneman*
...
### [179] [Stabilising Explainability Fragility in Cybersecurity AI: The Impact and Mitigation of Multicollinearity in Public Benchmark Datasets](https://arxiv.org/abs/2605.22529)
...
### [180] [MoSA: Motion-constrained Stress Adaptation for Mitigating Real-to-Sim Gap in Continuum Dynamics via Learning Residual Anisotropy](https://arxiv.org/abs/2605.22597)
...
### [181] [ASAP: Attention Sink Anchored Pruning](https://arxiv.org/abs/2605.22372)
...
### [185] [Target-Aligned Bellman Backup for Cross-domain Offline Reinforcement Learning](https://arxiv.org/abs/2605.22376)
*Wei Liu,Ting Long*
...
### [186] [Towards Explainability of SLMs by investigating Token Level Activation](https://arxiv.org/abs/2605.22377)
...
### [187] [Efficient Higher-order Subgraph Attribution via Message Passing](https://arxiv.org/abs/2605.22385)
...
### [188] [Proxy-Based Approximation of Shapley and Banzhaf Interactions](https://arxiv.org/abs/2605.22738)
...
### [189] [Hybrid Kolmogorov-Arnold Network and XGBoost Framework for Week-Ahead Price Forecasting in Australia's National Electricity Market](https://arxiv.org/abs/2605.22387)
...
### [190] [Cyber-Physical Anomaly Detection in IoT-Enabled Smart Grids Using Machine Learning and Metaheuristic Feature Optimization](https://arxiv.org/abs/2605.22749)
...
### [191] [A Posterior-Predictive Variance Decomposition for Epistemic and Aleatoric Uncertainty in Wind Power Forecasting](https://arxiv.org/abs/2605.22390)
...
### [192] [CogAdapt: Transferring Clinical ECG Foundation Models to Wearable Cognitive Load Assessment via Lead Adaptation](https://arxiv.org/abs/2605.22774)
...
### [193] [Cross-Species RSA Reveals Conserved Early Visual Alignment but Divergent Higher-Area Rankings Across Human fMRI and Macaque Electrophysiology](https://arxiv.org/abs/2605.22401)
*Nils Leutenegger*
...
### [194] [MambaGaze: Bidirectional Mamba with Explicit Missing Data Modeling for Cognitive Load Assessment from Eye-Gaze Tracking Data](https://arxiv.org/abs/2605.22775)
*Amir Mousavi,Mohammad Sadegh Sirjani,Erfan Nourbakhsh,Mimi Xie,Rocky Slavin,Leslie Neely,John Davis,John Quarles*
...
### [195] [SDPM: Survival Diffusion Probabilistic Model for Continuous-Time Survival Analysis](https://arxiv.org/abs/2605.22776)
...
### [196] [The Matching Principle: A Geometric Theory of Loss Functions for Nuisance-Robust Representation Learning](https://arxiv.org/abs/2605.22800)
*Vishal Rajput*
...
### [197] [AMUSE: Anytime Muon with Stable Gradient Evaluation](https://arxiv.org/abs/2605.22432)
...
### [198] [Vector Policy Optimization: Training for Diversity Improves Test-Time Search](https://arxiv.org/abs/2605.22817)
...
### [199] [Lost in Tokenization: Fundamental Trade-offs in Graph Tokenization for Transformers](https://arxiv.org/abs/2605.22471)
...
### [200] [Winner-Take-All bottlenecks enforce disentangled symbolic representations in multi-task learning](https://arxiv.org/abs/2605.22472)
...
### [201] [Structured-Sparse Attention for Entity Tracking with Subquadratic Sequence Complexity](https://arxiv.org/abs/2605.22476)
...
### [202] [When Stronger Triggers Backfire: A High-Dimensional Theory of Backdoor Attacks](https://arxiv.org/abs/2605.22481)
...
### [203] [Represented Is Not Computed: A Causal Test of Candidate Algorithmic Intermediates in a Transformer](https://arxiv.org/abs/2605.22488)
...
### [204] [The Signal in the Noise: OOD Detection Through Goodness-of-Fit Testing in Factorised Latent Spaces](https://arxiv.org/abs/2605.22496)
...
### [205] [Generative Modeling by Value-Driven Transport](https://arxiv.org/abs/2605.22507)
...
### [206] [Disentanglement Beyond Generative Models with Riemannian ICA](https://arxiv.org/abs/2605.22531)
*Edmond Cunningham*
...
### [207] [Relational Linear Properties in Language Models: An Empirical Investigation](https://arxiv.org/abs/2605.22532)
...
### [208] [F-TIS: Harnessing Diverse Models in Collaborative GRPO](https://arxiv.org/abs/2605.22537)
...
### [209] [ImplicitTerrainV2: Wavelet-Guided Spatially Adaptive Neural Terrain Representation](https://arxiv.org/abs/2605.22556)
...
### [210] [Neural Flow Operators can Approximate any Operator: Abstract Frameworks and Universal Approcimations](https://arxiv.org/abs/2605.22557)
...
### [212] [GraphFlow: A Graph-Based Workflow Management for Efficient LLM-Agent Serving](https://arxiv.org/abs/2605.22566)
...
### [213] [A Tutorial on Diffusion Theory: From Differential Equations to Diffusion Models](https://arxiv.org/abs/2605.22586)
*Jiayi Fu,Yuxia Wang*
...
### [214] [Do Deep Ensembles Actually Capture Uncertainty in Graph Neural Networks?](https://arxiv.org/abs/2605.22593)
...
### [215] [Factored Diffusion Policies:Compositionally Generalized Robot Control with a Single Score Network](https://arxiv.org/abs/2605.22596)
...
### [216] [Benchmarking Machine Learning Architectures for Antimicrobial Stewardship in Pediatric ICUs](https://arxiv.org/abs/2605.22611)
...
### [221] [Why SGD is not Brownian Motion: A New Perspective on Stochastic Dynamics](https://arxiv.org/abs/2605.22644)
...
### [222] [Posterior Collapse as Automatic Spectral Pruning](https://arxiv.org/abs/2605.22691)
*Johannes Hirn*
...
### [223] [Clipping Bottleneck: Stabilizing RLVR via Stochastic Recovery of Near-Boundary Signals](https://arxiv.org/abs/2605.22703)
...
### [225] [Multiple Neural Operators Achieve Near-Optimal Rates for Multi-Task Learning](https://arxiv.org/abs/2605.22724)
...
### [226] [Ternary Decision Trees with Locally-Adaptive Uncertainty Zones](https://arxiv.org/abs/2605.22740)
*William Smits*
...
### [227] [SeqLoRA: Bilevel Orthogonal Adaptation for Continual Multi-Concept Generation](https://arxiv.org/abs/2605.22743)
...
### [228] [Plug-in Losses for Evidential Deep Learning: A Simplified Framework for Uncertainty Estimation that Includes the Softmax Classifier](https://arxiv.org/abs/2605.22746)
...
### [231] [Remember to be Curious: Episodic Context and Persistent Worlds for 3D Exploration](https://arxiv.org/abs/2605.22814)
...
### [232] [Integrable Elasticity via Neural Demand Potentials](https://arxiv.org/abs/2605.22820)
