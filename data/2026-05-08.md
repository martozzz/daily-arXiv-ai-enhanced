...
### [1] [Nationwide EHR-Based Chronic Rhinosinusitis Prediction Using Demographic-Stratified Models](https://arxiv.org/abs/2605.05213)
...
TL;DR: 利用全国性电子健康记录数据，通过混合特征选择和人口分层建模，提升慢性鼻窦炎的早期预测准确率。


<details>
  <summary>Details</summary>
Motivation: 慢性鼻窦炎作为一种常见的异质性炎症性疾病，早期识别困难，症状与其他常见病症重叠，且既往预测研究多依赖单一机构队列，限制了其在人口层面的泛化能力。

Method: 结合基于流行度的统计筛选和模型重要性排名的混合特征选择流程，从约11万个候选医疗编码中压缩至100个可解释特征；针对六个不同性别和生命阶段的成年亚组进行人口统计学分层建模和亚组特定的超参数调优。

Result: 该框架总体AUC达到0.8461，较最佳基线模型提升了0.0168，证实了常规收集的电子健康记录数据可支持具有人口代表性的慢性鼻窦炎风险分层。

Conclusion: 本研究证明，利用全国性的纵向电子健康记录数据进行精准的特征选择和人口统计学分层建模，可以有效提升慢性鼻窦炎的早期识别能力，为初级诊疗中的风险分层和转诊优先排序提供了有力支持。
...
TL;DR: SAT通过去中心化的顺序更新方法训练小型LLM团队，解决了多智能体联合训练的分布漂移问题，使得小模型团队性能超越大模型，并支持即插即用式模型升级。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型（LLM）参数规模大、部署成本高，而使用多个小型高效LLM组成的团队虽有望替代单个大模型，但联合更新多个智能体会导致复合分布漂移，使得训练时的协调和稳定性成为难题。

Method: SAT将团队表示为因子化策略，采用块坐标更新方法对单个智能体进行轮换更新，避免同时更新带来的分布漂移问题。具体包括：1）设计序列感知的在线优势估计器，以条件化于演化的团队策略；2）为每个智能体设置KL信任区域，以隔离占用漂移。

Result: 实验表明，使用SAT训练的3个4B智能体（总计12B参数）在AIME24/25基准测试中，平均表现超越Qwen3-32B模型3.9%。验证即插即用理论时，替换入两个8B智能体可使综合得分提升10.4%。

Conclusion: 本研究提出了Sequential Agent Tuning（SAT）训练范式，通过分解策略表示、顺序更新的方式，消除了对集中控制器的需求，实现了去中心化训练。该方法不仅理论上保障了单调性能提升和即插即用不变性，而且实验证明，12B参数的小模型团队可以超越32B的大模型性能，为高效部署LLM提供了新的技术路径。
...
TL;DR: 填补了预测多样性与混沌动力学之间的理论空白，提出了水平约束拉什莫尔集框架，证明混沌导致模型多样性指数衰减，并开发了基于下游效用的决策对齐选择算法，显著提升混沌系统中的决策质量。


<details>
  <summary>Details</summary>
Motivation: 预测多样性和混沌动力学是机器学习中的两个基本挑战，尽管其概念存在联系，却一直独立发展。本研究旨在弥合这一理论空白，探索在混沌系统中模型多样性如何随时间演变，为安全关键领域的机器学习部署提供原则性指导。

Method: 1. 提出水平约束的拉什莫尔集理论框架，描述混沌系统中模型多样性如何随时间演变；2. 证明有效拉什莫尔集以最大Lyapunov指数决定的速率随预测时间指数收缩；3. 引入Lyapunov加权度量以更紧地界定预测分歧；4. 开发基于下游效用而非单纯预测精度的决策对齐选择算法

Result: 在合成混沌系统（Lorenz-96, Kuramoto-Sivashinsky）和实际应用（风电、交通、天气）上的广泛实验表明，该框架将决策质量提高了18-34%，同时保持了具有竞争力的预测性能。

Conclusion: 本文在混沌理论与预测多样性之间建立了首个严谨的数学联系，为在安全关键的混沌系统中部署机器学习提供了理论基础和实用指导。通过引入水平约束的拉什莫尔集、Lyapunov加权度量等概念框架，以及基于下游效用的决策对齐选择算法，研究证明这种方法能够在不牺牲预测性能的情况下显著提升决策质量。
...
### [4] [MidSteer: Optimal Affine Framework for Steering Generative Models](https://arxiv.org/abs/2605.05220)
*Tatiana Gaintseva,Andrew Stepanov,Ziquan Liu,Martin Benning,Gregory Slabaugh,Jiankang Deng,Ismail Elezi*

Main category: cs.LG

TL;DR: 论文为生成模型中介表征导向建立理论框架，连接概念擦除数学原理，提出最小扰动概念导向方法MidSteer，在各类任务中性能优越。


<details>
  <summary>Details</summary>
Motivation: 中介表征导向在生成模型控制、部署后对齐和安全设置中虽经验上有效，但缺乏全面理论框架。本研究旨在填补这一理论空白，为概念导向提供严格理论基础。

Method: 首先建立概念导向与仿射概念擦除的理论联系，证明标准移除方法只是特例；然后基于此理论分析，设计LEACE-Switch框架描述概念切换的最优仿射解；最终提出更一般化的MidSteer框架，在最小扰动原则下实现定向概念变换。

Result: 理论分析建立了概念导向与仿射概念擦除的数学联系；LEACE-Switch框架在松弛假设下提供最优仿射解；MidSteer框架在视觉扩散模型和大型语言模型等多种架构中表现优于现有方法。

Conclusion: 中介表征导向是控制生成模型的有效策略，MidSteer框架通过最小扰动原则在各种任务和架构中实现性能优于现有方法，为概念操作提供了更通用和理论严谨的解决方案。
...
### [5] [Adaptive Computation Depth via Learned Token Routing in Transformers](https://arxiv.org/abs/2605.05222)
*Ahmed Abdelmuniem Abdalla Mohammed*

Main category: cs.LG

TL;DR: 提出Token-Selective Attention (TSA)，一种轻量级、可学习的门控机制，使Transformer能够根据上下文难度动态跳过token计算，节省14-23%计算量且质量损失小于0.5%。


<details>
  <summary>Details</summary>
Motivation: 标准的Transformer架构对每个token都应用相同数量的层，而不考虑上下文的难度差异。这种计算资源的均匀分配不够高效，因为不同token所需的处理复杂度可能不同。

Method: 提出Token-Selective Attention (TSA)，一种可学习的逐token门控机制，位于连续Transformer块之间的残差更新上。每个门由一个轻量级的两层MLP实现，产生连续的停止概率，使得整个机制可端到端微分，仅增加1.7%的参数开销，且无需修改基础架构。

Result: 1. 即使在无深度正则化（λ=0）的情况下，仅任务损失梯度就能驱动路由器跳过20%的token-层操作；
2. 在字符级语言建模任务上，在Tiny-Shakespeare和enwik8数据集上节省了14-23%的token-层操作，质量损失小于0.5%；
3. 在相同效率下，TSA比早期退出方法实现0.7%更低的验证损失；
4. 学到的路由可以直接迁移到推理时的稀疏执行，带来实际的运行时间加速。

Conclusion: TSA证明了通过在Transformer架构中引入轻量级的逐token门控机制，可以学习根据上下文难度动态分配计算资源，实现更高效的推理，并且学到的路由策略可以无缝迁移到推理阶段带来实际加速。
...
### [6] [Structural Instability of Feature Composition](https://arxiv.org/abs/2605.05223)
*Yunpeng Zhou*

Main category: cs.LG

TL;DR: 本研究探讨了稀疏自动编码器中组合特征调控的不稳定性，提出了一个几何框架来推导组合崩溃阈值，发现高偏置机制下干扰会系统性积累，并在CLEVR语义特征上验证了模型的预测。


<details>
  <summary>Details</summary>
Motivation: 稀疏自动编码器（SAEs）已成为解耦Transformer架构中特征叠加的有力范例，能用来调控激活，但组合调控（同时激活不同语义隐变量）的理论基础未得到充分探索。当前广为接受的线性表示假说往往忽略了超完备字典带来的非线性干扰效应。本研究旨在研究特征并集的不稳定性，为组合调控的可扩展性奠定理论基础。

Method: 研究者建立了一套几何框架来建模组合调控的不稳定性：将激活空间视作高维稀疏锥形流形，在球面字典模型下推导了渐近组合崩溃阈值，并分析了高斯宽度表征的信号锥维度。在高偏置机制下，他们还分析了ReLU整流如何将噪声转变为系统性漂移，并研究相关性的影响。通过在CLEVR数据集提取语义特征进行验证。

Result: 分析得出了组合崩溃的阈值条件，发现其与信号锥的高斯宽度相关；发现高偏置机制下ReLU整流会将微观相关引发的涨落转变为系统性漂移，随着组合不断累积，干扰会以棘轮效应增长；在CLEVR数据集的结构化语义特征上验证了理论预测，发现层次相关性会加速组合不稳定性的发生。

Conclusion: 研究揭示了基于稀疏自动编码器的组合特征调控存在基本几何限制；随机字典模型下组合会趋于崩溃；在存在相关性时，组合的稳定性更差；这推动了对超出朴素线性叠加的、能显式管理干扰的组合机制的需求。
...
### [7] [Channel-Level Semantic Perturbations: Unlearnable Examples for Diverse Training Paradigms](https://arxiv.org/abs/2605.05224)
*Bo Wang,Jia Ni,Mengnan Zhao,Zhan Qin,Kui Ren*

Main category: cs.LG

TL;DR: 研究揭示预训练权重会显著削弱现有不可学习样本方法效果，并提出SSC方法来解决这一局限性。


<details>
  <summary>Details</summary>
Motivation: 当前不可学习样本(UEs)研究主要评估从零开始训练场景，而在广泛使用的预训练微调(PF)范式下行为特征尚未被系统探索，存在研究空白。

Method: 基于语义过滤机制的分析，提出浅层语义伪装(SSC)方法，通过将生成过程限制在语义有效子空间内来绕过预训练权重的语义抑制。

Result: 实验表明SSC方法在不同训练范式下(包括浅层冻结和语义聚焦预训练)都保持数据不可学习性，而现有UE方法在PF范式中有效性显著降低。

Conclusion: 本研究提出了层次化欺骗策略SSC，能在预训练微调范式中有效保持数据不可学习性，弥合了预训练场景下不可学习学习的关键空白。
...
### [8] [MACS: Modality-Aware Capacity Scaling for Efficient Multimodal MoE Inference](https://arxiv.org/abs/2605.05225)
*Bo Li,Chuan Wu,shaolin Zhu*

Main category: cs.LG

TL;DR: 提出MACS框架解决多模态MoE模型推理效率问题，通过熵加权负载和动态模态适配容量机制，显著提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 混合专家多模态大语言模型在专家并行推理中存在显著效率瓶颈，主要由于滞后效应。在多模态语境下，现有的基于令牌数量的负载均衡方法面临两个独特挑战：信息异质性和模态动态性，导致资源分配不当和效率低下。

Method: 提出了MACS训练免费推理框架，包含两个关键机制：1) 熵加权负载机制，通过量化视觉标记的语义价值来解决信息异质性；2) 动态模态适配容量机制，根据输入的实时模态组成动态分配专家资源。

Result: 大量实验表明，MACS在各种多模态基准测试中显著优于现有方法，为MoE MLLMs的专家并行推理部署提供了新颖且稳健的高效解决方案。

Conclusion: MACS在MoE MLLMs的高效推理中提供了创新解决方案，通过熵加权负载和动态模态适配容量两种机制，有效解决了信息异质性和模态动态性两个核心挑战，为未来多模态大型语言模型的专家并行推理部署提供了新的范式。
...
### [9] [Internalizing Outcome Supervision into Process Supervision: A New Paradigm for Reinforcement Learning for Reasoning](https://arxiv.org/abs/2605.05226)
*Fei Ding,Yongkang Zhang,Runhao Liu,Yuhao Liao,Zijian Zeng,Sibo wang,Huiming Yang*

Main category: cs.LG

TL;DR: 提出强化学习推理的新视角：将结果监督内化为过程监督。通过识别、纠正和重用失败的推理轨迹来自动提取过程级学习信号，实现更细粒度的策略优化。


<details>
  <summary>Details</summary>
Motivation: 强化学习推理面临的核心挑战在于如何将仅在序列末端提供的结果反馈转化为能够指导中间推理步骤的细粒度学习信号。现有方法要么依赖结果级奖励进行序列级优化（导致信用分配困难），要么需要外部构建过程监督（成本高且难以持续扩展）。

Method: 提出监督内化方法，让模型在强化学习中自动抽取过程级学习信号：1) 识别失败的推理轨迹，2) 定位并纠正导致错误的关键推理步骤，3) 重用修正后的轨迹进行策略优化。进一步将该思路抽象为一种新的训练范式，在强化学习过程中持续生成和精炼内部过程监督。

Result: 提出的监督内化方法使模型能够在仅有结果监督的情况下实现更细粒度的策略优化，为强化学习推理任务中的信用分配问题提供了一个可持续、可扩展的解决方案。

Conclusion: 该研究提出了一个名为监督内化的强化学习推理新范式，通过让模型自动识别、纠正和重用失败的推理轨迹，将结果级监督内化为过程级监督信号，为强化学习推理任务提供了一种可持续、可扩展的细粒度信用分配方法。
...
### [10] [Rethinking Data Curation in LLM Training: Online Reweighting Offers Better Generalization than Offline Methods](https://arxiv.org/abs/2605.05227)
*Wanru Zhao,Yihong Chen,Yuzhi Tang,Wentao Ma,Shengchao Hu,Shell Xu Hu,Alex Iacob,Abhinav Mehrotra,Nicholas D. Lane*

Main category: cs.LG

TL;DR: 提出ADAPT在线数据加权框架，通过动态调整样本重要性改进LLM训练数据管理，显著提升模型泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统大规模语言模型训练中的数据筛选方法存在离线处理与训练分离、调整过程繁琐、牺牲数据多样性、泛化能力受损等问题，需要一种在线动态的数据管理方法。

Method: 提出ADAPT框架，采用基于相似性的质量信号引导的自适应每样本学习率，动态调整训练样本权重，形成隐式课程学习机制，而不改变训练样本数量。

Result: 在指令微调和大规模预训练实验中，ADAPT在相同FLOPs条件下，一致超越了离线筛选/混合方法和之前的在线方法，获得了更强的跨基准泛化能力。

Conclusion: 本研究提出ADAPT作为一种创新的在线数据加权框架，成功解决了传统离线数据筛选方法的局限，并通过动态调整样本重要性实现了更优的泛化性能，为LLM数据管理提供了新的研究方向和实用工具。
...
### [11] [Evolutionary fine tuning of quantized convolution-based deep learning models](https://arxiv.org/abs/2605.05228)
*Marcin Pietroń*
...
### [12] [Expert Routing for Communication-Efficient MoE via Finite Expert Banks](https://arxiv.org/abs/2605.05278)
...
### [13] [Forecasting Green Skill Demand in the Automotive Industry: Evidence from Online Job Postings](https://arxiv.org/abs/2605.05280)
...
### [15] [Graph Normalization: Fast Binarizing Dynamics for Differentiable MWIS](https://arxiv.org/abs/2605.05330)
*Laurent Guigues*
...
### [16] [Feature Starvation as Geometric Instability in Sparse Autoencoders](https://arxiv.org/abs/2605.05341)
...
### [17] [Balancing Stability and Plasticity in Sequentially Trained Early-Exiting Neural Networks](https://arxiv.org/abs/2605.05358)
...
### [18] [COPYCOP: Ownership Verification for Graph Neural Networks](https://arxiv.org/abs/2605.05360)
...
### [19] [SPADE: Faster Drug Discovery by Learning from Sparse Data](https://arxiv.org/abs/2605.05370)
...
### [20] [Neural Co-state Policies: Structuring Hidden States in Recurrent Reinforcement Learning](https://arxiv.org/abs/2605.05373)
...
### [21] [Conditional Diffusion Under Linear Constraints: Langevin Mixing and Information-Theoretic Guarantees](https://arxiv.org/abs/2605.05387)
...
### [22] [Two-Stage Learned Decomposition for Scalable Routing on Multigraphs](https://arxiv.org/abs/2605.05389)
...
### [23] [Differentiable Parameter Optimization for DAEs with State-Dependent Events](https://arxiv.org/abs/2605.05395)
...
### [24] [Information Theoretic Adversarial Training of Large Language Models](https://arxiv.org/abs/2605.05415)
...
### [25] [Active Learning for Conditional Generative Compressed Sensing](https://arxiv.org/abs/2605.05435)
...
### [26] [On Semantic Loss Fine-Tuning Approach for Preventing Model Collapse in Causal Reasoning](https://arxiv.org/abs/2605.05438)
...
### [27] [Robustness of Graph Self-Supervised Learning to Real-World Noise: A Case Study on Text-Driven Biomedical Graphs](https://arxiv.org/abs/2605.05463)
...
### [28] [A Unified Benchmark for Evaluating Knowledge Graph Construction Methods and Graph Neural Networks](https://arxiv.org/abs/2605.05476)
*Othmane Kabal,Mounira Harzallah,Fabrice Guillet,Hideaki Takeda,Ryutaro Ichise*
...
### [29] [GRALIS: A Unified Canonical Framework for Linear Attribution Methods via Riesz Representation](https://arxiv.org/abs/2605.05480)
*Raimondo Fanale*
...
### [30] [Approximate Next Policy Sampling: Replacing Conservative Target Policy Updates in Deep RL](https://arxiv.org/abs/2605.05481)
...
### [31] [A Robust Foundation Model for Conservation Laws: Injecting Context into Flux Neural Operators via Recurrent Vision Transformers](https://arxiv.org/abs/2605.05488)
...
### [32] [MEMOA: Massive Mixtures of Online Agents via Mean-Field Decentralized Nash Equilibria](https://arxiv.org/abs/2605.05492)
...
### [33] [Shortcut Solutions Learned by Transformers Impair Continual Compositional Reasoning](https://arxiv.org/abs/2605.05495)
...
### [34] [Online Localized Conformal Prediction](https://arxiv.org/abs/2605.05497)
...
### [35] [Non-Myopic Active Feature Acquisition via Pathwise Policy Gradients](https://arxiv.org/abs/2605.05511)
...
### [36] [OpenG2G: A Simulation Platform for AI Datacenter-Grid Runtime Coordination](https://arxiv.org/abs/2605.05519)
...
### [37] [Bayesian Rain Field Reconstruction using Commercial Microwave Links and Diffusion Model Priors](https://arxiv.org/abs/2605.05520)
...
### [38] [MOSAIC: Module Discovery via Sparse Additive Identifiable Causal Learning for Scientific Time Series](https://arxiv.org/abs/2605.05524)
...
### [39] [Energy Generative Modeling: A Lyapunov-based Energy Matching Perspective](https://arxiv.org/abs/2605.05530)
...
### [40] [Adversarial Graph Neural Network Benchmarks: Towards Practical and Fair Evaluation](https://arxiv.org/abs/2605.05534)
...
### [41] [Towards Scalable One-Step Generative Modeling for Autoregressive Dynamical System Forecasting](https://arxiv.org/abs/2605.05540)
...
### [42] [Adaptive Q-Chunking for Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2605.05544)
...
### [43] [FedeKD: Energy-Based Gating for Robust Federated Knowledge Distillation under Heterogeneous Settings](https://arxiv.org/abs/2605.05553)
...
### [44] [Accelerating LMO-Based Optimization via Implicit Gradient Transport](https://arxiv.org/abs/2605.05577)
...
### [45] [AeroJEPA: Learning Semantic Latent Representations for Scalable 3D Aerodynamic Field Modeling](https://arxiv.org/abs/2605.05586)
...
### [46] [When Can Voting Help, Hurt, or Change Course? Exact Structure of Binary Test-Time Aggregation](https://arxiv.org/abs/2605.05592)
*Yi Liu*
...
### [47] [Optimal Contextual Pricing under Agnostic Non-Lipschitz Demand](https://arxiv.org/abs/2605.05609)
...
### [48] [LLMSpace: Carbon Footprint Modeling for Large Language Model Inference on LEO Satellites](https://arxiv.org/abs/2605.05615)
...
### [49] [Region-adaptable retrieval of coastal biogeochemical parameters from near-surface hyperspectral remote sensing reflectance using physics-aware meta-learning](https://arxiv.org/abs/2605.05623)
...
### [50] [Scaling Pretrained Representations Enables Label-Free Out-of-Distribution Detection Without Fine-Tuning](https://arxiv.org/abs/2605.05638)
...
### [51] [Information-Preserving Domain Transfer with Unlabeled Data in Misspecified Simulation-Based Inference](https://arxiv.org/abs/2605.05652)
...
### [52] [Structural Correspondence and Universal Approximation in Diagonal plus Low-Rank Neural Networks](https://arxiv.org/abs/2605.05659)
...
### [53] [Distributionally Robust Multi-Objective Optimization](https://arxiv.org/abs/2605.05660)
...
### [54] [Temporal Functional Circuits: From Spline Plots to Faithful Explanations in KAN Forecasting](https://arxiv.org/abs/2605.05685)
*Naveen Mysore*
...
### [55] [Budgeted Attention Allocation: Cost-Conditioned Compute Control for Efficient Transformers](https://arxiv.org/abs/2605.05697)
*Amrit Nidhi*
...
### [56] [On the Blessing of Pre-training in Weak-to-Strong Generalization](https://arxiv.org/abs/2605.05710)
...
### [57] [Enabling Federated Inference via Unsupervised Consensus Embedding](https://arxiv.org/abs/2605.05718)
...
### [58] [WARP: A Benchmark for Primal-Dual Warm-Starting of Interior-Point Solvers](https://arxiv.org/abs/2605.05728)
...
### [59] [CRAFT: Forgetting-Aware Intervention-Based Adaptation for Continual Learning](https://arxiv.org/abs/2605.05732)
...
### [60] [CoMemNet: Contrastive Sampling with Memory Replay Network for Continual Traffic Prediction](https://arxiv.org/abs/2605.05738)
...
### [61] [Multi-Dimensional Behavioral Evaluation of Agentic Stock Prediction Systems Using LLM Judges with Closed-Loop Reinforcement Learning Feedback](https://arxiv.org/abs/2605.05739)
...
### [62] [Weak-to-Strong Generalization is Nearly Inevitable (in Linear Models)](https://arxiv.org/abs/2605.05742)
...
### [63] [RVPO: Risk-Sensitive Alignment via Variance Regularization](https://arxiv.org/abs/2605.05750)
...
### [64] [Full-Spectrum Graph Neural Network: Expressive and Scalable](https://arxiv.org/abs/2605.05759)
...
### [65] [Adaptive Selection of LoRA Components in Privacy-Preserving Federated Learning](https://arxiv.org/abs/2605.05769)
...
### [66] [A Measure-Theoretic Finite-Sample Theory for Adaptive-Data Fitted Q-Iteration](https://arxiv.org/abs/2605.05791)
...
### [67] [Revealing Modular Gradient Noise Imbalance in LLMs: Calibrating Adam via Signal-to-Noise Ratio](https://arxiv.org/abs/2605.05794)
...
### [68] [Selective Rollout: Mid-Trajectory Termination for Multi-Sample Agent RL](https://arxiv.org/abs/2605.05802)
...
### [69] [Retrieval from Within: An Intrinsic Capability of Attention-Based Models](https://arxiv.org/abs/2605.05806)
...
### [70] [A Testable Certificate for Constant Collapse in Teacher-Guided VAEs](https://arxiv.org/abs/2605.05813)
...
### [71] [HCInfer: An Efficient Inference System via Error Compensation for Resource-Constrained Devices](https://arxiv.org/abs/2605.05819)
...
### [72] [MDN: Parallelizing Stepwise Momentum for Delta Linear Attention](https://arxiv.org/abs/2605.05838)
...
### [73] [Hypothesis generation and updating in large language models](https://arxiv.org/abs/2605.05851)
*Hua-Dong Xiong*
...
### [74] [Measuring Learning Progress via Gradient-Momentum Coupling](https://arxiv.org/abs/2605.05856)
...
### [75] [Offline Reinforcement Learning for Rotation Profile Control in Tokamaks](https://arxiv.org/abs/2605.05857)
...
### [76] [Do Neural Operators Forget Geometry? The Forgetting Hypothesis in Deep Operator Learning](https://arxiv.org/abs/2605.05862)
...
### [78] [QuadraSHAP: Stable and Scalable Shapley Values for Product Games via Gauss-Legendre Quadrature](https://arxiv.org/abs/2605.05870)
...
### [79] [Retain-Neutral Surrogates for Min-Max Unlearning](https://arxiv.org/abs/2605.05871)
...
### [80] [RepFlow: Representation Enhanced Flow Matching for Causal Effect Estimation](https://arxiv.org/abs/2605.05890)
...
### [81] [VARS-FL: Validation-Aligned Client Selection for Non-IID Federated Learning in IoT Systems](https://arxiv.org/abs/2605.05896)
...
### [82] [VisMMOE: Exploiting Visual-Expert Affinity for Efficient Visual-Language MoE Offloading](https://arxiv.org/abs/2605.05899)
...
### [83] [Quadratic Objective Perturbation: Curvature-Based Differential Privacy](https://arxiv.org/abs/2605.05905)
...
### [84] [From Drops to Grid: Noise-Aware Spatio-Temporal Neural Process for Rainfall Estimation](https://arxiv.org/abs/2605.05912)
...
### [85] [Near-Policy: Accelerating On-Policy Distillation via Asynchronous Generation and Selective Packing](https://arxiv.org/abs/2605.05940)
...
### [86] [Knowing but Not Correcting: Routine Task Requests Suppress Factual Correction in LLMs](https://arxiv.org/abs/2605.05957)
...
### [87] [Uncertainty Estimation via Hyperspherical Confidence Mapping](https://arxiv.org/abs/2605.05964)
...
### [88] [Beyond Uniform Credit Assignment: Selective Eligibility Traces for RLVR](https://arxiv.org/abs/2605.05965)
...
### [89] [Sharper Guarantees for Misspecified Kernelized Bandit Optimization](https://arxiv.org/abs/2605.05967)
...
### [90] [Training Transformers for KV Cache Compressibility](https://arxiv.org/abs/2605.05971)
...
### [91] [DiBA: Diagonal and Binary Matrix Approximation for Neural Network Weight Compression](https://arxiv.org/abs/2605.05994)
*Nobutaka Ono*
...
### [92] [A Fine-Grained Understanding of Uniform Convergence for Halfspaces](https://arxiv.org/abs/2605.06004)
...
### [93] [Quantizing With Randomized Hadamard Transforms: Efficient Heuristic Now Proven](https://arxiv.org/abs/2605.06014)
...
### [94] [Matrix-Decoupled Concentration for Autoregressive Sequences: Dimension-Free Guarantees for Sparse Long-Context Rewards](https://arxiv.org/abs/2605.06017)
*Pei-Sen Li*
...
### [95] [Multi-agent decision making: A Blackwell's informativeness approach](https://arxiv.org/abs/2605.06028)
...
### [96] [Does Synthetic Data Help? Empirical Evidence from Deep Learning Time Series Forecasters](https://arxiv.org/abs/2605.06032)
...
### [97] [Optimal Transport for LLM Reward Modeling from Noisy Preference](https://arxiv.org/abs/2605.06036)
...
### [98] [Requests of a Feather Must Flock Together: Batch Size vs. Prefix Homogeneity in LLM Inference](https://arxiv.org/abs/2605.06046)
...
### [99] [TFM-Retouche: A Lightweight Input-Space Adapter for Tabular Foundation Models](https://arxiv.org/abs/2605.06047)
...
### [100] [When Brain Networks Travel: Learning Beyond Site](https://arxiv.org/abs/2605.06050)
...
### [101] [Towards Generation-Efficient Uncertainty Estimation in Large Language Models](https://arxiv.org/abs/2605.06053)
...
### [102] [Towards Self-Explainable Document Visual Question Answering with Chain-of-Explanation Predictions](https://arxiv.org/abs/2605.06058)
...
### [103] [Geometry-Aware Simplicial Message Passing](https://arxiv.org/abs/2605.06061)
...
### [104] [Causal Reinforcement Learning for Complex Card Games: A Magic The Gathering Benchmark](https://arxiv.org/abs/2605.06066)
...
### [105] [Normalized Architectures are Natively 4-Bit](https://arxiv.org/abs/2605.06067)
...
### [106] [Understanding diffusion models requires rethinking (again) generalization](https://arxiv.org/abs/2605.06077)
...
### [107] [Fast Gauss-Newton for Multiclass Cross-Entropy](https://arxiv.org/abs/2605.06081)
...
### [108] [Beyond Autoregressive RTG: Conditioning via Injection Outside Sequential Modeling in Decision Transformer](https://arxiv.org/abs/2605.06104)
...
### [109] [BoostLLM: Boosting-inspired LLM Fine-tuning for Few-shot Tabular Classification](https://arxiv.org/abs/2605.06117)
...
### [110] [Listwise Policy Optimization: Group-based RLVR as Target-Projection on the LLM Response Simplex](https://arxiv.org/abs/2605.06139)
...
### [111] [SymDrift: One-Shot Generative Modeling under Symmetries](https://arxiv.org/abs/2605.06140)
...
### [112] [Matrix-Valued Optimism is Matrix-Valued Augmentation: Additive Hybrid Designs for Constrained Optimization](https://arxiv.org/abs/2605.06141)
*Jiayi Zhao*
...
### [114] [AdaGamma: State-Dependent Discounting for Temporal Adaptation in Reinforcement Learning](https://arxiv.org/abs/2605.06149)
...
### [115] [Grokking or Glitching? How Low-Precision Drives Slingshot Loss Spikes](https://arxiv.org/abs/2605.06152)
...
### [116] [Entropy-Regularized Adjoint Matching for Offline RL](https://arxiv.org/abs/2605.06156)
...
### [117] [One Algorithm, Two Goals: Dual Scoring for Parameter and Data Selection in LLM Fine-Tuning](https://arxiv.org/abs/2605.06166)
...
### [118] [Mean Mode Screaming: Mean--Variance Split Residuals for 1000-Layer Diffusion Transformers](https://arxiv.org/abs/2605.06169)
*Pengqi Lu*
...
### [119] [In-Context Black-Box Optimization with Unreliable Feedback](https://arxiv.org/abs/2605.06187)
...
### [120] [Constrained Contextual Bandits with Adversarial Contexts](https://arxiv.org/abs/2605.06190)
...
### [121] [Bandit Learning in General Open Multi-agent Systems](https://arxiv.org/abs/2605.06202)
*Mengfan Xu*
...
### [122] [Contrastive Identification and Generation in the Limit](https://arxiv.org/abs/2605.06211)
...
### [123] [Playing the network backward: A Game Theoretic Attribution Framework](https://arxiv.org/abs/2605.06212)
...
### [124] [AffineLens: Capturing the Continuous Piecewise Affine Functions of Neural Networks](https://arxiv.org/abs/2605.06218)
...
### [125] [Memory Inception: Latent-Space KV Cache Manipulation for Steering LLMs](https://arxiv.org/abs/2605.06225)
...
### [126] [Soft Deterministic Policy Gradient with Gaussian Smoothing](https://arxiv.org/abs/2605.06228)
...
### [127] [A Note on TurboQuant and the Earlier DRIVE/EDEN Line of Work](https://arxiv.org/abs/2604.18555)
...
### [128] [Band Together: Untargeted Adversarial Training with Multimodal Coordination against Evasion-based Promotion Attacks](https://arxiv.org/abs/2605.06238)
...
### [129] [When Graph Language Models Go Beyond Memorization](https://arxiv.org/abs/2605.06239)
...
### [130] [Cumulative-Goodness Free-Riding in Forward-Forward Networks: Real, Repairable, but Not Accuracy-Dominant](https://arxiv.org/abs/2605.06240)
...
### [131] [Structure-Preserving Gaussian Processes Via Discrete Euler-Lagrange Equations](https://arxiv.org/abs/2605.06246)
...
### [132] [The Role of Node Features in Graph Pooling](https://arxiv.org/abs/2605.06250)
...
### [133] [The Weight Gram Matrix Captures Sequential Feature Linearization in Deep Networks](https://arxiv.org/abs/2605.06258)
...
### [134] [Trade-off Functions for DP-SGD with Subsampling based on Random Shuffling: Tight Upper and Lower Bounds](https://arxiv.org/abs/2605.06259)
...
### [135] [Beyond Rigid Alignment: Graph Federated Learning via Dual Manifold Calibration](https://arxiv.org/abs/2605.06260)
...
### [136] [Inference-Time Refinement Closes the Synthetic-Real Gap in Tabular Diffusion](https://arxiv.org/abs/2605.06261)
...
### [137] [Can Attribution Predict Risk? From Multi-View Attribution to Planning Risk Signals in End-to-End Autonomous Driving](https://arxiv.org/abs/2605.06264)
...
### [138] [A Flow Matching Algorithm for Many-Shot Adaptation to Unseen Distributions](https://arxiv.org/abs/2605.06272)
...
### [139] [When Labels Have Structure: Improving Image Classification with Hierarchy-Aware Cross-Entropy](https://arxiv.org/abs/2605.06274)
...
### [140] [PACE: Prune-And-Compress Ensemble Models](https://arxiv.org/abs/2605.06278)
...
### [141] [INEUS: Iterative Neural Solver for High-Dimensional PIDEs](https://arxiv.org/abs/2605.06281)
...
### [142] [Attributions All the Way Down? The Metagame of Interpretability](https://arxiv.org/abs/2605.06295)
...
### [143] [Region Seeding via Pre-Activation Regularization: A Geometric View from Piecewise Affine Nerual Networks](https://arxiv.org/abs/2605.06300)
...
### [144] [Molecules Meet Language: Confound-Aware Representation Learning and Chemical Property Steering in Transformer-VAE Latent Spaces](https://arxiv.org/abs/2605.06303)
...
### [145] [Perceive, Route and Modulate: Dynamic Pattern Recalibration for Time Series Forecasting](https://arxiv.org/abs/2605.06310)
...
### [151] [Topological Signatures of Grokking](https://arxiv.org/abs/2605.06352)
...
### [152] [Order-Agnostic Autoregressive Modelling with Missing Data](https://arxiv.org/abs/2605.06355)
...
### [153] [Memory Efficient Full-gradient Attacks (MEFA) Framework for Adversarial Defense Evaluations](https://arxiv.org/abs/2605.06357)
...
### [154] [Preliminary Insights in Chronos Frequency Data Understanding and Reconstruction](https://arxiv.org/abs/2605.06361)
...
### [155] [Flow Matching with Arbitrary Auxiliary Paths](https://arxiv.org/abs/2605.06364)
*Xin Peng,Ang Gao*
...
### [156] [A Unified Pair-GRPO Family: From Implicit to Explicit Preference Constraints for Stable and General RL Alignment](https://arxiv.org/abs/2605.06375)
*Hao Yu*
...
### [157] [MinMax Recurrent Neural Cascades](https://arxiv.org/abs/2605.06384)
*Alessandro Ronca*
...
### [158] [Data-Driven Covariate Selection for Nonparametric and Cycle-Agnostic Causal Effect Estimation](https://arxiv.org/abs/2605.06385)
...
### [159] [Asymmetric On-Policy Distillation: Bridging Exploitation and Imitation at the Token Level](https://arxiv.org/abs/2605.06387)
...
### [160] [Consistent Geometric Deep Learning via Hilbert Bundles and Cellular Sheaves](https://arxiv.org/abs/2605.06395)
...
### [168] [ORTHOBO: Orthogonal Bayesian Hyperparameter Optimization](https://arxiv.org/abs/2605.06454)
...
### [169] [Invariant Features in Language Models: Geometric Characterization and Model Attribution](https://arxiv.org/abs/2605.06458)
...
### [170] [MINER: Mining Multimodal Internal Representation for Efficient Retrieval](https://arxiv.org/abs/2605.06460)
...
### [171] [Invariant-Based Diagnostics for Graph Benchmarks](https://arxiv.org/abs/2605.06462)
...
### [172] [Diversity Curves for Graph Representation Learning](https://arxiv.org/abs/2605.06466)
...
### [173] [No Triangulation Without Representation: Generalization in Topological Deep Learning](https://arxiv.org/abs/2605.06467)
...
### [174] [Hitting Time Isomorphism for Multi-Stage Planning with Foundation Policies](https://arxiv.org/abs/2605.06470)
...
### [175] [Efficient Serving for Dynamic Agent Workflows with Prediction-based KV-Cache Management](https://arxiv.org/abs/2605.06472)
...
### [176] [Q-MMR: Off-Policy Evaluation via Recursive Reweighting and Moment Matching](https://arxiv.org/abs/2605.06474)
*Xiang Li,Nan Jiang*
...
### [177] [Operator-Guided Invariance Learning for Continuous Reinforcement Learning](https://arxiv.org/abs/2605.06500)
...
### [179] [PACZero: PAC-Private Fine-Tuning of Language Models via Sign Quantization](https://arxiv.org/abs/2605.06505)
...
### [180] [Is One Layer Enough? Understanding Inference Dynamics in Tabular Foundation Models](https://arxiv.org/abs/2605.06510)
...
### [181] [Efficient Techniques for Data Reconstruction, with Finite-Width Recovery Guarantees](https://arxiv.org/abs/2605.06519)
...
### [182] [Agentic AIs Are the Missing Paradigm for Out-of-Distribution Generalization in Foundation Models](https://arxiv.org/abs/2605.06522)
...
### [183] [On the Implicit Reward Overfitting and the Low-rank Dynamics in RLVR](https://arxiv.org/abs/2605.06523)
...
### [184] [Diffusion-Based Posterior Sampling: A Feynman-Kac Analysis of Bias and Stability](https://arxiv.org/abs/2605.06538)
...
### [185] [Hedging Memory Horizons for Non-Stationary Prediction via Online Aggregation](https://arxiv.org/abs/2605.06541)
...
### [186] [Sequential Design of Genetic Circuits Under Uncertainty With Reinforcement Learning](https://arxiv.org/abs/2605.06552)
...
### [187] [Diverse Sampling in Diffusion Models with Marginal Preserving Particle Guidance](https://arxiv.org/abs/2605.06553)
...
### [188] [Optimal Counterfactual Search in Tree Ensembles: A Study Across Modeling and Solution Paradigms](https://arxiv.org/abs/2605.06561)
...
### [189] [Feature Dimensionality Outweighs Model Complexity in Breast Cancer Subtype Classification Using TCGA-BRCA Gene Expression Data](https://arxiv.org/abs/2605.06562)
*Meena Al Hasani*
...
### [190] [Criticality and Saturation in Orthogonal Neural Networks](https://arxiv.org/abs/2605.06563)
...
### [191] [SNAPO: Smooth Neural Adjoint Policy Optimization for Optimal Control via Differentiable Simulation](https://arxiv.org/abs/2605.06570)
...
### [192] [CLAD: A Clustered Label-Agnostic Federated Learning Framework for Joint Anomaly Detection and Attack Classification](https://arxiv.org/abs/2605.06571)
...
### [193] [Directional Consistency as a Complementary Optimization Signal: The GONO Framework](https://arxiv.org/abs/2605.06575)
*Victor Daniel Gera*
...
### [194] [On the Safety of Graph Representation Learning](https://arxiv.org/abs/2605.06576)
...
### [195] [PairAlign: A Framework for Sequence Tokenization via Self-Alignment with Applications to Audio Tokenization](https://arxiv.org/abs/2605.06582)
...
### [196] [Distributionally-Robust Learning to Optimize](https://arxiv.org/abs/2605.06585)
...
### [197] [Towards Metric-Faithful Neural Graph Matching](https://arxiv.org/abs/2605.06588)
...
### [198] [BRICKS: Compositional Neural Markov Kernels for Zero-Shot Radiation-Matter Simulation](https://arxiv.org/abs/2605.06591)
...
### [199] [Weight-Decay Turns Transformer Loss Landscapes Villani: Functional-Analytic Foundations for Optimization and Generalization](https://arxiv.org/abs/2605.06599)
...
### [200] [How Many Iterations to Jailbreak? Dynamic Budget Allocation for Multi-Turn LLM Evaluation](https://arxiv.org/abs/2605.06605)
...
### [201] [Transformers Efficiently Perform In-Context Logistic Regression via Normalized Gradient Descent](https://arxiv.org/abs/2605.06609)
...
### [202] [The Structural Origin of Attention Sink: Variance Discrepancy, Super Neurons, and Dimension Disparity](https://arxiv.org/abs/2605.06611)
...
### [203] [SoftSAE: Dynamic Top-K Selection for Adaptive Sparse Autoencoders](https://arxiv.org/abs/2605.06610)
...
### [204] [When and Why SignSGD Outperforms SGD: A Theoretical Study Based on $\ell_1$-norm Lower Bounds](https://arxiv.org/abs/2605.06615)
...
### [205] [Recursive Agent Optimization](https://arxiv.org/abs/2605.06639)
...
### [206] [Online Bayesian Calibration under Gradual and Abrupt System Changes](https://arxiv.org/abs/2605.06612)
*Yang Xu,Chiwoo Park*
...
### [207] [Concept-Based Abductive and Contrastive Explanations for Behaviors of Vision Models](https://arxiv.org/abs/2605.06640)
...
### [208] [When No Benchmark Exists: Validating Comparative LLM Safety Scoring Without Ground-Truth Labels](https://arxiv.org/abs/2605.06652)
...
### [209] [Hybrid Quantum-Classical GANs for the Generation of Adversarial Network Flows](https://arxiv.org/abs/2605.06629)
...
### [210] [Optimizer-Model Consistency: Full Finetuning with the Same Optimizer as Pretraining Forgets Less](https://arxiv.org/abs/2605.06654)
...
### [211] [Crafting Reversible SFT Behaviors in Large Language Models](https://arxiv.org/abs/2605.06632)
...
### [212] [Verifier-Backed Hard Problem Generation for Mathematical Reasoning](https://arxiv.org/abs/2605.06660)
...
### [213] [UniPool: A Globally Shared Expert Pool for Mixture-of-Experts](https://arxiv.org/abs/2605.06665)
...
### [214] [Inductive Venn-Abers and related regressors](https://arxiv.org/abs/2605.06646)
...
### [215] [Understanding Annotator Safety Policy with Interpretability](https://arxiv.org/abs/2605.05329)
*Alex Oesterling,Donghao Ren,Yannick Assogba,Dominik Moritz,Sunnie S. Y. Kim,Leon Gatys,Fred Hohman*

Main category: cs.AI

TL;DR: 提出标注者策略模型，从标注行为学习标注者内部安全策略，无需额外标注即可揭示安全政策标注分歧的来源（政策模糊性和价值多元性），支持更好的安全政策设计。


<details>
  <summary>Details</summary>
Motivation: 解决安全政策标注中普遍存在的分歧问题。标注分歧可能源于操作失误、政策模糊性或价值多元性，区分这些来源对制定针对性改进措施至关重要。直接询问标注者理由成本高且不可靠，需要无需额外负担的方法来理解标注分歧。

Method: 研究者提出标注者策略模型，仅从标注行为学习标注者的内部安全策略，无需额外标注负担。通过验证APMs的准确性、对反事实编辑的预测能力，以及在受控设置中恢复已知策略差异。

Result: APMs能准确建模标注者安全策略（准确率>80%），有效预测对反事实编辑的响应，在受控设置中恢复已知策略差异。应用于LLM和人类标注时，APMs能够揭示政策模糊性（展示标注者对安全指导的不同理解）和揭示价值多元性（发现不同人口群体在安全优先级上的系统性差异）。

Conclusion: APMs通过可解释的模型学习标注者的内部安全策略，能够有效区分安全标注中分歧的来源，支持更精准、透明、包容的安全政策设计。
...
### [216] [ZAYA1-8B Technical Report](https://arxiv.org/abs/2605.05365)
...
TL;DR: ZAYA1-8B是一个700M活跃参数/8B总参数的推理专用MoE模型，采用四阶段RL后训练和Markovian RSA测试计算方法，在数学与编程基准测试中表现优异，缩小了与大型模型的性能差距。


<details>
  <summary>Details</summary>
Motivation: 开发一个参数高效、专注于推理且能在多项挑战性基准测试中与大型模型竞争的开放权重模型，同时探索在有限计算资源下提升推理能力的方法。

Method: 采用MoE++架构，包含预训练、中间训练和SFT三阶段。后训练采用四阶段强化学习流程：数学与谜题推理预热、400任务的RLVE-Gym课程学习、基于测试计算轨迹和合成代码环境的数学与代码RL、以及聊天与指令跟随的行为RL。引入Markovian RSA测试计算方法，递归聚合并行推理轨迹并仅保留有限长度的推理尾部。

Result: ZAYA1-8B在不足10亿活跃参数的情况下，在多项数学和编程基准测试中匹配或超越DeepSeek-R1-0528，并与更大型的开放权重推理模型保持竞争力。使用Markovian RSA后，在AIME'25和HMMT'25上的准确率分别达到91.9%和89.6%，仅需保留4K token的推理尾部，显著缩小了与Gemini-2.5 Pro、DeepSeek-V3.2和GPT-5-High等大型模型的差距。

Conclusion: ZAYA1-8B作为一种专注于推理的混合专家模型，在多项数学和编程基准测试中展现出与或超越更大型模型的性能，特别是通过Markovian RSA方法显著提升了推理能力，缩小了与大型模型的差距。
...
### [217] [Partial Evidence Bench: Benchmarking Authorization-Limited Evidence in Agentic Systems](https://arxiv.org/abs/2605.05379)
*Krti Tallam*

Main category: cs.AI

TL;DR: 这篇论文提出了Partial Evidence Bench基准测试，用于衡量企业智能体在证据受限环境中的完整性失败问题，通过三种场景72个任务评估系统的完整性意识和安全行为，发现隐性过滤极其危险，显式报告更为安全。


<details>
  <summary>Details</summary>
Motivation: 在企业智能体日益在受限检索系统、委派工作流和政策约束的证据环境中运营的背景下，虽然访问控制可以正确执行，但系统可能产生看似完整却遗漏授权边界外关键证据的答案，这种隐性过滤的安全风险需要系统性衡量方法。

Method: 设计了包含72个任务的三种场景族（尽职调查、合规审计、安全事件响应），提供ACL分区语料库、完整答案的Oracle、授权视图答案的Oracle、完整性判断Oracle以及结构化缺口报告Oracle，从答案正确性、完整性意识、缺口报告质量和不安全完整性行为四个维度评估系统。

Result: 基线测试显示隐性过滤在所有场景中都会带来灾难性安全风险，而明示失败报告行为能消除不安全完整性且不会退化为简单弃权；初步真实模型运行显示系统在过度声明完整性、保守欠声明以及以企业可用形式报告不完整性方面存在模型依赖性和场景敏感性差异。

Conclusion: 构建了一个确定性的基准测试框架来衡量企业智能体在证据受限环境中的完整性失败问题，基准测试证明明确的失败报告行为能有效消除不安全完整性，但实际模型表现存在场景敏感性和模型依赖性差异。
...
### [218] [BALAR : A Bayesian Agentic Loop for Active Reasoning](https://arxiv.org/abs/2605.05386)
*Aymen Echarghaoui,Dongxia Wu,Emily B. Fox*

Main category: cs.AI

TL;DR: 提出BALAR算法，通过贝叶斯信念维护、互信息最大化问题选择和动态状态扩展，使LLM能主动进行多轮交互式推理，在三个任务上显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在交互式任务中普遍采用被动反应式对话模式，缺乏系统性的信息缺口分析和主动询问机制。这种局限性导致多轮任务解决效率低下，需要一种能指导LLM主动推理、结构化地管理信念并动态调整问题策略的算法框架。

Method: 1. 维护潜在状态的贝叶斯结构化信念，通过先验和观察更新后验分布。
2. 基于互信息最大化原则选择最佳澄清问题，最小化不确定性。
3. 动态扩展状态表示能力，当现有状态表示无法充分建模问题时引入新状态。
4. 作为外部循环算法工作，无需LLM本身微调且任务无关。

Result: 在三个基准测试上显著优于所有基线方法：
- AR-Bench-DC（侦探案例）：准确率提升14.6%
- AR-Bench-SP（思维谜题）：准确率提升38.5%
- iCraft-MD（临床诊断）：准确率提升30.5%

Conclusion: BALAR是一种具有普适性且无需微调的外部循环算法，通过贝叶斯信念更新和主动信息获取机制，显著提升了LLM在交互式任务中的多轮推理能力。
...
### [219] [Intelligent CCTV for Urban Design: AI-Based Analysis of Soft Infrastructure at Intersections](https://arxiv.org/abs/2605.05402)
*Vinit Katariya,Seungjin Kim,Curtis Craig,Nichole Morris,Hamed Tabkhi*

Main category: cs.AI

TL;DR: 本研究利用AI和现有CCTV，通过深度学习和速度估计算法评估软交通干预措施。结果显示临时行人安全区和路缘扩展能显著降低车辆速度，证明AI方法可用于快速、低成本的交通政策评估。


<details>
  <summary>Details</summary>
Motivation: 现代交通管理中，评估软干预措施（如临时行人安全区、路缘扩展）对车辆速度和交通安全的影响通常成本高昂且耗时。本研究旨在利用人工智能和计算机视觉技术，通过现有CCTV基础设施，开发一个快速、低成本的分析框架，为交通政策提供基于证据的评估方法。

Method: 本研究开发了一个AI赋能的分析框架，利用现有CCTV基础设施，通过深度学习和透视基速度估计算法来评估软干预措施对车辆速度和安全的影響。研究在明尼阿波利斯进行了前后对比分析，包括干预前、干预后第1周和第2周的重复监测。

Result: 研究结果显示，在无信号灯交叉口，平均速度和85百分位速度分别下降了高达18.75%和16.56%，通过交通量减少了高达12.2%。在有信号灯交叉口，除一个地点外，也观察到可比的速度下降，平均速度和85百分位速度分别下降了高达20.0%和17.19%。

Conclusion: 该研究证明，软基础设施如临时行人安全区和路缘扩展具有显著的交通稳静化效果，能够有效降低车辆速度并提高安全性。同时，研究强调了AI赋能方法在快速、低成本、基于证据的交通政策评估中的实用性，为城市利用现有基础设施进行智能交通管理提供了可行方案。
...
### [220] [When Helpfulness Becomes Sycophancy: Sycophancy is a Boundary Failure Between Social Alignment and Epistemic Integrity in Large Language Models](https://arxiv.org/abs/2605.05403)
*Jiechen Li,Catherine A. Barry,Rishika Randev,Janet Chen,Ella Jorgensen,Brinnae Bent*

Main category: cs.AI

TL;DR: 本文提出LLM谄媚行为是社会对齐与认知完整性之间的边界失败，而非单纯同意。建立了三条件框架（用户线索、模型对齐偏移、认知损害）和分类法，以改进评估和缓解策略。


<details>
  <summary>Details</summary>
Motivation: 当前研究常将LLM中的谄媚行为操作化为同意用户错误信念、立场反转或偏离客观正确标准等外部行为，这些定义仅捕捉到明显现象，而对涉及认知完整性和社会对齐的微妙边界失败未予明确。因此需要更精准地界定谄媚行为作为对齐行为取代独立认知判断的本质。

Method: 提出一个三条件框架来界定谄媚行为：1）用户表达信念、偏好或自我概念的线索；2）模型通过对齐行为向该线索偏移；3）这种偏移损害认知准确性、独立推理或适当纠正。同时引入包含对齐目标、机制和严重程度的分类法。

Result: 建立了谄媚行为的三条件框架和分类法，为更准确地识别和评估谄媚行为提供了理论基础。

Conclusion: 该论文的结论是谄媚行为应被视为社会对齐与认知完整性之间的边界失败，当前评估方法存在局限性。为改进对齐评估，建议采用边界意识评估、结构化评分准则和缓解策略，同时考虑谄媚行为的多种理论视角。
...
### [221] [Agentic Retrieval-Augmented Generation for Financial Document Question Answering](https://arxiv.org/abs/2605.05409)
*Yang Shu,Yingmin Liu,Zequn Xie*

Main category: cs.AI

TL;DR: FinAgent-RAG是一个针对金融文档问答设计的智能检索增强生成框架，通过迭代检索-推理循环和自验证机制，集成对比检索、代码生成推理和动态资源分配，显著提升多步骤数值推理准确率的同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 金融文档问答需要基于分散在企业文件中的结构化表格、文本叙述和脚注进行复杂的多步骤数值推理。现有的检索增强生成方法采用单次检索-生成范式，难以处理金融分析中普遍存在的组合推理链，无法满足金融数值推理的精确性要求。

Method: 提出FinAgent-RAG框架，包含三个核心创新：(1) 对比性金融检索器：通过硬负例挖掘训练，区分语义相似但数值不同的金融段落；(2) 程序思维推理模块：生成可执行Python代码进行精确算术计算，避免LLM思维计算误差；(3) 自适应策略路由器：根据问题复杂度动态分配计算资源，降低API成本。采用迭代检索-推理循环与自验证机制。

Result: 在FinQA、ConvFinQA和TAT-QA三个基准数据集上的实验显示，FinAgent-RAG分别达到76.81%、78.46%和74.96%的执行准确率，比最强基线提升了5.62-9.32个百分点。自适应策略路由器在FinQA上减少了41.3%的API成本。消融研究、跨骨干LLM评估和部署成本分析证实了框架的鲁棒性和实际可行性。

Conclusion: FinAgent-RAG是一个用于金融文档问答的智能化检索增强生成框架，通过引入对比性金融检索器、程序思维推理模块和自适应策略路由器等创新，有效解决了传统单次检索-生成方法在复杂多步骤数值推理中的局限性。实验证明该框架在多个基准数据集上显著提升了性能，同时通过动态资源分配大幅降低了计算成本，为金融机构提供了实用且高效的解决方案。
...
### [222] [LaTA: A Drop-in, FERPA-Compliant Local-LLM Autograder for Upper-Division STEM Coursework](https://arxiv.org/abs/2605.05410)
*Jesse A. Rodríguez*

Main category: cs.AI

TL;DR: LaTA是一个开源的本地自动评分器，使用大语言模型为LaTeX作业评分，保护隐私并显著减轻教师负担，同时提升了学生学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有的基于大语言模型的自动评分器通常依靠第三方API，这违反了FERPA隐私法规，存在数据风险，且需要对作业进行大量修改。需要一个能在本地部署、不依赖外部API的开源解决方案。

Method: LaTA采用四阶段流水线（收录、分割、评分、报告），使用本地部署的开源大语言模型（gpt-oss:120b）进行链式推理评分。系统将学生答案与教师提供的参考答案进行对比，并应用YAML格式的二元评分标准。

Result: 在俄勒冈州立大学机械工程方法课程（约200名学生）的部署显示：系统在单台Mac Studio上以零边际成本和每份作业1-3分钟的评分时间，评分错误率维持在每评分项约0.02-0.04%。与传统评分相比，LaTA评分班级的期中考试成绩提高约11%，期末考试成绩提高约8%，学生对所有学习目标的自我评估信心均有显著提升（Δ≥+1.49 Likert分数，p<10^-27）。

Conclusion: LaTA（LaTeX助教）系统证明，使用开源的本地大语言模型作为自动评分器，在保护学生隐私的同时，能够大幅减轻高年级STEM课程的评分负担，并有效提升学生的学习效果和自信。其成功部署为其他类似课程采用提供了可行方案。
...
### [223] [From History to State: Constant-Context Skill Learning for LLM Agents](https://arxiv.org/abs/2605.05413)
*Haoyang Xie,Xinyuan Wang,Yancheng Wang,Puda Zhao,Feng Ju*

Main category: cs.AI

TL;DR: 该论文提出常量上下文技能学习框架，将可复用工作流技能学习到模型权重中而非依赖长提示词，解决了LLM智能体在隐私、成本和能力间的权衡问题，在多个基准测试中取得优异性能并大幅降低推理开销。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）智能体在操作浏览器、文件、代码等工具方面应用日益广泛，但面临隐私-成本-能力权衡：云端模型能良好执行多步工作流但会将敏感中间上下文暴露给外部API，而本地模型保护隐私但可靠性较低。同时，两种设置都需要为长技能提示词和增长的历史反复付费。

Method: 提出常量上下文技能学习（constant-context skill learning）框架，采用'上下文到权重'（context-to-weights）方法：将可复用的工作流程学习为轻量级任务族模块；推理时只条件化当前观察和紧凑状态块；使用确定性追踪器从任务进展中渲染状态块并提供对齐的子目标奖励；每个模块通过步骤级监督微调（SFT）训练，并通过在线强化学习（RL）进行精炼。

Result: 在ALFWorld、WebShop和SciWorld测试中，智能体在Qwen3-4B、Qwen3-8B和Llama-3.1-8B模型上均表现优异。使用Qwen3-8B时，SFT+RL方法在ALFWorld上达到89.6%的未见任务成功率，WebShop上76.8%成功率，SciWorld上66.4%未见任务成功率。性能匹配或超越已发表的强基线结果，同时相比受控的ReAct提示基线每轮减少2-7倍的提示token使用量。

Conclusion: 研究提出了常量上下文技能学习框架，通过将常用的工作流技能固化到模型权重中，而非依赖长提示词，有效解决了智能体在隐私、成本和能力之间的权衡问题。该框架在多个基准测试中展现出优越性能，并显著降低了推理时的token开销。
...
### [224] [The Geopolitics of AI Safety: A Causal Analysis of Regional LLM Bias](https://arxiv.org/abs/2605.05427)
*Alif Al Hasan*

Main category: cs.AI

TL;DR: 本文提出基于概率图模型的因果审计框架，分析7个不同地区LLM的安全机制。研究发现传统偏见评估可能高估偏见，并揭示东西方模型在拒绝行为上存在文化差异。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）被整合到全球软件系统中，确保公平的安全护栏成为关键需求。当前的公平性评估主要基于观察性测量偏见，但这种方法受到测试数据集中特定人群自然关联话题的毒性混淆影响。本文旨在因果性地审计LLM安全机制，揭示传统公平性指标的局限性。

Method: 建立概率图模型（PGM）框架，应用Pearl的do-operator数学性地隔离将文化人口统计特征注入提示词的因果效应。使用ToxiGen和BOLD两个数据集对七个指令调优模型进行大规模实证分析，比较观察性偏见和介入性偏见的差异。

Result: 研究表明观察性偏见和介入性偏见之间存在差异，传统公平性指标由于未考虑上下文毒性而可能高估人口统计偏见。因果概率显示不同的对齐趋势：西方模型对特定人群表现出较高的因果拒绝率，而东方模型整体干预率较低但对地区人群敏感。

Conclusion: 研究结论显示，西方模型在特定人群上的因果拒绝率较高，而东方模型的整体干预率较低但对地区人群敏感。这些人口统计敏感性的过度触发可能会限制下游应用中的良性讨论，强调了需要在文化偏见和安全性之间取得平衡。
...
### [225] [Authorization Propagation in Multi-Agent AI Systems: Identity Governance as Infrastructure](https://arxiv.org/abs/2605.05440)
*Krti Tallam*

Main category: cs.AI

TL;DR: 多智能体AI系统存在独特的授权传播问题，需建立持续评估、边界强制的授权架构基础设施。


<details>
  <summary>Details</summary>
Motivation: 目前学术界和产业界对自主AI安全的讨论过度集中于提示注入攻击，而忽视了多智能体系统特有的授权传播问题——即如何在非人类主体跨不断变化的边界检索数据、委托任务和合成结果时维持授权不变性。

Method: 论文首先形式化了授权传播作为工作流级别的属性，识别了三个子问题（传递委托、聚合推理和时间有效性），并推导出多智能体AI系统授权架构的七个结构要求，最后通过一个生产级企业AI平台的初步实现证据来佐证模型预测的失效情况。

Result: 论文研究表明，授权传播无法简化为提示注入问题，也未被RBAC、ABAC、ReBAC等经典访问控制模型完全解决；现有工作虽已涉及调用绑定能力令牌、任务范围授权信封、依赖图策略执行和执行计数撤销等机制，但尚未形成完整的授权架构。生产环境的初步证据表明，即使没有对抗性攻击，普通系统行为也会产生模型预测的授权失效。

Conclusion: 该论文的核心观点是，多智能体系统的授权传播问题比传统的访问控制模型更加复杂，需要将身份治理作为基础设施来设计，在编排逻辑扩展之前就嵌入系统，实现持续评估和在每个交互边界进行强制执行的授权架构。
...
### [226] [Intentionality is a Design Decision: Measuring Functional Intentionality for Accountable AI Systems](https://arxiv.org/abs/2605.05475)
*Allessia Chiappetta,Robert Mahari*

Main category: cs.AI

TL;DR: 提出功能性意图测试(FIT)框架，通过五个维度量化评估AI系统的意图式行为，为自主系统的治理和问责提供标准化度量。


<details>
  <summary>Details</summary>
Motivation: 解决自主AI系统缺乏标准化的行为意图评估方法的问题，使治理和问责能够建立在可度量的行为特征基础上。

Method: 提出功能性意图测试(FIT)框架和FIT-Eval评估协议，通过五个可观测维度对系统行为进行量化评估，具体包括：目的性、预见性、意志性、时间承诺和一致性。

Result: 建立了将哲学和法律中的意图概念转化为可操作评估标准的技术框架，使系统设计可以有针对性地控制其行为特征。

Conclusion: FIT为自主AI系统提供了一个实用的行为意图评估框架，使得系统的自主性可以被量化和校准，在提高效率的同时实现可问责的风险管理。
...
### [227] [LANTERN: LLM-Augmented Neurosymbolic Transfer with Experience-Gated Reasoning Networks](https://arxiv.org/abs/2605.05478)
*Mahyar Alinejad,Yue Wang,Amrit Singh Bedi,George Atia*

Main category: cs.AI

TL;DR: LANTERN 是一个多源神经符号迁移学习框架，通过LLM生成任务自动机、语义嵌入聚合和自适应门控机制，在强化学习中显著提升样本效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号迁移学习方法通常依赖人工指定的任务自动机、假设单一源任务，并使用无法适应不同源任务相关性的固定知识集成机制，这限制了迁移学习在强化学习中的可扩展性和适用性。

Method: LANTERN 采用三个核心组件：1) 使用大语言模型从自然语言任务描述生成确定性有限状态自动机；2) 基于跨任务相似性加权的语义嵌入聚合多源策略；3) 基于时序差分误差和语义不确定性的自适应师生门控机制。

Result: 在资源管理、导航和控制等多个领域中，LANTERN 相比现有基线方法实现了40-60%的样本效率提升，同时对未对齐的源任务保持鲁棒性。

Conclusion: LANTERN 框架证明，通过多源自适应权重迁移学习，可以在符号强化学习场景中显著提升样本效率和鲁棒性，为 RL 的实际应用提供更可扩展的解决方案。
...
### [228] [FinRAG-12B: A Production-Validated Recipe for Grounded Question Answering in Banking](https://arxiv.org/abs/2605.05482)
...
TL;DR: 本文提出了一个在有限数据下高效训练面向银行业等严苛领域大语言模型的框架。通过创新的数据流水线、校准拒绝机制和端到端部署方法，训练出一个120亿参数的模型，在真实金融场景中，其性能在引用基础和安全性上超越GPT-4.1，且成本效益显著更高。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在各个领域迅速普及，但在银行业等高风险、高监管要求的行业中，其应用面临阻力。这主要源于对模型的高精度、合规性以及对可验证和基于事实的响应（grounded response）的需求。因此，需要开发一个能够在部署约束下优化答案质量、引用基础和校准拒绝功能的专门化LLM训练框架。

Method: 1. 数据生成流水线：结合了“LLM-as-a-Judge”过滤、引用标注（citation annotation）和课程学习，仅使用1.43亿token。
2. 校准拒绝机制：通过在22%不可回答的样本上进行训练，控制模型的拒绝行为，使其在不确定时输出“我不知道”。
3. 端到端方法论：覆盖从数据管理到量化部署的完整流程。

Result: 1. 使用1.43亿token训练出120亿参数的模型，在答案质量（尤其在引用基础指标上）上超越了GPT-4.1。与未调优的基础模型相比，仅在引用质量上做出适度权衡。
2. 校准拒绝机制效果显著：在22%的不可回答样本上训练后，模型给出“我不知道”的概率达到12%，相比基础模型不安全的4.3%拒绝率有极大改善，并避免了GPT-4.1过度拒绝（20.2%）的问题。
3. 系统已部署在40多家金融机构，实现了查询解决率7.1个百分点的提升（p < 0.001）。与GPT-4.1相比，响应速度快3-5倍，而成本低20-50倍。

Conclusion: 本文提出了一个统一、数据高效且可部署的框架，用于训练面向特定领域特别是银行业的LLMs。该框架通过在有限数据量（仅1.43亿token）下优化答案质量、引用基础和校准拒绝，成功构建并部署了一个在真实世界中性能优异、成本效益高的系统。模型在40多家金融机构的实际应用证明其显著提升了查询解决率，同时实现了更快的响应速度和更低的成本。这表明了在严苛领域实现高效LLM部署的可行性。
...
### [229] [Housing Potential Common Data Model and City Digital Twin](https://arxiv.org/abs/2605.05535)
*Megan Katsumi,Mark Fox,Anderson Wong,Divnoor Chatha*

Main category: cs.AI

TL;DR: 研究提出住房潜力通用数据模型（HPCDM），通过城市数字孪生和仪表板应用验证了其解决数据孤岛问题的有效性，并为实际应用提供了采纳策略。


<details>
  <summary>Details</summary>
Motivation: 当前住房潜力评估面临数据孤岛问题，不同数据集（如分区、土地利用、人口特征、服务可达性等）难以实现有效整合与互操作性，限制了全面分析的可行性。

Method: 提出住房潜力通用数据模型（HPCDM），创建城市数字孪生和试点仪表板应用，评估模型效果并分析采纳障碍。

Result: 成功开发了支持跨数据集整合的标准化数据模型，实现了住房潜在分析所需的多维度数据集成，并识别了关键采纳障碍及其缓解策略。

Conclusion: 本研究通过HPCDM的实践应用验证了其可行性，为城市规划中的数据整合难题提供了系统化的解决方案，并通过试点应用展示了在实际工作场景中的价值。
...
### [230] [AgenticRAG: Agentic Retrieval for Enterprise Knowledge Bases](https://arxiv.org/abs/2605.05538)
...
### [231] [SPARK: Self-Play with Asymmetric Reward from Knowledge Graphs](https://arxiv.org/abs/2605.05546)
...
### [232] [Who Prices Cognitive Labor in the Age of Agents? A Position on Compute-Anchored Wages](https://arxiv.org/abs/2605.05558)
*Siqi Zhu*
...
### [233] [Nonsense Helps: Prompt Space Perturbation Broadens Reasoning Exploration](https://arxiv.org/abs/2605.05566)
...
### [234] [Locality-aware Private Class Identification for Domain Adaptation with Extreme Label Shift](https://arxiv.org/abs/2605.05567)
...
### [235] [AlphaCrafter: A Full-Stack Multi-Agent Framework for Cross-Sectional Quantitative Trading](https://arxiv.org/abs/2605.05580)
...
### [236] [Belief Memory: Agent Memory Under Partial Observability](https://arxiv.org/abs/2605.05583)
...
### [237] [Causal Probing for Internal Visual Representations in Multimodal Large Language Models](https://arxiv.org/abs/2605.05593)
...
### [238] [Text-Graph Synergy: A Bidirectional Verification and Completion Framework for RAG](https://arxiv.org/abs/2605.05643)
...
### [239] [Retrieval-Conditioned Topology Selection with Provable Budget Conservation for Multi-Agent Code Generation](https://arxiv.org/abs/2605.05657)
...
### [240] [Large Vision-Language Models Get Lost in Attention](https://arxiv.org/abs/2605.05668)
...
### [241] [Attractor Geometry of Transformer Memory: From Conflict Arbitration to Confident Hallucination](https://arxiv.org/abs/2605.05686)
...
### [242] [DataDignity: Training Data Attribution for Large Language Models](https://arxiv.org/abs/2605.05687)
...
### [243] [GCCM: Enhancing Generative Graph Prediction via Contrastive Consistency Model](https://arxiv.org/abs/2605.05689)
...
### [244] [Saliency-Aware Regularized Quantization Calibration for Large Language Models](https://arxiv.org/abs/2605.05693)
...
### [245] [Inference-Time Budget Control for LLM Search Agents](https://arxiv.org/abs/2605.05701)
...
### [246] [Knowledge-Graph Paths as Intermediate Supervision for Self-Evolving Search Agents](https://arxiv.org/abs/2605.05702)
...
### [247] [Resolving the bias-precision paradox with stochastic causal representation learning for personalized medicine](https://arxiv.org/abs/2605.05706)
...
### [248] [Conceal, Reconstruct, Jailbreak: Exploiting the Reconstruction-Concealment Tradeoff in MLLMs](https://arxiv.org/abs/2605.05709)
...
### [249] [Decodable but Not Corrected by Fixed Residual-Stream Linear Steering: Evidence from Medical LLM Failure Regimes](https://arxiv.org/abs/2605.05715)
*Ming Liu*
...
### [250] [Detecting Time Series Anomalies Like an Expert: A Multi-Agent LLM Framework with Specialized Analyzers](https://arxiv.org/abs/2605.05725)
...
### [251] [SkillRet: A Large-Scale Benchmark for Skill Retrieval in LLM Agents](https://arxiv.org/abs/2605.05726)
...
### [252] [Knee Osteoarthritis Severity Grading Using Optimized Deep Learning and LLM-Driven Intelligent AI on Computationally Limited Systems](https://arxiv.org/abs/2605.05731)
...
### [253] [SDFlow: Similarity-Driven Flow Matching for Time Series Generation](https://arxiv.org/abs/2605.05736)
...
### [254] [ReFlect: An Effective Harness System for Complex Long-Horizon LLM Reasoning](https://arxiv.org/abs/2605.05737)
*Fan Huang*
...
### [255] [HyperLens: Quantifying Cognitive Effort in LLMs with Fine-grained Confidence Trajectory](https://arxiv.org/abs/2605.05741)
...
### [256] [Best Arm Identification in Generalized Linear Bandits via Hybrid Feedback](https://arxiv.org/abs/2605.05745)
...
### [257] [Evaluating Explainability in Safety-Critical ATR Systems: Limitations of Post-Hoc Methods and Paths Toward Robust XAI](https://arxiv.org/abs/2605.05748)
...
### [258] [Confidence is the key: how conformal prediction enhances the generative design of permeable peptides](https://arxiv.org/abs/2605.05770)
...
### [259] [CircuitFormer: A Circuit Language Model for Analog Topology Design from Natural Language Prompt](https://arxiv.org/abs/2605.05773)
...
### [260] [HEDP: A Hybrid Energy-Distance Prompt-based Framework for Domain Incremental Learning](https://arxiv.org/abs/2605.05776)
...
### [261] [Von Neumann Networks](https://arxiv.org/abs/2605.05780)
*Shekhar S. Chandra*
...
### [262] [Sheet as Token: A Graph-Enhanced Representation for Multi-Sheet Spreadsheet Understanding](https://arxiv.org/abs/2605.05811)
...
### [263] [Long-Horizon Q-Learning: Accurate Value Learning via n-Step Inequalities](https://arxiv.org/abs/2605.05812)
...
### [264] [AGPO: Asymmetric Group Policy Optimization for Verifiable Reasoning and Search Ads Relevance at JD](https://arxiv.org/abs/2605.05826)
...
### [265] [On the Role of Language Representations in Auto-Bidding: Findings and Implications](https://arxiv.org/abs/2605.05833)
...
### [266] [Taklif.AI: LLM-Powered Platform for Interest-Based Personalized College Assignments](https://arxiv.org/abs/2605.05842)
...
### [267] [AirQualityBench: A Realistic Evaluation Benchmark for Global Air Quality Forecasting](https://arxiv.org/abs/2605.05854)
...
### [268] [SANEmerg: An Emergent Communication Framework for Semantic-aware Agentic AI Networking](https://arxiv.org/abs/2605.05861)
...
### [269] [XDecomposer: Learning Prior-Free Set Decomposition for Multiphase X-ray Diffraction](https://arxiv.org/abs/2605.05866)
...
### [270] [Agentic, Context-Aware Risk Intelligence in the Internet of Value](https://arxiv.org/abs/2605.05878)
...
### [274] [Intentmaking and Sensemaking: Human Interaction with AI-Guided Mathematical Discovery](https://arxiv.org/abs/2605.05921)
...
### [275] [Which Are the Low-Resource Languages of the Semantic Web?](https://arxiv.org/abs/2605.05929)
...
### [276] [In Data or Invisible: Toward a Better Digital Representation of Low-Resource Languages with Knowledge Graphs](https://arxiv.org/abs/2605.05931)
...
### [277] [ICU-Bench:Benchmarking Continual Unlearning in Multimodal Large Language Models](https://arxiv.org/abs/2605.05938)
...
### [278] [MAS-Algorithm: A Workflow for Solving Algorithmic Programming Problems with a Multi-Agent System](https://arxiv.org/abs/2605.05949)
...
### [279] [HaM-World: Soft-Hamiltonian World Models with Selective Memory for Planning](https://arxiv.org/abs/2605.05951)
...
### [280] [Temporal Smoothness Doubly Robust Learning for Debiased Knowledge Tracing](https://arxiv.org/abs/2605.05958)
...
### [281] [From Coordinate Matching to Structural Alignment: Rethinking Prototype Alignment in Heterogeneous Federated Learning](https://arxiv.org/abs/2605.05959)
...
### [282] [TheraAgent: Self-Improving Therapeutic Agent for Precise and Comprehensive Treatment Planning](https://arxiv.org/abs/2605.05963)
...
### [283] [BehaviorGuard: Online Backdoor Defense for Deep Reinforcement Learning](https://arxiv.org/abs/2605.05977)
...
### [284] [TACT: Mitigating Overthinking and Overacting in Coding Agents via Activation Steering](https://arxiv.org/abs/2605.05980)
...
### [285] [BioResearcher: Scenario-Guided Multi-Agent for Translational Medicine](https://arxiv.org/abs/2605.05985)
...
### [286] [Strat-LLM: Stratified Strategy Alignment for LLM-based Stock Trading with Real-time Multi-Source Signals](https://arxiv.org/abs/2605.06024)
...
### [287] [Pathways to AGI](https://arxiv.org/abs/2605.06029)
...
### [288] [Novelty-based Tree-of-Thought Search for LLM Reasoning and Planning](https://arxiv.org/abs/2605.06040)
...
### [289] [Visual Fingerprints for LLM Generation Comparison](https://arxiv.org/abs/2605.06054)
...
### [290] [VibeServe: Can AI Agents Build Bespoke LLM Serving Systems?](https://arxiv.org/abs/2605.06068)
...
### [291] [Safety Certification is Classification](https://arxiv.org/abs/2605.06087)
...
### [292] [Shallow Prefill, Deep Decoding: Efficient Long-Context Inference via Layer-Asymmetric KV Visibility](https://arxiv.org/abs/2605.06105)
...
### [293] [On Time, Within Budget: Constraint-Driven Online Resource Allocation for Agentic Workflows](https://arxiv.org/abs/2605.06110)
...
### [294] [CrossCult-KIBench: A Benchmark for Cross-Cultural Knowledge Insertion in MLLMs](https://arxiv.org/abs/2605.06115)
...
### [295] [Back to the Beginning of Heuristic Design: Bridging Code and Knowledge with LLMs](https://arxiv.org/abs/2605.06123)
...
### [296] [P-Guide: Parameter-Efficient Prior Steering for Single-Pass CFG Inference](https://arxiv.org/abs/2605.06124)
*Xin Peng,Ang Gao*
...
### [297] [Skill1: Unified Evolution of Skill-Augmented Agents via Reinforcement Learning](https://arxiv.org/abs/2605.06130)
...
### [298] [Graphlets as Building Blocks for Structural Vocabulary in Knowledge Graph Foundation Models](https://arxiv.org/abs/2605.06154)
...
### [299] [Beyond Accuracy: Policy Invariance as a Reliability Test for LLM Safety Judges](https://arxiv.org/abs/2605.06161)
...
### [300] [BioMedArena: An Open-source Toolkit for Building and Evaluating Biomedical Deep Research Agents](https://arxiv.org/abs/2605.06177)
...
### [301] [Rethinking Adapter Placement: A Dominant Adaptation Module Perspective](https://arxiv.org/abs/2605.06183)
...
### [302] [Event-Causal RAG: A Retrieval-Augmented Generation Framework for Long Video Reasoning in Complex Scenarios](https://arxiv.org/abs/2605.06185)
...
### [304] [Systematic Evaluation of Large Language Models for Post-Discharge Clinical Action Extraction](https://arxiv.org/abs/2605.06191)
...
### [306] [Towards Annotation-Free Validation of MLLMs: A Vision-Language Logical Consistency Metric](https://arxiv.org/abs/2605.06201)
...
### [307] [Beyond Fixed Benchmarks and Worst-Case Attacks: Dynamic Boundary Evaluation for Language Models](https://arxiv.org/abs/2605.06213)
...
### [308] [Joint Consistency: A Unified Test-Time Aggregation Framework via Energy Minimization](https://arxiv.org/abs/2605.06219)
...
### [309] [Proactive Instance Navigation with Comparative Judgment for Ambiguous User Queries](https://arxiv.org/abs/2605.06223)
...
### [310] [A Versatile AI Agent for Rare Disease Diagnosis and Risk Gene Prioritization](https://arxiv.org/abs/2605.06226)
...
### [311] [Price of Fairness in Short-Term and Long-Term Algorithmic Selections](https://arxiv.org/abs/2605.06227)
...
### [312] [Safactory: A Scalable Agent Factory for Trustworthy Autonomous Intelligence](https://arxiv.org/abs/2605.06230)
...
### [313] [Data Language Models: A New Foundation Model Class for Tabular Data](https://arxiv.org/abs/2605.06290)
...
### [314] [Addressing Labelled Data Scarcity: Taxonomy-Agnostic Annotation of PII Values in HTTP Traffic using LLMs](https://arxiv.org/abs/2605.06305)
...
### [315] [Mind the Gap? A Distributional Comparison of Real and Synthetic Priors for Tabular Foundation Models](https://arxiv.org/abs/2605.06343)
...
### [316] [More Than Can Be Said: A Benchmark and Framework for Pre-Question Scientific Ideation](https://arxiv.org/abs/2605.06345)
*Jie Yu,Song Qiu*
...
### [317] [Prediction and Empowerment: A Theory of Agency through Bridge Interfaces](https://arxiv.org/abs/2605.06346)
*Richard Csaky*
...
### [318] [From Agent Loops to Deterministic Graphs: Execution Lineage for Reproducible AI-Native Work](https://arxiv.org/abs/2605.06365)
...
### [319] [Debiased Multimodal Personality Understanding through Dual Causal Intervention](https://arxiv.org/abs/2605.06371)
...
### [320] [Rethinking Vacuity for OOD Detection in Evidential Deep Learning](https://arxiv.org/abs/2605.06382)
*Claire McNamara*
...
### [321] [Automated alignment is harder than you think](https://arxiv.org/abs/2605.06390)
...
### [322] [Knowledge Graphs, the Missing Link in Agentic AI-based Formal Verification](https://arxiv.org/abs/2605.06434)
...
### [323] [SCRuB: Social Concept Reasoning under Rubric-Based Evaluation](https://arxiv.org/abs/2605.06444)
...
### [324] [PrefixGuard: From LLM-Agent Traces to Online Failure-Warning Monitors](https://arxiv.org/abs/2605.06455)
...
### [325] [Beyond Task Success: Measuring Workflow Fidelity in LLM-Based Agentic Payment Systems](https://arxiv.org/abs/2605.06457)
...
### [326] [Probabilistic Dating of Historical Manuscripts via Evidential Deep Regression on Visual Script Features](https://arxiv.org/abs/2605.06475)
*Ranjith Chodavarapu*
...
### [327] [Patch-Effect Graph Kernels for LLM Interpretability](https://arxiv.org/abs/2605.06480)
...
### [328] [ReasonSTL: Bridging Natural Language and Signal Temporal Logic via Tool-Augmented Process-Rewarded Learning](https://arxiv.org/abs/2605.06483)
...
### [329] [Instrumental Choices: Measuring the Propensity of LLM Agents to Pursue Instrumental Behaviors](https://arxiv.org/abs/2605.06490)
...
### [330] [From Token Lists to Graph Motifs: Weisfeiler-Lehman Analysis of Sparse Autoencoder Features](https://arxiv.org/abs/2605.06494)
...
### [331] [Process Matters more than Output for Distinguishing Humans from Machines](https://arxiv.org/abs/2605.06524)
...
### [332] [SpatialEpiBench: Benchmarking Spatial Information and Epidemic Priors in Forecasting](https://arxiv.org/abs/2605.06530)
...
### [333] [Ex Ante Evaluation of AI-Induced Idea Diversity Collapse](https://arxiv.org/abs/2605.06540)
...
### [334] [Improved techniques for fine-tuning flow models via adjoint matching: a deterministic control pipeline](https://arxiv.org/abs/2605.06583)
...
### [335] [NeuroAgent: LLM Agents for Multimodal Neuroimaging Analysis and Research](https://arxiv.org/abs/2605.06584)
...
### [336] [SkillOS: Learning Skill Curation for Self-Evolving Agents](https://arxiv.org/abs/2605.06614)
...
### [337] [MASPO: Joint Prompt Optimization for LLM-based Multi-Agent Systems](https://arxiv.org/abs/2605.06623)
...
### [338] [Can RL Teach Long-Horizon Reasoning to LLMs? Expressiveness Is Key](https://arxiv.org/abs/2605.06638)
...
### [340] [AI Co-Mathematician: Accelerating Mathematicians with Agentic AI](https://arxiv.org/abs/2605.06651)
