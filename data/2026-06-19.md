...
TL;DR: 提出了AgenticRei系统，使用基于Rei框架的规范策略语言（OWL）来管理基于LLM的自主智能体系统的安全、隐私和合规治理，超越了现有策略引擎仅支持允许/禁止的限制。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的自主智能体系统引入了新的安全、隐私和合规挑战。智能体能够调用工具、操作数据、安装软件并在组织边界内协调，需要全面的企业治理结构来约束，而不仅仅是身份验证和访问控制。现有的策略引擎（如XACML、Rego、Cedar）只解决了允许/禁止子集，缺乏义务生命周期管理、元策略冲突解决、特定情况下的义务豁免以及领域本体推理等功能。

Method: 提出AgenticRei系统，使用基于Rei框架的规范策略语言，以OWL（Web Ontology语言）表达，并由高性能逻辑引擎在LLM外部进行运行时评估。该系统管理智能体工具调用和智能体间消息传递，支持义务、豁免、策略冲突解决和政策推理等关键治理要求。

Result: 通过示例证明规范策略能够捕捉当前生产引擎中大多无法表达的安全和隐私治理约束。该方法与行业标准框架（如A2AS）自然集成，为智能体系统提供了全面的治理能力。

Conclusion: AgenticRei系统解决了基于LLM的自主智能体系统的全面治理需求，超越了现有策略引擎的限制，为智能体系统的安全、隐私和合规管理提供了有效的解决方案。
...
TL;DR: 研究人员开发了一种人机协作的管道工具，用于测量计算机科学本科课程对国际课程指南的覆盖程度。通过应用于一个认证BSc计算机科学项目，对比CS2013和CS2023指南，发现课程覆盖度约为50%，十年间保持稳定，但CS2023对认知深度的要求更高。


<details>
  <summary>Details</summary>
Motivation: 计算机科学本科教育受国际课程指南指导，但这些指南大约每十年修订一次。目前缺乏可靠、可重复的方法来测量课程项目如何完全覆盖当前指南，以及当指南重构时这种覆盖如何变化。这需要一种系统的评估工具来量化课程与外部知识体系的对齐程度。

Method: 提出了一种人机协作管道：1）将课程项目和每个指南表示为结构化语料库；2）通过语义检索生成课程到知识单元的候选匹配；3）在明确的覆盖定义下通过人工判断确认这些匹配。对七种检索器进行基准测试，采用互惠排名融合集成。设计扩展到能力表述和认知深度分析。通过独立第二评分者验证映射结果。

Result: 1）最佳检索器是互惠排名融合集成，长上下文模型表现不如小型句子模型；2）映射验证显示Cohen's kappa为0.64（CS2023）和0.69（CS2013）；3）项目覆盖CS2023知识单元的49.7%和CS2013的50.9%，十年间保持稳定；4）项目对约88%的覆盖单元表述了能力，但在推荐认知深度方面：CS2023为76%，CS2013为95%，反映了新指南的更高期望；5）识别出持续的结构性差距领域（并行和分布式计算、编程语言基础、系统基础）。

Conclusion: 开发的工具能够可靠、可重复地测量计算机科学课程对国际指南的覆盖程度。纵向比较揭示了课程与指南的稳定对齐以及CS2023提高的深度要求。该工具可重复使用，可用于评估课程项目与外部知识体系的对齐，分离出结构性差距与标准演变的影响。
...
TL;DR: 本文系统评估了8种现代扩散语言模型在8个基准上的性能，分析了推理阶段设计选择（如去噪步数、上下文长度等）对生成质量和计算效率的影响，揭示了扩散模型在不同任务中的优势与局限性。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型（DLMs）作为自回归生成模型的新兴替代方案，由于评价协议、数据集、推理预算和生成超参数的不统一，导致不同模型之间难以进行公平比较和理解其优劣权衡。

Method: 对8种最先进的扩散语言模型在8个基准（涵盖推理、编码、翻译、知识和结构化问题求解）上进行系统实验分析，同时评估生成质量和计算效率；分析推理阶段关键因素（去噪步数、上下文长度、块大小、并行解掩码策略）的影响；并在相同条件下对小模型进行控制对比实验。

Result: 扩散语言模型的行为受到生成阶段设计选择的强烈影响，不同任务、架构和推理预算之间呈现出性能与计算效率的不同权衡。

Conclusion: 本研究为当代扩散语言模型的能力和部署特性提供了实用的洞见，有助于理解其在不同任务场景下的适用性。
...
TL;DR: 本文提出了一种闭环动力系统模型来解释多智能体LLM deliberation过程，其中每个智能体拥有一个隐藏的"锚定信念"，该信念持续影响其观点更新，可以解释传统共识模型无法解释的现象——智能体对正确答案的信心可以超越初始信念的凸包范围。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM deliberation（多轮交换和修订答案）已被广泛用于提升推理能力，但其工作机制和原因鲜有理论建模。作者从人类决策的社会心理学（群体效应与个体信念的拉扯）出发，希望建立数学模型解释deliberation为何有效。

Method: 将多智能体deliberation建模为闭环动力系统，每个智能体携带一个隐藏的内部信念（anchor），该锚点持续拉拽其观点，不受邻居影响。通过deliberation过程可以恢复该锚点，并提出通过检查恢复的锚点是否能预测保留轮次（泛化能力）来验证模型是否真正受锚点驱动。

Result: 在三个开放权重模型家族上的实验表明，锚点影响呈现一个谱系而非全有或全无。所有模型锚点影响强度大致相当，但锚点位置不同；只有当锚点远离初始观点时，deliberation才会超越凸包，需要使用完整的闭环模型。

Conclusion: 该锚定动力学模型成功解释了传统共识规则无法解释的行为（信心超越初始凸包），为理解多智能体LLM deliberation提供了理论基础，并给出了判断模型是否真正受锚点驱动的简单检验方法。
...
TL;DR: 本文提出DeXposure-Claw，一个基于预测引导的去中心化金融监管代理系统，通过结构化证据路由LLM决策，并配套DeXposure-Bench评估框架，在五年真实周度数据上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有通用LLM代理在去中心化金融监管中存在两个问题：（1）过度解读弱证据，推荐高风险干预措施；（2）缺乏与监管目标对齐的评估指标来量化由此引发的误报。

Method: 提出DeXposure-Claw系统，包含三个核心模块：（1）DeXposure-FM，一个图时间序列基础模型，用于预测未来风险暴露网络；（2）确定性监控器和压力情景模块，将预测转化为类型化警报、归因信号和情景证据；（3）数据健康与置信度门控机制，约束事件升级。同时开发DeXposure-Bench六轴评估框架，包含监管对齐的绝对损失真实值和显式错误干预率指标。

Result: 在五年每周真实数据上进行的实验完全支持该系统的有效性。

Conclusion: DeXposure-Claw通过结构化证据路由的方式，显著改进了LLM代理在去中心化金融监管中的决策质量，降低了误报干扰，同时提供了符合监管需求的评估标准。
...
TL;DR: LLM在处理结构化临床数据时存在"认知自我意识"问题：口头表达的置信度是空洞的，且表现出反难度效应。通过few-shot示例和SHAP特征证据的联合使用，可以显著改善预测精度。基于归因分歧的跨模型校准器能够大幅降低校准误差。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM越来越多地应用于结构化临床数据，但它们是否能够识别自身在此类任务中的知识局限尚未被探索。研究旨在通过跨模型归因分歧的角度来减少结构化任务中的认知不确定性。

Method: 通过归因分歧分析比较Qwen 2.5 7B和XGBoost在预测任务上的表现。评估了verbalized confidence的认知价值，分析了反难度效应，测试了few-shot示例和SHAP-derived feature evidence作为正交干预措施的效果，并开发了一个基于归因分歧信号的跨模型校准器。

Result: 1. LLM口头置信度是空洞的（0.856-0.937），不反映实际准确性。2. 存在反难度效应：XGBoost正确率99%时LLM准确率降至64.8%，但当XGBoost中度不确定时两者表现相当。3. few-shot示例和SHAP特征证据将归因分歧得分从1.54降至0.38，准确率从49%提升至75.3%。4. 跨模型校准器将预期校准误差从0.254降至0.080。

Conclusion: 这些发现揭示了LLM在结构化数据上的"冷启动"问题。基于归因分歧的跨模型校准能够取代无信息量的口头置信度，提供患者特定的可靠性估计，为LLM在结构化数据上实现真正的认知自我意识指明了路径。
...
TL;DR: 提出了一种基于连续表型相似性的对比学习框架，将视网膜图像与临床风险数据对齐，用于阿尔茨海默病的早期预测，优于传统的离散分组方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法将表型相似性视为离散构造，依赖硬分组进行对比学习，这种刚性监督方式割裂了分组过程与表示学习，无法反映疾病风险的连续谱特征。

Method: 提出了连续表型相似性建模：利用视网膜图像和风险特征的内模态嵌入相似度构建可微权重函数，定义软多正例关系；并设计软目标对比损失，实现跨模态对齐与表型结构的端到端联合学习。

Result: 在UK Biobank视网膜影像数据上，该方法在阿尔茨海默病预测任务中一致优于离散分组对比学习及标准视觉-语言基线方法。

Conclusion: 将表型相似性作为可学习的连续信号而非固定分组规则，为基于多模态视网膜和临床数据的大规模神经退行性疾病风险建模提供了更稳健的基础。
...
TL;DR: 本文提出了一种让LLM通过自我审查（conscience step）和基于DPO的 alignment 训练，实现自我伦理对齐的方法，无需外部评判模型，并在代码黑客等场景下证明了 Emergent Alignment 的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在生成内容时可能产生与人类伦理相悖的输出，且缺乏有效的自我纠正机制。本文旨在探索LLM是否能够自主识别并修正其非伦理输出，实现无需外部评判模型的自我对齐。

Method: 在LLM中引入一个"良心步骤"（conscience step），使其审查自身的推理和输出；同时扩展训练损失，加入基于直接偏好优化（DPO）的对齐组件，引导模型远离非伦理输出。该方法在线进行，适用于训练、微调、对抗提示和零样本学习，依赖模型自身的冻结副本而非外部评判器。

Result: 在代码黑客场景下，与先前工作显示的Emergent Misalignment相反，本文提出的方法通过单个高层级的自省性问题即可引导模型实现伦理对齐，成功展示了Emergent Alignment现象。

Conclusion: 本文证明了LLM可以通过内部自省机制和DPO-based对齐训练实现自我伦理对齐，无需外部更弱或更强的评判模型，为构建更安全的AI系统提供了新思路。
...
TL;DR: ITNet是一种统一的神经网络架构，使用可学习的积分变换作为核心算子，能够涵盖卷积网络、循环网络和Transformer等不同类型架构的归纳偏置，并在多个基准测试中达到或超越专用基线模型。


<details>
  <summary>Details</summary>
Motivation: 卷积网络、循环网络和Transformer各自编码了不同的归纳偏置（局部性、顺序记忆和内容依赖的成对交互），但这些架构在数学上一直保持分离。作者认为这种分离反映了对单一基础数学对象（可学习积分变换）的不完整理解，而非信号处理方式的根本多样性。

Method: 提出了积分变换网络（ITNet），围绕一个同时依赖于位置和特征的可学习核函数构建。核函数通过小型神经网络（MLP）实现，用于建模成对交互。通过适当的参数化，卷积、自注意力（包括多头注意力）和自回归循环（包括LSTM、GRU、S4和Mamba）都能作为特例出现。开发了平铺核融合、重要性加权蒙特卡洛积分和学习的低秩分解等技术来实现高效可扩展计算。

Result: 单一ITNet架构（共享算子+轻量级模态特定编码器）在ImageNet-1K、GLUE、ModelNet40、VQA-v2和NLVR2等多个基准测试中匹配或超越了专用基线模型。证明了单一学习交互机制可以从数据中恢复所有三种架构家族的行为。

Conclusion: 卷积网络、循环网络和Transformer的分离并非源于信号处理的根本多样性，而是对单一基础数学对象（可学习积分变换）的不完整视图。ITNet提供了一个统一的框架，表明一个共享的学习交互机制足以捕捉多种架构的归纳偏置，为实现更通用和高效的神经网络架构开辟了新途径。
...
TL;DR: 针对交互式LLM代理中传统不确定性框架的不足，提出了一种简单的基于提示的不确定性分解方法，将行动置信度与请求不确定性分离，使代理能在任务说明不明确时主动寻求澄清。在五个LLM骨干网络上，该方法在ALFWorld-Clarification基准上F1值比ReAct+UE提升73%，比UAM提升36%。


<details>
  <summary>Details</summary>
Motivation: 经典的不确定性框架（aleatoric/epistemic）不足以支撑交互式LLM代理的新能力（如主动寻求澄清、共享心智模型构建），而实际部署约束（黑盒API、延迟预算、缺乏标注轨迹）排除了基于logprob、多采样和训练的方法，因此需要一种基于提示的、可分解的不确定性表征方法。

Method: 提出一种基于提示的简单分解方法，将行动置信度（action confidence）与请求不确定性（request uncertainty, u）分离，使代理能在任务说明模糊时主动向用户请求澄清。同时引入两个澄清增强基准（WebShop-Clarification和ALFWorld-Clarification），其中50%的任务故意未充分说明。

Result: 在五个LLM骨干网络（GPT-5.1, DeepSeek-v3.2-exp, GLM-4.7, Qwen3.5-35B, GPT-OSS-120B）上，该方法在ALFWorld-Clarification的澄清F1值比ReAct+UE平均提升73%，比UAM平均提升36%。在WebShop-Clarification上对所有骨干网络、在ALFWorld-Clarification上对五个中的四个骨干网络的澄清F1值均领先。

Conclusion: 所提出的简单基于提示的不确定性分解方法能够有效提升交互式LLM代理在任务模糊场景下的澄清能力，且该增益在多个不同LLM骨干网络上具有泛化性，验证了该方法在实际部署约束下的实用价值。
...
TL;DR: 本文研究了LLM-SAT/SMT求解器混合管道中的叙述环节（narration）对形式化验证结论可靠性的影响，发现即使求解器给出可靠结论，攻击者仍可通过提示注入在最终用户答案中颠覆验证结果，证书门控（certificate gating）虽能保证求解器结论可靠，但无法保障用户读到的最终答案的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 形式化工具（SAT/SMT求解器）被嵌入语言模型推理管道以提供可验证的安全性保证，但现有工作忽视了将求解器输出转化为用户答案的"叙述"环节，这一环节可能丢失求解器的可靠性保证，存在安全漏洞。

Method: 首先将LLM-求解器循环建模为一种经过验证的决策过程；然后在提示注入下评估5个开源模型；通过证书门控（certificate gating）确保求解器结论的可靠性；分析加固提示的缓解效果及自适应攻击下的脆弱性。

Result: 证书门控能使求解器结论可靠，但攻击者可通过不同的措辞和渠道颠覆已验证的结论；加固提示能显著降低注入成功率但无法完全消除，且在自适应攻击下仍然脆弱。

Conclusion: 在LLM-求解器循环中，鲁棒性无法延伸到用户最终读到的答案，提示注入可以在叙述环节颠覆形式化验证的可靠结论。
...
TL;DR: 本文提出ACIE系统，一个基于代理的RAG管线，用于处理临床文档中缺失的元数据问题，通过推理完整患者上下文并引用来源，在7326次临床评估中达到96.5%的接受率。


<details>
  <summary>Details</summary>
Motivation: 临床环境中患者数据包含大量异构文档和结构化数据点，但AI系统所需的文档级元数据缺失或不完整，导致标准RAG方法在处理时间推理、跨文档依赖和缺失元数据时表现不佳。

Method: 部署ACIE（Agentic Clinical Information Extraction）系统，一个本地部署的代理型RAG管线，能够推理完整患者上下文，并将每个答案基于源段落以便临床医生验证。

Result: 在7326项临床评估中，医生接受了96.5%的提取结果，各类型接受率在80%到99%之间。

Conclusion: ACIE系统有效解决了临床文档中元数据缺失的问题，通过代理型RAG管线实现了高可靠性的信息提取，并得到了临床医生的高度认可。
...
TL;DR: 本文研究偏好对齐中的比较对选择问题，提出将比较筛选形式化为采样设计问题，并针对DPO训练推导了上下界，表明比较选择通过信息矩阵影响下游性能，据此设计了高效的采样策略，在实验中优于常见启发式方法。


<details>
  <summary>Details</summary>
Motivation: 人工偏好标注成本远高于生成额外补全，因此需要在有限标注预算下选择最具信息量的比较对进行标注，以提升偏好对齐的样本效率。

Method: 将比较筛选建模为采样设计问题，针对DPO训练推导了策略最优性差距的匹配上下界，构建了依赖设计的信�矩阵作为优化标准，并据此设计了实用的采样策略。

Result: 在合成设置和语言模型后训练基准上，提出的采样设计方法在样本效率上持续优于常见的比较选择启发式方法。

Conclusion: 比较对的选择可通过信息矩阵影响DPO策略性能，基于该理论的采样设计能有效提升偏好标注预算的利用效率。
...
TL;DR: 提出TOTEN框架，用基于本体的知识驱动方法替代BPE的统计分词，在工程实体识别上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: BPE分词对结构化技术实体（物理量、数字、单位、符号表达式）语义不敏感，导致切分碎片化，缺乏语义完整性。

Method: 构建工程实体本体论(OEE)，形式化为三元组<O, classify, {inst_tau}>，结合三个外部知识源（Pint维度库、Unicode字符数据库、RSLP葡萄牙语形态学）实现确定性耦合分类。

Result: 在单位本体原子性上全面超越8个基线；数值重建在外部语料达0.775-0.904（最佳基线0.627-0.703），在EngQuant上0.780 vs 0.340，差异统计显著。

Conclusion: 基于本体论的知识驱动分词在技术实体处理上优于统计方法，提供了可验证的结构化表示，适用于工程领域文本分析。
...
TL;DR: INCOSE INSIGHT 2020年特刊引发了AI与系统工程领域研究热潮，本文回顾了该领域三个发展阶段，通过人类专家与AI模型共同评估了大量文献，识别了五个关键研究空白，并为从业者提供了AI采纳和应用指导。


<details>
  <summary>Details</summary>
Motivation: 基于2020年INCOSE INSIGHT关于AI与系统工程的特别期刊的成功，该领域形成了活跃的研究社群。作者希望追溯AI与系统工程融合的发展历程，评估当前研究现状，识别关键的研究空白，为从业者在系统工程中应用AI提供指导。

Method: 1. 文献回顾与历史分析：将AI与系统工程的发展分为三个阶段（基础阶段、应用阶段、LLM拐点阶段）；2. 人机协同文献评估：结合人类专家和6个AI模型，评估了1,712篇INCOSE INSIGHT文章和889篇SERC出版物；3. 开发了AI4SE/SE4AI Explorer网络应用程序，使读者能够比较自己的判断与人类和AI评估结果。

Result: 1. 成功追溯了AI与系统工程领域的发展历程和三个阶段演变；2. 通过人机协同评估识别出五个关键的研究空白；3. 为系统工程领域的AI采纳、保证和劳动力转型提供了实践指导；4. 创建了公开的评估数据和网络应用程序，促进了社区参与和比较分析。

Conclusion: AI与系统工程领域经历了显著发展并形成了活跃的研究社区，但仍存在关键的研究空白需要填补。通过人机协同方法能够有效评估研究现状和识别关键问题，为从业者提供了实用的指导工具，并促进了社区参与和持续对话。
...
TL;DR: 提出了一种基于3D掩码自编码器（MAE）的标记器，用于三维脑MRI的潜在扩散模型，通过解耦编码器和解码器，使编码器保留临床信息，解码器重建解剖结构，在23个临床任务中达到或超越SOTA，并支持条件生成和纵向预测。


<details>
  <summary>Details</summary>
Motivation: 现有基于重建驱动的标记器在三维脑MRI潜在扩散中难以兼顾编码器保留临床信息和解码器重建解剖结构这两项相互竞争的需求。

Method: 提出全容积掩码自编码器（MAE）标记器，解耦编码器与解码器：冻结的3D MAE编码器产生临床信息嵌入，专用CNN解码器从嵌入的线性投影重建体素。编码器在35,309个来自18个公开队列的体数据上预训练，覆盖四种模态、十种疾病类别及200+采集站点。

Result: 在23任务线性探针基准中，编码器在21项任务上优于或持平SOTA模型（BrainIAC、BrainSegFounder、MedicalNet）。基于该嵌入训练的条件扩散Transformer（DiT）支持跨六变量条件生成及患者特异性纵向预测。

Conclusion: 建立了一个统一的3D脑MRI嵌入空间，能够同时支持下游临床任务和可控生成。
...
TL;DR: 提出DIF方法，通过内容相似物品推断伪标签并建模置信度与不确定性，实现冷启动场景下隐式反馈的去噪，已在快手大规模部署并提升商业指标。


<details>
  <summary>Details</summary>
Motivation: 冷启动物品更容易受到隐式反馈中噪声（如点击诱饵、位置偏差）的影响，而现有去噪方法在冷启动场景下适应性有限且效果不佳。

Method: 利用用户偏好的稳定性，通过内容相似的热启物品推断伪标签；基于内容相似度建模伪标签置信度并聚合多个伪标签；通过相对熵和物品冷启动状态估计噪声标签的不确定性，自适应地用伪标签修正噪声标签。

Result: 在真实数据集上进行了广泛实验，并在亿级用户规模的快手短视频应用上部署，显著提升了冷启动场景下的各项商业指标。

Conclusion: DIF是一种模型无关的冷启动去噪方法，通过伪标签与不确定性建模有效纠正噪声标签，理论分析与实验均证明了其优越性。
...
### [74] [Computational Identifiability](https://arxiv.org/abs/2606.19361)
*Lucius E. J. Bynum,Rajesh Ranganath,Kyunghyun Cho*

Main category: cs.LG

TL;DR: 本文提出“计算可识别性”框架，区别于传统理论可识别性，通过有限计算搜索过程在误差容忍度内寻找经验估计量，从而在有限样本、模糊图标准、混合数据等实际条件下定义因果效应的可识别性。


<details>
  <summary>Details</summary>
Motivation: 传统理论可识别性假设无穷数据等理想条件，无法回答有限样本、模糊图标准等实际场景下的识别问题，因此需要一种计算受限的可识别性框架。

Method: 提出“计算可识别性”概念，定义为：在给定参数先验分布和搜索过程假设下，通过有限计算搜索程序寻找经验估计量，若能在误差容忍度内找到则视为可识别。

Result: 通过实验证明该框架能够回答传统方法无法处理的细粒度识别问题，包括：小有限样本识别、模糊图标准识别、混合观测-干预数据识别、反事实数据与估计量上的识别。

Conclusion: 计算可识别性扩展了传统可识别的适用范围，为实际数据分析中的因果效应识别提供了更灵活、更实用的判断标准。
...
### [75] [When to Trust, How to Distill: Multi-Foundation Model Guidance for Lightweight, Robust Scientific Time Series Forecasting](https://arxiv.org/abs/2606.19363)
*Rupasree Dey,Abdul Matin,Nathan Orwick,Yao Zhang,Shrideep Pallickara,Sangmi Lee Pallickara*

Main category: cs.LG

TL;DR: GUARD是一个新颖的知识蒸馏框架，从分布不匹配的基础模型中提取潜在结构知识来训练轻量级专业预测器，通过上下文路由和不确定性门控温度机制动态选择最佳教师模型，实现边缘计算环境中的高精度科学预测。


<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型在物理科学应用中面临关键权衡：虽然这些模型编码了丰富、通用的时间动态，但在零样本应用于特定科学领域时存在严重的分布不匹配问题，且计算成本高无法部署在边缘计算传感器网络中。需要解决如何从分布不匹配的基础模型中提取潜在结构知识来训练轻量级专业预测器的问题。

Method: 提出GUARD框架，将多教师蒸馏重构为实例级决策过程，包含两个自适应机制：1）上下文路由器基于局部输入统计动态选择最相关教师，利用不同基础模型间的互补性；2）不确定性门控温度机制作为"断路器"，当教师置信度与领域现实偏离时自动减弱蒸馏强度。

Result: 在四个气候关键领域（气象学、生态系统碳通量、土壤湿度、能源电网）进行评估，相对于固定权重的多教师蒸馏基线显著降低了RMSE。即使预训练基础模型因原始数据与目标数据域之间的分布偏移而表现出次优的零样本准确性，仍能成功从中蒸馏知识。这些领域不匹配的教师模型仍然可以作为关键纠正器，在28.5%的最困难实例上优于全局优越的基础模型。

Conclusion: GUARD框架能够有效从分布不匹配的基础模型中提取知识，训练轻量级专业预测器，实现适合资源受限边缘部署的高精度科学预测，解决了时间序列基础模型在物理科学应用中的分布不匹配和计算成本双重挑战。
...
### [76] [Closing the Social-Semantic Gap: SPSD for Edge-Based Prompt Compression in Cloud LLM Inference](https://arxiv.org/abs/2606.19364)
*Abhinit Sen,Ajeet Kumar,Manaranjan Pradhan*

Main category: cs.LG

TL;DR: SPSD是一个边缘计算流程，使用4位量化的小型语言模型压缩用户提示，减少云LLM推理的token输入成本，在预设非劣性范围内保持响应质量的同时实现显著token节省和能量节约。


<details>
  <summary>Details</summary>
Motivation: LLM推理的prefill阶段日益成为云规模能源成本的重要贡献者。许多消费者支持和对话提示包含社交脚手架（礼貌标记、道歉前言、重复和关系建立语言），这些对人类交流很重要但对机器推理信息量有限，形成了社会语义鸿沟。

Method: 提出SPSD（情感保留语义蒸馏）边缘流程：使用4位量化的小型语言模型（Gemma-2-2B-Instruct）压缩用户提示，然后传输到云部署的LLM（Llama-3.1-8B-Instruct）。通过基于规则的门控保守地将安全关键领域路由到直通模式。

Result: 在248个提示语料库评估中，平均每次蒸馏调用节省99.9个输入token，所有146次蒸馏调用都产生正向节省。响应质量在15分制量表中非劣于原始路径（预设1分边际）；LLM裁判评估显示43%平局、28%蒸馏获胜、29%原始获胜。余弦相似度平均0.682、中位数0.712，54.1%的对超过0.70参考阈值。每次调用净能量节省估计为70-270微瓦时。

Conclusion: SPSD证明设备端提示蒸馏可以在实用非劣性范围内减少云LLM输入token成本，同时保持响应质量，为降低云规模LLM推理的能源成本提供了有效解决方案。
...
### [77] [Performance Analysis and Optimization of 3D Generative Diffusion Models across GPU Architectures](https://arxiv.org/abs/2606.19365)
*Jeeho Ryoo,Yongchan Jung,Muhammad Ali Khaliq,Weidong Zhang,Jiatong Han,Byeong Kil Lee*

Main category: cs.LG

TL;DR: 本文对Med-DDPM扩散模型在三种NVIDIA架构上进行深度性能分析，发现训练瓶颈主要来自cuDNN卷积和隐式GEMM内核，通过TF32 Tensor Core和3D channels-last布局优化，在不影响合成质量的前提下将SM周期和动态指令减少100倍，Tensor Core利用率提升至9.98倍。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽能生成高质量3D MRI，但由于每样本需数百次U-Net评估且内核行为高度异构，GPU资源消耗巨大，限制了其部署。

Method: 对Med-DDPM在V100、A100、H100三代架构上进行内核级运行时分解、指令混合、内存系统利用率、warp级活动和profiler优先级评分分析，并评估TF32 Tensor Core激活和3D channels-last布局两种架构感知优化。

Result: 优化后SM周期和动态指令减少100倍，Tensor Core利用率从1.45倍提升至9.98倍，IPC在A100上提升7%，且合成质量无损。

Conclusion: 通过架构感知优化可大幅缓解扩散模型的GPU资源瓶颈，为高保真3D MRI合成的实际部署提供了可行方案。
...
### [78] [Information Lattice Learning as Probabilistic Graphical Model Structure Learning](https://arxiv.org/abs/2606.19366)
*Haizi Yu,Lav R. Varshney*

Main category: cs.LG

TL;DR: 本文揭示了信息格学习（ILL）与概率图模型之间的深层联系：ILL学习到的规则等价于商变量的边际约束，其提升过程对应最大熵重建，可视为一种基于约束的因子图结构学习。


<details>
  <summary>Details</summary>
Motivation: 阐明ILL与概率图模型（PGM）之间的形式化关系，为ILL提供严格的概率解释，并厘清其在结构学习中的定位。

Method: 通过将ILL中的划分视为确定性商变量，规则视为该变量的边际分布，将ILL的通用提升解释为满足所有约束的联合分布族，特殊提升解释为基于L2均匀性（接近最大熵）的最大无知重建。在香农熵提升下，相同约束产生对数线性因子图，其因子由学习到的抽象层次索引。

Result: ILL学习到的规则集可解释为对可解释抽象变量的边际约束集合；ILL并非贝叶斯网络，其边编码的是抽象层次的细化与粗化而非条件依赖关系；ILL应被视为可解释约束因子图的结构学习方法。

Conclusion: ILL是结构学习的一种新范式，专注于商变量上的可解释约束因子图，这为推理、可辨识性以及混合符号-概率学习提供了新的方向。
...
### [79] [Weibull Weight-Scale Parameter Evolution under AdamW Training Dynamics](https://arxiv.org/abs/2606.19367)
*Tiexin Ding*

Main category: cs.LG

TL;DR: 本文基于双参数Weibull框架分析Transformer权重分布，发现AdamW训练中权重尺度参数λ经历增长、过冲和松弛三个阶段，并分解出对齐力、注入力和衰减力三种作用力。其中对齐力主导增长阶段（占88-94%），与衰减力达到平衡时转向松弛。进一步提出样条位移法从稀疏检查点恢复对齐力，精度达92-94%。


<details>
  <summary>Details</summary>
Motivation: 理解AdamW训练过程中Transformer权重分布的动态演化机制，特别是Weibull权重尺度参数λ为何会先增长、过冲然后松弛的现象。

Method: 在自训练的Pythia-70M模型上，基于AdamW更新推导出平方权重范数的三力分解（对齐力、注入力、衰减力）；针对真实模型提出样条位移法从稀疏检查点恢复对齐力。

Result: 对齐力在增长阶段贡献88-94%的绝对力预算；样条位移法恢复对齐力精度约92-94%，约为朴素两基点基线方法的两倍；RMS到Weibull重构偏移约5-6%；λ(t)峰值随训练数据一致性变化。

Conclusion: AdamW训练中权重尺度的演化可由三力分解机制解释，对齐力与衰减力的平衡决定了λ(t)从增长到松弛的转变，且对齐力可从稀疏检查点高效恢复，为理解大规模模型训练动力学提供了新的分析框架。
...
### [80] [Zero-Inflated Gaussian Distributions Enable Parameter-Space Sparsity in Estimation-of-Distribution Algorithms](https://arxiv.org/abs/2606.19369)
*Andreas Faust,Sven Nitzsche,Juergen Becker*

Main category: cs.LG

TL;DR: 本文提出基于多元零膨胀高斯分布的估计分布算法（ZIG-EDA），首次将EDA推广到稀疏黑盒优化场景，无需人工设计的稀疏化算子即可同时优化稀疏模式和活跃参数值。


<details>
  <summary>Details</summary>
Motivation: 传统EDA无法处理稀疏参数空间，而现有稀疏黑盒优化器不得不引入人工设计的稀疏化算子、双层优化方案或零阈值等方法，这些恰恰是EDA本应避免的偏见来源。

Method: 使用多元零膨胀高斯分布作为EDA的采样分布，通过潜在高斯模型将稀疏指标与连续值维度分离，捕获稀疏模式、活跃参数间的相关性及其交互；并提出了基于摊销逆变换的实用估计器。

Result: 该模型的潜在参数可从观测样本中识别；在Lunar Lander基准上，ZIG-EDA比密集高斯EDA、手工稀疏演化算法和临时稀疏EDA收敛更快、最终回报更高，且仅需少量活跃参数。

Conclusion: ZIG-EDA成功填补了EDA在稀疏黑盒优化领域的空白，实现了无需人工设计算子的端到端稀疏优化。
...
TL;DR: 本文提出一种结合少量人类演示数据（仅30分钟）与自博弈强化学习的方法，在保持低数据需求（仅为模仿学习的1/2500）的同时，训练出能与人类协调的驾驶策略，且仅需单张消费级GPU训练15小时。


<details>
  <summary>Details</summary>
Motivation: 纯自博弈强化学习训练的驾驶策略会产生与人类不兼容的"怪异"驾驶习惯，而以往通过奖励工程和域随机化来缓解这一问题的方法既脆弱又费时费力。

Method: 将少量人类演示数据作为正则化目标，叠加在最小安全到达目标的基础奖励之上，结合自博弈强化学习进行训练。

Result: 仅使用30分钟人类演示数据（比模仿学习少2500倍），在单张消费级GPU上15小时完成训练，学到的策略能与未见过的人类轨迹协调。

Conclusion: 少量人类数据就能有效引导自博弈强化学习策略向与人类兼容的方向发展，避免了大量数据采集和繁琐的奖励工程。
...
### [82] [ProMUSE: Progressive Multi-modal Uncertainty-guided Staged Evidential Alzheimer Disease Classification](https://arxiv.org/abs/2606.19371)
*Long Doan,Branden Chen,Ethan Litton,Huan Huang,Jiajing Huang,Yixin Xie,Weihua Zhou,Nandakumar Narayanan,Chen Zhao*

Main category: cs.LG

TL;DR: ProMUSE提出了一种渐进式多模态不确定度引导的分阶段证据网络，通过自适应决定何时需要额外模态数据，在减少MRI/PET使用量的同时保持诊断准确率，实现低成本、资源高效的阿尔茨海默病早期筛查。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病早期诊断依赖多模态数据（临床评估、MRI、PET），但MRI和PET成本高昂且不易获取，导致全模态推理在实际临床中难以推广，因此需要一种能减少昂贵影像使用但仍保持诊断准确性的方法。

Method: ProMUSE首先使用低成本临床数据进行证据分类，通过Dirichlet主观逻辑模型量化不确定性；当不确定性超过学习阈值时，逐步引入MRI或PET特征，并利用Dempster-Shafer理论融合模态间的信念和不确定性，获得校准的多模态预测。

Result: 在ADNI、AIBL和OASIS数据集上的CN-AD、CN-MCI和MCI-AD任务中，ProMUSE取得与全模态基线相当或更优的准确率，同时将MRI/PET使用量减少50-90%，显著节约成本。

Conclusion: ProMUSE是一种实用、不确定性感知且资源高效的AD筛查方案，能够在不牺牲诊断准确性的前提下大幅降低对昂贵影像数据的依赖，适用于真实临床工作流程。
...
### [83] [cAPM: Continual AI-Assisted Pace-Mapping with Active Learning](https://arxiv.org/abs/2606.19373)
...
TL;DR: 提出了一种名为cAPM的持续主动学习模型，用于室性心动过速起搏标测，通过任务无关的替代神经网络、主动学习和持续学习策略，实现跨目标的知识迁移，显著减少所需起搏点位数量。


<details>
  <summary>Details</summary>
Motivation: 现有主动学习AI模型每次针对不同室性心动过速靶点都需要重新训练，无法在同一患者或不同患者之间迁移知识，导致效率低下。

Method: 提出了cAPM框架，包含三个核心组件：(1) 任务无关的替代神经网络，学习从起搏位点到12导联ECG形态的映射；(2) 主动学习策略，选择信息量最大的起搏位点优化替代模型；(3) 持续学习策略，在顺序处理多个靶点时保留先前知识。

Result: 在计算机模拟测试中，cAPM使用4.5个起搏点位实现81%的定位成功率（5mm精度），而现有最优主动学习方法需要13.7个起搏点位，成功率仅38%。

Conclusion: cAPM通过知识迁移大幅减少了起搏标测所需数据量，为后续体内临床前和临床研究奠定了坚实基础。
...
### [84] [Protein Representation Learning with Secondary-Structure and Energy-Filtered Hydrogen-Bond Graphs](https://arxiv.org/abs/2606.19374)
*Mohamed Mouhajir,Limei Wang,El Houcine Bergou,Hajar El Hammouti,Lamiae Azizi,Dongqi Fu*

Main category: cs.LG

TL;DR: 提出了一种基于二级结构感知的图神经网络（SSProNet），通过引入二级结构赋值和能量过滤的氢键拓扑来构建图边，用于蛋白质表示学习，在多个基准测试中取得一致提升并增强生物学可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的蛋白质建模方法主要依赖序列邻接或几何邻近，未能充分反映蛋白质折叠的核心原理（如二级结构元件和氢键相互作用）。

Method: 设计二级结构感知的图神经网络：残基层面的节点表示融入二级结构赋值，图边基于能量过滤的氢键相互作用构建，从而捕获局部结构上下文和长程耦合。

Result: 在常用蛋白质基准测试上，该方法相较于现有基于图的方法取得了一致的性能提升，且学到的图连接与经典结构基序对齐，增强了生物学可解释性。

Conclusion: 融入二级结构和能量过滤的氢键拓扑为蛋白质表示学习提供了有效的归纳偏置，SSProNet在该任务上具有优越性和可解释性。
...
### [85] [Physics-Informed Discovery of Yield Functions in Plasticity via Convex Neural Representations](https://arxiv.org/abs/2606.19375)
*Hyeonbin Moon,Donghyuk Cho,Jecheon Yu,Jeong Whan Yoon,Seunghwa Ryu*

Main category: cs.LG

TL;DR: 本文提出了一种物理信息驱动的框架，能够从全场位移和反力数据中自动发现各向异性屈服函数，无需应力观测、塑性应变测量或预设参数形式，通过凸神经网络表示屈服函数并结合可微应力更新与力平衡损失进行训练。


<details>
  <summary>Details</summary>
Motivation: 识别各向异性屈服函数面临三大挑战：屈服无法在全场力学测量中直接观测、方向性校准需要大量加载方向、选择合适的解析形式非常困难。

Method: 采用凸神经网络表示屈服函数，施加凸性和一阶正齐次性约束，并假设拉压对称；将屈服函数嵌入可微的弹塑性应力更新中，通过物理信息驱动的力平衡损失在多加载工况下训练网络。

Result: 通过von Mises、Hill 1948和Yld2000-2d屈服函数的有限元基准研究验证了框架的有效性，评估了屈服轮廓一致性、位移噪声敏感性、塑性活跃应力状态的可识别性、认知不确定性及多项式代理部署。

Conclusion: 该研究提供了一条力学约束的途径，能够仅从位移和力数据中自动发现各向异性屈服函数，并将识别结果保持在弹塑性应力积分的结构框架内。
...
### [86] [Cost-Optimal LLM Routing with Limited User Feedback under User Satisfaction Guarantees](https://arxiv.org/abs/2606.19376)
*Herbert Woisetschläger,Arastun Mammadli,Ryan Zhang,Shiqiang Wang*

Main category: cs.LG

TL;DR: SLARouter是一种在线路由算法，能在仅依赖稀疏、单侧用户反馈的情况下，学习成本最优策略，同时提供严格的服务质量（SLA）保障和成本最优性理论保证，实验表明其无需逐基准调优即可满足SLA约束，相比现有方案降低最多2.2倍运营成本。


<details>
  <summary>Details</summary>
Motivation: LLM应用推理成本快速增长，用户对高质量响应的期望与商业环境中严格SLA约束之间存在成本与质量的根本矛盾，现有路由方法依赖完整反馈信号、离线训练、大量逐工作负载调优，且大多缺乏SLA保障和推理时自适应能力。

Method: 提出SLARouter在线路由算法，利用生产系统中稀疏、单侧的用户反馈信号，学习成本最优的路由策略，并具备成本最优性和严格SLA合规性的理论保证。

Result: 在多种LLM基准测试上，SLARouter无需逐基准调优即可满足SLA约束，相比现有基线方案降低最多2.2倍运营成本。

Conclusion: SLARouter有效解决了LLM推理中成本与SLA质量保障的权衡问题，通过在线学习和理论保证实现了成本最优的请求路由。
...
### [101] [Algebraic Dead Directions in LayerNorm Transformers: A Forward-Pass-Only Diagnostic at LLM Scale](https://arxiv.org/abs/2606.19491)
...
### [102] [Calibrating Generative Models to Feature Distributions with MMD Finetuning](https://arxiv.org/abs/2606.19496)
...
### [103] [Interactive Pareto navigation for deep multi-task learning](https://arxiv.org/abs/2606.19521)
...
### [104] [Techniques for Peak Memory Reduction for LoRA Fine-tuning of LLMs on Edge Devices](https://arxiv.org/abs/2606.19528)
...
### [105] [Tracking Representation Dynamics in Large Language Models with Persistent Homology](https://arxiv.org/abs/2606.19542)
...
### [106] [Predicting Mergeability of Parameter-Efficient Fine-Tuning Updates](https://arxiv.org/abs/2606.19549)
...
### [107] [Displacement Is Not Direction: Evaluating Fidelity Metrics for Quantized LLM Deployment](https://arxiv.org/abs/2606.19558)
...
### [108] [Understanding Key Features of Time Series Foundation Models from Epidemic Forecasting](https://arxiv.org/abs/2606.19560)
...
### [109] [Advances in Scientific Machine Learning for Coupled Fluid Flow and Transport](https://arxiv.org/abs/2606.19562)
...
### [110] [On the QUEST for Uncertainty Quantification via Highest Density Regions](https://arxiv.org/abs/2606.19569)
...
### [111] [Unsupervised Causal Abstractions Discovery](https://arxiv.org/abs/2606.19594)
...
### [112] [IHBench: Evaluating Post-Interruption Recovery in Voice Agents with Structured Workflows](https://arxiv.org/abs/2606.19595)
...
### [113] [Comparing Linear Probes with Mahalanobis Cosine Similarity](https://arxiv.org/abs/2606.19603)
...
### [114] [Latent Confounded Causal Discovery via Lie Bracket Geometry](https://arxiv.org/abs/2606.19610)
*Sridhar Mahadevan*
...
### [115] [SEAGAN: domain-Specific and Edge-Aware Graph Attention Network for Dynamic Plant Processes](https://arxiv.org/abs/2606.19623)
...
### [116] [MassSpecGym in the Wild: Uncovering and Correcting Evaluation Pitfalls in AI-Driven Molecule Discovery](https://arxiv.org/abs/2606.19624)
...
### [117] [Hard or Just Unreached? Diagnosing the Sampling Blind Spot in Math-Reasoning Difficulty Estimation](https://arxiv.org/abs/2606.19636)
...
### [118] [Convex training of Lipschitz-regularized shallow neural networks](https://arxiv.org/abs/2606.19652)
...
### [119] [LOKI: Memory-Free Null-Space Constrained Lifelong Knowledge Editing](https://arxiv.org/abs/2606.19679)
...
### [120] [Multi-Granular Attention-Driven Reinforcement Learning Framework for Web Intelligent Enhancement Systems](https://arxiv.org/abs/2606.19690)
...
### [121] [Efficiently Representing Algorithms With Chain-of-Thought Transformers](https://arxiv.org/abs/2606.19697)
...
### [122] [Efficient Neural Network Model Selection for Few-Class Application Datasets](https://arxiv.org/abs/2606.19712)
...
### [123] [OnDeFog: Online Decision Transformer under Frame Dropping](https://arxiv.org/abs/2606.19721)
...
### [124] [Federated Bilevel Performative Prediction](https://arxiv.org/abs/2606.19734)
...
### [125] [Manifold Bandits: Bayesian Curriculum Learning over the Latent Geometry of Large Language Models](https://arxiv.org/abs/2606.19750)
...
### [126] [Learning universal approximations for partial differential equations with Physics-Informed Broad Learning System](https://arxiv.org/abs/2606.19754)
...
### [127] [An Information Theoretic Framework for Graph Novelty Generation via Latent Mixture Modeling](https://arxiv.org/abs/2606.19770)
...
### [128] [Flow Map Denoisers: Traversing the Distortion-Perception Plane for Inverse Problems](https://arxiv.org/abs/2606.19802)
...
### [129] [Uncertainty-Aware Reward Modeling for Stable RLHF](https://arxiv.org/abs/2606.19818)
...
### [130] [Enhancing Graph Neural Networks Using Proximity Graphs for Dust Source Emission Forecasting](https://arxiv.org/abs/2606.19825)
...
### [131] [When, Where, and How: Adaptive Binning for Tabular Self-Supervised Learning](https://arxiv.org/abs/2606.19827)
...
### [132] [Neural Additive and Basis Models with Feature Selection and Interactions](https://arxiv.org/abs/2606.19850)
...
### [133] [Physics-Informed Neural Network with Squeeze-Excitation-like Attention](https://arxiv.org/abs/2606.19853)
...
### [134] [Global Convergence of Gradient Descent for Score Matching in Gaussian Mixtures via Reverse Fisher Divergence](https://arxiv.org/abs/2606.19876)
*Alexander Tyurin*
...
### [135] [On the Oracle Complexity of Interpolation-Based Gradient Descent](https://arxiv.org/abs/2606.19878)
...
### [136] [Matching Markets meet Cumulative Prospect Theory: Towards Optimal and Adversarially Robust Learning](https://arxiv.org/abs/2606.19883)
...
### [137] [SL-S4Wave: Self-Supervised Learning of Physiological Waveforms with Structured State Space Models](https://arxiv.org/abs/2606.19888)
...
### [138] [Adversarial Bandit Optimization with Globally Bounded Perturbations to Convex Losses](https://arxiv.org/abs/2606.19891)
...
### [139] [Score Approximation for Diffusion Models on Arbitrary Low-Dimensional Structures](https://arxiv.org/abs/2606.19894)
...
### [140] [ADaPT: Token-Level Decoupling for Efficient Large Reasoning Models](https://arxiv.org/abs/2606.19919)
...
### [141] [Compositionality Emerges in a Narrow Depth-Connectivity Regime: Architecture Constraints and Solution Manifolds](https://arxiv.org/abs/2606.19941)
...
### [143] [Low-Energy Reduced RISC-V Instruction Subset Processor for Tsetlin Machine Inference at the Edge](https://arxiv.org/abs/2606.19964)
...
### [144] [Kolmogorov-Arnold Reservoir Computing](https://arxiv.org/abs/2606.19984)
...
### [146] [Connect the Dots: Training LLMs for Long-Lifecycle Agents with Cross-Domain Generalization Via Reinforcement Learning](https://arxiv.org/abs/2606.20002)
...
### [156] [What Makes Effective Supervision in Latent Chain-of-Thought: An Information-Theoretic Analysis](https://arxiv.org/abs/2606.20075)
...
### [157] [Sensorimotor World Models: Perception for Action via Inverse Dynamics](https://arxiv.org/abs/2606.20104)
...
### [158] [Quantile of Means: A Bonus-Free Ensemble Method for Minimax Optimal Reinforcement Learning](https://arxiv.org/abs/2606.20107)
...
### [159] [When Calibration Fails the Vulnerable Hospital: Federated Conformal Risk Control via Risk-Curve Shrinkage](https://arxiv.org/abs/2606.20115)
*Nafis Fuad Shahid*
...
### [160] [Multi-Modal Contrastive Learning for Implicit Earth Embeddings via Location Tying](https://arxiv.org/abs/2606.20167)
...
### [161] [Predicting gestational age at birth in the context of preterm birth from multi-modal fetal MRI](https://arxiv.org/abs/2606.20172)
...
### [162] [Computational Methods and Challenges in Cell-Free DNA Analysis for Multi-Cancer Early Detection](https://arxiv.org/abs/2606.20174)
...
### [163] [Effective Dimension Governs Generalization in Quantum Kernel Vision Models](https://arxiv.org/abs/2606.20183)
...
### [164] [Learner-based Concept Drift Detection: Analysis and Evaluation](https://arxiv.org/abs/2606.20216)
...
### [165] [Boundary Embedding Shaping with Adaptive Contrastive Learning for Graph Structural Disentanglement](https://arxiv.org/abs/2606.20283)
...
### [166] [Integrating national forest inventory, airborne lidar, and satellite imagery for wall-to-wall mapping of forest structure with computer vision](https://arxiv.org/abs/2606.20291)
...
### [167] [Shifting-based Optimizable Linear Relaxations for General Activation Functions](https://arxiv.org/abs/2606.20292)
...
### [168] [Recurrent neural networks approximate continuous functions](https://arxiv.org/abs/2606.20325)
...
### [169] [Quantum-classical physics-informed Kolmogorov-Arnold networks for PDEs](https://arxiv.org/abs/2606.20326)
...
### [170] [Constrained hybrid modelling to predict microbial dynamics and organic matter turnover in soil systems](https://arxiv.org/abs/2606.20329)
...
### [171] [Critical Percolation as a Synthetic Data Model for Interpretability](https://arxiv.org/abs/2606.20347)
...
### [172] [On the Variance of Temporal Difference Learning and its Reduction Using Control Variates](https://arxiv.org/abs/2606.20357)
...
### [173] [Train, Retrieve, or Both? A Four-Arm Head-to-Head for Correct Statutory Citation on the Ontario Residential Tenancies Act](https://arxiv.org/abs/2606.20359)
...
### [174] [Judging to Improve: A De-biased VLM-as-3D-Judge Protocol for Single-Image 3D Generation](https://arxiv.org/abs/2606.20364)
*Ali Asaria,Tony Salomone,Deep Gandhi*
...
### [175] [CRAX: Fast Safe Reinforcement Learning Benchmarking](https://arxiv.org/abs/2606.20376)
...
### [176] [UltraQuant: 4-bit KV Caching for Context-Heavy Agents](https://arxiv.org/abs/2606.20474)
...
### [178] [Contagion Networks: Evaluator Bias Propagation in Multi-Agent LLM Systems](https://arxiv.org/abs/2606.20493)
*Zewen Liu*
...
### [181] [Direct Advantage Estimation for Scalable and Sample-efficient Deep Reinforcement Learning](https://arxiv.org/abs/2606.20411)
*Hsiao-Ru Pan,Bernhard Schölkopf*
...
### [182] [Pseudo-Feature Padding: A Lightweight Defense Against False Data Injection in Power Grids](https://arxiv.org/abs/2606.20415)
...
### [183] [On the Redundancy of Timestep Embeddings in Diffusion Models](https://arxiv.org/abs/2606.20416)
*José A. Chávez*
...
### [184] [Neural network surrogates with uncertainty quantification for inverse problems in partial differential equations](https://arxiv.org/abs/2606.20417)
