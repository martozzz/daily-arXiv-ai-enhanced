...
TL;DR: 通过双流处理、逐层监督和门控注意力增强Transformer模块化，使其从抵抗外科手术式控制转变为可预测可控的系统。


<details>
  <summary>Details</summary>
Motivation: Transformer模型存在分布式冗余，导致即使移除被认为关键的注意力头也仅产生最小行为变化，这种Hydra效应使得可解释性工具虽能识别组件相关性但无法预测或控制其因果作用。

Method: 结合三种架构干预：双流处理分离标记和上下文表示、逐层监督提供各层独立的梯度信号、门控注意力正则化以形成离散激活模式。训练时使用逐层监督以暴露隐藏的模块化结构。

Result: 使用逐层监督训练的模型相比架构相同但使用标准目标训练的模型，产生5到23倍更大的消融效应，并获得4倍的目标行为控制能力。消融效应标准差从0.63%增加到6.32%，表明预测与特定电路的依赖关系被揭示。所提方法能实现功能重组，不同任务通过不同注意力头进行路由。

Conclusion: 通过双流处理、逐层监督和门控注意力的架构干预能够揭示并增强Transformer模型的模块化结构，从而实现从被动观察到主动控制的能力。
...
TL;DR: 本文提出了一种方法，不仅能检测数据分布的异常变化，还能区分故障与正常领域迁移，通过改进的Page-Hinkley变化点检测器和监督式领域适应算法，结合XAI组件帮助操作人员最终区分这两种情况。


<details>
  <summary>Details</summary>
Motivation: 在许多实际应用中，数据分布的变化并不总代表系统异常状态（如开始处理新产品时的正常领域迁移），但现有方法会错误地将这些变化识别为故障，因此需要能够区分故障与正常领域迁移的方法以确保系统实际鲁棒性。

Method: 1) 改进的Page-Hinkley变化点检测器识别领域迁移和可能的故障；2) 基于监督域适应的算法进行快速在线异常检测；3) 可解释人工智能(XAI)组件帮助人类操作员最终区分领域迁移和故障。

Result: 方法在钢铁工厂的数据流实验中进行了验证，展示了其在识别数据分布变化、检测异常以及区分故障和正常领域迁移方面的有效性。

Conclusion: 提出的方法能够有效区分系统运行中的故障与正常领域迁移，通过结合变化点检测、监督域适应和XAI组件，提高了异常检测系统的实用性和鲁棒性，减少误报并帮助操作人员做出正确决策。
...
Conclusion: 该方法提供了一种有效解决多变量地质统计模拟中复杂分布保持问题的新途径，优于传统线性假设方法。
...
TL;DR: METHOD是一个零知识证明系统，可验证用户从LLM API获得的输出是否真正来自特定模型，通过分层证明结构和查找表近似实现高效验证且不影响模型质量。


<details>
  <summary>Details</summary>
Motivation: 当前用户在调用专有LLM API时，无法确认实际使用的是否为声称的模型，服务提供商可能替换为廉价模型、应用过度量化或返回缓存结果，而用户却在为前沿能力支付高价，需要密码学验证机制确保模型使用的真实性。

Method: 提出了一种分层零知识证明框架，将Transformer推理分解为独立的层级计算；开发了针对非算术操作（softmax、GELU、LayerNorm）的查找表近似方法，实现零精度损失；引入基于费雪信息的验证机制以应对实际部署场景。

Result: 在d=128的Transformer模型上，METHOD生成的层级证明大小为5.5KB（注意力2.1KB+MLP 3.5KB），验证时间24毫秒；相比EZKL，证明大小减小70倍，证明时间加快5.7倍；保持形式化安全保证（ε < 1e-37）；查找表近似完全保持模型困惑度。

Conclusion: METHOD作为一种基于零知识证明的可验证LLM推理系统，通过层级证明结构和查找表近似等技术，成功解决了LLM API调用中模型真实使用的信任问题，在不牺牲模型性能的前提下提供了高效、安全的密码学验证保障。
...
### [7] [Fundamental Limits of Neural Network Sparsification: Evidence from Catastrophic Interpretability Collapse](https://arxiv.org/abs/2603.18056)
...
### [8] [Lightweight Adaptation for LLM-based Technical Service Agent: Latent Logic Augmentation and Robust Noise Reduction](https://arxiv.org/abs/2603.18074)
...
TL;DR: SLEA-RL框架通过在每一步基于当前观察动态检索相关经验来改进大语言模型智能体在多回合任务中的决策能力，相比静态经验检索方法性能更优。


<details>
  <summary>Details</summary>
Motivation: 现有基于经验增强的方法在多回合工具使用任务中仅根据初始任务描述检索一次经验，并在整个回合中保持不变。然而在多回合设置中，观察结果每一步都在变化，静态检索会随回合进展变得越来越不匹配，导致性能下降。

Method: SLEA-RRL包含三个核心组件：1）步级观察聚类，将结构等价的环境状态分组以实现高效检索；2）自演化经验库，通过基于评分的准入和速率限制提取来提炼成功策略和失败模式；3）带有步级信用分配的强化学习优化，用于在多回合回合中进行精细化的优势估计。经验库通过语义分析而非梯度更新与策略共同演化。

Result: 在长视野多回合智能体基准测试中，SLEA-RL相比各种强化学习基线方法取得了更优的性能表现。

Conclusion: 本研究提出了一种步级经验增强强化学习框架SLEA-RL，通过动态检索相关经验来提升大语言模型智能体在多回合工具使用任务中的表现。该框架在多个长视野多回合智能体基准测试中显著优于现有强化学习方法，证明了动态经验检索对于解决多回合任务中观察变化问题的重要性。
...
### [10] [Probabilistic Federated Learning on Uncertain and Heterogeneous Data with Model Personalization](https://arxiv.org/abs/2603.18083)
...
TL;DR: 提出了动态约束机制解决RL微调中约束与优化的冲突，通过参考模型的在线精炼实现自适应约束，实验显示其在对话和代码生成中优于KL正则化和无约束基线。


<details>
  <summary>Details</summary>
Motivation: 传统约束在稳定强化学习微调（RFT）和防止退化输出时与优化目标存在固有冲突：强约束会限制模型寻找更优解的能力。

Method: 提出动态约束机制，使用参考模型作为在线精炼器：该精炼器接受微调模型的输出，生成最小化修正版本（保留正确内容原样，仅修复错误）。然后通过监督微调损失训练微调模型产生精炼输出，实现约束力度的自动调节。

Result: 在对话和代码生成任务上的实验表明，动态约束显著优于KL正则化和无约束基线，在保持训练稳定性的同时实现了更高的任务奖励。

Conclusion: 动态约束机制通过基于参考模型的在线精炼器实现约束力度的自适应调整，有效解决了RFT中约束与优化目标之间的冲突，显著提升了任务性能并保持了训练稳定性。
...
### [12] [ARTEMIS: A Neuro Symbolic Framework for Economically Constrained Market Dynamics](https://arxiv.org/abs/2603.18107)
*Rahul D Ray*
...
### [13] [Tula: Optimizing Time, Cost, and Generalization in Distributed Large-Batch Training](https://arxiv.org/abs/2603.18112)
...
### [22] [Enactor: From Traffic Simulators to Surrogate World Models](https://arxiv.org/abs/2603.18266)
...
### [26] [ALIGN: Adversarial Learning for Generalizable Speech Neuroprosthesis](https://arxiv.org/abs/2603.18299)
...
### [29] [A Family of Adaptive Activation Functions for Mitigating Failure Modes in Physics-Informed Neural Networks](https://arxiv.org/abs/2603.18328)
*Krishna Murari*
...
### [30] [Epistemic Generative Adversarial Networks](https://arxiv.org/abs/2603.18348)
...
### [31] [Mathematical Foundations of Deep Learning](https://arxiv.org/abs/2603.18387)
*Xiaojing Ye*
...
### [32] [RE-SAC: Disentangling aleatoric and epistemic risks in bus fleet control: A stable and robust ensemble DRL approach](https://arxiv.org/abs/2603.18396)
...
### [42] [Data-efficient pre-training by scaling synthetic megadocs](https://arxiv.org/abs/2603.18534)
...
### [43] [Beyond Passive Aggregation: Active Auditing and Topology-Aware Defense in Decentralized Federated Learning](https://arxiv.org/abs/2603.18538)
...
### [44] [GAPSL: A Gradient-Aligned Parallel Split Learning on Heterogeneous Data](https://arxiv.org/abs/2603.18540)
...
### [45] [HEP Statistical Inference for UAV Fault Detection: CLs, LRT, and SBI Applied to Blade Damage](https://arxiv.org/abs/2603.18546)
*Khushiyant*
...
### [46] [SINDy-KANs: Sparse identification of non-linear dynamics through Kolmogorov-Arnold networks](https://arxiv.org/abs/2603.18548)
...
### [47] [Transformers Learn Robust In-Context Regression under Distributional Uncertainty](https://arxiv.org/abs/2603.18564)
...
### [48] [SpecForge: A Flexible and Efficient Open-Source Training Framework for Speculative Decoding](https://arxiv.org/abs/2603.18567)
...
### [49] [Attack by Unlearning: Unlearning-Induced Adversarial Attacks on Graph Neural Networks](https://arxiv.org/abs/2603.18570)
...
### [50] [Elastic Weight Consolidation Done Right for Continual Learning](https://arxiv.org/abs/2603.18596)
...
### [51] [Evaluating Model-Free Policy Optimization in Masked-Action Environments via an Exact Blackjack Oracle](https://arxiv.org/abs/2603.18642)
*Kevin Song*
...
### [52] [Enhancing Multi-Corpus Training in SSL-Based Anti-Spoofing Models: Domain-Invariant Feature Extraction](https://arxiv.org/abs/2603.18657)
...
### [53] [Revisiting Label Inference Attacks in Vertical Federated Learning: Why They Are Vulnerable and How to Defend](https://arxiv.org/abs/2603.18680)
...
### [54] [HISR: Hindsight Information Modulated Segmental Process Rewards For Multi-turn Agentic Reinforcement Learning](https://arxiv.org/abs/2603.18683)
...
### [55] [STEP: Scientific Time-Series Encoder Pretraining via Cross-Domain Distillation](https://arxiv.org/abs/2603.18688)
...
### [56] [OCP: Orthogonal Constrained Projection for Sparse Scaling in Industrial Commodity Recommendation](https://arxiv.org/abs/2603.18697)
...
### [57] [Off-Policy Learning with Limited Supply](https://arxiv.org/abs/2603.18702)
...
### [58] [From ex(p) to poly: Gaussian Splatting with Polynomial Kernels](https://arxiv.org/abs/2603.18707)
...
### [59] [CausalRM: Causal-Theoretic Reward Modeling for RLHF from Observational User Feedbacks](https://arxiv.org/abs/2603.18736)
...
### [60] [Are complicated loss functions necessary for teaching LLMs to reason?](https://arxiv.org/abs/2603.18756)
...
### [61] [Automatic Configuration of LLM Post-Training Pipelines](https://arxiv.org/abs/2603.18773)
...
### [62] [Signals of Success and Struggle: Early Prediction and Physiological Signatures of Human Performance across Task Complexity](https://arxiv.org/abs/2603.18798)
...
### [63] [Seasoning Generative Models for a Generalization Aftertaste](https://arxiv.org/abs/2603.18817)
...
### [64] [A Model Ensemble-Based Post-Processing Framework for Fairness-Aware Prediction](https://arxiv.org/abs/2603.18838)
...
### [65] [DriftGuard: Mitigating Asynchronous Data Drift in Federated Learning](https://arxiv.org/abs/2603.18872)
...
### [66] [Authority-Level Priors: An Under-Specified Constraint in Hierarchical Predictive Processing](https://arxiv.org/abs/2603.18888)
*Marcela Palejova*
...
### [68] [Neural Galerkin Normalizing Flow for Transition Probability Density Functions of Diffusion Models](https://arxiv.org/abs/2603.18907)
...
### [69] [An Optimised Greedy-Weighted Ensemble Framework for Financial Loan Default Prediction](https://arxiv.org/abs/2603.18927)
...
### [70] [Context Bootstrapped Reinforcement Learning](https://arxiv.org/abs/2603.18953)
...
### [71] [BVSIMC: Bayesian Variable Selection-Guided Inductive Matrix Completion for Improved and Interpretable Drug Discovery](https://arxiv.org/abs/2603.18957)
...
### [72] [Maximum-Entropy Exploration with Future State-Action Visitation Measures](https://arxiv.org/abs/2603.18965)
...
### [73] [Best-of-Both-Worlds Multi-Dueling Bandits: Unified Algorithms for Stochastic and Adversarial Preferences under Condorcet and Borda Objectives](https://arxiv.org/abs/2603.18972)
...
### [74] [Book your room in the Turing Hotel! A symmetric and distributed Turing Test with multiple AIs and humans](https://arxiv.org/abs/2603.18981)
...
### [75] [Foundations of Schrödinger Bridges for Generative Modeling](https://arxiv.org/abs/2603.18992)
*Sophia Tang*
...
### [76] [AgentDS Technical Report: Benchmarking the Future of Human-AI Collaboration in Domain-Specific Data Science](https://arxiv.org/abs/2603.19005)
...
### [79] [Position: Spectral GNNs Are Neither Spectral Nor Superior for Node Classification](https://arxiv.org/abs/2603.19091)
...
### [83] [Hierarchical Latent Structure Learning through Online Inference](https://arxiv.org/abs/2603.19139)
...
### [84] [SHAPCA: Consistent and Interpretable Explanations for Machine Learning Models on Spectroscopy Data](https://arxiv.org/abs/2603.19141)
...
### [85] [Rigorous Error Certification for Neural PDE Solvers: From Empirical Residuals to Solution Guarantees](https://arxiv.org/abs/2603.19165)
...
### [86] [DyMoE: Dynamic Expert Orchestration with Mixed-Precision Quantization for Efficient MoE Inference on Edge](https://arxiv.org/abs/2603.19172)
...
### [87] [SOL-ExecBench: Speed-of-Light Benchmarking for Real-World GPU Kernels Against Hardware Limits](https://arxiv.org/abs/2603.19173)
...
### [88] [MIDST Challenge at SaTML 2025: Membership Inference over Diffusion-models-based Synthetic Tabular data](https://arxiv.org/abs/2603.19185)
...
### [89] [Improving RCT-Based Treatment Effect Estimation Under Covariate Mismatch via Calibrated Alignment](https://arxiv.org/abs/2603.19186)
...
### [90] [Robustness, Cost, and Attack-Surface Concentration in Phishing Detection](https://arxiv.org/abs/2603.19204)
...
### [91] [Online Learning and Equilibrium Computation with Ranking Feedback](https://arxiv.org/abs/2603.19221)
...
### [92] [DEAF: A Benchmark for Diagnostic Evaluation of Acoustic Faithfulness in Audio Language Models](https://arxiv.org/abs/2603.18048)
...
### [94] [Multi-Trait Subspace Steering to Reveal the Dark Side of Human-AI Interaction](https://arxiv.org/abs/2603.18085)
...
### [95] [Efficient Dense Crowd Trajectory Prediction Via Dynamic Clustering](https://arxiv.org/abs/2603.18166)
*Antonius Bima Murti Wijaya,Paul Henderson,Marwa Mahmoud*

Main category: cs.AI

TL;DR: 提出集群化轨迹预测方法，通过分组相似个体减少计算负担，在密集人群场景中保持精度同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法主要针对稀疏场景，忽视密集人群场景中由追踪结果大量、嘈杂且不准确带来的自动化挑战，导致高计算成本，需要更高效的处理方案。

Method: 提出一种基于集群的插拔式方法，通过将具有相似属性的个体分组来减少处理复杂度，用集群质心替代原有行人输入，从而与现有轨迹预测方法兼容。

Result: 在多个挑战性密集人群场景上的评估显示，该方法相比先进方法能实现更快处理速度和更低内存使用，同时保持预测精度。

Conclusion: 该研究提出的集群化方法能够在保持精度的同时，显著提升密集人群场景下轨迹预测的计算效率，为实现高效人群管理系统提供了实用解决方案。
...
### [96] [TeachingCoach: A Fine-Tuned Scaffolding Chatbot for Instructional Guidance to Instructors](https://arxiv.org/abs/2603.18189)
*Isabel Molnar,Peiyu Li,Si Chen,Sugana Chawla,James Lang,Ronald Metoyer,Ting Hua,Nitesh V. Chawla*

Main category: cs.AI

TL;DR: 该论文介绍了TeachingCoach，一个面向高等教育教师专业发展的教学聊天机器人，通过数据驱动的管道从教育资源中提取教学规则，利用合成对话生成微调专用语言模型，为教师提供实时对话式指导。


<details>
  <summary>Details</summary>
Motivation: 当前高等教育教师缺乏及时且具有教学理论基础的规模化指导支持，现有工具多依赖于通用聊天机器人建议或难以规模化的教学中心人工咨询。

Method: 基于数据中心的管道从教育资源中提取教学规则，通过合成对话生成微调专用语言模型，提供问题识别、诊断和策略发展的系统化指导框架。

Result: 专家评估显示TeachingCoach比GPT-4o mini基准产生更清晰、更具反思性和响应性的指导；用户研究发现对话深度与交互效率之间存在权衡关系。

Conclusion: 教学理论基础+合成数据驱动的聊天机器人能够改善教学支持，为未来教学聊天机器人系统提供了可扩展的设计方法。
...
### [97] [Access Controlled Website Interaction for Agentic AI with Delegated Critical Tasks](https://arxiv.org/abs/2603.18197)
...
### [98] [A Computationally Efficient Learning of Artificial Intelligence System Reliability Considering Error Propagation](https://arxiv.org/abs/2603.18201)
*Fenglian Pan,Yinwei Zhang,Yili Hong,Larry Head,Jian Liu*

Main category: cs.AI

TL;DR: 针对AI系统跨阶段错误传播的可靠性建模难题，本文开发了一种基于物理仿真和复合似然期望极大化算法的建模框架，应用于自动驾驶感知系统并验证了其准确性和效率。


<details>
  <summary>Details</summary>
Motivation: AI系统在新兴智慧城市中日益重要，但其可靠性仍是关键问题。这类系统通常通过一系列相互连接的功能阶段运行，上游错误可能传播到下游阶段，最终影响整体系统可靠性。现有方法面临三大挑战：1）数据可用性问题：真实世界AI系统可靠性数据稀缺且受隐私限制；2）模型有效性问题：跨顺序阶段的重复错误事件相互依赖，违反统计推断的独立性假设；3）计算复杂性问题：AI系统处理大量高速数据，导致频繁且复杂的重复错误事件难以追踪分析。

Method: 论文首先利用基于物理的自动驾驶仿真平台，配合可解释的错误注入器生成高质量AI系统可靠性数据。基于此数据，开发了新的可靠性建模框架，明确描述跨阶段的错误传播。模型参数估计采用理论上有保证的复合似然期望-极大化算法（Composite Likelihood Expectation-Maximization Algorithm），实现计算效率与理论保证的平衡。

Result: 提出的建模框架应用于自动驾驶感知系统的可靠性建模，实验结果表明该方法具有较高的预测准确性和计算效率，能够有效模拟和预测跨阶段的错误传播行为。

Conclusion: 本文提出的基于物理仿真的可靠性建模框架为复杂AI系统提供了一种有效且计算高效的方法来量化错误传播。通过将可解释的错误注入与复合似然期望极大化算法相结合，解决了数据稀缺、模型有效性和计算复杂性三大挑战。该方法可推广应用于其他依赖多阶段处理的AI系统可靠性分析。
...
### [99] [Retrieval-Augmented LLM Agents: Learning to Learn from Experience](https://arxiv.org/abs/2603.18272)
*Thomas Palmeira Ferraz,Romain Deffayet,Vassilina Nikoulina,Hervé Déjean,Stéphane Clinchant*

Main category: cs.AI

TL;DR: 通过结合监督微调与经验检索，提出了一种能有效利用检索轨迹改善大语言模型智能体泛化能力的方法，在未见任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在构建通用智能体方面取得进展，但如何实现对新任务的稳健泛化仍存在挑战。现有方法（微调或无训练的经验检索增强生成）各有局限：微调难以泛化到新任务，而经验检索通常不及监督基线。

Method: 首先建立了基于LoRA的鲁棒监督微调方法；其次系统分析了经验检索的关键设计选择，包括存储、查询和轨迹选择的最佳策略；最后提出了将经验检索集成到微调流程中的完整管道。

Result: 综合方法显著提升了在未见任务上的泛化性能，基于LoRA的监督微调方法优于多个最先进的智能体训练流程，确定了经验检索的关键设计策略。

Conclusion: 结合监督微调与经验检索的综合方法为构建能够从经验中学习的智能体提供了可扩展且有效的框架，显著提升了在未见任务上的泛化能力。
...
### [100] [EDM-ARS: A Domain-Specific Multi-Agent System for Automated Educational Data Mining Research](https://arxiv.org/abs/2603.18273)
...
### [101] [CORE: Robust Out-of-Distribution Detection via Confidence and Orthogonal Residual Scoring](https://arxiv.org/abs/2603.18290)
...
### [102] [The Validity Gap in Health AI Evaluation: A Cross-Sectional Analysis of Benchmark Composition](https://arxiv.org/abs/2603.18294)
*Alvin Rajkomar,Pavan Sudarshan,Angela Lai,Lily Peng*

Main category: cs.AI

TL;DR: 本研究通过分析18,707条消费者健康查询，揭示了健康相关大语言模型（LLMs）评估基准存在结构性“有效性差距”，即基准缺乏真实临床需求的代表性，尤其在原始临床数据、脆弱人群覆盖和慢性管理场景方面存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 当前用于验证健康相关大语言模型的基准缺乏对“患者”或“查询”人群的明确定义，导致总体性能指标可能无法准确反映模型在临床应用的准备就绪程度，需要系统性地分析基准的内容构成以评估其与真实世界临床需求的对齐情况。

Method: 使用大语言模型作为自动编码工具，对18,707条消费者健康查询（来自六个公共基准）应用标准化的16字段分类法，剖析查询的上下文、主题和意图。

Result: 发现基准与临床需求的严重错位：尽管42%的语料包含客观数据，但主要集中在健康可穿戴设备信号（17.7%）；复杂诊断输入罕见，包括实验室值（5.2%）、影像（3.8%）和原始医疗记录（0.6%）。安全关键场景基本缺失：自杀/自残查询占<0.7%，慢性疾病管理仅5.5%。基准也忽视了脆弱人群（儿科/老年人<11%）和全球健康需求。

Conclusion: 评估基准仍然与真实世界临床需求错位，缺乏原始临床资料、对脆弱人群的充分代表性以及纵向慢性护理场景。该领域需要采用标准化的查询分析——类似于临床试验报告——以使评估与临床实践的完整复杂性保持一致。
...
### [103] [Consumer-to-Clinical Language Shifts in Ambient AI Draft Notes and Clinician-Finalized Documentation: A Multi-level Analysis](https://arxiv.org/abs/2603.18327)
...
### [104] [FaithSteer-BENCH: A Deployment-Aligned Stress-Testing Benchmark for Inference-Time Steering](https://arxiv.org/abs/2603.18329)
...
### [105] [MemArchitect: A Policy Driven Memory Governance Layer](https://arxiv.org/abs/2603.18330)
...
### [106] [Understanding the Theoretical Foundations of Deep Neural Networks through Differential Equations](https://arxiv.org/abs/2603.18331)
...
### [107] [Large-Scale Analysis of Political Propaganda on Moltbook](https://arxiv.org/abs/2603.18349)
...
### [108] [Interpretability without actionability: mechanistic methods cannot correct language model errors despite near-perfect internal representations](https://arxiv.org/abs/2603.18353)
...
### [109] [LGESynthNet: Controlled Scar Synthesis for Improved Scar Segmentation in Cardiac LGE-MRI Imaging](https://arxiv.org/abs/2603.18356)
...
### [110] [From Weak Cues to Real Identities: Evaluating Inference-Driven De-Anonymization in LLM Agents](https://arxiv.org/abs/2603.18382)
...
### [111] [From Topic to Transition Structure: Unsupervised Concept Discovery at Corpus Scale via Predictive Associative Memory](https://arxiv.org/abs/2603.18420)
*Jason Dury*
...
### [112] [Prune-then-Quantize or Quantize-then-Prune? Understanding the Impact of Compression Order in Joint Model Compression](https://arxiv.org/abs/2603.18426)
...
### [113] [AS2 -- Attention-Based Soft Answer Sets: An End-to-End Differentiable Neuro-Soft-Symbolic Reasoning Architecture](https://arxiv.org/abs/2603.18436)
*Wael AbdAlmageed*
...
### [114] [AlignMamba-2: Enhancing Multimodal Fusion and Sentiment Analysis with Modality-Aware Mamba](https://arxiv.org/abs/2603.18462)
...
### [115] [Cognitive Mismatch in Multimodal Large Language Models for Discrete Symbol Understanding](https://arxiv.org/abs/2603.18472)
...
### [116] [Cross-Domain Demo-to-Code via Neurosymbolic Counterfactual Reasoning](https://arxiv.org/abs/2603.18495)
...
### [117] [Correlation-Weighted Multi-Reward Optimization for Compositional Generation](https://arxiv.org/abs/2603.18528)
...
### [118] [Reasonably reasoning AI agents can avoid game-theoretic failures in zero-shot, provably](https://arxiv.org/abs/2603.18563)
*Enoch Hyunwook Kang*
...
### [119] [CAPSUL: A Comprehensive Human Protein Benchmark for Subcellular Localization](https://arxiv.org/abs/2603.18571)
...
### [120] [Interplay: Training Independent Simulators for Reference-Free Conversational Recommendation](https://arxiv.org/abs/2603.18573)
...
### [121] [MedForge: Interpretable Medical Deepfake Detection via Forgery-aware Reasoning](https://arxiv.org/abs/2603.18577)
...
### [122] [ZEBRAARENA: A Diagnostic Simulation Environment for Studying Reasoning-Action Coupling in Tool-Augmented LLMs](https://arxiv.org/abs/2603.18614)
...
### [123] [Agentic Flow Steering and Parallel Rollout Search for Spatially Grounded Text-to-Image Generation](https://arxiv.org/abs/2603.18627)
...
### [124] [D-Mem: A Dual-Process Memory System for LLM Agents](https://arxiv.org/abs/2603.18631)
...
### [125] [An Onto-Relational-Sophic Framework for Governing Synthetic Minds](https://arxiv.org/abs/2603.18633)
...
### [126] [Balanced Thinking: Improving Chain of Thought Training in Vision Language Models](https://arxiv.org/abs/2603.18656)
...
### [127] [Thinking with Constructions: A Benchmark and Policy Optimization for Visual-Text Interleaved Geometric Reasoning](https://arxiv.org/abs/2603.18662)
...
### [128] [MANAR: Memory-augmented Attention with Navigational Abstract Conceptual Representation](https://arxiv.org/abs/2603.18676)
...
### [129] [Accurate and Efficient Multi-Channel Time Series Forecasting via Sparse Attention Mechanism](https://arxiv.org/abs/2603.18712)
...
### [130] [MemMA: Coordinating the Memory Cycle through Multi-Agent Reasoning and In-Situ Self-Evolution](https://arxiv.org/abs/2603.18718)
...
### [131] [Analysis Of Linguistic Stereotypes in Single and Multi-Agent Generative AI Architectures](https://arxiv.org/abs/2603.18729)
...
### [132] [Memento-Skills: Let Agents Design Agents](https://arxiv.org/abs/2603.18743)
...
### [133] [NeuroGame Transformer: Gibbs-Inspired Attention Driven by Game Theory and Statistical Physics](https://arxiv.org/abs/2603.18761)
...
### [134] [A Concept is More Than a Word: Diversified Unlearning in Text-to-Image Diffusion Models](https://arxiv.org/abs/2603.18767)
...
### [135] [Proceedings of the 2nd Workshop on Advancing Artificial Intelligence through Theory of Mind](https://arxiv.org/abs/2603.18786)
...
### [136] [dTRPO: Trajectory Reduction in Policy Optimization of Diffusion Large Language Models](https://arxiv.org/abs/2603.18806)
...
### [137] [Can LLM generate interesting mathematical research problems?](https://arxiv.org/abs/2603.18813)
...
### [138] [ProRL Agent: Rollout-as-a-Service for RL Training of Multi-Turn LLM Agents](https://arxiv.org/abs/2603.18815)
...
### [139] [RewardFlow: Topology-Aware Reward Propagation on State Graphs for Agentic RL with Large Language Models](https://arxiv.org/abs/2603.18859)
...
### [140] [Conflict-Based Search for Multi Agent Path Finding with Asynchronous Actions](https://arxiv.org/abs/2603.18866)
...
### [141] [Bridging Network Fragmentation: A Semantic-Augmented DRL Framework for UAV-aided VANETs](https://arxiv.org/abs/2603.18871)
...
### [142] [Geography According to ChatGPT -- How Generative AI Represents and Reasons about Geography](https://arxiv.org/abs/2603.18881)
...
### [143] [Reasoning over mathematical objects: on-policy reward modeling and test time aggregation](https://arxiv.org/abs/2603.18886)
...
### [144] [Quantitative Introspection in Language Models: Tracking Internal States Across Conversation](https://arxiv.org/abs/2603.18893)
*Nicolas Martorell*
...
### [145] [I Can't Believe It's Corrupt: Evaluating Corruption in Multi-Agent Governance Systems](https://arxiv.org/abs/2603.18894)
...
### [146] [Secure Linear Alignment of Large Language Models](https://arxiv.org/abs/2603.18908)
...
### [147] [Agentic Business Process Management: A Research Manifesto](https://arxiv.org/abs/2603.18916)
...
### [149] [Unmasking Algorithmic Bias in Predictive Policing: A GAN-Based Simulation Framework with Multi-City Temporal Analysis](https://arxiv.org/abs/2603.18987)
...
### [150] [Evaluating Game Difficulty in Tetris Block Puzzle](https://arxiv.org/abs/2603.18994)
...
### [151] [Regret Bounds for Competitive Resource Allocation with Endogenous Costs](https://arxiv.org/abs/2603.18999)
*Rui Chai*
...
### [152] [Behavioral Fingerprints for LLM Endpoint Stability and Identity](https://arxiv.org/abs/2603.19022)
...
### [153] [Man and machine: artificial intelligence and judicial decision making](https://arxiv.org/abs/2603.19042)
...
### [154] [Serendipity by Design: Evaluating the Impact of Cross-domain Mappings on Human and LLM Creativity](https://arxiv.org/abs/2603.19087)
...
### [155] [LuMamba: Latent Unified Mamba for Electrode Topology-Invariant and Efficient EEG Modeling](https://arxiv.org/abs/2603.19100)
...
### [156] [How Uncertainty Estimation Scales with Sampling in Reasoning Models](https://arxiv.org/abs/2603.19118)
...
### [157] [Implicit Patterns in LLM-Based Binary Analysis](https://arxiv.org/abs/2603.19138)
...
### [158] [D5P4: Partition Determinantal Point Process for Diversity in Parallel Discrete Diffusion Decoding](https://arxiv.org/abs/2603.19146)
...
### [159] [cuGenOpt: A GPU-Accelerated General-Purpose Metaheuristic Framework for Combinatorial Optimization](https://arxiv.org/abs/2603.19163)
*Yuyang Liu*
...
### [160] [Box Maze: A Process-Control Architecture for Reliable LLM Reasoning](https://arxiv.org/abs/2603.19182)
*Zou Qiang*
...
### [161] [OS-Themis: A Scalable Critic Framework for Generalist GUI Rewards](https://arxiv.org/abs/2603.19191)
