...
### [1] [Towards CSI-Native Foundation Models: A Channel-Adaptive Roadmap for 6G](https://arxiv.org/abs/2606.20670)
*Chenyu Zhang,Xinchen Lyu,Chenshan Ren,Shuhan Liu,Qimei Cui*

Main category: cs.LG

TL;DR: 本文提出了一种面向信道状态信息（CSI）的原生基础模型框架，通过统一的预训练、位置建模和注意力控制机制，显著提升了6G无线系统的零样本泛化能力、尺度外推能力和推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有无线基础模型将CSI视为任务张量而非传播条件化的信道响应，未能捕捉无线环境固有的时-频-空几何特性，限制了CSI智能的重用和6G系统性能。

Method: 提出了信道自适应路线图，构建统一的CSI原生基础模型框架，该框架通过三个信道要求对齐：尺度感知异质暴露、物理时-频-天线坐标和相关性约束的token交互。

Result: 实验表明该框架在三个方面表现优异：零样本泛化能力（NMSE降低超过4 dB）、尺度外推能力（8倍天线缩放下增益达5.4 dB）、推理效率（移动性感知处理加速达18.8%）。系统级评估显示仅需7.01%密集导频开销，平均NMSE达-18.64 dB，网络频谱效率相比密集LMMSE提升36.6%。

Conclusion: CSI原生表示学习能够支持导频高效的无线接入，为6G系统提供了可重用的CSI智能解决方案，显著提升了无线通信系统的性能。
...
### [2] [NeuroShield: A Device-Agnostic Foundation Model for EEG Authentication](https://arxiv.org/abs/2606.20673)
*Matin Fallahi,Patricia Arias-Cabarcos,Thorsten Strufe*

Main category: cs.LG

TL;DR: NeuroShield是一个用于脑电（EEG）身份验证的可重用基础模型，采用双阶段Transformer架构处理可变通道和可变时长的EEG记录，通过预训练和微调在异构脑电采集设备上实现身份识别。


<details>
  <summary>Details</summary>
Motivation: EEG身份验证面临的主要挑战是模型通常绑定于训练时的采集设备设置（如头戴设备硬件、通道布局、信号时长等），导致不同设备或数据集需要单独开发模型，限制了多数据集学习、知识迁移和模型可重用性。

Method: 提出NeuroShield模型，采用双阶段Transformer架构学习身份判别性嵌入表示，处理可变通道和可变时长的EEG记录。在三个公共EEG数据集（15,762名受试者，28,116个会话）上进行预训练，然后在两个未见的下游数据集上进行迁移评估。

Result: 经过微调后，NeuroShield相对于现有最优方法降低了0.44-8.06个百分点的等错误率。模型能够泛化到比训练时更长的信号段，并能在预训练时未见过的通道布局上工作。

Conclusion: NeuroShield被确立为一个可重用和自适应的EEG身份编码器，能够在异构记录设备设置中工作，并通过开源发布支持可重复性和社区采用。
...
### [3] [Massive Activations Are Architecturally Robust: A Controlled Scratch/Commitment Residual Stream Test](https://arxiv.org/abs/2606.20743)
*Maruthi Vemula*

Main category: cs.LG

TL;DR: 论文提出Ledger Residuals架构，将残差流分为可变临时流（Deliberation）和受保护解码流（Commitment），以测试巨大激活是否是残差流读写角色过载的伪影。实验表明即使在专用解码通道中，模型仍会重建起始令牌的巨大激活，表明这是功能需求而非架构伪影。


<details>
  <summary>Details</summary>
Motivation: 研究Transformer中出现的巨大激活（少数隐藏维度幅度远高于中位数且集中在序列起始令牌）的本质，测试这些异常值到底是残差流过载读写角色的可移除伪影，还是模型功能上的必要特性。

Method: 提出Ledger Residuals架构：将传统的残差流分为可变临时流（Deliberation，中间计算可自由覆盖）和受保护解码流（Commitment，仅解码时读取的累积表示）。在160M和290M参数规模的语言模型上匹配损失进行对比实验。

Result: 1. 专用解码通道并未消除巨大激活需求：模型在受保护通道内重建了典型的固定维度、起始令牌异常值。
2. 重建特征幅度小于标准Transformer但更集中于起始令牌。
3. 更强的稀疏惩罚使异常值更持久、更集中而非消除。
4. 巨大激活在模型解码的任何表示中都会重新出现。

Conclusion: 巨大激活具有架构鲁棒性：无论在模型的哪个表示层解码，它们都会重新出现。这表明巨大激活是Transformer的功能特性而非偶然产物，支持它们是功能性需求而非可移除伪影的观点。
...
### [4] [CIExplainer++: Generating Causal and Interpretable Explanations for Graph Neural Networks](https://arxiv.org/abs/2606.20747)
*Francisco Caldas,Sahil Satish Kumar,Ruben Belo,Cláudia Soares*

Main category: cs.LG

TL;DR: CIExplainer使用因果推断解释GNN预测，G2TeXplainer将因果子图转化为自然语言解释


<details>
  <summary>Details</summary>
Motivation: 可解释人工智能需要使黑盒模型更可信，这需要识别对输出有真实因果影响的组件并将这些结构转化为可解释的表示

Method: 提出了CIExplainer（基于因果推断的扰动方法）和G2TeXplainer（将因果子图转化为自然语言的方法）。CIExplainer使用潜在结果框架识别对GNN预测具有最高因果效应的子图，G2TeXplainer将因果子图转化为捕捉特征级和关系信息的自然语言解释

Result: 在多种GNN架构（GCN、GraphSAGE、GAT、GIN）和数据集上评估和比较了CIExplainer

Conclusion: 通过结合因果子图识别和自然语言解释生成，提出了一个全面的GNN解释框架，使黑盒模型更可信
...
### [5] [Evidential Fusion Network for Multimodal Survival Prediction under Missing Modalities](https://arxiv.org/abs/2606.20757)
*Yucheng Xing,Hailan Mo,Zi Wang,Ling Huang,Mengling Feng*

Main category: cs.LG

TL;DR: 提出了EMMS模型，一种基于证据理论的多模态生存预测方法，能够在缺失模态的情况下保持鲁棒性和计算效率，并提供校准的不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 现有多模态生存预测模型通常假设数据完整性，但对缺失模态的鲁棒性有限，而临床实践中经常遇到数据不完整的情况，需要能够处理缺失模态的解决方案。

Method: 提出Evidential Missing Modality Survival Fusion (EMMS)模型，采用Dempster-Shafer理论和Gaussian Random Fuzzy Numbers进行多模态决策融合，将缺失模态视为空证据，无需生成缺失数据，同时考虑随机不确定性和认知不确定性以及模态可靠性。

Result: 在四个癌症数据集上的实验表明，EMMS达到了最先进的性能，在不完整多模态观察下提供校准和可解释的不确定性估计，且不增加额外计算开销。

Conclusion: EMMS为临床环境中常见的缺失模态问题提供了一种简单、计算高效且鲁棒的解决方案，能够提供可靠的不确定性估计，增强了多模态生存预测的实用性。
...
### [6] [ELADO: Elliptic PDE Assessment Datasets for Operator Learning](https://arxiv.org/abs/2606.20771)
*Frank Ehebrecht,Toni Scharle,Martin Atzmueller*

Main category: cs.LG

TL;DR: ELADO是用于评估神经网络算子学习椭圆偏微分方程解算子时失败模式的系统基准套件，通过可控数据生成过程创建多个数据集来隔离不同困难源


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注平均性能，而ELADO旨在突出椭圆PDE问题中自然出现的挑战，特别是神经网络算子在学习椭圆PDE解算子时的失败模式

Method: 构建围绕泊松方程和亥姆霍兹方程的多个数据集，建立可控数据生成过程来创建专门数据集，隔离五种具体困难源

Result: 对多种神经网络算子架构进行评估，发现重尾目标、谱偏移和输入敏感性都会导致预测精度大幅下降，而标准数据集和指标可能掩盖这些问题

Conclusion: ELADO提供了一个系统性基准套件，能够揭示神经网络算子在学习椭圆PDE解算子时的特定失败模式，强调了需要更精细的评估方法来捕捉标准指标可能掩盖的性能问题
...
### [7] [B[FM]$^2$: Brain Foundation Model via Flow Matching with SplitUNet](https://arxiv.org/abs/2606.20812)
*Jaedong Hwang,Kathleen Zhang,Wei Dai,Konstantinos Kontras,Maarten Vanmarcke,Maarten De Vos,Ila Fiete,Paul Pu Liang*

Main category: cs.LG

TL;DR: 本文提出B[FM]²，一种基于流匹配的脑电基础模型，直接在原始连续信号上预训练，无需分块或离散化，在仅需现有模型1/30预训练数据的情况下，在7/9个下游任务上达到SOTA，并能生成神经科医生无法区分的合成EEG。


<details>
  <summary>Details</summary>
Motivation: 现有EEG基础模型将连续多通道脑电波离散化为块或码本令牌，破坏了连续的脑节律并模糊了细粒度的时间动态，需要大量预训练数据且缺乏对脑电信号连续本质的归纳偏置。

Method: 提出B[FM]²模型，采用连续时间流匹配（Flow Matching）直接在原始EEG信号上预训练，无需分块、分词或掩码；设计SplitUNet速度网络，将每个块分解为独立的1D时间卷积和1D电极卷积，仅沿时间维度下采样以保持电极拓扑结构。

Result: 在9个标准EEG下游分类任务中7个达到SOTA；预训练仅需36,895个片段（约307小时），比现有模型少1-2个数量级（约30倍）；生成的合成EEG经两名认证神经科医生盲审无法与真实脑电区分（Cohen's κ=-0.096）。

Conclusion: B[FM]²通过连续流匹配和针对时间-电极不对称性设计的SplitUNet架构，以极低的预训练成本实现了优越的下游任务性能，并具备高质量合成EEG的能力，验证了在原始连续信号上预训练的有效性。
...
### [8] [CELEUS: Certifiable and Efficient LLM Evaluation via E-Processes](https://arxiv.org/abs/2606.20820)
*Zhijian Zhou,Zesheng Ye,Zhaorun Chen,Bo Li,Feng Liu*

Main category: cs.LG

TL;DR: 本文提出Celeus框架，通过E-processes构建任意有效置信区间，结合不确定性引导采样与代理辅助近似，使LLM评估在保持统计有效性的同时减少54-62%的评估样本。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估方法在迭代更新置信区间并依据其决定何时停止时，无法保证声称的覆盖率（如95%），缺乏任意有效性（anytime-valid），导致了理论严谨性与实践之间的差距。

Method: 提出Celeus框架，利用E-processes构建任意有效置信区间，核心包含两个组件：(i) 不确定性引导的采样——选择信息量大的样本进行评估；(ii) 代理辅助近似——对未评估样本进行近似估计。两者结合形成无偏信号，支持统计上合理的任意有效E-process置信区间。

Result: Celeus达到目标精度时所需评估样本比基线方法少54-62%，同时保持了任意有效的覆盖率。理论上，Celeus获得的置信区间可以以接近参数率（仅差对数因子）的速度收缩。

Conclusion: 本文提出的Celeus框架能够在不牺牲统计有效性的前提下，大幅减少评估所需样本量，弥合了LLM评估在理论严谨性与实际应用之间的差距。
...
### [9] [Evolutionary Discovery of Developmental Reward Schedules in Deep Reinforcement Learning](https://arxiv.org/abs/2606.20858)
*Alan Nadelsticher Ruvalcaba*

Main category: cs.LG

TL;DR: 本研究提出了一个进化框架，用于发现在深度强化学习中随时间变化的奖励调度，通过演化三种生物启发的动机组件（自主性、新颖性和反应性）的时间变化权重，在MiniGrid稀疏奖励任务上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统的强化学习中奖励组成的时间结构通常是人工设计并在整个训练过程中保持固定的，这使得动机优先级的演变过程在很大程度上未被探索。研究旨在探索如何通过进化算法发现优化的、随时间变化的奖励调度。

Method: 提出一个进化框架，将三种生物启发的动机组件（自主性、新颖性和反应性）通过随时间变化的权重组合。评估了四种进化算法：CMA-ES、xNES、DE和L-SHADE，在DoorKey-6x6和KeyCorridorS3R1两个稀疏奖励MiniGrid任务上进行比较，基准包括外部动机基线和三种手工设计方法。

Result: 在DoorKey-6x6任务上，所有进化方法都优于非进化基线，其中L-SHADE表现最佳，相对于仅外部动机基线有约11.4%的相对平均改进。在KeyCorridorS3R1任务上，CMA-ES表现最好，但其他进化方法的泛化能力较弱且不太可靠。有趣的是，发现的调度与定义的发育顺序不同，新颖性在训练早期始终成为主导信号。

Conclusion: 进化优化是深度强化学习中发育奖励调度发现的有前景的方法，计算环境中进化找到的最优方案可能与生物学环境中的最优方案不同。这为理解动机优先级在训练过程中的动态演变提供了新视角。
...
### [10] [Machine Learning Classification of Cryopathy Syndromes: A Comprehensive Comparative Study](https://arxiv.org/abs/2606.20874)
*Nataliya Shakhovska,Valentyna Chopyak,Ivan Izonin,Vira Haievska*

Main category: cs.LG

TL;DR: 本研究开发并比较了机器学习方法来自动分类冷球蛋白血症综合征，旨在为临床决策提供支持。通过分析2,686名患者的实验室数据，评估了12种建模策略，发现基于树的集成方法表现最佳。


<details>
  <summary>Details</summary>
Motivation: 冷球蛋白血症综合征难以分类，因为实验室模式在不同诊断类别中常重叠，某些诊断罕见。这使得常规解释冷球蛋白相关测试具有挑战性，增加了对专家判断的依赖。本研究旨在开发自动化分类方法，为临床决策提供实用支持策略。

Method: 分析2,686名患者（14个诊断类别）的实验室记录，包括人口统计学变量、冷球蛋白测量、沉淀试验、血凝素和溶血素滴度。数据预处理包括清洗、编码、插补、归一化和构建临床交互特征。评估了12种建模策略：随机森林、梯度提升树、多层感知机、软投票集成、SMOTE类平衡、层次分类、周期感知模型、目标二元分类器和概率校准。使用分层训练-测试评估和分层5折交叉验证评估性能。

Result: 由于明显的类别不平衡和诊断间的临床重叠，整体任务具有挑战性。最佳多类性能由随机森林和梯度提升树的软投票集成实现。交叉验证证实平衡随机森林模型具有稳定性能。基于树的方法始终优于神经网络模型。特征工程提高了区分能力，最具信息量的预测因子来自冷球蛋白的交互特征。

Conclusion: 机器学习方法特别是基于树的集成模型，可以有效辅助冷球蛋白血症综合征的自动化分类。特征工程尤其是临床交互特征的构建对提高模型性能至关重要。该研究为临床决策支持提供了实用策略。
...
### [11] [Understanding Latent Flow Models for Tabular Data Synthesis: Targets, Paths, and Sampling](https://arxiv.org/abs/2606.20878)
*Bahrul Ilmi Nasution*

Main category: cs.LG

TL;DR: 本文通过实证研究探索了表格潜在流模型在不同学习目标、概率路径和采样策略下的效果，为受计算预算和风险约束的模型选择提供实用配置指南。


<details>
  <summary>Details</summary>
Motivation: 合成表格数据在监管领域应用广泛，但连续时间生成模型需要在分析效用、披露风险和计算成本之间进行权衡。尽管潜在空间流模型很灵活，但不同的学习目标、概率路径和采样动力学理论上等价，但在有限步积分和明确计算预算下可能表现出不同的行为，需要进行实证比较。

Method: 在七个数据集上对表格潜在流模型进行实证研究，评估了速度匹配、分数匹配、噪声匹配和后验匹配四种学习目标，分别采用最优传输(OT)和方差保持(VP)概率路径，以及ODE和SDE两种采样方法，同时考虑不同的积分预算。

Result: 研究发现：1)学习目标主要决定了效用-风险权衡关系，速度和后验匹配倾向于提供更高效用，而分数和噪声匹配倾向于实现更低披露风险；2)配置和采样选择显著影响性能，中点采样通常能改善分布保真度，OT路径比VP路径能容忍更早停止，可在固定预算或风险阈值下节省计算成本；3)基于这些发现提炼出了可行的默认配置和实践指导。

Conclusion: 本研究为在披露风险和资源约束下进行预发布模型选择提供了实证依据和实用配置指南，并通过开源代码实现增强了研究的可复现性和实际应用价值。
...
### [12] [Temporal Causal Prior-Data Fitted Networks for Panel Data with Learned Reliability Signals](https://arxiv.org/abs/2606.20889)
*Shravan Talupula,Saurabh Sharma*

Main category: cs.LG

TL;DR: 本文提出TCPFN，一个用于零样本时间序列因果推断的基础模型，能够同时输出因果效应估计和可靠性信号，在19个基准数据集上达到SOTA性能，并在工业规模数据集上展现出远超传统方法的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有因果基础模型（如CausalPFN、CausalFM）仅适用于静态横截面数据；神经时序方法（如CRN、G-Net）需要每个数据集单独训练；同时缺乏能输出每对变量可靠性信号的零样本时间序列因果发现方法。工业时间序列因果推断亟需一个既能处理时序动态、时变治疗和未观测混杂，又能提供可靠性信号的可扩展基础模型。

Method: TCPFN包含四大创新：(1)因果判断头，联合预测零效应概率、混杂强度、可识别性、中介分数和因果机制；(2)混合训练先验，覆盖六种因果机制（独立、直接、混杂、中介、时变混杂、反馈）及前门/工具变量先验；(3)离散令牌面板数据架构，采用交叉注意力掩码防止跨时间步信息泄漏；(4)基于FAISS的上下文选择和一步后验校正实现工业规模零样本推理。

Result: 在19个基准数据集上，TCPFN在零样本因果发现中达到AUROC 0.96（田纳西伊士曼）、0.93（SWaT）、0.98（Causal Rivers）、0.97（CAUSRCA）。零效应检测器NullF1达0.94，AUROC 0.99。在工业纸浆数据集上，TCPFN在单GPU上6小时处理V=1,275；而PCMCI在V=666子集上耗时81.5小时，外推至V=1,275需约12.5天。

Conclusion: TCPFN是首个面向工业时间序列的零样本因果基础模型，兼具高精度因果发现、可靠性信号输出和工业级可扩展性，在因果发现质量上超越现有方法，在计算效率上实现数个数量级的提升。
...
### [13] [MMGNN: Multi-level, multi-color graph neural networks for molecular property prediction](https://arxiv.org/abs/2606.20906)
*Trung Nguyen,Duc Duy Nguyen*

Main category: cs.LG

TL;DR: MMGNN是一种用于分子性质预测的多层次多图神经网络，通过分解分子图为原子类型对特定子图来捕捉不同类型的化学相互作用，分为基于共价连接的2D版本和基于空间几何的3D版本，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统分子消息传递神经网络通常通过单一图传播化学多样性相互作用，这可能混合特定相互作用的信号，并且需要深层传播来捕获长程效应，限制了模型的表达能力和效率。

Method: 提出多层次多图神经网络（MMGNN），将分子图分解为重叠的原子类型对特定子图，同时保持原子级分辨率。MMGNN-2D从共价连接构建化学着色子图，MMGNN-3D从空间邻近构建几何着色子图并增强边缘的距离、角度和扭转描述符。两个变体都对每个子图应用共享的消息传递骨干网络，并通过原子级聚合和分子读出组合结果表示。

Result: 在MoleculeNet的五个分类和三个回归基准测试中，使用常见支架分割和五次独立运行进行评估。MMGNN-2D在分类数据集中获得最高的宏观平均AUC-ROC（0.838），在ESOL上获得最低RMSE（0.803）。MMGNN-3D在BBBP上获得最高的平均AUC-ROC（0.956），在FreeSolv上获得最低RMSE（1.793）。结构和留一分析进一步说明了子图分解如何影响学习表示和原子类型对敏感性。

Conclusion: 重叠的相互作用特定图分解是分子性质预测的竞争性策略，拓扑和几何表示具有互补优势，该方法能够有效分离不同类型化学相互作用的信号并提高预测性能。
...
### [14] [Physics-Guided Dual-Stream Heterogeneous Graph Neural Network for Predicting Full-Field Structural Response of Stiffened Panels](https://arxiv.org/abs/2606.20916)
*Yuecheng Cai,Jasmin Jelovica*

Main category: cs.LG

TL;DR: 提出DS-HGNN模型用于薄壁结构全场应力位移预测，采用双流异构图神经网络处理结构信息，在多种工况下取得最佳精度


<details>
  <summary>Details</summary>
Motivation: 大型复杂结构的迭代设计和优化需要快速准确预测应力位移等场变量，有限元分析计算成本高，现有神经网络代理难以处理变化的拓扑和复杂边界条件

Method: 基于板级异构图表示，引入物理引导的边状态初始化，通过分离纵向和横向结构信息的双流消息传递更新，结合FiLM调制的1-D谱卷积和光谱旁路低秩读出重建物理场

Result: DS-HGNN在加筋板数据集上相比6个基准异构图神经网络模型获得最低的应力和位移RMSE，使用19%-38%更少的训练样本达到可比精度，并能捕捉屈服和屈服后应力特征

Conclusion: DS-HGNN为薄壁结构全场应力位移预测提供了有效的神经网络代理，克服了传统代理难以处理复杂边界和拓扑变化的局限
...
### [15] [Short-Term Electricity Demand Forecasting for New England Using a Hybrid Transformer-XGBoost Framework with Weather, Calendar, and COVID-19 Indicators](https://arxiv.org/abs/2606.20918)
*Reza Ghanavati,Behrooz Mosallaei*

Main category: cs.LG

TL;DR: 提出了一种结合Transformer编码器和XGBoost的混合框架用于新英格兰地区短期电力需求预测，通过综合气象、日历、假期、自回归滞后和COVID-19变量等多种特征，实现了比纯XGBoost基准更好的预测精度。


<details>
  <summary>Details</summary>
Motivation: 准确的短期电力需求预测对电力系统可靠运行、能源市场规划和基础设施优化至关重要。传统预测方法在处理复杂的时变特征和突发事件（如COVID-19大流行）影响时存在局限，需要开发能够更好捕捉时间依赖性和外部冲击影响的混合预测框架。

Method: 提出了一种混合预测框架：使用Transformer编码器进行时序特征提取，结合梯度提升决策树（XGBoost）进行最终预测。整合了来自新英格兰六个州的气象观测数据、日历和假期效应、自回归需求滞后变量以及COVID-19流行病学变量。采用Optuna超参数优化工具，使用多元树结构Parzen估计器进行500次试验。数据按时间顺序进行70/15/15的泄漏式分割（训练-验证-测试）。

Result: 混合模型在测试集上达到RMSE 8,876 MWh、MAPE 2.05%、R² 0.906；纯XGBoost基准模型为RMSE 9,304 MWh、MAPE 2.21%、R² 0.896。Diebold-Mariano检验显示两者427.7 MWh的差异统计上不显著。消融研究发现COVID-19特征改善了训练精度但对测试集有不对称影响：移除特征使混合模型RMSE恶化3.2%，但使纯XGBoost模型略微改善1.2%。SHAP时域分析显示8个COVID特征中有5个在疫情后测试集中的重要性高于疫情活跃训练期，表明模型过度应用学习到的疫情模式。

Conclusion: 研究表明时间有效性衰减是核心机制：COVID-19行为扰动在2020-2021年间产生了强烈的需求信号，但到2022年中适应已完成，流行病学特征成为放大过拟合陈旧疫情模式的噪声。这强调了电力需求预测模型需要考虑外部冲击效应的时变性，避免将临时性扰动模式过度推广到后疫情时期。
...
### [16] [$Ω$: Operator-based Mixture Ensemble for Generative Assimilation](https://arxiv.org/abs/2606.20920)
*Pouria Behnoudfar,Nan Chen*

Main category: cs.LG

TL;DR: 介绍Ω（基于算子的混合集成生成同化）框架，通过条件高斯代理建模、无监督分数学习和生成采样，解决高维非线性系统中非高斯后验分布的建模难题。


<details>
  <summary>Details</summary>
Motivation: 部分观测的高维非线性系统中的非高斯后验分布表征是一个根本性挑战。集合卡尔曼滤波器依赖高斯近似，在强非高斯情况下不准确；粒子滤波器存在可扩展性问题；基于分数的生成方法通常需要带真实后验样本的监督训练，这在实践中不可行。

Method: 1. 条件高斯代理建模提供非线性非高斯基线近似，并为未解析变量提供闭式条件后验分布；2. 利用闭式条件分布解析恢复高维未观测分量，减少计算成本；3. 仅学习超出解析基线的残差差异，通过仅使用集合轨迹的去噪分数匹配，无需真实后验样本；4. 通过高斯混合表示重构完整非高斯后验分布；5. 采用退火朗之万采样迭代优化集成成员。

Result: 在多个具有间歇性和极端事件的湍流模型上验证Ω框架，始终提高后验精度。

Conclusion: Ω框架成功整合了条件高斯代理建模、无监督分数学习和生成采样，为高维非线性系统中的非高斯后验分布表征提供了可扩展解决方案，无需真实后验样本，显著降低了学习负担。
...
### [17] [Hierarchical Pooling for Sheaf Neural Networks](https://arxiv.org/abs/2606.20932)
*Dionisia Naddeo,Carlo Abate,Pietro Liò,Nicola Toschi,Filippo Maria Bianchi*

Main category: cs.LG

TL;DR: 本文提出HiSP，一种基于局部谱粗化的层化神经池化框架，通过剪裁低频特征模态实现图信号在粗化空间上的表示，并支持与层化神经网络集成。


<details>
  <summary>Details</summary>
Motivation: 现有的层化神经网络（SNNs）主要在固定图分辨率下操作，缺乏一种原则性的池化机制来构建层级表示，无法像传统GNNs那样进行分层建模。

Method: 提出HiSP（Hierarchical Sheaf Pool），基于局部谱粗化：给定图分区后，通过将细层特征投影到簇内层拉普拉斯算子的低频特征模态上构建粗层茎，利用Galerkin算子实现能量表示，并区分截断损失与实现损失来分析粗化近似误差。

Result: 将HiSP实现为兼容SNNs的GNN池化层，提供PyG实现，支持批处理、提升层拉普拉斯算子和层级架构。

Conclusion: HiSP为层化神经网络提供了一种原则性的谱粗化池化机制，能有效构建层级表示并量化粗化过程中的信息损失。
...
### [18] [Towards Robust Training in NNGPT AutoML Pipeline: A Loss-Optimizer Pairing Selection Study](https://arxiv.org/abs/2606.20933)
...
### [19] [Learning through Internalization](https://arxiv.org/abs/2606.20937)
...
### [20] [Grouped Query Experts: Mixture-of-Experts on GQA Self-Attention](https://arxiv.org/abs/2606.20945)
...
### [21] [A Gated Graph Neural Network Approach to Fast-Convergent Dynamic Average Estimation](https://arxiv.org/abs/2606.20955)
...
### [22] [Right Knowledge, Wrong Answer: Test-Time Steering for Temporal Fact Conflicts in Open-Weight Language Models](https://arxiv.org/abs/2606.20959)
...
### [23] [Is Our Benchmark Enough? An Analysis of Continual Learning for MLLMs](https://arxiv.org/abs/2606.20961)
...
### [24] [Formalizing Task-Space Complexity for Zero-Shot Generalization](https://arxiv.org/abs/2606.20967)
...
### [25] [Detecting Satellites in Radio-Frequency Data via Semi-Supervised Learning](https://arxiv.org/abs/2606.20976)
...
### [26] [Physics-Guided Fully Convolutional Spatiotemporal Learning Toward Digital-Twin-Enabled Microstructure Evolution Prediction](https://arxiv.org/abs/2606.20983)
...
### [27] [Continuous-Time Probabilistic Correctors for Uncertainty-Aware Physics-Based Spacecraft Trajectory Forecasting](https://arxiv.org/abs/2606.21021)
...
### [28] [Structure-Aware Graph Multi-Task Learning for Dynamic Sparse OD Demand Prediction](https://arxiv.org/abs/2606.21022)
*Ming Xu,Jiawei Cao*
...
### [29] [Demystifying Numerical Instability in LLM Inference: Achieving Reproducible Inference for Mission-Critical Tasks with HEAL](https://arxiv.org/abs/2606.21023)
...
### [30] [An Efficient and Effective Architecture for Large-Scale Traffic Prediction via Geometry-Adaptive Square Partitioning](https://arxiv.org/abs/2606.21072)
...
### [31] [Sim2O: Efficient Offline-to-Online MARL via Joint Action Composition](https://arxiv.org/abs/2606.21085)
...
### [32] [BASIL: Bayesian Application for Scientific Iteration and Learning](https://arxiv.org/abs/2606.21092)
...
### [33] [SLeDGe: Semi-Supervised Learning on Data Streams with Graph Structure Learning](https://arxiv.org/abs/2606.21096)
...
### [34] [Enhancing Differentially Private Mechanisms via Empirical Bayes](https://arxiv.org/abs/2606.21107)
...
### [35] [Chem2Gen-Bench: Benchmarking Chemical-to-Genetic Translation in Perturbation Response Space](https://arxiv.org/abs/2606.21109)
...
### [36] [What Accuracy and Gradient Cosine Miss: Evaluating Feedback Alignment via Scale Stability, Reference Validity, and Depth Utility](https://arxiv.org/abs/2606.21126)
...
### [37] [Horizon Adaptive Offline Policy Learning via Value Stitching](https://arxiv.org/abs/2606.21136)
...
### [38] [Dead-Direction Signatures: A Cheap Spectral Reading of Singular Complexity](https://arxiv.org/abs/2606.21158)
...
### [39] [Inverting the Bellman Equation: From $Q$-Values to World Models](https://arxiv.org/abs/2606.21173)
...
### [40] [Rejections Based on Predictive Uncertainty Enable Reliable Routine Soil Spectroscopy](https://arxiv.org/abs/2606.21179)
...
### [41] [TF-SNO: Time-Frequency Gated Spectral Neural Operators for Learning Non-Stationary Partial Differential Equations](https://arxiv.org/abs/2606.21189)
...
### [42] [DCD-PFN: A Decoupling-Aware Foundation Model for Causal Discovery](https://arxiv.org/abs/2606.21212)
...
### [44] [Comparative Evaluation of Machine Learning and Deep Learning Models for Wound-Rotor Synchronous Motor Performance Prediction](https://arxiv.org/abs/2606.21230)
...
### [45] [Does RoPE Prevent or Degrade Retrieval Heads? A Mechanistic Analysis Across Model Families](https://arxiv.org/abs/2606.21249)
*Cengizhan Bayram*
...
### [46] [Gradient-Free Warm-Start Library Recovery: an Amortized-Regret Separation](https://arxiv.org/abs/2606.21253)
*Jianwei Lou*
...
### [47] [Intrinsic Flow Matching on Quantum Pure-State Manifolds with Phase-Aligned Transport](https://arxiv.org/abs/2606.21256)
...
### [48] [An Empirical Study of OpenPangu Quantization on Ascend NPUs](https://arxiv.org/abs/2606.21257)
...
### [49] [Reward-free Pretraining for Reinforcement Learning via Occupancy Coverage Maximization](https://arxiv.org/abs/2606.21271)
...
### [50] [Reconstructing Randomly Masked Spectra Helps DNNs Identify Discriminant Wavenumbers](https://arxiv.org/abs/2606.21289)
...
### [51] [Topological Neural Dynamics: A Neuron-wise Framework for Sequence Modeling](https://arxiv.org/abs/2606.21295)
*Borui Cai,Yao Zhao*
...
### [52] [NASDAQ: Normalized Observation Space Dynamics-Augmented Q-Learning](https://arxiv.org/abs/2606.21297)
...
### [53] [Task-Differentiated Atomic Skill Expansion and Routing for Continual Learning Across Highly Heterogeneous Tasks](https://arxiv.org/abs/2606.21307)
...
### [54] [Objective-Behavior Alignment: Diagnostics for MORL Policy Selection](https://arxiv.org/abs/2606.21321)
...
### [55] [Distinguishing indistinguishable attractors: Unsupervised anomaly detection with reservoir computers](https://arxiv.org/abs/2606.21322)
...
### [56] [MedTS-TTT: Test-Time Training for Medical Time Series Classification](https://arxiv.org/abs/2606.21329)
...
### [57] [Ramanujan Graph Rewiring with Non Negative Resistance Curvature](https://arxiv.org/abs/2606.21333)
...
### [58] [DataClaw0: Agentic Tailoring Multimodal Data from Raw Streams](https://arxiv.org/abs/2606.21337)
...
### [59] [A Reward-Petri-Net Interpretation of Temporal Behavior Trees](https://arxiv.org/abs/2606.21350)
...
### [60] [Urban Power Grid Topology and Hierarchy Identification from Open Data](https://arxiv.org/abs/2606.21352)
...
### [61] [SOHET: Sequence Of Heterogeneous Events Transformer with Self-Supervised Pre-Training](https://arxiv.org/abs/2606.21356)
...
### [62] [Predictive Repair Management Using a Multi-Head Attention Transformer and Online Learning](https://arxiv.org/abs/2606.21364)
...
### [63] [Enhancing Creativity in 3D Generative Design via a TRIZ-Inspired Text-to-CAD Framework](https://arxiv.org/abs/2606.21378)
...
### [64] [Unsupervised Disentanglement Without Compromises : How Functional Orthogonality Enforces Identifiability](https://arxiv.org/abs/2606.21385)
...
### [65] [VLA-FAIL: Efficient Task Failure Detection for Finetuned Vision-Language-Action Models](https://arxiv.org/abs/2606.21386)
...
### [66] [Atomistic Language Models Understand and Generate Materials](https://arxiv.org/abs/2606.21395)
...
### [67] [One Size does not Fit All: Heterogeneous Latent Space Alignment for Unsupervised Domain Adaptation](https://arxiv.org/abs/2606.21415)
...
### [68] [Federated Temporal Attention Intelligence for Cyber-Resilient IoMT: Lightweight Digital Twins and PPO-Driven Honeypot Deception](https://arxiv.org/abs/2606.21422)
...
### [69] [Dual-Attention Convolution Experts for Sparse Tensor Completion](https://arxiv.org/abs/2606.21427)
...
### [70] [Universal Encoders for Modular Relational Deep Learning](https://arxiv.org/abs/2606.21434)
...
### [71] [Fusing Backdoors, Machine Learning, and Optimization for Large-Scale Parametric Mixed-Integer Programs](https://arxiv.org/abs/2606.21440)
...
### [72] [Fast-TurboQuant: A Multiplier-Free Online Vector Quantization Approach](https://arxiv.org/abs/2606.21448)
...
### [73] [Post-Training Speech Enhancement Language Models with Perceptual Rewards](https://arxiv.org/abs/2606.21458)
...
### [74] [Deep Learning for Soil Moisture Estimation: Fusing Satellite Data with Optimally-Lagged Meteorological Features](https://arxiv.org/abs/2606.21475)
...
### [75] [Robustness Cannot be Reduced to Regularization: Studying Adversarial Training Beyond the Linear Case](https://arxiv.org/abs/2606.21488)
...
### [76] [Predicting High-Risk Colorectal Polyps in African Americans Using Pre-Colonoscopy Clinical Features: Machine Learning Model Development and Temporal Validation](https://arxiv.org/abs/2606.21492)
...
### [77] [Breaking chains with trees: Deep learning with $\mathcal{O}(\log N)$ parallel time complexity](https://arxiv.org/abs/2606.21497)
...
### [78] [Privacy-Preserving Federated Temporal Graph Learning with Digital Twin--Guided Adaptive Deception for Cyber-Resilient IoMT](https://arxiv.org/abs/2606.21513)
*Syed Zeeshan Haider,Anwar Shah,Muneeb Arif,Hamza Iftikhar,Waqas Ali*
...
### [79] [Towards Understanding the Power and Limits of the Muon Optimizer: A River-Valley Perspective](https://arxiv.org/abs/2606.21514)
...
### [80] [Backpropagating Through Simulation: Analytic Policy Gradients for Sample and Learning Efficient Differentiable Continuous Control](https://arxiv.org/abs/2606.21525)
*Yueci Deng*
...
### [81] [Memory Is No Longer a Bottleneck: Memory-Efficient Graph Filtering for Scalable Collaborative Filtering](https://arxiv.org/abs/2606.21540)
...
### [82] [LIG: Layer-wise Integrated Gradients for Within-Layer Flow Analysis in Transformers](https://arxiv.org/abs/2606.21564)
...
### [83] [The Cost Geometry of Belief: finite-resource inference under noisy observation](https://arxiv.org/abs/2606.21585)
*Laurent Caraffa*
...
### [84] [FAST: A Framework for Aligned Sampling and Training in Parallel Reinforcement Learning for Autonomous Driving](https://arxiv.org/abs/2606.21587)
...
### [85] [Geometric and Information Compression of Representations in Deep Learning](https://arxiv.org/abs/2606.21593)
...
### [86] [Learning to Place Guards by Reinforcement: A Geo-Free Neural Policy for the Vertex-Guard Art Gallery Problem](https://arxiv.org/abs/2606.21604)
...
### [87] [The Two-Hump Problem: Bridging the Difficulty Gap in Mathematical Reinforcement Learning](https://arxiv.org/abs/2606.21611)
...
### [88] [HERALD: High-Throughput Block Diffusion LLM Serving via CPU-GPU Cooperative KV Cache Retrieval](https://arxiv.org/abs/2606.21633)
...
### [89] [A new classification method based on Minimum Spanning Trees](https://arxiv.org/abs/2606.21639)
...
### [90] [When Is an LLM Worth It for Hyperparameter Optimization? A Budget-Matched Study on Tabular Data Finds the Warm-Start Is a Default Configuration, Not the Model](https://arxiv.org/abs/2606.21641)
...
### [91] [A Framework for Directed Acyclic Hypergraph Learning](https://arxiv.org/abs/2606.21668)
...
### [92] [Decodable but Not Faithful: Coupling Natural-Language Rationales to Programmatic Verifiers](https://arxiv.org/abs/2606.21678)
...
### [93] [Expressivity Saturation: Reduced Affine Region Usage Under Increasing Task Complexity](https://arxiv.org/abs/2606.21687)
...
### [94] [Embedding Linear Equality Constraints in Probabilistic Neural Networks for Dynamic Modelling](https://arxiv.org/abs/2606.21728)
...
### [95] [AdaPrivate-TS: Private Thompson Sampling for Contextual Bandits with Privacy Amplification](https://arxiv.org/abs/2606.21757)
...
### [96] [Decision-Focused Learning: When and Why Traditional Prediction Models Fail](https://arxiv.org/abs/2606.21773)
*Mo Liu*
...
### [97] [Beyond the Next Step: Variable-Length Latent World Models for Long-Horizon Planning](https://arxiv.org/abs/2606.21775)
...
### [98] [A Causal DAG Prior for Synthetic Time-Series Classification Datasets](https://arxiv.org/abs/2606.21776)
...
### [99] [RocketPFN: Accurate Time Series Classification via In-Context Learning](https://arxiv.org/abs/2606.21786)
*Franco Martino O'Rourke,Ana Trisovic,Dimitris Bertsimas*
...
### [100] [What Do Lorentz-Equivariant Jet Taggers Learn?](https://arxiv.org/abs/2606.21790)
...
### [101] [Discretizing Reward Models](https://arxiv.org/abs/2606.21795)
...
### [102] [Causal Variational Deep Embedding: A Family of Interventional Generators for Confounded Images](https://arxiv.org/abs/2606.21806)
...
### [103] [Causal Gaussian Processes for Robust Treatment Effect Evaluation with Unobserved Confounding](https://arxiv.org/abs/2606.21809)
...
### [104] [Local Causal Attribution of Chain-of-Thought Reasoning](https://arxiv.org/abs/2606.21821)
...
### [105] [Mat-Pref: Verifiable-Reward Training Improves Compositional Reasoning in Inorganic Materials](https://arxiv.org/abs/2606.21830)
...
### [106] [UniRank: Unified Rank Allocation for Low-Rank LLM Compression](https://arxiv.org/abs/2606.21847)
...
### [107] [WiSP: A Working-Set View of Mixture-of-Experts Serving on Extremely Low-Resource Hardware](https://arxiv.org/abs/2606.21868)
...
### [108] [Protein contacts are already in the attention: a single-forward-pass alternative to the Categorical Jacobian](https://arxiv.org/abs/2606.21876)
*Rome Thorstenson*
...
### [109] [A Verifiable Search Is Not a Learnable Chain-of-Thought](https://arxiv.org/abs/2606.21884)
*Harsh Patel*
...
### [110] [Cohort-Anchored Foundation Models for Electronic Health Records: From Risk Scores to Auditable Peer Cohorts](https://arxiv.org/abs/2606.21885)
*Kaiping Zheng*
...
### [111] [Ranking-and-Selection with Multiple Correct Answers and Non-Answerable Estimates](https://arxiv.org/abs/2606.21889)
...
### [112] [AgroSense 2.0: Cross-Modal Transformer Fusion with Geospatial Raster Integration and Interpretable Multi-Task Learning for Precision Crop Recommendation](https://arxiv.org/abs/2606.21892)
...
### [113] [Data Pruning: Redundant, Problematic, and Interdependent Samples](https://arxiv.org/abs/2606.21916)
...
### [114] [Selective Ensemble Based on Preference-Directed Multi-Objective Bandits](https://arxiv.org/abs/2606.21929)
...
### [115] [DevoTG: Temporal Graph Neural Networks for Modeling C. elegans Developmental Connectomics](https://arxiv.org/abs/2606.21940)
...
### [116] [Modularized Reinforcement Learning on LLMs: From MDP Creation to Exploration and Learning](https://arxiv.org/abs/2606.21943)
...
### [117] [On the Curse of Dimensionality in Private Sparse Covariance Estimation and PCA](https://arxiv.org/abs/2606.21951)
...
### [118] [Learning by Shifting: Temporal View Construction for Time Series Contrastive Learning](https://arxiv.org/abs/2606.21957)
...
### [119] [A Standard Processing Pipeline for High-accuracy Measurement of Few-shot Regression on Laser Induced Breakdown Spectroscopy](https://arxiv.org/abs/2606.21960)
*Hao Li*
...
### [120] [VegSim: A Geospatial World Model for Scenario-Conditioned Vegetation Simulation](https://arxiv.org/abs/2606.21961)
...
### [121] [SPOTR: Spatio-temporal Pooling One-Token Reconstruction for Universal Physiological Signal Self-supervised Learning](https://arxiv.org/abs/2606.21973)
...
### [122] [Prefix-Guided On-Policy Distillation: Mining Golden Trajectories from Rollouts](https://arxiv.org/abs/2606.21994)
...
### [123] [Load Testing for Machine Learning Model Serving Systems at Scale](https://arxiv.org/abs/2606.22013)
...
### [124] [Channel Location Constrains the Auditability of Subliminal Learning](https://arxiv.org/abs/2606.22019)
*Tamas Madl*
...
### [125] [Cluster-Specific Localized Drift Detection for Efficient Batch Model Adaptation under Controlled Distribution Shift](https://arxiv.org/abs/2606.22026)
...
### [126] [A Completion-Aware Framework for Impactful Counterfactual Explainability in Graph Neural Networks](https://arxiv.org/abs/2606.22033)
...
### [127] [Gradient-Descent Steps to Success over Mean Accuracy: A Paradigm Shift for ML](https://arxiv.org/abs/2606.22053)
...
### [128] [Provably Efficient Policy-Reward Co-Pretraining for Adversarial Imitation Learning](https://arxiv.org/abs/2606.22056)
...
### [129] [New Smooth Loss functions for Robust Regression that Closely Approximate Absolute Error and Provide Improved Performance on Datasets With Significant Outliers](https://arxiv.org/abs/2606.22068)
...
### [130] [Frequency-Domain Neural ODEs for Modeling Non-Linear Dynamical Systems](https://arxiv.org/abs/2606.22075)
...
### [131] [OphthaDT: Generative Digital Twins for Forecasting Visual Acuity Trajectories in Ophthalmology](https://arxiv.org/abs/2606.22101)
...
### [132] [Meta-Reinforcement Learning via Evolution for Multi-Objective Combinatorial Supply Chain Optimisation](https://arxiv.org/abs/2606.22146)
...
### [133] [Parameterized Representations via Implicit Stochastic Modulation for High-Dimensional and High-Order Neural PDE Solvers](https://arxiv.org/abs/2606.22150)
...
### [134] [Drowning in Routine: Signal Dilution in Multi-Turn Agent Training](https://arxiv.org/abs/2606.22164)
...
### [135] [Early-Exit Graph Neural Networks for Link Prediction](https://arxiv.org/abs/2606.22167)
...
### [136] [Beyond Time Series: Spatial Reasoning for Epidemic Forecasting via Multimodal Learning](https://arxiv.org/abs/2606.22171)
...
### [137] [Gated MLPs as Symmetry-Broken Rank-1 Bilinear Attention](https://arxiv.org/abs/2606.22172)
*Nathan Breslow*
...
### [138] [Residue-Level Attributions in Protein Language Models Do Not Recover Allergen Epitopes](https://arxiv.org/abs/2606.22181)
...
### [139] [Bayesian Adaptation Gym: A Benchmark for the Bayesian Low-Rank Adaptation of Multi-Modal Language Models](https://arxiv.org/abs/2606.22188)
...
### [140] [L20-Edu-135M: An Auditable Single-GPU Study of Data-Efficient Small Language Modeling](https://arxiv.org/abs/2606.22189)
*Yin Li*
...
### [141] [Neural Conjugate Aggregation: Identifiable Unsupervised Multi-Sensor Regression under Heterogeneous Sensor Bias](https://arxiv.org/abs/2606.22200)
...
### [142] [Sequential Minimal Optimization Algorithm for One-Class Support Vector Machines With Privileged Information](https://arxiv.org/abs/2606.22210)
...
### [143] [SamatNext v0.2-B: An Exploratory Study of RMS-Normalized Hybrid Decoders for Curriculum Retention in Small Code Models](https://arxiv.org/abs/2606.22248)
*Samat Zharassov*
...
### [144] [On the Expressive Power of Weight Quantization in Large Language Models](https://arxiv.org/abs/2606.22249)
*Shao-Qun Zhang*
...
### [145] [Evolving Spatial Weights for Cartographic Synthesis](https://arxiv.org/abs/2606.22252)
...
### [146] [From Handcrafted Features to Functional Edge Learning: Evolution of EEG Seizure Detection Frameworks](https://arxiv.org/abs/2606.22258)
...
### [147] [Learning a Normal World Model for Few-Shot Boundary-Calibrated Abnormality Detection](https://arxiv.org/abs/2606.22261)
...
### [148] [SCENIC: Semantic-Conditioned Edge-Aware Neural Framework for Structured IoT Command Generation](https://arxiv.org/abs/2606.22296)
...
### [149] [Encoder-Decoder Manifold Alignment for Idempotent Generation](https://arxiv.org/abs/2606.22304)
...
### [150] [Enhancing Protein Representation Learning via Manifold Restore Mixing](https://arxiv.org/abs/2606.22307)
...
### [151] [Diffusion Integrated Gradients: Controllable Path Generation for Flexible Feature Attribution](https://arxiv.org/abs/2606.22314)
...
### [152] [Curriculum Reinforcement Learning Can Incentivize Reasoning Capacity in LLMs Beyond the Base Model](https://arxiv.org/abs/2606.22317)
...
### [153] [All Routes Lead to Collapse](https://arxiv.org/abs/2606.22325)
*K. R. Balasubramanian*
...
### [154] [Select-to-Act: Hierarchical Reinforcement Learning via Adaptive Language Guidance](https://arxiv.org/abs/2606.22350)
...
### [155] [Reliability-Guided Adaptive Ensembling for Robust Test-Time Adaptation](https://arxiv.org/abs/2606.22351)
...
### [156] [On the Sparsity-Storage-Accuracy Tradeoff in Parsimoniously Activated Dictionary Learning](https://arxiv.org/abs/2606.22352)
...
### [157] [Multigrid Training for Molecular Generation using Graph Neural Networks](https://arxiv.org/abs/2606.22377)
...
### [158] [Bypassing Minimization Bias: A Shift-Invariant Variance Estimator for Off-Equilibrium Local Learning Coefficients](https://arxiv.org/abs/2606.22389)
*Yingjia Cai*
...
### [159] [Asymptotic Signal Subspace Recovery in Softmax Attention Models](https://arxiv.org/abs/2606.22406)
*Lan V. Truong*
...
### [160] [QeHDC: Hyperdimensional Computing based on Quantum-enhanced binding and SuperClass Construction](https://arxiv.org/abs/2606.22421)
...
### [161] [Enhancing LLMs for Graph Tasks via Graph-aware LoRA Generation](https://arxiv.org/abs/2606.22429)
...
### [162] [Escaping the Variance Trap: Jacobian-Free Dynamics for Root-Finding Bilevel Optimization](https://arxiv.org/abs/2606.22433)
...
### [163] [Distribution-Aware Robust Bilevel Optimization: Quantile-Guided Huber Updates in Two-Timescale Stochastic Approximation](https://arxiv.org/abs/2606.22436)
*Zhiyu Li,Xi Xuan,Davide Carbone*
...
### [164] [Adaptive Recurrent Message Passing for Test Time Computing on Graphs](https://arxiv.org/abs/2606.22462)
*Junshu Sun,Wanxing Chang,Qingming Huang,Shuhui Wang*
...
### [165] [Federated learning with heavy-tailed gradient noise and communication noise: a variance-reduction based algorithm](https://arxiv.org/abs/2606.22466)
...
### [166] [Fed-CausalDiff: Decoupled Synchronization for Federated Do-Simulation and Policy Evaluation](https://arxiv.org/abs/2606.22510)
...
### [167] [The Scissors Effect: When Resize-Based Input Diversity Helps or Hurts Transfer Attacks](https://arxiv.org/abs/2606.22516)
...
### [168] [Generative Robust Optimisation](https://arxiv.org/abs/2606.22536)
...
### [169] [Mitigating Measurement-Induced Training Instability in Hybrid Quantum Neural Networks for Protein Classification](https://arxiv.org/abs/2606.22551)
...
### [170] [Concept-Constrained Prompt Learning for Few-Shot CLIP Adaptation](https://arxiv.org/abs/2606.22567)
...
### [171] [Stationary Robust Mean-Field Games under Model Mismatches](https://arxiv.org/abs/2606.22579)
*Yue Wang*
...
### [172] [Training-free Task Classification for Multi-Task Model Merging](https://arxiv.org/abs/2606.22589)
...
### [173] [On the Position Bias of On-Policy Distillation](https://arxiv.org/abs/2606.22600)
...
### [174] [Federated Learning for Global Carbon Emission Forecasting: A Hybrid Time-Series Approach with Statistical and Neural Models](https://arxiv.org/abs/2606.22618)
...
### [175] [Scalable Maximum Entropy Reinforcement Learning for Diffusion Policies via Adjoint Matching](https://arxiv.org/abs/2606.22630)
...
### [176] [A Markov Chain Approach to Preference Alignment](https://arxiv.org/abs/2606.22652)
...
### [177] [Foundation Models for Epileptogenic Zone Identification in Drug-Resistant Epilepsy](https://arxiv.org/abs/2606.22657)
...
### [178] [LSTM Variants for Chaotic Dynamical Systems: An Empirical Study on the Lorenz Attractor](https://arxiv.org/abs/2606.22662)
*Ruslan Gokhman*
...
### [179] [Clipping the Price of Adaptivity at the Tail](https://arxiv.org/abs/2606.22669)
...
### [180] [SCRUB-FL: Sanitizing and Cleansing Representations via Unlearning of Backdoors](https://arxiv.org/abs/2606.22700)
...
### [181] [Paraphrasing Attack Resilience of Various AI-Generated Text Detection Methods](https://arxiv.org/abs/2605.14240)
...
### [182] [Subspace-Constrained Federated Learning with Low-Rank Adaptation](https://arxiv.org/abs/2606.22724)
...
### [183] [GRADE: Graph Representation of LLM Agent Dependency and Execution](https://arxiv.org/abs/2606.22741)
*Yue Zhao*
...
### [184] [Error Highways: Scaling Predictive Coding to Very Deep Networks](https://arxiv.org/abs/2606.22744)
...
### [185] [One-Step Flow Matching for Generative Modeling of Path-Dependent Physical Fields](https://arxiv.org/abs/2606.22752)
...
### [186] [Factored Gossip DiLoCo: Reducing Blocking Communication in DiLoCo](https://arxiv.org/abs/2606.22768)
...
### [187] [Noise is Signal: Density-Based Outliers as Leading Indicators of Occupational Emergence in Labor Market Text](https://arxiv.org/abs/2606.22769)
*Shreyash Rawat*
...
### [188] [Statistical Matching via Schrödinger Bridge beyond Conditional Independence](https://arxiv.org/abs/2606.22770)
...
### [189] [GeoRouteNet: Geometry-Enhanced Non-Autoregressive Neural Solver for the Traveling Salesman Problem](https://arxiv.org/abs/2606.22776)
*Xiang Li*
...
### [190] [Explainable AI for Mental Health Prediction in Drug-Affected Populations with Dragonfly Algorithm and GAN Oversampling](https://arxiv.org/abs/2606.22780)
...
### [191] [Towards Robust Personalized Federated Learning: Vulnerability Assessment and Defense Co-Design](https://arxiv.org/abs/2606.22782)
...
### [192] [Retrieval-Augmented Multimodal Learning for Enzyme-Substrate Interaction Prediction Under Low-Homology Shift](https://arxiv.org/abs/2606.22823)
...
### [193] [BranchShine: Compact Raw-Audio-to-IPA Transcription with a RoPE E-Branchformer Encoder](https://arxiv.org/abs/2606.22824)
...
### [194] [CLIP-guided Diffusion Model for Backdoor Generation in Sensor-based Human Activity Recognition](https://arxiv.org/abs/2606.22837)
...
### [195] [RLM-Cascade: Response-Level Speculative Decoding for Cost-Efficient LLM API Serving](https://arxiv.org/abs/2606.22840)
...
### [196] [The Unseen Hand: Manipulating Model Fairness and SHAP with Targeted Identity Re-Association Attacks](https://arxiv.org/abs/2606.22858)
...
### [197] [When AUC 0.998 Is Not Enough: A Candidate Evaluation Protocol for Hidden-State Probes of Indirect Prompt Injection in Multimodal Computer-Use Agents](https://arxiv.org/abs/2606.22864)
...
### [198] [Discovering Crystal Structure Prediction Algorithms with an AI Co-Scientist](https://arxiv.org/abs/2606.22866)
...
### [199] [SpotAttention: Plug-In Block-Sparse Routing for Pretrained Long-Context Transformers](https://arxiv.org/abs/2606.22874)
...
### [200] [Priority-Aware Learning-Unlearning Correction for Dynamic Decentralized LoRA Fine-Tuning](https://arxiv.org/abs/2606.22878)
...
### [201] [Physiology-Aware CNN and Zero-Shot Multimodal LLMs for ECG Image Classification: A Comparative Study](https://arxiv.org/abs/2606.22889)
...
### [202] [Learning Graphs through Continuous Information Entropy Fields](https://arxiv.org/abs/2606.22895)
...
### [203] [PromptDyG: Test-Time Prompt Adaptation on Dynamic Graphs](https://arxiv.org/abs/2606.22914)
...
### [204] [GRAIN: Group Aggregation via Min-Norm Objective](https://arxiv.org/abs/2606.22917)
...
### [205] [Hierarchical Reinforcement Learning for Sparse-Reward Search in Commutative Algebra](https://arxiv.org/abs/2606.22922)
...
### [206] [EEG Benchmarking Needs a Task Specification Layer: NeuroDoc for Rulebook-Guided, Executable Benchmark Construction](https://arxiv.org/abs/2606.22925)
...
### [207] [FORGE: Fused On-Register Gradient Elimination for Memory-Efficient LLM Training](https://arxiv.org/abs/2606.22932)
...
### [208] [Provable Benefits of RLVR over SFT for Reasoning Models: Learning to Backtrack Efficiently](https://arxiv.org/abs/2606.22938)
...
### [209] [Neural Operator Processes for Probabilistic Operator Learning under Partial Observations](https://arxiv.org/abs/2606.22946)
...
### [210] [DT-GOL: Dual-Track Geometric Online Learning in Nonstationary Environment with Label Delay](https://arxiv.org/abs/2606.22950)
...
### [211] [PG-MAP: Joint MAP Optimization for Inference-Time Alignment of Diffusion and Flow-Matching Models](https://arxiv.org/abs/2606.22958)
...
### [212] [Attacking the Trusted Imagination: Oracle-Level Integrity Attacks on Imagine-then-Act World Models](https://arxiv.org/abs/2606.22966)
...
### [213] [Topological Out-of-Domain Generalization in Dynamical Systems Reconstruction](https://arxiv.org/abs/2606.22969)
...
### [214] [TaLK: Text-attributed Graph Dataset Distillation via Coupling Language Model with Graph-Aware Kernel](https://arxiv.org/abs/2606.22975)
...
### [215] [Understanding Parallel Samplers in Masked Diffusion via Random Walks on Graphs](https://arxiv.org/abs/2606.22976)
...
### [216] [Neural Architecture Search of Sample Reweighting Networks for Complex Distribution Shift](https://arxiv.org/abs/2606.22991)
...
### [217] [Do Sparse Autoencoders Learn Meaningful Concept Hierarchies?](https://arxiv.org/abs/2606.22994)
...
### [218] [Group-Graph Policy Optimization for Long-Horizon Agentic Reinforcement Learning](https://arxiv.org/abs/2606.22995)
...
### [219] [A Novel Approach to Temporal QoS Estimation via Extended Kalman Filter-Incorporated Latent Feature Analysis](https://arxiv.org/abs/2606.23010)
...
### [220] [Counterfactual learning of new adaptive instructional policies using logged data](https://arxiv.org/abs/2606.23015)
...
### [221] [EvoRubrics: Dynamic Rubrics as Rewards via Adversarial Co-Evolution for LLM Reinforcement Learning](https://arxiv.org/abs/2606.23038)
...
### [222] [Prime Fourier Embeddings: A Principled Basis for Modular Arithmetic](https://arxiv.org/abs/2606.23044)
...
### [223] [PeLAP-A: Adaptive Latent Pruning for Lightweight Latent Diffusion Models](https://arxiv.org/abs/2606.23086)
...
### [224] [FlowTrain: Flow-Based Decoupled Training for Industrial-Grade Vision-Language Models](https://arxiv.org/abs/2606.23087)
...
### [225] [FLFL: Federated Latent Factor Learning for Private Recovery of Spatio-Temporal Signals](https://arxiv.org/abs/2606.23091)
...
### [226] [Minimax Quantile Lower Bounds for Interactive Statistical Decision Making with Privacy](https://arxiv.org/abs/2606.23096)
...
### [227] [ReNIO: Reweighting Negative Trajectory Importance for LLM On-Policy Distillation](https://arxiv.org/abs/2606.23104)
...
### [228] [Self-Evolution for Multi-Turn Tool-Calling Agents via Divergence-Point Preference Learning](https://arxiv.org/abs/2606.23112)
*Jiaqiang Tang*
...
### [229] [Temporal-Spectral Alignment with Frequency Adaptation for Source-Free Time-Series Adaptation](https://arxiv.org/abs/2606.23120)
...
### [230] [The Fractal Neural Operator: Overcoming Spectral Bias in Chaotic Attractors via Prime-Harmonic Weierstrass Encodings](https://arxiv.org/abs/2606.23123)
*Kanishk Awadhiya*
...
### [231] [Weighted Score-Oriented Losses for Temporally Localized Event Prediction](https://arxiv.org/abs/2606.23145)
...
### [232] [Substitution-Based Analysis of Structural Novelty for Generative Models of Materials](https://arxiv.org/abs/2606.23166)
...
### [233] [Position: Correct Answer, Wrong Mechanism -- When AI Scientists Defend General Claims Their Own Data Contradicts](https://arxiv.org/abs/2606.23175)
*Steven Young Eulig*
...
### [234] [EML Trees Are Universal Approximators](https://arxiv.org/abs/2606.23179)
...
### [235] [Stage-dependent integer-binary encoding in factorization-machine black-box optimization](https://arxiv.org/abs/2606.23188)
...
### [236] [Memory Contagion: Cross-Temporal Propagation of Evaluator Bias via Agent Memory](https://arxiv.org/abs/2606.23195)
*Zewen Liu*
...
### [237] [Bridge the Gaps: Heterogeneous Attributed Graph Clustering via Quaternion Representation Learning](https://arxiv.org/abs/2606.23199)
...
### [238] [Leveraging AutoML for Sustainable Deep Learning: A Multi-Objective HPO Approach on Deep Shift Neural Networks](https://arxiv.org/abs/2606.23208)
...
### [239] [Efficient Network Inference via Hardware-Aware Architecture Search, Model Pruning & Quantization](https://arxiv.org/abs/2606.23210)
...
### [264] [The Energy Consumption of Transformer Fine-Tuning: A Roofline-Inspired Scaling Model](https://arxiv.org/abs/2606.23546)
...
### [265] [Scheduling Thoughts: Learning the Order of Thought in Diffusion Language Models](https://arxiv.org/abs/2606.23567)
...
### [266] [SVD-Surgeon: Optimal Singular-Value Surgery for Large Language Model Compression](https://arxiv.org/abs/2606.23568)
...
### [267] [Patient-Aware Contrastive Learning Preserves Per-Patient Structure in RR-Interval Representations](https://arxiv.org/abs/2606.23570)
...
### [268] [A Spectral Theory of Normalized Corrected GNN Propagation](https://arxiv.org/abs/2606.23572)
...
### [269] [Solve for the Hyperparameter, Skip the Search: Kolmogorov-Optimal Scaling Laws for Spline Regression](https://arxiv.org/abs/2606.23575)
...
### [270] [It's Much Easier for Neural Networks to learn Game of Life Dynamics with the Right Activation Function: Polynomial Kolmogorov-Arnold Networks](https://arxiv.org/abs/2606.23587)
...
### [271] [Quantifying the Agreement Between Data-Influence and Data-Similarity to Understand LLM Behavior](https://arxiv.org/abs/2606.23591)
...
### [272] [MORL-A2C: Multi-Objective Reinforcement Learning Reranker for Optimizing Healthiness in MOPI-HFRS](https://arxiv.org/abs/2606.23603)
...
### [273] [Scaling Linear Mode Connectivity and Merging to Billion Parameter Pretrained Transformers](https://arxiv.org/abs/2606.23607)
...
### [274] [Discovering Latent Groups for Robust Classification](https://arxiv.org/abs/2606.23609)
...
### [275] [DiT-Reward: Generative Representations for Text-to-Image Reward Modeling](https://arxiv.org/abs/2606.23626)
...
### [276] [Muown Implicitly Performs Angular Step-size Decay](https://arxiv.org/abs/2606.23637)
...
### [277] [Learning Process Rewards via Success Visitation Matching for Efficient RL](https://arxiv.org/abs/2606.23640)
...
### [278] [Dynamic estimation of slowly varying sequences](https://arxiv.org/abs/2606.23655)
...
### [279] [MAS-PromptBench: When Does Prompt Optimization Improve Multi-Agent LLM Systems?](https://arxiv.org/abs/2606.23664)
...
### [280] [On the Limits of Prompt-Conditioned Language Models as General-Purpose Learners](https://arxiv.org/abs/2606.23668)
...
### [281] [Tapered Language Models](https://arxiv.org/abs/2606.23670)
...
### [282] [Open Problem: Is AdamW Effective Under Heavy-Tailed Noise?](https://arxiv.org/abs/2606.23676)
...
### [283] [On the Identifiability of User Adaptation in Co-Adaptive Neural Interfaces](https://arxiv.org/abs/2606.20569)
*Philip Waggoner*

Main category: cs.AI

TL;DR: 本文分析了共适应人机系统中的可辨识性问题，指出闭环编码器估计不能唯一识别用户适应，而是反映联合系统的特性，并讨论了行为适应解读的含义及识别条件。


<details>
  <summary>Details</summary>
Motivation: 人机共适应系统中，闭环编码器估计无法唯一识别用户适应行为，阻碍了对行为适应的准确解读。

Method: 通过理论分析闭环系统中编码器估计的可辨识性，揭示其反映的是联合系统特性而非单一用户适应。

Result: 证明了闭环编码器估计不能唯一识别用户适应，而是反映联合系统属性。

Conclusion: 提出了实现可辨识的条件，为正确解读行为适应提供了理论基础。
...
### [284] [Beyond Fixed Budgets: Characterizing the Inelasticity and Limitations of Tree-of-Thought Reasoning Strategies](https://arxiv.org/abs/2606.20599)
*Atkia Mahila,Avinash Maurya,M. Mustafa Rafique,Bogdan Nicolae*

Main category: cs.AI

TL;DR: 本文系统评估了两种树搜索方法（DPTS和SSDP）在不同计算预算、模型规模和问题难度下的表现，发现DPTS在低预算时存在冷启动瓶颈，而SSDP则面临前沿耗尽问题，两者都无法在计算资源连续变化的情况下保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: 树搜索方法在提升大语言模型推理能力方面前景广阔，但不同搜索策略在不同计算预算、模型规模和问题难度下的行为缺乏系统性的研究。

Method: 在Math500和GSM8K两个数学推理基准上，使用Llama-3B和Llama-8B两种模型，在3k-10k四种token预算下，评估了基于蒙特卡洛树搜索的DPTS方法和基于语义去重的SSDP方法。

Result: DPTS在低预算时因需要充分探索才能获得可靠价值估计而存在冷启动瓶颈，但高预算下扩展性良好；SSDP能高效找到候选解，但由于激进的节点合并导致前沿耗尽，无法利用剩余预算进一步改进。

Conclusion: 固定探索策略和固定剪枝策略都无法适应计算资源的连续变化，有效的科学推理搜索需要能够根据搜索进展和可用资源自适应调整的策略。
...
### [285] [The New Associationism: Lessons from Deep Learning](https://arxiv.org/abs/2606.20600)
*Daniel Rothschild*

Main category: cs.AI

TL;DR: 现代AI的成功支持了一种温和的联想主义学习观：监督学习（基于评价反馈的学习）是多种AI系统的共同核心机制，验证了联想主义关于统一、渐进、误差驱动的学习机制跨领域适用的理念，同时深度学习架构也超越了经典联想主义的设想。


<details>
  <summary>Details</summary>
Motivation: 探讨现代AI的成功对人类学习机制的启示，回应"联想主义机制过于局限，无法解释人类认知能力"这一曾颇具影响力的批评。

Method: 通过分析当代AI系统（从大语言模型到游戏智能体）中的监督学习机制，论证其作为统一学习模式的核心地位。

Result: 监督学习是多种现代AI系统的共同基础，验证了联想主义的核心理念；但深度学习架构已超越经典联想主义的设想范围。

Conclusion: 现代AI支持一种温和的联想主义：监督学习作为核心机制是合理的，但需结合更复杂的计算架构，而非作为学习的完整解释。
...
### [286] [Specifying AI-SDLC Processes: A Protocol Language for Human-Agent Boundaries](https://arxiv.org/abs/2606.20615)
*Ylli Prifti*

Main category: cs.AI

TL;DR: 提出一种用于规范AI-SDLC（人工智能软件开发生命周期）过程的领域特定语言，通过形式化语法、良好形态条件和操作语义来定义人机协作的责任边界、审批门限和治理约束。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理已成为软件开发流程的一等成员，但缺乏规范人机协作责任边界、审批门限和治理约束的规范语言。现有方法要么将流程编码在提示中（易漂移），要么针对相邻领域（工作流管理、业务流程），或只解决部分问题（访问控制、审批门限）。

Method: 设计一种领域特定语言，将AI-SDLC过程指定为协议，具有形式化语法、良好形态条件、操作语义和执行不变式。该语言区分策略（声明意图）和机制（结构执行），通过验证令牌和能力边界等原语约束流程非确定性。

Result: 1）故障率分析显示结构执行可将系统故障率限制在代理和验证器故障率的加权乘积内，而行为合规性则允许累积或近饱和增长；2）提出2+N团队模式（两个人类控制角色加N个专业代理成员），形式化了AI-SDLC的经典职责分离；3）协调循环的Kleene闭包和反射协议遵从性验证成为设计属性而非特殊情况结构。

Conclusion: 该工作的创新在于将多智能体框架、工作流规范和基于能力的安全性等概念进行特定整合，而非创造单一原语。提供了可行性实现，实证评估是未来工作方向。该语言为人机协作软件开发流程提供了正式规范基础。
...
### [287] [PEAR: Permutation-Equivariant Adaptive Routing Multi-Agent Debate](https://arxiv.org/abs/2606.20621)
*Yang Feng,Ziwei Xu,Xia Hu,Fengxiang He*

Main category: cs.AI

TL;DR: PEAR是一种排列等变自适应路由多智能体辩论协议，通过动态重新配置通信角色和稀疏拓扑来克服固定拓扑带来的位置偏差和角色敏感性问题，在多个推理基准上显著提升了LLM的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体辩论方法采用固定拓扑结构，导致持续的位置偏差、不可靠智能体的放大效应以及对角色分配的高度敏感性问题。

Method: 提出排列等变自适应路由多智能体辩论协议（PEAR），在连续辩论轮次中动态重新配置通信角色和稀疏拓扑，根据智能体状态变化战略性地切换智能体到角色的分配。

Result: 在四个推理基准和六个不同的LLM骨干网络上，PEAR相比最强的辩论基线显著提高了平均准确率。

Conclusion: PEAR作为一种等变稀疏路由器，在保持智能体重标号下的准确性的同时，降低了路由复杂度并提升了泛化能力，有效解决了多智能体辩论中固定拓扑的固有问题。
...
### [288] [Darwin Mobile Agent: A Roadmap for Self-Evolution](https://arxiv.org/abs/2606.20622)
*Daniel Beechey,Derek Yuen,Jianheng Liu,Dezhao Luo,Tiantian He,Weilin Luo,Jun Wang,Kun Shao*

Main category: cs.AI

TL;DR: 本文提出通过移动GUI作为"大世界"代理，构建Darwin Mobile Agent基础设施，通过异步云手机环境实现自主强化学习，并规划了去除人工先验的路线图，为自进化GUI代理奠定基础。


<details>
  <summary>Details</summary>
Motivation: 受"苦涩教训"启发，认为实现通用人工智能的最有效途径是系统性地去除人工先验，让智能在与复杂环境的交互中自然涌现。现有方法依赖大量人工标注和数据收集，存在瓶颈。

Method: 提出移动GUI作为"大世界"代理；构建Darwin Mobile Agent开源基础设施，采用异步智能体-环境循环跨并行云手机实例运行，解决数据收集瓶颈；规划去除任务课程、结果验证、内存管理三大支柱中人工先验的路线图。

Result: 验证了Darwin基础设施在GUI策略优化阶段所需的稳定性和可扩展性，为自主、自进化GUI代理的第一阶段提供了实践和理论基础。

Conclusion: 该工作建立了迈向真正自主、自进化GUI代理所需的实践与理论基础，通过去人工先验的方式推动通用人工智能的发展。
...
### [289] [Path-dependent program induction under resource constraints explains human sequence learning](https://arxiv.org/abs/2606.20623)
*Hanqi Zhou,David G. Nagy,Peter Dayan,Charley M. Wu*

Main category: cs.AI

TL;DR: 该研究通过结合率失真理论与程序归纳，构建了分层适配语法模型，解释人们在有限认知资源下如何从序列经验中构建抽象可重用知识。


<details>
  <summary>Details</summary>
Motivation: 探索人们在有限认知资源下如何从序列经验中构建抽象、可重用的知识，以及先验知识如何影响未来结构的编码和发现。

Method: 整合率失真理论与程序归纳最新进展，构建分层适配语法（HAG）模型，包含局部（任务内）和全局（任务间）库，并受记忆和计算约束共同控制。

Result: 在模拟中，HAG比固定语法或浅层分块方法实现更好的率失真权衡和更强的泛化能力。在在线旋律序列学习实验中，参与者的回忆错误反映了系统性简化，反应时间在推断的程序边界处增加。逐试次拟合显示分层库最能解释回忆和样本外延续选择中的个体差异，优于所有替代模型。

Conclusion: 这些发现将结构化学习视为有界程序归纳，其中经验顺序塑造学习者构建的未来抽象。
...
### [290] [In LLM Reasoning, there is Irrationality on top of Value Misalignment](https://arxiv.org/abs/2606.20624)
*Kejiang Qian,Fengxiang He*

Main category: cs.AI

TL;DR: 即使在大语言模型经过良好对齐训练后，在推理过程中仍可能无法最大化对齐的价值函数。论文将这种差距形式化为"理性价值风险"，即实际部署的推理策略与理性对应策略之间的效用差距。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在（后）训练中已经与目标价值函数良好对齐，但在实际推理过程中仍可能无法最大化这些对齐的价值。这种差距表明对齐训练并不保证在推理时的理性行为，需要对这一现象进行数学形式化研究。

Method: 1. 提出"理性价值风险"的数学形式化定义：模型部署的推理策略与其理性对应策略（在最陡峭方向上最大化期望效用的响应）之间的效用差距
2. 将理性价值风险的估计误差分解为三个组成部分：有限候选响应、有限提示和不完美验证器
3. 在多个模型（Llama-3.1、Qwen-2.5、Tulu-3系列、GPT-5.2、GPT-5.5、DeepSeek-V4）和多个基准（UltraFeedback、AlpacaEval、GSM8K、MATH、HumanEval、MathArena）上进行广泛实验验证

Result: 实验验证了四个主要发现：1）理性价值风险普遍存在；2）价值对齐可以减少但无法消除这种风险；3）风险对推理时的推理策略高度敏感；4）更长的推理会提高理性但收益递减

Conclusion: 论文强调了即使经过良好对齐训练的大语言模型，在推理阶段仍存在理性价值风险。这种风险无法通过价值对齐完全消除，而是受到推理策略的显著影响。研究为理解和改善大语言模型的理性行为提供了理论框架和实证基础。
...
### [291] [AlphaMemo: Structured Search-Process Memory for Self-Evolving Alpha Mining Agents](https://arxiv.org/abs/2606.20625)
*Hang Yu,Zifan Zheng,Jeff Z. Pan,Tongliang Liu,Zhiyong Wang,Fengxiang He*

Main category: cs.AI

TL;DR: AlphaMemo提出了一种结构化搜索-过程记忆机制的自进化因子挖掘智能体，通过记录AST差异中的编辑模式、置信度门控残差记忆和非对称否决控制，有效提升了因子挖掘的效率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM因子挖掘智能体面临组合搜索空间巨大、非平稳噪声反馈、冗余发现以及过度拟合等多重挑战，简单复用历史成功经验容易导致过拟合。

Method: 提出AlphaMemo框架，记录AST差异中的可复用编辑模式，采用置信度门控残差记忆叠加在搜索分类账先验之上，并使用非对称否决控制来抑制高置信度的失败模式。

Result: 在CSI 500和S&P 500上的实验表明，该方法在样本外性能和固定预算发现效率上均有提升，消融实验验证了残差学习、置信度门控、AST差异模式和否决记忆各模块的有效性。

Conclusion: AlphaMemo通过结构化搜索-过程记忆机制有效解决了LLM因子挖掘中的关键挑战，是一种自进化的高效因子挖掘方法。
...
### [292] [Latent Goal Prediction from Language for Model-Based Planning](https://arxiv.org/abs/2606.20627)
*Samuel Barbeau,Simon Roy,Giovanni Beltrame,Christian Desrosiers,Nicolas Thome*

Main category: cs.AI

TL;DR: LAGO框架通过语言指令预测中间潜态子目标，实现在同一潜空间中进行长期规划，避免了以往方法在长时域规划中的性能急剧下降问题。


<details>
  <summary>Details</summary>
Motivation: 基于世界模型的规划面临两个瓶颈：预测误差的累积和优化目标的定义困难。视觉目标提供精确的局部梯度但缺乏远距离指导，语言具有灵活性但受限于噪声跨模态对齐或依赖不适合基于模型规划高采样特性的大模型。

Method: 提出LAGO框架，从语言指令预测中间目标状态序列和动作条件轨迹，所有操作都在同一潜空间进行。动态将指令分解为显式预测的、局部可处理的潜态子目标，通过在线更新这些子目标并在规划中使用软最小轨迹成本。

Result: 在多个环境和规划时域的评估表明，LAGO避免了先前方法的急剧性能退化，实现了仅从语言出发的鲁棒且精确的长期规划。

Conclusion: LAGO通过潜态目标预测，弥合了视觉目标的精确性和文本引导控制的灵活性，为纯语言驱动的鲁棒长时域规划提供了有效解决方案。
...
### [293] [Human Decision-Making with AI Assistance under Correlated Features](https://arxiv.org/abs/2606.20628)
*Yanru Guan,Naveen Raman,Fei Fang*

Main category: cs.AI

TL;DR: 论文研究AI辅助决策中如何平衡短期决策质量和长期人类学习的问题，特别关注特征相关性对测试推荐策略的影响，提出了探索-承诺策略框架和相应算法。


<details>
  <summary>Details</summary>
Motivation: 在AI辅助人类决策的场景中（如医生诊断），AI需要推荐诊断测试，但面临着平衡短期决策质量与长期人类学习的矛盾。特别是当不同特征（测试结果）之间存在相关性时，传统的固定策略可能效果不佳，需要研究如何设计最优的测试推荐策略。

Method: 采用理论分析和算法设计相结合的方法。首先证明特征相关性导致固定策略性能可能任意差，然后证明最优策略必须遵循探索-承诺结构。针对计算最优策略的NP-hard问题，开发了基于动态规划的有限时间算法，并提出了通过缩短规划时域和添加稳态后缀的近似方法。

Result: 理论证明：特征相关性导致固定策略性能任意差，最优策略必须是探索-承诺结构，探索长度取决于特征相关程度。算法方面：动态规划算法能找到有限时域最优策略，近似方法能获得接近最优的性能。实证结果表明：更强的特征相关性导致更长的探索阶段。

Conclusion: 在AI辅助决策中，当特征存在相关性时，推荐策略需要从简单的固定策略转变为具有探索阶段的动态策略。探索-承诺框架提供了理论上的最优结构，而提出的算法和近似方法为实际应用提供了可行的解决方案，平衡了短期决策质量和长期人类学习的目标。
...
### [294] [Harnessing Agent Skills: Architectural Patterns and a Reference Architecture for Skill-Mediated LLM Agents](https://arxiv.org/abs/2606.20631)
*Boming Xia,Liming Zhu,Zhenchang Xing,Qinghua Lu,Dino Sejdinovic,Xiwei Xu*

Main category: cs.AI

TL;DR: 本文提出技能利用（skill harnessing）的概念，指从技能工件到技能使用（skill-in-use）的架构职责，并总结了十个经验驱动的模式和一个四层参考架构（供应链、中介、执行控制、证据与反馈），通过8个系统跨实例化评估，为分析智能体系统的技能利用职责提供了词汇和诊断框架。


<details>
  <summary>Details</summary>
Motivation: 智能体技能工件在静态时是存储的，但实际使用时涉及选择、绑定上下文与权限、被随机智能体解释并记录为运行证据。现有研究缺乏对这种"技能使用"（skill-in-use）关系的系统性架构职责分析。

Method: 通过经验驱动的方式归纳十个架构模式（五个核心、五个支撑），并将其综合为四层参考架构（供应链、中介、执行控制、证据与反馈），最后在8个选定的系统中进行跨实例化评估。

Result: 提出了十个经验驱动的架构模式和一个四层参考架构，并在8个系统中验证了其跨实例化的有效性。

Conclusion: 所提出的模式和参考架构为分析跨智能体系统的技能利用职责提供了统一的词汇表和诊断框架。
...
### [295] [DEMM-Bench: A Cross-Regime Benchmark for Agent-Runtime Governance-Evidence Sufficiency](https://arxiv.org/abs/2606.20634)
*Oleg Solozobov*

Main category: cs.AI

TL;DR: DEMM-Bench是一个跨体制的智能体运行时治理证据充分性基准，通过决策证据成熟度模型（DEMM）衡量八个证据体制的记录是否足以重构决策级属性，实验表明现有基线存在过度声明问题，而所提属性级候选评分器在零过度声明下达到56.25%的平均属性充分性准确率。


<details>
  <summary>Details</summary>
Motivation: 现有智能体运行时系统会生成追踪、账本、溯源图等多种记录，但这些记录容器并不一定能回答关于特定决策的治理问题，缺乏评估决策证据充分性的标准化方法。

Method: 提出DEMM-Bench基准，基于决策证据成熟度模型（DEMM），通过适配器标准化八个证据体制，对参与者、权限、动作、策略等八个决策级属性提问，并应用八种确定性退化条件进行评估。

Result: 在64个手稿案例上，trace-present和schema-present基线在75%案例上过度声明，ledger-present在50%上过度声明，而redacted属性级候选评分器零过度声明，平均属性充分性准确率为56.25%。

Conclusion: DEMM-Bench提供了标准化数据集和可复现评估框架，支持在异构智能体运行时证据基板上进行决策证据成熟度的可复现评估。
...
### [296] [SkillHarness: Harnessing Safe Skills for Computer-Use Agents](https://arxiv.org/abs/2606.20636)
...
### [297] [Constituency Optimisation Through Hamiltonian Representation Of Mandates (COTHROM): Algorithmic Redistricting of Irish Election Boundaries](https://arxiv.org/abs/2606.20637)
...
### [298] [RIZZ: Routing Interactions to Near Zero-Interference Zones for Continual Adaptation of Black-Box Agents](https://arxiv.org/abs/2606.20638)
...
### [299] [An LLM-Explainable DRL Framework for Passenger-Directed Autonomous Driving](https://arxiv.org/abs/2606.20640)
...
### [300] [Hypothesis-Disciplined Multi-Agent Automated Formalization of Asymptotic Statistical Theory](https://arxiv.org/abs/2606.20642)
...
### [301] [SPARC: A Multi-Agent System for Electrical Circuit Question Answering](https://arxiv.org/abs/2606.20643)
...
### [302] [Bridging Multi-Valued Heuristics and Dimensionality Reduction in Multi-Objective Search](https://arxiv.org/abs/2606.20644)
...
### [303] [Learning Splitting Heuristics for Parallel String Solvers](https://arxiv.org/abs/2606.20656)
...
### [304] [A-Evolve-Training: Autonomous Post-Training of a 30B Model](https://arxiv.org/abs/2606.20657)
...
### [305] [Expected Free Energy-based Planning as Variational Inference](https://arxiv.org/abs/2606.20658)
...
### [306] [Skill Coverage: A Test Adequacy Metric for Agent Skills](https://arxiv.org/abs/2606.20659)
...
### [307] [From Knowing to Acting: Benchmarking Self-Awareness Capability of LLM Agents](https://arxiv.org/abs/2606.20661)
...
### [308] [Confidence Laundering in Agent Systems: Why Uncertainty Needs a Latent Carrier](https://arxiv.org/abs/2606.20662)
...
### [309] [DrugBench: Evaluating AI Control Protocols for Medication Harm Mitigation](https://arxiv.org/abs/2606.20663)
...
### [310] [A Quantum-Assisted Agentic Distributed Artificial Intelligence Framework for Deadline-Bounded Orchestration of Hybrid Renewable Microgrids](https://arxiv.org/abs/2606.20667)
...
### [311] [Agent Behavior Mining: Generative AI Agent Governance in Business Processes](https://arxiv.org/abs/2606.20669)
...
### [312] [Democratizing and accelerating AI-driven pathology research through agentic intelligence](https://arxiv.org/abs/2606.20677)
...
### [313] [From Question Answering to Task Completion: A Survey on Agent System and Harness Design](https://arxiv.org/abs/2606.20683)
...
### [314] [Artificial Intelligence as Monism: Ontological, Organisational, and Methodological Implications](https://arxiv.org/abs/2606.20704)
*Bertrand K. Hassani*
...
### [315] [Simulated Customers Never Walk Away: Decision Fidelity of LLM User Simulators Measured Against Real Purchase Outcomes](https://arxiv.org/abs/2606.20708)
*Liang Chen*
...
### [316] [FairTutor: Equity-Aware Pedagogical LLM Routing for Budget-Constrained AI Tutoring](https://arxiv.org/abs/2606.20713)
*Qingyang Xu*
...
### [317] [Escape from Delusional Echo Trap: Symmetry Breaking, Stochastic Dynamics and Mathematical Mitigation Strategies for Algorithmic Sycophancy](https://arxiv.org/abs/2606.20718)
...
### [318] [When Web Agents Finish but Still Fail: Reproducible Triggers and Trace Diagnostics for Parallel Web Exploration](https://arxiv.org/abs/2606.20724)
...
### [319] [Repeated Shared Access Enables Grokking, but Edit Propagation Depends on a Fine-Grained Addressable Memory](https://arxiv.org/abs/2606.20737)
*Yanan Niu*
...
### [320] [Fara-1.5: Scalable Learning Environments for Computer Use Agents](https://arxiv.org/abs/2606.20785)
...
### [321] [What Shapes Emergent Misalignment? Insights from Training Dynamics, Model Priors, and Data](https://arxiv.org/abs/2606.20814)
...
### [322] [Study on Quantitative Dynamic Epistemic Logic for Belief Revision](https://arxiv.org/abs/2606.20837)
...
### [323] [Process-Reward Tactic Evolution for Long-Horizon Bioinformatics Workflows](https://arxiv.org/abs/2606.20839)
...
### [324] [SignVLA: Real-Time Sign Language-Guided Robotic Manipulation via Attention LSTM and Vision-Language-Action Models](https://arxiv.org/abs/2606.20857)
...
### [325] [When Do Intrinsic Rewards Work for Code Reasoning? A Comprehensive Study](https://arxiv.org/abs/2606.20881)
...
### [326] [Neurosymbolic Clinical Trial Matching via LLM-Driven Abduction and Logical Verification](https://arxiv.org/abs/2606.20895)
...
### [327] [Root Cause Analysis with Latent Confounders using Partial Ancestral Graphs](https://arxiv.org/abs/2606.20912)
...
### [328] [Power Systems Agent Benchmark: Executable Evaluation of AI Agents in Electric Power Engineering](https://arxiv.org/abs/2606.20950)
*Sergei Trashchenkov*
...
### [329] [Generative Responsible AI Data Evaluation Schema (GRAIDES) for AI Assurance in Local Government](https://arxiv.org/abs/2606.20963)
...
### [330] [AutoACSL: Synthesizing ACSL Specifications by Integrating LLMs with CPG-Based Static Analysis](https://arxiv.org/abs/2606.20969)
...
### [331] [How Should Agents Read Demonstrations? Hierarchical Structure Beats Flat Action Logs](https://arxiv.org/abs/2606.20978)
...
### [332] [BioInsight: Multi-Agent Orchestration for Interactive Biomedical Knowledge Discovery](https://arxiv.org/abs/2606.20997)
...
### [333] [Building Agent Harnesses for Scientific Curation from Multimodal Sources](https://arxiv.org/abs/2606.21005)
...
### [334] [Closure of Self-Determining System Based on Causal and Constitutive Relations](https://arxiv.org/abs/2606.21010)
...
### [335] [Agentic Time Machine as an Infrastructure for Future-Event Forecasting](https://arxiv.org/abs/2606.21013)
...
### [336] [The AI Evaluability Gap: The Missing Layer for Managing Risk and Sustaining Value](https://arxiv.org/abs/2606.21015)
...
### [337] [Negative Knowledge as Failure-aware Shared Memory for AutoResearch](https://arxiv.org/abs/2606.21024)
*Hanchun Wang*
...
### [338] [Coherence Under Commitment: Probing Generalization and Vacuous Memorization in LLM Logical Reasoning](https://arxiv.org/abs/2606.21083)
...
### [339] [Repeated post-training is not Self-improving: Diagnosing Scientific Amnesia in Continual DPO Pipelines](https://arxiv.org/abs/2606.21089)
...
### [340] [Self-Improvement Can Self-Regress: The Rise-and-Collapse Failure Mode of LLM Self-Training](https://arxiv.org/abs/2606.21090)
*Jianzhe Lin*
...
### [341] [Answer Engineering: Local Trajectory Editing for Protocol-Constrained Decision Making in Large Language Models](https://arxiv.org/abs/2606.21121)
...
### [342] [PulseCX: Breaking the Closed-World Assumption in Real-Time CX](https://arxiv.org/abs/2606.21124)
...
### [343] [Learning Burst-Aware Early Warning Models for Capacity Stress under AI Workload Surges in Hyperscale Data Centers](https://arxiv.org/abs/2606.21130)
...
### [344] [Trip+: Benchmarking Agents in Personalized Interactive Travel Planning](https://arxiv.org/abs/2606.21169)
...
### [345] [Whistleblowing and the machine -- towards a considered position](https://arxiv.org/abs/2606.21201)
...
### [346] [ARCO: Adaptive Rubric with Co-Evolution for Multi-Step LLM-Based Agents](https://arxiv.org/abs/2606.21262)
...
### [347] [Towards Dys-XAI: Influence-Based Explanations for Dysarthria Severity Assessment](https://arxiv.org/abs/2606.21306)
...
### [348] [Social World Model for Lifelong Social Intelligence](https://arxiv.org/abs/2606.21315)
*Yu Luo*
...
### [349] [Mind the Noise: Sensitivity of Transformer-based Interaction-Aware Trajectory Prediction Models to Noisy Data](https://arxiv.org/abs/2606.21344)
...
### [350] [Calibration Is Not Control: Why LLM-Agent Oversight Needs Intervention](https://arxiv.org/abs/2606.21399)
...
### [351] [Don't Blindly Trust It: How Unreliable Feedback Breaks Tool-Using LLM Agents](https://arxiv.org/abs/2606.21409)
...
### [352] [AutoRAS: Learning Robust Agentic Systems with Primitive Representations](https://arxiv.org/abs/2606.21445)
...
### [353] [Towards Transparent Mental Health Insights: An Explainable AI Model for Career-Related Depression and Anxiety Among University Students Using Structured Data](https://arxiv.org/abs/2606.21474)
...
### [354] [Balancing Performance and Diversity in GRPO Autoregressive Text-to-Image Post-Training](https://arxiv.org/abs/2606.21498)
...
### [355] [AI Alignment From Social Choice Perspectives](https://arxiv.org/abs/2606.21550)
...
### [356] [Composing Verifiable Conceptual Models via Building Blocks: Towards Design-Time Verification of Agentic AI Workflows](https://arxiv.org/abs/2606.21565)
...
### [357] [Counsel: A Meta-Evaluation Dataset for Agentic Tasks](https://arxiv.org/abs/2606.21627)
...
### [358] [ChainWorld: Composing Long-Horizon Desktop Workloads from Atomic OSWorld Tasks](https://arxiv.org/abs/2606.21654)
...
### [359] [Hallucination as Context Drift: Synchronization Protocols for Multi-Agent LLM Systems](https://arxiv.org/abs/2606.21666)
*Carson Rodrigues*
...
### [360] [Entropy Objectives in Markov Decision Processes](https://arxiv.org/abs/2606.21726)
...
### [361] [Training the Orchestrator: A Supervised Approach to End-to-End PDDL Planning with LLM Agents](https://arxiv.org/abs/2606.21740)
...
### [362] [AgentCAT: Simulating Computerized Adaptive Testing via Multi-Agent Large Language Models](https://arxiv.org/abs/2606.21832)
...
### [363] [Measuring What Persists: Conditioning Mechanisms and a Geometric Framework for AI Agent Identity](https://arxiv.org/abs/2606.21843)
*Andrew Tanner*
...
### [364] [ForEx: A Formal Verification Framework for Explainable Reasoning in Logical Fallacy Detection and Annotation](https://arxiv.org/abs/2606.21867)
...
### [365] [AgentRiskBOM: A Risk-Scoping Security Bill of Materials for Agentic AI Systems](https://arxiv.org/abs/2606.21877)
...
### [366] [Learning the ARTS of Search for Automated Discovery](https://arxiv.org/abs/2606.21891)
...
### [367] [Holmes: Multimodal Agentic Diagnosis for Mixed-Language Mobile Crashes at Industrial Scale](https://arxiv.org/abs/2606.21963)
...
### [368] [REBA: A Revealed Belief Automaton Framework for Online Planning in Continuous POMDPs](https://arxiv.org/abs/2606.21971)
...
### [369] [Human vs Machine Mathematical Difficulty on Project Euler: An Experimental Analysis](https://arxiv.org/abs/2606.21972)
...
### [370] [IRumAI: Reinforcement Learning for Indian Rummy](https://arxiv.org/abs/2606.21975)
*Vignesh Mohan*
...
### [371] [CFAgentBench: A Reproducible Environment and Benchmark for Autonomous Construction-Finance Agents](https://arxiv.org/abs/2606.22000)
*Rishi Srivastava*
...
### [372] [Nous: A Predictive World Model for Long-Term Agent Memory](https://arxiv.org/abs/2606.22030)
*Pranav Singh*
...
### [373] [Attractor Domain Theory: A Mathematical Framework for Cardiovascular Attractor Analysis with Wearable Photoplethysmography (PPG) Validation](https://arxiv.org/abs/2606.22039)
...
### [374] [When Does a Video-Language Model Stop Watching? Reward Strength Controls the Formation and Reversal of Visual Shortcuts in Multimodal RLVR](https://arxiv.org/abs/2606.22043)
*Zekun Xu*
...
### [375] [Can Reasoning Models Detect Changes to their Chains of Thought?](https://arxiv.org/abs/2606.22085)
...
### [376] [Geometry-Aware Online Scheduling for LLM Serving: From Theoretical Bound to System Practice](https://arxiv.org/abs/2606.22327)
...
### [377] [Hypothesis-Driven Skill Optimization for LLM Agents](https://arxiv.org/abs/2606.22330)
...
### [378] [Reference-Free Assessment of Physical Consistency in World Model-based Video Generation](https://arxiv.org/abs/2606.22363)
*Yun Oh,Sukmin Yun*
...
### [379] [ARIA: A Causal-Aware Framework for Rescuing LLM Reasoning in Trustworthy Materials Discovery](https://arxiv.org/abs/2606.22375)
...
### [380] [MetaPS: Adaptive Programmatic Strategy Selection for Market Agents](https://arxiv.org/abs/2606.22385)
...
### [381] [PlanBench-XL: Evaluating Long-Horizon Planning of LLM Tool-Use Agents in Large-Scale Tool Ecosystems](https://arxiv.org/abs/2606.22388)
...
### [382] [Code Isn't Memory: A Structural Codebase Index Inside a Coding Agent](https://arxiv.org/abs/2606.22417)
...
### [383] [SVGym (SciVerseGym): An Environment for Reinforcement Learning and Bayesian Optimization in Crystal Discovery](https://arxiv.org/abs/2606.22425)
*Bin Cao*
...
### [384] [Efficient Multimodal Clinical Question Answering for Pulmonary Embolism Risk Assessment](https://arxiv.org/abs/2606.22442)
...
### [385] [A Differentiable Atari VCS:A Complex, Fully Known Ground Truth for Explainable AI](https://arxiv.org/abs/2606.22447)
...
### [386] [Self-Evolving Cognitive Framework via Causal World Modeling for Embodied Scientific Intelligence](https://arxiv.org/abs/2606.22449)
...
### [387] [PRIME: Evaluating Prompt Resolution Under Incompatible Instructions in LLMs](https://arxiv.org/abs/2606.22470)
...
### [388] [VADAOrchestra: Neurosymbolic Orchestration of Adaptive Reasoning Workflows](https://arxiv.org/abs/2606.22485)
...
### [389] [SCOPE: Evolving Symbolic World for Planning in Open-Ended Environments](https://arxiv.org/abs/2606.22488)
...
### [390] [Deep Learning-Based Sign Language Recognition from Videos and Cross-Lingual Translation to Indian Vernaculars](https://arxiv.org/abs/2606.22494)
...
### [391] [Grounded Scaling: Why Agentic AI Needs Deterministic Environments](https://arxiv.org/abs/2606.22495)
...
### [392] [Imagine to Ensure Safety in Hierarchical Reinforcement Learning](https://arxiv.org/abs/2606.22509)
...
### [393] [Governance Decay: How Context Compaction Silently Erases Safety Constraints in Long-Horizon LLM Agents](https://arxiv.org/abs/2606.22528)
*Shiyang Chen*
...
### [394] [MacAgentBench: Benchmarking AI Agents on Real-World macOS Desktop](https://arxiv.org/abs/2606.22557)
...
### [395] [Text2DSL: LLM-Based Code Generation for Domain-Specific Languages](https://arxiv.org/abs/2606.22586)
...
### [396] [PaperClaw: Harnessing Agents for Autonomous Research and Human-in-the-Loop Refinement](https://arxiv.org/abs/2606.22610)
...
### [397] [SkillAudit: From Fixed-Suite Benchmarking to Skill-Centered Assessment](https://arxiv.org/abs/2606.22613)
...
### [398] [Confident but Conflicted: Internal Uncertainty and Cognitive Dissonance Resolution in LLMs](https://arxiv.org/abs/2606.22633)
...
### [399] [AgentLens: Interpretable Safety Steering via Mechanistic Subspaces for Multi-Turn Coding Agent](https://arxiv.org/abs/2606.22673)
...
### [400] [Skin-Deep: A Geometric Diagnostic for Alignment Fragility in Large Language Model Representations](https://arxiv.org/abs/2606.22676)
...
### [401] [VISTA Architect: A graph database-oriented health AI system demonstrated in multidisciplinary tumor boards](https://arxiv.org/abs/2606.22692)
...
### [402] [Safety-Aware Evaluation of LLM-Generated Driver Intervention Messages through Multi-Task Risk Fusion](https://arxiv.org/abs/2606.22706)
*Keito Inoshita*
...
### [403] [Beyond Penalizing Mistakes: Stabilizing Efficiency Training in Large Reasoning Models via Adaptive Correct-Only Rewards](https://arxiv.org/abs/2606.22716)
...
### [404] [Text Dictates, Music Decorates: Energy-based Attention for Editable Dance Motion Generation](https://arxiv.org/abs/2606.22726)
...
### [405] [Closed-loop Auto Research for Molecular Property Prediction: Discovering and Certifying Generalizable Improvements](https://arxiv.org/abs/2606.22731)
...
### [406] [GroundEval: A Deterministic Replacement for LLM-as-Judge in Stateful Agent Evaluation](https://arxiv.org/abs/2606.22737)
*Jeffrey Flynt*
...
### [407] [Learning Filters with Certainty](https://arxiv.org/abs/2606.22786)
...
### [408] [The Origins of Stochasticity: Comprehensive Investigations on Uncertainty Quantification for Large Language Models](https://arxiv.org/abs/2606.22792)
...
### [409] [A Formula-Driven Survey and Research Agenda for On-Policy Distillation](https://arxiv.org/abs/2606.22793)
*Bowen Zhang*
...
### [410] [Measuring Behavior Portability in Large Language Models](https://arxiv.org/abs/2606.22797)
...
### [411] [AI-Assisted Help-Seeking Trajectories in Programming Education from an SRL-Informed Perspective](https://arxiv.org/abs/2606.22809)
...
### [412] [Active Inference as the Test-Time Scaling Law for Physical AI Agents](https://arxiv.org/abs/2606.22813)
...
### [413] [MINCE: Shrinking LLM Evaluation Datasets via Few-Model Monte Carlo Calibration](https://arxiv.org/abs/2606.22826)
...
### [414] [Finding the Evidence: Discovering Decision-Supporting Tokens for On-Policy Reasoning Distillation](https://arxiv.org/abs/2606.22830)
...
### [415] [RaMem: Contextual Reinstatement for Long-term Agentic Memory](https://arxiv.org/abs/2606.22844)
...
### [416] [AI Scientists as Engines of Discovery: A Case for Development within Reformed Institutions](https://arxiv.org/abs/2606.22859)
...
### [417] [CLI-Universe: Towards Verifiable Task Synthesis Engine for Terminal Agents](https://arxiv.org/abs/2606.22883)
...
### [418] [Agent-as-a-Router: Agentic Model Routing for Coding Tasks](https://arxiv.org/abs/2606.22902)
...
### [419] [ThermoLLM: Thermodynamics-Aware HVAC Control with Spatial-Semantic Knowledge Graph](https://arxiv.org/abs/2606.22911)
...
### [420] [Intent-Governed Tool Authorization for AI Agents](https://arxiv.org/abs/2606.22916)
...
### [421] [When Agents Commit Too Soon: Diagnosing Premature Commitment in LLM Agents](https://arxiv.org/abs/2606.22936)
*Aman Mehta*
...
### [422] [ENVS: Environment-Native Verified Search for Long-Horizon GUI Agents](https://arxiv.org/abs/2606.22948)
...
### [423] [Plans Don't Persist: Why Context Management Is Load Bearing for LLM Agents](https://arxiv.org/abs/2606.22953)
...
### [424] [The Impact of VAE Design on Latent Pose Representations for Diffusion-based Sign Language Production](https://arxiv.org/abs/2606.22959)
...
### [425] [When Preferences Fail to Become Incentives: A Utility-Behavior Gap in Large Language Models](https://arxiv.org/abs/2606.22974)
...
### [426] [Joint Air Traffic Flow and Capacity Management via Answer Set Programming](https://arxiv.org/abs/2606.22978)
...
### [427] [A Stackelberg Framework for Resource-Aware LLM Agents: Learning, Repair, and Conditional Guarantees](https://arxiv.org/abs/2606.23026)
*Baoxun Wang*
...
### [428] [From numerical proportions to analogical proportions between probabilities](https://arxiv.org/abs/2606.23029)
...
### [429] [IPO Finance Agent: Evaluation of LLM Financial Analysts beyond Finance Agent v2, with Automated Rubric Generation -- the Case of the SpaceX (SPCX) IPO](https://arxiv.org/abs/2606.23032)
*Mostapha Benhenda*
...
### [430] [Some Results about the Expressivity of Preference-Incomplete Structured Argumentation Frameworks](https://arxiv.org/abs/2606.23055)
*Antonio Yuste-Ginel*
...
### [431] [Cognitive Digital Twins: Ethical Risks and Governance for AI Systems That Model the Mind](https://arxiv.org/abs/2606.23094)
...
### [432] [TTFT-Aware Graph Chain-of-Thought:Distance-Indexed Neural A* for Low-Hallucination Multi-Hop Medical Reasoning](https://arxiv.org/abs/2606.23108)
...
### [433] [A Matter of Time: Towards a General Theory of Agency](https://arxiv.org/abs/2606.23122)
...
### [434] [Managing Procedural Memory in LLM Agents: Control, Adaptation, and Evaluation](https://arxiv.org/abs/2606.23127)
...
### [435] [Decomposing Financial Market Dynamics via Mechanism Analysis in an Evolutionary Multi-Agent Simulation](https://arxiv.org/abs/2606.23158)
*Zhibao Chen*
...
### [436] [DART: Draft-Agreement Routing for Training-Free Adaptive Thinking Budgets in Hybrid Reasoning Models](https://arxiv.org/abs/2606.23181)
...
### [437] [Capable but Careless: Do Computer-Use Agents Follow Contextual Integrity?](https://arxiv.org/abs/2606.23189)
...
### [438] [SPADE: Structure-Prior Adaptive Decision Estimation](https://arxiv.org/abs/2606.23219)
*Yifan Wang*
...
### [439] [HOLMES: Evaluating Higher-Order Logical Reasoning in LLMs](https://arxiv.org/abs/2606.23238)
...
### [440] [GIF: Locally Sound Geometric Information Flow Control for LLMs](https://arxiv.org/abs/2606.23277)
...
### [441] [EHR-Complex: Benchmarking Medical Agents for Complex Clinical Reasoning](https://arxiv.org/abs/2606.23301)
...
### [442] [Abstract representational geometry supports inference in large language models](https://arxiv.org/abs/2606.23345)
...
### [443] [Litmus: Zero-Label, Code-Driven Metric Specification for Evaluating AI Systems](https://arxiv.org/abs/2606.23403)
...
### [444] [Digital Humanism and Evolutionary Design](https://arxiv.org/abs/2606.23417)
*Wolfgang Höhl*
...
### [445] [Cross-Architectural Mixture-of-Experts with Adaptive Soft Routing for Plant Leaf Disease Classification](https://arxiv.org/abs/2606.23441)
...
### [446] [AOHP: An Open-Source OS-Level Agent Harness for Personalized, Efficient and Secure Interaction](https://arxiv.org/abs/2606.23449)
...
### [447] [CADRE: Stable, Parameter Efficient Adaptation of Medical Vision Language Models with Bounded Forgetting and Prior Drift](https://arxiv.org/abs/2606.23487)
...
### [448] [POTracker: Optimizing Large Language Models for Standard-Compliant Power Outage Report Generation](https://arxiv.org/abs/2606.23533)
...
### [449] [VeriEvol: Scaling Multimodal Mathematical Reasoning via Verifiable Evol-Instruct](https://arxiv.org/abs/2606.23543)
...
### [450] [The Topology of Ill-Posed Questions: Persistent Homology for Detection and Steering in LLMs](https://arxiv.org/abs/2606.23590)
...
### [451] [SPIRAL: Learning to Search and Aggregate](https://arxiv.org/abs/2606.23595)
...
### [452] [Against Proxy Optimization](https://arxiv.org/abs/2606.23597)
*Sven Neth*
...
### [453] [Causal Discovery in the Era of Agents](https://arxiv.org/abs/2606.23608)
...
### [454] [AI-driven Optimisation of Quality of Recovery (QoR) in Remote Patient Monitoring](https://arxiv.org/abs/2606.23631)
...
### [455] [AI Exposure Scores: what they measure, what they miss, and what comes next](https://arxiv.org/abs/2606.23633)
...
### [456] [TailorMind: Towards Preference-Aligned Multimodal Content Generation](https://arxiv.org/abs/2606.23643)
...
### [457] [Teaching LLMs String Matching, Backtracking, and Error Recovery to Deduce Bases and Truth Tables for the Combinatorially Exploding Bit Manipulation Puzzles](https://arxiv.org/abs/2606.23672)
...
### [458] [PsyBridge: A Hybrid Intelligent Framework for Multi-Dimensional Mental Health Assessment and Decision Support](https://arxiv.org/abs/2606.23673)
