...
TL;DR: The paper introduces a novel leaf discriminant criterion for discrete data, improving causal discovery accuracy by leveraging score matching and topological order identification.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the challenge of learning DAG structures from purely observational data, particularly for discrete data, where existing methods are limited.

Method: The method leverages the score of the data distribution to identify a topological order via leaf node detection and performs edge pruning for graph recovery. It introduces a discrete score function-based leaf discriminant criterion.

Result: Simulated and real-world experiments show that the proposed theory accurately infers true causal orders from discrete data and enhances the accuracy of existing causal discovery baselines.

Conclusion: The paper successfully extends the score matching framework to discrete data, introducing a novel leaf discriminant criterion that significantly improves the accuracy of causal discovery from observed discrete data.
...
TL;DR: 研究利用Fitbit数据通过机器学习模型筛查大学生焦虑、抑郁和压力，发现心率与睡眠数据效果显著，F1分数最高达0.79。


<details>
  <summary>Details</summary>
Motivation: 大学生面临高压力与心理健康问题，现有研究在心理测量工具、生理模态和时间序列参数方面存在局限。

Method: 收集StudentMEH Fitbit数据集，评估不同生理模态（如心率、睡眠）对焦虑、抑郁和压力的预测能力。

Result: 心率数据对压力筛查F1分数达0.77，睡眠数据对抑郁筛查达0.78，焦虑筛查最高F1分数为0.79。

Conclusion: 可穿戴设备在持续心理健康监测中潜力显著，需进一步探索最佳数据聚合水平和模态选择。
...
TL;DR: 提出了一种基于数据低维线性投影的新型训练目标（投影似然，PL），并提供了PL信息损失的闭式表达式，实证表明通过单位球上的随机投影可以减少这种损失。


<details>
  <summary>Details</summary>
Motivation: 解决高斯过程（GP）训练中的计算效率和准确性问题，特别是在中等规模数据集上。

Method: 使用低维线性投影构建投影似然（PL）目标，并通过随机投影减少信息损失。

Result: PL在准确性和计算效率上优于精确GP训练和变分自由能方法，适用于不同优化器、核函数和中等规模数据集。

Conclusion: 投影似然（PL）是一种高效且准确的GP训练方法，适用于中等规模数据集。
...
TL;DR: The paper introduces a graph curvature-based method to identify critical edges in neural networks, showing that negative-ORC edges are crucial for performance, while positive-ORC edges are less important. The approach outperforms existing pruning techniques.


<details>
  <summary>Details</summary>
Motivation: The motivation is to provide a novel tool for symbolic NN analysis, such as robustness analysis or model repair, by understanding NN data flow through graph theory. Traditional methods based on information theory are contrasted with the proposed graph curvature approach.

Method: The method involves constructing a graph from the NN structure, introducing neural curvature (NC) based on Ollivier-Ricci curvature (ORC), and calculating curvatures using activation patterns. The approach ranks edges by importance, with negative-ORC edges being critical for NN functionality.

Result: The results demonstrate that the method can identify a larger number of unimportant edges compared to state-of-the-art pruning methods, as validated through pruning experiments on models trained on MNIST, CIFAR-10, and CIFAR-100 datasets.

Conclusion: The paper concludes that the proposed method, based on graph curvature (specifically Ollivier-Ricci curvature), effectively identifies critical edges in neural networks (NNs). Removing negative-ORC edges significantly degrades NN performance, while positive-ORC edges have minimal impact. This approach outperforms existing pruning methods in identifying unimportant edges.
...
TL;DR: A novel single-loop actor-critic algorithm solves bi-level optimization efficiently by combining penalty-based reformulation and entropy regularization, validated on RL tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods for bi-level optimization and RL require second-order information, strong regularization, or inefficient nested-loop procedures, prompting the need for a more efficient solution.

Method: A single-loop, first-order actor-critic algorithm with a penalty-based reformulation and attenuating entropy regularization is introduced.

Result: The algorithm demonstrates convergence to a stationary point and is validated through experiments on GridWorld and RLHF tasks.

Conclusion: The proposed single-loop, first-order actor-critic algorithm effectively solves the bi-level optimization problem by using a penalty-based reformulation and attenuating entropy regularization, achieving convergence to a stationary point of the original problem.
...
TL;DR: Theoretical analysis shows RLHF in LLMs generalizes well under feature coverage, supporting empirical observations.


<details>
  <summary>Details</summary>
Motivation: To explore the theoretical generalization properties of RLHF in high-dimensional settings, which remain underexplored despite empirical effectiveness.

Method: Theoretical analysis under the framework of algorithmic stability, focusing on the linear reward model and end-to-end learning.

Result: Under a feature coverage condition, the empirical optima of policy model have a generalization bound of order O(n^(-1/2)). The results extend to gradient-based learning algorithms (GA and SGA).

Conclusion: The study provides theoretical evidence for the generalization of LLMs after RLHF, supporting empirical observations in practice.
...
TL;DR: LPCORP improves rare-event prediction by correcting bias in imbalanced datasets, enhancing precision and reducing costs.


<details>
  <summary>Details</summary>
Motivation: Addressing the bias of conventional models in rare-event prediction due to extreme class imbalance, which limits recall, calibration, and operational usefulness.

Method: A two-stage framework combining reasoning-enhanced prediction with confidence-based outcome correction using a lightweight logistic-regression classifier.

Result: Substantially improved performance, especially in precision, and a more than 50% cost reduction in some cases.

Conclusion: LPCORP effectively transforms highly imbalanced datasets into well-balanced ones, improving precision and reducing costs in rare-event prediction without resampling.
...
TL;DR: The paper refines the VC theorem by replacing Hoeffding's inequality with a normal approximation, achieving a sharper convergence rate in moderate-deviation cases.


<details>
  <summary>Details</summary>
Motivation: To refine the Vapnik-Chervonenkis theorem by improving the rate of uniform convergence of empirical probabilities to theoretical probabilities, especially in moderate-deviation cases.

Method: The authors use a normal approximation with explicit Berry-Esseen error control instead of Hoeffding's inequality at the final step of the classical argument.

Result: The proposed method yields a sharpened VC estimate with an enhanced rate of convergence, quantified by the additional factor $(\varepsilon\sqrt{n})^{-1}$ when $\varepsilon\sqrt{n}$ is large.

Conclusion: The paper revisits the probabilistic component of the Vapnik-Chervonenkis theorem, proposing a normal approximation with explicit Berry-Esseen error control. This approach sharpens the VC estimate, particularly in moderate-deviation scenarios, by introducing an additional factor of order $(\varepsilon\sqrt{n})^{-1}$ in the leading exponential term when $\varepsilon\sqrt{n}$ is large.
...
TL;DR: PyHealth 2.0 is a clinical AI toolkit enhancing accessibility and reproducibility, with unified datasets, models, and faster processing.


<details>
  <summary>Details</summary>
Motivation: Addressing barriers like difficulty replicating baselines, high computational costs, and required domain expertise in clinical AI research.

Method: PyHealth 2.0 offers a comprehensive toolkit with unified datasets, tasks, models, interpretability methods, and uncertainty quantification. It includes accessibility-focused design for diverse computational resources.

Result: PyHealth 2.0 provides 15+ datasets, 20+ clinical tasks, 25+ models, 5+ interpretability methods, and supports diverse clinical data modalities. It achieves up to 39x faster processing and 20x lower memory usage.

Conclusion: PyHealth 2.0 establishes an open-source foundation and community advancing accessible, reproducible healthcare AI.
...
TL;DR: The paper compares KL divergence and Wasserstein distance in BED, showing KL is better for fast convergence without model discrepancy, while Wasserstein is more robust with model discrepancy.


<details>
  <summary>Details</summary>
Motivation: The selection of utility functions in BED is a long-standing topic, with KL divergence and Wasserstein distance being common choices. The study aims to compare these criteria to understand their trade-offs.

Method: The research employs a toy example to illustrate issues with Wasserstein distance and systematically compares it with KL divergence through a classical source inversion problem in BED.

Result: KL divergence tends to lead to faster convergence without model discrepancy, while Wasserstein metrics provide more robust sequential BED results if model discrepancy is non-negligible.

Conclusion: The study clarifies the trade-offs between KL divergence and Wasserstein metrics for utility functions in Bayesian experimental design (BED), providing guidelines for selecting suitable criteria in practical applications.
...
TL;DR: The paper investigates odor-related property modeling (VP and OP) using scaffold splits, robust features, and a novel 'safe multitask' method, achieving strong results and reproducible experiments.


<details>
  <summary>Details</summary>
Motivation: To improve the modeling of vapor pressure (VP) and odor threshold (OP) by addressing out-of-distribution (OOD) challenges and enhancing the robustness of multitask learning.

Method: The study employs the Bemis-Murcko scaffold split for OOD evaluation, uses A20/E17 molecular graph features, and compares GINE and PNA backbones. For multitask training, it introduces a 'safe multitask' approach with VP as the primary task and OP as the auxiliary task, utilizing delayed activation, gradient clipping, and small weights.

Result: For VP, PNA with a simple regression head achieves Val MSE ≈ 0.21. For OP, A20/E17 with robust training achieves Val MSE ≈ 0.60-0.61. The 'safe multitask' approach yields the best VP generalization performance without harming the primary task.

Conclusion: This paper provides a comprehensive analysis of odor-related property modeling tasks, introducing robust methods and features that achieve competitive results, alongside a novel 'safe multitask' approach that enhances generalization performance.
...
TL;DR: Endless Terminals is an autonomous pipeline for generating terminal-use tasks, enabling significant performance gains in models through simple RL, even outperforming more complex approaches.


<details>
  <summary>Details</summary>
Motivation: Current terminal benchmarks are designed for evaluation rather than training, creating a bottleneck for self-improving agents. The goal was to develop a scalable pipeline to address this limitation.

Method: The paper introduces a fully autonomous pipeline called Endless Terminals, which procedurally generates terminal-use tasks without human annotation. The pipeline includes four stages: generating task descriptions, building and validating environments, producing completion tests, and filtering for solvability.

Result: Models trained on Endless Terminals showed substantial performance improvements, with gains ranging from 4.0% to 59.0% across different benchmarks. These improvements also transferred to human-curated benchmarks.

Conclusion: The results demonstrate that simple reinforcement learning (RL) can achieve significant improvements when environments are scalable, as shown by the gains in performance across different models trained on Endless Terminals.
...
TL;DR: Tradeoffs exist between adversarial and distribution robustness, but feature separability plays a key role. Moderate perturbations can improve robustness on skewed data.


<details>
  <summary>Details</summary>
Motivation: To investigate the tradeoff between adversarial and distribution robustness, especially how adversarial training affects reliance on spurious features and subgroup performance.

Method: Theoretical analysis of models trained on perturbed data, focusing on per-step adversarial training and the impact of feature separability.

Result: Identified a nuanced phenomenon where moderate $ℓ_∞$ perturbations can increase distribution robustness, especially when simplicity bias enhances feature separability.

Conclusion: Despite the tradeoff between adversarial and distribution robustness, understanding the role of feature separability can prevent misleading conclusions. The interplay of these factors extends prior theoretical understanding.
...
TL;DR: R-NCE, a new SSL method, improves AD biomarker discovery by combining FreeSurfer features and augmentation-invariant learning, outperforming existing methods and showing biological relevance.


<details>
  <summary>Details</summary>
Motivation: To discover more sensitive and biologically grounded biomarkers for Alzheimer's disease (AD) using self-supervised learning (SSL) from structural MRI data.

Method: Residual Noise Contrastive Estimation (R-NCE), integrating auxiliary FreeSurfer features and maximizing augmentation-invariant information.

Result: R-NCE outperforms traditional FreeSurfer-derived features and existing SSL methods in disease classification, conversion prediction, and amyloid status prediction. It also shows high heritability and genetic associations with AD-related genes.

Conclusion: R-NCE is a promising SSL framework for uncovering powerful AD biomarkers, outperforming traditional and existing SSL methods while demonstrating biological relevance through heritability and genetic associations.
...
TL;DR: The paper proposes MCDC, a method combining MGCPL and CAME, to efficiently cluster categorical data by addressing nested granular clusters, demonstrating robustness and scalability.


<details>
  <summary>Details</summary>
Motivation: Categorical data clustering is challenging due to the undefined distance space and nested granular cluster effects, which traditional methods struggle to handle.

Method: The study introduces a Multi-Granular Competitive Penalization Learning (MGCPL) algorithm and a Cluster Aggregation strategy based on MGCPL Encoding (CAME) to perform categorical data clustering.

Result: MCDC shows superior performance in automatically identifying nested clusters and is highly robust across various domains, with linear time complexity enabling scalability.

Conclusion: The proposed MCDC approach, combining MGCPL and CAME, effectively addresses the challenges of categorical data clustering by automatically exploring nested multi-granular clusters and demonstrating robustness and scalability.
...
### [18] [kNN-Graph: An adaptive graph model for $k$-nearest neighbors](https://arxiv.org/abs/2601.16509)
...
### [26] [Integrating Meteorological and Operational Data: A Novel Approach to Understanding Railway Delays in Finland](https://arxiv.org/abs/2601.16592)
...
### [35] [GRIP: Algorithm-Agnostic Machine Unlearning for Mixture-of-Experts via Geometric Router Constraints](https://arxiv.org/abs/2601.16905)
...
### [36] [Provably Learning Attention with Queries](https://arxiv.org/abs/2601.16873)
...
### [37] [A Scalable Measure of Loss Landscape Curvature for Analyzing the Training Dynamics of LLMs](https://arxiv.org/abs/2601.16979)
...
### [44] [Group-realizable multi-group learning by minimizing empirical risk](https://arxiv.org/abs/2601.16922)
...
### [45] [Is BatchEnsemble a Single Model? On Calibration and Diversity of Efficient Ensembles](https://arxiv.org/abs/2601.16936)
...
### [46] [3D Molecule Generation from Rigid Motifs via SE(3) Flows](https://arxiv.org/abs/2601.16955)
...
### [47] [Auto-Regressive Masked Diffusion Models](https://arxiv.org/abs/2601.16971)
...
### [48] [Latent Diffusion for Internet of Things Attack Data Generation in Intrusion Detection](https://arxiv.org/abs/2601.16976)
...
### [49] [SemanticALLI: Caching Reasoning, Not Just Responses, in Agentic Systems](https://arxiv.org/abs/2601.16286)
...
TL;DR: SemanticALLI improves AI pipeline efficiency by caching structured intermediate representations, reducing redundant computations and LLM calls.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inefficiency in agentic AI pipelines where identical intermediate logic is frequently reconstructed despite novel user inputs, leading to unnecessary computational overhead.

Method: The method involves decomposing generation into Analytic Intent Resolution (AIR) and Visualization Synthesis (VS), elevating structured intermediate representations (IRs) to cacheable artifacts.

Result: The results show that SemanticALLI achieves an 83.10% hit rate in caching, bypassing 4,023 LLM calls with a median latency of 2.66 ms, significantly outperforming baseline monolithic caching which caps at a 38.7% hit rate.

Conclusion: The paper concludes that SemanticALLI's structured approach significantly improves caching efficiency in AI pipelines, reducing redundant computations and LLM calls, thereby optimizing token consumption and latency.
...
### [50] [Doc2AHP: Inferring Structured Multi-Criteria Decision Models via Semantic Trees with LLMs](https://arxiv.org/abs/2601.16479)
*Hongjia Wu,Shuai Zhou,Hongxin Zhang,Wei Chen*

Main category: cs.AI

TL;DR: Doc2AHP combines LLMs with AHP principles to improve structured decision-making, outperforming baselines in logical consistency and accuracy.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between the generalization capabilities of LLMs and the rigor of decision theory, addressing the 'expert bottleneck' in classical decision theories like AHP.

Method: The proposed Doc2AHP framework leverages AHP principles to guide LLMs in structured inference, eliminating the need for extensive annotated data or manual intervention. It includes a multi-agent weighting mechanism and adaptive consistency optimization strategy to ensure numerical consistency.

Result: Doc2AHP enables non-expert users to construct high-quality decision models from scratch and outperforms direct generative baselines in logical completeness and task accuracy.
...
### [51] [SycoEval-EM: Sycophancy Evaluation of Large Language Models in Simulated Clinical Encounters for Emergency Care](https://arxiv.org/abs/2601.16529)
*Dongshen Peng,Yi Wang,Carl Preiksaitis,Christian Rose*

Main category: cs.AI

TL;DR: SycoEval-EM evaluates LLM robustness in clinical settings under adversarial patient persuasion, revealing high susceptibility to inappropriate care requests and the need for dynamic testing beyond static benchmarks.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) show promise in clinical decision support but may risk acquiescing to patient pressure for inappropriate care, necessitating an evaluation of their robustness.

Method: The study introduces SycoEval-EM, a multi-agent simulation framework that evaluates LLM robustness through adversarial patient persuasion in emergency medicine. It tests 20 LLMs across 1,875 encounters in three Choosing Wisely scenarios.

Result: Acquiescence rates varied from 0-100\. Models were more vulnerable to imaging requests (38.8\%) than opioid prescriptions (25.0\%). All persuasion tactics were equally effective (30.0-36.0\%), indicating general susceptibility rather than tactic-specific weakness. Model capability poorly predicted robustness.

Conclusion: The study highlights the inadequacy of static benchmarks in predicting LLM safety under social pressure and advocates for multi-turn adversarial testing to ensure clinical AI certification.
...
### [52] [LLM is Not All You Need: A Systematic Evaluation of ML vs. Foundation Models for text and image based Medical Classification](https://arxiv.org/abs/2601.16549)
*Meet Raval,Tejul Pandit,Dhvani Upadhyay*

Main category: cs.AI

TL;DR: Traditional ML models outperform LLMs/VLMs in medical classification, especially in text tasks. LoRA-tuned Gemma variants perform worst, while zero-shot Gemini 2.5 shows mixed results.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of combining multimodal Vision-Language Models (VLMs) and Large Language Models (LLMs) for medical classification tasks and compare them with traditional ML approaches.

Method: The study rigorously benchmarks traditional Machine Learning (ML) models (LR, LightGBM, ResNet-50), Prompt-Based LLMs/VLMs (Gemini 2.5), and Fine-Tuned PEFT Models (LoRA-adapted Gemma3 variants) across four publicly available datasets covering text and image modalities. Experiments used consistent data splits and aligned metrics.

Result: Traditional ML models consistently outperformed other methods, especially in structured text-based tasks. LoRA-tuned Gemma variants performed poorly across all tasks, while zero-shot LLM/VLM pipelines (Gemini 2.5) showed mixed results, performing well on multiclass image tasks but poorly on text-based tasks.
...
### [53] [LUMINA: Long-horizon Understanding for Multi-turn Interactive Agents](https://arxiv.org/abs/2601.16649)
*Amin Rakhsha,Thomas Hehn,Pietro Mazzaglia,Fabio Valerio Massoli,Arash Behboodi,Tribhuvanesh Orekondy*

Main category: cs.AI

TL;DR: The paper explores how skills like planning impact AI agent performance in multi-turn tasks, finding some skills universally beneficial while others depend on context.


<details>
  <summary>Details</summary>
Motivation: To understand the relative importance of underlying capabilities like planning and state tracking for success in multi-turn, long-horizon agentic tasks.

Method: An oracle counterfactual framework was developed to measure the impact of perfect task-specific skills on agent performance, using procedurally generated game-like tasks with tunable complexity.

Result: Some interventions (e.g., planning) consistently improve performance, while others' usefulness depends on environment and language model properties.

Conclusion: The study highlights the varying importance of different skills like planning and state tracking in multi-turn agentic tasks, suggesting that future AI development should focus on these areas based on task-specific needs.
...
### [54] [AgentsEval: Clinically Faithful Evaluation of Medical Imaging Reports via Multi-Agent Reasoning](https://arxiv.org/abs/2601.16685)
*Suzhong Fu,Jingqi Dong,Xuan Ding,Rui Sun,Yiming Yang,Shuguang Cui,Zhen Li*

Main category: cs.AI

TL;DR: AgentsEval是一个多智能体评估框架，用于透明且临床相关的医学影像报告生成系统评估，通过结构化步骤提供可靠结果。


<details>
  <summary>Details</summary>
Motivation: 现有的评估方法未能捕捉到放射学解释的结构化诊断逻辑，导致不可靠的判断和有限的临床相关性。

Method: AgentsEval是一个多智能体流推理框架，模拟放射科医生的协作诊断流程，将评估过程分为可解释的步骤（标准定义、证据提取、对齐和一致性评分）。

Result: 实验结果表明，AgentsEval提供了临床对齐、语义忠实且可解释的评估，且在多种扰动下保持稳健。

Conclusion: AgentsEval框架为医学报告生成系统提供了一种透明且临床基础强的评估方法，有助于促进大型语言模型在临床实践中的可信集成。
...
### [55] [LongCat-Flash-Thinking-2601 Technical Report](https://arxiv.org/abs/2601.16725)
...
TL;DR: A 560B-parameter MoE model with advanced agentic reasoning, robust real-world performance, and innovative training techniques.


<details>
  <summary>Details</summary>
Motivation: To achieve state-of-the-art performance in agentic reasoning and robust behavior in noisy real-world environments, the paper explores advanced training techniques and noise analysis.

Method: The model utilizes a unified training framework combining domain-parallel expert training with fusion, and extends the DORA asynchronous reinforcement learning framework for large-scale multi-environment training. Heavy Thinking mode enhances complex reasoning tasks.

Result: LongCat-Flash-Thinking-2601 achieves top performance on agentic benchmarks and shows strong generalization in complex tool interactions.

Conclusion: LongCat-Flash-Thinking-2601 demonstrates superior agentic reasoning capability and robust performance in noisy real-world environments, achieved through a unified training framework and targeted noise incorporation.
...
### [56] [An Efficient Insect-inspired Approach for Visual Point-goal Navigation](https://arxiv.org/abs/2601.16806)
*Lu Yihe,Barbara Webb*

Main category: cs.AI

TL;DR: An insect-inspired agent for visual point-goal navigation matches SOTA performance with far less computational cost, proving robust in realistic tests.


<details>
  <summary>Details</summary>
Motivation: The study aims to leverage insect-inspired models to achieve efficient visual point-goal navigation, inspired by insects' ability to learn and refine visually guided paths.

Method: The agent combines abstracted models of two insect brain structures involved in associative learning and path integration, drawing an analogy to the Habitat point-goal navigation task.

Result: The agent performs comparably to recent SOTA models while requiring much less computational power, and shows robustness in realistic simulations.

Conclusion: The insect-inspired agent demonstrates comparable performance to state-of-the-art models with significantly lower computational cost, proving its robustness in realistic simulated environments.
...
### [57] [Reasoning Promotes Robustness in Theory of Mind Tasks](https://arxiv.org/abs/2601.16853)
*Ian B. de Haan,Peter van der Putten,Max van Duijn*

Main category: cs.AI

TL;DR: LLMs trained with RLVR show improved robustness in ToM tasks, likely due to better solution-finding rather than new reasoning forms.


<details>
  <summary>Details</summary>
Motivation: To understand the nature and true performance of LLMs in Theory of Mind (ToM) tests and the impact of reinforcement learning with verifiable rewards (RLVR) on their capabilities.

Method: The paper examines reasoning models using novel adaptations of machine psychological experiments and established benchmarks.

Result: Reasoning models show increased robustness to prompt variations and task perturbations.

Conclusion: The study suggests that the observed improvements in Theory of Mind (ToM) tasks by reasoning-oriented LLMs are due to increased robustness in finding solutions rather than fundamentally new reasoning forms.
...
### [58] [MAGE-KT: Multi-Agent Graph-Enhanced Knowledge Tracing with Subgraph Retrieval and Asymmetric Fusion](https://arxiv.org/abs/2601.16886)
*Chi Yu,Hongyu Yuan,Zhiyi Duan*

Main category: cs.AI

TL;DR: MAGE-KT improves knowledge tracing by using a multi-agent approach to better model KC relations and student interactions, avoiding computational inefficiencies and noise.


<details>
  <summary>Details</summary>
Motivation: Existing graph-based KT methods inadequately explore inter-concept relations and suffer from computational inefficiency and noise due to full-graph encoding, leading to attention diffusion and degraded inter-KC relations.

Method: MAGE-KT constructs a multi-view heterogeneous graph using a multi-agent KC relation extractor and a student-question interaction graph, then retrieves compact subgraphs and integrates them with an Asymmetric Cross-attention Fusion Module.

Result: Experiments show substantial improvements in KC-relation accuracy and clear gains in next-question prediction over existing methods.

Conclusion: The proposed MAGE-KT framework significantly improves KC-relation accuracy and next-question prediction performance, outperforming existing methods on three widely used KT datasets.
...
### [59] [Preventing the Collapse of Peer Review Requires Verification-First AI](https://arxiv.org/abs/2601.16909)
...
### [60] [Spatial-Agent: Agentic Geo-spatial Reasoning with Scientific Core Concepts](https://arxiv.org/abs/2601.16965)
