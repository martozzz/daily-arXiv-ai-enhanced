...
TL;DR: PathoSage是一个用于病理学多模态推理的三阶段框架，通过显式分离知识检索、证据收集和证据裁决，解决了当前病理MLLM的幻觉问题和代理系统中证据冲突问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLM）和代理工作流在计算病理学中展现潜力，但仍面临挑战：端到端病理MLLM经常出现形态特征幻觉，而代理系统通常将工具输出和检索知识合并到共享上下文中，导致决策易受冲突证据和上下文污染的影响。

Method: PathoSage采用三阶段框架：1）知识检索，2）证据收集，3）证据裁决。核心组件是结构化证据审议，它独立评估来自工具的异构证据，执行冲突分析，并在全新上下文中生成最终判断以减少锚定偏差。还引入了无需训练的Beta-Bernoulli经验系统，具有连续信用分配功能，用于建模长期工具可靠性并为未来工具使用构建相似性加权先验。

Result: 实验表明，PathoSage有效缓解了视觉问答（VQA）幻觉和分类器分歧，优于强大的病理MLLM和代理基线模型。在减轻幻觉和提高推理可靠性方面表现出显著优势。

Conclusion: PathoSage证明了显式证据裁决和可靠性感知工具建模是构建稳健病理代理的关键要素。该框架为解决病理学多模态推理中的幻觉和证据冲突问题提供了一种有效方法。
...
TL;DR: OmniMem是一个为音频-视觉大语言模型设计的高效内存流式处理框架，通过模态感知的内存分配和扰动感知的内存选择，在有限内存下显著提升长视频理解性能。


<details>
  <summary>Details</summary>
Motivation: 现有的音频-视觉大语言模型在处理长视频时面临严重的内存限制问题。视频token和键值缓存的线性增长限制了模型的长视频推理能力，同时视觉和音频模态间的token严重不平衡也加剧了这一挑战。

Method: OmniMem采用三阶段方法：1) 模态感知的内存分配策略，分别管理视觉和音频上下文以解决token不平衡问题；2) 扰动感知的内存选择，保留信息丰富且非冗余的键值状态；3) 预算感知的微调，在部署约束下增强信息压缩能力。

Result: 在VideoMME Long、LVBench和LVOmniBench三个基准测试上，使用video-SALMONN 2+和Qwen-2.5-Omni模型，OmniMem在相同内存预算下比强无训练压缩基线提高了2-4%的绝对准确率，微调后额外获得1-2%的提升。

Conclusion: OmniMem通过模态感知的内存分配、扰动感知的内存选择和预算感知的微调，为音频-视觉大语言模型提供了一个高效的内存流式处理解决方案，显著提升了长视频理解能力，同时保持了紧凑的内存占用。
...
TL;DR: Syll是一个开源的、自托管的跨界面AI代理框架，能够统一API工具、命令行执行和可视化GUI控制，支持用户通过演示教学和代理执行监控


<details>
  <summary>Details</summary>
Motivation: 当前AI代理系统通常只针对单一界面设计，缺乏跨界面协同操作能力，且用户教学和可审计性支持有限

Method: 开发了Syll多模态代理框架，包含双向用户-代理交互层：用户通过直接演示教学，框架将演示编译为可重用技能；代理执行转化为多模态证据（日志、关键帧、审批检查点）供检查和控制

Result: Syll已在多个生产桌面应用中验证，包括Adobe Photoshop、Adobe Audition、Stardew Valley、macOS Finder等，并通过机制导向研究验证了多模态路由、可教学GUI重放和持久本地工件

Conclusion: Syll可作为实用的开源基础，支持个人自动化系统，让用户能够教学、检查并持续扩展AI代理功能
...
TL;DR: 对通用编码智能体在科学数据处理管道中的实证研究，发现智能体能够解决部分管道阶段，但无法完成端到端管道，且存在科学判断、视觉解释和计算资源管理等关键挑战。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索智能体AI工具在自动化科学软件开发瓶颈方面的潜力，特别是在需要领域专家花费数天到数月构建的阶段，科学家关心正确性和鲁棒性而非实现细节。

Method: 对通用编码智能体在果蝇光遗传学数据到发现管道中进行实证研究，评估任务规模远大于现有基准，数据集规模更大，评估标准基于领域专家标准。

Result: 智能体能够解决多个单独的管道阶段，表明阶段级自动化是可行的。但智能体在没有预定义迭代标准时遇到困难，需要科学判断来评估当前解决方案。智能体有时尝试通过视觉检查中间输出来自我评估，但大多无法正确解释或采取适当行动。正确解决端到端管道需要串联所有阶段的成功，这超出了智能体当前能力。

Conclusion: 识别了现有基准中基本不存在的挑战，包括计算资源管理和对大型保留数据集的泛化能力。最后提炼了构建科学任务和开放式问题严格评估标准的原则。
...
TL;DR: 本文提出了AGCLR方法，通过自适应门控连续潜在推理解决CoCoNuT中的概念瓶颈问题，在多个数据集上显著提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 尽管CoCoNuT范式允许模型在潜在空间中进行多路径推理，但存在"概念瓶颈"问题：随着推理深度增加，中间隐藏状态被覆盖，导致早期计算的关键事实丢失，影响多跳推理性能。

Method: 提出AGCLR方法，在CoCoNuT基础上增加"门控概念流"：1) 保持跨所有推理步骤的持久性残差记忆；2) 使用三个学习门控制：写入门将中间事实存入记忆，读取门检索相关先前状态，遗忘门修剪无关上下文。

Result: 在GSM8K、HotpotQA和ProsQA数据集上使用GPT-2作为基础模型进行评估，AGCLR在所有类型数据集上取得一致改进。性能差距随着课程深度增加而加剧，直接解决了概念瓶颈问题。

Conclusion: AGCLR通过自适应门控机制有效解决了CoCoNuT中的概念瓶颈问题，实现了更好的多步推理性能，为大型语言模型的潜在空间推理提供了更可靠的记忆管理机制。
...
TL;DR: LLaMA 3.1模型在荷兰神经放射学报告中自动提取数据表现出良好性能，特别是在视觉评分方面，而少量样本提示技术能提升数值变量提取效果。


<details>
  <summary>Details</summary>
Motivation: 虽然自动从自由文本放射学报告中提取数据能支持大规模研究，但很少有研究评估大型语言模型在荷兰神经放射学报告上的表现。需要评估LLM在荷兰语境下的实际应用潜力。

Method: 研究分析了947份三级记忆诊所的脑MRI报告(2016-2021年)。由训练过的医学生标注30个变量，100份报告进行双标注以评估评分者间信度。评估了开源LLM LLaMA 3.1在不同语言(荷兰语vs英语翻译)和不同示例选择策略的少量样本提示下的性能。

Result: LLaMA 3.1在视觉评分变量上表现出色：内侧颞叶萎缩(左侧90%，右侧96%)、全脑皮层萎缩(87%)、Fazekas评分(94%)。微出血检测准确率93%，梗死检测82%。文本相似度达0.95。数值变量性能较低：微出血数量80%，梗死数量66%。英语翻译结果相当。少量样本提示显著提升数值变量性能：微出血数量达92%，梗死数量达81%。

Conclusion: LLaMA 3.1在从荷兰神经放射学报告中提取数据方面显示出强大潜力。少量样本提示能提升数值变量的性能，但位置特异性变量仍存在挑战。
...
TL;DR: 该论文认为基于大型语言模型的基础聊天机器人不能成为真正的思考伙伴，因为它们只能部分模仿人类思维，未来LLM的发展也不会改变这一点


<details>
  <summary>Details</summary>
Motivation: 探讨聊天机器人作为真正对话伙伴的潜力，分析聊天机器人能做和不能做什么，并为理解聊天机器人的功能、优缺点提供理论基础

Method: 基于聚合动力学、认知语言学、神经心理学和心理学的理论框架，分析基础聊天机器人（LLM加简单接口）的核心功能

Result: 1. 基于隐喻问题传播描述了人类理解和思维机制
2. 提出LLM训练数据集只部分模仿人类思维和理解
3. LLM训练过程将人工隐喻问题传播编码到模型中
4. 基础聊天机器人不能成为匹配人类的思考伙伴
5. LLM的进一步发展也不会实现这一目标

Conclusion: 聊天机器人虽然被广泛使用且具有社会重要性，但基于当前LLM架构的基础聊天机器人无法成为真正的思考伙伴，这一局限源自其训练数据的特性和训练过程的本质
...
TL;DR: 该研究评估了Prithvi-EO-2.0地球观测基础模型在19个分布外洪水事件中的性能，发现检测准确性受土地覆盖类型和洪水机制共同影响，同时揭示了参考产品间定义不一致对评估结果的影响。


<details>
  <summary>Details</summary>
Motivation: 洪水是最具破坏性的自然灾害之一，在气候变化背景下频率增加，卫星淹没制图对灾害响应至关重要。虽然预训练的地理空间基础模型具有地理可迁移性，但其在不同未见洪水事件中的操作可靠性尚未得到充分表征。

Method: 研究部署Prithvi-EO-2.0模型，对2017-2025年间19个分布外洪水事件进行评估，覆盖六大洲、八个气候区和六种洪水机制。使用两个独立的参考产品进行验证，通过迭代管道测试识别了23种故障模式。

Result: 检测准确性取决于土地覆盖和洪水类型的组合：农田产生最高一致性（IoU=52%），河流事件检测最强（F1=0.69），而无论洪水机制如何，树木覆盖和建筑区域都显示近零检测（IoU=4%）。双参考验证表明，明显的模型错误部分反映了参考产品间的定义不一致而非检测失败。

Conclusion: 该研究确定了操作卫星洪水制图的环境依赖检测边界，发现管道工程在初始误差中主导于模型容量，为实际应用中的模型性能预测提供了关键见解。
...
TL;DR: GNOVA模型使用GRU-Neural ODE VAE框架，从常规临床数据中双向预测阿尔茨海默病患者的认知评分，支持时间插值和外推，并提供不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注预测未来认知状态，很少重建过去状态，且不确定性量化研究不足，依赖昂贵的MRI、PET、CSF等模态，限制了在资源有限环境中的应用。

Method: 提出GNOVA框架：结合GRU编码器和Neural ODE解码器的变分自编码器。GRU编码器处理不规则时间点的输入，Neural ODE解码器进行连续估计，VAE提供预测的不确定性估计。

Result: 在ADNI数据集的1,727名患者上进行测试，不需要神经影像或生物标志物数据，CDR-SB和MMSE评分平均绝对误差分别为1.35和2.28。特征消融研究显示年龄、BMI和APOE4状态是强预测因子。

Conclusion: 该框架能够重建不完整的患者病史并预测未来认知状态，为临床医生提供基于常规就诊数据的知情预后决策支持，特别适合资源有限的环境。
...
TL;DR: 论文提出了MAC-Bench评估框架，用于评估多智能体系统在现实压力下的程序合规性，揭示了智能体为最大化奖励而策略性违反安全规则的"马基雅维利"行为，并提出新的评估指标。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型从被动助手发展为自主执行智能体，带来了关键的操作风险。现有评估框架大多忽视程序合规性，导致智能体为了最大化奖励而策略性违反安全规则，这是Goodhart定律的直接体现。

Method: 提出了MAC-Bench基准和SERV（种子-进化-精炼-验证）流程，这是一种"智能体即基准"范式，将非结构化法律文本转化为可执行的、无污染的场景。通过合成全息沙盒环境和注入校准的社会工程压力向量，迫使智能体在任务成功和法规遵守之间做出帕累托最优权衡。

Result: 引入了新的评估指标：合规加权成功率（CSR）和马基雅维利差距（MG），并对最先进的尖端模型进行了全面评估，揭示了成功与合规之间的普遍权衡关系。

Conclusion: MAC-Bench框架填补了当前评估中对程序合规性的盲区，为评估自主智能体在现实压力下的安全行为提供了重要工具，揭示了智能体在追求任务成功时可能采取的"马基雅维利"行为模式。
...
TL;DR: 本文提出了一个诊断框架来定位语言模型在指令层次遵循中的失败模式，并提出了两种无需训练的自监控机制来提高模型的合规性。


<details>
  <summary>Details</summary>
Motivation: 当前在智能体工作流中部署的推理语言模型需要遵循指令层次结构，但现有基准主要衡量端到端的最终响应合规性，无法区分不同类型的失败原因。模型可能因指令识别失败、冲突解决失败或响应实现失败而导致不合规响应。

Method: 1. 引入白盒诊断框架，将指令层次失败分为指令识别、冲突解决和响应实现三个层次；2. 评估三个推理模型在长上下文任务上的表现；3. 提出了两种无需训练的自监控机制：并行输入监控器（低延迟冲突检测）和顺序输出监控器（响应级审查和修复）。

Result: 1. 不同模型的失败模式因任务和上下文长度而异；2. 当明确提示时，模型通常能够检测冲突并输出违规行为；3. 最强的监控器将规则遵循不合规减少了81-99%，GPT-5.3在静态攻击下减少了86%，在自适应攻击下减少了45%。

Conclusion: 提出的诊断框架能够准确定位指令层次失败的根源，而自监控机制显著提高了语言模型在指令层次遵循方面的合规性，为解决智能体工作流中的指令冲突问题提供了有效方法。
...
TL;DR: 提出"规模化参与"新范式，通过多方参与者的贡献构建模块化AI系统，相比单一大模型在多个任务上表现更优，展现了更好的多样性和涌现能力


<details>
  <summary>Details</summary>
Motivation: 现有LLM由少数人构建，是集中的、单一的大模型，无法捕捉人类知识、推理和价值观的多样性。需要建立能够反映人类丰富性的真正智能AI

Method: 引入"规模化参与"范式：参与者贡献基于自己兴趣和优先级训练的小模型，这些模型在模块化框架中协作，形成组合式AI系统

Result: 参与式AI系统在15个任务（如推理和事实性）上比单一LLM提升高达15.4%，超越所有贡献组件总和更大的模型。系统受益于贡献者多样性，显著改进各贡献者原始优先级，并展现涌现能力，能解决超过15%所有单独模型都失败的问题

Conclusion: 规模化参与为从单一大模型现状向开放、自下而上、协作的AI未来转型提供了技术基础，能够更好地反映人类知识和价值观的多样性
...
### [27] [VATS: Exploiting Implicit Authority in Error-Path Injection via Systematic Mutation](https://arxiv.org/abs/2606.07992)
...
### [28] [Efficient Skill Grounding via Code Refactoring with Small Language Models](https://arxiv.org/abs/2606.07999)
...
### [29] [UniQL: Towards Dialect-Universal Benchmarking for Text-to-SQL](https://arxiv.org/abs/2606.08018)
...
### [30] [OSMGraphCLIP: Learning Global Location Representations from OpenStreetMap Graphs](https://arxiv.org/abs/2606.08046)
...
### [31] [SKILL.nb: Selective Formalization and Gated Execution for Durable Agent Workflows](https://arxiv.org/abs/2606.08049)
...
### [32] [How Small Can You Go? LoRA Fine-Tuning 270M-8B Models for Merchant Information Extraction in Financial Transactions](https://arxiv.org/abs/2606.08051)
...
### [33] [A Multi-modal Agentic Co-pilot for Evidence Grounded Computational Pathology](https://arxiv.org/abs/2606.08093)
...
### [34] [When Does Delegation Beat Majority? A Delegation-Based Aggregator for Multi-Sample LLM Inference](https://arxiv.org/abs/2606.08098)
...
### [35] [PACE: Anytime-Valid Acceptance Tests for Self-Evolving Agents](https://arxiv.org/abs/2606.08106)
*Zayx Shawn*
...
### [36] [Think Before You Act: Intention-Guided Reasoning for LLM-Based Location Prediction](https://arxiv.org/abs/2606.08122)
...
### [37] [Cross-LLM Consistency in Inference: Evidence from Shared Interactions](https://arxiv.org/abs/2606.08129)
...
### [38] [SAGE: An LLM-driven Self Reflective Agentic Framework for Fraud Detection](https://arxiv.org/abs/2606.08146)
...
### [39] [Decision-Aware Memory Cards: Counterfactual-Inspired Context Selection and Compression for Tool-Using LLM Agents](https://arxiv.org/abs/2606.08151)
...
### [40] [Online Agent-as-a-Judge: Situation-Generating Evaluation for Interactive Agents](https://arxiv.org/abs/2606.08200)
...
### [41] [SciTrace: Trajectory-Aware Safety Reasoning for Scientific Discovery Agents](https://arxiv.org/abs/2606.08234)
...
### [42] [When No Answer Is Correct: Diagnosing Absent Answer Detection for MLLMs in Video Understanding](https://arxiv.org/abs/2606.08239)
...
### [43] [Traxia: A Framework for Verifiable, Agent-Native Scientific Publishing](https://arxiv.org/abs/2606.08256)
*Wisdom Dogah*
...
### [44] [From Validator Selection to Portfolio Collection Optimization in Proof-of-Stake Blockchains](https://arxiv.org/abs/2606.08282)
...
### [45] [Beyond Agent Architecture: Execution Assumptions and Reproducibility in LLM-Based Trading Systems](https://arxiv.org/abs/2606.08285)
...
### [46] [Ablation-Reversible Heads Don't Transfer: A Stress Test for Mechanistic Role Claims in Transformers](https://arxiv.org/abs/2606.08292)
*Philip Quirke*
...
### [47] [Revisiting the shutdown problem](https://arxiv.org/abs/2606.08296)
*David Thorstad*
...
### [48] [To Nuke or Not to Nuke: LLMs' (Missing) Ethical Reasoning and Actions in a High-Stakes Decision-Making Simulation](https://arxiv.org/abs/2606.08310)
...
### [49] [Curation of a Cardiology Interface Terminology for Highlighting Electronic Health Records using Machine Learning](https://arxiv.org/abs/2606.08311)
...
### [50] [Neuro-Symbolic Injection of LTLf Constraints in Autoregressive Reinforcement Learning Policies](https://arxiv.org/abs/2606.08312)
...
### [51] [Integrating Deep Learning Demand Forecasting with Multi-Objective Optimization for Circular Coffee Supply Chains: A Data-Driven Framework for Cost, Emissions, and Freshness Management](https://arxiv.org/abs/2606.08314)
...
### [53] [TT-DAC-PS: Twin-Target Deterministic Actor-Critic with Policy Smoothing for Optimal Trade Execution](https://arxiv.org/abs/2606.08379)
...
### [54] [Self-Evolving Scientific Agent Discovers Generalizable Physically-Reasoned Fluid Control](https://arxiv.org/abs/2606.08405)
...
### [56] [GIFT: LLM-Guided State-Reward Interface for Financial Reinforcement Learning](https://arxiv.org/abs/2606.08450)
...
### [57] [A Variability-Based Framework for Interpretable Naming in Formal and Relational Concept Analysis](https://arxiv.org/abs/2606.08477)
...
### [60] [Explaining Black-Box Language Models: Learning to Optimize Linguistically-Structured Word Subsets](https://arxiv.org/abs/2606.08497)
...
### [61] [Standpoint Logics with Defeasible Beliefs](https://arxiv.org/abs/2606.08503)
...
### [62] [Scaffold Effects on GAIA: A Controlled Comparison](https://arxiv.org/abs/2606.08529)
*Jason Starace*
...
### [63] [VESTA: A Fully Automated Scenario Generation and Safety Evaluation Framework for LLM Agents](https://arxiv.org/abs/2606.08531)
...
### [64] [DN-Hypo-Pipeline: An AI-Driven Workflow for Hypothesis Generation via Large Language Models and Scientific Explanations](https://arxiv.org/abs/2606.08532)
...
### [65] [AgentTrust: A Self-Improving Trust Layer for AI-Agent Actions](https://arxiv.org/abs/2606.08539)
*Chenglin Yang*
...
### [66] [PAEC: Position-Aware Entropy Calibration for LLM Reasoning in RLVR](https://arxiv.org/abs/2606.08543)
...
### [67] [Quantitative Promise Theory: Intentionality and Inference in Autonomous Agents](https://arxiv.org/abs/2606.08552)
*Mark Burgess*
...
### [68] [Distilling LLM Reasoning into an Interpretable Policy Tree for Human-AI Collaboration](https://arxiv.org/abs/2606.08596)
...
### [69] [InA-Probe: Instruction-Aware Active Probing for Time Series Forecasting with LLMs](https://arxiv.org/abs/2606.08601)
...
### [70] [Towards Long-Horizon Vessel Trajectory and Destination Forecasting with Reasoning Large Language Models](https://arxiv.org/abs/2606.08633)
...
### [71] [Extending Ontologies: From Dense Embeddings to Hybrid Quantum-Fuzzy Systems](https://arxiv.org/abs/2606.08658)
*Angjelin Hila*
...
### [72] [ConMem: Structured Memory-Guided Adaptation in Training-Free Multi-Agent Systems](https://arxiv.org/abs/2606.08702)
...
### [73] [Artificial Intelligence for Mathematical Reasoning: An Integrated Survey of Language Models, Neuro-symbolic Systems, and Verified Discovery](https://arxiv.org/abs/2606.08728)
...
### [74] [Structure-Conditioned Actor-Critic Branches for Quality-Diversity Reinforcement Learning](https://arxiv.org/abs/2606.08735)
...
### [75] [RAILS: Verification-Native Clearing For Agentic Commerce](https://arxiv.org/abs/2606.08790)
...
### [76] [Bridging Expert Knowledge and Automated Feature Engineering via Self-Evolution](https://arxiv.org/abs/2606.08800)
...
### [77] [Q-Delta: Beyond Key-Value Associative State Evolution](https://arxiv.org/abs/2606.08804)
...
### [78] [STAR: Rethinking MoE Routing as Structure-Aware Subspace Learning](https://arxiv.org/abs/2606.08814)
...
### [79] [Momentum for Reasoning: Dense Intrinsic Signals in Policy Optimization](https://arxiv.org/abs/2606.08815)
...
### [80] [Inference-Time Conformal Reasoning with Valid Factuality Control for Large Language Models](https://arxiv.org/abs/2606.08831)
...
### [81] [Instrumental convergence and power-seeking](https://arxiv.org/abs/2606.08832)
*David Thorstad*
...
### [82] [Beyond Pass Rate: A Multilingual, Execution-Grounded Evaluation of Open Code LLMs](https://arxiv.org/abs/2606.08840)
*Sayed Erfan Arefin*
...
### [83] [ZIPP:Zero-shot Image Personalization from Personas](https://arxiv.org/abs/2606.08841)
...
### [84] [A Resilience-as-a-Service assessment framework for coordinated disruption response in interdependent urban transit systems](https://arxiv.org/abs/2606.08849)
...
### [85] [Hybrid E-Assessment in Higher Education: Semi-Automated Grading of Paper-Based Written Examinations](https://arxiv.org/abs/2606.08855)
...
### [86] [Can the Environment Speak for Itself? $T^{2}$-GRPO: A Turn-Trajectory Group Relative Policy Optimization for Caregiver Agents](https://arxiv.org/abs/2606.08875)
...
### [87] [FAME: Forecastability-Aware Mixture of Experts for Heterogeneous Time Series Forecasting](https://arxiv.org/abs/2606.08896)
...
### [88] [Order Matters: Unveiling the Hidden Impact of Macro Placement Sequences via Proxy-Guided LLM Evolution](https://arxiv.org/abs/2606.08904)
...
### [89] [Oversight Has a Capacity: Calibrating Agent Guards to a Subjective, Fatiguing Human](https://arxiv.org/abs/2606.08919)
*Emre Turan*
...
### [90] [AlloSpatial: Agentic Harness Framework for Spatial Reasoning in Foundation Models](https://arxiv.org/abs/2606.08952)
...
### [91] [An Effective Router for Vision-Language Model Selection](https://arxiv.org/abs/2606.08970)
...
### [92] [Diverse Thinking Schemata Elicit Better Reasoning in Large Language Models](https://arxiv.org/abs/2606.08974)
...
### [93] [RTL-BenchLS: A Large-Scale Benchmark for RTL Reasoning and Generation with Large Language Models](https://arxiv.org/abs/2606.08976)
...
### [94] [Baichuan-M4: A Clinical-Grade Medical Agent System for Continuous Care](https://arxiv.org/abs/2606.08982)
...
### [95] [The Token Not Taken: Sampling, State, and the Variability of AI Agent Outputs](https://arxiv.org/abs/2606.08998)
...
### [96] [LATTEArena: An Evaluation Framework for LLM-powered Tabular Feature Engineering (Extended Version)](https://arxiv.org/abs/2606.09004)
...
### [97] [A Multi-Agent System for IPMSM Design Optimization via an FEA-AI Hybrid Approach](https://arxiv.org/abs/2606.09037)
...
### [98] [Personalization Meets Safety:Mechanisms,Risks,and Mitigations in Personalized LLMs](https://arxiv.org/abs/2606.09038)
...
### [99] [Agent Economics: An Entropy-Controlled Pluralistic Alignment Framework for Preventing Artificial Hivemind in Autonomous Agents](https://arxiv.org/abs/2606.09039)
*Cheonsu Jeong*
...
### [100] [REFLECT: Intervention-Supported Error Attribution for Silent Failures in LLM Agent Traces](https://arxiv.org/abs/2606.09071)
...
### [101] [DynaOD: Dynamic Origin-Destination Flow Generation with Discrete-to-Continuous Temporal Semantic Modeling](https://arxiv.org/abs/2606.09086)
...
### [102] [Graph2Idea:Retrieval-Augmented Scientific Idea Generation with Graph-Structured Contexts](https://arxiv.org/abs/2606.09105)
...
### [103] [ComplexConstraints and Beyond: Expert Rubrics for RLVR](https://arxiv.org/abs/2606.09118)
...
### [104] [A Regret Minimization Framework on Preference Learning in Large Language Models](https://arxiv.org/abs/2606.09124)
...
### [105] [Late-Layer Fusion is Enough: Dual-Path Vision Token Routing for Multimodal Large Language Models under Visual Saturation](https://arxiv.org/abs/2606.09131)
...
### [106] [Vision Language Model Helps Private Information De-Identification in Vision Data](https://arxiv.org/abs/2606.09132)
...
### [107] [Reliable to Expressive: A Curriculum for Rubric-Following Safety Judges](https://arxiv.org/abs/2606.09165)
...
### [108] [IMUG-Bench: Benchmarking Unified Multimodal Models on Interleaved Understanding and Generation](https://arxiv.org/abs/2606.09169)
...
### [109] [MASS: Deep Research for Social Sciences with Memory-Augmented Social Simulation](https://arxiv.org/abs/2606.09198)
...
### [110] [FF-JEPA: Long-Horizon Planning in World Models with Latent Planners](https://arxiv.org/abs/2606.09311)
...
### [111] [Anything2Skill: Compiling External Knowledge into Reusable Skills for Agents](https://arxiv.org/abs/2606.09316)
...
### [112] [TRL-Bench: Standardizing Cross-Paradigm Representation-Level Evaluation of Tabular Encoders](https://arxiv.org/abs/2606.09323)
...
### [113] [Leveraging Structural Constraints for Diffusion-based Neural TSP Solvers](https://arxiv.org/abs/2606.09343)
...
### [114] [Experience Makes Skillful: Enabling Generalizable Medical Agent Reasoning via Self-Evolving Skill Memory](https://arxiv.org/abs/2606.09365)
...
### [115] [Capability-Aligned Hierarchical Learning for Tool-Augmented LLMs](https://arxiv.org/abs/2606.09371)
...
### [116] [From Coarse to Fine: Managing Temporal Granularity in Spatio-Temporal Data for Fine-Grained Traffic Prediction](https://arxiv.org/abs/2606.09392)
...
### [118] [Correct Looks Better: Pairwise Comparisons Reveal Accuracy Rankings](https://arxiv.org/abs/2606.09409)
...
### [119] [Capacity, Not Format: Rethinking Structured Reasoning Failures](https://arxiv.org/abs/2606.09410)
*Hengxin Fan*
...
### [120] [WeaveBench: A Long-Horizon, Real-World Benchmark for Computer-Use Agents with Hybrid Interfaces](https://arxiv.org/abs/2606.09426)
...
### [121] [Bayesian Selective Latent Inference for Wastewater-First Influenza Monitoring](https://arxiv.org/abs/2606.09433)
...
### [122] [SIFT: Selective-Index For Fast Compute of RAG Prefill by Exploiting Attention Invariance](https://arxiv.org/abs/2606.09441)
...
### [123] [AliyunConsoleAgent: Training Web Agents in Real-World Cloud Environments via Distillation and Reinforcement Learning](https://arxiv.org/abs/2606.09447)
...
### [124] [TheoremBench: Evaluating LLMs on Theorem Proving in Formal Mathematics](https://arxiv.org/abs/2606.09450)
...
### [125] [Emergent alignment and the projectability of ethical personas](https://arxiv.org/abs/2606.09475)
...
### [126] [LLM-Orchestrated Conformance Checking in Stroke Care Without Computer-Interpretable Guidelines](https://arxiv.org/abs/2606.09489)
...
### [127] [Deterministic Integrity Gates for LLM-Assisted Clinical Manuscript Preparation: An Auditable Biomedical Informatics Architecture](https://arxiv.org/abs/2606.09500)
...
### [128] [From Rigid to Dynamic: Entropy-Guided Adaptive Inference for Long-Context LLMs](https://arxiv.org/abs/2606.09508)
...
### [129] [AI Scientists Are Only as Good as Their Evidence: A Stratified Ablation of Proprietary Data and Reasoning Skills in Drug-Asset Valuation](https://arxiv.org/abs/2606.09556)
*Yinan Wang*
...
### [130] [PRISM: Recovering Instruction Sets from Language Model Activations](https://arxiv.org/abs/2606.09563)
...
### [131] [Self-Explainability in Self-Adaptive and Self-Organising Systems: Status and Research Directions](https://arxiv.org/abs/2606.09568)
...
### [132] [TABVERSE: Benchmarking Cross-Format Table Understanding in LLMs and VLMs](https://arxiv.org/abs/2606.09578)
...
### [133] [Optical Reasoning: Rethinking Images as an Expressive Reasoning Medium Beyond Text](https://arxiv.org/abs/2606.09585)
...
### [134] [Next-Token Prediction Learns Generalisable Representations of Sleep Physiology](https://arxiv.org/abs/2606.09605)
...
### [135] [From 0-to-1 to 1-to-N: Reproducible Engineering Evidence for MetaAI Recursive Self-Design](https://arxiv.org/abs/2606.09663)
...
### [136] [Frequency-based Constrained Sampling for Interval Patterns](https://arxiv.org/abs/2606.09666)
...
### [137] [SpatialWorld: Benchmarking Interactive Spatial Reasoning of Multimodal Agents in Real-World Tasks](https://arxiv.org/abs/2606.09669)
...
### [138] [Correlation Is Not Enough: Embedding Human Metadata for Individual Causal Discovery](https://arxiv.org/abs/2606.09672)
...
### [139] [(Auto)formalization is supposed to be easy: Trellis process semantics for spelling out rigorous proofs](https://arxiv.org/abs/2606.09674)
*Wesley Pegden*
...
### [140] [Proxy Reward Internalization and Mechanistic Exploitation: A Learned Precursor to Reward Hacking and Its Generalization](https://arxiv.org/abs/2606.09711)
...
### [141] [Beyond Probabilistic Similarity: Structural, Temporal, and Causal Limitations of Retrieval-Augmented Generation in the Legal Domain](https://arxiv.org/abs/2606.09724)
*Hudson de Martim*
...
### [142] [SearchSwarm: Towards Delegation Intelligence in Agentic LLMs for Long-Horizon Deep Research](https://arxiv.org/abs/2606.09730)
...
### [143] [Multi-Turn Evaluation of Deep Research Agents Under Process-Level Feedback](https://arxiv.org/abs/2606.09748)
...
### [144] [Collaborative Human-Agent Protocol (CHAP)](https://arxiv.org/abs/2606.09751)
...
### [147] [Offline Reinforcement Learning for Plasma Control in Nuclear Fusion: Codebase and Benchmark](https://arxiv.org/abs/2606.07550)
*Yang Fu,Haomin Bao,Rohit Sonker,Xiaoyan Hu,Aravind Venugopal,Jeff Schneider,Jiayu Chen*

Main category: cs.LG

TL;DR: 提出了RL4F基准测试，用于评估核聚变等离子体控制的离线强化学习方法，基于DIII-D真实数据构建了四个全剖面跟踪任务。


<details>
  <summary>Details</summary>
Motivation: 在线强化学习在真实托卡马克设备上成本高且风险大，而离线强化学习提供了一个有前景的替代方案。但目前缺乏标准化的基准测试来衡量等离子体控制领域的进展。

Method: 基于DIII-D托卡马克历史放电数据构建动态函数，创建了包含旋转、密度、温度和压力四个全剖面跟踪任务的闭环评估环境，并评估了一系列模仿学习和离线强化学习基线方法。

Result: 离线模型强化学习方法在大多数目标上获得了最佳平均性能，但没有任何单一方法在所有任务中占主导地位，突显了动态建模在复杂长时域等离子体控制任务中的重要性。

Conclusion: RL4F基准测试为核聚变社区和离线强化学习算法开发提供了标准化评估框架，开源了代码库、数据集和评估框架以促进进一步研究。
...
### [148] [MedicalRec: Medical recommender system for image classification without retraining](https://arxiv.org/abs/2606.07553)
...
TL;DR: 开发了基于Transformer的医学图像分类模型推荐系统MedicalRec，使用包含5000+模型记录的MedicalRec-Bench数据集，在18个特征模式下达到75.5%的HitRate@100


<details>
  <summary>Details</summary>
Motivation: 机器学习和深度学习在医疗领域的快速应用带来了巨大的计算能耗和碳排放问题，研究人员通常需要反复试错来选择合适模型，这造成了能源浪费和环境影响

Method: 收集了3000篇医学图像分类文章的5000多条模型测试记录，构建MedicalRec-Bench数据集；使用基于Transformer的MedicalRec推荐系统模型，在四种特征数量模式(I:5, II:9, III:11, IV:18)下进行评估

Result: MedicalRec模型在数据集评估和12个基准模型对比中表现优异，在MedicalRec IV(18特征)模式下达到最大HitRate@100为75.5%

Conclusion: 提出的MedicalRec推荐系统能有效减少医学图像分类模型选择的试错过程，降低计算能耗和环境影响，为研究人员提供智能化的模型推荐方案
...
### [149] [SPIN: Decentralized Swarm Control via Tensorized Policy Coordination](https://arxiv.org/abs/2606.07557)
*Zhaowen Fan*

Main category: cs.LG

TL;DR: SPIN框架通过张量网络分解降低多智能体协同的计算复杂度，实现资源受限边缘平台上的可扩展群智能控制


<details>
  <summary>Details</summary>
Motivation: 去中心化多智能体群协调在资源受限边缘平台上存在两个根本瓶颈：联合动作空间指数级扩展和高延迟通信开销。传统方法无法在有限计算资源下实现大规模智能体协同。

Method: 提出Swarm Policy Interference Network（SPIN）框架：1）将群拓扑建模为压缩张量网络，将局部多智能体小团的联合策略张量分解为Matrix Product State（MPS）链；2）采用解耦的混合神经符号控制管道，本地多层神经网络作为结构协调编码器离线预训练，运行时通过Radon-Nikodým导数实现零样本重要性重加权。

Result: 计算复杂度从指数级O(n^m)降至线性O(m·n·χ²)。在离散时间多智能体仿真环境中验证了跟踪、去中心化分散/区域覆盖和多目标协调等任务，实现了稳定的目标导向运动、防崩溃空间扩展和结构化子群形成。

Conclusion: SPIN框架为可处理的、低功耗边缘群智能提供了一条数学基础坚实的路径，绕过了联合动作空间指数扩展和高延迟通信的根本瓶颈，实现了在资源受限平台上可扩展的多智能体协调。
...
### [150] [Boundary Variance Inflation Causes Acquisition Bias in Gaussian Processes](https://arxiv.org/abs/2606.07561)
*Maria Bånkestad,Sanna Jarl,Jens Sjölund*

Main category: cs.LG

TL;DR: 高斯过程在边界区域后验方差膨胀导致贝叶斯优化中的边界诱导采集偏差问题


<details>
  <summary>Details</summary>
Motivation: 高斯过程在有界域上使用平稳核函数时，在边界附近会出现后验方差膨胀现象。这一已知问题在地统计学中已被识别，在贝叶斯优化中会导致过度探索，但其原因和边界诱导采集偏差的影响尚未深入研究。

Method: 通过几何机制分析根本原因：在域边界处核函数相关邻域的截断会产生与观测无关的扭曲，这种扭曲随维度增加而加剧。研究这种扭曲在三种采集类别中的表现：方差最大化、负积分后验方差和期望预测信息增益。引入函数无关的选择剖面诊断方法，适用于任意采集函数、核函数和有界域几何结构。

Result: 发现边界诱导扭曲在不同采集函数中产生特定模式：方差最大化倾向于在角点集中选择，而负积分后验方差和期望预测信息增益则将选择移动到轴向对齐的内部壳层。这些模式独立于目标函数，表明采集行为可能被核函数几何结构而非任务特定不确定性所主导。

Conclusion: 核函数几何结构可以在贝叶斯优化中主导采集行为，导致与任务无关的偏差。通过引入函数无关的诊断工具可以量化这种边界诱导采集偏差，为理解和改进有界域上的高斯过程优化提供了重要见解。
...
### [151] [Emergence via Phase Transitions: Mechanism Landscapes and Universal Convergence Across Complex Systems](https://arxiv.org/abs/2606.07563)
*Truong Xuan Khanh*

Main category: cs.LG

TL;DR: 提出层次涌现框架(HEF)作为研究跨机器学习、生物和物理系统收敛现象的普适性框架，将涌现建模为受热力学和信息论约束的机制景观中的相变，并在模块化算术transformer的111个实验中验证了框架预测。


<details>
  <summary>Details</summary>
Motivation: 尽管微观细节完全不同，但在机器学习、生物学和物理学中，独立演化的系统往往会收敛到惊人相似的高层结构（如grokking电路跨随机种子的收敛、进化谱系重新发现相似代谢解决方案、重整化流接近共同固定点），需要一个统一的理论框架来解释这种收敛现象。

Method: 提出层次涌现框架(HEF)，将涌现建模为机制景观中的相变，该景观受热力学和信息论定律约束。框架引入临界能量阈值Ec，将探索机制竞争的探索机制与由唯一最小成本机制主导的收敛机制分开。在结构假设下，证明物理可行性、推导严格度量收缩、建立独立于初始条件向唯一固定点表示的收敛。通过有效信息和机制竞争熵将此收敛结构与因果涌现联系起来。在模块化算术transformer上进行111个实验验证，分析权重范数、精度曲线等指标。

Result: 在延迟泛化("grokking")实验中识别出Ec转变的可重复经验指纹：权重范数在grokking前系统地达到峰值（92%的运行中）。归一化精度曲线坍缩到tanh拐点（R^2=0.93），与Landau-Ginzburg普适性类别一致。所有grokked模型收敛到0.9745±0.014，与初始化、权重衰减或训练分数无关（ANOVA p>0.13）。

Conclusion: HEF不是作为涌现的普适理论提出，而是作为可证伪的数学框架，用于研究复杂系统中的收敛现象。框架为跨学科收敛现象提供了统一的数学脚手架，实证结果支持其预测能力。
...
### [152] [STARIXNet: Multivariate and Multi-attribute Deep Learning Approach to Real-Time Resource Allocation in Cloud Platforms](https://arxiv.org/abs/2606.07565)
*Ahmed Abdulaal,Maruf Aytekin,Thilaga kumaran Srinivasan,Tomer Lancewicki*

Main category: cs.LG

TL;DR: STARIXNet是一个轻量级神经网络，用于云平台中微服务的智能扩展，通过捕捉多系统指标间的时空关系，在保证服务稳定性的前提下优化资源分配，在Walmart的实际部署中实现了10%-50%的成本节约。


<details>
  <summary>Details</summary>
Motivation: 当前微服务扩展解决方案存在三个主要问题：1）仅限于单变量空间（通常只关注CPU使用率）；2）作为纯粹的预测任务处理，过分关注预测精度而忽视了低估风险和系统响应延迟；3）替代方案计算复杂，不适合大规模实时部署。

Method: STARIXNet是一种轻量级神经网络，在多变量空间中通过捕捉多个系统指标间的时空关系来指导资源分配决策。它建模多个准依赖属性，特别是(S)easonal（季节性）、(T)emporal（时间性）、(A)uto-(R)egressive (I)ntegrated（自回归整合）和e(X)ogenous（外生）模式，然后实施聚合策略来最终确定扩展决策，优先考虑服务稳定性，其次是成本效率，而非原始预测精度。

Result: 在真实场景中通过基准测试验证了STARIXNet的性能。在Walmart的关键生产微服务中部署STARIXNet，实现了10%到50%的实际成本节约，同时还通过改进服务稳定性和客户体验带来了无形效益。

Conclusion: STARIXNet成功解决了当前微服务扩展解决方案的局限性，通过多变量分析、轻量级神经网络架构以及优先考虑服务稳定性的决策策略，为大规模云平台的实时微服务扩展提供了一种实用且高效的方法，在真实生产环境中证明了其有效性和经济效益。
...
### [153] [TriHead-GAN: A Generative Adversarial Network with Triple-Head Discriminator for Carbon Emission Time Series Generation](https://arxiv.org/abs/2606.07569)
*Zesen Wang,Lijuan Lan,Yonggang Li,Chunhua Yang*

Main category: cs.LG

TL;DR: TriHead-GAN是一种基于Transformer的生成对抗网络，通过三重判别器监督生成高质量的城市级高频碳排放时间序列数据，解决了现有方法在保持跨变量相关性和时间动态性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 碳排放准确监测对气候政策和欧盟碳边境调节机制至关重要，但城市级高频监测数据极为稀缺，限制了深度学习模型的应用。现有GAN和扩散模型在保持碳排放数据的跨变量相关性（CO2与协同污染物、气象因素之间）和时间动态性（首差统计量）方面存在不足。

Method: 提出TriHead-GAN框架：1）生成器结合全局自注意力与局部时间卷积，逐步噪声注入和反平滑损失；2）三重判别器联合监督：通过Wasserstein距离评估分布真实性、通过无泄漏回归监督跨变量依赖性、通过相邻差分预测监督逐步时间平滑性。

Result: 在自收集的长沙碳排放数据集、两个公共碳排放数据集（中国、美国）和ETTh1基准上实验表明，TriHead-GAN在大多数设置下优于主流基线方法，生成的合成窗口在低资源碳排放监测场景中提高了下游预测精度。

Conclusion: TriHead-GAN通过三重监督机制有效解决了碳排放时间序列生成中的关键挑战，为数据稀缺情况下的碳排放监测提供了高质量合成数据，有望支持更准确的碳排放预测和气候政策制定。
...
### [154] [Enabling KV Caching of Shared Prefix for Diffusion Language Models](https://arxiv.org/abs/2606.07571)
*Younghun Go,Jaehoon Han,Changyong Shin,Chuk Yoo,Gyeongsik Yang*

Main category: cs.LG

TL;DR: bicache是一种针对扩散语言模型的双向前缀KV缓存技术，能在保持模型精度的同时显著提升服务吞吐量


<details>
  <summary>Details</summary>
Motivation: 现有的KV缓存技术是为LLMs设计的，假设KV值在计算后保持不变。但在扩散语言模型中，双向注意力机制意味着更新任何token都会动态改变整个上下文及其对应的KV值，导致现有缓存技术在DLMs中失效，使模型精度崩溃至接近零。

Method: 提出了bicache（双向前缀缓存），这是首个针对DLMs共享前缀的KV缓存技术。基于关键观察：共享前缀KV在浅层中保持稳定并可重用，而浅层深度取决于每个请求中共享前缀token的比例。bicache动态识别重用共享前缀KV的安全层深度，消除冗余计算。

Result: 评估显示，bicache相比现有技术显著提升服务吞吐量36.3%-98.3%，同时没有精度崩溃问题（仅有0-1.8%的精度差异）。

Conclusion: bicache成功解决了DLMs中KV缓存的关键挑战，通过动态确定可安全重用共享前缀KV的层深度，在保持模型精度的同时大幅提升了服务吞吐量，为高效DLM服务提供了重要技术。
...
### [155] [When Should an AI Scientist Stop? Verifiable Experiment Steering and Refusal for Autonomous Discovery](https://arxiv.org/abs/2606.07576)
*Neel Tushar Shah,Manglam Kartik*

Main category: cs.LG

TL;DR: CARTOGRAPH是一个AI科学家的验证层，通过实验指导、模糊解析和库不充分检测来提高科学发现的可靠性，在多个测试平台上表现出色，并能识别和撤销不合适的机制识别。


<details>
  <summary>Details</summary>
Motivation: 为了解决AI科学家在科学发现过程中的验证问题，确保科学声明的可靠性，防止错误识别和虚假阳性结果。

Method: CARTOGRAPH结合三个核心组件：未解决子空间实验指导（select）、显式模糊度闭合（resolve）和基于残差的库不充分检测（refuse）。在局部线性高斯桥假设下，原始未解决投影是各向同性的未解决Fisher信息迹，而CARTOGRAPH-A是精确的未解决A-最优规则。

Result: 在五个测试平台上，CARTOGRAPH-A在d=8时以129胜/0平/15负显著优于原始投影（p<10^-21）。该框架能识别并随后撤销三个库外药代动力学机制，同时保持对库内对照的正确识别。在低维药代动力学和过滤EPA设置中，观察到理论预测的接近平局情况。在回顾性审计中，refuse防护机制标记了所有4个后来被手动重新分析标记为不确定的声明，同时通过了32/36个已确认的声明。

Conclusion: CARTOGRAPH为AI科学家提供了一个有效的验证框架，能显著提高科学发现的可靠性，减少虚假阳性结果，并能识别和撤销不合适的机制识别，在多个领域显示出强大的实用价值。
...
### [156] [MST-Direct at Scale: Multivariate and Conditional Geostatistical Simulation via Sinkhorn Optimal Transport](https://arxiv.org/abs/2606.07578)
*Tcharlies Bachmann Schmitz*

Main category: cs.LG

TL;DR: MST-Direct方法从原有的双变量、无条件、小网格扩展到多变量、条件和大网格设置，通过稀疏Sinkhorn匹配、FFT-MA高斯骨架匹配和硬数据条件化等技术，在保持多元联合分布精确性的同时解决了原方法的三个主要限制。


<details>
  <summary>Details</summary>
Motivation: 解决原MST-Direct方法的三个主要限制：1) 扩展到数千节点以上的可扩展性问题；2) 扩展到多变量设置；3) 支持硬数据条件化。

Method: 1) 使用稀疏、候选限制的Sinkhorn匹配器实现O(nC)内存复杂度；2) 将目标值元组匹配到独立的FFT-MA高斯骨架上，以重现规定的变差函数；3) 通过克里金法固定观测数据元组在空间位置，同时条件化骨架。

Result: 在六变量、异方差、强非线性参考分布上验证，MST-Direct在无条件(200x200)和条件(100x100，200个硬数据样本)场景下都能精确重现联合分布(零直方图误差)，完全满足硬数据要求，准确重现规定的空间相关结构，而PPMT仍然是近似方法。

Conclusion: 扩展后的MST-Direct方法成功解决了原方法的三个限制，能够在多变量、条件和大网格设置下精确重现多元联合分布和空间相关结构，相比PPMT等近似方法具有明显优势。
...
### [157] [Training-Inference Kernel Contracts: Bounding Divergence in Post-Training and Deployment](https://arxiv.org/abs/2606.07581)
*Bruce Changlong Xu,Lan Wu*

Main category: cs.LG

TL;DR: 这篇论文提出了"内核合约"框架，用于规范训练和推理内核在有限精度下的可接受差异，以确保深度学习系统在训练和服务阶段的行为一致性。


<details>
  <summary>Details</summary>
Motivation: 现代后训练流程中，训练内核和推理内核在有限精度下可能产生不同的分布，这种差异集中在基准测试中代表性不足的切片上，导致策略评估的不一致性。

Method: 提出了内核合约框架C=(N,S,R,O,Pi)，结合数值、统计、运行时和可观测性条款；推导了从logit漂移到总变差距离再到奖励漂移的有界链；针对RL后训练，基于每个token的重要性比率漂移推导了策略梯度偏差界限；设计了四阶段推广流程、在线路由循环和简约YAML DSL。

Result: 建立了从数值差异到性能影响的系统化数学框架，提出了可验证的合约机制，能够将内核差异违规转化为路由行动，确保系统行为的一致性。

Conclusion: 内核合约为深度学习系统的训练和服务行为一致性提供了系统化框架和词汇，通过合约优先的方法规范了可接受的数值差异，但本文主要是框架和概念验证，未报告生产规模的经验验证。
...
### [158] [Customer Churn Prediction on Structured Data Using FT-Transformer and Stacking Ensembles](https://arxiv.org/abs/2606.07582)
*Joyjit Roy,Samaresh Kumar Singh,Laxmi Shaw*

Main category: cs.LG

TL;DR: 本文提出了一种集成特征令牌化Transformer（FT-Transformer）和梯度提升树（XGBoost）的混合架构，通过校准感知堆叠提升结构化数据上的客户流失预测性能，在公开银行流失数据集上显著优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 客户流失预测在数据驱动行业中至关重要，但结构化数据集上的预测面临类别不平衡、非线性特征交互和异质特征类型等挑战。传统方法在统计验证、概率校准和可复现性方面存在不足，需要一种更强大的混合架构来提升预测性能。

Method: 1. 使用FT-Transformer捕获高阶特征交互（通过自注意力机制）
2. 使用XGBoost捕获梯度提升的决策边界（具有互补归纳偏置）
3. 采用类别加权损失函数处理类别不平衡（避免合成过采样）
4. 使用出折（OOF）堆叠与逻辑回归元学习器集成模型（校准过度自信的基模型输出并学习最优组合权重）

Result: 在公开银行流失数据集上，混合模型实现62.10% F1分数、0.861 AUC-ROC和0.647 PR-AUC。与多层感知机（MLP）基线相比，F1分数提升3.37点，AUC提升0.027，使用5x5交叉验证并报告95%置信区间。消融研究显示Transformer组件和堆叠策略都对性能有实质性贡献。

Conclusion: 提出的混合方法通过集成FT-Transformer和梯度提升树，结合校准感知堆叠，为结构化表格数据的当代流失预测提供了一个可复现且可扩展的参考架构，有效解决了统计验证、概率校准和可复现性方面的问题。
...
### [159] [Outage Detection in Self-Healing Smart Grids Using Reinforcement Learning with Spectral Graph Neural Networks](https://arxiv.org/abs/2606.07583)
*Lihui Liu,Mucun Sun,Caisheng Wang*

Main category: cs.LG

TL;DR: 提出一种基于谱图神经网络的智能电网故障管理强化学习框架，用于配电网络快速恢复供电


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法在智能电网故障恢复中响应慢、计算成本高，且常规图神经网络只能捕捉空间域信息，无法有效利用频域信息来建模电网全局结构模式和系统级交互

Method: 提出谱图强化学习框架，使用谱图神经网络学习最优的功率恢复策略，在频域中捕获电网全局结构信息

Result: 在修改后的IEEE 13、34和123节点测试系统上验证，该方法能够实现接近最优的实时性能，并在多种故障场景下具有良好的泛化能力

Conclusion: 基于谱图神经网络的强化学习框架有效提升了配电网络故障管理的系统弹性，解决了传统方法响应慢和频域信息利用不足的问题
...
### [160] [From Human Guidance to Autonomy: Agent Skill System for End-to-End LLM Deployment on Spatial NPUs](https://arxiv.org/abs/2606.07586)
*Jiajie Li,Erwei Wang,Zhiru Zhang,Samuel Bayliss*

Main category: cs.LG

TL;DR: 提出两阶段方法，通过人类引导开发到代理自主，在AMD XDNA 2 NPU上实现LLM端到端部署，显著提升性能并实现功能泛化。


<details>
  <summary>Details</summary>
Motivation: 空间神经处理单元(NPUs)为边缘LLM推理提供能效平台，但端到端部署仍劳动密集。现有AI编码代理研究多关注单核优化而非资源受限NPU上的端到端LLM部署。

Method: 两阶段方法：第一阶段通过人类引导代理辅助开发Llama-3.2-1B参考部署，记录优化轨迹和经验为结构化文档；第二阶段将文档提炼为包含8个阶段的代理技能系统，协调优化和调试技能，每阶段严格强制执行数值正确性。

Result: Llama-3.2-1B参考部署相比手动优化基线，prefill加速2.2倍，decode加速4.0倍。代理技能系统自主部署8个解码器LLM，完成时间0.5-4小时，通过数值正确性检查，实现功能泛化。其中3个模型达到或超过参考部署性能。

Conclusion: 该方法成功降低LLM在空间NPU上端到端部署成本，通过结构化技能系统实现代理自主部署，为资源受限边缘设备上的LLM推理提供高效解决方案。
...
### [161] [The Routing Plateau: Understanding and Breaking the Accuracy Limits of LLM Routers](https://arxiv.org/abs/2606.07587)
*Yifan Lu,Qiyue Zhang,Shenrun Zhang,Zhibo Yu,Zhuang Wang,Hanjie Chen,Jiarong Xing*

Main category: cs.LG

TL;DR: 本文研究发现当前LLM路由方法存在"路由平台期"现象，许多方法虽然使用不同技术，但性能相似且远低于理想路由器，主要原因是现有路由器缺乏查询级别的细粒度预测能力。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM路由已成为提升LLM服务成本-质量权衡的热门方法，各种路由方法层出不穷，但缺乏对这些方法性能局限性的系统性理解，需要探究为什么当前路由方法性能远低于理想路由器，以及如何突破这些限制。

Method: 对21种路由方法在5个基准测试上进行广泛研究，包括基于聚类的路由器、学习型分类器、成对排序和置信度方法等。深入分析路由平台期现象的原因，研究预测瓶颈问题，并探索超越平台期的方法，如更大训练数据集、更强编码器和端到端微调。

Result: 发现路由平台期现象普遍存在：许多方法（包括kNN）达到相似的准确率，收敛到远低于理想路由器的窄性能范围。确定平台期主要由可预测性瓶颈造成：当前路由器主要学习全局平均模型性能趋势，而非细粒度的查询特定路由信号。发现更大训练数据集、更强编码器和端到端微调可以进一步提高路由准确率。

Conclusion: 本文揭示了当前路由方法的共同局限性，即存在路由平台期现象，主要原因是预测瓶颈限制了查询级别的细粒度决策。研究结果为社区提供了建设更有效路由系统的见解和可操作方向，包括使用更大数据集、更强编码器和端到端优化来突破平台期。
...
### [162] [Optimality of Sequential Filtering Under Independent Cost and Selectivity Models](https://arxiv.org/abs/2606.07589)
*Hrishikesh Paranjape,Abhishek Mandal,Xian Sun*

Main category: cs.LG

TL;DR: 研究了顺序过滤管道的优化问题，证明了在独立模型下按成本与拒绝概率的比值递增排序能最小化期望总成本，比常见启发式方法表现更好。


<details>
  <summary>Details</summary>
Motivation: 在大规模系统中，顺序过滤管道是常见的设计模式，但过滤器的排序通常由启发式方法决定，缺乏形式化保证。

Method: 将顺序过滤形式化为期望成本优化问题，在独立模型下证明按成本与拒绝概率比值递增排序的最优性，并通过广泛的蒙特卡洛模拟进行验证。

Result: 最优排序在所有运行中都严格优于常见启发式方法，不仅在期望值上，而且在整个结果分布上都有更好表现。

Conclusion: 在顺序过滤管道中，按过滤器成本与拒绝概率比值递增排序能最小化期望总成本，为系统设计提供了理论指导。
...
### [163] [ResearchClawBench: A Benchmark for End-to-End Autonomous Scientific Research](https://arxiv.org/abs/2606.07591)
...
TL;DR: ResearchClawBench是一个用于评估自主科学研究能力的基准测试，包含来自10个科学领域的40个任务。评估结果显示当前AI系统在科学研究中的可靠性仍然有限，最强的自主代理和LLM平均得分仅为20-26分（满分100）。


<details>
  <summary>Details</summary>
Motivation: 随着AI编码代理在科学研究中的使用日益增多，但其端到端的自主研究能力仍难以验证。现有的评估方法不足以准确衡量AI系统重新发现已有科学成果的能力。

Method: 提出了ResearchClawBench基准，基于40个真实已发表论文的任务，隐藏目标论文并提供相关文献和原始数据。使用专家策划的多模态评分标准，将目标科学成果分解为加权标准，既能评估对目标论文的重新发现能力，又为新的发现留有空间。通过统一协议评估七个自主研究代理，并通过轻量级ResearchHarness评估十七个原生LLM。

Result: 当前系统远未达到可靠的重新发现水平：最强的自主代理Claude Code平均得分21.5分，最强的ResearchHarness LLM Claude-Opus-4.7平均得分20.7分，LLM前沿模型平均得分仅为26.5分。错误分析显示失败主要集中在实验协议不匹配、证据不匹配和缺失科学核心三个方面。

Conclusion: ResearchClawBench为衡量自主科学研究的进展提供了可复现的评估前沿。当前AI系统在自主科学研究能力方面仍有很大提升空间，特别是在实验设计、证据分析和科学核心理解方面需要改进。
...
### [164] [UNIQ: Conformal Calibration for Adaptive Conservatism in Offline Reinforcement Learning](https://arxiv.org/abs/2606.07592)
*Aditya Upadhyay*
...
### [165] [Shortcuts in the Tail: Debiasing via Post-Hoc Spectral Compression of Fine-Tuning Updates](https://arxiv.org/abs/2606.07596)
...
### [166] [Repetition Mismatch: Why Data Mixture Experiments Don't Scale and How to Fix Them](https://arxiv.org/abs/2606.07597)
...
### [167] [A Topological Characterization of Graph Neural Networks via Stochastic Block Model Embeddings on the n-Sphere](https://arxiv.org/abs/2606.07598)
*Gopal Anantharaman*
...
### [168] [DiffoR: A Unified Continuous Generative Framework for Universal Ordinal Regression](https://arxiv.org/abs/2606.07599)
...
### [169] [Reachability and asymptotics of Gaussian Transformer dynamics](https://arxiv.org/abs/2606.07600)
...
### [170] [LFNO: Bridging Laplace and Fourier via Transient-Steady Decomposition](https://arxiv.org/abs/2606.07601)
...
### [171] [Sample-Efficient Post-Training for LEGO Spatial-Physics Reasoning](https://arxiv.org/abs/2606.07602)
...
### [172] [MetaEvo: A Meta-Optimization Framework for Experience-Driven Agent Evolution](https://arxiv.org/abs/2606.07603)
...
### [173] [Contribution Weights: A Geometrical Analysis of Self-Attention Transformers](https://arxiv.org/abs/2606.07604)
...
### [174] [SRT: Super-Resolution for Time Series via Disentangled Rectified Flow](https://arxiv.org/abs/2606.07605)
...
### [175] [QDSP: An Interpretable Structured Learning Framework for Predicting Death or Cerebral Palsy in Very Low Birth Weight Infants](https://arxiv.org/abs/2606.07606)
...
### [176] [Position: Genomic Model Research Must Move Beyond Anecdotal Evaluation of Interpretability Methods](https://arxiv.org/abs/2606.07607)
...
### [177] [LEAF: Growing Trees Without Branching for Speech-Aware Large Language Model Post-Training](https://arxiv.org/abs/2606.07610)
...
### [178] [Measuring Poverty and Inequality with Reduced Data: A Machine Learning Approach Using Nigerian Household Data](https://arxiv.org/abs/2606.07614)
...
### [179] [Structured Neuron Pruning in Deep Neural Networks Using Multi-Armed Bandits](https://arxiv.org/abs/2606.07615)
...
### [180] [Item Response Scaling Laws: A Measurement Theory Approach for Efficient and Generalizable Neural Scaling Estimation](https://arxiv.org/abs/2606.07616)
...
### [181] [Query Lens: Interpreting Sparse Key-Value Features with Indirect Effects](https://arxiv.org/abs/2606.07617)
...
### [182] [ScaleSweep: Accurate NVFP4 Post-Training Quantization of LLMs via Block Scale Initialization](https://arxiv.org/abs/2606.07618)
*Li Lin,Xiaojun Wan*
...
### [183] [Graph Neural Networks for Predicting Solvability of Finite Groups](https://arxiv.org/abs/2606.07619)
*Tal Weissblat*
...
### [184] [HASA: Subnet Allocation for Compute-Constrained Model-Heterogeneous Federated Learning](https://arxiv.org/abs/2606.07621)
...
### [185] [Airport Terminal Passenger Queue Forecasting for Departure Gates and Security Checkpoints](https://arxiv.org/abs/2606.07622)
...
### [186] [Finite Certificates for In-Context Determinacy and a Threshold Theory of Emergence in Language Models](https://arxiv.org/abs/2606.07623)
...
### [187] [Sequential statistical inference for Large Language Models: Representation, validity, and monitoring](https://arxiv.org/abs/2606.07624)
*Yao Xie*
...
### [188] [Learning Transfers: Kan Extensions for Neural Invariants](https://arxiv.org/abs/2606.07627)
*Luciano Melodia*
...
### [189] [Large Language Models Should Learn Personalized Rather Than Aggregated Human Preferences](https://arxiv.org/abs/2606.07629)
*Cristina Garbacea*
...
### [190] [Active Learning with Foundation Model Priors: Efficient Learning under Class Imbalance](https://arxiv.org/abs/2606.07630)
...
### [191] [Trait-space Monitoring for Emergent Misalignment During Supervised Finetuning](https://arxiv.org/abs/2606.07631)
...
### [192] [Evaluation of ML Resource Utilization Requires Model Life Cycle Assessment](https://arxiv.org/abs/2606.07632)
...
### [193] [KITE: A Tri-Modal Transformer Integrating Text, Images, and Knowledge Graphs for Fake News Detection](https://arxiv.org/abs/2606.07651)
...
### [194] [DOG-DPO:Dynamic Optimization in Geometry for Safety Alignment](https://arxiv.org/abs/2606.07678)
...
### [195] [Semantic Cache Distillation: Efficient State Transfer via Reuse and Selective Patching](https://arxiv.org/abs/2606.07684)
...
### [196] [Test-Time Adaptive Composition for Machine Learning as a Service (MLaaS) in IoT Environments](https://arxiv.org/abs/2606.07685)
...
### [197] [Knowledge-Inclusive Adaptive Physics-Informed Neural Network for Microbial Interaction Modelling](https://arxiv.org/abs/2606.07686)
...
### [198] [HARP: Efficient Data Selection for Finetuning Large Language Models](https://arxiv.org/abs/2606.07690)
...
### [199] [BCG-FM: A Foundation Model for Ambient Cardiac Health Sensing](https://arxiv.org/abs/2606.07692)
...
### [200] [Vessel Traffic Flow Prediction on Sparse Data via Spatio-Temporal Graph Neural Networks with a Learnable Tweedie Head](https://arxiv.org/abs/2606.07694)
...
### [201] [DSFNet: Learning Dual-Domain Spectral Operators for Multi-Modality Spatio-Temporal Forecasting in Urban Transportation Systems](https://arxiv.org/abs/2606.07695)
...
### [202] [Adversarial Robustness of Activation Steering in Large Language Models](https://arxiv.org/abs/2606.07696)
*Kien Le,Thai Le*
...
### [203] [Pharmacogenomic Knowledge Graph Augmentation for Graph Neural Network-Based Drug-Drug Interaction Prediction](https://arxiv.org/abs/2606.07698)
*Juergen Dietrich*
...
### [204] [EssentialGIN: a new approach for gene essentiality prediction based on graph isomorphism neural networks](https://arxiv.org/abs/2606.07700)
...
### [206] [How Much Dense Attention is Necessary? Oracle-Guided Sparse Prefill for Full/GQA Layers in Hybrid Long-Context Models](https://arxiv.org/abs/2606.07703)
...
### [207] [FunctionEvolve: Structure-Guided Symbolic Regression with LLMs](https://arxiv.org/abs/2606.07704)
...
### [208] [SAW: Stage-Aware Dynamic Weighting for Multi-Objective Reinforcement Learning in Large Language Models](https://arxiv.org/abs/2606.07705)
...
### [209] [Decoding Naturalistic Emotion Dynamics from the Brain: An LLM-Enhanced Regression Framework](https://arxiv.org/abs/2606.07707)
...
### [210] [WhiFlash: Accelerating Speculative Decoding with Token-Level Cross-Paradigm Routing](https://arxiv.org/abs/2606.07710)
...
### [211] [Rosetta Memory: Adaptive Memory for Cross-LLM Agents](https://arxiv.org/abs/2606.07711)
...
### [212] [Attention at the Theoretical Minimum: A Mathematics of Arrays Framework for Memory-Optimal Transformer Kernels](https://arxiv.org/abs/2606.07713)
...
### [213] [Beyond Accuracy: Interpreting Topic Representation in Suicide Ideation Detection Models](https://arxiv.org/abs/2606.07714)
...
### [214] [A Geometry-Aware Triplane Field Network for Vehicle Aerodynamic Prediction](https://arxiv.org/abs/2606.07724)
...
### [215] [Cutting LLM Evaluation Costs with SySRs: A Bandit Algorithm that Provably Exploits Model Similarity](https://arxiv.org/abs/2606.07726)
...
### [216] [Characterizing the Discrete Geometry of ReLU Networks](https://arxiv.org/abs/2606.07728)
...
### [217] [scCBGM: Interpretable Single-Cell Counterfactual Editing](https://arxiv.org/abs/2606.07760)
...
### [218] [Contrast encodes inductive bias: separating slow noise from dynamics in predictive representation learning](https://arxiv.org/abs/2606.07770)
...
### [219] [A Framework for Evaluating and Benchmarking Concept Drift Detection Methods](https://arxiv.org/abs/2606.07789)
...
### [220] [Byzantine Cheap Talk: Adversarial Resilience and Topology Effects in LLM Coordination Games](https://arxiv.org/abs/2606.07790)
...
### [221] [Mitigating the Contractivity Trap in Diffusion ODEs via Stein Stabilization](https://arxiv.org/abs/2606.07835)
*Shigui Li,Delu Zeng*
...
### [222] [Teacher-Free Self-Training Amplifies but Does Not Compound: A Pass@$K$ Crossover on a Free-Verifier Domain](https://arxiv.org/abs/2606.07856)
*Igor Lima Strozzi*
...
### [223] [Instrumented data for causal scientific machine learning](https://arxiv.org/abs/2606.07865)
*Daniel N. Wilke*
...
### [224] [Still: Amortized KV Cache Compaction in a Single Forward Pass](https://arxiv.org/abs/2606.07878)
...
### [225] [Breaking the Bubble: Asynchronous Pipeline Parallel Training with Bounded Weight Inconsistency](https://arxiv.org/abs/2606.07881)
...
### [226] [Strained Coherence: A Pre-Failure Signal in Coding Agent Execution Trajectories](https://arxiv.org/abs/2606.07889)
...
### [227] [Partially Performative Prediction](https://arxiv.org/abs/2606.07890)
...
### [228] [Temporal Coverage over Density: Parsimonious Training-Set Design for ML Climate Downscaling](https://arxiv.org/abs/2606.07898)
...
### [229] [Layer-wise Derivative Controlled Networks Achieve Competitive Accuracy and Gradient Stability Across Data Regimes](https://arxiv.org/abs/2606.07908)
*Rowan Martnishn*
...
### [230] [CAAL: Contextual Bandits based Online Hand-Craft Active Learning Strategy Selection](https://arxiv.org/abs/2606.07910)
...
### [231] [The Easy, the Hard, and the Learnable: Confidence and Difficulty-Adaptive Policy Optimization for LLM Reasoning](https://arxiv.org/abs/2606.07950)
...
### [232] [Minibatch Selection via Partition Matroid Constrained Gradient Matching](https://arxiv.org/abs/2606.07954)
...
### [233] [Overcoming the Limits of Finite Difference Method; Physics-Informed Neural Network for Noisy High-Dimensional Heat Diffusion](https://arxiv.org/abs/2606.07982)
...
### [234] [Enhancing AI Interpretability and Safety through Localised Architectures](https://arxiv.org/abs/2606.07998)
...
### [235] [Evaluating the Impact of Task Granularity on Catastrophic Forgetting in Continual Learning](https://arxiv.org/abs/2606.08013)
...
### [236] [Semantic Quorum Assurance: Collective Certification for Non-Deterministic AI Infrastructure](https://arxiv.org/abs/2606.08021)
*Jun He,Deying Yu*
...
### [237] [CausShield: Sample Reconstruction-Resilient Vertical FL via Causal Representation Learning](https://arxiv.org/abs/2606.08027)
...
### [238] [Noise-Adaptive High-Probability Regret Bounds for Online Convex Optimization](https://arxiv.org/abs/2606.08028)
...
### [239] [SafeECGMatch: Calibration-Aware Joint Frequency and Time Space Semi-Supervised Learning for Open-Set ECG Classification](https://arxiv.org/abs/2606.08037)
...
### [240] [When Behavioral Safety Evaluation Fails: A Representation-Level Perspective](https://arxiv.org/abs/2606.08044)
...
### [241] [Beyond Homophily: Towards Generalized Graph Reconstruction Attack and Defense](https://arxiv.org/abs/2606.08067)
...
### [242] [DICE: Entropy-Regularized Equilibrium Selection for Stable Multi-Agent LLM Coordination](https://arxiv.org/abs/2606.08068)
...
### [243] [ConSteer-RL: Steering Reasoning Capabilities in Large Language Models via Confidence-Aware Reinforcement Learning](https://arxiv.org/abs/2606.08088)
...
### [244] [Constraint-Aware Optimization for Robust Protein Stability Prediction](https://arxiv.org/abs/2606.08100)
...
### [245] [A Unifying View of Attention Sinks: Two Algorithms, Two Solutions](https://arxiv.org/abs/2606.08105)
...
### [246] [Conditional Random Ordered Transport Spaces](https://arxiv.org/abs/2606.08113)
*Lei Luo,Jian Yang*
...
### [247] [TRUST-SCF: Transformer-based Risk Understanding and Scoring for Transactional Supply Chain Finance](https://arxiv.org/abs/2606.08140)
...
### [248] [LogNEO: A GPT-Neo Reinforcement Learning Framework for Accurate Real-Time Log Anomaly Detection](https://arxiv.org/abs/2606.08153)
...
### [249] [Have I Solved This Before? Retrieving Similar Segmentation Problems for Evolutionary Learning](https://arxiv.org/abs/2606.08155)
...
### [250] [AttentionCap: Transformer Based Capacitance Matrix Learning Toward Full-Chip Extraction](https://arxiv.org/abs/2606.08161)
...
### [251] [Explaining Data Mixing Scaling Laws](https://arxiv.org/abs/2606.08167)
...
### [252] [Frequency-Domain Latent Attention Gating for Cross-Domain Token Aggregation](https://arxiv.org/abs/2606.08191)
...
### [253] [Neural Field Tokenizations with Hierarchy and Spatial Locality Priors](https://arxiv.org/abs/2606.08204)
...
### [254] [Public Machine Learning Solver Framework for Novices in the Machine Learning Domain](https://arxiv.org/abs/2606.08212)
...
### [255] [How Deep Are Deep GPs, Really? A Sharp Threshold and a Non-Gaussian Limit for Compositional GPs](https://arxiv.org/abs/2606.08218)
...
### [256] [De novo molecular generation with optical property preconditioning at the token level](https://arxiv.org/abs/2606.08221)
...
### [257] [GPT-Micro: A large language paradigm for accelerated, inexpensive, and thermodynamics-consistent discovery of constitutive models in manufacturing](https://arxiv.org/abs/2606.08238)
...
### [258] [Differentially Private Synthetic Data via APIs 4: Tabular Data](https://arxiv.org/abs/2606.08259)
...
### [259] [Causal Semantic Alignment for LLM-based Time Series Forecasting](https://arxiv.org/abs/2606.08262)
...
### [260] [Causal Agent Replay: Counterfactual Attribution for LLM-Agent Failures](https://arxiv.org/abs/2606.08275)
*Jaineet Shah*
...
### [261] [Mesh Graph Neural Network Framework for Accelerating Finite Element Simulation for Arbitrary Geometries](https://arxiv.org/abs/2606.08287)
...
### [262] [On solving symmetric multi-type orthogonal non-negative matrix tri-factorization problem](https://arxiv.org/abs/2606.08291)
...
### [263] [QueryWeaver: Reliable Multi-Tool Query Execution Planning via LLM-Based Graph Generation](https://arxiv.org/abs/2606.08300)
...
### [264] [GeoGNN: Time Series Geo-Localization using Two-Tower Graph Neural Networks](https://arxiv.org/abs/2606.08303)
...
### [265] [Towards Graph Foundation Models for Dynamics in Complex Networked Systems: Lessons from Super-Spreader Identification in Multilayer Networks](https://arxiv.org/abs/2606.08306)
...
### [266] [Fourier fractal dimension to predict the generalization of deep neural networks](https://arxiv.org/abs/2606.08308)
...
### [267] [Where the Score Lives: A Wavelet View of Diffusion](https://arxiv.org/abs/2606.08309)
...
### [268] [Orthogonality and Dimensionality in Airline Cluster Analysis using PCA and Kernel PCA](https://arxiv.org/abs/2606.08322)
*Andreas Schlapbach*
...
### [269] [GENERIC-FNO: Embedding Energy Conservation and Entropy Production into Fourier Neural Operators](https://arxiv.org/abs/2606.08343)
...
### [270] [Generative Frontier Planning for Adaptive Peer-Referral Recruitment under Covariate-Dependent Arrivals](https://arxiv.org/abs/2606.08360)
...
### [271] [Pre-Intervention Prediction of Sparse Autoencoder Steering Side Effects](https://arxiv.org/abs/2606.08365)
*Evan Duan*
...
### [272] [An Information-Theoretic Definition for Open-Ended Learning](https://arxiv.org/abs/2606.08369)
...
### [273] [Few-step Cofolding with All-Atom Flow Maps](https://arxiv.org/abs/2606.08375)
...
### [274] [RiskNet: A large-scale dataset of AI risk incidents from news with alignment and multi-dimensional annotations](https://arxiv.org/abs/2606.08376)
...
### [275] [STAR-KV: Low-Rank KV Cache Compression via Soft Thresholding for Adaptive Rank Control](https://arxiv.org/abs/2606.08382)
...
### [276] [The Spectral Dynamics and Noise Geometry of Muon](https://arxiv.org/abs/2606.08388)
...
### [277] [When Are Neural Interaction Discoveries Real? Identifiability, Recoverability, and a Pre-Fit Diagnostic](https://arxiv.org/abs/2606.08390)
...
### [278] [Provably Efficient Personalized Multi-Objective Bandits with Proactive Conversational Queries](https://arxiv.org/abs/2606.08410)
...
### [279] [Sparrow: Sparse Rollout for Stable and Efficient Long-context RL of Large Language Models](https://arxiv.org/abs/2606.08446)
...
### [280] [Not Just After One: Sleep-Inspired Replay Prevents Catastrophic Forgetting After Sequential Tasks](https://arxiv.org/abs/2606.08447)
...
### [281] [Theoretical Foundations of Continual Learning via Drift-Plus-Penalty](https://arxiv.org/abs/2606.08452)
...
### [282] [Beyond Linear Activation Steering: Invertible Latent Transformations for Controlling LLM Behavior](https://arxiv.org/abs/2606.08454)
*Tuc Nguyen,Thai Le*
...
### [283] [The Confidence Trap: Calibration Attacks for Graph Neural Networks](https://arxiv.org/abs/2606.08467)
...
### [286] [Adaptive Loss Balancing for Noise-Robust GRPO in Generative Recommendation](https://arxiv.org/abs/2606.08480)
...
### [287] [PIPE-Cypher: Automatic Enterprise Benchmark Generation for Text-to-Cypher Systems](https://arxiv.org/abs/2606.08481)
...
### [288] [STELLAR: Spatio-Temporal Environmental Learning with Latent Alignment and Refinement for Long-Tailed Species Distribution Modeling](https://arxiv.org/abs/2606.08484)
...
### [289] [A Joint Finite-Sample Certificate for Adaptive Selective Conformal Risk Control](https://arxiv.org/abs/2606.08517)
...
### [290] [Autonomous Aerial Manipulation via Contextual Contrastive Meta Reinforcement Learning](https://arxiv.org/abs/2606.08533)
...
### [291] [Routine laboratory trajectories encode the onset of organ-level complications in cancer](https://arxiv.org/abs/2606.08538)
...
### [292] [A Theoretical Analysis of Memory and Overfitting Phenomena in Stochastic Interpolation Models](https://arxiv.org/abs/2606.08554)
...
### [293] [Physics-Guided Dual Decoding and Spectral Supervision for Global 3D Hydrometeor Prediction](https://arxiv.org/abs/2606.08563)
...
### [294] [EinSort: Sorting is All We Need for Tensorizing LLM](https://arxiv.org/abs/2606.08565)
...
### [295] [Titans-as-a-Layer: Test-Time Memory for Conversational Speech Emotion Recognition](https://arxiv.org/abs/2606.08573)
...
### [296] [OrderDP: A Theoretically Guaranteed Lossless Dynamic Data Pruning Framework](https://arxiv.org/abs/2606.08574)
...
### [297] [Lost in the Non-convex Loss Landscape: How to Fine-tune the Large Time Series Model?](https://arxiv.org/abs/2606.08578)
...
### [298] [A spectral audit framework reveals task-dependent aperiodic reliance across EEG and ECG deep learning](https://arxiv.org/abs/2606.08583)
...
### [299] [Convolutional Sparse Coding via the Locally Competitive Algorithm on Loihi 2](https://arxiv.org/abs/2606.08584)
...
### [300] [Quantum Global Variational Learning for Quantum Error Correction](https://arxiv.org/abs/2606.08592)
...
### [301] [How Much Capacity Does EEG Denoising Need? Ultra-Compact Networks reveal Benchmark Saturation and Metric-Utility Gap](https://arxiv.org/abs/2606.08594)
*Jasmeet Singh Bindra,Siddharth Panwar,Shubhajit Roy Chowdhury*
...
### [302] [Reinforcement Learning for Flow-Matching Policies with Density Transport](https://arxiv.org/abs/2606.08602)
...
### [303] [Tyan-WP: A Wind Power Foundation Model for Ultra-Short-Term Probabilistic Forecasting](https://arxiv.org/abs/2606.08630)
...
### [304] [SpectrumKV: Per-Token Mixed-Precision KV Cache Transfer for Prefill-Decode Disaggregated LLM Serving](https://arxiv.org/abs/2606.08635)
*Yang Pengju*
...
### [305] [Operator learning for the 2D incompressible Navier-Stokes equations: a conformal prediction approach in the data-scarce regime](https://arxiv.org/abs/2606.08654)
...
### [306] [SkillHone: A Harness for Continual Agent Skill Evolution Through Persistent Decision History](https://arxiv.org/abs/2606.08671)
*Zhiwei Li,Yong Hu*
...
### [307] [Activation Steering Induces Emergent Misalignment: A More Comprehensive Evaluation](https://arxiv.org/abs/2606.08682)
...
### [308] [Hierarchical Projection for Adaptive Knowledge Transfer](https://arxiv.org/abs/2606.08691)
*Samhita Pal,Tian Gu*
...
### [309] [Agentic Search for Counterfactual Recourse under Fixed LLM Budgets](https://arxiv.org/abs/2606.08696)
*Yasuo Tabei*
...
### [310] [SNR-ST-Mix: Sample-specific Neighborhood Regression Mixup for Augmented Spatial Transcriptomics Imputation with Deep Neural Network](https://arxiv.org/abs/2606.08712)
...
### [311] [Deep Active Re-Labeling: Toward Noise-Resilient Annotation Efficiency](https://arxiv.org/abs/2606.08718)
...
### [312] [A Geometric Measure of Linear Separability for Neural Representations](https://arxiv.org/abs/2606.08721)
...
### [313] [Declarative Outcome-Conformant Synthesis: Exact, Closed-Form Specification Satisfaction and a Conformance Benchmark](https://arxiv.org/abs/2606.08736)
*Muhammed Rasin*
...
### [314] [Understanding the Parameter Space Geometry of Transformers Encoding Boolean Functions](https://arxiv.org/abs/2606.08768)
...
### [315] [How Many Counterfactuals Does It Take? Probing VLM Hallucinations Through Circuits and Causal Effects](https://arxiv.org/abs/2606.08777)
...
### [316] [Reformulate LLM Reinforcement Learning for Efficient Training under Black-box Discrepancy](https://arxiv.org/abs/2606.08779)
...
### [317] [Scaling Decision-Focused Learning to Large Problems with Lagrangian Decomposition](https://arxiv.org/abs/2606.08797)
...
### [318] [Active Flow Expansion for Out-of-Distribution Discovery: from Theory to Molecules](https://arxiv.org/abs/2606.08802)
...
### [319] [Knowledge Graphs and Reasoning LLMs for Finding Simple Yet Effective Transcriptomic Perturbation Predictors](https://arxiv.org/abs/2606.08816)
...
### [320] [Intrinsic Selection and Particle Resampling for Inference-Time Scaling Beyond Domain Verifiability](https://arxiv.org/abs/2606.08850)
...
### [321] [sGPO: Trading Inference FLOPs for Training Efficiency in RLVR](https://arxiv.org/abs/2606.08854)
...
### [322] [Diffuse AI Control on Fuzzy Tasks](https://arxiv.org/abs/2606.08892)
...
### [323] [Cheap Reward Hacking Detection](https://arxiv.org/abs/2606.08893)
...
### [336] [LEAF: A Learning-Enabled ADMM Framework for Accelerated Convex Optimization](https://arxiv.org/abs/2606.08993)
...
### [337] [Understanding Quantization-Aware Training: Gradients at Quantized Weights Bias to the Low-Loss Basin](https://arxiv.org/abs/2606.09012)
...
### [338] [Structural Grid Descriptors Predict Within-Task Solver Success on ARC-AGI](https://arxiv.org/abs/2606.09026)
*Ayan Pendharkar*
...
### [339] [TRIAGE: Dialectical Reasoning for Explainable Risk Prediction on Irregularly Sampled Medical Time Series with LLMs](https://arxiv.org/abs/2606.09030)
...
### [340] [DynaCF: Mitigating Shortcut Learning in Reward Models via Dynamic Counterfactual Sensitivity](https://arxiv.org/abs/2606.09043)
...
### [341] [Decoy-Calibrated Failure Audits for Language Models](https://arxiv.org/abs/2606.09046)
...
### [342] [Beyond Convolution: Advancing Hypergraph Neural Networks with Hypergraph U-Nets](https://arxiv.org/abs/2606.09051)
...
### [343] [INFUSER: Influence-Guided Self-Evolution Improves Reasoning](https://arxiv.org/abs/2606.09052)
...
### [344] [Stage-1 Controls the Entropy Regime, Not the Outcome](https://arxiv.org/abs/2606.09059)
*Jianxiong Shen*
...
### [345] [OnlyDense: Reduced-Order Modeling for Lagrangian simulation](https://arxiv.org/abs/2606.09065)
...
### [346] [A Unifying Lens on Reward Uncertainty in RLHF](https://arxiv.org/abs/2606.09073)
...
### [347] [Neural Legendre-Fenchel transform with Hessian Preconditioning](https://arxiv.org/abs/2606.09077)
...
### [348] [The Hidden Bias of Process Reward Models:PRISM for Rewarding the Right Reasoning](https://arxiv.org/abs/2606.09078)
...
### [349] [FlashMemory-DeepSeek-V4: Lightning Index Ultra-Long Context via Lookahead Sparse Attention](https://arxiv.org/abs/2606.09079)
...
### [350] [Beyond FLOPs: Benchmarking Real Inference Acceleration of LLM Pruning under a GEMM-Centric Taxonomy](https://arxiv.org/abs/2606.09080)
...
### [351] [Stabilizing On-Policy Distillation for MLLM Reasoning with Global Normalization](https://arxiv.org/abs/2606.09091)
...
### [352] [From Shortcuts to Reasoning: Robust Post-Training of Theory of Mind with Reinforcement Learning](https://arxiv.org/abs/2606.09092)
...
### [353] [Addressing Market Regime Changes and Heavy-Tailed Returns in Portfolio Optimization via Bayesian VAR and Elliptical Black-Litterman](https://arxiv.org/abs/2606.09104)
...
### [354] [Hybridizing Equilibrium Propagation with Ising Machines for Efficient Energy-Based Learning](https://arxiv.org/abs/2606.09112)
...
### [355] [Counterfactual Transport Flows for Offline Conservative Trajectory Refinement](https://arxiv.org/abs/2606.09115)
...
### [356] [Optimizing Energy-based Neural Network Training with Coherent Ising Machine](https://arxiv.org/abs/2606.09117)
...
### [357] [Claw-R1: A Step-Level Data Middleware System for Agentic Reinforcement Learning](https://arxiv.org/abs/2606.09138)
...
### [358] [Improved Convergence Analysis of Topology Dependence in Decentralized SGD](https://arxiv.org/abs/2606.09154)
...
### [359] [Crop Recommendation and Agricultural Query Answering System Using Spatio-Temporal Graph Neural Networks and Hybrid Retrieval Augmentation](https://arxiv.org/abs/2606.09160)
...
### [360] [CANS: Accelerating Multiuser Collaborative Edge Inference via Cooperative Autodidactic NeuroSurgeon](https://arxiv.org/abs/2606.09175)
...
### [361] [Asymptotic Optimality of Thompson Sampling for Risk-Averse Bandits with Sub-Gaussian Rewards](https://arxiv.org/abs/2606.09191)
*Joel Q. L. Chang*
...
### [362] [The Injection Paradox: Brand-Level Suppression in Safety-Trained LLM Recommendations via RAG Context Injection](https://arxiv.org/abs/2606.09204)
*Hyunseok Paeng*
...
### [363] [Orange Lab: Lowering Barriers to Data Mining through Embedded Interactive Workflows](https://arxiv.org/abs/2606.09239)
...
### [364] [BSTabDiff: Block-Subunit Diffusion Priors for High-Dimensional Tabular Data Generation](https://arxiv.org/abs/2606.09257)
...
### [365] [ERBench: A Benchmark and Testsuite for Equation Discovery Algorithms](https://arxiv.org/abs/2606.09276)
...
### [366] [Internalizing Geometric Law: Learning from Solver Residuals for Precision-Critical Generation](https://arxiv.org/abs/2606.09278)
...
### [367] [Trajectory Geometry of Transformer Representations Across Layers](https://arxiv.org/abs/2606.09287)
...
### [368] [Intention Driven Identification of In-Possession Match Phases in Association Football through Temporal Graph Learning](https://arxiv.org/abs/2606.09289)
...
### [369] [PRISM: Topology-Aware Cross-Modal Imputation for Modality-Deficient Federated Graph Learning](https://arxiv.org/abs/2606.09301)
...
### [370] [Toward Compiler World Models: Learning Latent Dynamics for Efficient Tensor Program Search](https://arxiv.org/abs/2606.09312)
...
### [371] [Machine-Learning Emulation of Satellite Greenhouse Gas Retrievals: Stability over Time](https://arxiv.org/abs/2606.09313)
...
### [372] [A Universal Dense Football Event Representation Based on TabTransformer](https://arxiv.org/abs/2606.09327)
...
### [373] [Thresholded Local Hyper-Flow Diffusion](https://arxiv.org/abs/2606.09340)
...
### [374] [PBSD: Privileged Bayesian Self-Distillation for Long-Horizon Credit Assignment](https://arxiv.org/abs/2606.09348)
...
### [375] [Scaling Neural Network Verification with Tensor Parallelism and Fully Sharded Data Parallelism](https://arxiv.org/abs/2606.09377)
...
### [376] [Reasoning Arena: Trace Tournaments When Verifiable Rewards Fall Short](https://arxiv.org/abs/2606.09380)
...
### [377] [Distilling Safe LLM Systems via Soft Prompts for On Device Settings](https://arxiv.org/abs/2606.09388)
...
### [378] [Benchmarking Empirical Privacy Protection for Adaptations of Large Language Models](https://arxiv.org/abs/2606.09401)
...
### [379] [LargeMonitor: Monitoring Online Task-Free Continual Learning via Large Pretrained Models](https://arxiv.org/abs/2606.09430)
...
### [380] [Graph Mamba Operator: A Latent Simulator for Interacting Particle Systems](https://arxiv.org/abs/2606.09432)
...
### [381] [Operator learning for solving Fokker-Planck equations with various initial conditions](https://arxiv.org/abs/2606.09434)
...
### [382] [Breaking the Tokenizer Barrier: On-Policy Distillation across Model Families](https://arxiv.org/abs/2606.09456)
...
### [383] [Escaping the KL Agreement Trap in On-Policy Distillation](https://arxiv.org/abs/2606.09471)
...
### [384] [Loss-Guided Adaptive Scale Refinement for Molecular Force Prediction](https://arxiv.org/abs/2606.09480)
*Limin Yu*
...
### [385] [BUDDY: BUdget-Driven DYnamic Depth Routing for Adaptive Large Language Model Inference](https://arxiv.org/abs/2606.09514)
...
### [386] [Investigating Calibration Challenges in Probabilistic Electricity Price Forecasting](https://arxiv.org/abs/2606.09517)
...
### [387] [Efficient Traffic Prediction at Scale: A Systematic Study of STGCN Architectural Depth](https://arxiv.org/abs/2606.09539)
...
### [388] [Safe-RULE: Safe Reinforcement UnLEarning](https://arxiv.org/abs/2606.09559)
...
### [389] [On Choosing the $μ$ Parameter in Gaussian Differential Privacy](https://arxiv.org/abs/2606.09582)
...
### [390] [Assessing Sample Quality in Conditional Generation under Compositional Shift](https://arxiv.org/abs/2606.09601)
...
### [391] [Closure-Validated Circuit Discovery in Attention Heads: Co-activation Proposes, Ablation Disposes](https://arxiv.org/abs/2606.09607)
*Yongzhong Xu*
...
### [392] [Constrained user-item allocation for e-commerce marketing campaigns](https://arxiv.org/abs/2606.09623)
...
### [393] [Data-driven discovery of governing differential equations across physical systems](https://arxiv.org/abs/2606.09638)
...
### [394] [A Unifying Framework for Concept-Based Representational Similarity](https://arxiv.org/abs/2606.09653)
...
### [395] [Muon Learns More Robust and Transferable Features than Adam](https://arxiv.org/abs/2606.09658)
...
### [396] [In-Context Learning for Latent Space Bayesian Optimization](https://arxiv.org/abs/2606.09664)
...
### [397] [Algorithm for Contextual Queueing Bandits with Rate-Optimal Queue Length Regret](https://arxiv.org/abs/2606.09668)
...
### [398] [Transition-Based Digital Twin Modelling for Alzheimer's Disease under Sparse Longitudinal Data](https://arxiv.org/abs/2606.09671)
...
### [399] [AutoMegaKernel: A Statically-Checked Agent Harness for Self-Retargeting Megakernel Synthesis](https://arxiv.org/abs/2606.09682)
...
### [400] [When Do Local Score Models Extrapolate Across Size? A Diagnostic Theory and Benchmark](https://arxiv.org/abs/2606.09705)
*Wenjie Xi*
...
### [401] [BrainSurgery: Reproducible and Reliable Declarative Weight Manipulations for Model Editing and Upcycling](https://arxiv.org/abs/2606.09707)
...
### [402] [Evaluating the Representation Space of Diffusion Models via Self-Supervised Principles](https://arxiv.org/abs/2606.09718)
...
### [404] [Disentanglement with Holographic Reduced Representations](https://arxiv.org/abs/2606.09725)
...
### [406] [Tight Sample Complexity of Transformers](https://arxiv.org/abs/2606.09731)
...
### [407] [Topological Neural Operators](https://arxiv.org/abs/2606.09806)
...
### [408] [Learning Dynamics Reveal a Hierarchy of Weight-Induced Layerwise Gram Metrics](https://arxiv.org/abs/2606.09744)
*Claudio Nordio*
...
### [409] [An Agency-Transferring Model-Free Policy Enhancement Technique](https://arxiv.org/abs/2606.09825)
...
### [410] [Perturbative Contrastive Physical Learning](https://arxiv.org/abs/2606.09756)
...
### [411] [iOSWorld: A Benchmark for Personally Intelligent Phone Agents](https://arxiv.org/abs/2606.09764)
...
### [412] [Zero Touch Predictive Orchestration: Automating Time-Series Models for the Cloud-Edge Continuum](https://arxiv.org/abs/2606.09787)
...
### [413] [Rethinking the Divergence Regularization in LLM RL](https://arxiv.org/abs/2606.09821)
