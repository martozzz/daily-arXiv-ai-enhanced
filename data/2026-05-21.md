...
### [1] [AgentCo-op: Retrieval-Based Synthesis of Interoperable Multi-Agent Workflows](https://arxiv.org/abs/2605.20425)
*Shuaike Shen,Wenduo Cheng,Shike Wang,Mingqian Ma,Jian Ma*

Main category: cs.AI

TL;DR: AgentCo-op：一个基于检索的合成框架，通过类型化工件传递将可复用技能、工具和外部智能体组合成可执行工作流，在证据表明失败时应用有界自引导局部修复。


<details>
  <summary>Details</summary>
Motivation: 在开放世界科学环境中设计多智能体工作流面临困难：任务缺乏策划的训练集、可靠的标量评估指标、现有工具和智能体之间的标准化接口。

Method: 提出AgentCo-op框架：1)通过类型化工件传递组合可复用技能、工具和外部智能体为可执行工作流；2)当执行证据表明失败时，应用有界自引导局部修复；3)可以将搜索到的工作流作为结构先验导入并通过检索组件和局部修复进行改进。

Result: 在两个基因组学案例中成功组合独立开发的科学智能体和外部工具库为可审计工作流；在六个编码、数学和问答基准测试中，四个基准上获得最佳结果，在统一骨干设置下获得最佳平均分数，同时相对于多智能体基线持续降低每任务成本。

Conclusion: 基于检索的合成可以将自动智能体工作流设计从基准优化的智能体图扩展到由现有智能体、工具和类型化工件构建的开放世界工作流，合成和搜索是互补的。
...
### [2] [\ECUAS{n}: A family of metrics for principled evaluation of uncertainty-augmented systems](https://arxiv.org/abs/2605.20490)
*Lautaro Estienne,Erik Ernst,Matías Vera,Pablo Piantanida,Luciana Ferrer*

Main category: cs.AI

TL;DR: 本文提出了一种新的评估指标家族ECUASn，用于评估具有不确定性增强的预测系统在决策制定中的整体性能。


<details>
  <summary>Details</summary>
Motivation: 在自动化决策中，获得预测不确定性对于用户根据特定成本权衡接受或拒绝预测至关重要。当前文献中对不确定性增强系统的评估方法存在不足，使用单独指标评估预测和不确定性分数，或设置固定拒绝成本，这无法充分评估系统在不确定性下的整体决策性能。

Method: 提出ECUASn指标家族，作为适用于特定任务的适当评分规则。参数n控制错误预测成本和不完美不确定性之间的权衡，可根据用例需求调整。通过理论分析和在多种分类和生成数据集上的实验验证该方法，包括TriviaQA的手动标注子集。

Result: ECUASn指标在理论上和实证上都显示出优势，能够更好地评估不确定性增强系统的整体决策性能，适应不同使用场景的需求。

Conclusion: 新提出的ECUASn指标家族为解决不确定性增强系统的评估问题提供了更合适的框架，能够在决策制定中综合考虑预测质量和不确定性估计，优于现有的评估方法。
...
### [3] [Mahjax: A GPU-Accelerated Mahjong Simulator for Reinforcement Learning in JAX](https://arxiv.org/abs/2605.20577)
*Soichiro Nishimori,Shinri Okano,Keigo Habara,Sotetsu Koyamada,Eason Yu,Masashi Sugiyama*

Main category: cs.AI

TL;DR: 论文介绍了一个名为Mahjax的完全向量化日本麻将（Riichi Mahjong）环境，使用JAX实现以在GPU上实现大规模并行化，旨在促进强化学习研究。


<details>
  <summary>Details</summary>
Motivation: 日本麻将是多人不完全信息博弈，具有随机性和高维状态空间的特点，这些特性模拟了现实世界中的复杂决策问题。先前研究主要依赖于人类游戏日志的监督学习进行策略预训练，而能够从零开始学习的算法（如AlphaZero系列）具有更广泛的适用性潜力。为了促进这类研究，需要开发高效的环境。

Method: 开发了Mahjax环境，这是一个完全向量化的日本麻将环境，使用JAX实现以充分利用GPU并行计算能力。同时提供了高质量的可视化工具，便于调试和与训练好的智能体交互。

Result: Mahjax在8个NVIDIA A100 GPU上实现了最高200万步/秒（无红规则）和100万步/秒（红规则）的吞吐量。实验验证了环境对强化学习的实用性，显示智能体可以被有效训练以提升对抗基线策略的排名。

Conclusion: Mahjax为日本麻将的强化学习研究提供了一个高效、可扩展的环境，特别适用于需要从零开始学习的算法开发。其高性能和可视化工具将有助于推动不完全信息博弈和复杂决策问题的研究。
...
### [4] [COAgents: Multi-Agent Framework to Learn and Navigate Routing Problems Search Space](https://arxiv.org/abs/2605.20618)
*Oleksandr Yakovenko,Mahdi Mostajabdaveh,Cheikh Ahmed,Abdullah Ali Sivas,Xiaorui Li,Zirui Zhou,Mao Kun*

Main category: cs.AI

TL;DR: COAgents是一个合作多智能体框架，将车辆路径问题的搜索过程建模为图，通过动态构建的部分搜索图（PSG）和三个专门智能体（节点选择、移动选择和跳跃智能体）来指导搜索过程，在CVRP和VRPTW基准测试中取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 车辆路径问题（VRP）在实际应用中至关重要，但由于组合复杂性，在大规模情况下计算难以处理。传统启发式方法依赖手工规则的局部改进和偶尔的"跳跃"来逃离局部最优，但往往难以在不同实例间泛化。需要一种能够更好指导搜索过程并提高泛化能力的方法。

Method: 提出了COAgents合作多智能体框架，将搜索过程建模为图：节点表示解决方案，边对应局部细化或用于多样化的大扰动（跳跃）。动态构建部分搜索图（PSG），并训练三个专门智能体：节点选择智能体和移动选择智能体指导强化搜索，跳跃智能体触发对新区域的适时探索。与端到端学习方法不同，该方法将问题无关的搜索控制与紧凑的领域特定编码清晰分离。

Result: 在CVRP和VRPTW基准测试中，COAgents在CVRP上与多个学习搜索基线保持竞争力，在更具挑战性的VRPTW实例上建立了基于学习方法的新最佳状态。相对于最强的神经求解器（POMO），在N=100时将与最佳已知解决方案的差距缩小了14%，在N=50时缩小了44%；相对于ALNS，分别缩小了21%和40%。

Conclusion: COAgents框架通过合作多智能体方法和动态构建的搜索图，有效解决了车辆路径问题的搜索优化挑战。该方法将问题无关的搜索控制与领域特定编码分离的设计促进了跨任务的适应性，在复杂VRPTW问题上表现出色，显著缩小了与最佳已知解决方案的差距。
...
### [5] [Evaluating Temporal Semantic Caching and Workflow Optimization in Agentic Plan-Execute Pipelines](https://arxiv.org/abs/2605.20630)
*Alimurtaza Mustafa Merchant,Krish Veera,Sajal Kumar Goyla,Shambhawi Bhure,Dhaval Patel,Kaoutar El Maghraoui*

Main category: cs.AI

TL;DR: 本文提出了针对工业资产运营工作流的两层优化方案：时间语义缓存和MCP工作流优化，以解决现有LLM缓存技术在参数丰富的工业查询中的失效问题，显著降低了延迟。


<details>
  <summary>Details</summary>
Motivation: 工业资产运营工作流对延迟敏感，现有LLM缓存技术（如KV-cache重用和基于嵌入的语义缓存）是为聊天机器人服务设计的，在输出有效性依赖于时间、资产或传感器参数时会失效。

Method: 提出了两个互补的优化层：1）时间语义缓存，考虑时间维度；2）MCP工作流优化，包括磁盘支持的工具发现缓存和依赖感知的并行步骤执行。

Result: MCP工作流优化带来1.67倍加速，中位数端到端延迟降低约40%；时间语义缓存在缓存命中时实现中位数30.6倍加速。同时揭示了纯语义缓存在参数丰富的工业查询中的具体失效模式。

Conclusion: 该研究为MCP支持的智能体基准测试中的缓存选择与评估正确性提供了关键分析，证明考虑时间维度的语义缓存和工作流优化能有效解决工业场景的特殊需求。
...
### [6] [VBFDD-Agent for Electric Vehicle Battery Fault Detection and Diagnosis: Descriptive Text Modeling of Battery Digital Signals](https://arxiv.org/abs/2605.20742)
...
### [7] [Conflict-Aware Additive Guidance for Flow Models under Compositional Rewards](https://arxiv.org/abs/2605.20758)
...
### [8] [Interaction Locality in Hierarchical Recursive Reasoning](https://arxiv.org/abs/2605.20784)
...
### [9] [Conditional Equivalence of DPO and RLHF: Implicit Assumption, Failure Modes, and Provable Alignment](https://arxiv.org/abs/2605.20834)
...
### [10] [PlanningBench: Generating Scalable and Verifiable Planning Data for Evaluating and Training Large Language Models](https://arxiv.org/abs/2605.20873)
...
### [11] [Governance by Construction for Generalist Agents](https://arxiv.org/abs/2605.20874)
...
### [12] [For How Long Should We Be Punching? Learning Action Duration in Fighting Games](https://arxiv.org/abs/2605.20911)
...
### [13] [Playing Devil's Advocate: Off-the-Shelf Persona Vectors Rival Targeted Steering for Sycophancy](https://arxiv.org/abs/2605.21006)
...
### [15] [ScenePilot: Controllable Boundary-Driven Critical Scenario Generation for Autonomous Driving](https://arxiv.org/abs/2605.21168)
...
### [16] [Insights Generator: Systematic Corpus-Level Trace Diagnostics for LLM Agents](https://arxiv.org/abs/2605.21347)
...
### [17] [Towards Resilient and Autonomous Networks: A BlueSky Vision on AI-Native 6G](https://arxiv.org/abs/2605.21395)
...
TL;DR: GraphDiffMed是一个基于双尺度差分注意力的知识约束用药推荐框架，通过院内和院际层面的注意力机制过滤噪声，并结合药理学约束，提高推荐质量和安全性平衡


<details>
  <summary>Details</summary>
Motivation: 从电子健康记录中推荐安全有效的药物组合是核心临床AI问题，但面临患者轨迹长、噪声大、临床异质性的挑战。现有方法要么擅长跨就诊时间建模，要么擅长药理学知识整合，但难以同时实现且有效抑制噪声

Method: 提出GraphDiffMed框架，基于双尺度差分注意力v2，在院内（intra-visit）和院际（inter-visit）两个层面应用差分注意力来过滤就诊内和纵向历史中的虚假信号，同时在训练过程中融入药理学约束

Result: 在MIMIC-III数据集上的实验和消融研究表明，该方法在推荐质量和排名上持续优于强基线，实现了更优的安全性-性能平衡。研究发现，在实验设置下，最佳配置仅使用人口统计学辅助特征

Conclusion: GraphDiffMed证明，将噪声感知注意力与药理学约束相结合，能够产生更可靠、更具临床意义的药物推荐。该方法提高了临床AI用药推荐的实用性和安全性
...
TL;DR: TabPFN-MT：基于扩展多目标合成先验训练的模型，用于表格数据的多任务上下文学习，通过一次前向传播同时推理多个任务，在中小数据集上达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 传统的Prior-Data Fitted networks (PFNs)只能处理单任务推理，预测多个目标值需要重复的前向传播调用，且无法实现任务间信息共享。这限制了多目标表格应用的计算效率

Method: 扩展了多目标合成先验来捕获任务间依赖关系，使用扩展的y编码器和共享解码器头，实现多任务上下文学习和同时推理。模型专为中小型数据集设计，依赖上下文学习而非传统的基于梯度的训练

Result: 在344个数据集上的广泛评估表明，TabPFN-MT在平均少于1000个样本的小到中型数据集上，为深度表格多任务学习建立了新的最先进水平。在多任务数据集上获得总体准确率排名4.89，是所有测试模型中的最高平均排名。与最新的单任务集成模型相比仍具有高度竞争力

Conclusion: TabPFN-MT在多目标表格应用中实现了高度竞争力的性能，同时将T个任务的推理成本从O(T)降低到O(1)次前向传播，为多目标表格应用提供了巨大的计算效率提升。模型专为中小型数据集设计，通过一次前向传播实现多任务推理
...
TL;DR: 论文提出了一种称为SiLD（Score-induced Latent Diffusion）的两阶段扩散模型框架，通过分数函数驱动的"折叠-细化"机制，在低维流形上高效学习数据分布，避免了维度灾难问题。


<details>
  <summary>Details</summary>
Motivation: 扩散模型能在高维数据生成中取得卓越质量，但当数据支撑在低维流形上时，其训练过程如何通过分数函数学习绕过维度诅咒的理论机制尚未得到解释。

Method: 提出Score-induced Latent Diffusion (SiLD)两阶段框架：1) 小噪声尺度下，分数函数的奇异发散驱动去噪映射快速降维折叠到数据流形投影；2) 中等噪声尺度下，训练细化学习到的流形上的内在密度分布。该框架从单一的去噪分数匹配目标中同时实现流形学习和密度估计。

Result: 理论证明SiLD的样本复杂度仅依赖于内在维度而非环境维度。在Stacked MNIST、CelebA变体和分子生成基准上的实验表明，SiLD在生成质量上与基于VAE的潜在扩散模型相当或更优，并持续改善重建质量。

Conclusion: 研究揭示了扩散模型训练中的"折叠-细化"机制，并提出SiLD框架将流形学习和密度估计统一于单一目标中，理论证明了其样本效率的优势，为理解扩散模型在高维数据学习中的成功提供了理论依据。
...
TL;DR: 作者发布了MagBridge-Battery v1.0合成数据集，包含6,760个磁场特征，结合了真实磁场形态与健康状态标签，用于电池磁场传感诊断的公共基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前电池健康诊断主要依赖电池端子电化学信号，而磁场传感能提供额外信息，但缺乏公开的配对磁场测量与退化标签数据集，阻碍了方法开发。

Method: 创建合成数据集，将Mohammadi-Jerschow OSF档案中的真实磁场形态与PulseBat数据集的健康状态标签进行桥接，包含5,600个条件化样本、600个传感器异常样本和560个低电压外推样本。

Result: 数据集包含6,760个磁场特征，定义了三种主要基准任务：健康状态回归、二次寿命分类和异常检测，以及辅助的异常子类型分类任务。标签随机化消融实验确认数据集编码了非平凡的输入健康状态信息。

Conclusion: 该工作提供了电池磁场传感诊断的公共基准，解决了配对磁电化学测量稀缺的问题，数据集在Zenodo上发布，代码和基准套件在Apache-2.0许可下发布。
...
TL;DR: Geometry-Lite是一个紧凑的提示级安全探测框架，用于分析大型语言模型中的多层安全信号几何结构，通过边界位置、层间变化和粗略形状等维度分解安全证据，在多个模型和基准测试中表现优于单层探测。


<details>
  <summary>Details</summary>
Motivation: 现有的提示级安全探测方法虽然能在平均水平上区分安全与不安全提示，但无法解释这种分离的几何结构。不清楚安全证据如何在各层形成，哪些层间几何特征支持低假阳性决策，以及在基准迁移下哪些几何偏差保持稳定。

Method: 提出Geometry-Lite框架：将每层最终提示标记的隐藏状态表示映射到有符号边界（使用质心、局部邻域和监督线性边界读取器），然后通过边界位置、层间变化和粗略形状三个维度总结边界轮廓。在9个指令调优模型（1.2B-70B）和7个安全基准上进行评估。

Result: Geometry-Lite在多基准测试中优于单层探测，接近原始多层分数堆叠。分解显示安全证据主要通过持久边界位置几何表达：最终或极端边界以及不安全侧层占用主导聚合检测性能。层间漂移和结构总结对总体AUROC贡献有限，但漂移可在低假阳性率阈值下提供小规模召回修正。在基准迁移下，优化的线性边界在训练集上较尖锐，而类条件平均几何在难测试子集上保持分离更可靠。

Conclusion: 提示级安全证据主要不是层间运动信号，而是持久的层间边界几何，其有用组件和读取层面的偏置在决策关键区域变得可见。Geometry-Lite可作为分析多层安全信号的有用工具。
...
TL;DR: 开发了LEAP框架，结合领域专业大语言模型和主动学习，用于钙钛矿太阳能电池添加剂的高效筛选，通过实验验证显示出性能提升。


<details>
  <summary>Details</summary>
Motivation: 钙钛矿太阳能电池添加剂的高效发现对提升性能至关重要，但巨大的化学空间使得传统的试错筛选方法效率低下。

Method: 开发LEAP框架，结合领域专业大语言模型（从钙钛矿添加剂文献中提取机理相关知识，生成可解释的描述符）和贝叶斯优化工作流，在低数据条件下进行不确定性感知的优先级排序，采用专家参与的闭环验证流程。

Result: 在未见文献上的基准测试显示，领域专业模型在机理一致性推理方面优于通用模型。在专家参与的验证研究中，经过三轮筛选，6-CDQ处理器件平均PCE达20.13%，2-CNA处理器件达20.87%，对照组为19.25%，最佳器件PCE达21.32%。

Conclusion: 基于文献的机理描述符，结合贝叶斯优化和专家可行性评审，能够支持钙钛矿光伏领域中具有机理意识的添加剂优先级排序，为高效添加剂发现提供了新的方法路径。
...
### [29] [GROW: Aligning GRPO with State-Action Modeling for Open-World VLM Agents](https://arxiv.org/abs/2605.20246)
*Xiongbin Wu,Zhihao Luo,Shanzhe Lei,Lechao Zhang,Xuhong Wang,Jie Yang,Zhonglong Zheng,Yuanjie Zheng,Xin Tan,Wei Liu*

Main category: cs.LG

TL;DR: 提出了GROW强化学习框架，用于开放世界视觉语言模型（VLM）智能体，通过将完整轨迹分解为状态-动作样本来解决多轮RL训练中的长上下文和噪声问题


<details>
  <summary>Details</summary>
Motivation: 现有的VLM智能体主要依赖专家演示的监督微调（SFT），而高级强化学习算法（特别是GRPO）在多轮RL任务中应用效果不佳，因为标准GRPO需要完整轨迹作为训练样本，导致过长的上下文和噪声问题

Method: 提出了GROW框架，将收集的轨迹分解为状态-动作样本，并在这些样本之间计算优势值，而不是将完整轨迹视为单一实体。通过替代分析表明，即使分组样本基于不同的局部状态而非相同提示上下文，目标函数仍能在简化假设下保留GRPO的核心相对策略优化信号

Result: 在800多个Minecraft任务上的实验表明，GROW方法达到了最先进的性能水平，证明了所提RL框架对开放世界VLM智能体的有效性

Conclusion: GROW框架成功地解决了开放世界VLM智能体多轮强化学习训练中的挑战，通过创新的轨迹分解和优势计算机制，实现了优于现有方法的性能表现
...
### [30] [CP-MoE: Consistency-Preserving Mixture-of-Experts for Continual Learning](https://arxiv.org/abs/2605.20247)
*Yang Liu,Toan Nguyen,Flora D. Salim*

Main category: cs.LG

TL;DR: CP-MoE是一个用于大型语言模型和视觉语言模型持续学习的新框架，通过引入瞬态专家来解决Mixture-of-Experts架构中的灾难性遗忘问题，在平衡知识转移和防止遗忘方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LoRA的MoE持续学习方法面临根本性权衡：要么过于隔离专家限制跨任务知识转移，要么让任务特定更新覆盖重要现有参数导致严重遗忘。需要解决这一权衡问题。

Method: 提出CP-MoE框架：1)引入瞬态专家捕获早期任务特定更新；2)一致性保持路由偏差，利用瞬态专家评估与稳定专家的表示相似性，引导更兼容的专家选择；3)瞬态专家指导的正则化机制，在合并过程中选择性保护重要历史参数。

Result: 在SuperNI基准测试中，CP-MoE实现了最先进的性能，并展现出对未见任务更强的零样本转移能力。在VQA v2数据集上，有效扩展到多模态视觉推理，持续减少遗忘，并超越强MoE基线。

Conclusion: CP-MoE通过瞬态专家架构和一致性保持机制，有效减少了参数干扰和灾难性遗忘，同时保持了跨任务知识转移能力，为MoE模型的持续学习提供了有效的解决方案。
...
### [31] [Graph Transductive Sharpening: Leveraging Unlabeled Predictions in Node Classification](https://arxiv.org/abs/2605.20248)
*Brown Zaz,Mar Gonzàlez I Català,Ferran Hernandez Caralt,Moshe Eliasof,Pietro Liò*

Main category: cs.LG

TL;DR: 提出Transductive Sharpening（TS）训练目标，通过在无标签节点上最小化预测熵同时在有标签节点上平衡该效应，在转导式节点分类任务中实现一致的性能提升。


<details>
  <summary>Details</summary>
Motivation: 在转导式设置中，半监督节点分类研究主要关注架构创新。作者发现转导式模型在训练时为所有节点（包括无标签节点）生成预测，但这些预测信号在标准监督目标中被丢弃，因为缺乏真实标签。受交叉熵分解为标签相关对齐项和标签无关熵项的启发，作者提出使用预测置信度作为在缺乏标签情况下提取这些信号的自然方法。

Method: 提出Transductive Sharpening（TS）方法，这是一种损失级别的修改：在无标签节点上最小化预测熵，同时在有标签节点上平衡这种效应。该方法不需要改变骨干架构，可以应用于现有的转导式节点分类模型。

Result: 在广泛的节点分类基准测试中评估Transductive Sharpening，观察到一致的性能提升，无需对骨干架构进行任何修改。

Conclusion: 该研究证明，除了架构创新外，训练目标也是一个重要的改进方向。通过有效利用无标签节点的预测信号，Transductive Sharpening能够提升转导式半监督节点分类的性能。
...
### [32] [Automated Kernel Discovery Towards Understanding High-dimensional Bayesian Optimization](https://arxiv.org/abs/2605.20249)
*Taeyoung Yun,Woocheol Shin,Inhyuck Song,Jaewoo Lee,Jinkyoo Park*

Main category: cs.LG

TL;DR: 这篇论文提出了一个名为Kernel Discovery的LLM驱动进化框架，用于解决高维贝叶斯优化中的核函数设计问题。该方法采用两阶段策略：首先用LLM生成新的数学形式，然后再转换为可执行代码，并通过LOO-CRPS选择准则避免过拟合。在5个高维BO基准测试中获得了1.2/17的平均排名。


<details>
  <summary>Details</summary>
Motivation: 高斯过程核函数对贝叶斯优化至关重要，但高维问题中设计有效核函数仍严重依赖人工工程。现有自动化方法在高维中面临两个瓶颈：1）核搜索空间仅限于基础核函数的加法和乘法组合；2）基于LLM的方法需要在原始观测数据上条件化，这因上下文长度限制和难以提取有意义模式而变得不可行。

Method: 提出Kernel Discovery框架，采用两阶段方法：第一阶段使用LLM提出新颖的数学形式，第二阶段用另一个LLM调用将每个数学形式转换为经过验证的可执行代码。还提出了留一连续排名概率得分（LOO-CRPS）作为选择准则，惩罚过拟合核函数。

Result: 在五个高维贝叶斯优化基准测试中，该方法平均排名为1.2（共17个对比方法），优于竞争基线。论文还分析了发现的核函数，识别出哪些核函数能提升高维BO性能。

Conclusion: Kernel Discovery是一个有效的LLM驱动进化框架，能够自动发现超越预定义组合规则的高维BO核函数，无需在观测数据上条件化，并在高维基准测试中显著优于现有方法。
...
### [33] [Multi-Agent Reinforcement Learning for Safe Autonomous Driving Under Pedestrian Behavioral Uncertainty](https://arxiv.org/abs/2605.20255)
*Prakash Aryan,Kaushik Raghupathruni,Timo Kehrer,Sebastiano Panichella*

Main category: cs.LG

TL;DR: 通过多智能体强化学习共同训练自动驾驶汽车和行人，比固定行人策略训练能产生更真实的交互场景，减少碰撞并提高目标达成率。


<details>
  <summary>Details</summary>
Motivation: 当前的自动驾驶汽车仿真测试通常依赖于脚本化或简化的行人模型，无法捕捉真实人类过街行为的异质性和不确定性，特别是在乱穿马路场景中，这些行为受潜在人格特质影响且车辆无法观察。

Method: 构建了多智能体强化学习环境，使用MAPPO共同训练自动驾驶汽车和12个行人。行人运动采用脚本化的Dijkstra路径规划，而高层"走/等"决策由RL策略控制。乱穿马路的概率取决于每个行人在情节开始时采样的人格特质，且对自动驾驶汽车隐藏。

Result: 在500个情节的评估中，共同训练的自动驾驶汽车达到78%的目标达成率和14%的碰撞率，而最佳基于规则的基线为35%目标达成率和33%碰撞率。乱穿马路仅占过街事件的13%，但导致了62%的碰撞。共同训练使碰撞率相比单智能体RL减少了30%。

Conclusion: 多智能体强化学习的共同训练方法能够产生更真实的行人-车辆交互，自动驾驶汽车在与乱穿马路者近距离（0-3米）接触时速度差异显著（快2.65米/秒），表明无法预测此类行为。该方法提高了自动驾驶汽车在复杂行人环境中的安全性和适应性。
...
### [34] [FBOS-RL: Feedback-Driven Bi-Objective Synergistic Reinforcement Learning](https://arxiv.org/abs/2605.20256)
*Xikai Zhang,Yongzhi Li,Likang Xiao,Yingze Zhang,Yanhua Cheng,Quan Chen,Peng Jiang,Wenjun Wu,Liu Liu*

Main category: cs.LG

TL;DR: FBOS-RL：一个反馈驱动的双目标协同强化学习框架，通过反馈引导的探索增强和两个相互促进的训练目标，解决GRPO在任务超出模型当前能力时训练停滞的问题，显著提升训练效率和最终性能。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO及其变体在强化学习中采用简单的采样方案，当任务超出策略模型当前能力时，很少能产生高质量rollout，导致策略更新缺乏有意义的梯度方向，从而使训练停滞不前。需要解决这个问题来提升训练效果。

Method: 提出FBOS-RL框架：1）基于环境反馈进行反馈引导的探索增强；2）设计两个相互促进的训练目标：利用导向的策略对齐(EPA)和探索导向的能力培养(ECC)。这两个目标形成正反馈循环。

Result: 实验表明EPA和ECC能够相互促进，形成正向飞轮效应。在相同数量的rollout下，FBOS-RL比GRPO和基于反馈的基线方法学习速度显著更快，最终达到更高的性能上限。训练过程中表现出更高的策略熵和更低的梯度范数。

Conclusion: FBOS-RL通过反馈驱动的双目标协同方法，有效解决了GRPO在困难任务中的训练停滞问题，显著提升了强化学习的训练效率和最终性能，为大规模模型的对齐和推理能力解锁提供了更有效的框架。
...
### [35] [Instance Discrimination for Link Prediction](https://arxiv.org/abs/2605.20257)
*Valentin Cuzin-Rambaud,Mathieu Lefort,Rémy Cazabet*

Main category: cs.LG

TL;DR: 该论文将实例判别学习应用于图领域的链接预测任务，提出了基于社区结构的新型结构增强方法，并引入了两个新模型L-GRACE和L-BGRL，这些模型在无属性图上表现优异，与最先进方法相当。


<details>
  <summary>Details</summary>
Motivation: 实例判别模型在图像领域的自监督学习中取得了成功，在图领域（特别是节点分类）也表现出色，但在链接预测任务上的研究较少。作者旨在将现有方法适配到链接预测的背景下。

Method: 首先对现有自监督模型在链接预测任务上进行严格评估，发现性能主要依赖于增强过程。然后提出基于社区结构的新结构增强方法。主要贡献是引入了两个新模型：L-GRACE和L-BGRL，这些模型基于链接表示而非节点表示。

Result: 新模型提升了现有方法的性能，特别是在无属性图上表现优异。在监督和自监督两种上下文中，L-GRACE和L-BGRL都能与最先进方法相媲美。

Conclusion: 将实例判别学习应用于链接预测是有效的，基于社区的结构增强和基于链接表示的新模型能够显著提升性能，特别是在无属性图上，为图自监督学习在链接预测任务上的应用提供了新的解决方案。
...
### [36] [Catching a Moving Subspace: Low-Rank Bandits Beyond Stationarity](https://arxiv.org/abs/2605.20269)
...
### [37] [Conformal Selective Acting: Anytime-Valid Risk Control for RLVR-Trained LLMs](https://arxiv.org/abs/2605.20270)
*Hamed Khosravi,Xiaoming Huo*
...
### [38] [Smaller Abstract State Spaces Enable Cross-Scale Generalization in Reinforcement Learning](https://arxiv.org/abs/2605.20272)
...
### [39] [Modality-Decoupled Online Recursive Editing](https://arxiv.org/abs/2605.20273)
...
### [40] [OmniISR: A Unified Framework for Centralized and Federated Learning via Intermediate Supervision and Regularization](https://arxiv.org/abs/2605.20276)
...
### [41] [ClaimDiff-RL: Fine-Grained Caption Reinforcement Learning through Visual Claim Comparison](https://arxiv.org/abs/2605.20278)
...
### [42] [Introspective X Training: Feedback Conditioning Improves Scaling Across all LLM Training Stages](https://arxiv.org/abs/2605.20285)
...
### [43] [FusionCell: Cross-Attentive Fusion of Layout Geometry and Netlist Topology for Standard-Cell Performance Prediction](https://arxiv.org/abs/2605.20287)
...
### [44] [Plug-and-Play Spiking Operators: Breaking the Nonlinearity Bottleneck in Spiking Transformers](https://arxiv.org/abs/2605.20289)
...
### [45] [Weasel: Out-of-Domain Generalization for Web Agents via Importance-Diversity Data Selection](https://arxiv.org/abs/2605.20291)
...
### [46] [TreeText-CTS: Compact, Source-Traceable Tree-Path Evidence for Irregular Clinical Time-Series Prediction](https://arxiv.org/abs/2605.20292)
...
### [47] [Closed-form predictive coding via hierarchical Gaussian filters](https://arxiv.org/abs/2605.20293)
...
### [48] [Quant.npu: Enabling Efficient Mobile NPU Inference for on-device LLMs via Fully Static Quantization](https://arxiv.org/abs/2605.20295)
...
### [49] [Spectral Unforgetting: Post-Hoc Recovery of Damaged Capabilities Without Retraining](https://arxiv.org/abs/2605.20296)
...
### [50] [Mechanisms of Misgeneralization in Physical Sequence Modeling](https://arxiv.org/abs/2605.20299)
...
### [51] [Robust Subspace-Constrained Quadratic Models for Low-Dimensional Structure Learning](https://arxiv.org/abs/2605.20300)
...
### [52] [Neural Collapse by Design: Learning Class Prototypes on the Hypersphere](https://arxiv.org/abs/2605.20302)
...
### [53] [AirfoilGen: A valid-by-construction and performance-aware latent diffusion model for airfoil generation](https://arxiv.org/abs/2605.20303)
...
### [54] [WaveGraphNet: Physics-Consistent Guided-Wave Damage Localization through Coupled Inverse-Forward Graph Learning](https://arxiv.org/abs/2605.20311)
...
### [55] [Less Data, Faster Training: repeating smaller datasets speeds up learning via sampling biases](https://arxiv.org/abs/2605.20314)
...
### [56] [Causal Unlearning in Collaborative Optimization: Exact and Approximate Influence Reversal under Adversarial Contributions](https://arxiv.org/abs/2605.20341)
...
### [57] [Symmetrization of Loss Functions for Robust Training of Neural Networks in the Presence of Noisy Labels](https://arxiv.org/abs/2605.20347)
...
### [58] [Consistently Informative Soft-Label Temperature for Knowledge Distillation](https://arxiv.org/abs/2605.20357)
...
### [59] [Nonlocal operator learning for fMRI encoding and decoding tasks](https://arxiv.org/abs/2605.20389)
...
### [60] [Score-Based Causal Discovery of Latent Variable Causal Models](https://arxiv.org/abs/2605.20396)
...
### [62] [Spectral Souping: A Unified Framework for Online Preference Alignment](https://arxiv.org/abs/2605.20408)
...
### [63] [Supervised Latent Restructuring for Small-Data Quantum Learning in Plant Phenomics](https://arxiv.org/abs/2605.20413)
...
### [64] [Miller-Index-Based Latent Crystallographic Fracture Plane Reasoning with Vision-Language Models](https://arxiv.org/abs/2605.20416)
...
### [65] [Can Conversational XAI Improve User Performance? An Experimental Study](https://arxiv.org/abs/2605.20439)
...
### [66] [Group-Algebraic Tensors: Provably-optimal Equivariant Learning and Physical Symmetry Discovery](https://arxiv.org/abs/2605.20440)
...
### [67] [Weight Decay Regimes in Grokking Transformers: Cheap Online Diagnostics](https://arxiv.org/abs/2605.20441)
*Lucky Verma*
...
### [68] [LLM Pretraining Shapes a Generalizable Manifold: Insights into Cross-Modal Transfer to Time Series](https://arxiv.org/abs/2605.20449)
...
### [69] [CASCADE Conformal Prediction: Uncertainty-Adaptive Prediction Intervals for Two-Stage Clinical Decision Support](https://arxiv.org/abs/2605.20468)
...
### [70] [Training Language Agents to Learn from Experience](https://arxiv.org/abs/2605.20477)
...
### [71] [Quadratic Characterizations for Reachability Analysis of Neural Networks](https://arxiv.org/abs/2605.20482)
...
### [72] [ZEBRA: Zero-shot Budgeted Resource Allocation for LLM Orchestration](https://arxiv.org/abs/2605.20485)
...
### [73] [Tippett-minimum Fusion of Representation-space Diffusion Models for Multi-Encoder Out-of-Distribution Detection](https://arxiv.org/abs/2605.20502)
*Neelkamal Bhuyan*
...
### [74] [Reinforcing Human Behavior Simulation via Verbal Feedback](https://arxiv.org/abs/2605.20506)
...
### [75] [Fast Reconstruction of Exact Maxwell Dynamics from Sparse Data](https://arxiv.org/abs/2605.20514)
...
### [76] [Online Conformal Prediction with Corrupted Feedback](https://arxiv.org/abs/2605.20515)
...
### [77] [An exponential mechanism based on quadratic approximations for fine-tuning machine learning models with privacy guarantees](https://arxiv.org/abs/2605.20521)
...
### [78] [Machine-Learning-Enhanced Non-Invasive Testing for MASLD Fibrosis: Shallow-Deep Neural Networks Versus FIB-4, Tabular Foundation Models, and Large Language Models](https://arxiv.org/abs/2605.20523)
...
### [79] [Ada2MS: A Hybrid Optimization Algorithm Based on Exponential Mixing of Elementwise and Global Second-Moment Estimates](https://arxiv.org/abs/2605.20533)
...
### [80] [Axiomatizing Neural Networks via Pursuit of Subspaces](https://arxiv.org/abs/2605.20534)
...
### [81] [OpenSeisML: Open Large-Scale Real Seismic and well-log Dataset for Generative AI](https://arxiv.org/abs/2605.20539)
...
### [82] [Latent Process Generator Matching](https://arxiv.org/abs/2605.20547)
...
### [83] [Complementing reinforcement learning with SFT through logit averaging in the post training of LLMs](https://arxiv.org/abs/2605.20555)
...
### [84] [Deep Learning Surrogates for Emulating Stochastic Climate Tipping Dynamics](https://arxiv.org/abs/2605.20580)
...
### [85] [TriForces: Augmenting Atomistic GNNs for Transferable Representations](https://arxiv.org/abs/2605.20581)
...
### [86] [ReversedQ: Opportunities for Faster Q-Learning in Episodic Online Reinforcement Learning](https://arxiv.org/abs/2605.20592)
...
### [87] [Unsupervised clustering and classification of upper limb EMG signals during functional movements: a data-driven](https://arxiv.org/abs/2605.20599)
...
### [88] [Mechanistic Interpretability for Learning Assurance of a Vision-Based Landing System](https://arxiv.org/abs/2605.20607)
...
### [89] [Compositional Transduction with Latent Analogies for Offline Goal-Conditioned Reinforcement Learning](https://arxiv.org/abs/2605.20609)
...
### [90] [Matryoshka Concept Bottleneck Models](https://arxiv.org/abs/2605.20612)
...
### [91] [SURF: Steering the Scalarization Weight to Uniformly Traverse the Pareto Front](https://arxiv.org/abs/2605.20619)
...
### [92] [Dynamic Shapley Computation](https://arxiv.org/abs/2605.20620)
...
### [93] [AVSD: Adaptive-View Self-Distillation by Balancing Consensus and Teacher-Specific Privileged Signals](https://arxiv.org/abs/2605.20643)
...
### [94] [The General Theory of Localization Methods](https://arxiv.org/abs/2605.20635)
*Congwei Song*
...
### [95] [Design for Manufacturing: A Manufacturability Knowledge-Integrated Reinforcement Learning Framework for Free-Form Pipe Routing in Aeroengines](https://arxiv.org/abs/2605.20644)
...
### [96] [Same Target, Different Basins: Hard vs. Soft Labels for Annotator Distributions](https://arxiv.org/abs/2605.20642)
...
### [97] [Dynamic TMoE: A Drift-Aware Dynamic Mixture of Experts Framework for Non-Stationary Time Series Forecasting](https://arxiv.org/abs/2605.20678)
...
### [98] [LT2: Linear-Time Looped Transformers](https://arxiv.org/abs/2605.20670)
...
### [99] [Hack-Verifiable Environments: Towards Evaluating Reward Hacking at Scale](https://arxiv.org/abs/2605.20744)
...
### [100] [Modular Multimodal Classification Without Fine-Tuning: A Simple Compositional Approach](https://arxiv.org/abs/2605.20674)
...
### [101] [The Hidden Signal of Verifier Strictness: Controlling and Improving Step-Wise Verification via Selective Latent Steering](https://arxiv.org/abs/2605.20745)
...
### [102] [The Devil is in the Condition Numbers: Why is GLU Better than non-GLU Structure?](https://arxiv.org/abs/2605.20749)
...
### [103] [Distributed Direct Preference Optimization](https://arxiv.org/abs/2605.20696)
*Zhanhong Jiang*
...
### [104] [Decision-Path Patterns as Tree Reliability Signals: Path-based Adaptive Weighting for Random Forest Classification](https://arxiv.org/abs/2605.20716)
*Youngjoon Park*
...
### [105] [Correcting Stochastic Update Bias in Preconditioned Language Model Optimizers](https://arxiv.org/abs/2605.20756)
...
### [106] [Tunable MAGMAX: Preference-Aware Model Merging for Continual Learning](https://arxiv.org/abs/2605.20803)
...
### [107] [Multi-Step Likelihood-Ratio Correction for Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2605.20865)
...
### [108] [Memory-Efficient Partitioned DNN Inference on Resource-Constrained Android Crowds](https://arxiv.org/abs/2605.20723)
...
### [109] [Runtime-Certified Bounded-Error Quantized Attention](https://arxiv.org/abs/2605.20868)
*Dean Calver*
...
### [110] [Winfree Oscillatory Neural Network](https://arxiv.org/abs/2605.20922)
*Jiawen Dai,Yue Song*
...
### [111] [DASH: Fast Differentiable Architecture Search for Hybrid Attention in Minutes on a Single GPU](https://arxiv.org/abs/2605.20936)
...
### [112] [A Sharper Picture of Generalization in Transformers](https://arxiv.org/abs/2605.20988)
...
### [113] [Divide et Calibra: Multiclass Local Calibration via Vector Quantization](https://arxiv.org/abs/2605.21060)
...
### [114] [ShapeBench: A Scalable Benchmark and Diagnostic Suite for Standardized Evaluation in Aerodynamic Shape Optimization](https://arxiv.org/abs/2605.20763)
...
### [115] [Cumulative Meta-Learning from Active Learning Queries for Robustness to Spurious Correlations](https://arxiv.org/abs/2605.20771)
...
### [116] [Behavior-Consistent Deep Reinforcement Learning](https://arxiv.org/abs/2605.21214)
...
### [117] [Learning to Think in Physics: Breaking Shortcut Learning in Scientific Diffusion via Representation Alignment](https://arxiv.org/abs/2605.20780)
...
### [118] [PREFINE: Preference-Based Implicit Reward and Cost Fine-Tuning for Safety Alignment](https://arxiv.org/abs/2605.21225)
...
### [119] [Causal Machine Learning Is Not a Panacea: A Roadmap for Observational Causal Inference in Health](https://arxiv.org/abs/2605.20782)
...
### [120] [OCTOPUS: Optimized KV Cache for Transformers via Octahedral Parametrization Under optimal Squared error quantization](https://arxiv.org/abs/2605.21226)
...
### [121] [Beyond Numerical Features: CNN-Driven Algorithm Selection via Contour Plots for Continuous Black-Box Optimization](https://arxiv.org/abs/2605.20797)
...
### [122] [APEX: Autonomous Policy Exploration for Self-Evolving LLM Agents](https://arxiv.org/abs/2605.21240)
...
### [123] [Most Transformer Modifications Still Do Not Transfer at 1-3B: A 2020-2026 Update to Narang et al. (2021) with Downstream Evaluation and a Noise Floor](https://arxiv.org/abs/2605.20798)
...
### [124] [How Much Online RL is Enough? Informative Rollouts for Offline Preference Optimization in RLVR](https://arxiv.org/abs/2605.21266)
...
### [125] [Markovian Circuit Tracing for Transformer State Dynamic](https://arxiv.org/abs/2605.20824)
*Abdullah X*
...
### [126] [TimeSRL: Generalizable Time-Series Behavioral Modeling via Semantic RL-Tuned LLMs -- A Case Study in Mental Health](https://arxiv.org/abs/2605.21295)
...
### [127] [Finite-Time Regret Analysis of Retry-Aware Bandits](https://arxiv.org/abs/2605.20854)
...
### [128] [From Circuit Evidence to Mechanistic Theory: An Inductive Logic Approach](https://arxiv.org/abs/2605.21303)
...
### [129] [DeCoR: Design and Control Co-Optimization for Urban Streets Using Reinforcement Learning](https://arxiv.org/abs/2605.21311)
...
### [130] [LOSCAR-SGD: Local SGD with Communication-Computation Overlap and Delay-Corrected Sparse Model Averaging](https://arxiv.org/abs/2605.20866)
...
### [131] [Data-Efficient Neural Operator Training via Physics-Based Active Learning](https://arxiv.org/abs/2605.21348)
...
### [132] [On the Regularity and Generalization of One-Step Wasserstein-guided Generative Models for PDE-Induced Measures](https://arxiv.org/abs/2605.21388)
...
### [133] [FedCritic: Serverless Federated Critic Learning-based Resource Allocation for Multi-Cell OFDMA in 6G](https://arxiv.org/abs/2605.21418)
...
### [134] [CIG: Exploration via Conditional Information Gain](https://arxiv.org/abs/2605.20878)
...
### [135] [HiRes: Inspectable Precedent Memory for Reaction Condition Recommendation](https://arxiv.org/abs/2605.21420)
...
### [136] [NeighborDiv: Training-free Zero-shot Generalist Graph Anomaly Detection via Neighbor Diversity](https://arxiv.org/abs/2605.20879)
...
### [137] [torchtune: PyTorch native post-training library](https://arxiv.org/abs/2605.21442)
...
### [138] [Learning fMRI activations dictionaries across individual geometries via optimal transport](https://arxiv.org/abs/2605.20883)
...
### [139] [Approximation Theory for Neural Networks: Old and New](https://arxiv.org/abs/2605.21451)
...
### [140] [Training distribution determines the ceiling of drug-blind cancer sensitivity prediction](https://arxiv.org/abs/2605.20885)
*Taekyung Heo*
...
### [141] [Agent JIT Compilation for Latency-Optimizing Web Agent Planning and Scheduling](https://arxiv.org/abs/2605.21470)
...
### [142] [Quantifying Hyperparameter Transfer and the Importance of Embedding Layer Learning Rate](https://arxiv.org/abs/2605.21486)
...
### [143] [Variance Reduction for Expectations with Diffusion Teachers](https://arxiv.org/abs/2605.21489)
...
### [144] [A Deployment Audit of Release-Side Risk in Conformal Triage under Prevalence Shift](https://arxiv.org/abs/2605.20956)
...
### [145] [Choose Wisely and Privately: Proactive Client Selection for Fair and Efficient Federated Learning](https://arxiv.org/abs/2605.20975)
...
### [146] [Point Cloud Sequence Encoding for Material-conditioned Graph Network Simulators](https://arxiv.org/abs/2605.20978)
...
### [147] [Modeling Temporal scRNA-seq Data with Latent Gaussian Process and Optimal Transport](https://arxiv.org/abs/2605.20989)
...
### [148] [Beyond the Bellman Recursion: A Pontryagin-Guided Framework for Non-Exponential Discounting](https://arxiv.org/abs/2605.20996)
...
### [149] [Efficient Banzhaf-Based Data Valuation for $k$-Nearest Neighbors Classification](https://arxiv.org/abs/2605.21033)
...
### [150] [A Dialogue between Causal and Traditional Representation Learning: Toward Mutual Benefits in a Unified Formulation](https://arxiv.org/abs/2605.21058)
...
### [151] [Robust Personalized Recommendation under Hidden Confounding in MNAR](https://arxiv.org/abs/2605.21066)
...
### [152] [Towards Understanding Self-Pretraining for Sequence Classification](https://arxiv.org/abs/2605.21070)
...
### [153] [Reviving Error Correction in Modern Deep Time-Series Forecasting](https://arxiv.org/abs/2605.21088)
...
### [154] [UOTIP: Unbalanced Optimal Transport Map for Unpaired Inverse Problems](https://arxiv.org/abs/2605.21094)
...
### [155] [A Typed Tensor Language for Federated Learning](https://arxiv.org/abs/2605.21103)
...
### [157] [Improved Guarantees for Constrained Online Convex Optimization via Self-Contraction](https://arxiv.org/abs/2605.21107)
...
### [158] [A Unified Framework for Uncertainty-Aware Explainable Artificial Intelligence: A Case Study in Power Quality Disturbance Classification](https://arxiv.org/abs/2605.21114)
...
### [159] [Advantage Collapse in Group Relative Policy Optimization: Diagnosis and Mitigation](https://arxiv.org/abs/2605.21125)
...
### [160] [Reasoning-Trace Collapse: Evaluating the Loss of Explicit Reasoning During Fine-Tuning](https://arxiv.org/abs/2605.21127)
...
### [161] [Learning First Integrals via Backward-Generated Data and Guided Reinforcement Learning](https://arxiv.org/abs/2605.21160)
...
### [162] [ChunkFT: Byte-Streamed Optimization for Memory-Efficient Full Fine-Tuning](https://arxiv.org/abs/2605.21177)
...
### [163] [Domain-Adaptable Reinforcement Learning for Code Generation with Dense Rewards](https://arxiv.org/abs/2605.21180)
...
### [164] [Divide and Contrast: Learning Robust Temporal Features without Augmentation](https://arxiv.org/abs/2605.21241)
...
### [165] [Graph Navier Stokes Networks](https://arxiv.org/abs/2605.21247)
...
### [166] [On the Cost and Benefit of Chain of Thought: A Learning-Theoretic Perspective](https://arxiv.org/abs/2605.21260)
...
### [167] [Nonparametric Learning and Earning with One-Point Feedback under Nonstationarity](https://arxiv.org/abs/2605.21263)
...
### [168] [FedCoE: Bridging Generalization and Personalization via Federated Coordinated Dual-level MoEs](https://arxiv.org/abs/2605.21264)
...
### [169] [A Mechanistic Study of Tabular Foundation Models](https://arxiv.org/abs/2605.21288)
...
### [170] [Automatic Discovery of Disease Subgroups by Contrasting with Healthy Controls](https://arxiv.org/abs/2605.21301)
...
### [171] [A New Framework to Analyse the Distributional Robustness of Deep Neural Networks](https://arxiv.org/abs/2605.21313)
...
### [172] [CRAFT: Conflict-Resolved Aggregation for Federated Training](https://arxiv.org/abs/2605.21317)
...
### [173] [Optimized Federated Knowledge Distillation with Distributed Neural Architecture Search](https://arxiv.org/abs/2605.21322)
...
### [174] [Fast and Stable Triangular Inversion for Delta-Rule Linear Transformers](https://arxiv.org/abs/2605.21325)
...
### [175] [Classification of Single and Mixed Partial Discharges under Switching Voltage Using an AWA-CNN Framework](https://arxiv.org/abs/2605.21352)
...
### [176] [Adaptive Signal Resuscitation: Channel-wise Post-Pruning Repair for Sparse Vision Networks](https://arxiv.org/abs/2605.21426)
...
### [177] [Polynomial-Time Robust Multiclass Linear Classification under Gaussian Marginals](https://arxiv.org/abs/2605.21428)
...
### [178] [Gaussian Sheaf Neural Networks](https://arxiv.org/abs/2605.21435)
...
### [179] [Mitigating Label Bias with Interpretable Rubric Embeddings](https://arxiv.org/abs/2605.21455)
...
### [180] [A Machine Learning Framework for Weighted Least Squares GNSS Positioning based on Activation Functions](https://arxiv.org/abs/2605.21461)
...
### [181] [DelTA: Discriminative Token Credit Assignment for Reinforcement Learning from Verifiable Rewards](https://arxiv.org/abs/2605.21467)
...
### [182] [You Only Need Minimal RLVR Training: Extrapolating LLMs via Rank-1 Trajectories](https://arxiv.org/abs/2605.21468)
...
### [184] [EvoStruct: Bridging Evolutionary and Structural Priors for Antibody CDR Design via Protein Language Model Adaptation](https://arxiv.org/abs/2605.21485)
...
### [185] [Equilibrium Reasoners: Learning Attractors Enables Scalable Reasoning](https://arxiv.org/abs/2605.21488)
