...
TL;DR: The AIdentifyAGE ontology standardizes forensic dental age assessment, addressing methodological and interoperability challenges with a semantically coherent framework that integrates manual and AI-based workflows, enhancing transparency and decision support in medico-legal contexts.


<details>
  <summary>Details</summary>
Motivation: Current dental age assessment practices face challenges like methodological heterogeneity, fragmented data representation, and limited interoperability between clinical, forensic, and legal systems. These issues hinder transparency and reproducibility, especially with the rise of AI-based methods. The AIdentifyAGE ontology aims to address these limitations by providing a standardized framework.

Method: The AIdentifyAGE ontology is developed collaboratively with domain experts, building on established biomedical, dental, and machine learning ontologies. It models the complete medico-legal workflow, including judicial context, individual-level information, forensic examination data, dental developmental assessment methods, radiographic imaging, statistical reference studies, and AI-based estimation methods.

Result: The ontology enables traceable linkage between observations, methods, reference data, and reported outcomes, integrating both manual and AI-assisted workflows. It ensures interoperability, extensibility, and FAIR compliance, supporting ontology-driven decision systems in medico-legal contexts.

Conclusion: The AIdentifyAGE ontology is a significant advancement in forensic dental age assessment, providing a standardized and semantically coherent framework that enhances consistency, transparency, and explainability. It integrates diverse data sources and methods, ensuring interoperability and compliance with FAIR principles, and lays a foundation for ontology-driven decision support systems in medico-legal contexts.
...
TL;DR: 论文探讨了自适应系统中单一状态复用的基本表示后果，证明上下文性不仅是量子力学的特性，也是经典概率表示中单一状态复用的必然结果。


<details>
  <summary>Details</summary>
Motivation: 研究自适应系统在多种上下文中运行时，由于内存、表示或物理资源的限制而重复使用固定内部状态空间的基本表示后果。

Method: 通过将上下文建模为作用于共享内部状态的干预措施，证明任何再现上下文结果统计的经典模型必须承担不可约简的信息理论成本。

Result: 证明了上下文性作为自适应智能的一般表示约束，独立于物理实现。

Conclusion: 上下文性是自适应智能中单一状态复用的必然结果，非经典概率框架通过放松单一全局联合概率空间的假设避免了这一障碍。
...
TL;DR: MobCache框架通过重构缓存和轻量级解码器，显著提升大规模人类移动模拟的效率，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 解决基于大语言模型的人类移动模拟方法计算成本高、难以扩展的问题。

Method: 设计MobCache框架，包括推理组件（编码推理步骤为嵌入并重用）和解码组件（轻量级解码器）。

Result: 实验表明MobCache在多个维度显著提升效率，性能接近最先进方法。

Conclusion: MobCache为大规模人类移动模拟提供了一种高效且性能优越的解决方案。
...
TL;DR: 研究发现近一半的LLM基准测试饱和，专家设计的基准测试更耐用，隐藏测试数据无助于防止饱和。


<details>
  <summary>Details</summary>
Motivation: AI基准测试在模型开发和部署决策中起关键作用，但许多基准测试很快饱和，无法区分最佳性能模型，降低了其长期价值。

Method: 分析了60个大型语言模型（LLM）基准测试，通过14个属性（包括任务设计、数据构建和评估格式）来表征基准测试，并测试了五个假设以探讨每个属性对饱和率的影响。

Result: 近一半的基准测试表现出饱和现象，专家设计的基准测试比众包的基准测试更能抵抗饱和，隐藏测试数据对防止饱和无显著效果。

Conclusion: 研究发现，近一半的基准测试表现出饱和现象，且随着时间推移，饱和率增加。专家设计的基准测试比众包的基准测试更能抵抗饱和，而隐藏测试数据对防止饱和没有显著效果。
...
TL;DR: 简单基线方法在代码进化任务中表现优异，复杂方法未显著超越；未来需改进评估方法和实践。


<details>
  <summary>Details</summary>
Motivation: 研究动机是验证简单基线方法在代码进化任务中的表现，并揭示当前代码进化技术的局限性。

Method: 论文通过测试两种简单基线方法在三个领域（数学边界优化、智能体支架设计和机器学习竞赛）中的表现，并与复杂代码进化技术进行对比。

Result: 结果显示，简单基线方法在所有三个领域中均表现优于或等同于复杂方法。数学边界优化中，搜索空间和领域知识起主要作用；智能体支架设计中，高方差和小数据集导致选择次优支架。

Conclusion: 论文指出，简单的基线方法在多个领域中表现优于或等同于复杂的代码进化技术，并揭示了当前代码进化方法在开发和评估中的不足。未来工作需要更严谨的代码进化实践和更好的评估方法。
...
TL;DR: Improved upper bound for hyperplanes slicing hypercube edges, using automated tools for construction discovery.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the previously known upper bound of $S(n) \leq \lceil\frac{5n}{6} \rceil$ from 1971 and explore the slicing of hypercube edges more efficiently.

Method: The authors use a construction of 8 hyperplanes for $Q_{10}$ and leverage CPro1, an automatic tool combining reasoning LLMs and hyperparameter tuning, to discover mathematical constructions.

Result: The main result is the improved upper bound $S(n) \leq \lceil \frac{4n}{5} \rceil$ (with an exception for odd multiples of 5). Additionally, new lower bounds for edge slicing with fewer hyperplanes are established.

Conclusion: The paper concludes with an improved upper bound for the minimum number of hyperplanes needed to slice all edges of an $n$-dimensional hypercube, specifically $S(n) \leq \lceil \frac{4n}{5} \rceil$ (with an exception for odd multiples of 5). It also introduces new lower bounds for the maximum number of edges sliced by fewer hyperplanes.
...
TL;DR: NeuDiff Agent, a governed AI workflow, cuts analysis time by 4.6-5.0x for crystallography, ensuring validated results and traceability.


<details>
  <summary>Details</summary>
Motivation: To address the latency in analysis and reporting for large-scale facilities, particularly for complex samples requiring iterative processes, by improving time-to-result and analysis efficiency.

Method: NeuDiff Agent is a governed, tool-using AI workflow that processes instrument data through reduction, integration, refinement, and validation, enforcing fail-closed verification gates and capturing complete provenance.

Result: In benchmark tests, NeuDiff Agent reduced wall time from 435 minutes (manual) to 86.5-94.4 minutes (4.6-5.0x faster) and produced validated CIFs with no critical alerts.

Conclusion: NeuDiff Agent provides a practical solution for deploying agentic AI in facility crystallography, significantly reducing analysis time while maintaining traceability and validation standards.
...
TL;DR: 本文提出基于有序理论的犹豫模糊集评分框架，证明对称顺序满足关键规范标准，并引入优势函数类支持决策。


<details>
  <summary>Details</summary>
Motivation: 传统犹豫模糊集评分方法缺乏有序理论的正式基础，导致评分机制不够灵活和一致。本文旨在填补这一理论空白，并提供更灵活的评分框架。

Method: 通过分析犹豫模糊元素的经典顺序，证明其不构成格结构，并基于对称顺序定义评分函数。引入优势函数类，包括离散优势函数和相对优势函数，用于构建模糊偏好关系。

Result: 证明了对称顺序定义的评分函数满足强单调性和Gärdenfors条件。优势函数类能够有效比较犹豫模糊元素，并支持群体决策。

Conclusion: 本文提出了一个基于有序理论的统一框架，用于定义犹豫模糊集的评分函数，并通过对称顺序满足强单调性和Gärdenfors条件等关键规范标准。此外，引入了优势函数类，用于比较犹豫模糊元素，并支持群体决策。
...
### [9] [LLM-WikiRace: Benchmarking Long-term Planning and Reasoning over Real-World Knowledge Graphs](https://arxiv.org/abs/2602.16902)
*Juliusz Ziomek,William Bankes,Lorenz Wolf,Shyam Sundhar Ramesh,Xiaohang Tang,Ilija Bogunovic*

Main category: cs.AI

TL;DR: LLM-Wikirace 是一个评估语言模型规划与推理能力的基准测试，结果显示模型在困难任务中表现不佳，规划和长时程推理是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 为了评估大型语言模型在规划、推理和世界知识方面的能力，设计了一个基于维基百科链接导航的任务。

Method: 通过评估一系列开源和闭源模型（如 Gemini-3、GPT-5 和 Claude Opus 4.5）在 LLM-Wikirace 任务中的表现，分析其成功与失败的原因。

Result: 在简单任务中，模型表现优异甚至超越人类，但在困难任务中表现显著下降（如 Gemini-3 仅成功 23%）。世界知识是成功的关键，但超过一定阈值后，规划和长时程推理能力成为主导因素。

Conclusion: LLM-Wikirace 是一个简单但有效的基准测试，揭示了当前推理系统的明显局限性，尤其是在规划能力和长时程推理方面。
...
### [10] [DeepContext: Stateful Real-Time Detection of Multi-Turn Adversarial Intent Drift in LLMs](https://arxiv.org/abs/2602.16935)
*Justin Albrethsen,Yash Datta,Kunal Kumar,Sharath Rajasekar*

Main category: cs.AI

TL;DR: DeepContext是一种状态化监控框架，通过RNN捕捉多轮对话中的风险累积，显著提升了安全性检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM安全防护机制缺乏时序感知能力，导致在多轮对话中容易被绕过，因此需要一种状态化的监控框架。

Method: DeepContext采用RNN架构，通过传播隐藏状态来捕捉对话中的风险累积，替代了传统的孤立评估模型。

Result: DeepContext在多轮越狱检测中取得了0.84的F1分数，显著优于现有基线，并在T4 GPU上保持了低于20ms的推理延迟。

Conclusion: DeepContext通过引入状态感知机制，显著提升了多轮对话中的安全性检测能力，证明了时序建模在意图识别中的重要性。
...
### [11] [Mind the GAP: Text Safety Does Not Transfer to Tool-Call Safety in LLM Agents](https://arxiv.org/abs/2602.16943)
...
### [12] [LLM4Cov: Execution-Aware Agentic Learning for High-coverage Testbench Generation](https://arxiv.org/abs/2602.16953)
...
### [21] [Phase-Aware Mixture of Experts for Agentic Reinforcement Learning](https://arxiv.org/abs/2602.17038)
...
### [22] [Dynamic System Instructions and Tool Exposure for Efficient Agentic LLMs](https://arxiv.org/abs/2602.17046)
*Uria Franko*
...
### [23] [IntentCUA: Learning Intent-level Representations for Skill Abstraction and Multi-Agent Planning in Computer-Use Agents](https://arxiv.org/abs/2602.17049)
...
### [24] [RFEval: Benchmarking Reasoning Faithfulness under Counterfactual Reasoning Intervention in Large Reasoning Models](https://arxiv.org/abs/2602.17053)
...
### [25] [Retaining Suboptimal Actions to Follow Shifting Optima in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2602.17062)
...
### [26] [Predictive Batch Scheduling: Accelerating Language Model Training Through Loss-Aware Sample Prioritization](https://arxiv.org/abs/2602.17066)
*Sumedh Rasal*
...
### [27] [How AI Coding Agents Communicate: A Study of Pull Request Description Characteristics and Human Review Responses](https://arxiv.org/abs/2602.17084)
...
### [28] [Agentic Wireless Communication for 6G: Intent-Aware and Continuously Evolving Physical-Layer Intelligence](https://arxiv.org/abs/2602.17096)
...
### [29] [Toward Trustworthy Evaluation of Sustainability Rating Methodologies: A Human-AI Collaborative Framework for Benchmark Dataset Construction](https://arxiv.org/abs/2602.17106)
...
### [30] [Owen-based Semantics and Hierarchy-Aware Explanation (O-Shap)](https://arxiv.org/abs/2602.17107)
...
### [31] [Instructor-Aligned Knowledge Graphs for Personalized Learning](https://arxiv.org/abs/2602.17111)
...
### [32] [Epistemology of Generative AI: The Geometry of Knowing](https://arxiv.org/abs/2602.17116)
*Ilya Levin*
...
### [33] [Efficient Parallel Algorithm for Decomposing Hard CircuitSAT Instances](https://arxiv.org/abs/2602.17130)
...
### [34] [Bonsai: A Framework for Convolutional Neural Network Acceleration Using Criterion-Based Pruning](https://arxiv.org/abs/2602.17145)
...
### [37] [Continual learning and refinement of causal models through dynamic predicate invention](https://arxiv.org/abs/2602.17217)
...
### [38] [From Labor to Collaboration: A Methodological Experiment Using AI Agents to Augment Research Perspectives in Taiwan's Humanities and Social Sciences](https://arxiv.org/abs/2602.17221)
*Yi-Chih Huang*
...
### [39] [Decoding the Human Factor: High Fidelity Behavioral Prediction for Strategic Foresight](https://arxiv.org/abs/2602.17222)
...
### [40] [Mechanistic Interpretability of Cognitive Complexity in LLMs via Linear Probing using Bloom's Taxonomy](https://arxiv.org/abs/2602.17229)
...
### [41] [All Leaks Count, Some Count More: Interpretable Temporal Contamination Detection in LLM Backtesting](https://arxiv.org/abs/2602.17234)
...
### [42] [Web Verbs: Typed Abstractions for Reliable Task Composition on the Agentic Web](https://arxiv.org/abs/2602.17245)
...
### [43] [ArXiv-to-Model: A Practical Study of Scientific LM Training](https://arxiv.org/abs/2602.17288)
*Anuj Gupta*
...
### [44] [MedClarify: An information-seeking AI agent for medical diagnosis with case-specific follow-up questions](https://arxiv.org/abs/2602.17308)
...
### [45] [Dataless Weight Disentanglement in Task Arithmetic via Kronecker-Factored Approximate Curvature](https://arxiv.org/abs/2602.17385)
...
### [46] [Visual Model Checking: Graph-Based Inference of Visual Routines for Image Retrieval](https://arxiv.org/abs/2602.17386)
...
### [47] [A Contrastive Variational AutoEncoder for NSCLC Survival Prediction with Missing Modalities](https://arxiv.org/abs/2602.17402)
...
### [48] [A Privacy by Design Framework for Large Language Model-Based Applications for Children](https://arxiv.org/abs/2602.17418)
...
### [50] [Pareto Optimal Benchmarking of AI Models on ARM Cortex Processors for Sustainable Embedded Systems](https://arxiv.org/abs/2602.17508)
...
### [51] [Enhancing Large Language Models (LLMs) for Telecom using Dynamic Knowledge Graphs and Explainable Retrieval-Augmented Generation](https://arxiv.org/abs/2602.17529)
...
### [52] [Evaluating Chain-of-Thought Reasoning through Reusability and Verifiability](https://arxiv.org/abs/2602.17544)
...
### [53] [KLong: Training LLM Agent for Extremely Long-horizon Tasks](https://arxiv.org/abs/2602.17547)
...
### [54] [ODESteer: A Unified ODE-Based Steering Framework for LLM Alignment](https://arxiv.org/abs/2602.17560)
...
### [55] [A Hybrid Federated Learning Based Ensemble Approach for Lung Disease Diagnosis Leveraging Fusion of SWIN Transformer and CNN](https://arxiv.org/abs/2602.17566)
...
### [56] [AI Gamestore: Scalable, Open-Ended Evaluation of Machine General Intelligence with Human Games](https://arxiv.org/abs/2602.17594)
...
### [57] [MolHIT: Advancing Molecular-Graph Generation with Hierarchical Discrete Diffusion Models](https://arxiv.org/abs/2602.17602)
...
### [58] [AutoNumerics: An Autonomous, PDE-Agnostic Multi-Agent Pipeline for Scientific Computing](https://arxiv.org/abs/2602.17607)
...
### [60] [MMCAformer: Macro-Micro Cross-Attention Transformer for Traffic Speed Prediction with Microscopic Connected Vehicle Driving Behavior](https://arxiv.org/abs/2602.16730)
*Lei Han,Mohamed Abdel-Aty,Younggun Kim,Yang-Jun Joo,Zubayer Islam*

Main category: cs.LG

TL;DR: MMCAformer模型结合宏观交通和微观驾驶行为特征，显著提升了速度预测的准确性和不确定性估计能力，尤其在拥堵条件下效果更佳。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要依赖宏观交通流数据进行速度预测，但忽略了微观驾驶行为的影响。通过利用Connected Vehicle (CV)数据提供的丰富驾驶行为特征，可以进一步提升预测精度。

Method: 提出了Macro-Micro Cross-Attention Transformer (MMCAformer)模型，利用自注意力机制学习宏观交通流的内部依赖关系，并通过交叉注意力机制捕捉宏观交通状态与微观驾驶行为之间的时空交互。

Result: 实验表明，MMCAformer在四个佛罗里达州高速公路上表现优于基线模型。引入微观驾驶行为特征后，预测准确性（如RMSE、MAE和MAPE分别降低9.0%、6.9%和10.2%）和不确定性估计（预测区间平均减少10.1-24.0%）均显著提升。

Conclusion: MMCAformer通过结合宏观交通特征和微观驾驶行为特征，显著提高了速度预测的准确性和模型的不确定性估计能力，尤其是在拥堵和低速交通条件下表现更为突出。
...
### [61] [A Few-Shot LLM Framework for Extreme Day Classification in Electricity Markets](https://arxiv.org/abs/2602.16735)
*Saud Alghumayjan,Ming Yi,Bolun Xu*

Main category: cs.LG

TL;DR: 本文提出了一种基于大语言模型（LLMs）的小样本分类框架，用于预测实时电价是否会在次日出现峰值。该方法通过将系统状态信息（如电力需求、可再生能源发电、天气预报和近期电价）转化为自然语言提示，并结合通用指令输入LLM，最终输出峰值日的概率和置信度。实验表明，该方法在数据有限时优于传统监督学习模型。


<details>
  <summary>Details</summary>
Motivation: 在电力市场中，准确预测电价峰值对市场参与者至关重要，但传统方法在数据稀缺时表现不佳。本文旨在探索LLMs在数据高效分类中的潜力。

Method: 将电力系统状态信息转化为自然语言提示，结合通用指令输入LLM，进行小样本分类。

Result: 在德州电力市场历史数据上的实验表明，该方法在数据有限时优于支持向量机和XGBoost。

Conclusion: LLMs可作为数据稀缺场景下电价峰值分类的高效工具。
...
### [62] [Real-time Secondary Crash Likelihood Prediction Excluding Post Primary Crash Features](https://arxiv.org/abs/2602.16739)
*Lei Han,Mohamed Abdel-Aty,Zubayer Islam,Chenzhu Wang*

Main category: cs.LG

TL;DR: A hybrid framework using real-time traffic data and ensemble learning predicts secondary crashes with high accuracy (91%) and low false alarms (0.20), outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing approaches rely on post-crash features, which are rarely available in real time, limiting practical applicability. The study aims to address this limitation by proposing a real-time prediction framework.

Method: A hybrid framework with a dynamic spatiotemporal window extracts real-time features. It includes three models (primary crash and two secondary crash models) and uses an ensemble learning strategy with six machine learning algorithms and a voting-based mechanism.

Result: The framework correctly identifies 91% of secondary crashes with a false alarm rate of 0.20. The hybrid model's AUC improves to 0.952, outperforming individual models (0.654, 0.744, 0.902) and previous studies.

Conclusion: The proposed hybrid framework significantly improves secondary crash likelihood prediction by leveraging real-time traffic flow and environmental features, achieving high accuracy and low false alarm rates.
...
### [63] [Quantifying LLM Attention-Head Stability: Implications for Circuit Universality](https://arxiv.org/abs/2602.16740)
*Karan Bali,Jack Stanley,Praneet Suresh,Danilo Bzdok*

Main category: cs.LG

TL;DR: 研究变压器电路中注意力头的跨实例稳定性，发现中层头最不稳定但最具代表性，权重衰减优化可提高稳定性，为AI系统监控提供依据。


<details>
  <summary>Details</summary>
Motivation: 探讨变压器电路中报告的电路是否具有跨实例的稳定性，以确定其是否具有普遍性，从而提升在安全关键设置中的可信度。

Method: 通过系统地研究不同大小和复杂度的变压器语言模型，逐层量化注意力头在独立初始化训练运行中的表示学习相似性。

Result: 研究发现：（1）中间层头最不稳定但最具代表性；（2）更深模型的中层分歧更强；（3）深层中不稳定的头功能上更重要；（4）权重衰减优化显著提高注意力头的稳定性；（5）残差流相对稳定。

Conclusion: 研究结果表明，跨实例的电路鲁棒性是实现可扩展监督的重要前提，为AI系统的白盒监控性提供了可能。
...
### [64] [DeepVision-103K: A Visually Diverse, Broad-Coverage, and Verifiable Mathematical Dataset for Multimodal Reasoning](https://arxiv.org/abs/2602.16742)
*Haoxiang Sun,Lizhen Xu,Bing Zhao,Wotao Yin,Wei Wang,Boyu Yang,Rui Wang,Hu Wei*

Main category: cs.LG

TL;DR: DeepVision-103K是一个多样化的数据集，用于提升多模态模型的视觉反思和推理能力，显著提高了模型在数学和通用推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有数据集多源于小规模手动构建或现有资源的重组，缺乏多样性和覆盖范围，限制了模型性能的进一步提升。

Method: 引入DeepVision-103K数据集，该数据集覆盖了K12数学主题、广泛的知识点和丰富的视觉元素，用于强化学习与可验证奖励（RLVR）的训练。

Result: 在DeepVision上训练的模型在多模态数学基准测试中表现优异，并能有效泛化到通用多模态推理任务中。

Conclusion: DeepVision-103K数据集通过其多样性和全面性，显著提升了多模态模型在数学任务和通用多模态推理任务中的表现，验证了其在增强视觉感知、反思和推理能力方面的有效性。
...
### [65] [PETS: A Principled Framework Towards Optimal Trajectory Allocation for Efficient Test-Time Self-Consistency](https://arxiv.org/abs/2602.16745)
*Zhangyi Liu,Huaizhi Qu,Xiaowei Yin,He Sun,Yanjun Han,Tianlong Chen,Zhun Deng*

Main category: cs.LG

TL;DR: PETS通过优化框架实现高效自一致性测试，显著减少预算并优于均匀分配。


<details>
  <summary>Details</summary>
Motivation: 解决在有限预算下实现样本高效的自一致性测试的开放挑战。

Method: PETS引入自一致性率作为新度量，并通过优化框架研究轨迹分配问题。离线设置中，将推理轨迹建模为众包工人；在线设置中，提出基于离线框架的新方法。

Result: 在GPQA数据集上，PETS在离线与在线设置中均实现了完美的自一致性，同时分别减少了75%和55%的采样预算。

Conclusion: PETS通过优化框架实现了样本高效的自一致性测试，显著减少了采样预算，同时在离线与在线设置中均表现优于均匀分配。
...
### [66] [Low-Dimensional and Transversely Curved Optimization Dynamics in Grokking](https://arxiv.org/abs/2602.16746)
*Yongzhong Xu*

Main category: cs.LG

TL;DR: The paper explores grokking in transformers via geometric analysis, finding that generalization involves escaping a low-dimensional, high-curvature subspace, with motion along this subspace being essential.


<details>
  <summary>Details</summary>
Motivation: The motivation is to better understand grokking, the delayed transition from memorization to generalization in small algorithmic tasks, which remains poorly understood.

Method: The paper employs PCA of attention weight trajectories to analyze optimization dynamics in transformers trained on modular arithmetic. It measures commutator defects and projects them onto the learned subspace to probe loss-landscape geometry. Causal intervention experiments are also conducted.

Result: The results reveal that training evolves predominantly within a low-dimensional execution subspace, with curvature growing sharply in orthogonal directions. Curvature growth precedes generalization, and motion along the learned subspace is necessary for grokking.

Conclusion: The study provides a geometric account of grokking, showing that it involves escape from a metastable regime characterized by low-dimensional confinement and transverse curvature accumulation. Motion along the learned subspace is necessary for grokking, while artificially increasing curvature is insufficient.
...
### [67] [LiveClin: A Live Clinical Benchmark without Leakage](https://arxiv.org/abs/2602.16747)
*Xidong Wang,Shuqi Guo,Yue Shen,Junying Chen,Jian Wang,Jinjie Gu,Ping Zhang,Lei Liu,Benyou Wang*

Main category: cs.LG

TL;DR: LiveClin是一个动态临床基准，通过真实病例和多模态评估揭示医学LLM与人类专家的差距，推动其可靠性提升。


<details>
  <summary>Details</summary>
Motivation: 解决医学LLM评估中因数据污染和知识过时导致的可靠性问题，并模拟真实临床实践。

Method: 通过构建基于当代同行评审病例报告的LiveClin基准，采用AI-人类协作流程（涉及239名医生），将真实病例转化为多模态评估场景。

Result: 在LiveClin上评估的26个模型中，表现最佳的模型案例准确率仅为35.7%，远低于人类专家（如主任医师和主治医师）。

Conclusion: LiveClin提供了一个持续演进的临床基准框架，旨在缩小医学LLM与人类专家之间的差距，提升其可靠性和实际应用价值。
...
### [68] [Attending to Routers Aids Indoor Wireless Localization](https://arxiv.org/abs/2602.16762)
*Ayush Roy,Tahsin Fuad Hassan,Roshan Ayyalasomayajula,Vishnu Suresh Lokhande*

Main category: cs.LG

TL;DR: 论文提出了一种基于注意力机制的Wi-Fi信号定位方法，通过差异化加权路由器信息，显著提升了定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有算法在聚合不同路由器信息时未能合理加权，导致收敛不理想和精度下降，传统加权三角测量方法启发了这一研究。

Method: 在标准机器学习定位架构中引入注意力层，差异化加权每个路由器的贡献。

Result: 在公开数据集上的评估显示，该方法比基准架构的精度提高了30%以上。

Conclusion: 通过注意力机制加权路由器信息，能显著提升Wi-Fi信号定位的性能，适用于多样化环境。
...
### [69] [Omitted Variable Bias in Language Models Under Distribution Shift](https://arxiv.org/abs/2602.16784)
*Victoria Lin,Louis-Philippe Morency,Eli Ben-Michael*

Main category: cs.LG

TL;DR: 本文提出一种框架，通过映射未观测变量强度到泛化性能边界，优化语言模型在分布偏移下的表现，实验证明其优于标准方法。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型在多种任务上表现优异，但对分布偏移表现脆弱，尤其是未观测变量导致的偏差可能影响评估和优化。本文旨在解决这一问题。

Method: 本文描述了一种将分布偏移分为可观测和不可观测组件的框架，并通过将未观测变量的强度映射到泛化性能边界来优化语言模型。

Result: 实验证明，该框架在语言模型评估和优化中提供了更原则性的方法，优于标准分布偏移调整方法，并能推断未观测变量的强度。

Conclusion: 本文提出了一种框架，通过将未观测变量的强度映射到分布偏移下语言模型最坏情况泛化性能的边界，从而为语言模型的评估和优化提供了更原则性的方法。实验表明，该方法在真实分布偏移性能上优于标准调整方法，并能在目标分布标签可用时推断未观测变量的强度。
...
### [70] [Better Think Thrice: Learning to Reason Causally with Double Counterfactual Consistency](https://arxiv.org/abs/2602.16787)
*Victoria Lin,Xinnuo Xu,Rachel Lawrence,Risa Ueno,Amit Sharma,Javier Gonzalez,Niranjani Prasad*

Main category: cs.LG

TL;DR: 本文提出了一种名为双反事实一致性（DCC）的轻量级推理方法，用于评估和提升大型语言模型（LLMs）的因果推理能力，无需标记的反事实数据。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在推理基准测试中表现优异，但在面对反事实问题时表现脆弱，显示出其因果推理能力的不足，而现有方法难以大规模生成标记的反事实数据。

Method: 提出DCC方法，通过验证模型在因果干预和反事实预测两个关键环节的能力，评估其因果推理能力。

Result: DCC在多种推理任务和干预中评估了领先LLMs的因果推理能力，并作为无训练测试时拒绝采样标准，显著提升了多模型家族在推理任务中的表现。

Conclusion: DCC是一种有效的轻量级方法，能够无需标记数据即可提升LLMs的因果推理能力，并在实际应用中表现出显著效果。
...
### [71] [Escaping the Cognitive Well: Efficient Competition Math with Off-the-Shelf Models](https://arxiv.org/abs/2602.16793)
*Xingyu Dang,Rohit Agarwal,Rodrigo Porto,Anirudh Goyal,Liam H Fowl,Sanjeev Arora*

Main category: cs.LG

TL;DR: 本文提出了一种低成本、高性能的推理管道，用于解决IMO风格数学问题，通过猜想提取和独立验证显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有公开模型在解决IMO数学问题时成本过高，而定制模型虽性能优异但不公开。本文旨在开发一种低成本、高性能的通用解决方案。

Method: 通过猜想提取和独立验证（上下文分离）解决认知陷阱问题，利用通用现成模型构建推理管道。

Result: 在IMO-ProofBench Advanced上，使用Gemini 3.0 Pro实现了67.1%的性能，平均成本为31美元/问题，显著优于其他公开方法。

Conclusion: 该方法在低成本下实现了最佳性能，为数学推理领域提供了高效且可访问的解决方案。
...
### [72] [Efficient Tail-Aware Generative Optimization via Flow Model Fine-Tuning](https://arxiv.org/abs/2602.16796)
*Zifan Wang,Riccardo De Santi,Xiaoyu Mo,Michael M. Zavlanos,Andreas Krause,Karl H. Johansson*

Main category: cs.LG

TL;DR: TFFT是一种基于CVaR的分布微调算法，通过两阶段分解实现高效的尾部优化，适用于多种任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注期望奖励，缺乏对尾部行为的控制，而尾部行为在实际应用中至关重要。

Method: TFFT采用两阶段分解方法：轻量级一维阈值优化和基于伪奖励的熵正则化微调。

Result: TFFT在文本到图像生成和分子设计等实验中表现出色，证明了其有效性。

Conclusion: TFFT通过CVaR的双重变分公式，实现了高效且计算成本可控的尾部优化，适用于多种实际应用场景。
...
### [73] [TopoFlow: Physics-guided Neural Networks for high-resolution air quality prediction](https://arxiv.org/abs/2602.16821)
...
### [74] [Formal Mechanistic Interpretability: Automated Circuit Discovery with Provable Guarantees](https://arxiv.org/abs/2602.16823)
...
### [75] [HiVAE: Hierarchical Latent Variables for Scalable Theory of Mind](https://arxiv.org/abs/2602.16826)
...
### [76] [Learning under noisy supervision is governed by a feedback-truth gap](https://arxiv.org/abs/2602.16829)
...
### [77] [VAM: Verbalized Action Masking for Controllable Exploration in RL Post-Training -- A Chess Case Study](https://arxiv.org/abs/2602.16833)
...
### [78] [A Residual-Aware Theory of Position Bias in Transformers](https://arxiv.org/abs/2602.16837)
...
### [79] [Training Large Reasoning Models Efficiently via Progressive Thought Encoding](https://arxiv.org/abs/2602.16839)
...
### [80] [What is the Value of Censored Data? An Exact Analysis for the Data-driven Newsvendor](https://arxiv.org/abs/2602.16842)
...
### [81] [On the Mechanism and Dynamics of Modular Addition: Fourier Features, Lottery Ticket, and Grokking](https://arxiv.org/abs/2602.16849)
...
### [82] [Position: Why a Dynamical Systems Perspective is Needed to Advance Time Series Modeling](https://arxiv.org/abs/2602.16864)
...
### [83] [ML-driven detection and reduction of ballast information in multi-modal datasets](https://arxiv.org/abs/2602.16876)
*Yaroslav Solovko*
...
### [84] [Construction of a classification model for dementia among Brazilian adults aged 50 and over](https://arxiv.org/abs/2602.16887)
...
### [85] [Exact Certification of Data-Poisoning Attacks Using Mixed-Integer Programming](https://arxiv.org/abs/2602.16944)
...
### [86] [Beyond Message Passing: A Symbolic Alternative for Expressive and Interpretable Graph Learning](https://arxiv.org/abs/2602.16947)
...
### [87] [Multi-Agent Lipschitz Bandits](https://arxiv.org/abs/2602.16965)
...
### [88] [A Unified Framework for Locality in Scalable MARL](https://arxiv.org/abs/2602.16966)
*Sourav Chakraborty,Amit Kiran Rege,Claire Monteleoni,Lijun Chen*
...
### [89] [Early-Warning Signals of Grokking via Loss-Landscape Geometry](https://arxiv.org/abs/2602.16967)
*Yongzhong Xu*
...
### [90] [Fail-Closed Alignment for Large Language Models](https://arxiv.org/abs/2602.16977)
...
### [91] [Discovering Universal Activation Directions for PII Leakage in Language Models](https://arxiv.org/abs/2602.16980)
...
### [92] [Dynamic Delayed Tree Expansion For Improved Multi-Path Speculative Decoding](https://arxiv.org/abs/2602.16994)
...
### [93] [Action-Graph Policies: Learning Action Co-dependencies in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2602.17009)
...
### [94] [Malliavin Calculus as Stochastic Backpropogation](https://arxiv.org/abs/2602.17013)
*Kevin D. Oden*
...
### [95] [WS-GRPO: Weakly-Supervised Group-Relative Policy Optimization for Rollout-Efficient Reasoning](https://arxiv.org/abs/2602.17025)
...
### [96] [Transforming Behavioral Neuroscience Discovery with In-Context Learning and AI-Enhanced Tensor Methods](https://arxiv.org/abs/2602.17027)
...
### [97] [Forecasting Anomaly Precursors via Uncertainty-Aware Time-Series Ensembles](https://arxiv.org/abs/2602.17028)
...
### [98] [Multi-Probe Zero Collision Hash (MPZCH): Mitigating Embedding Collisions and Enhancing Model Freshness in Large-Scale Recommenders](https://arxiv.org/abs/2602.17050)
...
### [99] [Sign Lock-In: Randomly Initialized Weight Signs Persist and Bottleneck Sub-Bit Model Compression](https://arxiv.org/abs/2602.17063)
...
### [100] [Spatio-temporal dual-stage hypergraph MARL for human-centric multimodal corridor traffic signal control](https://arxiv.org/abs/2602.17068)
...
### [101] [AdvSynGNN: Structure-Adaptive Graph Neural Nets via Adversarial Synthesis and Self-Corrective Propagation](https://arxiv.org/abs/2602.17071)
...
### [103] [MeGU: Machine-Guided Unlearning with Target Feature Disentanglement](https://arxiv.org/abs/2602.17088)
...
### [104] [Synergizing Transport-Based Generative Models and Latent Geometry for Stochastic Closure Modeling](https://arxiv.org/abs/2602.17089)
...
### [105] [A Locality Radius Framework for Understanding Relational Inductive Bias in Database Learning](https://arxiv.org/abs/2602.17092)
...
### [106] [FLoRG: Federated Fine-tuning with Low-rank Gram Matrices and Procrustes Alignment](https://arxiv.org/abs/2602.17095)
...
### [107] [Operationalization of Machine Learning with Serverless Architecture: An Industrial Operationalization of Machine Learning with Serverless Architecture: An Industrial Implementation for Harmonized System Code Prediction](https://arxiv.org/abs/2602.17102)
...
### [108] [Online Learning with Improving Agents: Multiclass, Budgeted Agents and Bandit Learners](https://arxiv.org/abs/2602.17103)
...
### [109] [i-PhysGaussian: Implicit Physical Simulation for 3D Gaussian Splatting](https://arxiv.org/abs/2602.17117)
...
### [110] [TIFO: Time-Invariant Frequency Operator for Stationarity-Aware Representation Learning in Time Series](https://arxiv.org/abs/2602.17122)
...
### [111] [VP-VAE: Rethinking Vector Quantization via Adaptive Vector Perturbation](https://arxiv.org/abs/2602.17133)
...
### [112] [When More Experts Hurt: Underfitting in Multi-Expert Learning to Defer](https://arxiv.org/abs/2602.17144)
...
### [113] [TimeOmni-VL: Unified Models for Time Series Understanding and Generation](https://arxiv.org/abs/2602.17149)
...
### [114] [Powering Up Zeroth-Order Training via Subspace Gradient Orthogonalization](https://arxiv.org/abs/2602.17155)
...
### [115] [In-Context Learning in Linear vs. Quadratic Attention Models: An Empirical Study on Regression Tasks](https://arxiv.org/abs/2602.17171)
...
### [116] [Continual uncertainty learning](https://arxiv.org/abs/2602.17174)
...
### [117] [SoftDTW-CUDA-Torch: Memory-Efficient GPU-Accelerated Soft Dynamic Time Warping for PyTorch](https://arxiv.org/abs/2602.17206)
...
### [118] [CounterFlowNet: From Minimal Changes to Meaningful Counterfactual Explanations](https://arxiv.org/abs/2602.17244)
...
### [119] [Structured Prototype-Guided Adaptation for EEG Foundation Models](https://arxiv.org/abs/2602.17251)
...
### [120] [Learning a Latent Pulse Shape Interface for Photoinjector Laser Systems](https://arxiv.org/abs/2602.17263)
...
### [122] [RLGT: A reinforcement learning framework for extremal graph theory](https://arxiv.org/abs/2602.17276)
...
### [123] [Efficient privacy loss accounting for subsampling and random allocation](https://arxiv.org/abs/2602.17284)
...
### [124] [LexiSafe: Offline Safe Reinforcement Learning with Lexicographic Safety-Reward Hierarchy](https://arxiv.org/abs/2602.17312)
...
### [125] [Flickering Multi-Armed Bandits](https://arxiv.org/abs/2602.17315)
*Sourav Chakraborty,Amit Kiran Rege,Claire Monteleoni,Lijun Chen*
...
### [126] [The Sound of Death: Deep Learning Reveals Vascular Damage from Carotid Ultrasound](https://arxiv.org/abs/2602.17321)
...
### [127] [SubQuad: Near-Quadratic-Free Structure Inference with Distribution-Balanced Objectives in Adaptive Receptor framework](https://arxiv.org/abs/2602.17330)
...
### [128] [From Subtle to Significant: Prompt-Driven Self-Improving Optimization in Test-Time Graph OOD Detection](https://arxiv.org/abs/2602.17342)
...
### [129] [Shortcut learning in geometric knot classification](https://arxiv.org/abs/2602.17350)
...
### [130] [2Mamba2Furious: Linear in Complexity, Competitive in Accuracy](https://arxiv.org/abs/2602.17363)
...
### [131] [A feature-stable and explainable machine learning framework for trustworthy decision-making under incomplete clinical data](https://arxiv.org/abs/2602.17364)
...
### [132] [Convergence Analysis of Two-Layer Neural Networks under Gaussian Input Masking](https://arxiv.org/abs/2602.17423)
...
### [133] [Variational Grey-Box Dynamics Matching](https://arxiv.org/abs/2602.17477)
...
### [134] [Linear Convergence in Games with Delayed Feedback via Extra Prediction](https://arxiv.org/abs/2602.17486)
...
### [135] [Retrospective In-Context Learning for Temporal Credit Assignment with Large Language Models](https://arxiv.org/abs/2602.17497)
...
### [136] [LORA-CRAFT: Cross-layer Rank Adaptation via Frozen Tucker Decomposition of Pre-trained Attention Weights](https://arxiv.org/abs/2602.17510)
...
### [137] [Variational inference via radial transport](https://arxiv.org/abs/2602.17525)
...
### [138] [The Anxiety of Influence: Bloom Filters in Transformer Attention Heads](https://arxiv.org/abs/2602.17526)
*Peter Balogh*
...
### [139] [Provably Explaining Neural Additive Models](https://arxiv.org/abs/2602.17530)
...
### [140] [Position: Evaluation of ECG Representations Must Be Fixed](https://arxiv.org/abs/2602.17531)
...
### [141] [MASPO: Unifying Gradient Utilization, Probability Mass, and Signal Reliability for Robust and Sample-Efficient LLM Reasoning](https://arxiv.org/abs/2602.17550)
...
### [142] [A Theoretical Framework for Modular Learning of Robust Generative Models](https://arxiv.org/abs/2602.17554)
...
### [143] [Revisiting Weight Regularization for Low-Rank Continual Learning](https://arxiv.org/abs/2602.17559)
...
### [144] [Be Wary of Your Time Series Preprocessing](https://arxiv.org/abs/2602.17568)
...
### [145] [Canonicalizing Multimodal Contrastive Representation Learning](https://arxiv.org/abs/2602.17584)
...
### [146] [Asymptotic Smoothing of the Lipschitz Loss Landscape in Overparameterized One-Hidden-Layer ReLU Networks](https://arxiv.org/abs/2602.17596)
*Saveliy Baturin*
...
### [148] [Guarding the Middle: Protecting Intermediate Representations in Federated Split Learning](https://arxiv.org/abs/2602.17614)
...
### [149] [Stable Asynchrony: Variance-Controlled Off-Policy RL for LLMs](https://arxiv.org/abs/2602.17616)
...
### [150] [Catastrophic Forgetting Resilient One-Shot Incremental Federated Learning](https://arxiv.org/abs/2602.17625)
...
### [155] [Pushing the Frontier of Black-Box LVLM Attacks via Fine-Grained Detail Targeting](https://arxiv.org/abs/2602.17645)
...
### [156] [Multi-Round Human-AI Collaboration with User-Specified Requirements](https://arxiv.org/abs/2602.17646)
...
### [157] [MARS: Margin-Aware Reward-Modeling with Self-Refinement](https://arxiv.org/abs/2602.17658)
