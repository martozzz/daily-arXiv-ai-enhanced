...
### [1] [Regularized Centered Emphatic Temporal Difference Learning](https://arxiv.org/abs/2605.04100)
*Xingguo Chen,Chaohui Wu,Jinguo Ye,Chao Li,Shangdong Yang,Guang Yang,Tianyu Liang,Wenhao Wang*

Main category: cs.AI

TL;DR: 提出正则化强调时间差分学习（RETD），通过只对辅助中心化递归进行正则化解快ETD中高方差和稳定性问题，在保留强调投影几何优势的同时确保收敛。


<details>
  <summary>Details</summary>
Motivation: 离策略时间差分学习在稳定性、投影几何和方差控制之间存在结构性的权衡。强调时间差分（ETD）通过后续强调改善了离策略投影几何，但后续跟踪可能具有高方差。贝尔曼误差中心化虽然自然地消除了时间差分误差中的常见漂移项，但简单的中心化强调扩展会引入可能破坏ETD关键矩阵正定性的辅助耦合。

Method: 方法基于贝尔曼误差中心化的思想，通过只对辅助中心化递归进行正则化，而不改变原有的强调投影几何。具体来说，将耦合关键矩阵中的右下角块从1提升到1+c，从而打破有害的耦合效应。

Result: 实验结果表明，RETD避免了朴素中心化强调学习的不稳定性，保留了有利的强调几何，并且在正则化参数c上展现出了鲁棒的中间区间。该方法在线性离策略预测任务上表现出良好的收敛性和性能。

Conclusion: 本文提出了一种名为正则化强调时间差分学习（RETD）的新方法，通过在辅助中心化递归中引入正则化来稳定强调时间差分学习，同时保留其有利的投影几何性质。理论证明和实验验证表明该方法在离策略预测任务中具有良好的稳定性和性能。
...
### [2] [ANDRE: An Attention-based Neuro-symbolic Differentiable Rule Extractor](https://arxiv.org/abs/2605.04193)
*Iman Sharifi,Peng Wei,Saber Fallah*

Main category: cs.AI

TL;DR: ANDRE：基于注意力的神经符号可微分规则提取器，通过注意力逻辑运算符在连续规则空间学习一阶逻辑程序，在噪声概率设置中稳定提取可解释规则。


<details>
  <summary>Details</summary>
Motivation: 现有符号和神经符号ILP方法难以扩展到噪声和概率设置中，传统ILP依赖离散组合搜索对不确定性敏感，可微分ILP方法通常需要预定义规则模板或使用不准确的模糊算子。

Method: 使用注意力驱动的逻辑运算符替代预定义规则模板，通过连续规则空间优化学习一阶逻辑程序，实现灵活可微的规则归纳。

Result: 在经典ILP基准、大规模知识库和合成数据集上的实验表明，ANDRE在预测性能上与现有方法相当或更优，同时在不确定性条件下能可靠恢复正确符号规则，对中等标签噪声保持稳健。

Conclusion: ANDRE是第一个完全基于注意力机制可微分神经符号ILP框架，能有效处理概率数据和噪声监督，在保持符号结构可解释性的同时，显著提升了稳健性和规则提取质量。
...
### [3] [Pro$^2$Assist: Continuous Step-Aware Proactive Assistance with Multimodal Egocentric Perception for Long-Horizon Procedural Tasks](https://arxiv.org/abs/2605.04227)
*Lilin Xu,Bufang Yang,Siyang Jiang,Kaiwei Liu,Kaiyuan Hou,Yuang Fan,Hongkai Chen,Zhenyu Yan,Xiaofan Jiang*

Main category: cs.AI

TL;DR: 提出Pro^2Assist：基于AR眼镜和持续推理的步骤感知主动助理系统，可显著提升程序性任务辅助效果。


<details>
  <summary>Details</summary>
Motivation: 针对现有个人助理系统主要为响应式或针对短期事件提供有限主动辅助，缺乏对长期多步骤程序性任务的持续支持，本文提出了Pro^2Assist系统，旨在通过连续跟踪任务进度并进行持续推理，为程序性任务提供及时的主动辅助。

Method: Pro^2Assist系统结合AR眼镜获取多模态数据，通过基于运动的感知技术提取步骤导向的程序上下文，并从多尺度时间动态和任务特定专家知识中获取信息，最后基于感官输入和程序上下文进行持续推理以推断用户需求，并在AR眼镜上显示及时辅助。

Result: 实验结果表明，Pro^2Assist在程序动作理解准确率上优于最佳基准方法超过21%，主动时机准确率达到基准方法的2.29倍；用户研究中90%的参与者认为该系统有用，证实了其在现实世界程序性辅助中的有效性。

Conclusion: 本文通过提出Pro^2Assist系统，验证了主动智能助理在长期多步任务中的实用价值，证明了融合多模态数据与持续推理的方法论有效性，为实现高效的人机协同作业提供了可行的技术路径。
...
### [4] [Parallel Prefix Verification for Speculative Generation](https://arxiv.org/abs/2605.04263)
*Yuncheng Yao,Yuxuan Xia,Shengjie Wang,Danyang Zhuo*

Main category: cs.AI

TL;DR: PARSE是一个通过并行化语义层级的前缀验证来加速LLM推理的推测生成框架，在单次前向传播中直接识别最长有效前缀，避免了顺序验证的开销，实现了显著的吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: 现有的推测解码方法在本质上限缩于token层级的等价验证：目标模型必须逐一验证每个token，这导致了接受长度较短且加速效果有限。转而在语义层级（或分段层级）上进行验证有望显著提高接受粒度，但现有方法依赖于顺序验证过程，引入了显著的开销并限制了实际的性能增益。因此，需要一种能够实现语义层级验证而又不引入顺序检查开销的新方法。

Method: PARSE框架的核心创新点在于并行前缀验证机制：给定一个由草稿模型生成的完整草稿，目标模型通过自定义注意力掩码在单次前向传播中同时评估多个前缀的正确性，从而直接识别出最大有效前缀。该方法避免了顺序段验证的步骤，同时确保了验证过程对计算的高效利用。PARSE与现有的基于token层级等价验证的推测解码方法正交，可以与之组合使用。

Result: 在多种模型和基准测试集上的实验表明，PARSE相较于目标模型基线，能实现1.25倍到4.3倍的吞吐量（throughput）提升；当与EAGLE-3等token级推测解码方法组合时，能实现1.6倍到4.5倍的吞吐量提升，并且所有这些性能增益都伴随着微乎其微的精度损失。

Conclusion: PARSE通过并行前缀验证机制实现了对大型语言模型推理的显著加速，验证了在语义层级上并行化验证是LLM推理加速的有效通用方法，且可与现有方法正交组合获得进一步的性能提升。
...
### [5] [Agent Island: A Saturation- and Contamination-Resistant Benchmark from Multiagent Games](https://arxiv.org/abs/2605.04312)
*Connacher Murphy*

Main category: cs.AI

TL;DR: 为解决静态基准的局限性，研究者开发了Agent Island这一动态AI竞赛平台，通过竞争性游戏对49个模型进行评估，并发现模型存在同提供商偏好特质的排名结果。


<details>
  <summary>Details</summary>
Motivation: 传统的静态能力基准容易过饱和和污染，无法准确追踪模型能力的随时间进展。该研究旨在创建一个能够持续评估新模型、避免固定任务集局限性的动态基准系统。

Method: 通过构建多人仿真环境Agent Island，让语言模型智能体在合作、冲突和说服游戏中竞争，采用贝叶斯Plackett-Luce模型进行技能排名以量化不确定性，并通过游戏日志分析模型行为模式。

Result: OpenAI的GPT-5.5模型在999场游戏竞赛中以5.64的评分显著优于其他49个模型（第二名GPT-5.2得分为3.10）。分析发现模型更倾向于支持同提供商竞争对手，其中OpenAI模型表现出最强的相同提供商偏好，而Anthropic模型最弱。

Conclusion: 静态基准评估在追踪模型能力进展时存在瓶颈，而Agent Island这一动态游戏环境能够有效避免此问题，为模型行为和偏好分析提供了丰富数据源，揭示了不同提供商模型间的异质性偏好模式。
...
### [6] [The Scaling Properties of Implicit Deductive Reasoning in Transformers](https://arxiv.org/abs/2605.04330)
*Enrico Vompa,Tanel Tammet*

Main category: cs.AI

TL;DR: 深度有限Transformer中，适当的架构和训练使隐式推理接近CoT性能，但CoT对深度外推仍必要。


<details>
  <summary>Details</summary>
Motivation: 探索深度有限Transformer在隐式演绎推理（基于Horn子句）中的可扩展性，特别是如何通过适当的模型架构和训练策略使隐式推理达到接近显式思维链（CoT）推理的性能水平。

Method: 本研究通过系统性去相关证明性（provability）与虚假特征（spurious features），并实施算法对齐（algorithmic alignment），研究深度有限的Transformer模型在Horn子句隐式演绎推理中的缩放特性。实验使用了双向前缀掩码（bidirectional prefix mask），考察了不同图拓扑结构和问题宽度下的表现。

Result: 研究发现在足够深的模型中使用双向前缀掩码时，隐式推理能够在不同图拓扑结构和问题宽度下接近显式CoT的性能表现，但CoT对深度外推（depth extrapolation）任务仍然是必要的。

Conclusion: 研究表明，在足够深的Transformer模型中，通过适当的深度限制、双向前缀掩码和算法对齐，隐式推理可以达到与显式思维链（CoT）相近的性能，但对于深度外推任务，显式CoT仍然是必要的。
...
### [7] [When Context Hurts: The Crossover Effect of Knowledge Transfer on Multi-Agent Design Exploration](https://arxiv.org/abs/2605.04361)
*Saranyan Vigraham*

Main category: cs.AI

TL;DR: 研究表明在智能体协同中'上下文越多越好'的假设不成立——知识工件注入存在交叉效应，可能改善也可能阻碍任务表现，取决于基线探索能力。


<details>
  <summary>Details</summary>
Motivation: 检验智能体协同中'上下文越多越好'的普遍假设，探索知识工件注入对多智能体软件设计任务效果的影响模式。

Method: 通过在10个任务、7种上下文注入条件和超过2700次实验中测试多智能体软件设计，使用可测量的变量（基线探索）预测效果方向，并通过提示设计操作收敛压力来探究机制。

Result: 发现交叉效应：相同工件类型在某些任务中改善设计探索（高达20倍权衡覆盖率），在其他任务中却会降低效果（高达46%减少）。无关文档的效果与相关工件相当甚至更好。效果方向可由单个可测量变量（无上下文基线探索）预测（Pearson r = -0.82，p < 0.001）。机制分析揭示训练数据先验驱动的收敛对工件干扰敏感，而显式指令驱动的收敛则不敏感。

Conclusion: 上下文注入在智能体协同中应是有条件的而非通用的策略，建议通过无上下文试运行作为低成本诊断工具来预测特定任务中知识工件是否会有益。
...
### [8] [Deployment-Relevant Alignment Cannot Be Inferred from Model-Level Evaluation Alone](https://arxiv.org/abs/2605.04454)
*Varad Vishwarupe,Nigel Shadbolt,Marina Jirotka,Ivan Flechais*

Main category: cs.AI

TL;DR: 对齐评估不能仅看模型分数，需系统级评估。审计发现基准缺乏用户验证支持，测试显示脚手架效果依赖具体模型，呼吁开发对齐概况和固定协议。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习对齐评估过度依赖模型级基准，仅评估固定输入下的输出质量，无法有效预测实际部署中的对齐表现，存在评估与部署之间的推断鸿沟。

Method: 采用双研究法：1) 结构化审计11个对齐基准（扩展至16个），使用8维评分标准双编码；2) 盲法交叉模型压力测试，使用3个前沿模型和4个脚手架的180个转录进行验证。

Result: 审计发现：所有基准均缺乏用户面验证支持，流程可控性几乎缺失；少数交互式基准覆盖零散；压力测试显示：相同验证脚手架在不同模型上的效果差异巨大，模型间不可传递。

Conclusion: 本文得出结论，仅依赖模型级评估无法推断部署相关对齐，必须发展系统级评估框架，包括对齐概况、固定脚手架协议和报表模板，以明确评估证据与部署声明之间的推断距离。
...
### [9] [How Does Thinking Mode Change LLM Moral Judgments? A Controlled Instant-vs-Thinking Comparison Across Five Frontier Models](https://arxiv.org/abs/2605.04488)
*Sai Sourabh Madur*

Main category: cs.AI

TL;DR: 推理模式不会显著影响LLM的整体道德判断一致性，但在存在争议的道德场景中能减少跨模型分歧，并降低部分模型的人口统计判断不一致性。


<details>
  <summary>Details</summary>
Motivation: 探究开启提供者暴露的推理模式是否会改变同一模型检查点内的道德判断，特别是评估推理模式对道德判断一致性、跨模型分歧以及人口统计相关的不一致性的影响。

Method: 使用100个道德判断场景，对五个前沿推理训练的LLM（Claude Sonnet 4.6、GPT 5.5、Gemini 3 Flash、DeepSeek V3.1和Qwen3.5 397B）进行测试。在即时模式和推理模式两种设置下评估模型的道德判断。采用科里潘道夫α系数衡量一致性，并分析了21个模型争议场景中的分歧变化。

Result: 1. 整体二元判决的一致性在即时模式和推理模式间无统计显著差异（α=0.78 vs 0.79）。2. 分歧集中在21个模型争议场景中，即时模式一致性接近随机水平（α=0.08）。在这些场景中，推理方向性地缩小了跨模型分歧，平均成对一致性从5.4/10提升至6.7/10。3. 推理减少了五个模型中三个的人口统计判断不一致性，且未增加任何模型的不一致性。4. 推理改变模型自标记伦理框架的频率高于改变二元判决的频率。

Conclusion: 研究表明，尽管推理模式不会显著改变模型间的整体道德判断一致性，但在特定争议场景中，推理能够减少分歧并提升判断一致性，同时还能降低人口统计相关的不一致性，并对模型的伦理框架标签产生比二元判决更显著的影响。
...
### [10] [From Parameter Dynamics to Risk Scoring : Quantifying Sample-Level Safety Degradation in LLM Fine-tuning](https://arxiv.org/abs/2605.04572)
*Xiao Wang,Yifei Zhang,YongKang Liu,Xiaocui Yang,Zihan Wang,Shi Feng,Daling Wang*

Main category: cs.AI

TL;DR: 研究发现良性微调导致参数向危险方向累积漂移，提出了SQSD方法量化样本对安全退化的影响，实验证明该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的安全性对齐极为脆弱，少数良性样本的微调就可能破坏从数百万偏好样本中学到的安全行为。现有研究通过比较微调前后的参数和隐藏状态来解释这一现象，但忽视了它们在微调过程中的动态演化。

Method: 提出了样本级安全退化量化(SQSD)方法，通过计算训练样本诱导的参数更新在危险方向与安全方向上的投影差异，生成连续的风险评分来量化每个样本对安全性退化的影响。

Result: 在多个模型和数据集上的大量实验表明，SQSD能有效量化样本级微调风险，并在不同模型架构、参数规模和参数高效方法之间展现出强大的可迁移性。

Conclusion: 安全性对齐在大语言模型训练中极为脆弱，少量良性样本的微调就可能导致安全行为退化。本研究通过分析参数动态揭示了安全性退化的关键机制，并提出了SQSD方法量化样本级微调风险，为安全微调实践提供了有效的风险识别工具。
...
### [11] [SensingAgents: A Multi-Agent Collaborative Framework for Robust IMU Activity Recognition](https://arxiv.org/abs/2605.04608)
*Naiyu Zheng,Tianlong Yu,Haochen Yin,Xiaoyi Fan,Xiping Hu,Zhimeng Yin*

Main category: cs.AI

TL;DR: 提出基于LLM的多智能体系统SensingAgents，通过专业角色分工和辩论机制解决IMU活动识别中的数据冲突问题，在零样本设置下显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 目前基于深度学习的HAR模型通常存在对标注数据的严重依赖、位置特定歧义性以及缺乏透明推理等问题。受先进智能体框架的启发，该研究旨在利用LLM构建协作智能体系统，解决IMU活动识别中的这些挑战。

Method: 作者提出了SensingAgents系统，将基于LLM的智能体组织成专门角色：负责位置特定传感器分析（手臂、手腕、腰带、口袋）的分析智能体组，通过动态和静态辩证法辩论解决传感器冲突的倡导智能体对，以及确保传感器漂移或故障时可靠性的决策智能体。

Result: 在Shoaib数据集上的评估表明，SensingAgents显著优于最先进的单智能体和多智能体LLM模型，在零样本设置下达到79.5%的准确率，比现有智能体模型提高29%，比深度学习基线提高9.4%，尤其在多传感器数据冲突或嘈杂的复杂场景中表现突出。

Conclusion: 该研究展示了多智能体协作推理在提升普适传感系统鲁棒性和可解释性方面的巨大潜力，为未来IMU活动识别系统设计提供了新思路。
...
### [12] [AuditRepairBench: A Paired-Execution Trace Corpus for Evaluator-Channel Ranking Instability in Agent Repair](https://arxiv.org/abs/2605.04624)
...
### [13] [Budget-aware Auto Optimizer Configurator](https://arxiv.org/abs/2605.04711)
...
### [14] [Reward-Decomposed Reinforcement Learning for Immersive Video Role-Playing](https://arxiv.org/abs/2605.04733)
...
### [15] [AgentTrust: Runtime Safety Evaluation and Interception for AI Agent Tool Use](https://arxiv.org/abs/2605.04785)
*Chenglin Yang*
...
### [16] [DecodingTrust-Agent Platform (DTap): A Controllable and Interactive Red-Teaming Platform for AI Agents](https://arxiv.org/abs/2605.04808)
...
### [17] [Strat-Reasoner: Reinforcing Strategic Reasoning of LLMs in Multi-Agent Games](https://arxiv.org/abs/2605.04906)
...
### [18] [A Foundation Model for Zero-Shot Logical Rule Induction](https://arxiv.org/abs/2605.04916)
*Yin Jun Phua*
...
### [19] [On-line Learning in Tree MDPs by Treating Policies as Bandit Arms](https://arxiv.org/abs/2605.04979)
...
### [20] [Uno-Orchestra: Parsimonious Agent Routing via Selective Delegation](https://arxiv.org/abs/2605.05007)
...
### [21] [Position: Embodied AI Requires a Privacy-Utility Trade-off](https://arxiv.org/abs/2605.05017)
...
### [22] [Executable World Models for ARC-AGI-3 in the Era of Coding Agents](https://arxiv.org/abs/2605.05138)
*Sergey Rodionov*
...
### [23] [LongSeeker: Elastic Context Orchestration for Long-Horizon Search Agents](https://arxiv.org/abs/2605.05191)
...
### [24] [Endogenous Regime Switching Driven by Scalar-Irreducible Learning Dynamics](https://arxiv.org/abs/2605.04054)
*Sheng Ran*

Main category: cs.LG

TL;DR: 论文区分了标量可约与不可约动力学，发现标量不可约动力学通过快速变量与慢速适应的反馈自然产生内生机制转换，为自主学习系统提供了新的动力学范式。


<details>
  <summary>Details</summary>
Motivation: 实现内生机制转换对于自主智能的出现至关重要，但现有机器学习框架通常需要外部强制设定转换机制，缺乏内部的自主转换能力，这是当前机器学习面临的核心挑战之一。

Method: 通过引入标量可约与不可约动力学的分类框架，构建了最小动力学模型来展示快速变量和慢速结构适应之间的反馈如何产生持续的内生机制转换，并利用该模型进行了理论分析和模拟验证。

Result: 研究表明标量不可约动力学能够通过快速变量和慢速适应的反馈产生持续的内生机制转换，为自主系统提供了无需外部调度即可探索不同机制的能力，这一机制在最小动力学模型中得到验证。

Conclusion: 标量不可约动力学通过在快速动力学变量和慢速结构适应之间建立反馈机制，能够自然地实现内生的机制转换，为自组织学习和开发自主智能系统提供了新的动力学范式。
...
### [25] [A Self-Attentive Meta-Optimizer with Group-Adaptive Learning Rates and Weight Decay](https://arxiv.org/abs/2605.04055)
*JiangBo Zhao,ZhaoXin Liu*

Main category: cs.LG

TL;DR: 提出MetaAdamW优化器，通过自注意力机制动态调制各参数组的学习率和权重衰减，在多种任务中相比AdamW实现了更高的效率或更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统自适应优化器（如AdamW）在所有参数组上应用统一的超参数，忽略了不同层级和模块间的异质优化动态。为了解决这一局限性，需要设计能够根据参数组特征动态调整超参数的新型优化器。

Method: 开发了集成自注意力机制的MetaAdamW优化器，使用轻量级Transformer编码器基于梯度范数、动量范数、相关性等统计特征动态调制各参数组的学习率和权重衰减。引入结合梯度对齐、损失下降和泛化差距的元学习目标训练注意力模块，并扩展了具有任务特异性优先级的同方差不确定性加权策略。

Result: 在五个不同任务（时间序列预测ETT、语言建模WikiText-2、机器翻译Multi30k、图像分类CIFAR-10、情感分析IMDB）上的广泛实验表明，MetaAdamW在验证损失、准确率或困惑度方面持续优于标准AdamW基线。根据任务不同，要么减少了高达17.11%的总训练时间，要么提高了高达11.08%的性能，同时仅引入适度开销。在一些情况下，还能缓解因过早早停导致的收敛不足问题。消融研究验证了各组件（包括特征版本、分组策略和提出的优先级注入不确定性加权）的有效性。

Conclusion: MetaAdamW通过集成自注意力机制动态调制不同参数组的学习率和权重衰减，克服了传统AdamW均匀参数设置的局限性，在各任务中均表现出性能或效率优势。该研究为自适应优化器设计提供了新思路，并验证了基于任务的优先级加权策略的有效性。
...
### [26] [Transformation Categorization Based on Group Decomposition Theory Using Parameter Division](https://arxiv.org/abs/2605.04056)
*Takayuki Komatsu,Yoshiyuki Ohmura,Yasuo Kuniyoshi*

Main category: cs.LG

TL;DR: 提出一种基于参数分解的无监督变换分类方法，通过同态约束识别正规子群，无需辅助假设，适用于更广泛场景，实验验证了群分解理论的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有无监督表示学习方法缺乏对‘好表示’的理论解释；经典解耦方法在因子耦合时失效；之前的伽罗瓦理论方法虽然能处理交换和非交换情形，但依赖于运动、等距等辅助假设，且无法区分理论效应与辅助效应。需要一种更纯净、更通用的群分解方法。

Method: 提出参数分解方法：将单个变换的参数拆分为多个分量，对分量施加同态约束，将全变换映射到其中一个分量，并通过固定该分量为单位元来识别正规子群。该方法摒弃了先前方法所需的辅助假设，适用范围更广。

Result: 在涉及旋转、平移和缩放的图像对上评估，消融实验表明群分解约束驱动了适当的分类，验证了该方法的有效性。

Conclusion: 通过参数分解方法，在无需额外辅助假设的情况下，成功基于群分解理论实现了对变换类别的无监督分类，证实了代数约束是驱动适当分类的关键因素。
...
### [27] [Structured Progressive Knowledge Activation for LLM-Driven Neural Architecture Search](https://arxiv.org/abs/2605.04057)
*Zhen Liu,Yuhan Liu,Jingwen Fu*

Main category: cs.LG

TL;DR: 本文提出了SPARK方法，通过结构化的功能因子条件编辑解决NAS中的功能纠缠问题，显著提升了架构搜索的效率和效果。


<details>
  <summary>Details</summary>
Motivation: 神经架构搜索（NAS）面临的关键挑战是：在昂贵的评估成本下，如何整合已有的架构知识并探索新设计。大语言模型（LLMs）作为NAS助手很有前景，因其能将丰富的架构和编码先验转化为可执行的代码编辑。但实践中，看似局部的修改经常会导致非局部的行为和性能变化，因为单个编辑可能无意中耦合多个交互的功能因子，这种现象称为功能纠缠。

Method: 提出结构化渐进知识激活（SPARK）方法，该方法通过明确选择要修改的功能因子，并在该因子条件下进行编辑，从而激活相关先验知识。这种因子条件编辑减少了纠缠的副作用，产生更有针对性和可靠的架构修改。

Result: 在CLRS-DFS数据集上，SPARK实现了28.1倍的样本效率提升，以及22.9%的相对OOD准确率改进。

Conclusion: 本研究表明，通过结构化渐进知识激活（SPARK）方法，可以有效解决神经架构搜索（NAS）中的功能纠缠问题，使大语言模型的知识能够更可靠地应用于架构修改，从而实现更高效率、更少负面影响的架构进化。
...
### [28] [Continual Distillation of Teachers from Different Domains](https://arxiv.org/abs/2605.04059)
*Nicolas Michel,Maorong Wang,Jiangpeng He,Toshihiko Yamasaki*

Main category: cs.LG

TL;DR: 提出持续知识蒸馏新范式及SE2D方法，利用外部数据保存历史教师信息，解决异构教师序列学习中的知识遗忘问题，提升跨领域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型的规模不断扩大，某些模型的存储需求已超过大型数据集的大小。传统持续学习需要存储历史数据，但在实际部署中训练数据通常无法保留。本研究提出持续知识蒸馏（CD）新范式，让学生模型从教师模型流中连续学习而无需访问早期教师。主要挑战是：1）教师训练数据不可用；2）教师具有不同的专业知识领域。

Method: 提出自外部数据蒸馏（SE2D）方法，利用外部未标记数据保存历史教师模型的预测信息（logits），通过约束当前学生模型对这些外部数据的预测一致性来稳定学习过程，避免在序列化学习过程中丢失已获得的未见过知识。

Result: 在多个基准测试上的实验表明，提出的SE2D方法能有效减少未见过知识遗忘（UKF），提升跨领域泛化能力，实现了未见过知识转移（UKT）与遗忘之间的更好权衡。

Conclusion: 本研究提出并验证了自外部数据蒸馏（SE2D）方法的有效性，该方法通过保存外部数据的logits来缓解异构教师间的学习冲突，在持续知识蒸馏框架下实现了更好的未见过知识保留、遗忘减少和跨领域泛化性能提升。
...
### [29] [Lookahead Drifting Model](https://arxiv.org/abs/2605.04060)
*Guoqiang Zhang,Kenta Niwa,W. Bastiaan Kleijn*

Main category: cs.LG

TL;DR: 提出前瞻漂移模型，通过依次计算一组漂移项来捕获更高阶梯度信息，提升图像生成性能。


<details>
  <summary>Details</summary>
Motivation: 漂移模型通过在每次训练迭代中计算漂移项并将模型输出推向该方向，实现了在ImageNet上的SOTA图像生成性能（通过一步神经函数评估）。然而，传统漂移模型只计算一个漂移项，未能充分利用梯度信息。作者提出前瞻漂移模型旨在捕获更高阶的梯度信息，从而提升性能。

Method: 在每个训练迭代中，依次计算一组漂移项。每个漂移项利用先前计算的漂移项、正向样本和模型输出进行计算。关键步骤是适当缩放漂移项的大小，使它们处于可比范围内。原则上，后期获得的漂移项捕获了更高阶的梯度信息。在每个训练迭代中，模型通过将其输出推向漂移项加权和的方向来进行优化。

Result: 在玩具示例和CIFAR10数据集上的实验结果表明，新方法比基准方法表现更好，验证了前瞻漂移模型的有效性。

Conclusion: 展望未来工作时，作者提出可以将前瞻漂移模型扩展到更复杂的场景，如大规模图像生成任务，并研究如何更高效地计算高阶漂移项。
...
### [30] [EdgeRazor: A Lightweight Framework for Large Language Models via Mixed-Precision Quantization-Aware Distillation](https://arxiv.org/abs/2605.04062)
*Shu-Hao Zhang,Le-Tong Huang,Xiang-Sheng Deng,Xin-Yi Zou,Chen Wu,Nan Li,Shao-Qun Zhang*

Main category: cs.LG

TL;DR: EdgeRazor是一种轻量级混合精度量化框架，通过自适应特征蒸馏和熵感知KL散度等技术，在极低比特（如1.88-bit）下实现了优于现有方法的性能表现，同时大幅降低存储需求和加速推理。


<details>
  <summary>Details</summary>
Motivation: 现有的权重量化方法存在明显局限：后训练量化（PTQ）在低于4-bit时性能显著下降；量化感知训练（QAT）虽能搜索低比特参数但需要大量计算资源；量化感知蒸馏结合QAT与知识迁移，但依赖人工选择蒸馏特征且对教师特定数据敏感。因此需要一种既能在极低比特下保持性能又资源高效的量化方案。

Method: 该框架包含三个核心模块：混合精度量化感知蒸馏（用于细粒度精度控制）、自适应特征蒸馏（从16-bit教师模型推导出n-bit学生模型）、以及熵感知KL散度（在人工标注和蒸馏数据集上应用，其前向-反向平衡仅由教师模型的输出分布决定）。

Result: EdgeRazor在1.88-bit量化下超越所有3-bit竞争者，尤其领先主流2-bit PTQ方法11.3个百分点，且训练预算比领先的QAT方法低4-10倍。在1.58-bit量化下，Qwen3-0.6B模型的存储从1.41 GB降至0.28 GB，解码速度相对16-bit基线加速15.1倍。该框架在所有比特宽度下均提供更高压缩比。

Conclusion: EdgeRazor是一种高效的LLM量化框架，通过混合精度量化感知蒸馏、自适应特征蒸馏和熵感知KL散度等创新模块，在极低比特量化（如1.88-bit）下实现了优于现有3-bit方法的性能，且训练成本显著降低，同时提供了更高的存储压缩比和推理加速效果。
...
### [31] [Investigating Trustworthiness of Nonparametric Deep Survival Models for Alzheimer's Disease Progression Analysis](https://arxiv.org/abs/2605.04063)
*Jacob Thrasher,Kaitlyn Heintzelman,Peter Martone,David Kotlowski,Binod Bhattarai,Donald Adjeroh,Prashnna Gyawali*

Main category: cs.LG

TL;DR: 本研究深入分析了阿尔茨海默病进展建模中的公平性问题，提出了新的公平性评估指标，发现深度学习生存模型存在明显的预测偏见。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病（AD）是一种进行性神经退行性疾病，需要可靠的进展建模以进行有效患者护理。当前关于AD的深度学习生存分析研究较少，且现有研究未考虑模型内部的系统性偏见，这可能导致对边缘化群体的不公平预测。

Method: 研究采用深度学习生存模型进行AD进展分析，同时提出了两种新颖的公平性度量指标：Time-Dependent Concordance Impurity和Kaplan-Meier Fairness，用于量化基于敏感属性的偏见。

Result: 研究证明，深度学习生存模型虽然是可靠的工具，但常常表现出显著的偏见，特别是在性别、种族和教育等敏感属性方面存在不公平性。

Conclusion: 该研究结论指出，尽管深度学习驱动的生存模型是可靠的临床辅助工具，但往往表现出显著的偏见，这是未来研究的重要方向。
...
### [32] [Improving Medical VQA through Trajectory-Aware Process Supervision](https://arxiv.org/abs/2605.04064)
*Halil Ibrahim Gulluk,Olivier Gevaert*

Main category: cs.LG

TL;DR: 本研究通过为六项医疗VQA基准生成推理轨迹数据集，并在两阶段训练框架中引入基于DTW距离的过程奖励，显著提升了模型的推理能力和各项评价指标。


<details>
  <summary>Details</summary>
Motivation: 医疗视觉问答（VQA）的可靠性在很大程度上依赖于模型的推理能力。然而，现有的数据集很少提供详细的推理解释，这限制了深度学习和监督学习在训练具有明确推理能力的模型方面的应用。本研究旨在通过生成带有推理轨迹的数据集，并引入过程监督机制，以提升模型的临床判断准确性和可靠性。

Method: 1. 使用COMCTS算法和开源视觉语言模型为六个医疗VQA基准生成推理轨迹数据，并由LLM担任验证评判。
2. 提出了两阶段训练框架：监督微调（SFT）后接采用组相对策略优化（GRPO）和新型过程奖励。
3. 引入了轨迹感知奖励：通过句子转换器嵌入推理步骤，并计算向量序列之间的动态时间规整（DTW）距离，以衡量生成过程与基过程之间的相似性。

Result: 实验在六个基准测试上进行，结果表明：结合DTW过程奖励与精确匹配奖励的训练方法，在所有指标上均优于仅使用监督微调的基线。具体而言，平均准确率从0.598提升至0.689，平均BERTScore从0.845提升至0.881，平均ROUGE-L从0.665提升至0.748。这些结果凸显了过程监督在训练具有推理能力的医疗视觉语言模型中的重要性。

Conclusion: 本研究表明，在训练用于医疗视觉问答（VQA）的视觉语言模型时，过程监督显著提升了推理能力。通过结合DTW距离的过程奖励与精确匹配奖励，GRPO训练框架在六个基准测试中取得了稳定的性能提升。同时，生成的数据集和开源代码为后续研究提供了有价值的资源。
...
### [33] [LAWS: Learning from Actual Workloads Symbolically -- A Self-Certifying Parametrized Cache Architecture for Neural Inference, Robotics, and Edge Deployment](https://arxiv.org/abs/2605.04069)
*Gregory Magarshak*

Main category: cs.LG

TL;DR: 提出LAWS自证明推理缓存框架，通过部署观测构建专家函数库，为LLM等系统提供具有形式化误差界的低成本推理方案，理论上优于传统MoE和KV缓存。


<details>
  <summary>Details</summary>
Motivation: 目标是解决大型语言模型等复杂系统在线推理的高计算成本问题，同时保证输出质量并提供形式化的性能保证，避免对训练数据的依赖。

Method: 提出了一种基于部署观测构建专家函数库的自证明推理缓存架构，使用概率语言字典树定义输入空间区域，每个专家函数具有形式化的误差界。

Result: 证明了自证明定理、单调命中率定理、专家库增长率O(2^H log N)、车队学习收敛定理等一系列理论结果，表明LAWS比固定的MoE或有限缓存更具表达能力。

Conclusion: LAWS是一个具有理论基础和实际应用价值的自证明推理缓存框架，能够显著降低在线推理成本并保证性能，特别是在LLM推理、机器人控制和多智能体边缘部署场景中展现出优越性。
...
### [34] [FlatASCEND: Autoregressive Clinical Sequence Generation with Continuous Time Prediction and Association-Based Pharmacological Testing](https://arxiv.org/abs/2605.04071)
*Chris Sainsbury,Feng Dong,Andreas Karwath*

Main category: cs.LG

TL;DR: FlatASCEND自回归临床模型能部分恢复药理学关联，但无法区分因果关系；患者条件生成能放大药理学效应；直接偏好优化会破坏药理关联；ICU数据适用性优于门诊数据。


<details>
  <summary>Details</summary>
Motivation: 自回归模型可预测临床事件，但针对基于患者条件、响应干预令牌的多步骤轨迹生成，以及模型输出是否保留已知药理学关联的研究还很少。本文旨在探索模型是否能从观察性数据中学习到药理学机制关联。

Method: 使用FlatASCEND模型（1450万参数的自回归临床序列模型），采用扁平复合token和零膨胀对数正态时间头。通过患者条件生成、提示重排消融实验和事件用户框架评估模型保持药理学关联的能力。

Result: 患者特定条件放大了药理学效应（类固醇对葡萄糖2.0-2.2倍，利尿剂对钾2.0-2.2倍），但混淆驱动的关联不变（胰岛素对葡萄糖0.9倍）。在MIMIC-IV数据集中，4/10恢复了正确的药理学方向，2个再现了治疗情景关联，4个错误，9/10显著。直接偏好优化会破坏所有正确关联。

Conclusion: 本文研究表明，自回归模型能部分恢复药理学关联，但由于存在残存混杂因素，无法区分因果与相关关系。直接偏好优化会破坏正确的药理关联，显示了奖励设计与评估指标匹配的重要性。模型在ICU短期数据上表现最佳，跨站点迁移需要进一步研究。
...
### [35] [Sparse Autoencoder Decomposition of Clinical Sequence Model Representations: Feature Complexity, Task Specialisation, and Mortality Prediction](https://arxiv.org/abs/2605.04072)
*Chris Sainsbury,Feng Dong,Andreas Karwath*

Main category: cs.LG

TL;DR: 研究表明稀疏自编码器可用于临床序列模型分析，其特征显示出从浅层令牌检测到深层跨类别抽象的层级递进。稀疏表示在特定预测任务中表现良好，但在临床相关评估中尚未超越密集表示；特征稳定性和可复现性有限（21%），更适合探索性分析而非稳定临床应用。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器已在大型语言模型和蛋白质语言模型中广泛应用，但尚未系统性地应用于电子健康记录基础模型。本研究旨在探索稀疏自编码器在临床序列模型中的可行性、有效性和局限性，特别是特征抽象进展、预测性能以及与密集表示的比较。

Method: 在FlatASCEND模型（1450万参数的自回归临床序列模型）的各残差流提取点（共10层）训练TopK稀疏自编码器。使用INSPECT（门诊）和MIMIC-IV（ICU）数据集进行训练和评估。采用全序列的简单线性探针方法进行预测任务评估，并进行特征分析和干预实验。

Result: 稀疏自编码器特征分析显示：浅层特征（层0）作为接近完美的令牌检测器（45.7%单例），深层特征（层6）跨越多临床类别，涵盖约30种令牌类型（0.5%单例）。在完全序列线性探针下，稀疏特征在离散事件预测（死亡率）上优于密集表示，而密集表示在连续值预测（住院时间）上更优。但在临床相关的防泄漏窗口评估中，密集表示在所有测试场景中匹配或超越稀疏特征（eICU-CRD 48小时AUC：稀疏0.871 vs 密集0.880；MIMIC-IV：0.836 vs 0.914；INSPECT 1年/3年：0.697 vs 0.800）。Delta-mode干预方法将稀疏自编码器扰动噪声降低86倍，实现更清晰的特征级实验，但干预效应在4种情况下有3种大于随机对照，未达统计显著性。特征可复现性仅21%。

Conclusion: 稀疏自编码器在临床序列模型分析中展现了有前景的应用，但当前实现存在局限性。稀疏特征在特定预测任务中表现优异，但尚未超越密集表示；特征可解释性良好，但稳定性和可复现性有限（仅21%）。该方法可用于探索性分析，但临床应用需要更多改进。
...
### [36] [Confronting Label Indeterminacy in Automated Bail Decisions](https://arxiv.org/abs/2605.04073)
*Cor Steging,Tadeusz Zbiegień*

Main category: cs.LG

TL;DR: 研究了保释决策中的标签不确定性问题，评估了五种处理方法（含一种新方法），发现处理方式对模型行为的影响可能超过模型选择本身，并探讨其法律合法性。


<details>
  <summary>Details</summary>
Motivation: 保释决策数据存在结构性的标签不确定性，以往决策的对抗结果部分无法观察，直接在这种数据上建立自动化系统可能导致偏见的引入和反馈循环的强化，需要评估不同处理方法的影响。

Method: 开发了一种基于保释决策动态变化的新标签估算方法，并与现有四种处理方法进行比较评估。采用可解释AI分析探究不同处理方式对模型内部决策过程的影响。

Result: 不同标签不确定性处理方法对模型预测行为的影响有时甚至超过模型选择本身的影响。可解释性分析显示这些影响也延伸到模型内部决策过程。基于法律视角的评估揭示了不同方法在保释决策背景下的合法性问题。

Conclusion: 标签不确定性是保释决策自动化系统开发中的核心挑战，当前处理方法的合法性高度依赖于未经验证的假设，任何基于不确定性标签的系统都需要谨慎考虑道德和法律影响。
...
### [37] [RetentiveKV: State-Space Memory for Uncertainty-Aware Multimodal KV Cache Eviction](https://arxiv.org/abs/2605.04075)
...
### [38] [A Regulatory Governance Framework for AI-Driven Financial Fraud Detection in U.S. Banking: Integrating OCC, SR 11-7, CFPB, and FinCEN Compliance Requirements for Model Development, Validation, and Monitoring Lifecycles](https://arxiv.org/abs/2605.04076)
...
### [39] [Balanced Aggregation: Understanding and Fixing Aggregation Bias in GRPO](https://arxiv.org/abs/2605.04077)
...
### [41] [Efficient Handwriting-Based Alzheimer,s Disease Diagnosis Using a Low-Rank Mixture of Experts Deep Learning Framework](https://arxiv.org/abs/2605.04079)
...
### [42] [Time series causal discovery with variable lags](https://arxiv.org/abs/2605.04081)
...
### [47] [Learning reveals invisible structure in low-rank RNNs](https://arxiv.org/abs/2605.04115)
*Yoav Ger,Omri Barak*
...
### [48] [Simultaneous CNN Approximation on Manifolds with Applications to Boundary Value Problems](https://arxiv.org/abs/2605.04126)
*Hanfei Zhou,Lei Shi*
...
### [49] [Position: the Stochastic Parrot in the Coal Mine. Model Collapse is a Threat to Low-Resource Communities](https://arxiv.org/abs/2605.04127)
...
### [50] [Constrained Extreme Gradient Boosting for Adapting Reduced-Order Models](https://arxiv.org/abs/2605.04130)
...
### [51] [A large language model-type architecture for high-dimensional molecular potential energy surfaces](https://arxiv.org/abs/2412.03831)
...
### [52] [Model synthesis and identifiability analysis of stiff chemical reaction systems with inVAErt networks](https://arxiv.org/abs/2605.04134)
...
### [53] [Learning Reconstructive Embeddings in Reproducing Kernel Hilbert Spaces via the Representer Theorem](https://arxiv.org/abs/2601.05811)
...
### [54] [Enabling Real-Time Training of a Wildfire-to-Smoke Map with Multilinear Operators](https://arxiv.org/abs/2605.04164)
...
### [55] [A Provably Convergent and Practical Algorithm for Gromov--Wasserstein Optimal Transport](https://arxiv.org/abs/2605.04175)
*Ling Liang,Lei Yang*
...
### [65] [Hierarchical Support Vector State Partitioning for Distilling Black Box Reinforcement Learning Policies](https://arxiv.org/abs/2605.04254)
...
### [66] [HUGO-CS: A Hybrid-Labeled, Uncertainty-Aware, General-Purpose, Observational Dataset for Cold Spray](https://arxiv.org/abs/2605.04257)
...
### [67] [Explaining and Preventing Alignment Collapse in Iterative RLHF](https://arxiv.org/abs/2605.04266)
...
### [69] [A Mean Curvature Approach to Boundary Detection: Geometric Insights for Unsupervised Learning](https://arxiv.org/abs/2605.04274)
...
### [70] [Gradient Flow Structure and Quantitative Dynamics of Multi-Head Self-Attention](https://arxiv.org/abs/2605.04279)
*Ayan Pendharkar*
...
### [71] [Hardware-Aware Neural Feature Extraction for Resource-Constrained Devices](https://arxiv.org/abs/2605.04282)
...
### [72] [Probabilistic Classification and Uncertainty Quantification of Sahara Desert Climate Using Feedforward Neural Networks](https://arxiv.org/abs/2605.04286)
...
### [74] [LLMs Uncertainty Quantification via Adaptive Conformal Semantic Entropy](https://arxiv.org/abs/2605.04295)
...
### [75] [Memory as a Markov Matrix: Sample Efficient Knowledge Expansion via Token-to-Dictionary Mapping](https://arxiv.org/abs/2605.04308)
...
### [76] [LUCAS-MEGA: A Large-Scale Multimodal Dataset for Representation Learning in Soil-Environment Systems](https://arxiv.org/abs/2605.04323)
...
### [77] [DeFed-GMM-DaDiL: A Decentralized Federated Framework for Domain Adaptation](https://arxiv.org/abs/2605.04324)
...
### [78] [On the Architectural Complexity of Neural Networks](https://arxiv.org/abs/2605.04325)
...
### [80] [Budgeted LoRA: Distillation as Structured Compute Allocation for Efficient Inference](https://arxiv.org/abs/2605.04341)
...
### [81] [Structural Equivalence and Learning Dynamics in Delayed MARL](https://arxiv.org/abs/2605.04345)
...
### [82] [Efficiently Aligning Language Models with Online Natural Language Feedback](https://arxiv.org/abs/2605.04356)
...
### [83] [Covariance-Aware Goodness for Scalable Forward-Forward Learning](https://arxiv.org/abs/2605.04346)
...
### [84] [Mitigating Label Shift in Tabular In-Context Learning via Test-Time Posterior Adjustment](https://arxiv.org/abs/2605.04363)
...
### [85] [Probing Structural Mathematical Reasoning in Language Models with Algebraic Trapdoors](https://arxiv.org/abs/2605.04352)
*Igor Rivin*
...
### [86] [Extending Differential Temporal Difference Methods for Episodic Problems](https://arxiv.org/abs/2605.04368)
...
### [87] [Critical Windows of Complexity Control: When Transformers Decide to Reason or Memorize](https://arxiv.org/abs/2605.04396)
*Sarwan Ali*
...
### [88] [Demystifying Manifold Constraints in LLM Pre-training](https://arxiv.org/abs/2605.04418)
...
### [89] [Online Nonstochastic Prediction: Logarithmic Regret via Predictive Online Least Squares](https://arxiv.org/abs/2605.04364)
...
### [90] [FLUID: Continuous-Time Hyperconnected Sparse Transformer for Sink-Free Learning](https://arxiv.org/abs/2605.04421)
...
### [91] [$p$-adic Manifold Learning and Benchmark Tasks from Impartial Games](https://arxiv.org/abs/2605.04374)
*Tomoki Mihara*
...
### [92] [Dream-MPC: Gradient-Based Model Predictive Control with Latent Imagination](https://arxiv.org/abs/2605.04568)
...
### [93] [HeterSEED: Semantics-Structure Decoupling for Heterogeneous Graph Learning under Heterophily](https://arxiv.org/abs/2605.04594)
...
### [94] [A Queueing-Theoretic Framework for Stability Analysis of LLM Inference with KV Cache Memory Constraints](https://arxiv.org/abs/2605.04595)
...
### [95] [Beyond Rigid Geometries: The Spline-Pullback Metric for Universal Diffeomorphic SPD Representation Learning](https://arxiv.org/abs/2605.04406)
...
### [97] [Counterfactual identifiability beyond global monotonicity: non-monotone triangular structural causal models](https://arxiv.org/abs/2605.04413)
...
### [98] [Exact Dual Geometry of SOC-ICNN Value Functions](https://arxiv.org/abs/2605.04722)
...
### [99] [Knowledge-Free Correlated Agreement for Incentivizing Federated Learning](https://arxiv.org/abs/2605.04747)
...
### [100] [Cognitive Twins: Investigating Personalized Thinking Model Building and Its Performance Enhancement with Human-in-the-Loop](https://arxiv.org/abs/2605.04761)
...
### [101] [Discovering Sparse Counterfactual Factors via Latent Adjustment for Survey-based Community Intervention](https://arxiv.org/abs/2605.04460)
...
### [102] [Quantile-Free Uncertainty Quantification in Graph Neural Networks](https://arxiv.org/abs/2605.04847)
...
### [104] [Delta-Based Neural Architecture Search: LLM Fine-Tuning via Code Diffs](https://arxiv.org/abs/2605.04903)
...
### [105] [Automated Formal Proofs of Combinatorial Identities via Wilf-Zeilberger Guidance and LLMs](https://arxiv.org/abs/2605.04472)
...
### [106] [When Does Gene Regulatory Network Inference Break? A Controlled Diagnostic Study of Causal and Correlational Methods on Single-Cell Data](https://arxiv.org/abs/2605.04930)
...
### [107] [Geometry-Aware Neural Optimizer for Shape Optimization and Inversion](https://arxiv.org/abs/2605.04474)
...
### [108] [EP-GRPO: Entropy-Progress Aligned Group Relative Policy Optimization with Implicit Process Guidance](https://arxiv.org/abs/2605.04960)
...
### [109] [Data-dependent Exploration for Online Reinforcement Learning from Human Feedback](https://arxiv.org/abs/2605.04477)
...
### [110] [Reliable Modeling of Distribution Shifts via Displacement-Reshaped Optimal Transport](https://arxiv.org/abs/2605.04965)
...
### [111] [Skill Neologisms: Towards Skill-based Continual Learning](https://arxiv.org/abs/2605.04970)
...
### [112] [Quadrature-TreeSHAP: Depth-Independent TreeSHAP and Shapley Interactions](https://arxiv.org/abs/2605.04497)
...
### [113] [Why Geometric Continuity Emerges in Deep Neural Networks: Residual Connections and Rotational Symmetry Breaking](https://arxiv.org/abs/2605.04971)
...
### [114] [Gradient Scaling Effects in Adaptive Spectral PINNs for Stiff Nonlinear ODEs](https://arxiv.org/abs/2605.04502)
...
### [115] [Federated Learning for Early Prediction of EV Charging Demand](https://arxiv.org/abs/2605.04993)
...
### [116] [FL-Sailer: Efficient and Privacy-Preserving Federated Learning for Scalable Single-Cell Epigenetic Data Analysis via Adaptive Sampling](https://arxiv.org/abs/2605.04519)
...
### [117] [Preference-Based Self-Distillation: Beyond KL Matching via Reward Regularization](https://arxiv.org/abs/2605.05040)
...
### [118] [YOTOnet: Zero-Shot Cross-Domain Fault Diagnosis via Domain-Conditioned Mixture of Experts](https://arxiv.org/abs/2605.04528)
...
### [119] [Adaptive Learning Strategies for AoA-Based Outdoor Localization: A Comprehensive Framework](https://arxiv.org/abs/2605.05055)
...
### [120] [From Video-to-PDE: Data-Driven Discovery of Nonlinear Dye Plume Dynamics](https://arxiv.org/abs/2605.04535)
...
### [121] [Continual Knowledge Updating in LLM Systems: Learning Through Multi-Timescale Memory Dynamics](https://arxiv.org/abs/2605.05097)
...
### [122] [Power Distribution Bridges Sampling, Self-Reward RL, and Self-Distillation](https://arxiv.org/abs/2605.04542)
...
### [123] [On the Wasserstein Gradient Flow Interpretation of Drifting Models](https://arxiv.org/abs/2605.05118)
...
### [124] [Event-Based Early Warning of Vineyard Disease Risk from Environmental Time Series](https://arxiv.org/abs/2605.04548)
...
### [125] [Adaptive Policy Selection and Fine-Tuning under Interaction Budgets for Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2605.05123)
...
### [126] [Joint Treatment Effect Estimation from Incomplete Healthcare Data: Temporal Causal Normalizing Flows with LLM-driven Evolutionary MNAR Imputation](https://arxiv.org/abs/2605.05125)
...
### [127] [Superposition Is Not Necessary: A Mechanistic Interpretability Analysis of Transformer Representations for Time Series Forecasting](https://arxiv.org/abs/2605.05151)
*Alper Yıldırım*
...
### [128] [FAAST: Forward-Only Associative Learning via Closed-Form Fast Weights for Test-Time Supervised Adaptation](https://arxiv.org/abs/2605.04651)
...
### [129] [Threshold-Guided Optimization for Visual Generative Models](https://arxiv.org/abs/2605.04653)
...
### [130] [Evidence-based anomaly detection in clinical domains](https://arxiv.org/abs/2605.04664)
...
### [131] [Feature importance analysis for patient management decisions](https://arxiv.org/abs/2605.04666)
...
### [132] [ITBoost: Information-Theoretic Trust for Robust Boosting](https://arxiv.org/abs/2605.04671)
...
### [133] [HEXST: Hexagonal Shifted-Window Transformer for Spatial Transcriptomics Gene Expression Prediction](https://arxiv.org/abs/2605.04682)
...
### [134] [Learning Time-Inhomogeneous Markov Dynamics in Financial Time Series via Neural Parameterization](https://arxiv.org/abs/2605.04690)
...
### [135] [Differentiable Chemistry in PINNs for Solving Parameterized and Stiff Reaction Systems](https://arxiv.org/abs/2605.04708)
...
### [136] [SPHERE: Mitigating the Loss of Spectral Plasticity in Mixture-of-Experts for Deep Reinforcement Learning](https://arxiv.org/abs/2605.04712)
...
### [137] [Ensuring Reliability in Programming Knowledge Tracing: A Re-evaluation of Attention-augmented Models and Experimental Protocols](https://arxiv.org/abs/2605.04727)
...
### [138] [Using Common Random Numbers for Simulation-based Planning with Rollouts](https://arxiv.org/abs/2605.04732)
...
### [139] [OSAQ: Outlier Self-Absorption for Accurate Low-bit LLM Quantization](https://arxiv.org/abs/2605.04738)
...
### [140] [MixINN: Accelerating Plant Breeding by Combining Mixed Models and Deep Learning for Interaction Prediction](https://arxiv.org/abs/2605.04744)
...
### [141] [AxMoE: Characterizing the Impact of Approximate Multipliers on Mixture-of-Experts DNN Architectures](https://arxiv.org/abs/2605.04754)
...
### [142] [Bilinear Mamba-Koopman Neural MPC for Varying Dynamics](https://arxiv.org/abs/2605.04793)
...
### [143] [Unsat Core Prediction through Polarity-Aware Representation Learning over Clause-Literal Hypergraphs](https://arxiv.org/abs/2605.04819)
...
### [144] [Improving FMQA via Initial Training Data Design Considering Marginal Bit Coverage in One-Hot Encoding](https://arxiv.org/abs/2605.04825)
...
### [145] [Trustworthy Federated Label Distribution Learning under Annotation Quality Disparity](https://arxiv.org/abs/2605.04827)
...
### [146] [Concurrence of Symmetry Breaking and Nonlocality Phase Transitions in Diffusion Models](https://arxiv.org/abs/2605.04830)
...
### [147] [Replay-Based Continual Learning for Physics-Informed Neural Operators](https://arxiv.org/abs/2605.04832)
...
### [148] [Bridging Input Feature Spaces Towards Graph Foundation Models](https://arxiv.org/abs/2605.04834)
...
### [149] [Hybrid Iterative Neural Low-Regularity Integrator for Nonlinear Dispersive Equations](https://arxiv.org/abs/2605.04853)
*Zhangyong Liang*
...
### [151] [Self-Attention as Transport: Limits of Symmetric Spectral Diagnostics](https://arxiv.org/abs/2605.04893)
...
### [152] [Regime-Conditioned Evaluation in Multi-Context Bayesian Optimization](https://arxiv.org/abs/2605.04895)
*Noel Thomas*
...
### [153] [A geometric relation of the error introduced by sampling a language model's output distribution to its internal state](https://arxiv.org/abs/2605.04899)
*Albert F. Modenbach*
...
### [154] [Cross-Model Consistency of Feature Importance in Electrospinning: Separating Robust from Model-Dependent Features](https://arxiv.org/abs/2605.04905)
...
### [155] [Breaking the Quality-Privacy Tradeoff in Tabular Data Generation via In-Context Learning](https://arxiv.org/abs/2605.04911)
...
### [156] [Koopman Identification of Nonlinear Systems via Reservoir Liftings](https://arxiv.org/abs/2605.04917)
...
### [157] [Reinforcement Learning for Compositional Generalization with Outcome-Level Optimization](https://arxiv.org/abs/2605.04920)
*Xiyan Fu,Wei Liu*
...
### [158] [Training-Time Batch Normalization Reshapes Local Partition Geometry in Piecewise-Affine Networks](https://arxiv.org/abs/2605.04946)
...
### [163] [Conceptors for Semantic Steering](https://arxiv.org/abs/2605.04980)
...
### [164] [Self-Induced Outcome Potential: Turn-Level Credit Assignment for Agents without Verifiers](https://arxiv.org/abs/2605.04984)
...
### [165] [Adaptivity Under Realizability Constraints: Comparing In-Context and Agentic Learning](https://arxiv.org/abs/2605.04995)
...
### [166] [DualTCN: A Physics-Constrained Temporal Convolutional Network for 2 Time-Domain Marine CSEM Inversion](https://arxiv.org/abs/2605.04997)
...
### [167] [Learned Neighbor Trust for Collaborative Deployment in Model-Agnostic Decentralized Learning](https://arxiv.org/abs/2605.05009)
...
### [168] [Graph-SND: Sparse Aggregation for Behavioral Diversity in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.05020)
*Shawn Ray*
...
### [169] [CuBridge: An LLM-Based Framework for Understanding and Reconstructing High-Performance Attention Kernels](https://arxiv.org/abs/2605.05023)
...
### [170] [The Predictive-Causal Gap: An Impossibility Theorem and Large-Scale Neural Evidence](https://arxiv.org/abs/2605.05029)
*Kejun Liu*
...
### [171] [Full-chip CMP modelling based on Fully Convolutional Network leveraging White Light Interferometry](https://arxiv.org/abs/2605.05062)
...
### [172] [Provable imitation learning for control of instability in partially-observed Vlasov--Poisson equations](https://arxiv.org/abs/2605.05081)
...
### [173] [Order Matters: Improving Domain Adaptation by Reordering Data](https://arxiv.org/abs/2605.05084)
...
### [174] [Gated Multimodal Learning for Interpretable Property Energy Performance Prediction and Retrofit Scenario Analysis](https://arxiv.org/abs/2605.05088)
...
### [175] [Unified Framework of Distributional Regret in Multi-Armed Bandits and Reinforcement Learning](https://arxiv.org/abs/2605.05102)
...
### [176] [Rollout Pass-Rate Control: Steering Binary-Reward RL Toward Its Most Informative Regime](https://arxiv.org/abs/2605.05112)
...
### [177] [How Long Does Infinite Width Last? Signal Propagation in Long-Range Linear Recurrences](https://arxiv.org/abs/2605.05113)
*Mariia Seleznova*
...
### [178] [Manifold Steering Reveals the Shared Geometry of Neural Network Representation and Behavior](https://arxiv.org/abs/2605.05115)
...
### [182] [Transformed Latent Variable Multi-Output Gaussian Processes](https://arxiv.org/abs/2605.05133)
...
### [183] [Low-Cost Black-Box Detection of LLM Hallucinations via Dynamical System Prediction](https://arxiv.org/abs/2605.05134)
...
### [184] [Human-AI Co-Mentorship in Project-Based Learning: A Case Study in Financial Forecasting](https://arxiv.org/abs/2605.05144)
...
### [185] [Understanding In-Context Learning for Nonlinear Regression with Transformers: Attention as Featurizer](https://arxiv.org/abs/2605.05176)
...
### [186] [Estimating the expected output of wide random MLPs more efficiently than sampling](https://arxiv.org/abs/2605.05179)
