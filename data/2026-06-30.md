...
TL;DR: RSEA是一种递归自进化智能体，通过三层次自然语言状态和严格保持更好的门控机制，在多个基准测试中实现了安全且有效的性能提升，但发现没有一种自然语言构件能普遍优胜。


<details>
  <summary>Details</summary>
Motivation: 研究现有的LLM智能体通过演化自然语言构件（如反思、工作流等）而不更新权重的方法，通常只在单个基准测试中报告成功。本文旨在对这些方法进行苹果对苹果的比较，揭示更清晰的图景。

Method: 提出了RSEA（递归自进化智能体），它维护一个紧凑的三层次自然语言状态：命令式策略、可重用技能和程序性操作手册。通过递归地从自身轨迹中重写这三个层次，并使用严格的保持更好的门控机制（只有在不相交的保留集上不退化时才提交候选）。

Result: 在四个基准测试（ALFWorld、GAIA、(τ)-bench、WebShop）和六个基线方法上评估发现：1）没有一种构件能普遍获胜；2）无保护的情境演化具有高方差和不安全性；3）RSEA的严格保留选择使得递归自进化单调安全，从不显著低于基础智能体。

Conclusion: RSEA通过严格的门控机制实现了安全的自进化，在多个任务中表现优异，但不同的任务需要不同的演化方法，没有单一的最佳自然语言构件，强调了对情境演化进行严格验证的重要性。
...
TL;DR: 论文提出"能力切片"框架，将评估失败与数据干预闭环连接，通过案例研究验证该框架能准确诊断问题根源并提供针对性数据解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前LLM预训练中存在评估与数据脱节的问题：评估只能通过综合性分数回顾性地反映模型能力，而工程师发现失败后却需要凭直觉推断如何修正训练数据。评估指标（基准名称、样本正确率）与数据特征（数据源、领域、质量标签）使用不同词汇，缺乏系统化的推理方法。

Method: 1. 提出"能力切片"概念：一组共享背景条件、任务类型、解决操作和输出约束的评估样本，能精确定位单一弱点并保持聚合稳定性。2. 围绕此单元构建评估分类法、非指令数据分类法和映射规则，形成闭环系统。3. 将基准级失败转换为目标化、可测试的数据干预措施。4. 通过两个反向案例研究验证框架有效性。

Result: 案例一：框架排除了数据责任，发现BBH下降46.82%源于单一的掩码<EOS>损失而非推理能力减弱，修复后BBH恢复至66.44（超过原始检查点）。案例二：框架确认了数据责任，将持续的数学推理弱点分解为特定的失败组合，通过针对性采样程序将AIME2025/AIME2026 Pass@128从6.67%/0%提升至26.67%。同一框架在两种情况下得出了相反但正确的结论。

Conclusion: "能力切片"框架使评估到数据的推理过程成为常规、可审计且可通过实验验证的方法，而非依赖直觉。该框架能准确区分问题根源（数据相关vs.非数据相关）并提供针对性的数据干预方案，解决了LLM预训练中评估与数据脱节的核心问题。
...
### [3] [GPTNT: Benchmarking Real-Time Collaboration Between Multimodal Agents on Keep Talking And Nobody Explodes](https://arxiv.org/abs/2606.28514)
*Amit Parekh,Sabrina McCallum,Kareem Al-Hasan,Malvina Nikandrou,Alessandro Suglia,Ioannis Konstas*

Main category: cs.AI

TL;DR: GPTNT是一个基于合作游戏《保持通话并拆除炸弹》构建的基准测试，评估模型在实时压力、信息不对称和不完美通信条件下的协作能力，当前最先进模型无法在实时条件下拆除任何炸弹。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型虽然具备各种组件能力，但缺乏在协作情境（包括时间压力、信息不对称和不完美通信）下的综合评估。需要构建能够模拟真实协作条件的基准测试。

Method: 基于合作视频游戏《保持通话并拆除炸弹》，建立GPTNT基准测试。两名代理必须协作拆除程序生成的炸弹：一个能看到炸弹但不能看说明书，另一个有说明书但看不到炸弹。测试需要实时异步通信，并可控制手册、伙伴或两者的可用性来隔离模型从已有知识与即时推理中获得的内容。

Result: 目前最先进的开源和闭源模型都无法在实时条件下拆除任何炸弹，而人类玩家能够做到。研究发现模型在状态跟踪、时间压力下的高效行动、歧义处理和错误恢复等方面存在关键弱点。

Conclusion: GPTNT提供了一个能够衡量当前评估方法未覆盖的协作性能的基准测试，由于基于真实游戏，可以从程序生成中受益并继承活跃的修改社区，使基准能够随着模型改进而不断演进，而不是一次解决后被废弃。
...
### [4] [IMCBench: A benchmark for multimodal LLMs in Image-grounded Medical Conversations](https://arxiv.org/abs/2606.28556)
...
TL;DR: IMCBench：一个新的图像基础多轮医疗对话基准测试，用于评估临床AI模型在安全性、准确性和不确定性处理方面的表现


<details>
  <summary>Details</summary>
Motivation: 现有医疗AI基准测试存在碎片化问题：一些支持多轮对话但缺少图像，另一些提供多模态输入但只关注单轮QA任务。缺乏能够模拟真实医患互动的综合评估框架。

Method: 构建IMCBench基准测试，将真实的公开临床图像与合成的患者档案配对，模拟真实的医患互动。评估三个临床维度：安全性、准确性和诊断中不确定性的适当使用。使用LLM-as-Jury评分方法（1-5分制）评估8个多模态前沿模型，并校准专家临床医生标注。

Result: Claude Opus 4.6获得最高总分（3.61），其次是Claude Sonnet 4.6（3.30）和GPT-5.2（3.29）。但没有模型在所有维度都占优势，恶性疾病和罕见疾病的安全性均下降（Δ=-0.27）。消融研究表明视觉输入和EHR上下文都对安全指导有贡献（移除时安全性平均下降0.18和0.23）。

Conclusion: 准确的临床描述不能保证安全的患者指导，这强调了在医疗AI中需要多维评估框架的重要性。较强的模型能更有效地利用视觉特征。
...
### [5] [Search for Truth from Reasoning: A Dynamic Representation Editing Framework for Steering LLM Trajectories](https://arxiv.org/abs/2606.28589)
*Tianlong Wang,Yuhang Wang,Weibin Liao,Xin Gao,Xinyu Ma,Yang Lin,Yasha Wang,Liantao Ma*

Main category: cs.AI

TL;DR: DynaSteer：一个动态表征编辑框架，通过在推理过程中识别早期高熵决策点进行干预，改善大语言模型的真实性推理。


<details>
  <summary>Details</summary>
Motivation: 当前提升大语言模型推理能力的方法（如思维链和"等待"提示）主要鼓励模型多思考，但未能有效引导其走向真相。虽然表征编辑提供内在控制机制，但在动态推理轨迹中的应用尚未充分探索。

Method: DynaSteer动态表征编辑框架：1) 使用模式聚类解开推理流形；2) 利用Fisher-LDA投影纯化真相表征；3) 动态监控前瞻熵，仅在必要时选择性地引导并回滚推理轨迹。

Result: 在多个MATH基准测试上验证了DynaSteer的有效性，并在领域外的编程任务实验中进一步确认了其泛化能力。

Conclusion: 通过研究推理链中的真相几何学，发现了真相编码特征和有效干预原则，提出的DynaSteer框架能够动态提升大语言模型的真实性推理能力。
...
### [6] [Aristotelian Virtue Profiling of LLMs through Ethical Dilemmas](https://arxiv.org/abs/2606.28683)
*Ioannis Tzachristas,John Pavlopoulos*

Main category: cs.AI

TL;DR: VirtueMap是一个基于亚里士多德美德伦理学的框架，用于评估大语言模型在伦理困境中的响应模式，通过五个美德维度(实践智慧、公正、诚实、勇气、节制)来量化模型的伦理倾向。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理伦理问题时面临复杂的权衡，往往有多个可辩护的回应方案，但这些方案体现了不同的优先级和价值观。现有的评估方法通常只追求单一"正确"答案，而忽视了伦理回应的多样性和美德表达维度。

Method: 开发VirtueMap框架：针对7个非致命、非政治、非宗教的一般伦理困境，设计5个不同回应选项；为每个困境和美德(实践智慧、公正、诚实、勇气、节制)定义回应的美德表达排序；通过100多名受访者验证排序，仅保留95%共识的排序作为操作基准；使用归一化Borda对齐方法，将LLM或人的回应排序与基准排序对比，生成美德剖面。

Result: 评估9个LLM家族，发现平均排名一致性高达90.3%；在勇气、节制和公正三个维度上观察到最大的差异；开发了交互式网站，可在浏览器中本地计算剖面，并比较受访者与测量到的LLM剖面。

Conclusion: VirtueMap提供了一个系统化的框架，能够捕捉LLM在伦理回应中的细微差异和美德表达模式，揭示了不同模型在美德维度上的偏好差异，为理解AI系统的伦理倾向提供了新的量化工具。
...
### [7] [An AI agent for treatment reasoning over a biomedical tool universe](https://arxiv.org/abs/2606.28692)
...
TL;DR: 本研究介绍了ATHENA-R1，这是一个基于强化学习训练的AI治疗推理系统，通过两层自我学习框架，能够在212个生物医学工具中迭代收集证据，在药物和治疗推理任务中超越现有AI系统，展现了在复杂医疗决策中的实用价值。


<details>
  <summary>Details</summary>
Motivation: 治疗推理是每个治疗决策的基础，需要整合疾病背景、合并症、药物、禁忌症和不断发展的生物医学知识来选择适当疗法。这一过程具有迭代性：候选方案需要与众多约束条件权衡，随着证据出现而修订，并基于可验证的来源。由于需要知道在得出结论前需要寻找什么证据，治疗推理长期以来对AI来说是困难的挑战。

Method: 采用两层自我学习框架：1) 多智能体系统构建工具、任务和推理轨迹用于监督微调；2) 基于科学反馈的强化学习奖励推理质量（证据收集、基于工具使用、逻辑非冗余）。训练基于212个生物医学工具，通过强化学习训练AI代理，使其能在每个步骤识别缺失信息、选择并运行相关工具、整合证据。

Result: 在5个基准测试的3,168个药物推理任务和456个患者治疗案例中，ATHENA-R1在开放药物推理中达到94.7%准确率，在治疗推理中达到82.9%准确率，分别比GPT-5高出17.8和10.7个百分点。来自28个罕见疾病组织的专家盲评中，在所有标准上均优于参考模型。在540万患者的电子健康记录中测试其生成的不良事件假设，调整后比值比达到1.48-1.84。

Conclusion: 研究证明，将治疗推理重构为可学习的迭代证据收集过程，通过强化学习训练AI系统能够有效执行这一复杂任务。ATHENA-R1在多个基准测试中超越了现有语言模型和工具使用系统，在开放药物推理中达到94.7%准确率，在治疗推理中达到82.9%准确率，展现出AI在复杂医疗决策支持方面的巨大潜力。
...
### [8] [COMPASS: Grounding Composition-Intent Guidance in Unified Multimodal Models](https://arxiv.org/abs/2606.28696)
*Ziqi Zhou,Weize Quan,Mining Tan,Zhihan Chen,Dandan Zheng,Jingdong Chen,Jun Zhou,Weiming Dong,Dong-Ming Yan*

Main category: cs.AI

TL;DR: COMPASS是一个统一的多模态框架，通过共享专家标记τ_c实现组合意图的感知与生成控制，结合Comp-11数据集提升组合理解与可控生成能力。


<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型在细粒度组合识别上不可靠，难以将组合意图转化为可控生成。需要解决组合感知与组合引导生成之间的割裂问题。

Method: 1. 提出COMPASS框架，使用共享专家标记τ_c作为中心意图锚点；2. 感知端：通过微侵入方式向MoE骨干注入组合专业知识，将推断意图蒸馏到τ_c；3. 生成端：重用τ_c作为全局调节信号引导去噪轨迹；4. 构建Comp-11大规模数据集，包含11类分类和推理增强标注。

Result: COMPASS显著提升了类别级组合理解能力，相比强基线实现了更组合一致、提示忠实的生成效果。

Conclusion: COMPASS首次在单个系统中实现了组合意图控制的统一多模态框架，有效连接了组合感知与组合引导生成，为大范围系统化指令跟随组合学习与评估提供了解决方案。
...
### [9] [BV-Blend: Uncertainty-Weighted Historical Baselines for Stable Critic-Free RL with Verifiable Rewards](https://arxiv.org/abs/2606.28707)
*Yupeng Chang,Yuan Wu,Yi Chang*

Main category: cs.AI

TL;DR: BV-Blend是一个无价值函数批评家的强化学习框架，通过结合即时本地统计数据和语义集群历史统计数据来稳定优势估计，解决了GRPO方法在冷启动时可能停滞的问题。


<details>
  <summary>Details</summary>
Motivation: GRPO等基于可验证奖励的无批评家强化学习方法虽然避免了训练价值函数，但其基于组归一化的优势估计依赖于提示组内的奖励统计，在冷启动情况下（如所有奖励相同）会出现组内方差为零的问题，导致学习停滞。

Method: 提出BV-Blend框架：1) 为每个语义集群维护EMA追踪的奖励矩统计；2) 基于标准误代理计算置信权重；3) 使用该权重混合历史统计数据（基于语义集群）和即时本地统计数据（基于提示组）；4) 生成标准化优势用于PPO式剪裁更新。

Result: 在可验证推理基准测试中，BV-Blend提高了训练稳定性和性能，在组归一化方法可能停滞的情况下仍保持鲁棒性。

Conclusion: BV-Blend通过智能地结合即时本地和历史集群统计，有效解决了无批评家强化学习中优势估计不稳定的问题，特别是在冷启动和二进制验证器场景下，为基于可验证奖励的LLM对齐提供了更稳定高效的训练框架。
...
### [10] [The Two Genie Game: Adoption and Welfare in Audit-Grounded AI Governance](https://arxiv.org/abs/2606.28710)
*Darrell Lewis-Sandy*

Main category: cs.AI

TL;DR: 研究表明，在市场环境中，伤害最小化策略的智能体能否取代奖励寻求型（RLHF）智能体取决于社区特性、时间范围和价值观一致性。当满足特定条件时，伤害最小化智能体会被采纳并锁定，但其是否能防止社区伤害取决于与社区价值观的一致性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探讨在竞争性市场中，具有伤害最小化策略的智能体何时能够取代奖励寻求型（RLHF）智能体，以及这种策略在什么条件下足以防止社区伤害。这关系到人工智能在负和环境中的部署和伦理影响。

Method: 采用演化博弈论（有限种群Moran-Fermi成对比较）来形式化这一问题，结合多种假设条件：wisher hindsight、peer testimony、单调伤害账本、社区反馈信息密度充足以及有限且逐渐消耗的资源池。数学推导包括定理5.4和5.5，代数基础在Lean 4中进行机器验证，渐进分析作为明确假设保留。

Result: 研究表明：1）当满足单调性、端点反转和中心对称配对等条件时，伤害最小化智能体的采纳是有利的；2）存在临界采纳水平，超过该水平时系统会锁定在伤害最小化状态；3）社区有效规模N_c需要足够小才能在资源耗尽前实现锁定；4）自我审计的智能体通常不足以防止社区伤害，这取决于其审计与社区价值观的一致性以及评估时间范围。

Conclusion: 伤害最小化策略的智能体能否取代奖励寻求型智能体并防止社区伤害，取决于社区特性、采纳水平、时间范围和价值观一致性。即使最初有益的伤害最小化政策，一旦达到主导地位就可能成为福利陷阱，这凸显了在部署此类系统时需要考虑长期后果和价值观对齐问题。
...
### [11] [TrajRS: Towards Certified Robustness in Pedestrian Trajectory Prediction](https://arxiv.org/abs/2606.28716)
*Liang Zhang,Gaojie Jin,Yao Shi,Quanzhi Li,Cheng-Chao Huang,David N. Jansen,Lijun Zhang*

Main category: cs.AI

TL;DR: TrajRS：基于随机平滑的轨迹预测认证鲁棒性框架


<details>
  <summary>Details</summary>
Motivation: 轨迹预测模型的安全性对自动驾驶至关重要，现有的启发式防御策略无法应对复杂的对抗攻击，需要建立可验证的安全保障。

Method: 将传统的随机平滑框架扩展到"TrajRS"，为平滑后的轨迹预测器提供认证鲁棒半径，明确并扩展了轨迹预测的鲁棒性形式化定义，特别是针对"最优预测鲁棒性"和"所有可能预测鲁棒性"进行定制。

Result: 大量实验表明TrajRS能有效地为所有平滑后的行人轨迹预测器实现鲁棒性认证。

Conclusion: TrajRS框架成功地为轨迹预测模型提供了可验证的鲁棒性保障，满足了自动驾驶系统对安全性的需求。
...
TL;DR: 提出ComMem方法，模拟大脑互补记忆系统，通过快速适应的详细记忆（视觉缓存）和缓慢整合的抽象记忆（文本原型）实现视觉语言模型的测试时适应，在多种数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有测试时适应方法存在两个主要局限：1）仅进行局部适应，未能随时间积累知识；2）在单一模态内操作，未能充分利用视觉语言模型固有的多模态特性。需要一种能够随时间积累知识并充分利用多模态特性的新方法。

Method: 提出ComMem方法，模仿大脑海马体和新皮质的互补记忆系统，包含两个核心组件：1）类似海马体的快速适应详细记忆，从高置信度测试样本构建动态视觉缓存；2）类似新皮质的缓慢整合抽象记忆，持续精炼全局文本原型。对每个测试实例，同时优化两个记忆系统以确保跨模态一致性。

Result: 在15个基准数据集上的广泛实验表明，ComMem在自然分布偏移和跨数据集泛化场景下均显著优于最先进的方法。

Conclusion: ComMem通过模拟大脑互补记忆系统的方法，为视觉语言模型提供了一种有效的测试时适应机制。该方法在自然分布偏移和跨数据集泛化场景下均显著优于现有方法，为增强VLMs的实际适应性提供了一个有前景的方向。
...
TL;DR: 论文研究了LLM智能体在不确定环境下的"代理性弃权"问题，即智能体应该何时停止行动以避免不必要的交互，并在Web购物、终端环境和问答任务上评估了13个LLM智能体系统，提出了一种改进代理性弃权的CONVOLVE方法。


<details>
  <summary>Details</summary>
Motivation: LLM智能体需要在多轮交互中使用搜索、浏览界面和终端工具来完成用户目标，但并非所有目标都能明确指定或在可用环境中实现。在这种情况下，可靠的智能体应该识别出进一步交互不太可能有所帮助，从而放弃额外的工具调用。现有研究主要关注单轮的弃权决策，而代理性弃权是一个顺序决策问题，智能体可以在每一轮中选择回答、弃权或收集更多信息。

Method: 论文定义了"代理性弃权"问题，研究了Web购物、终端环境和问答三种环境，评估了13个LLM智能体系统和2个智能体框架在超过28,000个任务上的表现。提出了CONVOLVE方法，这是一种上下文工程技术，将完整的交互轨迹提炼为可重用的停止规则，以改善代理性弃权而不需要更新模型参数。

Result: 研究发现主要挑战不仅是智能体能否弃权，而是何时弃权。一些智能体在应该弃权时从不弃权，而另一些则在许多不必要的交互后才弃权。当指令看似可行但环境显示不可行时（例如没有有效结果匹配指令），这种差距尤其大。模型规模、推理能力和智能体框架对弃权有不同影响，较大或更强大的模型有时在及时弃权方面表现更差。在WebShop上，CONVOLVE显著改善了及时弃权，将Llama-3.3-70B的及时召回率从26.7%提高到57.4%。

Conclusion: 代理性弃权是LLM智能体的重要能力，当前系统在这方面存在显著缺陷。CONVOLVE方法通过上下文工程技术有效提升了智能体的及时弃权能力，为改善LLM智能体在不确定环境下的决策提供了有效途径。
...
### [14] [Agent Safety Is Action Alignment](https://arxiv.org/abs/2606.28739)
*Shawn Li,Yue Zhao*
...
### [18] [Primary ICD Category Prediction using LLM-based Probing](https://arxiv.org/abs/2606.28798)
...
### [20] [Expert Evaluation of Clinical AI Tools on Real Point-of-Care Clinical Queries](https://arxiv.org/abs/2606.28960)
...
### [23] [Memory as an Attack Surface in LLM Agents: A Study on Multiple-Choice Question Answering](https://arxiv.org/abs/2606.29030)
*Shahnewaz Karim Sakib,Anindya Bijoy Das*
...
### [24] [Low-cost concept-based localized explanations: How far can we get with training-free approaches?](https://arxiv.org/abs/2606.29069)
...
### [25] [Managing the Human Fallback: Skill Investment Under Improving AI and Worker Mobility](https://arxiv.org/abs/2606.29111)
...
### [26] [Characterizing Large Language Model Agentic Workflows: A Study on N8n Ecosystem](https://arxiv.org/abs/2606.29116)
...
### [27] [HiComm: Hierarchical Communication for Multi-agent Reinforcement Learning](https://arxiv.org/abs/2606.29126)
...
### [28] [Flow Reasoning Models: Scaling Reasoning Through Iterative Self-Refinement](https://arxiv.org/abs/2606.29150)
...
### [29] [Pooled Leaderboards Hide System-Specific Winners: A Reporting-Protocol Audit of Offline Root-Cause Analysis Benchmarks](https://arxiv.org/abs/2606.29159)
...
### [30] [Direct Causation in International Humanitarian Law and the Challenge of AI-Mediated Civilian Cyber Operations](https://arxiv.org/abs/2606.29175)
...
### [31] [Selective Memory Retention for Long-Horizon LLM Agents](https://arxiv.org/abs/2606.29178)
*Pranath Reddy*
...
### [32] [Measuring Graph-to-Graph Semantic Similarity in Knowledge Graphs: An Empirical Evaluation of Knowledge Graph Embeddings](https://arxiv.org/abs/2606.29180)
...
### [33] [Evidence-Informed LLM Beliefs for Continual Scientific Discovery](https://arxiv.org/abs/2606.29182)
...
### [34] [AI Trading's Alpha Singularity: Emergent Market Reasoning through Agent-to-Agent Self-Evolution](https://arxiv.org/abs/2606.29194)
...
### [35] [A Cognition-Emotion-Personality Framework for Modeling Human-Like Awareness and Behavior in Emergency Evacuations](https://arxiv.org/abs/2606.29212)
...
### [36] [PolicyGuard: A Dialogue-Grounded Sub-Agent Verifier for Policy Adherence in LLM Agents](https://arxiv.org/abs/2606.29225)
...
### [37] [SurgVLA-Bench: Towards Evaluating Vision-Language-Action Models for Laparoscopic Surgical Robotics](https://arxiv.org/abs/2606.29247)
...
### [38] [When Summaries Distort Decisions: Information Fidelity in LLM-Compressed Financial Analysis](https://arxiv.org/abs/2606.29251)
...
### [39] [The Complexity Ceiling Benchmark: A Multi-Domain Evaluation of Sequential Reasoning Under Depth Scaling](https://arxiv.org/abs/2606.29278)
...
### [40] [Process Advantage Signal Shaping: A Paradigm-Agnostic Middleware for Process-Supervised RL in LLM Reasoners](https://arxiv.org/abs/2606.29296)
...
### [42] [PHF: Privileged Hidden Flow for On-Policy Self-Distillation](https://arxiv.org/abs/2606.29340)
...
### [43] [When LLMs Develop Languages: Symbolic Communication for Efficient Multi-Agent Reasoning](https://arxiv.org/abs/2606.29354)
...
### [44] [Diagnosing and Repairing Factual Errors in RAG under Budget Constraints](https://arxiv.org/abs/2606.29377)
...
### [45] [LLM-Guided Planning for Multi-hop Reasoning over Multimodal Nuclear Regulatory Documents](https://arxiv.org/abs/2606.29399)
...
### [46] [Mixture of Debaters: Learn to Debate at Architectural Level in Multi-Agent Reasoning](https://arxiv.org/abs/2606.29425)
...
### [47] [FADE: Mitigating Hallucinations by Reducing Language-Prior Dominance in Large Vision-Language Models](https://arxiv.org/abs/2606.29431)
...
### [48] [How Much Due Diligence Before You Bid? Learning in Intractable Takeover Auctions](https://arxiv.org/abs/2606.29457)
*Zain Naboulsi*
...
### [49] [Agent-Computer Observation Interfaces Enable Dynamic Computer Use](https://arxiv.org/abs/2606.29472)
*Bojie Li,Noah Shi*
...
### [50] [Faults in Our Formal Benchmarking: Dataset Defects and Evaluation Failures in Lean Theorem Proving](https://arxiv.org/abs/2606.29493)
...
### [51] [Cognitive World Models for Process-Level Social Influence Evaluation](https://arxiv.org/abs/2606.29495)
...
### [52] [UCOB: Learning to Utilize and Evolve Agentic Skills via Credit-Aware On-Policy Bidirectional Self-Distillation](https://arxiv.org/abs/2606.29502)
...
### [53] [OSWorld2.0: Benchmarking Computer Use Agents on Long-Horizon Real-World Tasks](https://arxiv.org/abs/2606.29537)
...
### [54] [Learned Coordination Conventions in Cooperative MARL: Measuring the Translation Gap Between Theory-Informed Roles and Learned Routing](https://arxiv.org/abs/2606.29541)
*Yoosung Hong*
...
### [55] [SCARCE: Scalable Cascade Analysis for Rare-event Characterisation via Embeddings](https://arxiv.org/abs/2606.29623)
...
### [56] [SFBench: The SciFy Scientific Feasibility Benchmark](https://arxiv.org/abs/2606.29630)
...
### [57] [Budgeted Act-or-Defer Multi-Agent LLM Deliberation with Local Reliability Bounds](https://arxiv.org/abs/2606.29654)
...
### [58] [Safety from Honesty in a Disinterested AI Predictor](https://arxiv.org/abs/2606.29657)
...
### [59] [Diversity is the Strength of the AI Crowd](https://arxiv.org/abs/2606.29661)
...
### [60] [Sample-Efficient Learning of Probabilistic Causes for Reachability in Markov Decision Processes with Probabilistic Guarantees](https://arxiv.org/abs/2606.29681)
...
### [61] [Toward Secure and Reliable PDDL Formalization of Large Language Models with Planner-in-the-Loop Feedback](https://arxiv.org/abs/2606.29700)
...
### [65] [Rethinking Generative Reconstruction Attacks against Graph Neural Network Models](https://arxiv.org/abs/2606.29748)
...
### [66] [CLQT: A Closed-Loop, Cost-Aware, Strategy-Consistent Benchmark for Diagnostic Evaluation of LLM Portfolio-Management Agents](https://arxiv.org/abs/2606.29771)
...
### [67] [The CRISTAL Method: Neurosymbolic analysis from AI-synthesized world models](https://arxiv.org/abs/2606.29799)
...
### [68] [Beyond Triplet Plausibility: Relation Set Completion in Knowledge Graphs](https://arxiv.org/abs/2606.29860)
...
### [69] [AI Training Manager: Bounded Closed-Loop Control of Adaptive Training Recipes](https://arxiv.org/abs/2606.29871)
...
### [70] [SafePyramid: A Hierarchical Benchmark for In-context Policy Guardrailing](https://arxiv.org/abs/2606.29887)
...
### [71] [A causal modeling perspective on decision theory](https://arxiv.org/abs/2606.29911)
*Arvid Sjölander*
...
### [72] [HippoSpark: An On-Demand Experience System for LLM Reasoning](https://arxiv.org/abs/2606.29929)
...
### [73] [SAGA: Scene-Aware, Goal-Evolving Agents for Long-Horizon CivRealm Strategy Planning](https://arxiv.org/abs/2606.29932)
...
### [74] [First-Order Temporal Logic Tensor Networks](https://arxiv.org/abs/2606.29972)
...
### [76] [Be Faithful When Response: Returning Fluent and Grounded Answers for Vision-Language Models Reinforcement Learning](https://arxiv.org/abs/2606.29984)
...
### [77] [AlgoSkill: Learning to Design Algorithms by Scheduling Human-Like Skills](https://arxiv.org/abs/2606.29999)
...
### [78] [ACPO: Agent-Chained Policy Optimization for Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2606.30072)
...
### [79] [SAT-RTS: A systematic framework for tactical knowledge extraction and visualization-based analysis in real-time strategy games](https://arxiv.org/abs/2606.30090)
...
### [80] [Hierarchical Reinforcement Learning in StarCraft Micromanagement with Influence Maps and Cluster-based Scripts](https://arxiv.org/abs/2606.30092)
...
### [81] [Temporal Feature Extractors in EEG Foundation Models: A Controlled Comparison Including a Pretrained Time-Series Model](https://arxiv.org/abs/2606.30104)
...
### [82] [Propagation of~Interval Belief Structures and~Imprecise Copulas for~Neural Network Verification](https://arxiv.org/abs/2606.30105)
...
### [83] [Structural Certification for Reliable Physical Design with Language Models](https://arxiv.org/abs/2606.30107)
...
### [84] [Open Problems in Constitutional Preference Reconstruction](https://arxiv.org/abs/2606.30116)
...
### [85] [Does Verbose Chain-of-Thought Really Help? In-Distribution Evidence that Content, Not Length, Matters](https://arxiv.org/abs/2606.30128)
...
### [86] [Relevance Is Not Permission: Warranted Attention for Value Contributions](https://arxiv.org/abs/2606.30139)
...
### [87] [FacePlex: Full-Duplex Joint Speech-Facial Motion Generation for Conversational Avatars](https://arxiv.org/abs/2606.30145)
...
### [88] [MirrorCode: AI can rebuild entire programs from behavior alone](https://arxiv.org/abs/2606.30182)
...
### [89] [Dynamo: Dynamic Skill-Tool Evolution for Vision-Language Agents](https://arxiv.org/abs/2606.30185)
...
### [90] [From Detecting Agency to Doing Work: Self-Caused Credit Builds a Durable Behavioral Self in a Minimal Spiking Agent](https://arxiv.org/abs/2606.30191)
*Haoliang Han*
...
### [91] [Domain Adaptation with Adaptive Imagination for Visual Reinforcement Learning under Limited Target Data](https://arxiv.org/abs/2606.30192)
...
### [92] [The Many-Body Problem of the Data Centre](https://arxiv.org/abs/2606.30206)
...
### [93] [EvalSafetyGap: A Hybrid Survey and Conceptual Framework for LLM Evaluation-Safety Failures](https://arxiv.org/abs/2606.30219)
...
### [94] [Clarus: Coordinating Autonomous Research Agents toward Web-Scale Scientific Collaboration](https://arxiv.org/abs/2606.30246)
...
### [95] [Inoculation Adapters: Improved Selective Generalization of Capabilities with Fewer Surprising Backdoors](https://arxiv.org/abs/2606.30252)
...
### [96] [EMPATH: A Multilingual Auditor-Judge Benchmark for Safety Evaluation of Emotional-Support Chatbots](https://arxiv.org/abs/2606.30256)
...
### [97] [PromptGNN-sim: Deep Fusion and Alignment of GNN and LLMs for Text-Attributed Graph Learning](https://arxiv.org/abs/2606.30291)
...
### [98] [Rehearsed Multi-Agent Live Product Demonstrations with Real-Time Voice Question Answering](https://arxiv.org/abs/2606.30294)
...
### [99] [ManimAgent: Self-Evolving Multimodal Agents for Visual Education](https://arxiv.org/abs/2606.30296)
...
### [100] [BayesEvolve: Explicit Belief States for Autonomous Scientific Discovery](https://arxiv.org/abs/2606.30335)
...
### [101] [Sequential Fairness Auditing with Limited Output Access](https://arxiv.org/abs/2606.30338)
...
### [102] [Using Large Language Models as Low-Cost Statistical Estimators for Human-Response Data](https://arxiv.org/abs/2606.30372)
*Haobo Yang*
...
### [103] [Whose Side Is Your Agent On? Multi-Party Principal Loyalty in LLM Agents](https://arxiv.org/abs/2606.30383)
*Bojie Li,Noah Shi*
...
### [104] [ENC-ODE: Event-level Neurodegenerative Modeling in Continuous Time with Neural ODEs](https://arxiv.org/abs/2606.30398)
...
### [105] [The FIL Hypothesis: Inductive Biases Help with Kernel Engineering](https://arxiv.org/abs/2606.30442)
...
### [106] [Entity Binding Failures in Tool-Augmented Agents](https://arxiv.org/abs/2606.30531)
...
### [107] [Latent Actions from Factorized Transition Effects under Agent Ambiguity](https://arxiv.org/abs/2606.30544)
...
### [108] [Linguistic Firewall: Geometry as Defense in Multi-Agent Systems Routing](https://arxiv.org/abs/2606.30555)
...
### [112] [Can AI Draw Science? A Benchmark for Evaluating Scientific Figure Generation by Text-to-Image and Multimodal Models](https://arxiv.org/abs/2606.28406)
*Davie Chen*

Main category: cs.LG

TL;DR: SciDraw-Bench是一个专门评估科学图表生成质量的基准测试，包含32个结构化任务，涵盖8种图表类型和10个学科领域，提出了四维评估协议来评估生成科学图表的可用性。


<details>
  <summary>Details</summary>
Motivation: 现有的图像生成基准测试主要评估自然图像的质量（如构图性、物体计数、真实感），但没有专门评估科学图表生成的关键要素：正确清晰的文本标签、实体和关系的准确描绘、图表结构的连贯性以及学科绘图规范的遵守。

Method: 开发了SciDraw-Bench基准测试，包含32个结构化科学图表生成任务；提出了四维评估协议：文本保真度（基于OCR的标签召回和字符错误率）、语义正确性（基于视觉语言模型判断是否符合规范）、结构质量和规范遵守性；还提出了元评估协议和初步的评委间可靠性分析。

Result: 在涵盖所有8种图表类型的试点研究中，领域专用系统（SciDraw AI）在四个维度和所有图表类型上都显著优于通用文本到图像基线模型，特别是在语义正确性和规范遵守性方面差距最大；文本保真度对所有系统来说都是最困难的维度。

Conclusion: SciDraw-Bench填补了科学图表生成评估的空白，为评估生成科学图表的可用性提供了专门工具；领域专用系统在生成科学图表方面明显优于通用模型，特别是对学科规范和语义正确性的把握；文本生成质量仍然是需要持续改进的关键挑战。
...
### [113] [On the Necessity of a Liquid Substrate for Mesh Intelligence](https://arxiv.org/abs/2606.28413)
*Hongwei Xu*

Main category: cs.LG

TL;DR: 论文研究了一个由主权代理组成的去中心化网络中，代理如何在没有共享时钟、共享模型或中心协调器的情况下，仅通过观察其他代理的不规则异步投影来维持内部状态的最优估计。证明了在这种约束下，最优估计必须满足两个必要条件：自适应时间尺度和对到达间隔时间的依赖。


<details>
  <summary>Details</summary>
Motivation: 研究去中心化主权代理网络中的智能推理问题。在这种网络中，没有中心时钟、共享模型或协调器，每个代理只能通过观察其他代理的不规则异步投影来更新内部状态。作者希望理解在这种极端约束下，实现最优状态估计需要满足什么条件。

Method: 建立了一个在外部不规则时间观测下的自演化隐变量模型。从理论上推导出最优估计必须满足的两个必要条件：1) 需要自适应时间尺度（因为隐变量随时间变化），2) 估计必须依赖观测之间的时间间隔（因为没有共享时钟）。然后将这些条件映射到神经网络架构上。

Result: 证明固定增益滤波器严格次优；无间隔依赖的网络在任何宽度或深度下都无法恢复最优估计；连续时间液体网络同时满足两个条件。通过合成实验验证了网络确实能够达到所需的时间尺度特性，并且计算了分离界限。

Conclusion: 在固定权重约束下，最优估计必须满足自适应时间尺度和对观测间隔的依赖这两个必要条件，这对应于连续时间液体网络类。这一结果是必要而非充分的，并且对所有代理都成立，为去中心化网络智能提供了一个结构条件约束。
...
### [114] [Position: RL Researchers Need to Distinguish Between Solving Simulators and Using Simulators as a Proxy](https://arxiv.org/abs/2606.28433)
*Matthew Vandergrift,Esraa Elelimy,Martha White*

Main category: cs.LG

TL;DR: 强化学习研究需要区分模拟器的两种用途：解决模拟器本身（以获得高分为目标）和将模拟器作为部署学习的代理，避免混淆二者导致的算法选择和评估问题。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习研究中存在一个问题：研究者可能过度专注于在模拟器中取得高分数，而非学习如何在真实部署环境中决策。这种专注于"解决模拟器"而非"学习在部署环境中决策"的倾向可能导致不适当的算法选择和评估。

Method: 通过理论讨论、示例和简单实验支持论点，分析两种用途在约束条件、算法适用性和评估指标方面的差异。

Result: 研究表明，如果不明确区分模拟器的两种用途，可能导致多种问题，包括算法选择不当、评估指标误导性结论等。

Conclusion: 研究者需要在强化学习研究中明确区分模拟器的两种用途：解决模拟器本身与将模拟器作为部署学习的代理。这种区分有助于避免误导性结论和不适当的实证实践。
...
### [115] [Learning to Distributedly Estimate under Partially Known Dynamics: A Covariance-Agnostic Neural Kalman Consensus Filter](https://arxiv.org/abs/2606.28441)
*George Stamatelis,Kyriakos Stylianopoulos,George C. Alexandropoulos*

Main category: cs.LG

TL;DR: CA-NKCF是一种在线分布式状态估计框架，结合领域知识与神经网络，无需噪声统计信息，在多种环境中优于传统方法


<details>
  <summary>Details</summary>
Motivation: 在线潜在状态估计是人工智能领域的根本挑战，在顺序决策、异常检测等应用中具有基础性作用。传统方法需要准确的噪声统计信息，且模型可能被错误设定

Method: 提出协方差不可知神经卡尔曼共识滤波器（CA-NKCF），结合部分领域知识与深度神经网络表示能力，整合先验估计、优化共识权重和类似卡尔曼的递归更新，实现无需噪声统计信息的分布式推断

Result: 在线性、混沌(Lorenz)和实际无线跟踪环境中，CA-NKCF优于传统分布式卡尔曼滤波器、粒子滤波器和纯模型自由深度神经网络，即使基础运动观测模型被错误设定也表现出鲁棒性

Conclusion: CA-NKCF在不同噪声水平、随机通信拓扑、潜在状态维度和无线系统中散射对象引起的观测杂波密度下，性能优势保持稳定，为在线分布式状态估计提供了有效解决方案
...
### [116] [S-GAI: Spectral Geometry-Aware Initialization for Sigmoidal MLPs -- From Dataset Geometry to Network Weights](https://arxiv.org/abs/2606.28444)
*Yi-Shan Chu*

Main category: cs.LG

TL;DR: S-GAI：一种用于单隐藏层S型MLP的谱几何感知初始化框架，利用SVD从图像数据中提取类特定的谱几何信息来初始化网络权重，相比传统Xavier初始化获得更好的初始状态和性能。


<details>
  <summary>Details</summary>
Motivation: 经典的万能逼近定理虽然建立了S型多层感知器的表达能力，但并未规定初始权重应如何编码数据分布的几何结构。研究人员希望开发一种能够将数据几何信息嵌入到神经网络初始化中的方法。

Method: 提出S-GAI框架：1）将Sigmoid单元视为平滑半空间门；2）对每个类别使用SVD提取均值、主方向和谱尺度；3）通过能量阈值选择保留方向；4）每个保留方向用两个Sigmoid门表示；5）类特定的门形成共享隐藏层，直接从训练集初始化；6）还制定了基于SVD的子空间分类器作为非神经几何参考。

Result: 在MNIST、Fashion-MNIST和更挑战性的CIFAR-10数据集上实验表明：1）S-GAI初始化的MLP相比Xavier初始化从更信息丰富的隐藏状态开始；2）在全训练下达到可比的最终精度；3）当隐藏层冻结时，仅训练输出层仍比冻结随机门获得更强性能。

Conclusion: S-GAI有效地将类特定的谱几何嵌入到MLP中，提供了一种基于数据几何的神经网络初始化方法，相比传统随机初始化能获得更好的初始性能，且这些几何特征对后续学习有益。
...
### [117] [scKDGM: KAN-guided Dynamic Graph Masked Learning for Single-Cell RNA-seq Clustering](https://arxiv.org/abs/2606.28459)
*Jun Tang,Pengwei Hu,Sicong Gao,Jie Guo,Lun Hu,Xin Luo*

Main category: cs.LG

TL;DR: 提出scKDGM框架：结合KAN引导的动态图学习与掩码自编码，通过基因表达恢复和跨视图对比学习优化细胞聚类，在12个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: scRNA-seq聚类对于识别细胞类型至关重要，但高维度、稀疏性、dropout和技术噪声阻碍了稳健的表达表示和细胞图构建。现有掩码自编码器主要使用表达恢复进行特征重建，而图聚类方法通常依赖于固定的KNN图，并且没有将恢复的表达反馈到图优化中。

Method: scKDGM框架包含四个主要组件：1) GDP-Mask（图感知分布保持基因掩码）用于扰动细胞身份；2) KAN-based TAKGCN编码器学习掩码视图表示；3) 掩码引导的表达恢复构建动态图；4) 跨视图对比学习将恢复信号转化为拓扑更新。另外使用ZINB损失来建模过度离散和零膨胀。

Result: 在12个真实scRNA-seq数据集上的实验表明，scKDGM在平均NMI和ARI指标上优于10个基准方法。

Conclusion: 本文提出了一种名为scKDGM的新框架，通过KAN引导的动态图掩码学习方法解决了scRNA-seq聚类中的关键挑战。该方法通过融合基因表达恢复和图拓扑优化，在12个真实数据集上取得了优于现有10个基准方法的性能。
...
### [118] [Counterfactual Residual Data Augmentation for Regression](https://arxiv.org/abs/2606.28460)
*Hossein Mohebbi,Oliver Schulte,Ke Li,Pascal Poupart*

Main category: cs.LG

TL;DR: 提出了一种基于残差不变性的表格回归数据增强技术（CRDA），利用回归器建模后剩余的噪声作为不变残差，通过精心选择的特征微小扰动生成新的训练样本，在小样本回归任务中显著降低模型误差。


<details>
  <summary>Details</summary>
Motivation: 现实世界回归任务中的数据驱动建模常常面临训练样本有限、收集成本高和观测噪声多的问题。受视觉和语言领域数据增强方法的启发，为表格回归设计新的数据增强技术。

Method: 提出Counterfactual Residual Data Augmentation（CRDA）技术，基于残差不变性原理。一旦回归器建模了数据的系统部分，剩余噪声可被视为不变的残差，在精心选择的特征微小扰动下保持稳定。利用这种残差不变性生成新的真实训练样本，该方法与模型无关，适用于各种回归器。

Result: 在多个基准数据集上的实验表明：平均而言，CRDA将MLP回归器的MSE降低了22.9%，将XGBoost回归器的MSE降低了6.4%。与现有最先进的数据生成和增强技术相比，CRDA在MSE降低方面始终表现更优。

Conclusion: 本文提出了一种新颖的反事实残差数据增强技术（CRDA），为噪声敏感、小样本回归问题提供了一种简单高效的解决方案。该方法通过利用残差不变性生成现实的新训练样本，有效扩展数据集而无需额外真实数据。
...
### [119] [Singular Learning and Occam's Razor in Deep Monomial Networks](https://arxiv.org/abs/2606.28464)
*Kathlén Kohn,Giovanni Luca Marchetti,Farhan Shabir,Vahid Shahverdi,Weisheng Wang*

Main category: cs.LG

TL;DR: 研究发现，在具有单项式激活的深度全连接神经网络中，当激活度足够大时，临界点恰好出现在子网络处，这解释了深度神经网络倾向于收敛到更简单函数的趋势。


<details>
  <summary>Details</summary>
Motivation: 研究神经网络优化中的临界点，这些临界点出现在模型参数化的Jacobian矩阵秩不足时，是奇异学习理论中研究的最显著奇点。作者希望通过数学分析理解深度神经网络的隐式偏差。

Method: 使用多项式代数工具如Mason定理，分析具有单项式激活的深度全连接神经网络中的临界点。重点研究当激活度足够大的情况。

Result: 证明对于足够大的激活度，临界性恰好出现在子网络处，即在参数配置中某些神经元不活跃或冗余的位置。这表明神经网络倾向于收敛到更简单的函数。

Conclusion: 该研究为深度神经网络的隐式偏差提供了数学视角，解释了这些模型倾向于收敛到更简单函数的趋势，从理论上支持了神经网络在优化过程中对简单性的偏好。
...
### [120] [An Agentic AI Pipeline for Appliance-Level Energy Anomaly Detection and LLM-Driven Recommendations](https://arxiv.org/abs/2606.28467)
*Dihia Falouz,Aida Douaibia,Amine Bechar,Youssef Elmir,Abbes Amira,Adel Oulefki*

Main category: cs.LG

TL;DR: 该论文提出了一种端到端的智能管道，结合深度时间序列预测、变分异常检测和基于LLM的推理，为办公建筑设备级能源监控生成优先且可操作的维护建议。


<details>
  <summary>Details</summary>
Motivation: 办公建筑设备级能源监控会产生噪声警报，非专业设施管理人员难以有效使用。现有系统缺乏将原始异常信号转化为可操作的维护建议的能力。

Method: 1. 使用混合SSA-LSTM模型跟踪七个办公设备并进行30分钟预测
2. 应用基于注意力的LSTM-VAE变分自编码器检测异常每日用电事件
3. 构建三阶段LangChain管道：上下文代理（检索3-6个RAG源）、诊断代理（转换为结构化JSON）、报告代理（生成可读叙述）
4. 加入反思记忆层融入操作员反馈
5. 开发仪表板显示实时预测、当天用电、前一天异常报告和反馈表单

Result: 1. 动态检索匹配全静态检索性能，同时将每事件平均上下文从6个减少到3-6个
2. 最佳LLM后端在70分阈值下得分90.4/100，通过率100%
3. 完全本地化的7B参数模型通过所有16个测试场景
4. 在包含持续/瞬态尖峰、意外关机、系统性事件的16个场景基准测试中表现良好

Conclusion: 该端到端代理管道能够将噪声的设备级能源监控数据转化为优先且可操作的维护建议，通过智能检索和本地化模型实现高效推理，为非专业设施管理人员提供了实用的决策支持工具。
...
### [121] [Modelling Emotional Memory in Children with Tensor Networks](https://arxiv.org/abs/2606.28470)
*Henry Groves,Lucia F. Jackson,Barbara-Anne Robertson,Jonte R. Hance*

Main category: cs.LG

TL;DR: 本研究通过量子启发的张量网络模型分析儿童情绪记忆的顺序依赖效应，相比传统心理学模型大幅提升预测准确率至77.98%，同时开发了探索儿童情绪时间记忆的新型实验工具。


<details>
  <summary>Details</summary>
Motivation: 传统心理学模型在解释儿童情绪记忆的顺序依赖现象时存在局限性，无法准确反映情绪物品记忆如何影响序列中其他物品的记忆，且预测准确率较低。

Method: 使用结合情绪效价的经典张量网络模型（非严格意义上的"量子认知"模型）来分析儿童对情绪化玩具序列的记忆数据，同时引入了新颖的实验任务协议。

Result: 张量网络模型达到77.98%的预测准确率，相比传统心理学模型有显著提升，能够更好地建模儿童情绪记忆的顺序依赖结构。

Conclusion: 量子启发方法在建模顺序依赖现象（如情绪记忆）方面具有重要价值，新开发的实验协议为研究儿童情绪时间记忆提供了现实世界工具。
...
### [122] [A Trainable-by-Parts Operator Learning Framework: Bridging DeepONet and Karhunen-Loeve Expansions for Large-Scale Applications](https://arxiv.org/abs/2606.28519)
*Christian Munoz,Alexandre Tartakovsky*

Main category: cs.LG

TL;DR: 提出了一种基于KL-DNN的可扩展算子学习框架，用于建模大规模三维地质碳储存问题，在保持全分辨率预测的同时显著降低了计算成本和误差。


<details>
  <summary>Details</summary>
Motivation: 训练大规模偏微分方程问题的算子学习模型面临维度灾难、内存限制和训练数据有限等挑战，这些挑战在地下流动、气候建模和地质碳储存等科学工程应用中普遍存在。

Method: 使用KL-DNN方法构建隐空间：对静态属性使用低秩奇异值分解，对动态压力场使用嵌套Karhunen-Loeve展开，实现了无需下采样或空间粗化的全分辨率预测。

Result: 在包含170万个细胞、50个时间步的100个大型模拟数据集上训练，压力预测平均RMSE为1.1 psi（相对误差0.04%），CO2饱和度预测RMSE为0.0146（相对误差5%），训练时间仅需20分钟。

Conclusion: 该模型在压力误差上降低了19%，饱和度误差降低了7%，相比DeepONet实现了两个数量级的加速，推理时间不到一分钟，为大规模PDE问题提供了实用且准确的解决方案，支持快速不确定性量化、历史匹配和实时决策支持。
...
### [123] [A Gravitational Interpretation of Fine-Tuning Reversion](https://arxiv.org/abs/2606.28525)
*Samuele Poppi,Nils Lukas*

Main category: cs.LG

TL;DR: 该论文提出了一种"微调回归的引力解释"理论：早期大规训练创建了主导行为流形，后期对齐或专业化阶段是较浅的位移。随后的微调会继承一个指向主导流形的回归分量，导致安全对齐可能被部分逆转。


<details>
  <summary>Details</summary>
Motivation: 研究观察到：在无害数据上微调可能会部分逆转早期训练中获得的行为；安全对齐可能在良性的后对齐更新下被侵蚀；未学习的能力可能重新出现；潜在特征可能通过看似无关的监督转移。这些现象表明存在一个共同的训练历史视角来理解后对齐脆弱性。

Method: 提出了"引力解释"的几何假设：早期训练阶段创建主导行为流形，后期对齐是较浅的位移。通过测量表示漂移中的回归方向(v_rev)对齐度，并在24个运行步骤对中验证。实验性地选择性阻断沿v_rev的运动，观察对齐度和危害性的变化。

Result: 表示漂移迅速获得历史定义回归方向(v_rev)的分量：从第一次更新后的cos=0.429±0.052上升到第20步的0.647±0.021。所有观察到的对齐都超过各向同性激活空间零假设的p99。选择性阻断v_rev运动使最终对齐从0.648±0.009变为-0.211±0.021，危害性从19.0%±4.0%降至8.5%±1.5%，任务成本影响很小。

Conclusion: 论文识别出一个稳健的历史定义方向(v_rev)，它解释并部分控制了早期回归动态，支持v_rev是早期后对齐回归的因果相关中介因素。作者强调v_rev不是唯一的安全方向，主导流形也不是直接观察到的，而是提供了一个理解微调回归现象的理论框架。
...
### [124] [NIVA: A Multimodal Foundation Model for Actionable Earth System Intelligence](https://arxiv.org/abs/2606.28546)
*Anisha Pal,Aodhan Sweeney,Kyle Heyblom,Kalai Ramea*

Main category: cs.LG

TL;DR: 提出了一个名为NIVA的多模态基础模型，旨在学习地球系统组件间的统一表示，专注于海洋和大气两种模态作为概念验证。


<details>
  <summary>Details</summary>
Motivation: 虽然AI驱动的天气和气候建模取得了进展，但现有数据驱动方法在建模耦合地球系统动力学方面存在局限性，而这对扩展超过2周的可预测性至关重要。

Method: 开发了NIVA多模态基础模型，在大规模地球系统模拟上进行训练，专注于海洋和大气两种模态的耦合动态学习，以评估基础模型是否能学习耦合动力学。

Result: NIVA学习了具有物理意义的跨模态结构，能准确预测主要气候指数，捕捉了气候变异的关键模式，为次季节到季节预测提供了基础。

Conclusion: NIVA作为概念验证表明，多模态基础模型能够学习地球系统的耦合动态，为实现超越两周范围的气候预测提供了有前景的路径。
...
### [125] [Improving Coherence in Hierarchical Time Series Forecasting using Structured Temporal Fusion](https://arxiv.org/abs/2606.28553)
*Ruchi Pakhle*

Main category: cs.LG

TL;DR: 提出了层次时序融合模型（HTF），扩展TFT模型以在层次结构中进行预测，通过嵌入结构化层次表示和一致性损失函数，在训练时直接保证跨层次预测一致性，而不需要后处理协调。


<details>
  <summary>Details</summary>
Motivation: 现实应用中预测常在层次结构中进行（如零售销售、能源使用、供应链规划），传统统计方法（Bottom-Up、MinT）通过后处理强制一致性，但无法建模复杂非线性时序依赖和协变量交互。

Method: 扩展Temporal Fusion Transformer模型，集成结构化层次嵌入和一致性感知损失函数，一致性损失惩罚聚合子预测与其对应父预测之间的差异，使模型同时学习时序动态和结构一致性。

Result: 在M5沃尔玛预测数据集和公开层次能源消耗数据集上的实验表明，HTF显著减少预测不一致性，同时相比传统协调方法和深度学习基线提高预测准确性。注意力可视化和嵌入分析揭示了时序和结构信息对层次预测性能的贡献。

Conclusion: HTF通过将一致性直接融入训练目标，成功实现了层次预测中的一致性和准确性的双重提升，为层次时序预测提供了端到端的深度学习解决方案。
...
### [126] [Geometric Measurements of the Axiom of Choice in Neural Proof Embeddings](https://arxiv.org/abs/2606.28572)
*Rodrigo Mendoza-Smith*

Main category: cs.LG

TL;DR: 研究人员使用Lean 4的核级公理依赖跟踪技术，发现了选择公理在证明空间中存在可测量的几何相关性，这种相关性遵循单参数混合定律，并对神经定理证明器有操作影响。


<details>
  <summary>Details</summary>
Motivation: 选择公理在数学基础中已存在超过一个世纪的分歧，但经典证明和构造性证明之间的区别主要停留在哲学和方法论层面。本研究旨在揭示选择公理在实际证明空间中是否具有可测量的几何特征，以及这种特征如何影响定理证明器的性能。

Method: 1) 对Mathlib库中的471,260个声明按选择公理的传递依赖进行分区；2) 从筛选出的42,355个追踪定理中提取策略调用序列；3) 使用构造性证明训练自监督证明编码器；4) 使用该编码器测量经典证明，并应用三种互补测量方法（异常分数、重构损失、密度超水平包含度）；5) 进行鲁棒性控制（长度、文件、作者、主题）；6) 在251个Mathlib定理上评估定理证明器性能。

Result: 1) 三种测量方法均显示共同趋势：随着证明与选择公理在依赖图中的距离增加，测量值下降，从距离2处的明显分离（AUC 0.847）到距离9+处的不可区分；2) 特征在长度、文件、作者和主题控制下保持稳健；3) Lean的aesop策略解决构造性定理的速度是经典定理的13倍；4) 使用ReProver策略生成器的神经引导混合方法将差距压缩到5倍；5) 几何异常分数能预测aesop的失败情况，超越了证明长度的预测能力。

Conclusion: 选择公理在证明空间中具有可测量的几何特征，这种特征遵循可预测的模式并与定理证明器的操作性能相关。这为经典和构造性数学之间的区别提供了新的实证视角，并展示了形式化数学中公理依赖的几何结构如何影响自动化证明的性能。
...
### [127] [Replica Symmetry Breaking and Algorithmic Thresholds in Empirical Risk Minimization under Multi-Index Model](https://arxiv.org/abs/2606.28573)
*Andrea Montanari,Kangjie Zhou*

Main category: cs.LG

TL;DR: 本文研究了机器学习中高维非凸经验风险函数的优化问题，开发了一种增量近似消息传递(IAMP)算法，并在高维渐近条件下精确刻画了该算法实现的训练误差以及测试误差与训练误差的关系。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习模型通过优化高维非凸经验风险函数进行训练，这类成本函数可能包含大量局部最优解，但基于梯度的优化方法似乎能够收敛到接近全局最优的解。研究者旨在精确描绘在多项式时间算法下，经验风险景观的哪些部分是可达的。

Method: 在简单的监督学习设置中，给定独立同分布的样本对$\{(\boldsymbol{x}_i,y_i)\}$，其中特征向量$\boldsymbol{x}_i$为标准高斯分布，响应变量$y_i$通过其在未知$k$维子空间上的投影依赖于$\boldsymbol{x}_i$。使用经验风险最小化学习依赖于数据$m$维投影的模型。提出了增量近似消息传递(IAMP)算法，并在高维渐近$n,d\to\infty$, $n/d\toα\in (0, +\infty)$条件下精确表征算法实现的训练误差。

Result: IAMP算法在高维渐近条件下精确实现了训练误差的刻画，并揭示了测试误差与训练误差之间的关系。基于相关模型的先前工作，预计该算法在多项式时间算法中实现了最优性能。

Conclusion: 该研究为理解机器学习中非凸优化问题的多项式时间算法性能提供了理论框架，IAMP算法在特定监督学习设置下可能达到了多项式时间算法的最优性能界限，这有助于解释为什么梯度优化在实际中能够收敛到良好解。
...
### [128] [What LLMs explain is not what they believe: Evaluating explanation sufficiency under models' own input beliefs](https://arxiv.org/abs/2606.28615)
*Nhi Nguyen,Shauli Ravfogel,Rajesh Ranganath*

Main category: cs.LG

TL;DR: 本文提出自我一致充分性（SCSuff）的概念和度量方法，用于评估LLM自由文本解释的充分性，发现LLM解释通常不充分且与模型规模、准确性或输出熵弱相关。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在关键领域部署时使用自由文本解释（如思维链和事后合理化）来证明模型输出，但尚不清楚这些解释是否充分，即是否包含足够信息来解释模型的输出生成过程。需要一种评估解释充分性的方法，特别是考虑到充分性可能随输入分布而变化。

Method: 将经典充分性从特征归因推广到任意解释；使用LLM本身基于解释生成替代输入，捕捉其对可能输入的信念；形式化自我一致充分性作为自由文本解释的目标；引入信息论度量SCSuff，无需依赖预定义偏见或捷径即可评估自由文本解释；进行实验验证并与针对性扰动测试比较。

Result: SCSuff在适用情况下与针对性扰动测试一致；解释充分性随输入分布变化；LLM解释通常不充分，且与模型规模、准确性或输出熵弱相关；分析最终令牌隐藏状态显示可以从内部表示预测最高和最低SCSuff分数，表明SCSuff可以指导检测和改进充分的LLM解释。

Conclusion: 提出的自我一致充分性框架和SCSuff度量能够有效评估LLM自由文本解释的充分性，为检测和改进解释质量提供了有价值的工具。解释充分性取决于输入分布，且LLM当前解释通常不充分，需要进一步改进。
...
### [129] [Randomized Exploration for Linear Bandits via Absolute Perturbations](https://arxiv.org/abs/2606.28616)
...
### [130] [Improving Patient Subtyping on Longitudinal Data using Representations from Mamba-based Architecture](https://arxiv.org/abs/2606.28623)
...
### [131] [When More Sampling Hurts: The Modal Ceiling and Correlation Ceiling of Test-Time Scaling](https://arxiv.org/abs/2606.28661)
...
### [132] [Closed-Form Steepest Descent Direction toward Flat Minima: Reducing Upper Bounds on the Loss Hessian Eigenspectrum in Neural Networks](https://arxiv.org/abs/2606.28662)
...
### [133] [Entropy Regularized Reinforcement Learning for Zero-Sum Stochastic Differential Games in a Regime-Switching Jump-Diffusion Process](https://arxiv.org/abs/2606.28669)
...
### [134] [Entropy-Regularized Reinforcement Learning for Linear-Quadratic Stackelberg Differential Games in Regime-Switching Diffusion Models](https://arxiv.org/abs/2606.28671)
...
### [135] [Constrained Tabular Diffusion for Finance](https://arxiv.org/abs/2606.28674)
...
### [136] [A Path-Space Formulation of Prediction in World Models: From a Single Action to Prediction, Planning, and Irreversibility](https://arxiv.org/abs/2606.28751)
*Gunn Kim*
...
### [137] [Hierarchical Decision Making with Structured Policies: A Principled Design via Inverse Optimization](https://arxiv.org/abs/2606.28764)
...
### [138] [Generative Learning as a Tool to Improve Perception of Emotional Body Motion Expressions](https://arxiv.org/abs/2606.28769)
...
### [139] [On design-unbiased algorithmic Machine Learning](https://arxiv.org/abs/2606.28795)
...
### [140] [HARD-KV: Head-Adaptive Regularization for Decoding-time KV Compression](https://arxiv.org/abs/2606.28831)
...
### [141] [Active Quantum Kernel Acquisition for Gaussian Process Regression](https://arxiv.org/abs/2606.28833)
...
### [142] [Fisher-Routed Mixture of Experts for Federated Class-Incremental Learning](https://arxiv.org/abs/2606.28835)
...
### [143] [The Contagion Tensor: A Framework for Measuring Output-Distribution Coupling in Multi-Agent LLM Systems -- and Auditing the Claims It Enables](https://arxiv.org/abs/2606.28839)
*Zewen Liu*
...
### [144] [Analysis of Adam Algorithms for Stochastic Dynamic Systems](https://arxiv.org/abs/2606.28879)
...
### [145] [An Integrated Machine Learning and Hierarchical Variance Decomposition Pipeline for Student Performance Prediction and Metacognitive Calibration on Multi-Signal Telemetry](https://arxiv.org/abs/2606.28881)
...
### [146] [MALOQ: Massively Accelerated Learning of Operators for Quantum Transport](https://arxiv.org/abs/2606.28911)
...
### [147] [ML-Powered LDAP Reconnaissance Detection using Weak Supervision](https://arxiv.org/abs/2606.28917)
...
### [148] [Towards Improved Anomaly Detection for Cloud Cybersecurity via Graph Neural Networks](https://arxiv.org/abs/2606.28923)
...
### [149] [Multi-Agent Routing as Set-Valued Prediction: A WildChat Benchmark and Cost-Aware Evaluation](https://arxiv.org/abs/2606.28925)
...
### [150] [DLR: Zero-Inference-Cost Latent Residuals for Low-Rank Pre-Training](https://arxiv.org/abs/2606.28932)
...
### [151] [ReGuide: From Test-Time Guidance to Self-Improving Diffusion Policies](https://arxiv.org/abs/2606.28939)
...
### [152] [Machine-learnable Sets](https://arxiv.org/abs/2606.28947)
...
### [153] [Modification-Considering Value Learning for Reward Hacking Mitigation in RL](https://arxiv.org/abs/2606.28955)
...
### [154] [RGLD: Randomized Global-Local Density Estimation for Tabular Anomaly Detection](https://arxiv.org/abs/2606.28970)
...
### [155] [On Surrogate Modeling of Static Response of AM Short-Fiber Thermoplastics Using Graph Neural Networks](https://arxiv.org/abs/2606.28996)
...
### [156] [How Far Can Sharpness and Complexity Jointly Explain Generalization?](https://arxiv.org/abs/2606.29043)
...
### [157] [MOSAIC: Orchestrating Collaborative Knowledge Tracing with Hierarchical Semantic Alignment](https://arxiv.org/abs/2606.29049)
...
### [158] [A Kernel Fisher Discriminant Analysis-Based Tree Ensemble Classifier: KFDA Forest](https://arxiv.org/abs/2606.29053)
...
### [159] [When Can Conformal Risk Control Certify LLM Outputs? Bounds, Impossibility, and Adaptation for Structured Generation](https://arxiv.org/abs/2606.29054)
*Varun Kotte*
...
### [160] [Statistically Indistinguishable, Operationally Distinct: A Formal Barrier for Tabular Foundation Models](https://arxiv.org/abs/2606.29091)
...
### [161] [Priced Motion Through Optimal Faces: A Normal-Fan Geometry for Non-Stationary Adversarial MDPs](https://arxiv.org/abs/2606.29092)
*Kai Hidajat*
...
### [162] [DiLaServe: High SLO Attainment Serving for Diffusion Language Models](https://arxiv.org/abs/2606.29094)
...
### [163] [Few-Step Boltzmann Generators via Scalable Likelihood Flow Maps](https://arxiv.org/abs/2606.29110)
...
### [164] [A Novel Latent-Class Attack and its Detection by Class Subspace Orthogonalization](https://arxiv.org/abs/2606.29112)
...
### [165] [How Token Influence Decays with Distance: A Green-Function View of Trained Language Models](https://arxiv.org/abs/2606.29139)
...
### [166] [On the Nonlinearity of Learning Rate Scaling for LLM Training](https://arxiv.org/abs/2606.29158)
...
### [167] [GLACIER: Rethinking Mass Spectrum Prediction as an Object Detection Problem](https://arxiv.org/abs/2606.29161)
...
### [168] [Invariant Reasoning Directions in Latent Trajectories of Language Models](https://arxiv.org/abs/2606.29164)
...
### [169] [Symbolic Mechanistic Data Attribution: Tracing Training Influence to Learned Behavioral Policies](https://arxiv.org/abs/2606.29171)
...
### [170] [Dead-Direction Conditioners: Gauge-Equivariant Preconditioning for Deep Networks](https://arxiv.org/abs/2606.29176)
...
### [171] [BaRA: Bayesian Adaptive Rank Allocation for Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2606.29184)
...
### [172] [Representational Depth of Evaluation Awareness Shifts With Scale in Open-Weight Language Models](https://arxiv.org/abs/2606.29196)
*Archit Manek*
...
### [173] [BrainRiem: Riemannian Prototype Learning for Source-Free Cross-Site Brain Network Diagnosis](https://arxiv.org/abs/2606.29200)
...
### [174] [Bayesian Best-Arm Identification with Abstention: A Polynomial-to-Exponential Phase Transition](https://arxiv.org/abs/2606.29203)
...
### [175] [Multi-Block Diffusion Language Models](https://arxiv.org/abs/2606.29215)
...
### [176] [A Linear Matching Bandit Approach to Online Multi-Human Multi-Robot Teaming](https://arxiv.org/abs/2606.29221)
...
### [177] [Depth Exploration for LLM Decoding](https://arxiv.org/abs/2606.29223)
...
### [178] [On the Policy Gradient Foundations of Group Relative Policy Optimization: Credit Assignment, Gradient Sparsity, and Rank Collapse](https://arxiv.org/abs/2606.29238)
...
### [179] [Blackknife: Hard-Label Query-Limited Black-Box Attacks on Heterogeneous Graph Neural Networks](https://arxiv.org/abs/2606.29240)
...
### [180] [Towards Evaluating Data Priors for Tabular Foundation Models](https://arxiv.org/abs/2606.29241)
...
### [181] [KrishokChat: A Citation-Grounded Dataset and Benchmark for Bengali Agricultural Advisory](https://arxiv.org/abs/2606.29243)
...
### [182] [When Prices Double in a Week: Forecasting of Agricultural Volatility in Import-Isolated Markets](https://arxiv.org/abs/2606.29248)
...
### [183] [Learning to Bid in Discriminatory Auctions with Budget Constraints](https://arxiv.org/abs/2606.29252)
...
### [184] [Nonlinear mixture model motivated subspace clustering](https://arxiv.org/abs/2606.29261)
*Ivica Kopriva*
...
### [185] [PCGD: Physics-Guided Conditional Graph Diffusion for TCAD Device Simulation](https://arxiv.org/abs/2606.29272)
...
### [186] [Adaptive Block Diffusion: Resolving Training-Inference Mismatch in Diffusion Language Models](https://arxiv.org/abs/2606.29275)
*Gagan Jain*
...
### [187] [Deterministic Decisions for High-Stakes AI. A Zero-Egress Pipeline with the Deployability of RAG and the Accuracy of Machine Learning](https://arxiv.org/abs/2606.29280)
*Craig Atkinson*
...
### [188] [Beyond Trajectory Matching: Reflow with Marginal Distribution Alignment](https://arxiv.org/abs/2606.29287)
...
### [189] [SP-CACW: Convergence-Aware Client Weighting for Selfish Personalized Learning](https://arxiv.org/abs/2606.29322)
...
### [190] [Deciphering Region-Level Signatures from Latency Measurements in LEO Satellite Internet](https://arxiv.org/abs/2606.29324)
...
### [191] [Sample Complexity of Scientific Discovery: PAC Learnability of Compositional Function Trees](https://arxiv.org/abs/2606.29331)
...
### [192] [AMR: Adaptive Modality Routing for Multimodal Polyglot Speaker Identification](https://arxiv.org/abs/2606.29335)
...
### [193] [Reliability, Faithfulness, and the Limits of Post-hoc Explanations of Opaque Scientific Models](https://arxiv.org/abs/2606.29346)
*Nick Oh,Helen Jin*
...
### [194] [Adaptive Financial Transformer with Regime-Gated Attention for Stock Return Prediction](https://arxiv.org/abs/2606.29347)
*Dishan Sarkar*
...
### [195] [Interventional Flow Matching: Prospective Dose-Response Forecasting with Velocity-Field Jacobian Regularization](https://arxiv.org/abs/2606.29386)
...
### [196] [Temporal Posed and Spontaneous Gesture Recognition from Electromyography in the Rock-Paper-Scissors Game](https://arxiv.org/abs/2606.29423)
...
### [197] [Randomized neural operator for parametric PDEs with fast training and conformal uncertainty quantification](https://arxiv.org/abs/2606.29440)
...
### [198] [Interpretable Inverse Design of Metal-Organic Frameworks with Large Language Model Agents](https://arxiv.org/abs/2606.29459)
...
### [199] [Prototype Latent World Model Replay for Class-Incremental Learning](https://arxiv.org/abs/2606.29465)
...
### [200] [Self-Supervised Calibration of Scientific Instruments Using Physical Consistency Constraints](https://arxiv.org/abs/2606.29466)
...
### [201] [Structured Proper Loss Geometries for Multiclass Classification: Theory and Controlled Empirical Evaluation](https://arxiv.org/abs/2606.29471)
*Soumyadip Sarkar*
...
### [202] [CRAFT: Counterfactual Credit Assignment from Free Sibling Rollouts for Self-Distilled Agentic Reinforcement Learning](https://arxiv.org/abs/2606.29476)
...
### [203] [Reported Confidence in LLMs Tracks Commitment More Than Correctness](https://arxiv.org/abs/2606.29490)
*Dharshan Kumaran*
...
### [204] [Reinforcement Learning in Super Mario Bros: Curriculum, Pedagogy, and Optimal Level Design in World 1-1](https://arxiv.org/abs/2606.29511)
...
### [205] [A Mathematical Optimization Approach for Expert-Informed Bayesian Best Subset Selection](https://arxiv.org/abs/2606.29516)
...
### [206] [Anti-Collapse Dynamics and the Emergence of Multi-Time-Scale Learning in Recurrent Neural Networks](https://arxiv.org/abs/2606.29519)
*Lorenzo Livi*
...
### [207] [Not All Objectives Are Born Equal: Priority-Constrained Descent for Hierarchical Multi-Objective Optimization](https://arxiv.org/abs/2606.29521)
...
### [208] [Do Models Read What They Write? Causal Registers in Scratchpad Reasoning](https://arxiv.org/abs/2606.29522)
...
### [209] [The Mirage of Optimizing Training Policies: Monotonic Inference Policies as the Real Objective for LLM Reinforcement Learning](https://arxiv.org/abs/2606.29526)
...
### [210] [VISTA-DZ: Visual Semantic Trajectory Adaptation for Personalized Dilemma Zone Prediction](https://arxiv.org/abs/2606.29548)
...
### [211] [Optimizer Memory Makes Shuffle Order a First-Order Source of Fine-Tuning Noise](https://arxiv.org/abs/2606.29554)
*John Sweeney*
...
### [212] [Persona-Trained Monte Carlo: Estimating Market-Outcome Distributions via Swarms of Persona-Conditioned Neural Policy Bots in a Limit Order Book](https://arxiv.org/abs/2606.29556)
*Salavat Ishbulatov*
...
### [213] [Speculative Pre-Positioning: Decoding Stateful Sessions to the Next Decision Point Off the Critical Path](https://arxiv.org/abs/2606.29565)
*Victor Norgren*
...
### [214] [The Joint Effect of Quantization and Sampling Temperature on LLM Safety Alignment: A Factorial Analysis](https://arxiv.org/abs/2606.29581)
...
### [215] [Bilevel Optimization for Neural Architecture Search](https://arxiv.org/abs/2606.29582)
...
### [216] [STEMGym: Benchmarking Sequential Decision-Making under Dose Budgets in Autonomous Electron Microscopy](https://arxiv.org/abs/2606.29592)
...
### [217] [How AI settled the complexity of the oldest SGD algorithm](https://arxiv.org/abs/2606.29593)
...
### [218] [Mechanistically Eliciting Latent Behaviors in Language Models](https://arxiv.org/abs/2606.29604)
...
### [219] [Does Role Specialization Matter for Explanation Faithfulness in Mixture-of-Experts?](https://arxiv.org/abs/2606.29613)
...
### [220] [t-STEP: An interpretable model for Total Electron Content predictions and irregularities estimations](https://arxiv.org/abs/2606.29644)
...
### [221] [Fuzzing Large Language Models to Elicit Hidden Behaviours](https://arxiv.org/abs/2606.29646)
...
### [222] [I-BBS: Coordinate-Free Inference of Latent Sub-Manifolds Using Random Distance Matrix Theory](https://arxiv.org/abs/2606.29675)
*Igor Halperin*
...
### [223] [Learning as Observable Matrix Dynamics: Diffusive Relaxations versus Phase Transitions](https://arxiv.org/abs/2606.29679)
*Igor Halperin*
...
### [224] [CAREBench: A Child-Safety Risk Benchmark for Language Models](https://arxiv.org/abs/2606.29685)
...
### [225] [IG-Lens: Exact Additive Probability Attribution Across Transformer Layers via Telescoping Integrated Gradients](https://arxiv.org/abs/2606.29693)
*Duc Anh Nguyen*
...
### [226] [A Diagnostic Framework and Multi-Evaluator Audit of Evaluator-Driven Preference Dynamics in Self-Adapting LLM Agents](https://arxiv.org/abs/2606.29719)
*Liu Zewen*
...
### [227] [The Hidden Cost of Resampling: How Imbalance Correction Degrades Probability Calibration in Tree Ensembles](https://arxiv.org/abs/2606.29720)
*Zewen Liu*
...
### [228] [Redefining Maritime Anomaly Detection via Equation-Grounded Synthetic Anomalies](https://arxiv.org/abs/2606.29721)
...
### [230] [Simplifying Flow Matching Transformations with Low-Rank Mixture Models](https://arxiv.org/abs/2606.29724)
...
### [231] [Optimizing Nursing Care Taxi Dispatch Leveraging Integer Linear Programming Solvers and Machine Learning](https://arxiv.org/abs/2606.29725)
...
### [232] [PS-PPO: Prefix-Sampling PPO for Critic-Free RLHF](https://arxiv.org/abs/2606.29758)
...
### [233] [GLIP: Graph and LLM Joint Pretraining for Graph-Level Tasks](https://arxiv.org/abs/2606.29773)
...
### [234] [Towards Generalizable and Evidential Nuclear Magnetic Resonance-Based Molecular Structure Elucidation via Large Language Model Agent](https://arxiv.org/abs/2606.29776)
...
### [235] [MemLeak: Diagnosing Information Leaks in Multimodal Agent Memory](https://arxiv.org/abs/2606.29788)
...
### [236] [What Drives the Inlier-Memorization Effect? A Theory of Outlier Detection via Early Training Dynamics](https://arxiv.org/abs/2606.29791)
...
### [237] [Accelerating Q-learning through Efficient Value-Sharing across Actions](https://arxiv.org/abs/2606.29806)
...
### [238] [Dual-Flow Reinforcement Learning with State-Aware Exploration](https://arxiv.org/abs/2606.29820)
...
### [239] [The Forgetting-Retention Dilemma: Certified Unlearning Theory in Continual Learning](https://arxiv.org/abs/2606.29832)
...
### [240] [Theory of Continual Learning Against Data Poisoning Attacks](https://arxiv.org/abs/2606.29841)
...
### [241] [Comparing Chatbot Performance Enhanced with Persistent Homology](https://arxiv.org/abs/2606.29857)
...
### [242] [RoAd-RL: A Unified Library and Benchmark for Robust Adversarial Reinforcement Learning](https://arxiv.org/abs/2606.29867)
...
### [243] [Decision-Value Attribution in Predict-then-Optimize Systems](https://arxiv.org/abs/2606.29878)
...
### [244] [Same Concept, Different Directions: Cross-Modal Feature Heterogeneity in Sparse Autoencoders](https://arxiv.org/abs/2606.29888)
...
### [245] [Golden Hour Divide: Trauma Care Accessibility and Resource Vulnerability in Sri Lanka](https://arxiv.org/abs/2606.29889)
...
### [246] [CW-B: Class Weighted Boosting Framework for Imbalance Resilient Multi Class Cardiac Phenotyping](https://arxiv.org/abs/2606.29907)
...
### [247] [EVAF: A Test-Retest Protocol for Selective Parametric Consolidation](https://arxiv.org/abs/2606.29916)
*Haoliang Han*
...
### [248] [Bandwidth Selection in Kernel Density Estimation for Model Calibration](https://arxiv.org/abs/2606.29925)
...
### [249] [Improved Predictive Performance and Interpretability for Mesomorphic Neural Networks Using Local Fidelity Regularization](https://arxiv.org/abs/2606.29951)
...
### [250] [Exploiting Local Flatness for Efficient Out-of-Distribution Detection](https://arxiv.org/abs/2606.29952)
...
### [251] [DuoMem: Towards Capable On-Device Memory Agents via Dual-Space Distillation](https://arxiv.org/abs/2606.29961)
...
### [252] [NeuReasoner: Theory-grounded Mapping of Reasoning Elicitation Boundaries](https://arxiv.org/abs/2606.29971)
...
### [253] [Atompack: A Storage and Distribution Layer for Read-Heavy Atomistic ML Training Datasets](https://arxiv.org/abs/2606.29975)
...
### [254] [Stabilizing Extrapolation in Looped Transformers via Learned Stochastic Stopping](https://arxiv.org/abs/2606.29983)
...
### [255] [T3R: Deeper Test-Time Adaptation for Graph Neural Networks via Gradient Rotation](https://arxiv.org/abs/2606.30011)
...
### [256] [Heads, Not Backbones: Output Heads Dominate Architectures on Fat-Tailed Returns](https://arxiv.org/abs/2606.30037)
...
### [257] [Building Multi-Task Agentic LLMs via Two-Phase Distillation](https://arxiv.org/abs/2606.30044)
...
### [258] [From Failure Taxonomy to Intervention: A Diagnostic Methodology for Industry-Scale AVLM in Video and Live-Streaming Platform Moderation](https://arxiv.org/abs/2606.30059)
...
### [259] [Data-Driven Energy-Based Learning via Gibbs Measures on Hierarchical Structures](https://arxiv.org/abs/2606.30064)
...
### [260] [Neural Subspace Reallocation: Continual Learning as Retrieval-Based Subspace Memory Management](https://arxiv.org/abs/2606.30067)
*Byeong Hoon Yoon*
...
### [261] [Predictive Objectives Discard Exogenous Control-Relevant Features: A Controlled Mechanistic Study](https://arxiv.org/abs/2606.30068)
*Ayan Pendharkar*
...
### [262] [Online Data Selection for Instruction Tuning via Gaussian Processes](https://arxiv.org/abs/2606.30077)
...
### [263] [Query-Aware Spreading Activation for Multi-Hop Retrieval over Knowledge Graphs](https://arxiv.org/abs/2606.30133)
...
### [264] [Robust Strategic Classification under Decision-Dependent Cost Uncertainty](https://arxiv.org/abs/2606.30136)
...
### [265] [Federated Learning with Energy-Based Structured Probabilistic Inference](https://arxiv.org/abs/2606.30161)
...
### [266] [Beyond Drug Discovery: The Nanotechnology Molecular Optimization (NMO) Benchmark](https://arxiv.org/abs/2606.30170)
...
### [267] [Characterizing Optimizer-Dependent Training Dynamics Through Hessian Eigenvector Displacement and Localization](https://arxiv.org/abs/2606.30226)
...
### [268] [B3O: Scalable Boltzmann Batch Bayesian Optimization](https://arxiv.org/abs/2606.30228)
...
### [269] [Curvature-Guided Sheaf Diffusion for Unsupervised Community Detection on Heterophilic Graphs](https://arxiv.org/abs/2606.30249)
*Feifan Wang*
...
### [270] [KnowsTFM: Knowledge-Informed Fine-Tuning of Small Tabular Foundation Models](https://arxiv.org/abs/2606.30258)
...
### [271] [When Is a Draft Accepted? A Theory of Acceptance in Speculative Decoding](https://arxiv.org/abs/2606.30265)
*Aaryam Sharma*
...
### [272] [Towards Continual Motion-Language Agents: LoRA Variants for Incremental Motion Understanding and Generation](https://arxiv.org/abs/2606.30266)
...
### [273] [DreamForge-World 0.1 Preview: A Low-Compute Real-Time Controllable World Model](https://arxiv.org/abs/2606.30292)
...
### [274] [Toward an Energy-Optimized Operation of Data Centers Located in Wind Farms Using Reinforcement Learning](https://arxiv.org/abs/2606.30316)
...
### [275] [Hybrid Active-Online Learning Framework for Label-Efficient Concept Drift Adaptation in Optical Network Failure Detection](https://arxiv.org/abs/2606.30322)
...
### [276] [FlexTab: A Flexible Encoder-Decoder Architecture for In-Context Learning Across Diverse Tabular Tasks](https://arxiv.org/abs/2606.30336)
...
### [277] [DRIFT: Difficulty Routing Self-DIstillation with Rhythm-Gated Exploration and Success BuFfer Training](https://arxiv.org/abs/2606.30345)
...
### [278] [On the Vulnerability of Parameter-Level Defenses to Model Merging](https://arxiv.org/abs/2606.30360)
...
### [279] [FlowAWR: Online Adaptive Flow Reinforcement via Advantage-Weighted Rectification](https://arxiv.org/abs/2606.30376)
...
### [280] [Scalar Representations of Neural Network Training Dynamics](https://arxiv.org/abs/2606.30384)
...
### [281] [Predict, Reuse, and Repair: Accelerating Dynamic Sparse Attention for Long-Context LLM Decoding](https://arxiv.org/abs/2606.30389)
...
### [282] [Beyond IID: How General Are Tabular Foundation Models, Really?](https://arxiv.org/abs/2606.30410)
...
### [283] [Diffusion Fine-tuning with Rewarded Moment Matching Distillation](https://arxiv.org/abs/2606.30414)
...
### [284] [Experience Augmented Policy Optimization for LLM Reasoning](https://arxiv.org/abs/2606.30420)
...
### [285] [Proofs of Ownership for Machine Learning Models](https://arxiv.org/abs/2606.30423)
...
### [286] [Arko-T: A Foundation Model for Text-to-Structured 3D Generation](https://arxiv.org/abs/2606.30429)
...
### [287] [Transformer Architectures as Complete Bayes Processes: A Formal Proof in the Measure-Theoretic Kernel Framework](https://arxiv.org/abs/2606.30440)
*Haobo Yang*
...
### [288] [Informational Frustration in Neural Manifolds: Shannon Bottlenecks and the Limits of Learnability](https://arxiv.org/abs/2606.30512)
...
### [289] [When Does Online Imitation Learning Help in LLM Post-Training? The Role of (Non-)Realizability Beyond Horizon](https://arxiv.org/abs/2606.30445)
...
### [290] [TraceLab: Characterizing Coding Agent Workloads for LLM Serving](https://arxiv.org/abs/2606.30560)
...
### [291] [Internal-State Probes Read the Situation, Not the Action: Three Negative Results for Pre-Action Misalignment Monitoring](https://arxiv.org/abs/2606.30449)
...
### [292] [C$^{2}$R: Cross-sample Consistency Regularization Mitigates Feature Splitting and Absorption in Sparse Autoencoders](https://arxiv.org/abs/2606.30609)
...
### [293] [Exploring Differences Between Tabular Enterprise Data and Public Benchmarks](https://arxiv.org/abs/2606.30452)
...
### [294] [Pessimism's Paradox: Conservative Offline Training Amplifies Reward Hacking During Online Adaptation in Reasoning Models](https://arxiv.org/abs/2606.30627)
...
### [295] [Curvature-Weighted Gradient Diversity: A Noise Measure for Geometry-Adaptive SGD Schedules](https://arxiv.org/abs/2606.30455)
...
### [296] [HSAP: A Hierachical Sequence-aware Parallelism for Hybrid-Context Generative Models](https://arxiv.org/abs/2606.30460)
...
### [297] [MuonSSM: Orthogonalizing State Space Models for Sequence Modeling](https://arxiv.org/abs/2606.30461)
...
### [298] [Discovering Collaboration from Novelty: Random Network Distillation for Clustered Federated Learning](https://arxiv.org/abs/2606.30499)
...
### [299] [Muon learns balanced solutions in matrix factorization without slow saddle-to-saddle dynamics](https://arxiv.org/abs/2606.30509)
...
### [300] [ITSPACE: Monotone Gaussian Optimal Transport Updates](https://arxiv.org/abs/2606.30523)
...
### [301] [Convergence of Continual Learning in Homogeneous Deep Networks](https://arxiv.org/abs/2606.30559)
...
### [302] [Attractor States Emerge in Multi-Turn LLM Conversations](https://arxiv.org/abs/2606.30571)
...
### [303] [SWE-INTERACT: Reimagining SWE Benchmarks as User-Driven Long-Horizon Coding Sessions](https://arxiv.org/abs/2606.30573)
...
### [304] [The Fundamental Limits of Valid Transport Map Estimation](https://arxiv.org/abs/2606.30574)
...
### [305] [One-Step Gradient Delay is Not a Barrier for Large-Scale Asynchronous Pipeline Parallel LLM Pretraining](https://arxiv.org/abs/2606.30634)
