...
TL;DR: 本研究开发了一个深度学习框架，直接从下肢步态运动学预测髋部肌肉力量和关节力矩，比较了三种序列模型（LSTM、Transformer、Mamba），发现Transformer在健康受试者和股骨头坏死患者的外部验证中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 传统的髋部肌肉力量和关节力矩估计依赖于肌肉骨骼模拟，这种方法虽然信息丰富但耗时且难以在临床环境中应用。因此需要开发更快速、实用的替代方法。

Method: 收集60名健康成年人在三种节奏条件下的步态数据，使用10个双侧下肢关节角度作为输入，OpenSim推导的髋部肌肉力量和关节力矩作为参考输出。在相同的数据分割、预处理管道和评估指标下训练并比较LSTM、Transformer和Mamba三种深度学习模型。最佳模型随后在9名股骨头坏死患者的外部队列中进行零样本测试。

Result: 在健康受试者基准测试中，Transformer在髋部肌肉力量预测（RMSE = 1.33 N/kg, MAE = 0.57 N/kg, R2 = 0.819）和髋关节力矩预测（RMSE = 0.11 Nm/kg, MAE = 0.07 Nm/kg, R2 = 0.862）方面表现最佳。在外部验证中，Transformer对股骨头坏死患者的髋部肌肉力量预测（RMSE = 1.51 N/kg, MAE = 0.70 N/kg, R2 = 0.537）和髋关节力矩预测（RMSE = 0.17 Nm/kg, MAE = 0.12 Nm/kg, R2 = 0.569）仍保持中等预测能力。

Conclusion: 该研究支持了从步态运动学估计髋部动力学的可行性，确定Transformer作为强有力的基线模型，但在临床应用前需要进行更广泛的病理学验证和改进模型的泛化能力。
...
TL;DR: Unicorn（Universal Correlation Network）是一个用于高维时间序列多数据集预训练的可扩展框架，通过潜在原型码本解耦相关性建模与通道身份，在异构数据集间学习可重用交互模式，显著优于现有预测架构，特别在少样本迁移场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现代时间序列架构面临根本性权衡：通道无关模型随数据量增加扩展性好但忽略关键通道间依赖，而通道相关模型表达能力强但受维度限制，难以在异构数据集间泛化。需要弥合这一鸿沟。

Method: Unicorn采用潜在原型码本，将异质通道投影到共享潜在空间，解耦相关性建模与特定通道身份，学习身份无关、可重用的交互模式。这种设计允许模型在不同维度特征和语义的数据集间迁移。

Result: 大量实验表明，Unicorn显著优于最先进的预测架构，特别是在少样本迁移场景中表现优异。该框架为构建多元时间序列基础模型提供了可扩展路径。

Conclusion: Unicorn通过创新的潜在原型码本设计，成功解决了时间序列模型中扩展性与表达能力的权衡问题，为开发可扩展的多元时间序列基础模型提供了有效方案，在跨域迁移学习中展现出显著优势。
...
TL;DR: 本研究通过多模型实验，发现在LoRA微调下，线性探针可在早期层（1-3层）近乎完美地检测合成欺骗，且这种欺骗表示具有跨领域泛化能力和鲁棒性，支持线性表示假说。


<details>
  <summary>Details</summary>
Motivation: 欺骗对齐是AI安全的核心挑战，虽然战略欺骗是主要长期担忧，但通过直接优化错误答案诱导的合成欺骗为研究学习到的欺骗的表示基础提供了可控测试平台。

Method: 引入多模型范式，对五个transformer模型（Pythia-1.4B, Gemma-2-2B/9B, Qwen2.5-7B, Llama-3.1-8B）使用LoRA在同一问题分布上进行诚实和欺骗变体的微调。使用线性探针在平均池化的隐藏状态上检测合成欺骗，并进行机理分析包括费舍尔判别比、有效秩、质心几何、方向稳定性、跨域对齐和校准等。

Result: 线性探针在四个架构的1-3层能以接近完美的AUC（≥0.99）检测合成欺骗；逻辑回归探针一致匹配或优于MLP探针；基于TruthfulQA训练的探针在MMLU测试集上泛化损失接近零；Gemma-2模型展现出异常稳定性；欺骗方向在更深层逐步巩固；在1-4层可实现最佳校准（除Pythia外ECE<0.01）。

Conclusion: 研究结果表明，通过适度的监督微调，鲁棒、领域不变的欺骗表示可以快速巩固，这对基于激活的监控具有重要意义，揭示了两种表示机制：Pythia/Llama/Qwen中的表示崩溃与Gemma-2中的高维保持。
...
TL;DR: 该论文提出了一种新的海事AIS数据异常检测系统框架，引入了无监督的质量评估指标MADQI，无需标注数据即可评估异常检测模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有无监督学习算法（如Isolation Forest）在海事AIS异常检测中缺乏系统性和有意义的质量评估指标，无法有效评价异常检测性能。

Method: 提出Maritime Anomaly Detection Quality Index (MADQI)框架，整合四个相互关联的指标：异常率一致性(ARC)、物理合理性评分(PPS)、分数分布分离(SDS)和极端案例证据(ECE)，采用Haversine距离计算分析AIS数据。

Result: 在AIS数据集上实验显示，该框架获得80.37%的MADQI分数，其中ECE和ARC分别达到0.907和1.000的高分，表明在检测极端异常和保持异常率一致性方面表现优秀。

Conclusion: 该框架为海事AIS数据中的无监督异常检测提供了可靠且有意义的评估方法，能够有效识别船舶的速度、位置跳跃、时间间隔和转弯角度等异常行为。
...
TL;DR: 论文提出NumLeak框架，用于检测LLM是否记忆而非真正理解公开数值基准数据，发现顶级LLM能高精度回忆金融市场数据，且通过实验验证这是记忆而非学习模式


<details>
  <summary>Details</summary>
Motivation: 公开数值基准数据出现在预训练中，如果评估时依赖日期信息，可能是在测量记忆回忆而非样本外技能。需要区分模型是真正理解还是仅仅记忆数据

Method: 提出NumLeak测量框架，结合生产模型的API边界探测和白盒控制验证。使用相关性分析、拒绝率分析、logprob排序、残差分析等方法检测记忆模式

Result: 顶级LLM能准确回忆Fama-French市场超额回报（r=0.97-0.99），失业率、CPI通胀、温度数据也有类似保真度。模型在近期数据上解析率下降但相关性仍高，验证了记忆通道存在。系统提示防御可99.8%阻止非自适应单轮后缀攻击

Conclusion: LLM确实记忆而非真正学习公开数值基准数据，传统闭API黑盒探测低估了记忆通道的影响。简单的系统提示防御可有效缓解此问题，对概念和历史叙述查询的效用成本接近零
...
### [6] [Calibrated Preference Learning: The Case of Label Ranking](https://arxiv.org/abs/2605.30447)
*Santo M. A. R. Thies,Viktor Bengs,Timo Kaufmann,Sebastian J. Vollmer,Eyke Hüllermeier*

Main category: cs.LG

TL;DR: 本文首次形式化地提出了概率标签排序的校准问题，建立了涵盖完整排序、子排序和top-k排序的校准概念层次结构，并在实证中发现现有模型校准效果不佳，且在强化学习人类反馈奖励模型中发现校准与基准准确率相关但不完全一致。


<details>
  <summary>Details</summary>
Motivation: 虽然分类和回归中的校准问题已得到广泛研究，但在概率标签排序领域，校准问题尚未得到正式解决。将排序简单视为类别会忽略其结构特征，且无法捕捉重要的预测模态如成对预测和top-k预测。

Method: 形式化定义了标签排序的校准概念，建立了包括完整排序、子排序和top-k排序的校准层次结构。通过理论分析证明了完整排序校准蕴含其他校准概念但反之不成立，同时证明子排序校准和top-k排序校準是不可比较的。在实证研究中，评估了流行标签排序模型的校准性能，并将框架应用于RLHF奖励模型。

Result: 理论分析表明完整排序校准是最强的校准概念，它蕴含子排序和top-k校准，但子排序和top-k校准相互独立。实证发现流行的标签排序模型通常校准效果不佳，且子排序和top-k校准指标之间存在显著差异。在RLHF奖励模型中，校准与基准准确率存在强相关但不完全一致，表明校准捕捉了超越top-1准确率的质量维度。

Conclusion: 本研究填补了概率标签排序中校准研究的空白，提出的校准框架为评估和改进标签排序模型的可靠性提供了理论基础。校准作为超越传统准确率指标的质量维度，值得进一步研究其在下游任务中的影响，并开发相应的校准校正方法。
...
### [7] [Bounded Behavioral Indistinguishability for Black-Box LLM Distillation](https://arxiv.org/abs/2605.30448)
*Munawar Hasan*

Main category: cs.LG

TL;DR: 论文提出了行为不可区分性（bounded behavioral indistinguishability）的概念，用于评估黑盒LLM蒸馏效果，而不仅仅是输出相似性。通过在Qwen和Llama师生对上实验，发现LoRA蒸馏提高了语义相似度，但对抗性评估仍能检测到行为差异，主要集中在风格/格式、鲁棒性和领域技术提示方面。


<details>
  <summary>Details</summary>
Motivation: 当前黑盒LLM蒸馏通常被评估为输出匹配问题，即学生的响应与老师的语义相似度。然而，输出相似性并不意味着学生在行为上与模仿的老师模型不可区分。需要更全面的评估方法。

Method: 提出了$(ε,q,t,\mathbb{A})$-行为不可区分性的形式化定义，包含区分优势界限ε、查询数界限q、计算界限t和对抗者类别𝔸。使用包含5000个提示的行为探针套件在Qwen和Llama师生对上实例化这一概念，比较基础学生模型和LoRA蒸馏学生模型，进行对抗性评估和类别分析。

Result: LoRA蒸馏将Qwen的语义相似度从0.788提高到0.862，Llama从0.814提高到0.874。但对抗性评估发现仍然存在行为差异：学习到的判别器保持非零优势，成对类别分析显示差异集中在风格/格式、鲁棒性和领域技术提示。使用不同家族的Llama判别器和A/B交换一致性过滤后，Qwen的区分优势从基础学生的0.158降至LoRA蒸馏后的0.081。查询预算实验表明基于分歧的获取并不总是优于分层随机采样。

Conclusion: 语义保真度有用但不足够：黑盒LLM蒸馏需要有限制性、对抗性和类别感知的评估。行为不可区分性提供比单纯输出相似性更全面的评估框架。
...
### [8] [VeriGate: Verifier-Gated Step-Level Supervision for GRPO](https://arxiv.org/abs/2605.30451)
*Aakriti Agrawal,Minghui Liu,Furong Huang*

Main category: cs.LG

TL;DR: VeriGate是一种改进GRPO的方法，通过验证器门控机制结合结果奖励和过程监督，解决了GRPO中梯度消失和信用分配问题，显著提升了推理模型的性能。


<details>
  <summary>Details</summary>
Motivation: GRPO在训练推理模型时存在两个主要问题：1）当所有采样轨迹获得相同验证器奖励时，组相对优势归零导致学习停滞；2）仅使用结果奖励无法提供步级信用分配，限制了探索和鲁棒推理的学习。需要一种能同时利用结果验证和过程监督的方法。

Method: VeriGate通过三个关键设计：1）验证器门控机制，根据验证器奖励的区分度动态选择使用结果奖励或过程监督；2）将过程奖励模型的步级分数转换为未来累积奖励，实现延续感知的信用分配；3）将这些奖励转换为组归一化的令牌级优势，恢复信息梯度并保持细粒度信用分配。

Result: 在MATH数据集上使用1.5B和7B Qwen2.5-Instruct模型进行训练，并在六个推理基准上评估，VeriGate将平均准确率分别提高了约20%和12%，显著减少了零梯度失败，降低了奖励破解行为，并提高了推理质量。

Conclusion: VeriGate有效解决了GRPO的稀疏监督和信用分配问题，通过验证器门控机制和过程监督的智能结合，显著提升了推理模型的性能和鲁棒性，为基于验证器的强化学习方法提供了重要改进。
...
### [9] [A Unified Framework for Gradient Aggregation in Multi-Objective Optimization](https://arxiv.org/abs/2605.30452)
*Zeou Hu,Kelvin Ho,Yaoliang Yu*

Main category: cs.LG

TL;DR: 提出一个统一的多目标优化梯度聚合框架，建立帕累托平稳的收敛速率，引入充分对齐条件和非冲突方向理论，扩展可行性保证，并通过原始优化视角涵盖现有算法，设计新的 capped MGDA 方法


<details>
  <summary>Details</summary>
Motivation: 现有的多目标优化方法动机各异、分析个案化、梯度聚合方式不同，缺乏统一的理论框架和收敛保证

Method: 建立梯度聚合的统一框架，引入充分对齐条件证明非冲突方向是收敛的基本条件，通过双重锥投影确保可行性，提出原始优化视角涵盖现有算法，设计 capped MGDA（基于 CVaR）新方法

Result: 建立了帕累托平稳的收敛速率理论，证明非冲突方向构成收敛的充分条件，扩展了可行性保证范围，涵盖了现有算法并揭示其理论关系，设计了 capped MGDA 并在对抗联邦学习中验证了鲁棒性

Conclusion: 该工作为多目标优化梯度聚合提供了统一的理论框架，建立了收敛保证，扩展了方法适用范围，并通过原始优化视角实现了算法设计与分析，capped MGDA 在对抗联邦学习中展现出鲁棒性优势
...
### [10] [Scalable Constrained Multi-Agent Reinforcement Learning via State Augmentation and Consensus for Separable Dynamics](https://arxiv.org/abs/2605.30461)
...
### [11] [idSCD: Identifying Training Datasets through Semantic Correlation Descriptors](https://arxiv.org/abs/2605.30462)
...
### [12] [Can Subgraph Explanations Be Weaponized to Steal Graph Neural Networks?](https://arxiv.org/abs/2605.30470)
...
### [13] [Universal Multiclass Transductive Online Learning](https://arxiv.org/abs/2605.30479)
...
### [14] [Discovering a Zeta Map Algorithm on Dyck Paths via Mechanistic Interpretability](https://arxiv.org/abs/2605.30482)
...
### [15] [Graph-Conditioned Mixture of Graph Neural Network Experts for Traffic Forecasting](https://arxiv.org/abs/2605.30486)
...
### [20] [The Long-Term Effects of Data Selection in LLM Fine-Tuning](https://arxiv.org/abs/2605.30537)
...
### [25] [Supervised Training Rapidly Degrades Early Visual Cortex Alignment Across Biologically Plausible Learning Rules](https://arxiv.org/abs/2605.30556)
*Nils Leutenegger*
...
### [27] [Benchmarking Machine Learning Uncertainty Quantification Methodologies for Predicting Turbine Gas Temperature Degradation](https://arxiv.org/abs/2605.30585)
...
### [29] [Learning Transferable Predictability Representations](https://arxiv.org/abs/2605.30592)
...
### [30] [Scientific Machine Learning for Engine Health Management and Remaining Useful Life Prediction](https://arxiv.org/abs/2605.30593)
*Jostein Barry-Straume,Changmin Son,Adrian Sandu,Gavan Burke,Rekha Sundararajan,Andrew Rimell,James G. Steinrock*
...
### [31] [Improving Relative Representations with Learned Anchors and Whitened Inner Products](https://arxiv.org/abs/2605.30596)
...
### [35] [TASER: Task-Aware Stein Regularisation for Geometry-Driven Robustness](https://arxiv.org/abs/2605.30601)
...
### [36] [Constrained Flow Optimization via Sequential Fine Tuning for Molecular Design](https://arxiv.org/abs/2605.30610)
...
### [37] [Improving Selective Classification with Pairwise Queries for Binary Classification](https://arxiv.org/abs/2605.30615)
...
### [38] [Active Timepoint Selection for Learning Measure-Valued Trajectories](https://arxiv.org/abs/2605.30625)
...
### [39] [CellBRIDGE: Learning Cellular Trajectories via Interaction-Aware Alignment](https://arxiv.org/abs/2605.30635)
...
### [40] [Score Broadcast and Decorrelation: A General Framework for Broadcast-Based Credit Assignment](https://arxiv.org/abs/2605.30638)
...
### [41] [CSULoRA: Closest Safe Update Low-Rank Adaptation](https://arxiv.org/abs/2605.30640)
...
### [42] [Diffusion Models Preferentially Memorize Prototypical Examples or: Why Does My Diffusion Model Love Slop?](https://arxiv.org/abs/2605.30642)
...
### [43] [Convergence of Steepest Descent and Adam under Non-Uniform Smoothness](https://arxiv.org/abs/2605.30648)
...
### [44] [LARK: Learnability-Grounded Trajectory Selection for Efficient Reasoning Distillation](https://arxiv.org/abs/2605.30651)
...
### [45] [Bridging the Gap Between Natural Language and Market Dynamics via High-Dimensional Representation Learning](https://arxiv.org/abs/2605.30652)
...
### [46] [Learning to Perceive the World Through Control: Empowerment-Based Representation Learning](https://arxiv.org/abs/2605.30656)
...
### [47] [BOKBO (Best of K Bad Options): Calibrated Abstention for VLA Policies](https://arxiv.org/abs/2605.30660)
...
### [49] [Spatio-temporal stochastic graph-based learning for infectious disease forecasting](https://arxiv.org/abs/2605.30662)
...
### [50] [Kalimati Vegetable Price Index Forecasting with a Momentum Corrected Online Stacking Ensemble](https://arxiv.org/abs/2605.30720)
*Sahaj Raj Malla*
...
### [51] [Universal Decision Learners](https://arxiv.org/abs/2605.30694)
*Sridhar Mahadevan*
...
### [53] [A Context-Aware Middleware for Medical Image Based Reports: An approach based on image feature extraction and association rules](https://arxiv.org/abs/2605.30699)
...
### [56] [Unlearning in Diffusion Models: A Unified Framework with KL Divergence and Likelihood Constraints](https://arxiv.org/abs/2605.30825)
...
### [61] [Reducing the GPU Memory Bottleneck with Lossless Compression for ML -- Extended](https://arxiv.org/abs/2605.30728)
...
### [62] [De-attribute to Forget for LLM Unlearning](https://arxiv.org/abs/2605.30919)
...
### [63] [DEM: A Distilled Explanation Model for Interpretable Anomaly Detection in Physiological Sensor Networks](https://arxiv.org/abs/2605.31007)
...
### [64] [Beyond Accuracy: Evaluating Efficiency, Robustness and Explainability in Deep Learning for Malaria Diagnosis](https://arxiv.org/abs/2605.30734)
...
### [65] [Annealed Softmax Greedy in Many-Armed Bayesian Bandits](https://arxiv.org/abs/2605.31034)
...
### [66] [Learning to Solve and Optimize by Evolving Code](https://arxiv.org/abs/2605.31049)
...
### [67] [FLAG: Flow Policy MaxEnt-RL by Latent Augmented Guidance](https://arxiv.org/abs/2605.30749)
...
### [68] [STEP: Learning STructured Embeddings for Progressive Time Series](https://arxiv.org/abs/2605.31061)
...
### [69] [Chain-of-Thought and Compressed Looped Transformers: A Memory-Budget Separation](https://arxiv.org/abs/2605.30757)
*Haozhou Zhang*
...
### [70] [Trust-Region Behavior Blending for On-Policy Distillation](https://arxiv.org/abs/2605.31159)
...
### [71] [Efficient and Uncertainty-Aware Diffusion Framework for Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2605.30776)
...
### [72] [What changes after deployment? A survey on On-device Learning in TinyML](https://arxiv.org/abs/2605.31226)
...
### [73] [AbstainGNN: Teaching Graph Neural Networks to Abstain for Graph Classification](https://arxiv.org/abs/2605.30786)
...
### [74] [EchoRL: Reinforcement Learning via Rollout Echoing](https://arxiv.org/abs/2605.31228)
...
### [75] [Learning Cardiac Latent Representations in Vectorcardiogram Space](https://arxiv.org/abs/2605.31249)
...
### [76] [Conformal Reliability: A New Evaluation Metric for Conditional Generation](https://arxiv.org/abs/2605.30807)
...
### [77] [Why Linear Recurrent Memory Works in Partially Observable Reinforcement Learning](https://arxiv.org/abs/2605.31261)
...
### [79] [The Terminal Representation in Reinforcement Learning](https://arxiv.org/abs/2605.31289)
...
### [80] [Non-destructive Identification of Oyster Species is possible from Hyperspectral Images with Machine Learning](https://arxiv.org/abs/2605.30811)
...
### [81] [Inconsistency-Aware Minimization: Improving Generalization with Unlabeled Data](https://arxiv.org/abs/2605.31324)
...
### [82] [Learning Permutation-invariant Macroscopic Dynamics](https://arxiv.org/abs/2605.30812)
...
### [83] [dashi: A Python library for Dataset Shift Characterization to Support Trustworthy AI Development and Deployment](https://arxiv.org/abs/2605.31360)
...
### [84] [Scaling Higher-Order Graph Learning with Maximal Clique Complexes](https://arxiv.org/abs/2605.31373)
...
### [85] [Cross-Layer Subspace Coupling for LLM Compression: A Unifying Framework and Its Empirical Limits](https://arxiv.org/abs/2605.30836)
...
### [86] [PithTrain: A Compact and Agent-Native MoE Training System](https://arxiv.org/abs/2605.31463)
...
### [87] [CoMem: Context Management with A Decoupled Long-Context Model](https://arxiv.org/abs/2605.30842)
...
### [88] [GPU Forecasters: Language Models as Selective Surrogates for Kernel Runtime Optimization](https://arxiv.org/abs/2605.31464)
...
### [89] [A Lecture Note on Offline RL and IRL, Part II: Foundations of Inverse Reinforcement Learning and Dynamic Discrete Choice Models](https://arxiv.org/abs/2605.30843)
*Enoch Hyunwook Kang*
...
### [90] [On Efficient Scaling of GNNs via IO-Aware Layers Implementations](https://arxiv.org/abs/2605.31500)
...
### [91] [ForecastCompass: Guiding Agentic Forecasting with Adaptive Factor Memory](https://arxiv.org/abs/2605.30858)
...
### [92] [Skill Reuse as Compression in Agentic RL](https://arxiv.org/abs/2605.31509)
...
### [93] [Positional versus Symbolic Attention Heads: Learning Dynamics, RoPE Geometry, and Length Generalization](https://arxiv.org/abs/2605.31558)
...
### [94] [GlucoFM: A Dual-Stream Foundation Model for Continuous Glucose Monitoring](https://arxiv.org/abs/2605.30865)
...
### [95] [Bandwidth Allocation with Device Partitioning for Federated Learning over Industrial IoT networks](https://arxiv.org/abs/2605.30892)
...
### [96] [Zero Collapse: A Failure Mode of Policy Gradient Methods in Discontinuous Reward Environments](https://arxiv.org/abs/2605.30896)
...
### [97] [Density-Guided Robust Counterfactual Explanations on Tabular Data under Model Multiplicity](https://arxiv.org/abs/2605.30901)
...
### [98] [PINNs Failure Modes are Overfitting](https://arxiv.org/abs/2605.30910)
...
### [99] [Automating Formal Verification with Reinforcement Learning and Recursive Inference](https://arxiv.org/abs/2605.30914)
*Max Tan*
...
### [100] [Welfare, Improvability, and Variance: A Principal-Agent Approach to Optimal Benchmark Item Aggregation](https://arxiv.org/abs/2605.30916)
...
### [101] [Unsupervised Diffusion Solver for Combinatorial Optimization via Combinatorial Adjoint Matching](https://arxiv.org/abs/2605.30920)
...
### [102] [Local linear convergence of gradient methods for overparameterized Gaussian mixtures](https://arxiv.org/abs/2605.30936)
...
### [103] [Spectral Anatomy of Quantum Gaussian Process Kernels](https://arxiv.org/abs/2605.30952)
...
### [104] [Revisiting Zeroth-Order Hessian Approximation: A Single-Step Policy Optimization Lens](https://arxiv.org/abs/2605.30960)
...
### [105] [Learning Multi-Agent Coordination via Sheaf-ADMM](https://arxiv.org/abs/2605.31005)
...
### [106] [Physics-Informed Coarsening for Multigrid Graph Neural Surrogates](https://arxiv.org/abs/2605.31013)
...
### [107] [An Efficient and Scalable Graph Condensation with Structure-Preserving](https://arxiv.org/abs/2605.31016)
...
### [108] [Augmented Lagrangian Predictive Coding](https://arxiv.org/abs/2605.31022)
...
### [109] [Multi-Scale Separable Fourier Neural Networks for Solving High-Frequency PDEs](https://arxiv.org/abs/2605.31027)
...
### [110] [UniRTL: Unifying Code and Graph for Robust RTL Representation Learning](https://arxiv.org/abs/2605.31040)
...
### [111] [The Challenges of Using Reinforcement Learning for Controlling Industrial Energy Systems](https://arxiv.org/abs/2605.31044)
...
### [112] [Best-Arm Identification-Based Trust Region Selection for Bayesian Optimization on Multimodal Functions](https://arxiv.org/abs/2605.31050)
...
### [113] [Learning to Bid in FCR Markets: A Best-of-Both-Worlds Approach](https://arxiv.org/abs/2605.31070)
...
### [114] [Riemannian Diffusion Models on General Manifolds via Physics-Informed Neural Networks](https://arxiv.org/abs/2605.31106)
...
### [115] [Subspace-Decomposed JEPAs: Disentangling Progression and Content in Latent World Models](https://arxiv.org/abs/2605.31111)
*Lucas Thil,Jesse Read,Rim Kaddah,Guillaume Doquet*
...
### [116] [Scalable Bayesian Inference for Nonlinear Conservation Laws](https://arxiv.org/abs/2605.31127)
...
### [117] [Generalizing Multi-Scale Time-Series Modeling with a Single Operator](https://arxiv.org/abs/2605.31129)
...
### [118] [Learning Hyperspherical Time-Frequency Representations for Time-Series Out-of-Distribution Detection](https://arxiv.org/abs/2605.31155)
...
### [119] [TabCausal: Pretraining Across Causal Environments for Tabular Causal Discovery](https://arxiv.org/abs/2605.31156)
...
### [120] [Convergence of Two-Timescale Markovian Stochastic Approximations with Applications in Reinforcement Learning](https://arxiv.org/abs/2605.31172)
...
### [121] [Retriever Portfolios: A Principled Approach to Adaptive RAG](https://arxiv.org/abs/2605.31176)
...
### [122] [How well does Classification Accuracy capture Concept Drift Detection Quality? An overview of Concept Drift Detection evaluation](https://arxiv.org/abs/2605.31186)
*Joanna Komorniczak*
...
### [123] [FlagGAM: Rule-Based Generalized Additive Modeling for Explainable Tabular Prediction](https://arxiv.org/abs/2605.31189)
...
### [124] [Student Capacity Moderates Knowledge Distillation Effectiveness: A Systematic Study Across ResNet Teacher-Student Pairs on CIFAR-10](https://arxiv.org/abs/2605.31191)
*Umut Onur Yasar*
...
### [125] [Geometry-based Schrödinger Bridges for Trustworthy Multimodal Fusion](https://arxiv.org/abs/2605.31193)
...
### [126] [Beyond Additive Decompositions: Interpretability Through Separability](https://arxiv.org/abs/2605.31200)
...
### [127] [Fixed-Point Masked Generative Modeling](https://arxiv.org/abs/2605.31215)
...
### [128] [Multivariate Distributional Reinforcement Learning Using Sliced Divergences](https://arxiv.org/abs/2605.31222)
...
### [129] [Bifurcated Remaining Useful Life Prediction: A Hybrid Approach for Realistic Uncertainty Characterization](https://arxiv.org/abs/2605.31241)
...
### [130] [Spectral Reach: Understanding Neural Scaling as Progress into the Spectral Tail](https://arxiv.org/abs/2605.31244)
...
### [131] [Toward Identifiable Sparse Autoencoders](https://arxiv.org/abs/2605.31245)
...
### [132] [Fraud Type Decomposition and the Observation-Mechanism Taxonomy:Class-Specific Detection Limits in Payment Networks](https://arxiv.org/abs/2605.31257)
*Gaurav Dhama*
...
### [133] [Lightweight CNN-Based Anomaly Detection for High Voltage Converter Modulators in the Spallation Neutron Source](https://arxiv.org/abs/2605.31259)
...
### [134] [Algorithmic Recourse of In-Context Learning for Tabular Data](https://arxiv.org/abs/2605.31272)
...
### [135] [Survival Reinforcement Learning: Toward Scalable Self-Supervised RL](https://arxiv.org/abs/2605.31273)
...
### [136] [Learning Parametric Nitrogen Fertilizer Response Curves Using Neuro Symbolic Regression](https://arxiv.org/abs/2605.31276)
...
### [137] [Interpretability Without Tradeoffs: Disentangling Polysemanticity At Equal Predictive Performance](https://arxiv.org/abs/2605.31304)
...
### [138] [Non-Asymptotic Convergence of Stochastic Iterative Algorithms: A Lyapunov Framework](https://arxiv.org/abs/2605.31309)
...
### [139] [Graph Neural Networks Are Not Continuous Across Graph Resolutions](https://arxiv.org/abs/2605.31315)
...
### [140] [Forgetting Has Neighbors: Localized Collateral Forgetting in Machine Unlearning](https://arxiv.org/abs/2605.31317)
...
### [141] [Generalized Intention Modeling in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2605.31318)
...
### [142] [Trading Complexity for Expressivity Through Structured Generalized Linear Token Mixing](https://arxiv.org/abs/2605.31367)
...
### [143] [A Unifying View of Variational Generative Wasserstein Flows](https://arxiv.org/abs/2605.31369)
...
### [144] [Softsign: Smooth Sign in Your Optimizer For Better Parameter Heterogeneity Handling](https://arxiv.org/abs/2605.31371)
...
### [145] [Constrained Multi-Objective Reinforcement Learning with Max-Min Criterion](https://arxiv.org/abs/2605.31388)
...
### [146] [Fixed Universal Transformers](https://arxiv.org/abs/2605.31423)
...
### [147] [DG-CoLearn: An Efficient Collaborative Learning Framework for Dynamic Graphs](https://arxiv.org/abs/2605.31427)
...
### [148] [Flow map learning in nonlinear vector autoregressive models: influence of the feature-library structure on the training error](https://arxiv.org/abs/2605.31438)
*Markus Gross*
...
### [149] [DRIFT: Decoupled Rollouts and Importance-Weighted Fine-Tuning for Efficient Multi-Turn Optimization](https://arxiv.org/abs/2605.31455)
...
### [150] [Balanced LoRA: Removing Parameter Invariance to Accelerate Convergence](https://arxiv.org/abs/2605.31484)
...
### [151] [Graphical einops: bridging tensor networks and computation graphs](https://arxiv.org/abs/2605.31485)
...
### [152] [Assign and Add: A Mechanistic Study of Compositional Arithmetic](https://arxiv.org/abs/2605.31497)
...
### [153] [Scalable Inference-Time Annealing with Surrogate Likelihood Estimators](https://arxiv.org/abs/2605.31498)
...
### [155] [On the Relationship Between Activation Outliers and Feature Death in Sparse Autoencoders](https://arxiv.org/abs/2605.31518)
...
### [156] [Chem-PerturBridge: a harmonized compendium of small molecule perturbation transcriptomic effects](https://arxiv.org/abs/2605.31522)
...
### [157] [Value Functions as Supermartingale Certificates](https://arxiv.org/abs/2605.31524)
...
### [158] [The Dynamic-Probabilistic Consistency Gap in Chaotic Surrogate Modeling](https://arxiv.org/abs/2605.31547)
...
### [159] [Functional Attention: From Pairwise Affinities to Functional Correspondences](https://arxiv.org/abs/2605.31559)
...
### [160] [Effective Biological Representation Learning by Masking Gene Expression](https://arxiv.org/abs/2605.31562)
...
### [161] [Giving Sensors a Voice: Multimodal JEPA for Semantic Time-Series Embeddings](https://arxiv.org/abs/2605.31580)
...
### [162] [A Tight Theory of Error Feedback Algorithms in Distributed Optimization](https://arxiv.org/abs/2605.31594)
...
### [163] [PhyDrawGen: Physically Grounded Diagram Generation from Natural Language](https://arxiv.org/abs/2605.30512)
*Nafiul Haque,Syed Nazmus Sakib,Shifat E Arman*

Main category: cs.AI

TL;DR: PhyDrawGen是一个神经符号管道，用于从文本生成物理图表，通过解耦语义场景理解和物理约束满足来解决当前生成模型违反物理定律的问题。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型在生成物理图表时虽然能产生视觉上合理的结果，但会系统性地产生错误（如错误的力量向量、忽视守恒定律、违反几何约束），需要确保生成的图表严格遵循物理定律。

Method: 采用三步神经符号管道：1）使用大语言模型从问题文本中提取类型化场景图；2）通过确定性求解器将场景图转换为平面直线图（PSLG），将力平衡、光路和场拓扑编码为精确几何基元；3）使用微调的Qwen-VL模型实现视觉基础的建议-验证循环，迭代修正约束违规。

Result: 在涵盖力学、光学和电磁学的1449个问题基准测试中，PhyDrawGen显著优于GPT-5-image、Gemini 2.5 Flash和Gemini 3 Pro，即使在不常见物体问题上也展现出稳健的物理准确性。

Conclusion: PhyDrawGen通过结合神经模型的语义理解能力和符号系统的精确物理约束执行，成功解决了文本到物理图表生成中的物理准确性挑战，为物理教育和技术应用提供了可靠工具。
...
### [164] [Physically Viable World Models: A Case for Query-Conditioned Embodied AI](https://arxiv.org/abs/2605.30542)
*Adam J. Thorpe,Stepan Tretiakov,Cheng-Hsi Hsiao,Su Ann Low,Xingjian Li,Hassan Iqbal,Neel P. Bhatt,Ufuk Topcu,Krishna Kumar*

Main category: cs.AI

TL;DR: 论文提出了一种新的世界建模方法，强调构建物理上可行的模型来回答干预查询，而不是仅预测未来观测


<details>
  <summary>Details</summary>
Motivation: 现有基于观测预测的世界模型会产生视觉合理但物理错误的推演，无法区分看似相同但物理结构不同的系统，这在干预场景下会导致错误决策

Method: 提出了模块化的物理抽象世界模型框架，包含环境表示、潜在状态估计、动作规范、干预动态和查询响应等组件，以及一个能够根据查询动态组合合适组件的自主编排器

Result: 展示了该方法在现有系统失败的查询上能够正确回答，并为规划、控制和验证任务提供了一种可动态组装和适应的物理可行模型构建方法

Conclusion: 具身AI需要能够识别最简单物理抽象的世界模型，该模型应保持决定干预结果的结构，使模型可解释、组件可验证、输出可审计，为世界模型设计提供了新原则
...
### [165] [Transforming and Encoding FTS for SAT Solving: What Helps, What Hurts (Extended Version)](https://arxiv.org/abs/2605.30563)
*João Filipe,Álvaro Torralba,Gregor Behnke*

Main category: cs.AI

TL;DR: 本文研究如何将分解任务编码到SAT中，提出多种将分解转移关系转化为命题逻辑的策略，并分析了并行性和任务转换对基于SAT的规划器性能的影响。


<details>
  <summary>Details</summary>
Motivation: 分解任务是SAS+规划的扩展，支持有限形式的析取前置条件、条件效应和天使非确定性，能比STRIPS或SAS+更紧凑地表示任务。然而现有的分解任务规划方法仅限于启发式搜索，需要探索基于SAT的规划方法。

Method: 提出了多种将分解任务编码为SAT的方法，重点关注将分解转移关系转化为命题逻辑的不同策略，并分析如何在多个层面利用并行性，以及研究常见任务转换对性能的影响。

Result: 建立了将分解任务编码为SAT的理论框架，提出了不同的编码策略，分析了并行化潜力和任务转换的影响，为基于SAT的分解任务规划提供了方法基础。

Conclusion: 分解任务可以通过多种策略成功编码到SAT中，为基于SAT的规划器提供了新的应用方向，并行化和任务转换能显著影响规划器性能，这为高效解决分解任务开辟了新途径。
...
### [166] [Procedural Generation of First Person Shooter Maps using Map-Elites](https://arxiv.org/abs/2605.30570)
*Simone de Donato,Pier Luca Lanzi,Daniele Loiacono*

Main category: cs.AI

TL;DR: MAP-Elites算法在FPS游戏关卡设计中的应用，通过引入新的地图表示法（Point-Line和Spatial-Layout）和改进的特征度量，生成比传统方法更多样化和高质量的FPS地图。


<details>
  <summary>Details</summary>
Motivation: 目前FPS游戏关卡设计通常使用传统的All-Black和Grid-Graph表示法，这些方法在表征FPS地图方面存在局限性，需要新的表示法来更好地捕捉FPS地图的特征，以提高生成地图的多样性和质量。

Method: 1) 引入两种新的地图表示法：Point-Line和Spatial-Layout；2) 定义拓扑属性度量和涌现属性度量；3) 通过深入分析确定最适合引导MAP-Elites光照过程的特征；4) 应用带滑动边界的MAP-Elites（MESB）算法进化FPS地图种群。

Result: 新提出的表示法（Point-Line和Spatial-Layout）能够生成比先前用于FPS地图进化的表示法（All-Black和Grid-Graph）具有更高多样性和质量的地图。

Conclusion: MAP-Elites算法结合新的地图表示法和合适的特征度量，可以有效应用于FPS游戏关卡设计，生成多样化和高质量的游戏地图，为游戏内容生成提供了有效的方法。
...
### [167] [EHRBench: An Automated and Reliable EHR-based Benchmark for Clinical Decision Making with LLMs](https://arxiv.org/abs/2605.30637)
*Yuzhang Xie,Keqi Han,Yunpeng Xiao,Hejie Cui,Guanchen Wu,Ziyang Zhang,Kai Shu,Jiaying Lu,Xiao Hu,Carl Yang*

Main category: cs.AI

TL;DR: 本文提出了EHRBench，一个基于电子健康记录（EHR）的自动化、可扩展的临床决策基准，用于评估基于大语言模型（LLM）的临床决策能力。


<details>
  <summary>Details</summary>
Motivation: 当前临床决策支持中，LLM的应用日益增多，但其在实际临床决策任务中的可靠性尚未得到充分理解。现有评估方法缺乏规模化和基于真实患者数据的基准，难以全面评估LLM在需要实质性生物医学知识和临床推理的实践任务中的表现。

Method: 通过EHR-LLM-KB（知识库）交互管道构建基准：1）使用专门LLM自动将就诊级EHR轨迹转换为结构化模板；2）确定性实例化模板为问答项目；3）并行应用基于知识库的系统验证和丰富化，过滤幻觉或模糊关系以提高可靠性。

Result: 构建了包含近100万（960,067）个问答项目的基准，涵盖诊断、治疗和预后三个核心推理需求的临床决策任务。对30多个代表性LLM进行了基准测试，提供了性能和鲁棒性的详细分析。

Conclusion: EHRBench为评估LLM在临床决策中的能力提供了可靠、可扩展的基准，揭示了不同设置下一致的能力趋势，验证了基准的可靠性，并突出了实现临床可靠LLM系统需要解决的关键差距。
...
### [168] [Structure-Induced Information for Rerooting Levin Tree Search](https://arxiv.org/abs/2605.30664)
*Jake Tuero,Michael Buro,Laurent Orseau,Levi H. S. Lelis*

Main category: cs.AI

TL;DR: 本文提出一种基于重根器的策略树搜索方法，通过学习重根器来替代显式子目标生成，在保持搜索效果的同时显著降低计算开销，实现了更好的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 基于显式子目标的策略树搜索在复杂单智能体确定性问题上有效，但显式子目标生成会产生大量开销并阻碍可扩展性。需要一种更高效的替代方法来克服这些限制。

Method: 通过√LTS算法使用学习的"重根器"隐式地将问题分解为软子任务。提出三种重根器设计：(1)基于聚类的重根器利用全局状态空间结构；(2)基于启发式的重根器利用学习的代价估计；(3)结合两者信号的混合重根器。避免了显式重建和推理生成的子目标。

Result: 该方法在复杂环境中能够扩展到基于子目标的策略树搜索失败的情况，并在测试领域中实现了最先进的在线训练效率。

Conclusion: 基于重根器的策略树搜索框架通过避免显式子目标生成，实现了可扩展的搜索努力分配，显著降低了计算开销，在复杂环境中表现出色并达到最先进的训练效率。
...
### [169] [Generating Graph-like Rules for Knowledge Graph Reasoning via Diffusion Models](https://arxiv.org/abs/2605.30747)
*Haoxiang Cheng,Yunfei Wang,Chao Chen,Kewei Cheng,Zhipeng Lin,Haoxuan Li,Changjun Fan,Shixuan Liu*

Main category: cs.AI

TL;DR: GRiD是一种基于扩散模型和强化学习的框架，用于从知识图谱中发现图状规则（如循环和分支结构），以弥补传统链状规则挖掘方法的不足。


<details>
  <summary>Details</summary>
Motivation: 现有规则挖掘方法主要关注简单链状规则，忽略了图状结构（如循环和分支）中的丰富关系信息，并且存在计算瓶颈（搜索空间的组合爆炸）。生成式方法（如扩散模型）由于训练目标与规则学习不匹配，且不可微的规则质量指标无法直接指导优化，无法直接应用于规则挖掘。

Method: GRiD将图状规则发现重新定义为基于目标关系的离散生成过程。采用两阶段训练策略：首先通过监督预训练从知识图谱元图中采样子图捕获结构先验；然后使用强化学习（策略梯度优化）根据不可微的规则质量指标进行微调。

Result: 在六个基准数据集上的实验表明，GRiD在知识图谱补全任务上取得了竞争性性能。消融研究证实了GRiD的效率和鲁棒性，并进一步显示图状规则对链状规则在知识图谱补全中的补充作用。

Conclusion: GRiD框架成功解决了传统规则挖掘方法在处理图状结构时的局限性，通过结合监督学习和强化学习，实现了高效、高质量的图状规则发现，为知识图谱推理提供了更丰富的规则类型。
...
### [170] [PReMISE: Policy Rubrics as Measurement Specifications for LLM Judges](https://arxiv.org/abs/2605.30803)
...
TL;DR: PReMISE框架用于评估LLM打分准则的有效性，通过分析准则的结构适用性、可靠性、偏好拟合和对抗鲁棒性来优化准则设置


<details>
  <summary>Details</summary>
Motivation: LLM评委在评估开放式回答时，评分结果强烈依赖于使用的准则。模糊的准则可能导致评分奖励虚构事实或违背用户意图的回答，因此需要系统方法来评估和优化准则的质量

Method: 提出PReMISE框架，基于成对人类偏好数据，首先发现策略级准则集合，然后从四个维度审计任何准则集合：结构适用性、可靠性、偏好拟合和对抗鲁棒性。同时提出两种修复操作：偏好排序选择和可靠性约束优化

Result: 不同准则来源都无法同时满足可靠性、偏好预测性和对抗鲁棒性；高评分者间一致性并不意味着低可利用性。PReMISE是唯一能够在适用性、具体性和有效维度上同时得分显著的准则来源。修复操作提高了评委准确率和对抗鲁棒性

Conclusion: PReMISE框架提供了系统评估LLM评分准则的方法，通过修复操作能够显著改善准则质量，为可靠的LLM评委评估提供了实用工具
...
### [171] [SLAT: Segment-Level Adaptive Trimming for Efficient CoT Reasoning](https://arxiv.org/abs/2605.30832)
*Jian Yao,Xiongcai Luo,Ran Cheng,Kay Chen Tan*

Main category: cs.AI

TL;DR: 本文提出了SLAT框架，通过分段级自适应修剪解决大推理模型中链式思维过程的结构冗余问题，在保持竞争性准确度的同时将推理长度减少50%。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过强化学习显著改进了链式思维能力，但生成的推理链经常出现结构冗余（过度思考），导致高计算开销而没有改善答案正确性。现有的缓解策略通常依赖统一的长度惩罚，这种方法提供粗糙的、与分段无关的压力来缩短输出，可能会无意中抑制有用的推理和冗余。

Method: 作者发现低效性集中在高概率但边际效用低的段中。他们在正确性-长度权衡目标下推导了段次优性的理论特征，并提出了SLAT（分段级自适应修剪）框架。这是一个强化学习框架，根据该标准选择性地抑制冗余段。

Result: 在标准基准测试上的实验结果表明，SLAT建立了优越的准确度-效率帕累托前沿，相对于未压缩的基线，推理长度减少了50%，同时保持了竞争性的准确度。

Conclusion: 基于理论基础的、具有分段感知能力的修剪是在大型语言模型中实现高效链式思维推理的有前景方向。
...
### [172] [Distilling LLM Feedback for Lean Theorem Proving](https://arxiv.org/abs/2605.30861)
*Gaetan Narozniak,Gérard Biau,Rémi Munos,Ahmad Rammal,Pierre Marion*

Main category: cs.AI

TL;DR: 提出Feedback Distillation方法，通过条件语言模型生成特权反馈，在token级别匹配自身分布，解决GRPO在推理模型后训练中的稀疏奖励、有限探索和模式崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 目前推理模型的后训练通常结合监督微调和基于可验证奖励的强化学习（主要是GRPO），但GRPO存在稀疏奖励、有限探索和模式崩溃等问题，需要更有效的训练方法。

Method: 基于自蒸馏的思想，提出Feedback Distillation方法：模型在token级别学习匹配自身在条件语言模型生成的特权反馈下的分布，提供token级监督并注入外部知识。

Result: 在Lean4定理证明任务中，Feedback Distillation比GRPO保持更大的轨迹多样性，产生更高的策略熵和更好的pass@k缩放效果。两种方法互补：从Feedback Distillation检查点初始化GRPO优于单独使用任一方法。

Conclusion: Feedback Distillation为复杂推理的后训练提供了有前景的改进方向，结合token级监督和外部知识注入，能有效缓解传统强化学习方法的问题。
...
### [173] [UniScale: Adaptive Unified Inference Scaling via Online Joint Optimization of Model Routing and Test-Time Scaling](https://arxiv.org/abs/2605.30898)
...
### [175] [A Persona-Based Evaluation Framework for Pluralistic Alignment in Generative AI](https://arxiv.org/abs/2605.31021)
*Atahan Karagoz*
...
### [176] [HADT: A Heterogeneous Multi-Agent Differential Transformer for Autonomous Earth Observation Satellite Cluster](https://arxiv.org/abs/2605.31023)
...
### [177] [GraphARC: A Comprehensive Benchmark for Graph-Based Abstract Reasoning](https://arxiv.org/abs/2605.31031)
...
### [178] [Vector Linking via Cross-Model Local Isometric Consistency](https://arxiv.org/abs/2605.31100)
...
### [179] [LLM-FACETS: A Privacy-Preserving Framework for Evaluating LLM Transparency and Accountability](https://arxiv.org/abs/2605.31167)
...
### [180] [Formalizing and falsifying causal pathways of rare events](https://arxiv.org/abs/2605.31254)
...
### [181] [COLLEAGUE.SKILL: Automated AI Skill Generation via Expert Knowledge Distillation](https://arxiv.org/abs/2605.31264)
...
### [182] [Industrializing Prediction-Powered Inference: The GLIDE Library for Reliable GenAI and Agentic Systems Evaluation](https://arxiv.org/abs/2605.31278)
...
### [183] [TraceGraph: Shared Decision Landscapes for Diagnosing and Improving Agent Trajectories](https://arxiv.org/abs/2605.31308)
...
### [184] [Diagnosing Failure Modes of Shared-State Collaboration in Resource-Constrained Visual Agents](https://arxiv.org/abs/2605.31354)
*Yunpeng Zhou*
...
### [185] [Learning to Adapt: Self-Improving Web Agent via Cognitive-Aware Exploration](https://arxiv.org/abs/2605.31365)
...
### [186] [HypoAgent: An Agentic Framework for Interactive Abductive Hypothesis Generation over Knowledge Graphs](https://arxiv.org/abs/2605.31370)
...
### [187] [FAM-Bench: A Multimodal Benchmark for Condition-Aware Food-as-Medicine Reasoning](https://arxiv.org/abs/2605.31410)
...
### [188] [Answer-Set-Programming-based Abstractions for Reinforcement Learning](https://arxiv.org/abs/2605.31444)
...
### [189] [AutoSci: A Memory-Centric Agentic System for the Full Scientific Research Lifecycle](https://arxiv.org/abs/2605.31468)
...
### [190] [LinTree: Improving LLM Reasoning with Explicitly Structured Search Histories](https://arxiv.org/abs/2605.31492)
...
### [191] [Choosing the Lens: Strategic Perspective Activation in Context-Dependent Argumentation](https://arxiv.org/abs/2605.31581)
