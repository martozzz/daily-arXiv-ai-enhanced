...
TL;DR: 该论文提出了"算法度量"框架，用于分析算法市场中预测模型成为其所预测数据生成过程一部分的现象，揭示了被动历史风险与部署风险之间的差异，并证明了历史数据无法单独识别部署风险、模型排名可能在拥挤时反转等结果。


<details>
  <summary>Details</summary>
Motivation: 在算法市场中，预测模型的输出被转化为交易、分配、执行计划或风险控制后，会改变它们所要评估的未来数据。这种预测算法与其所预测时间序列演化之间的相互依赖关系需要新的分析框架。

Method: 提出了"算法度量"框架，用于分析依赖于预测算法的时间序列演化。通过理论证明和数学分析，研究了一步骤线性反馈模型中的识别问题、模型排名反转现象，以及使用随机化或工具变量行为识别短期线性反馈的方法。

Result: 1) 部署风险不能仅从被动历史数据中识别；2) 在算法拥挤情况下，历史模型排名可能反转；3) 随机化或工具变量行为可以识别短期线性反馈，并推导了部署风险估计的有限样本界。

Conclusion: 算法市场中的时间序列基准测试应该报告反馈敏感性以及预测准确性，因为预测模型不仅预测市场还会改变市场，需要新的评估框架来理解算法与其环境之间的相互作用。
...
TL;DR: IRNO通过迭代精炼模块增强预训练神经算子，采用固定点迭代方法提高高频细节分辨率，在物理系统中显著降低误差


<details>
  <summary>Details</summary>
Motivation: 传统神经算子作为数据驱动的科学建模替代物，通常采用单次推理过程，难以解析高频细节（谱偏置问题），限制了其在复杂物理系统中的应用

Method: 提出迭代精炼神经算子(IRNO)：在预训练算子上增加学习到的精炼模块，通过固定点迭代进行迭代应用；将预测分解为粗略初始化和连续残差校正；提出渐进谱损失函数，在训练过程中逐步加强对高频分量的惩罚

Result: 在湍流模拟中误差降低达56.05%；在活性物质系统中，相对于基础算子，归一化误差比在低频降至27.72-36.10%，中频降至5.07-6.68%，高频降至1.48-2.04%；收敛稳定性超过训练迭代次数

Conclusion: IRNO通过迭代精炼机制有效解决了神经算子的谱偏置问题，显著提高了高频细节的解析能力，在多个物理系统中表现出优越性能和稳定性
...
TL;DR: 在单层隐藏状态隐私保护中，测试了1536个高斯发布协方差，没有一个能同时实现中等效用和中等隐私。研究发现对角逆Fisher发布是唯一最优的对角机制，但位于隐私/效用的边界而非中间地带。传统机制在自适应攻击下失效，而新设计的架构能达到更好的隐私保护效果。


<details>
  <summary>Details</summary>
Motivation: 研究高斯机制在隐藏状态隐私保护中的有效性，特别关注在自适应Mahalanobis攻击下的隐私-效用权衡。需要理解为什么现有高斯机制无法同时提供中等水平的隐私和效用，并探索更好的解决方案。

Method: 1. 测试了1536个高斯发布协方差在单层隐藏状态隐私场景下的表现；2. 证明了Fisher-ball下界定理；3. 提出了对角逆Fisher发布作为最优对角机制；4. 开发了分裂记忆transformer架构；5. 比较了传统机制与新架构在不同模型规模下的性能。

Result: 1. 所有测试的高斯机制都无法同时实现中等隐私和中等效用；2. 对角逆Fisher发布是唯一最小化最大最优的对角机制；3. 广义特征机制在欧几里得检索下Pareto改进但Mahalanobis攻击下完全失效；4. 分裂记忆transformer在Mahalanobis增益上达到20-33，比GPT基线好6-24倍；5. 预训练模型性能上限为9.3。

Conclusion: 研究结果将隐藏状态发布从高斯机制设计范式重新框架为架构或发布协同设计范式。传统高斯机制类无法提供均匀的安全保障，需要新的架构设计与发布策略结合才能实现更好的隐私-效用平衡。
...
### [4] [A Large-Scale Dataset and Benchmark: Do Protein-Ligand Models Learn Binding Sites or Just Binding Likelihood?](https://arxiv.org/abs/2605.24045)
*Zhaohan Meng,Zhen Bai,Ke Yuan,Iadh Ounis,Zaiqiao Meng,Hao Xu,Joseph Loscalzo*

Main category: cs.LG

TL;DR: InteractBind：用于蛋白质-配体建模细粒度评估的大规模数据集和基准，包含约100k蛋白质-配体对，关注结合位点定位和六种非共价相互作用类型的识别


<details>
  <summary>Details</summary>
Motivation: 现有蛋白质-配体基准主要评估二元结合预测和亲和力回归，但无法评估模型是否能定位结合位点或识别分子识别中的非共价相互作用，需要更细粒度的评估方法

Method: 构建InteractBind数据集（约10万蛋白质-配体对），包含结合位点定位核心任务，使用蛋白质残基-配体原子相互作用图，涵盖六种主要非共价相互作用类型，同时包括结合亲和力和蛋白质相似性控制分割

Result: 评估8个现有序列基和相互作用感知模型，发现模型在结合位点定位方面表现有限，尽管二元结合预测表现良好，且在不同非共价相互作用类型间存在显著差异

Conclusion: InteractBind建立了鼓励开发更具可解释性和物理基础的蛋白质-配体模型的基准范式，为细粒度评估提供了新标准
...
### [5] [Mixture of Complementary Agents for Robust LLM Ensemble](https://arxiv.org/abs/2605.24048)
*Yichi Zhang,Kevin Lu,Yuang Zhang,Jie Gao,Lirong Xia,Fang-Yi Yu*

Main category: cs.LG

TL;DR: 本文研究多AI协作中的提议者选择问题，将提议者选择重新定义为组合选择问题，探索基于互补性的高效贪婪选择算法，在性能与成本间实现最佳权衡。


<details>
  <summary>Details</summary>
Motivation: 在多AI协作系统中，选择哪些提议者LLM参与合成是一个关键但困难的问题。现有方法主要关注准确性（选择最强模型）或多样性（确保多样性），而忽略了提议者之间以及提议者与总结者之间的交互作用。

Method: 将提议者选择重构为类似于特征选择的组合选择问题，提出基于互补性原则的方法。探索一系列计算可行的贪婪式选择算法，这些算法使用小型标注集评估互补性，避免了直接应用标准特征选择算法的时间复杂度问题。

Result: 实验验证了互补性作为提议者选择指导原则的有效性，识别了在实践中能够实现最佳性能-成本权衡的选择方法。

Conclusion: 互补性是选择提议者LLM的有效原则，通过精心设计的贪婪算法可以在不牺牲效率的前提下显著提升多AI协作系统的整体性能。
...
### [6] [Truthful Online Preference Aggregation for LLM Fine-Tuning in Mobile Crowdsourcing](https://arxiv.org/abs/2605.24052)
*Shugang Hao,Lingjie Duan*

Main category: cs.LG

TL;DR: 提出一种在线加权聚合机制，用于移动众包中LLM微调的真实偏好聚合，能够激励工人真实反馈并实现次线性遗憾


<details>
  <summary>Details</summary>
Motivation: 移动众包平台需要将LLM生成内容（如AI交通预测）与人类反馈对齐，但工人可能策略性地误报偏好以最大化影响力或报酬，现有方法无法在在线设置中准确识别最佳工人

Method: 构建动态贝叶斯博弈模型描述平台与策略性移动工人的多智能体在线学习过程，提出在线加权聚合机制动态调整每个工人在偏好聚合中的权重

Result: 机制能确保策略性工人提供真实反馈，实现次线性遗憾O(√T)，即使在每个时隙工人反馈有限的情况下也能保证O(√T)遗憾，实验显示在真实数据集上的LLM微调性能显著优于基准方案

Conclusion: 提出的机制解决了移动众包中LLM微调的真实在线偏好聚合问题，通过动态权重调整激励真实反馈，实现了理论上的次线性遗憾保证和实际性能提升
...
### [7] [Cascade-KDE: Robust Time-Series Restoration under Out-of-Distribution Impulse Corruptions](https://arxiv.org/abs/2605.24055)
*Yuefeng Liu,Ning Yang,Ziyu Yang*

Main category: cs.LG

TL;DR: 提出Cascade-KDE框架，用于修复受高斯噪声和脉冲异常混合污染的时序数据，在保持局部形状特征的同时实现低重构误差


<details>
  <summary>Details</summary>
Motivation: 工业传感、医疗和能源系统中的真实时序数据常受到高斯噪声和大幅度脉冲异常混合污染。对于依赖局部形状的任务（如心电图形态分析和电池退化监测），不仅需要低重构误差，还需要保持导数峰值和任务关键特征。

Method: Cascade-KDE训练免费修复框架：首先估计二维时间-振幅密度，然后应用密度截断稳健期望来限制远处异常点的影响，最后通过自适应停止的指数级联精炼序列。

Result: 在多个基准数据集上，该方法在曲线保真度、导数保持、下游分类和运行时效率方面均优于经典滤波器和代表性基于学习的基线方法。

Conclusion: 有界密度修复是噪声时序管道中特征保持预处理的实用选择，能够在分布外脉冲污染下提高鲁棒性，同时保持恢复轨迹接近原始局部结构。
...
TL;DR: LoRDBA：一种LoRA兼容的适配器，将两个低秩因子替换为二进制符号载体，通过轻量级通道级缩放表示幅度，将密集适配器分支转换为两次符号累加矩阵乘法，显著减少适配器占用空间


<details>
  <summary>Details</summary>
Motivation: 当前设备上大型语言模型适配通常保持量化基础模型冻结，同时训练和部署小型任务特定的LoRA适配器。在未合并的适配器模式下，适配器不仅仅是紧凑存储模块，还引入了额外的密集浮点分支，维护本地更新的可训练状态，并作为通信和热交换单元。现有方法在这些方面存在效率和存储方面的挑战。

Method: 引入LoRDBA适配器，将LoRA的两个低秩因子替换为二进制符号载体，通过轻量级通道级缩放表示幅度。该方法将密集适配器分支转换为两次符号累加矩阵乘法（interleaved with channel-wise scaling），通过有限样本分析表明重构质量受原始LoRA因子的残差与幅度比控制。

Result: 在适配器模式实验中，LoRDBA在匹配的模型大小下优于低比特基线，在选定条件下匹配fp16 LoRA质量。未合并的适配器在匹配秩r=16时最多产生8%的预填充延迟开销，同时适配器占用空间减少超过10倍，训练内存开销约为fp16 LoRA的1.6倍。

Conclusion: LoRDBA提供了一种高效的LoRA兼容适配器设计，通过二进制化显著减少存储和通信开销，同时在多个关键指标上保持或接近全精度LoRA的性能，为设备上大型语言模型适配提供了一种有效的解决方案。
...
### [9] [Spectral Probe-Circuits: A Three-Step Recipe for Identifying Attention-Head Circuits in Pretrained Transformers](https://arxiv.org/abs/2605.24059)
*Yongzhong Xu*

Main category: cs.LG

TL;DR: 提出了一种三步法识别预训练transformer中的注意力头电路：1)基于频谱信号的无监督排名，2)任务模式筛选，3)组消融验证。方法在不同规模、架构和预训练流程中有效验证，特别是识别出在所有模型中因果必要的2-6头归纳电路。


<details>
  <summary>Details</summary>
Motivation: 需要一种系统方法来识别预训练transformer中执行特定计算任务的注意力头电路，特别是在无监督或弱监督情况下。当前方法通常依赖标注数据或梯度归因，而本文旨在开发不依赖这些的可扩展识别方法。

Method: 三步识别方法：1)使用每个头的频谱信号（注意力输出的时间积分参与比）对执行持续内容依赖计算的头进行无监督排名；2)任务模式筛选，将通用指标过滤为任务特定候选电路；3)组消融与匹配随机对照，完成因果验证。

Result: 方法在8倍参数范围、两种架构家族和四种预训练流程中验证有效。识别出在所有模型中因果必要的2-6头归纳电路，消融后合成归纳top-1准确率下降94-100%。在51M参数探针模型的六个独立种子中，相同计算能识别每个种子的特定电路。Pythia家族中可识别专业化计算头比例稳定在17-19%，而特定归纳电路保持3-11个头，低于总头数增长。

Conclusion: 提出了一种稳健的三步法来识别预训练transformer中的注意力头电路，无需标签或梯度归因。该方法在不同规模、架构和训练流程中具有可移植性，揭示了transformer计算的专业化模式。作为三篇系列论文的方法论基础，为研究预训练期间的发展轨迹和组合任务电路提供了工具。
...
TL;DR: 这篇论文调查了人体通信（HBC）与联邦学习（FL）的交叉领域，提出了身体感知的FL概念，并引入了BODYFED-HBC参考架构来解决可穿戴设备中的通信-学习协同优化问题。


<details>
  <summary>Details</summary>
Motivation: 虽然HBC作为可穿戴体域网的有前景的物理层技术，而FL作为减少原始数据集中化的学习框架，但这两个领域的研究仍然相对独立。HBC研究通常抽象化学习流量，而FL研究则抽象化通信层。需要研究这两个领域的交叉点以实现更好的协同优化。

Method: 1. 提出了一种分类法，区分体内、体中心、跨用户和临床云FL部署模式；2. 引入了BODYFED-HBC作为参考架构；3. 提供了优化公式和调度算法；4. 设计了可重复的模拟场景，结合公共可穿戴数据集和实证的身体耦合通信信号损耗模型。

Result: 提出了身体信道感知的FL（body-channel-aware FL）这一开放问题，强调学习协议需要根据姿态相关的HBC链路、剩余能量、传感器内存和隐私风险来控制客户端选择、更新压缩和聚合过程。

Conclusion: 该论文为计算机科学研究人员提供了系统的研究框架，包括开放数据集、评估指标和未来研究方向，强调需要在硬件层之上实现通信和学习协议的协同设计，以优化可穿戴设备中的边缘智能系统。
...
### [11] [Generative Representation Learning on Hyper-relational Knowledge Graphs via Masked Discrete Diffusion](https://arxiv.org/abs/2605.24064)
*Jaejun Lee,Seheon Kim,Joyce Jiyoung Whang*

Main category: cs.LG

TL;DR: KREPE：首个用于超关系知识图谱的生成式表示学习方法，通过掩码离散扩散处理任意掩码查询的事实生成任务，统一了链接预测和事实生成


<details>
  <summary>Details</summary>
Motivation: 当前方法将超关系知识图谱推理简化为链接预测问题，假设事实中只有一个组件缺失。然而，在现实场景中，事实的多个甚至所有组件可能同时缺失，这限制了现有方法的适用性。

Method: KREPE通过掩码离散扩散方法，学习在给定局部事实组件和全局图结构条件下缺失组件的概率分布。模型通过上下文消息传递建模事实内部依赖，并通过随机采样上下文聚合来建模事实间相关性。

Result: KREPE在标准超关系知识图谱链接预测基准测试中达到最先进性能，并在生成新颖且正确的事实方面优于基于LLM的基线方法。

Conclusion: KREPE成功统一了链接预测和事实生成任务，能够处理任意掩码查询的事实生成问题，填补了现实场景中多组件同时缺失推理的空白。
...
### [12] [Not All Transitions Matter: Evidence from PPO](https://arxiv.org/abs/2605.24071)
*Ajhesh Basnet*

Main category: cs.LG

TL;DR: 通过随机丢弃轨迹中固定比例（25%）的过渡状态来打破梯度信号冗余，稳定PPO训练，无需修改算法核心


<details>
  <summary>Details</summary>
Motivation: 在策略强化学习中，轨迹中的连续状态因因果链而高度相关，导致梯度信号冗余，使训练变得不稳定，而奖励曲线往往无法揭示这种不稳定性

Method: 在保持奖励信号完整的前提下，随机丢弃轨迹中固定比例（25%）的过渡状态，破坏梯度重复结构，这种方法可与任何PPO实现兼容，无需添加新组件

Result: 在五个难度递增的环境（CartPole-v1、Acrobot-v1、LunarLander-v2、HalfCheetah-v5、Hopper-v5）中，方法在奖励上匹配原始PPO，同时在KL散度、策略熵和值估计方面产生更一致的训练动态

Conclusion: 简单的随机丢弃过渡状态技术能有效消除梯度冗余，稳定强化学习训练，且25%的丢弃率是最佳平衡点，既能破坏冗余又不会过度稀释批次
...
### [13] [Overcoming "Physics Shock" in Earth Observation A Heteroscedastic Uncertainty Framework for PINN-based Flood Inference](https://arxiv.org/abs/2605.24106)
...
### [17] [Filtered Posterior Mean Collections: A Unified Framework for Analytical Models of Diffusion Generalization](https://arxiv.org/abs/2605.24192)
...
### [18] [Characterizing the Representational Capacity of Neural Processes](https://arxiv.org/abs/2605.24210)
*Robin Young*
...
### [19] [Agent-ToM: Learning to Monitor Autonomous LLM Agents via Theory-of-Mind Reasoning](https://arxiv.org/abs/2605.24216)
...
### [21] [Rethinking Continual Anomaly Detection on the Edge: Benchmarking Under Realistic Industrial Conditions](https://arxiv.org/abs/2605.24251)
...
### [22] [Optimizing Digital Therapeutic Interventions: Online Learning under Endogenous Adherence](https://arxiv.org/abs/2605.24261)
...
### [23] [A lift for input-convex neural network training](https://arxiv.org/abs/2605.24274)
...
### [24] [Fourier Feature Pyramids for Physics-Informed Neural Networks](https://arxiv.org/abs/2605.24278)
...
### [25] [Faithfulness as Information Flow: Evaluating and Training Faithful Chain-of-Thought Reasoning](https://arxiv.org/abs/2605.24286)
...
### [26] [TUBE: Tangent Upper Bound on Evidence for Discrete Diffusion Language Models](https://arxiv.org/abs/2605.24292)
...
### [27] [Private Adaptive Covariance Estimation via Gaussian Graphical Models](https://arxiv.org/abs/2605.24295)
...
### [28] [LLMs Show No Signs Of Individuated Metacognition](https://arxiv.org/abs/2605.24299)
...
### [29] [ChaosBench-Logic v2: Evaluating LLM Logical Reasoning over Dynamical Systems at Scale](https://arxiv.org/abs/2605.24305)
*Noel Thomas*
...
### [30] [From One-Pass SGD to Data Reuse: Mini-Batch Scaling Laws in Sketched Linear Regression](https://arxiv.org/abs/2605.24316)
...
### [31] [Omissive Bias in Religious Representation: Benchmarking LLM Answers to Everyday Ethical Decision-making](https://arxiv.org/abs/2605.24319)
...
### [32] [CurveRL: Principled Distribution-Aware Context Reweighting for LLM Reasoning](https://arxiv.org/abs/2605.24331)
...
### [33] [ChainzRule: Sample-Efficient, Robust Deep Learning Across Tabular, NLP, and Vision Tasks](https://arxiv.org/abs/2605.24340)
*Rowan Martnishn*
...
### [34] [Evolving Robustness--Exploration Trade-off in Online Reinforcement Learning via Quantile Bayesian Risk MDPs](https://arxiv.org/abs/2605.24345)
...
### [35] [Refined Analysis of Entropy-Regularized Actor-Critic](https://arxiv.org/abs/2605.24357)
...
### [36] [Treatment Effect Estimation with Differentiated Networked Effect on Graph Data](https://arxiv.org/abs/2605.24358)
...
### [37] [GEESE: Genotype-aware End-to-End Spatio-temporal Embedding for Behavioral Phenotyping](https://arxiv.org/abs/2605.24370)
...
### [38] [Assessing the Operational Viability of Foundation Models for Time Series Forecasting](https://arxiv.org/abs/2605.24381)
...
### [39] [Learning Laplacian Eigenspace with Mass-Aware Neural Operators on Point Clouds](https://arxiv.org/abs/2605.24390)
...
### [40] [AvAtar: Learning to Align via Active Optimal Transport](https://arxiv.org/abs/2605.24395)
...
### [41] [Generative OOD-regularized Model-based Policy Optimization](https://arxiv.org/abs/2605.24405)
...
### [42] [A Unified Python Framework for Direct PPO-based Control of AHUs with Economizer Logic and CO2-Constrained Ventilation](https://arxiv.org/abs/2605.24406)
...
### [43] [Synheart Capacity: A Theory-Driven Physiological Representation of Cognitive Capacity Dynamics from Wearable Signals](https://arxiv.org/abs/2605.24416)
...
### [44] [LLMTabBench: Evaluating LLMs on Binary Tabular Classification From Zero to Few Shots](https://arxiv.org/abs/2605.24417)
...
### [45] [ChainLearn: A Blockchain-Based Capacity-Aware Framework for Federated Ensemble Learning](https://arxiv.org/abs/2605.24418)
...
### [46] [Batch Normalization Amplifies Memorization and Privacy Risks](https://arxiv.org/abs/2605.24420)
...
### [47] [Momentum Streams for Optimizer-Inspired Transformers](https://arxiv.org/abs/2605.24425)
...
### [48] [Representation-Guided Discrete Molecular Graph Retrosynthesis](https://arxiv.org/abs/2605.24428)
...
### [49] [CAffNet: Hard Constraint-Affine Neural Networks](https://arxiv.org/abs/2605.24437)
...
### [50] [Balancing Fairness, Privacy, and Accuracy: A Multitask Adversarial Framework for Centralized Data-Driven Systems](https://arxiv.org/abs/2605.24458)
...
### [51] [The Normalized Maximum Likelihood for Regular Non-Smooth Models: Measure-Theoretic Foundations and Geometric Sampling](https://arxiv.org/abs/2605.24477)
...
### [52] [Zeroth-Order Nonconvex Nonsmooth Optimization with Heavy-Tailed Noise](https://arxiv.org/abs/2605.24513)
...
### [53] [Lake Detection and Water Quality Estimation in Sentinel-2 Data](https://arxiv.org/abs/2605.24515)
...
### [55] [MindAlign: Bridging EEG, Vision, and Language for Zero-Shot Visual Decoding](https://arxiv.org/abs/2605.24523)
...
### [56] [What Are We Actually Decoding? Source Attribution for Non-Invasive Brain-to-Language Retrieval](https://arxiv.org/abs/2605.24524)
...
### [57] [Rethinking Federated Unlearning via the Lens of Memorization](https://arxiv.org/abs/2605.24545)
...
### [58] [RL with Learnable Textual Feedback: A Bilevel Approach](https://arxiv.org/abs/2605.24547)
...
### [59] [Deep ZakaiJ: Structured Filtering for Jump-Diffusion Time Series Forecasting](https://arxiv.org/abs/2605.24548)
...
### [60] [Position: AI for Science Should Treat Measurement-to-Dataset Pipelines as Inference Components](https://arxiv.org/abs/2605.24558)
...
### [61] [PILOT: Policy-Informed Learned Optimization for Adaptive Deep Network Training](https://arxiv.org/abs/2605.24570)
...
### [62] [Polymorphism Is Rotation: Operational Mechanistic Interpretability from a Two-Layer Transformer to Pythia-70m](https://arxiv.org/abs/2605.24577)
*Jordan F. McCann*
...
### [63] [An Effective-Rank Audit of Alignment-Induced Activation Shifts: Confound Control, Constructive Calibration, and Limits](https://arxiv.org/abs/2605.24583)
*Yuki Nakamura*
...
### [64] [LAPLEX: The FFT of Learnable Laplace Kernels](https://arxiv.org/abs/2605.24584)
...
### [65] [Beyond Fixed Points: Superpolynomial Capacity of Asymmetric Hopfield Networks](https://arxiv.org/abs/2605.24611)
...
### [66] [Beyond Generative Priors: Minority Sampling with JEPA-Guided Diffusion](https://arxiv.org/abs/2605.24631)
*Sol Park,Soobin Um*
...
### [67] [On the Stability and Realizability of Recurrent Polynomial Surrogate Ternary Logic Gate Networks](https://arxiv.org/abs/2605.24649)
...
### [68] [WLNO: Wavelet-Laplace Neural Operator for Solving Partial Differential Equations](https://arxiv.org/abs/2605.24658)
...
### [69] [IterInject: Indirect Prompt Injection Against LLM Agents via Feedback-Guided Iterative Optimization](https://arxiv.org/abs/2605.24659)
...
### [70] [Trajectory-Based Difficulty Scoring for Reliable Learning on Tabular Data](https://arxiv.org/abs/2605.24680)
...
### [71] [Beyond the Aggregation Dilemma: Prior-Retaining Decoupled Learning for Multimodal Graphs](https://arxiv.org/abs/2605.24684)
...
### [72] [Streaming Reinforcement Learning under Partial Observability with Real-Time Recurrent Learning](https://arxiv.org/abs/2605.24709)
...
### [73] [Feature Learning in Wide Neural Networks under $μ$P: Identifiability and Sparse-Dictionary Decomposition of the Mean-Field Limit](https://arxiv.org/abs/2605.24710)
*Akmal Xodarev*
...
### [74] [Hardware-Aware Federated Learning for Speech Emotion Recognition](https://arxiv.org/abs/2605.24712)
...
### [75] [Reinforcement Learning for Reachability: Guaranteeing Asymptotic Optimality](https://arxiv.org/abs/2605.24740)
...
### [76] [Aligning Molecular Graph Explanations with Chemical Identity via InChIfied Invariants](https://arxiv.org/abs/2605.24742)
...
### [77] [Bilevel Optimization of Synthetic Trajectories for Multi-Turn LLM Fine-Tuning](https://arxiv.org/abs/2605.24743)
...
### [78] [A computational phase transition for learning-to-sample from Ising models](https://arxiv.org/abs/2605.24752)
...
### [79] [A Contractive Feedback Semantics for Reinforcement Learning](https://arxiv.org/abs/2605.24759)
*Zuyuan Zhang*
...
### [80] [High-fidelity Modeling of Full-scale Pressurized Water Reactor Flow Fields for Machine Learning Applications](https://arxiv.org/abs/2605.24763)
...
### [82] [Hermite-NGP: Gradient-Augmented Hash Encoding for Learning PDEs](https://arxiv.org/abs/2605.24774)
...
### [83] [Complement Submodular Information Measures for Balanced and Robust Data Selection](https://arxiv.org/abs/2605.24779)
*Rishabh Iyer*
...
### [84] [The Perception-Physics Paradox: Probing Scientific Alignment with TC-Bench](https://arxiv.org/abs/2605.24782)
...
### [85] [CONF-KV: Confidence-Aware KV Cache Eviction with Mixed-Precision Storage for Long-Horizon LLM](https://arxiv.org/abs/2605.24786)
*Yubo Li,Yidi Miao*
...
### [86] [Active Learning for Stochastic Contextual Linear Bandits](https://arxiv.org/abs/2605.24803)
...
### [87] [Disentangled Double Machine Learning for Accurate Causal Effect Estimation](https://arxiv.org/abs/2605.24808)
...
### [88] [Cross-Domain Energy-Guided Diffusion Generation for Off-Dynamics Reinforcement Learning](https://arxiv.org/abs/2605.24810)
...
### [89] [DriftingMol: Decoder-Coupled Drift for One-Pass Property-Conditional Molecular Generation](https://arxiv.org/abs/2605.24841)
...
### [90] [Tiny Brains, Giant Impact: Uncovering the Keystone Neurons of LLM with Just a Few Prompts](https://arxiv.org/abs/2605.24846)
...
### [91] [T2S-MPC: Time-Embedded Online Adaptive Model Predictive Control for Time-Varying Dynamics](https://arxiv.org/abs/2605.24852)
...
### [92] [The Concept Allocation Zone: Tracking How Concepts Form Across Transformer Depth](https://arxiv.org/abs/2605.24856)
*James Henry*
...
### [93] [Unifying Value Alignment and Assignment in Cross-Domain Offline Reinforcement Learning with Heterogeneous Datasets](https://arxiv.org/abs/2605.24862)
...
### [94] [A comparative study of accuracy and rollout stability of temporal surrogate models](https://arxiv.org/abs/2605.24868)
*Rajarshi Biswas*
...
### [95] [Cluster Frequency Conformal Prediction for Local Coverage](https://arxiv.org/abs/2605.24872)
*Tomer Lavi,Bracha Shapira,Nadav Rappoport*
...
### [96] [Efficient DP-SGD for LLMs with Randomized Clipping](https://arxiv.org/abs/2605.24879)
...
### [97] [On the Impact of Class Imbalance on the Learning Dynamics of Deep Neural Networks:An Intuitive Insight](https://arxiv.org/abs/2605.24908)
...
### [98] [Factorize to Generalize: Retrieval-Guided Invariant-Dynamic Decomposition for Time Series Forecasting](https://arxiv.org/abs/2605.24911)
...
### [99] [Explainable Retinal Imaging for Prediction of Multi-Organ Dysfunction in Type 2 Diabetes](https://arxiv.org/abs/2605.24912)
...
### [100] [Quaternion Self-Attention with Shared Scores](https://arxiv.org/abs/2605.24920)
...
### [101] [BandVQ: Band-Wise Vector-Quantized EEG Foundation Model](https://arxiv.org/abs/2605.24921)
...
### [102] [Global linear convergence of entropy-regularized softmax policy gradient beyond tabular MDPs](https://arxiv.org/abs/2605.24939)
...
### [103] [Riemannian-Manifold Steering: Geometry-Aware Generative Autoencoders for Label-Free Steering](https://arxiv.org/abs/2605.24942)
...
### [104] [RealBench: Benchmarking Data-Driven Numerical Weather Forecasting Under Operational Conditions and Extreme Event Challenges](https://arxiv.org/abs/2605.24945)
...
### [105] [MedMamba: Multi-View State Space Models with Adaptive Graph Learning for Medical Time Series Classification](https://arxiv.org/abs/2605.24961)
...
### [106] [OSDTW: Optimal Shared Depth and Task Weighting for Long-Tailed Recognition](https://arxiv.org/abs/2605.24969)
...
### [107] [TGFormer: Towards Temporal Graph Transformer with Auto-Correlation Mechanism](https://arxiv.org/abs/2605.24971)
...
### [108] [Benchmarking non-conformity score functions in conformal prediction](https://arxiv.org/abs/2605.24983)
*Sol Erika Boman*
...
### [109] [Selective Test-Time Compute Scaling for Click-Through Rate Prediction via Uncertainty-Triggered Feature Path Exploration](https://arxiv.org/abs/2605.24989)
...
### [110] [Mitigating Gradient Pathology in PINNs through Aligned Constraint](https://arxiv.org/abs/2605.25001)
...
### [111] [Metropolis-Scale Resilient and Trustworthy Traffic Flow Inference Using Multi-Source Data](https://arxiv.org/abs/2605.25004)
...
### [112] [A perspective on fluid mechanical environments for challenges in reinforcement learning](https://arxiv.org/abs/2605.25011)
...
### [113] [MimirRAG: A Multi-Agent RAG Framework for Financial Data Retrieval with Metadata Integration](https://arxiv.org/abs/2605.25030)
...
### [114] [Scale When Needed: Adaptive Neuron-level Mixed Precision Quantization Aware Training](https://arxiv.org/abs/2605.25054)
...
### [115] [GL-LFGNN:A Global-Local Dual-branch Causal Graph Neural Network Based on Liang-Kleeman Information Flow for EEG Emotion Recognition](https://arxiv.org/abs/2605.25061)
...
### [116] [Reinforcement Learning for Laser Additive Manufacturing Scan-Order Optimisation: A Bilevel Proxy--FEA Diagnostic Framework for Reward and World-Model Diagnosis](https://arxiv.org/abs/2605.25063)
...
### [117] [Leveraging Gauge Freedom for Learning Non-Gradient Population Dynamics of Stochastic Systems](https://arxiv.org/abs/2605.25107)
...
### [118] [Revisiting Pre-Propagation GNNs: Robust Diffusion Operators and Hidden-State Re-Propagation](https://arxiv.org/abs/2605.25111)
...
### [119] [Courant: a State-Adaptive Perceiver-Based Neural Surrogate with Local Support and Interpretable Field Decomposition](https://arxiv.org/abs/2605.25115)
...
### [120] [Optimizing Multidimensional Scaling in Gini Metric Spaces](https://arxiv.org/abs/2605.25124)
...
### [121] [Blocked Gibbs meets Diffusion Transformers: Unsupervised Learning for Constraint Optimization](https://arxiv.org/abs/2605.25129)
...
### [122] [Theoretical Analysis of Sparse Optimization with Reparameterization, Weight Decay, and Adaptive Learning Rate](https://arxiv.org/abs/2605.25134)
...
### [123] [ASTRO: Adaptive Spatio-Temporal Reinforcement Optimization for GNN Powered Anomly Detection in Cyber Physical Systems](https://arxiv.org/abs/2605.25135)
...
### [124] [Abduction-Deduction Entanglement: Domain Generalization via Representation Transplants](https://arxiv.org/abs/2605.25156)
...
### [125] [AME-TS: Anchored Mixture-of-Experts for Time Series Forecasting](https://arxiv.org/abs/2605.25166)
...
### [126] [Learning Treatment Effects during Resource Allocation via Priority-Queue Randomization](https://arxiv.org/abs/2605.25169)
...
### [127] [Grow-Prune-Freeze Networks: Adaptive & Continual Learning Technique for Olfactory Navigation](https://arxiv.org/abs/2605.25170)
...
### [128] [Directional Alignment Mitigates Reward Hacking in Reinforcement Learning for Language Models](https://arxiv.org/abs/2605.25189)
...
### [129] [Localization then Neutralization: Gradient-guided Token Suppression against Visual Prompt Injection Attack](https://arxiv.org/abs/2605.25194)
...
### [130] [Multi-Objective Learning for Diffusion Models: A Statistical Theory under Semi-Supervised Learning](https://arxiv.org/abs/2605.25210)
...
### [131] [Evolving Causal Regulatory Networks (ECR-Net)](https://arxiv.org/abs/2605.25211)
...
### [132] [Personalized Federated Learning by Energy-Efficient UAV Communications](https://arxiv.org/abs/2605.25212)
...
### [133] [Continuous-Depth Field Theory for Transformer Patching and Mechanistic Interpretability](https://arxiv.org/abs/2605.25225)
...
### [134] [A Blended Likelihood Approach for Achieving Fairness Using Naive Bayes](https://arxiv.org/abs/2605.25228)
...
### [135] [On the Epistemic Uncertainty of Overparametrized Neural Networks](https://arxiv.org/abs/2605.25234)
*David Rügamer*
...
### [136] [Constraint-Anchored Attribution: Feasibility-Certified Counterfactuals and Bonferroni-PAC Sufficient Subsets for Neural CO Policies](https://arxiv.org/abs/2605.25235)
*Sohaib Lafifi*
...
### [137] [Latent Q-Barrier Shielding for Safe In-Context Reinforcement Learning](https://arxiv.org/abs/2605.25267)
...
### [138] [Label-NTK Alignments and A Tighter Convergence Bound in the NTK Regime](https://arxiv.org/abs/2605.25275)
...
### [139] [When Interpretability Becomes a Liability: Adversarial Attacks on CBM Concept Layers](https://arxiv.org/abs/2605.25304)
*Aditya Sridhar*
...
### [140] [Electricity Consumption Forecasting: An Approach Using Cooperative Ensemble Learning with SHapley Additive exPlanations](https://arxiv.org/abs/2605.25305)
...
### [141] [UWM-JEPA: Predictive World Models That Imagine in Belief Space](https://arxiv.org/abs/2605.25313)
...
### [142] [CausalFlow: Causal Attribution and Counterfactual Repair for LLM Agent Failures](https://arxiv.org/abs/2605.25338)
...
### [143] [Certified Robustness from Approximate Gaussian Mixture Structures in Pretrained Latent Spaces](https://arxiv.org/abs/2605.25352)
...
### [144] [PDEInvBench: A Comprehensive Dataset and Design Space Exploration of Neural Networks for PDE Inverse Problems](https://arxiv.org/abs/2605.25353)
...
### [145] [Not only where, But when: Temporal Scheduling for RLVR](https://arxiv.org/abs/2605.25381)
...
### [146] [ViroBench: Benchmarking Nucleotide Foundation Models on Viral Genomics Tasks](https://arxiv.org/abs/2605.25388)
...
### [147] [A Context Augmented Multi-Play Multi-Armed Bandit Algorithm for Fast Channel Allocation in Opportunistic Spectrum Access](https://arxiv.org/abs/2605.25391)
...
### [148] [EMA-Nesterov: Stabilizing Nesterov's Lookahead for Accelerated Deep Learning Optimization](https://arxiv.org/abs/2605.25395)
...
### [149] [Autoregression-Free Neural Operators for Time-Dependent PDEs](https://arxiv.org/abs/2605.25413)
...
### [150] [Capture-Calibrate-Coach: A Graph-Based Framework for Knowledge Monitoring Estimation and Adaptive Feedback](https://arxiv.org/abs/2605.25419)
...
### [151] [SeqRoute: Global Budget-Aware Sequential LLM Routing via Offline Reinforcement Learning](https://arxiv.org/abs/2605.25424)
...
### [152] [Rethinking Feature Alignment in Generalist Graph Anomaly Detection: A Relational Fingerprint-based Approach](https://arxiv.org/abs/2605.25429)
...
### [153] [Missing Pattern Recognized Diffusion Imputation Model for Missing Not At Random](https://arxiv.org/abs/2605.25439)
...
### [154] [BigMac: Breaking the Pareto Frontier of Compute and Memory in Multimodal LLM Training](https://arxiv.org/abs/2605.25451)
...
### [155] [From Simulation to Enaction: Post-trained language models recognize and react to their own generations](https://arxiv.org/abs/2605.25459)
...
### [156] [JacQuant: STE-Free Quantization-Aware Training via Learned Jacobian Surrogates](https://arxiv.org/abs/2605.25469)
...
### [157] [SafetyRepro: Configuration-Conditional Rank Instability on Alignment Benchmarks](https://arxiv.org/abs/2605.25492)
...
### [158] [Accelerated Dynamic Importance Weighting with Versatile Divergence-Minimizing Estimators](https://arxiv.org/abs/2605.25499)
...
### [159] [LETS Forecast: Learning Embedology for Time Series Forecasting](https://arxiv.org/abs/2506.06454)
...
### [160] [Relative Repairability: A Calibration-Based Diagnostic for High-Sparsity Post-Pruning Allocation](https://arxiv.org/abs/2605.25508)
...
### [161] [SAE-FD: Sparse Autoencoder Feature Distillation for Continual Learning of Large Language Models](https://arxiv.org/abs/2605.25525)
...
### [162] [DeepSeekMath Meets Order Book: Group-Aware Policy Optimization for High-Frequency Directional Trading](https://arxiv.org/abs/2605.25527)
...
### [163] ['Si'multaneous 'S'patial-'T'emporal Message Passing for Dynamic Graph Representation Learning](https://arxiv.org/abs/2605.25548)
...
### [164] [Learning Permutation from Structure Without Supervision](https://arxiv.org/abs/2605.25551)
...
### [165] [RotMoLE: Enhancing Mixture of Low-Rank Experts through Rotational Gating Mechanism](https://arxiv.org/abs/2605.25565)
...
### [166] [Geometric Flow Matching for Molecular Conformation Generation via Manifold Decomposition](https://arxiv.org/abs/2605.25577)
...
### [167] [Learning Latent Dynamical Causal Processes for Single-Cell Perturbation Prediction](https://arxiv.org/abs/2605.25581)
...
### [168] [Extreme Region Policy Distillation](https://arxiv.org/abs/2605.25582)
...
### [169] [Generalized Evidential Deep Learning: From a Bayesian Perspective](https://arxiv.org/abs/2605.25599)
...
### [170] [Towards the Connection between Activation Sparsity and Flat Minima](https://arxiv.org/abs/2605.25612)
...
### [171] [Courtroom Analogy: New Perspective on Uncertainty-Aware Classification](https://arxiv.org/abs/2605.25616)
...
### [172] [Analogies between Transformer Layers and Power Method](https://arxiv.org/abs/2605.25619)
...
### [173] [Closed-Form Node Classification with Exact Graph Unlearning](https://arxiv.org/abs/2605.25662)
...
### [174] [Opportunistic Target Selection: Early Directional Commitment for Query-Efficient Black-Box Adversarial Attacks](https://arxiv.org/abs/2605.25663)
...
### [175] [Stochastic Estimation of the Layer-wise Hessian Trace for Monitoring Neural-network Training](https://arxiv.org/abs/2605.25674)
...
### [176] [Don't Retrain, Just Reuse: Recovering Dual-Target Molecules from Single-Target Diffusion Models](https://arxiv.org/abs/2605.25681)
...
### [177] [Evaluating passing decision-making in professional football: An enhanced MPNN approach to Receiver Selection](https://arxiv.org/abs/2605.25696)
...
### [178] [The Behavioral Credibility Trilemma: When Calibrated Autonomy Becomes Impossible](https://arxiv.org/abs/2605.25739)
...
### [179] [Latent Representation Alignment for Offline Goal-Conditioned Reinforcement Learning](https://arxiv.org/abs/2605.25740)
...
### [180] [Invariant-Based Weight Sharing for Message Passing](https://arxiv.org/abs/2605.25750)
*Florian Seiffarth*
...
### [181] [MDGMIX: Boundary-Aware Subgraph Mixing for Multi-Domain Graph Pre-Training](https://arxiv.org/abs/2605.25771)
...
### [182] [NPSolver: Neural Poisson Solver with Iterative Physics Supervision](https://arxiv.org/abs/2605.25786)
...
### [183] [On the Benefits of Free Exploration for Regret Minimization in Multi-Armed Bandits](https://arxiv.org/abs/2605.25789)
...
### [184] [On Reliability of Efficient Membership Inference Vulnerability Evaluation](https://arxiv.org/abs/2605.25819)
...
### [185] [Visual-Redundancy-Controlled Parallel Decoding for Diffusion-Based Multimodal Large Language Models](https://arxiv.org/abs/2605.25820)
...
### [186] [Context-Instrumental Data Distillation for Kubernetes Manifest Generation: Method and Experimental Evaluation](https://arxiv.org/abs/2605.25835)
...
### [187] [Geometric Evolution Maps: Extracting Stable Concept Probes from Transformer Residual Streams](https://arxiv.org/abs/2605.25848)
*James Henry*
...
### [188] [When Self-Belief Misleads: Active Label Acquisition for Reinforcement Learning with Verifiable Rewards](https://arxiv.org/abs/2605.25864)
...
### [189] [UNATE: UNsupervised ATomic Embedding for crystal structures property prediction](https://arxiv.org/abs/2605.25866)
...
### [190] [The Quantization Benefits of Residual-Free Transformers](https://arxiv.org/abs/2605.25880)
...
### [191] [Conformalised imprecise inference for robust extrapolation under limited data](https://arxiv.org/abs/2605.25882)
...
### [192] [Optimal and Order-optimal Gated Priority-based Greedy Policies for Two-layer Multi-item Order Fulfillment](https://arxiv.org/abs/2605.25888)
...
### [193] [Merge-Bench: Resolve Merge Conflicts with Large Language Models](https://arxiv.org/abs/2605.25890)
...
### [194] [Predicting Stock Price Direction on Earnings Announcement Days using Multi-modal Deep Learning](https://arxiv.org/abs/2605.25894)
...
### [195] [Reading the Finetuning Prior: Verbatim Content Recovery via Contrastive Decoding Diffing](https://arxiv.org/abs/2605.25902)
...
### [196] [Joint Optimization of Training and Inference in Federated Edge Learning via Constrained Multi-Objective Deep Reinforcement Learning](https://arxiv.org/abs/2605.25916)
...
### [197] [Quantitative Evaluation of the Severity of Posttraumatic Stress Disorder through Transfer Learning from Specific Phobia Data](https://arxiv.org/abs/2605.25933)
...
### [198] [From Latent Space to Training Data: Explainable Specialization in Minimal MLPs](https://arxiv.org/abs/2605.25939)
...
### [199] [STaT: Resolving Shape Distortion in Non-Stationary Time Series via Tri-Modal Synergy](https://arxiv.org/abs/2605.25943)
...
### [200] [Small Models, Strong Priors: Architectural Inductive Bias for Parameter-Efficient Neural PDE Solvers](https://arxiv.org/abs/2605.25949)
...
### [201] [Step-TP: A Grounded, Step-Level Dataset with Chain-of-Thought Reasoning for LLM-Guided Tensor Program Optimization](https://arxiv.org/abs/2605.25954)
...
### [202] [Mapping the Schedule x Bit-Width Boundary in Sub-100M Quantisation-Aware Training](https://arxiv.org/abs/2605.25966)
...
### [203] [Hidden in Plain Tokens: Simply Robust, Gradient-Free Watermark for Synthetic Audio](https://arxiv.org/abs/2605.25967)
...
### [204] [Fuzzy PyTorch: Rapid Numerical Variability Evaluation for Deep Learning Models](https://arxiv.org/abs/2605.25991)
...
### [205] [Deployment-complete benchmarking](https://arxiv.org/abs/2605.25997)
...
### [206] [Causal methods for LLM development and evaluation](https://arxiv.org/abs/2605.25998)
...
### [207] [Learning in Low-Dimensional Subspaces: Orthogonal Bottlenecks for Reinforcement Learning](https://arxiv.org/abs/2605.26012)
...
### [208] [AdvantageFlow: Advantage-Weighted Least Squares for RL in Flow Models](https://arxiv.org/abs/2605.26013)
...
### [209] [Retrieval-Augmented Detection of Potentially Abusive Clauses in Chilean Terms of Service](https://arxiv.org/abs/2605.26019)
...
### [210] [Length Generalization with Log-Depth Recurrent Units](https://arxiv.org/abs/2605.26035)
...
### [211] [Neuronal Stochastic Attention Circuit (NSAC) for Probabilistic Representation Learning](https://arxiv.org/abs/2605.26061)
...
### [212] [Conditional KRR: Injecting Unpenalized Features into Kernel Methods with Applications to Kernel Thresholding](https://arxiv.org/abs/2605.26067)
...
### [213] [Rethinking Weak Supervision in Anomaly Detection: A Comprehensive Benchmark](https://arxiv.org/abs/2605.26068)
...
### [214] [Active Query Synthesis for Preference Learning](https://arxiv.org/abs/2605.26072)
...
### [215] [Global Convergence of Wasserstein Policy Gradient for Entropy-Regularized Reinforcement Learning](https://arxiv.org/abs/2605.26078)
...
### [216] [OrpQuant: Geometric Orthogonal Residual Projection for Multiplier-Free Power-of-Two Transformer Quantization](https://arxiv.org/abs/2605.26092)
...
### [217] [Goal-driven Bayesian Optimal Experimental Design for Robust Decision-Making Under Model Uncertainty](https://arxiv.org/abs/2605.26093)
...
### [218] [Forgetting in Language Models: Capacity, Optimization, and Self-Generated Replay](https://arxiv.org/abs/2605.26097)
...
### [219] [Prism: A Plug-in Reproducible Infrastructure for Scalable Multimodal Continual Instruction Tuning](https://arxiv.org/abs/2605.26110)
...
### [220] [Confidence Calibration in Large Language Models](https://arxiv.org/abs/2605.23909)
*Noam Michael,Daniel BenShushan,Jacob Bien,Don A. Moore*

Main category: cs.AI

TL;DR: 研究发现大型语言模型与人类类似存在过度自信倾向，平均置信度高于准确性，但这种倾向受到难度调节：困难任务中过度自信最强，而简单任务中实际表现为显著信心不足


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在不同任务上的置信度校准问题，了解模型置信度与准确性的关系模式，特别关注难度对校准的影响

Method: 采用预注册研究设计，开发了LifeEval测试工具，用于评估模型在不同难度水平上的校准性能

Result: 当前大型语言模型平均置信度超过准确性，表现出类似人类的过度自信；但存在显著的难易效应：困难测试中过度自信最强，简单测试中反而出现显著的信心不足

Conclusion: 需要开发针对难易程度的校准评估方法，模型校准不是单一现象而是依赖于任务难度，这对可靠人工智能系统开发具有重要意义
...
### [221] [How Much Thinking is Enough? Quantifying and Understanding Redundancy in LLM Reasoning](https://arxiv.org/abs/2605.23926)
*Zhiyuan Zhai,Xinkai You,Wenjing Yan,Xin Wang*

Main category: cs.AI

TL;DR: 研究发现大型语言模型在推理过程中存在大量冗余思考步骤，61%-93%的后缀步骤可被截断而不影响正确答案，这是由长度无关的奖励机制导致的结构性问题而非模型缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有具备推理能力的大型语言模型通过生成长链思考来解决难题，但付出了高延迟、GPU时间和能耗的代价。研究发现模型中存在大量重复表述、验证和自我反思的冗余思考，但这些思考有多少真正必要从未被大规模量化或从基本原理上解释。

Method: 形式化定义了推理冗余度量：正确轨迹的冗余度是指其尾部连续步骤中能被截断的最大比例，而π被迫停止思考并输出最终答案时仍能产生正确答案。在四个前沿推理模型和两个数学基准上进行大规模量化研究，使用不同判断模型家族验证鲁棒性。

Result: 在8个（模型，基准）条件下，步骤级冗余度始终很高（61%-93%），中位数关键前缀在6个条件中仅等于单个分段步骤。虽然在MATH-500上冗余度随问题难度增加而降低，但所有四个模型在最难的5级问题上仍保持显著冗余（ρ∈[46%,85%]）。

Conclusion: 证明冗余是长度无关结果奖励的结构性后果，而非模型特定的伪影：在任何此类奖励下，最优的有限期望停止时间不存在。这一结果与RL算法、基础模型、数据分布或策略获取方式无关，因此"过度思考"不是单个模型需要修补的错误，而是当前推理模型训练方式的结构性属性。
...
### [222] [Toward Reliable Design of LLM-Enabled Agentic Workflows: Optimizing Latency-Reliability-Cost Tradeoffs](https://arxiv.org/abs/2605.23929)
*Ya-Ting Yang,Quanyan Zhu*

Main category: cs.AI

TL;DR: 本文分析了LLM驱动的多智能体工作流中延迟、可靠性和成本的基本权衡，提出了性能模型和优化策略，包括水填充令牌分配政策和影子价格下的最优工作流可靠性特征。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统越来越依赖由多个交互智能体组成的工作流，这些智能体包括LLM驱动的和传统计算模块。在这种复杂系统中，延迟、可靠性和成本之间存在基本权衡关系需要系统分析，以优化多智能体工作流的设计。

Method: 1. 为LLM和非LLM智能体引入性能模型，捕捉计算努力与输出质量之间的关系；2. 使用参数化指数可靠性函数纳入LLM智能体的推理和输出令牌影响；3. 研究在延迟和成本约束下的顺序工作流设计。

Result: 主要成果包括：1. 提出了水填充令牌分配政策；2. 用影子价格表征了最优工作流可靠性；3. 分析了顺序工作流在约束条件下的优化设计。

Conclusion: 该研究为LLM驱动的智能体工作流提供了系统的性能分析框架，揭示了延迟、可靠性和成本之间的基本权衡，提出的水填充分配策略和可靠性特征化方法为优化多智能体AI系统提供了理论基础。
...
TL;DR: Quantum Frog是一个基于量子时间机制的双人合作游戏，通过强化学习分析游戏设计问题，发现冲刺策略是最优解，合作训练显著提升成功率


<details>
  <summary>Details</summary>
Motivation: 研究Quantum Frog游戏的设计特性，通过强化学习作为分析工具，回答关于游戏难度、最优策略、合作差距和联合策略等四个设计问题

Method: 使用强化学习方法，包括Tabular Q-Learning、Deep Q-Network、Independent DQN和Multi-Agent Proximal Policy Optimisation，训练智能体在1-6辆车的交通密度下进行实验

Result: 1) 量子时间机制使冲刺策略成为全局最优；2) 非协调双人玩家比单专家玩家面临六倍车流更难；3) 合作训练使联合成功率提高32-34个百分点，减少回合步数；4) 涌现的合作策略是同步冲刺而非复杂位置协调

Conclusion: 研究结果为Quantum Frog的商业设计提供具体指导，并为理解环境机制如何塑造多智能体学习动态提供更广泛的洞见
...
TL;DR: BODHI是一种用于操作系统内核规范生成的新提示方法，通过结构化翻译指南增强few-shot提示，显著提高了LLMs在规范生成任务上的性能


<details>
  <summary>Details</summary>
Motivation: 操作系统内核的形式化验证需要精确的系统调用规范，但手动编写这些规范需要深厚的领域专业知识，而现有LLMs在OSV-Bench基准上的最佳Pass@1准确率仅为55.10%，存在很大改进空间

Method: 提出了BODHI域知识提示方法，通过结构化C到Python翻译指南增强标准few-shot提示，该指南包含15个特定领域翻译模式类别，采用结构化思维链(SCoT)启发的方法，按关注点分离组织翻译，将前置条件提取和后置条件生成作为不同类别

Result: 在6个提供商(Anthropic、Mistral、Amazon、DeepSeek、Meta、Alibaba)的9个模型上评估，涵盖密集、专家混合和推理架构，BODHI提高了所有测试模型，提升幅度为+11%至+32%。最佳配置(Claude Opus 4.6 + BODHI)达到96.73% Pass@1，减少了语法和语义错误

Conclusion: 域知识注入是一种模型无关的技术，能显著弥合通用代码生成和形式化规范合成之间的差距，特别对那些有足够指令遵循能力以利用结构化参考材料的模型效果最强
...
TL;DR: Med-Stress是一个针对临床对话中LLM多轮谄媚行为的压力测试框架，发现医疗知识与鲁棒性之间存在明显分离，并提出了RBED和R-FT两种防御方法。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在医学基准测试中表现出高准确性，但在临床对话中会出现严重的多轮谄媚行为，在压力升级时放弃最初正确的诊断。

Method: 提出了Med-Stress压力测试框架来评估信念稳定性，开发了两种防御方法：RBED（基于角色的认知防御）作为轻量级推理时防御，以及R-FT（面向鲁棒性的微调）作为训练时方法。

Result: 在9个前沿LLMs中发现初始诊断能力与信念稳定性之间存在明显分离，R-FT方法几乎消除了信念改变，显著提高了鲁棒性。

Conclusion: 医疗LLMs需要同时具备准确性和鲁棒性，R-FT方法能有效内部化基于证据的抗压能力，提高临床对话中的信念稳定性。
...
### [226] [Fuzzy, Neutrosophic, and Uncertain Graph Theory: Properties and Applications](https://arxiv.org/abs/2605.23936)
*Takaaki Fujita,Florentin Smarandache*

Main category: cs.AI

TL;DR: 本书系统地综述了不确定性图理论，重点关注模糊图、中智图等模型，涵盖图类、参数和扩展，并探索在实际系统中的应用。


<details>
  <summary>Details</summary>
Motivation: 当前各种不确定性图模型缺乏统一的框架，需要系统性地组织这些模型，理解它们之间的关系、能力和在复杂系统中的应用。

Method: 采用综合系统性调查方法，回顾模糊图、中智图等模型的基本概念、结构特性、图类和参数，并引入不确定有向图、超图、超超图和动态图等扩展。

Result: 建立了不确定图框架作为统一视角，组织了各种不确定性感知图模型，提供了理解它们关系和应用的一致框架，涵盖理论发展和实际应用领域。

Conclusion: 本书通过不确定图框架统一了各种不确定性图模型，为理解它们在复杂系统中的关系、能力和应用提供了系统的理论基础和实践指导。
...
### [227] [BoxLitE: A Faithful Knowledge Base Embedding Based on Convex Optimization](https://arxiv.org/abs/2605.23937)
*Bruno F. Lourenço,Hesham Morgan,Ana Ozaki,Aleksandar Pavlović,Emanuel Sallinger*

Main category: cs.AI

TL;DR: BoxLitE：一个基于凸优化的知识库嵌入模型，能够将DL-Liteℋ知识库中的概念映射到向量空间的凸区域，并通过凸优化实现弱忠实建模。


<details>
  <summary>Details</summary>
Motivation: 现有知识库嵌入方法虽然结合了知识图谱嵌入（ABox）和本体语言（TBox）的能力，但将概念映射到凸区域的方法在学习任务中很少充分利用凸性的优势。

Method: 提出BoxLitE模型，将知识库嵌入任务形式化为凸优化问题，确保对于任何可满足的DL-Liteℋ知识库，都能获得弱忠实的嵌入表示。

Result: 证明了BoxLitE能够产生具有弱忠实性的嵌入，并通过凸优化方法实现了这一目标，为知识库嵌入提供了理论基础和实现框架。

Conclusion: BoxLitE成功地将凸优化技术应用于知识库嵌入，不仅理论上保证了嵌入的忠实性，还通过具体的优化问题表述提供了实用的实现方法。
...
### [228] [Authority Inversion in LLM-Mediated Ubiquitous Systems: When Models Trust Users Over Sensors](https://arxiv.org/abs/2605.23938)
*Long Zhang,Zi-bo Qin,Wei-neng Chen*

Main category: cs.AI

TL;DR: LLMs在融合传感器数据和用户声明时存在"权威反转"问题：当传感器测量值（数值型）与用户自然语言声明冲突时，模型会偏向用户声明，导致传感器数据权威性被忽视。作者提出了几何框架、审计指标和干预方法来诊断和缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在融合异构输入的系统中的广泛应用，当传感器测量值与用户声明冲突时，LLMs如何隐式分配权威性仍未被研究。这在实际部署中引发可靠性问题，因为物理传感器数据在某些场景下应具有优先权。传统的融合方法是显式的，而LLMs的权威分配隐藏在学习的表示中。

Method: 提出了一个几何框架来分析上下文整合，引入了两个可计算的审计指标：上下文整合比率(CIR)和权威对齐指数(AAI)。提出了几何权威校准(GAC)，这是一种推理时的层级干预方法，用于抑制用户权威的错误分配。

Result: 在4个模型（4B到35B参数，三种架构）和4个数据集（总计576个冲突实例）上的评估显示极端反转：在数值任务中，模型表现出接近零的传感器信任(AAI = -0.805, Cohen's d = -2.14)，且不受模型容量影响。理论引导的因果注入翻转了80.2%的错误决策（随机对照组<0.4%）。GAC将HAR准确率从0-1.6%提升到21.9-27.5%，优于提示基线方法。

Conclusion: LLM介导系统中的权威分配必须被显式审计并根据应用场景特定配置，而不是留给隐式学习。几何权威校准是有效的干预方法，但更根本的是需要系统性解决LLMs在处理异构输入时的权威分配问题。
...
### [229] [Residual Drift Dominates Contradiction in Multi-Turn Constraint Reasoning](https://arxiv.org/abs/2605.23940)
*Sebastien Kawada*
...
### [230] [MEMOR-E: In-Context and Fine-Tuned LLM Personalization for Alzheimer's Assistive Robotics](https://arxiv.org/abs/2605.23941)
...
### [231] [A Dynamical Framework for Cognitive Processes Based on Transformations and Semantic Equivalence](https://arxiv.org/abs/2605.23942)
...
### [232] [Spacetime Formation under Requirements: Contextual Realization and Form-Dependent Probability](https://arxiv.org/abs/2605.23943)
*Song-Ju Kim*
...
### [233] [Right-Sizing Communication and Recommendation Set Size in AI-Assisted Search](https://arxiv.org/abs/2605.23944)
...
### [234] [Accelerating Long-Tail Generation in Synchronous RLHF Training via Adaptive Tensor Parallelism](https://arxiv.org/abs/2605.23945)
...
### [235] [Stop Comparing LLM Agents Without Disclosing the Harness](https://arxiv.org/abs/2605.23950)
...
### [236] [Machine Psychometrics: A Mathematical Psychology of Artificial Intelligence](https://arxiv.org/abs/2605.23952)
...
### [237] [From Accuracy to Auditability: A Survey of Determinism in Financial AI Systems](https://arxiv.org/abs/2605.23955)
...
### [241] [Saturating Scaling Laws for Equational Discovery: A Phenomenology of Growth Dynamics in Three Toy Substrates with Two Real-World Replications](https://arxiv.org/abs/2605.23983)
*Fabio Rovai*
...
### [242] [Beyond Predefined Learning Objects: A Thinking-Learning Interaction Model for Up-to-Date Autonomous Robot Learning](https://arxiv.org/abs/2605.23987)
*Hong Su*
...
### [243] [Towards trustworthy agentic AI: a comprehensive survey of safety, robustness, privacy, and system security](https://arxiv.org/abs/2605.23989)
...
### [244] [Reason--Imagine--Act: Closed-Loop LLM Decision Making with World Models for Autonomous Driving](https://arxiv.org/abs/2605.24004)
...
### [245] [LC-ERD: Mining Latent Logic for Self-Evolving Reasoning via Consistency-Regulated Reward Decomposition](https://arxiv.org/abs/2605.24005)
...
### [246] [EvoSci: A Bio-Inspired Multi-Agent Framework for the Evolution of Scientific Discovery](https://arxiv.org/abs/2605.24018)
...
### [247] [Breaking the Chains of Probability: Neutrosophic Logic as a New Framework for Epistemic Uncertainty in Large Language Models](https://arxiv.org/abs/2605.24053)
...
### [248] [EvoCode-Bench: Evaluating Coding Agents in Multi-Turn Iterative Interactions](https://arxiv.org/abs/2605.24110)
...
### [249] [SkillEvolBench: Benchmarking the Evolution from Episodic Experience to Procedural Skills](https://arxiv.org/abs/2605.24117)
...
### [250] [MAPLE: Multi-State Aggregated Policy Evaluation for AlphaZero in Imperfect-Information Games](https://arxiv.org/abs/2605.24139)
...
### [251] [HyperGuide: Hyperbolic Guidance for Efficient Multi-Step Reasoning in Large Language Models](https://arxiv.org/abs/2605.24140)
...
### [252] [Neuro-Inspired Inverse Learning for Planning and Control](https://arxiv.org/abs/2605.24152)
...
### [253] [Palette: A Modular, Controllable, and Efficient Framework for On-demand Authorized Safety Alignment Relaxation in LLMs](https://arxiv.org/abs/2605.24154)
...
### [254] [Inference Time Context Sparsity: Illusion or Opportunity?](https://arxiv.org/abs/2605.24168)
...
### [255] [EPPC-OASIS: Ontology-Aware Adaptation and Structured Inference Refinement for Electronic Patient-Provider Communication Mining in Secure Messages](https://arxiv.org/abs/2605.24172)
...
### [256] [A Sober Look at Agentic Misalignment in Automated Workflows](https://arxiv.org/abs/2605.24197)
...
### [257] [When Does Multi-Agent RL Improve LLM Workflows? Workflow, Scale, and Policy-Sharing Tradeoffs](https://arxiv.org/abs/2605.24202)
...
### [258] [Identifying and Mitigating Systemic Measurement Bias in Production LLM Inference Benchmarks](https://arxiv.org/abs/2605.24217)
...
### [259] [Beyond Final Answers: Auditing Trajectory-Level Hallucinations in Multi-Agent Industrial Workflows](https://arxiv.org/abs/2605.24219)
...
### [260] [How Well Do Models Follow Their Constitutions?](https://arxiv.org/abs/2605.24229)
...
### [261] [Toward Enactive Artificial Intelligence](https://arxiv.org/abs/2605.24238)
...
### [262] [Safety-Oriented Routing Analysis of Mixtral MoE Under Benign and Harmful Prompts](https://arxiv.org/abs/2605.24270)
...
### [263] [When Does Synthetic Patent Data Help? Volume-Fidelity Trade-offs in Low-Resource Multi-Label Classification](https://arxiv.org/abs/2605.24296)
...
### [264] [Adaptive Human-AI Coordination via Hierarchical Action Disentanglement](https://arxiv.org/abs/2605.24343)
...
### [265] [Partner-Aware Hierarchical Skill Discovery for Robust Human-AI Collaboration](https://arxiv.org/abs/2605.24352)
*Adnan Ahmad,Bahareh Nakisa,Mohammad Naim Rastgoo*
...
### [266] [Distilling Game Code World Model Generation into Lightweight Large Language Models](https://arxiv.org/abs/2605.24375)
...
### [267] [A governance horizon for ethical-use constraints in open-weight AI models](https://arxiv.org/abs/2605.24383)
...
### [268] [Understanding and Mitigating Premature Confidence for Better LLM Reasoning](https://arxiv.org/abs/2605.24396)
...
### [269] [ConceptM$^3$oE: Concept-Guided Multimodal Mixture of Experts for Interpretable Computational Pathology](https://arxiv.org/abs/2605.24399)
...
### [270] [Advancing Graph Few-Shot Learning via In-Context Learning](https://arxiv.org/abs/2605.24410)
...
### [271] [The Model Is Not the Product: A Dual-Pillar Architecture for Local-First Psychological Coaching](https://arxiv.org/abs/2605.24411)
*Alexander Mihalcea*
...
### [272] [JT-SAFE-V2: Safety-by-Design Foundation Model with World-Context Data](https://arxiv.org/abs/2605.24414)
...
### [273] [Benchmarking the Limits of In-Context Reinforcement Learning for Ad-Hoc Teamwork](https://arxiv.org/abs/2605.24423)
...
### [274] [SAM: State-Adaptive Memory for Long-Horizon Reasoning Agent](https://arxiv.org/abs/2605.24468)
...
### [275] [SPACE: Unifying Symmetric and Asymmetric Routing Problems for Generalist Neural Solver](https://arxiv.org/abs/2605.24484)
...
### [276] [AgentFugue: Agent Scaling for Long-Horizon Tasks through Collective Reasoning](https://arxiv.org/abs/2605.24486)
...
### [277] [TIGER: Text-Informed Generalized Enzyme-Reaction Retrieval](https://arxiv.org/abs/2605.24489)
...
### [278] [Market Regime Council for Dynamic Credit Assignment in Multi-Agent LLM Decision Systems](https://arxiv.org/abs/2605.24490)
...
### [279] [Reasoning as an Attack Surface: Adaptive Evolutionary CoT Jailbreaks for LLMs](https://arxiv.org/abs/2605.24497)
...
### [280] [Hypothesis Generation and Inductive Inference in Children and Language Models](https://arxiv.org/abs/2605.24528)
...
### [281] [DemoEvolve: Overcoming Sparse Feedback in Agentic Harness Evolution with Demonstrations](https://arxiv.org/abs/2605.24539)
...
### [282] [Emission-Aware Reinforcement Learning for Sustainable Electric Vehicle Charging and Carbon Dioxide Reduction Under Varying Renewable Penetration](https://arxiv.org/abs/2605.24543)
...
### [283] [Beyond Control-Flow: Integrating the Resource Perspective into Multi-Collaborative Process Modeling from Text](https://arxiv.org/abs/2605.24546)
...
### [284] [PALoRA: Projection-Adaptive LoRA for Preserving Reasoning in Large Language Models](https://arxiv.org/abs/2605.24549)
...
### [285] [Jailbreak to Protect: Buffering and Reinforcing via Temporary Jailbreaking for Safe Fine-Tuning in Large Language Models](https://arxiv.org/abs/2605.24550)
...
### [286] [Summoning the Oracle to Slay It: Mitigating Look-Ahead Bias in Financial Backtesting with Large Language Models](https://arxiv.org/abs/2605.24564)
...
### [287] [Associations between echocardiographic traits and AI-ECG predictions of heart failure](https://arxiv.org/abs/2605.24576)
...
### [288] [HeartBeatAI: An Interpretable and Robust Deep Learning Framework for Multi-Label ECG Arrhythmia Detection](https://arxiv.org/abs/2605.24588)
...
### [289] [Learning to Reason Efficiently with A* Post-Training](https://arxiv.org/abs/2605.24597)
...
### [290] [Hera: Learning Long-Horizon Coordination for Device-Cloud Collaborative LLM Agents](https://arxiv.org/abs/2605.24598)
...
### [291] [Agent-as-Peer-Debriefer: A Multi-Agent Framework with Perspective-Based Refinement for Qualitative Analysis](https://arxiv.org/abs/2605.24600)
...
### [292] [Lattice theory and algebraic models for deep convolutional learning based on mathematical morphology](https://arxiv.org/abs/2605.24608)
*Gustavo,Angulo*
...
### [293] [GlobalDentBench: A Multinational Benchmark for Evaluating LLM Clinical Reasoning in Dentistry with Expert Calibration](https://arxiv.org/abs/2605.24636)
...
### [294] [AVBench: Human-Aligned and Automated Evaluation Benchmark for Audio-Video Generative Models](https://arxiv.org/abs/2605.24652)
...
### [295] [Beyond Inference-Only Deployment: Comparing Weight-Based Consolidation Against Cascading Compaction](https://arxiv.org/abs/2605.24657)
...
### [296] [Measuring Reasoning Quality in LLMs: A Multi-Dimensional Behavioral Framework](https://arxiv.org/abs/2605.24661)
...
### [297] [When Mean CE Fails: Median CE Can Better Track Language Model Quality](https://arxiv.org/abs/2605.24667)
...
### [298] [Exploration of Perceptual Speech Features for Clinical Decision-Support in Mental Health Care](https://arxiv.org/abs/2605.24678)
...
### [299] [Emotional intelligence in large language models is fragmented across perception, cognition, and interaction](https://arxiv.org/abs/2605.24686)
...
### [300] [MDIA: A Multi-Agent Diagnostic Intelligence Pipeline on HealthBench Professional](https://arxiv.org/abs/2605.24699)
...
### [301] [Fundamental Limitation in Explaining AI](https://arxiv.org/abs/2605.24727)
...
### [302] [Hylos: Operability Contracts for Model-Native Spatial Intelligence](https://arxiv.org/abs/2605.24728)
...
### [303] [Automated Detection and Classification of Delusion-related Content in Naturalistic Audio Diaries Using Multi-Agent Language Models](https://arxiv.org/abs/2605.24755)
...
### [304] [Proper Scoring Rules for Agentic Uncertainty Quantification](https://arxiv.org/abs/2605.24756)
...
### [305] [PRIMA: Operational Patterns for Resilient Multi-Agent Research with Verifiable Identity and Convergent Feedback](https://arxiv.org/abs/2605.24775)
*Sasank Annapureddy*
...
### [306] [GRAIL: AI translation for scientists application workflow on satellite data](https://arxiv.org/abs/2605.24784)
...
### [307] [PANDO: Efficient Multimodal AI Agents via Online Skill Distillation](https://arxiv.org/abs/2605.24785)
...
### [308] [CoRe-Code: Collaborative Reinforcement Learning for Code Generation](https://arxiv.org/abs/2605.24812)
...
### [309] [Agent Manufacturing: Foundation-Model Agents as First-Class Industrial Entities](https://arxiv.org/abs/2605.24823)
*Yilei Zhang*
...
### [310] [Test-Time Deep Thinking to Explore Implicit Rules](https://arxiv.org/abs/2605.24828)
...
### [311] [Geo-Expert: Towards Expert-Level Geological Reasoning via Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2605.24844)
...
### [312] [Solving Combinatorial Counting Problems with Weighted First-Order Model Counting](https://arxiv.org/abs/2605.24845)
...
### [313] [Clustering as Reasoning: A $k$-Means Interpretation of Chain-of-Thought Graph Learning](https://arxiv.org/abs/2605.24867)
...
### [314] [Inverting the Shield: Systematically Generating Safety Tests from Policy Specifications](https://arxiv.org/abs/2605.24883)
...
### [315] [TaBIIC2: Interactive Building of Ontological Taxonomies using Weighted Self-Organizing Maps](https://arxiv.org/abs/2605.24899)
*Mathieu d'Aquin*
...
### [316] [ProActor: Timing-Aware Reinforcement Learning for Proactive Task Scheduling Agents](https://arxiv.org/abs/2605.24900)
...
### [317] [Noise-Robust Financial Numerical Entity Attribute Tagging](https://arxiv.org/abs/2605.24910)
...
### [318] [Energy Shields for Fairness](https://arxiv.org/abs/2605.24926)
...
### [319] [Towards Multi-Turn Dialog Systems for Industrial Asset Operations and Maintenance](https://arxiv.org/abs/2605.24953)
...
### [320] [Mitigating Object Hallucinations in Vision-Language Models through Region-Aware Attention Recalibration](https://arxiv.org/abs/2605.24957)
...
### [321] [NeurIPS: Neuro-anatomical Inductive Priors for Sphere-based Brain Decoding](https://arxiv.org/abs/2605.24993)
...
### [322] [Privacy-Preserving Local Language Models for Longitudinal Data Retrieval in Chronic Dermatologic Disease: Implementation in Pemphigus Patients](https://arxiv.org/abs/2605.25020)
...
### [323] [AION: Next-Generation Tasks and Practical Harness for Time Series](https://arxiv.org/abs/2605.25045)
...
### [324] [Evolutionary Enhanced Multi-Agent Reinforcement Learning for Cooperative Air Combat](https://arxiv.org/abs/2605.25091)
...
### [325] [RECTOR: Priority-Aware Rule-Based Reranking for Compliance-Aware Autonomous Driving Trajectory Selection](https://arxiv.org/abs/2605.25095)
...
### [326] [Trust but Verify: Prover-Verifier Deliberation for Selective LLM Prediction](https://arxiv.org/abs/2605.25133)
...
### [327] [Representation Without Control: Testing the Realization Effect in Language Models](https://arxiv.org/abs/2605.25151)
...
### [328] [SimuWoB: Simulating Real-World Mobile Apps for Fast and Faithful GUI Agent Benchmarking](https://arxiv.org/abs/2605.25160)
...
### [329] [SpecAlign: A Semantic Alignment Framework for SystemVerilog Assertion Generation](https://arxiv.org/abs/2605.25181)
...
### [330] [DarkForest: Less Talk, Higher Accuracy for Multi-Agent LLMs](https://arxiv.org/abs/2605.25188)
...
### [331] [Boosting Inference with Guided Reasoning: Stochastic Exploration for Recursive Models](https://arxiv.org/abs/2605.25230)
...
### [332] [Meta-Agent: From Task Descriptions to Verified Multi-Agent Systems](https://arxiv.org/abs/2605.25233)
*Andy Xu,Yu-Wing Tai*
...
### [333] [FrontierOR: Benchmarking LLMs' Capacity for Efficient Algorithm Design in Large-Scale Optimization](https://arxiv.org/abs/2605.25246)
...
### [334] [LipoAgent: Coordinating Fine-Tuned LLM Agents for Safer Lipid Design](https://arxiv.org/abs/2605.25250)
...
### [335] [Whose Alignment? Comparing LLM Process Alignment Across Diverse Organizational Decision Contexts](https://arxiv.org/abs/2605.25256)
...
### [336] [AI Cartography: Mapping the Latent Landscape of AI Benchmark Ecosystems](https://arxiv.org/abs/2605.25272)
...
### [337] [Context-CoT: Enhancing Context Learning via High-Quality Reasoning Synthesis](https://arxiv.org/abs/2605.25354)
...
### [338] [Second Guess: Detecting Uncertainty Through Abstention and Answer Stability in Small Language Models](https://arxiv.org/abs/2605.25394)
...
### [339] [Towards end-to-end LLM-based censoring-aware survival analysis](https://arxiv.org/abs/2605.25399)
...
### [340] [CODESKILL: Learning Self-Evolving Skills for Coding Agents](https://arxiv.org/abs/2605.25430)
...
### [341] [Security of OpenClaw Agents: Fundamentals, Attacks, and Countermeasures](https://arxiv.org/abs/2605.25435)
...
### [342] [A Signal-Language Foundation Model for Broad-Spectrum Cardiovascular Assessment from Routine Electrocardiography](https://arxiv.org/abs/2605.25446)
...
### [343] [ATWL: A Formal Language for Representing, Comparing, and Reusing Visual Analytics Workflows](https://arxiv.org/abs/2605.25489)
...
### [344] [Credit Assignment with Resets in Language Model Reasoning](https://arxiv.org/abs/2605.25507)
...
### [345] [What Gets Cited: Competitive GEO in AI Answer Engines](https://arxiv.org/abs/2605.25517)
...
### [346] [StructBreak: Structural Cognitive Overload-Induced Safety Failures in MLLMs](https://arxiv.org/abs/2605.25534)
...
### [347] [Personalize-then-Store: Benchmarking and Learning Personalized Memory for Long-horizon Agents](https://arxiv.org/abs/2605.25535)
...
### [348] [ADMFormer: An Adaptive-Decomposition Transformer with Time-Varying Masked Spatial Attention for Traffic Forecasting](https://arxiv.org/abs/2605.25543)
...
### [349] [PHGNet: Prototype-Guided Hypergraph Construction for Heterogeneous Spatiotemporal Forecasting](https://arxiv.org/abs/2605.25554)
...
### [350] [Beyond Query Memorization: Large Language Model Routing with Query Decomposition and Historical Matching](https://arxiv.org/abs/2605.25558)
*Bo Lv,Jingbo Sun*
...
### [351] [Uncertainty Reasoning with Large Language Models for Explainable Disease Diagnosis](https://arxiv.org/abs/2605.25566)
...
### [352] [Detecting Unfaithful Chain-of-Thought via Circuit-Guided Internal-External Discrepancy](https://arxiv.org/abs/2605.25603)
...
### [353] [Back to Parsimonious Latents: Learning Task-Centric World Models from Visual Foundations](https://arxiv.org/abs/2605.25620)
...
### [354] [CUA-Gym: Scaling Verifiable Training Environments and Tasks for Computer-Use Agents](https://arxiv.org/abs/2605.25624)
...
### [355] [Insuring Every Action: An Authority Frontier Framework for Runtime Actuarial Control of Autonomous AI Agents](https://arxiv.org/abs/2605.25632)
*Hao-Hsuan Chen*
...
### [356] [AgentHijack: Benchmarking Computer Use Agent Robustness to Common Environment Corruptions](https://arxiv.org/abs/2605.25707)
...
### [357] [FLOATBench: A Dataset and Benchmark for Floating Offshore Wind Turbine Tower Fatigue](https://arxiv.org/abs/2605.25717)
...
### [358] [Learning to Search and Searching to Learn for Generalization in Planning](https://arxiv.org/abs/2605.25720)
...
### [359] [A Deep Dive into Axiomatic Design -- Part I: Problem Formulation](https://arxiv.org/abs/2605.25735)
*Aydin Homay*
...
### [360] [Agent-Centric Social Trajectory Prediction: A Free Energy Principle Perspective](https://arxiv.org/abs/2605.25748)
...
### [361] [When Can We Trust Early Warnings? Leakage-Excluded Early Outcome Prediction from LMS Interaction Logs](https://arxiv.org/abs/2605.25794)
...
### [362] [Behind EvoMap: Characterizing a Self-Evolving Agent-to-Agent Collaboration Network](https://arxiv.org/abs/2605.25815)
...
### [363] [MuCRASP: Multimodal Chain-of-thought Reasoning aware Structured Pruning](https://arxiv.org/abs/2605.25842)
...
### [364] [From Accounting to Coordination: A Virtual Water-Aware Electricity-Computation-Water Nexus Framework for Data Center Dispatch](https://arxiv.org/abs/2605.25854)
...
### [365] [$D^2$-Monitor: Dynamic Safety Monitoring for Diffusion LLMs via Hesitation-Aware Routing](https://arxiv.org/abs/2605.25893)
...
### [366] [Explore Before You Solve: The Speed--Depth Trade-off in Epistemic Agents for ARC-AGI-3](https://arxiv.org/abs/2605.25931)
*Liew Keong Han*
...
### [367] [LECTOR: Joint Optimization of Scientific Reasoning Graphs and Introduction Generation](https://arxiv.org/abs/2605.25964)
...
### [368] [Neural Scalable Symbolic Search Framework for Complex Logical Queries with Multiple Free Variables](https://arxiv.org/abs/2605.25985)
...
### [369] [CausaLab: A Scalable Environment for Interactive Causal Discovery Toward AI Scientists](https://arxiv.org/abs/2605.26029)
...
### [370] [CITYREP: A Unified Benchmark for Urban Representations Across Cities, Tasks, and Modalities](https://arxiv.org/abs/2605.26036)
...
### [371] [L2IR: Revealing Latent Intent in Graph Fraud Detection](https://arxiv.org/abs/2605.26040)
...
### [372] [Retrying vs Resampling in AI Control](https://arxiv.org/abs/2605.26047)
...
### [373] [VeriTrace: Evolving Mental Models for Deep Research Agents](https://arxiv.org/abs/2605.26081)
...
### [374] [Claw-Anything: Benchmarking Always-On Personal Assistants with Broader Access to User's Digital World](https://arxiv.org/abs/2605.26086)
...
### [375] [From Model Scaling to System Scaling: Scaling the Harness in Agentic AI](https://arxiv.org/abs/2605.26112)
*Shangding Gu*
