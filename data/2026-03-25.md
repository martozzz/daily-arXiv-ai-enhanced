...
TL;DR: 提出一种离线安全RL算法，通过定义安全条件可达性集合解耦奖励和约束，从固定数据集中学习安全策略，无需环境交互，在性能和安全上均有优势。


<details>
  <summary>Details</summary>
Motivation: 现实任务需要在最大化奖励和满足安全约束之间取得平衡，现有方法往往导致不稳定或对抗性优化。传统的可达性分析方法多针对硬安全约束，而很少扩展到累积成本约束，因此需要一种能有效处理累积安全成本约束且稳定的方法。

Method: 基于离线数据集进行学习，无需与环境交互。具体包括：1. 定义安全条件可达性集合，用于解耦奖励最大化和累积安全成本约束；2. 利用该集合在不使用不稳定min/max或拉格朗日优化的情况下强制安全性。

Result: 实验表明，该方法在标准离线安全RL基准测试及实际海上航行任务中，能够与或优于现有技术基线，同时保证安全性。

Conclusion: 该方法在学习安全策略方面表现出色，特别是在没有环境交互的情况下，通过解耦奖励最大化和累积安全成本的约束，能够有效保证安全性。实验结果验证了该方法在保持安全性的同时，性能与现有技术相当或更优。
...
TL;DR: 分析嵌入空间中合成数据的分布，发现密度与准确率相关，提出了一种改进数据多样性的采样方法，提高了性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决基于大语言模型的合成数据生成方法中数据质量和多样性不足的核心挑战。

Method: 通过分析嵌入空间中生成数据的分布密度与预测准确率的关联性，提出了一种基于嵌入的目标采样流程来增强数据多样性。

Result: 该方法在多个基准测试中一致提升了性能，证明了密度与准确率之间的强相关性。

Conclusion: 本研究展示了一种基于嵌入空间的针对性数据采样方法，能有效提升合成数据的多样性和下游任务的性能。
...
TL;DR: 提出一种基于单次前向传播的跨层一致性不确定性估计方法，在跨数据集迁移和量化条件下优于传统内部表示探测方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)经常表现出过度自信的错误，因此可靠的不确定性估计至关重要。现有的输出启发式方法虽然廉价但脆弱，而探测内部表示的方法有效但维度高且难以迁移。

Method: 提出一种紧凑的、基于单次前向传播的方法，通过评分内部表示中的跨层一致性模式来进行不确定性估计。研究者分析了不同层之间的交互模式，揭示了不同模型编码不确定性的差异。

Result: 在三个模型上的实验表明，该方法在分布内与探测方法性能相当（AUPRC差异最多-1.8百分点，Brier分数+4.9点）。在跨数据集迁移中，它始终优于探测方法，获得高达+2.86 AUPRC和+21.02 Brier点的提升。在4位权重量化下，该方法依然稳健，平均比探测方法提高+1.94 AUPRC点和+5.33 Brier点。

Conclusion: 该研究提出了一种轻量级、紧凑型的跨层一致性不确定性估计方法，能够在不同数据集和量化条件下稳定地优于内部表示探测方法，为LLMs不确定性估计提供了一个高效且可迁移的解决方案。
...
TL;DR: 提出Sparse Feature Attention (SFA)方法，通过特征稀疏性而非传统序列稀疏性来高效扩展Transformer到超长上下文，在保持性能的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前扩展Transformer到超长上下文的主要挑战是自注意力O(n²d)的计算成本。现有方法沿序列轴减少成本（如局部窗口、核近似或token级稀疏）但会降低准确性。本文探索了不同的特征稀疏性轴来解决这一问题。

Method: 提出Sparse Feature Attention (SFA)方法，将查询和键表示为k稀疏编码，将注意力计算成本从Θ(n²d)降低到Θ(n²k²/d)。同时开发FlashSFA内核，扩展FlashAttention直接在稀疏重叠上操作，无需生成密集分数矩阵。

Result: 在GPT-2和Qwen3预训练中，SFA在保持密集基线性能的同时，速度提升最高达2.5倍，FLOPs和KV缓存减少近50%。在合成和下游基准测试中，SFA保持了长上下文下的检索准确性和鲁棒性，优于特征多样性崩溃的短嵌入基线。

Conclusion: SFA通过创新的特征稀疏性方法，实现了在保持模型性能的同时显著降低计算成本和资源消耗，为Transformer扩展到超长上下文场景提供了新的有效途径。
...
TL;DR: 本文提出了一个几何框架，将LLM隐藏状态解释为潜语义流形上的点，揭示了词汇离散化导致的语义失真（表达性间隙）存在理论下界并随词汇量线性增长，为LLM的架构设计和缩放定律提供新视角。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在连续向量空间中进行内部计算，但输出离散令牌——这种根本性不匹配的几何后果尚未被充分理解。本文旨在开发一个数学框架来解释LLM隐藏状态的几何结构，量化词汇离散化带来的语义失真。

Method: 建立了基于Fisher信息度量的潜语义流形理论框架，将LLM隐藏状态解释为该流形上的点，词汇标记对应流形的Voronoi区域划分。定义了表达性间隙这一几何度量，并运用微分几何（coarea公式）、信息论（速率失真理论）和统计方法进行理论分析与实验验证。

Result: 理论证明：(1) 对于任意有限词汇表，存在速率失真理论下界；(2) 通过coarea公式推导出表达性间隙的线性体积缩放定律。实验验证：在六个Transformer架构（1.24亿-15亿参数）上验证了：普遍存在沙漏形固有维数分布；平滑曲率结构；表达性间隙线性缩放（斜率0.87-1.12，R^2 > 0.985）；边界邻近表示的

Conclusion: 本论文提出的几何框架揭示了LLM隐藏状态的几何结构，表明词汇离散化导致的语义失真不可避免（速率失真理论下界），并量化了表达性间隙随词汇量线性增长的定律。该几何视角为LLM的架构设计、模型压缩、解码策略和缩放定律提供了新的理论基础。
...
TL;DR: 本研究使用K-means聚类算法分析大学生数据，将学生分为四类，基于聚类结果为每类学生提供针对性的职业指导建议，提升就业成功率。


<details>
  <summary>Details</summary>
Motivation: 随着信息技术的发展，人工智能和机器学习在教育领域的应用展现出巨大潜力。现有方法主要集中在职业路径预测上，但对于不同特征组合学生在特定职业方向适应性的研究较少。本研究旨在探索如何利用K-means聚类算法为大学生提供精准的职业指导。

Method: 研究分析了3000多名学生的CET-4成绩、GPA、性格特征和学生干部经历等数据，使用K-means聚类算法将学生分成四个主要群体。该算法通过最小化簇内平方误差，将具有相似特征的学生归为一组，确保同一簇内的学生特征高度相似，不同簇之间的差异最大化。

Result: 研究结果表明，具有不同特征组合的学生适合不同的职业方向，这为个性化职业指导提供了科学依据，有效提升了学生的就业成功率。

Conclusion: 该研究通过K-means聚类算法对大学生进行聚类分析，为不同特征组合的学生提供有针对性的职业指导建议，有效提升了学生的就业成功率，为个性化职业指导提供了科学依据。未来研究可以通过扩大样本量、增加特征变量和考虑外部因素来进一步提高聚类的精度和指导效果。
...
TL;DR: 提出基于Wasserstein距离的分布复杂性度量方法，用于无需训练的大语言模型幻觉检测。


<details>
  <summary>Details</summary>
Motivation: 为了解决大语言模型中存在的幻觉问题，需要开发准确、轻量化且广泛适用的检测方法。由于大语言模型在给定提示下定义了一个条件分布，作者认为该分布的复杂性可以作为检测幻觉的指标。

Method: 通过计算样本间标记嵌入的Wasserstein距离矩阵来量化LLM条件分布的复杂性，进而提出两种互补信号：测量平均成本的AvgWD和测量成本复杂性的EigenWD，形成无需训练的幻觉检测器。

Result: 实验表明，AvgWD和EigenWD与强不确定性基线方法竞争激烈，并在不同模型和数据集上表现出互补特性，验证了分布复杂性作为LLM真实性检测信号的有效性。

Conclusion: 该研究表明，基于分布复杂性的Wasserstein距离方法为大型语言模型的幻觉检测提供了一种新颖、有效且无需训练的解决方案，特别是在黑盒设置下具有实际应用价值。
...
TL;DR: 提出渐进式量化（ProVQ）方法解决向量量化中的过早离散化问题，通过动态调整量化硬度引导码本学习数据流形，显著提升重建和生成性能。


<details>
  <summary>Details</summary>
Motivation: 向量量化（VQ）是多模态大语言模型和扩散模型中的关键token化方法，但现有VQ范式存在一个基本矛盾：它们在编码器捕捉底层数据流形之前就强制离散化，导致性能受限。作者将这种现象称为'过早离散化（Premature Discretization）'，旨在解决这一问题。

Method: 提出渐进式量化（ProVQ）方法，通过将量化过程设计为从连续潜空间平滑过渡到离散空间的课程学习，解决了向量量化的过早离散化问题。该方法通过量化硬度（quantization hardness）的动态调整，引导码本学习到充分展开的数据流形。

Result: 实验结果显示ProVQ在不同模态上都有广泛的有效性。在ImageNet-1K和ImageNet-100基准测试中，ProVQ在重建和生成性能上都有提升。此外，在复杂生物序列建模方面，ProVQ在StrutTokenBench排行榜上为蛋白质结构tokenization创造了新的性能记录。

Conclusion: ProVQ通过规避过早离散化问题并引入渐进式量化机制，显著提升了多模态LLM和扩散模型中向量量化的性能，在图像生成和生物序列建模等多个领域都展现了强大的泛化能力。
...
### [9] [CN-Buzz2Portfolio: A Chinese-Market Dataset and Benchmark for LLM-Based Macro and Sector Asset Allocation from Daily Trending Financial News](https://arxiv.org/abs/2603.22305)
*Liyuan Chen,Shilong Li,Jiangpeng Yan,Shuoling Liu,Qiang Yang,Xiu Li*

Main category: cs.LG

TL;DR: 提出了一个基于中文市场的可复现基准CN-Buzz2Portfolio，用于评估LLMs作为金融智能体的能力，重点关注从每日趋势新闻到宏观及行业资产配置的映射，而非传统的个股选择。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型正从静态NLP任务转向复杂金融环境中的动态决策智能体，但现有评估范式存在困境：实时交易不可复现且易混淆运气与技能，而现有静态基准往往局限于个股层面，忽视了更广泛的市场关注。

Method: 提出了一个可复现的基准数据集CN-Buzz2Portfolio，覆盖2024年至2025年中期，模拟真实公众关注流。设计了三阶段CPA智能体工作流（压缩、感知、分配），用于评估LLMs在ETF等广泛资产类别上的表现，而非仅关注个股。

Result: 对九个大型语言模型进行了广泛实验，结果显示不同模型在将宏观层面叙事转化为投资组合权重方面存在显著差异，为通用推理与金融决策的对齐提供了新见解。

Conclusion: 本文提出了一个创新的三阶段CPA智能体工作流和CN-Buzz2Portfolio基准，为金融智能体研究提供了新的评估框架和数据集，揭示了LLMs在宏观叙事转化为投资组合决策方面的显著差异，有助于推动可持续的金融智能体研究发展。
...
TL;DR: 本文提出一种用于老年人实时跌倒检测的多模态深度学习框架，通过融合多种传感器数据、多尺度特征提取、注意力机制和迁移学习等方法，在SisFall数据集上取得了优越性能，满足实时部署要求。


<details>
  <summary>Details</summary>
Motivation: 全球老龄化人口增加加剧了对可靠健康监测系统的需求，特别是能够检测老年人跌倒等关键事件的系统。传统的基于单一模态加速度数据的跌倒检测方法存在误报率高的问题，而传统的机器学习方法需要大量手工特征工程。

Method: 1. 多模态传感器数据融合：融合了三轴加速度计、陀螺仪和四通道生理信号；
2. 多尺度CNN特征提取器：捕捉不同时间分辨率的运动动态；
3. 多头自注意力机制：实现动态时间权重分配；
4. Focal Loss：缓解严重的类别不平衡问题；
5. 辅助活动分类任务：用于正则化；
6. 迁移学习：从UCI HAR数据集迁移到SisFall数据集。

Result: 在SisFall数据集上的广泛实验表明，该框架实现了F1分数98.7、召回率98.9和AUC-ROC 99.4，显著优于包括传统机器学习和标准深度学习方法在内的基准方法。模型在边缘设备上保持了亚50ms的推理延迟，确认了其在老年护理环境中实时部署的适用性。

Conclusion: 本文提出的MultiModalFallDetector框架通过多模态传感器数据融合、创新的深度学习架构设计、以及有效的训练策略，显著提升了老年人跌倒检测的准确性和实时性，为老年人健康监护系统的实际部署提供了可行的解决方案。
...
TL;DR: 提出了一种基于决策变换器的离线、回报条件紧急走廊优化框架，显著减少应急车辆行程时间，并通过多智能体扩展实现了更好的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 当前部署的应急车辆信号优先策略是反应式且不可控的，导致应急响应时间不够优化，需要一种可调度、高效且可扩展的解决方案。

Method: 采用离线、条件回报序列建模，将紧急走廊优化问题转化为回报条件的序列建模任务。构建了基于决策变换器的单智能体框架，以及利用图注意力机制进行空间协调的多智能体决策变换器扩展。

Result: 在4x4网格上，决策变换器相对于固定定时优先策略减少了37.7%的应急车辆平均行程时间；多智能体决策变换器在8x8网格上进一步将减少率提升至45.2%。回报条件机制允许通过单一目标回报标量平滑调节应急车辆行程时间与民用延迟之间的权衡。

Conclusion: 决策变换器及其多智能体扩展为紧急走廊优化提供了一个高效、可扩展且易于调度的框架，可在最小化应急车辆行程时间和减少对民用交通的影响之间灵活权衡，而无需额外的环境交互或再训练。
...
### [13] [Geometric Mixture-of-Experts with Curvature-Guided Adaptive Routing for Graph Representation Learning](https://arxiv.org/abs/2603.22317)
...
### [14] [Sparsely-Supervised Data Assimilation via Physics-Informed Schrödinger Bridge](https://arxiv.org/abs/2603.22319)
...
### [15] [Bridging the Gap Between Climate Science and Machine Learning in Climate Model Emulation](https://arxiv.org/abs/2603.22320)
...
### [16] [AEGIS: An Operational Infrastructure for Post-Market Governance of Adaptive Medical AI Under US and EU Regulations](https://arxiv.org/abs/2603.22322)
...
### [17] [A Multi-Task Targeted Learning Framework for Lithium-Ion Battery State-of-Health and Remaining Useful Life](https://arxiv.org/abs/2603.22323)
...
### [18] [DAQ: Delta-Aware Quantization for Post-Training LLM Weight Compression](https://arxiv.org/abs/2603.22324)
...
### [19] [Hybrid Associative Memories](https://arxiv.org/abs/2603.22325)
...
### [20] [A Direct Classification Approach for Reliable Wind Ramp Event Forecasting under Severe Class Imbalance](https://arxiv.org/abs/2603.22326)
...
### [21] [Beyond the Mean: Distribution-Aware Loss Functions for Bimodal Regression](https://arxiv.org/abs/2603.22328)
...
### [23] [Conformal Risk Control for Safety-Critical Wildfire Evacuation Mapping: A Comparative Study of Tabular, Spatial, and Graph-Based Models](https://arxiv.org/abs/2603.22331)
*Baljinnyam Dayan*
...
### [24] [Large Language Models for Missing Data Imputation: Understanding Behavior, Hallucination Effects, and Control Mechanisms](https://arxiv.org/abs/2603.22332)
...
### [25] [Problems with Chinchilla Approach 2: Systematic Biases in IsoFLOP Parabola Fits](https://arxiv.org/abs/2603.22339)
...
### [26] [Cloud-Edge Collaborative Large Models for Robust Photovoltaic Power Forecasting](https://arxiv.org/abs/2603.22343)
...
### [27] [First-Mover Bias in Gradient Boosting Explanations: Mechanism, Detection, and Resolution](https://arxiv.org/abs/2603.22346)
...
### [28] [COMPASS-Hedge: Learning Safely Without Knowing the World](https://arxiv.org/abs/2603.22348)
...
### [29] [WIST: Web-Grounded Iterative Self-Play Tree for Domain-Targeted Reasoning Improvement](https://arxiv.org/abs/2603.22352)
...
### [30] [Unveiling the Mechanism of Continuous Representation Full-Waveform Inversion: A Wave Based Neural Tangent Kernel Framework](https://arxiv.org/abs/2603.22362)
...
### [31] [FAAR: Format-Aware Adaptive Rounding for NVFP4](https://arxiv.org/abs/2603.22370)
...
### [32] [Rethinking Multimodal Fusion for Time Series: Auxiliary Modalities Need Constrained Fusion](https://arxiv.org/abs/2603.22372)
...
### [33] [Three Creates All: You Only Sample 3 Steps](https://arxiv.org/abs/2603.22375)
...
### [34] [Instruction-Tuned, but Not More Verifiable Instruction-Following: A Cross-Task Diagnosis for LoRA Adapters](https://arxiv.org/abs/2603.22379)
*Junyi Zou*
...
### [35] [Symbolic Graph Networks for Robust PDE Discovery from Noisy Sparse Data](https://arxiv.org/abs/2603.22380)
...
### [36] [Learning When to Act: Interval-Aware Reinforcement Learning with Predictive Temporal Structure](https://arxiv.org/abs/2603.22384)
*Davide Di Gioia*
...
### [37] [Neural Structure Embedding for Symbolic Regression via Continuous Structure Search and Coefficient Optimization](https://arxiv.org/abs/2603.22429)
...
### [38] [Model Predictive Control with Differentiable World Models for Offline Reinforcement Learning](https://arxiv.org/abs/2603.22430)
...
### [39] [SkillRouter: Retrieve-and-Rerank Skill Selection for LLM Agents at Scale](https://arxiv.org/abs/2603.22455)
...
### [41] [Adversarial Vulnerabilities in Neural Operator Digital Twins: Gradient-Free Attacks on Nuclear Thermal-Hydraulic Surrogates](https://arxiv.org/abs/2603.22525)
...
### [42] [Multimodal Training to Unimodal Deployment: Leveraging Unstructured Data During Training to Optimize Structured Data Only Deployment](https://arxiv.org/abs/2603.22530)
...
### [43] [MIOFlow 2.0: A unified framework for inferring cellular stochastic dynamics from single cell and spatial transcriptomics data](https://arxiv.org/abs/2603.22564)
...
### [44] [A Foundation Model for Instruction-Conditioned In-Context Time Series Tasks](https://arxiv.org/abs/2603.22586)
...
### [45] [Precision-Varying Prediction (PVP): Robustifying ASR systems against adversarial attacks](https://arxiv.org/abs/2603.22590)
...
### [46] [Causal Discovery in Action: Learning Chain-Reaction Mechanisms from Interventions](https://arxiv.org/abs/2603.22620)
...
### [47] [Transfer learning via interpolating structures](https://arxiv.org/abs/2603.22621)
...
### [48] [Generalizing Dynamics Modeling More Easily from Representation Perspective](https://arxiv.org/abs/2603.22655)
...
### [49] [Bounding Box Anomaly Scoring for simple and efficient Out-of-Distribution detection](https://arxiv.org/abs/2603.22660)
...
### [50] [Vision-based Deep Learning Analysis of Unordered Biomedical Tabular Datasets via Optimal Spatial Cartography](https://arxiv.org/abs/2603.22675)
...
### [51] [Coordinate Encoding on Linear Grids for Physics-Informed Neural Networks](https://arxiv.org/abs/2603.22700)
...
### [52] [Non-Adversarial Imitation Learning Provably Free of Compounding Errors: The Role of Bellman Constraints](https://arxiv.org/abs/2603.22713)
...
### [53] [Double Coupling Architecture and Training Method for Optimization Problems of Differential Algebraic Equations with Parameters](https://arxiv.org/abs/2603.22724)
...
### [54] [Spiking Personalized Federated Learning for Brain-Computer Interface-Enabled Immersive Communication](https://arxiv.org/abs/2603.22727)
...
### [55] [Behavioral Heterogeneity as Quantum-Inspired Representation](https://arxiv.org/abs/2603.22729)
...
### [56] [Multitask-Informed Prior for In-Context Learning on Tabular Data: Application to Steel Property Prediction](https://arxiv.org/abs/2603.22738)
...
### [57] [From Arithmetic to Logic: The Resilience of Logic and Lookup-Based Neural Networks Under Parameter Bit-Flips](https://arxiv.org/abs/2603.22770)
...
### [58] [Caterpillar of Thoughts: The Optimal Test-Time Algorithm for Large Language Models](https://arxiv.org/abs/2603.22784)
...
### [59] [Transformers Trained via Gradient Descent Can Provably Learn a Class of Teacher Models](https://arxiv.org/abs/2603.22801)
...
### [60] [Universal and efficient graph neural networks with dynamic attention for machine learning interatomic potentials](https://arxiv.org/abs/2603.22810)
...
### [61] [Towards The Implicit Bias on Multiclass Separable Data Under Norm Constraints](https://arxiv.org/abs/2603.22824)
...
### [62] [The Coordinate System Problem in Persistent Structural Memory for Neural Architectures](https://arxiv.org/abs/2603.22858)
*Abhinaba Basu*
...
### [63] [Confidence Calibration under Ambiguous Ground Truth](https://arxiv.org/abs/2603.22879)
...
### [64] [TreeTeaming: Autonomous Red-Teaming of Vision-Language Models via Hierarchical Strategy Exploration](https://arxiv.org/abs/2603.22882)
...
### [65] [Balancing Safety and Efficiency in Aircraft Health Diagnosis: A Task Decomposition Framework with Heterogeneous Long-Micro Scale Cascading and Knowledge Distillation-based Interpretability](https://arxiv.org/abs/2603.22885)
...
### [66] [Conditionally Identifiable Latent Representation for Multivariate Time Series with Structural Dynamics](https://arxiv.org/abs/2603.22886)
...
### [67] [VLGOR: Visual-Language Knowledge Guided Offline Reinforcement Learning for Generalizable Agents](https://arxiv.org/abs/2603.22892)
...
### [68] [Weak-PDE-Net: Discovering Open-Form PDEs via Differentiable Symbolic Networks and Weak Formulation](https://arxiv.org/abs/2603.22951)
...
### [69] [Asymptotic Learning Curves for Diffusion Models with Random Features Score and Manifold Data](https://arxiv.org/abs/2603.22962)
...
### [74] [Generalization Bounds for Physics-Informed Neural Networks for the Incompressible Navier-Stokes Equations](https://arxiv.org/abs/2603.23072)
...
### [75] [MsFormer: Enabling Robust Predictive Maintenance Services for Industrial Devices](https://arxiv.org/abs/2603.23076)
...
### [76] [Policy-based Tuning of Autoregressive Image Models with Instance- and Distribution-Level Rewards](https://arxiv.org/abs/2603.23086)
...
### [77] [SpecXMaster Technical Report](https://arxiv.org/abs/2603.23101)
...
### [78] [A Bayesian Learning Approach for Drone Coverage Network: A Case Study on Cardiac Arrest in Scotland](https://arxiv.org/abs/2603.23134)
...
### [79] [DAK-UCB: Diversity-Aware Prompt Routing for LLMs and Generative Models](https://arxiv.org/abs/2603.23140)
...
### [80] [A Schrödinger Eigenfunction Method for Long-Horizon Stochastic Optimal Control](https://arxiv.org/abs/2603.23173)
...
### [81] [Sparser, Faster, Lighter Transformer Language Models](https://arxiv.org/abs/2603.23198)
...
### [82] [A One-Inclusion Graph Approach to Multi-Group Learning](https://arxiv.org/abs/2603.23208)
...
### [84] [GEM: Guided Expectation-Maximization for Behavior-Normalized Candidate Action Selection in Offline RL](https://arxiv.org/abs/2603.23232)
...
### [85] [Neural ODE and SDE Models for Adaptation and Planning in Model-Based Reinforcement Learning](https://arxiv.org/abs/2603.23245)
...
### [87] [Permutation-Symmetrized Diffusion for Unconditional Molecular Generation](https://arxiv.org/abs/2603.23255)
...
### [88] [SynForceNet: A Force-Driven Global-Local Latent Representation Framework for Lithium-Ion Battery Fault Diagnosis](https://arxiv.org/abs/2603.23265)
...
### [89] [SafeSeek: Universal Attribution of Safety Circuits in Language Models](https://arxiv.org/abs/2603.23268)
...
### [90] [A Comparative Study of Machine Learning Models for Hourly Forecasting of Air Temperature and Relative Humidity](https://arxiv.org/abs/2603.23282)
*Jiaqi Dong*
...
### [91] [Graph Energy Matching: Transport-Aligned Energy-Based Modeling for Graph Generation](https://arxiv.org/abs/2603.23398)
...
### [92] [Robustness Quantification for Discriminative Models: a New Robustness Metric and its Application to Dynamic Classifier Selection](https://arxiv.org/abs/2603.23318)
...
### [93] [SortedRL: Accelerating RL Training for LLMs through Online Length-Aware Scheduling](https://arxiv.org/abs/2603.23414)
...
### [94] [Off-Policy Value-Based Reinforcement Learning for Large Language Models](https://arxiv.org/abs/2603.23355)
...
### [95] [Central Dogma Transformer III: Interpretable AI Across DNA, RNA, and Protein](https://arxiv.org/abs/2603.23361)
*Nobuyuki Ota*
...
### [96] [Similarity-Aware Mixture-of-Experts for Data-Efficient Continual Learning](https://arxiv.org/abs/2603.23436)
...
### [97] [End-to-End Efficient RL for Linear Bellman Complete MDPs with Deterministic Transitions](https://arxiv.org/abs/2603.23461)
...
### [98] [Byzantine-Robust and Differentially Private Federated Optimization under Weaker Assumptions](https://arxiv.org/abs/2603.23472)
...
### [99] [Estimating Flow Velocity and Vehicle Angle-of-Attack from Non-invasive Piezoelectric Structural Measurements Using Deep Learning](https://arxiv.org/abs/2603.23496)
...
### [100] [Memory Bear AI Memory Science Engine for Multimodal Affective Intelligence: A Technical Report](https://arxiv.org/abs/2603.22306)
*Deliang Wen,Ke Sun,Yu Wang*

Main category: cs.AI

TL;DR: 提出了一个基于内存科学的多模态情感智能框架，通过结构化情感记忆单元实现情感信息的长期保存和演化，显著提升了情感识别的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态情感识别系统主要针对短期推理优化，缺乏对持久情感记忆、长时程依赖建模以及在输入不完善情况下的稳健解释能力的支持，而实际交互中的情感判断往往依赖于先验轨迹、累积上下文和多模态证据。

Method: 提出Memory Bear AI Memory Science Engine，这是一个以内存为中心的框架，通过情感记忆单元(EMUs)将多模态信号转化为结构化表示，并组织为内存形成、工作内存聚合、长期整合、内存驱动检索、动态融合校准和持续更新的处理流程。

Result: 在基准测试和实际应用场景中均表现出优于对比系统的性能提升，特别是在噪声或模态缺失条件下展现出更强的准确性和鲁棒性。

Conclusion: 该框架通过内存科学驱动的结构化方法，将情感建模从瞬时预测转变为持续可演化的变量，为实际应用中的情感智能提供了更连续、更稳健的解决方案。
...
### [101] [The Efficiency Attenuation Phenomenon: A Computational Challenge to the Language of Thought Hypothesis](https://arxiv.org/abs/2603.22312)
*Di Zhang*

Main category: cs.AI

TL;DR: 通过“AI私有语言”实验研究发现，高效协作认知不基于人类可理解的符号语言，而是源于亚符号计算——挑战了语言思维假说


<details>
  <summary>Details</summary>
Motivation: 探索思维是否需要一个类似语言的媒介，测试语言思维假说（LoT）的合理性，通过AI模拟思想实验来检验高效协作认知是否必须以符号化结构为中介。

Method: 研究提出“AI私有语言”思想实验：让两个人工智能体通过多智能体强化学习（MARL）发展高效但难以理解的内在交流协议，并在部分可观察的合作导航任务中，比较使用自发协议与预设人类可理解符号协议的性能差异（即效率衰减现象EAP）。

Result: 结果显示，使用自发生成（难以理解）协议的智能体比使用预设人类可理解符号协议的智能体效率高出50.5%，确认了效率衰减现象。

Conclusion: 研究表明，高效协作认知不一定需要基于符号化结构的思维，可以源于从亚符号计算中自然耦合的思维方式，挑战了思维需要语言式媒介的假设，为认知架构的多元主义提供了经验支持，并对AI伦理有启示。
...
### [102] [Session Risk Memory (SRM): Temporal Authorization for Deterministic Pre-Execution Safety Gates](https://arxiv.org/abs/2603.22350)
*Florin Adrian Chitan*

Main category: cs.AI

TL;DR: 论文提出会话风险记忆（SRM），一个轻量级模块，能够为智能体系统提供轨迹层级的授权安全增强，有效检测多步分布式攻击，保持100%检测率的同时显著降低误报。


<details>
  <summary>Details</summary>
Motivation: 确定性预执行安全门能评估智能体的单步行为是否符合角色要求，但对于将有害意图分解为多个合规步骤的分布式攻击，这种系统存在结构性盲区。

Method: 提出了会话风险记忆（SRM），一个轻量级确定性模块，通过维护会话演化行为轮廓的简洁语义质心，基于基线与门输出差值进行指数移动平均累积风险信号。SRM与底层门使用相同的语义向量表示，无需额外模型组件、训练或概率推理。

Result: 在80轮多回合基准测试（包含慢速数据窃取、逐步权限提升和合规漂移场景）中，ILION+SRM的F1=1.0000，false positive率为0%，而无状态ILION的F1=0.9756，false positive率为5%。两者检测率均为100%。每回合开销低于250微秒。

Conclusion: SRM在保持检测率100%的基础上，能有效减少false positives，为智能体系统增加了会话层安全保证的概念框架。
...
### [103] [From Static Templates to Dynamic Runtime Graphs: A Survey of Workflow Optimization for LLM Agents](https://arxiv.org/abs/2603.22386)
*Ling Yue,Kushal Raj Bhandari,Ching-Yun Ko,Dhaval Patel,Shuxin Lin,Nianjun Zhou,Jianxi Gao,Pin-Yu Chen,Shaowu Pan*

Main category: cs.AI

TL;DR: 本文通过引入agentic计算图（ACG）概念，综述了LLM agent工作流优化方法，提出了基于结构确定时机的分类框架（静态vs动态）和三维度分析体系，为领域研究提供了统一词汇、分析框架和评估标准。


<details>
  <summary>Details</summary>
Motivation: 随着LLM-based系统通过构建可执行工作流来解决问题的日益普遍，需要对设计优化这类工作流（称为agentic计算图）的各种方法进行系统性综述，以提供清晰统一的分类框架和分析视角。

Method: 采用基于工作流结构确定时机的分类法，将文献分为静态方法（部署前确定可复用工作流框架）和动态方法（执行前或执行中为特定运行选择、生成或修改工作流），并沿三个维度组织：结构确定时机、优化的工作流部分、引导优化的评估信号，同时区分可复用的工作流程图、运行时特定实现图和执行轨迹。

Result: 提出了基于工作流结构确定时机的文献分类法，建立了三维度分析框架（何时确定结构、优化什么部分、使用何种评估信号），区分了不同类型的工作流表示（模板、实现图、执行轨迹），并提出了考虑图级属性、执行成本、鲁棒性和结构变化的评估视角。

Conclusion: 文章为LLM agent工作流优化研究提供了一个清晰的词汇体系、统一的分析框架、更具可比性的文献综述视角，以及促进未来工作可复现性的评估标准，强调通过明确的工作流结构视角来统一和推进该领域研究。
...
### [104] [Computational Arbitrage in AI Model Markets](https://arxiv.org/abs/2603.22404)
*Ricardo Olmedo,Bernhard Schölkopf,Moritz Hardt*

Main category: cs.AI

TL;DR: 研究表明AI模型市场存在显著套利机会，套利者通过跨模型分配推理预算可创造40%利润，知识蒸馏增强套利机会，多竞争套利者降低消费者价格但为小型提供商创造市场准入。


<details>
  <summary>Details</summary>
Motivation: 探讨AI模型市场中是否存在套利机会，即套利者能否通过在不同模型提供商之间高效分配推理预算，以低于市场价格提供可验证的解决方案，从而创造无模型开发风险的竞争性产品。

Method: 通过实证研究AI模型市场中的套利现象，采用简单和鲁棒的套利策略，在SWE-bench GitHub问题解决领域进行深入案例研究，使用GPT-5 mini和DeepSeek v3.2两种代表性模型，并探索知识蒸馏对套利机会的影响。

Result: 在可验证领域（SWE-bench GitHub问题解决）中，简单的套利策略可实现高达40%的净利润率；鲁棒的套利策略在不同领域中仍能保持盈利能力；知识蒸馏创造强大的套利机会（可能牺牲教师模型收入）；多个竞争套利者推动消费者价格下降并降低模型提供商的边际收益；套利同时减少市场分割并为小型模型提供商创造早期收入获取的市场进入机会。

Conclusion: 本研究表明，AI模型市场中的套利行为可能成为一种强大力量，将对模型开发、知识蒸馏和市场部署产生深远影响，既能降低消费者价格，也能为小型模型提供商创造市场准入机会。
...
### [105] [AI Mental Models: Learned Intuition and Deliberation in a Bounded Neural Architecture](https://arxiv.org/abs/2603.22561)
*Laurence Anthony*

Main category: cs.AI

TL;DR: 有限双通道神经架构在三段论推理基准上展现了直觉与审议的功能分化：审议通道性能显著优于直觉通道，并发展出有结构化的内部状态，与类推理内部组织一致但不完全复制人类多步推理过程。


<details>
  <summary>Details</summary>
Motivation: 论文旨在探究有限神经架构能否在经典64项三段论推理基准上展现出有意义的直觉与审议分工现象。这个问题涉及AI研究中对世界模型和多阶段推理的持续讨论，为测试学习系统是否能够发展结构化内部计算（而非仅进行一次性关联预测）提供了受控环境。

Method: 研究设计了两个实验：实验1评估了预测9种人类响应分布的神经基线模型；实验2引入了受心智模型理论启发的有限双通道架构，包含直觉和审议两条分离路径。模型在64项三段论基准上进行了5折交叉验证评估，同时进行了80:20可解释性运行和5种子稳定性分析来考察内部结构。

Result: 在交叉验证中，有限直觉通道达到总体相关性r=0.7272，而有限审议通道达到r=0.8152，各折之间的审议优势显著（p=0.0101）。最大保持外提升出现在NVC、Eca和Oca项目上，表明对拒绝响应和c-a结论的处理能力得到改善。可解释性运行和稳定性分析进一步表明，审议通道发展出了稀疏、分化的内部结构，包括偏向Oac的状态、主导工作状态以及多个弱使用或未使用状态（具体索引各运行间有差异）。

Conclusion: 研究提供了经验证据，证明有限的双通道架构可以在经典三段论推理基准上表现出可区分的直觉与审议分工。审议通道达到了显著更高的预测性能，特别是在处理否定响应和c-a型结论时表现出最大提升，并发展出了具有解释意义的稀疏、分化内部结构，包括专用状态主导状态和未用状态等模式。这些发现与有限条件下类推理内部组织一致，但并未声称模型完全复现了人类完整顺序推理过程。
...
### [106] [Understanding LLM Performance Degradation in Multi-Instance Processing: The Roles of Instance Count and Context Length](https://arxiv.org/abs/2603.22608)
*Jingxuan Chen,Mohammad Taher Pilehvar,Jose Camacho-Collados*

Main category: cs.AI

TL;DR: LLM处理多实例任务时，实例数量是比上下文长度更关键的性能影响因素。


<details>
  <summary>Details</summary>
Motivation: 用户经常依赖大语言模型处理多个文档或对多个实例进行分析，但现有研究较少关注LLM在处理多实例输入时的性能，尤其是在LLM在单个任务上表现良好的情况下。

Method: 对LLM在多实例输入任务中的表现进行全面评估，分析不同实例数量（从少量到大量）下的性能变化模式。

Result: 所有LLM都表现出相似的模式：在少量实例（大约20-100个）时性能略有下降，而在更大实例数量时出现性能崩溃。关键发现是，尽管上下文长度与性能下降相关，但实例数量对最终结果的影响更强。

Conclusion: 研究发现，当优化LLM的多实例处理性能时，应同时关注上下文长度，特别是实例数量，因为实例数量对最终结果有更强的影响。
...
### [107] [Bridging the Know-Act Gap via Task-Level Autoregressive Reasoning](https://arxiv.org/abs/2603.22619)
...
TL;DR: 提出DeIllusionLLM框架解决LLMs会错误回答有缺陷输入的'知行鸿沟'问题，通过任务级自回归和自蒸馏统一判别性与生成式能力，效果显著。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的一个关键问题是它们会对有缺陷或不当的输入产生看似合理的答案，这种判别性认知与生成性行为之间的'知行鸿沟'源于模型无法有效整合两种模式。

Method: 1. 构建FaultyScience大型跨学科故障科学问题基准；2. 分析揭示鸿沟来源于基于token的自回归中的任务选择问题；3. 提出DeIllusionLLM框架，采用任务级自回归明确建模验证与回答的决策过程；4. 通过自蒸馏将判别性判断与生成性推理统一到单一骨干模型中。

Result: DeIllusionLLM显著减少了在自然提示下'即使在错误情况下也回答'的失败情况，同时保持了通用的推理性能，证明自蒸馏是弥合判别性-生成性知行鸿沟的有效且可扩展的解决方案。

Conclusion: 该研究提出的DeIllusionLLM框架通过任务级自回归和自蒸馏成功弥合了判别性认知与生成性行为之间的鸿沟，为大语言模型在面对有缺陷输入时的可靠响应提供了可扩展的有效解决方案。
...
### [108] [Graph-Aware Late Chunking for Retrieval-Augmented Generation in Biomedical Literature](https://arxiv.org/abs/2603.22633)
*Pouria Mortezaagha,Arya Rahgozar*

Main category: cs.AI

TL;DR: 传统RAG评估过度关注检索精度，忽视结构覆盖率；GraLC-RAG通过结构智能显著提升跨章节证据检索能力


<details>
  <summary>Details</summary>
Motivation: 当前基于排名指标（如MRR）的评估范式不完全，它奖励检索精度但忽视了检索广度——即从文档各结构章节中展现证据的能力

Method: 提出了GraLC-RAG框架，统一了晚期分块与图感知结构智能，包含结构感知分块边界检测、UMLS知识图谱注入和图引导混合检索

Result: 内容相似度方法获得最高MRR（0.517）但仅从单一章节检索，而结构感知方法从最多15.6倍更多章节检索；KG增强检索将答案质量差距缩小至delta-F1 = 0.009，同时保持4.6倍的章节多样性

Conclusion: 标准指标系统性地低估了结构检索的价值，解决跨章节合成差距是生物医学RAG的关键开放问题
...
### [109] [Benchmarking Multi-Agent LLM Architectures for Financial Document Processing: A Comparative Study of Orchestration Patterns, Cost-Accuracy Tradeoffs and Production Scaling Strategies](https://arxiv.org/abs/2603.22651)
*Siddhant Kulkarni,Yukta Kulkarni*

Main category: cs.AI

TL;DR: 针对金融文档信息抽取，系统评估了四种多智能体LLM架构。结果表明分层架构在成本-准确率权衡上最优，而混合策略能以较低成本获得接近最佳架构的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在从金融文档中提取结构化信息方面应用迅速增长，但实际生产部署面临基础架构决策，缺乏经验指导。

Method: 系统性地比较了四种多智能体编排架构（顺序流水线、并行分发-合并、分层监督者-工作者、自反自校正循环），在包含10,000份SEC文件（10-K、10-Q和8-K表格）的语料库上，使用五个前沿和开放权重的LLM进行评估。评估涵盖25个信息抽取字段类型，包括公司治理结构、高管薪酬和财务指标，从字段级F1、文档级准确率、端到端延迟、单文档成本和令牌效率五个维度进行测量。还包括对语义缓存、模型路由和自适应重试策略的消融研究，以及从每日1K到100K文档的规模化分析。

Result: 研究发现，自反架构实现了最高的字段级F1分数（0.943），但成本是顺序基准的2.3倍。分层架构在成本-准确率帕累托前沿上占据最有利位置（F1分数0.921，成本是基准的1.4倍）。消融研究表明，混合配置可以恢复自反架构89%的准确率提升，而成本仅为基准的1.15倍。从每日1K到100K文档的规模化分析揭示了非明显的吞吐量-准确率退化曲线，为容量规划提供了依据。

Conclusion: 在监管严格的金融环境中部署多智能体LLM系统时，分层监督者-工作者架构在成本与准确性之间提供了最有利的平衡，而混合策略可以显著降低高性能架构的成本开销
...
### [110] [MuQ-Eval: An Open-Source Per-Sample Quality Metric for AI Music Generation Evaluation](https://arxiv.org/abs/2603.22677)
*Di Zhu,Zixuan Li*

Main category: cs.AI

TL;DR: MUQ-EVAL是一种开源的逐样本AI生成音乐质量评估指标，基于冻结的MuQ特征构建，与人类评分高度相关，并在实时性和可访问性方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有的分布性指标（如Fréchet Audio Distance）无法对单个音乐片段进行评分，且与人类判断相关性较差；而目前唯一具有高人类相关性的逐样本学习指标是闭源的。

Method: 通过使用MusicEval数据集（包含来自31个文本-音乐系统的生成音乐片段及专家质量评分），在冻结的MuQ-310M特征之上训练轻量级预测头来构建逐样本质量评估指标。进行了训练目标和适配策略的系统性消融实验。

Result: 最简单模型（冻结特征配合注意力池化和两层MLP）在系统级别获得SRCC = 0.957，在话语级别获得SRCC = 0.838，与人类平均意见评分高度相关。使用LoRA适配的模型在仅用150个片段训练时即可获得可用的相关性，并揭示了模型对信号级别伪影有选择性敏感，但对音乐结构失真不敏感。

Conclusion: MUQ-EVAL 作为一个开源、实时的逐样本音乐质量评估指标，能够有效评估AI生成音乐的质量，且在系统级别和话语级别都与人类评分高度相关。该研究还表明，编码器的选择比所有架构和训练决策更为重要，且冻结的MuQ表示已能捕获质量相关信息。
...
### [111] [HyFI: Hyperbolic Feature Interpolation for Brain-Vision Alignment](https://arxiv.org/abs/2603.22721)
*Sangmin Jo,Wootaek Jeong,Da-Woon Heo,Yoohwan Hwang,Heung-Il Suk*

Main category: cs.AI

TL;DR: 提出了基于双曲空间的HyFI框架，通过双曲空间中的特征插值解决脑信号解码中的模态差距和特征纠缠问题，显著提升了脑-图像对齐性能


<details>
  <summary>Details</summary>
Motivation: 以往的研究未能解决两个关键挑战：1）脑信号与图像之间由于信息量不同导致的模态差距；2）语义和感知特征在神经活动中的高度纠缠性。双曲空间天然适合处理信息量差异问题，且具有测地线向原点弯曲的几何特性

Method: 利用双曲空间的几何特性（特别是测地线向原点弯曲的特性），提出了Hyperbolic Feature Interpolation框架，在双曲空间中沿着测地线对语义和感知视觉特征进行插值，实现特征的融合和压缩

Result: HyFI在零样本脑到图像检索任务中实现了最先进的性能，在THINGS-EEG数据集上达到Top-1准确率提升+17.3%，在THINGS-MEG数据集上提升+9.1%

Conclusion: 该研究通过引入双曲空间来处理脑信号与图像之间模态差距以及特征纠缠问题，提出的Hyperbolic Feature Interpolation（HyFI）框架能有效利用双曲空间的几何特性进行语义和感知特征的插值与融合，在脑信号解码任务中显示了优越性能
...
### [112] [Beyond Binary Correctness: Scaling Evaluation of Long-Horizon Agents on Subjective Enterprise Tasks](https://arxiv.org/abs/2603.22744)
*Abhishek Chandwani,Ishan Gupta*

Main category: cs.AI

TL;DR: LH-Bench提出三支柱评估框架解决主观企业任务评估难题，结合专家标准、分步评估和人类偏好验证，在真实企业场景中实现可靠评估


<details>
  <summary>Details</summary>
Motivation: 传统LLM评估主要针对客观验证的任务（如数学、编程），但现实企业工作通常是主观且情境依赖的，成功取决于组织目标、用户意图和工作流程中的中间产物质量。现有评估方法无法准确评估这类复杂工作。

Method: 引入三支柱评估设计：1）专家驱动的评估标准为LLM评委提供领域上下文；2）手动标注的真实工件支持逐步奖励信号；3）成对人类偏好评估用于收敛验证。在两个环境中验证：Figma-to-code转换任务和编程内容创作任务。

Result: 专家制定的评估标准比LLM制定的标准显著更可靠（kappa = 0.60 vs. 0.46），人类偏好判断确认了相同的顶级分离（p < 0.05），证明基于专家的评估既可靠又可扩展。在两个实际任务环境中验证了框架有效性。

Conclusion: LH-Bench 通过专家驱动评估框架解决主观企业任务评估难题，实现了可靠、可扩展的自动化评估，比传统二元正确性评估更贴近真实世界工作质量判断。
...
### [113] [Can LLM Agents Generate Real-World Evidence? Evaluating Observational Studies in Medical Databases](https://arxiv.org/abs/2603.22767)
...
### [114] [AgriPestDatabase-v1.0: A Structured Insect Dataset for Training Agricultural Large Language Model](https://arxiv.org/abs/2603.22777)
...
### [115] [ABSTRAL: Automatic Design of Multi-Agent Systems Through Iterative Refinement and Topology Optimization](https://arxiv.org/abs/2603.22791)
...
### [116] [Reliable Classroom AI via Neuro-Symbolic Multimodal Reasoning](https://arxiv.org/abs/2603.22793)
*Sina Bagheri Nezhad*
...
### [117] [Learning What Matters Now: Dynamic Preference Inference under Contextual Shifts](https://arxiv.org/abs/2603.22813)
...
### [118] [Empirical Comparison of Agent Communication Protocols for Task Orchestration](https://arxiv.org/abs/2603.22823)
*Ivan Dobrovolskyi*
...
### [124] [Separating Diagnosis from Control: Auditable Policy Adaptation in Agent-Based Simulations with LLM-Based Diagnostics](https://arxiv.org/abs/2603.22904)
...
### [125] [ProGRank: Probe-Gradient Reranking to Defend Dense-Retriever RAG from Corpus Poisoning](https://arxiv.org/abs/2603.22934)
...
### [126] [Ran Score: a LLM-based Evaluation Score for Radiology Report Generation](https://arxiv.org/abs/2603.22935)
...
### [127] [Optimizing Small Language Models for NL2SQL via Chain-of-Thought Fine-Tuning](https://arxiv.org/abs/2603.22942)
...
### [128] [PersonalQ: Select, Quantize, and Serve Personalized Diffusion Models for Efficient Inference](https://arxiv.org/abs/2603.22943)
...
### [129] [Where Experts Disagree, Models Fail: Detecting Implicit Legal Citations in French Court Decisions](https://arxiv.org/abs/2603.22973)
...
### [130] [JFTA-Bench: Evaluate LLM's Ability of Tracking and Analyzing Malfunctions Using Fault Trees](https://arxiv.org/abs/2603.22978)
...
### [131] [On the use of Aggregation Operators to improve Human Identification using Dental Records](https://arxiv.org/abs/2603.23003)
...
### [132] [Can Large Language Models Reason and Optimize Under Constraints?](https://arxiv.org/abs/2603.23004)
...
### [137] [SAiW: Source-Attributable Invisible Watermarking for Proactive Deepfake Defense](https://arxiv.org/abs/2603.23178)
...
### [138] [PERMA: Benchmarking Personalized Memory Agents via Event-Driven Preference and Realistic Task Environments](https://arxiv.org/abs/2603.23231)
...
### [139] [MemCollab: Cross-Agent Memory Collaboration via Contrastive Trajectory Distillation](https://arxiv.org/abs/2603.23234)
...
### [140] [Online library learning in human visual puzzle solving](https://arxiv.org/abs/2603.23244)
...
### [141] [LLM Olympiad: Why Model Evaluation Needs a Sealed Exam](https://arxiv.org/abs/2603.23292)
...
### [142] [RelayS2S: A Dual-Path Speculative Generation for Real-Time Dialogue](https://arxiv.org/abs/2603.23346)
*Long Mai*
...
### [143] [Beyond Preset Identities: How Agents Form Stances and Boundaries in Generative Societies](https://arxiv.org/abs/2603.23406)
...
### [144] [Bilevel Autoresearch: Meta-Autoresearching Itself](https://arxiv.org/abs/2603.23420)
*Yaonan Qu,Meng Lu*
...
### [145] [Mecha-nudges for Machines](https://arxiv.org/abs/2603.23433)
