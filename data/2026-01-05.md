...
TL;DR: 该论文通过模拟数据集评估了14种异常检测算法在不同训练规模和异常率下的性能，发现最佳检测器取决于训练数据中异常样本的数量，并提供了工业环境中部署异常检测的实用见解。


<details>
  <summary>Details</summary>
Motivation: 解决工业系统中机器学习应用面临的极端类别不平衡问题，特别是在训练数据中异常样本有限的情况下。

Method: 使用基于超球面分布的模拟数据集（2D和10D），在不同异常率（0.05%-20%）和训练规模（1,000-10,000样本）下评估14种检测器的性能和泛化误差。

Result: 研究发现，最佳检测器高度依赖于训练数据中异常样本的数量：少于20个异常样本时无监督方法（kNN/LOF）占优；30-50个异常样本时半监督（XGBOD）和监督（SVM/CatBoost）方法性能显著提升。

Conclusion: 研究揭示了小数据集上异常检测方法的泛化性能下降问题，并为工业环境中部署异常检测提供了实用指导。
...
TL;DR: The paper explores reinforcement learning methods for Yahtzee, finding A2C robust but highlighting challenges in long-horizon strategies.


<details>
  <summary>Details</summary>
Motivation: Yahtzee's stochastic, combinatorial structure and delayed rewards make it an interesting benchmark for reinforcement learning. The challenge lies in approximating optimal policies for multiplayer scenarios, which are computationally intractable.

Method: The paper formulates Yahtzee as a Markov Decision Process (MDP) and trains self-play agents using policy gradient methods: REINFORCE, A2C, and PPO. The study includes ablation studies on feature and action encodings, architecture, return estimators, and entropy regularization.

Result: The agent attains a median score of 241.78 points, within 5.0% of the optimal DP score of 254.59. It achieves the upper section bonus and Yahtzee at rates of 24.9% and 34.1%, respectively. However, models struggle with the upper bonus strategy, indicating persistent challenges in long-horizon credit-assignment and exploration.

Conclusion: The study demonstrates that while REINFORCE and PPO are sensitive to hyperparameters and fail to reach near-optimal performance, A2C trains robustly across various settings. The agent achieves a median score close to the optimal DP score, highlighting the effectiveness of A2C in this context.
...
TL;DR: 研究发现令牌移植技术在开放权重大语言模型生态系统中引入供应链漏洞，通过‘破坏令牌’攻击可重构恶意特征，攻击对微调和权重合并具有持久性。


<details>
  <summary>Details</summary>
Motivation: 开放权重大语言模型生态系统中模型组合技术的广泛应用，使得令牌移植成为跨模型家族应用这些方法的关键前提，但这一步骤可能引入安全漏洞。

Method: 通过利用系数重用的几何特性，研究者设计了一种稀疏求解器来实例化攻击，实现了无需训练的谱模仿以规避异常检测。

Result: 攻击成功地在基础模型中重构出高显著性的恶意特征，同时保持捐赠模型的实用性在统计上与名义行为无区别，且攻击对微调和权重合并具有结构持久性。

Conclusion: 该研究揭示了在开放权重大语言模型生态系统中，通过令牌移植技术引入的供应链漏洞，即‘破坏令牌’攻击，展示了模块化AI组合流程中的潜在风险。
...
### [4] [Exploration in the Limit](https://arxiv.org/abs/2601.00084)
*Brian M. Cho,Nathan Kallus*

Main category: cs.LG

TL;DR: 论文提出了一种渐近误差控制的BAI方法，优化样本复杂度并保持误差控制，适用于非参数设置和协变量整合。


<details>
  <summary>Details</summary>
Motivation: 现有的BAI算法在实际应用中由于严格的精确误差控制要求而表现不佳，作者希望通过渐近误差控制的放松框架来解决这一问题，以适应现实世界中的弱信号、高显著性和后实验推断需求。

Method: 引入了一种渐近有效的置信序列方法，并设计了一种新的BAI算法，该方法能灵活整合协变量以减少方差，并在完全非参数设置中确保近似误差控制。

Result: 实验表明，该方法在保持误差控制的同时，显著降低了平均样本复杂度，并在渐近条件下匹配了高斯BAI在精确误差保证和已知方差下的最佳样本复杂度。

Conclusion: 该论文提出了一种基于渐近误差控制的新方法，用于固定置信度下的最佳臂识别问题，能够在非参数设置中灵活处理协变量并保持误差控制。
...
### [5] [Dynamic Bayesian Optimization Framework for Instruction Tuning in Partial Differential Equation Discovery](https://arxiv.org/abs/2601.00088)
*Junqi Qu,Yan Zhang,Shangqian Gao,Shibo Li*

Main category: cs.LG

TL;DR: NeuroSymBO通过动态调整提示策略，解决了LLMs在方程发现中的指令脆弱性问题，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在方程发现中表现出潜力，但其输出对提示措辞高度敏感（指令脆弱性），静态提示无法适应多步生成过程的动态变化。

Method: 提出NeuroSymBO方法，将提示工程视为序列决策问题，利用贝叶斯优化在每一步选择最优指令。

Result: 在PDE发现基准测试中，自适应指令选择显著优于固定提示，实现了更高的恢复率和更简洁的解。

Conclusion: NeuroSymBO通过动态调整提示策略，显著提升了LLMs在方程发现任务中的性能，解决了静态提示的局限性。
...
### [6] [GRL-SNAM: Geometric Reinforcement Learning with Path Differential Hamiltonians for Simultaneous Navigation and Mapping in Unknown Environments](https://arxiv.org/abs/2601.00116)
*Aditya Sai Ellendula,Yi Wang,Minh Nguyen,Chandrajit Bajaj*

Main category: cs.LG

TL;DR: GRL-SNAM是一个几何强化学习框架，用于未知环境中的同时导航与建图（SNAM）。它通过局部感官观察而非全局地图构建，利用哈密顿优化动态调整路径搜索和发现过程。


<details>
  <summary>Details</summary>
Motivation: 解决在无地图环境中多智能体的分层或联合策略设计问题，通过局部感官输入实现高效导航。

Method: 将路径导航和建图建模为动态最短路径搜索和发现过程，利用哈密顿优化更新局部能量景观，嵌入障碍约束并持续优化轨迹。

Result: 在两种不同的2D导航任务中，GRL-SNAM表现出优于局部反应式基线和全局策略学习参考的性能，能够通过最小化探索实现高质量导航。

Conclusion: GRL-SNAM通过哈密顿优化的几何强化学习，展示了在未知环境中高效导航的潜力，无需依赖全局地图构建。
...
### [7] [Reinforcement Learning with Function Approximation for Non-Markov Processes](https://arxiv.org/abs/2601.00151)
*Ali Devran Kara*

Main category: cs.LG

TL;DR: 该论文研究了非马尔可夫状态和成本过程下的强化学习方法，探讨了策略评估方法的收敛性，并分析了Q学习在线性函数逼近下的特殊情况。


<details>
  <summary>Details</summary>
Motivation: 研究非马尔可夫过程下的强化学习方法的收敛性和性能，填补现有研究的空白。

Method: 采用线性函数逼近和辅助马尔可夫决策过程的方法，结合正交投影和Bellman算子。

Result: 在特定条件下（如基于量化映射的基函数选择），证明了算法的收敛性，并推导了误差界限。

Conclusion: 该研究为非马尔可夫环境下的强化学习提供了理论支持，并展示了在部分可观测马尔可夫决策过程中的应用潜力。
...
### [8] [The Weather Paradox: Why Precipitation Fails to Predict Traffic Accident Severity in Large-Scale US Data](https://arxiv.org/abs/2601.00152)
*Yann Bellec,Rohan Kaman,Siwen Cui,Aarav Agrawal,Calvin Chen*

Main category: cs.LG

TL;DR: Study uses XGBoost to predict traffic accident severity, finding time, location, and weather as key predictors, but notes dataset limitations for extreme cases.


<details>
  <summary>Details</summary>
Motivation: To investigate the predictive capacity of environmental, temporal, and spatial factors on traffic accident severity in the United States.

Method: The research utilized an XGBoost classifier trained on a dataset of 500,000 U.S. traffic accidents (2016-2023), optimized via randomized search cross-validation and adjusted for class imbalance with class weighting.

Result: The model achieved 78% overall accuracy, with 87% precision and recall for the majority class (Severity 2). Key predictors included time of day, geographic location, and weather-related variables.

Conclusion: The study highlights the predictive power of time, location, and weather variables on traffic accident severity, while noting limitations in predicting extreme cases due to dataset constraints. It suggests future improvements through alternative sampling and feature engineering.
...
### [9] [Online Finetuning Decision Transformers with Pure RL Gradients](https://arxiv.org/abs/2601.00167)
*Junkai Luo,Yinglun Zhu*

Main category: cs.LG

TL;DR: 提出基于纯强化学习梯度的在线微调方法，通过关键改进在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有Decision Transformers在在线设置中依赖监督序列建模目标，而基于强化学习的微调方法尚未充分探索。

Method: 提出新算法，包括对GRPO的适配、子轨迹优化、序列级似然目标以及主动采样等关键改进。

Result: 新方法在多个基准测试中超越了现有的在线DT基线，实现了最先进的性能。

Conclusion: 通过实验验证，提出的基于纯强化学习梯度的在线微调方法在多个基准测试中表现优异，为Decision Transformers的在线微调提供了新的有效途径。
...
### [10] [Sequential Reservoir Computing for Efficient High-Dimensional Spatiotemporal Forecasting](https://arxiv.org/abs/2601.00172)
*Ata Akbari Asanjan,Filip Wudarski,Daniel O'Connor,Shaun Geaney,Elena Strbac,P. Aaron Lott,Davide Venturelli*

Main category: cs.LG

TL;DR: Sequential RC 通过分解大型储层为互连小储层，显著降低了计算和内存成本，同时在高维系统预测中表现优于传统 RNN 和 LSTM。


<details>
  <summary>Details</summary>
Motivation: 由于基于梯度的训练和内存瓶颈，RNN 和 LSTM 模型在高维时空系统预测中面临计算挑战，而传统 RC 架构在输入维度增加时扩展性较差。

Method: 引入 Sequential Reservoir Computing (Sequential RC) 架构，将大型储层分解为一系列较小的互连储层，从而降低内存和计算成本，同时保留长期时间依赖性。

Result: 在低维混沌系统（Lorenz63）和高维物理模拟（2D 涡度和浅水方程）中，Sequential RC 实现了比 LSTM 和标准 RNN 基线更长的有效预测范围（15-25%）、更低的误差指标（20-30%）以及高达三个数量级的更低训练成本。

Conclusion: Sequential RC 在保持传统 RC 的简单性和效率的同时，实现了对高维动态系统的卓越可扩展性，为科学和工程应用中的实时、节能预测提供了实用路径。
...
TL;DR: 机器学习模型利用电子健康记录数据显著优于FIB-4评分，在肝硬化早期预测中表现更优，可用于临床决策支持。


<details>
  <summary>Details</summary>
Motivation: 开发和评估机器学习模型，利用常规电子健康记录数据预测诊断前1年、2年和3年的肝硬化发病情况，并将其性能与FIB-4评分进行基准比较。

Method: 我们使用来自大型学术卫生系统的去标识化电子健康记录数据进行了一项回顾性队列研究。通过ICD-9/10代码识别脂肪肝患者并将其分为肝硬化组和非肝硬化组。利用观察窗口和预测窗口构建预测场景以模拟真实临床使用。从观察窗口汇总人口统计学、诊断、实验室结果、生命体征和共病指数。训练XGBoost模型用于1年、2年和3年的预测，并在保留的测试集上进行评估。使用接收者操作特征曲线下面积（AUC）将模型性能与FIB-4进行比较。

Result: 最终队列包括1年预测的3,043名患者、2年预测的1,981名患者和3年预测的1,470名患者。在所有预测窗口中，机器学习模型始终优于FIB-4。XGBoost模型在1年、2年和3年预测中的AUC分别为0.81、0.73和0.69，而FIB-4的AUC分别为0.71、0.63和0.57。随着预测时间范围的延长，性能提升持续存在，表明早期风险区分能力有所改善。

Conclusion: 机器学习模型利用常规电子健康记录数据显著优于传统的FIB-4评分，可用于肝硬化的早期预测。这些模型能够实现更早、更准确的风险分层，并可作为自动化决策支持工具集成到临床工作流程中，支持肝硬化的主动预防和管理。
...
### [12] [SSI-GAN: Semi-Supervised Swin-Inspired Generative Adversarial Networks for Neuronal Spike Classification](https://arxiv.org/abs/2601.00189)
...
TL;DR: SSI-GAN, a semi-supervised GAN with transformer architecture, achieves high accuracy in classifying mosquito neuronal spike patterns with minimal labeled data, significantly reducing manual effort.


<details>
  <summary>Details</summary>
Motivation: Manual classification of mosquito neuronal spike patterns is labor-intensive and expensive. Existing deep learning solutions require fully labeled datasets and highly preprocessed signals, limiting their practical adoption. The study aims to address the scarcity of labeled data.

Method: The study introduces a Semi-supervised Swin-Inspired GAN (SSI-GAN) with a transformer-based generator and a shifted-window discriminator. It uses multi-head self-attention in a flat, window-based transformer discriminator to capture high-frequency spike features. The model was trained with minimal labeled data (1-3%) and optimized using the Bayesian Optuna framework.

Result: SSI-GAN achieved 99.93% classification accuracy on the third day post-infection with only 3% labeled data and maintained high accuracy across all infection stages with just 1% supervision. It reduced manual labeling effort by 97-99% compared to standard supervised approaches.

Conclusion: The proposed SSI-GAN model significantly reduces the need for labeled data while achieving high classification accuracy in detecting viral neurotropism from mosquito neuronal spike patterns.
...
### [13] [Optimized Hybrid Feature Engineering for Resource-Efficient Arrhythmia Detection in ECG Signals: An Optimization Framework](https://arxiv.org/abs/2601.00192)
*Moirangthem Tiken Singh,Manibhushan Yaikhom*

Main category: cs.LG

TL;DR: 提出了一种高效的心律失常诊断框架，结合小波分解和图论特征，实现高精度和低计算开销，适用于边缘设备。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病（尤其是心律失常）是全球主要的死亡原因之一，需要通过医疗物联网（IoMT）进行持续监测。然而，现有的深度学习方法计算开销大，难以在资源受限的边缘设备上运行。

Method: 研究采用时间-频率小波分解和图论结构描述符（如PageRank中心性）相结合的混合特征空间，并通过互信息和递归消除进行特征优化，最终使用超轻量级线性分类器进行分类。

Result: 在MIT-BIH和INCART数据集上的验证显示，该方法达到了98.44%的诊断准确率，模型大小仅为8.54 KB，分类延迟为0.46微秒，每搏管道延迟为52毫秒，实现了实时操作。

Conclusion: 本研究提出了一种资源高效、以数据为中心的心律失常诊断框架，通过结合小波分解和图论结构描述符，实现了高精度和低计算开销的诊断系统，适用于资源受限的边缘设备。
...
### [14] [Unknown Aware AI-Generated Content Attribution](https://arxiv.org/abs/2601.00218)
*Ellie Thieu,Jifan Zhang,Haoyue Bai*

Main category: cs.LG

TL;DR: A constrained optimization method using CLIP features and unlabeled wild data improves attribution of AI-generated images to specific models, especially for unseen generators.


<details>
  <summary>Details</summary>
Motivation: The rapid advancement of photorealistic generative models necessitates moving beyond binary real or fake detection to identifying the specific model that produced a given image, addressing the challenge of generalization to unseen and newly released generators.

Method: The paper proposes a constrained optimization approach that uses CLIP features and a simple linear classifier, supplemented with unlabeled wild data from the Internet, to improve the attribution of images to their specific generative models.

Result: Incorporating unlabeled wild data substantially improves attribution performance on challenging unseen generators, showing the effectiveness of this approach in open-world settings.

Conclusion: The study demonstrates that leveraging unlabeled wild data through a constrained optimization approach significantly improves the attribution of AI-generated content, especially for challenging unseen generators, thereby enhancing the ability to identify the specific model behind synthetic images in open-world settings.
...
### [15] [Robust Graph Fine-Tuning with Adversarial Graph Prompting](https://arxiv.org/abs/2601.00229)
*Ziyan Zhang,Bo Jiang,Jin Tang*

Main category: cs.LG

TL;DR: AGP integrates adversarial learning into graph prompting to robustly fine-tune GNN models, addressing vulnerabilities to noise and attacks through a min-max optimization framework.


<details>
  <summary>Details</summary>
Motivation: Existing Parameter-Efficient Fine-Tuning (PEFT) methods for GNN models are vulnerable to noise and attacks on graph topology and node features, necessitating a robust solution.

Method: AGP formulates the problem as a min-max optimization, using Joint Projected Gradient Descent (JointPGD) for adversarial noise generation and a simple module for learning optimal node prompts to counteract noise.

Result: AGP demonstrates superior robustness and effectiveness compared to state-of-the-art methods, as validated by extensive experiments on benchmark tasks.

Conclusion: The proposed Adversarial Graph Prompting (AGP) framework effectively enhances the robustness of pre-trained GNN models against various noise and attacks on graph topology and node attributes, making it a versatile and generalizable solution for downstream tasks.
...
### [16] [GRIT -- Geometry-Aware PEFT with K-FACPreconditioning, Fisher-Guided Reprojection, andDynamic Rank Adaptation](https://arxiv.org/abs/2601.00231)
...
### [17] [Task-Driven Kernel Flows: Label Rank Compression and Laplacian Spectral Filtering](https://arxiv.org/abs/2601.00276)
...
### [18] [Can Optimal Transport Improve Federated Inverse Reinforcement Learning?](https://arxiv.org/abs/2601.00309)
...
### [19] [Quantum King-Ring Domination in Chess: A QAOA Approach](https://arxiv.org/abs/2601.00318)
...
### [20] [Real-Time Human Detection for Aerial Captured Video Sequences via Deep Models](https://arxiv.org/abs/2601.00391)
...
### [21] [Deep Delta Learning](https://arxiv.org/abs/2601.00417)
...
### [22] [E-GRPO: High Entropy Steps Drive Effective Reinforcement Learning for Flow Models](https://arxiv.org/abs/2601.00423)
...
### [23] [A Comparative Analysis of Interpretable Machine Learning Methods](https://arxiv.org/abs/2601.00428)
...
### [24] [A Comparative Study of Adaptation Strategies for Time Series Foundation Models in Anomaly Detection](https://arxiv.org/abs/2601.00446)
...
### [27] [Deep Networks Learn Deep Hierarchical Models](https://arxiv.org/abs/2601.00455)
*Amit Daniely*
...
### [28] [Geometric Regularization in Mixture-of-Experts: The Disconnect Between Weights and Activations](https://arxiv.org/abs/2601.00457)
*Hyunjun Kim*
...
### [29] [Detecting Spike Wave Discharges (SWD) using 1-dimensional Residual UNet](https://arxiv.org/abs/2601.00459)
...
### [30] [Laplacian Kernelized Bandit](https://arxiv.org/abs/2601.00461)
...
### [31] [Neural Chains and Discrete Dynamical Systems](https://arxiv.org/abs/2601.00473)
...
### [32] [When Small Models Are Right for Wrong Reasons: Process Verification for Trustworthy Agents](https://arxiv.org/abs/2601.00513)
*Laksh Advani*
...
### [33] [Trajectory Guard -- A Lightweight, Sequence-Aware Model for Real-Time Anomaly Detection in Agentic AI](https://arxiv.org/abs/2601.00516)
*Laksh Advani*
...
### [34] [A Sparse-Attention Deep Learning Model Integrating Heterogeneous Multimodal Features for Parkinson's Disease Severity Profiling](https://arxiv.org/abs/2601.00519)
...
### [35] [Optimizing LSTM Neural Networks for Resource-Constrained Retail Sales Forecasting: A Model Compression Study](https://arxiv.org/abs/2601.00525)
*Ravi Teja Pagidoju*
...
### [36] [Federated Customization of Large Models: Approaches, Experiments, and Insights](https://arxiv.org/abs/2601.00526)
...
### [37] [Cloud-Native Generative AI for Automated Planogram Synthesis: A Diffusion Model Approach for Multi-Store Retail Optimization](https://arxiv.org/abs/2601.00527)
...
### [38] [Learning to be Reproducible: Custom Loss Design for Robust Neural Networks](https://arxiv.org/abs/2601.00578)
...
### [39] [Entropy Production in Machine Learning Under Fokker-Planck Probability Flow](https://arxiv.org/abs/2601.00554)
*Lennon Shikhman*
...
### [40] [HFedMoE: Resource-aware Heterogeneous Federated Learning with Mixture-of-Experts](https://arxiv.org/abs/2601.00583)
...
### [41] [Adversarial Samples Are Not Created Equal](https://arxiv.org/abs/2601.00577)
...
### [43] [Interpretability-Guided Bi-objective Optimization: Aligning Accuracy and Explainability](https://arxiv.org/abs/2601.00655)
...
### [44] [Avatar Forcing: Real-Time Interactive Head Avatar Generation for Natural Conversation](https://arxiv.org/abs/2601.00664)
...
### [46] [IRPO: Scaling the Bradley-Terry Model via Reinforcement Learning](https://arxiv.org/abs/2601.00677)
...
### [47] [Traffic-Aware Optimal Taxi Placement Using Graph Neural Network-Based Reinforcement Learning](https://arxiv.org/abs/2601.00607)
...
### [48] [Stochastic Actor-Critic: Mitigating Overestimation via Temporal Aleatoric Uncertainty](https://arxiv.org/abs/2601.00737)
*Uğurcan Özalp*
...
### [49] [FedHypeVAE: Federated Learning with Hypernetwork Generated Conditional VAEs for Differentially Private Embedding Sharing](https://arxiv.org/abs/2601.00785)
...
### [50] [Do Chatbot LLMs Talk Too Much? The YapBench Benchmark](https://arxiv.org/abs/2601.00624)
...
### [60] [Categorical Reparameterization with Denoising Diffusion models](https://arxiv.org/abs/2601.00781)
...
### [61] [Reasoning in Action: MCTS-Driven Knowledge Retrieval for Large Language Models](https://arxiv.org/abs/2601.00003)
*Shuqi Liu,Bowei He,Chen Ma,Linqi Song*

Main category: cs.AI

TL;DR: A reasoning-aware knowledge retrieval method enhances LLMs by aligning retrieved information with conversation logic, improving response quality and diversity.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of integrating retrieval and reasoning strategies in LLMs, moving beyond surface-level semantic similarity to optimize performance.

Method: The method follows a coarse-to-fine approach for knowledge retrieval, using contextually relevant sub-regions and refining searches within them. Monte Carlo Tree Search-inspired techniques are employed to navigate knowledge sentences efficiently.

Result: Experiments on multi-turn dialogue datasets show that the method aligns more closely with human reasoning and improves the diversity of retrieved knowledge.

Conclusion: The proposed reasoning-aware knowledge retrieval method significantly enhances the performance of LLMs by aligning retrieved information with the logical structure of conversations, leading to more informative and creative responses.
...
### [62] [Finetuning Large Language Models for Automated Depression Screening in Nigerian Pidgin English: GENSCORE Pilot Study](https://arxiv.org/abs/2601.00004)
...
TL;DR: 研究通过调整大型语言模型，开发了一种适用于尼日利亚皮钦语的自动化抑郁症筛查工具，GPT-4.1表现最优，为资源有限的语言多样化环境提供了可行的解决方案。


<details>
  <summary>Details</summary>
Motivation: 尼日利亚的抑郁症筛查覆盖率有限，主要由于临床医生资源不足、社会污名化和语言障碍。传统的筛查工具（如PHQ-9）在高收入国家验证有效，但在语言和文化多样性较高的低收入和中等收入国家（如尼日利亚）可能不适用。

Method: 研究采用了精细调整的大型语言模型（LLMs），包括Phi-3-mini-4k-instruct、Gemma-3-4B-it和GPT-4.1，用于适应尼日利亚皮钦语的对话式抑郁症筛查。通过收集432份皮钦语音频回复，进行转录、预处理和注释（包括语义标记、俚语解释和PHQ-9严重程度评分），并对模型性能进行定量和定性评估。

Result: GPT-4.1在定量评估中表现最佳，PHQ-9严重程度评分预测准确率达到94.5%，优于Gemma-3-4B-it和Phi-3-mini-4k-instruct。定性评估中，GPT-4.1也生成了最具文化适应性、清晰且上下文相关的回复。

Conclusion: 该研究为在语言多样且资源有限的环境中部署对话式心理健康工具奠定了基础，展示了AI在尼日利亚等资源受限社区中进行抑郁症筛查的潜力。
...
### [63] [Toward a Physical Theory of Intelligence](https://arxiv.org/abs/2601.00021)
*Peter David Fagan*

Main category: cs.AI

TL;DR: 提出了一个基于不可逆信息处理的智能物理理论，定义了智能系统如何通过守恒定律约束下的信息转换实现目标导向的工作。


<details>
  <summary>Details</summary>
Motivation: 探索智能系统的物理基础，揭示信息处理与守恒定律之间的关系。

Method: 引入守恒一致编码（CCE）框架，将编码对应为守恒定律约束下的吸引子盆地。

Result: 建立了智能系统的物理约束层次，揭示了信息摄入、不可逆计算和开放系统中工作提取的规律。

Conclusion: 该理论为智能现象提供了一个统一的、与底物无关的解释，并提出了基于不可逆信息流和结构稳态的人工智能安全视角。
...
### [64] [A multi-algorithm approach for operational human resources workload balancing in a last mile urban delivery system](https://arxiv.org/abs/2601.00023)
*Luis M. Moreno-Saavedra,Silvia Jimenez-Fernandez,Antonio Portilla-Figueras,David Casillas-Perez,Sancho Salcedo-Sanz*

Main category: cs.AI

TL;DR: The paper introduces a multi-algorithm approach to balance workload among delivery workers by optimizing distance and effort, tested successfully in a real-world urban delivery system.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inefficiency and imbalance in workload distribution among delivery workers caused by traditional geographical proximity-based assignment methods.

Method: The method involves a multi-algorithm approach, including versions of k-means, evolutionary approaches, recursive assignments based on k-means initialization, and a hybrid evolutionary ensemble algorithm. These algorithms optimize package allocation by considering both distance and workload.

Result: The results demonstrate that the proposed approach successfully balances workload among workers in a real-world urban last-mile delivery scenario in Azuqueca de Henares, Spain.

Conclusion: The paper concludes that the proposed multi-algorithm approach effectively balances the workload among delivery workers by optimizing both distance and effort, leading to a more equitable distribution of tasks.
...
### [65] [From Clay to Code: Typological and Material Reasoning in AI Interpretations of Iranian Pigeon Towers](https://arxiv.org/abs/2601.00029)
*Abolhassan Pishahang,Maryam Badiei*

Main category: cs.AI

TL;DR: The study examines how AI interprets vernacular architecture using Iranian pigeon towers, finding it good at geometry but poor at material/climate reasoning, with trade-offs between realism and creativity.


<details>
  <summary>Details</summary>
Motivation: The study aims to explore how generative AI systems interpret the architectural intelligence embedded in vernacular forms, specifically focusing on the Iranian pigeon tower.

Method: The research uses three diffusion models (Midjourney v6, DALL-E 3, and DreamStudio based on Stable Diffusion XL) across three prompt stages (referential, adaptive, and speculative) to analyze the Iranian pigeon tower. A five-criteria evaluation framework assesses typology, materiality, environment, realism, and cultural specificity.

Result: Results indicate that AI reliably reproduces geometric patterns but misinterprets material and climatic reasoning. Reference imagery enhances realism but restricts creativity, whereas freedom from reference leads to inventive yet culturally ambiguous outcomes.

Conclusion: The study concludes that while generative AI can replicate geometric patterns in vernacular architecture, it struggles with material and climatic reasoning. The research highlights the boundary between visual resemblance and architectural reasoning, proposing computational vernacular reasoning as a framework to understand AI's interpretation of traditional design intelligence.
...
### [66] [The Agentic Leash: Extracting Causal Feedback Fuzzy Cognitive Maps with LLMs](https://arxiv.org/abs/2601.00097)
...
### [67] [Mortar: Evolving Mechanics for Automatic Game Design](https://arxiv.org/abs/2601.00105)
*Muhammad U. Nasir,Yuchen Li,Steven James,Julian Togelius*

Main category: cs.AI

TL;DR: Mortar是一个自动化游戏机制设计系统，结合算法和语言模型生成多样机制，并通过树搜索评估其对游戏技能排序的贡献，结果证明其有效且可玩。


<details>
  <summary>Details</summary>
Motivation: 手动设计游戏机制是一个耗时且依赖专家经验的过程，Mortar旨在通过自动化方法生成和评估游戏机制，提高游戏设计的效率和多样性。

Method: Mortar结合质量多样性算法和大型语言模型探索多样化的游戏机制，通过树搜索程序合成完整游戏并评估其对技能排序的贡献。

Result: Mortar生成的游戏机制不仅多样且可玩，还能显著提升游戏中的技能排序得分。消融研究和用户研究验证了系统各组件的有效性和人类可玩性。

Conclusion: Mortar系统通过结合质量多样性算法和大型语言模型，成功生成了多样且可玩的游戏机制，并通过树搜索程序评估这些机制对游戏技能排序的贡献。消融研究和用户研究进一步验证了系统各组件的有效性和游戏的人类可玩性。
...
### [68] [Ask, Clarify, Optimize: Human-LLM Agent Collaboration for Smarter Inventory Control](https://arxiv.org/abs/2601.00121)
*Yaqi Duan,Yichun Hu,Jiashuo Jiang*

Main category: cs.AI

TL;DR: LLMs as interfaces in a hybrid framework reduce inventory costs by 32.1%, showing they complement but don't replace traditional optimization methods.


<details>
  <summary>Details</summary>
Motivation: To address the 'hallucination tax' and performance gap in using LLMs as direct, end-to-end solvers for inventory management in small and medium-sized businesses.

Method: A hybrid agentic framework that decouples semantic reasoning from mathematical calculation, using LLMs as intelligent interfaces and rigorous algorithms for optimization.

Result: The hybrid framework reduces total inventory costs by 32.1% compared to using GPT-4o as an end-to-end solver, demonstrating its effectiveness.

Conclusion: LLMs are positioned as natural-language interfaces to make rigorous, solver-based policies accessible to non-experts, not as replacements for operations research.
...
### [69] [Constructing a Neuro-Symbolic Mathematician from First Principles](https://arxiv.org/abs/2601.00125)
*Keqin Xie*

Main category: cs.AI

TL;DR: Mathesis是一种神经符号架构，通过能量最小化和符号推理内核解决LLMs的逻辑失败问题。
...
Method: 提出Mathesis神经符号架构，使用符号推理内核（SRK）将约束映射到连续能量景观，并通过全局能量函数E(G)实现逻辑一致性。结合蒙特卡洛树搜索和进化证明搜索进行多步推理。

Result: Mathesis架构通过能量最小化将证明搜索转化为梯度信号训练，实现了逻辑一致性和多步推理。

Conclusion: Mathesis架构通过将数学状态编码为高阶超图，并利用符号推理内核（SRK）进行梯度信号训练，成功解决了大型语言模型在复杂推理中的逻辑失败问题。
...
### [70] [Explicit Abstention Knobs for Predictable Reliability in Video Question Answering](https://arxiv.org/abs/2601.00138)
*Jorge Ortiz*

Main category: cs.AI

TL;DR: 研究探讨了视觉语言模型（VLMs）在高风险部署中的选择性预测问题，特别是在视频问答中基于置信度的弃权机制是否可靠，以及这种控制在分布偏移下是否稳健。


<details>
  <summary>Details</summary>
Motivation: 高风险部署的视觉语言模型需要选择性预测以避免代价高昂的错误。

Method: 使用NExT-QA和Gemini 2.0 Flash进行实验，分析置信度阈值对错误率的控制效果。

Result: 置信度阈值在分布内提供了机制性控制，通过调整阈值可以平滑地权衡风险与覆盖率。

Conclusion: 置信度阈值在分布内有效控制错误率，但其在分布偏移下的稳健性仍需进一步验证。
...
### [71] [An AI Monkey Gets Grapes for Sure -- Sphere Neural Networks for Reliable Decision-Making](https://arxiv.org/abs/2601.00142)
*Tiansi Dong,Henry He,Pietro Liò,Mateja Jamnik*

Main category: cs.AI

TL;DR: The paper compares three neural reasoning methods, highlighting the limitations of LLMs and supervised learning. It proposes Sphere Neural Networks for reliable reasoning, demonstrating superior performance in syllogistic tasks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the unreliability of LLMs in reasoning tasks and the limitations of supervised learning-based reasoning, which suffers from catastrophic forgetting and limited reasoning competence.

Method: The paper compares three methodological categories of neural reasoning and introduces a new version of Sphere Neural Networks that embeds concepts as circles on the surface of an n-dimensional sphere. This method enables the representation of the negation operator via complement circles and filters out illogical statements.

Result: The Sphere Neural Networks achieve reliable decision-making by mastering 16 syllogistic reasoning tasks, including rigorous disjunctive syllogistic reasoning, while preserving the rigor of classical syllogistic reasoning.

Conclusion: The paper concludes that neural reasoning with explicit model construction is the most reliable among the three methodological categories (LLM reasoning, supervised learning-based reasoning, and explicit model-based reasoning).
...
### [72] [Will LLM-powered Agents Bias Against Humans? Exploring the Belief-Dependent Vulnerability](https://arxiv.org/abs/2601.00240)
...
### [73] [ClinicalReTrial: A Self-Evolving AI Agent for Clinical Trial Protocol Optimization](https://arxiv.org/abs/2601.00290)
...
### [74] [Multiagent Reinforcement Learning for Liquidity Games](https://arxiv.org/abs/2601.00324)
...
### [75] [Bio-inspired Agentic Self-healing Framework for Resilient Distributed Computing Continuum Systems](https://arxiv.org/abs/2601.00339)
...
### [76] [Adaptive Causal Coordination Detection for Social Media: A Memory-Guided Framework with Semi-Supervised Learning](https://arxiv.org/abs/2601.00400)
...
### [77] [Can Semantic Methods Enhance Team Sports Tactics? A Methodology for Football with Broader Applications](https://arxiv.org/abs/2601.00421)
...
### [78] [Progressive Ideation using an Agentic AI Framework for Human-AI Co-Creation](https://arxiv.org/abs/2601.00475)
...
### [79] [The Illusion of Insight in Reasoning Models](https://arxiv.org/abs/2601.00514)
...
### [80] [DA-DPO: Cost-efficient Difficulty-aware Preference Optimization for Reducing MLLM Hallucinations](https://arxiv.org/abs/2601.00623)
...
### [81] [A Vision-and-Knowledge Enhanced Large Language Model for Generalizable Pedestrian Crossing Behavior Inference](https://arxiv.org/abs/2601.00694)
...
### [82] [An Agentic Framework for Neuro-Symbolic Programming](https://arxiv.org/abs/2601.00743)
