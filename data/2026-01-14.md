...
TL;DR: 本研究提出了一种混合可解释AI框架，结合模糊逻辑和SHAP解释，显著提升了母体健康风险预测的可信度和临床适用性。


<details>
  <summary>Details</summary>
Motivation: 解决机器学习在母体健康风险预测中缺乏可解释性和信任的问题，尤其是在资源受限的临床环境中。

Method: 开发了一个基于模糊-XGBoost的模型，并在1,014份母体健康记录上进行了验证，准确率达到88.67%（ROC-AUC: 0.9703）。通过14名医疗专业人员的反馈验证了混合解释方法的有效性。

Result: 模型准确率为88.67%，ROC-AUC为0.9703。医疗专业人员对混合解释方法的偏好率为71.4%，54.8%表示信任其临床使用。SHAP分析显示医疗可及性是主要预测因子。

Conclusion: 本研究通过结合模糊逻辑和SHAP解释的混合可解释AI框架，提升了机器学习在母体健康风险预测中的可信度和临床适用性。验证结果显示，该框架在资源受限环境下具有实际应用潜力。
...
TL;DR: The paper proposes a training-free method for model calibration to help models recognize when they don't know, leading to applications like model cascading and data cleaning, which improve efficiency and reliability in AI systems.


<details>
  <summary>Details</summary>
Motivation: The motivation is to enable models to recognize when they do not know, which can lead to more reliable and trustworthy AI. The paper addresses the challenge of how to reliably measure a model's confidence in its predictions.

Method: The paper proposes a training-free method for model calibration that applies to both vision and language models. It leverages empirical observations about confidence and accuracy, and introduces two applications: model cascading with calibrated advantage routing and data cleaning based on model ensemble.

Result: The results show that calibrated confidence is reliable and comparable, enabling practical applications like model cascading (improving efficiency without compromising accuracy) and data cleaning (identifying mislabeled samples in datasets like ImageNet and MMLU).

Conclusion: The paper concludes that enabling models to recognize when they do not know is a practical step toward more efficient, reliable, and trustworthy AI, as demonstrated by the applications of model cascading and data cleaning using calibrated confidences.
...
TL;DR: CADA enhances LLM safety by using case-augmented reasoning instead of rigid safety rules, improving harmlessness and adaptability.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of ensuring LLMs adhere to safety principles without refusing benign requests, especially in open-source LLMs which lack advanced reasoning capabilities.

Method: The study evaluates the impact of explicitly specifying extensive safety codes versus demonstrating them through illustrative cases, proposing CADA which utilizes reinforcement learning on self-generated safety reasoning chains.

Result: Referencing explicit codes inconsistently improves harmlessness and systematically degrades helpfulness, whereas training on case-augmented simple codes yields more robust and generalized safety behaviors.

Conclusion: CADA, a case-augmented deliberative alignment method, effectively enhances harmlessness, improves robustness against attacks, and reduces over-refusal while preserving utility across diverse benchmarks.
...
TL;DR: 本文分析了2025年美国和欧盟对前沿AI内部部署的监管漏洞，揭示了三个主要问题及其原因，并提出了可能的解决方案，以帮助政策制定者更有效地监管内部AI系统。


<details>
  <summary>Details</summary>
Motivation: 前沿AI的监管主要集中在面向外部用户的系统上，而忽视了企业内部部署的高风险应用。这种内部部署可能导致监管失效，从而引发潜在的高风险问题。本文旨在填补这一研究空白。

Method: 本文通过分析美国和欧盟2025年的前沿AI监管框架，识别了三个监管漏洞，并探讨了这些漏洞存在的原因和可能的解决途径。研究采用了政策分析和案例研究的方法。

Result: 研究发现，美国和欧盟的监管框架存在三个主要漏洞：(1)监管范围模糊，(2)静态合规评估无法应对系统的持续演化，(3)信息不对称阻碍监管。这些漏洞源于可测量性、激励和信息获取的冲突。

Conclusion: 本文通过分析美国和欧盟在2025年对前沿AI内部部署的监管框架，揭示了三个可能导致监管失效的漏洞，并探讨了这些漏洞存在的原因及可能的解决方案。作者希望通过这一分析，促使政策制定者在面对内部部署的AI系统时能够更加审慎地做出决策。
...
TL;DR: SCASED是一种基于物联网的系统，结合自动考勤和面部情绪识别，帮助教师监测课堂参与度，情绪分类准确率达89.5%。


<details>
  <summary>Details</summary>
Motivation: 该论文的动机是解决高等教育中智能教室技术主要关注自动考勤而忽视学生情绪和认知参与的问题，从而帮助教师实时识别学生的脱离情况并调整教学策略。

Method: 该论文提出的SCASED系统基于物联网技术，结合了自动考勤跟踪和面部情绪识别功能。系统使用树莓派摄像头和OpenCV进行人脸检测，并通过微调的MobileNetV2模型对四种学习相关情绪状态（投入、无聊、困惑和沮丧）进行分类。此外，系统采用基于会话的机制来管理考勤和情绪监测。

Result: 实验评估使用DAiSEE数据集，情绪分类准确率达到89.5%。结果表明，将考勤数据与情绪分析相结合可以为教师提供更多课堂动态的洞察。

Conclusion: 该论文的结论是，通过将考勤数据与情绪分析相结合，SCASED系统能够为教师提供更多关于课堂动态的洞察，支持更具响应性的教学实践。
...
TL;DR: A new Deep Reinforcement Learning framework for dairy farms combines Forecast Aware PPO and PID KL PPO to optimize energy use, reducing costs and grid reliance more effectively than traditional methods.


<details>
  <summary>Details</summary>
Motivation: Dairy farming is highly energy-intensive and reliant on grid electricity. The intermittent nature of renewable energy sources makes real-time supply-demand balancing challenging. Existing RL-based scheduling methods often assume complete knowledge of future conditions, which is unrealistic. This study aims to address these gaps by developing a more adaptive and efficient energy management system.

Method: The study introduces a Deep Reinforcement Learning framework with two variants: Forecast Aware PPO (incorporating short-term forecasts of demand and renewable generation) and PID KL PPO (using a PID controller to regulate KL divergence for stable policy updates). The framework is trained on real-world dairy farm data to optimize battery storage and water heating under operational constraints.

Result: The proposed method achieves up to 1% lower electricity costs than standard PPO, 4.8% lower than DQN, and 1.5% lower than SAC. For battery scheduling, PPO reduces grid imports by 13.1%, demonstrating significant improvements in cost savings and grid independence.

Conclusion: The study successfully demonstrates that the proposed Deep Reinforcement Learning framework, incorporating Forecast Aware PPO and PID KL PPO variants, effectively reduces electricity costs and grid dependence in dairy farming. The method outperforms traditional PPO, DQN, and SAC approaches, highlighting its scalability and practical applicability for sustainable energy management.
...
TL;DR: 本文提出了一种新的算法，用于计算神经反馈系统的后向可达集，并结合前向分析技术，构建了一个统一的验证框架。


<details>
  <summary>Details</summary>
Motivation: 现有的后向可达性方法在可扩展性方面存在局限，因此需要新的算法来改进神经反馈系统的验证。

Method: 引入新的算法计算后向可达集的上下近似，并将其与前向分析技术结合。

Result: 构建了一个统一的验证框架，提高了神经反馈系统的验证效率和准确性。

Conclusion: 通过结合前后向分析方法，本文提出的框架显著提升了神经反馈系统的验证能力。
...
TL;DR: 论文首次全面研究了负指令失败的机制，发现语义压力与违反概率呈逻辑关系，并揭示了两种失败模式及其成因。
...
Result: 发现违反概率与语义压力呈紧密逻辑关系，并识别出两种机制不同的失败模式：启动失败（87.5%）和覆盖失败（12.5%）。

Conclusion: 研究揭示了负约束设计中的根本矛盾：命名被禁止的词语反而会促使模型生成该词语。
...
TL;DR: MemoBrain enhances long-horizon reasoning by managing memory, improving task alignment and logical continuity.


<details>
  <summary>Details</summary>
Motivation: Addresses the issue of reasoning traces and transient tool artifacts accumulating and straining the working context of large language models, disrupting logical continuity and task alignment.

Method: Proposes MemoBrain, an executive memory model that constructs dependency-aware memory over reasoning steps, pruning invalid steps and folding completed sub-trajectories.

Result: Evaluation on benchmarks like GAIA, WebWalker, and BrowseComp-Plus shows MemoBrain's effectiveness.

Conclusion: MemoBrain demonstrates consistent improvements over strong baselines in handling long-horizon reasoning tasks by managing memory effectively.
...
TL;DR: 本文研究了交通事故后车辆变道行为（post crash LCs），发现其持续时间更长、插入速度更低且碰撞风险更高。通过无人机视频构建数据集，提出了一种新的轨迹预测框架，显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 交通事故后车辆变道行为的特征和运动模式尚不明确，且目标车道车辆的不让行行为增加了变道的复杂性和碰撞风险。

Method: 通过无人机视频提取车辆轨迹构建数据集，并开发了一种基于图注意力模块的轨迹预测框架，显式建模让行行为。

Result: 与现有基线相比，新模型在轨迹预测性能上提升了10%以上，并提供了更可靠的碰撞风险分析。

Conclusion: 本文提出的框架在预测交通事故后车辆变道行为和碰撞风险方面表现出色，具有较高的可迁移性。
...
TL;DR: 提出了一种在边缘设备上实现低延迟AI伴侣系统的内存范式，通过交替活跃和非活跃阶段优化资源使用。


<details>
  <summary>Details</summary>
Motivation: 边缘设备计算资源有限，现有AI伴侣和内存系统无法直接应用。

Method: 采用活跃与非活跃阶段交替的内存范式，活跃阶段进行低延迟实时对话，非活跃阶段进行内存提取和整合。

Result: 在量化模型Qwen2.5-7B-Instruct上，系统性能优于无内存的原始LLM，并与GPT-3.5（16k上下文窗口）相当。

Conclusion: 该设计在嵌入式硬件严格限制下，实现了低延迟和长期个性化，为边缘设备AI伴侣系统提供了可行方案。
...
TL;DR: Project Synapse is a hierarchical multi-agent framework for autonomously resolving last-mile delivery disruptions, validated with real-world data and LLM-based evaluation.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of last-mile delivery disruptions through an autonomous and scalable agentic framework.

Method: The framework employs a central Resolution Supervisor agent for strategic task decomposition and specialized worker agents for tactical execution, orchestrated using LangGraph.

Result: Validated using a benchmark dataset of 30 complex disruption scenarios derived from real-world user reviews, with performance evaluated via an LLM-as-a-Judge protocol.

Conclusion: Project Synapse presents a hierarchical multi-agent framework for autonomous last-mile delivery disruption resolution, validated through real-world scenario benchmarks and an LLM-as-a-Judge protocol.
...
TL;DR: 提出了一种基于模型的分层多智能体强化学习框架，用于多核平台的热能和能源感知调度，显著提升了能源效率和任务完成时间。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖忽略停滞时间的基于利用率的启发式方法，要么需要大量离线分析生成表格，无法适应运行时变化。因此，需要一种更高效、适应性更强的调度方法。

Method: 采用基于模型的分层多智能体强化学习（MARL）框架，结合回归技术预测热力学和性能状态，并通过LLM提取语义特征实现零样本部署。

Result: 实验结果表明，该方法在能源效率上比Linux ondemand governor高出7.09倍，任务完成时间缩短4.0倍，首次决策延迟比基于表格的分析快8,300倍。

Conclusion: 该论文提出的基于模型的分层多智能体强化学习框架在动态嵌入式系统中表现出色，显著提升了能源效率和任务完成时间，同时大幅降低了首次决策延迟，适用于实际部署。
...
TL;DR: HS2C框架利用图同质性进行结构和语义压缩，提升了大型语言模型在图理解任务中的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的图理解方法受限于上下文窗口，通常采用随机采样，导致噪声和推理不稳定性。作者认为图本身包含丰富的结构和语义信息，有效利用这些信息可以提升大型语言模型的推理性能。

Method: HS2C框架通过结构熵最小化原则进行全局层次划分，识别同质性社区并丢弃噪声；在语义上，通过社区类型进行差异化的语义聚合，压缩冗余背景信息并保留与目标节点对齐的语义信息。

Result: 在10个节点级基准测试和7个图级基准测试中，HS2C框架显著提升了压缩率和下游推理准确性，验证了其优越性和任务通用性。

Conclusion: HS2C框架通过利用图同质性，在结构和语义上进行压缩，显著提升了大型语言模型在图理解任务中的推理性能和压缩效率，并在多种基准测试中验证了其优越性和可扩展性。
...
### [19] [Large Artificial Intelligence Model Guided Deep Reinforcement Learning for Resource Allocation in Non Terrestrial Networks](https://arxiv.org/abs/2601.08254)
...
### [20] [T3: Benchmarking Sycophancy and Skepticism in Causal Judgment](https://arxiv.org/abs/2601.08258)
*Edward Y. Chang*
...
### [21] [VGG Induced Deep Hand Sign Language Detection](https://arxiv.org/abs/2601.08262)
...
### [22] [Sparsity Is Necessary: Polynomial-Time Stability for Agentic LLMs in Large Action Spaces](https://arxiv.org/abs/2601.08271)
*Angshul Majumdar*
...
### [23] [ToolACE-MCP: Generalizing History-Aware Routing from MCP Tools to the Agent Web](https://arxiv.org/abs/2601.08276)
...
### [24] [Greedy Is Enough: Sparse Action Discovery in Agentic LLMs](https://arxiv.org/abs/2601.08280)
*Angshul Majumdar*
...
### [25] [OpenMic: A Multi-Agent-Based Stand-Up Comedy Generation System](https://arxiv.org/abs/2601.08288)
...
### [26] [AtomMem : Learnable Dynamic Agentic Memory with Atomic Memory Operation](https://arxiv.org/abs/2601.08323)
...
### [27] [Semantic Laundering in AI Agent Architectures: Why Tool Boundaries Do Not Confer Epistemic Warrant](https://arxiv.org/abs/2601.08333)
...
### [28] [Thematic Working Group 5 -- Artificial Intelligence (AI) literacy for teaching and learning: design and implementation](https://arxiv.org/abs/2601.08380)
...
### [29] [A Qualitative Model to Reason about Object Rotations (QOR) applied to solve the Cube Comparison Test (CCT)](https://arxiv.org/abs/2601.08382)
*Zoe Falomir*
...
### [30] [Deconstructing Pre-training: Knowledge Attribution Analysis in MoE and Dense Models](https://arxiv.org/abs/2601.08383)
...
### [31] [Creativity in AI as Emergence from Domain-Limited Generative Models](https://arxiv.org/abs/2601.08388)
*Corina Chutaux*
...
### [32] [Owen-Shapley Policy Optimization (OSPO): A Principled RL Algorithm for Generative Search LLMs](https://arxiv.org/abs/2601.08403)
...
### [33] [WebTrap Park: An Automated Platform for Systematic Security Evaluation of Web Agents](https://arxiv.org/abs/2601.08406)
...
### [34] [Hybrid Distillation with CoT Guidance for Edge-Drone Control Code Generation](https://arxiv.org/abs/2601.08412)
...
### [35] [RubricHub: A Comprehensive and Highly Discriminative Rubric Dataset via Automated Coarse-to-Fine Generation](https://arxiv.org/abs/2601.08430)
...
### [36] [YaPO: Learnable Sparse Activation Steering Vectors for Domain Adaptation](https://arxiv.org/abs/2601.08441)
...
### [37] [Beyond Linearization: Attributed Table Graphs for Table Reasoning](https://arxiv.org/abs/2601.08444)
...
### [38] [An Under-Explored Application for Explainable Multimodal Misogyny Detection in code-mixed Hindi-English](https://arxiv.org/abs/2601.08457)
...
### [39] [SUMMPILOT: Bridging Efficiency and Customization for Interactive Summarization System](https://arxiv.org/abs/2601.08475)
...
### [40] [What If TSF: A Benchmark for Reframing Forecasting as Scenario-Guided Multimodal Forecasting](https://arxiv.org/abs/2601.08509)
...
### [41] [Sketch-Based Facade Renovation With Generative AI: A Streamlined Framework for Bypassing As-Built Modelling in Industrial Adaptive Reuse](https://arxiv.org/abs/2601.08531)
...
### [42] [Learner-Tailored Program Repair: A Solution Generator with Iterative Edit-Driven Retrieval Enhancement](https://arxiv.org/abs/2601.08545)
...
### [43] [WaterCopilot: An AI-Driven Virtual Assistant for Water Management](https://arxiv.org/abs/2601.08559)
...
### [44] [ViDoRe V3: A Comprehensive Evaluation of Retrieval Augmented Generation in Complex Real-World Scenarios](https://arxiv.org/abs/2601.08620)
...
### [45] [Resisting Manipulative Bots in Memecoin Copy Trading: A Multi-Agent Approach with Chain-of-Thought Reasoning](https://arxiv.org/abs/2601.08641)
...
### [46] [Prism: Towards Lowering User Cognitive Load in LLMs via Complex Intent Understanding](https://arxiv.org/abs/2601.08653)
...
### [47] [From Classical to Quantum Reinforcement Learning and Its Applications in Quantum Control: A Beginner's Tutorial](https://arxiv.org/abs/2601.08662)
...
### [48] [Parallel Context-of-Experts Decoding for Retrieval Augmented Generation](https://arxiv.org/abs/2601.08670)
...
### [49] [Why AI Alignment Failure Is Structural: Learned Human Interaction Structures and AGI as an Endogenous Evolutionary Shock](https://arxiv.org/abs/2601.08673)
...
### [50] [Advancing ESG Intelligence: An Expert-level Agent and Comprehensive Benchmark for Sustainable Finance](https://arxiv.org/abs/2601.08676)
...
### [57] [Pervasive Annotation Errors Break Text-to-SQL Benchmarks and Leaderboards](https://arxiv.org/abs/2601.08778)
...
### [58] [Uncovering Political Bias in Large Language Models using Parliamentary Voting Records](https://arxiv.org/abs/2601.08785)
...
TL;DR: HSS压缩方案通过稀疏和低秩分解两阶段方法，显著减少了LLMs的内存和计算需求，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 现代大型语言模型对内存和计算资源的需求极高，因此需要高效的压缩方法以支持部署和持续训练。

Method: 提出了一种名为HSS的两阶段压缩方案：第一阶段移除最大幅值的权重到稀疏矩阵S，第二阶段对剩余密集矩阵应用递归的层次稀疏可分离低秩分解。采用了递归降秩策略和RCM排列以优化压缩效果。

Result: 在LLaMA-7B模型中，仅压缩自注意力投影部分（1.6B参数）即可实现显著的内存节省。例如，在30%稀疏预算和外部秩为512的情况下，sHSS-RCM的困惑度为1.64，优于密集基线和传统稀疏加SVD变体。

Conclusion: HSS压缩方案通过两阶段方法（稀疏矩阵和低秩分解）有效减少了大型语言模型的内存和计算需求，同时保持了模型的性能。实验证明，在LLaMA-7B模型中，仅压缩自注意力投影部分即可实现显著的内存节省，且不影响模型在WikiText数据集上的困惑度表现。
...
TL;DR: Regularisation-based continual learning methods (e.g., EWC, SI, MAS) are ineffective for EEG-based emotion classification due to misaligned stability-plasticity trade-offs and other limitations.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of generalisation to unseen subjects in EEG-based emotion classification, given high inter- and intra-subject variability, and to explore the suitability of regularisation-based CL methods.

Method: The study theoretically and empirically evaluates regularisation-based CL methods (EWC, SI, MAS) on EEG-based emotion classification using DREAMER and SEED datasets, focusing on subject-incremental sequences.

Result: Regularisation-based CL methods show limited performance, with no statistically significant improvement in forward transfer over sequential fine-tuning. Issues include unreliable parameter importance heuristics, gradient interference, over-constraining, and subject order sensitivity.

Conclusion: Regularisation-based continual learning approaches are limited for robust generalisation to unseen subjects in EEG-based emotion classification due to misalignment in the stability-plasticity trade-off, unreliable heuristics under noisy data, and sensitivity to subject order.
...
### [61] [HOSC: A Periodic Activation with Saturation Control for High-Fidelity Implicit Neural Representations](https://arxiv.org/abs/2601.07870)
*Michal Jan Wlodarczyk,Danzel Serrano,Przemyslaw Musialski*

Main category: cs.LG

TL;DR: HOSC是一种新型周期性激活函数，通过可调参数控制梯度幅度，在多个领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决周期性激活函数在隐式神经表示（INRs）中梯度不稳定和多尺度行为控制有限的问题。

Method: 引入Hyperbolic Oscillator with Saturation Control (HOSC)激活函数，通过数学分析和标准化训练协议进行综合实证研究。

Result: HOSC在多个领域中表现出色，提供了对梯度幅度的直接控制，并在实验中显示出显著优势或竞争力。

Conclusion: HOSC激活函数通过引入可调参数β和ω₀，提供了对梯度幅度的直接控制，同时在多个领域（如图像、音频、视频、NeRFs和SDFs）中表现出色，与现有方法相比具有显著优势或竞争力。
...
### [62] [Multiplicative Orthogonal Sequential Editing for Language Models](https://arxiv.org/abs/2601.07873)
*Hao-Xiang Xu,Jun-Yu Ma,Ziqi Peng,Yuhao Sun,Zhen-Hua Ling,Jia-Chen Gu*

Main category: cs.LG

TL;DR: MOSE提出了一种乘法正交序列编辑范式，通过正交矩阵乘法保持数值稳定性，显著提升了顺序编辑性能并保留了模型的通用能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型知识编辑方法主要通过加法范式更新参数矩阵，但这种方法会损害数值稳定性指标（如条件数和范数），尤其是在顺序编辑场景中。因此，需要一种新的编辑范式来从根本上解决这一问题。

Method: MOSE采用乘法编辑范式，将新知识融入正交矩阵并与原始参数矩阵相乘，从而保持矩阵的数值稳定性。

Result: 实验结果表明，MOSE在顺序编辑性能上提升了12.08%，并在下游任务中保持了95.73%的通用能力。

Conclusion: MOSE通过引入乘法正交序列编辑范式，有效解决了现有加法编辑范式在数值稳定性上的问题，显著提升了顺序编辑性能并保持了模型的通用能力。
...
### [63] [E^2-LLM: Bridging Neural Signals and Interpretable Affective Analysis](https://arxiv.org/abs/2601.07877)
*Fei Ma,Han Lin,Yifan Xie,Hongwei Ren,Xiaoyu Shen,Wenbo Ding,Qi Tian*

Main category: cs.LG

TL;DR: E^2-LLM is a novel framework for interpretable emotion analysis from EEG signals, combining EEG encoders with LLMs to improve accuracy and understanding.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of emotion recognition from EEG signals, including high inter-subject variability, limited labeled data, and the lack of interpretable reasoning in existing approaches.

Method: E^2-LLM integrates a pretrained EEG encoder with Qwen-based LLMs through learnable projection layers, employing a multi-stage training pipeline that encompasses emotion-discriminative pretraining, cross-modal alignment, and instruction tuning with chain-of-thought reasoning.

Result: E^2-LLM achieves excellent performance on emotion classification, with larger variants showing enhanced reliability and superior zero-shot generalization to complex reasoning scenarios.

Conclusion: E^2-LLM establishes a new paradigm combining physiological signals with LLM reasoning capabilities, demonstrating improved recognition accuracy and interpretable emotional understanding in affective computing.
...
### [64] [Sliced-Wasserstein Distribution Alignment Loss Improves the Ultra-Low-Bit Quantization of Large Language Models](https://arxiv.org/abs/2601.07878)
*Deyu Cao,Yixin Yin,Samin Aref*

Main category: cs.LG

TL;DR: 本文提出了一种基于切片Wasserstein损失的分布感知校准方法，用于超低位后训练量化，显著提升了量化模型的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在部署时存在资源效率低下的问题，量化虽能改善但超低位量化常导致性能下降。

Method: 引入切片Wasserstein损失函数，通过随机线性投影对齐全精度和量化模型的输出分布。

Result: 在LLaMA-2-7B、OPT-6.7B和LLaMA-2-13B上分别恢复了4.12-20.37%、0.93-7.65%和2.26-6.20%的准确率损失。

Conclusion: 分布对齐方法简单有效，可显著提升前沿量化方法的性能。
...
### [65] [KVzap: Fast, Adaptive, and Faithful KV Cache Pruning](https://arxiv.org/abs/2601.07891)
*Simon Jegou,Maximilian Jeblick*

Main category: cs.LG

TL;DR: KVzap是一种高效的KV缓存压缩方法，适用于长上下文和推理任务，在多个模型上实现了2-4倍的压缩且精度损失可忽略。


<details>
  <summary>Details</summary>
Motivation: 解决基于Transformer的语言模型中KV缓存的推理瓶颈问题。

Method: 引入KVzap，一种快速、输入自适应的KVzip近似方法，适用于预填充和解码阶段。

Result: 在Qwen3-8B、Llama-3.1-8B-Instruct和Qwen3-32B等模型上，KVzap实现了2-4倍的KV缓存压缩，且精度损失可忽略。

Conclusion: KVzap在KVpress排行榜上实现了最先进的性能，代码和模型已开源。
...
### [66] [Sherry: Hardware-Efficient 1.25-Bit Ternary Quantization via Fine-grained Sparsification](https://arxiv.org/abs/2601.07892)
*Hong Huang,Decheng Wu,Qiangqiang Hu,Guanghua Yu,Jinhai Yang,Jianchen Zhu,Xue Liu,Dapeng Wu*

Main category: cs.LG

TL;DR: Sherry是一种硬件高效的三元量化框架，通过3:4稀疏性和Arenas机制，在保持性能的同时减少模型大小和提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在资源受限的边缘设备上部署时，面临内存和计算需求过高的问题，而现有的三元量化方法在硬件对齐和效率上存在不足。

Method: Sherry框架采用3:4细粒度稀疏性，将四个权重打包为五位，实现1.25位宽度的正则化，并通过Arenas机制解决稀疏三元训练中的权重陷阱问题。

Result: 在LLaMA-3.2的五个基准测试中，Sherry在保持最先进三元性能的同时显著减少了模型大小，并在Intel i7-14700HX CPU上实现了零精度损失、25%的比特节省和10%的速度提升。

Conclusion: Sherry框架通过3:4细粒度稀疏性和Arenas机制，在保持模型性能的同时显著减少了模型大小和计算资源需求，并在实际硬件上实现了速度和存储效率的提升。
...
### [67] [Revealing the Attention Floating Mechanism in Masked Diffusion Models](https://arxiv.org/abs/2601.07894)
*Xin Dai,Pengcheng Huang,Zhenghao Liu,Shuo Wang,Yukun Yan,Chaojun Xiao,Yu Gu,Ge Yu,Maosong Sun*

Main category: cs.LG

TL;DR: MDMs通过动态分散的注意力机制（Attention Floating）在知识密集型任务中表现优于ARMs，性能提升了一倍。


<details>
  <summary>Details</summary>
Motivation: 探索MDMs内部注意力机制，以解释其与ARMs性能差距缩小的原因。

Method: 研究了MDMs中的注意力行为，揭示了Attention Floating现象，并分析了其浅层结构感知、深层内容聚焦的注意力机制。

Result: 发现MDMs的注意力机制具有动态分散的锚点，浅层构建全局结构框架，深层聚焦语义内容，从而增强了上下文学习能力。

Conclusion: MDMs通过独特的注意力机制（如Attention Floating现象）在知识密集型任务中表现优于ARMs，性能提升了一倍。
...
### [68] [Large Language Models and Algorithm Execution: Application to an Arithmetic Function](https://arxiv.org/abs/2601.07898)
*Farah Ben Slama,Frédéric Armetta*

Main category: cs.LG

TL;DR: The paper introduces LLM-DAL, a training model that enhances LLMs' algorithmic reasoning by focusing on decomposition and supervised learning, showing significant improvements.


<details>
  <summary>Details</summary>
Motivation: The motivation behind the study is to address the limitations of LLMs in internalizing data and autonomously executing algorithms, aiming to enhance their capabilities through specialized training.

Method: The paper introduces a training model called LLM-DAL (Large Language Model - Decompositional Algorithmic Learning), which uses supervised training to guide LLMs in decomposing and executing algorithms.

Result: The results demonstrate that LLMs' ability to perform complex algorithmic inferences and generalize can be significantly improved with the proposed training method.

Conclusion: The paper concludes that LLMs can achieve significant improvements in algorithmic reasoning and generalization through specialized training methods like LLM-DAL, which focuses on reasoning decomposition.
...
### [69] [Enhancing Large Language Models for Time-Series Forecasting via Vector-Injected In-Context Learning](https://arxiv.org/abs/2601.07903)
*Jianqi Zhang,Jingyao Wang,Wenwen Qiang,Fanjiang Xu,Changwen Zheng*

Main category: cs.LG

TL;DR: LVICL通过向量注入的上下文学习方法提升LLM4TSF的预测性能，同时减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 解决LLM4TSF在预测性能和计算开销上的双重挑战，探索一种在冻结LLM参数的同时提升预测性能的方法。

Method: 提出LVICL方法，利用可学习的上下文向量适配器从多个示例中自适应提取上下文向量，并将该向量注入LLM的每一层以提升预测性能。

Result: 大量实验证明了LVICL方法的有效性。

Conclusion: LVICL通过向量注入的上下文学习方法，在不增加提示长度的情况下，有效提升了LLM4TSF的预测性能，同时冻结所有LLM参数以减少计算开销。
...
### [70] [Towards Specialized Generalists: A Multi-Task MoE-LoRA Framework for Domain-Specific LLM Adaptation](https://arxiv.org/abs/2601.07935)
...
### [71] [Coupled Diffusion-Encoder Models for Reconstruction of Flow Fields](https://arxiv.org/abs/2601.07946)
...
### [72] [Reinforcement Learning Methods for Neighborhood Selection in Local Search](https://arxiv.org/abs/2601.07948)
...
### [73] [Hybrid SARIMA LSTM Model for Local Weather Forecasting: A Residual Learning Approach for Data Driven Meteorological Prediction](https://arxiv.org/abs/2601.07951)
...
### [74] [DataScribe: An AI-Native, Policy-Aligned Web Platform for Multi-Objective Materials Design and Discovery](https://arxiv.org/abs/2601.07966)
...
### [75] [Beyond the Next Port: A Multi-Task Transformer for Forecasting Future Voyage Segment Durations](https://arxiv.org/abs/2601.08013)
...
### [78] [LUT-Compiled Kolmogorov-Arnold Networks for Lightweight DoS Detection on IoT Edge Devices](https://arxiv.org/abs/2601.08044)
*Oleksandr Kuznetsov*
...
### [79] [Q-realign: Piggybacking Realignment on Quantization for Safe and Efficient LLM Deployment](https://arxiv.org/abs/2601.08089)
...
### [80] [Local-Global Feature Fusion for Subject-Independent EEG Emotion Recognition](https://arxiv.org/abs/2601.08094)
...
### [81] [STO-RL: Offline RL under Sparse Rewards via LLM-Guided Subgoal Temporal Order](https://arxiv.org/abs/2601.08107)
...
### [82] [Intra-tree Column Subsampling Hinders XGBoost Learning of Ratio-like Interactions](https://arxiv.org/abs/2601.08121)
*Mykola Pinchuk*
...
### [83] [Generalization Analysis and Method for Domain Generalization for a Family of Recurrent Neural Networks](https://arxiv.org/abs/2601.08122)
...
### [84] [Reverse Flow Matching: A Unified Framework for Online Reinforcement Learning with Diffusion and Flow Policies](https://arxiv.org/abs/2601.08136)
...
### [85] [Dynamic Graph Structure Learning via Resistance Curvature Flow](https://arxiv.org/abs/2601.08149)
...
### [86] [VBO-MI: A Fully Gradient-Based Bayesian Optimization Framework Using Variational Mutual Information Estimation](https://arxiv.org/abs/2601.08172)
*Farhad Mirkarimi*
...
### [87] [TabPFN Through The Looking Glass: An interpretability study of TabPFN and its internal representations](https://arxiv.org/abs/2601.08181)
...
### [88] [Scalable Multiagent Reinforcement Learning with Collective Influence Estimation](https://arxiv.org/abs/2601.08210)
...
### [91] [GADPN: Graph Adaptive Denoising and Perturbation Networks via Singular Value Decomposition](https://arxiv.org/abs/2601.08230)
*Hao Deng,Bo Liu*
...
### [92] [Incorporating Cognitive Biases into Reinforcement Learning for Financial Decision-Making](https://arxiv.org/abs/2601.08247)
*Liu He*
...
### [93] [Hyperbolic Heterogeneous Graph Transformer](https://arxiv.org/abs/2601.08251)
...
### [94] [LDLT L-Lipschitz Network Weight Parameterization Initialization](https://arxiv.org/abs/2601.08253)
...
### [95] [On Evaluation of Unsupervised Feature Selection for Pattern Classification](https://arxiv.org/abs/2601.08257)
...
### [96] [A Usable GAN-Based Tool for Synthetic ECG Generation in Cardiac Amyloidosis Research](https://arxiv.org/abs/2601.08260)
...
### [97] [Demystifying the Slash Pattern in Attention: The Role of RoPE](https://arxiv.org/abs/2601.08297)
...
### [98] [ORBIT: On-policy Exploration-Exploitation for Controllable Multi-Budget Reasoning](https://arxiv.org/abs/2601.08310)
...
### [99] [Deep Exploration of Epoch-wise Double Descent in Noisy Data: Signal Separation, Large Activation, and Benign Overfitting](https://arxiv.org/abs/2601.08316)
...
### [100] [Automated Machine Learning in Radiomics: A Comparative Evaluation of Performance, Efficiency and Accessibility](https://arxiv.org/abs/2601.08334)
...
### [101] [Decodable but not structured: linear probing enables Underwater Acoustic Target Recognition with pretrained audio embeddings](https://arxiv.org/abs/2601.08358)
...
### [102] [Training-Free Distribution Adaptation for Diffusion Models via Maximum Mean Discrepancy Guidance](https://arxiv.org/abs/2601.08379)
...
### [103] [Controlled LLM Training on Spectral Sphere](https://arxiv.org/abs/2601.08393)
...
### [104] [Out-of-distribution generalization of deep-learning surrogates for 2D PDE-generated dynamics in the small-data regime](https://arxiv.org/abs/2601.08404)
...
### [105] [Taxon: Hierarchical Tax Code Prediction with Semantically Aligned LLM Expert Guidance](https://arxiv.org/abs/2601.08418)
...
### [106] [Coverage Improvement and Fast Convergence of On-policy Preference Learning](https://arxiv.org/abs/2601.08421)
...
### [107] [DiffMM: Efficient Method for Accurate Noisy and Sparse Trajectory Map Matching via One Step Diffusion](https://arxiv.org/abs/2601.08482)
...
### [108] [Temporal Fusion Nexus: A task-agnostic multi-modal embedding model for clinical narratives and irregular time series in post-kidney transplant care](https://arxiv.org/abs/2601.08503)
...
### [109] [Your Group-Relative Advantage Is Biased](https://arxiv.org/abs/2601.08521)
...
### [110] [Contrastive and Multi-Task Learning on Noisy Brain Signals with Nonlinear Dynamical Signatures](https://arxiv.org/abs/2601.08549)
...
### [111] [EviNAM: Intelligibility and Uncertainty via Evidential Neural Additive Models](https://arxiv.org/abs/2601.08556)
...
### [112] [M$^2$FMoE: Multi-Resolution Multi-View Frequency Mixture-of-Experts for Extreme-Adaptive Time Series Forecasting](https://arxiv.org/abs/2601.08631)
...
### [113] [Provably Safe Reinforcement Learning using Entropy Regularizer](https://arxiv.org/abs/2601.08646)
...
### [114] [TRACE: Reconstruction-Based Anomaly Detection in Ensemble and Time-Dependent Simulations](https://arxiv.org/abs/2601.08659)
...
### [115] [Soft Partition-based KAPI-ELM for Multi-Scale PDEs](https://arxiv.org/abs/2601.08719)
...
### [116] [Model-Agnostic Solutions for Deep Reinforcement Learning in Non-Ergodic Contexts](https://arxiv.org/abs/2601.08726)
...
### [118] [Adaptive Requesting in Decentralized Edge Networks via Non-Stationary Bandits](https://arxiv.org/abs/2601.08760)
