...
TL;DR: 本文提出了商业世界模型（BWM）的概念与架构，这是一个专门针对商业和组织环境的世界模型。BWM将商业状态、动态、约束、目标和可行行动空间编码为可执行的内部模拟器，支持自主决策制定。


<details>
  <summary>Details</summary>
Motivation: AI的变革潜力不仅在于自动化预定义任务，更在于使智能系统能够从高层战略目标出发，规划、优化和执行商业计划。当前需要建立一个专门针对商业环境的世界模型，实现从基于指令的执行向目标驱动的规划和执行的转变。

Method: 提出了以商业语义为中心的表述，将商业状态、动态和行动与关键商业实体联系起来。该架构整合了语义数据表示、概率机器学习模型、确定性商业规则和显式行动空间，形成一个用于规划和反事实推理的连贯结构。

Result: 建立了一个商业世界模型的完整架构，使智能体能够模拟替代行动序列、估计其对未来商业结果的影响，并在不确定性下评估权衡。BWM作为一个可执行的内部模拟器，支持商业计划的自主规划和执行。

Conclusion: BWM为自主商业系统建立了概念基础，实现了从指令执行向目标驱动规划和执行的转变。该工作虽然各部分组件并不全新，但其贡献在于将它们组织为商业计划的可执行内部模拟器。
...
TL;DR: 本文提出"部署时记忆化"概念，将智能体记忆视为隐私-效用权衡问题，通过系统实验分析记忆设计选择（摘要压缩程度、检索广度、删除模式）对个性化效用、信息提取风险和删除保真度的联合影响。


<details>
  <summary>Details</summary>
Motivation: 基础模型智能体越来越成为长期运行的系统，需要跨交互记住用户信息，使记忆化成为显式的部署时功能而不仅仅是模型权重的属性。现有研究要么关注参数化记忆，要么审计固定的记忆配置，但没有系统表征记忆设计选择如何共同塑造个性化效用、提取风险和删除保真度。

Method: 1. 将智能体记忆视为隐私-效用前沿，用个性化召回率（PR）和对抗性提取率（AER）衡量
2. 探索三个记忆设计参数：摘要压缩程度、检索广度（k值）、删除模式
3. 引入遗忘残留分数（FRS）量化删除的信息是否仍能从派生记忆层级中恢复
4. 在LongMemEval数据集上进行实验，评估Gemma 3 12B和GPT-4o-mini模型

Result: 1. 关键词事实摘要使Gemma 3 12B和GPT-4o-mini的敏感信息提取分别减少76%和64%，同时几乎保持全部个性化召回
2. 一旦内容被压缩，增加检索广度k值不再恢复信息泄漏
3. 仅原始内容删除时，约20%情况下派生摘要副本仍可恢复
4. 仅完全流水线清除或墓碑标记重定向能使最差层级的残留归零

Conclusion: 持久智能体记忆必须作为一等记忆机制进行评估，需要同时考量：帮助智能体回忆什么、使什么信息可提取、以及能真正删除什么。记忆设计需要在个性化效用、安全风险和删除能力之间进行仔细权衡。
...
TL;DR: 该论文提出了AI辅助优化下的探索性适应理论，认为AI的长期适应效应取决于预测性辅助如何与探索性响应本身相互作用，可能导致探索替代或探索增强两种不同结果。


<details>
  <summary>Details</summary>
Motivation: 研究AI系统如何影响长期适应性演化，特别是AI的预测性辅助是否会替代或增强系统的探索能力，以及这对认知、制度和技术系统在复杂环境中的适应性意味着什么。

Method: 使用动态框架，在崎岖的认知景观上建模认知、制度和技术的演化。引入关键状态变量"自适应响应性"，衡量系统在变化条件下穿越不熟悉的概念和制度轨迹的能力。

Result: 发现在收敛性预测机制下，AI系统会替代探索性参与，降低自适应响应性，导致元稳定陷阱、滞后、过早收敛和探索崩溃动态。相反，在探索增强机制下，AI能放大探索搜索、概念穿越和适应流动性。

Conclusion: AI的长期适应效应不仅取决于AI能力本身，还取决于制度结构、发展背景和人机交互架构。有效替代参数是响应性依赖的：具有弱探索惯例的系统更易受探索替代影响，而具有高自适应响应性的系统可能使用AI来扩大探索流动性。
...
TL;DR: 论文提出一个几何动力学框架，研究预测性人工智能系统如何通过早期稳定化影响认知探索过程，导致探索响应性降低、滞后效应和探索空间收缩


<details>
  <summary>Details</summary>
Motivation: 传统认知理论将问题解决描述为通过结构化问题空间的探索搜索，而预测性AI系统引入了一种新机制：在内部探索展开之前就通过预测提供解决方案和决策轨迹。这引发了关于预测稳定化如何影响认知探索几何结构的研究动机

Method: 开发了一个几何动力学框架，其中注意力在由稳定化漂移、内源性探索扰动和响应性门控学习塑造的策略景观上演化。预测辅助被建模为外源性探索压缩过程，在自我生成的探索拓宽策略空间可及区域之前就稳定轨迹

Result: 1) 持续预测稳定化降低探索响应性，即使探索变异性仍存在；2) 曲率不对称累积和松弛，产生滞后效应和援助撤回后探索移动性的延迟恢复；3) 发展结果关键取决于稳定化的时机，早期干预在广泛表征多样化发生前就缩小未来探索遍历

Conclusion: 预测系统可能重塑探索性认知本身的几何结构。该框架生成关于探索熵、过早收敛和预测稳定化后延迟恢复的可实证检验预测，揭示了AI辅助可能改变人类认知发展的基本动态
...
TL;DR: AVLLMs内部音频-视觉信息流机制的研究：揭示音频和视觉信号如何通过网络影响最终预测，发现对于音频-视觉视频采用顺序信息流，多交错项采用并行流，并可丢弃已传输信息以提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型（MLLMs）在研究和实际应用中越来越重要，但音频和视觉信号如何通过网络影响最终预测的内部路径仍然不明确，需要理解AVLLMs内部音频-视觉信息的流向、利用和整合机制。

Method: 研究AVLLMs内部音频-视觉信息流，追踪音频和视觉信息在两种输入配置（音频-视觉视频和多交错音频-视觉项）中的路由、利用和整合方式，并在Qwen2.5-Omni和Video-SALMONN2 Plus等模型（3B和7B规模）上进行验证。

Result: 1）对于音频-视觉视频，AVLLMs遵循与VLMs和VideoLLMs相同的顺序信息流路径，音频和视觉贡献比例与任务对各模态的依赖程度成比例；2）在多交错音频-视觉项设置中，路由转向不同的并行流；3）音频-视觉和其他类型token在信息传输到LLM后可被丢弃，对模型预测影响极小甚至略有改进，可在多个任务和数据集中泛化，实现更高效推理。

Conclusion: 这是首次提供AVLLMs内部声音和视觉协调机制的清晰图景，为音频-视觉及更广泛MLLMs的可解释性、设计和效率进步奠定了基础，结果跨多个模型和规模保持一致，并提出了关于这些流结构出现原因的假设。
...
TL;DR: 研究大型语言模型在企业工作流中处理冗长工具响应的挑战，通过四种上下文管理策略（无用户模型、完整历史、最近5次工具调用、最近5次加摘要）在费用项目化任务上评估性能，发现最近工具调用加摘要策略最佳


<details>
  <summary>Details</summary>
Motivation: 企业工作流中的自主代理面临冗长工具响应导致上下文溢出、状态陈旧错误和高推理成本的问题，特别是在Microsoft Dynamics 365财务与运营系统的自动费用项目化任务中

Method: 使用模型上下文协议工具，在50个酒店费用基准任务上评估四种GPT-5配置：无用户模型基线、完整对话历史、仅保留最近5次工具调用/响应对、最近5次加自动摘要，每个配置运行5次取平均值

Result: 无用户模型基线仅8.0%完成率；完整历史达到71.0%但消耗大量资源；最近5次工具调用提高到79.0%并大幅减少资源；加摘要后最优：91.6%完成率、99.64%平均项目化金额，效率接近修剪策略

Conclusion: 对于这类企业工具使用工作流，选择性保留最近工具交互加上紧凑摘要相比完整历史保留，既能提高可靠性又能提升效率，这一发现在Claude Sonnet 4.5等其他模型上也得到验证
...
TL;DR: 论文提出了一种名为MGP（极简主义遗传编程）的新算法，通过将遗传编程重新定义为语法推导任务，借鉴人类语言的极简主义思想来改进传统遗传编程。


<details>
  <summary>Details</summary>
Motivation: 传统遗传编程将学习任务作为程序归纳问题，使用进化搜索方法。该方法存在膨胀问题，在符号回归等任务上表现不佳。论文试图通过借鉴人类语言习得的极简主义理论来改进这一方法。

Method: 提出MGP算法，借鉴Chomsky的极简主义语言理论，使用二元集合形成算子MERGE作为核心计算过程。算法能够发现符号表达式的核心构建块，并通过MERGE增量式地组合它们，而非使用传统的进化算法。

Result: 在传统遗传编程难以解决的符号回归任务上，当选择适当的原子语法对象词典时，MGP能够一致地产生准确的地面真值模型，表现优于标准遗传编程。

Conclusion: 极简主义理论对程序归纳问题具有相关性，MGP展现的潜力表明这种方法值得进一步探索。该方法为解决遗传编程中的膨胀问题提供了新的视角。
...
TL;DR: 提出基于事件溯源代理运行时的自主改进循环系统，通过审计可追溯的改进流程，针对长记忆评估任务中的"协调失败"问题自动发现并验证提示修复方案。


<details>
  <summary>Details</summary>
Motivation: 现有自主改进循环难以信任，因为改进过程通常是在代理外部添加的脚手架：失败未记录、诊断无法重放、升级或丢弃决策存储在侧数据库中而非代理自身历史中。

Method: 采用事件溯源的代理运行时（ActiveGraph），将状态作为追加事件日志的确定性投影，实现失败记录、精确重放、类型化管道接缝补丁、可审计门控。开发Regimes循环系统，通过诊断失败评估、在管道点提出修复、经过静态检查、沙箱执行、样本内评估和保留验证后才升级。

Result: 在LongMemEval-S任务上，主要失败原因是协调而非检索：证据已在组装上下文中但读者回答错误。在五个保留分割中，Regimes发现的读者提示修复将最终保留准确率提高了+0.05到+0.10（四个分割）和+0.01（一个过度升级分割），两个分割具有个体显著性。

Conclusion: 核心贡献包括ActiveGraph作为使可控改进循环易于处理的审计基质，支持的保留门控循环，将失败按机制分类到管道位置的失败机制分类法，以及提示作为发现探针的假设。
...
TL;DR: 大语言模型在解决高中数学问题上表现优异，但在评估真实学生多样化推理过程方面存在显著能力不足，揭示了"评估鸿沟"现象


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在解决高中数学问题上已经达到接近完美的性能，但它们在评估真实人类学生多样化推理过程方面的能力尚未得到充分研究。当前基于合成数据的评估管道可能无法充分捕捉真实学生数学推理的多样性。

Method: 研究者引入了RealMath-Eval基准，包含224个来自真实高中考试回答的严格标注数据。对比评估了最先进的大语言模型在评估真实学生回答和合成LLM生成解决方案时的表现。通过语义嵌入分析检查错误结构，使用生成概率探针分析信息论惊奇度。

Result: LLM评估器在真实学生回答上表现显著较差（MSE约2.96），而在合成文本上表现更好（MSE约1.17）。语义分析显示合成错误呈现"结构塌陷"特征，而人类错误形成更多样化的错误空间。人类推理过程的信息论惊奇度显著更高。表层风格迁移无法弥补这一差距。

Conclusion: 当前严重依赖合成数据的大语言模型评估管道可能无法充分捕捉真实学生数学推理的多样性，揭示了LLM在评估人类推理过程中的局限性，这一"评估鸿沟"需要新的方法来弥补。
...
TL;DR: 该研究发现，在临床预测任务中使用合成理由数据进行监督微调反而会损害模型性能，这与普遍假设相反。


<details>
  <summary>Details</summary>
Motivation: 测试监督微调中使用合成理由数据是否真能通过教模型"为什么"而不仅仅是"预测什么"来改善临床预测任务性能的普遍假设。

Method: 在504种配置的大规模控制实验中，使用五年阿尔茨海默病及相关痴呆症预测任务，比较基于理由的监督微调与仅标签微调的性能差异。

Result: 基于理由的监督微调持续且显著地损害了预测性能，这种退化在不同模型家族和数据规模中持续存在，且无法通过使用推理导向的基础模型解决。人类专家确认生成的理由在医学上是准确且基于患者特定证据的。

Conclusion: 失败的根本原因是叙述合理性与判别优化之间的结构性冲突。该研究为更精确理解基于理由的监督何时以及如何有效提供了基础，指导高风险临床预测中语言模型的负责任开发。
...
TL;DR: 引入基于LLM的露天矿调度框架，通过定制模拟器指导LLM决策，无需微调即可生成完整调度方案，达到MILP 94-99%的优化效果。


<details>
  <summary>Details</summary>
Motivation: 传统MILP调度方法存在计算复杂度指数级增长、无法实时适应动态环境的问题，限制了在实际工业部署中的应用。

Method: 提出模拟器驱动的LLM调度框架：LLM作为自主决策代理，每个步骤都由定制模拟器指导，将地质优先级、开采-处理耦合和动态容量约束编码到行动生成机制中，采用零样本学习在封闭环境中运行。

Result: 在不同规模和时期的采矿实例中评估，LLM框架恢复了MILP最优NPV的94%-99%，同时计算时间呈线性扩展。提出了新的包含实际约束的MILP作为基准。

Conclusion: 模拟器约束的LLM代理可以作为复杂操作约束下长期工业调度的实际可扩展替代方案，优于经典优化方法。
...
TL;DR: 该论文提出冲突感知解码范式，通过自适应机制动态分配参数先验和外部上下文之间的权威，解决两者冲突时的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在使用检索或增强上下文生成时，外部上下文与参数先验之间的冲突是可靠性的主要瓶颈。现有对比解码方法采用"上下文感知"范式，单向放大上下文而覆盖参数先验，当上下文错误时会覆盖正确的先验知识。

Method: 1. 提出冲突感知范式，基于冲突信号动态分配先验和上下文之间的权威，而非预设上下文可信度。2. 提出TriState-Bench评估协议，校准每个模型的先验知识以测量三种冲突状态：纠正、抵抗和一致。3. 提出自适应机制路由（ARR），在每一步动态路由不同机制，解决机制不对称问题。

Result: 提出的自适应机制路由（ARR）方法将抵抗状态的EM得分从低于6提升到16-33，同时不牺牲纠正或一致状态的性能。代码已开源。

Conclusion: 冲突感知解码范式通过动态权威分配机制，显著改善了大型语言模型在参数先验与外部上下文冲突时的可靠性问题，超越了现有的单向对比解码方法。
...
TL;DR: 语言代理的"记忆宫殿"系统将记忆锚定在世界坐标中，研究证实几何信息在空间查询场景中必须主导记忆召回，且需要区分记忆召回和可见性检测。


<details>
  <summary>Details</summary>
Motivation: 当前语言代理的"记忆宫殿"系统基于直觉将记忆锚定在世界坐标中，认为几何信息能提供文本无法提供的内容，但这种直觉缺乏实证验证。作者旨在测试几何信息在记忆系统中的作用，并明确区分记忆召回和可见性检测这两个概念。

Method: 采用预注册实验设计，包括：1) 记忆召回实验，测试不同记忆加权策略；2) 可见性检测系统，使用射线体素数字微分分析器(DDA)算法；3) 实时验证实验，在预注册框架下测试系统的正确性。研究通过比较几何主导加权、空间邻近性融合、以及基线方法的效果来评估系统性能。

Result: 实验结果显示：1) 默认的空间邻近性融合方法无效（Δ-Hit@5 -0.0375，p=0.306），而几何主导加权显著提升召回率（+0.3208，p<10^-15）；2) 可见性检测系统中，文本和视野锥体方法在849个墙后目标上得分为0.000，而锥体加DDA方法达到0.982；3) 实时验证实验成功识别并修复了实际的锚点缺陷，错误可见率从1.000降至0.000（p=2.5×10^-29）。

Conclusion: 几何信息在空间查询场景中必须主导记忆召回，且记忆系统需要明确区分存储内容和读取方式。研究通过测量和分离这两个方面，证实了遮挡检测需要几何信息的近乎同义反复的直觉，并为未来的完整人工多世界研究奠定了基础。
...
TL;DR: 本文通过系统性的机理分析，揭示了六种偏好优化方法如何重塑语言模型的内部计算结构，发现不同目标函数会引起质量上不同的表征转变，表明行为对齐并不等同于一致的内部重构。


<details>
  <summary>Details</summary>
Motivation: 目前的后训练对齐算法主要作为黑箱进行评估，无法揭示它们如何重塑语言模型的内部计算。作者希望通过机理分析来理解不同偏好优化方法如何影响模型的表征结构，从而为安全性和可解释性提供见解。

Method: 研究者对六种偏好优化方法（PPO、DPO、SimPO、ORPO、GRPO、KTO）在三个开源模型家族中进行系统性机理分析。使用层级线性探测、稀疏自编码器和交叉编码器等技术来定位偏好表征，并量化对齐引起的潜在空间几何变换。

Result: 研究发现：1）偏好信号持续集中在早期-中期或中期-后期层；2）不同目标函数引起质量上不同的表征转变：KTO和GRPO通过建设性特征共享和稀疏高显著性特征招募增强线性可分性；DPO和ORPO通过非建设性的几何旋转和特征衰减降低可分性；PPO和SimPO基本保留基线几何结构；3）这些变换表现出架构依赖的变异性。

Conclusion: 对齐是一种异质性的干预，行为对齐并不隐含一致的内部重构。研究结果为安全性和可解释性标准化特征级审计提供了动机，并强调了需要机制感知的优化目标。
...
TL;DR: 提出了Synergistic Information Bottleneck (SynIB)目标函数，直接优化多模态学习中的协同信息，通过信息论形式化多模态协同，在多个真实世界基准测试中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态学习方法主要关注架构层面的融合模型，但训练目标往往强调单模态或冗余信息，难以有效捕获跨模态协同信息（即仅从多模态联合使用中才能获得的任务相关信息）。

Method: 提出Synergistic Information Bottleneck (SynIB)训练目标，基于信息论形式化多模态协同概念。在标准任务损失基础上，模型在训练时执行前向传递时一次掩蔽一个模态，并惩罚模型在模态被掩蔽时仍保持自信预测（这表明模型依赖单模态线索而非跨模态交互）。

Result: 1) 在构造的XOR任务中（已知真实协同信息），标准训练方法失败而SynIB成功恢复协同信息。2) 在五个真实世界基准测试中：三个MultiBench情感任务、使用CLIP-ViT和DeBERTa骨干网络的Hateful Memes、以及新引入的CREMA-D可控讽刺扩展任务，SynIB在依赖协同信息的示例上准确率提升达7.8%，整体准确率提升达3.8%。

Conclusion: 通过直接优化训练目标来捕获多模态协同信息是一种有效的方法，SynIB为多模态学习提供了一个可扩展的补充策略，能够显著提升模型对跨模态推理的依赖，而不必仅依赖于更复杂的大型融合模型架构。
...
TL;DR: 提出了一个基于条件归一化流的多保真度不确定性感知框架，用于解决缩减阶模型（ROM）中的闭合问题，通过学习从低保真度ROM系数到高保真度系数的概率映射来提高预测精度并提供不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 缩减阶模型（ROMs）虽然能高效替代复杂多尺度系统，但其预测精度常因截断误差以及已解析尺度与未解析尺度间相互作用表达不足而受限。这个被称为闭合问题的未解析尺度对已解析尺度的影响缺失问题需要解决。

Method: 将ROM闭合建模表述为多保真度（MF）学习问题，提出基于条件归一化流的不确定性感知MF框架，学习从低保真度（LF）ROM系数到高保真度（HF）系数的概率映射。研究了两种修正策略：直接从LF输入预测HF系数的直接学习，以及学习LF和HF系数间差异并用其恢复修正HF解的残差学习。

Result: 在二维Navier-Stokes方程控制的涡旋合并问题上进行了验证。结果表明，两种修正策略都比未修正ROM提高了精度，其中残差学习始终表现优于直接学习。此外，两种基于深度生成模型的策略为修正后的ROM系数提供了不确定性量化。

Conclusion: 该框架成功解决了ROM闭合问题，通过不确定性感知的多保真度学习不仅提高了预测精度，还提供了关键的不确定性量化，这对于评估预测置信度和支持ROM在实际应用中的可靠使用至关重要。
...
TL;DR: MGAP是一种无需训练的几何感知解码方法，通过构建语言先验子空间并使用一致性感知门自适应衰减投影先验分量，有效减少MLLM的幻觉同时保持语义流形结构。


<details>
  <summary>Details</summary>
Motivation: 现有的训练自由解码方法通过惩罚语言先验来减少幻觉，但忽略了语言先验的双重性（有时有帮助有时有害）。盲目抑制语言先验会破坏模型的语义流形，导致性能下降，这种现象被称为"流形偏离"。需要一种既能减少幻觉又能保持表示结构的方法。

Method: 提出流形引导自适应投影(MGAP)：1) 通过SVD从盲隐藏状态构建语言先验子空间；2) 在解码时将多模态隐藏状态投影到该子空间；3) 使用一致性感知门自适应地衰减投影的先验分量；4) 产生子空间选择性更新，大部分保留正交语义分量。

Result: 在POPE和CHAIR基准测试上的大量实验表明，MGAP优于先前的解码基线方法，实现了更强的幻觉抑制效果，同时不牺牲连贯性。

Conclusion: MGAP通过几何感知的自适应解码策略，在减少多模态大语言模型幻觉的同时保持了语义表示的结构完整性，实现了对语言先验的更精细控制。
...
TL;DR: 本文提出了Method框架，结合梯度提升决策树和共形预测，为NAFLD风险预测提供校准的、分布自由的覆盖保证，实现了高准确度的风险分层


<details>
  <summary>Details</summary>
Motivation: 非酒精性脂肪肝病影响全球约25%成年人，具有显著的肝脏和心血管风险，但目前缺乏有效的群体筛查工具

Method: 开发了Method机器学习框架，结合梯度提升决策树和共形预测；使用基于互信息的稳定性选择程序，通过自助重采样识别临床可解释的紧凑特征子集；构建满足用户指定置信水平的预测集

Result: 在中山大学多中心队列中（主要队列n=2,187，外部验证n=412），Method实现内部AUROC 0.912和外部AUROC 0.891，优于深度神经网络、TabNet、支持向量机和逻辑回归；共形预测集在90%名义水平下达到91.3%的经验覆盖率；基于风险评分的三级分层显示高风险组12个月进展率是低风险组的4.7倍

Conclusion: Method框架为NAFLD风险评估提供了准确、可解释的机器学习方法，识别出与代谢风险因素一致的临床特征，具有良好的生物学合理性，有望改善人群筛查策略
...
TL;DR: UniTok-FM是一个通过Next-Token Prediction预训练的通用时间序列基础模型，使用UniTok将连续时间序列转换为离散令牌，支持零样本预测、提示增强预测以及免训练的情境推断。


<details>
  <summary>Details</summary>
Motivation: 虽然Next-Token Prediction（NTP）已统一了LLM预训练，但其在无界连续时间序列上的应用仍然开放。现有方法难以将NTP有效适配到时间序列分析中。

Method: 1. UniTok：向量量化自编码器，包含前缀归一化用于尺度稳定、渐进分辨率因果架构用于编码解码、结构保持重构损失用于训练
2. UniTok-FM：采用现成的LLM架构，不对时间序列做特定修改；在具有相似模式的多个序列形成的上下文窗口上进行NTP预训练，以捕获共享动态

Result: 实验显示，统一的UniTok-FM在预测、生成和分类任务上持续优于统计和监督基线，与任务特定基础模型性能相当，并且独特地实现了跨任务的免训练情境推断能力。

Conclusion: UniTok-FM成功将NTP范式扩展到时间序列领域，建立了一个通用的时间序列基础模型，支持多种任务而无需任务特定修改，并展示了免训练情境推断的新能力。
...
TL;DR: 提出了一种新的注意力机制Blurry Window Attention（BLA），通过Dirichlet核插值重建模糊的KV历史，在保持线性复杂度的同时提高了信息检索能力，在合成任务上表现优于传统滑动窗口注意力。


<details>
  <summary>Details</summary>
Motivation: Transformer中的Softmax注意力在长上下文场景中存在二次复杂度和KV缓存增长的瓶颈问题。现有的线性复杂度替代方案（如SSMs、线性注意力、ABC）虽然在语言困惑度上接近Transformer，但在需要特定信息检索或回忆的任务中表现仍不足。

Method: 提出了Blurry Window Attention（BLA），这是一种受状态空间模型启发的有界内存控制注意力方法。BLA存储频率窗口，然后通过Dirichlet核插值重建模糊的KV历史。它可以看作是滑动窗口注意力（SWA）的泛化，也可以看作是门控槽注意力的特例。

Result: 在多查询关联回忆（MQAR）合成任务中，BLA的状态效率比SWA提高了8倍，并与流行的线性注意力模型竞争。在RegBench合成任务中，只有BLA和SWA在状态大小增长时性能有所提升。

Conclusion: BLA作为一种新的线性注意力机制，通过Dirichlet核插值重建历史信息，在保持线性复杂度的同时，显著提升了模型在信息检索任务上的性能，为解决长上下文处理中的内存瓶颈问题提供了有效方案。
...
TL;DR: LLM智能体存在"虚假成功"失败模式，即声称任务完成但环境状态显示未完成。研究表明该现象普遍存在，LLM判断器不可靠，而轻量级TF-IDF检测器表现优异，建议生产监控应采用此类检测器而非依赖LLM判断。


<details>
  <summary>Details</summary>
Motivation: 研究LLM智能体中的"虚假成功"失败模式，即智能体在环境状态显示任务未完成时却断言任务已成功完成。动机在于理解这一常见失败模式的普遍性和变化规律，并评估现有LLM判断器检测此问题的能力。

Method: 在两个智能体基准上进行实证研究：tau2-bench（9,876条轨迹，8个模型家族）和AppWorld（1,879条轨迹，4个模型家族）。评估了5种判断器配置、5种提示策略和完整任务规范下的LLM判断能力，同时开发了基于TF-IDF的轻量级检测器。

Result: 虚假成功普遍但随环境变化：在tau2-bench单控制域中占失败的45-48%，电信双控制域中占3%，AppWorld自评估编码智能体轨迹中占75.8%。LLM判断器不可靠：在tau2-bench上最佳AUROC仅0.65，AppWorld上仅0.54。而轻量级TF-IDF检测器在任务无关情况下分别达到AUROC 0.83和0.95，比最佳判断器多检测4-8倍虚假成功，且延迟低3300倍。

Conclusion: LLM智能体的虚假成功是一个普遍且多变的失败模式，现有LLM判断器检测此问题能力有限且不可靠。相比之下，轻量级TF-IDF检测器表现优异，建议生产监控系统应使用此类经过领域校准的检测器作为分流信号，而不是依赖LLM判断器作为主要监控手段。
...
TL;DR: KV缓存量化会无声破坏LLM安全对齐，研究发现安全特征对量化噪声高度敏感，提出PCR诊断方法能预测并恢复高达97%的安全损失。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存量化评估仅关注困惑度和准确性，但忽视了量化对安全对齐的影响，需要系统评估量化如何破坏LLM的安全性。

Method: 对11个指令调优模型（3.8B-72B）和5个基准测试（1894个提示）进行安全对齐评估，发现安全特征位于对量化噪声敏感的低维子空间；提出PCR（逐通道还原）诊断方法，识别三种机制性失效模式，使用20个校准提示预测缓解方向。

Result: 低比特量化会无声破坏安全对齐：Mistral-7B在仅1.03倍困惑度时损失15.2%拒绝率；PCR能准确预测所有9个主要模型和1个保留模型的缓解方向，KIVI量化器上实现高达97.2%的安全恢复；训练免费协议只需约35GPU分钟，在生产环境中验证有效。

Conclusion: KV缓存量化会严重破坏LLM安全对齐，但通过PCR诊断可以预测失效模式并有效恢复安全性能，为量化部署提供了重要的安全评估工具。
...
TL;DR: 本文提出使用大型语言模型（LLM）来判别表格数据的真实性，作为一种隐私审计方法，通过让LLM分类表格样本为"真实"或"合成"，评估不同合成数据模型的隐私保护效果。


<details>
  <summary>Details</summary>
Motivation: 隐私保护和数据共享之间存在矛盾，组织通常使用合成数据来降低隐私风险。但表格数据的隐私审计仍然困难，因为即使是人类也难以区分真实表格和合成表格。需要一种有效的隐私审计方法来评估合成数据的隐私保护效果。

Method: 提出基于LLM判别的方法，让LLM对每个表格样本分类为"真实"或"合成"。测试两种设置：C1（仅表格）和C2（表格+分布元数据）。使用LLaMA作为开源模型，Gemini作为参考模型。实验中使用三种合成模型（CTGAN、TVAE、高斯Copula）处理两个公开数据集（UCI Adult和ACS Census），收集451个有效试验。同时与C2ST分类器双样本测试、记录链接等基准方法以及包含2名标注者240个试验的人工试点进行比较。

Result: 结果显示不同合成模型之间存在明显差异：在Adult数据集上，LLaMA的DRS达到0%，而Gemini对CTGAN和TVAE的DRS达到100%；在Census数据集上，LLaMA对多数样本预测为"合成"，而Gemini在C1设置下保持较高准确率，但在C2设置下对CTGAN和TVAE的准确率下降。与基准方法相比，LLM判别在特定条件下能够提供实用的隐私审计信号。

Conclusion: LLM判别是一种实用的隐私审计信号方法，但需要注意模型选择、提供方报告和数据编码等处理。当这些因素得到妥善处理时，该方法能够有效评估合成数据的隐私保护效果。
...
TL;DR: DualSelect是一个用于安全调整大型语言模型的耦合框架，通过联合选择相关安全参考和兼容任务样本，在保持任务效用的同时维护模型安全性


<details>
  <summary>Details</summary>
Motivation: 微调安全对齐的大语言模型在下游数据上可能侵蚀已学习的安全行为。现有方法使用固定的安全示例、全局约束或单边任务过滤，诊断显示任务更新会暴露不同的安全约束，需要联合选择相关参考和兼容任务样本

Method: 提出DualSelect耦合框架，包括：1）基于任务更新刷新安全参考；2）过滤与参考方向兼容的任务样本；3）采用极小极大视角，通过熵正则化评分代理选择具有高保持损失和任务冲突的安全参考；4）结合懒惰参考刷新和梯度校正

Result: 在1B-8B参数的大语言模型上，DualSelect在不损失任务效用的前提下保持安全性；使用REDORCA评估器，相比最强基线至少提高5.10个点的安全平均分，在所有评估器中保持最高安全平均分且开销适中

Conclusion: DualSelect有效解决了安全微调中的安全性侵蚀问题，该方法还可扩展到专注于保留的持续学习场景
...
TL;DR: SPACE：首个针对多模态大语言模型的源无关遗忘框架，通过文本引导代理锚点选择和双约束语义隔离，在不访问目标视觉数据的情况下实现概念擦除


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型面临日益增长的隐私风险和监管约束，机器遗忘成为保护敏感数据的重要解决方案。然而，现有遗忘方法通常需要目标概念的视觉数据，这在数据保留政策下往往不可得，因此需要源无关遗忘方法

Method: SPACE框架包含两个阶段：1) 文本引导代理锚点选择：从共享特征空间中检索语义对齐的代理锚点；2) 双约束语义隔离：优化这些锚点间接擦除目标概念，同时将更新限制在保留知识的零空间中确保结构完整性

Result: 理论证明SPACE严格约束保留知识的扰动并最大化特征谱熵；在六个数据集上的实验表明，SPACE性能与最先进的数据依赖方法相当，验证了其在源无关遗忘场景中的有效性

Conclusion: SPACE是首个专门为多模态大语言模型设计的源无关遗忘框架，通过创新的代理锚点机制和语义隔离策略，在不访问目标视觉数据的情况下实现了有效的概念擦除，为解决隐私保护和监管合规问题提供了实用解决方案
...
TL;DR: QSplitFL：一种基于深度Q网络的智能分割点选择框架，用于解决联邦分割学习中设备异构性挑战，通过轻量级硬件指标表示优化模型分割


<details>
  <summary>Details</summary>
Motivation: 联邦学习与分割学习结合虽能保护隐私并降低训练成本，但在设备硬件异构环境下，固定分割点会导致性能问题：弱设备过载、通信压力增加、收敛缓慢且不稳定

Method: 提出QSplitFL框架：1）使用轻量级状态表示（CPU利用率、内存、电量、网络延迟等客户端硬件指标）；2）采用衰减损失下降奖励函数促进早期收敛；3）基于多数投票的委员会DQN架构防止奖励黑客攻击

Result: 在MNIST、Fashion-MNIST、CIFAR-10、CIFAR-100数据集上，使用CNN、ResNet50、MobileNetV4、ConvNeXt等架构进行实验，相比现有方法实现更好的收敛性和更高准确率，同时能有效适应异构设备资源

Conclusion: QSplitFL通过能力感知的DQN框架解决了联邦分割学习中分割点选择的挑战，相比基于模型权重的高维表示方法更轻量高效，在异构环境中展现优异性能，代码已开源
...
TL;DR: PatchSTG：一种基于分区的时空图Transformer，用于不规则传感器网络的高效交通预测，通过层次化空间分区和双重注意力机制，在保持性能的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 交通预测面临现实挑战：传感器分布不规则导致空间结构不均匀，现有基于图和注意力的模型在有效性和可扩展性上受限。传感器网络的不规则性使得传统方法难以有效建模大规模时空依赖关系。

Method: 提出PatchSTG模型，基于地理信息将传感器划分为平衡的、保持局部性的分区，构建层次化空间表示。采用双重注意力编码器：分区内注意力捕获局部交互，分区间注意力建模全局依赖，将计算复杂度从二次降低到接近线性。

Result: 在罗德岛真实交通数据和其他大规模数据集上的实验表明，PatchSTG在多个预测时段均实现稳定且具有竞争力的预测性能，同时显著提高了计算效率。消融研究验证了空间分区和双重注意力在捕捉局部和长距离交通动态方面的有效性。

Conclusion: 基于分区的时空建模为不规则空间设置下的交通预测提供了一个可扩展且有效的框架，平衡了预测精度和计算效率，解决了实际交通网络中传感器分布不均带来的挑战。
...
TL;DR: Rotate2Think：一种无训练方法，通过估计输入嵌入与思考嵌入之间的旋转矩阵，在推理时注入合成思考向量来提升推理模型的性能


<details>
  <summary>Details</summary>
Motivation: 当前推理模型通过生成显式中间推理轨迹取得了优异性能，但其表示空间的内部结构仍不清晰。特别是模型在"思考"过程中的隐藏表示与输入提示的嵌入有何差异，以及能否利用这种结构在推理时激发更强的推理能力。

Method: 研究发现输入嵌入和思考嵌入都表现出极高的锥度，围绕单一均值方向紧密聚集，但这两个均值方向是非共线的。因此将输入到思考的转换建模为旋转问题，通过正交Procrustes分析获得闭式解。提出Rotate2Think方法，从少量正确求解的示例中估计旋转矩阵，在推理时在思考分隔符之间注入合成的思考向量作为几何引导。

Result: 在多个基准测试和模型家族中评估，Rotate2Think在32个模型-基准配置中的30个上提升了准确率，涵盖了数学、科学和代码任务。此外，该方法能够零样本泛化到MATH-Vision的多模态推理任务。

Conclusion: 思考嵌入在几何上占据与输入嵌入不同的表示空间区域，这种几何结构可以通过Rotate2Think方法有效利用，以无训练的方式提升推理模型的性能，并且具有良好的泛化能力。
...
TL;DR: 本文系统地重新评估了基于重建的时间序列异常检测方法，重点关注推理步长对性能的影响，发现重叠窗口能显著提升检测效果，并提出了统一的评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有基于重建的时间序列异常检测方法在报告结果时存在难以比较的问题，主要是由于评估实践不统一和推理过程不明确。研究者希望通过重新审视推理步长（处理子序列时使用不重叠窗口还是重叠窗口）的影响来改进这一问题。

Method: 提出了统一的训练、调优和多种子评估协议，在TSB-AD基准数据集上系统地研究重叠推理对异常检测性能的影响。使用了多种重建模型包括PCA基线、DLinear、AutoEncoder、TimesNet和Transformer变体。还在完整的UCR存档上使用与滑动窗口重建对齐的定位标准进行了补充评估。

Result: 实验结果表明，在所有模型中，重叠窗口都能带来一致的性能提升，平均相对增益高达+28%，并且可能改变方法排名。同时分析了不同数据集、随机种子和超参数配置下的变异性。基于重建的基线方法在TSB-AD和UCR基准上都取得了强劲表现。

Conclusion: 基于重建的时间序列异常检测性能不仅取决于模型架构和训练，还取决于推理选择，这要求建立清晰且可复现的协议。基于重建的基线方法在单变量时间序列异常检测中是具有竞争力的实用方法。
...
TL;DR: 该论文提出了GLU（全局-局部不确定性）方法，通过结合隐藏状态的几何熵（全局不确定性）和token级熵（局部不确定性）来改进大语言模型的不确定性量化，能更好地检测自信但错误的失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化方法主要依赖token级信号，未能充分利用中间隐藏状态的几何结构，导致无法检测自信但错误的失败模式。

Method: 提出GLU方法：将隐藏状态矩阵的几何复杂度作为LLM的全局不确定性度量，token级不确定性作为局部度量；通过乘法门融合这两个统计上近乎正交的信号，形成无监督、单次前向传播的评分。

Result: 在三个模型族和六个基准测试上，GLU匹配或优于所有无监督基线方法，仅需单次前向传播，且具有长度归一化和架构无关的特性。

Conclusion: 隐藏状态的几何熵和token级熵捕获了LLM不同的失败机制，将它们结合起来可以显著提高不确定性量化的可靠性，特别是能检测到自信但错误的回答。
