...
TL;DR: 今天我们提出ARC-AGI-3或许的标准化环境" 为传让之为具" 轮嵌环境及其以人类行动线为基准的效率评分框架，用于评估机器现有人种智能的能力


<details>
  <summary>Details</summary>
Motivation: 为了建立一个没有语言和外部知识的纯游戏环境，来评估机器现有人种智能在新特定任务上的流小能力的表现

Method: 使用模式算法来构建、验证和宣券环境，并采用以人类行动线为基准的效率评分框架

Result: 人类從叶雷管老实现完成了认为编验现有人种智能的机器，人动一日四月就算说，指指单局产生1%的并约，本未能解决多少个环境

Conclusion: 该研究通过ARC-AGI-3标准化环境和效率基准形式的评分框架，最终提出了一种用于评估机器现有人种智能表现的实用方法，并将为人工智能基本能力的经济学研究提供任务
...
TL;DR: 通过最小模型QSG揭示了LLM多智能体系统共识形成的微观机制——互上下文学习导致记忆漂移，解释了无先验偏好下系统快速达成共识的现象，并推导了关键参数的标度定律。


<details>
  <summary>Details</summary>
Motivation: 研究旨在揭示大型语言模型（LLM）驱动的多智能体系统中集体决策形成的微观机制，特别是当个体智能体无明显先验偏好时，系统如何快速打破对称性并达成共识。理解这种共识形成过程对于确保多智能体系统决策的公平性和可靠性至关重要。

Method: 论文引入了量化的单纯形传播（QSG）最小模型，将智能体信念建模为内部概率分布，并通过抽样输出相互学习。通过理论分析推导了漂移主导和选择主导两种机制的标度定律，并在QSG模拟和基于LLM的命名游戏实验中进行了验证。

Result: 研究发现共识形成的微观机制是互上下文学习中出现的记忆漂移（memetic drift），即一个智能体的任意选择会成为其他智能体的证据并累积导向共识。理论推导了漂移诱导极化与种群大小、通信带宽、上下文适应率和智能体内部不确定性之间的标度关系，并在实验中验证了这些关系。

Conclusion: 该论文通过QSG模型揭示了多智能体系统中集体共识形成的微观机制，强调了互上下文学习在对称性打破中的关键作用。研究提出了一个分析框架，可用于理解和设计更可靠的多智能体系统，特别是在社会表征形成过程中控制偏差和增强公平性方面具有重要意义。
...
TL;DR: 演化博弈模型表明，只有在对不安全AI的惩罚足够高且用户能以低成本偶尔监控的条件下，AI系统才会向安全且被广泛采用的理想状态演化；单一监管或盲目信任均不可靠。


<details>
  <summary>Details</summary>
Motivation: 现有AI治理的演化模型主要集中在安全开发和有效监管的激励上，通常将用户信任视为一次性采用决策，而非由重复交互塑造的动态演化过程。然而，实际上用户与AI开发者之间的信任常表现为在可能付出监控成本的情况下减少监督。研究这种动态交互如何影响安全和不安全AI的演化具有重要意义。

Method: 使用演化博弈论、复制动力学，并结合有限种群的随机演化过程及基于Q-learning的强化学习模拟来研究这一复杂动态。

Result: 模型发现了三种长期稳定的演化模式：1）AI未被采用且开发不安全；2）不安全的AI被广泛采用；3）安全的AI被广泛采用（理想状态）。理想状态的出现需要满足对不安全行为的惩罚超过安全开发的额外成本，以及用户的监控成本足够低以允许偶尔的检查。

Conclusion: 为了维持人工智能安全和广泛采用的理想状态，需要将强有力的监管（包括对不安全行为的有意义制裁）与透明的低监控成本环境相结合。单纯依赖监管或盲目的用户信任都不足以防止系统向不安全或低采用率状态的演化漂移。
...
TL;DR: 针对多模态大模型路由性能下降问题，提出了注意力探针和KL正则化LoRA探针两种方法，通过提升隐藏状态质量显著改善路由效果。


<details>
  <summary>Details</summary>
Motivation: 在仅文本LLMs中，基于隐藏状态预测小模型正确性的探针路由方法有效，但在多模态LLMs（MLLMs）中性能大幅下降。研究发现视觉输入的存在削弱了隐藏状态中正确性信号的可分离性，使得传统探针设计难以提取这些信号。

Method: 提出了两种互补的路由改进方法：1) 注意力探针（Attention Probe），基于注意力分数聚合前一层的隐藏状态来恢复分散的正确性信号；2) KL正则化LoRA探针（ReLope），插入轻量级LoRA适配器并应用KL正则化来学习路由感知的表示。

Result: 综合实验表明，提出的方法在各项基准测试中持续优于基线方法，验证了通过改进隐藏状态质量提升MLLMs路由效果的有效性。

Conclusion: 通过提升隐藏状态质量是MLLMs中有效路由的关键，注意力探针和ReLope方法为多模态大模型路由系统提供了新的解决方案。
...
TL;DR: 本文指出，AI前端的人性化设计是一种价值驱动的选择，显著影响用户的认知模型、信任水平与行为反应；通过Chayn的案例说明，伦理应通过交互选择实现，而不仅嵌入系统逻辑。


<details>
  <summary>Details</summary>
Motivation: 现有AI伦理讨论多集中于后端数据治理、模型训练与算法决策，而对前端交互与表征设计中的伦理意涵关注不足，这在使用自然语言处理的人机对话界面（CUI）中尤为突出。

Method: 借鉴人机交互（HCI）、对话式AI与价值导向设计的研究成果，通过对非营利组织Chayn为性别暴力幸存者开发的两个AI系统的案例分析，探讨界面设计如何引导用户信任、期望与自主性。

Result: 案例分析表明，以用户福祉为核心的伦理考量可引导设计者在前端采取克制策略，反对当前以吸引用户为主导的设计常规。

Conclusion: 作者认为，在前端设计中做出伦理考量是一项「程序化伦理」实践——它通过具体的交互选择来实现，而不仅依赖于系统逻辑的设定。
...
TL;DR: 证明了平台确定性是可信AI的必要充分条件，提出确定性命题和信任熵概念，构建纯整数推理引擎解决浮点运算的非确定性问题，实现跨硬件架构的比特级一致性。


<details>
  <summary>Details</summary>
Motivation: 现有AI系统缺乏平台确定性，导致难以建立信任。IEEE 754浮点运算在不同硬件架构上产生不一致结果，这从根本上违反了确定性要求，使得AI系统的公平性、鲁棒性、隐私性、安全性和对齐性等信任属性无法保证。

Method: 1. 提出确定性命题，引入信任熵量化非确定性成本；2. 证明验证失败概率精确公式；3. 提出确定性-验证崩塌理论；4. 构建纯整数推理引擎，避免浮点运算的非确定性。

Result: 1. 82次跨架构测试（模型高达67亿参数）实现零哈希不匹配；2. 四个地理分布式节点产生相同输出，通过356个链上证明交易验证；3. 基于99000行Rust代码的系统在三大洲部署，证明跨ARM和x86架构的比特级相同输出。

Conclusion: AI信任的核心是平台确定性，IEEE 754浮点运算违反确定性要求。本文通过整数推理引擎实现跨架构比特级一致性输出，证明AI信任本质上是一个算术问题。
...
TL;DR: 提出LogitScope框架，通过token级信息度量分析LLM不确定性，无需标注数据即可揭示模型置信度模式并识别潜在问题。


<details>
  <summary>Details</summary>
Motivation: 传统评估方法无法在生成过程中深入了解模型在单个token位置上的置信度，限制了可靠部署的可能性。

Method: 通过计算每个生成步骤中概率分布的熵和变熵等信息度量标准，对LLM的不确定性进行token级分析，采用延迟评估提高计算效率。

Result: LogitScope能够揭示模型置信度模式、识别潜在幻觉、暴露高不确定性决策点，为不确定性量化、模型行为分析和生产监控提供支持。

Conclusion: LogitScope提供了无需标注数据即可深入分析LLM不确定性的实用框架，其模型无关性和计算高效性使其在研究和生产部署中都有广泛应用前景。
...
TL;DR: 本研究开发了一个识别加密货币推文中预测性声明的两阶段分类框架。第一阶段区分预测/非预测推文，第二阶段将预测性推文细分为上涨、下跌或中性类别，结合GPT标注和数据增强提升模型性能，并在多种加密货币上验证了情感与预测模式的关联。


<details>
  <summary>Details</summary>
Motivation: 随着加密货币的日益普及，社交媒体上相关的投资讨论和投机活动激增。然而，缺乏有效的方法来系统识别和分析这些社交平台上的预测性声明，这限制了对加密货币市场情绪和投资行为的研究。

Method: 研究提出了一个两阶段的加密货币推文预测性声明分类框架。第一阶段(Task 1)进行预测性/非预测性的二分类；第二阶段(Task 2)对预测性推文进一步分类为上涨性、下跌性或中性。研究结合人工标注和GPT标注构建数据集，使用SenticNet提取情感特征，采用GPT生成的改述进行数据增强以解决类别不平衡问题。评估了多种机器学习、深度学习和transformer模型。

Result: 研究表明GPT平衡策略显著提升了模型性能。在第一阶段任务中，transformer模型获得了最高的F1分数；而在第二阶段任务中，传统机器学习模型表现最佳。情感分析揭示了不同加密货币的各预测类别具有独特的情感模式特征。

Conclusion: 该研究通过创新的两阶段分类框架成功识别了加密货币推文的预测性特征，并建立了预测类别与情感模式之间的关联。其中，transformer模型在第一阶段表现最优，而传统机器学习方法在第二阶段更具优势。GPT辅助数据平衡显著提升了模型性能，同时情感特征分析揭示了不同加密货币预测类别的独特情感模式。
...
TL;DR: 实证分析3100万携程用户使用LLM助理“问道”的行为，发现嵌入式购物AI主要作为传统搜索的补充，专门服务探索性产品发现，用户采纳模式与传统AI工具相反。


<details>
  <summary>Details</summary>
Motivation: 了解消费者在电商平台如何采纳和使用嵌入式购物AI，特别是大型语言模型（LLM）助理在购物过程中的角色，以填补现有文献中关于平台嵌入式AI使用行为的大规模实证证据缺乏的空白。

Method: 使用携程平台上3100万用户的数据，研究集成在平台内的LLM助理“问道”。通过分析用户使用数据，从用户特征、使用模式、查询内容等多维度进行实证研究。

Result: 首先，用户采纳呈现与传统通用AI工具不同的模式：高龄消费者、女性用户和高参与度现有用户采纳率最高。
其次，AI聊天与传统搜索出现在购物旅程的同一阶段，并且通常交替使用。
第三，消费者倾向于使用AI助手处理探索性、难以关键词化的任务，例如吸引力查询占到聊天请求的42%。聊天意图随聊天时间、搜索相对时序和后来购买的产品类别而有系统变化。

Conclusion: 结果表明，嵌入式购物AI在电商中不仅是搜索的替代品，更扮演着辅助和补充的传统搜索的角色，专门服务探索性的产品发现需求，并具有独特的用户采纳模式和交互特征。
...
TL;DR: ScratchMath是首个专注于分析和分类学生手写数学草稿错误的基准测试，包含1,720个中国学生样本、两个任务和七种错误类型。评估显示当前多模态大模型在视觉识别和逻辑推理方面与人类专家存在明显差距。


<details>
  <summary>Details</summary>
Motivation: 学生手写草稿评估对于个性化教育反馈至关重要，但存在笔迹多样、布局复杂、解题方法各异等独特挑战。现有教育NLP主要关注文本响应，忽视了真实手写草稿的复杂性和多模态性。当前的MLLMs虽然擅长视觉推理，但通常采用'考生视角'，优先生成正确答案而非诊断学生错误。

Method: 1) 构建包含1,720个中国中小学学生真实数学手写样本的数据集，支持错误原因解释和错误原因分类两个任务，定义了七种错误类型； 2) 通过多人机协作的多阶段专家标注、审查和验证流程进行精细标注； 3) 对16个领先的MLLMs进行系统评估。

Result: 研究发现：1) 所有评估模型与人类专家存在显著性能差距； 2) 专有模型明显优于开源模型； 3) 大型推理模型在错误解释方面显示出强大潜力； 4) 视觉识别和逻辑推理是主要难点。

Conclusion: ScratchMath基准测试凸显了当前MLLMs在分析和评估学生手写数学草稿方面的显著能力差距，特别是在视觉识别和逻辑推理方面。数据集和评估框架的公开将促进教育评估领域多模态AI模型的进步。
...
### [11] [The Anatomy of Uncertainty in LLMs](https://arxiv.org/abs/2603.24967)
*Aditya Taparia,Ransalu Senanayake,Kowshik Thopalli,Vivek Narayanaswamy*

Main category: cs.AI

TL;DR: 本文提出将大语言模型的不确定性分解为三个语义组分：输入模糊性、知识缺口和解码随机性，通过实验验证该框架能更好地理解模型可靠性，检测幻觉，并为针对性改进提供指导。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型的不确定性评估方法存在局限，仅提供单一不确定性分数或依赖经典的偶然性-认知性二分法，无法为改进生成模型提供可操作的见解。现有方法不足以深入理解LLM的不确定性来源，因此需要新的分解框架来更精细地分析不确定性的语义组成部分。

Method: 该论文采用实验分析方法，设计系列实验研究不同模型规模和任务下三类不确定性组分（输入模糊性、知识缺口、解码随机性）的主导性变化，从而验证分解框架的有效性和适用性。

Result: 实验表明，输入模糊性、知识缺口和解码随机性这三种不确定性组分的相对重要性会随着模型规模和任务类型的变化而发生转移，验证了所提分解框架能够更精细地理解LLM的不确定性来源。

Conclusion: 该工作提出的三组分不确定性分解框架为理解和审计LLM的可靠性提供了新视角，通过识别主导不确定性来源（输入模糊性、知识缺口、解码随机性），能够针对性地提升模型性能，为构建更可信赖的生成系统铺平道路。
...
### [12] [Rethinking Failure Attribution in Multi-Agent Systems: A Multi-Perspective Benchmark and Evaluation](https://arxiv.org/abs/2603.25001)
...
TL;DR: 本文重新审视多智能体系统故障归因问题，提出多视角故障归因范式、MP-Bench基准和配套评估协议，发现现有基准设计低估了LLM的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的MAS故障归因工作大多假设每次故障只有一个确定性的根本原因，忽视了实际中由于复杂的智能体间依赖性和模糊的执行轨迹导致的多种可能归因。

Method: 提出多视角故障归因实践范式，引入MP-Bench基准和新评估协议，并进行广泛的实验评估对比现有方法。

Result: 实验表明，先前关于LLM在故障归因方面表现不佳的结论很大程度上是由于现有基准设计的局限性造成的。多视角基准和评估协议能更准确反映实际应用场景。

Conclusion: 多视角故障归因框架对于实现现实和可靠的MAS系统调试是必要的，现有的单归因假设会扭曲对LLM能力的评估。
...
### [13] [A Public Theory of Distillation Resistance via Constraint-Coupled Reasoning Architectures](https://arxiv.org/abs/2603.25022)
*Peng Wei,Wesley Shu*
...
### [14] [System-Anchored Knee Estimation for Low-Cost Context Window Selection in PDE Forecasting](https://arxiv.org/abs/2603.25025)
...
### [15] [From Stateless to Situated: Building a Psychological World for LLM-Based Emotional Support](https://arxiv.org/abs/2603.25031)
...
### [16] [Mechanistically Interpreting Compression in Vision-Language Models](https://arxiv.org/abs/2603.25035)
...
### [19] [ElephantBroker: A Knowledge-Grounded Cognitive Runtime for Trustworthy AI Agents](https://arxiv.org/abs/2603.25097)
...
### [23] [Trace2Skill: Distill Trajectory-Local Lessons into Transferable Agent Skills](https://arxiv.org/abs/2603.25158)
...
### [25] [Probabilistic Abstract Interpretation on Neural Networks via Grids Approximation](https://arxiv.org/abs/2603.25266)
...
### [26] [Distribution and Clusters Approximations as Abstract Domains in Probabilistic Abstract Interpretation to Neural Network Analysis](https://arxiv.org/abs/2603.25273)
*Zhuofan Zhang,Herbert Wiklicky*
...
### [32] [Agentic Trust Coordination for Federated Learning through Adaptive Thresholding and Autonomous Decision Making in Sustainable and Resilient Industrial Networks](https://arxiv.org/abs/2603.25334)
...
### [33] [4OPS: Structural Difficulty Modeling in Integer Arithmetic Puzzles](https://arxiv.org/abs/2603.25356)
*Yunus E. Zeytuncu*
...
### [34] [Does Structured Intent Representation Generalize? A Cross-Language, Cross-Model Empirical Study of 5W3H Prompting](https://arxiv.org/abs/2603.25379)
*Peng Gang*
...
### [35] [Beyond Content Safety: Real-Time Monitoring for Reasoning Vulnerabilities in Large Language Models](https://arxiv.org/abs/2603.25412)
...
### [36] [Modernising Reinforcement Learning-Based Navigation for Embodied Semantic Scene Graph Generation](https://arxiv.org/abs/2603.25415)
...
### [37] [Retraining as Approximate Bayesian Inference](https://arxiv.org/abs/2603.25480)
*Harrison Katz*
...
### [39] [Voxtral TTS](https://arxiv.org/abs/2603.25551)
...
### [40] [Agent Factories for High Level Synthesis: How Far Can General-Purpose Coding Agents Go in Hardware Optimization?](https://arxiv.org/abs/2603.25719)
...
### [41] [R-C2: Cycle-Consistent Reinforcement Learning Improves Multimodal Reasoning](https://arxiv.org/abs/2603.25720)
...
### [42] [Back to Basics: Revisiting ASR in the Age of Voice Agents](https://arxiv.org/abs/2603.25727)
...
### [43] [Training the Knowledge Base through Evidence Distillation and Write-Back Enrichment](https://arxiv.org/abs/2603.25737)
...
TL;DR: 提出DyMRL动态多空间表示学习方法，通过多空间特征整合和对称双注意力机制解决多模态知识动态获取与融合问题，在事件预测任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 针对事件预测中多模态知识表示的动态获取与融合问题。现有研究主要关注静态场景，忽略：1）如何学习时间敏感的动态结构模态特征；2）如何捕获不同模态对历史事件贡献的演化特征。现有方法难以捕获深层关系感知几何特征和模态权重动态变化。

Method: DyMRL采用动态多空间表示学习方法，包括：1）在欧几里得、双曲和复数空间中整合时间特异性结构特征到关系消息传递框架；2）采用对称的双融合-演化注意力机制动态分配不同模态的学习权重；3）利用预训练模型赋予时间敏感的视觉和语言智能。

Result: DyMRL在构建的四个多模态时序知识图基准测试中，超越了最先进的动态单模态和静态多模态基准方法，证明了其学习到的多模态时序知识在事件预测中的有效性。

Conclusion: DyMRL方法通过创新的动态多模态知识表示学习框架，有效解决了事件预测中的两大挑战，在多模态时序知识图基准测试中展现出优于现有动态单模态和静态多模态方法的性能，证实了动态知识获取与融合在真实世界事件预测中的重要性。
...
### [45] [How unconstrained machine-learning models learn physical symmetries](https://arxiv.org/abs/2603.24638)
*Michelangelo Domina,Joseph William Abbott,Paolo Pegolo,Filippo Bigi,Michele Ceriotti*

Main category: cs.LG

TL;DR: 提出量化无约束ML模型中对称性内容的严格度量标准，分析Transformer架构如何处理对称信息，建立诊断谱分布失败模式框架，并通过策略性注入最少归纳偏置实现更准确的物理模拟。


<details>
  <summary>Details</summary>
Motivation: 针对物理模拟中机器学习模型需满足对应物理量基本对称性的要求，而许多情况下，不遵守旋转对称性的未约束模型却能表现出竞争性能，并能通过简单数据增强策略学习到高度近似的等变行为，因此需要更严格的方法来量化这些模型中的对称性内容。

Method: 引入严格的度量标准来量化学习表示中的对称性内容，并应用于两个基于Transformer的未约束模型，分析架构层间如何处理和习得对称信息，基于此建立诊断ML模型中谱分布失败模式的严格框架。

Result: 通过严格度量标准分析，发现可以在无约束架构中通过策略性注入最少必要的归纳偏置，在保持高表达性和可扩展性的同时，实现更高的稳定性和准确性，有效保证物理保真度。

Conclusion: 通过在无约束架构中策略性注入最少必要归纳偏置，可以在保持高表达性和可扩展性的同时，有效提高模型的物理保真度，从而规避谱分布错误模式，实现更稳定、更准确的物理模拟。
...
### [46] [Experiential Reflective Learning for Self-Improving LLM Agents](https://arxiv.org/abs/2603.24639)
*Marc-Antoine Allard,Arnaud Teinturier,Victor Xing,Gautier Viaud*

Main category: cs.LG

TL;DR: ERL框架通过反思单次尝试经验提取可迁移启发式规则，实现智能体自主环境适应和性能提升。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型能够支持复杂推理和多步骤问题解决的智能体开发，但这些智能体难以适应特定环境，且无法利用过往交互经验，每次新任务都从零开始，缺乏经验积累。

Method: 提出了一种名为ERL的自改进框架，该方法通过反思任务执行轨迹和结果来生成启发式规则，这些规则捕捉了可操作的经验教训。在测试时，基于当前任务检索相关启发式规则并将其注入到智能体的上下文中，以指导执行过程。

Result: 在Gaia2基准测试中，ERL相对于ReAct基线将成功率提高了7.8%，在任务完成可靠性方面取得了显著提升，并且优于先前的经验学习方法。

Conclusion: 通过系统性消融实验验证，选择性检索对性能至关重要，且启发式规则比少样本轨迹提示具有更好的普适性迁移能力。研究证明了通过反思单次尝试经验来提取可迁移启发式规则能够有效实现智能体的自我改进。
...
### [47] [Can LLMs Beat Classical Hyperparameter Optimization Algorithms? A Study on autoresearch](https://arxiv.org/abs/2603.24647)
*Fabio Ferreira,Lucca Wobbe,Arjun Krishnakumar,Frank Hutter,Arber Zela*

Main category: cs.LG

TL;DR: 研究表明经典超参数优化方法在固定搜索空间优于LLM智能体，但允许LLM直接编辑代码的开放搜索空间方法缩小了这一差距。混合经典优化器与小型LLM的Centaur方法取得最佳结果，0.8B模型变体甚至优于27B变体。


<details>
  <summary>Details</summary>
Motivation: 传统超参数优化（HPO）方法通常依赖于有限的搜索空间，而大型语言模型（LLM）智能体具有在开放搜索空间中进行灵活探索的潜力。论文旨在探索LLM智能体在自动机器学习（AutoML）中是否能够超越经典HPO方法，并设计更有效的混合方法。

Method: 研究采用autoresearch作为测试平台，在固定的计算预算下比较经典HPO算法（如CMA-ES、TPE）与基于LLM的方法在小型语言模型超参数调优上的表现。重点方法包括：1）允许LLM在无约束搜索空间中直接编辑训练源代码；2）引入Centaur混合方法，将CMA-ES的内部状态（均值向量、步长、协方差矩阵）与LLM结合。

Result: 在固定搜索空间中，经典HPO方法（CMA-ES、TPE）持续优于基于LLM的智能体。但允许LLM直接编辑训练代码的无约束搜索空间方法显著缩小了与经典方法的差距，尽管仅使用了自托管的27B开源模型。避免内存故障的方法比搜索多样性高的方法表现更好，表明可靠性比探索广度更重要。Centaur混合方法在实验中取得了最佳结果，其中0.8B变体优于27B变体。0.8B模型在无约束代码编辑中不足，但在混合优化中足够，而扩展到27B在固定搜索空间方法中并未带来优势。

Conclusion: 论文表明，经典超参数优化方法在固定搜索空间中优于LLM智能体，但允许LLM直接编辑训练代码的开放搜索空间方法显著缩小了这种差距。然而，最关键的是结合经典优化器状态与LLM知识的混合方法（Centaur）取得了最佳结果，尤其是0.8B小模型变体胜过27B大模型变体，表明强大的经典优化器配以经济的小型LLM即可实现高效优化。
...
TL;DR: 提出一种能耗高效的三层层次化联邦学习框架，用于水下异常检测，通过压缩更新传输和选择性雾节点协同，在保持高检测精度的同时显著降低通信能耗。


<details>
  <summary>Details</summary>
Motivation: 水下物联网中的异常检测面临严峻挑战：声学链路带宽低、能耗高，且传感器通常无法直接与水面网关通信，导致标准平面联邦学习在水下部署中存在昂贵的长距离传输和参与度受限的问题。

Method: 采用了基于物理模型的水下声学模型进行综合评估，包括：可行性感知的传感器-雾节点关联、压缩模型更新传输、雾节点之间选择性协同聚合的三层架构，并在大型合成部署和三个真实基准数据集上进行了实验验证。

Result: 在200个传感器的合成部署中，仅有约48%的传感器能直接到达网关，而层次化学习通过可行的雾节点路径实现了全参与度；选择性协同在保持与始终开启的雾节点间交换相同检测精度的同时，能降低31-33%的能耗；压缩上传在匹配的灵敏度测试中能将总能耗降低71-95%；在三个真实基准上的实验显示，该低开销层次化方法在检测质量上仍具竞争力。

Conclusion: 该研究提出了一种用于水下异常检测的层次化联邦学习框架，该框架通过本地化短距离集群内通信、选择性雾节点合作和压缩更新的方法，能在严重声学通信约束下保持高检测精度并显著降低能耗，为水下部署提供了实用的设计指导。
...
### [49] [Amplified Patch-Level Differential Privacy for Free via Random Cropping](https://arxiv.org/abs/2603.24695)
*Kaan Durmaz,Jan Schuchardt,Sebastian Schmidt,Stephan Günnemann*

Main category: cs.LG

TL;DR: 随机裁剪在视觉任务的差分隐私训练中可以作为第三种随机性来源，概率性排除敏感内容，从而在不增加成本的情况下增强隐私保护，理论与实证均验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 随机裁剪是计算机视觉中最常见的数据增强技术之一，但其在训练差分隐私模型中的内在随机性作用尚未被探索。作者观察到当敏感内容（如人脸或车牌）在图像中空间局部化时，随机裁剪可以概率性地将这些内容排除在模型输入之外，这为差分隐私训练引入了第三种随机性来源。

Method: 提出了视觉数据的补丁级别邻近关系，为差分隐私随机梯度下降（DP-SGD）结合随机裁剪推导了紧致的隐私界限，量化了补丁包含概率及其与小批量采样的复合效应。

Result: 实证验证表明，补丁级别的放大改进了多种分割架构和数据集上的隐私-效用权衡，有效降低了有效采样率，从而在不修改模型架构或训练过程的情况下增强了差分隐私保护。

Conclusion: 通过将隐私计算与数据结构和已有的随机源对齐，可以无需额外成本地获得更强的隐私保证，为视觉任务的差分隐私训练提供了新的优化方向。
...
### [50] [Training LLMs for Multi-Step Tool Orchestration with Constrained Data Synthesis and Graduated Rewards](https://arxiv.org/abs/2603.24709)
...
TL;DR: 通过真实API缓存和分级奖励设计，显著提升了大语言模型在多步骤工具编排任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 多步骤工具编排中，大语言模型需要按正确顺序调用多个依赖API并传播中间输出，现有模型在这方面表现不佳。训练模型处理此类工作流面临两个障碍：现有环境专注于简单的每轮函数调用和模拟数据，以及二进制奖励无法为部分正确性提供信号。

Method: 构建了一个基于大规模真实API响应缓存的强化学习环境，开发了数据合成流程来采样有效的多步骤编排轨迹；提出了分级奖励设计，将正确性分解为原子有效性（单个函数调用正确性）和编排正确性（正确的工具排序和依赖关系处理）。

Result: 在ComplexFuncBench上的实验显示，该方法在轮次准确率方面取得了实质性改进。消融研究证实两个奖励组件都是必需的：单独使用其中任何一个都会显著降低性能。

Conclusion: 该研究提出的框架通过结合真实API响应缓存和分级奖励设计，有效解决了多步骤具身编排的两个主要挑战。实验结果表明，该方法在复杂函数基准测试中显著提高了轮次准确率，且两个奖励组件都是必不可少的。
...
### [51] [Can an Actor-Critic Optimization Framework Improve Analog Design Optimization?](https://arxiv.org/abs/2603.24714)
*Sounak Dutta,Fin Amin,Sushil Panda,Jonathan Rabe,Yuejiang Wen,Paul Franzon*

Main category: cs.LG

TL;DR: 该文提出了一个基于actor-critic的模拟电路尺寸优化框架（ACOF），通过分离提议和评估功能，在保持标准仿真流程兼容性的同时，使搜索过程更加审慎、稳定和可解释，显著提升优化效果。


<details>
  <summary>Details</summary>
Motivation: 模拟电路设计过程中，即使微小的参数变化也需要昂贵的仿真周期，高质量解决方案通常只占据巨大搜索空间中非常狭窄的部分。现有优化器虽然减轻了部分负担，但缺乏设计师在决定下一步搜索方向时所使用的判断能力。

Method: 采用actor-critic优化框架，将提议和评估角色分离：actor负责建议有前景的设计空间区域，critic负责评估这些选择、确保设计合法性并在遇到瓶颈时重新引导搜索。

Result: 在测试电路中，ACOF在top-10性能指标上比最强基线平均提升38.9%，平均减少24.7%的遗憾度。单个电路的峰值提升达到70.5%的性能指标改进和42.2%的遗憾度降低。

Conclusion: 该研究提出的actor-critic优化框架（ACOF）为模拟电路尺寸优化提供了一种更透明、可控的自动化方案。通过将推理与仿真驱动搜索结合，该框架能够有效处理挑战性设计空间，为模拟电路自动化尺寸调整开辟了更有前景的路径。
...
### [52] [Grokking as a Falsifiable Finite-Size Transition](https://arxiv.org/abs/2603.24746)
*Yuda Bi,Chenyu Zhang,Qiheng Wang,Vince D Calhoun*

Main category: cs.LG

TL;DR: 研究发现grokking中的相位转变描述可以作为可量化的有限尺寸主张进行验证，而不仅仅是类比，通过凝聚态物理诊断方法证实了有限尺寸边界和平滑交叉的不可能性。


<details>
  <summary>Details</summary>
Motivation: grokking现象（在早期记忆化后延迟出现泛化）通常用相变语言描述，但这种说法缺乏可证伪的有限尺寸输入。

Method: 将群阶$p$作为可扩展变量，并将预留下的谱头尾对比作为表征层级的序参量。采用凝聚态物理风格的诊断链进行粗网格扫描和密集近临界附加审计，包括Binder类交叉分析。

Result: Binder类交叉揭示了共享的有限尺寸边界，敏感性比较强烈反对平滑交叉解释（近临界审计中ΔAIC=16.8）。

Conclusion: 相位转变语言在grokking现象中可以作为一种可定量的有限尺寸主张进行测试，而不仅仅是作为类比使用，不过目前转变阶数尚未解决。
...
### [53] [Light Cones For Vision: Simple Causal Priors For Visual Hierarchy](https://arxiv.org/abs/2603.24753)
*Manglam Kartik,Neel Tushar Shah*

Main category: cs.LG

TL;DR: Worldline Slot Attention将物体建模为时空世界线，提出洛伦兹几何（而非欧几里得几何）是实现层次化物体发现的关键，因为它编码了不对称的因果关系，在多项任务上比欧几里得模型提升6倍性能。


<details>
  <summary>Details</summary>
Motivation: 标准的视觉模型将物体视为欧几里得空间中的独立点，无法捕捉层次结构（如部分与整体的关系）。为了解决这个问题，研究者希望探索层次化物体发现所需的基本几何结构，特别是是否需要因果结构（时间依赖）而非仅仅是树结构（径向分支）。

Method: 该方法引入了Worldline Slot Attention，将物体建模为通过时空世界线的持久轨迹。每个物体在多个层次级别上拥有多个槽位，这些槽位共享相同的空间位置但在时间坐标上不同。研究比较了欧几里得世界线和洛伦兹世界线两种几何结构，验证了几何结构对于层次建模的关键作用。

Result: 实验显示：欧几里得世界线只能达到0.078的层级准确率，低于随机机会（0.33）。而洛伦兹世界线在三个数据集上达到了0.479-0.661的准确率，比欧几里得模型提高了6倍，这一结果在20多次独立运行中具有可重复性。洛伦兹几何也优于双曲嵌入，表明视觉层次需要因果结构而非树结构。

Conclusion: 本文的研究表明，层次化物体发现需要能够编码不对称因果关系的几何结构，这是欧几里得空间所缺乏的归纳偏置，但在洛伦兹光锥中自然存在。即使是仅有11K参数的小型模型，也能通过洛伦兹几何实现显著的性能提升，证明了正确的几何结构对于学习物体层次关系的重要性。
...
### [54] [Transformers in the Dark: Navigating Unknown Search Spaces via Bandit Feedback](https://arxiv.org/abs/2603.24780)
*Jungtaek Kim,Thomas Zeng,Ziqian Lin,Minjae Lee,Chungpa Lee,Jy-yong Sohn,Hyung Il Koo,Kangwook Lee*

Main category: cs.LG

TL;DR: 研究表明，完全基于Transformer的语言模型（即无需外部分组件）可以近似树搜索算法，并能泛化到新搜索条件，从而可能改变LLM在结构化问题解决中的应用方式。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLM）与外部搜索算法结合可以增强问题解决能力，但外部分组件的引入增加了处理复杂性。因此，作者想要探究LLM或其底层Transformer架构能否近似搜索算法，从而简化整体问题解决流程。

Method: 作者提出了一个名为'具有赌博机反馈的未知树搜索'的简化框架，通过外部分配树扩展和反馈信号，以受控的方式评估Transfomer的搜索能力。研究使用Transformer模型从头开始训练，使其学习不同的搜索策略，并通过微调预训练模型进行任务导向的训练。

Result: 实验表明，Transformer在理论表达上足以实现不同的搜索策略，且可以从头训练来近似这些策略。训练好的Transformer模型在面对未见过的搜索条件（如更长的时间步或更深的树）时能展现出泛化能力。此外，聚焦任务的训练（通过微调LLM）能进一步解锁预训练模型的搜索能力。

Conclusion: 本文的结论是，基于Transformer架构的语言模型不仅能够近似搜索算法，而且在经过特定训练后，可以在未知的树搜索问题中表现出优秀的搜索能力，展示了它们在问题解决中替代传统搜索算法的潜力。
...
### [55] [A Practical Guide Towards Interpreting Time-Series Deep Clinical Predictive Models: A Reproducibility Study](https://arxiv.org/abs/2603.24828)
...
### [56] [Flow matching on homogeneous spaces](https://arxiv.org/abs/2603.24829)
*Francesco Ruscelli*
...
### [59] [Once-for-All Channel Mixers (HYPERTINYPW): Generative Compression for TinyML](https://arxiv.org/abs/2603.24916)
*Yassien Shaalan*
...
### [60] [GraphER: An Efficient Graph-Based Enrichment and Reranking Method for Retrieval-Augmented Generation](https://arxiv.org/abs/2603.24925)
...
### [62] [A Systematic Empirical Study of Grokking: Depth, Architecture, Activation, and Regularization](https://arxiv.org/abs/2603.25009)
...
### [63] [Optimal High-Probability Regret for Online Convex Optimization with Two-Point Bandit Feedback](https://arxiv.org/abs/2603.25029)
*Haishan Ye*
...
### [64] [Epistemic Compression: The Case for Deliberate Ignorance in High-Stakes AI](https://arxiv.org/abs/2603.25033)
*Steffen Lukas*
...
### [65] [Intern-S1-Pro: Scientific Multimodal Foundation Model at Trillion Scale](https://arxiv.org/abs/2603.25040)
...
### [66] [The Order Is The Message](https://arxiv.org/abs/2603.25047)
*Jordan LeDoux*
...
### [67] [SIGMA: Structure-Invariant Generative Molecular Alignment for Chemical Language Models via Autoregressive Contrastive Learning](https://arxiv.org/abs/2603.25062)
...
### [68] [Process-Aware AI for Rainfall-Runoff Modeling: A Mass-Conserving Neural Framework with Hydrological Process Constraints](https://arxiv.org/abs/2603.25093)
...
### [69] [Layer-Specific Lipschitz Modulation for Fault-Tolerant Multimodal Representation Learning](https://arxiv.org/abs/2603.25103)
...
### [70] [SEVerA: Verified Synthesis of Self-Evolving Agents](https://arxiv.org/abs/2603.25111)
...
### [71] [Vision Hopfield Memory Networks](https://arxiv.org/abs/2603.25157)
...
### [72] [Train at Moving Edge: Online-Verified Prompt Selection for Efficient RL Training of Large Reasoning Model](https://arxiv.org/abs/2603.25184)
...
### [74] [Offline Decision Transformers for Neural Combinatorial Optimization: Surpassing Heuristics on the Traveling Salesman Problem](https://arxiv.org/abs/2603.25241)
...
### [75] [How Pruning Reshapes Features: Sparse Autoencoder Analysis of Weight-Pruned Language Models](https://arxiv.org/abs/2603.25325)
...
### [76] [From Intent to Evidence: A Categorical Approach for Structural Evaluation of Deep Research Agents](https://arxiv.org/abs/2603.25342)
...
### [77] [Hessian-informed machine learning interatomic potential towards bridging theory and experiments](https://arxiv.org/abs/2603.25373)
...
### [78] [GlowQ: Group-Shared LOw-Rank Approximation for Quantized LLMs](https://arxiv.org/abs/2603.25385)
...
### [79] [Maximum Entropy Behavior Exploration for Sim2Real Zero-Shot Reinforcement Learning](https://arxiv.org/abs/2603.25464)
...
### [80] [Not a fragment, but the whole: Map-based evaluation of data-driven Fire Danger Index models](https://arxiv.org/abs/2603.25469)
...
### [81] [Causal-INSIGHT: Probing Temporal Models to Extract Causal Structure](https://arxiv.org/abs/2603.25473)
...
### [82] [How Class Ontology and Data Scale Affect Audio Transfer Learning](https://arxiv.org/abs/2603.25476)
...
### [83] [Interpretable PM2.5 Forecasting for Urban Air Quality: A Comparative Study of Operational Time-Series Models](https://arxiv.org/abs/2603.25495)
...
### [84] [Missing-Aware Multimodal Fusion for Unified Microservice Incident Management](https://arxiv.org/abs/2603.25538)
...
### [85] [An Integrative Genome-Scale Metabolic Modeling and Machine Learning Framework for Predicting and Optimizing Biofuel-Relevant Biomass Production in Saccharomyces cerevisiae](https://arxiv.org/abs/2603.25561)
...
### [86] [Revisiting On-Policy Distillation: Empirical Failure Modes and Simple Fixes](https://arxiv.org/abs/2603.25562)
...
### [87] [Spatiotemporal System Forecasting with Irregular Time Steps via Masked Autoencoder](https://arxiv.org/abs/2603.25597)
...
### [88] [Anchored-Branched Steady-state WInd Flow Transformer (AB-SWIFT): a metamodel for 3D atmospheric flow in urban environments](https://arxiv.org/abs/2603.25635)
...
### [89] [Uncertainty-Guided Label Rebalancing for CPS Safety Monitoring](https://arxiv.org/abs/2603.25670)
...
### [90] [Longitudinal Digital Phenotyping for Early Cognitive-Motor Screening](https://arxiv.org/abs/2603.25673)
...
### [91] [On Neural Scaling Laws for Weather Emulation through Continual Training](https://arxiv.org/abs/2603.25687)
...
### [92] [A Unified Memory Perspective for Probabilistic Trustworthy AI](https://arxiv.org/abs/2603.25692)
...
### [93] [Neural Network Conversion of Machine Learning Pipelines](https://arxiv.org/abs/2603.25699)
