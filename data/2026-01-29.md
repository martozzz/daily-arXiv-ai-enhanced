...
TL;DR: 本文提出了一种名为Gap-K%的新型预训练数据检测方法，通过分析LLM预训练优化动态，利用top-1预测与目标token之间的对数概率差距，结合滑动窗口策略捕获局部相关性，在WikiMIA和MIMIR基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于token似然的最先进方法通常忽略了模型top-1预测的偏离和相邻token之间的局部相关性，而大规模预训练语料库的不透明性引发了严重的隐私和版权问题。

Method: Gap-K%方法基于LLM预训练的优化动态，利用top-1预测token与目标token之间的对数概率差距，采用滑动窗口策略来捕获局部相关性并缓解token级波动。

Result: 在WikiMIA和MIMIR基准上的广泛实验表明，Gap-K%实现了最先进的性能，在不同模型大小和输入长度下均优于现有基线方法。

Conclusion: Gap-K%是一种有效的预训练数据检测方法，通过利用预训练优化动态中的梯度信号差异，显著提升了检测性能，为解决LLM预训练数据的隐私和版权问题提供了新思路。
...
TL;DR: 本文提出了一种新的去中心化联邦学习聚合方法，通过捕捉局部模型参数变化来应对数据和模型异质性，在减少通信成本的同时提高局部模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 去中心化联邦学习中，设备间的个体经验差异和交互水平不同导致数据和模型初始化异质性，这些异质性造成局部模型参数变化，从而减缓收敛速度。

Method: 引入一种新颖的聚合方法，通过近似局部模型在其本地数据集上的二阶信息来生成共识权重，利用这些权重对邻域更新进行缩放，然后聚合到全局邻域表示中。

Result: 在计算机视觉任务的广泛实验中，所提出的方法在减少通信成本的同时显示出局部模型的强大泛化能力。

Conclusion: 该方法通过显式处理局部模型参数级别的变化证据可信度，有效解决了数据和模型异质性问题，实现了更快的收敛和更好的模型性能。
...
TL;DR: 本文提出了Oculomix，一种用于视网膜图像分析的层次化采样策略，用于改进传统的图像级混合样本数据增强方法，通过考虑患者和检查层次关系来更好地保留患者特异性特征。


<details>
  <summary>Details</summary>
Motivation: 传统图像级混合样本数据增强方法（如CutMix和MixUp）会扰动患者特异性属性（如医学合并症和临床因素），因为它们只考虑图像和标签。在眼科学（Oculomics）中，预测心血管疾病和痴呆等系统性疾病时，保留这些患者特异性特征至关重要。

Method: 提出了Oculomix层次化采样策略，基于两个临床先验：1）检查级别：同一患者在同一时间点采集的图像共享相同属性；2）患者级别：同一患者在不同时间点采集的图像具有软时间趋势（发病率通常随时间增加）。该方法将混合空间约束到患者和检查级别，以更好地保留患者特异性特征并利用其层次关系。

Result: 在大型多样化人群（Alzeye）中预测5年主要不良心血管事件（MACE）的实验中，使用ViT模型验证了Oculomix。结果显示，Oculomix在AUROC指标上比图像级CutMix和MixUp方法一致性地高出高达3%。

Conclusion: Oculomix方法在眼科学中具有必要性和价值，通过考虑患者和检查层次关系来改进混合样本数据增强，能更好地保留患者特异性特征，从而提高疾病预测性能。
...
### [4] [Continuous-Flow Data-Rate-Aware CNN Inference on FPGA](https://arxiv.org/abs/2601.19940)
*Tobias Habermann,Michael Mecik,Zhenyu Wang,César David Vera,Martin Kumm,Mario Garrido*

Main category: cs.LG

TL;DR: A novel data-rate-aware CNN architecture improves FPGA hardware utilization by interleaving signals and sharing units, enabling efficient implementation of complex CNNs like MobileNet.


<details>
  <summary>Details</summary>
Motivation: The issue of underutilized hardware units in fully parallel CNN implementations due to data reduction in pooling and convolutional layers with stride larger than one needs to be addressed.

Method: The approach involves analyzing CNN data flow, interleaving low data rate signals, sharing hardware units, and optimizing parallelization to achieve high throughput.

Result: The method achieves close to 100% hardware utilization and saves a significant amount of arithmetic logic, allowing efficient FPGA implementation of complex CNNs.

Conclusion: The proposed data-rate-aware, continuous-flow CNN architecture significantly improves hardware utilization, enabling complex CNNs like MobileNet to be implemented on a single FPGA with high throughput.
...
### [5] [Latent Object Permanence: Topological Phase Transitions, Free-Energy Principles, and Renormalization Group Flows in Deep Transformer Manifolds](https://arxiv.org/abs/2601.19942)
*Faruk Alpay,Bugra Kilictas*

Main category: cs.LG

TL;DR: 论文通过几何和统计物理视角研究Transformer语言模型中多步推理能力的涌现，发现隐藏状态轨迹在黎曼流形上的流动特征，观测到相变现象和瞬态类对象形成。


<details>
  <summary>Details</summary>
Motivation: 研究深度Transformer语言模型中多步推理能力的涌现机制，从几何和统计物理的角度理解模型内部表示的演化过程。

Method: 1. 将隐藏状态轨迹视为隐式黎曼流形上的流动；2. 分析激活的层间协方差谱；3. 跟踪与随机矩阵主体的偏差；4. 基于稀疏性/局部化的序参数分析；5. 将前向传播形式化为离散粗粒化映射；6. 理论条件连接逻辑可分性与谱衰减；7. 在多模型家族上进行层间探针验证。

Result: 1. 在1.5B-30B模型规模范围内观察到有效维度的急剧减少，与相变一致；2. 基于稀疏性的序参数在临界归一化深度γ_c≈0.42处出现不连续性；3. 前向传播作为粗粒化映射形成稳定的"概念盆地"；4. 低熵状态下出现谱尾崩溃和瞬态类对象(TCOs)形成；5. 理论预测与实验验证相符。

Conclusion: Transformer语言模型中的多步推理能力通过几何相变机制涌现，表现为表示空间中有效维度的减少和瞬态类对象的形成，这些结构为逻辑推理提供了可重用的中间表示。
...
### [6] [Emergent Specialization in Learner Populations: Competition as the Source of Diversity](https://arxiv.org/abs/2601.19943)
*Yuhao Li*

Main category: cs.LG

TL;DR: 竞争可诱导学习者群体自发形成多样化行为分工，NichePopulation算法在多个领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 探索学习者群体如何在无显式通信或多样性激励的情况下，通过竞争自发形成多样化的行为分工。

Method: 提出了NichePopulation算法，结合竞争排斥和生态位亲和性跟踪，验证了其在六个现实领域的表现。

Result: 在六个现实领域中，NichePopulation算法实现了平均Specialization Index为0.75，效果显著（Cohen's d > 20）。多样化的群体表现优于同质化基线26.5%，且优于MARL基线4.3倍，速度提升4倍。

Conclusion: 竞争可以诱导学习者群体自发形成多样化的行为分工，无需显式通信或多样性激励。NichePopulation算法通过结合竞争排斥和生态位亲和性跟踪，在多个现实领域验证了其有效性。
...
### [7] [NCSAM Noise-Compensated Sharpness-Aware Minimization for Noisy Label Learning](https://arxiv.org/abs/2601.19947)
*Jiayu Xu,Junbiao Pang*

Main category: cs.LG

TL;DR: 本文提出NCSAM方法，通过理论分析和实验验证，利用SAM扰动缓解标签噪声损害，在多个任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界的数据集通常包含错误的标注，而现有研究主要关注复杂的标签校正机制。本文从损失景观的平坦性与标签噪声之间的关系出发，探索一种新的理论视角。

Method: 本文提出了一种名为Noise-Compensated Sharpness-aware Minimization（NCSAM）的方法，通过利用SAM的扰动来补偿标签噪声的影响。

Result: 实验结果表明，NCSAM在多个基准数据集上表现优于现有的最先进方法，测试精度表现出与无噪声数据集相似的行为。

Conclusion: 本文通过理论分析和实验验证，提出了一种新的方法NCSAM，该方法通过利用Sharpness-Aware Minimization（SAM）的扰动来缓解标签噪声的损害，并在多个基准数据集上展示了其优越性。
...
TL;DR: 提出了一种基于概率神经元的智能传感范式，显著提升能源效率并保持数据完整性，实验验证了其高效性和准确性。


<details>
  <summary>Details</summary>
Motivation: 为了解决传统确定性数据采集方法可能丢失信息的问题，同时实现能源效率的显著提升。

Method: 研究采用了受自主神经系统启发的概率神经元（p-neuron），并结合模拟特征提取电路，实现了微秒级的响应时间。

Result: 实验验证表明，该方法在主动地震调查数据中实现了无损概率数据采集，归一化均方误差为0.41%，系统主动运行时间和生成的样本数节省了93%。

Conclusion: 该研究提出了一种基于概率神经元的传感范式，能够在数据采集过程中实现实时智能自主激活，显著提高了系统的能源效率，同时保持了数据的完整性。
...
<details>
  <summary>Details</summary>
Motivation: 现有缓存方法依赖瞬时速度信息，在高加速比下容易导致严重轨迹偏差和误差累积，需要更稳定的缓存方案。

Method: 引入平均速度视角，利用缓存的雅可比-向量积构建区间平均速度；开发轨迹稳定性调度策略，采用峰值抑制最短路径算法确定缓存时机。

Result: 在FLUX.1、Qwen-Image和HunyuanVideo上分别实现4.12倍、4.56倍和3.59倍加速，生成质量优于现有缓存基线。

Conclusion: 该方法为Flow Matching推理提供了新视角，有望推动商业级生成模型的稳定性驱动加速研究。
...
### [10] [Cross-Session Decoding of Neural Spiking Data via Task-Conditioned Latent Alignment](https://arxiv.org/abs/2601.19963)
*Canyang Zhao,Bolin Peng,J. Patrick Mayo,Ce Ju,Bing Liu*

Main category: cs.LG

TL;DR: 本文提出了一种任务条件潜在对齐框架（TCLA），用于解决植入式脑机接口中神经活动的跨会话非平稳性问题，通过在源会话中学习低维神经动态表示，并在目标会话中对齐潜在表示，实现有效的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 植入电极记录的神经活动存在跨会话非平稳性，导致在一个会话中训练的译码器难以泛化到后续会话，特别是在新会话数据有限的情况下，重新训练或调整译码器变得尤为困难。

Method: 基于自编码器架构，TCLA首先从数据充足的源会话中学习神经动态的低维表示。对于数据有限的目标会话，TCLA以任务条件的方式将目标潜在表示与源会话对齐，实现学习神经动态的有效迁移。

Result: 在猕猴运动和眼动中心-外数据集上的评估显示，与仅使用目标会话数据训练的基线方法相比，TCLA在数据集和译码设置上一致提高了译码性能，在运动数据集的y坐标速度译码中决定系数增益高达0.386。

Conclusion: TCLA为从源会话到目标会话的知识迁移提供了有效策略，在数据有限条件下实现了更稳健的神经译码。
...
### [11] [Modeling Cascaded Delay Feedback for Online Net Conversion Rate Prediction: Benchmark, Insights and Solutions](https://arxiv.org/abs/2601.19965)
*Mingxuan Luo,Guipeng Xv,Sishuo Chen,Xinyu Li,Li Zhang,Zhangming Chan,Xiang-Rong Sheng,Han Zhu,Jian Xu,Bo Zheng,Chen Lin*

Main category: cs.LG

TL;DR: 论文提出了CASCADE数据集和TESLA框架，用于解决推荐系统中净转化率预测的挑战，通过级联建模和在线连续训练显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 传统转化率指标忽略退款行为，无法完全反映推荐效果的真实商业价值。净转化率预测面临级联延迟反馈、开源数据集缺乏、在线连续训练方案不足等挑战。

Method: 1) 构建CASCADE数据集，首个大规模开源在线连续净转化率预测数据集；2) 提出TESLA框架，采用CVR-退款率级联架构、分阶段去偏和延迟时间感知排序损失。

Result: TESLA在CASCADE数据集上显著优于现有方法，净转化率预测的RI-AUC提升12.41%，RI-PRAUC提升14.94%。

Conclusion: 级联建模结合在线连续训练是解决净转化率预测挑战的有效方法，延迟时间作为重要特征能显著提升预测性能，开源数据集和框架为领域研究提供了基础。
...
### [12] [BayPrAnoMeta: Bayesian Proto-MAML for Few-Shot Industrial Image Anomaly Detection](https://arxiv.org/abs/2601.19992)
*Soham Sarkar,Tanmay Sen,Sayantan Banerjee*

Main category: cs.LG

TL;DR: BayPrAnoMeta：一种基于贝叶斯原型的元学习方法，用于少样本工业图像异常检测，通过概率正态模型和贝叶斯后验预测似然实现不确定性感知的异常评分。
...
Method: 提出BayPrAnoMeta，将原型替换为任务特定的概率正态模型，使用Normal-Inverse-Wishart先验对正常支持嵌入建模，产生Student-t预测分布。内循环适应通过贝叶斯后验预测似然实现，支持不确定性感知和重尾异常评分。进一步扩展到联邦元学习框架，结合监督对比正则化处理异构工业客户端。

Result: 在MVTec AD基准测试中，相比MAML、Proto-MAML和基于PatchCore的方法，在少样本异常检测设置中实现了持续且显著的AUROC改进。

Conclusion: BayPrAnoMeta通过概率建模和贝叶斯适应，在少样本工业图像异常检测中表现出优越性能，特别是在极端少样本情况下具有更好的鲁棒性。该方法可扩展到联邦学习环境，适用于异构工业客户端。
...
### [13] [Decomposing multimodal embedding spaces with group-sparse autoencoders](https://arxiv.org/abs/2601.20028)
*Chiraag Kaushik,Davis Barch,Andrea Fanelli*

Main category: cs.LG

TL;DR: A new SAE-based approach improves multimodal alignment in embeddings, enhancing interpretability and control for cross-modal tasks.


<details>
  <summary>Details</summary>
Motivation: To address the issue of 'split dictionaries' in SAEs for multimodal embeddings, ensuring better alignment of concepts across modalities.

Method: The study introduces a new SAE-based method using cross-modal random masking and group-sparse regularization, applied to CLIP and CLAP embeddings.

Result: The method learns a more multimodal dictionary, reduces dead neurons, and improves feature semanticity.

Conclusion: The proposed approach successfully improves multimodal alignment in SAEs, enhancing interpretability and control in cross-modal tasks.
...
TL;DR: TONEL框架通过噪声感知嵌入学习，提升了边缘设备上RAG的噪声鲁棒性和领域适应性。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上部署RAG面临效率瓶颈和噪声干扰问题，尤其在动态多领域场景中，准确性和适应性至关重要。

Method: TONEL采用噪声感知投影模型，学习任务特定的嵌入表示，以适应CiM硬件约束，并在噪声条件下实现准确检索。

Result: 在个性化基准测试中，TONEL在噪声场景下表现出色，优于现有基线方法。

Conclusion: TONEL框架通过噪声感知投影模型，显著提升了在噪声边缘环境中RAG的检索精度和领域适应性，为个性化虚拟助手的实际部署提供了有效解决方案。
...
### [15] [Regime-Adaptive Bayesian Optimization via Dirichlet Process Mixtures of Gaussian Processes](https://arxiv.org/abs/2601.20043)
...
### [16] [Externally Validated Longitudinal GRU Model for Visit-Level 180-Day Mortality Risk in Metastatic Castration-Resistant Prostate Cancer](https://arxiv.org/abs/2601.20046)
...
### [17] [Domain Expansion: A Latent Space Construction Framework for Multi-Task Learning](https://arxiv.org/abs/2601.20069)
...
### [18] [Techno-economic optimization of a heat-pipe microreactor, part II: multi-objective optimization analysis](https://arxiv.org/abs/2601.20079)
...
### [19] [Quantization-Aware Distillation for NVFP4 Inference Accuracy Recovery](https://arxiv.org/abs/2601.20088)
...
### [20] [In-Context Reinforcement Learning From Suboptimal Historical Data](https://arxiv.org/abs/2601.20116)
...
### [21] [A Reinforcement Learning Based Universal Sequence Design for Polar Codes](https://arxiv.org/abs/2601.20118)
...
### [22] [Going NUTS with ADVI: Exploring various Bayesian Inference techniques with Facebook Prophet](https://arxiv.org/abs/2601.20120)
...
### [23] [Membership Inference Attacks Against Fine-tuned Diffusion Language Models](https://arxiv.org/abs/2601.20125)
...
### [24] [Scaling Next-Brain-Token Prediction for MEG](https://arxiv.org/abs/2601.20138)
*Richard Csaky*
...
### [25] [Spectral Ghost in Representation Learning: from Component Analysis to Self-Supervised Learning](https://arxiv.org/abs/2601.20154)
...
### [26] [PASS: Ambiguity Guided Subsets for Scalable Classical and Quantum Constrained Clustering](https://arxiv.org/abs/2601.20157)
...
### [27] [What's the plan? Metrics for implicit planning in LLMs and their application to rhyme generation and question answering](https://arxiv.org/abs/2601.20164)
...
### [28] [Local Duality for Sparse Support Vector Machines](https://arxiv.org/abs/2601.20170)
...
### [29] [Loss Landscape Geometry and the Learning of Symmetries: Or, What Influence Functions Reveal About Robust Generalization](https://arxiv.org/abs/2601.20172)
...
### [30] [MAPLE: Self-supervised Learning-Enhanced Nonlinear Dimensionality Reduction for Visual Analysis](https://arxiv.org/abs/2601.20173)
...
### [31] [NeuraLSP: An Efficient and Rigorous Neural Left Singular Subspace Preconditioner for Conjugate Gradient Methods](https://arxiv.org/abs/2601.20174)
...
### [32] [Causal-Driven Feature Evaluation for Cross-Domain Image Classification](https://arxiv.org/abs/2601.20176)
*Chen Cheng,Ang Li*
...
### [33] [On the Computational Complexity of Performative Prediction](https://arxiv.org/abs/2601.20180)
...
### [34] [Meta-Cognitive Reinforcement Learning with Self-Doubt and Recovery](https://arxiv.org/abs/2601.20193)
...
### [35] [DeRaDiff: Denoising Time Realignment of Diffusion Models](https://arxiv.org/abs/2601.20198)
...
### [36] [Minimum-Cost Network Flow with Dual Predictions](https://arxiv.org/abs/2601.20203)
...
### [37] [Hyperparameter Transfer with Mixture-of-Expert Layers](https://arxiv.org/abs/2601.20205)
...
### [38] [Spark: Strategic Policy-Aware Exploration via Dynamic Branching for Long-Horizon Agentic Learning](https://arxiv.org/abs/2601.20209)
...
### [39] [An Accounting Identity for Algorithmic Fairness](https://arxiv.org/abs/2601.20217)
...
### [40] [Parametric and Generative Forecasts of Day-Ahead Market Curves for Storage Optimization](https://arxiv.org/abs/2601.20226)
...
### [41] [ProFlow: Zero-Shot Physics-Consistent Sampling via Proximal Flow Guidance](https://arxiv.org/abs/2601.20227)
...
### [42] [Certificate-Guided Pruning for Stochastic Lipschitz Optimization](https://arxiv.org/abs/2601.20231)
...
### [43] [Order-Optimal Sample Complexity of Rectified Flows](https://arxiv.org/abs/2601.20250)
...
### [44] [Robust SDE Parameter Estimation Under Missing Time Information Setting](https://arxiv.org/abs/2601.20268)
...
### [45] [HE-SNR: Uncovering Latent Logic via Entropy for Guiding Mid-Training on SWE-BENCH](https://arxiv.org/abs/2601.20255)
...
### [46] [The Forecast After the Forecast: A Post-Processing Shift in Time Series](https://arxiv.org/abs/2601.20280)
...
### [47] [C2:Cross learning module enhanced decision transformer with Constraint-aware loss for auto-bidding](https://arxiv.org/abs/2601.20257)
...
### [48] [Cheap2Rich: A Multi-Fidelity Framework for Data Assimilation and System Identification of Multiscale Physics -- Rotating Detonation Engines](https://arxiv.org/abs/2601.20295)
...
### [49] [Truthfulness Despite Weak Supervision: Evaluating and Training LLMs Using Peer Prediction](https://arxiv.org/abs/2601.20299)
...
### [52] [LLM-AutoDP: Automatic Data Processing via LLM Agents for Model Fine-tuning](https://arxiv.org/abs/2601.20375)
...
### [54] [FedRD: Reducing Divergences for Generalized Federated Learning via Heterogeneity-aware Parameter Guidance](https://arxiv.org/abs/2601.20397)
...
### [55] [Fair Recourse for All: Ensuring Individual and Group Fairness in Counterfactual Explanations](https://arxiv.org/abs/2601.20449)
...
### [56] [CCMamba: Selective State-Space Models for Higher-Order Graph Learning on Combinatorial Complexes](https://arxiv.org/abs/2601.20518)
...
### [62] [Ranking-aware Reinforcement Learning for Ordinal Ranking](https://arxiv.org/abs/2601.20585)
...
### [63] [TINNs: Time-Induced Neural Networks for Solving Time-Dependent PDEs](https://arxiv.org/abs/2601.20361)
...
### [67] [Learning Contextual Runtime Monitors for Safe AI-Based Autonomy](https://arxiv.org/abs/2601.20666)
...
### [68] [Adapting the Behavior of Reinforcement Learning Agents to Changing Action Spaces and Reward Functions](https://arxiv.org/abs/2601.20714)
...
### [69] [ScatterFusion: A Hierarchical Scattering Transform Framework for Enhanced Time Series Forecasting](https://arxiv.org/abs/2601.20401)
*Wei Li*
...
### [70] [HESTIA: A Hessian-Guided Differentiable Quantization-Aware Training Framework for Extremely Low-Bit LLMs](https://arxiv.org/abs/2601.20745)
...
### [71] [AWGformer: Adaptive Wavelet-Guided Transformer for Multi-Resolution Time Series Forecasting](https://arxiv.org/abs/2601.20409)
*Wei Li*
...
### [72] [Conditional PED-ANOVA: Hyperparameter Importance in Hierarchical & Dynamic Search Spaces](https://arxiv.org/abs/2601.20800)
...
### [73] [Concept Component Analysis: A Principled Approach for Concept Extraction in LLMs](https://arxiv.org/abs/2601.20420)
...
### [74] [Reinforcement Learning via Self-Distillation](https://arxiv.org/abs/2601.20802)
...
### [75] [Nonlinear Dimensionality Reduction with Diffusion Maps in Practice](https://arxiv.org/abs/2601.20428)
...
### [76] [GNN Explanations that do not Explain and How to find Them](https://arxiv.org/abs/2601.20815)
...
### [77] [TimeCatcher: A Variational Framework for Volatility-Aware Forecasting of Non-Stationary Time Series](https://arxiv.org/abs/2601.20448)
...
### [78] [Training Reasoning Models on Saturated Problems via Failure-Prefix Conditioning](https://arxiv.org/abs/2601.20829)
...
### [79] [Reward Models Inherit Value Biases from Pretraining](https://arxiv.org/abs/2601.20838)
...
### [80] [Implicit Hypothesis Testing and Divergence Preservation in Neural Network Representations](https://arxiv.org/abs/2601.20477)
...
### [81] [$\mathbb{R}^{2k}$ is Theoretically Large Enough for Embedding-based Top-$k$ Retrieval](https://arxiv.org/abs/2601.20844)
...
### [82] [An explainable framework for the relationship between dementia and glucose metabolism patterns](https://arxiv.org/abs/2601.20480)
...
### [83] [Post-Training Fairness Control: A Single-Train Framework for Dynamic Fairness in Recommendation](https://arxiv.org/abs/2601.20848)
...
### [84] [Exploring Transformer Placement in Variational Autoencoders for Tabular Data Generation](https://arxiv.org/abs/2601.20854)
...
### [85] [Evolutionary Strategies lead to Catastrophic Forgetting in LLMs](https://arxiv.org/abs/2601.20861)
...
### [86] [Reinforcement Unlearning via Group Relative Policy Optimization](https://arxiv.org/abs/2601.20568)
...
### [87] [CoBA: Integrated Deep Learning Model for Reliable Low-Altitude UAV Classification in mmWave Radio Networks](https://arxiv.org/abs/2601.20605)
...
### [89] [DIVERSE: Disagreement-Inducing Vector Evolution for Rashomon Set Exploration](https://arxiv.org/abs/2601.20627)
...
### [90] [A Foundation Model for Virtual Sensors](https://arxiv.org/abs/2601.20634)
...
### [91] [An Empirical Investigation of Neural ODEs and Symbolic Regression for Dynamical Systems](https://arxiv.org/abs/2601.20637)
...
### [92] [MuRAL-CPD: Active Learning for Multiresolution Change Point Detection](https://arxiv.org/abs/2601.20686)
...
### [93] [Positive-Unlabeled Reinforcement Learning Distillation for On-Premise Small Models](https://arxiv.org/abs/2601.20687)
...
### [94] [Optimal Transport Group Counterfactual Explanations](https://arxiv.org/abs/2601.20692)
...
### [95] [Is Pure Exploitation Sufficient in Exogenous MDPs with Linear Function Approximation?](https://arxiv.org/abs/2601.20694)
...
### [96] [Structurally Human, Semantically Biased: Detecting LLM-Generated References with Embeddings and GNNs](https://arxiv.org/abs/2601.20704)
...
### [97] [Deep Semi-Supervised Survival Analysis for Predicting Cancer Prognosis](https://arxiv.org/abs/2601.20729)
...
### [98] [Continual GUI Agents](https://arxiv.org/abs/2601.20732)
...
### [103] [COMET-SG1: Lightweight Autoregressive Regressor for Edge and Embedded AI](https://arxiv.org/abs/2601.20772)
*Shakhyar Gogoi*
...
### [104] [Smoothing the Black-Box: Signed-Distance Supervision for Black-Box Model Copying](https://arxiv.org/abs/2601.20773)
...
### [105] [When More Data Doesn't Help: Limits of Adaptation in Multitask Learning](https://arxiv.org/abs/2601.20774)
...
### [106] [Active Learning for Decision Trees with Provable Guarantees](https://arxiv.org/abs/2601.20775)
...
### [107] [PatchFormer: A Patch-Based Time Series Foundation Model with Hierarchical Masked Reconstruction and Cross-Domain Transfer Learning for Zero-Shot Multi-Horizon Forecasting](https://arxiv.org/abs/2601.20845)
...
### [108] [NeuroAI and Beyond](https://arxiv.org/abs/2601.19955)
*Jean-Marc Fellous,Gert Cauwenberghs,Cornelia Fermüller,Yulia Sandamisrkaya,Terrence Sejnowski*

Main category: cs.AI

TL;DR: 论文探讨神经科学与AI的协同潜力，提出NeuroAI概念，总结当前进展并展望未来方向，附有SWOT分析。


<details>
  <summary>Details</summary>
Motivation: 神经科学与AI在过去几年取得了显著进展，但两者之间的联系较为松散。论文旨在探讨两者之间的协同潜力，推动NeuroAI的发展。

Method: 基于2025年8月的一次研讨会，论文识别了神经科学与AI之间当前和未来的协同领域，并聚焦于具体子领域（如具身性、语言与通信、机器人学、人类与机器学习、神经形态工程）进行总结与展望。

Result: 论文总结了当前进展，提出了未来可能的研究方向，并附有研究人员和学员对NeuroAI的SWOT分析，描述了其益处与风险。

Conclusion: 论文主张发展NeuroAI，认为这种神经科学启发的人工智能有潜力显著提升AI算法的范围和效率，同时改变我们对生物神经计算的理解。
...
### [109] [Teaching LLMs to Ask: Self-Querying Category-Theoretic Planning for Under-Specified Reasoning](https://arxiv.org/abs/2601.20014)
*Shuhui Qu*

Main category: cs.AI

TL;DR: SQ-BCP通过显式表示前提条件和双向搜索，显著减少部分可观测性下的资源违规率。


<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在部分可观测性下容易产生幻觉或违反硬约束的问题，提高推理时规划的准确性和可靠性。

Method: SQ-BCP采用双向搜索和基于拉回的验证器，通过自我查询和桥接假设解决部分可观测性问题，并使用距离评分进行排序和剪枝。

Result: 在WikiHow和RecipeNLG任务中，SQ-BCP将资源违规率分别降低至14.9%和5.8%，优于基线模型。

Conclusion: SQ-BCP通过显式表示前提条件状态并引入自我查询和桥接假设，显著减少了资源违规率，同时在目标兼容性验证和计划生成方面表现出色。
...
### [110] [Fuzzy Categorical Planning: Autonomous Goal Satisfaction with Graded Semantic Constraints](https://arxiv.org/abs/2601.20021)
*Shuhui Qu*

Main category: cs.AI

TL;DR: 本文提出模糊范畴论规划（FCP），通过引入模糊度标注和t-范数组合来处理自然语言规划中的模糊谓词，相比现有方法能更好地跟踪多步计划中的质量退化。


<details>
  <summary>Details</summary>
Motivation: 现有范畴论规划器将适用性视为二元判断，无法处理自然语言规划中常见的模糊谓词（如"合适的替代品"、"足够稳定"），导致需要设定阈值而丢失有意义的质量区分。

Method: FCP为每个动作标注[0,1]区间的模糊度，使用Lukasiewicz t-范数组合计划质量，通过回拉验证保持可执行性检查，利用LLM进行模糊适用性标注，并支持基于残差的后向需求中间相遇搜索。

Result: 在PDDL3偏好/超额订阅基准和RecipeNLG-Subs食谱规划基准上的评估显示，FCP相比LLM-only和ReAct基线提高了成功率并减少了硬约束违反，同时与经典PDDL3规划器保持竞争力。

Conclusion: FCP方法有效解决了自然语言规划中的模糊谓词处理问题，在保持可执行性验证的同时能够跟踪计划质量退化，在多个基准测试中表现出优越性能。
...
### [111] [Scaling Medical Reasoning Verification via Tool-Integrated Reinforcement Learning](https://arxiv.org/abs/2601.20221)
*Hang Zhang,Ruheng Wang,Yuelyu Ji,Mingu Kwak,Xizhi Wu,Chenyu Li,Li Zhang,Wenqi Shi,Yifan Peng,Yanshan Wang*

Main category: cs.AI

TL;DR: 论文提出了一种名为$\method$的框架，通过迭代查询外部医学知识库来验证医学推理的准确性，显著提升了性能并减少了采样预算需求。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医学推理基准上表现优异，但在临床部署中需要确保事实准确性。现有奖励模型方法缺乏明确解释且依赖单次检索，限制了验证效果。

Method: $\method$框架结合工具增强验证和迭代强化学习范式，动态调整训练数据分布，通过迭代查询外部医学知识库进行验证。

Result: 在四个医学推理基准上，$\method$显著优于现有方法，MedQA准确率提升23.5%，MedXpertQA提升32.0%，采样预算需求减少8倍。

Conclusion: 基于动态检索证据的验证方法为构建更可靠的医学推理系统提供了有效路径。
...
### [112] [Endogenous Reprompting: Self-Evolving Cognitive Alignment for Unified Multimodal Models](https://arxiv.org/abs/2601.20305)
*Zhenchen Tang,Songlin Yang,Zichuan Wang,Bo Peng,Yang Li,Beibei Dong,Jing Dong*

Main category: cs.AI

TL;DR: 提出SEER框架，通过内循环机制提升多模态模型的生成能力，实验表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决统一多模态模型在生成过程中缺乏自我增强能力的问题，即认知鸿沟。

Method: 引入SEER框架，包括两阶段内循环：RLVR激活模型的潜在评估能力，RLMT利用奖励信号优化生成推理策略。

Result: SEER在评估准确性、重新提示效率和生成质量上均优于现有基线。

Conclusion: 通过提出的SEER框架，成功弥合了认知鸿沟，提升了生成质量，同时保持了多模态能力。
...
### [113] [AMA: Adaptive Memory via Multi-Agent Collaboration](https://arxiv.org/abs/2601.20352)
*Weiquan Huang,Zixuan Wang,Hehai Lin,Sudong Wang,Bo Xu,Qian Li,Beier Zhu,Linyi Yang,Chengwei Qin*

Main category: cs.AI

TL;DR: AMA是一个基于多智能体协作的自适应记忆框架，通过分层记忆设计和动态粒度对齐来解决LLM智能体记忆系统中的检索粒度不匹配和逻辑不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体记忆系统存在刚性检索粒度、积累式维护策略和粗粒度更新机制的问题，导致存储信息与任务需求不匹配，且随时间推移产生逻辑不一致。

Method: AMA采用多智能体协作框架：Constructor和Retriever负责多粒度记忆构建和自适应查询路由；Judge验证检索内容的相关性和一致性；Refresher检测逻辑冲突并执行定向更新。

Result: 在长上下文基准测试中，AMA显著优于现有最优方法，同时相比全上下文方法减少约80%的token消耗，证明了其在保持检索精度和长期记忆一致性方面的有效性。

Conclusion: AMA框架通过多智能体协作和分层记忆设计，有效解决了LLM智能体记忆系统中的核心挑战，为长期交互和复杂推理提供了更可靠的记忆支持。
...
### [114] [Policy of Thoughts: Scaling LLM Reasoning via Test-time Policy Evolution](https://arxiv.org/abs/2601.20379)
*Zhengbo Jiao,Hongyu Xian,Qinglong Wang,Yunpu Ma,Zhebo Wang,Zifan Zhang,Dezhang Kong,Meng Han*

Main category: cs.AI

TL;DR: PoT 框架通过动态优化推理策略，显著提升大型语言模型在复杂任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 针对大型语言模型在复杂、长时推理任务中的不稳定性问题，提出通过实时学习和反馈优化推理策略。

Method: PoT 框架通过高效的探索机制生成多样候选解，并利用 Group Relative Policy Optimization (GRPO) 更新瞬态 LoRA 适配器，实现推理策略的动态优化。

Result: 实验表明，PoT 显著提升了模型性能，4B 参数模型在 LiveCodeBench 上的准确率达到 49.71%，优于 GPT-4o 和 DeepSeek-V3。

Conclusion: Policy of Thoughts (PoT) 通过动态优化推理策略，显著提升了大型语言模型在复杂任务中的表现，证明了实时学习和反馈的重要性。
...
### [115] [CtrlCoT: Dual-Granularity Chain-of-Thought Compression for Controllable Reasoning](https://arxiv.org/abs/2601.20467)
*Zhenxuan Fan,Jie Cao,Yang Dai,Zheqi Lv,Wenqiao Zhang,Zhongle Xie,Peng LU,Beng Chin Ooi*

Main category: cs.AI

TL;DR: CtrlCoT通过双粒度压缩框架优化CoT推理，减少令牌使用并提高准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的CoT压缩方法在语义级或令牌级单独操作时存在保守或激进的问题，且难以结合。

Method: 提出了Hierarchical Reasoning Abstraction、Logic-Preserving Distillation和Distribution-Alignment Generation三个组件，实现了语义和令牌级的协同压缩。

Result: 在MATH-500数据集上，CtrlCoT减少了30.7%的令牌使用，同时准确率提高了7.6个百分点。

Conclusion: CtrlCoT通过双粒度压缩框架，结合语义抽象和令牌级修剪，显著提高了推理效率，同时保持了正确性。
...
### [116] [Normative Equivalence in human-AI Cooperation: Behaviour, Not Identity, Drives Cooperation in Mixed-Agent Groups](https://arxiv.org/abs/2601.20487)
...
### [117] [PathWise: Planning through World Model for Automated Heuristic Design via Self-Evolving LLMs](https://arxiv.org/abs/2601.20539)
...
### [118] [Online Risk-Averse Planning in POMDPs Using Iterated CVaR Value Function](https://arxiv.org/abs/2601.20554)
...
### [119] [Dialogical Reasoning Across AI Architectures: A Multi-Model Framework for Testing AI Alignment Strategies](https://arxiv.org/abs/2601.20604)
*Gray Cox*
...
### [120] [Harder Is Better: Boosting Mathematical Reasoning via Difficulty-Aware GRPO and Multi-Aspect Question Reformulation](https://arxiv.org/abs/2601.20614)
...
### [121] [Investigating the Development of Task-Oriented Communication in Vision-Language Models](https://arxiv.org/abs/2601.20641)
...
### [122] [Implementing Metric Temporal Answer Set Programming](https://arxiv.org/abs/2601.20735)
