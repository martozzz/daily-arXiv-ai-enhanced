...
TL;DR: 提出基于LLM的级联商业化框架，通过主题和关键词生成改进电商首页个性化，在保持混合基础设施的同时实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统电商首页组装模式依赖于独立组件(静态主题、检索系统和点式排序器)，虽然能优化总体偏好但缺乏灵活性，限制了跨页面区块的个性化和语义连贯性，难以适应动态目标和不断变化的商业需求。

Method: 使用预训练语言模型开发级联商业化框架，包括页面区块级主题生成和约束性关键词生成；采用教师-学生微调优化生产延迟和成本；融合生成内容与传统排序模型创建混合基础设施；建立AI驱动的内容评估和质量筛选框架。

Result: 在在线实验中，该框架相比强基线实现了页面视图到购物车添加的+2.7%提升；微调模型消融实验显示其性能接近闭源重量级LLM水平；同时建立了安全、自动化的动态内容部署框架。

Conclusion: 该研究提出了一种级联商业化框架，通过主题和关键词生成结合教师-学生微调，在保持混合基础设施的同时显著提升电商个性化首页的语义一致性和动态适应能力。
...
TL;DR: EVOCHAMBER：一种训练自由的多智能体测试时进化框架，通过CODREAM协议实现非对称知识转移，在异构任务流中显著超越基线，并自发形成专业化分工。


<details>
  <summary>Details</summary>
Motivation: 现有测试时进化方法要么将经验局限于个体智能体，丧失了跨智能体学习；要么对称广播给所有智能体，消除了专业化的价值。需要一种能促进专业化同时实现有效知识转移的多智能体进化框架。

Method: 提出了EVOCHAMBER框架，包含三个层次：核心的CODREAM协议（任务失败时的协作反思、知识提炼和非对称路由）、团队级操作符（基于利基条件的团队组装和在线协作结构选择）和种群级生命周期操作符（分支、合并、修剪和播种）。

Result: 在数学竞赛任务上达到63.9%，代码任务上75.7%，多领域推理上87.1%的准确率，比最佳基线在数学上相对提升32%。从相同初始化的智能体中自发涌现出4-5个稳定的利基专家，验证了非对称跨智能体转移的有效性。

Conclusion: EVOCHAMBER框架展示了在多智能体系统中，通过非对称知识流转和专业化演进，能够实现超越单智能体方法的协作效率和任务性能，为训练自由的智能体协同进化提供了有效方案。
...
TL;DR: TLDR：提出RankQ离线到在线Q学习方法，用排序损失替代统一悲观性，在稀疏奖励和视觉机器人任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 动机：现有离线到在线强化学习方法通常通过惩罚分布外动作引入悲观性，但这种方式相当于行为克隆锚点，当数据集动作次优时会限制在线策略改进。需要新方法来避免统一悲观性，同时有效引导策略改进。

Method: 方法：提出RankQ目标，结合时序差分学习与自监督多项目排序损失，学习相对动作偏好而非统一惩罚未见动作，从而引导Q函数朝更优行为方向优化。

Result: 结果：在稀疏奖励D4RL基准上，RankQ性能优于7种现有方法；在视觉机器人学习中，低数据环境下成功率比次优方法高42.7%，高数据环境下性能提升13.7%；实际机器人方块堆叠任务中成功率从43.1%提升至84.7%。

Conclusion: 结论：RankQ通过引入结构化排序损失来优化离线到在线Q学习，在稀疏奖励任务和视觉机器人学习中均表现出色，尤其在实际应用中将方块堆叠成功率从43.1%显著提升到84.7%。
...
### [4] [The Many Faces of On-Policy Distillation: Pitfalls, Mechanisms, and Fixes](https://arxiv.org/abs/2605.11182)
*Siqi Zhu,Xuyan Ye,Hongyu Lu,Weiye Shi,Ge Liu*

Main category: cs.AI

TL;DR: 本研究全面实证分析了在线蒸馏和在线自蒸馏的有效性条件，发现其成功率取决于教师选择、损失公式和特权信息类型，并揭示了三种失败机制及相应缓解方法。


<details>
  <summary>Details</summary>
Motivation: 现有的关于OPD和OPSD有效性的研究结果相互矛盾，因此需要系统地研究它们何时有效、何时失败及其原因。

Method: 通过一系列实验设置，系统地测试了OPD和OPSD在不同情境下的表现，如数学推理、系统提示知识内部化等，并分析了失败机制。

Result: 发现数学推理中的OPD对教师选择和损失公式高度敏感；而OPSD在测试时缺乏实例特定特权信息时失败；但在共享潜在规则下有效。同时识别了三种失败机制。

Conclusion: OPSD在共享潜在规则（如系统提示或对齐偏好）下有效，但当特权信息是实例特定时失败。本文识别了三种失败机制，并提出了相应的缓解策略如stop-gradient TopK目标、RLVR适应的教师和SFT稳定的学生等。
...
### [5] [Don't Look at the Numbers: Visual Anchoring Bias and Layer-wise Representation in VLMs](https://arxiv.org/abs/2605.11218)
*M. Shalankin*

Main category: cs.AI

TL;DR: 图像上的数字锚定会显著偏置视觉语言模型的质评判断，这种偏差源自表征动态而非视觉变化，不同模型架构对锚定信息的融合方式存在差异。


<details>
  <summary>Details</summary>
Motivation: 探究图像上的嵌入式数字锚定是否系统性地影响视觉语言模型的质评判断，并阐明其因果机制，而非简单地归因于视觉特征的退化。

Method: 通过6个来自5种不同架构家族的视觉语言模型，采用层间探测方法分析锚定分类与质量预测的表征动态，并使用融合分析探究不同架构对锚定信息的整合模式。

Result: 数字锚定在所有测试模型中均显著偏置质评判断，偏差效应是严重图像质量退化的2.5倍。层探测显示，锚定分类最优的层（L12-L34）并非质评预测的最佳层，后者位于更深的层。融合分析揭示出架构依赖的锚定信息整合方式，两个模型在L1-L2层实现即时融合，而其他三个模型则为部分或无融合。

Conclusion: 视觉锚定偏差在视觉语言模型中具有稳健的因果解释，揭示了其与表征动态的关联，而非源于视觉特征改变。研究表明，不同类型的模型架构对锚定信息的融合方式有所差异，部分模型在早期层就完全融合，而其他模型则表现为部分或无融合。
...
### [6] [Do Vision-Language-Models show human-like logical problem-solving capability in point and click puzzle games?](https://arxiv.org/abs/2605.11223)
*Dominik Helfenstein,Marco Menner,Maximilian Triebel*

Main category: cs.AI

TL;DR: 本文引入VLATIM基准评估视觉-语言模型在物理解谜游戏中的表现，发现现有模型在逻辑规划上表现良好，但在精确视觉接地和执行上仍有差距，未达到人类水平。


<details>
  <summary>Details</summary>
Motivation: 现有基准往往忽略了点击式解谜游戏中所需的复杂物理推理能力。VLATIM旨在评估视觉-语言（-动作）模型在真实物理交互环境下的类人类逻辑问题解决能力，填补高级逻辑推理与需要精确鼠标交互的连续动作空间之间的关键空白。

Method: 本文引入了一个名为VLATIM（Vision-Language Against The Incredible Machine）的基准测试，该基准基于经典物理解谜游戏《The Incredible Machine 2》。基准分为五个渐进部分，评估从基础视觉接地和领域理解到多步操作及完整解谜的能力。

Result: 实验结果显示推理与执行之间存在显著差距。大型专有模型在规划能力上表现优越，但在精确视觉接地方面表现不佳，因此尚未展现出类人类的解谜能力。

Conclusion: 现有的大型专有模型虽然具备优越的规划能力，但在精确的视觉接地方面表现不佳，因此尚未展现出类人类的解谜能力。这表明在视觉-语言模型中实现类人类的物理推理和交互能力仍是一个重要挑战。
...
### [7] [PIVOT: Bridging Planning and Execution in LLM Agents via Trajectory Refinement](https://arxiv.org/abs/2605.11225)
*Tuo Zhang,Alin-Ionut Popa,Yan Xu,Rui Song,Dimitrios Dimitriadis*

Main category: cs.AI

TL;DR: PIVOT是一个自我监督框架，通过迭代的计划-检查-演化轨迹优化解决LLM智能体计划与执行不一致的问题，实现了显著的性能提升和计算效率。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型（LLM）的智能体常常生成看似连贯但执行时失败的计划，原因包括不可行的动作、约束违规以及在长时间跨度上的累积错误。需要解决计划与执行之间的不一致问题。

Method: 该框架包含四个阶段：1) PLAN - 生成候选轨迹 2) INSPECT - 执行轨迹并计算结构化损失，生成编码计划-执行差异的文本梯度 3) EVOLVE - 应用这些信号生成改进的轨迹 4) VERIFY - 对任务约束进行最终全局检查。同时采用单调接受过程确保解决方案质量不下降。

Result: 在DeepPlanning和GAIA上的实证评估展示了最先进的性能：在有人类在环（HITL）反馈的情况下，PIVOT实现了高达94%的相对改进（约束满意度），而其完全自主变体也保持了显著收益。计算效率方面，比竞争方法少用3到5倍的token。

Conclusion: PIVOT提出了一种自我监督的轨迹优化框架，通过迭代的
...
### [8] [Unlocking LLM Creativity in Science through Analogical Reasoning](https://arxiv.org/abs/2605.11258)
*Andrew Shen,Shaul Druckmann,James Zou*

Main category: cs.AI

TL;DR: 通过引入类比推理（AR）方法有效缓解AI系统在开放性问题解决中的模式崩溃，显著提升方案多样性和新颖性，在多个生物医学应用中验证其可行性。


<details>
  <summary>Details</summary>
Motivation: 开发能够在复杂开放性问题中持续生成新颖且多样化解决方案的AI系统，以提升自主科学发现能力，特别是在生物医学等复杂领域。

Method: 通过构建跨领域问题的类比，基于共享的关系结构进行类比推理，并使用这些类比来搜索新颖的解决方案。

Result: AR方法相比基线显著提高了解决方案多样性指标（提升90-173%），在超过50%的情况下生成新颖方案（而基线仅为1.6%），在四个生物医学问题实施中获得一致性的定量收益：扰动效应预测分布指标提升近13倍；预测细胞间通信的AUPRC超过所有基线；推断大脑区域相互作用与已发表方法具有高相关性（ρ=0.729）；在寡核苷酸特性预测的2个数据集中达到最先进的性能。

Conclusion: 类比推理（AR）作为一种开放性问题解决方案生成方法，能够有效缓解生成模型中的模式崩溃问题，显著提升解决方案的多样性和新颖性，在多个生物医学应用中展示了实用价值。
...
### [9] [LatentRouter: Can We Choose the Right Multimodal Model Before Seeing Its Answer?](https://arxiv.org/abs/2605.11301)
*Xueqi Cheng,Yushun Dong*

Main category: cs.AI

TL;DR: 提出多模态大语言模型路由器LatentRouter，通过隐式通信进行反事实多模态效用预测，根据图像-问题的视觉、布局与推理需求匹配最佳MLLM，显著优于现有路由方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLM）在OCR、图表理解、空间推理、视觉问答等任务上的能力与效率存在显著差异，然而现有路由方法通常仅评估查询难度，未能有效将输入的多模态需求与各候选模型的具体能力进行匹配，导致系统整体效率不佳，因此亟需一种能够根据输入的具体视觉、布局与推理要求动态选择最优MLLM的路由机制。

Method: 提出LatentRouter方法，核心技术包括：1. 从图像-问题输入中提取可学习的多模态路由胶囊；2. 将候选MLLM表示为模型能力标记；3. 通过隐式通信在路由胶囊与能力标记之间进行潜在状态交互，预测各模型反事实性能；4. 分布化结果头预测模型特定质量；5. 引入有界胶囊修正以优化边界决策。该方法支持基于性能或性能-成本权衡的路由策略，并通过共享模型评分和可用性掩码处理动态候选池。

Result: 在MMR-Bench和VL-RouterBench数据集上的实验表明，LatentRouter在路由性能上显著超越了固定模型策略、基于特征的路由方法以及其他学习型路由器。进一步分析显示，其在视觉、布局敏感或推理导向任务类型上的优势最为明显，隐式通信机制是性能提升的主要贡献因素。

Conclusion: 研究提出了一种名为LatentRouter的新型多模态大语言模型（MLLM）路由框架，通过基于隐式通信的反事实多模态效用预测，实现了对多种MLLM任务的精细匹配与高效调度，其性能优于固定模型、特征级及学习型路由基线，尤其在视觉、布局敏感或推理导向的任务上表现突出。
...
### [10] [Constraint-Data-Value-Maximization: Utilizing Data Attribution for Effective Data Pruning in Low-Data Environments](https://arxiv.org/abs/2605.11312)
*Danilo Brajovic,David A. Kreplin,Marco F. Huber*

Main category: cs.AI

TL;DR: 针对低数据量场景下的数据剪枝问题,作者提出CDVM方法,相比传统Shapley数据值方法,在最大化数据重要性的同时约束个别数据过度影响,在基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于Shapley的数据值在仅保留少量数据的剪枝场景中效果不佳,需要一种针对低数据量情景的数据属性方法。

Method: 提出了约束数据值最大化（CDVM）方法,通过将数据剪枝问题转化为约束优化问题,在最大化总影响的同时惩罚过高的每条测试数据贡献。

Result: 在OpenDataVal基准测试中,CDVM在低数据量场景下表现出优异的性能,并且运行时性能具有竞争力。

Conclusion: 本文提出的CDVM方法在低数据量场景下有效解决了现有Shapley数据值不适用于剪枝有限数据的问题。
...
### [11] [Rethinking Evaluation for LLM Hallucination Detection: A Desiderata, A New RAG-based Benchmark, New Insights](https://arxiv.org/abs/2605.11330)
*Wenbo Chen,Veena Padmanabhan,Tootiya Giyahchi,Elaine Wong,Leman Akoglu*

Main category: cs.AI

TL;DR: 提出新的RAG幻觉检测基准TRIVIA+，包含长上下文样本和多种标签噪声，填补现有基准不足，实验显示当前检测器在RAG任务中表现有限且受标签噪声影响。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测基准在RAG任务的两个关键方面存在不足：缺乏基于长上下文的基准样本（因为长度增加了人工标注难度），以及缺乏用于压力测试检测器的现实标签噪声数据。这限制了对RAG场景下幻觉检测器的有效评估。

Method: 1. 建立幻觉检测基准的期望属性标准 2. 分析现有基准与标准的差距 3. 构建TRIVIA+基准，重点包括长上下文样本和设计四种不同的标签噪声方案 4. 在RAG基准上对现有SOTA检测器进行实验评估

Result: 1. TRIVIA+基准包含文献中最长的上下文样本 2. 设计了样本依赖性和样本独立性噪声的四种标签噪声方案 3. 实验发现：当前检测器在RAG基准上仍有很大提升空间；LLM-as-a-Judge基线表现具有竞争力；标签噪声显著影响检测性能

Conclusion: 本研究提出的TRIVIA+基准不仅填补了现有RAG幻觉检测基准的空白，还揭示了当前检测器在RAG任务中的性能不足和标签噪声的影响，为推动RAG幻觉检测研究提供了重要工具和见解。
...
### [12] [Causal Bias Detection in Generative Artifical Intelligence](https://arxiv.org/abs/2605.11365)
*Drago Plecko*

Main category: cs.AI

TL;DR: 该文将因果公平分析扩展到生成式AI领域，提出了统一的理论框架来量化生成模型中的偏见，包括因果路径分解和机制替代分析，并开发了相应的识别条件和估计方法。


<details>
  <summary>Details</summary>
Motivation: 随着基于人工智能的自动化系统在关键领域（如医疗、金融、法律等）的广泛部署，公平性问题变得越来越重要。传统的因果公平研究主要集中在标准机器学习情景中，决策者构建单一预测机制。然而，生成式AI环境则复杂得多，生成模型可以从任意变量的条件分布中抽样，隐式地构建自己对所有因果机制的信念。这种根本差异要求开发新的因果公平方法学。

Method: 作者首先形式化了生成式AI中的因果公平问题，将其与标准机器学习设置统一在一个共同的理论框架下。然后提出了新的因果分解结果，使得可以沿着不同的因果路径以及生成模型机制替代真实世界机制的过程，对公平性影响进行精细化量化。建立了因果量的识别条件，并引入了高效的估计器。最后通过在多个数据集上分析大语言模型中的种族和性别偏见来验证方法的价值。

Result: 作者开发了一套统一的因果公平理论框架，能够同时处理标准机器学习和生成式AI设置。提出的新因果分解方法能够量化沿着不同因果路径的公平性影响，以及生成模型机制替代真实世界机制的效果。建立的识别条件和估计器能够有效分析生成式AI模型中的偏见问题。实证分析显示，该方法能够有效检测和量化大语言模型中的种族和性别偏见。

Conclusion: 研究表明，将标准机器学习环境下的因果公平分析框架扩展到生成式人工智能领域，需要新的方法论发展。通过形式化生成式AI中的因果公平问题，并与标准机器学习方法在一个统一的理论框架下进行整合，作者提出了新的因果分解方法、识别条件和高效的估计器，能够更细致地量化生成模型中的偏见影响。这些进展对于推动生成式AI在关键领域中的公平部署具有重要意义。
...
### [13] [Causal Algorithmic Recourse: Foundations and Methods](https://arxiv.org/abs/2605.11373)
...
### [14] [LLM-X: A Scalable Negotiation-Oriented Exchange for Communication Among Personal LLM Agents](https://arxiv.org/abs/2605.11376)
...
### [15] [Revisiting Privacy Preservation in Brain-Computer Interfaces: Conceptual Boundaries, Risk Pathways, and a Protection-Strength Grading Framework](https://arxiv.org/abs/2605.11386)
...
### [16] [Transformer Interpretability from Perspective of Attention and Gradient](https://arxiv.org/abs/2605.11392)
...
### [17] [AcuityBench: Evaluating Clinical Acuity Identification and Uncertainty Alignment](https://arxiv.org/abs/2605.11398)
...
### [18] [Attributing Emergence in Million-Agent Systems](https://arxiv.org/abs/2605.11404)
...
### [19] [What Do EEG Foundation Models Capture from Human Brain Signals?](https://arxiv.org/abs/2605.11410)
...
### [20] [Under the Hood of SKILL.md: Semantic Supply-chain Attacks on AI Agent Skill Registry](https://arxiv.org/abs/2605.11418)
...
### [21] [A Mechanistic Investigation of Supervised Fine Tuning](https://arxiv.org/abs/2605.11426)
*Ruhaan Chopra*
...
### [22] [Adaptive Teacher Exposure for Self-Distillation in LLM Reasoning](https://arxiv.org/abs/2605.11458)
...
### [23] [Breaking $\textit{Winner-Takes-All}$: Cooperative Policy Optimization Improves Diverse LLM Reasoning](https://arxiv.org/abs/2605.11461)
...
### [24] [CAMPA: Efficient and Aligned Multimodal Graph Learning via Decoupled Propagation and Aggregation](https://arxiv.org/abs/2605.11468)
...
### [25] [TOPPO: Rethinking PPO for Multi-Task Reinforcement Learning with Critic Balancing](https://arxiv.org/abs/2605.11473)
...
### [26] [FibQuant: Universal Vector Quantization for Random-Access KV-Cache Compression](https://arxiv.org/abs/2605.11478)
...
### [27] [Engagement Process: Rethinking the Temporal Interface of Action and Observation](https://arxiv.org/abs/2605.11484)
...
### [28] [The Evaluation Differential: When Frontier AI Models Recognise They Are Being Tested](https://arxiv.org/abs/2605.11496)
...
### [29] [Selective Off-Policy Reference Tuning with Plan Guidance](https://arxiv.org/abs/2605.11505)
...
### [30] [Hierarchical LLM-Driven Control for HAPS-Assisted UAV Networks: Joint Optimization of Flight and Connectivity](https://arxiv.org/abs/2605.11509)
...
### [31] [AutoLLMResearch: Training Research Agents for Automating LLM Experiment Configuration -- Learning from Cheap, Optimizing Expensive](https://arxiv.org/abs/2605.11518)
...
### [32] [Controllable User Simulation](https://arxiv.org/abs/2605.11519)
...
### [33] [Read, Grep, and Synthesize: Diagnosing Cross-Domain Seed Exposure for LLM Research Ideation](https://arxiv.org/abs/2605.11532)
*Yunju Choi,Min Song*
...
### [34] [Optimal LTLf Synthesis](https://arxiv.org/abs/2605.11544)
...
### [35] [Hindsight Hint Distillation: Scaffolded Reasoning for SWE Agents from CoT-free Answers](https://arxiv.org/abs/2605.11556)
...
### [36] [Dual-Temporal LSTM with Hybrid Attention for Airline Passenger Load Factor Forecasting: Integrating Intra-Flight and Inter-Flight Booking Dynamics](https://arxiv.org/abs/2605.11569)
...
### [37] [Native Explainability for Bayesian Confidence Propagation Neural Networks: A Framework for Trusted Brain-Like AI](https://arxiv.org/abs/2605.11595)
...
### [38] [GAR: Carbon-Aware Routing for LLM Inference via Constrained Optimization](https://arxiv.org/abs/2605.11603)
...
### [39] [CuSearch: Curriculum Rollout Sampling via Search Depth for Agentic RAG](https://arxiv.org/abs/2605.11611)
...
### [40] [Nice Fold or Hero Call: Learning Budget-Efficient Thinking for Adaptive Reasoning](https://arxiv.org/abs/2605.11625)
...
### [42] [Seirênes: Adversarial Self-Play with Evolving Distractions for LLM Reasoning](https://arxiv.org/abs/2605.11636)
...
### [43] [A CAP-like Trilemma for Large Language Models: Correctness, Non-bias, and Utility under Semantic Underdetermination](https://arxiv.org/abs/2605.11672)
...
### [44] [OOM-Free Alpamayo via CPU-GPU Memory Swapping for Vision-Language-Action Models](https://arxiv.org/abs/2605.11678)
...
### [45] [Explaining and Breaking the Safety-Helpfulness Ceiling via Preference Dimensional Expansion](https://arxiv.org/abs/2605.11679)
...
### [46] [Persistent and Conversational Multi-Method Explainability for Trustworthy Financial AI](https://arxiv.org/abs/2605.11687)
*Georgios Makridis,Georgios Fatouros,John Soldatos,George Katsis,Dimosthenis Kyriazis*
...
### [47] [Toward Stable Value Alignment: Introducing Independent Modules for Consistent Value Guidance](https://arxiv.org/abs/2605.11712)
...
### [48] [SafeSteer: A Decoding-level Defense Mechanism for Multimodal Large Language Models](https://arxiv.org/abs/2605.11716)
...
### [49] [OptArgus: A Multi-Agent System to Detect Hallucinations in LLM-based Optimization Modeling](https://arxiv.org/abs/2605.11738)
...
### [50] [When Reasoning Traces Become Performative: Step-Level Evidence that Chain-of-Thought Is an Imperfect Oversight Channel](https://arxiv.org/abs/2605.11746)
...
### [51] [Towards Visually Grounded Multimodal Summarization via Cross-Modal Transformer and Gated Attention](https://arxiv.org/abs/2605.11753)
...
### [52] [Beyond Inefficiency: Systemic Costs of Incivility in Multi-Agent Monte Carlo Simulations](https://arxiv.org/abs/2605.11789)
...
### [53] [Why Users Go There: World Knowledge-Augmented Generative Next POI Recommendation](https://arxiv.org/abs/2605.11807)
...
### [54] [Beyond World-Frame Action Heads: Motion-Centric Action Frames for Vision-Language-Action Models](https://arxiv.org/abs/2605.11809)
...
### [55] [Automated Reformulation of Robust Optimization via Memory-Augmented Large Language Models](https://arxiv.org/abs/2605.11813)
...
### [56] [MedMemoryBench: Benchmarking Agent Memory in Personalized Healthcare](https://arxiv.org/abs/2605.11814)
...
### [57] [On-Policy Self-Evolution via Failure Trajectories for Agentic Safety Alignment](https://arxiv.org/abs/2605.11882)
...
### [58] [From Clever Hans to Scientific Discovery: Interpreting EEG Foundational Transformers with LRP](https://arxiv.org/abs/2605.11885)
...
### [59] [Toward Modeling Player-Specific Chess Behaviors](https://arxiv.org/abs/2605.11893)
...
### [60] [Rethinking Supervision Granularity: Segment-Level Learning for LLM-Based Theorem Proving](https://arxiv.org/abs/2605.11905)
...
### [61] [Rethinking Positional Encoding for Neural Vehicle Routing](https://arxiv.org/abs/2605.11910)
...
### [62] [Domain Restriction via Multi SAE Layer Transitions](https://arxiv.org/abs/2605.11920)
...
### [63] [When Simulation Lies: A Sim-to-Real Benchmark and Domain-Randomized RL Recipe for Tool-Use Agents](https://arxiv.org/abs/2605.11928)
...
### [67] [On the Limitations of Large Language Models for Conceptual Database Modeling](https://arxiv.org/abs/2605.11986)
...
### [68] [Random-Set Graph Neural Networks](https://arxiv.org/abs/2605.11987)
...
### [69] [BadSKP: Backdoor Attacks on Knowledge Graph-Enhanced LLMs with Soft Prompts](https://arxiv.org/abs/2605.11996)
...
### [70] [LegalCheck: Retrieval- and Context-Augmented Generation for Drafting Municipal Legal Advice Letters](https://arxiv.org/abs/2605.12012)
...
### [71] [LLMs and the ZPD](https://arxiv.org/abs/2605.12016)
*Peter Wallis*
...
### [72] [OmniRefine: Alignment-Aware Cooperative Compression for Efficient Omnimodal Large Language Models](https://arxiv.org/abs/2605.12056)
...
### [73] [SAGE: A Self-Evolving Agentic Graph-Memory Engine for Structure-Aware Associative Memory](https://arxiv.org/abs/2605.12061)
...
### [74] [Intermediate Artifacts as First-Class Citizens: A Data Model for Durable Intermediate Artifacts in Agentic Systems](https://arxiv.org/abs/2605.12087)
...
### [75] [Autonomy and Agency in Agentic AI: Architectural Tactics for Regulated Contexts](https://arxiv.org/abs/2605.12105)
...
### [76] [Large Language Models as Amortized Pareto-Front Generators for Constrained Bi-Objective Convex Optimization](https://arxiv.org/abs/2605.12106)
...
### [77] [Adaptive Multi-Round Allocation with Stochastic Arrivals](https://arxiv.org/abs/2605.12111)
...
### [78] [To Whom Do Language Models Align? Measuring Principal Hierarchies Under High-Stakes Competing Demands](https://arxiv.org/abs/2605.12120)
...
### [79] [Rollout Cards: A Reproducibility Standard for Agent Research](https://arxiv.org/abs/2605.12131)
...
### [80] [BoolXLLM: LLM-Assisted Explainability for Boolean Models](https://arxiv.org/abs/2605.12139)
...
### [81] [MM-OptBench: A Solver-Grounded Benchmark for Multimodal Optimization Modeling](https://arxiv.org/abs/2605.12154)
...
### [82] [Do Enterprise Systems Need Learned World Models? The Importance of Context to Infer Dynamics](https://arxiv.org/abs/2605.12178)
...
### [83] [MolDeTox: Evaluating Language Model's Stepwise Fragment Editing for Molecular Detoxification](https://arxiv.org/abs/2605.12181)
...
### [84] [Goal-Oriented Reasoning for RAG-based Memory in Conversational Agentic LLM Systems](https://arxiv.org/abs/2605.12213)
...
### [85] [No Action Without a NOD: A Heterogeneous Multi-Agent Architecture for Reliable Service Agents](https://arxiv.org/abs/2605.12240)
...
### [86] [Why Conclusions Diverge from the Same Observations: Formalizing World-Model Non-Identifiability via an Inference](https://arxiv.org/abs/2605.12255)
*Toru Takahashi*
...
### [87] [Missingness-MDPs: Bridging the Theory of Missing Data and POMDPs](https://arxiv.org/abs/2605.12262)
...
### [88] [How Useful Is Cross-Domain Generalization for Training LLM Monitors?](https://arxiv.org/abs/2605.12265)
...
### [89] [NARA: Anchor-Conditioned Relation-Aware Contextualization of Heterogeneous Geoentities](https://arxiv.org/abs/2605.12276)
...
### [90] [Executable Agentic Memory for GUI Agent](https://arxiv.org/abs/2605.12294)
...
### [91] [LISA: Cognitive Arbitration for Signal-Free Autonomous Intersection Management](https://arxiv.org/abs/2605.12321)
...
### [92] [Towards Automated Air Traffic Safety Assessment Around Non-Towered Airports Using Large Language Models](https://arxiv.org/abs/2605.12332)
...
### [93] [Reinforcing VLAs in Task-Agnostic World Models](https://arxiv.org/abs/2605.12334)
...
### [94] [$δ$-mem: Efficient Online Memory for Large Language Models](https://arxiv.org/abs/2605.12357)
...
### [95] [Classifier Context Rot: Monitor Performance Degrades with Context Length](https://arxiv.org/abs/2605.12366)
*Sam Martin,Fabien Roger*
...
### [96] [ProfiliTable: Profiling-Driven Tabular Data Processing via Agentic Workflows](https://arxiv.org/abs/2605.12376)
...
### [97] [Semantic Reward Collapse and the Preservation of Epistemic Integrity in Adaptive AI Systems](https://arxiv.org/abs/2605.12406)
*William Parris*
...
### [98] [Formalize, Don't Optimize: The Heuristic Trap in LLM-Generated Combinatorial Solvers](https://arxiv.org/abs/2605.12421)
...
### [99] [CAAFC: Chronological Actionable Automated Fact-Checker for misinformation / non-factual hallucination detection and correction](https://arxiv.org/abs/2605.12436)
...
### [100] [Towards Affordable Energy: A Gymnasium Environment for Electric Utility Demand-Response Programs](https://arxiv.org/abs/2605.12462)
...
### [101] [Reward Hacking in Rubric-Based Reinforcement Learning](https://arxiv.org/abs/2605.12474)
...
### [102] [ToolCUA: Towards Optimal GUI-Tool Path Orchestration for Computer Use Agents](https://arxiv.org/abs/2605.12481)
...
### [103] [Steering Without Breaking: Mechanistically Informed Interventions for Discrete Diffusion Language Models](https://arxiv.org/abs/2605.10971)
*Hanhan Zhou,Shamik Roy,Rashmi Gangadharaiah*

Main category: cs.LG

TL;DR: 研究发现DLMs中不同属性在去噪时形成模式不同，统一干预会损害质量。提出自适应调度器，仅在属性形成关键步骤干预，显著提升多属性控制效果而不损失生成质量。


<details>
  <summary>Details</summary>
Motivation: 离散扩散语言模型（DLMs）在并行去噪生成文本方面有优势，但其受控生成方法通常从自回归模型中引入，采用统一的干预计划。这种统一调度被证明会损害生成质量，尤其是当多个属性需要同时控制时，损害会进一步加剧。

Method: 本文首先在四种不同规模的DLMs（124M至8B参数）上训练稀疏自编码器来分析属性在去噪过程中的形成模式，发现不同属性（如主题和情感）有着不同的形成调度。基于这些发现，提出了一个自适应调度器，仅在属性形成的活跃步骤进行干预，而对其他步骤保持不干预。

Result: 在四个DLMs和七个控制任务上的实验结果显示，本文提出的自适应调度方法能够在不损害生成质量的情况下实现精确控制。在处理同时控制三个属性的挑战性任务时，该方法达到了93%的控制强度，比最强的基线方法高出15个百分点，同时保留了生成质量。

Conclusion: 研究表明，统一干预计划在离散扩散语言模型（DLMs）中进行受控生成会损害生成质量，尤其是在多属性联合控制时。通过稀疏自编码器分析发现，不同属性在去噪过程中以不同调度方式形成。因此，本文提出的自适应调度器能够将干预集中在属性形成的关键步骤上，从而提高控制精度并保持生成质量。
...
### [104] [Vertex-Softmax: Tight Transformer Verification via Exact Softmax Optimization](https://arxiv.org/abs/2605.10974)
*Navid Rezazadeh,Arash Gholami Davoodi*

Main category: cs.LG

TL;DR: 本文针对Transformer注意力验证，提出了能提供最紧可靠边界的“顶点-softmax”原语。理论证明了其最优性，实验证明其能高效显著提升认证比例与精度。


<details>
  <summary>Details</summary>
Motivation: 在Transformer注意力的认证验证中，现有的验证器独立地松弛softmax函数，而不考虑下游目标，这导致了可避免的松弛和精度损失。因此，需要一种方法为区间约束下的pre-softmax分数提供更紧密、可靠的softmax边界。

Method: 1. 提出“顶点-softmax”原语，利用“阈值结构定理”在排序后的目标系数中，将最优解候选数限制为线性个，计算复杂度为对数线性。2. 将其集成到基于CROWN式凸松弛的验证器中，提供了形式化的可靠性保证。3. 在MNIST、Fashion-MNIST和CIFAR-10的注意力模型上进行实验评估。

Result: 1. 顶点-softmax显著提高了在MNIST、Fashion-MNIST和CIFAR-10注意力模型上的认证鲁棒性比例。2. 大幅收紧了验证下界。3. 在远低于基线方法（如alpha-CROWN和分支定界）的成本下，性能始终匹配或超越基线。

Conclusion: Vertex-Softmax是基于分数区间所能获得的最紧的、可靠的上界，其形式最优性得到了证明。在Transformer注意力模型验证中，该原语相较于现有方法显著提高了计算效率，并带来了认证鲁棒性比例的大幅提升与下界的大幅收紧，验证了其方法的有效性和优越性。
...
### [105] [Hierarchical Multi-Scale Graph Neural Networks: Scalable Heterophilous Learning with Oversmoothing and Oversquashing Mitigation](https://arxiv.org/abs/2605.10975)
*Md Sazzad Hossen,Avimanyu Sahoo*

Main category: cs.LG

TL;DR: HMH是一种新型谱图学习框架，通过层次化多视图Haar变换解决异配图中的中心节点主导聚合和过平滑问题，在节点和图分类任务上优于现有方法且线性可扩展。


<details>
  <summary>Details</summary>
Motivation: 解决异配图中中心节点主导聚合和过平滑问题。现有谱GNN方法使用次优多项式滤波器，存在近似误差并混合了远距离信号，导致中心节点主导聚合和过平滑。

Method: 首先通过异配性感知编码器学习特征和结构感知的带符号亲和度；然后构建由这些嵌入指导的软图层次结构；在每个层次级别上构建稀疏、正交且局部感知的Haar基，在频域应用可学习的谱滤波器；最后通过跳跃连接反池化层将所有层次级别的输出结合回原始图中。

Result: HMH在节点分类上优于现有最优谱基线最多3%，在图分类上最多优于7%，同时保持线性可扩展性。

Conclusion: HMH框架通过层次化多视图Haar变换有效解决了异配图中的中心节点主导聚合和过平滑问题，在节点和图形分类任务上均优于现有谱方法，并保持线性可扩展性。
...
### [106] [$ξ$-DPO: Direct Preference Optimization via Ratio Reward Margin](https://arxiv.org/abs/2605.10981)
*Zhengyuan Fan,Zhonghua Wu,Yuxuan Du,Qun Chen*

Main category: cs.LG

TL;DR: 本文提出ξ-DPO方法来解决SimPO中超参数调优困难的问题，通过比率奖励边界使其更可解释和易于调优。


<details>
  <summary>Details</summary>
Motivation: SimPO通过消除显式参考模型实现了高效的无参考偏好优化，但其联合调优超参数β和γ仍然是一个核心挑战。作者认为这个困难源于SimPO中的边界公式在具有不同奖励差距结构的数据集上不易解释。

Method: 首先通过等变换重新表述偏好目标，将优化目标从最大化奖励差距的似然改为最小化奖励差距与最优边界之间的距离。然后以比率形式重新定义奖励，消除β的影响，得到有界且可解释的边界ξ。这个边界可以从初始奖励差距分布中确定，避免了反复试错调优。

Result: ξ-DPO解决了SimPO中复杂的超参数调优问题，提供了一个可以直接从奖励差距分布确定的边界参数ξ，实现了更可解释和稳定的优化性能。

Conclusion: 论文提出了一种新的偏好优化方法ξ-DPO，通过比率奖励边界的形式解决了SimPO中超参数调优的困难，实现了更直观可解释的性能边界调节。
...
### [107] [TMPO: Trajectory Matching Policy Optimization for Diverse and Efficient Diffusion Alignment](https://arxiv.org/abs/2605.10983)
...
TL;DR: TMPO解决了扩散模型RL对齐中的奖励黑客问题，用轨迹级奖励分布匹配替代标量奖励最大化，显著提升生成多样性同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在扩散模型对齐中普遍存在严重的奖励黑客攻击问题，导致生成多样性下降和视觉模式崩溃。根本原因是这些方法具有模式寻求性质，在最大化期望奖励时无法有效约束可接受轨迹的概率分布。

Method: 提出了轨迹匹配策略优化方法，核心是Softmax轨迹平衡目标，用于将策略的K个轨迹概率匹配到奖励诱导的玻尔兹曼分布。另外采用了动态随机树采样技术来减少多轨迹训练的计算开销。

Result: 在多个人类偏好、组合生成和文本渲染等对齐任务上的实验表明，TMPO相比最先进方法提升了9.1%的生成多样性，在所有下游任务和效率指标上都达到了有竞争力的性能，实现了奖励与多样性的最优权衡。

Conclusion: TMPO通过轨迹级奖励分布匹配成功解决了奖励黑客攻击问题，在保持高质量生成的同时显著提升了扩散模型的生成多样性，在各种下游任务中都实现了奖励与多样性的最佳平衡，为强化学习在扩散模型对齐中的应用提供了新的有效框架。
...
### [108] [AESOP: Adversarial Execution-path Selection to Overload Deep Learning Pipelines](https://arxiv.org/abs/2605.10987)
*Tingxi Li,Mingfang Ji,Ravishka Shemal Rathnasuriya,Simin Chen,Yitao Hu,Wei Yang*

Main category: cs.LG

TL;DR: 论文揭示了机器学习推理管线的效率攻击面，提出AESOP路径攻击框架，在多场景下显著提升计算负载和延迟，证明现有防御对整体管线安全防护不足。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习部署越来越多地将专门化模型组合成动态推理管线，这种结构创造了现有单模型攻击方法无法利用的效率攻击面，研究需要揭示和理解这种新型攻击模式。

Method: 提出了AESOP框架，结合漏洞导向的路径排序和自适应损失加权方法，将攻击优化为针对整条推理路径而非单个模型。

Result: AESOP在五个流水线及生产部署变种中表现优异：白盒设置下实现FLOPs增加2407倍、延迟增加419倍，灰盒设置下分别实现58倍和17倍增加；在系统级防御下，攻击虽然被重定向但未完全被中和，迫使管道在吞吐量崩溃和数据严重丢失之间做出选择。

Conclusion: 论文证明了机器学习推理管线存在显著的效率攻击面，AESOP框架能够有效利用这种结构漏洞，在多种实际部署场景下造成严重资源消耗和数据丢失，提示需要重新考虑针对整个推理链的安全防御机制。
...
### [109] [Test-Time Personalization: A Diagnostic Framework and Probabilistic Fix for Scaling Failures](https://arxiv.org/abs/2605.10991)
*Linhai Zhang,Yulan He*

Main category: cs.LG

TL;DR: 通过从个性化策略模型采样多个候选答案并用个性化奖励模型选择最佳，实现LLM推理时的可扩展性，提出统一扩展定律并设计概率化奖励模型来解决现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM个性化方法主要关注构建更好的个性化模型或输入，而将推理视为单次过程。本工作探索测试时个性化中推理计算扩展这一未充分研究的维度。

Method: 提出测试时个性化框架，将推理视为从个性化策略模型中采样N个候选答案并使用个性化奖励模型选择最佳答案的过程；推导统一的扩展定律来分析奖励模型的性能；设计概率化个性化奖励模型来避免失败模式。

Result: 实验证实测试时个性化框架在多种策略模型和个性化文本生成任务中均能实现一致的扩展性能，提出的扩展定律与实际观察到的扩展曲线高度匹配，概率化奖励模型有效缓解了用户级别崩溃和查询级别奖励破解问题。

Conclusion: 该研究成功实现了测试时个性化推理的可扩展性，提出的概率化个性化奖励模型解决了用户级别崩溃和查询级别奖励破解两个关键问题，验证了测试时扩展的实际可行性和所提出扩展定律的准确性。
...
### [110] [Finite Volume-Informed Neural Network Framework for 2D Shallow Water Equations: Rugged Loss Landscapes and the Importance of Data Guidance](https://arxiv.org/abs/2605.11001)
*Xiaofeng Liu*
...
### [111] [DisagMoE: Computation-Communication overlapped MoE Training via Disaggregated AF-Pipe Parallelism](https://arxiv.org/abs/2605.11005)
...
### [112] [RT-Transformer: The Transformer Block as a Spherical State Estimator](https://arxiv.org/abs/2605.11007)
*Peter Racioppo*
...
### [113] [When and How to Canonize: A Generalization Perspective](https://arxiv.org/abs/2605.11008)
...
### [114] [ACSAC: Adaptive Chunk Size Actor-Critic with Causal Transformer Q-Network](https://arxiv.org/abs/2605.11009)
...
### [115] [A Comparative Study of Federated Learning Aggregation Strategies under Homogeneous and Heterogeneous Data Distributions](https://arxiv.org/abs/2605.11010)
...
### [116] [LoopUS: Recasting Pretrained LLMs into Looped Latent Refinement Models](https://arxiv.org/abs/2605.11011)
...
### [117] [Backbone-Equated Diffusion OOD via Sparse Internal Snapshots](https://arxiv.org/abs/2605.11014)
...
### [118] [Simpson's Paradox in Behavioral Curves: How Aggregation Distorts Parametric Models of User Dynamics](https://arxiv.org/abs/2605.11017)
*Chao Zhou*
...
### [119] [Efficient LLM Reasoning via Variational Posterior Guidance with Efficiency Awareness](https://arxiv.org/abs/2605.11019)
...
### [120] [Trust Region Inverse Reinforcement Learning: Explicit Dual Ascent using Local Policy Updates](https://arxiv.org/abs/2605.11020)
...
### [121] [ASD-Bench: A Four-Axis Comprehensive Benchmark of AI Models for Autism Spectrum Disorder](https://arxiv.org/abs/2605.11091)
...
### [122] [Enabling Performant and Flexible Model-Internal Observability for LLM Inference](https://arxiv.org/abs/2605.11093)
...
### [123] [Newton's Lantern: A Reinforcement Learning Framework for Finetuning AC Power Flow Warm Start Models](https://arxiv.org/abs/2605.11102)
...
### [124] [GRAFT-ATHENA: Self-Improving Agentic Teams for Autonomous Discovery and Evolutionary Numerical Algorithms](https://arxiv.org/abs/2605.11117)
...
### [125] [Language Modeling with Hyperspherical Flows](https://arxiv.org/abs/2605.11125)
...
### [126] [HEPA: A Self-Supervised Horizon-Conditioned Event Predictive Architecture for Time Series](https://arxiv.org/abs/2605.11130)
...
### [127] [Steerable Neural ODEs on Homogeneous Spaces](https://arxiv.org/abs/2605.11133)
...
### [128] [Spurious Correlation Learning in Preference Optimization: Mechanisms, Consequences, and Mitigation via Tie Training](https://arxiv.org/abs/2605.11134)
...
### [129] [Rank Is Not Capacity: Spectral Occupancy for Latent Graph Models](https://arxiv.org/abs/2605.11142)
...
### [130] [CORE: Cyclic Orthotope Relation Embedding for Knowledge Graph Completion](https://arxiv.org/abs/2605.11159)
...
### [131] [Interpretability Can Be Actionable](https://arxiv.org/abs/2605.11161)
...
### [132] [Unlearning with Asymmetric Sources: Improved Unlearning-Utility Trade-off with Public Data](https://arxiv.org/abs/2605.11170)
...
### [133] [Optimistic Dual Averaging Unifies Modern Optimizers](https://arxiv.org/abs/2605.11172)
...
### [134] [Oversmoothing as Representation Degeneracy in Neural Sheaf Diffusion](https://arxiv.org/abs/2605.11178)
...
### [135] [Muon is Not That Special: Random or Inverted Spectra Work Just as Well](https://arxiv.org/abs/2605.11181)
...
### [136] [CATS: Cascaded Adaptive Tree Speculation for Memory-Limited LLM Inference Acceleration](https://arxiv.org/abs/2605.11186)
...
### [137] [Deep Learning for Protein Complex Prediction and Design](https://arxiv.org/abs/2605.11189)
*Ziwei Xie*
...
### [138] [Variational Linear Attention: Stable Associative Memory for Long-Context Transformers](https://arxiv.org/abs/2605.11196)
...
### [139] [FeatMap: Understanding image manipulation in the feature space and its implications for feature space geometry](https://arxiv.org/abs/2605.11203)
...
### [140] [The Scaling Law of Evaluation Failure: Why Simple Averaging Collapses Under Data Sparsity and Item Difficulty Gaps, and How Item Response Theory Recovers Ground Truth Across Domains](https://arxiv.org/abs/2605.11205)
*Jung Min Kang*
...
### [141] [Enforcing Constraints in Generative Sampling via Adaptive Correction Scheduling](https://arxiv.org/abs/2605.11214)
...
### [142] [Leveraging RAG for Training-Free Alignment of LLMs](https://arxiv.org/abs/2605.11217)
*John T. Halloran*
...
### [143] [ADMM-Q: An Improved Hessian-based Weight Quantizer for Post-Training Quantization of Large Language Models](https://arxiv.org/abs/2605.11222)
...
### [144] [LiBaGS: Lightweight Boundary Gap Synthesis for Targeted Synthetic Data Selection](https://arxiv.org/abs/2605.11231)
...
### [145] [A Comparative Study of Model Selection Criteria for Symbolic Regression](https://arxiv.org/abs/2605.11233)
...
### [146] [Internalizing Curriculum Judgment for LLM Reinforcement Fine-Tuning](https://arxiv.org/abs/2605.11235)
...
### [147] [DeconDTN-Toolkit: A Library for Evaluation and Enhancement of Robustness to Provenance Shift](https://arxiv.org/abs/2605.11237)
...
### [148] [Extending Kernel Trick to Influence Functions](https://arxiv.org/abs/2605.11239)
...
### [149] [Support-Proximity Augmented Diffusion Estimation for Offline Black-Box Optimization](https://arxiv.org/abs/2605.11246)
...
### [150] [A Proof-of-Concept Simulation-Driven Digital Twin Framework for Decision-Aware Diabetes Modeling](https://arxiv.org/abs/2605.11247)
*Zarrin Monirzadeh*
...
### [151] [Curriculum Learning-Guided Progressive Distillation in Large Language Models](https://arxiv.org/abs/2605.11260)
...
### [152] [Latent Chain-of-Thought Improves Structured-Data Transformers](https://arxiv.org/abs/2605.11262)
...
### [153] [Localization Boosting for Growth Markets: Mitigating Cross-Locale Behavioral Bias in Learning-to-Rank](https://arxiv.org/abs/2605.11272)
...
### [154] [Beyond Similarity: Temporal Operator Attention for Time Series Analysis](https://arxiv.org/abs/2605.11287)
...
### [155] [Quotient-Categorical Representations for Bellman-Compatible Average-Reward Distributional Reinforcement Learning](https://arxiv.org/abs/2605.11289)
...
### [156] [Optimal Representations for Generalized Contrastive Learning with Imbalanced Datasets](https://arxiv.org/abs/2605.11291)
...
### [157] [Primal Generation, Dual Judgment: Self-Training from Test-Time Scaling](https://arxiv.org/abs/2605.11299)
...
### [158] [A Theory of Time-Sensitive Language Generation: Sparse Hallucination Beats Mode Collapse](https://arxiv.org/abs/2605.11302)
...
### [159] [Couple to Control: Joint Initial Noise Design in Diffusion Models](https://arxiv.org/abs/2605.11311)
...
### [160] [Error whitening: Why Gauss-Newton outperforms Newton](https://arxiv.org/abs/2605.11316)
...
### [161] [$\varepsilon$-Good Action Identification in Fixed-Budget Monte Carlo Tree Search](https://arxiv.org/abs/2605.11324)
...
### [162] [Neural Statistical Functions](https://arxiv.org/abs/2605.11327)
...
### [163] [Epistemic Uncertainty for Test-Time Discovery](https://arxiv.org/abs/2605.11328)
...
### [164] [Physics-Informed Teacher-Student Ensemble Learning for Traffic State Estimation with a Varying Speed Limit Scenario](https://arxiv.org/abs/2605.11346)
...
### [165] [Gradient-Free Noise Optimization for Reward Alignment in Generative Models](https://arxiv.org/abs/2605.11347)
...
### [166] [gym-invmgmt: An Open Benchmarking Framework for Inventory Management Methods](https://arxiv.org/abs/2605.11355)
...
### [167] [The tractability landscape of diffusion alignment: regularization, rewards, and computational primitives](https://arxiv.org/abs/2605.11361)
...
### [168] [Causal Fairness for Survival Analysis](https://arxiv.org/abs/2605.11362)
*Drago Plecko*
...
### [169] [LPDP: Inference-Time Reward Control for Variable-Length DNA Generation with Edit Flows](https://arxiv.org/abs/2605.11368)
...
### [170] [Test-Time Compute for Dense Retrieval: Agentic Program Generation with Frozen Embedding Models](https://arxiv.org/abs/2605.11374)
*Han Xiao*
...
### [171] [TRACE: Temporal Routing with Autoregressive Cross-channel Experts for EEG Representation Learning](https://arxiv.org/abs/2605.11380)
...
### [172] [Behavioral Mode Discovery for Fine-tuning Multimodal Generative Policies](https://arxiv.org/abs/2605.11387)
...
### [173] [MuonQ: Enhancing Low-Bit Muon Quantization via Directional Fidelity Optimization](https://arxiv.org/abs/2605.11396)
...
### [174] [More Than Meets the Eye: A Semantics-Aware Traffic Augmentation Framework for Generalizable Website Fingerprinting](https://arxiv.org/abs/2605.11402)
...
### [175] [fg-expo: Frontier-guided exploration-prioritized policy optimization via adaptive kl and gaussian curriculum](https://arxiv.org/abs/2605.11403)
...
### [176] [20/20 Vision Language Models: A Prescription for Better VLMs through Data Curation Alone](https://arxiv.org/abs/2605.11405)
...
### [177] [A Boundary-Aware Non-parametric Granular-Ball Classifier Based on Minimum Description Length](https://arxiv.org/abs/2605.11406)
...
### [178] [MaskTab: Scalable Masked Tabular Pretraining with Scaling Laws and Distillation for Industrial Classification](https://arxiv.org/abs/2605.11408)
...
### [179] [Generative Diffusion Prior Distillation for Long-Context Knowledge Transfer](https://arxiv.org/abs/2605.11414)
...
### [180] [FastUMAP: Scalable Dimensionality Reduction via Bipartite Landmark Sampling](https://arxiv.org/abs/2605.11428)
*Hongmin Li*
...
### [181] [Deep Minds and Shallow Probes](https://arxiv.org/abs/2605.11448)
...
### [182] [Beyond Prediction: Interval Neural Networks for Uncertainty-Aware System Identification](https://arxiv.org/abs/2605.11460)
...
### [183] [Robust Multi-Agent Path Finding under Observation Attacks: A Principled Adversarial-Plus-Smoothing Training Recipe](https://arxiv.org/abs/2605.11469)
*Riad Ahmed*
...
### [184] [On the Approximation Complexity of Matrix Product Operator Born Machines](https://arxiv.org/abs/2605.11471)
...
### [185] [Efficient Adjoint Matching for Fine-tuning Diffusion Models](https://arxiv.org/abs/2605.11480)
...
### [186] [Adaptive Calibration in Non-Stationary Environments](https://arxiv.org/abs/2605.11490)
...
### [187] [Understanding and Preventing Entropy Collapse in RLVR with On-Policy Entropy Flow Optimization](https://arxiv.org/abs/2605.11491)
...
### [188] [CTFusion: A CTF-based Benchmark for LLM Agent Evaluation](https://arxiv.org/abs/2605.11504)
...
### [189] [EqOD: Symmetry-Informed Stability Selection for PDE Identification](https://arxiv.org/abs/2605.11524)
...
### [190] [OverNaN: NaN-Aware Oversampling for Imbalanced Learning with Meaningful Missingness](https://arxiv.org/abs/2605.11525)
*Amanda S Barnard*
...
### [191] [FERMI: Exploiting Relations for Membership Inference Against Tabular Diffusion Models](https://arxiv.org/abs/2605.11527)
...
### [193] [Fast MoE Inference via Predictive Prefetching and Expert Replication](https://arxiv.org/abs/2605.11537)
...
### [194] [Sharpen Your Flow: Sharpness-Aware Sampling for Flow Matching](https://arxiv.org/abs/2605.11547)
...
### [195] [VNDUQE: Information-Theoretic Novelty Detection using Deep Variational Information Bottleneck](https://arxiv.org/abs/2605.11551)
...
### [196] [A Controlled Counterexample to Strong Proxy-Based Explanations of OOD Performance: in a Fixed Pretraining-and-Probing Setup](https://arxiv.org/abs/2605.11554)
*Hongmin Li*
...
### [197] [Stargazer: A Scalable Model-Fitting Benchmark Environment for AI Agents under Astrophysical Constraints](https://arxiv.org/abs/2604.15664)
...
### [198] [A Composite Activation Function for Learning Stable Binary Representations](https://arxiv.org/abs/2605.11558)
...
### [199] [OUI as a Structural Observable: Towards an Activation-Centric View of Neural Network Training](https://arxiv.org/abs/2605.11570)
...
### [205] [Anti-Self-Distillation for Reasoning RL via Pointwise Mutual Information](https://arxiv.org/abs/2605.11609)
...
### [206] [From Generic Correlation to Input-Specific Credit in On-Policy Self Distillation](https://arxiv.org/abs/2605.11613)
...
### [207] [MIST: Reliable Streaming Decision Trees for Online Class-Incremental Learning via McDiarmid Bound](https://arxiv.org/abs/2605.11617)
...
### [208] [Evolutionary Task Discovery: Advancing Reasoning Frontiers via Skill Composition and Complexity Scaling](https://arxiv.org/abs/2605.11666)
...
### [209] [Partial Model Sharing Improves Byzantine Resilience in Federated Conformal Prediction](https://arxiv.org/abs/2605.11684)
...
### [210] [Shaping Zero-Shot Coordination via State Blocking](https://arxiv.org/abs/2605.11688)
...
### [211] [Slicing and Dicing: Configuring Optimal Mixtures of Experts](https://arxiv.org/abs/2605.11689)
...
### [212] [Compositional Neural Operators for Multi-Dimensional Fluid Dynamics](https://arxiv.org/abs/2605.11691)
...
### [213] [Augmented Lagrangian Method for Last-Iterate Convergence for Constrained MDPs](https://arxiv.org/abs/2605.11694)
...
### [214] [GRAFT: Graph-Tokenized LLMs for Tool Planning](https://arxiv.org/abs/2605.11706)
...
### [215] [Unlocking Compositional Generalization in Continual Few-Shot Learning](https://arxiv.org/abs/2605.11710)
...
### [216] [Debiased Model-based Representations for Sample-efficient Continuous Control](https://arxiv.org/abs/2605.11711)
...
### [217] [Block-R1: Rethinking the Role of Block Size in Multi-domain Reinforcement Learning for Diffusion Large Language Models](https://arxiv.org/abs/2605.11726)
...
### [218] [Persona-Conditioned Adversarial Prompting: Multi-Identity Red-Teaming for Adversarial Discovery and Mitigation](https://arxiv.org/abs/2605.11730)
...
### [219] [U-STS-LLM A Unified Spatio-Temporal Steered Large Language Model for Traffic Prediction and Imputation](https://arxiv.org/abs/2605.11735)
*Yichen Zhang,Jun Li*
...
### [220] [Online Continual Learning with Dynamic Label Hierarchies](https://arxiv.org/abs/2605.11742)
...
### [221] [Learning Feature Encoder with Synthetic Anomalies for Weakly Supervised Graph Anomaly Detection](https://arxiv.org/abs/2605.11749)
...
### [222] [Federated Client Selection under Partial Visibility: A POMDP Approach with Spatio-Temporal Attention](https://arxiv.org/abs/2605.11752)
...
### [223] [One-Step Generative Modeling via Wasserstein Gradient Flows](https://arxiv.org/abs/2605.11755)
...
### [224] [Is Monotonic Sampling Necessary in Diffusion Models?](https://arxiv.org/abs/2605.11773)
*Muhammad Haris Khan*
...
### [225] [Entropy Polarity in Reinforcement Fine-Tuning: Direction, Asymmetry, and Control](https://arxiv.org/abs/2605.11775)
...
### [226] [ROMER: Expert Replacement and Router Calibration for Robust MoE LLMs on Analog Compute-in-Memory Systems](https://arxiv.org/abs/2605.11800)
...
### [227] [Stop Marginalizing My Dreams: Model Inversion via Laplace Kernel for Continual Learning](https://arxiv.org/abs/2605.11804)
...
### [228] [Fed-BAC: Federated Bandit-Guided Additive Clustering in Hierarchical Federated Learning](https://arxiv.org/abs/2605.11815)
...
### [229] [More Edits, More Stable: Understanding the Lifelong Normalization in Sequential Model Editing](https://arxiv.org/abs/2605.11836)
...
### [230] [Gradient Clipping Beyond Vector Norms: A Spectral Approach for Matrix-Valued Parameters](https://arxiv.org/abs/2605.11838)
...
### [232] [GEAR: Granularity-Adaptive Advantage Reweighting for LLM Agents via Self-Distillation](https://arxiv.org/abs/2605.11853)
...
### [233] [Improving the Performance and Learning Stability of Parallelizable RNNs Designed for Ultra-Low Power Applications](https://arxiv.org/abs/2605.11855)
...
### [234] [Beyond Parameter Aggregation: Semantic Consensus for Federated Fine-Tuning of LLMs](https://arxiv.org/abs/2605.11857)
...
### [235] [Information theoretic underpinning of self-supervised learning by clustering](https://arxiv.org/abs/2605.11870)
...
### [236] [LOFT: Low-Rank Orthogonal Fine-Tuning via Task-Aware Support Selection](https://arxiv.org/abs/2605.11872)
...
### [237] [Sobolev Regularized MMD Gradient Flow](https://arxiv.org/abs/2605.11884)
...
### [238] [Incentivizing Truthfulness and Collaborative Fairness in Bayesian Learning](https://arxiv.org/abs/2605.11889)
...
### [239] [Procedural-skill SFT across capacity tiers: A W-Shaped pre-SFT Trajectory and Regime-Asymmetric Mechanism on 0.8B-4B Qwen3.5 Models](https://arxiv.org/abs/2605.11907)
*Igor Strozzi*
...
### [240] [Delightful Gradients Accelerate Corner Escape](https://arxiv.org/abs/2605.11908)
...
### [241] [Understanding Sample Efficiency in Predictive Coding](https://arxiv.org/abs/2605.11911)
...
### [242] [STAGE: Tackling Semantic Drift in Multimodal Federated Graph Learning](https://arxiv.org/abs/2605.11919)
...
### [243] [NOFE -- Neural Operator Function Embedding](https://arxiv.org/abs/2605.11970)
...
### [244] [Towards Order Fairness: Mitigating LLMs Order Sensitivity through Dual Group Advantage Optimization](https://arxiv.org/abs/2605.11974)
...
### [245] [Stochastic Minimum-Cost Reach-Avoid Reinforcement Learning](https://arxiv.org/abs/2605.11975)
...
### [246] [QDSB: Quantized Diffusion Schrödinger Bridges](https://arxiv.org/abs/2605.11983)
...
### [247] [Split the Differences, Pool the Rest: Provably Efficient Multi-Objective Imitation](https://arxiv.org/abs/2605.12000)
...
### [248] [Estimating Subgraph Importance with Structural Prior Domain Knowledge](https://arxiv.org/abs/2605.12009)
...
### [249] [Limits of Learning Linear Dynamics from Experiments](https://arxiv.org/abs/2605.12010)
...
### [250] [Efficient and Adaptive Human Activity Recognition via LLM Backbones](https://arxiv.org/abs/2605.12019)
...
### [251] [Approximation Theory of Laplacian-Based Neural Operators for Reaction-Diffusion System](https://arxiv.org/abs/2605.12025)
...
### [252] [Resilient Vision-Tabular Multimodal Learning under Modality Missingness](https://arxiv.org/abs/2605.12031)
...
### [253] [Scaling Laws and Tradeoffs in Recurrent Networks of Expressive Neurons](https://arxiv.org/abs/2605.12049)
...
### [254] [Learning plug-in surrogate endpoints for randomized experiments](https://arxiv.org/abs/2605.12051)
...
### [255] [Hölder Policy Optimisation](https://arxiv.org/abs/2605.12058)
...
### [256] [Missing Old Logits in Asynchronous Agentic RL: Semantic Mismatch and Repair Methods for Off-Policy Correction](https://arxiv.org/abs/2605.12070)
...
### [257] [Elicitation-Augmented Bayesian Optimization](https://arxiv.org/abs/2605.12079)
...
### [258] [Disentangled Sparse Representations for Concept-Separated Diffusion Unlearning](https://arxiv.org/abs/2605.12122)
...
### [259] [Fused Gromov-Wasserstein Distance with Feature Selection](https://arxiv.org/abs/2605.12161)
...
### [260] [On What We Can Learn from Low-Resolution Data](https://arxiv.org/abs/2605.12168)
...
### [261] [Lower bounds for one-layer transformers that compute parity](https://arxiv.org/abs/2605.12171)
*Daniel Hsu*
...
### [262] [Expected Batch Optimal Transport Plans and Consequences for Flow Matching](https://arxiv.org/abs/2605.12174)
...
### [263] [Multi-Task Representation Learning for Conservative Linear Bandits](https://arxiv.org/abs/2605.12176)
...
### [264] [DriftXpress: Faster Drifting Models via Projected RKHS Fields](https://arxiv.org/abs/2605.12183)
...
### [265] [Fair Conformal Classification via Learning Representation-Based Groups](https://arxiv.org/abs/2605.12195)
...
### [266] [ECTO: Exogenous-Conditioned Temporal Operator for Ultra-Short-Term Wind Power Forecasting](https://arxiv.org/abs/2605.12196)
...
### [267] [Overtrained, Not Misaligned](https://arxiv.org/abs/2605.12199)
...
### [268] [A Unified Graph Language Model for Multi-Domain Multi-Task Graph Alignment Instruction Tuning](https://arxiv.org/abs/2605.12197)
...
### [269] [Not How Many, But Which: Parameter Placement in Low-Rank Adaptation](https://arxiv.org/abs/2605.12207)
...
### [270] [No More, No Less: Task Alignment in Terminal Agents](https://arxiv.org/abs/2605.12233)
...
### [272] [PriorZero: Bridging Language Priors and World Models for Decision Making](https://arxiv.org/abs/2605.12289)
...
### [273] [On the Importance of Multistability for Horizon Generalization in Reinforcement Learning](https://arxiv.org/abs/2605.12206)
...
### [274] [KAN-CL: Per-Knot Importance Regularization for Continual Learning with Kolmogorov-Arnold Networks](https://arxiv.org/abs/2605.12306)
*Minjong Cheon*
...
### [275] [Transferable Delay-Aware Reinforcement Learning via Implicit Causal Graph Modeling](https://arxiv.org/abs/2605.12312)
...
### [277] [Manifold Sampling via Entropy Maximization](https://arxiv.org/abs/2605.12338)
...
### [278] [BSO: Safety Alignment Is Density Ratio Matching](https://arxiv.org/abs/2605.12339)
...
### [279] [A New Technique for AI Explainability using Feature Association Map](https://arxiv.org/abs/2605.12350)
...
### [280] [Instruction Lens Score: Your Instruction Contributes a Powerful Object Hallucination Detector for Multimodal Large Language Models](https://arxiv.org/abs/2605.12258)
...
### [281] [Agent-Based Post-Hoc Correction of Agricultural Yield Forecasts](https://arxiv.org/abs/2605.12375)
...
### [282] [Delay-Empowered Causal Hierarchical Reinforcement Learning](https://arxiv.org/abs/2605.12261)
...
### [283] [Discrete Flow Matching for Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2605.12379)
...
### [284] [Hypernetworks for Dynamic Feature Selection](https://arxiv.org/abs/2605.12278)
...
### [285] [Trust the Batch, On- or Off-Policy: Adaptive Policy Optimization for RL Post-Training](https://arxiv.org/abs/2605.12380)
...
### [286] [Targeted Neuron Modulation via Contrastive Pair Search](https://arxiv.org/abs/2605.12290)
...
### [287] [OGLS-SD: On-Policy Self-Distillation with Outcome-Guided Logit Steering for LLM Reasoning](https://arxiv.org/abs/2605.12400)
...
### [288] [STRABLE: Benchmarking Tabular Machine Learning with Strings](https://arxiv.org/abs/2605.12292)
...
### [290] [Approximation of Maximally Monotone Operators : A Graph Convergence Perspective](https://arxiv.org/abs/2605.12301)
...
### [291] [Solve the Loop: Attractor Models for Language and Reasoning](https://arxiv.org/abs/2605.12466)
...
### [293] [In-context learning to predict critical transitions in dynamical systems](https://arxiv.org/abs/2605.12308)
...
### [294] [Beyond GRPO and On-Policy Distillation: An Empirical Sparse-to-Dense Reward Principle for Language-Model Post-Training](https://arxiv.org/abs/2605.12483)
...
### [295] [Learning, Fast and Slow: Towards LLMs That Adapt Continually](https://arxiv.org/abs/2605.12484)
...
### [296] [Autoregressive Learning in Joint KL: Sharp Oracle Bounds and Lower Bounds](https://arxiv.org/abs/2605.12316)
...
### [297] [Grid Games: The Power of Multiple Grids for Quantizing Large Language Models](https://arxiv.org/abs/2605.12327)
...
### [298] [Neural-Schwarz Tiling for Geometry-Universal PDE Solving at Scale](https://arxiv.org/abs/2605.12343)
...
### [299] [From Message-Passing to Linearized Graph Sequence Models](https://arxiv.org/abs/2605.12358)
...
### [300] [MetaColloc: Optimization-Free PDE Solving via Meta-Learned Basis Functions](https://arxiv.org/abs/2605.12368)
*Zichuan Yang*
...
### [301] [Aligning Flow Map Policies with Optimal Q-Guidance](https://arxiv.org/abs/2605.12416)
...
### [302] [Learning Minimally Rigid Graphs with High Realization Counts](https://arxiv.org/abs/2605.12427)
...
### [303] [Environment-Adaptive Preference Optimization for Wildfire Prediction](https://arxiv.org/abs/2605.12435)
*Enyi Jiang,Wu Sun*
...
### [304] [ORCE: Order-Aware Alignment of Verbalized Confidence in Large Language Models](https://arxiv.org/abs/2605.12446)
...
### [305] [Multi-Stream LLMs: Unblocking Language Models with Parallel Streams of Thoughts, Inputs and Outputs](https://arxiv.org/abs/2605.12460)
...
### [306] [Search Your Block Floating Point Scales!](https://arxiv.org/abs/2605.12464)
...
### [307] [High-arity Sample Compression](https://arxiv.org/abs/2605.12465)
...
### [308] [MEME: Multi-entity & Evolving Memory Evaluation](https://arxiv.org/abs/2605.12477)
